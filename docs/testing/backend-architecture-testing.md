...
describe('CORS Middleware', () => {
  it('sets correct CORS headers', async () => {
    ...
    expect(res.headers.get('Access-Control-Allow-Origin')).toBe('https://neonpro.app');
...
describe('Database Integration', () => {
  ...
  it('handles database transactions correctly', async () => {
    ...
    const res = await app.request('/patients/bulk', {
      ...
      body: JSON.stringify({
        patients: [
          { name: 'João Silva', cpf: '123.456.789-00', birthDate: '1985-03-15' },
          { name: 'Maria Santos', cpf: '987.654.321-00', birthDate: '1990-08-22' },
          { name: 'Pedro Costa', cpf: '456.789.123-00', birthDate: '1978-12-05' },
        ]
      })
    });
  ...
  it('rolls back transaction on partial failure', async () => {
    ...
    const res = await app.request('/patients/bulk', {
      ...
      body: JSON.stringify({
        patients: [
          { name: 'João Silva', cpf: '123.456.789-00', birthDate: '1985-03-15' },
          { name: 'Invalid Patient', cpf: 'invalid-cpf', birthDate: '1990-08-22' }, // Invalid CPF
          { name: 'Pedro Costa', cpf: '456.789.123-00', birthDate: '1978-12-05' },
        ]
      })
    });
...
export class TestApiClient {
  constructor(private app: Hono, private options: TestApiOptions = {}) {}
...
describe.concurrent('Patient API Endpoints', () => {
  ...
  test('GET /patients', async () => { /* test implementation */ });
  test('GET /patients/:id', async () => { /* test implementation */ });
  test('POST /patients', async () => { /* test implementation */ });
});
...
import { createMockPatient, createMockAppointment } from './fixtures';
...
describe('Healthcare API Integration', () => {
  ...
  it('handles complete patient care workflow', async () => {
    ...
    const createAppointmentRes = await client.post('/appointments', appointmentData);
    ...
    const updateRes = await client.put(`/patients/${patient.patient.id}/medical`, medicalUpdate);
    ...
    const appointmentCheckRes = await client.get(`/appointments/${appointment.appointment.id}`);
