...
class DataRetentionService {
  ...
  private async cleanupAppointments(now: Date) {
    const cutoffDate = subDays(now, retentionPolicies.appointmentData.retentionPeriod);
...
class CalendarAuthService {
  static validateToken(token: string): CalendarAuthToken | null {
    try {
      const decoded = jwt.verify(token, process.env.JWT_SECRET) as CalendarAuthToken;
  ...
  static generateSecureToken(userId: string, clinicId: string, role: UserRole): string {
...
function useCalendarAuth() {
  ...
  const canViewAppointments = () => {
    return CalendarAuthService.hasPermission(token, Permission.VIEW_APPOINTMENTS);
  };
  
  const canCreateAppointments = () => {
    return CalendarAuthService.hasPermission(token, Permission.CREATE_APPOINTMENTS);
  };
  ...
  const canEditAppointment = (appointment: CalendarEvent) => {
    ...
    return CalendarAuthService.hasPermission(token, Permission.EDIT_APPOINTMENTS) ||
  ...
  const canDeleteAppointment = (appointment: CalendarEvent) => {
    ...
    return CalendarAuthService.hasPermission(token, Permission.DELETE_APPOINTMENTS);
  };
...
class SecurityMonitor {
  ...
  private static async createSecurityAlert(alert: Omit<SecurityAlert, 'id' | 'timestamp'>) {
  ...
  .query('list', {
    ...
    async resolve({ input, ctx }) {
      ...
      await AuditService.logAppointmentAction(
        ...
        ctx.user.id,
        ctx.user.role,
        { count: appointments.length, dateRange: { start: input.start, end: input.end } },
        'success'
      );
...
class SecureWebSocket {
  ...
  private validateMessage(message: any): boolean {
    ...
    const validTypes = ['appointment_update', 'appointment_created', 'appointment_deleted'];
...
describe('Calendar Security', () => {
  describe('Input Validation', () => {
    test('should reject XSS attempts in event titles', () => {
      ...
      expect(() => EventValidator.validateEvent({
  ...
  describe('Authorization', () => {
    test('should restrict access based on permissions', () => {
      ...
      expect(CalendarAuthService.hasPermission(patientToken, Permission.DELETE_APPOINTMENTS))
        .toBe(false);
    });
  });
...
const penTestScenarios = [
  {
    ...
    tests: [
      ...
      { payload: "'; DROP TABLE appointments; --", expected: 'Should be rejected' }
    ]
  },
  {
    name: 'XSS Attacks',
  ...
  },
  {
    ...
    tests: [
      { scenario: 'Access protected endpoint without token', expected: '401 Unauthorized' },
  ...
  },
  {
    ...
    tests: [
      { scenario: 'Patient accessing admin functions', expected: '403 Forbidden' },
  ...
  },
  {
    ...
    tests: [
      { scenario: 'Large date range requests', expected: 'Should be rate limited' },
      { scenario: 'Bulk data export without authorization', expected: 'Should be blocked' }
    ]
  }
];
```
