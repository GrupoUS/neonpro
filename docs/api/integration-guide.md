...
async function predictNoShow(patientId: string, appointmentDetails: any) {
  try {
    ...
    if (prediction.data.risk_category === "high") {
      await implementNoShowInterventions(
        patientId,
...
async function monitorServiceHealth() {
  try {
    ...
    healthStatus.data.forEach((service) => {
      console.log(
        `${service.service}: ${service.status} (${service.uptime_percentage}% uptime)`,
      );
...
async function optimizeAppointments(requests: any[]) {
  ...
  console.log(
    `Scheduled ${optimization.data.optimized_appointments.length} appointments`,
  );
  console.log(
    `Utilization rate: ${optimization.data.optimization_summary.utilization_rate}`,
  );
  ...
  try {
    ...
    if (error.code === "RATE_LIMIT_EXCEEDED") {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, error.details.retry_after_seconds * 1000),
      );
...
class TokenManager {
  ...
  private async refreshAccessToken() {
    const response = await fetch(
      "https://api.neonpro.healthcare/auth/refresh",
      {
        method: "POST",
        headers: {
