        ...
        (payload) => {
          ...
          queryClient.setQueryData(['patients'], (oldData: Patient[]) => {
            ...
            switch (payload.eventType) {
              ...
              case 'DELETE':
                return oldData.filter(patient => patient.id !== payload.old.id)
              default:
                return oldData
            }
          })
...
const patientName = usePatientStore(useShallow(state => state.selectedPatient?.name))
...
const filteredPatients = usePatientStore(
  useCallback(
    state => state.patients.filter(patient => 
      patient.name.toLowerCase().includes(state.filters.search.toLowerCase()) &&
