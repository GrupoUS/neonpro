...
interface AppointmentService {
  scheduleAppointment(command: ScheduleAppointmentCommand): Promise<AppointmentId>
    ...
    try {
      for (const event of events) {
        await transaction.query(
          ...
          [aggregateId, event.eventType, JSON.stringify(event.eventData), event.version]
        )
      }
...
class HealthCheckService {
  async checkSystemHealth(): Promise<HealthStatus> {
    ...
    checks.forEach((result, index) => {
      const checkName = ['database', 'redis', 'messageQueue', 'externalServices'][index]
