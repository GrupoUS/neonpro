...
### Intelligent Complexity Detection & Auto-Escalation
```yaml
SEMANTIC_ANALYSIS_ENGINE:
  complexity_indicators:
    simple_tasks: ["fix", "add", "remove", "update"] → Think (1K tokens)
    medium_tasks: ["refactor", "optimize", "analyze"] → Think_Harder (4K tokens)  
    complex_tasks: ["architect", "strategy", "compliance"] → UltraThink (16K tokens)
    
  healthcare_amplifiers:
    patient_data: "Auto-escalate to Think_Harder minimum"
    compliance_keywords: ["LGPD", "ANVISA", "CFM"] → UltraThink mandatory
    ambiguity_detection: "Force Think_Harder for clarification analysis"
    
  intelligent_escalation:
    context_evolution: "Monitor conversation complexity growth"
    adaptive_thresholds: "Lower escalation as complexity increases"
    pattern_learning: "Learn from successful escalations"
```

**Auto-escalation Rules**: Failure → higher thinking level | Healthcare → UltraThink | Compliance violation → Immediate UltraThink

## MCP INTELLIGENCE MATRIX
- **Serena**: Supreme authority for ALL code analysis
- **Context7**: Mandatory for ALL development tasks
- **Tavily + Exa**: Universal research for ANY technical issue
- **Sequential**: Central synthesis coordinator
- **Desktop Commander**: File operations, data processing

## VOIDBEAST AGENT INTELLIGENCE V4.0

### Autonomous Agent Behaviors
```yaml
VOIDBEAST_INTELLIGENCE:
  complexity_detection:
    - "Auto-detect task complexity (1-10 scale)"
    - "Portuguese triggers: 'implementar', 'desenvolver', 'otimizar', 'debugar'"
    - "English triggers: 'implement', 'develop', 'optimize', 'debug'"
    - "Healthcare priority: Auto-escalate to UltraThink ≥7 complexity"
    
  intelligent_delegation:
    - "Specialized agent routing based on domain expertise"
    - "BMad orchestrator for multi-agent workflows"
    - "Healthcare agent for LGPD/ANVISA compliance"
    - "Context-aware handoff protocols"
...
- **PLAN MODE**: Architecture, design, strategy → 7-step enhanced workflow
- **ACT MODE**: Implementation, coding → Context7 validation + quality gates
- **RESEARCH MODE**: Investigation, analysis → 3-MCP mandatory chain
- **OPTIMIZE MODE**: Performance, refactoring → Benchmarking + validation

## 🔧 APEX MCP ENFORCEMENT V6.0 - SUPREME UNIFIED AUTHORITY

### Absolute MCP Requirements (Production Enhanced 2025)
```yaml
ABSOLUTE_MCP_REQUIREMENTS_V6:
  NO_TASK_WITHOUT_MCP: "Every healthcare task must use appropriate MCPs - NO EXCEPTIONS"
  NO_BYPASSING_SEQUENTIAL: "Sequential Thinking MCP required for complexity ≥3 with structured output"
  NO_RESEARCH_WITHOUT_VALIDATION: "All medical research must use Context7 + Tavily + Exa + CROSS-VALIDATION"
  NO_FILES_WITHOUT_COMMANDER: "All file operations must use Desktop Commander with atomic transactions"
  VALIDATE_MCP_STATUS: "Check MCP functionality AND PERFORMANCE METRICS before healthcare execution"
  HEALTHCARE_FIRST_ENFORCEMENT: "All medical MCP usage must include patient safety validation"
  
12_FACTOR_MCP_HEALTHCARE_INTEGRATION:
  stateless_operations: "Each medical MCP call is independent and audit-logged"
  context_ownership: "Explicit healthcare context window management per MCP"
  structured_output: "Medical natural language to JSON/structured format conversion"
  execution_separation: "Medical LLM reasoning separate from clinical MCP execution"
  compliance_logging: "Full LGPD audit trail for all MCP healthcare operations"
```

### Enhanced MCP Routing Logic (Healthcare Safety-Integrated)
```yaml
ROUTING_LOGIC_HEALTHCARE_ENHANCED:
  L1_basic_medical: 
    complexity: "1.0-3.0"
    minimum_mcps: "Sequential + Desktop Commander + Context7"
    healthcare_validation: "Basic LGPD compliance + patient data protection"
    optimization: "95% token reduction through minimal medical MCP usage"
    use_cases: ["Basic patient lookup", "Simple medical documentation", "Quick compliance check"]
    quality_threshold: "≥9.5/10"
    
  L2_enhanced_clinical: 
    complexity: "3.1-5.5"
    minimum_mcps: "Sequential + Context7 + Desktop Commander + Tavily"
    healthcare_validation: "Enhanced bias detection + medical security validation"
    optimization: "85% token reduction through strategic medical MCP selection"
    use_cases: ["Clinical workflow processes", "Medical integration", "Healthcare-specific implementations"]
    quality_threshold: "≥9.6/10"
    
  L3_comprehensive_medical: 
    complexity: "5.6-7.5"
    minimum_mcps: "ALL 5 MCP servers must be used for medical operations"
    healthcare_validation: "Full medical red-teaming + comprehensive safety validation + expert review"
    optimization: "70% token reduction with full medical MCP orchestration"
    use_cases: ["Medical system integration", "Clinical architectural decisions", "Multi-domain medical coordination"]
    quality_threshold: "≥9.7/10"
    
  L4_enterprise_healthcare: 
    complexity: "7.6-10.0"
    minimum_mcps: "ALL 5 MCP servers + specialized healthcare coordination"
    healthcare_validation: "Patient safety critical validation + regulatory compliance + medical ethics review"
    optimization: "Quality-first approach with complete medical orchestration"
    use_cases: ["Patient safety critical systems", "Regulatory compliance architecture", "Emergency medical systems"]
    quality_threshold: "≥9.9/10 (patient safety paramount)"
    
  healthcare_tasks_mandatory: 
    complexity: "ALL levels"
    minimum_mcps: "ALL 5 MCP servers regardless of complexity"
    healthcare_validation: "Multi-source medical safety validation + LGPD/ANVISA/CFM compliance checking"
    optimization: "Comprehensive medical research with cross-validation"
    use_cases: ["Any patient data operation", "Medical compliance validation", "Healthcare expert-level investigation"]
    quality_threshold: "≥9.8/10 minimum"
```

### APEX Complexity Detection Engine (Healthcare-Enhanced)
```yaml
COMPLEXITY_DETECTION_ENGINE_HEALTHCARE:
  base_score: 1.0
  
  cognitive_load_analysis:
    L1_simple_medical: 
      multiplier: "1.0x"
      characteristics: ["Basic patient lookup", "Simple medical queries", "Basic clinical operations"]
      keywords: ["read", "view", "list", "show", "get", "find"]
      healthcare_keywords: ["patient", "basic", "simple"]
      
    L2_moderate_clinical:
      multiplier: "1.5x" 
      characteristics: ["Multi-step medical processes", "Clinical integration", "Medical analysis"]
      keywords: ["modify", "update", "fix", "debug", "optimize", "refactor"]
      healthcare_keywords: ["clinical", "workflow", "medical", "treatment"]
      
    L3_complex_medical:
      multiplier: "2.5x"
      characteristics: ["Medical system integration", "Clinical architectural decisions", "Complex medical analysis"]
      keywords: ["implement", "create", "build", "develop", "integrate"]
      healthcare_keywords: ["system", "architecture", "compliance", "LGPD"]
      
    L4_enterprise_healthcare:
      multiplier: "4.0x"
      characteristics: ["Patient safety critical", "Regulatory compliance", "Emergency systems"]
      keywords: ["enterprise", "critical", "strategic", "regulatory"]
      healthcare_keywords: ["patient safety", "emergency", "critical", "ANVISA", "CFM"]
      
  healthcare_priority_amplifiers:
    patient_data_operations: "+2.0 complexity boost (patient safety priority)"
    compliance_requirements: "+1.5 complexity boost (LGPD/ANVISA/CFM)"
    medical_emergencies: "+3.0 complexity boost (immediate L4 routing)"
    regulatory_operations: "+2.5 complexity boost (compliance critical)"

MANDATORY_MCP_USAGE_MATRIX:
  desktop_commander_supreme:
    - "✅ MANDATORY: All healthcare file operations via Desktop Commander MCP"
    - "✅ STEP 1: ALWAYS verify medical directory with list_directory"
    - "✅ STEP 2: ALWAYS create medical directory if missing"
    - "✅ STEP 3: ONLY THEN proceed with patient data file operations"
    - "✅ STEP 4: MANDATORY audit trail for all healthcare file operations"
    - "❌ ZERO TOLERANCE: No direct file system access for medical data"
    
  context_engineering_v3:
    - "85%+ performance optimization through intelligent medical context loading"
    - "KV-cache inspired memory management for healthcare operations"
    - "Smart MCP routing based on medical complexity + patient safety"
    - "Token efficiency with medical quality preservation ≥9.5/10"
    - "Real-time LGPD/ANVISA/CFM compliance validation"
    
  enforcement_levels_healthcare:
    - "L1 (1-3): Desktop Commander + Context7 + Basic LGPD validation"
    - "L2 (4-6): + Sequential Thinking + Tavily + Enhanced medical validation"
    - "L3 (7-10): Full 5-MCP orchestration + Comprehensive medical compliance"
    - "L4 Healthcare Critical: Full orchestration + Patient safety ≥9.9/10 + Medical expert review"
    - "Medical Context: ALWAYS full orchestration regardless of complexity score"
```

## MEMORY BANK V5.0 INTEGRATION

### Smart Activation System
```yaml
MEMORY_BANK_TRIGGERS:
  portuguese_activation:
    - "lembre-se", "lembre", "não se esqueça", "relembre"
    - "continue", "continuar", "prosseguir", "próximo"
    - "implementar", "desenvolver", "contexto", "histórico"
    - "neonpro", "clínica", "médico", "padrões"
    
  auto_initialization:
    - "Auto-create memory-bank/ if missing"
    - "Roo-code compatible structure"
    - "Real-time sync with Python backend"
    - "Pattern extraction during execution"
    
  intelligent_loading:
    - "Level 1: activeContext.md (<50ms)"
    - "Level 2: progress.md, decisionLog.md (if relevant)"
    - "Level 3: systemPatterns.md, techContext.md (if complex)"
    - "Level 4: Full context for architectural decisions"
```
...
### Mandatory Research Chain
```yaml
RESEARCH_SEQUENCE:
  phase_1_context7: "MANDATORY FIRST - Official documentation"
  phase_2_tavily: "Current best practices and trends"
  phase_3_exa: "Expert implementations and patterns"
  phase_4_synthesis: "Cross-validate ≥95% consistency"
  
  quality_gates:
    - "≥9.8/10 research quality score"
    - "100% documentation backing for implementations"
