...
class ConditionalInstructionsLoader {
  constructor() {
    ...
    this.contexts = {
      'nextjs react typescript': {
        ...
        instructions: 'Focus on Next.js App Router, React Server Components, TypeScript best practices',
        priority: 10
      },
    ...
    const checks = [
      {
        ...
        condition: () => this.hasFiles(['package.json', 'next.config.js', 'tsconfig.json']) ||
      ...
      },
      {
        ...
        condition: () => this.hasFiles(['.env.local', '.env']) ||
  ...
  loadInstructions(context) {
    ...
    console.log(`🧠 Context: ${contextConfig.name}`);
    console.log(`📋 Instructions: ${contextConfig.instructions}`);
    console.log(`⭐ Priority: ${contextConfig.priority}`);
   ...
   */
  reset() {
    console.log('🔄 Resetting to base context...');
    ...
    const tests = [
      ...
      },
      {
        ...
        test: () => {
          ...
          return config && config.name && config.instructions;
        }
      },
      {
