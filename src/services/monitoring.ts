...
export class MonitoringService {
  ...
  error(message: string, service: string, error?: Error, metadata?: any): void {
  ...
  private getTransactionDuration(transactionId: string): number {
    ...
    const timestamp = parseInt(transactionId.split('_')[1]);
    return Date.now() - timestamp;
  }
  
  private async checkApiRoutes(): Promise<{ status: 'pass' | 'fail'; message?: string; duration: number }> {
  ...
  private async checkAuthSystem(): Promise<{ status: 'pass' | 'fail'; message?: string; duration: number }> {
    const start = Date.now();
    try {
  ...
  private calculateErrorRate(metrics: MetricData[]): number {
    const totalRequests = metrics.filter(m => m.name === 'requests.count')
      .reduce((acc, m) => acc + m.value, 0);
    
    const errorRequests = metrics.filter(m => m.name === 'requests.error')
      .reduce((acc, m) => acc + m.value, 0);
