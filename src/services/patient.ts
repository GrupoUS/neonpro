...
export class PatientService {
  ...
  async updatePatient(patientId: string, updates: PatientUpdateRequest, userId: string): Promise<Patient | null> {
    try {
      ...
      if (updates.personalInfo) {
        ...
        if (updates.personalInfo.dateOfBirth) updateData.date_of_birth = updates.personalInfo.dateOfBirth.toISOString();
        ...
        if (updates.personalInfo.nationality !== undefined) updateData.nationality = updates.personalInfo.nationality;
        if (updates.personalInfo.maritalStatus !== undefined) updateData.marital_status = updates.personalInfo.maritalStatus;
        if (updates.personalInfo.profession !== undefined) updateData.profession = updates.personalInfo.profession;
        if (updates.personalInfo.photoUrl !== undefined) updateData.photo_url = updates.personalInfo.photoUrl;
      }
      ...
      if (updates.contactInfo) {
        ...
        if (updates.contactInfo.whatsapp !== undefined) updateData.whatsapp = updates.contactInfo.whatsapp;
        if (updates.contactInfo.preferredContactMethod) updateData.preferred_contact_method = updates.contactInfo.preferredContactMethod;
        
        if (updates.contactInfo.address) {
          if (updates.contactInfo.address.street) updateData.street = updates.contactInfo.address.street;
          if (updates.contactInfo.address.number) updateData.street_number = updates.contactInfo.address.number;
          if (updates.contactInfo.address.complement !== undefined) updateData.complement = updates.contactInfo.address.complement;
          if (updates.contactInfo.address.neighborhood) updateData.neighborhood = updates.contactInfo.address.neighborhood;
          if (updates.contactInfo.address.city) updateData.city = updates.contactInfo.address.city;
          if (updates.contactInfo.address.state) updateData.state = updates.contactInfo.address.state;
          if (updates.contactInfo.address.zipCode) updateData.zip_code = updates.contactInfo.address.zipCode;
          if (updates.contactInfo.address.country) updateData.country = updates.contactInfo.address.country;
        }
      }
      
      // Medical info updates
      if (updates.medicalInfo) {
        if (updates.medicalInfo.bloodType !== undefined) updateData.blood_type = updates.medicalInfo.bloodType;
        if (updates.medicalInfo.allergies) updateData.allergies = updates.medicalInfo.allergies;
        if (updates.medicalInfo.medications) updateData.medications = updates.medicalInfo.medications;
        if (updates.medicalInfo.medicalConditions) updateData.medical_conditions = updates.medicalInfo.medicalConditions;
        if (updates.medicalInfo.familyHistory) updateData.family_history = updates.medicalInfo.familyHistory;
        if (updates.medicalInfo.smokingStatus) updateData.smoking_status = updates.medicalInfo.smokingStatus;
        if (updates.medicalInfo.alcoholConsumption) updateData.alcohol_consumption = updates.medicalInfo.alcoholConsumption;
        if (updates.medicalInfo.exerciseFrequency) updateData.exercise_frequency = updates.medicalInfo.exerciseFrequency;
        if (updates.medicalInfo.notes !== undefined) updateData.medical_notes = updates.medicalInfo.notes;
      }
      
      // Emergency contact updates
      if (updates.emergencyContact) {
        if (updates.emergencyContact.name !== undefined) updateData.emergency_contact_name = updates.emergencyContact.name;
        if (updates.emergencyContact.relationship !== undefined) updateData.emergency_contact_relationship = updates.emergencyContact.relationship;
        if (updates.emergencyContact.phone !== undefined) updateData.emergency_contact_phone = updates.emergencyContact.phone;
        if (updates.emergencyContact.email !== undefined) updateData.emergency_contact_email = updates.emergencyContact.email;
      }
      ...
      if (error) {
        monitoring.error('Patient update failed', 'patient-service', new Error(error.message), { patientId });
        throw new Error(error.message);
      }
  ...
  async deletePatient(patientId: string, userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        monitoring.error('Patient deletion failed', 'patient-service', new Error(error.message), { patientId });
        throw new Error(error.message);
      }
  ...
  async searchPatients(filters: PatientSearchFilters, userId: string): Promise<{ patients: Patient[]; total: number }> {
    try {
      ...
      if (error) {
        monitoring.error('Patient search failed', 'patient-service', new Error(error.message), { filters });
        throw new Error(error.message);
      }
  ...
  async getPatientHistory(patientId: string, userId: string): Promise<PatientHistory[]> {
    try {
      ...
      monitoring.error('Get patient history error', 'patient-service', error as Error, { patientId });
      throw error;
    }
  }
  ...
  async getPatientConsents(patientId: string, userId: string): Promise<PatientConsent[]> {
    try {
      ...
      monitoring.error('Get patient consents error', 'patient-service', error as Error, { patientId });
      throw error;
    }
  }
  ...
  async getPatientStats(tenantId: string, userId: string): Promise<{
    ...
    try {
      ...
      const patientsByLoyaltyLevel = patients?.reduce((acc, patient) => {
        acc[patient.loyalty_level] = (acc[patient.loyalty_level] || 0) + 1;
        return acc;
      }, {} as Record<string, number>) || {};
      
      // Analyze risk levels
      const patientsByRiskLevel = patients?.reduce((acc, patient) => {
