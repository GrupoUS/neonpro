...
export class ComplianceService {
  ...
  async searchPolicies(filters: ComplianceFilters, userId: string): Promise<{ policies: CompliancePolicy[]; total: number }> {
    try {
      monitoring.debug('Searching compliance policies', 'compliance-service', { filters });
  ...
  private async assessDataImpact(affectedData: Omit<DataImpactAssessment, 'notificationRequired' | 'regulatoryReportingRequired'>): Promise<DataImpactAssessment> {
  ...
  private async autoAssignIncident(incidentId: string, tenantId: string): Promise<void> {
    ...
    monitoring.info('Auto-assigning incident', 'compliance-service', { incidentId, tenantId });
  }
  ...
  private async calculateAverageResponseTime(tenantId: string, periodStart: Date, periodEnd: Date): Promise<number> {
  ...
  private async createTrendsAnalysis(tenantId: string, periodStart: Date, periodEnd: Date): Promise<TrendsAnalysis> {
  ...
  private async createComplianceByFramework(tenantId: string): Promise<Record<ComplianceFramework, FrameworkCompliance>> {
