...
export class NotificationService {
  ...
  async sendNotification(request: SendNotificationRequest, userId?: string): Promise<Notification> {
    try {
      ...
      if (error) {
        monitoring.error('Notification creation failed', 'notification-service', new Error(error.message), { 
          tenantId: request.tenantId 
        });
  ...
  async sendBulkNotification(request: SendBulkNotificationRequest, userId: string): Promise<NotificationBatch> {
    try {
  ...
  async sendTemplatedNotification(
    ...
    try {
      ...
      monitoring.error('Send templated notification error', 'notification-service', error as Error, { 
        templateId,
        recipientId 
      });
      throw error;
    ...
    try {
      ...
      try {
        ...
        if (success) {
          ...
          monitoring.info('Notification sent successfully', 'notification-service', { notificationId });
          return true;
        } else {
      ...
      monitoring.error('Process notification error', 'notification-service', error as Error, { notificationId });
      return false;
    }
  }
  ...
  async getNotification(notificationId: string, userId: string): Promise<Notification | null> {
    try {
      ...
      monitoring.error('Get notification error', 'notification-service', error as Error, { notificationId });
      return null;
    }
  }
  
  async searchNotifications(filters: NotificationFilters, userId: string): Promise<{ notifications: Notification[]; total: number }> {
    try {
      ...
      monitoring.error('Search notifications error', 'notification-service', error as Error, { filters });
      throw error;
    }
  }
  ...
  async createTemplate(request: CreateTemplateRequest, userId: string): Promise<NotificationTemplate> {
    try {
  ...
  async getTemplate(templateId: string, userId: string): Promise<NotificationTemplate | null> {
    try {
      ...
      monitoring.error('Get template error', 'notification-service', error as Error, { templateId });
      return null;
    }
  }
  ...
  async getNotificationAnalytics(
    ...
    try {
      ...
      const totalDelivered = notifications.filter(n => n.status === NotificationStatus.DELIVERED || n.status === NotificationStatus.READ).length;
      ...
      Object.values(NotificationChannel).forEach(channel => {
        ...
        const channelDelivered = channelNotifications.filter(n => n.status === NotificationStatus.DELIVERED || n.status === NotificationStatus.READ).length;
      ...
      Object.values(NotificationType).forEach(type => {
        ...
        const typeDelivered = typeNotifications.filter(n => n.status === NotificationStatus.DELIVERED || n.status === NotificationStatus.READ).length;
        const typeFailed = typeNotifications.filter(n => n.status === NotificationStatus.FAILED).length;
      ...
      Object.values(NotificationPriority).forEach(priority => {
        ...
        const priorityDelivered = priorityNotifications.filter(n => n.status === NotificationStatus.DELIVERED || n.status === NotificationStatus.READ).length;
        const priorityFailed = priorityNotifications.filter(n => n.status === NotificationStatus.FAILED).length;
        
        // Calculate average delivery time
        const deliveredNotifications = priorityNotifications.filter(n => n.sent_at && (n.status === NotificationStatus.DELIVERED || n.status === NotificationStatus.READ));
  ...
  private async checkNotificationPreferences(
    ...
    try {
      ...
      monitoring.error('Check notification preferences error', 'notification-service', error as Error, { 
        recipientId,
        type,
        channel 
      });
  ...
  private async handleNotificationFailure(notificationId: string, errorMessage: string): Promise<void> {
