...
export class AuthenticationService {
  ...
  async disableMfa(userId: string, password: string): Promise<boolean> {
    try {
      ...
      if (error) {
        monitoring.error('MFA disable failed', 'auth-service', new Error(error.message), { userId });
        return false;
      }
  ...
  async invalidateAllUserSessions(userId: string, excludeSessionId?: string): Promise<boolean> {
    try {
      ...
      monitoring.error('Invalidate all user sessions error', 'auth-service', error as Error, { userId });
      return false;
    }
  }
  ...
  private async storeTempMfaData(userId: string, secret: string, backupCodes: string[]): Promise<void> {
    // Implementation would store temporary MFA data
  }
  
  private async getTempMfaData(userId: string): Promise<{ secret: string; backupCodes: string[] } | null> {
