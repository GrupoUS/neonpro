...
export const databaseService = {
  ...
  insert: async (tableName: string, record: any) => {
    const { data, error } = await supabase.from(tableName as any).insert(record).select();
    if (error) throw error;
    return data?.[0];
  },
  ...
  update: async (tableName: string, id: string, updates: any) => {
    const { data, error } = await supabase.from(tableName as any).update(updates).eq('id', id).select();
    if (error) throw error;
    return data?.[0];
  },
