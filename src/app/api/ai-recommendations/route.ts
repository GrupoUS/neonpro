...
export async function GET(request: NextRequest) {
  try {
    ...
    const [appointmentsResult, patientsResult, treatmentsResult] =
      await Promise.all([
        supabase
          .from("appointments")
          .select("*")
...
async function generateAIRecommendations(data: {
  ...
  try {
    ...
    const dataSummary = {
      ...
      activeTreatments: treatments.filter((t) => t.status === "in_progress")
        .length,
      ...
      treatmentTypes: treatments.reduce((acc, treatment) => {
        acc[treatment.treatment_name] =
          (acc[treatment.treatment_name] || 0) + 1;
        return acc;
      }, {}),
    };
    ...
    - Appointment Status Distribution: ${JSON.stringify(
      dataSummary.appointmentsByStatus
    )}
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an AI assistant specialized in medical aesthetic clinic management and optimization.",
        },
        {
          role: "user",
          content: prompt,
...
async function generatePersonalizedRecommendations(data: {
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a medical aesthetic AI assistant providing safe, evidence-based treatment recommendations.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return {
      ...
      treatmentType: data.treatmentType,
      recommendations:
        "Unable to generate personalized recommendations at this time. Please consult with a medical professional.",
      generatedAt: new Date().toISOString(),
      confidence: 0,
    };
  }
  ...
  const recommendations = [
    {
      ...
      description:
        "Optimize current treatment protocols based on patient response data",
      confidence: 88,
      priority: "high",
      recommendations: [
    ...
    },
    {
      ...
      description:
        "Improve scheduling efficiency to reduce wait times and increase satisfaction",
      confidence: 82,
      priority: "medium",
      recommendations: [
        "Implement time-slot optimization",
