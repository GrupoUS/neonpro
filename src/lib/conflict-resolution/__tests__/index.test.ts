import { IntelligentConflictResolutionSystem } from '../index';
import { 
  ConflictType, 
  ConflictSeverity, 
  ResolutionStrategy,
...
describe('IntelligentConflictResolutionSystem', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      detection: {
        enabledTypes: [ConflictType.TIME_OVERLAP, ConflictType.STAFF_CONFLICT, ConflictType.ROOM_CONFLICT],
        severityThresholds: {
          low: 0.3,
          medium: 0.6,
          high: 0.8
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle invalid appointment data', async () => {
      ...
      await expect(system.detectAndResolveConflicts(invalidAppointment as any))
        .rejects.toThrow();
    });
    ...
    it('should handle database connection failures', async () => {
      ...
      await expect(system.detectAndResolveConflicts(appointment))
    ...
    it('should handle empty date ranges gracefully', async () => {
      ...
      await expect(system.optimizeResourceAllocation(emptyDateRange))
        .rejects.toThrow();
    });
      ...
      mockSupabase.from.mockReturnValue({
        select: jest.fn(() => ({
          gte: jest.fn(() => ({
            lte: jest.fn(() => ({
              eq: jest.fn(() => ({
                neq: jest.fn(() => new Promise((_, reject) => {
