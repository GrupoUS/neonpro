...
export class PatientJourneyMappingEngine {
   ...
   */
  private async checkAndProgressStage(patientId: string, eventType: JourneyEventType): Promise<void> {
    try {
  ...
  private calculateConversionProbability(currentState: JourneyState, stages: any[]): number {
    ...
    const averageDuration = stages.reduce((sum, stage) => {
    ...
    const avgSatisfaction = stages
      .filter(s => s.satisfaction_score)
      .reduce((sum, s) => sum + s.satisfaction_score, 0) / 
  ...
  private async calculateChurnRiskScore(
    ...
    const latestSatisfaction = stages
      .filter(s => s.satisfaction_score)
      .pop()?.satisfaction_score
  ...
  private generateRecommendations(
    ...
    const daysSinceLastActivity = events.length > 0 ? Math.round(
      (new Date().getTime() - new Date(events[events.length - 1].timestamp).getTime()) / (1000 * 60 * 60 * 24)
    ) : 0
  ...
  private identifySuccessFactors(stages: any[], events: any[]): string[] {
    ...
    // Quick progression
    const avgStageDuration = stages.reduce((sum, stage) => {
      return sum + (stage.duration_minutes || 0)
    }, 0) / stages.length
    ...
    // Consistent activity
    const eventSpread = events.length > 1 ? 
