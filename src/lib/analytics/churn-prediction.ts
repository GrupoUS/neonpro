...
export interface FeatureImportanceAnalysis {
  analysis_date: Date
  model_version: string
  feature_importance: Record<ChurnPredictionFeature, {
...
export class ChurnPredictionEngine {
  ...
  async createPreventionCampaign(
    ...
    try {
      ...
      const campaign: ChurnPreventionCampaign = {
        ...
        campaign_name: campaignConfig.campaign_name || `Retention Campaign ${new Date().toISOString().split('T')[0]}`,
  ...
  async monitorChurnRisk(): Promise<{ 
    ...
    try {
      ...
      if (highRiskPatients.length >= 5) { // Threshold for auto-campaign
  ...
  private async makePrediction(features: Record<string, number>): Promise<{ probability: number; confidence: number }> {
    // Make prediction using trained model
  ...
  private async identifyContributingFactors(
    ...
    for (const [feature, value] of Object.entries(features)) {
      ...
      const trend = await this.getFeatureTrend(prediction.patient_id || '', feature as ChurnPredictionFeature)
  ...
  private async calculateFeatureValue(patientId: string, feature: ChurnPredictionFeature): Promise<number> {
  ...
  private async getFeatureTrend(patientId: string, feature: ChurnPredictionFeature): Promise<'improving' | 'stable' | 'declining'> {
