...
const DataClassificationSchema = z.object({
  ...
  overrideClassification: z.object({
...
const ProfessionalValidationSchema = z.object({
  ...
  validationType: z.enum(['license', 'credentials', 'ethics', 'comprehensive']).default('comprehensive'),
...
async function handleApiError(error: any, context: string) {
  console.error(`[COMPLIANCE_API] Error in ${context}:`, error);
...
export async function classifyData(req: NextRequest) {
  try {
    ...
    const { data: classificationResult, error } = await supabase
      .rpc('auto_classify_data_element', {
    ...
    const { data: storedClassification, error: storeError } = await supabase
      ...
      .upsert({
        ...
        encryption_required: finalClassification.encryption_required || false,
    ...
    if (storeError) {
      console.warn('Failed to store classification:', storeError);
    }
  ...
  try {
    ...
    const { data: request, error: createError } = await supabase
      ...
      .insert({
        ...
        target_completion_date: calculateCompletionDate(requestData.requestType, requestData.urgency)
      })
      .select()
      .single();
    ...
    if (requestData.identityVerificationData.verificationMethod === 'digital_signature') {
      ...
      if (!verifyError) {
        ...
        const { data: processResult, error: processError } = await supabase
          .rpc('process_data_subject_request', {
    ...
    return NextResponse.json({
      ...
      next_steps: generateNextSteps(requestData.requestType, requestData.identityVerificationData.verificationMethod)
    });
...
export async function getConsentStatus(req: NextRequest, { params }: { params: { dataSubjectId: string } }) {
  try {
...
export async function validateSoftware(req: NextRequest) {
  try {
    ...
    const { data: validationResult, error } = await supabase
      .rpc('validate_iec62304_compliance', {
...
export async function getSoftwareLifecycle(req: NextRequest, { params }: { params: { itemName: string } }) {
  try {
...
export async function validateProfessional(req: NextRequest) {
  try {
    ...
    const { data: validationResult, error } = await supabase
      .rpc('validate_cfm_professional_status', {
    ...
    let recommendations = [];
    if (includeRecommendations && (!validationResult.valid || validationResult.warnings?.length > 0)) {
    ...
    return NextResponse.json({
      ...
      recommendations,
      certificate: validationResult.valid ? await generateProfessionalCertificate(professionalId, validationResult) : null
    });
...
export async function getProfessionalStatus(req: NextRequest, { params }: { params: { professionalId: string } }) {
  try {
    ...
    return NextResponse.json({
      ...
      quick_validation: {
        license_valid: professional.license_status === 'active' && professional.license_expiry_date > new Date().toISOString(),
...
export async function getComplianceStatus(req: NextRequest) {
  try {
    ...
    const { data: complianceStatus, error } = await supabase
      .rpc('monitor_overall_compliance_status', {
...
function assessConsentCompliance(consents: any[]) {
  ...
  return {
    ...
    compliance_percentage: activeConsents.length > 0 ? 
      ((activeConsents.length - expiredConsents.length) / activeConsents.length) * 100 : 100
  };
}
