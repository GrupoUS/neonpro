...
class ComprehensiveFixer {
  ...
  fixRemainingPatterns(content) {
    ...
    const patterns = [
      ...
      { from: /http\.createServer\(_(_req,_res)\)/g, to: 'http.createServer((req, res))' },
      { from: /\.listen\(_(_this\.config\.httpPort),(_this\.config\.host),_(_\(\)\s*=>)/g, to: '.listen(this.config.httpPort, this.config.host, () =>' },
      { from: /\.listen\(_(_this\.config\.port),(_this\.config\.host),_(_\(\)\s*=>)/g, to: '.listen(this.config.port, this.config.host, () =>' },
      { from: /Promise\(_(_resolve,_reject)\)/g, to: 'Promise((resolve, reject))' },
      { from: /_new Promise<void>\(_(closeResolve)\)/g, to: 'new Promise<void>(closeResolve' },
      { from: /Promise\.all\(stopPromises\)\.then\(_(_\(\)\s*=>)/g, to: 'Promise.all(stopPromises).then(() =>' },
      { from: /process\.on\(_'SIGTERM',_\(_\(\)\s*=>/g, to: "process.on('SIGTERM', () =>" },
      { from: /process\.on\(_'SIGINT',_\(_\(\)\s*=>/g, to: "process.on('SIGINT', () =>" },
      { from: /certbot\.on\(_'close',_\(code:_number\)\s*=>/g, to: "certbot.on('close', (code: number) =>" },
    ...
    for (const pattern of patterns) {
      ...
      if (matches) {
        ...
        this.log(`Applied pattern: ${pattern.from.toString()} - ${matches.length} matches`, 'debug');
      }
    }
