...
console.log(
  `Found ${fixes.length} fixes needed across ${Object.keys(fileGroups).length} files`,
);
...
Object.entries(fileGroups).forEach(([filePath, fileFixes]) => {
  ...
  if (modified) {
    ...
    console.log(
      `  ✅ Fixed ${fileFixes.length} issues in ${path.basename(filePath)}`,
    );
  } else {
...
try {
  const afterOutput = execSync(
    'cd /home/vibecode/neonpro/apps/web && npx oxlint src 2>&1 | grep "is imported but never used" | wc -l',
    { encoding: "utf8" },
  );
