...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im93bmtveHJ5c3dva2NkYW5yZGdqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjQ2MTEwNDQsImV4cCI6MjA0MDE4NzA0NH0.2kD9rN4tOFgJQWbOhYHDxMkVqJ_3-0EfP5gK2vKC0-0';
...
async function testPatientStats() {
  ...
  try {
    ...
    console.log('Patients result:', patientsResult.error || patientsResult.data?.length);
    console.log('New patients result:', newPatientsResult.error || newPatientsResult.data?.length);
    console.log('Appointments result:', appointmentsResult.error || appointmentsResult.data?.length);
    console.log('Revenue result:', revenueResult.error || revenueResult.data?.length);
