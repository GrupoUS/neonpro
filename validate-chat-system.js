...
const healthcareKeywords = [
  'portuguese', 'brasileiro', 'lgpd', 'cfm', 'anvisa', 'brazilian', 
  'dermatologia', 'estética', 'emergência', 'pt-BR'
];
...
for (const filePath of CHAT_SYSTEM_FILES) {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    if (foundKeywords.length >= 2) {
      totalScore += 5;
      console.log(`  ✅ ${path.basename(filePath)} - Healthcare context: ${foundKeywords.length} keywords`);
...
if (fs.existsSync(lgpdPath)) {
  ...
  const lgpdFeatures = [
    'consent', 'encryption', 'retention', 'erasure', 'portability', 'audit'
  ];
  
  const foundFeatures = lgpdFeatures.filter(feature => 
  ...
  console.log(`  ✅ LGPD Compliance: ${foundFeatures.length}/${lgpdFeatures.length} features (${score}/20 points)`);
...
if (fs.existsSync(realtimePath)) {
  ...
  const realtimeFeatures = [
    'supabase', 'websocket', 'subscription', 'realtime', 'channel'
  ];
  ...
  console.log(`  ✅ Real-time: ${foundFeatures.length}/${realtimeFeatures.length} features (${score}/15 points)`);
