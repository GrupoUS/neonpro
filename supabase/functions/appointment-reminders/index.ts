...
interface ReminderRequest {
  ...
  patientId: string;
  clinicId: string;
  reminderType: 'confirmation' | 'reminder_24h' | 'reminder_2h' | 'no_show_prevention' | 'rescheduling';
...
*Mensagem automática conforme LGPD - seus dados estão protegidos* 🔒`,
    ...
    sms: {
      message: "CONFIRMAÇÃO: Consulta {appointmentDate} às {appointmentTime} na {clinicName}. Dr(a) {doctorName}. Cancelar/reagendar: {clinicPhone}",
    },
    email: {
      subject: "Confirmação de Consulta - {clinicName}",
      html: `<!DOCTYPE html>
...
*LGPD: dados protegidos* 🔒`,
    ...
    sms: {
      message: "LEMBRETE: Consulta AMANHÃ {appointmentDate} às {appointmentTime} - {clinicName}. Dr(a) {doctorName}. Chegue 15min antes. Confirme: {clinicPhone}",
    },
  },
  reminder_2h: {
    whatsapp: {
...
*Dados protegidos LGPD* 🔒`,
    ...
    sms: {
      message: "URGENTE: Consulta EM 2H às {appointmentTime} - {clinicName}. Saia às {suggestedDepartureTime}. Cancelar: {clinicPhone}",
    },
  },
  no_show_prevention: {
    whatsapp: {
      subject: "🎯 Consulta importante hoje - {clinicName}",
      message: `{patientName}, detectamos que você pode ter dificuldades para comparecer à consulta hoje. 🎯
 ...
 */
async function getPatientPreferences(patientId: string, clinicId: string): Promise<{
 ...
 * Send SMS message
 */
async function sendSMSMessage(phone: string, message: string): Promise<DeliveryStatus> {
  if (!SMS_API_KEY) {
    return {
      status: 'failed',
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      details: {
        ...
        successful_deliveries: Object.values(deliveryStatus).filter((s: any) => s.status === 'sent').length,
        ...metadata,
      },
...
serve(async (req) => {
  ...
  try {
    ...
    if (!requestData.appointmentId || !requestData.patientId || !requestData.clinicId) {
      return new Response(
    ...
    const patientPrefs = await getPatientPreferences(requestData.patientId, requestData.clinicId);
    ...
    const templateVars = {
      ...
      appointmentDate: new Date(appointment.scheduled_for).toLocaleDateString('pt-BR'),
      appointmentTime: new Date(appointment.scheduled_for).toLocaleTimeString('pt-BR', { 
        hour: '2-digit', 
        minute: '2-digit' 
      }),
      ...
      suggestedDepartureTime: new Date(new Date(appointment.scheduled_for).getTime() - 30 * 60000)
    ...
    // WhatsApp delivery
    if (channels.includes('whatsapp') && patientPrefs.whatsappConsent && patientPrefs.phone) {
      const message = replaceTemplateVariables(template.whatsapp?.message || '', templateVars);
      deliveryStatus.whatsapp = await sendWhatsAppMessage(patientPrefs.phone, message);
    }

    // SMS delivery
    if (channels.includes('sms') && patientPrefs.phone) {
      const message = replaceTemplateVariables(template.sms?.message || '', templateVars);
      deliveryStatus.sms = await sendSMSMessage(patientPrefs.phone, message);
    }

    // Email delivery
    if (channels.includes('email') && patientPrefs.email) {
      const subject = replaceTemplateVariables(template.email?.subject || '', templateVars);
      const html = replaceTemplateVariables(template.email?.html || '', templateVars);
      deliveryStatus.email = await sendEmailMessage(patientPrefs.email, subject, html);
    }
    ...
    const response: ReminderResponse = {
      success: Object.values(deliveryStatus).some(status => status.status === 'sent'),
      reminderJobId,
      deliveryStatus,
      fallbackChain,
