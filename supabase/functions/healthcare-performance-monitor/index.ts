...
interface PerformanceMetric {
  metric_type: 'response_time' | 'error_rate' | 'throughput' | 'availability' | 'user_sessions'
  metric_value: number
  timestamp: string
  service_name: string
  endpoint?: string
...
interface PerformanceAlert {
  alert_type: 'performance_degradation' | 'high_error_rate' | 'system_down' | 'sla_breach'
...
async function handleMetricsCollection(req: Request, supabase: any) {
  ...
  try {
    ...
    const realtimeResponse = await fetch(`${supabaseUrl}/realtime/v1/websocket`, {
      method: 'HEAD',
      headers: {
    ...
    healthResults.push({
      ...
      status: realtimeResponse.ok ? 
        (realtimeTime > 50 ? 'degraded' : 'healthy') : 'unhealthy',
  ...
  try {
    ...
    healthResults.push({
      ...
      status: healthyFunctions === functionChecks.length ? 'healthy' : 
             healthyFunctions > 0 ? 'degraded' : 'unhealthy',
...
async function generatePerformanceAlerts(
  ...
  for (const [serviceKey, serviceMetrics] of Object.entries(metricsByService)) {
    ...
    if (metricType === 'response_time') {
      const avgResponseTime = serviceMetrics.reduce(
      ...
      if (avgResponseTime > PERFORMANCE_THRESHOLDS.response_time.poor) {
        alerts.push({
          ...
          metrics: { avg_response_time: avgResponseTime, threshold: PERFORMANCE_THRESHOLDS.response_time.poor },
          timestamp: new Date().toISOString()
        })
      } else if (avgResponseTime > PERFORMANCE_THRESHOLDS.response_time.acceptable) {
        alerts.push({
          alert_type: 'performance_degradation',
          severity: 'medium',
          ...
          affected_services: [serviceName],
          metrics: { avg_response_time: avgResponseTime, threshold: PERFORMANCE_THRESHOLDS.response_time.acceptable },
          timestamp: new Date().toISOString()
        })
      }
    }
    ...
    if (metricType === 'error_rate') {
      const avgErrorRate = serviceMetrics.reduce(
        (sum, m) => sum + m.metric_value, 0
      ) / serviceMetrics.length
      ...
      if (avgErrorRate > PERFORMANCE_THRESHOLDS.error_rate.poor) {
        alerts.push({
          ...
          affected_services: [serviceName],
          metrics: { avg_error_rate: avgErrorRate, threshold: PERFORMANCE_THRESHOLDS.error_rate.poor },
          timestamp: new Date().toISOString()
        })
      }
    }
  ...
  for (const [serviceName, data] of Object.entries(serviceMetrics)) {
    const avgResponseTime = data.response_times.length > 0 ? 
      data.response_times.reduce((a: number, b: number) => a + b, 0) / data.response_times.length : 0
    const avgErrorRate = data.error_rates.length > 0 ?
      data.error_rates.reduce((a: number, b: number) => a + b, 0) / data.error_rates.length : 0
    
    summary.services[serviceName] = {
      avg_response_time: Math.round(avgResponseTime),
      error_rate: parseFloat(avgErrorRate.toFixed(2)),
      total_requests: data.total_requests,
      sla_compliance: avgResponseTime < PERFORMANCE_THRESHOLDS.response_time.good ? 100 : 
                     avgResponseTime < PERFORMANCE_THRESHOLDS.response_time.acceptable ? 75 : 50
    }
  ...
  const totalResponseTimeEntries = Object.values(serviceMetrics)
    .reduce((sum, data) => sum + data.response_times.length, 0)
  const totalErrorRateEntries = Object.values(serviceMetrics)
    .reduce((sum, data) => sum + data.error_rates.length, 0)
  
  summary.total_requests = totalRequests
  summary.avg_response_time = totalResponseTimeEntries > 0 ? 
    Math.round(totalResponseTime / totalResponseTimeEntries) : 0
  summary.error_rate = totalErrorRateEntries > 0 ? 
    parseFloat((totalErrors / totalErrorRateEntries).toFixed(2)) : 0
  summary.availability = summary.error_rate < 1 ? 99.9 : 99.0
  summary.sla_compliance = summary.avg_response_time < PERFORMANCE_THRESHOLDS.response_time.good ? 100 : 
                          summary.avg_response_time < PERFORMANCE_THRESHOLDS.response_time.acceptable ? 75 : 50
