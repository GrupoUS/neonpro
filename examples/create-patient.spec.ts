// Example: Patient Management E2E Test
...
test.describe('Patient Management - Create Patient', () => {
  ...
  test('should require LGPD consent', async ({ page }) => {
    ...
    // Fill all required fields except LGPD consent
    await page.getByLabel(/nome completo/i).fill('Test User');
    await page.getByLabel(/cpf/i).fill('123.456.789-09');
    await page.getByLabel(/telefone/i).fill('(11) 99999-9999');
    await page.getByLabel(/email/i).fill('test@email.com');

    // Try to submit without consent
    await page.getByRole('button', { name: /salvar paciente/i }).click();

    // Verify LGPD validation error
    await expect(
      page.getByText(/consentimento lgpd é obrigatório/i)
    ).toBeVisible();
  });
});
