...
const brazilianKeywords = [
  // CFM Keywords
  'cfm', 'crm', 'conselho', 'medicina', 'medico', 'especialidade',
  
  // ANVISA Keywords  
  'anvisa', 'controlada', 'substancia', 'prescricao', 'receituario',
  'amarelo', 'azul', 'branco', 'notificacao',
  
  // LGPD Keywords
  'lgpd', 'consentimento', 'titular', 'dados', 'privacidade', 'erasure',
  'portabilidade', 'base legal', 'finalidade',
  
  // Brazilian Healthcare Context
  'brasileiro', 'brasil', 'samu', 'sus', 'coren', 'crf'
];
...
for (const filePath of COMPLIANCE_SYSTEM_FILES) {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    if (score >= 6) {
      console.log(`  ✅ ${path.basename(filePath)} - Brazilian context: ${fileKeywords.length}/${brazilianKeywords.length} keywords`);
    } else if (score >= 4) {
      console.log(`  ⚠️  ${path.basename(filePath)} - Limited Brazilian context: ${fileKeywords.length}/${brazilianKeywords.length} keywords`);
    } else {
      console.log(`  ❌ ${path.basename(filePath)} - Missing Brazilian context: ${fileKeywords.length}/${brazilianKeywords.length} keywords`);
...
if (fs.existsSync(cfmPath)) {
  ...
  console.log(`  ✅ CFM Validation System: ${foundFeatures.length}/${cfmFeatures.length} features (${score}/25 points)`);
...
if (fs.existsSync(anvisaPath)) {
  ...
  console.log(`  ✅ ANVISA System: ${foundFeatures.length}/${anvisaFeatures.length} features (${score}/25 points)`);
...
if (fs.existsSync(lgpdPath)) {
  ...
  console.log(`  ✅ LGPD System: ${foundFeatures.length}/${lgpdFeatures.length} features (${score}/25 points)`);
...
if (fs.existsSync(typesPath)) {
  ...
  console.log(`  ✅ TypeScript Types: ${foundTypes.length}/${typeDefinitions.length} definitions (${score}/20 points)`);
...
console.log(`   CFM Professional Validation: ${fs.existsSync(path.join(__dirname, cfmFile)) ? 'Implemented' : 'Missing'}`);
console.log(`   ANVISA Controlled Substances: ${fs.existsSync(path.join(__dirname, anvisaFile)) ? 'Implemented' : 'Missing'}`);
console.log(`   LGPD Consent Management: ${fs.existsSync(path.join(__dirname, lgpdFile)) ? 'Implemented' : 'Missing'}`);
...
console.log('   ANVISA (Agência Nacional de Vigilância Sanitária): ✅ Controlled substances tracking ready');
