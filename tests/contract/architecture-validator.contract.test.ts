...
} from '../../specs/003-monorepo-audit-optimization/contracts/architecture-validator.contract'
import type { CodeAsset } from '../../specs/003-monorepo-audit-optimization/contracts/file-scanner.contract'
...
describe('ArchitectureValidator Contract Tests', () => {
  ...
  describe('Framework-Specific Validation', () => {
    ...
    it('should validate Hono routing patterns correctly', async () => {
      ...
      violations.forEach((violation: ArchitectureViolation) => {
        expect(['hono_route_structure', 'hono_middleware_usage', 'hono_context_handling'].includes(violation.category))
      })
    })
  })
  ...
  describe('Contract Compliance', () => {
    it('should satisfy all interface requirements', async () => {
      if (!validator) {
        console.log('⚠️  ArchitectureValidator implementation missing - test will fail as expected (TDD)')
