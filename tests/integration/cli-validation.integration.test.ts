...
describe('CLI Validation Integration Tests', () => {
  ...
  describe('CLI Installation and Availability', () => {
    ...
    it('should be available via bun run', async () => {
      ...
      try {
        const { stdout, stderr } = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool --help')
        expect(stdout || stderr).toContain('audit-tool')
      } catch (error) {
    ...
    it('should display version with --version flag', async () => {
      ...
      try {
        const { stdout } = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool --version')
    ...
    it('should exit with code 0 for version command', async () => {
      // This will FAIL until CLI version command is implemented
      try {
        const result = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool --version')
    ...
    it('should validate workspace configuration', async () => {
      ...
      try {
        const { stdout } = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool --validate-config')
    ...
    it('should handle invalid workspace gracefully', async () => {
      // This will FAIL until CLI config validation is implemented
      try {
        const { stdout, stderr } = await execAsync('cd /tmp && bun run audit-tool --validate-config')
    ...
    it('should display help information', async () => {
      ...
      try {
        const { stdout } = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool --help')
  ...
  describe('Error Handling', () => {
    it('should handle invalid commands gracefully', async () => {
      ...
      try {
        const { stdout, stderr } = await execAsync('cd /home/vibecoder/neonpro && bun run audit-tool invalid-command')
