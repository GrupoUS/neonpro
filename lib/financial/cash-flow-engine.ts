...
export interface CashFlowAlert {
  id: string
  type: 'low_balance' | 'negative_trend' | 'high_burn_rate' | 'threshold_breach'
...
export class CashFlowEngine {
  ...
  async getCashFlowSummary(clinicId: string): Promise<CashFlowSummary> {
    try {
      ...
      const yesterdayData = historicalData.find(d => d.date === yesterday.toISOString().split('T')[0])
      const weekAgoData = historicalData.find(d => d.date === weekAgo.toISOString().split('T')[0])
      const monthAgoData = historicalData[0]

      const dailyChange = yesterdayData ? currentBalance - yesterdayData.closing_balance : 0
      const weeklyChange = weekAgoData ? currentBalance - weekAgoData.closing_balance : 0
      const monthlyChange = monthAgoData ? currentBalance - monthAgoData.closing_balance : 0

      // Calculate trend direction and percentage
      const recentTrend = this.calculateTrend(historicalData.slice(-7))
      const trendDirection = recentTrend > 0.05 ? 'up' : recentTrend < -0.05 ? 'down' : 'stable'
      const trendPercentage = Math.abs(recentTrend) * 100

      // Calculate cash burn rate (average daily outflow)
      const avgDailyOutflow = historicalData.reduce((sum, d) => sum + Math.abs(Math.min(d.net_cash_flow, 0)), 0) / historicalData.length
      const runwayDays = avgDailyOutflow > 0 ? Math.floor(currentBalance / avgDailyOutflow) : Infinity
  ...
  private async calculateDailyInflows(clinicId: string, date: string) {
    ...
    const operating = payments?.filter(p => p.category === 'treatment' || p.category === 'product').reduce((sum, p) => sum + p.amount, 0) || 0
    const investing = payments?.filter(p => p.category === 'investment').reduce((sum, p) => sum + p.amount, 0) || 0
    const financing = payments?.filter(p => p.category === 'loan' || p.category === 'equity').reduce((sum, p) => sum + p.amount, 0) || 0
  ...
  private async calculateDailyOutflows(clinicId: string, date: string) {
    ...
    const operating = expenses?.filter(e => e.category === 'operational' || e.category === 'staff').reduce((sum, e) => sum + e.amount, 0) || 0
    const investing = expenses?.filter(e => e.category === 'equipment' || e.category === 'technology').reduce((sum, e) => sum + e.amount, 0) || 0
    const financing = expenses?.filter(e => e.category === 'loan_payment' || e.category === 'interest').reduce((sum, e) => sum + e.amount, 0) || 0
   ...
   */
  private async projectCashFlow(clinicId: string, days: number): Promise<number> {
    try {
      ...
      const avgDailyCashFlow = historicalData.reduce((sum, d) => sum + d.net_cash_flow, 0) / historicalData.length
   ...
   */
  async getCashFlowMetrics(clinicId: string, days: number = 30): Promise<CashFlowMetrics> {
    try {
      ...
      const dailyAverageInflow = dailyInflows.reduce((sum, val) => sum + val, 0) / dailyInflows.length
      const dailyAverageOutflow = dailyOutflows.reduce((sum, val) => sum + val, 0) / dailyOutflows.length
      ...
      const variance = dailyChanges.reduce((sum, val) => sum + Math.pow(val - avgDailyChange, 2), 0) / dailyChanges.length
      const weeklyVolatility = Math.sqrt(variance) * Math.sqrt(7) // Annualized to weekly

      // Calculate growth rate
      const firstBalance = balances[0]
      const lastBalance = balances[balances.length - 1]
      const monthlyGrowthRate = firstBalance > 0 ? ((lastBalance - firstBalance) / firstBalance) * 100 : 0
