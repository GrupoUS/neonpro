...
export class RevenueAnalyticsEngine {
  ...
  async analyzeRevenueByService(filters: RevenueAnalysisFilters): Promise<ServiceRevenueData[]> {
    ...
    const enrichedData = await Promise.all(
      serviceData.map(async (service: any) => {
        const costAllocation = await this.calculateServiceCostAllocation(service.service_id, dateRange);
  ...
  async calculateServiceProfitability(
    ...
    const directCostTotal = directCosts?.reduce((sum, cost) => sum + parseFloat(cost.amount), 0) || 0;
    ...
    return {
      ...
      grossProfit,
      marginPercent,
      breakEvenVolume: totalCost > 0 ? Math.ceil(totalCost / (totalRevenue / (revenueData?.[0]?.transaction_count || 1))) : 0
    };
  }
   ...
   */
  async analyzeProviderPerformance(filters: RevenueAnalysisFilters): Promise<ProviderRevenueData[]> {
    ...
    const enrichedData = await Promise.all(
      providerData.map(async (provider: any) => {
        const utilizationRate = await this.calculateProviderUtilization(provider.provider_id, dateRange);
        const conversionRate = await this.calculateProviderConversionRate(provider.provider_id, dateRange);
        const growthTrend = await this.calculateProviderGrowthTrend(provider.provider_id, dateRange);
  ...
  async generateTimePeriodAnalysis(
    ...
    const { data: timeSeriesData, error } = await this.supabase.rpc('generate_time_series_analysis', {
      p_clinic_id: clinicId,
  ...
  async getRevenueAnalyticsDashboard(
    ...
    return {
      ...
      summary: {
        ...
        averageProfitMargin: serviceRevenue.reduce((sum, s) => sum + s.profitMargin, 0) / serviceRevenue.length,
