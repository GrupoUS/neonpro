...
export class ANVISACompliance {
  ...
  async registerProduct(product: Omit<ANVISAProduct, 'id' | 'created_at' | 'updated_at'>): Promise<ANVISAProduct | null> {
    try {
  ...
  async validateProductCompliance(productId: string): Promise<boolean> {
    try {
      ...
      console.error('Error validating product compliance:', error);
      return false;
    }
  }

  async getExpiringSoonProducts(days: number = 30): Promise<ANVISAProduct[]> {
    try {
  ...
  async classifyProcedure(procedure: Omit<ANVISAProcedure, 'id' | 'created_at' | 'updated_at'>): Promise<ANVISAProcedure | null> {
    try {
      const { data, error } = await this.supabase
      ...
      console.error('Error classifying procedure:', error);
      return null;
    }
  }
  ...
  async validateProcedureQualifications(procedureId: string, professionalQualifications: string[]): Promise<boolean> {
    try {
      ...
      console.error('Error validating procedure qualifications:', error);
      return false;
    }
  }
  ...
  private async scheduleANVISAReport(eventId: string): Promise<void> {
    ...
    try {
      ...
      console.error('Error scheduling ANVISA report:', error);
    }
  }
  ...
  async getPendingANVISAReports(): Promise<any[]> {
    try {
      ...
      console.error('Error fetching pending ANVISA reports:', error);
      return [];
    }
  }
    ...
    try {
      ...
      return {
        ...
        products: {
          ...
          approved: products.data?.filter(p => p.regulatory_status === 'approved').length || 0,
          expiring_soon: expiringSoon.length,
          suspended: products.data?.filter(p => p.regulatory_status === 'suspended').length || 0
        },
        procedures: {
          total: procedures.data?.length || 0,
          by_risk: {
            low_risk: procedures.data?.filter(p => p.classification === 'low_risk').length || 0,
            medium_risk: procedures.data?.filter(p => p.classification === 'medium_risk').length || 0,
            high_risk: procedures.data?.filter(p => p.classification === 'high_risk').length || 0,
            surgical: procedures.data?.filter(p => p.classification === 'surgical').length || 0
          }
        },
        adverse_events: {
          total: events.data?.length || 0,
          by_severity: {
            mild: events.data?.filter(e => e.event_type === 'mild').length || 0,
            moderate: events.data?.filter(e => e.event_type === 'moderate').length || 0,
            severe: events.data?.filter(e => e.event_type === 'severe').length || 0,
            life_threatening: events.data?.filter(e => e.event_type === 'life_threatening').length || 0
          },
  ...
  private calculateComplianceScore(products: any[], procedures: any[], events: any[], pendingReports: any[]): number {
    let score = 100;
  ...
  async getProductByRegistration(registrationNumber: string): Promise<ANVISAProduct | null> {
    try {
      ...
      console.error('Error fetching product by registration:', error);
      return null;
    }
  }
}