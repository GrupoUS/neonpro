    ...
    try {
      ...
      return {
        ...
        products: {
          ...
          approved:
            products.data?.filter((p) => p.regulatory_status === 'approved')
              .length || 0,
          expiring_soon: expiringSoon.length,
          suspended:
            products.data?.filter((p) => p.regulatory_status === 'suspended')
              .length || 0,
        },
        procedures: {
          ...
          by_risk: {
            low_risk:
              procedures.data?.filter((p) => p.classification === 'low_risk')
                .length || 0,
            medium_risk:
              procedures.data?.filter((p) => p.classification === 'medium_risk')
                .length || 0,
            high_risk:
              procedures.data?.filter((p) => p.classification === 'high_risk')
                .length || 0,
            surgical:
              procedures.data?.filter((p) => p.classification === 'surgical')
                .length || 0,
          },
        },
        adverse_events: {
          ...
          by_severity: {
            mild:
              events.data?.filter((e) => e.event_type === 'mild').length || 0,
            moderate:
              events.data?.filter((e) => e.event_type === 'moderate').length ||
              0,
            severe:
              events.data?.filter((e) => e.event_type === 'severe').length || 0,
            life_threatening:
              events.data?.filter((e) => e.event_type === 'life_threatening')
                .length || 0,
          },
