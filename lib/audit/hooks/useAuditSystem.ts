...
interface UseSecurityAlertsReturn {
  ...
  updateStatus: (alertId: string, status: SecurityAlert['status']) => Promise<void>
...
interface UseAuditReportsReturn {
  ...
  exportReport: (reportId: string, format: 'json' | 'csv' | 'pdf') => Promise<void>
  refresh: () => Promise<void>
}
...
interface UseAuditLoggerReturn {
  logEvent: (event: Omit<AuditEvent, 'id' | 'timestamp' | 'checksum'>) => Promise<void>
  logUserAction: (
    action: string,
    resourceType?: string,
    resourceId?: string,
  ...
  const exportLogs = useCallback(async (format: 'json' | 'csv') => {
    try {
      ...
      if (format === 'json') {
        ...
        const headers = ['Timestamp', 'Event Type', 'Severity', 'User ID', 'Description', 'IP Address']
        const csvContent = [
          headers.join(','),
          ...events.map(event => [
...
export function useSecurityAlerts(): UseSecurityAlertsReturn {
  ...
  const fetchAlerts = useCallback(async () => {
    try {
      ...
      const formattedAlerts: SecurityAlert[] = data?.map(alert => ({
        ...alert,
  ...
  const assignAlert = useCallback(async (alertId: string, userId: string) => {
    try {
      const { error } = await supabase
        .from('security_alerts')
...
export function useAuditReports(): UseAuditReportsReturn {
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      const formattedReports: AuditReport[] = data?.map(report => ({
        ...report,
  ...
  }, [user, auditSystem, fetchReports, toast])
...
export function useAuditStatistics(): UseAuditStatisticsReturn {
  ...
  const generateStats = useCallback(async (filters: Partial<AuditQueryFilters>) => {
    try {
      setLoading(true)
      setError(null)
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao gerar estatísticas'
      setError(errorMessage)
      toast({
        title: 'Erro',
        description: errorMessage,
