...
export class CohortAnalyzer {
  ...
  async calculateCohortRetention(
    ...
    for (const cohort of cohorts) {
      for (let period = 0; period <= config.periods; period++) {
        ...
        const { data: periodMetrics } = await this.supabase
        ...
        if (periodMetrics) {
          ...
          const churnRate = cohort.userCount > 0
  ...
  private calculateRetentionTrends(periodGroups: Record<number, CohortMetrics[]>) {
    ...
    for (const [period, metrics] of Object.entries(periodGroups)) {
      ...
      const avgRevenue = metrics.reduce((sum, m) => sum + m.averageRevenuePerUser, 0) / metrics.length
  ...
  private compareCohortPerformance(metrics: CohortMetrics[]) {
    const cohortGroups = metrics.reduce((groups, metric) => {
    ...
    for (const [cohortId, cohortMetrics] of Object.entries(cohortGroups)) {
      const avgRetention = cohortMetrics.reduce((sum, m) => sum + m.retentionRate, 0) / cohortMetrics.length
      const totalRevenue = cohortMetrics.reduce((sum, m) => sum + m.revenue, 0)
      const lifetimeValue = cohortMetrics.length > 0 ? totalRevenue / cohortMetrics[0].totalUsers : 0
  ...
  private performSignificanceTests(periodGroups: Record<number, CohortMetrics[]>) {
    ...
    const periods = Object.keys(periodGroups).map(Number).sort((a, b) => a - b)
    
    for (let i = 0; i < periods.length - 1; i++) {
      const currentPeriod = periodGroups[periods[i]]
      const nextPeriod = periodGroups[periods[i + 1]]
      
      const currentAvgRetention = currentPeriod.reduce((sum, m) => sum + m.retentionRate, 0) / currentPeriod.length
      const nextAvgRetention = nextPeriod.reduce((sum, m) => sum + m.retentionRate, 0) / nextPeriod.length
