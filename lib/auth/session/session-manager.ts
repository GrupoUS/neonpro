...
export const SessionActivitySchema = z.object({
  sessionId: z.string(),
  activityType: z.enum(['page_view', 'api_call', 'user_action', 'idle', 'warning_shown']),
  timestamp: z.date(),
...
export class SessionManager {
  ...
  async createSession(
    ...
    try {
      ...
      const now = new Date()
      const expiresAt = new Date(now.getTime() + config.timeoutMinutes * 60 * 1000)
  ...
  private shouldExtendSession(
    ...
    return significantActivities.includes(activityType) && timeSinceLastActivity > fiveMinutes
  }
   ...
   */
  private async showSessionWarning(sessionId: string, minutesRemaining: number): Promise<void> {
    try {
      // Log warning event
  ...
  private async calculateSecurityScore(params: {
    ...
    try {
      ...
      if (recentSessions && params.location) {
        const hasRecentLocationMatch = recentSessions.some(session => 
          session.location?.country === params.location?.country
        )
