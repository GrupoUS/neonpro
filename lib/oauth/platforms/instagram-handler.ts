...
import { OAuthConfig, OAuthTokens, OAuthState, OAuthUserProfile } from '../types';
...
export class InstagramOAuthHandler extends BaseOAuthHandler {
   ...
   */
  async exchangeCodeForTokens(code: string, stateParam: string): Promise<OAuthTokens> {
    try {
      ...
      const shortLivedResponse = await axios.post(this.config.tokenUrl, {
        client_id: this.config.clientId,
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(`Instagram token exchange failed: ${errorData?.error_description || error.message}`);
      }
  ...
  async refreshTokens(accessToken: string): Promise<OAuthTokens> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        const errorData = error.response?.data;
        throw new Error(`Instagram token refresh failed: ${errorData?.error?.message || error.message}`);
      }
  ...
  async getUserProfile(accessToken: string): Promise<OAuthUserProfile> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        const errorData = error.response?.data;
        throw new Error(`Instagram profile fetch failed: ${errorData?.error?.message || error.message}`);
      }
  ...
  async getUserMedia(accessToken: string, limit: number = 25): Promise<any[]> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        const errorData = error.response?.data;
        throw new Error(`Instagram media fetch failed: ${errorData?.error?.message || error.message}`);
      }
      throw new Error(`Media fetch failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async getAccountInsights(accessToken: string, period: 'day' | 'week' | 'days_28' = 'day'): Promise<any> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(`Instagram insights fetch failed: ${errorData?.error?.message || error.message}`);
      }
      throw new Error(`Insights fetch failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}