...
export function useOptimizedSupabase(options: UseOptimizedSupabaseOptions): SupabaseHookResult {
  const { clinicId, operationType = 'standard', autoRetry = true, healthMonitoring = true } = options
 ...
 */
export function useHealthcareCompliantSupabase(clinicId: string, operationType: HealthcareOperationType = 'standard') {
  ...
  const executeHealthcareQuery = useCallback(async (
...
export function useCriticalHealthcareSupabase(clinicId: string) {
  ...
  const executeCriticalOperation = useCallback(async (
    queryFn: (client: SupabaseClient<Database>) => Promise<any>,
    auditInfo: { action: string; patientId?: string; professionalId: string }
  ) => {
