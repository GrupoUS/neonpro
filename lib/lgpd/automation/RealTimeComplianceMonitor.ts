...
export interface ComplianceAlert {
  ...
  category: 'consent' | 'breach' | 'retention' | 'rights' | 'documentation' | 'third_party'
  title: string
  description: string
...
export interface ComplianceDashboard {
  ...
  trends: {
    consent_trends: Array<{ date: string; granted: number; withdrawn: number; expired: number }>
    request_trends: Array<{ date: string; access: number; rectification: number; erasure: number; portability: number }>
    breach_trends: Array<{ date: string; incidents: number; resolved: number; avg_response_time: number }>
    compliance_trends: Array<{ date: string; score: number; category_scores: Record<string, number> }>
  }
...
export class RealTimeComplianceMonitor {
  ...
  async startMonitoring(intervalMinutes: number = 5): Promise<void> {
    try {
      ...
      console.log(`Real-time compliance monitoring started (${intervalMinutes}min intervals)`)
    } catch (error) {
    ...
    try {
      ...
      if (breachIncidents && breachIncidents.length > 0) {
        alerts.push({
          ...
          affected_users: breachIncidents.reduce((sum, incident) => sum + (incident.affected_users || 0), 0),
          created_at: new Date().toISOString(),
          status: 'active'
        })
      }
  ...
  private async checkRetentionCompliance(): Promise<ComplianceAlert[]> {
    ...
    try {
      ...
      const { data: retentionViolations, error } = await this.supabase
  ...
  private async checkDocumentationCompliance(): Promise<ComplianceAlert[]> {
    ...
    try {
      ...
      const { data: documentationIssues, error } = await this.supabase
  ...
  private async checkThirdPartyCompliance(): Promise<ComplianceAlert[]> {
    ...
    try {
      ...
      const { data: thirdPartyIssues, error } = await this.supabase
  ...
  private async triggerAlertNotification(alert: ComplianceAlert): Promise<void> {
