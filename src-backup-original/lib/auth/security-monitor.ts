...
export class SecurityMonitor extends EventEmitter {
  ...
  async getSecurityMetricsForPeriod(
    startDate: Date,
    endDate: Date
  ...
  private async detectDeviceAnomaly(
    ...
    if (activity.user_agent && 
        !this.isUserAgentSimilar(activity.user_agent, 
  ...
  private calculateMetricsFromEvents(events: any[]): SecurityMetrics {
    ...
    metrics.highRiskEvents = events.filter(e => 
  ...
  private async checkConcurrentSessions(session: UserSession): Promise<void> {
    ...
    if (activeSessions && activeSessions.length > 3) { // Max 3 concurrent sessions
      await this.handleSecurityEvent({
        session_id: session.id,
        user_id: session.user_id,
