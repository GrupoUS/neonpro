...
Permissions are expressed in the format `{action}:{resource}`, where action can be *read*, *write*, *delete*, or *admin*, and resource includes types like *agent_sessions*, *patient_data*, or *financial_data*. Wildcard permissions (`*:*`) grant full access and are reserved for administrative roles.
