...
test.describe('Profile Page - Basic Information', () => {
  ...
  test('should display user profile information', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="profile-photo"]').or(page.locator('.profile-photo')).or(page.locator('.avatar'))).toBeVisible();
  });

  test('should edit basic profile information', async ({ page }) => {
    // Click edit profile button
    const editButton = page.locator('[data-testid="edit-profile"]').or(page.locator('button:has-text("Editar")')).first();
    ...
    await expect(page.locator('.success').or(page.locator('text=salvo')).or(page.locator('text=atualizado'))).toBeVisible();
  ...
  test('should upload profile photo', async ({ page }) => {
    ...
    if (await uploadSection.isVisible()) {
      ...
      if (await uploadButton.isVisible()) {
        ...
        await expect(page.locator('.upload-success').or(page.locator('text=carregada'))).toBeVisible({ timeout: 10000 });
      }
    }
  });
...
test.describe('Profile Page - Professional Information', () => {
  ...
  test('should display professional credentials', async ({ page }) => {
    ...
    const professionalTab = page.locator('[data-testid="professional-tab"]').or(page.locator('button:has-text("Profissional")')).first();
  ...
  test('should manage professional licenses', async ({ page }) => {
    ...
    if (await licensesSection.isVisible()) {
      ...
      const addLicenseButton = page.locator('[data-testid="add-license"]').or(page.locator('button:has-text("Adicionar")')).first();
  ...
  test('should update medical specialty', async ({ page }) => {
    ...
    const editSpecialty = page.locator('[data-testid="edit-specialty"]').or(page.locator('button:has-text("Editar Especialidade")')).first();
    
    if (await editSpecialty.isVisible()) {
      await editSpecialty.click();
      
      // Should show specialty dropdown
      const specialtySelect = page.locator('[data-testid="specialty-select"]').or(page.locator('select[name="specialty"]'));
...
test.describe('Profile Page - Security Settings', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const securityTab = page.locator('[data-testid="security-tab"]').or(page.locator('button:has-text("Segurança")')).first();
  ...
  test('should change password', async ({ page }) => {
    ...
    const changePasswordButton = page.locator('[data-testid="change-password"]').or(page.locator('button:has-text("Alterar Senha")')).first();
  ...
  test('should validate password strength', async ({ page }) => {
    ...
    if (await changePasswordButton.isVisible()) {
      ...
      const strengthIndicator = page.locator('[data-testid="password-strength"]').or(page.locator('.password-strength'));
  ...
  test('should enable two-factor authentication', async ({ page }) => {
    ...
    const twoFactorSection = page.locator('[data-testid="two-factor"]').or(page.locator('text=autenticação de dois fatores'));
    
    if (await twoFactorSection.isVisible()) {
      // Look for enable 2FA button
      const enable2FA = page.locator('[data-testid="enable-2fa"]').or(page.locator('button:has-text("Ativar")')).first();
  ...
  test('should manage active sessions', async ({ page }) => {
    ...
    if (await sessionsSection.isVisible()) {
      ...
      const terminateButton = page.locator('[data-testid="terminate-sessions"]').or(page.locator('button:has-text("Encerrar")')).first();
...
test.describe('Profile Page - Privacy & LGPD', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const privacyTab = page.locator('[data-testid="privacy-tab"]').or(page.locator('button:has-text("Privacidade")')).first();
  ...
  test('should display LGPD data rights', async ({ page }) => {
    ...
    if (await dataRights.isVisible()) {
      ...
      await expect(page.locator('text=acessar').or(page.locator('text=corrigir')).or(page.locator('text=excluir'))).toBeVisible();
    }
  });

  test('should export personal data', async ({ page }) => {
    // Look for data export option
    const exportButton = page.locator('[data-testid="export-data"]').or(page.locator('button:has-text("Exportar")')).first();
  ...
  test('should manage data sharing preferences', async ({ page }) => {
    ...
    if (await dataSharingSection.isVisible()) {
      ...
      const sharingToggles = page.locator('input[type="checkbox"]').filter({ hasText: /compartilhar|analytics|marketing/ });
      ...
      if (await sharingToggles.count() > 0) {
        ...
        // Save preferences
        const saveButton = page.locator('[data-testid="save-privacy"]').or(page.locator('button:has-text("Salvar")')).first();
  ...
  test('should request data deletion', async ({ page }) => {
    ...
    const deleteButton = page.locator('[data-testid="delete-account"]').or(page.locator('button:has-text("Excluir Conta")')).first();
    
    if (await deleteButton.isVisible()) {
      await deleteButton.click();
      
      // Should show confirmation dialog
      await expect(page.locator('[data-testid="delete-confirmation"]').or(page.locator('.delete-confirmation'))).toBeVisible();
...
test.describe('Profile Page - Notification Preferences', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const notificationsTab = page.locator('[data-testid="notifications-tab"]').or(page.locator('button:has-text("Notificações")')).first();
  ...
  test('should manage email notifications', async ({ page }) => {
    ...
    const emailSection = page.locator('[data-testid="email-notifications"]').or(page.locator('text=notificações por email'));
    
    if (await emailSection.isVisible()) {
      await expect(emailSection).toBeVisible();
      
      // Should have toggles for different notification types
      const appointmentNotifications = page.locator('input[type="checkbox"]').filter({ hasText: /consulta|appointment/ }).first();
      if (await appointmentNotifications.isVisible()) {
        await appointmentNotifications.click();
      }
      
      // Save notification preferences
      const saveButton = page.locator('[data-testid="save-notifications"]').or(page.locator('button:has-text("Salvar")')).first();
      if (await saveButton.isVisible()) {
        await saveButton.click();
  ...
  test('should set notification frequency', async ({ page }) => {
    ...
    if (await frequencySection.isVisible()) {
      ...
      const frequencySelect = page.locator('[data-testid="frequency-select"]').or(page.locator('select[name="frequency"]'));
