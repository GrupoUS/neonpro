...
test.describe('Consent Management - LGPD Data Consent', () => {
  ...
  test('should display consent management interface', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="consent-categories"]').or(page.locator('.consent-categories'))).toBeVisible();
  ...
  test('should create data processing consent', async ({ page }) => {
    ...
    const createConsentButton = page.locator('[data-testid="create-consent"]').or(page.locator('button:has-text("Criar Consentimento")')).first();
    ...
    await expect(page.locator('[data-testid="data-processing-form"]').or(page.locator('.data-processing-form'))).toBeVisible();
    
    // Fill consent details
    const purposeField = page.locator('[data-testid="processing-purpose"]').or(page.locator('textarea[name="purpose"]'));
    ...
    const retentionPeriod = page.locator('[data-testid="retention-period"]').or(page.locator('select[name="retentionPeriod"]'));
  ...
  test('should display data subject rights', async ({ page }) => {
    ...
    if (await rightsSection.isVisible()) {
      ...
      await expect(page.locator('text=acesso').or(page.locator('text=correção')).or(page.locator('text=exclusão'))).toBeVisible();
  ...
  test('should manage consent withdrawal', async ({ page }) => {
    ...
    if (await existingConsent.isVisible()) {
      ...
      const withdrawButton = page.locator('[data-testid="withdraw-consent"]').or(page.locator('button:has-text("Revogar")')).first();
      if (await withdrawButton.isVisible()) {
        await withdrawButton.click();
        
        // Should show withdrawal confirmation
        await expect(page.locator('[data-testid="withdraw-confirmation"]').or(page.locator('.withdraw-confirmation'))).toBeVisible();
        
        // Confirm withdrawal
        const confirmWithdraw = page.locator('[data-testid="confirm-withdraw"]').or(page.locator('button:has-text("Confirmar")')).first();
  ...
  test('should validate consent expiration', async ({ page }) => {
    ...
    if (await expiredConsent.isVisible()) {
      ...
      const renewButton = page.locator('[data-testid="renew-consent"]').or(page.locator('button:has-text("Renovar")')).first();
...
test.describe('Consent Management - Medical Procedure Consent', () => {
  ...
  test('should create medical procedure consent', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="medical-procedure-form"]').or(page.locator('.medical-procedure-form'))).toBeVisible();
    
    // Fill procedure details
    const procedureName = page.locator('[data-testid="procedure-name"]').or(page.locator('input[name="procedureName"]'));
    await procedureName.fill('Preenchimento com Ácido Hialurônico');
    
    // Fill procedure description
    const procedureDescription = page.locator('[data-testid="procedure-description"]').or(page.locator('textarea[name="procedureDescription"]'));
    await procedureDescription.fill('Procedimento estético para preenchimento facial com ácido hialurônico para redução de rugas e linhas de expressão.');
    ...
    const expectedResults = page.locator('[data-testid="expected-results"]').or(page.locator('textarea[name="expectedResults"]'));
    await expectedResults.fill('Redução visível de rugas e linhas de expressão, com resultados durando de 6 a 12 meses.');
    
    // Set procedure date
    const procedureDate = page.locator('[data-testid="procedure-date"]').or(page.locator('input[name="procedureDate"]'));
  ...
  test('should include ANVISA compliance for aesthetic procedures', async ({ page }) => {
    ...
    const anvisaCompliance = page.locator('text=ANVISA').or(page.locator('text=Agência Nacional de Vigilância Sanitária'));
    ...
    const productRegistration = page.locator('[data-testid="product-registration"]').or(page.locator('text=registro ANVISA'));
    ...
    const professionalQualification = page.locator('[data-testid="professional-qualification"]').or(page.locator('text=qualificação profissional'));
  ...
  test('should manage procedure consent versions', async ({ page }) => {
    ...
    const existingConsent = page.locator('[data-testid="procedure-consent"]').or(page.locator('.procedure-consent')).first();
    ...
    if (await existingConsent.isVisible()) {
      ...
      const newVersionButton = page.locator('[data-testid="new-version"]').or(page.locator('button:has-text("Nova Versão")')).first();
...
test.describe('Consent Management - Digital Signatures', () => {
  ...
  test('should capture digital signature', async ({ page }) => {
    ...
    if (await signatureSection.isVisible()) {
      ...
      const clearButton = page.locator('[data-testid="clear-signature"]').or(page.locator('button:has-text("Limpar")')).first();
  ...
  test('should generate signed consent document', async ({ page }) => {
    ...
    if (await signedConsent.isVisible()) {
      ...
      const generatePdfButton = page.locator('[data-testid="generate-pdf"]').or(page.locator('button:has-text("Gerar PDF")')).first();
...
test.describe('Consent Management - Audit & Compliance', () => {
  ...
  test('should track consent modifications', async ({ page }) => {
    ...
    const modifiedConsent = page.locator('[data-testid="modified-consent"]').or(page.locator('.modified-consent')).first();
    
    if (await modifiedConsent.isVisible()) {
      await modifiedConsent.click();
      
      // Should show modification history
      const modificationHistory = page.locator('[data-testid="modification-history"]').or(page.locator('.modification-history'));
  ...
  test('should generate compliance reports', async ({ page }) => {
    ...
    const generateReportButton = page.locator('[data-testid="generate-report"]').or(page.locator('button:has-text("Gerar Relatório")')).first();
    ...
    if (await generateReportButton.isVisible()) {
      ...
      if (await reportOptions.isVisible()) {
        ...
        const consentTypeFilter = page.locator('[data-testid="consent-type-filter"]').or(page.locator('select[name="consentTypeFilter"]'));
  ...
  test('should validate consent retention periods', async ({ page }) => {
    ...
    const retentionPolicy = page.locator('[data-testid="retention-policy"]').or(page.locator('text=política de retenção'));
...
test.describe('Consent Management - Accessibility & Performance', () => {
  ...
  test('should work on mobile devices', async ({ page }) => {
    ...
    const createButton = page.locator('[data-testid="create-consent"]').or(page.locator('button:has-text("Criar")')).first();
