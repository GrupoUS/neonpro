        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    if (input.search) {
                        query = query.or("name.ilike.%".concat(input.search, "%,email.ilike.%").concat(input.search, "%"));
                    }
                    if (input.status !== 'all') {
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    return [4 /*yield*/, supabase
                            .from('patients')
                            .insert(__assign(__assign({}, input), { created_by: user.id, created_at: new Date().toISOString(), status: 'active' }))
                            .select()
                            .single()];
                case 1:
                    _c = _d.sent(), data = _c.data, error = _c.error;
