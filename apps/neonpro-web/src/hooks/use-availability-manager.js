...
function useAvailabilityManager() {
    ...
    var _c = (0, react_1.useState)(false), isBooking = _c[0], setIsBooking = _c[1];
    ...
    }), timeSlots = _d.timeSlots, isConnected = _d.isConnected, isLoading = _d.isLoading, error = _d.error, bookSlot = _d.bookSlot, refetch = _d.refetch;
    // Slots filtrados e processados
    var filteredSlots = (0, react_1.useMemo)(function () {
        var filtered = timeSlots;
        // Filtrar por disponibilidade
        if (filters.onlyAvailable) {
            filtered = filtered.filter(function (slot) { return slot.is_available; });
        }
        // Filtrar por data de início
        if (filters.startDate) {
            var startDateStr_1 = (0, date_fns_1.format)(filters.startDate, 'yyyy-MM-dd');
            filtered = filtered.filter(function (slot) { return slot.date >= startDateStr_1; });
        }
        // Filtrar por data de fim
        if (filters.endDate) {
            var endDateStr_1 = (0, date_fns_1.format)(filters.endDate, 'yyyy-MM-dd');
            filtered = filtered.filter(function (slot) { return slot.date <= endDateStr_1; });
        }
        return filtered;
    }, [timeSlots, filters]);
    // Slots agrupados por data
    var groupedSlots = (0, react_1.useMemo)(function () {
        var grouped = {};
        ...
        Object.keys(grouped).forEach(function (date) {
            grouped[date].sort(function (a, b) { return a.start_time.localeCompare(b.start_time); });
        });
        return grouped;
    }, [filteredSlots]);
    // Estatísticas de disponibilidade
    var availability = (0, react_1.useMemo)(function () {
        var total = timeSlots.length;
        var available = timeSlots.filter(function (slot) { return slot.is_available; }).length;
    ...
    var bookSelectedSlot = (0, react_1.useCallback)(function (patientId) { return __awaiter(_this, void 0, void 0, function () {
        var result;
    ...
    var getNextAvailableSlots = (0, react_1.useCallback)(function (limit) {
    ...
    var hasTimeConflict = (0, react_1.useCallback)(function (newSlot, excludeSlotId) {
        return timeSlots.some(function (existingSlot) {
            ...
            var existingStart = new Date("".concat(existingSlot.date, "T").concat(existingSlot.start_time));
            var existingEnd = new Date("".concat(existingSlot.date, "T").concat(existingSlot.end_time));
            // Verificar sobreposição
            return (((0, date_fns_1.isBefore)(newStart, existingEnd) && (0, date_fns_1.isAfter)(newEnd, existingStart)) ||
                ((0, date_fns_1.isBefore)(existingStart, newEnd) && (0, date_fns_1.isAfter)(existingEnd, newStart)));
        });
    }, [timeSlots]);
    return {
        // Estado
