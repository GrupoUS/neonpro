...
function useSecurityMonitor() {
  ...
  var detectDeviceAnomalies = (devices, currentDevice) => {
    ...
    if (devices.length > session_config_1.SessionConfig.security.maxDevicesPerUser) {
      anomalies.push({
        description: "Too many devices registered ("
          .concat(devices.length, "/")
          .concat(session_config_1.SessionConfig.security.maxDevicesPerUser, ")"),
        metadata: {
          deviceCount: devices.length,
          maxAllowed: session_config_1.SessionConfig.security.maxDevicesPerUser,
        },
      });
    }
    ...
    if (currentDevice && currentDevice.location) {
      ...
      if (uniqueLocations.size > 3) {
        anomalies.push({
          description: "Multiple locations detected in 24h (".concat(
