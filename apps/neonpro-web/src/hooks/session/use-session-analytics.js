...
function useSessionAnalytics(initialTimeframe) {
    var _this = this;
    var _a, _b;
    if (initialTimeframe === void 0) { initialTimeframe = '7d'; }
    var supabase = (0, auth_helpers_nextjs_1.createClientComponentClient)();
    var _c = (0, react_1.useState)(initialTimeframe), timeframe = _c[0], setTimeframe = _c[1];
    ...
    // Load analytics data
    var loadAnalytics = (0, react_1.useCallback)(function (selectedTimeframe) { return __awaiter(_this, void 0, void 0, function () {
        var user, currentTimeframe, response, data, processedMetrics_1, processedDeviceAnalytics_1, processedSecurityAnalytics_1, processedTrends_1, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    return [4 /*yield*/, fetch("/api/auth/session/analytics?timeframe=".concat(currentTimeframe, "&includeDevices=true&includeSecurity=true"), {
                            method: 'GET',
                ...
                case 4:
                    ...
                    processedDeviceAnalytics_1 = processDeviceAnalytics(data.analytics.deviceBreakdown || []);
                    processedSecurityAnalytics_1 = processSecurityAnalytics(data.analytics.securityEvents || []);
                    processedTrends_1 = processTrends(data.analytics.trends || [], currentTimeframe);
                    setState(function (prev) { return (__assign(__assign({}, prev), { metrics: processedMetrics_1, deviceAnalytics: processedDeviceAnalytics_1, securityAnalytics: processedSecurityAnalytics_1, trends: processedTrends_1, lastUpdated: new Date() })); });
                    return [3 /*break*/, 7];
                case 5:
                    error_1 = _a.sent();
                    setState(function (prev) { return (__assign(__assign({}, prev), { error: error_1 instanceof Error ? error_1.message : 'Failed to load analytics' })); });
                    return [3 /*break*/, 7];
                case 6:
    ...
    var processDeviceAnalytics = function (deviceData) {
        return deviceData.map(function (device) { return ({
    ...
    var processSecurityAnalytics = function (securityData) {
        ...
        return Object.values(eventGroups).map(function (group) { return ({
    ...
    var calculateTrend = function (events) {
        ...
        if (events.length < 2)
            return 'stable';
        var now = Date.now();
        var halfPeriod = (((_a = TIMEFRAMES.find(function (tf) { return tf.value === timeframe; })) === null || _a === void 0 ? void 0 : _a.days) || 7) * 24 * 60 * 60 * 1000 / 2;
    ...
    var processTrends = function (trendsData, currentTimeframe) {
        var _a;
        var days = ((_a = TIMEFRAMES.find(function (tf) { return tf.value === currentTimeframe; })) === null || _a === void 0 ? void 0 : _a.days) || 7;
        var trends = [];
        var _loop_1 = function (i) {
            var date = new Date();
            date.setDate(date.getDate() - i);
            var dateStr = date.toISOString().split('T')[0];
            var dayData = trendsData.find(function (trend) { return trend.date === dateStr; }) || {
                sessions: 0,
                duration: 0,
                devices: 0,
                securityEvents: 0
    ...
    var exportAnalytics = (0, react_1.useCallback)(function () {
        var args_1 = [];
    ...
    var convertToCSV = function (data) {
        ...
        data.deviceAnalytics.forEach(function (device) {
            lines.push("".concat(device.deviceId, ",").concat(device.deviceName, ",").concat(device.deviceType, ",").concat(device.sessionCount, ",").concat(device.totalDuration, ",").concat(device.averageDuration, ",").concat(device.lastUsed.toISOString(), ",").concat(device.isTrusted, ",").concat(device.securityEvents));
        });
        lines.push('');
        ...
        data.securityAnalytics.forEach(function (security) {
            lines.push("".concat(security.eventType, ",").concat(security.count, ",").concat(security.severity, ",").concat(security.lastOccurrence.toISOString(), ",").concat(security.trend));
        });
        lines.push('');
        // Add trends section
        lines.push('TRENDS');
        ...
        data.trends.forEach(function (trend) {
            lines.push("".concat(trend.date, ",").concat(trend.sessions, ",").concat(trend.duration, ",").concat(trend.devices, ",").concat(trend.securityEvents));
        });
        return lines.join('\n');
    };
    // Change timeframe and reload data
    var changeTimeframe = (0, react_1.useCallback)(function (newTimeframe) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    var refreshAnalytics = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, loadAnalytics()];
                case 1:
                    _a.sent();
                    return [2 /*return*/];
            }
    ...
    var getAnalyticsSummary = (0, react_1.useCallback)(function () {
        if (!state.metrics)
            return null;
        var summary = {
            ...
            averageSessionsPerDay: state.trends.length > 0 ?
                state.trends.reduce(function (sum, trend) { return sum + trend.sessions; }, 0) / state.trends.length : 0,
            securityTrend: state.securityAnalytics.filter(function (sa) { return sa.trend === 'increasing'; }).length > 0 ? 'increasing' : 'stable'
        };
        return summary;
    ...
    return __assign(__assign({}, state), { timeframe: timeframe, timeframes: TIMEFRAMES, 
        // Actions
        loadAnalytics: loadAnalytics, changeTimeframe: changeTimeframe, refreshAnalytics: refreshAnalytics, getSessionStatus: getSessionStatus, exportAnalytics: exportAnalytics, 
        // Computed
        summary: getAnalyticsSummary(), hasData: state.metrics !== null, isEmpty: ((_a = state.metrics) === null || _a === void 0 ? void 0 : _a.totalSessions) === 0, isHealthy: (((_b = state.metrics) === null || _b === void 0 ? void 0 : _b.healthScore) || 0) >= 80 });
}
