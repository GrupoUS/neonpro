...
function useNotifications(_a) {
        ...
        (payload) => {
          ...
          if (payload.eventType === "INSERT") {
            ...
            setNotifications((prev) =>
              __spreadArray([newNotification_1], prev.slice(0, limit - 1), true),
            );
            // Auto-mark as read if enabled
            if (autoMarkAsRead) {
              setTimeout(() => markAsRead(newNotification_1.id), 5000);
            }
            ...
            setNotifications((prev) =>
              prev.map((notif) =>
                notif.id === updatedNotification_1.id ? updatedNotification_1 : notif,
              ),
            );
  ...
  var getNotificationsByType = (0, react_1.useCallback)(
    (type) => notifications.filter((notif) => notif.type === type),
    [notifications],
  );
