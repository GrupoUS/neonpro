...
export function useSubscription(
    ...
    try {
      ...
      if (mergedConfig.enableCaching && !force && cacheKey) {
        ...
        if (result) {
          ...
          const isStale = result.performance && 
          ...
          if (!isStale || backgroundUpdate) {
            ...
            setState(prev => ({
              ...
              performance: {
                ...
                cacheHitRate: (prev.performance.cacheHitRate * prev.performance.totalFetches + 1) / 
      ...
      setState(prev => {
        ...
        const newAverageResponseTime = 
          (prev.performance.averageResponseTime * prev.performance.totalFetches + duration) / newTotalFetches
   ...
   */
  const getCacheTTL = useCallback((strategy: string): number => {
    switch (strategy) {
      case 'aggressive':
  ...
  const derivedValues = useMemo(() => {
    ...
    const isInGracePeriod = data?.gracePeriod || false
