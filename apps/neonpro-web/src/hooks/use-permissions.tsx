...
export function usePermissions(options: UsePermissionsOptions = {}): UsePermissionsReturn {
  ...
  const checkPermission = useCallback(
    async (permission: string, context?: PermissionContext): Promise<PermissionCheckResult> => {
  ...
  const requestEmergencyOverride = useCallback(
    async (permission: string, reason: string): Promise<boolean> => {
      ...
      try {
        ...
        const { error: logError } = await supabase.from("emergency_override_logs").insert({
          user_id: user.id,
