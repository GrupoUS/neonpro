...
interface UsePermissionsReturn {
  // Permission checking
  checkPermission: (permission: string, context?: PermissionContext) => Promise<PermissionCheckResult>;
  checkPermissions: (permissions: string[], context?: PermissionContext) => Promise<PermissionCheckResult[]>;
  ...
  requestEmergencyOverride: (permission: string, reason: string) => Promise<boolean>;
...
export function usePermissions(options: UsePermissionsOptions = {}): UsePermissionsReturn {
  ...
  const loadUserPermissions = useCallback(async () => {
    ...
    try {
      ...
      const context: UserRoleContext = {
        ...
        medical_specialty: roleData.medical_licenses?.specialty as MedicalSpecialty || null,
        license_expiry: roleData.medical_licenses?.expires_at ? new Date(roleData.medical_licenses.expires_at) : null,
        license_active: roleData.medical_licenses?.active || false,
        additional_specialties: roleData.medical_licenses?.additional_specialties || [],
      ...
      const permissions = await rbacEngine.current.getUserEffectivePermissions(user.id);
  ...
  const checkPermission = useCallback(async (
    ...
    try {
      const result = await rbacEngine.current.checkPermission(user.id, permission, context);
  ...
  const licenseActive = userContext?.license_active || false;
  const licenseExpiry = userContext?.license_expiry || null;
  const hasEmergencyOverride = allowEmergencyOverride && (userContext?.emergency_access || false);
...
export function useClinicalPermissions(patientId?: string) {
  ...
  const checkClinicalPermission = useCallback(async (
    permission: string
  ): Promise<PermissionCheckResult> => {
    return permissions.checkPermission(permission, { patientId });
  }, [permissions.checkPermission, patientId]);

  const canAccessPatient = useCallback((targetPatientId?: string): boolean => {
  ...
  const canPerformProcedure = useCallback((procedureType: 'general' | 'specialty' = 'general'): boolean => {
    return permissions.hasPermission(`procedure.perform.${procedureType}`);
  }, [permissions.hasPermission]);

  const canPrescribe = useCallback((type: 'standard' | 'controlled' = 'standard'): boolean => {
...
export function useAdministrativePermissions() {
  ...
  const canManageScheduling = useCallback((scope: 'own' | 'clinic' = 'own'): boolean => {
    return permissions.hasPermission(`scheduling.manage.${scope}`);
  }, [permissions.hasPermission]);

  const canProcessBilling = useCallback(): boolean => {
  ...
  const canConfigureClinic = useCallback(): boolean => {
...
export function useCompliancePermissions() {
  ...
  const canGenerateReports = useCallback((type: 'cfm' | 'lgpd'): boolean => {
    return permissions.hasPermission(`compliance.report.${type}`);
  }, [permissions.hasPermission]);

  const canValidateLicenses = useCallback(): boolean => {
    return permissions.hasPermission('license.validate.cfm');
  }, [permissions.hasPermission]);

  const canMonitorLicenses = useCallback(): boolean => {
