...
export const useLGPDAudit = (): UseLGPDAuditReturn => {
  ...
  const logPatientAccess = useCallback(async (
    patientId: string,
    clinicId: string,
    action: 'view' | 'edit' | 'create' | 'delete',
    tableName = 'patients',
...
export const useConsentManagement = (patientId?: string, clinicId?: string): UseConsentManagementReturn => {
  ...
  const checkConsent = useCallback((consentType: LGPDConsentType): boolean => {
  ...
  const grantConsent = useCallback(async (
    ...
    try {
      ...
      // Log consent action
      await compliance.logConsentAction(
        patientId,
        clinicId,
        consentType,
        'granted',
...
export const useDataSubjectRights = (patientId?: string, clinicId?: string): UseDataSubjectRightsReturn => {
  ...
  const submitRequest = useCallback(async (
    ...
    try {
      ...
      if (data?.id) {
        await compliance.processDataSubjectRequest(
          patientId,
          clinicId,
          requestType,
          { description, ...details }
  ...
  const trackRequest = useCallback((requestId: string): DataSubjectRequest | null => {
  ...
  const requestDeletion = useCallback(async (reason?: string): Promise<{ success: boolean }> => {
