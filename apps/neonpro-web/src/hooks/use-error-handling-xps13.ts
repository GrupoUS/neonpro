...
export function useErrorHandling() {
  ...
  const createErrorContext = useCallback((component: string, action: string, metadata?: Record<string, any>): ErrorContext => {
    return {
      component,
      action, 
      sessionId: sessionId(),
  ...
  const logError = useCallback(async (error: AppError, originalError?: Error) => {
    try {
