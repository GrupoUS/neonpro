...
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, session, authError, searchParams, params, startDate, endDate, days, _b, clinic, clinicError, _c, existingMetrics, metricsError, performanceData, _d, stockData, stockError, _e, movementData, movementError, totalValue, totalConsumption, averageInventory, turnoverRate, dailyConsumption, daysCoverage, productsInRange, accuracyPercentage, wasteValue, wastePercentage, days, i, currentDate, variation, formattedData, weeklyData, i, weekData, avgData, monthlyData, i, monthData, avgData, normalizedData, error_1;
            ...
            switch (_f.label) {
                ...
                case 3:
                    ...
                    if (metricsError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Erro ao buscar métricas de performance' }, { status: 500 })];
                    }
                ...
                case 5:
                    ...
                    if (stockError || movementError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Erro ao buscar dados para cálculo' }, { status: 500 })];
                    }
                    ...
                    if (params.granularity === 'weekly' && performanceData.length > 7) {
                        ...
                        for (i = 0; i < performanceData.length; i += 7) {
                            ...
                            avgData = {
                                ...
                                turnoverRate: weekData.reduce(function (sum, day) { return sum + day.turnover_rate; }, 0) / weekData.length,
                                accuracy: weekData.reduce(function (sum, day) { return sum + day.accuracy_percentage; }, 0) / weekData.length,
                                wastePercentage: weekData.reduce(function (sum, day) { return sum + day.waste_percentage; }, 0) / weekData.length,
                                totalValue: weekData.reduce(function (sum, day) { return sum + day.total_value; }, 0) / weekData.length,
                                daysCoverage: weekData.reduce(function (sum, day) { return sum + day.days_coverage; }, 0) / weekData.length
                            };
                        ...
                        for (i = 0; i < performanceData.length; i += 30) {
                            ...
                            avgData = {
                                ...
                                turnoverRate: monthData.reduce(function (sum, day) { return sum + day.turnover_rate; }, 0) / monthData.length,
                                accuracy: monthData.reduce(function (sum, day) { return sum + day.accuracy_percentage; }, 0) / monthData.length,
                                wastePercentage: monthData.reduce(function (sum, day) { return sum + day.waste_percentage; }, 0) / monthData.length,
                                totalValue: monthData.reduce(function (sum, day) { return sum + day.total_value; }, 0) / monthData.length,
                                daysCoverage: monthData.reduce(function (sum, day) { return sum + day.days_coverage; }, 0) / monthData.length
                            };
                            monthlyData.push(avgData);
                        }
                        formattedData = monthlyData;
                    }
                    normalizedData = formattedData.map(function (item) { return ({
                        date: item.date,
