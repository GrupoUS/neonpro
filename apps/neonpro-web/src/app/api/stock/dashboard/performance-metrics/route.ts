...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!existingMetrics || existingMetrics.length === 0) {
      ...
      const totalValue =
        stockData?.reduce((sum, item) => sum + item.quantity_available * item.unit_cost, 0) || 0;

      const totalConsumption =
        movementData?.reduce(
          (sum, movement) => (movement.movement_type === "out" ? sum + movement.quantity_out : sum),
          0,
        ) || 0;
      ...
      const productsInRange =
        stockData?.filter((item) => item.quantity_available >= item.min_stock_level).length || 0;

      const accuracyPercentage = stockData?.length ? (productsInRange / stockData.length) * 100 : 0;
    ...
    if (params.granularity === "weekly" && performanceData.length > 7) {
      ...
      for (let i = 0; i < performanceData.length; i += 30) {
        ...
        const avgData = {
          ...
          accuracy:
            monthData.reduce((sum, day) => sum + day.accuracy_percentage, 0) / monthData.length,
          wastePercentage:
