...
export async function GET(request: NextRequest) {
  try {
    ...
    if (metricsError) {
      ...
      return NextResponse.json({ error: 'Erro ao buscar métricas de performance' }, { status: 500 })
    }
    ...
    if (!existingMetrics || existingMetrics.length === 0) {
      ...
      if (stockError || movementError) {
        ...
        return NextResponse.json({ error: 'Erro ao buscar dados para cálculo' }, { status: 500 })
      }
    ...
    if (params.granularity === 'weekly' && performanceData.length > 7) {
      ...
      for (let i = 0; i < performanceData.length; i += 7) {
        ...
        const avgData = {
          ...
          turnoverRate: weekData.reduce((sum, day) => sum + day.turnover_rate, 0) / weekData.length,
          accuracy: weekData.reduce((sum, day) => sum + day.accuracy_percentage, 0) / weekData.length,
          wastePercentage: weekData.reduce((sum, day) => sum + day.waste_percentage, 0) / weekData.length,
          totalValue: weekData.reduce((sum, day) => sum + day.total_value, 0) / weekData.length,
          daysCoverage: weekData.reduce((sum, day) => sum + day.days_coverage, 0) / weekData.length
        }
      ...
      for (let i = 0; i < performanceData.length; i += 30) {
        ...
        const avgData = {
          ...
          turnoverRate: monthData.reduce((sum, day) => sum + day.turnover_rate, 0) / monthData.length,
          accuracy: monthData.reduce((sum, day) => sum + day.accuracy_percentage, 0) / monthData.length,
          wastePercentage: monthData.reduce((sum, day) => sum + day.waste_percentage, 0) / monthData.length,
          totalValue: monthData.reduce((sum, day) => sum + day.total_value, 0) / monthData.length,
          daysCoverage: monthData.reduce((sum, day) => sum + day.days_coverage, 0) / monthData.length
        }
