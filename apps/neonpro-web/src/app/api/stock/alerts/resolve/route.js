...
function handleError(error, defaultMessage) {
    if (defaultMessage === void 0) { defaultMessage = 'Internal server error'; }
...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var authResult, userId, clinicId, supabase, body, resolveData, _a, alert_1, alertError, resolutionMetadata, _b, updatedAlert, updateError, transformedAlert, shouldTriggerReorder, error_1;
        var _c;
            ...
            switch (_d.label) {
                ...
                case 1:
                    ...
                    if ('error' in authResult) {
                        return [2 /*return*/, server_1.NextResponse.json({ success: false, error: authResult.error }, { status: authResult.status })];
                    }
                    userId = authResult.userId, clinicId = authResult.clinicId, supabase = authResult.supabase;
                    return [4 /*yield*/, request.json()];
                case 2:
                    body = _d.sent();
                    resolveData = stock_alerts_1.resolveAlertSchema.parse(__assign(__assign({}, body), { resolvedBy: userId // Override with authenticated user ID
                     }));
                    return [4 /*yield*/, supabase
                ...
                case 3:
                    ...
                    if (alertError || !alert_1) {
                        return [2 /*return*/, server_1.NextResponse.json({ success: false, error: 'Alert not found or not accessible' }, { status: 404 })];
                    }
                    ...
                    resolutionMetadata = __assign(__assign({}, alert_1.metadata), { resolution: {
                ...
                case 4:
                    ...
                    if (updateError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ success: false, error: 'Failed to resolve alert' }, { status: 500 })];
                    }
                    transformedAlert = {
                        id: updatedAlert.id,
                    ...
                    console.log("Alert ".concat(resolveData.alertId, " resolved by user ").concat(userId, " for clinic ").concat(clinicId));
                    // Update alert configurations for recurring issues
                    return [4 /*yield*/, updateAlertConfigurationsIfRecurring(supabase, alert_1, resolveData, clinicId)];
                case 5:
                    // Update alert configurations for recurring issues
                    _d.sent();
                    // Trigger analytics update for resolution metrics
                    return [4 /*yield*/, updateResolutionAnalytics(supabase, alert_1, resolveData, userId, clinicId)];
                case 6:
                    // Trigger analytics update for resolution metrics
                    _d.sent();
                ...
                case 7:
                    // Send notifications to stakeholders
                    _d.sent();
                    shouldTriggerReorder = alert_1.alert_type === 'low_stock' &&
                        ((_c = resolveData.actionsTaken) === null || _c === void 0 ? void 0 : _c.includes('reorder_initiated'));
                ...
                case 8:
                    // Integrate with purchasing system to track reorder status
                    _d.sent();
                    console.log("Reorder initiated for product ".concat(alert_1.product_id, " as part of alert resolution"));
                    _d.label = 9;
            ...
            switch (_b.label) {
                ...
                case 2:
                    ...
                    if (configError) {
                        ...
                        console.log("Updated alert configuration for recurring issue: Product ".concat(alert.product_id));
                    }
                    _b.label = 3;
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    if (error) {
                        ...
                        console.log("Analytics updated for alert ".concat(alert.id, ": ").concat(resolutionTimeHours, "h resolution time"));
                    }
                    return [4 /*yield*/, supabase.rpc('update_alert_metrics', {
            ...
            switch (_c.label) {
                case 0:
                    ...
                    notificationData = {
                        ...
                        clinicId: clinicId,
                        alertId: alert.id,
                        productName: ((_a = alert.product) === null || _a === void 0 ? void 0 : _a.name) || "Product ID: ".concat(alert.product_id),
                    ...
                    return [4 /*yield*/, fetch('/api/notifications/email', {
                            method: 'POST',
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch('/api/notifications/push', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                title: 'Alerta de Estoque Resolvido',
                                body: "".concat((_b = alert.product) === null || _b === void 0 ? void 0 : _b.name, ": ").concat(resolveData.resolution),
                                data: notificationData,
            ...
            switch (_c.label) {
                case 0:
                    ...
                    purchaseOrder = {
                        ...
                        supplier_id: ((_b = alert.product) === null || _b === void 0 ? void 0 : _b.supplier_id) || null,
                ...
                case 3:
                    _c.sent();
                    console.log("Purchase order ".concat(order.id, " created and tracking initiated for product ").concat(alert.product_id));
                    return [3 /*break*/, 5];
                case 4:
                    error_5 = _c.sent();
