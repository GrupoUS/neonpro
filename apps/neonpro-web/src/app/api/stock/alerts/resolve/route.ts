...
export async function POST(request: NextRequest) {
  try {
    ...
    const shouldTriggerReorder =
      alert.alert_type === "low_stock" && resolveData.actionsTaken?.includes("reorder_initiated");
...
async function updateAlertConfigurationsIfRecurring(
  ...
  try {
    ...
    if (recentAlerts && recentAlerts.length > 2) {
      // More than 2 alerts in 30 days
      // Update alert configuration to be more sensitive
      const { error: configError } = await supabase.from("alert_configs").upsert(
        {
          clinic_id: clinicId,
...
async function updateResolutionAnalytics(
  ...
  try {
    ...
    const { error } = await supabase.from("stock_alert_analytics").insert({
      clinic_id: clinicId,
      alert_id: alert.id,
      product_id: alert.product_id,
      alert_type: alert.alert_type,
...
async function integratePurchasingSystem(
  ...
  try {
    ...
    await supabase.from("purchase_order_tracking").insert({
      order_id: order.id,
      status: "order_placed",
