...
export async function POST(request: NextRequest) {
  try {
    ...
    if (templatesError) {
      ...
      return NextResponse.json({ error: 'Erro ao buscar templates de materiais' }, { status: 500 })
    }
    ...
    appointments?.forEach(appointment => {
      ...
      const requiredMaterials = materialTemplates?.filter(
    ...
    Array.from(materialPredictions.values()).forEach(prediction => {
      ...
      const stockAfterConsumption = availableQuantity - prediction.totalQuantity
    ...
    const dailyBreakdown = Array.from(dailyPredictions.values())
      .reduce((acc, item) => {
        if (!acc[item.date]) {
          acc[item.date] = {
            date: item.date,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          totalEstimatedCost: Array.from(materialPredictions.values())
            .reduce((sum, pred) => sum + (pred.totalQuantity * pred.unitCost), 0),
