...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    switch (action) {
      ...
      case "record_payment": {
        const { amount, paymentMethod, reference, notes } = z
          .object({
        ...
        const total =
          totalCollected?.reduce((sum, activity) => sum + (activity.amount_collected || 0), 0) || 0;

        // Update workflow status if fully paid
        if (total >= amount) {
          // This would need to compare against the actual debt amount
          await supabase
            .from("collection_workflows")
            .update({
              status: "completed",
      ...
      case "send_notification": {
        const { notificationType, message, channel } = z
          .object({
