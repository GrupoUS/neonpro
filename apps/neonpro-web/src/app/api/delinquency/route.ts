...
const GetDelinquencyQuerySchema = z.object({
  ...
  sortBy: z.enum(['daysOverdue', 'amount', 'riskLevel', 'customerName']).optional().default('daysOverdue'),
  ...
  try {
    ...
    switch (action) {
      ...
      case 'update_collection_workflow': {
        const { workflowId, currentStage, nextActionType, nextActionDate, notes } = z.object({
