...
export async function GET(request: NextRequest) {
  try {
    ...
    if (query.daysOverdue) {
      const daysOverdue = parseInt(query.daysOverdue);
    ...
    const { data: overduePayments, error: queryError, count } = await supabaseQuery;
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case "detect_overdue": {
        const overduePayments = await delinquencyManager.detectOverduePayments();
        return NextResponse.json({
          success: true,
          data: overduePayments,
      ...
      case "process_workflows": {
        ...
        if (validatedData.dryRun) {
          ...
          return NextResponse.json({ success: true, message: "Workflows processed successfully" });
        }
      }
      ...
      case "create_payment_plan": {
        ...
        const installmentAmount = validatedData.negotiatedAmount / validatedData.installments;
      ...
      case "calculate_risk_score": {
        const { customerId } = z.object({ customerId: z.string().uuid() }).parse(data);

        const riskProfile = await delinquencyManager.calculateRiskScore(customerId);
    ...
    return NextResponse.json({ error: "Failed to process delinquency action" }, { status: 500 });
  }
}
  ...
  try {
    ...
    switch (action) {
      ...
      case "update_collection_workflow": {
        const { workflowId, currentStage, nextActionType, nextActionDate, notes } = z
          .object({
    ...
    return NextResponse.json({ error: "Failed to update delinquency data" }, { status: 500 });
  }
}
  ...
  try {
    ...
    switch (action) {
      case "payment_plan": {
        ...
        return NextResponse.json({ success: true, message: "Payment plan deleted successfully" });
      }
      ...
      case "collection_workflow": {
        ...
        return NextResponse.json({ success: true, message: "Collection workflow completed" });
      }
    ...
    return NextResponse.json({ error: "Failed to delete delinquency data" }, { status: 500 });
  }
}
