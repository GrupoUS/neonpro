...
import type { createTreatmentOutcomeSchema } from "@/app/lib/validations/treatment-success";
  ...
  try {
    ...
    const filters = {
      ...
      success_rate_min: searchParams.get("success_rate_min")
        ? parseFloat(searchParams.get("success_rate_min")!)
        : undefined,
      success_rate_max: searchParams.get("success_rate_max")
        ? parseFloat(searchParams.get("success_rate_max")!)
        : undefined,
      satisfaction_min: searchParams.get("satisfaction_min")
        ? parseFloat(searchParams.get("satisfaction_min")!)
        : undefined,
    ...
    const result = await treatmentSuccessService.getTreatmentOutcomes(filters, page, limit);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outcome = await treatmentSuccessService.createTreatmentOutcome(validatedData);
