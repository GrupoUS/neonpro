...
var DashboardQuerySchema = zod_1.z.object({
    periodStart: zod_1.z.string().refine(function (date) { return !isNaN(Date.parse(date)); }, 'Invalid start date'),
    periodEnd: zod_1.z.string().refine(function (date) { return !isNaN(Date.parse(date)); }, 'Invalid end date'),
...
function GET(request_1, _a) {
    return __awaiter(this, arguments, void 0, function (request, _b) {
        var clinicValidation, clinicId, searchParams, queryValidation, _c, periodStart_1, periodEnd_1, includeMetrics, includePredictions, includeStrategies, includePerformance, metricsLimit, predictionsLimit, supabase, _d, user, authError, _e, userProfile, profileError, _f, clinic, clinicError, retentionService, dashboardData, additionalData, metrics, filteredMetrics, predictions, filteredPredictions, strategies, startDate, endDate, daysDiff, weeklyData, weeksCount, week, weekStart, weekEnd, alerts, response, error_1;
        var _g, _h;
        ...
        return __generator(this, function (_j) {
            switch (_j.label) {
                case 0:
                    ...
                    _c = queryValidation.data, periodStart_1 = _c.periodStart, periodEnd_1 = _c.periodEnd, includeMetrics = _c.includeMetrics, includePredictions = _c.includePredictions, includeStrategies = _c.includeStrategies, includePerformance = _c.includePerformance, metricsLimit = _c.metricsLimit, predictionsLimit = _c.predictionsLimit;
                ...
                case 4:
                    ...
                    return [4 /*yield*/, retentionService.generateRetentionAnalyticsDashboard(clinicId, periodStart_1, periodEnd_1)];
                case 5:
                    dashboardData = _j.sent();
                    additionalData = {};
                    if (!includeMetrics) return [3 /*break*/, 7];
                    return [4 /*yield*/, retentionService.getClinicRetentionMetrics(clinicId, metricsLimit, 0)];
                case 6:
                    metrics = _j.sent();
                    ...
                    additionalData.detailedMetrics = {
                        ...
                        summary: {
                            ...
                            high_risk_patients: filteredMetrics.filter(function (m) { return ['high', 'critical'].includes(m.churn_risk_level); }).length,
                            average_retention_rate: filteredMetrics.reduce(function (sum, m) { return sum + m.retention_rate; }, 0) / filteredMetrics.length || 0,
                            total_lifetime_value: filteredMetrics.reduce(function (sum, m) { return sum + m.lifetime_value; }, 0),
                        }
                    };
                    _j.label = 7;
                case 7:
                    if (!includePredictions) return [3 /*break*/, 9];
                    return [4 /*yield*/, retentionService.getChurnPredictions(clinicId, undefined, predictionsLimit, 0)];
                case 8:
                    predictions = _j.sent();
                    filteredPredictions = predictions.filter(function (prediction) {
                        var predictionDate = new Date(prediction.prediction_date);
                        return predictionDate >= new Date(periodStart_1) && predictionDate <= new Date(periodEnd_1);
                    });
                    additionalData.detailedPredictions = {
                        predictions: filteredPredictions,
                        summary: {
                            total_predictions: filteredPredictions.length,
                            critical_risk: filteredPredictions.filter(function (p) { return p.risk_level === retention_analytics_1.ChurnRiskLevel.CRITICAL; }).length,
                            high_risk: filteredPredictions.filter(function (p) { return p.risk_level === retention_analytics_1.ChurnRiskLevel.HIGH; }).length,
                            medium_risk: filteredPredictions.filter(function (p) { return p.risk_level === retention_analytics_1.ChurnRiskLevel.MEDIUM; }).length,
                            low_risk: filteredPredictions.filter(function (p) { return p.risk_level === retention_analytics_1.ChurnRiskLevel.LOW; }).length,
                            average_churn_probability: filteredPredictions.reduce(function (sum, p) { return sum + p.churn_probability; }, 0) / filteredPredictions.length || 0,
                        }
                    };
                    _j.label = 9;
                case 9:
                ...
                case 10:
                    ...
                    additionalData.strategies = {
                        ...
                        active_strategies: strategies.filter(function (s) { return s.is_active; }),
                        summary: {
                            total_strategies: strategies.length,
                            active_count: strategies.filter(function (s) { return s.is_active; }).length,
                            total_executions: strategies.reduce(function (sum, s) { return sum + s.execution_count; }, 0),
                            average_success_rate: strategies.reduce(function (sum, s) { return sum + (s.success_rate || 0); }, 0) / strategies.length || 0,
                        }
                    };
                    _j.label = 11;
                case 11:
                    if (includePerformance) {
                        ...
                        for (week = 0; week < weeksCount; week++) {
                            ...
                            weekEnd = new Date(Math.min(weekStart.getTime() + (6 * 24 * 60 * 60 * 1000), endDate.getTime()));
                            weeklyData.push({
                                week: week + 1,
                    ...
                    alerts = {
                        ...
                        recent_strategy_failures: ((_h = (_g = additionalData.strategies) === null || _g === void 0 ? void 0 : _g.all_strategies) === null || _h === void 0 ? void 0 : _h.filter(function (s) {
                            return s.execution_count > 0 && (s.success_rate || 0) < 0.5;
                        }).length) || 0,
                    };
                    response = {
                        success: true,
                        data: __assign({ dashboard: dashboardData, clinic: {
                                id: clinic.id,
                                name: clinic.name,
                            }, period: {
                                start: periodStart_1,
                                end: periodEnd_1,
                                duration_days: Math.ceil((new Date(periodEnd_1).getTime() - new Date(periodStart_1).getTime()) / (1000 * 60 * 60 * 24))
                            }, alerts: alerts }, additionalData),
                        metadata: {
