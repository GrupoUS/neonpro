...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var requestId, supabase, _a, session, sessionError, _b, connection, connectionError, tokenExpiresAt, now, timeUntilExpiry, minutesUntilExpiry, oauthHandler, newEncryptedToken, updateError, error_1;
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    _c.trys.push([1, 8, , 9]);
                    logger_1.Logger.info('HubSpot OAuth token refresh request initiated', {
                        requestId: requestId,
                        provider: 'hubspot',
                ...
                case 3:
                    ...
                    if (sessionError || !(session === null || session === void 0 ? void 0 : session.user)) {
                        logger_1.Logger.warn('HubSpot OAuth token refresh attempted without valid session', {
                            requestId: requestId,
                            provider: 'hubspot',
                ...
                case 4:
                    ...
                    if (connectionError || !connection) {
                        logger_1.Logger.error('HubSpot connection not found for token refresh', {
                            requestId: requestId,
                            provider: 'hubspot',
                    ...
                    if (connection.status !== 'connected') {
                        logger_1.Logger.warn('Attempted to refresh token for disconnected HubSpot account', {
                            requestId: requestId,
                            provider: 'hubspot',
                            userId: session.user.id,
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'HubSpot account not connected' }, { status: 400 })];
                    }
                    tokenExpiresAt = new Date(connection.token_expires_at);
                    now = new Date();
                    timeUntilExpiry = tokenExpiresAt.getTime() - now.getTime();
                    minutesUntilExpiry = timeUntilExpiry / (1000 * 60);
                    if (minutesUntilExpiry > 5) {
                        logger_1.Logger.info('HubSpot token still valid, refresh not needed', {
                            requestId: requestId,
                            provider: 'hubspot',
                            userId: session.user.id,
                            connectionId: connection.id,
                ...
                case 6:
                    ...
                    if (updateError) {
                        logger_1.Logger.error('Failed to update HubSpot connection with refreshed token', {
                            requestId: requestId,
                            provider: 'hubspot',
                            userId: session.user.id,
                            connectionId: connection.id,
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to save refreshed token' }, { status: 500 })];
                    }
                    // Log token refresh
                    return [4 /*yield*/, supabase
                ...
                case 7:
                    // Log token refresh
                    _c.sent();
                    logger_1.Logger.info('HubSpot OAuth token refresh successful', {
                        requestId: requestId,
                        provider: 'hubspot',
                        userId: session.user.id,
                        connectionId: connection.id,
                ...
                case 8:
                    error_1 = _c.sent();
                    logger_1.Logger.error('HubSpot OAuth token refresh error', {
                        requestId: requestId,
                        provider: 'hubspot',
