...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    Logger.info("HubSpot connection details request initiated", {
      requestId,
      provider: "hubspot",
      connectionId,
    ...
    if (sessionError || !session?.user) {
      Logger.warn("HubSpot connection details request without valid session", {
        requestId,
        provider: "hubspot",
        connectionId,
    ...
    if (connectionError || !connection) {
      Logger.error("HubSpot connection not found", {
        requestId,
        provider: "hubspot",
        connectionId,
    ...
    const minutesUntilExpiry = Math.round((tokenExpiresAt.getTime() - now.getTime()) / (1000 * 60));
    ...
    Logger.error("HubSpot connection details error", {
      requestId,
      provider: "hubspot",
      connectionId,
...
export async function DELETE(
  ...
  try {
    Logger.info("HubSpot connection disconnection request initiated", {
      requestId,
      provider: "hubspot",
      connectionId,
      userAgent: request.headers.get("user-agent"),
    ...
    if (sessionError || !session?.user) {
      Logger.warn("HubSpot disconnection request without valid session", {
        requestId,
        provider: "hubspot",
        connectionId,
        error: sessionError?.message,
    ...
    if (connectionError || !connection) {
      Logger.error("HubSpot connection not found for disconnection", {
        requestId,
        provider: "hubspot",
        connectionId,
        userId: session.user.id,
    ...
    try {
      ...
      Logger.warn("Failed to revoke HubSpot token, continuing with disconnection", {
        requestId,
        provider: "hubspot",
        connectionId,
    ...
    if (updateError) {
      Logger.error("Failed to update HubSpot connection status", {
        requestId,
        provider: "hubspot",
        connectionId,
      ...
      return NextResponse.json({ error: "Failed to disconnect HubSpot account" }, { status: 500 });
    }
    ...
    Logger.info("HubSpot disconnection successful", {
      requestId,
      provider: "hubspot",
      connectionId,
      userId: session.user.id,
    ...
    Logger.error("HubSpot disconnection error", {
      requestId,
      provider: "hubspot",
      connectionId,
      error: error instanceof Error ? error.message : "Unknown error",
