...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!code || !state) {
      ...
      return redirect('/dashboard/social-media?error=invalid_request&message=Missing authorization code or state');
    }
    ...
    if (!userProfile?.clinic_id) {
      ...
      return redirect('/dashboard/social-media?error=clinic_not_found&message=Clinic configuration missing');
    }
    ...
    console.log(`Facebook account connected successfully for user ${session.user.id}, clinic ${userProfile.clinic_id}, account ${accountId}`);
    ...
    return redirect('/dashboard/social-media?success=true&platform=facebook&message=Facebook account connected successfully');
    ...
    return redirect(`/dashboard/social-media?error=connection_failed&message=${encodeURIComponent(errorMessage)}`);
  }
}