...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, session, accountId, _a, account, accountError, encryptedAccessToken, currentAccessToken, facebookHandler, newTokens, newEncryptedAccessToken, updateError, error_1, supabase, session, logError_1;
            ...
            switch (_b.label) {
                ...
                case 2:
                    ...
                    if (!(session === null || session === void 0 ? void 0 : session.user)) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Unauthorized - Please log in to refresh tokens' }, { status: 401 })];
                    }
                    return [4 /*yield*/, request.json()];
                case 3:
                    accountId = (_b.sent()).accountId;
                    if (!accountId) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Account ID is required' }, { status: 400 })];
                    }
                    return [4 /*yield*/, supabase
                            ...
                            .single()];
                case 4:
                    _a = _b.sent(), account = _a.data, accountError = _a.error;
                    ...
                    currentAccessToken = token_encryption_1.TokenEncryptionService.decryptToken(encryptedAccessToken);
                    facebookHandler = new facebook_handler_1.FacebookOAuthHandler();
                    return [4 /*yield*/, facebookHandler.refreshTokens(currentAccessToken)];
                case 5:
                    newTokens = _b.sent();
                    newEncryptedAccessToken = token_encryption_1.TokenEncryptionService.encryptToken(newTokens.accessToken);
                    return [4 /*yield*/, supabase
                            .from('social_media_accounts')
                            .update({
                ...
                case 6:
                    ...
                    if (updateError) {
                        throw new Error("Failed to update tokens: ".concat(updateError.message));
                    }
                    // Log successful token refresh
                    console.log("Facebook tokens refreshed successfully for account ".concat(accountId, ", user ").concat(session.user.id));
                ...
                case 8:
                    error_1 = _b.sent();
                    console.error('Facebook token refresh error:', error_1);
                    _b.label = 9;
                case 9:
                ...
                case 14:
                    logError_1 = _b.sent();
                    console.error('Failed to log token refresh error:', logError_1);
                    return [3 /*break*/, 15];
                case 15: return [2 /*return*/, server_1.NextResponse.json({
