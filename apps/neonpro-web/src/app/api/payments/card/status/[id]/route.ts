...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const hasPermission =
      userProfile?.role && ["admin", "manager", "financial"].includes(userProfile.role);
    ...
    const stripePayment = await CardPaymentService.getPaymentStatus(paymentIntentId);
    ...
    if (stripePayment.status !== cardPayment.status) {
      ...
      if (stripePayment.status === "succeeded" && cardPayment.status !== "succeeded") {
        ...
        if (cardPayment.installment_plans && cardPayment.installment_plans.length > 0) {
    ...
    const response = {
      ...
      installments:
        cardPayment.installment_plans && cardPayment.installment_plans.length > 0
          ? {
              plan: {
              ...
              payments:
                cardPayment.installment_plans[0].installment_payments?.map((payment) => ({
                  id: payment.id,
