...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var userId, _a, profile, profileError, allowedRoles, body, validatedData, totalAmount, interestRate, paymentData, paymentResult, paymentRecordError, cardPayment, _b, installmentPlan, planError, installmentPayments, installmentAmount, i, dueDate, error_1;
            ...
            switch (_c.label) {
                ...
                case 1:
                    ...
                    if (profileError || !profile) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Forbidden', message: 'User profile not found' }, { status: 403 })];
                    }
                ...
                case 2:
                    ...
                    if (!isValidDocument(validatedData.customer.document)) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Validation Error', message: 'Invalid CPF/CNPJ' }, { status: 400 })];
                    }
                    totalAmount = validatedData.amount;
                    if (validatedData.installments > 1) {
                        interestRate = 0.0299;
                        totalAmount = Math.ceil(validatedData.amount * (1 + (validatedData.installments - 1) * interestRate));
                    }
                    paymentData = {
                        amount: totalAmount,
                        ...
                        metadata: __assign(__assign({}, validatedData.metadata), { payableId: validatedData.payableId || '', patientId: validatedData.patientId || '', installments: validatedData.installments.toString(), originalAmount: validatedData.amount.toString(), createdBy: userId }),
                        payableId: validatedData.payableId,
                        patientId: validatedData.patientId,
                    };
                    return [4 /*yield*/, card_payment_service_1.CardPaymentService.createPayment(paymentData)];
                case 3:
                ...
                case 10:
                    ...
                    return [2 /*return*/, server_1.NextResponse.json({
                            ...
                            currency: validatedData.currency,
                            installments: validatedData.installments,
                            installment_amount: validatedData.installments > 1
