  ...
  try {
    ...
    return NextResponse.json({
      ...
      timeout_config: {
        ...
        auto_extend_enabled: userTimeout?.auto_extend_enabled || false,
        role_based_timeout: TIMEOUT_CONFIGS[profile.role as keyof typeof TIMEOUT_CONFIGS],
  ...
  try {
    ...
    if (action === "extend_session") {
      try {
        ...
        return NextResponse.json({ error: "Erro ao estender sessão" }, { status: 500 });
      }
    }
