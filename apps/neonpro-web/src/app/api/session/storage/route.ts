...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "retrieve_token") {
      ...
      if (!token_type) {
        return NextResponse.json({ error: "Tipo de token é obrigatório" }, { status: 400 });
      }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: tokens, error: tokensError } = await query.order("created_at", {
      ascending: false,
    });
    ...
    const active_tokens =
      tokens?.filter((t) => t.is_active && new Date(t.expires_at) > new Date()).length || 0;
  ...
  try {
    ...
    if (action === "cleanup_expired") {
      ...
      if (cleanupError) {
        ...
        return NextResponse.json({ error: "Erro ao limpar tokens expirados" }, { status: 500 });
      }
    ...
    return NextResponse.json({ error: "Ação de exclusão não reconhecida" }, { status: 400 });
  } catch (error) {
