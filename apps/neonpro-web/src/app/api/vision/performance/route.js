...
exports.GET = (0, monitoring_1.withErrorMonitoring)(function (request) { return __awaiter(void 0, void 0, void 0, function () {
    var supabase, _a, user, authError, searchParams, queryParams, validatedParams_1, now, timeRangeMap, startTime, _b, performanceData, perfError, totalAnalyses, avgAccuracy, avgProcessingTime, avgConfidence, accuracyTarget_1, timeTarget_1, accuracyCompliance, timeCompliance, timeSeriesData, groupedData_1, treatmentBreakdown_1, treatmentStats, error_1;
    return __generator(this, function (_c) {
        switch (_c.label) {
            ...
            case 4:
                ...
                avgAccuracy = totalAnalyses > 0
                    ? performanceData.reduce(function (sum, item) { return sum + item.accuracy_score; }, 0) / totalAnalyses
                    : 0;
                avgProcessingTime = totalAnalyses > 0
                    ? performanceData.reduce(function (sum, item) { return sum + item.processing_time_ms; }, 0) / totalAnalyses
                    : 0;
                avgConfidence = totalAnalyses > 0
                    ? performanceData.reduce(function (sum, item) { return sum + item.confidence_score; }, 0) / totalAnalyses
                    : 0;
                accuracyTarget_1 = 0.95;
                timeTarget_1 = 30000;
                accuracyCompliance = totalAnalyses > 0
                    ? performanceData.filter(function (item) { return item.accuracy_score >= accuracyTarget_1; }).length / totalAnalyses
                    : 0;
                timeCompliance = totalAnalyses > 0
                    ? performanceData.filter(function (item) { return item.processing_time_ms <= timeTarget_1; }).length / totalAnalyses
                    : 0;
                timeSeriesData = [];
                if (validatedParams_1.includeDetails) {
                    ...
                    performanceData.forEach(function (item) {
                        ...
                        switch (validatedParams_1.groupBy) {
                            case 'hour':
                                groupKey = "".concat(date.getFullYear(), "-").concat(String(date.getMonth() + 1).padStart(2, '0'), "-").concat(String(date.getDate()).padStart(2, '0'), " ").concat(String(date.getHours()).padStart(2, '0'), ":00");
                                break;
                            case 'day':
                                groupKey = "".concat(date.getFullYear(), "-").concat(String(date.getMonth() + 1).padStart(2, '0'), "-").concat(String(date.getDate()).padStart(2, '0'));
                                break;
                            case 'week':
                                var weekStart = new Date(date);
                                weekStart.setDate(date.getDate() - date.getDay());
                                groupKey = "".concat(weekStart.getFullYear(), "-W").concat(Math.ceil(weekStart.getDate() / 7));
                                break;
                            default:
                        ...
                        if (item.accuracy_score >= accuracyTarget_1)
                            group.accuracyCompliant++;
                        if (item.processing_time_ms <= timeTarget_1)
                            group.timeCompliant++;
                    });
                    timeSeriesData = Array.from(groupedData_1.values()).map(function (group) { return ({
                ...
                return [2 /*return*/, server_1.NextResponse.json({
                        ...
                        data: __assign({ summary: {
                            ...
                            }, treatmentBreakdown: treatmentStats }, (validatedParams_1.includeDetails && { timeSeries: timeSeriesData }))
                    })];
            case 5:
                error_1 = _c.sent();
...
exports.POST = (0, monitoring_1.withErrorMonitoring)(function (request) { return __awaiter(void 0, void 0, void 0, function () {
    var supabase, _a, user, authError, body, analysisId, processingTimeMs, accuracyScore, confidenceScore, memoryUsageMb, errorCount, _b, data, error, error_2;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0: return [4 /*yield*/, (0, server_2.createClient)()];
            case 1:
            ...
            case 4:
                body = _c.sent();
                analysisId = body.analysisId, processingTimeMs = body.processingTimeMs, accuracyScore = body.accuracyScore, confidenceScore = body.confidenceScore, memoryUsageMb = body.memoryUsageMb, errorCount = body.errorCount;
                // Validate required fields
                if (!analysisId || typeof processingTimeMs !== 'number' || typeof accuracyScore !== 'number') {
                    return [2 /*return*/, server_1.NextResponse.json({ error: 'Campos obrigatórios: analysisId, processingTimeMs, accuracyScore' }, { status: 400 })];
                }
                return [4 /*yield*/, supabase
                        .from('analysis_performance')
                        .insert({
