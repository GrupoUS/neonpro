...
async function getSystemPhotoStats(timeframe: string) {
  ...
  try {
    ...
    const typeDistribution = photosByType?.reduce((acc: any, photo) => {
    ...
    const avgConfidence = confidenceData?.length
      ? confidenceData.reduce((sum, item) => sum + (item.confidence || 0), 0) / confidenceData.length
      : 0
    ...
    const privacyStats = privacyData?.reduce((acc: any, privacy) => {
