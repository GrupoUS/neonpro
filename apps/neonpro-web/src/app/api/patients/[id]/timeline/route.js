"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GET = GET;
exports.POST = POST;
var auth_helpers_nextjs_1 = require("@supabase/auth-helpers-nextjs");
var headers_1 = require("next/headers");
var server_1 = require("next/server");
/**
 * Get Patient Medical Timeline
 * Retrieves chronological medical history for a patient
 */
function GET(request_1, _a) {
    return __awaiter(this, arguments, void 0, function (request, _b) {
        var supabase, patientId, searchParams, limit, offset, eventType, _c, session, sessionError, userRole, query, _d, timeline, timelineError, countQuery, count, error_1;
        var params = _b.params;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    _e.trys.push([0, 6, , 7]);
                    supabase = (0, auth_helpers_nextjs_1.createRouteHandlerClient)({ cookies: headers_1.cookies });
                    return [4 /*yield*/, params];
                case 1:
                    patientId = (_e.sent()).id;
                    searchParams = new URL(request.url).searchParams;
                    limit = parseInt(searchParams.get('limit') || '50');
                    offset = parseInt(searchParams.get('offset') || '0');
                    eventType = searchParams.get('event_type');
                    return [4 /*yield*/, supabase.auth.getSession()];
                case 2:
                    _c = _e.sent(), session = _c.data.session, sessionError = _c.error;
                    if (sessionError || !session) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })];
                    }
                    return [4 /*yield*/, supabase
                            .from('user_roles')
                            .select('role')
                            .eq('user_id', session.user.id)
                            .single()];
                case 3:
                    userRole = (_e.sent()).data;
                    if (!userRole || !['admin', 'doctor', 'nurse'].includes(userRole.role)) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 })];
                    }
                    query = supabase
                        .from('medical_timeline')
                        .select('*')
                        .eq('patient_id', patientId)
                        .order('event_date', { ascending: false })
                        .range(offset, offset + limit - 1);
                    // Filter by event type if specified
                    if (eventType) {
                        query = query.eq('event_type', eventType);
                    }
                    return [4 /*yield*/, query];
                case 4:
                    _d = _e.sent(), timeline = _d.data, timelineError = _d.error;
                    if (timelineError) {
                        console.error('Error fetching medical timeline:', timelineError);
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to fetch medical timeline' }, { status: 500 })];
                    }
                    countQuery = supabase
                        .from('medical_timeline')
                        .select('*', { count: 'exact', head: true })
                        .eq('patient_id', patientId);
                    if (eventType) {
                        countQuery = countQuery.eq('event_type', eventType);
                    }
                    return [4 /*yield*/, countQuery];
                case 5:
                    count = (_e.sent()).count;
                    return [2 /*return*/, server_1.NextResponse.json({
                            timeline: timeline,
                            pagination: {
                                total: count || 0,
                                limit: limit,
                                offset: offset,
                                hasMore: (offset + limit) < (count || 0)
                            },
                            message: 'Medical timeline retrieved successfully'
                        })];
                case 6:
                    error_1 = _e.sent();
                    console.error('Error in GET /api/patients/[id]/timeline:', error_1);
                    return [2 /*return*/, server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 })];
                case 7: return [2 /*return*/];
            }
        });
    });
}
/**
 * Add Timeline Event
 * Adds a new event to the patient's medical timeline
 */
function POST(request_1, _a) {
    return __awaiter(this, arguments, void 0, function (request, _b) {
        var supabase, patientId, body, _c, session, sessionError, userRole, event_type, event_date, description, event_details, outcome_score, notes, provider_id, _d, newEvent, insertError, error_2;
        var params = _b.params;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    _e.trys.push([0, 6, , 7]);
                    supabase = (0, auth_helpers_nextjs_1.createRouteHandlerClient)({ cookies: headers_1.cookies });
                    return [4 /*yield*/, params];
                case 1:
                    patientId = (_e.sent()).id;
                    return [4 /*yield*/, request.json()];
                case 2:
                    body = _e.sent();
                    return [4 /*yield*/, supabase.auth.getSession()];
                case 3:
                    _c = _e.sent(), session = _c.data.session, sessionError = _c.error;
                    if (sessionError || !session) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })];
                    }
                    return [4 /*yield*/, supabase
                            .from('user_roles')
                            .select('role')
                            .eq('user_id', session.user.id)
                            .single()];
                case 4:
                    userRole = (_e.sent()).data;
                    if (!userRole || !['admin', 'doctor', 'nurse'].includes(userRole.role)) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 })];
                    }
                    event_type = body.event_type, event_date = body.event_date, description = body.description, event_details = body.event_details, outcome_score = body.outcome_score, notes = body.notes, provider_id = body.provider_id;
                    // Validate required fields
                    if (!event_type || !event_date || !description) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Missing required fields' }, { status: 400 })];
                    }
                    return [4 /*yield*/, supabase
                            .from('medical_timeline')
                            .insert({
                            patient_id: patientId,
                            event_type: event_type,
                            event_date: event_date,
                            description: description,
                            event_details: event_details || {},
                            outcome_score: outcome_score,
                            notes: notes,
                            provider_id: provider_id,
                            created_by: session.user.id,
                            created_at: new Date().toISOString()
                        })
                            .select()
                            .single()];
                case 5:
                    _d = _e.sent(), newEvent = _d.data, insertError = _d.error;
                    if (insertError) {
                        console.error('Error adding timeline event:', insertError);
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to add timeline event' }, { status: 500 })];
                    }
                    return [2 /*return*/, server_1.NextResponse.json({
                            event: newEvent,
                            message: 'Timeline event added successfully'
                        })];
                case 6:
                    error_2 = _e.sent();
                    console.error('Error in POST /api/patients/[id]/timeline:', error_2);
                    return [2 /*return*/, server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 })];
                case 7: return [2 /*return*/];
            }
        });
    });
}
