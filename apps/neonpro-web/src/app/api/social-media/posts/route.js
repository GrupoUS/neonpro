...
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, session, profile, searchParams, accountId, status_1, postType, campaignTag, fromDate, toDate, limit, offset, query, _a, posts, error, count, error_1;
            ...
            switch (_b.label) {
                ...
                case 4:
                    ...
                    if (error) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to fetch posts' }, { status: 500 })];
                    }
...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, session, profile, body, validatedData, account, contentValidation, postStatus, scheduledDate, postData, _a, newPost, error, error_2;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 7, , 8]);
                ...
                case 5:
                    ...
                    if (!account) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid social media account' }, { status: 400 })];
                    }
                    if (account.status !== 'active' || account.sync_status !== 'active') {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Social media account is not active or synchronized' }, { status: 400 })];
                    }
                    contentValidation = validatePostContent(account.platform_name, validatedData.post_type, validatedData.content_text || '', validatedData.media_urls);
                    if (!contentValidation.valid) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Post content validation failed', details: contentValidation.errors }, { status: 400 })];
                    }
                    postStatus = 'draft';
                    if (validatedData.scheduled_time) {
                        ...
                        if (scheduledDate > new Date()) {
                            ...
                            return [2 /*return*/, server_1.NextResponse.json({ error: 'Scheduled time must be in the future' }, { status: 400 })];
                        }
                    }
                    postData = __assign(__assign({}, validatedData), { clinic_id: profile.clinic_id, status: postStatus, created_by: session.user.id });
                    return [4 /*yield*/, supabase
                            .from('social_media_posts')
                            .insert([postData])
                            .select("\n        id,\n        account_id,\n        post_type,\n        content_text,\n        media_urls,\n        hashtags,\n        mentions,\n        scheduled_time,\n        status,\n        campaign_tag,\n        created_at,\n        social_media_accounts!inner(\n          platform_name,\n          account_name,\n          social_media_platforms!inner(\n            platform_display_name\n          )\n        )\n      ")
                ...
                case 6:
                    ...
                    if (error) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to create post' }, { status: 500 })];
                    }
