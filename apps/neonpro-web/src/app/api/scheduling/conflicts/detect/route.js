...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var auditLogger, supabase, _a, session, sessionError, body, startDate, endDate, conflictService, result, error_1;
            ...
            switch (_b.label) {
                ...
                case 3:
                    ...
                    if (!body.appointmentStart || !body.appointmentEnd || !body.professionalId || !body.treatmentType) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Missing required fields: appointmentStart, appointmentEnd, professionalId, treatmentType' }, { status: 400 })];
                    }
                    startDate = new Date(body.appointmentStart);
                    endDate = new Date(body.appointmentEnd);
                    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid date format. Use ISO 8601 format.' }, { status: 400 })];
                    }
                    if (startDate >= endDate) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Start time must be before end time' }, { status: 400 })];
                    }
                    conflictService = new conflict_resolution_1.ConflictDetectionService();
                    return [4 /*yield*/, conflictService.detectConflicts(startDate, endDate, body.professionalId, body.treatmentType, body.roomId, body.equipmentIds)];
                case 4:
                    result = _b.sent();
                    // Log the conflict detection
                    return [4 /*yield*/, auditLogger.logActivity('conflict_detection_api', "Conflict detection performed for ".concat(body.treatmentType, " appointment"), {
