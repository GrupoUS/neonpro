...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var startTime, supabase, _a, user, authError, tenantId, body, validatedData, requests, totalRequests, lgpdConsent, appointmentIds, patientIds, professionalIds, conflictsQuery, conditions, _b, conflicts_1, conflictsError, results, processingTime, apiCallsReduced, batchEfficiency, error_1, processingTime;
            ...
            switch (_c.label) {
                ...
                case 1:
                    ...
                    supabase = (0, ssr_1.createServerClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, {
                        cookies: {
                            get: function (name) { var _a; return (_a = request.cookies.get(name)) === null || _a === void 0 ? void 0 : _a.value; },
                        },
                    });
                ...
                case 3:
                    ...
                    if (!lgpdConsent) {
                        return [2 /*return*/, server_1.NextResponse.json({
                                success: false,
                                results: [],
                                performance: { totalRequests: totalRequests, processingTime: 0, apiCallsReduced: 0, batchEfficiency: '0%' },
                                error: 'LGPD consent required for batch operations'
                            }, { status: 400 })];
                    }
                    appointmentIds = requests
                        .map(function (req) { return req.appointmentId; })
                        .filter(Boolean);
                    patientIds = requests
                        .map(function (req) { return req.patientId; })
                        .filter(Boolean);
                    professionalIds = requests
                        .map(function (req) { return req.professionalId; })
                        .filter(Boolean);
                    ...
                    if (professionalIds.length > 0) {
                        conditions.push("appointments.professional_id.in.(".concat(professionalIds.join(','), ")"));
                    }
                ...
                case 4:
                    ...
                    if (conflictsError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({
                                success: false,
                                results: [],
                                performance: { totalRequests: totalRequests, processingTime: Date.now() - startTime, apiCallsReduced: 0, batchEfficiency: '0%' },
                    ...
                    results = requests.map(function (request) {
                        var matchingConflicts = conflicts_1.filter(function (conflict) {
                            ...
                            if (request.patientId && conflict.appointments.patient_id === request.patientId)
                                return true;
                            if (request.professionalId && conflict.appointments.professional_id === request.professionalId)
                                return true;
                            return false;
                        });
                        return {
                            requestId: request.id,
                            conflicts: matchingConflicts.map(function (conflict) { return ({
                                id: conflict.id,
                                type: conflict.type,
                    ...
                    batchEfficiency = totalRequests > 1
                    ...
                    return [4 /*yield*/, supabase.from('audit_logs').insert({
                            ...
                            metadata: {
                                ...
                                conflictsFound: results.reduce(function (sum, r) { return sum + r.conflicts.length; }, 0),
                                lgpdCompliant: true
                            },
                ...
                case 6:
                    ...
                    if (error_1 instanceof zod_1.z.ZodError) {
                        return [2 /*return*/, server_1.NextResponse.json({
                                success: false,
                                results: [],
                                performance: { totalRequests: 0, processingTime: processingTime, apiCallsReduced: 0, batchEfficiency: '0%' },
                                error: "Validation failed: ".concat(error_1.errors.map(function (e) { return e.message; }).join(', '))
                            }, { status: 400 })];
                    }
                    return [2 /*return*/, server_1.NextResponse.json({
                            success: false,
