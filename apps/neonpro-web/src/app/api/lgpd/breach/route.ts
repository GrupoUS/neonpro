...
const breachCreateSchema = z.object({
  ...
  breachType: z.enum(['unauthorized_access', 'data_loss', 'system_compromise', 'human_error', 'malicious_attack', 'other']),
...
const breachQuerySchema = z.object({
  status: z.enum(['reported', 'investigating', 'contained', 'resolved']).optional(),
  severity: z.enum(['low', 'medium', 'high', 'critical']).optional(),
  breachType: z.enum(['unauthorized_access', 'data_loss', 'system_compromise', 'human_error', 'malicious_attack', 'other']).optional(),
  startDate: z.string().datetime().optional(),
  endDate: z.string().datetime().optional(),
  search: z.string().optional(),
  page: z.coerce.number().min(1).default(1),
  limit: z.coerce.number().min(1).max(100).default(20),
  sortBy: z.enum(['discovered_at', 'severity', 'status', 'estimated_affected_users']).default('discovered_at'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (breachError) {
      throw new Error(`Failed to fetch breach incidents: ${breachError.message}`)
    }
    ...
    const severityStats = stats?.reduce((acc: Record<string, number>, breach) => {
      acc[breach.severity] = (acc[breach.severity] || 0) + 1
      return acc
    }, {}) || {}

    const statusStats = stats?.reduce((acc: Record<string, number>, breach) => {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateError) {
      throw new Error(`Failed to update breach incident: ${updateError.message}`)
    }
