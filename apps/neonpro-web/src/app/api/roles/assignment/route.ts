...
export async function POST(request: NextRequest) {
  try {
    ...
    if (targetProfile.role === new_role) {
      return NextResponse.json({ error: "O usuário já possui esta role" }, { status: 400 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: "Erro ao atribuir nova role" }, { status: 500 });
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats =
      roleStats?.reduce((acc: Record<string, number>, profile) => {
