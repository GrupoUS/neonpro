...
export async function GET(request: NextRequest) {
  try {
    ...
    if (mappingsError) {
      ...
      return NextResponse.json({ error: "Erro ao buscar mapeamentos de domínio" }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (checkError && checkError.code !== "PGRST116") {
      ...
      return NextResponse.json({ error: "Erro ao verificar domínio" }, { status: 500 });
    }
    ...
    if (createError) {
      ...
      return NextResponse.json({ error: "Erro ao criar mapeamento de domínio" }, { status: 500 });
    }
