...
var ROLE_HIERARCHY = {
    admin: {
        ...
        permissions: [
            ...
            'view_analytics', 'manage_billing', 'manage_subscriptions'
        ],
    ...
    doctor: {
        level: 4,
        permissions: [
            'read_patients', 'write_patients', 'read_appointments', 'write_appointments',
            'read_medical_records', 'write_medical_records', 'manage_prescriptions',
            'view_reports', 'manage_treatments'
        ],
    ...
    nurse: {
        ...
        permissions: [
            'read_patients', 'write_patients', 'read_appointments', 'write_appointments',
            'read_medical_records', 'assist_treatments', 'manage_schedules'
        ],
        can_manage: ['staff']
    },
    staff: {
        level: 2,
        permissions: [
            'read_patients', 'read_appointments', 'write_appointments',
            'manage_schedules', 'basic_reports'
        ],
        can_manage: []
    },
    professional: {
        level: 1,
        permissions: [
            'read_own_profile', 'write_own_profile', 'read_own_appointments',
            'view_own_schedule'
        ],
        can_manage: []
    }
...
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, user, authError, searchParams, role, action, targetRole, permission, hasPermission, canManageRole, roleInfo, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    ...
                    if (authError || !user) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Não autorizado' }, { status: 401 })];
                    }
                    ...
                    if (action === 'check') {
                        ...
                        if (!role || !permission) {
                            return [2 /*return*/, server_2.NextResponse.json({ error: 'Role e permissão são obrigatórias para verificação' }, { status: 400 })];
                        }
                    ...
                    if (role) {
                        ...
                        if (!roleInfo) {
                            return [2 /*return*/, server_2.NextResponse.json({ error: 'Role não encontrada' }, { status: 404 })];
                        }
                        return [2 /*return*/, server_2.NextResponse.json({
                                success: true,
                                role: role,
                ...
                case 3:
                    ...
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 })];
...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, user, authError, _b, profile, profileError, body, action, target_user_id, permissions, reason, user_role, required_permission, target_resource, hasAccess, logError, error_2;
            ...
            switch (_c.label) {
                ...
                case 3:
                    ...
                    if (profileError || (profile === null || profile === void 0 ? void 0 : profile.role) !== 'admin') {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Acesso negado. Apenas administradores podem gerenciar permissões.' }, { status: 403 })];
                    }
                    return [4 /*yield*/, request.json()];
                case 4:
                    body = _c.sent();
                    action = body.action, target_user_id = body.target_user_id, permissions = body.permissions, reason = body.reason;
                    if (!(action === 'validate_access')) return [3 /*break*/, 6];
                    user_role = body.user_role, required_permission = body.required_permission, target_resource = body.target_resource;
                    if (!user_role || !required_permission) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Role do usuário e permissão requerida são obrigatórias' }, { status: 400 })];
                    }
                ...
                case 6: return [2 /*return*/, server_2.NextResponse.json({ error: 'Ação não reconhecida' }, { status: 400 })];
                case 7:
                    error_2 = _c.sent();
