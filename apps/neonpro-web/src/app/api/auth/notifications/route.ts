...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get notifications
    const notifications = await sessionSystem.notificationService.getUserNotifications(user.id, {
      limit,
      offset,
      filters,
  ...
  try {
    ...
    switch (action) {
      case "send": {
        ...
        const { type, priority, title, message, channels, metadata, scheduledFor } =
          validation.data;
      ...
      case "mark_read": {
        ...
        const results = await Promise.all(
          notificationIds.map((id) => sessionSystem.notificationService.markAsRead(id, user.id)),
        );
