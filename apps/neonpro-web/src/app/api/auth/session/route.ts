  ...
  try {
    ...
    switch (action) {
      case "extend": {
        ...
        // Extend session
        const success = await sessionSystem.sessionManager.extendSession(sessionId, extendBy);
      ...
      case "validate": {
        ...
        // Validate session
        const isValid = await sessionSystem.sessionManager.validateSession(sessionId);
        const session = isValid ? await sessionSystem.sessionManager.getSession(sessionId) : null;
      ...
      case "activity": {
        ...
        // Record activity
        await sessionSystem.sessionManager.recordActivity(sessionId, activityType, {
          ...metadata,
          ipAddress: clientIP,
          userAgent,
        });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Update session
    const success = await sessionSystem.sessionManager.updateSession(sessionId, updates);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Terminate session
    const success = await sessionSystem.sessionManager.terminateSession(sessionId);
