  ...
  try {
    ...
    switch (action) {
      ...
      case "resolve": {
        ...
        // Resolve event
        const success = await sessionSystem.securityEventLogger.resolveEvent(eventId, resolution, {
          ...metadata,
          resolvedBy: user.id,
        ...
        if (!success) {
          return NextResponse.json({ error: "Failed to resolve event" }, { status: 400 });
        }
