...
async function getNFEStatus(supabase: any, clinicId: string) {
  ...
  if (error) {
    return NextResponse.json({ error: "Failed to fetch NFE status" }, { status: 500 });
  }
...
async function calculateTaxes(supabase: any, body: any) {
  ...
  if (error) {
    return NextResponse.json({ error: "Failed to store tax calculation" }, { status: 500 });
  }
...
async function generateNFE(supabase: any, body: any) {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: "Failed to store NFE document" }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: "NFE generation failed" }, { status: 500 });
  }
}
...
async function validateCNPJ(supabase: any, body: any) {
  ...
  try {
    ...
    return NextResponse.json({ error: "CNPJ validation failed" }, { status: 500 });
  }
}
