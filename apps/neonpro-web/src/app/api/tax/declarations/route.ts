...
async function listDeclarations(supabase: any, clinicId: string, searchParams: URLSearchParams) {
  let query = supabase.from("tax_declarations").select("*").eq("clinic_id", clinicId);
...
async function getDeclarationStatus(supabase: any, searchParams: URLSearchParams) {
  ...
  if (!declarationId) {
    return NextResponse.json({ error: "declaration_id parameter is required" }, { status: 400 });
  }
...
async function downloadDeclaration(supabase: any, searchParams: URLSearchParams) {
  ...
  try {
    ...
    const fileData = await declarationService.exportDeclaration(declarationId, format);
    ...
    return NextResponse.json({ error: "Failed to download declaration" }, { status: 500 });
  }
}
...
async function getDeclarationCalendar(supabase: any, clinicId: string) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate declaration calendar" }, { status: 500 });
  }
}
...
async function getComplianceStatus(supabase: any, clinicId: string) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to assess compliance" }, { status: 500 });
  }
}
...
async function generateDeclaration(supabase: any, body: any) {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: "Failed to store declaration" }, { status: 500 });
    }
...
async function validateDeclaration(supabase: any, body: any) {
  ...
  try {
    ...
    const validation = await declarationService.validateDeclaration(validatedData.declaration_id, {
...
async function submitDeclaration(supabase: any, body: any) {
  ...
  if (!declaration_id) {
    return NextResponse.json({ error: "declaration_id is required" }, { status: 400 });
  }
  ...
  if (declaration.status === "submitted" && !force_submit) {
    return NextResponse.json({ error: "Declaration already submitted" }, { status: 400 });
  }
...
async function scheduleDeclaration(supabase: any, body: any) {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: "Failed to schedule declaration" }, { status: 500 });
    }
