...
var cancelRequestSchema = zod_1.z.object({
    reason: zod_1.z.string().min(15, 'Reason must be at least 15 characters').max(255, 'Reason must be at most 255 characters')
});
function POST(request_1, _a) {
    return __awaiter(this, arguments, void 0, function (request, _b) {
        var supabase, _c, session, authError, id, body, reason, _d, nfeDocument, fetchError, _e, clinic, clinicError, authDate, now, hoursDiff, cancelResult, _f, updatedNfe, updateError, error_1;
            ...
            switch (_g.label) {
                ...
                case 4:
                    ...
                    if (nfeDocument.status !== 'authorized') {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Only authorized NFe documents can be cancelled' }, { status: 400 })];
                    }
                    authDate = new Date(nfeDocument.authorization_date);
                    now = new Date();
                    hoursDiff = (now.getTime() - authDate.getTime()) / (1000 * 60 * 60);
                    if (hoursDiff > 24) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'NFe can only be cancelled within 24 hours of authorization' }, { status: 400 })];
                    }
