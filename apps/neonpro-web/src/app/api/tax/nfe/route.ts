...
async function listNFEDocuments(supabase: any, clinicId: string, searchParams: URLSearchParams) {
  let query = supabase.from("nfe_documents").select("*").eq("clinic_id", clinicId);
...
async function downloadNFE(supabase: any, searchParams: URLSearchParams) {
  ...
  if (!nfeId) {
    return NextResponse.json({ error: "nfe_id parameter is required" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to download NFE" }, { status: 500 });
  }
}
...
async function validateNFE(supabase: any, searchParams: URLSearchParams) {
  ...
  try {
    ...
    return NextResponse.json({ error: "NFE validation failed" }, { status: 500 });
  }
}
...
async function getNFEOverview(supabase: any, clinicId: string) {
  ...
  if (statsError) {
    return NextResponse.json({ error: "Failed to fetch NFE statistics" }, { status: 500 });
  }
...
async function cancelNFE(supabase: any, body: any) {
  ...
  if (nfe.status !== "emitted") {
    return NextResponse.json({ error: "Only emitted NFEs can be cancelled" }, { status: 400 });
  }
  ...
  try {
    const cancellation = await nfeService.cancelNFE(nfe.chave_nfe, validatedData.justification);
...
async function batchEmitNFE(supabase: any, body: any) {
  ...
  if (!nfe_ids || !Array.isArray(nfe_ids)) {
    return NextResponse.json({ error: "nfe_ids array is required" }, { status: 400 });
  }
