...
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, session, authError, searchParams, clinicId, startDate, endDate, _b, clinic, clinicError, dateFilters, taxQuery_1, _c, taxCalculations, taxError, nfeQuery_1, _d, nfeDocuments, nfeError, stats_1, error_1;
        return __generator(this, function (_e) {
            switch (_e.label) {
                ...
                case 2:
                    ...
                    if (!clinicId) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'clinic_id is required' }, { status: 400 })];
                    }
                ...
                case 3:
                    ...
                    dateFilters.forEach(function (_a) {
                        var column = _a[0], operator = _a[1], value = _a[2];
                ...
                case 4:
                    ...
                    if (taxError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to fetch tax calculations' }, { status: 500 })];
                    }
                    nfeQuery_1 = supabase
                        .from('nfe_documents')
                        .select('*')
                ...
                case 5:
                    ...
                    stats_1 = {
                        tax_calculations: {
                            ...
                            total_base_amount: taxCalculations.reduce(function (sum, calc) { return sum + (calc.base_amount || 0); }, 0),
                            total_tax_amount: taxCalculations.reduce(function (sum, calc) {
                                var taxes = calc.taxes || {};
                                return sum + Object.values(taxes).reduce(function (taxSum, tax) { return taxSum + (tax.amount || 0); }, 0);
                            }, 0),
                            total_final_amount: taxCalculations.reduce(function (sum, calc) { return sum + (calc.total_amount || 0); }, 0),
                            by_service_type: {}
                        },
                        nfe_documents: {
                            ...
                            authorized_count: nfeDocuments.filter(function (nfe) { return nfe.status === 'authorized'; }).length,
                            cancelled_count: nfeDocuments.filter(function (nfe) { return nfe.status === 'cancelled'; }).length
                        },
                        compliance: {
                            authorization_rate: nfeDocuments.length > 0
                                ? (nfeDocuments.filter(function (nfe) { return nfe.status === 'authorized'; }).length / nfeDocuments.length) * 100
                                : 0,
                            average_processing_time: 0, // TODO: Calculate based on actual processing times
                            pending_authorizations: nfeDocuments.filter(function (nfe) { return nfe.status === 'draft'; }).length
                        }
                    };
                    ...
                    taxCalculations.forEach(function (calc) {
                        ...
                        var taxes = calc.taxes || {};
                        serviceStats.total_tax += Object.values(taxes).reduce(function (sum, tax) { return sum + (tax.amount || 0); }, 0);
                    });
