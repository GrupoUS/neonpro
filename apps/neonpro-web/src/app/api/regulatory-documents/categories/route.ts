...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      query = query.or(`name.ilike.%${search}%,authority_name.ilike.%${search}%,description.ilike.%${search}%`)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uniqueAuthorities = authorities?.reduce((acc, item) => {
      const existing = acc.find(a => a.authority_name === item.authority_name)
      if (!existing) {
        acc.push({
