...
const ListAlertsSchema = z.object({
  ...
  alert_type: z.enum(['90_days_before', '30_days_before', '7_days_before', 'expired', 'training_due', 'document_review']).optional(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { page = 1, limit = 10, alert_type, status, priority } = ListAlertsSchema.parse(queryParams)
    ...
    if (status) {
      const statusFilter = status === 'pending' ? 'is.null' : 
                          status === 'sent' ? 'not.is.null' : 
                          'not.is.null'
