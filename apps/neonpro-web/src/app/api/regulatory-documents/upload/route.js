...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, user, authError, profile, formData, file, documentId, category, fileExtension_1, timestamp, randomId, fileExtension, fileName, filePath, arrayBuffer, uint8Array, _b, uploadData, uploadError, publicUrlData, updateError, error_1;
        var _c;
                ...
                case 4:
                    ...
                    if (file.size > MAX_FILE_SIZE) {
                        return [2 /*return*/, server_1.NextResponse.json({
                                error: "File size too large. Maximum size is ".concat(MAX_FILE_SIZE / 1024 / 1024, "MB")
                            }, { status: 400 })];
                    }
                    // Validate file type
                    if (!ALLOWED_FILE_TYPES.includes(file.type)) {
                        fileExtension_1 = '.' + ((_c = file.name.split('.').pop()) === null || _c === void 0 ? void 0 : _c.toLowerCase());
                        if (!ALLOWED_FILE_EXTENSIONS.includes(fileExtension_1)) {
                            return [2 /*return*/, server_1.NextResponse.json({
                                    error: "File type not allowed. Supported types: ".concat(ALLOWED_FILE_EXTENSIONS.join(', '))
                                }, { status: 400 })];
                        }
                    }
                    timestamp = Date.now();
                    ...
                    filePath = "regulatory-documents/".concat(profile.clinic_id, "/").concat(category, "/").concat(fileName);
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 3:
                    ...
                    if (!filePath) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'File path is required' }, { status: 400 })];
                    }
                    return [4 /*yield*/, supabase.storage
                            .from('documents')
                            .remove([filePath])];
                case 4:
                    ...
                    if (deleteError) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Failed to delete file' }, { status: 500 })];
                    }
