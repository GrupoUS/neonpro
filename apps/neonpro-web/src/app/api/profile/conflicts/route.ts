...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (resolution) {
      ...
      case "merge_smart":
        resolvedProfile = {
          ...
          avatar_url: !localProfile.avatar_url ? googleProfile.picture : localProfile.avatar_url,
          updated_at: new Date().toISOString(),
        };
        break;
      ...
      default:
        return NextResponse.json({ error: "Estratégia de resolução inválida" }, { status: 400 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: "Erro ao resolver conflito de perfil" }, { status: 500 });
    }

    // Registrar log de auditoria
    const { error: logError } = await supabase.from("profile_audit_log").insert({
      profile_id: user.id,
