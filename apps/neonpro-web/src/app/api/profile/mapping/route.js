    ...
    return __generator(this, (_f) => {
      switch (_f.label) {
        ...
        case 3:
          ...
          emailDomain_1 =
            (_e = email.split("@")[1]) === null || _e === void 0 ? void 0 : _e.toLowerCase();
          if (!emailDomain_1) {
            return [
              2 /*return*/,
              server_2.NextResponse.json(
                { error: "Invalid email", message: "Could not extract domain from email" },
                { status: 400 },
              ),
            ];
          }
          mapping = EMAIL_DOMAIN_MAPPINGS.find((m) => m.domain === emailDomain_1);
          // Fallback to default mapping if no specific domain found
          if (!mapping) {
            mapping = EMAIL_DOMAIN_MAPPINGS.find((m) => m.domain === "*");
          }
          return [4 /*yield*/, supabase.from("profiles").select("*").eq("email", email).single()];
        case 4:
        ...
        case 6:
          return [
            ...
            supabase
              .from("profiles")
              .insert(
                __assign(__assign({ id: user.id, email: email }, profileUpdate), {
        ...
        case 9:
          ...
          return [
            ...
            server_2.NextResponse.json({
              success: true,
              message: "Profile mapped to "
                .concat(mapping.default_role, " role")
                .concat(mapping.auto_approve ? " and auto-approved" : ", pending approval"),
              mapping: {
                email: email,
                role: mapping.default_role,
    ...
    return __generator(this, (_b) => {
      switch (_b.label) {
        ...
        case 1:
          ...
          emailDomain_2 =
            (_a = email.split("@")[1]) === null || _a === void 0 ? void 0 : _a.toLowerCase();
          mapping = EMAIL_DOMAIN_MAPPINGS.find((m) => m.domain === emailDomain_2);
          if (!mapping) {
            mapping = EMAIL_DOMAIN_MAPPINGS.find((m) => m.domain === "*");
          }
          return [
        ...
        case 3:
          ...
          return [
            2 /*return*/,
            server_2.NextResponse.json(
              { error: "Internal server error", message: "Could not fetch mapping information" },
              { status: 500 },
            ),
          ];
        case 4:
