"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GET = GET;
exports.POST = POST;
// GET/POST /api/treatment-prediction/predictions - Get/Create predictions
var treatment_prediction_1 = require("@/app/lib/services/treatment-prediction");
var server_1 = require("@/lib/supabase/server");
var server_2 = require("next/server");
// GET /api/treatment-prediction/predictions - Get predictions with filters
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var searchParams, supabase, session, filters, page, limit, offset, predictionService, predictions, paginatedPredictions, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 4, , 5]);
                    searchParams = new URL(request.url).searchParams;
                    return [4 /*yield*/, (0, server_1.createServerClient)()];
                case 1:
                    supabase = _a.sent();
                    return [4 /*yield*/, supabase.auth.getSession()];
                case 2:
                    session = (_a.sent()).data.session;
                    if (!session) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })];
                    }
                    filters = {};
                    if (searchParams.get('patient_id')) {
                        filters.patient_id = searchParams.get('patient_id');
                    }
                    if (searchParams.get('treatment_type')) {
                        filters.treatment_type = searchParams.get('treatment_type');
                    }
                    if (searchParams.get('prediction_score_min')) {
                        filters.prediction_score_min = parseFloat(searchParams.get('prediction_score_min'));
                    }
                    if (searchParams.get('prediction_score_max')) {
                        filters.prediction_score_max = parseFloat(searchParams.get('prediction_score_max'));
                    }
                    if (searchParams.get('risk_assessment')) {
                        filters.risk_assessment = searchParams.get('risk_assessment');
                    }
                    if (searchParams.get('date_from')) {
                        filters.date_from = searchParams.get('date_from');
                    }
                    if (searchParams.get('date_to')) {
                        filters.date_to = searchParams.get('date_to');
                    }
                    if (searchParams.get('model_id')) {
                        filters.model_id = searchParams.get('model_id');
                    }
                    if (searchParams.get('outcome')) {
                        filters.outcome = searchParams.get('outcome');
                    }
                    if (searchParams.get('accuracy_validated')) {
                        filters.accuracy_validated = searchParams.get('accuracy_validated') === 'true';
                    }
                    page = parseInt(searchParams.get('page') || '1');
                    limit = parseInt(searchParams.get('limit') || '20');
                    offset = (page - 1) * limit;
                    predictionService = new treatment_prediction_1.TreatmentPredictionService();
                    return [4 /*yield*/, predictionService.getPredictions(filters)];
                case 3:
                    predictions = _a.sent();
                    paginatedPredictions = predictions.slice(offset, offset + limit);
                    return [2 /*return*/, server_2.NextResponse.json({
                            predictions: paginatedPredictions,
                            pagination: {
                                page: page,
                                limit: limit,
                                total: predictions.length,
                                total_pages: Math.ceil(predictions.length / limit)
                            }
                        })];
                case 4:
                    error_1 = _a.sent();
                    console.error('Error fetching predictions:', error_1);
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Failed to fetch predictions' }, { status: 500 })];
                case 5: return [2 /*return*/];
            }
        });
    });
}
// POST /api/treatment-prediction/predictions - Create new prediction
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, session, body, patient, predictionService, predictionResponse, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 6, , 7]);
                    return [4 /*yield*/, (0, server_1.createServerClient)()];
                case 1:
                    supabase = _a.sent();
                    return [4 /*yield*/, supabase.auth.getSession()];
                case 2:
                    session = (_a.sent()).data.session;
                    if (!session) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })];
                    }
                    return [4 /*yield*/, request.json()];
                case 3:
                    body = _a.sent();
                    // Validate required fields
                    if (!body.patient_id || !body.treatment_type) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Missing required fields: patient_id, treatment_type' }, { status: 400 })];
                    }
                    return [4 /*yield*/, supabase
                            .from('patients')
                            .select('id')
                            .eq('id', body.patient_id)
                            .single()];
                case 4:
                    patient = (_a.sent()).data;
                    if (!patient) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Patient not found' }, { status: 404 })];
                    }
                    predictionService = new treatment_prediction_1.TreatmentPredictionService();
                    return [4 /*yield*/, predictionService.generatePrediction(body)];
                case 5:
                    predictionResponse = _a.sent();
                    return [2 /*return*/, server_2.NextResponse.json(__assign(__assign({}, predictionResponse), { message: 'Treatment prediction generated successfully' }), { status: 201 })];
                case 6:
                    error_2 = _a.sent();
                    console.error('Error generating prediction:', error_2);
                    if (error_2 instanceof Error) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: error_2.message }, { status: 400 })];
                    }
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Failed to generate prediction' }, { status: 500 })];
                case 7: return [2 /*return*/];
            }
        });
    });
}
