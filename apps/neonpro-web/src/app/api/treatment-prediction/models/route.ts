...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch prediction models" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to create prediction model" }, { status: 500 });
  }
}
