...
var accuracyAnalysisSchema = zod_1.z.object({
    ...
    modelType: zod_1.z.enum(['exponential_smoothing', 'seasonal_decomposition', 'linear_regression', 'moving_average']).optional()
});
function GET(request) {
    return __awaiter(this, void 0, void 0, function () {
        var searchParams, params, validation, _a, clinicId, itemId, period, modelType, periodDays, startDate, supabase, query, _b, accuracyData, error, stats, byModel, byCategory, trends, error_1;
            ...
            switch (_c.label) {
                case 0:
                    ...
                    if (!validation.success) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Invalid parameters', details: validation.error.errors }, { status: 400 })];
                    }
                    _a = validation.data, clinicId = _a.clinicId, itemId = _a.itemId, period = _a.period, modelType = _a.modelType;
                    periodDays = {
                        '7d': 7,
                        '30d': 30,
                        '90d': 90,
                ...
                case 2:
                    ...
                    return [2 /*return*/, server_2.NextResponse.json({
                            ...
                            data: {
                                period: period,
                                totalForecasts: (accuracyData === null || accuracyData === void 0 ? void 0 : accuracyData.length) || 0,
                ...
                case 3:
                    ...
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Failed to analyze forecast accuracy' }, { status: 500 })];
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 4:
                    ...
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Failed to log accuracy data' }, { status: 500 })];
...
function calculateAccuracyStatistics(data) {
    ...
    var accuracies = data.map(function (d) { return d.accuracy_percentage; }).filter(function (a) { return a !== null; });
    var mapes = data.map(function (d) { return d.mape; }).filter(function (m) { return m !== null; });
    var rmses = data.map(function (d) { return d.rmse; }).filter(function (r) { return r !== null; });
    var confidenceHits = data.filter(function (d) { return d.within_confidence_interval === true; }).length;
    return {
        averageAccuracy: accuracies.reduce(function (sum, acc) { return sum + acc; }, 0) / accuracies.length,
        averageMAPE: mapes.reduce(function (sum, mape) { return sum + mape; }, 0) / mapes.length,
        averageRMSE: rmses.reduce(function (sum, rmse) { return sum + rmse; }, 0) / rmses.length,
...
function groupAccuracyByCategory(data) {
    ...
    data.forEach(function (item) {
        var _a;
        var category = ((_a = item.inventory_items) === null || _a === void 0 ? void 0 : _a.category) || 'Unknown';
    ...
    Object.keys(byCategory).forEach(function (category) {
        ...
        byCategory[category] = {
            ...
            itemCount: new Set(categoryData.map(function (d) { return d.item_id; })).size
        };
    });
    return byCategory;
}
function calculateAccuracyTrends(data, period) {
    ...
    var weeklyStats = Array.from(weeklyData.entries()).map(function (_a) {
        var week = _a[0], items = _a[1];
    ...
    var recentAvgAccuracy = recentWeeks.reduce(function (sum, w) { return sum + w.averageAccuracy; }, 0) / recentWeeks.length;
    var earlyAvgAccuracy = earlyWeeks.reduce(function (sum, w) { return sum + w.averageAccuracy; }, 0) / earlyWeeks.length;
    return {
...
function generateAccuracyRecommendations(overallStats, byModel, trends) {
    ...
    if (overallStats.averageAccuracy < 0.7) {
        recommendations.push('Overall forecast accuracy is below 70% - consider reviewing forecasting parameters and data quality');
    }
    if (overallStats.confidenceIntervalHitRate < 0.8) {
        recommendations.push('Confidence intervals are not performing well - consider adjusting confidence levels or improving uncertainty estimates');
    }
    // Model performance recommendations
    var modelPerformance = Object.entries(byModel).map(function (_a) {
    ...
    if (modelPerformance.length > 1) {
        ...
        if (bestModel.accuracy - worstModel.accuracy > 0.1) {
            recommendations.push("Consider using ".concat(bestModel.model, " more frequently - it shows ").concat(Math.round((bestModel.accuracy - worstModel.accuracy) * 100), "% better accuracy than ").concat(worstModel.model));
        }
    }
    // Trend recommendations
    if (trends.trend.direction === 'declining' && trends.trend.isSignificant) {
        recommendations.push('Forecast accuracy is declining over time - review data quality and model parameters');
    }
