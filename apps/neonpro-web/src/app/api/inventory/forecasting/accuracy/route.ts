...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to analyze forecast accuracy" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to log accuracy data" }, { status: 500 });
  }
}
...
function generateAccuracyRecommendations(overallStats: any, byModel: any, trends: any): string[] {
