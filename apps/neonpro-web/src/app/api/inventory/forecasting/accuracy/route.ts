...
const accuracyAnalysisSchema = z.object({
  ...
  modelType: z.enum(['exponential_smoothing', 'seasonal_decomposition', 'linear_regression', 'moving_average']).optional()
});
...
function calculateAccuracyStatistics(data: any[]) {
  ...
  const accuracies = data.map(d => d.accuracy_percentage).filter(a => a !== null);
  const mapes = data.map(d => d.mape).filter(m => m !== null);
  const rmses = data.map(d => d.rmse).filter(r => r !== null);
  const confidenceHits = data.filter(d => d.within_confidence_interval === true).length;
...
function calculateAccuracyTrends(data: any[], period: string) {
  ...
  const weeklyStats = Array.from(weeklyData.entries()).map(([week, items]) => ({
    week,
  ...
  const recentAvgAccuracy = recentWeeks.reduce((sum, w) => sum + w.averageAccuracy, 0) / recentWeeks.length;
  const earlyAvgAccuracy = earlyWeeks.reduce((sum, w) => sum + w.averageAccuracy, 0) / earlyWeeks.length;
...
function generateAccuracyRecommendations(
  ...
  const modelPerformance = Object.entries(byModel).map(([model, data]: [string, any]) => ({
    model,
  ...
  if (modelPerformance.length > 1) {
    ...
    if (bestModel.accuracy - worstModel.accuracy > 0.1) {
      recommendations.push(`Consider using ${bestModel.model} more frequently - it shows ${Math.round((bestModel.accuracy - worstModel.accuracy) * 100)}% better accuracy than ${worstModel.model}`);
    }
  }
