  ...
  return __awaiter(this, arguments, void 0, function (request, _b) {
    ...
    return __generator(this, (_e) => {
      switch (_e.label) {
        ...
        case 3:
          body = _e.sent();
          validatedData = approvalActionSchema.parse(
            __assign(__assign({}, body), { action: params.action }),
          );
          return [
            4 /*yield*/,
            supabase
          ...
          if (existingPO.status !== "pending_approval") {
            return [
              ...
              server_2.NextResponse.json(
                {
                  error:
                    "Purchase order must be in pending_approval status. Current status: ".concat(
                      existingPO.status,
                    ),
                },
                { status: 400 },
        ...
        case 5:
          ...
          if (!canApprove.allowed) {
            return [
              ...
              server_2.NextResponse.json(
                {
                  error:
                    canApprove.reason || "Insufficient permissions to approve this purchase order",
                },
                { status: 403 },
              ),
            ];
          ...
          if (validatedData.action === "approve") {
            ...
            if (validatedData.approval_level >= requiredApprovalLevel) {
              newStatus = "approved";
              approvalNotes = "Approved by "
                .concat(user.email, " (Level ")
                .concat(validatedData.approval_level, "). ")
                .concat(approvalNotes);
            } else {
              // Still needs higher level approval
              newStatus = "pending_approval";
              approvalNotes = "Level "
                .concat(validatedData.approval_level, " approval by ")
                .concat(user.email, ". Pending Level ")
                .concat(validatedData.approval_level + 1, " approval. ")
                .concat(approvalNotes);
            }
          } else {
            newStatus = "draft";
            approvalNotes = "Rejected by "
              .concat(user.email, " (Level ")
              .concat(validatedData.approval_level, "). ")
              .concat(approvalNotes);
          }
        ...
        case 6:
          ...
          return [
            4 /*yield*/,
            supabase.from("purchase_order_approvals").insert({
    ...
    return __generator(this, (_c) => {
      switch (_c.label) {
        ...
        case 1:
          ...
          clinicPermission =
            (_b = userProfile.clinic_permissions) === null || _b === void 0
              ? void 0
              : _b.find((cp) => cp.clinic_id === clinicId);
          if (!clinicPermission) {
          ...
          if (!threshold.requiredRole.includes(userRole)) {
            return [
              ...
              {
                allowed: false,
                reason: "Insufficient role permissions. Required: ".concat(
                  threshold.requiredRole.join(" or "),
                ),
              },
            ];
          }
