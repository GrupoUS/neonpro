            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    if (!(config === null || config === void 0 ? void 0 : config.webhook_verify_token)) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Webhook verify token not configured' }, { status: 400 })];
                    }
                    if (token === config.webhook_verify_token) {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid verify token' }, { status: 403 })];
                    }
                    _a.label = 3;
                case 3: return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid verification request' }, { status: 400 })];
                case 4:
                    error_1 = _a.sent();
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    if (!payload.object || payload.object !== 'whatsapp_business_account') {
                        ...
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid webhook object' }, { status: 400 })];
                    }
                    if (!payload.entry || !Array.isArray(payload.entry)) {
                        console.error('Invalid webhook entry structure');
                        return [2 /*return*/, server_1.NextResponse.json({ error: 'Invalid webhook entry' }, { status: 400 })];
                    }
                    // Process the webhook payload
                    return [4 /*yield*/, (0, whatsapp_service_1.createwhatsAppService)().handleWebhook(payload)];
                case 2:
                    // Process the webhook payload
                    _a.sent();
                ...
                case 4:
                    ...
                    return [4 /*yield*/, logWebhookError(error_2 instanceof Error ? error_2.message : 'Unknown error')];
                case 5:
                    // Log the error for debugging
                    _a.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 })];
