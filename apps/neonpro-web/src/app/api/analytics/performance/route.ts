...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = NextResponse.json({
      ...
      metrics: data?.map(d => ({ id: d.id, name: d.name, grade: d.grade }))
    })
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Authentication required for viewing metrics
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) {
      return NextResponse.json(
    ...
    const headers = CacheHeaders.apiResponse()
    headers.forEach((value, key) => {
      response.headers.set(key, value)
    })
...
async function checkPerformanceAlerts(metrics: any[], supabase: any) {
  for (const metric of metrics) {
    ...
    if (threshold && metric.grade === 'poor') {
      ...
      console.warn(`🚨 Critical performance issue: ${metric.name} = ${metric.value}ms (threshold: ${threshold.critical}ms)`)
