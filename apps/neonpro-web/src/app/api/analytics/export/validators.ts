...
export function validateRateLimit(
  ...
  const recentExports = exportHistory.filter((export_) => new Date(export_.createdAt) > oneHourAgo);

  const dailyExports = exportHistory.filter((export_) => new Date(export_.createdAt) > oneDayAgo);
