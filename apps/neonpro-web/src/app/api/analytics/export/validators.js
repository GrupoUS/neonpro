...
function validateFormat(format) {
    ...
    if (!types_1.SUPPORTED_FORMATS.includes(format)) {
        return {
            valid: false,
            errors: ["Unsupported format: ".concat(format, ". Supported formats: ").concat(types_1.SUPPORTED_FORMATS.join(', '))]
        };
    }
    return { valid: true };
}
...
function validateType(type) {
    ...
    if (!types_1.SUPPORTED_TYPES.includes(type)) {
        return {
            valid: false,
            errors: ["Unsupported type: ".concat(type, ". Supported types: ").concat(types_1.SUPPORTED_TYPES.join(', '))]
        };
    }
    return { valid: true };
}
    ...
    if (data.correlations && !Array.isArray(data.correlations)) {
        ...
        data.correlations.forEach(function (corr, index) {
            ...
            if (typeof corr.correlation !== 'number' || corr.correlation < -1 || corr.correlation > 1) {
                errors.push("Correlation ".concat(index, ": correlation must be a number between -1 and 1"));
            }
        });
    }
...
function validateFileSize(data, maxSize) {
    ...
    if (size > maxSize) {
        return {
            valid: false,
            errors: ["File size (".concat(Math.round(size / 1024 / 1024), "MB) exceeds maximum allowed size (").concat(Math.round(maxSize / 1024 / 1024), "MB)")]
        };
    }
    return { valid: true };
}
...
function validateRateLimit(userId, exportHistory, limits) {
    ...
    if (recentExports.length >= limits.maxExportsPerHour) {
        return {
            valid: false,
            errors: ["Rate limit exceeded: Maximum ".concat(limits.maxExportsPerHour, " exports per hour")]
        };
    }
...
function createValidationError(errors, code) {
    if (code === void 0) { code = types_1.ExportErrorCode.VALIDATION_FAILED; }
    return {
        success: false,
        error: errors.join('; '),
        code: code,
