...
function POST(request) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, user, authError, body, validatedRequest, type, format, data, _b, options, exportData, contentType, filename, _c, response, error_1;
        var _d, _e, _f, _g;
        return __generator(this, function (_h) {
            switch (_h.label) {
                ...
                case 3:
                    ...
                    type = validatedRequest.type, format = validatedRequest.format, data = validatedRequest.data, _b = validatedRequest.options, options = _b === void 0 ? {} : _b;
                    exportData = void 0;
                    contentType = void 0;
                    filename = void 0;
                    _c = format;
                    switch (_c) {
                        ...
                        case 'excel': return [3 /*break*/, 6];
                        case 'pdf': return [3 /*break*/, 8];
                        case 'json': return [3 /*break*/, 10];
                    }
                    return [3 /*break*/, 12];
                case 4: return [4 /*yield*/, generateCSVExport(type, data, options)];
                case 5:
                    (_d = _h.sent(), exportData = _d.data, contentType = _d.contentType, filename = _d.filename);
                    return [3 /*break*/, 13];
                case 6: return [4 /*yield*/, generateExcelExport(type, data, options)];
                case 7:
                    (_e = _h.sent(), exportData = _e.data, contentType = _e.contentType, filename = _e.filename);
                    return [3 /*break*/, 13];
                case 8: return [4 /*yield*/, generatePDFExport(type, data, options)];
                case 9:
                    (_f = _h.sent(), exportData = _f.data, contentType = _f.contentType, filename = _f.filename);
                    return [3 /*break*/, 13];
                case 10: return [4 /*yield*/, generateJSONExport(type, data, options)];
                case 11:
                    (_g = _h.sent(), exportData = _g.data, contentType = _g.contentType, filename = _g.filename);
                    return [3 /*break*/, 13];
                case 12: return [2 /*return*/, server_2.NextResponse.json({ error: 'Unsupported export format' }, { status: 400 })];
                case 13: 
                // Log export activity
                ...
                case 14:
                    ...
                    response.headers.set('Content-Disposition', "attachment; filename=\"".concat(filename, "\""));
                ...
                case 15:
                    ...
                    if (error_1 instanceof zod_1.z.ZodError) {
                        return [2 /*return*/, server_2.NextResponse.json({ error: 'Invalid request format', details: error_1.errors }, { status: 400 })];
                    }
                    return [2 /*return*/, server_2.NextResponse.json({ error: 'Export generation failed' }, { status: 500 })];
        ...
        return __generator(this, function (_a) {
            ...
            switch (type) {
                case 'cohort':
                    ...
                    filename = options.filename || "cohort-analysis-".concat(new Date().toISOString().split('T')[0], ".csv");
                    break;
                case 'forecast':
                    csvData = generateForecastCSV(data);
                    filename = options.filename || "forecast-data-".concat(new Date().toISOString().split('T')[0], ".csv");
                    break;
                case 'insights':
                    csvData = generateInsightsCSV(data);
                    filename = options.filename || "statistical-insights-".concat(new Date().toISOString().split('T')[0], ".csv");
                    break;
                case 'dashboard':
                    csvData = generateDashboardCSV(data);
                    filename = options.filename || "dashboard-metrics-".concat(new Date().toISOString().split('T')[0], ".csv");
                    break;
                case 'realtime':
                    csvData = generateRealtimeCSV(data);
                    filename = options.filename || "realtime-metrics-".concat(new Date().toISOString().split('T')[0], ".csv");
                    break;
                default:
...
function generateExcelExport(type, data, options) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            filename = "".concat(type, "-export-").concat(new Date().toISOString().split('T')[0], ".xlsx");
            switch (type) {
                case 'cohort':
                    addCohortSheetsToWorkbook(workbook, data, options);
                    filename = options.filename || "cohort-analysis-".concat(new Date().toISOString().split('T')[0], ".xlsx");
                    break;
                case 'forecast':
                    addForecastSheetsToWorkbook(workbook, data, options);
                    filename = options.filename || "forecast-report-".concat(new Date().toISOString().split('T')[0], ".xlsx");
                    break;
                case 'insights':
                    addInsightsSheetsToWorkbook(workbook, data, options);
                    filename = options.filename || "statistical-insights-".concat(new Date().toISOString().split('T')[0], ".xlsx");
                    break;
                case 'dashboard':
                    addDashboardSheetsToWorkbook(workbook, data, options);
                    filename = options.filename || "analytics-dashboard-".concat(new Date().toISOString().split('T')[0], ".xlsx");
                    break;
                default:
        ...
        return __generator(this, function (_a) {
            ...
            switch (type) {
                case 'cohort':
                    ...
                    filename = options.filename || "cohort-analysis-".concat(new Date().toISOString().split('T')[0], ".pdf");
                    break;
                case 'forecast':
                    addForecastContentToPDF(pdf, data, options);
                    filename = options.filename || "forecast-report-".concat(new Date().toISOString().split('T')[0], ".pdf");
                    break;
                case 'insights':
                    addInsightsContentToPDF(pdf, data, options);
                    filename = options.filename || "insights-report-".concat(new Date().toISOString().split('T')[0], ".pdf");
                    break;
                case 'dashboard':
                    addDashboardContentToPDF(pdf, data, options);
                    filename = options.filename || "dashboard-report-".concat(new Date().toISOString().split('T')[0], ".pdf");
                    break;
                default:
...
function generateJSONExport(type, data, options) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            exportData = data;
            filename = "".concat(type, "-export-").concat(new Date().toISOString().split('T')[0], ".json");
