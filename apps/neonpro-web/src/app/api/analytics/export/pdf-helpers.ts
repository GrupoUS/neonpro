...
export function generateForecastPDF(data: any, options: PDFOptions = {}): jsPDF {
  ...
  if (data.predictions && data.predictions.length > 0) {
    ...
    const tableData = data.predictions.slice(0, 20).map((prediction: any) => [
...
export function generateInsightsPDF(data: any, options: PDFOptions = {}): jsPDF {
  ...
  if (data.anomalies && data.anomalies.length > 0) {
    ...
    const anomalyData = data.anomalies.slice(0, 15).map((anomaly: any) => [
      anomaly.metric || '',
  ...
  if (data.predictions && data.predictions.length > 0) {
    ...
    data.predictions.slice(0, 10).forEach((pred: any, index: number) => {
      ...
      yPosition += 5
      doc.text(`Confidence: ${pred.confidence ? `${(pred.confidence * 100).toFixed(1)}%` : 'N/A'}`, 20, yPosition)
      yPosition += 5
  ...
  sections.forEach(section => {
    if (data[section.key] && data[section.key].length > 0) {
      ...
      if (section.key === 'insights') {
        ...
        const summaryData = data[section.key].slice(0, section.maxRows).map((item: any, index: number) => [
          `Item ${index + 1}`,
