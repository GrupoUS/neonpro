...
const CohortAnalysisRequestSchema = z.object({
  ...
  metrics: z.array(z.enum(['retention', 'revenue', 'churn', 'ltv'])).default(['retention']),
...
const ForecastingRequestSchema = z.object({
  ...
  model_type: z.enum(['auto', 'linear', 'polynomial', 'seasonal']).default('auto'),
  include_scenarios: z.boolean().default(false)
})

const StatisticalAnalysisRequestSchema = z.object({
  metrics: z.array(z.string()),
  analysis_type: z.enum(['correlation', 'regression', 'significance_test', 'all']).default('all'),
  confidence_level: z.number().min(0.8).max(0.99).default(0.95),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (endpoint) {
      ...
      default:
        return NextResponse.json({ error: 'Invalid endpoint. Use ?type=cohort-analysis|forecasting|statistical-analysis' }, { status: 404 })
    }
...
async function handleCohortAnalysis(request: NextRequest, supabase: any, userId: string) {
  try {
...
// Forecasting Handler
async function handleForecasting(request: NextRequest, supabase: any, userId: string) {
  try {
    const body = await request.json()
    ...
    const response = {
      ...
      data: {
        ...
        metadata: {
          ...
          model_type: forecast.model_info?.selected_model || validatedData.model_type,
          confidence_level: validatedData.confidence_level,
...
async function handleStatisticalAnalysis(request: NextRequest, supabase: any, userId: string) {
  try {
    const body = await request.json()
    ...
    // Regression Analysis
    if (validatedData.analysis_type === 'regression' || validatedData.analysis_type === 'all') {
    ...
    // Significance Tests
    if (validatedData.analysis_type === 'significance_test' || validatedData.analysis_type === 'all') {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (cacheError || !cachedData) {
      return NextResponse.json({ error: 'Cache miss or expired' }, { status: 404 })
    }
...
function processAnalyticsDataForStatistics(rawData: any[], metrics: string[]) {
  ...
  const dailyData = rawData.reduce((acc, event) => {
...
function getSignificanceCategory(pValue: number): 'high' | 'medium' | 'low' | 'none' {
...
async function performRegressionAnalysis(data: any[], dependent: string, independent: string[], confidenceLevel: number) {
...
async function performSignificanceTests(data: any[], metrics: string[], confidenceLevel: number) {
  const tests = []
...
async function evaluatePredictiveModels(data: any[], metrics: string[]) {
  ...
  return modelTypes.map(modelType => ({
    ...
    features: metrics,
    featureImportance: metrics.map(metric => ({
      feature: metric,
