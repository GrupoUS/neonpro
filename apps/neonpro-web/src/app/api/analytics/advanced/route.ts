...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (endpoint) {
      ...
      default:
        return NextResponse.json(
          { error: "Invalid endpoint. Use ?type=cohort-analysis|forecasting|statistical-analysis" },
          { status: 404 },
        );
    }
  } catch (error) {
...
async function handleStatisticalAnalysis(request: NextRequest, supabase: any, userId: string) {
  try {
    ...
    const dataQuality = await assessDataQuality(processedData, validatedData.include_outliers);

    // Predictive Model Evaluation
    const predictiveModels = await evaluatePredictiveModels(processedData, validatedData.metrics);
