...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (validatedParams.metric) {
      case 'revenue':
        analyticsData = await getRevenueAnalytics(startDate, endDate, validatedParams.granularity, validatedParams.userId || userId)
        break
      case 'subscriptions':
        analyticsData = await getSubscriptionAnalytics(startDate, endDate, validatedParams.granularity, validatedParams.userId || userId)
        break
      case 'trials':
        analyticsData = await getTrialAnalytics(startDate, endDate, validatedParams.granularity, validatedParams.userId || userId)
        break
      case 'conversions':
        analyticsData = await getConversionAnalytics(startDate, endDate, validatedParams.granularity, validatedParams.userId || userId)
        break
      case 'churn':
        analyticsData = await getChurnAnalytics(startDate, endDate, validatedParams.granularity, validatedParams.userId || userId)
        break
      default:
...
async function getDashboardAnalytics(startDate: string, endDate: string, userId: string, userRole: string | null) {
