...
export function useBulkScanning() {
  ...
  const processBulkScan = useCallback(
    async (operationId: string, scanValue: string, userId: string): Promise<ScanResult> => {
      try {
...
// Barcode Data Hook
export function useBarcodeData(itemId: string) {
  return useQuery({
    ...
    labelsHTML += `
      ...
      ">
        ...
        <div style="display: flex; justify-content: space-between; font-size: 10px;">
          <div>
            ${options.batch_number ? `Lote: ${options.batch_number}` : ""}
          </div>
          <div>
