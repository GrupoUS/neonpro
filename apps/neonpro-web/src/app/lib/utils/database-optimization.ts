...
export class OptimizedQueryBuilder {
  ...
  async batchInsert<T>(
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      try {
        const { data, error } = await this.supabase.from(this.tableName).insert(batch).select();
...
export class StockAlertQueries {
  ...
  async getDashboardData(clinicId: string, days: number = 30): Promise<any> {
    ...
    queryMonitor.logQuery({
      ...
      duration: Date.now() - startTime,
      rows:
        dashboardData.products.length + dashboardData.alerts.length + dashboardData.metrics.length,
      cached: false,
      timestamp: new Date(),
    });
