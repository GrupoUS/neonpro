...
export class IntelligentThresholdService {
  ...
  async getAlertStats(clinicId: string, dateRange?: { start: Date; end: Date }) {
    ...
    let query = supabase.from("reorder_alerts").select("*").eq("clinic_id", clinicId);
    ...
    const stats = {
      ...
      critical_alerts: alerts.filter((a) => a.priority === "critical" || a.priority === "emergency")
        .length,
