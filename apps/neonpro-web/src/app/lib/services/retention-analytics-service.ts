...
export class RetentionAnalyticsService {
  ...
  async calculatePatientRetentionMetrics(
    ...
    try {
      ...
      const daysSinceFirst = firstAppointment
        ? Math.floor(
            (now.getTime() - new Date(firstAppointment.date).getTime()) / (1000 * 60 * 60 * 24),
          )
        : 1;
      ...
      const satisfactionScores =
        responses?.filter((r) => r.satisfaction_rating).map((r) => r.satisfaction_rating) || [];
  ...
  async getChurnPredictions(
    ...
    try {
      let query = supabase.from("churn_predictions").select("*").eq("clinic_id", clinicId);
   ...
   */
  async createRetentionStrategy(strategy: CreateRetentionStrategy): Promise<RetentionStrategy> {
    try {
      const newStrategy = {
        ...strategy,
        success_rate: 0,
   ...
   */
  async getRetentionStrategies(clinicId: string, activeOnly = false): Promise<RetentionStrategy[]> {
    try {
      let query = supabase.from("retention_strategies").select("*").eq("clinic_id", clinicId);
