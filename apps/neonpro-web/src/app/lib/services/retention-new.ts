  ...
  static async getRetentionMetrics(params: {
    ...
    try {
      ...
      const avgLifetimeValue =
        churnSummary?.reduce((sum, item) => sum + (item.lifetime_value || 0), 0) / totalPatients ||
        0;
      const avgPredictedLTV =
        churnSummary?.reduce((sum, item) => sum + (item.predicted_ltv || 0), 0) / totalPatients ||
        0;
      const avgRetentionScore =
        churnSummary?.reduce((sum, item) => sum + (item.retention_score || 0), 0) / totalPatients ||
        0;
