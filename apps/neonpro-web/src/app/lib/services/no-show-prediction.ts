...
export class NoShowPredictionEngine {
  ...
  async createRiskFactor(input: CreateRiskFactorInput): Promise<RiskFactor> {
    const supabase = await createClient();
    const { data, error } = await supabase.from("risk_factors").insert(input).select().single();
  ...
  async getNoShowTrends(startDate: string, endDate: string): Promise<NoShowTrends> {
    ...
    const totalAppointments =
      analytics?.reduce((sum, a) => sum + (a.predicted_no_shows + a.actual_no_shows), 0) || 0;
    ...
    const avgAccuracy =
      analytics?.reduce((sum, a) => sum + a.accuracy_rate, 0) / (analytics?.length || 1) || 0;
