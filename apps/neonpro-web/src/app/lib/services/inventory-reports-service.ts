...
class InventoryReportsService {
    ...
    const reportData: StockValuationReportData[] =
      stockData?.map((stock: any) => {
        const daysSinceMovement = stock.last_updated
          ? Math.floor(
              (Date.now() - new Date(stock.last_updated).getTime()) / (1000 * 60 * 60 * 24),
            )
          : null;
  ...
  async generateTransferReport(filters: ReportFilters): Promise<ReportResult<"transfers">> {
    const startTime = Date.now();
  ...
  async generateLocationPerformanceReport(
    ...
    // Get all locations
    let locationsQuery = supabase.from("clinics").select("id, name");
  ...
  private async calculateLocationPerformance(clinicId: string, filters: ReportFilters) {
    ...
    const movementsIn =
      movements?.filter(
        (m) => ["in", "return", "adjustment"].includes(m.transaction_type) && m.quantity_change > 0,
      ).length || 0;
    const movementsOut =
      movements?.filter(
        (m) => ["out", "transfer", "waste"].includes(m.transaction_type) && m.quantity_change < 0,
      ).length || 0;
