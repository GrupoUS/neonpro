...
class InventoryReportsService {
  ...
  async generateStockMovementReport(
    ...
    const reportData: StockMovementReportData[] = movements?.map((movement: any) => ({
  ...
  private calculateStockMovementSummary(data: StockMovementReportData[]): StockMovementSummary {
    ...
    data.forEach(movement => {
      const isInbound = ['in', 'return', 'adjustment'].includes(movement.movement_type);
  ...
  async generateStockValuationReport(
    ...
    // Transform data to report format
    const reportData: StockValuationReportData[] = stockData?.map((stock: any) => {
      ...
      let stockStatus: 'adequate' | 'low' | 'critical' | 'overstock' = 'adequate';
  ...
  private calculateStockValuationSummary(data: StockValuationReportData[]): StockValuationSummary {
  ...
  async generateExpiringItemsReport(
    ...
    // Transform data to report format
    const reportData: ExpiringItemsReportData[] = expiringData?.map((item: any) => {
      const expiryDate = new Date(item.expiry_date);
      const today = new Date();
      const daysToExpiry = Math.ceil((expiryDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));

      let urgencyLevel: 'immediate' | 'urgent' | 'warning' | 'watch' = 'watch';
      let suggestedAction: 'use_immediately' | 'transfer' | 'discount' | 'dispose' = 'dispose';
  ...
  private calculateExpiringItemsSummary(data: ExpiringItemsReportData[]): ExpiringItemsSummary {
  ...
  async generateTransferReport(
    ...
    // Transform data to report format
    const reportData: TransferReportData[] = transfers?.map((transfer: any) => ({
  ...
  private calculateTransferSummary(data: TransferReportData[]): TransferReportSummary {
    ...
    const itemCounts: Record<string, { name: string; count: number; total_quantity: number }> = {};
  ...
  async generateLocationPerformanceReport(
    ...
    for (const clinic of clinics || []) {
      const performance = await this.calculateLocationPerformance(clinic.id, filters);
      reportData.push({
        clinic_id: clinic.id,
  ...
  private async calculateLocationPerformance(clinicId: string, filters: ReportFilters) {
    ...
    const totalValue = stockData?.reduce((sum, item) => 
      sum + (item.current_quantity * (item.cost_per_unit || 0)), 0) || 0;
    
    const totalMovements = movements?.length || 0;
    const movementsIn = movements?.filter(m => 
      ['in', 'return', 'adjustment'].includes(m.transaction_type) && m.quantity_change > 0).length || 0;
    const movementsOut = movements?.filter(m => 
      ['out', 'transfer', 'waste'].includes(m.transaction_type) && m.quantity_change < 0).length || 0;
    
    const valueIn = movements?.filter(m => m.quantity_change > 0)
      .reduce((sum, m) => sum + (Math.abs(m.quantity_change) * (m.unit_cost || 0)), 0) || 0;
    const valueOut = movements?.filter(m => m.quantity_change < 0)
      .reduce((sum, m) => sum + (Math.abs(m.quantity_change) * (m.unit_cost || 0)), 0) || 0;

    const lowStockItems = stockData?.filter(item => 
      item.current_quantity <= (item.inventory_items.minimum_quantity || 0)).length || 0;

    // Calculate performance metrics
    const turnoverRate = totalValue > 0 ? (valueOut / totalValue) * 100 : 0;
    const stockAccuracy = totalItems > 0 ? ((totalItems - lowStockItems) / totalItems) * 100 : 100;
    const transferRequests = (transfersOut?.length || 0) + (transfersIn?.length || 0);
    const utilizationRate = totalItems > 0 ? (movementsOut / totalItems) * 100 : 0;
  ...
  private calculateLocationPerformanceSummary(data: LocationPerformanceData[]): LocationPerformanceSummary {
    if (data.length === 0) {
      return {
        total_locations: 0,
    ...
    return {
      ...
      average_performance_score: data.reduce((sum, loc) => sum + loc.performance_score, 0) / data.length,
