...
export class NotificationService {
  ...
  async sendNotification(payload: NotificationPayload): Promise<NotificationResult[]> {
    try {
      ...
      if (validPayload.scheduledFor && validPayload.scheduledFor > new Date()) {
        return await this.scheduleNotification(validPayload, channels, preferences);
      }
  ...
  private determineChannels(
    ...
    if (payload.channel) {
      const channelEnabled = this.isChannelEnabled(payload.channel, payload.type, preferences);
      if (channelEnabled) {
  ...
  private getChannelsForType(
    ...
    Object.entries(preferences.channels).forEach(([channelKey, channelPrefs]) => {
  ...
  async getNotificationStatus(notificationId: string): Promise<{
    ...
    try {
      ...
      const scheduledStatus = await this.schedulingService.getNotificationStatus(notificationId);
      if (scheduledStatus) return scheduledStatus;

      // Check delivery status from services
      const emailStatus = await this.emailService.getDeliveryStatus(notificationId);
