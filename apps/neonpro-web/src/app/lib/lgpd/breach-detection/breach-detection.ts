...
import { lgpdAuditLogger } from '../audit/lgpd-audit'
import { realTimeComplianceMonitor } from '../monitoring/compliance-monitoring'
...
class BreachDetectionSystem {
  ...
  private setupDefaultRules(): void {
    const defaultRules: Omit<BreachDetectionRule, 'id' | 'createdAt' | 'updatedAt' | 'lastTriggered' | 'triggerCount'>[] = [
      {
  ...
  private detectFailedLogins(rule: BreachDetectionRule, events: any[]): any | null {
    ...
    for (const [userId, userEvents] of Object.entries(groupedByUser)) {
      if ((userEvents as any[]).length >= rule.thresholds.failed_attempts) {
        const timeSpan = new Date((userEvents as any[])[0].created_at).getTime() - 
