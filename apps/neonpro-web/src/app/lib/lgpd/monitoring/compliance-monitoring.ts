...
class RealTimeComplianceMonitor {
  ...
  private async calculateConsentScore(): Promise<number> {
    try {
      ...
      baseScore -=
        violations.length * MONITORING_CONFIG.VIOLATION_PENALTIES[ViolationType.CONSENT_VIOLATION];

      // Bonus for high consent rates
      const consentRate =
        consentMetrics.total > 0 ? consentMetrics.granted / consentMetrics.total : 1;
  ...
  private async calculateDataRightsScore(): Promise<number> {
    try {
      ...
      baseScore -=
        violations.length *
        MONITORING_CONFIG.VIOLATION_PENALTIES[ViolationType.RESPONSE_TIME_VIOLATION];
  ...
  private async calculateDataProtectionScore(): Promise<number> {
    try {
      ...
      baseScore -=
        violations.length * MONITORING_CONFIG.VIOLATION_PENALTIES[ViolationType.SECURITY_VIOLATION];
  ...
  private async calculateAuditScore(): Promise<number> {
    try {
      ...
      baseScore -=
        violations.length * MONITORING_CONFIG.VIOLATION_PENALTIES[ViolationType.AUDIT_VIOLATION];
  ...
  private async calculateRetentionScore(): Promise<number> {
    try {
      ...
      baseScore -=
        violations.length *
        MONITORING_CONFIG.VIOLATION_PENALTIES[ViolationType.RETENTION_VIOLATION];
