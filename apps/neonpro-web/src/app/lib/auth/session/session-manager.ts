...
class SessionManager {
   ...
   */
  async createSession(user: User, additionalData?: Record<string, any>): Promise<SessionData> {
    const now = Date.now()
   ...
   */
  async updateActivity(action: string = 'activity', metadata?: Record<string, any>): Promise<void> {
  ...
  async getCurrentSession(): Promise<SessionData | null> {
    try {
      const encryptedData = localStorage.getItem(`${SESSION_CONFIG.STORAGE_KEY_PREFIX}current`)
      if (!encryptedData) return null

      const sessionData = JSON.parse(this.decrypt(encryptedData)) as SessionData
  ...
  shouldRefreshToken(): boolean {
    ...
    const refreshThreshold = SESSION_CONFIG.TOKEN_REFRESH_THRESHOLD_MINUTES * 60 * 1000
  ...
  async getActiveSessions(): Promise<SessionData[]> {
    try {
      const allKeys = Object.keys(localStorage).filter(key => 
        key.startsWith(SESSION_CONFIG.STORAGE_KEY_PREFIX) && key !== `${SESSION_CONFIG.STORAGE_KEY_PREFIX}current`
      )
  ...
  async terminateSession(sessionId: string): Promise<boolean> {
    try {
      ...
      await this.logActivity('session_terminated', { terminatedSessionId: sessionId })
      return true
    } catch (error) {
  ...
  private async storeSession(session: SessionData): Promise<void> {
    try {
      ...
      localStorage.setItem(`${SESSION_CONFIG.STORAGE_KEY_PREFIX}current`, encryptedData)
      localStorage.setItem(`${SESSION_CONFIG.STORAGE_KEY_PREFIX}${session.sessionId}`, encryptedData)
    } catch (error) {
  ...
  private async logActivity(action: string, metadata?: Record<string, any>): Promise<void> {
    try {
      ...
      const activities = JSON.parse(localStorage.getItem('session_activities') || '[]')
