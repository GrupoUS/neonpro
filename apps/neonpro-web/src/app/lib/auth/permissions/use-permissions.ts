...
export interface UsePermissionsResult {
  ...
  isLoading: boolean
  error?: string
  checkPermission: (resource: ResourceType, action: Permission, resourceId?: string) => Promise<boolean>
  hasRole: (role: SystemRole) => boolean
  hasAnyRole: (roles: SystemRole[]) => boolean
  hasHigherRole: (targetRole: SystemRole) => boolean
  canAccess: (resource: ResourceType, action: Permission, resourceId?: string) => Promise<boolean>
...
export function usePermissions(userId: string): UsePermissionsResult {
  ...
  const hasHigherRole = useCallback((targetRole: SystemRole): boolean => {
    if (!userPermissions) return false
...
export function useMultiplePermissions(
  ...
  const checkPermissions = useCallback(async () => {
    ...
    try {
      ...
      const permissionResults = await checkMultiplePermissions(userId, permissions)
      
      const resultMap: Record<string, boolean> = {}
      permissions.forEach((permission, index) => {
        const key = permission.key || `${permission.resource}:${permission.action}:${permission.resourceId || 'none'}`
  ...
  return {
    ...
    can: (key: string) => results[key] || false
  }
}
...
export function withPermissionCheck<T extends object>(
  ...
  return function PermissionProtectedComponent(props: T & { userId: string; resourceId?: string }) {
    const { userId, resourceId, ...componentProps } = props
    const { allowed, isLoading } = usePermissionCheck(userId, resource, action, resourceId)
