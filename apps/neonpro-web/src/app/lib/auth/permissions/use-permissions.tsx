...
export function usePermissions(userId: string): UsePermissionsResult {
  ...
  const checkPermission = useCallback(
    async (resource: ResourceType, action: Permission, resourceId?: string): Promise<boolean> => {
...
export function withPermissionCheck<T extends object>(
  ...
  return function PermissionProtectedComponent(props: T & { userId: string; resourceId?: string }) {
    ...
    if (!allowed) {
      if (fallback) {
        const _FallbackComponent = fallback;
...
export function useMedicalAccess(userId: string) {
  ...
  const isMedicalStaff = useMemo(() => {
    return roles.some((role) =>
      [SystemRole.DOCTOR, SystemRole.NURSE, SystemRole.TECHNICIAN].includes(role),
    );
  }, [roles]);
