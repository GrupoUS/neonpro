  ...
  async getSecurityMetrics(hoursBack: number = 24): Promise<SecurityMetrics> {
    try {
      ...
      const metrics: SecurityMetrics = {
        ...
        suspiciousActivities: events.filter((e) => e.type === AuditEventType.SUSPICIOUS_ACTIVITY)
          .length,
    ...
    try {
      ...
      if (event.type === AuditEventType.LOGIN_FAILURE) {
        const failedLogins = recentEvents.filter(
          (e) =>
            ...
            e.email === event.email &&
            e.timestamp >
              Date.now() - SUSPICIOUS_PATTERNS.MULTIPLE_FAILED_LOGINS.timeWindowMinutes * 60 * 1000,
        );
      ...
      const rapidAttempts = recentEvents.filter(
        (e) =>
          ...
          e.timestamp >
            Date.now() - SUSPICIOUS_PATTERNS.RAPID_LOGIN_ATTEMPTS.timeWindowMinutes * 60 * 1000,
      );
