  ...
  const fetchSecurityData = useCallback(async () => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        isLoading: false,
        error: error instanceof Error ? error.message : "Erro ao carregar dados de segurança",
      }));
    }
...
async function getRecentAuditEvents(limit: number = 50): Promise<AuditEvent[]> {
  try {
...
export function useAuthAudit() {
  const logLoginAttempt = useCallback(async (email: string, method: string = "oauth") => {
  ...
  const logLoginSuccess = useCallback(
    async (userId: string, sessionId: string, method: string = "oauth") => {
  ...
  const logLoginFailure = useCallback(
    async (email: string, reason: string, method: string = "oauth") => {
  ...
  const logOAuthFlow = useCallback(
    async (stage: "start" | "success" | "failure", details: Record<string, any>) => {
