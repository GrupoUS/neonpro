...
export const DeliveryMethodSchema = z.enum(["email", "dashboard", "webhook", "file_system"]);

export const ReportFormatSchema = z.enum(["pdf", "excel", "csv", "powerpoint", "json"]);

export const PermissionLevelSchema = z.enum(["view", "edit", "admin", "owner"]);

export const ConnectorTypeSchema = z.enum(["internal", "database", "api", "file", "webhook"]);

export const TestStatusSchema = z.enum(["success", "failed", "unknown", "testing"]);
...
export const DataSourceTypeSchema = z.enum(["sql", "api", "file", "real_time", "cached"]);
...
export const ScheduleFrequencySchema = z.enum(["daily", "weekly", "monthly", "yearly", "custom"]);
