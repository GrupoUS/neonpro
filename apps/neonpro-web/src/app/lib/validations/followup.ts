...
export const patientFollowupSchema = z.object({
  ...
  scheduled_time: z.string().regex(/^\d{2}:\d{2}$/, 'Hora inválida').optional(),
  optimal_time_calculated: z.boolean().default(false),
  
  // Communication preferences
  preferred_channel: z.enum(['sms', 'whatsapp', 'email', 'phone']).default('sms'),
  backup_channels: z.array(z.enum(['sms', 'whatsapp', 'email', 'phone'])).default(['email', 'phone']),
  language_preference: z.string().max(10).default('pt-BR'),
  
  // Status tracking
  status: z.enum(['scheduled', 'sent', 'completed', 'missed', 'cancelled', 'escalated']).default('scheduled'),
...
export const followupCommunicationSchema = z.object({
  ...
  delivery_status: z.enum(['pending', 'sent', 'delivered', 'failed', 'read']).default('pending'),
...
export const createFollowupProtocolSchema = z.object({
  ...
  protocol_source: z.string().max(500).optional(),
  initial_followup_days: z.number().int().min(0).max(365),
  subsequent_intervals: z.array(z.number().int().min(1).max(365)).min(1, 'Pelo menos um intervalo é obrigatório'),
  max_followups: z.number().int().min(1).max(20).default(4),
  urgent_threshold_hours: z.number().int().min(1).max(168).default(24),
  automation_level: z.number().min(0.5).max(1.0),
  auto_schedule_enabled: z.boolean().default(true),
...
export const createPatientFollowupSchema = z.object({
  ...
  followup_type: z.enum(['initial', 'routine', 'urgent', 'outcome_check']),
  scheduled_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data inválida (use YYYY-MM-DD)'),
  scheduled_time: z.string().regex(/^\d{2}:\d{2}$/, 'Hora inválida (use HH:MM)').optional(),
  preferred_channel: z.enum(['sms', 'whatsapp', 'email', 'phone']).default('sms'),
  backup_channels: z.array(z.enum(['sms', 'whatsapp', 'email', 'phone'])).default(['email', 'phone']),
  language_preference: z.string().max(10).default('pt-BR'),
  shadow_test_variant: z.string().max(50).optional()
...
export const createTreatmentOutcomeSchema = z.object({
  ...
  outcome_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data inválida (use YYYY-MM-DD)'),
...
export const followupFiltersSchema = z.object({
  ...
  status: z.enum(['scheduled', 'sent', 'completed', 'missed', 'cancelled', 'escalated']).optional(),
  followup_type: z.enum(['initial', 'routine', 'urgent', 'outcome_check']).optional(),
