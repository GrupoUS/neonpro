...
var statusConfig = {
    "agendado": { label: "Agendado", color: "bg-yellow-100 text-yellow-800", icon: lucide_react_1.Clock },
    "confirmado": { label: "Confirmado", color: "bg-blue-100 text-blue-800", icon: lucide_react_1.CheckCircle },
    "em-andamento": { label: "Em Andamento", color: "bg-green-100 text-green-800", icon: lucide_react_1.Clock },
    "concluido": { label: "Concluído", color: "bg-gray-100 text-gray-800", icon: lucide_react_1.CheckCircle },
...
function AppointmentsPage() {
    ...
    var _g = (0, react_1.useState)(false), isDialogOpen = _g[0], setIsDialogOpen = _g[1];
    (0, react_1.useEffect)(function () {
        var loadAppointments = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var filteredAppointments = appointments.filter(function (appointment) {
        var matchesSearch = appointment.patientName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    var getTypeConfig = function (type) {
        return appointmentTypes.find(function (t) { return t.value === type; }) || appointmentTypes[0];
    };
    var StatusIcon = function (_a) {
        var status = _a.status;
        var config = statusConfig[status];
        var Icon = (config === null || config === void 0 ? void 0 : config.icon) || lucide_react_1.Clock;
    ...
    return (<main className="flex-1 space-y-6 p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-2">
          <dialog_1.Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <dialog_1.DialogContent className="sm:max-w-[600px]">
              ...
              <NewAppointmentForm onClose={function () { return setIsDialogOpen(false); }}/>
      ...
      <div className="grid gap-6 lg:grid-cols-4">
        ...
        <card_1.Card className="lg:col-span-1">
          ...
          <card_1.CardContent>
            <calendar_1.Calendar mode="single" selected={selectedDate} onSelect={function (date) { return date && setSelectedDate(date); }} className="rounded-md border w-full"/>
            ...
            <div className="mt-6 space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-green-600">
                  {filteredAppointments.filter(function (a) { return a.status === "confirmado"; }).length}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Pendentes:</span>
                <span className="font-medium text-yellow-600">
                  {filteredAppointments.filter(function (a) { return a.status === "agendado"; }).length}
                </span>
              </div>
            </div>
          </card_1.CardContent>
              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input_1.Input placeholder="Buscar paciente ou médico..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
                  </div>
                </div>
            ...
            return (<select_1.SelectItem key={value} value={value}>
                ...
                <select_1.Select value={typeFilter} onValueChange={setTypeFilter}>
                  ...
                  <select_1.SelectContent>
                    ...
                    {appointmentTypes.map(function (type) { return (<select_1.SelectItem key={type.value} value={type.value}>
                        {type.label}
                      </select_1.SelectItem>); })}
                  </select_1.SelectContent>
                </select_1.Select>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              {filteredAppointments.length === 0 ? (<div className="text-center py-12">
                  ...
                  {filteredAppointments.map(function (appointment) { return (<AppointmentCard key={appointment.id} appointment={appointment} onEdit={function (id) { return console.log('Edit', id); }} onCancel={function (id) { return console.log('Cancel', id); }}/>); })}
                </div>)}
            </card_1.CardContent>
          </card_1.Card>
        </div>
...
function AppointmentCard(_a) {
    var appointment = _a.appointment, onEdit = _a.onEdit, onCancel = _a.onCancel;
    var typeConfig = getTypeConfig(appointment.type);
    var statusConfig = statusConfig[appointment.status];
    return (<div className="p-4 rounded-lg border bg-card hover:shadow-md transition-all duration-200">
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-4 flex-1">
          <avatar_1.Avatar className="w-12 h-12">
            ...
            <avatar_1.AvatarFallback className="bg-neon-100 text-neon-700">
              {appointment.patientName.split(' ').map(function (n) { return n[0]; }).join('')}
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <badge_1.Badge variant="outline" className={statusConfig === null || statusConfig === void 0 ? void 0 : statusConfig.color}>
                  <StatusIcon status={appointment.status}/>
                  <span className="ml-1">{statusConfig === null || statusConfig === void 0 ? void 0 : statusConfig.label}</span>
                </badge_1.Badge>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
              ...
              <div className="flex items-center">
                {appointment.consultationType === "telemedicina" ? (<lucide_react_1.Video className="w-4 h-4 mr-1"/>) : (<lucide_react_1.MapPin className="w-4 h-4 mr-1"/>)}
            ...
            {(appointment.symptoms || appointment.notes) && (<div className="mt-3 p-3 bg-muted/50 rounded-md">
                <p className="text-sm">
        ...
        <div className="flex items-center space-x-2 ml-4">
          <button_1.Button variant="ghost" size="sm" onClick={function () { return onEdit(appointment.id); }} className="text-neon-600 hover:text-neon-700 hover:bg-neon-50">
            <lucide_react_1.Edit className="w-4 h-4"/>
          </button_1.Button>
          <button_1.Button variant="ghost" size="sm" onClick={function () { return onCancel(appointment.id); }} className="text-red-600 hover:text-red-700 hover:bg-red-50">
    ...
    return (<form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input_1.Input id="patientName" value={formData.patientName} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { patientName: e.target.value })); }} required/>
        </div>
        <div>
          <label_1.Label htmlFor="patientPhone">Telefone</label_1.Label>
          <input_1.Input id="patientPhone" value={formData.patientPhone} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { patientPhone: e.target.value })); }} required/>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <input_1.Input id="date" type="date" value={formData.date} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { date: e.target.value })); }} required/>
        </div>
        <div>
          <label_1.Label htmlFor="time">Horário</label_1.Label>
          <input_1.Input id="time" type="time" value={formData.time} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { time: e.target.value })); }} required/>
        </div>
        <div>
          <label_1.Label htmlFor="duration">Duração (min)</label_1.Label>
          <select_1.Select value={formData.duration} onValueChange={function (value) { return setFormData(__assign(__assign({}, formData), { duration: value })); }}>
            <select_1.SelectTrigger>
      ...
      <div>
        ...
        <textarea_1.Textarea id="symptoms" value={formData.symptoms} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { symptoms: e.target.value })); }} placeholder="Descreva os sintomas ou motivo da consulta..."/>
      </div>
