...
var PatientCard = function (_a) {
    var patient = _a.patient, index = _a.index;
    ...
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} whileHover={{
      ...
      <NeonGradientCard_1.NeonGradientCard gradient={planInfo.gradient} className="group">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className={"px-3 py-1 rounded-full bg-".concat(planInfo.color, "/20 border border-").concat(planInfo.color, "/30")}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 text-gray-300">
              ...
              <span className="text-sm">Última visita: {(0, utils_1.formatDate)(patient.lastVisit)}</span>
            </div>
            <div className="flex items-center space-x-2 text-gray-300">
        ...
        <div className="flex space-x-2">
          <CosmicGlowButton_1.CosmicGlowButton variant="primary" size="sm" className="flex-1 flex items-center justify-center">
            <lucide_react_1.Eye className="h-4 w-4 mr-1"/>
            Visualizar
          </CosmicGlowButton_1.CosmicGlowButton>
          <CosmicGlowButton_1.CosmicGlowButton variant="secondary" size="sm" className="flex-1 flex items-center justify-center">
            <lucide_react_1.Edit className="h-4 w-4 mr-1"/>
            Editar
          </CosmicGlowButton_1.CosmicGlowButton>
          <CosmicGlowButton_1.CosmicGlowButton variant="success" size="sm" className="flex-1 flex items-center justify-center">
...
var StatCard = function (_a) {
    var Icon = _a.icon, title = _a.title, value = _a.value, gradient = _a.gradient, subtitle = _a.subtitle;
...
function Pacientes() {
    ...
    var _c = (0, react_1.useState)('todos'), filterPlan = _c[0], setFilterPlan = _c[1];
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 animate-background-position-spin">
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard icon={lucide_react_1.Users} title="Total de Pacientes" value={pacientesData.stats.total} gradient="primary" subtitle="Registrados no sistema"/>
          <StatCard icon={lucide_react_1.UserPlus} title="Pacientes Ativos" value={pacientesData.stats.active} gradient="success" subtitle="Com consultas recentes"/>
          <StatCard icon={lucide_react_1.Activity} title="Pacientes Inativos" value={pacientesData.stats.inactive} gradient="danger" subtitle="Sem consultas há 3+ meses"/>
          <StatCard icon={lucide_react_1.Calendar} title="Novos Este Mês" value={pacientesData.stats.newThisMonth} gradient="accent" subtitle="Cadastrados em janeiro"/>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          <div className="md:col-span-2">
            <NeonGradientCard_1.NeonGradientCard gradient="secondary">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar por nome, email ou telefone..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-accent"/>
              </div>
            </NeonGradientCard_1.NeonGradientCard>
          </div>
          ...
          <NeonGradientCard_1.NeonGradientCard gradient="warning">
            <div className="relative">
              ...
              <select value={filterPlan} onChange={function (e) { return setFilterPlan(e.target.value); }} className="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-accent appearance-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <framer_motion_1.AnimatePresence>
            {filteredPatients.map(function (patient, index) { return (<PatientCard key={patient.id} patient={patient} index={index}/>); })}
          </framer_motion_1.AnimatePresence>
        </div>

        {filteredPatients.length === 0 && (<framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
