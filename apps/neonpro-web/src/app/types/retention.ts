...
export interface PatientRetentionAnalytics {
  ...
  risk_factors: string[];
  retention_interventions: InterventionRecord[];
...
export interface RetentionDashboardData {
  ...
  recent_predictions: PatientChurnPrediction[];
  active_interventions: RetentionIntervention[];
  campaign_performance: RetentionCampaignAnalytics[];
  ltv_segments: {
    segment: string;
    count: number;
    avg_ltv: number;
  ...
  model_performance: RetentionModelPerformance[];
