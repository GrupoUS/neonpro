...
var NeonGradientCard = function (_a) {
    ...
    return (<div ref={containerRef} style={{
        ...
        }} className={(0, utils_1.cn)("relative z-10 size-full rounded-[var(--border-radius)]", className)}>
      <div className={(0, utils_1.cn)("relative size-full min-h-[inherit] rounded-[var(--card-content-radius)] bg-background p-6", "before:absolute before:-left-[var(--border-size)] before:-top-[var(--border-size)] before:-z-10 before:block", "before:h-[var(--pseudo-element-height)] before:w-[var(--pseudo-element-width)] before:rounded-[var(--border-radius)] before:content-['']", "before:bg-[linear-gradient(0deg,var(--neon-first-color),var(--neon-second-color))] before:bg-[length:100%_200%]", "before:animate-[background-position-spin_3s_infinite_alternate]", "after:absolute after:-left-[var(--border-size)] after:-top-[var(--border-size)] after:-z-10 after:block", "after:h-[var(--pseudo-element-height)] after:w-[var(--pseudo-element-width)] after:rounded-[var(--border-radius)] after:blur-[var(--after-blur)] after:content-['']", "after:bg-[linear-gradient(0deg,var(--neon-first-color),var(--neon-second-color))] after:bg-[length:100%_200%] after:opacity-80", "after:animate-[background-position-spin_3s_infinite_alternate]")}>
        {children}
      </div>
    </div>);
};
...
function ThemeDemoPage() {
    var _a = (0, react_1.useState)(null), activeMetric = _a[0], setActiveMetric = _a[1];
    var metrics = [
        {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            return (<NeonGradientCard key={metric.title} className="h-48" neonColors={metric.neonColors}>
                  <div className="h-full flex flex-col justify-between cursor-pointer transition-all duration-300 hover:scale-105" onMouseEnter={function () { return setActiveMetric(metric.title); }} onMouseLeave={function () { return setActiveMetric(null); }}>
                    <div className="flex items-center justify-between mb-4">
                      <div className="p-3 rounded-xl bg-primary/10">
                        <Icon className="w-6 h-6 text-primary"/>
                      </div>
                      <badge_1.Badge variant={metric.trend === 'up' ? 'default' : 'destructive'} className="text-xs">
                        {metric.change}
                      </badge_1.Badge>
                    </div>
                    <div>
                    ...
                    {activeMetric === metric.title && (<div className="mt-4 flex items-center gap-2">
