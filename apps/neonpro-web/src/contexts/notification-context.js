...
function NotificationProvider(_a) {
    var _this = this;
    var children = _a.children, userId = _a.userId, _b = _a.disabled, disabled = _b === void 0 ? false : _b;
    ...
    var notifications = notificationHook.notifications, unreadCount = notificationHook.unreadCount, preferences = notificationHook.preferences, isLoading = notificationHook.isLoading, error = notificationHook.error, markAsRead = notificationHook.markAsRead, markAllAsRead = notificationHook.markAllAsRead, deleteNotification = notificationHook.deleteNotification, updatePreferences = notificationHook.updatePreferences, refreshNotifications = notificationHook.refreshNotifications, getNotificationsByType = notificationHook.getNotificationsByType, getUnreadNotifications = notificationHook.getUnreadNotifications, hasUnreadNotifications = notificationHook.hasUnreadNotifications;
    ...
    var showNotificationToast = function (notification) {
        var toastProps = {
            ...
            action: notification.action_url ? {
                label: 'Ver',
                onClick: function () { return window.location.href = notification.action_url; }
            } : undefined
        };
        switch (notification.type) {
            case 'appointment_confirmed':
                sonner_1.toast.success(notification.title, __assign({ description: notification.message }, toastProps));
                break;
            case 'appointment_cancelled':
                sonner_1.toast.error(notification.title, __assign({ description: notification.message }, toastProps));
                break;
            case 'appointment_reminder':
                sonner_1.toast.info(notification.title, __assign({ description: notification.message }, toastProps));
                break;
            ...
            case 'system':
                ...
                break;
            default:
                (0, sonner_1.toast)(notification.title, __assign({ description: notification.message }, toastProps));
        }
    };
    ...
    var sendPushNotification = function (title, options) {
        ...
        var defaultOptions = __assign({ icon: '/icon-192x192.png', badge: '/icon-192x192.png', tag: 'neonpro-notification', requireInteraction: false }, options);
        try {
    ...
    (0, react_1.useEffect)(function () {
        if (!disabled && notifications.length > 0) {
            ...
            var isNewNotification = !latestNotification.read_at &&
                new Date(latestNotification.created_at).getTime() > Date.now() - 5000; // Created in last 5 seconds
            if (isNewNotification) {
                // Show toast notification
                if ((preferences === null || preferences === void 0 ? void 0 : preferences.push_enabled) !== false) {
                    showNotificationToast(latestNotification);
                }
                // Send browser push notification for high priority
                if (latestNotification.priority === 'high' && (preferences === null || preferences === void 0 ? void 0 : preferences.push_enabled)) {
    ...
    (0, react_1.useEffect)(function () {
        if (!disabled && (preferences === null || preferences === void 0 ? void 0 : preferences.push_enabled) && Notification.permission === 'default') {
    ...
    (0, react_1.useEffect)(function () {
        var handleFocus = function () {
            if (hasUnreadNotifications && (preferences === null || preferences === void 0 ? void 0 : preferences.push_enabled)) {
                // Auto-mark recent notifications as read when user focuses the app
                var recentNotifications = getUnreadNotifications().filter(function (notif) {
                    return new Date(notif.created_at).getTime() > Date.now() - 30000;
                } // Last 30 seconds
                );
                recentNotifications.forEach(function (notif) {
                    setTimeout(function () { return markAsRead(notif.id); }, 2000);
                });
            }
        };
