...
function AccessibilityProvider(_a) {
    ...
    var _b = (0, react_1.useState)(false), isHighContrast = _b[0], setIsHighContrast = _b[1];
    var _c = (0, react_1.useState)(false), reduceMotion = _c[0], setReduceMotion = _c[1];
    var _d = (0, react_1.useState)('normal'), fontSize = _d[0], setFontSize = _d[1];
    ...
    var trapFocus = function (container) {
        var focusableElements = container.querySelectorAll('button:not([disabled]), [href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');
    ...
    var handleFontSizeChange = function (newSize) {
        ...
        announceToScreenReader("Tamanho da fonte alterado para ".concat(newSize === 'normal' ? 'normal' : newSize === 'large' ? 'grande' : 'extra grande'));
    };
    var value = {
    ...
    return (<AccessibilityContext.Provider value={value}>
      ...
      <div ref={announceRef} aria-live="polite" aria-atomic="true" className="sr-only" role="status"/>
...
function useKeyboardNavigation() {
    var handleArrowNavigation = function (e, items, currentIndex, onIndexChange, options) {
        ...
        var _e = options.horizontal, horizontal = _e === void 0 ? false : _e, _f = options.wrap, wrap = _f === void 0 ? true : _f, enterAction = options.enterAction;
        switch (e.key) {
            case horizontal ? 'ArrowLeft' : 'ArrowUp':
                e.preventDefault();
                var prevIndex = currentIndex > 0 ? currentIndex - 1 : wrap ? items.length - 1 : currentIndex;
                onIndexChange(prevIndex);
                (_a = items[prevIndex]) === null || _a === void 0 ? void 0 : _a.focus();
                break;
            case horizontal ? 'ArrowRight' : 'ArrowDown':
                e.preventDefault();
                var nextIndex = currentIndex < items.length - 1 ? currentIndex + 1 : wrap ? 0 : currentIndex;
