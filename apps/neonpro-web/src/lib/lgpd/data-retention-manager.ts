...
import { auditTrailManager, LGPDAuditEventType, LGPDAuditSeverity } from './audit-trail-manager';
...
export interface RetentionAnalytics {
  ...
  retentionByDataType: Record<LGPDDataType, {
    total: number;
    expiring: number;
    expired: number;
  }>;
...
export class DataRetentionManager {
  ...
  async registerDataForRetention(
    ...
    try {
      ...
      if (!policy) {
        ...
        return this.registerDataForRetention(dataSubjectId, clinicId, dataType, purpose, dataCreatedAt, customRetentionDays);
      }

      const retentionPeriodDays = customRetentionDays || policy.retentionPeriodDays;
      const retentionExpiresAt = new Date(dataCreatedAt.getTime() + retentionPeriodDays * 24 * 60 * 60 * 1000);
      ...
      logger.info(`Data registered for retention: ${data.id} expires ${retentionExpiresAt.toISOString()}`);
      return data;
      ...
      for (const record of records) {
        ...
        if (daysUntilExpiration <= policy.notificationDaysBefore && daysUntilExpiration > 0) {
          ...
          if (!lastNotification || (now.getTime() - lastNotification.getTime()) > 24 * 60 * 60 * 1000) {
            ...
            await this.updateRetentionRecordStatus(record.id, 'expiring_soon', { lastNotificationSent: now });
            notificationsSent++;
          }
        }
      }
  ...
  async getRetentionAnalytics(
    ...
    try {
      ...
      // Group by data type
      const retentionByDataType = {} as Record<LGPDDataType, {
        total: number;
        expiring: number;
        expired: number;
      }>;
      ...
      const upcomingExpirations = activeRecords
        ...
        .sort((a, b) => new Date(a.retentionExpiresAt).getTime() - new Date(b.retentionExpiresAt).getTime())
        .slice(0, 20);
      ...
      const retentionCompliance = totalManagedRecords > 0 
  ...
  private async sendExpirationNotification(
    ...
    try {
      ...
      logger.info(`Sending expiration notification for record ${record.id}: ${daysUntilExpiration} days remaining`);
