...
import type { AuditLogger, DataProcessingActivity } from "./audit-logger";
...
export class DataSubjectRightsManager {
   ...
   */
  static async getOverdueRequests(): Promise<
    Array<{
  ...
  private static async getProcessingActivities(requestId: string): Promise<
    Array<{
      purpose: string;
      legalBasis: string;
      dataCategories: string[];
  ...
  private static async getAuditTrail(requestId: string): Promise<
    Array<{
      timestamp: Date;
      activity: string;
      purpose: string;
  ...
  private static async generateSecureDownloadLink(data: any, expiresAt: Date): Promise<string> {
