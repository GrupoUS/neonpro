...
export class ConsentManager {
  ...
  static async generateConsentReport(userId: string): Promise<{
    ...
    return {
      ...
      thirdPartySharing: consents
        ...
        .flatMap(c => c.thirdParties!.map(tp => ({
          partner: tp.name,
          purpose: tp.purpose,
   ...
   */
  static async checkExpiringConsents(daysBeforeExpiry: number = 30): Promise<Array<{
    userId: string
    consentType: ConsentType
    expiresAt: Date
...
export const HEALTHCARE_CONSENT_PURPOSES = {
  [ConsentType.DATA_PROCESSING]: 'Processamento de dados pessoais para prestação de serviços médicos e administrativos',
  [ConsentType.SENSITIVE_DATA]: 'Tratamento de dados sensíveis de saúde para diagnóstico, tratamento e acompanhamento médico',
  [ConsentType.MARKETING]: 'Envio de comunicações promocionais sobre tratamentos e serviços da clínica',
  [ConsentType.DATA_SHARING]: 'Compartilhamento de dados com laboratórios, outros profissionais de saúde e planos de saúde',
  [ConsentType.PHOTO_VIDEO]: 'Captação e uso de imagens fotográficas ou videogrficas para documentação de tratamentos',
  [ConsentType.RESEARCH]: 'Uso de dados anonimizados para pesquisas científicas e melhoria de tratamentos',
  [ConsentType.COOKIES]: 'Uso de cookies e tecnologias similares no website da clínica',
  [ConsentType.BIOMETRIC]: 'Coleta e processamento de dados biométricos para identificação e segurança'
}
