...
export class AuditTrailManager {
  ...
  async getAuditAnalytics(
    ...
    try {
      ...
      const topDataTypes = Object.entries(dataTypeCounts)
        .map(([dataType, count]) => ({ dataType: dataType as LGPDDataType, count }))
  ...
  async generateComplianceReport(
    ...
    try {
      const analytics = await this.getAuditAnalytics(clinicId, startDate, endDate);
  ...
  private calculateComplianceScore(analytics: AuditTrailAnalytics): number {
    ...
    const overdueRequestsPenalty = Math.max(0, analytics.dataSubjectRequests.pending - 5) * 2;
   ...
   */
  private generateComplianceRecommendations(analytics: AuditTrailAnalytics): string[] {
