...
export class LGPDComplianceManager {
  ...
  async getDashboardMetrics(): Promise<LGPDMetrics> {
    try {
      const { data, error } = await this.supabase.rpc("get_lgpd_dashboard_metrics");
  ...
  async getBreachIncidents(
    ...
    try {
      ...
      query = query.order("discovered_at", { ascending: false }).range(offset, offset + limit - 1);
  ...
  async getComplianceAssessments(
    filters: { assessment_type?: string; status?: string; page?: number; limit?: number } = {},
  ) {
    try {
  ...
  async runAutomatedAssessment() {
    try {
      ...
      const { data, error } = await this.supabase.rpc("run_compliance_assessment");
  ...
  // Utility Methods
  async getAuditStatistics(filters: { start_date?: string; end_date?: string } = {}) {
    try {
      let query = this.supabase.from("lgpd_audit_trail").select("event_type, action");
