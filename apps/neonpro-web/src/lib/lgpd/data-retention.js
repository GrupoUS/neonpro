...
exports.dataRetentionManager = exports.DataRetentionManager = exports.LegalHoldStatus = exports.DeletionMethod = exports.RetentionTrigger = exports.RetentionUnit = exports.DataCategory = void 0;
...
var DataRetentionManager = /** @class */ (function (_super) {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        _a.label = 2;
                    case 2:
                        policy = __assign(__assign({}, policyData), { id: this.generateId('policy'), createdAt: new Date(), updatedAt: new Date() });
                        // Validate policy
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        activeSchedules = Array.from(this.schedules.values())
                            .filter(function (s) { return s.policyId === policyId && s.status !== 'completed' && s.status !== 'cancelled'; });
                        if (activeSchedules.length > 0) {
                            throw new Error("Cannot delete policy with ".concat(activeSchedules.length, " active schedules"));
                        }
     ...
     */
    DataRetentionManager.prototype.scheduleDataRetention = function (dataIdentifier, dataLocation, category, creationDate, metadata) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        _a.label = 2;
                    case 2:
                        hold = __assign(__assign({}, holdData), { id: this.generateId('hold'), affectedSchedules: [], notifications: [], createdAt: new Date(), updatedAt: new Date() });
                        affectedSchedules = Array.from(this.schedules.values())
                            .filter(function (s) {
                            return (hold.dataCategories.includes(s.category) ||
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        _i++;
                        return [3 /*break*/, 1];
                    case 4: return [4 /*yield*/, this.saveLegalHold(hold)];
                    case 5:
                        _b.sent();
    ...
    DataRetentionManager.prototype.processScheduledDeletions = function () {
        return __awaiter(this, void 0, void 0, function () {
            var now, gracePeriod, readySchedules, processed, successful, failed, skipped, _i, readySchedules_1, schedule, hold, result, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!!this.isInitialized) return [3 /*break*/, 2];
                    ...
                    case 2:
                        now = new Date();
                        gracePeriod = new Date(now.getTime() - (this.config.gracePeriodDays * 24 * 60 * 60 * 1000));
                        readySchedules = Array.from(this.schedules.values())
                            .filter(function (s) {
                            return s.status === 'scheduled' &&
                                s.scheduledDeletionDate <= gracePeriod;
                        })
                            .sort(function (a, b) { return a.scheduledDeletionDate.getTime() - b.scheduledDeletionDate.getTime(); })
                    ...
                    case 5:
                        ...
                        skipped++;
                        return [3 /*break*/, 11];
                    case 6: return [4 /*yield*/, this.executeDataDeletion(schedule)];
                    case 7:
                        result = _a.sent();
                        if (result.success) {
                    ...
                    case 12:
                        ...
                        return [2 /*return*/, { processed: processed, successful: successful, failed: failed, skipped: skipped }];
                }
            });
        });
    };
    ...
    DataRetentionManager.prototype.getRetentionSchedules = function (filters) {
        ...
        if (filters) {
            if (filters.status) {
                schedules = schedules.filter(function (s) { return s.status === filters.status; });
            }
            if (filters.category) {
                schedules = schedules.filter(function (s) { return s.category === filters.category; });
            }
            if (filters.policyId) {
                schedules = schedules.filter(function (s) { return s.policyId === filters.policyId; });
            }
        }
        return schedules.sort(function (a, b) { return a.scheduledDeletionDate.getTime() - b.scheduledDeletionDate.getTime(); });
    };
    /**
     * Get legal holds
     */
     ...
     */
    DataRetentionManager.prototype.generateRetentionReport = function (reportType, period, generatedBy) {
        return __awaiter(this, void 0, void 0, function () {
            var schedules, policies, holds, summary, policyCompliance, legalHoldStats, exceptions, recommendations, report;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        schedules = Array.from(this.schedules.values())
                            .filter(function (s) {
                            return s.createdAt >= period.startDate &&
                                s.createdAt <= period.endDate;
                        });
                        policies = Array.from(this.policies.values());
                        holds = Array.from(this.legalHolds.values())
                            .filter(function (h) {
                            return h.createdAt >= period.startDate &&
                                h.createdAt <= period.endDate;
                        });
                        summary = {
                            totalPolicies: policies.length,
                            activePolicies: policies.filter(function (p) { return p.isActive; }).length,
                            scheduledDeletions: schedules.filter(function (s) { return s.status === 'scheduled'; }).length,
                            completedDeletions: schedules.filter(function (s) { return s.status === 'completed'; }).length,
                            failedDeletions: schedules.filter(function (s) { return s.status === 'failed'; }).length,
                            onHoldDeletions: schedules.filter(function (s) { return s.status === 'on_hold'; }).length,
                            dataVolume: {
                                scheduled: schedules
                                    .filter(function (s) { return s.status === 'scheduled'; })
                                    .reduce(function (sum, s) { return sum + (s.metadata.dataSize || 0); }, 0),
                                deleted: schedules
                                    .filter(function (s) { return s.status === 'completed'; })
                                    .reduce(function (sum, s) { return sum + (s.metadata.dataSize || 0); }, 0),
                                archived: schedules
                                    .filter(function (s) { return s.status === 'completed' && s.metadata.archived; })
                                    .reduce(function (sum, s) { return sum + (s.metadata.dataSize || 0); }, 0)
                            }
                        };
                        policyCompliance = policies.map(function (policy) {
                            var policySchedules = schedules.filter(function (s) { return s.policyId === policy.id; });
                            var completed = policySchedules.filter(function (s) { return s.status === 'completed'; });
                            var failed = policySchedules.filter(function (s) { return s.status === 'failed'; });
                            var processingTimes = completed
                                .filter(function (s) { return s.actualDeletionDate; })
                                .map(function (s) { return s.actualDeletionDate.getTime() - s.scheduledDeletionDate.getTime(); });
                            var averageProcessingTime = processingTimes.length > 0 ?
                                processingTimes.reduce(function (sum, time) { return sum + time; }, 0) / processingTimes.length :
                                0;
                            return {
                                policyId: policy.id,
                        ...
                        legalHoldStats = {
                            active: holds.filter(function (h) { return h.status === LegalHoldStatus.ACTIVE; }).length,
                            released: holds.filter(function (h) { return h.status === LegalHoldStatus.RELEASED; }).length,
                            affectedData: holds.reduce(function (sum, h) { return sum + h.affectedSchedules.length; }, 0)
                        };
                        exceptions = [
                            {
                                ...
                                details: schedules
                                    ...
                                    .map(function (s) { var _a; return "".concat(s.dataIdentifier, ": ").concat(((_a = s.executionLog[s.executionLog.length - 1]) === null || _a === void 0 ? void 0 : _a.details) || 'Unknown error'); })
                            },
                            {
                                type: 'Overdue Deletions',
                                count: schedules.filter(function (s) {
                                    return s.status === 'scheduled' &&
                                        s.scheduledDeletionDate < new Date(Date.now() - (_this.config.gracePeriodDays * 24 * 60 * 60 * 1000));
                                }).length,
                                details: schedules
                                    .filter(function (s) {
                                    return s.status === 'scheduled' &&
                                    ...
                                    .map(function (s) { return "".concat(s.dataIdentifier, ": ").concat(Math.floor((Date.now() - s.scheduledDeletionDate.getTime()) / (1000 * 60 * 60 * 24)), " days overdue"); })
                            }
                        ];
                        recommendations = this.generateRetentionRecommendations(summary, policyCompliance, exceptions);
                        report = {
    ...
    DataRetentionManager.prototype.findApplicablePolicy = function (category) {
        return Array.from(this.policies.values())
            .filter(function (p) { return p.category === category && p.isActive; })
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        ...
                        switch (_a) {
                            case DeletionMethod.SOFT_DELETE: return [3 /*break*/, 4];
                            case DeletionMethod.HARD_DELETE: return [3 /*break*/, 6];
                            case DeletionMethod.ANONYMIZATION: return [3 /*break*/, 8];
                            case DeletionMethod.PSEUDONYMIZATION: return [3 /*break*/, 10];
                            case DeletionMethod.ARCHIVAL: return [3 /*break*/, 12];
                        }
                        return [3 /*break*/, 14];
                    case 4: return [4 /*yield*/, this.softDeleteData(schedule)];
                    case 5:
                        _b.sent();
                        return [3 /*break*/, 14];
                    case 6: return [4 /*yield*/, this.hardDeleteData(schedule)];
                    case 7:
                        _b.sent();
                        return [3 /*break*/, 14];
                    case 8: return [4 /*yield*/, this.anonymizeData(schedule)];
                    case 9:
                        _b.sent();
                        return [3 /*break*/, 14];
                    case 10: return [4 /*yield*/, this.pseudonymizeData(schedule)];
                    case 11:
                        _b.sent();
                        return [3 /*break*/, 14];
                    case 12: return [4 /*yield*/, this.archiveData(schedule, policy)];
                    case 13:
                        _b.sent();
                        return [3 /*break*/, 14];
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        affectedSchedules = Array.from(this.schedules.values())
                            .filter(function (s) { return s.policyId === policy.id && s.status === 'scheduled'; });
                    ...
                    case 1:
                        ...
                        newDeletionDate = this.calculateDeletionDate(schedule.triggerDate, policy.retentionPeriod);
     ...
     */
    DataRetentionManager.prototype.generateRetentionRecommendations = function (summary, policyCompliance, exceptions) {
        var recommendations = [];
        // Check for failed deletions
        if (summary.failedDeletions > 0) {
            recommendations.push("Review and resolve ".concat(summary.failedDeletions, " failed deletion(s) to maintain compliance"));
        }
        // Check for low compliance rates
        var lowCompliancePolicies = policyCompliance.filter(function (p) { return p.complianceRate < 90; });
        if (lowCompliancePolicies.length > 0) {
            recommendations.push("Investigate ".concat(lowCompliancePolicies.length, " policy(ies) with compliance rates below 90%"));
        }
        // Check for long processing times
        var slowPolicies = policyCompliance.filter(function (p) { return p.averageProcessingTime > 7; });
        if (slowPolicies.length > 0) {
            recommendations.push("Optimize processing for ".concat(slowPolicies.length, " policy(ies) with average processing time > 7 days"));
        }
        // Check for overdue deletions
        var overdueException = exceptions.find(function (e) { return e.type === 'Overdue Deletions'; });
        if (overdueException && overdueException.count > 0) {
            recommendations.push("Address ".concat(overdueException.count, " overdue deletion(s) to prevent compliance violations"));
        }
    ...
    DataRetentionManager.prototype.startProcessingInterval = function () {
        ...
        this.processingInterval = setInterval(function () { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.processScheduledDeletions()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
    ...
    DataRetentionManager.prototype.startReviewInterval = function () {
        var _this = this;
        this.reviewInterval = setInterval(function () { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.performPeriodicReview()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 7, , 8]);
                        activeHolds = Array.from(this.legalHolds.values())
                            .filter(function (h) { return h.status === LegalHoldStatus.ACTIVE; });
                    ...
                    case 5:
                        ...
                        return [3 /*break*/, 1];
                    case 6:
                        outdatedPolicies = Array.from(this.policies.values())
                            .filter(function (p) {
                        ...
                        if (outdatedPolicies.length > 0) {
                            this.emit('retention:compliance_alert', {
                                alert: "".concat(outdatedPolicies.length, " retention policies may need review (not updated in over 1 year)"),
                                details: {
                                    policies: outdatedPolicies.map(function (p) { return ({ id: p.id, name: p.name, lastUpdate: p.updatedAt }); }),
                                    timestamp: new Date()
                                }
                            });
                        }
    ...
    DataRetentionManager.prototype.getHealthStatus = function () {
        ...
        var activePolicies = Array.from(this.policies.values()).filter(function (p) { return p.isActive; });
        if (activePolicies.length === 0) {
            issues.push('No active retention policies');
        }
        var failedSchedules = Array.from(this.schedules.values())
            .filter(function (s) { return s.status === 'failed'; }).length;
        if (failedSchedules > 10) {
            issues.push("High number of failed schedules: ".concat(failedSchedules));
        }
        var status = issues.length === 0 ? 'healthy' :
