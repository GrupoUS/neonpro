...
export class DataRetentionAutomation {
  ...
  private async validateRetentionPolicy(policy: any): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = []
  ...
  private async calculateAffectedRecords(policy: RetentionPolicy): Promise<{ count: number; sample: any[] }> {
    try {
  ...
  private async validateExecutionConditions(schedule: any): Promise<{ valid: boolean; reason?: string }> {
  ...
  private isWithinExecutionWindow(date?: Date): boolean {
    ...
    const withinHours = hour >= this.config.execution_window.start_hour && 
  ...
  private async createDataArchive(executionId: string, policy: RetentionPolicy): Promise<DataArchive> {
  ...
  private async performHardDelete(policy: RetentionPolicy): Promise<{ deleted_count: number; processed_count: number }> {
    const { data: result, error } = await this.supabase
  ...
  private async performSoftDelete(policy: RetentionPolicy): Promise<{ deleted_count: number; processed_count: number }> {
    const { data: result, error } = await this.supabase
  ...
  private async performAnonymization(policy: RetentionPolicy): Promise<{ anonymized_count: number; processed_count: number }> {
    const { data: result, error } = await this.supabase
