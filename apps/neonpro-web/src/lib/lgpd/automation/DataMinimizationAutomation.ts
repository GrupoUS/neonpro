...
export interface DataMinimizationRule {
  ...
  table_name: string
  column_name: string
  data_category: 'personal' | 'sensitive' | 'biometric' | 'health' | 'financial' | 'location' | 'behavioral'
  minimization_type: 'anonymization' | 'pseudonymization' | 'aggregation' | 'deletion' | 'masking' | 'encryption'
...
export class DataMinimizationAutomation {
   ...
   */
  async startDataMinimization(analysisIntervalHours: number = 24): Promise<void> {
    try {
      ...
      console.log(`Data minimization automation started (${analysisIntervalHours}h intervals)`)
    } catch (error) {
  ...
  async createMinimizationRule(
    ...
    try {
      ...
      if (!validation.valid) {
        throw new Error(`Invalid minimization rule: ${validation.errors.join(', ')}`)
      }
  ...
  async scheduleMinimizationTask(
    ...
    try {
      ...
      const executionPlan = await this.generateExecutionPlan(rule, affectedRecords)

      // Generate business impact assessment
      const businessImpact = await this.assessBusinessImpact(rule, affectedRecords)
      ...
      try {
        ...
        switch (task.minimization_method) {
          ...
          case 'encryption':
            ...
            break
          default:
            throw new Error(`Unsupported minimization method: ${task.minimization_method}`)
        }
        ...
        for (const callback of this.minimizationCallbacks) {
          try {
            callback({ ...task, status: 'completed', completed_at: new Date().toISOString() })
          } catch (error) {
   ...
   * Get Data Inventory
   */
  async getDataInventory(includeAnalysis: boolean = true): Promise<DataInventory[]> {
    try {
  ...
  async analyzeMinimizationOpportunities(
    ...
    try {
      ...
      throw new Error(`Failed to analyze minimization opportunities: ${error.message}`)
    }
  }
    ...
    try {
      ...
      for (const task of tasks) {
        try {
          ...
          console.error(`Error executing scheduled task ${task.id}:`, taskError)
        }
      }
    } catch (error) {
  ...
  private async validateMinimizationRule(rule: any): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = []
  ...
  private async analyzeAffectedRecords(rule: DataMinimizationRule): Promise<{ count: number; sample: any[] }> {
    const { data: analysis, error } = await this.supabase
  ...
  private async generateExecutionPlan(rule: DataMinimizationRule, affectedRecords: any): Promise<any> {
    return {
  ...
  private async assessBusinessImpact(rule: DataMinimizationRule, affectedRecords: any): Promise<any> {
    return {
  ...
  private async generateRollbackPlan(rule: DataMinimizationRule, executionPlan: any): Promise<any> {
  ...
  private async executePseudonymization(task: MinimizationTask): Promise<number> {
  ...
  private estimateExecutionDuration(recordCount: number, minimizationType: string): number {
  ...
  private calculateStorageSavings(recordCount: number, minimizationType: string): string {
    ...
    const savings = recordCount * avgRecordSize * (savingsMultiplier[minimizationType] || 0)
