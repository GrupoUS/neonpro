...
export class DataMinimizationAutomation {
  ...
  async getDataInventory(includeAnalysis: boolean = true): Promise<DataInventory[]> {
    try {
      const { data: inventory, error } = await this.supabase.rpc("get_data_inventory", {
   ...
   */
  async analyzeMinimizationOpportunities(tableName?: string): Promise<MinimizationAnalysis[]> {
    try {
  ...
  async getMinimizationDashboard(): Promise<{
    ...
    try {
      const { data: dashboard, error } = await this.supabase.rpc("get_minimization_dashboard");
  ...
  private async analyzeAffectedRecords(
    ...
    const { data: analysis, error } = await this.supabase.rpc("analyze_affected_records", {
  ...
  private async executeAnonymization(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_anonymization", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
  ...
  private async executePseudonymization(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_pseudonymization", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
      execution_plan: task.execution_plan,
  ...
  private async executeAggregation(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_aggregation", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
      execution_plan: task.execution_plan,
  ...
  private async executeDeletion(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_secure_deletion", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
      execution_plan: task.execution_plan,
  ...
  private async executeMasking(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_data_masking", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
      execution_plan: task.execution_plan,
  ...
  private async executeEncryption(task: MinimizationTask): Promise<number> {
    ...
    const { data: result, error } = await this.supabase.rpc("execute_field_encryption", {
      task_id: task.id,
      table_name: task.target_table,
      columns: task.target_columns,
      execution_plan: task.execution_plan,
