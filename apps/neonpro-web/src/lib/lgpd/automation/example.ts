...
export class LGPDAutomationService {
  ...
  private setupMonitoringCallbacks(): void {
    ...
    this.orchestrator.onAlert((alert) => {
      console.log(`🚨 Alerta ${alert.alert_type.toUpperCase()}: ${alert.title}`)
    ...
    this.orchestrator.onStatusChange((statuses) => {
      ...
      if (errorModules.length > 0) {
        console.warn('⚠️ Módulos com erro:', errorModules.map(m => m.module))
      }
    })
  }
...
export class LGPDUsageExamples {
   ...
   */
  async collectUserConsent(userId: string, purpose: string, ipAddress: string, userAgent: string) {
   ...
   */
  async generateComplianceReport(reportType: 'daily' | 'weekly' | 'monthly' | 'quarterly') {
    const modules = this.service.getModules()
...
export const lgpdApiExamples = {
  ...
  async collectConsent(req: any, res: any) {
    try {
      ...
      const consent = await examples.collectUserConsent(userId, purpose, ipAddress, userAgent)
