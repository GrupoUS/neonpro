...
export class RealTimeComplianceMonitor {
  ...
  async performComplianceCheck(): Promise<{
    ...
    try {
      ...
      if (newAlerts.length > 0) {
        const { error: alertsError } = await this.supabase.from("lgpd_compliance_alerts").insert(
          newAlerts.map((alert) => ({
            type: alert.type,
  ...
  async generateComplianceReport(
    ...
    try {
      ...
      const { data: reportData, error } = await this.supabase.rpc("generate_compliance_report", {
  ...
  private async calculateComplianceMetrics(): Promise<ComplianceMetrics> {
    try {
      const { data: metrics, error } = await this.supabase.rpc("calculate_compliance_metrics");
    ...
    try {
      ...
      const { data: expiringConsents, error } = await this.supabase
        ...
        .lt(
          "expires_at",
          new Date(
            Date.now() + this.config.alert_thresholds.consent_expiry_days * 24 * 60 * 60 * 1000,
          ).toISOString(),
        );
    ...
    try {
      ...
      const { data: breachIncidents, error } = await this.supabase
        ...
        .lt(
          "created_at",
          new Date(
            Date.now() - this.config.alert_thresholds.breach_response_hours * 60 * 60 * 1000,
          ).toISOString(),
        );
  ...
  private async getComplianceTrends(): Promise<any> {
    const { data: trends, error } = await this.supabase.rpc("get_compliance_trends", {
      days_back: 30,
    });
  ...
  private async getUpcomingLegalDeadlines(): Promise<any[]> {
    const { data: deadlines, error } = await this.supabase.rpc("get_upcoming_legal_deadlines");
