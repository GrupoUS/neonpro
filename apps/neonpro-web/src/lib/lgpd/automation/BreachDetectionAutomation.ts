...
export class BreachDetectionAutomation {
  ...
  async getBreachDashboard(): Promise<{
    ...
    try {
      const { data: dashboard, error } = await this.supabase.rpc("get_breach_dashboard");
  ...
  private async executeDetectionRule(rule: BreachDetectionRule): Promise<void> {
    try {
      ...
      const { data: detectionResult, error } = await this.supabase.rpc("execute_detection_rule", {
        rule_id: rule.id,
...
Equipe de Segurança
  ...
  private async getAffectedUsers(incidentId: string): Promise<any[]> {
    const { data: users, error } = await this.supabase.rpc("get_affected_users_by_incident", {
      incident_id: incidentId,
    });
