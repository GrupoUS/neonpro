...
export class ConsentAutomationManager {
  ...
  async generateConsentAnalytics(dateRange: {
    ...
    try {
      const { data: analytics, error } = await this.supabase.rpc("generate_consent_analytics", {
      ...
      for (const childUserId of childUserIds) {
        for (const parentConsent of parentConsents || []) {
          try {
            ...
            if (shouldInherit) {
              await this.collectConsentWithTracking(childUserId, parentConsent.purpose_id, true, {
  ...
  private async autoWithdrawExpiredConsent(renewal: ConsentRenewalTask): Promise<void> {
    await this.processConsentWithdrawal(renewal.user_id, renewal.purpose_id, "automatic_expiry", {
      immediate: true,
    });
  }
  ...
  private async immediateDataProcessingCessation(userId: string, purposeId: string): Promise<void> {
