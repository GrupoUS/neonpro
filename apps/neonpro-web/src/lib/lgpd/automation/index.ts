...
export class LGPDAutomationOrchestrator {
  ...
  async getAutomationStatus(): Promise<AutomationStatus[]> {
    try {
      const { data: statusData, error } = await this.supabase.rpc("get_automation_status");
  ...
  async getAutomationMetrics(): Promise<AutomationMetrics> {
    try {
      const { data: metrics, error } = await this.supabase.rpc("get_automation_metrics");
  ...
  async getAutomationAlerts(
    ...
    try {
      let query = this.supabase.from("lgpd_automation_alerts").select("*", { count: "exact" });
