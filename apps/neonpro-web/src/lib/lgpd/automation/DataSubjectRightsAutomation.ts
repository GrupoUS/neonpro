...
export class DataSubjectRightsAutomation {
  ...
  async processDataRectificationRequest(
    ...
    try {
      ...
      await this.supabase.from("lgpd_rectification_log").insert({
        request_id: requestId,
        user_id: userId,
  ...
  async processDataPortabilityRequest(
    ...
    try {
      ...
      const downloadUrl = await this.createSecureDownloadLink(portabilityPackage, deliveryMethod);

      // Update package with download URL
      portabilityPackage.download_url = downloadUrl;

      // Store portability record
      await this.supabase.from("lgpd_portability_packages").insert({
        request_id: requestId,
        user_id: userId,
        format: exportFormat,
  ...
  private async generateDataAccessReport(
    ...
    // Generate comprehensive data access report
    const { data: reportData, error } = await this.supabase.rpc("generate_data_access_report", {
  ...
  private async analyzeDataForErasure(
    ...
    const { data: analysis, error } = await this.supabase.rpc("analyze_data_for_erasure", {
      target_user_id: userId,
  ...
  private async executeSecureDataErasure(
    ...
    const { data: result, error } = await this.supabase.rpc("execute_secure_data_erasure", {
      target_user_id: userId,
  ...
  private async generatePortableDataPackage(
    ...
    // Generate portable data package
    const { data: packageData, error } = await this.supabase.rpc("generate_portable_data_package", {
      target_user_id: userId,
      request_id: requestId,
  ...
  private async createSecureDownloadLink(
    ...
    // Create secure download link
    const { data: link, error } = await this.supabase.rpc("create_secure_download_link", {
