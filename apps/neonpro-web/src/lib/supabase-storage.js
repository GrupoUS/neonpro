...
exports.checkPatientPhotoConsent =
  exports.getPatientPhotoStats =
  exports.createPhotoSignedUrl =
  exports.downloadPatientPhoto =
  exports.deletePatientPhoto =
  exports.getPatientPhotos =
  exports.uploadMultiplePatientPhotos =
...
Promise <
  Blob >
  {
    const:
      ((_a = options.maxWidth),
      (maxWidth = _a === void 0 ? 1920 : _a),
      (_b = options.maxHeight),
      (maxHeight = _b === void 0 ? 1080 : _b),
      (_c = options.quality),
      (quality = _c === void 0 ? 0.8 : _c),
      (_d = options.format),
...
string;
{
  ...
  var fileExt =
    ((_o = fileName.split(".").pop()) === null || _o === void 0 ? void 0 : _o.toLowerCase()) ||
    "jpg";
...
var _p = await this.supabase.storage.from(this.BUCKET_NAME).upload(filePath, compressedBlob, {
    contentType: file.type,
    cacheControl: "3600",
    upsert: false,
  }),
  uploadData = _p.data,
  uploadError = _p.error;
if (uploadError) {
...
if (dbError) {
  ...
  await this.supabase.storage.from(this.BUCKET_NAME).remove([uploadData.path]);
  console.error("Database insert error:", dbError);
  return { success: false, error: 'Erro ao salvar metadados' };
}
// Get public URL
var urlData = (
  await this.supabase.storage.from(this.BUCKET_NAME).createSignedUrl(uploadData.path, 3600)
).data; // 1 hour
return {
    success: true,
    data: {
...
(0, exports.getPatientPhotos)(
  ...
  (number = _g.page),
  (number = _g.limit),
  ('before' | 'after' | 'during' = _g.category),
  (string = _g.treatmentType),
  _g),
);
{
  var _r = options.page,
          ...
          return __generator(this, function (_a) {
            switch (_a.label) {
              ...
              case 1:
                ...
                return [
                  2 /*return*/,
                  __assign(__assign({}, photo), {
                    publicUrl:
                      (urlData === null || urlData === void 0 ? void 0 : urlData.signedUrl) || "",
...
Promise <
  { success: boolean, blob: Blob, error: string } >
  {
    try: {
      const:
        ((_l = await this.supabase.storage.from(this.BUCKET_NAME).download(filePath)),
        (data = _l.data),
        (error = _l.error),
        _l),
      if: (error) => {},
...
Promise <
  { success: boolean, url: string, error: string } >
  {
    try: {
      const:
        ((_m = await this.supabase.storage
          .from(this.BUCKET_NAME)
...
exports.uploadMultiplePatientPhotos = exports.storageManager.uploadMultiplePhotos.bind(
  exports.storageManager,
);
...
exports.getPatientPhotoStats = exports.storageManager.getPatientStorageStats.bind(
  exports.storageManager,
);
exports.checkPatientPhotoConsent = exports.storageManager.checkPhotoConsent.bind(
  exports.storageManager,
);
