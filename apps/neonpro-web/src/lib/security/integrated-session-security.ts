...
export class IntegratedSessionSecurity {
  ...
  static async initializeSessionSecurity(
    ...
    try {
      ...
      if (fullConfig.hijackingProtection.enabled) {
        await SessionHijackingProtection.storeSessionFingerprint(sessionId, userId, fingerprint);
      }

      // Initialize session timeout
      if (fullConfig.timeout.enabled) {
        await SessionTimeoutManager.initializeSessionTimeout(sessionId, userId, {
      ...
      if (fullConfig.concurrentSessions.enabled) {
        ...
        if (concurrentCheck.hasExcess && fullConfig.concurrentSessions.terminateOldest) {
          await SessionHijackingProtection.terminateSessions(concurrentCheck.sessionsToTerminate);
        }
      }
   ...
   * Get security configuration
   */
  private static async getSecurityConfig(sessionId: string): Promise<SecurityConfig> {
    try {
      const supabase = await createClient();
  ...
  static async cleanupSecurityData(): Promise<void> {
    try {
      ...
      await supabase.from("security_events").delete().lt("timestamp", ninetyDaysAgo);
    } catch (error) {
