 ...
 */
export async function intelligentSecurityMiddleware(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const threatAnalysis = ThreatDetection.analyzeSuspiciousActivity(ip, userAgent, route, method);

    // 3. Apply rate limiting with adaptive limits
    const limits = IntelligentRateLimit.getAdaptiveLimits(route, method);
    const rateLimitResult = IntelligentRateLimit.checkRateLimit(ip, route, limits);
