...
var CSRFProtection = /** @class */ (function () {
    ...
    CSRFProtection.createTokenHash = function (tokenData) {
        return (0, crypto_1.createHash)('sha256')
            .update("".concat(tokenData.token, ":").concat(tokenData.sessionId, ":").concat(tokenData.userAgent))
            .digest('hex');
    };
    /**
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        ...
                        csrfToken = request.headers.get(this.HEADER_NAME) ||
                            ((_a = request.cookies.get(this.COOKIE_NAME)) === null || _a === void 0 ? void 0 : _a.value);
                        if (!csrfToken) {
                            return [2 /*return*/, new server_1.NextResponse(JSON.stringify({ error: 'CSRF token required' }), { status: 403, headers: { 'Content-Type': 'application/json' } })];
                        }
                        sessionId = ((_b = request.cookies.get('session-id')) === null || _b === void 0 ? void 0 : _b.value) ||
                            request.headers.get('X-Session-ID');
                        if (!sessionId) {
                            return [2 /*return*/, new server_1.NextResponse(JSON.stringify({ error: 'Session ID required' }), { status: 403, headers: { 'Content-Type': 'application/json' } })];
                        }
                    ...
                    case 1:
                        validation = _c.sent();
                        if (!validation.valid) {
                            return [2 /*return*/, new server_1.NextResponse(JSON.stringify({ error: validation.error || 'Invalid CSRF token' }), { status: 403, headers: { 'Content-Type': 'application/json' } })];
                        }
