...
export class WebhookManager {
  ...
  async deleteWebhook(webhookId: string): Promise<void> {
    try {
      ...
      const { error } = await this.supabase.from("webhook_endpoints").delete().eq("id", webhookId);
  ...
  private async storeDeliveryRecord(delivery: EventDelivery): Promise<void> {
    try {
      const { error } = await this.supabase.from("event_deliveries").insert({
        id: delivery.id,
  ...
  private async processDeliveryQueue(): Promise<void> {
    ...
    // Process deliveries
    await Promise.all(readyDeliveries.map((delivery) => this.processDelivery(delivery)));
  }
  ...
  private async handleDeliveryFailure(delivery: EventDelivery): Promise<void> {
    if (delivery.attempts >= delivery.maxAttempts) {
      ...
      console.error(
        `❌ Delivery ${delivery.id} failed permanently after ${delivery.attempts} attempts`,
      );
    } else {
      delivery.status = "pending";
      ...
      console.log(
        `⏳ Delivery ${delivery.id} scheduled for retry in ${delayMs}ms (attempt ${delivery.attempts}/${delivery.maxAttempts})`,
      );
    }
  }
