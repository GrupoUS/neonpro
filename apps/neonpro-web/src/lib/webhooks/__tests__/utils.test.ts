...
describe("WebhookUtils", () => {
  ...
  describe("parseSignatureHeader", () => {
    ...
    it("should return null for invalid signature header", () => {
      const invalidHeaders = ["invalid-format", "sha256", "sha256=", "=signature-only"];
...
describe("RetryUtils", () => {
  ...
  describe("executeWithRetry", () => {
    ...
    it("should fail after max attempts", async () => {
      ...
      await expect(RetryUtils.executeWithRetry(mockFn, 2, strategy)).rejects.toThrow("ECONNRESET");
    ...
    it("should not retry non-retryable errors", async () => {
      ...
      await expect(RetryUtils.executeWithRetry(mockFn, 3, strategy)).rejects.toThrow(
        "ValidationError",
      );
