...
exports.MonitoringUtils = exports.ValidationUtils = exports.RetryUtils = exports.RateLimitUtils = exports.WebhookUtils = exports.EventUtils = void 0;
...
var EventUtils = /** @class */ (function () {
    ...
    EventUtils.sanitizeEventForWebhook = function (event, webhook) {
        var sanitized = {
            ...
            metadata: __assign(__assign({}, event.metadata), { 
                ...
                internalId: undefined, debugInfo: undefined }),
    ...
    EventUtils.transformEventForWebhook = function (event, webhook) {
        ...
        return __assign(__assign({}, basePayload), { webhook: {
                id: webhook.id,
                name: webhook.name,
...
var WebhookUtils = /** @class */ (function () {
    ...
    WebhookUtils.generateWebhookHeaders = function (event, webhook, payload, enableSignature) {
        if (enableSignature === void 0) { enableSignature = true; }
        var headers = __assign({ 'Content-Type': 'application/json', 'User-Agent': 'NeonPro-Webhook/1.0', 'X-Event-Type': event.type, 'X-Event-ID': event.id, 'X-Event-Timestamp': event.timestamp.toISOString(), 'X-Webhook-ID': webhook.id, 'X-Webhook-Name': webhook.name, 'X-Delivery-Attempt': '1' }, webhook.headers);
        // Add signature if enabled
        if (enableSignature && webhook.secret) {
            headers['X-Webhook-Signature'] = "sha256=".concat(this.generateSignature(payload, webhook.secret));
        }
        return headers;
    };
    /**
...
var RetryUtils = /** @class */ (function () {
    ...
    RetryUtils.isRetryableError = function (error, httpStatus) {
        ...
        if (error.name === 'AbortError' || ((_a = error.message) === null || _a === void 0 ? void 0 : _a.includes('timeout'))) {
            return true;
        }
        return false;
    };
...
var MonitoringUtils = /** @class */ (function () {
    ...
    MonitoringUtils.calculateAverageResponseTime = function (responseTimes) {
        if (responseTimes.length === 0)
            return 0;
        var sum = responseTimes.reduce(function (acc, time) { return acc + time; }, 0);
    ...
    MonitoringUtils.calculatePercentileResponseTime = function (responseTimes, percentile) {
        if (responseTimes.length === 0)
            return 0;
        var sorted = __spreadArray([], responseTimes, true).sort(function (a, b) { return a - b; });
    ...
    MonitoringUtils.generatePerformanceMetrics = function (deliveries) {
        ...
        var successful = deliveries.filter(function (d) { return d.status === 'delivered'; }).length;
        var failed = deliveries.filter(function (d) { return d.status === 'failed'; }).length;
        var responseTimes = deliveries
            .filter(function (d) { return d.responseTimeMs !== undefined; })
            .map(function (d) { return d.responseTimeMs; });
        return {
