...
export class SchedulingCommunicationWorkflow {
    ...
    try {
      ...
      if (workflowConfig.reminderSettings.enabled24h) {
        ...
        if (reminderTime24h > now) {
          workflows.push(
            await this.createReminderWorkflow(appointment, "24h", reminderTime24h, workflowConfig),
          );
        }
      }
      ...
      if (workflowConfig.reminderSettings.enabled2h) {
        ...
        if (reminderTime2h > now) {
          workflows.push(
            await this.createReminderWorkflow(appointment, "2h", reminderTime2h, workflowConfig),
          );
        }
      }
      ...
      if (workflowConfig.reminderSettings.enabled30m) {
        ...
        if (reminderTime30m > now) {
          workflows.push(
            await this.createReminderWorkflow(appointment, "30m", reminderTime30m, workflowConfig),
          );
        }
      }
      ...
      if (workflowConfig.confirmationSettings.enableConfirmationRequests) {
        ...
        if (confirmationTime > now) {
          workflows.push(
            await this.createConfirmationWorkflow(appointment, confirmationTime, workflowConfig),
          );
        }
      }
      ...
      steps: [
        ...
        },
        {
          ...
          scheduledAt: new Date(
            scheduledTime.getTime() + config.confirmationSettings.timeoutHours * 60 * 60 * 1000,
          ),
  ...
  private async saveWorkflow(workflow: WorkflowExecution): Promise<void> {
    await this.supabase.from("communication_workflows").insert([
      {
        id: workflow.id,
