...
class SubscriptionCache {
  ...
  getStats() {
    ...
    return {
      ...
      oldestEntry:
        entries.length > 0 ? Math.min(...entries.map(([_, entry]) => entry.created)) : null,
      newestEntry:
        entries.length > 0 ? Math.max(...entries.map(([_, entry]) => entry.created)) : null,
    };
  }
}
...
export async function validateSubscriptionStatus(
  ...
  try {
    ...
    if (subscription.status === "active" || isTrialActive) {
      ...
      const isNearExpiry =
        expiresAt && expiresAt.getTime() - now.getTime() < 7 * 24 * 60 * 60 * 1000; // 7 days
