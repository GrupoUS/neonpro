  ...
  Object.entries(DETECTION_MODELS).forEach((_a) => {
    ...
    if (model.accuracy < PERFORMANCE_BENCHMARKS.minimumAccuracy) {
      errors.push(
        "Model "
          .concat(key, " accuracy ")
          .concat(model.accuracy, " below minimum ")
          .concat(PERFORMANCE_BENCHMARKS.minimumAccuracy),
      );
    }
    if (model.confidenceThreshold < 0.7 || model.confidenceThreshold > 0.99) {
      errors.push(
        "Model "
          .concat(key, " confidence threshold ")
          .concat(model.confidenceThreshold, " outside valid range [0.7, 0.99]"),
      );
    }
  });
