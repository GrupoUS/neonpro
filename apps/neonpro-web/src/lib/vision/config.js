...
exports.QUALITY_THRESHOLDS = exports.ERROR_CODES = exports.ANALYSIS_STATUS = exports.TREATMENT_TYPES = exports.VISION_CONFIG = void 0;
...
function validateVoidBeastCompliance(metrics) {
    ...
    if (metrics.accuracy < exports.VISION_CONFIG.PERFORMANCE.MIN_ACCURACY_THRESHOLD) {
        violations.push("Accuracy ".concat((metrics.accuracy * 100).toFixed(1), "% below minimum ").concat((exports.VISION_CONFIG.PERFORMANCE.MIN_ACCURACY_THRESHOLD * 100), "%"));
    }
    if (metrics.processingTime > exports.VISION_CONFIG.PERFORMANCE.MAX_PROCESSING_TIME_MS) {
        violations.push("Processing time ".concat(metrics.processingTime, "ms exceeds maximum ").concat(exports.VISION_CONFIG.PERFORMANCE.MAX_PROCESSING_TIME_MS, "ms"));
    }
    if (metrics.confidence < exports.VISION_CONFIG.PERFORMANCE.MIN_CONFIDENCE_THRESHOLD) {
        violations.push("Confidence ".concat((metrics.confidence * 100).toFixed(1), "% below minimum ").concat((exports.VISION_CONFIG.PERFORMANCE.MIN_CONFIDENCE_THRESHOLD * 100), "%"));
    }
    if (metrics.quality < exports.VISION_CONFIG.PERFORMANCE.QUALITY_THRESHOLD) {
        violations.push("Quality ".concat(metrics.quality.toFixed(1), " below VOIDBEAST standard ").concat(exports.VISION_CONFIG.PERFORMANCE.QUALITY_THRESHOLD));
    }
    var score = ((metrics.accuracy * 0.3) +
        (Math.min(1, exports.VISION_CONFIG.PERFORMANCE.TARGET_PROCESSING_TIME_MS / metrics.processingTime) * 0.2) +
        (metrics.confidence * 0.2) +
        (metrics.quality / 10 * 0.3)) * 10;
    return {
        compliant: violations.length === 0 && score >= exports.VISION_CONFIG.PERFORMANCE.QUALITY_THRESHOLD,
