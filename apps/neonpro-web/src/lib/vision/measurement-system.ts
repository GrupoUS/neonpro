...
export class ObjectiveMeasurementSystem {
  ...
  async performMeasurements(
    ...
    try {
      ...
      const beforeMeasurements = await this.extractMeasurements(beforeImage, protocol, "before");

      const afterMeasurements = await this.extractMeasurements(afterImage, protocol, "after");
      ...
      const comparisonScore =
        await this.statisticalAnalyzer.calculateComparisonScore(objectiveMeasurements);
  ...
  private async extractMeasurements(
    ...
    for (const measurementSpec of protocol.measurements) {
      try {
        const measurement = await this.performSingleMeasurement(image, measurementSpec);
  ...
  private async loadCalibrationData(): Promise<void> {
    ...
    try {
      const { data, error } = await this.supabase.from("measurement_calibration").select("*");
