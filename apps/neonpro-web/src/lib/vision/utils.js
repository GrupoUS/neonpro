...
exports.VisionUtils = exports.ErrorUtils = exports.DateUtils = exports.ExportUtils = exports.PerformanceUtils = exports.AnnotationUtils = exports.MeasurementUtils = exports.AnalysisUtils = exports.ImageUtils = void 0;
...
var ImageUtils = /** @class */ (function () {
    ...
    ImageUtils.validateImageFile = function (file) {
        ...
        if (fileSizeMB > maxSizeMB) {
            errors.push({
                field: 'file.size',
                code: 'FILE_TOO_LARGE',
                message: "Arquivo muito grande: ".concat(fileSizeMB.toFixed(1), "MB. M\u00E1ximo permitido: ").concat(maxSizeMB, "MB"),
                value: fileSizeMB
            });
        }
        // Check file format
        ...
        if (!fileExtension || !supportedFormats.includes(fileExtension)) {
            errors.push({
                field: 'file.format',
                code: 'INVALID_FORMAT',
                message: "Formato n\u00E3o suportado: ".concat(fileExtension, ". Formatos aceitos: ").concat(supportedFormats.join(', ')),
                value: fileExtension
            });
        }
        // Check file name
    ...
    ImageUtils.generateThumbnail = function (file_1) {
        return __awaiter(this, arguments, void 0, function (file, maxWidth, maxHeight) {
            ...
            if (maxHeight === void 0) { maxHeight = 200; }
            return __generator(this, function (_a) {
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        ...
                        img.onload = function () {
                            ...
                            var _a = _this.calculateThumbnailDimensions(img.width, img.height, maxWidth, maxHeight), width = _a.width, height = _a.height;
                            canvas.width = width;
     ...
     */
    ImageUtils.calculateThumbnailDimensions = function (originalWidth, originalHeight, maxWidth, maxHeight) {
...
var AnalysisUtils = /** @class */ (function () {
    ...
    AnalysisUtils.calculateOverallImprovement = function (changeMetrics) {
        var values = Object.values(changeMetrics).filter(function (v) { return typeof v === 'number' && !isNaN(v); });
        if (values.length === 0)
            return 0;
    ...
    AnalysisUtils.getQualityLevel = function (score) {
        if (score >= config_1.QUALITY_THRESHOLDS.EXCELLENT)
            return 'Excelente';
        if (score >= config_1.QUALITY_THRESHOLDS.GOOD)
            return 'Bom';
        if (score >= config_1.QUALITY_THRESHOLDS.FAIR)
            return 'Regular';
    ...
    AnalysisUtils.getAccuracyColor = function (accuracy) {
        if (accuracy >= config_1.VISION_CONFIG.PERFORMANCE.TARGET_ACCURACY)
            return '#10B981'; // Green
        if (accuracy >= config_1.VISION_CONFIG.PERFORMANCE.MIN_ACCURACY_THRESHOLD)
            return '#F59E0B'; // Yellow
        return '#EF4444'; // Red
    };
    /**
        ...
        if (!result.analysisData) {
            ...
            if (analysisData.accuracyScore < config_1.VISION_CONFIG.PERFORMANCE.MIN_ACCURACY_THRESHOLD) {
                warnings.push({
                    ...
                    code: 'LOW_ACCURACY',
                    message: "Precis\u00E3o baixa: ".concat((analysisData.accuracyScore * 100).toFixed(1), "%"),
...
var AnnotationUtils = /** @class */ (function () {
    ...
    AnnotationUtils.validateCoordinates = function (coordinates, imageWidth, imageHeight) {
        return (coordinates.x >= 0 && coordinates.x <= imageWidth &&
    ...
    AnnotationUtils.calculateBoundingBox = function (coordinates) {
        ...
        var minX = Math.min.apply(Math, coordinates.map(function (c) { return c.x; }));
        var maxX = Math.max.apply(Math, coordinates.map(function (c) { return c.x; }));
        var minY = Math.min.apply(Math, coordinates.map(function (c) { return c.y; }));
        var maxY = Math.max.apply(Math, coordinates.map(function (c) { return c.y; }));
        return {
            x: minX,
            y: minY,
            width: maxX - minX,
    ...
    ExportUtils.validateExportOptions = function (options) {
        ...
        if (options.format && !supportedFormats.includes(options.format)) {
            errors.push({
                field: 'format',
                code: 'INVALID_FORMAT',
                message: "Formato n\u00E3o suportado: ".concat(options.format, ". Formatos aceitos: ").concat(supportedFormats.join(', ')),
