...
var AccessibilityTester = /** @class */ (function () {
    ...
    AccessibilityTester.prototype.testColorContrast = function (container) {
        ...
        var textElements = container.querySelectorAll('p, span, div, h1, h2, h3, h4, h5, h6, button, a, label, input, textarea');
        textElements.forEach(function (element) {
            ...
            if (foregroundHex && backgroundHex) {
                var contrast = wcag_compliance_1.ContrastChecker.calculateContrastRatio(foregroundHex, backgroundHex);
                ...
                var meetsAA = wcag_compliance_1.ContrastChecker.meetsWCAGAA(foregroundHex, backgroundHex, isLargeText);
                var meetsAAA = contrast >= (isLargeText ? 4.5 : 7);
                results.push({
                    passed: meetsAA,
                    level: meetsAAA ? 'AAA' : meetsAA ? 'AA' : 'A',
                    message: "Contraste ".concat(contrast.toFixed(2), ":1 ").concat(meetsAA ? 'atende' : 'não atende', " WCAG AA"),
                    element: htmlElement,
                    suggestion: !meetsAA
                        ? "Aumentar contraste para pelo menos ".concat(isLargeText ? '3:1' : '4.5:1', " (WCAG AA)")
                        : undefined
                });
            }
        });
    ...
    AccessibilityTester.prototype.testFocusManagement = function (container) {
        ...
        results.push({
            ...
            suggestion: focusableElements.length === 0
        ...
        var elementsWithFocusStyle = Array.from(container.querySelectorAll('*:focus, *:focus-visible, .focus\\:ring-2')).length;
        results.push({
            ...
            suggestion: elementsWithFocusStyle === 0 && focusableElements.length > 0
    ...
    AccessibilityTester.prototype.testAriaLabels = function (container) {
        ...
        buttons.forEach(function (button) {
            ...
            var hasAccessibleName = ((_a = htmlButton.textContent) === null || _a === void 0 ? void 0 : _a.trim()) ||
    ...
    AccessibilityTester.prototype.testKeyboardNavigation = function (container) {
        ...
        var elementsWithKeyboardHandlers = container.querySelectorAll('[onkeydown], [onkeyup], [onkeypress]');
        // Check for elements that might need keyboard support
        var interactiveElements = container.querySelectorAll('button, a, input, select, textarea, [onclick], [tabindex]:not([tabindex="-1"])');
        results.push({
            passed: elementsWithKeyboardHandlers.length > 0 || interactiveElements.length === 0,
            level: 'A',
            message: "".concat(elementsWithKeyboardHandlers.length, " elementos com suporte a teclado encontrados"),
            suggestion: elementsWithKeyboardHandlers.length === 0 && interactiveElements.length > 0
        ...
        var customInteractive = container.querySelectorAll('[onclick]:not(button):not(a):not(input):not([role])');
        results.push({
            passed: customInteractive.length === 0,
            level: 'A',
            message: customInteractive.length === 0
            ...
            suggestion: customInteractive.length > 0
    ...
    AccessibilityTester.prototype.testSemanticStructure = function (container) {
        ...
        // Test landmarks
        var landmarks = container.querySelectorAll('main, nav, header, footer, aside, section[aria-label], [role="main"], [role="navigation"], [role="banner"], [role="contentinfo"], [role="complementary"]');
        results.push({
            passed: landmarks.length > 0,
            level: 'AA',
            message: "".concat(landmarks.length, " marcos (landmarks) encontrados"),
            suggestion: landmarks.length === 0
    ...
    AccessibilityTester.prototype.auditAccessibility = function (container) {
        ...
        var results = __spreadArray(__spreadArray(__spreadArray(__spreadArray(__spreadArray([], colorContrast, true), focusManagement, true), ariaLabels, true), keyboardNavigation, true), semanticStructure, true);
    ...
    AccessibilityTester.prototype.simulateScreenReader = function (element) {
        ...
        if (expanded)
            announcement += ", ".concat(expanded === 'true' ? 'expandido' : 'recolhido');
        if (selected)
            announcement += ", ".concat(selected === 'true' ? 'selecionado' : 'não selecionado');
        if (checked)
            announcement += ", ".concat(checked === 'true' ? 'marcado' : 'desmarcado');
        if (description)
            announcement += ", ".concat(description);
        return announcement;
    };
    ...
    AccessibilityTester.prototype.generateReport = function (auditReport) {
        ...
        report += "**Testes:** ".concat(auditReport.passedTests, "/").concat(auditReport.totalTests, " aprovados\n\n");
