...
export class ForecastModelManager {
  ...
  private async updateTrainingJob(job: ModelTrainingJob): Promise<void> {
    const { error } = await supabase.from("model_training_jobs").update(job).eq("id", job.id);
  ...
  private async evaluateConfigurationCV(config: ModelTrainingConfig, data: any[]): Promise<number> {
