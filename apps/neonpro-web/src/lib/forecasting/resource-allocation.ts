...
import type { DemandForecast, ResourceAllocation } from "./demand-forecasting";
...
export class ResourceAllocationOptimizer {
  ...
  private readonly COST_VARIANCE_THRESHOLD = 0.1; // 10% cost variance threshold
  private readonly EFFICIENCY_THRESHOLD = 0.8; // 80% minimum efficiency
    ...
    try {
      ...
      for (const member of staff || []) {
        try {
          ...
          const availabilityWindows = this.parseAvailabilitySchedule(member.availability_schedule);
    ...
    try {
      ...
      for (const item of equipment || []) {
        try {
          ...
          const maintenanceWindows = this.parseMaintenanceSchedule(item.maintenance_schedule);

          // Calculate efficiency rating
          const efficiencyRating = this.calculateEquipmentEfficiency(item, utilizationRate);
    ...
    try {
      ...
      for (const room of rooms || []) {
        try {
          ...
          const predictedDemand = this.calculateRoomBookings(room, typeDemand, planningPeriod);

          // Load existing bookings
          const existingBookings = await this.getExistingRoomBookings(room.id, planningPeriod);
          ...
          // Parse availability windows
          const availabilityWindows = this.parseAvailabilitySchedule(room.availability_schedule);
    ...
    try {
      ...
      for (const item of inventory || []) {
        try {
          ...
          const safetyStock = this.calculateSafetyStock(item, predictedConsumption);

          // Calculate stockout risk
          const stockoutRisk = this.calculateStockoutRisk(item, predictedConsumption, safetyStock);

          // Calculate expiration risk
          const expirationRisk = this.calculateExpirationRisk(item, predictedConsumption);
  ...
  private calculateStockoutRisk(item: any, consumption: number, safetyStock: number): number {
