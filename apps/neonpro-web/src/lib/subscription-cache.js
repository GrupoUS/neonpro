...
var SubscriptionCache = /** @class */ (function () {
    ...
    SubscriptionCache.prototype.set = function (key, data, customTTL) {
        ...
        else if (data.status === 'active' && ((_a = data.subscription) === null || _a === void 0 ? void 0 : _a.cancel_at_period_end)) {
    ...
    SubscriptionCache.prototype.getStats = function () {
        var now = Date.now();
        var entries = [];
        this.cache.forEach(function (entry) { return entries.push(entry); });
        var validEntries = entries.filter(function (entry) { return entry.expires > now; });
        var totalHits = this.hitCount + this.missCount;
        var hitRate = totalHits > 0 ? (this.hitCount / totalHits) * 100 : 0;
        var accessCounts = validEntries.map(function (entry) { return entry.accessCount; });
        var averageAccessCount = accessCounts.length > 0
            ? accessCounts.reduce(function (sum, count) { return sum + count; }, 0) / accessCounts.length
            : 0;
        ...
        return {
            ...
            oldestEntry: entries.length > 0 ? Math.min.apply(Math, entries.map(function (e) { return e.created; })) : null,
            newestEntry: entries.length > 0 ? Math.max.apply(Math, entries.map(function (e) { return e.created; })) : null,
    ...
    SubscriptionCache.prototype.optimize = function () {
        ...
        var lruRemoved = 0;
        while (this.cache.size > this.config.maxSize * 0.8) { // Keep at 80% of max
            this.evictLeastUseful();
            lruRemoved++;
        }
    ...
    SubscriptionCache.prototype.calculatePriority = function (entry) {
        ...
        else if (timeSinceAccess < 300000) { // Less than 5 minutes
            priority += 20;
        }
        ...
        if (remainingTTL > 180000) { // More than 3 minutes
            priority += 30;
        }
        else if (remainingTTL > 60000) { // More than 1 minute
            priority += 10;
        }
        return priority;
    };
...
exports.cacheManager = {
    ...
    getHealth: function () {
        ...
        if (stats.hitRate < 70) {
            healthy = false;
            recommendations.push('Low cache hit rate - consider increasing TTL or optimizing invalidation strategy');
        }
        // Check memory usage
