...
var AppointmentManager = /** @class */ (function () {
    function AppointmentManager(supabaseUrl, supabaseKey, auditLogger, lgpdManager, sessionManager, config) {
        this.supabase = (0, supabase_js_1.createClient)(supabaseUrl, supabaseKey);
        this.auditLogger = auditLogger;
        this.lgpdManager = lgpdManager;
        this.sessionManager = sessionManager;
        this.config = __assign({ maxAdvanceBookingDays: 90, minAdvanceBookingHours: 2, maxReschedulingAttempts: 3, cancellationDeadlineHours: 24, enableWaitingList: true, autoConfirmationEnabled: false, reminderIntervals: [24, 2] }, config);
    }
    /**
     * Get available time slots for booking
     */
    AppointmentManager.prototype.getAvailableSlots = function (patientId, sessionToken, serviceId, startDate, endDate, staffId) {
        return __awaiter(this, void 0, void 0, function () {
            var sessionValidation, now, maxDate, _a, service, serviceError, staffFilter, _b, availableStaff, staffError, _c, existingAppointments, appointmentsError, availableSlots, currentDate, _loop_1, this_1, error_1;
            var _d;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    ...
                    case 1:
                        ...
                        if (!sessionValidation.isValid || ((_d = sessionValidation.session) === null || _d === void 0 ? void 0 : _d.patientId) !== patientId) {
                            throw new Error('Invalid session or unauthorized access');
                        }
                        now = new Date();
                        maxDate = new Date(now.getTime() + this.config.maxAdvanceBookingDays * 24 * 60 * 60 * 1000);
                        if (startDate < now || endDate > maxDate) {
                            throw new Error("Booking dates must be between now and ".concat(this.config.maxAdvanceBookingDays, " days in advance"));
                        }
                    ...
                    case 4:
                        ...
                        _loop_1 = function () {
                            ...
                            for (var _i = 0, _f = availableStaff || []; _i < _f.length; _i++) {
                                var staff = _f[_i];
                                var dayAvailability = staff.staff_availability.find(function (avail) { return avail.day_of_week === dayOfWeek; });
                                if (dayAvailability) {
                                    var slots = this_1.generateTimeSlots(currentDate, dayAvailability.start_time, dayAvailability.end_time, service.duration, staff, service, existingAppointments);
                    ...
                    case 5:
                        // Log slot access
                        _e.sent();
                        return [2 /*return*/, availableSlots.sort(function (a, b) { return a.date.getTime() - b.date.getTime(); })];
                    case 6:
                        error_1 = _e.sent();
                        return [4 /*yield*/, this.auditLogger.log({
     ...
     */
    AppointmentManager.prototype.generateTimeSlots = function (date, startTime, endTime, serviceDuration, staff, service, existingAppointments) {
        var slots = [];
        ...
        var now = new Date();
        var minBookingTime = new Date(now.getTime() + this.config.minAdvanceBookingHours * 60 * 60 * 1000);
        var _loop_2 = function () {
            ...
            if (slotStart >= minBookingTime) {
                ...
                var hasConflict = existingAppointments.some(function (apt) {
                    ...
                    return ((slotStart >= aptDate && slotStart < aptEndTime) ||
                        (slotEndTime > aptDate && slotEndTime <= aptEndTime) ||
                        (slotStart <= aptDate && slotEndTime >= aptEndTime));
                });
                if (!hasConflict) {
                    slots.push({
                        id: "".concat(staff.id, "_").concat(date.toISOString().split('T')[0], "_").concat(slotStart.getHours(), ":").concat(slotStart.getMinutes().toString().padStart(2, '0')),
                        date: new Date(date),
                        startTime: "".concat(slotStart.getHours(), ":").concat(slotStart.getMinutes().toString().padStart(2, '0')),
                        endTime: "".concat(slotEndTime.getHours(), ":").concat(slotEndTime.getMinutes().toString().padStart(2, '0')),
                        staffId: staff.id,
    ...
    AppointmentManager.prototype.bookAppointment = function (request, sessionToken) {
        return __awaiter(this, void 0, void 0, function () {
            var sessionValidation, validationResult, _a, conflictingAppointments, conflictError, alternatives, _b, newAppointment, insertError, error_2;
            var _c;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    ...
                    case 1:
                        ...
                        if (!sessionValidation.isValid || ((_c = sessionValidation.session) === null || _c === void 0 ? void 0 : _c.patientId) !== request.patientId) {
                            throw new Error('Invalid session or unauthorized access');
                        }
                    ...
                    case 3:
                        ...
                        if (conflictError)
                            throw conflictError;
                        if (!(conflictingAppointments && conflictingAppointments.length > 0)) return [3 /*break*/, 5];
                        return [4 /*yield*/, this.getAlternativeSlots(request.serviceId, request.preferredDate, request.staffId)];
                    case 4:
                    ...
                    case 5: return [4 /*yield*/, this.supabase
                            .from('appointments')
                            .insert({
    ...
    AppointmentManager.prototype.validateBookingRequest = function (request) {
        return __awaiter(this, void 0, void 0, function () {
            var now, appointmentDateTime, minBookingTime, maxBookingTime, _a, service, serviceError, _b, staff, staffError;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        now = new Date();
                        appointmentDateTime = new Date("".concat(request.preferredDate.toISOString().split('T')[0], "T").concat(request.preferredTime));
                        minBookingTime = new Date(now.getTime() + this.config.minAdvanceBookingHours * 60 * 60 * 1000);
                        maxBookingTime = new Date(now.getTime() + this.config.maxAdvanceBookingDays * 24 * 60 * 60 * 1000);
                        // Check minimum advance booking time
                        if (appointmentDateTime < minBookingTime) {
                            return [2 /*return*/, {
                                    isValid: false,
                                    message: "Agendamentos devem ser feitos com pelo menos ".concat(this.config.minAdvanceBookingHours, " horas de anteced\u00EAncia.")
                                }];
                        }
                        ...
                        if (appointmentDateTime > maxBookingTime) {
                            return [2 /*return*/, {
                                    isValid: false,
                                    message: "Agendamentos podem ser feitos com at\u00E9 ".concat(this.config.maxAdvanceBookingDays, " dias de anteced\u00EAncia.")
                                }];
                        }
                        return [4 /*yield*/, this.supabase
                                .from('services')
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, this.getAvailableSlots('', // We'll skip session validation for internal use
                            '', serviceId, startDate, endDate, staffId)];
                    case 1:
