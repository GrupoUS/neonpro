...
var PerformanceMonitor = /** @class */ (function () {
     ...
     */
    PerformanceMonitor.prototype.recordDatabasePerformance = function (operation, duration, metadata) {
        var metric = {
    ...
    PerformanceMonitor.prototype.getPerformanceSummary = function (timeWindow) {
        if (timeWindow === void 0) { timeWindow = 5 * 60 * 1000; }
        var cacheKey = "summary_".concat(timeWindow, "_").concat(Date.now() - (Date.now() % 30000));
        var cached = aggregationCache.get(cacheKey);
        if (cached)
            return cached;
        var now = Date.now();
        var recentMetrics = this.metrics.filter(function (m) { return now - m.timestamp <= timeWindow; });
        var apiMetrics = recentMetrics.filter(function (m) { return m.name.startsWith('api.'); });
        var dbMetrics = recentMetrics.filter(function (m) { return m.name.startsWith('db.'); });
        var clientMetrics = recentMetrics.filter(function (m) { return m.name.startsWith('client.'); });
        var summary = {
    ...
    PerformanceMonitor.prototype.getRoutePerformance = function (route, timeWindow) {
        if (timeWindow === void 0) { timeWindow = 30 * 60 * 1000; }
        var now = Date.now();
        ...
        var values = routeMetrics.map(function (m) { return m.value; }).sort(function (a, b) { return a - b; });
        var errors = routeMetrics.filter(function (m) { var _a; return ((_a = m.metadata) === null || _a === void 0 ? void 0 : _a.statusCode) >= 400; }).length;
        return {
    ...
    PerformanceMonitor.prototype.generateAlerts = function (metrics) {
        ...
        var apiMetrics = metrics.filter(function (m) { return m.name.startsWith('api.'); });
        if (apiMetrics.length > 0) {
            var slowAPIs = apiMetrics.filter(function (m) { return m.value > 1000; }); // >1s
            if (slowAPIs.length > apiMetrics.length * 0.05) { // >5% slow
                alerts.push("High API latency detected: ".concat(slowAPIs.length, "/").concat(apiMetrics.length, " requests >1s"));
            }
            var errors = apiMetrics.filter(function (m) { var _a; return ((_a = m.metadata) === null || _a === void 0 ? void 0 : _a.statusCode) >= 500; });
            if (errors.length > 0) {
                alerts.push("".concat(errors.length, " server errors in last period"));
            }
        }
        // Database performance alerts  
        var dbMetrics = metrics.filter(function (m) { return m.name.startsWith('db.'); });
        if (dbMetrics.length > 0) {
            var slowQueries = dbMetrics.filter(function (m) { return m.value > 500; }); // >500ms
            if (slowQueries.length > dbMetrics.length * 0.1) { // >10% slow
                alerts.push("Slow database queries detected: ".concat(slowQueries.length, "/").concat(dbMetrics.length, " queries >500ms"));
            }
        }
        return alerts;
    };
    ...
    PerformanceMonitor.prototype.generateTrends = function (metrics, buckets) {
        if (buckets === void 0) { buckets = 10; }
        if (metrics.length === 0)
            return [];
        var sorted = metrics.sort(function (a, b) { return a.timestamp - b.timestamp; });
        ...
        for (var i = 0; i < buckets; i++) {
            ...
            if (bucketMetrics.length > 0) {
                var avgValue = bucketMetrics.reduce(function (sum, m) { return sum + m.value; }, 0) / bucketMetrics.length;
    ...
    PerformanceMonitor.prototype.aggregateMetrics = function (metrics) {
        ...
        var values = metrics.map(function (m) { return m.value; }).sort(function (a, b) { return a - b; });
