...
var CohortAnalyzer = /** @class */ (function () {
    ...
    CohortAnalyzer.prototype.generateCohorts = function (config) {
        return __awaiter(this, void 0, void 0, function () {
            var validConfig, dimension, tableName, cohortRanges, cohorts, _i, cohortRanges_1, range, userCount;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        cohortRanges = this.generateCohortRanges(validConfig.startDate, validConfig.endDate, validConfig.granularity);
                        cohorts = [];
                    ...
                    case 3:
                        _i++;
                        return [3 /*break*/, 1];
                    case 4: return [2 /*return*/, cohorts];
                }
            });
        });
    };
    ...
    CohortAnalyzer.prototype.calculateCohortRetention = function (cohorts, config) {
        return __awaiter(this, void 0, void 0, function () {
            var metrics, _i, cohorts_1, cohort, period, periodDate, periodMetrics, retentionRate, churnRate;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        metrics = [];
                    ...
                    case 2:
                        ...
                        return [4 /*yield*/, this.supabase
                                .rpc('calculate_cohort_period_metrics', {
                    ...
                    case 3:
                        ...
                        if (periodMetrics) {
                            retentionRate = cohort.userCount > 0
                                ? (periodMetrics.active_users / cohort.userCount) * 100
                                : 0;
                            churnRate = cohort.userCount > 0
                                ? (periodMetrics.churned_users / cohort.userCount) * 100
                                : 0;
                            metrics.push({
                                cohortId: cohort.id,
                                period: period,
                                ...
                                averageRevenuePerUser: periodMetrics.active_users > 0
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        cohort = cohorts_2[_i];
                        return [4 /*yield*/, this.supabase
                                .rpc('calculate_cohort_revenue_progression', {
                                p_cohort_start: cohort.startDate,
                                p_cohort_end: cohort.endDate,
                    ...
                    case 3:
                        _i++;
                        return [3 /*break*/, 1];
                    case 4: return [2 /*return*/, revenueCohorts];
                }
            });
        });
    };
    ...
    CohortAnalyzer.prototype.formatCohortName = function (dateStr, granularity) {
        ...
        if (granularity === 'daily') {
            ...
            return "Week of ".concat(date.toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric'
            }));
        }
    ...
    CohortAnalyzer.prototype.calculateCumulativeRevenue = function (revenueData) {
        return revenueData.reduce(function (sum, period) { return sum + (period.revenue || 0); }, 0);
    };
    ...
    CohortAnalyzer.prototype.calculateRetentionTrends = function (periodGroups) {
        ...
        for (var _i = 0, _a = Object.entries(periodGroups); _i < _a.length; _i++) {
            var _b = _a[_i], period = _b[0], metrics = _b[1];
            var avgRetention = metrics.reduce(function (sum, m) { return sum + m.retentionRate; }, 0) / metrics.length;
            var avgRevenue = metrics.reduce(function (sum, m) { return sum + m.averageRevenuePerUser; }, 0) / metrics.length;
            trends.push({
        ...
        return trends.sort(function (a, b) { return a.period - b.period; });
    };
    CohortAnalyzer.prototype.compareCohortPerformance = function (metrics) {
        ...
        for (var _i = 0, _a = Object.entries(cohortGroups); _i < _a.length; _i++) {
            var _b = _a[_i], cohortId = _b[0], cohortMetrics = _b[1];
            var avgRetention = cohortMetrics.reduce(function (sum, m) { return sum + m.retentionRate; }, 0) / cohortMetrics.length;
            var totalRevenue = cohortMetrics.reduce(function (sum, m) { return sum + m.revenue; }, 0);
        ...
        return comparison.sort(function (a, b) { return b.performanceScore - a.performanceScore; });
    };
    ...
    CohortAnalyzer.prototype.performSignificanceTests = function (periodGroups) {
        ...
        var tests = [];
        var periods = Object.keys(periodGroups).map(Number).sort(function (a, b) { return a - b; });
        for (var i = 0; i < periods.length - 1; i++) {
            var currentPeriod = periodGroups[periods[i]];
            var nextPeriod = periodGroups[periods[i + 1]];
            var currentAvgRetention = currentPeriod.reduce(function (sum, m) { return sum + m.retentionRate; }, 0) / currentPeriod.length;
            var nextAvgRetention = nextPeriod.reduce(function (sum, m) { return sum + m.retentionRate; }, 0) / nextPeriod.length;
    ...
    CohortAnalyzer.prototype.generatePredictiveInsights = function (metrics) {
        ...
        var retentionTrend = this.calculateTrendDirection(metrics.map(function (m) { return ({ period: m.period, value: m.retentionRate }); }));
        if (retentionTrend.direction === 'increasing') {
            insights.push({
                type: 'positive',
                category: 'retention',
                message: "Retention rates are improving with a ".concat(retentionTrend.rate, "% increase trend"),
            ...
            insights.push({
                type: 'warning',
                category: 'retention',
                message: "Retention rates are declining with a ".concat(Math.abs(retentionTrend.rate), "% decrease trend"),
                confidence: retentionTrend.confidence
            });
        }
        // Analyze revenue trends
        var revenueTrend = this.calculateTrendDirection(metrics.map(function (m) { return ({ period: m.period, value: m.averageRevenuePerUser }); }));
        if (revenueTrend.direction === 'increasing') {
            insights.push({
                type: 'positive',
                category: 'revenue',
                message: "Revenue per user is growing with a ".concat(revenueTrend.rate, "% increase trend"),
    ...
    CohortAnalyzer.prototype.calculateTrendDirection = function (data) {
        ...
        var sortedData = data.sort(function (a, b) { return a.period - b.period; });
...
exports.cohortUtils = {
    ...
    formatForHeatmap: function (metrics) {
        var heatmapData = [];
        var cohorts = __spreadArray([], new Set(metrics.map(function (m) { return m.cohortId; })), true);
        var periods = __spreadArray([], new Set(metrics.map(function (m) { return m.period; })), true).sort(function (a, b) { return a - b; });
        var _loop_1 = function (cohort) {
            var cohortData = { cohort: cohort };
            var _loop_2 = function (period) {
                var metric = metrics.find(function (m) { return m.cohortId === cohort && m.period === period; });
    ...
    calculateCohortSizes: function (cohorts) {
        var sizes = cohorts.map(function (c) { return c.userCount; });
        return {
            total: sizes.reduce(function (sum, size) { return sum + size; }, 0),
            average: Math.round(sizes.reduce(function (sum, size) { return sum + size; }, 0) / sizes.length),
            median: sizes.sort(function (a, b) { return a - b; })[Math.floor(sizes.length / 2)],
    ...
    generateComparisonData: function (metrics) {
        ...
        var _loop_3 = function (cohort) {
            var cohortMetrics = metrics.filter(function (m) { return m.cohortId === cohort; });
            var avgRetention = cohortMetrics.reduce(function (sum, m) { return sum + m.retentionRate; }, 0) / cohortMetrics.length;
            var totalRevenue = cohortMetrics.reduce(function (sum, m) { return sum + m.revenue; }, 0);
            comparison.push({
                cohort: cohort,
        ...
        return comparison.sort(function (a, b) { return b.averageRetention - a.averageRetention; });
    }
};
