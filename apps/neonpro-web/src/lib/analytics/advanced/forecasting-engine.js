...
var ForecastingEngine = /** @class */ (function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.getOrTrainModel(validConfig, trainingData)
                            // Generate forecasts
                        ];
                    case 2:
                        model = _a.sent();
                        forecasts = this.predictFuture(model, historicalData, features, validConfig.horizon, validConfig.confidenceLevel);
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        results = {};
                        forecastPromises = metrics.map(function (metric) { return __awaiter(_this, void 0, void 0, function () {
                    ...
                    case 1:
                        ...
                        forecastResults.forEach(function (_a) {
                            var metric = _a.metric, forecast = _a.forecast;
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        for (_i = 0, scenarios_1 = scenarios; _i < scenarios_1.length; _i++) {
                            ...
                            adjustedForecast = this.applyScenarioAdjustments(baselineForecast, scenario.adjustments);
    ...
    ForecastingEngine.prototype.evaluateForecastAccuracy = function (config_1) {
        return __awaiter(this, arguments, void 0, function (config, backtestPeriods) {
            var historicalData, predictions, errors, i, splitPoint, trainData, testData, features, trainingFeatures, model, forecast, j, actual, predicted, mae, mse, rmse, mape, accuracy_score;
            if (backtestPeriods === void 0) { backtestPeriods = 4; }
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.getHistoricalData(config.metric, config.frequency, true // extended data
                        )];
                    case 1:
                        historicalData = _a.sent();
                        predictions = [];
                    ...
                    case 3:
                        model = _a.sent();
                        forecast = this.predictFuture(model, trainData, features, testData.length, config.confidenceLevel);
                    ...
                    case 5:
                        ...
                        mae = errors.reduce(function (sum, e) { return sum + e.absolute; }, 0) / errors.length;
                        mse = errors.reduce(function (sum, e) { return sum + e.squared; }, 0) / errors.length;
                        rmse = Math.sqrt(mse);
                        mape = errors.reduce(function (sum, e) { return sum + e.percentage; }, 0) / errors.length;
    ...
    ForecastingEngine.prototype.prepareTrainingData = function (data, features) {
        var trainingData = [];
        var maxLag = Math.max.apply(Math, __spreadArray(__spreadArray([], features.lags, false), [0], false));
        var maxWindow = Math.max.apply(Math, __spreadArray(__spreadArray([], features.rolling_stats.map(function (rs) { return rs.window; }), false), [0], false));
        ...
        var _loop_1 = function (i) {
            ...
            features.rolling_stats.forEach(function (_a) {
                var window = _a.window, stats = _a.stats;
                if (i - window + 1 >= 0) {
                    var windowData_1 = data.slice(i - window + 1, i + 1).map(function (d) { return d.value; });
                    stats.forEach(function (stat) {
                        var value = 0;
                        if (stat === 'mean') {
                            value = windowData_1.reduce(function (sum, v) { return sum + v; }, 0) / windowData_1.length;
                        }
                        else if (stat === 'std') {
                            var mean_1 = windowData_1.reduce(function (sum, v) { return sum + v; }, 0) / windowData_1.length;
                            value = Math.sqrt(windowData_1.reduce(function (sum, v) { return sum + Math.pow(v - mean_1, 2); }, 0) / windowData_1.length);
                        }
                        else if (stat === 'min') {
    ...
    ForecastingEngine.prototype.trainModel = function (config, trainingData) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                X = trainingData.map(function (d) { return features.map(function (f) { return d.features[f] || 0; }); });
                y = trainingData.map(function (d) { return d.target; });
                ...
                return [2 /*return*/, __assign(__assign({}, model), { features: features, config: config, trainedAt: new Date().toISOString() })];
            });
        });
    };
    /**
     ...
     */
    ForecastingEngine.prototype.predictFuture = function (model, historicalData, features, horizon, confidenceLevel) {
        var predictions = [];
        var extendedData = __spreadArray([], historicalData, true);
        for (var i = 0; i < horizon; i++) {
            // Prepare features for next prediction
            var featureVector = this.prepareFeatureVector(extendedData, features, extendedData.length - 1 + i);
            // Make prediction
            var predicted = this.makePrediction(model, featureVector);
            // Calculate confidence interval
            var _a = this.calculateConfidenceInterval(predicted, model.residualStd || predicted * 0.1, // fallback to 10% of prediction
            confidenceLevel), lower_bound = _a.lower_bound, upper_bound = _a.upper_bound;
    ...
    ForecastingEngine.prototype.trainLinearRegression = function (X, y) {
        ...
        // Add bias term
        var XWithBias = X.map(function (row) { return __spreadArray([1], row, true); });
        ...
        for (var iter = 0; iter < 1000; iter++) {
            var predictions = XWithBias.map(function (row) {
                return row.reduce(function (sum, xi, i) { return sum + xi * weights[i]; }, 0);
            });
            var errors = predictions.map(function (pred, i) { return pred - y[i]; });
            var _loop_2 = function (j) {
                var gradient = errors.reduce(function (sum, error, i) {
        ...
        var residuals = finalPredictions.map(function (pred, i) { return Math.pow(pred - y[i], 2); });
        var residualStd = Math.sqrt(residuals.reduce(function (sum, r) { return sum + r; }, 0) / (n - 1));
        return {
            type: 'linear',
            weights: weights,
    ...
    ForecastingEngine.prototype.trainPolynomialRegression = function (X, y, degree) {
        ...
        var polyX = X.map(function (row) {
            var polyRow = [1]; // bias
            row.forEach(function (x) { return polyRow.push(x); }); // linear terms
            if (degree >= 2) {
                // Add quadratic terms
        ...
        return this.trainLinearRegression(polyX.map(function (row) { return row.slice(1); }), y);
    };
    ForecastingEngine.prototype.trainExponentialSmoothing = function (y) {
        ...
        var residuals = y.map(function (actual, i) { return Math.pow(actual - smoothedValues[i], 2); });
        var residualStd = Math.sqrt(residuals.reduce(function (sum, r) { return sum + r; }, 0) / (y.length - 1));
        return {
            type: 'exponential',
            alpha: alpha,
    ...
    ForecastingEngine.prototype.trainSeasonalModel = function (X, y, frequency) {
        ...
        var deseasonalized = y.map(function (value, i) { return value - seasonal[i % seasonalPeriod]; });
    ...
    ForecastingEngine.prototype.calculateSeasonalComponents = function (y, period) {
        ...
        var mean = seasonal.reduce(function (sum, s) { return sum + s; }, 0) / period;
        return seasonal.map(function (s) { return s - mean; });
    };
    ForecastingEngine.prototype.prepareFeatureVector = function (data, features, index) {
        ...
        features.rolling_stats.forEach(function (_a) {
            ...
            var windowData = data.slice(windowStart, windowEnd).map(function (d) { return d.value; });
            if (windowData.length > 0) {
                stats.forEach(function (stat) {
                    var value = 0;
                    if (stat === 'mean') {
                        value = windowData.reduce(function (sum, v) { return sum + v; }, 0) / windowData.length;
                    }
                    else if (stat === 'std') {
                        var mean_2 = windowData.reduce(function (sum, v) { return sum + v; }, 0) / windowData.length;
                        value = Math.sqrt(windowData.reduce(function (sum, v) { return sum + Math.pow(v - mean_2, 2); }, 0) / windowData.length);
                    }
                    else if (stat === 'min') {
    ...
    ForecastingEngine.prototype.makePrediction = function (model, featureVector) {
        if (model.type === 'linear' || model.type === 'polynomial') {
            var features = __spreadArray([1], model.features.map(function (f) { return featureVector[f] || 0; }), true);
            return features.reduce(function (sum, xi, i) { return sum + xi * (model.weights[i] || 0); }, 0);
        }
    ...
    ForecastingEngine.prototype.calculateConfidenceInterval = function (prediction, standardError, confidenceLevel) {
    ...
    ForecastingEngine.prototype.applyScenarioAdjustments = function (baselineForecast, adjustments) {
        return baselineForecast.map(function (forecast, i) {
            ...
            if (adjustments.external_factors) {
                var factorSum = Object.values(adjustments.external_factors).reduce(function (sum, factor) { return sum + factor; }, 0);
                adjusted *= (1 + factorSum / 100);
            }
            // Recalculate bounds proportionally
            var ratio = adjusted / forecast.predicted;
            return __assign(__assign({}, forecast), { predicted: Math.round(adjusted * 100) / 100, lower_bound: Math.round(forecast.lower_bound * ratio * 100) / 100, upper_bound: Math.round(forecast.upper_bound * ratio * 100) / 100 });
        });
    };
    ForecastingEngine.prototype.generateModelCacheKey = function (config) {
        return "".concat(config.metric, "_").concat(config.frequency, "_").concat(config.modelType, "_").concat(config.horizon, "_").concat(config.includeSeasonality, "_").concat(config.includeExogenous);
    };
...
exports.forecastUtils = {
    ...
    calculateAccuracyMetrics: function (actual, predicted) {
        ...
        var n = actual.length;
        var errors = actual.map(function (a, i) { return a - predicted[i]; });
        var absoluteErrors = errors.map(function (e) { return Math.abs(e); });
        var squaredErrors = errors.map(function (e) { return e * e; });
        var percentageErrors = actual.map(function (a, i) { return a !== 0 ? Math.abs((a - predicted[i]) / a) * 100 : 0; });
        return {
            mae: absoluteErrors.reduce(function (sum, e) { return sum + e; }, 0) / n,
            mse: squaredErrors.reduce(function (sum, e) { return sum + e; }, 0) / n,
            rmse: Math.sqrt(squaredErrors.reduce(function (sum, e) { return sum + e; }, 0) / n),
            mape: percentageErrors.reduce(function (sum, e) { return sum + e; }, 0) / n
        };
    },
    /**
    ...
    formatForChart: function (historical, forecasts) {
        return __spreadArray(__spreadArray([], historical.map(function (h) { return ({
            date: h.date,
            actual: h.value,
            type: 'historical'
        }); }), true), forecasts.map(function (f) { return ({
            date: f.date,
    ...
    detectAnomalies: function (actual, predicted, threshold) {
        ...
        var errors = actual.map(function (a, i) { return a - predicted[i]; });
        var meanError = errors.reduce(function (sum, e) { return sum + e; }, 0) / errors.length;
        var stdError = Math.sqrt(errors.reduce(function (sum, e) { return sum + Math.pow(e - meanError, 2); }, 0) / errors.length);
        return errors.map(function (error, i) { return ({
            index: i,
            actual: actual[i],
