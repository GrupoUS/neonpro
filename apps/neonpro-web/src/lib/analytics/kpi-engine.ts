...
export class KPIEngine {
  ...
  async calculateKPIs(request: KPICalculationRequest): Promise<KPICalculationResult[]> {
    ...
    try {
      ...
      const calculations = kpisToCalculate.map((kpi) => this.calculateSingleKPI(kpi, request));
  ...
  private async calculateOperationalKPI(
    ...
    if (kpi.kpi_name === "Patient Retention Rate") {
      const { data: patients } = await this.supabase.from("patients").select("id, created_at");
  ...
  private async getKPIsByIds(ids: string[]): Promise<FinancialKPI[]> {
    const { data, error } = await this.supabase.from("financial_kpis").select("*").in("id", ids);
  ...
  private async updateKPIValues(results: KPICalculationResult[]): Promise<void> {
    ...
    for (const update of updates) {
      await this.supabase.from("financial_kpis").update(update).eq("id", update.id);
    }
  }
