...
var DashboardBuilder = /** @class */ (function () {
    ...
    DashboardBuilder.prototype.createDashboard = function (userId_1, name_1) {
        return __awaiter(this, arguments, void 0, function (userId, name, layoutType, templateId) {
            ...
            if (layoutType === void 0) { layoutType = 'kpi_dashboard'; }
    ...
    DashboardBuilder.prototype.addWidget = function (layout, widgetType, position, kpiIds, config) {
        var widgetTemplate = this.widgetLibrary.find(function (w) { return w.type === widgetType; });
        if (!widgetTemplate)
        ...
        var updatedLayout = __assign(__assign({}, layout), { widget_configuration: __spreadArray(__spreadArray([], layout.widget_configuration, true), [newWidget], false) });
        return updatedLayout;
    };
    DashboardBuilder.prototype.removeWidget = function (layout, widgetId) {
        return __assign(__assign({}, layout), { widget_configuration: layout.widget_configuration.filter(function (w) { return w.id !== widgetId; }) });
    };
    DashboardBuilder.prototype.updateWidget = function (layout, widgetId, updates) {
        return __assign(__assign({}, layout), { widget_configuration: layout.widget_configuration.map(function (widget) {
    ...
    DashboardBuilder.prototype.duplicateWidget = function (layout, widgetId) {
        var widget = layout.widget_configuration.find(function (w) { return w.id === widgetId; });
        if (!widget)
            throw new Error('Widget not found');
        var duplicatedWidget = __assign(__assign({}, widget), { id: "widget_".concat(Date.now(), "_").concat(Math.random().toString(36).substr(2, 9)), position: __assign(__assign({}, widget.position), { x: widget.position.x + 1, y: widget.position.y + 1 }), configuration: __assign(__assign({}, widget.configuration), { title: "".concat(widget.configuration.title || 'Widget', " (Copy)") }) });
        return __assign(__assign({}, layout), { widget_configuration: __spreadArray(__spreadArray([], layout.widget_configuration, true), [duplicatedWidget], false) });
    };
    // Grid Layout Management
    DashboardBuilder.prototype.updateGridLayout = function (layout, gridUpdates) {
        return __assign(__assign({}, layout), { grid_layout: __assign(__assign({}, layout.grid_layout), gridUpdates) });
    };
    DashboardBuilder.prototype.optimizeLayout = function (layout) {
        ...
        widgets.sort(function (a, b) { return a.position.y - b.position.y || a.position.x - b.position.x; });
        var currentY = 0;
        var rowWidgets = [];
        ...
        rowWidgets.forEach(function (row) {
            var x = 0;
            var maxHeight = Math.max.apply(Math, row.map(function (w) { return w.position.h; }));
            row.forEach(function (widget) {
                optimizedWidgets.push(__assign(__assign({}, widget), { position: __assign(__assign({}, widget.position), { x: x, y: y }) }));
                x += widget.position.w;
            });
            y += maxHeight;
        });
    ...
    // Template Management
    DashboardBuilder.prototype.createTemplate = function (name_1, description_1, category_1, layout_1) {
        return __awaiter(this, arguments, void 0, function (name, description, category, layout, recommendedFor) {
            var template;
            if (recommendedFor === void 0) { recommendedFor = []; }
            return __generator(this, function (_a) {
                template = {
    ...
    DashboardBuilder.prototype.getTemplates = function (category) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                if (category) {
                    return [2 /*return*/, this.templates.filter(function (t) { return t.category === category; })];
                }
    ...
    DashboardBuilder.prototype.getTemplate = function (templateId) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this.templates.find(function (t) { return t.id === templateId; }) || null];
            });
        });
    };
    // Widget Library
    DashboardBuilder.prototype.getWidgetLibrary = function (category) {
        if (category) {
            return this.widgetLibrary.filter(function (w) { return w.category === category; });
        }
        return this.widgetLibrary;
    };
    DashboardBuilder.prototype.getWidgetTemplate = function (type) {
        return this.widgetLibrary.find(function (w) { return w.type === type; }) || null;
    };
    ...
    DashboardBuilder.prototype.validateLayout = function (layout) {
        ...
        var positions = layout.widget_configuration.map(function (w) { return w.position; });
        ...
        layout.widget_configuration.forEach(function (widget, index) {
            ...
            if (template) {
                var _a = widget.position, w = _a.w, h = _a.h;
        ...
        layout.widget_configuration.forEach(function (widget, index) {
            var _a;
            var template = _this.getWidgetTemplate(widget.type);
            if (template && template.requiredKpis.length > 0 && !((_a = widget.kpi_ids) === null || _a === void 0 ? void 0 : _a.length)) {
                errors.push("Widget ".concat(index + 1, " (").concat(widget.type, ") requires KPI selection"));
            }
        });
        return {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        categories = (kpis === null || kpis === void 0 ? void 0 : kpis.map(function (k) { return k.kpi_category; })) || [];
    ...
    DashboardBuilder.prototype.calculateLayoutPerformance = function (layout) {
        ...
        var chartWidgets = layout.widget_configuration.filter(function (w) { return w.type === 'chart'; }).length;
        var tableWidgets = layout.widget_configuration.filter(function (w) { return w.type === 'table'; }).length;
    ...
    DashboardBuilder.prototype.areKPICategoriesCompatible = function (categories, widgetType) {
        ...
        return categories.every(function (category) { return compatibleCategories.includes(category); });
    };
...
var createDashboardBuilder = function () { return new DashboardBuilder(); };
