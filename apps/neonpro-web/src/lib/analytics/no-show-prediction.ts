...
export class NoShowPredictionEngine {
   ...
   */
  async predictNoShow(appointmentId: string, appointmentData: any): Promise<NoShowPrediction> {
    try {
      // Gather comprehensive data for prediction
      const [patientHistory, appointmentFeatures, externalFactors, communicationPatterns] =
        await Promise.all([
