...
export class SupabaseStorageManager {
   ...
   */
  private compressImage(file: File, options: CompressionOptions = {}): Promise<Blob> {
    const {
      maxWidth = 1920,
      maxHeight = 1080,
      quality = 0.8,
  ...
  async checkPhotoConsent(patientId: string): Promise<boolean> {
    try {
      ...
      const consents = data?.lgpd_consents as { photo_consent?: boolean };
  ...
  async uploadMultiplePhotos(
    ...
    const uploadPromises = files.map(async (file, index) => {
      ...
      // Update progress after each upload completes
      if (onProgress) {
...
export const uploadPatientPhoto = storageManager.uploadPhoto.bind(storageManager);
export const uploadMultiplePatientPhotos = storageManager.uploadMultiplePhotos.bind(storageManager);
export const getPatientPhotos = storageManager.getPatientPhotos.bind(storageManager);
export const deletePatientPhoto = storageManager.deletePhoto.bind(storageManager);
export const downloadPatientPhoto = storageManager.downloadPhoto.bind(storageManager);
export const createPhotoSignedUrl = storageManager.createSignedUrl.bind(storageManager);
export const getPatientPhotoStats = storageManager.getPatientStorageStats.bind(storageManager);
export const checkPatientPhotoConsent = storageManager.checkPhotoConsent.bind(storageManager);
