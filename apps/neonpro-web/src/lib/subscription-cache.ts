...
export class SubscriptionCache {
  ...
  set(key: string, data: SubscriptionValidationResult, customTTL?: number): void {
    ...
    } else if (data.status === 'active' && data.subscription?.cancel_at_period_end) {
  ...
  clear(): void {
    ...
    console.log(`Subscription cache cleared: ${entriesCleared} entries removed`)
  }
  ...
  getStats(): CacheStats {
    ...
    return {
      ...
      oldestEntry: entries.length > 0 ? Math.min(...entries.map(e => e.created)) : null,
      newestEntry: entries.length > 0 ? Math.max(...entries.map(e => e.created)) : null,
...
export const cacheManager = {
   ...
   */
  async warmUp(userId: string, validationResult: SubscriptionValidationResult): Promise<void> {
