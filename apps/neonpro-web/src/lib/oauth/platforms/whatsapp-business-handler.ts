...
export class WhatsAppBusinessOAuthHandler extends BaseOAuthHandler {
  ...
  async getUserInfo(accessToken: string): Promise<OAuthUserInfo> {
    try {
      ...
      if (wabaResponse.ok) {
        ...
        whatsappAccounts = wabaData.data?.flatMap((business: any) => 
          business.whatsapp_business_accounts?.data || []
        ) || [];
      }

      // Get phone numbers for each WABA
      const phoneNumbers = await this.getPhoneNumbers(accessToken, whatsappAccounts);
  ...
  private async getPhoneNumbers(accessToken: string, whatsappAccounts: any[]): Promise<any[]> {
    const phoneNumbers = [];
    ...
    for (const account of whatsappAccounts) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          phoneNumbers.push(...(data.data || []).map((phone: any) => ({
            ...phone,
            waba_id: account.id,
        ...
        console.warn(`Failed to get phone numbers for WABA ${account.id}:`, error);
      }
    }
  ...
  private async getUserPermissions(accessToken: string, userId: string): Promise<string[]> {
    try {
      const response = await fetch(
  ...
  async getMessageTemplates(accessToken: string, wabaId: string): Promise<any[]> {
    try {
      const response = await fetch(
