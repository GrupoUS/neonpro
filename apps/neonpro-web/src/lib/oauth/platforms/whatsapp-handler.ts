...
import { OAuthProviderConfig, OAuthTokenResponse } from '../types';
import { EncryptedToken } from '../token-encryption';
...
export class WhatsAppOAuthHandler extends BaseOAuthHandler {
  ...
  private validateConfig(): void {
    ...
    if (missing.length > 0) {
      throw new Error(`Missing required WhatsApp OAuth configuration: ${missing.join(', ')}`);
    }
  }
  ...
  async getUserInfo(encryptedToken: string): Promise<any> {
    try {
      ...
      const response = await fetch(`${this.config.userInfoUrl}?fields=id,name,email&access_token=${token.accessToken}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'NeonPro/1.0'
  ...
  async validateToken(encryptedToken: string): Promise<boolean> {
    try {
      ...
      const response = await fetch(`${this.config.userInfoUrl}?access_token=${token.accessToken}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'NeonPro/1.0'
  ...
  async getBusinessAccounts(encryptedToken: string): Promise<any[]> {
    try {
      ...
      const response = await fetch(`https://graph.facebook.com/v18.0/me/businesses?access_token=${token.accessToken}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'NeonPro/1.0'
      ...
      if (!response.ok) {
        ...
        throw new Error(`Business accounts fetch failed: ${response.status} ${response.statusText}`);
      }
      ...
      throw error instanceof Error ? error : new Error('Business accounts fetch failed');
    }
  }

  async getPhoneNumbers(encryptedToken: string, businessId: string): Promise<any[]> {
    try {
      const token = await this.decryptToken(encryptedToken);
      
      const response = await fetch(`https://graph.facebook.com/v18.0/${businessId}/phone_numbers?access_token=${token.accessToken}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'NeonPro/1.0'
      ...
      if (!response.ok) {
        ...
        throw new Error(`Phone numbers fetch failed: ${response.status} ${response.statusText}`);
      }
      ...
      throw error instanceof Error ? error : new Error('Phone numbers fetch failed');
    }
  }
}