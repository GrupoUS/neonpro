...
export class TrialManagementEngine {
  ...
  private async calculateConversionFactors(
    ...
    factors.push({
      ...
      value: timeUrgency,
      impact:
        trial.daysRemaining <= 3 ? "positive" : trial.daysRemaining <= 7 ? "neutral" : "negative",
  ...
  private async getCollaborativeFilteringBoost(userId: string): Promise<number> {
    ...
    const { data: similarUsers } = await this.supabase.rpc("get_similar_trial_users", {
    ...
    const avgConversionRate =
      similarUsers.reduce((sum: number, user: any) => sum + (user.converted ? 1 : 0), 0) /
      similarUsers.length;
    ...
    const milestones = [
      {
        ...
        completedAt: events.find((e) => e.type === "page_view" && e.data.page === "dashboard")
          ?.timestamp,
        importance: 0.8,
