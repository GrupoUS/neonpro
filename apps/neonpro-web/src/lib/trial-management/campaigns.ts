...
export class CampaignManager {
  ...
  async createCampaign(campaignData: Partial<TrialCampaign>): Promise<TrialCampaign> {
  ...
  async getEligibleTrials(target: CampaignTarget): Promise<Trial[]> {
    ...
    if (error) throw new Error(`Failed to get eligible trials: ${error.message}`)
  ...
  private async initializeABTest(campaignId: string, abTestConfig: ABTestConfig): Promise<void> {
  ...
  async trackTestConversion(campaignId: string, userId: string, variantId: string): Promise<void> {
    await this.supabase
  ...
  async executeCampaign(campaignId: string, trial: Trial): Promise<void> {
    ...
    const variant = campaign.abTest ? 
      await this.assignTestVariant(trial.userId, campaignId) : 'control'

    // Personalize content
    const personalizedContent = await this.personalizeContent(campaign.content, trial)
  ...
  private async getTopUnusedFeatures(userId: string): Promise<string[]> {
    ...
    const used = new Set(usedFeatures?.map(e => e.event_data?.featureId) || [])
    
    // Return top unused features
    const allFeatures = ['dashboard', 'reports', 'integrations', 'automation', 'analytics']
  ...
  private async trackCampaignDelivery(campaignId: string, userId: string, variant: string): Promise<void> {
    await this.supabase
  ...
  private async scheduleCampaignExecution(campaign: TrialCampaign): Promise<void> {
