...
class FinancialManagementSystem {
  ...
  async createInvoiceWithPaymentTracking(invoiceData: {
    ...
    try {
      ...
      if (invoiceData.appointmentId) {
        invoice = await this.invoiceGenerator.generateFromAppointment(invoiceData.appointmentId, {
  ...
  async getFinancialSummary(filters?: {
    ...
    try {
      ...
      const period = {
        startDate:
          filters?.startDate || new Date(new Date().getFullYear(), new Date().getMonth(), 1),
