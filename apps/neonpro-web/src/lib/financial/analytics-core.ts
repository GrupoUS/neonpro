...
export class FinancialAnalyticsCore {
  ...
  async calculateFinancialKPIs(
    ...
    const receivablesTurnover =
      balanceSheet.assets.current_assets.accounts_receivable > 0
        ? profitLoss.revenue.total_revenue / balanceSheet.assets.current_assets.accounts_receivable
        : 0;
  ...
  async generateDashboardData(clinicId: string): Promise<FinancialDashboardData> {
    ...
    const [currentKPIs, lastMonthKPIs, revenueAnalytics, expenseAnalytics, performanceMetrics] =
      await Promise.all([
