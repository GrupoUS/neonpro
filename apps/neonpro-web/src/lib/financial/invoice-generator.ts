...
class AutomatedInvoiceGenerator {
  ...
  async trackPayment(
    ...
    try {
      ...
      // Store payment
      await this.supabase.from("invoice_payments").insert(payment);
    ...
    try {
      ...
      for (const invoice of invoices) {
        ...
        if (invoice.status === "paid") {
          ...
          if (lastPayment) {
            const paymentTime =
              new Date(lastPayment.paid_at).getTime() - new Date(invoice.issue_date).getTime();
  ...
  private async getPatientData(patientId: string): Promise<any> {
    const { data } = await this.supabase.from("patients").select("*").eq("id", patientId).single();

    return data;
  }

  private async getClinicData(clinicId: string): Promise<any> {
    const { data } = await this.supabase.from("clinics").select("*").eq("id", clinicId).single();
