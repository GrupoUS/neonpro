...
export class FinancialReportingEngine {
  ...
  async generateProfitLossStatement(
    ...
    const treatmentRevenue =
      revenueData
        ?.filter((inv) =>
          inv.invoice_items.some((item) => item.description?.includes("tratamento")),
        )
        .reduce((sum, inv) => sum + inv.total_amount, 0) || 0;
    ...
    const totalOperatingExpenses =
      staffCosts + rentUtilities + marketingExpenses + administrativeExpenses + otherExpenses;
   ...
   */
  async generateBalanceSheet(clinicId: string, asOfDate: string): Promise<BalanceSheet> {
    ...
    if (receivableError)
      throw new Error(`Receivable data fetch failed: ${receivableError.message}`);
    ...
    const totalCurrentAssets =
      cashAndEquivalents + accountsReceivable + inventory + prepaidExpenses + otherCurrentAssets;
  ...
  async getFinancialReports(
    ...
    query = query.order("generated_date", { ascending: false }).range(offset, offset + limit - 1);
