...
export interface InvoiceStatus {
  current: 'draft' | 'pending' | 'sent' | 'viewed' | 'partial' | 'paid' | 'overdue' | 'cancelled'
  history: {
...
export interface PaymentMetrics {
  ...
  paymentsByMethod: Record<PaymentMethod['type'], {
    count: number
    amount: number
    successRate: number
    averageTime: number
  }>
  paymentsByGateway: Record<string, {
    count: number
    amount: number
    fees: number
    successRate: number
...
export interface ComplianceCheck {
  id: string
  type: 'tax_calculation' | 'nfse_generation' | 'payment_processing' | 'data_privacy' | 'financial_reporting'
...
export interface BankIntegration {
  ...
  supportedOperations: ('balance' | 'transactions' | 'payments' | 'transfers')[]
}
