...
class PaymentTracker {
   ...
   */
  async reconcilePayments(gateway: PaymentGateway, date: Date): Promise<PaymentReconciliation> {
    try {
  ...
  private calculatePaymentAnalytics(payments: any[], filters?: any): PaymentAnalytics {
    const period = {
      startDate:
        filters?.startDate ||
        new Date(Math.min(...payments.map((p) => new Date(p.created_at).getTime()))),
      endDate:
        filters?.endDate ||
        new Date(Math.max(...payments.map((p) => new Date(p.created_at).getTime()))),
    };
    ...
    for (const [method, methodPayments] of Object.entries(methodGroups) as [
      PaymentMethod,
      any[],
    ][]) {
    ...
    for (const [gateway, gatewayPayments] of Object.entries(gatewayGroups) as [
      PaymentGateway,
      any[],
    ][]) {
  ...
  private async processDunningForInvoice(invoice: any): Promise<void> {
    ...
    const stage = this.config.dunning.stages.find(
      (s) =>
        daysPastDue >= s.daysAfterDue &&
        daysPastDue <
          (this.config.dunning.stages[this.config.dunning.stages.indexOf(s) + 1]?.daysAfterDue ||
            Infinity),
    );
  ...
  private async getInvoiceData(invoiceId: string): Promise<any> {
    const { data } = await this.supabase.from("invoices").select("*").eq("id", invoiceId).single();
