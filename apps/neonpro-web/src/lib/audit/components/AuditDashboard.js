...
function AuditDashboard(_a) {
    ...
    var _c = (0, react_1.useState)(null), selectedLogId = _c[0], setSelectedLogId = _c[1];
    ...
    // Estado para filtros
    var _d = (0, react_1.useState)({}), logFilters = _d[0], setLogFilters = _d[1];
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      {/* Cards de Resumo */}
      {summaryStats && (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          <card_1.Card>
          ...
          <card_1.Card>
            <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <card_1.CardTitle className="text-sm font-medium">Eventos de Alta Prioridade</card_1.CardTitle>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <card_1.Card>
            <card_1.CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <button_1.Button variant="outline" size="sm" onClick={function () { return logs.exportLogs('csv'); }}>
                    <lucide_react_1.Download className="h-4 w-4 mr-2"/>
                    Exportar CSV
                  </button_1.Button>
                  <button_1.Button variant="outline" size="sm" onClick={function () { return logs.exportLogs('json'); }}>
                    <lucide_react_1.Download className="h-4 w-4 mr-2"/>
                    Exportar JSON
                  </button_1.Button>
                </div>
            ...
            <card_1.CardContent>
              ...
              <div className="grid gap-4 md:grid-cols-4 mb-6">
                <div className="space-y-2">
                  ...
                  <select_1.Select value={logFilters.event_type || ''} onValueChange={function (value) {
            return setLogFilters(function (prev) { return (__assign(__assign({}, prev), { event_type: value })); });
        }}>
                    ...
                    <select_1.SelectContent>
                      ...
                      <select_1.SelectItem value="user.create">Criação de Usuário</select_1.SelectItem>
                      <select_1.SelectItem value="security.failed_login">Login Falhado</select_1.SelectItem>
                      <select_1.SelectItem value="security.suspicious_activity">Atividade Suspeita</select_1.SelectItem>
                    </select_1.SelectContent>
                  </select_1.Select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select_1.Select value={logFilters.severity || ''} onValueChange={function (value) {
            return setLogFilters(function (prev) { return (__assign(__assign({}, prev), { severity: value })); });
        }}>
                    <select_1.SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <input_1.Input id="user-id" placeholder="Filtrar por usuário" value={logFilters.user_id || ''} onChange={function (e) {
            return setLogFilters(function (prev) { return (__assign(__assign({}, prev), { user_id: e.target.value })); });
        }}/>
                </div>
                
                <div className="space-y-2">
                  <label_1.Label htmlFor="ip-address">Endereço IP</label_1.Label>
                  <input_1.Input id="ip-address" placeholder="Filtrar por IP" value={logFilters.ip_address || ''} onChange={function (e) {
            return setLogFilters(function (prev) { return (__assign(__assign({}, prev), { ip_address: e.target.value })); });
        }}/>
                </div>
              </div>
              
              <div className="flex items-center justify-between mb-4">
                <button_1.Button variant="outline" size="sm" onClick={function () { return logs.setFilters(logFilters); }}>
            ...
            logs.setFilters({});
              ...
              <div className="rounded-md border">
                <table_1.Table>
                  ...
                  <table_1.TableBody>
                    {logs.loading ? (<table_1.TableRow>
                        <table_1.TableCell colSpan={7} className="text-center py-8">
                          Carregando logs...
                        </table_1.TableCell>
                      </table_1.TableRow>) : logs.logs.length === 0 ? (<table_1.TableRow>
                        <table_1.TableCell colSpan={7} className="text-center py-8">
                          Nenhum log encontrado
                        </table_1.TableCell>
                      </table_1.TableRow>) : (logs.logs.map(function (log) { return (<table_1.TableRow key={log.id}>
              ...
              {logs.totalCount > 0 && (<div className="flex items-center justify-between mt-4">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {alerts.loading ? (<div className="text-center py-8">Carregando alertas...</div>) : alerts.alerts.length === 0 ? (<div className="text-center py-8">
                  ...
                  </div>) : (alerts.alerts.map(function (alert) { return (<card_1.Card key={alert.id} className="border-l-4 border-l-red-500">
                      <card_1.CardHeader>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <lucide_react_1.AlertTriangle className="h-5 w-5 text-red-500"/>
                            <card_1.CardTitle className="text-lg">{alert.alert_type}</card_1.CardTitle>
                          ...
                          <div className="flex items-center space-x-2">
                            <button_1.Button variant="outline" size="sm" onClick={function () { return alerts.markAsRead(alert.id); }} disabled={alert.status !== 'open'}>
                              Marcar como Lido
                            </button_1.Button>
                            <select_1.Select value={alert.status} onValueChange={function (value) {
                return alerts.updateStatus(alert.id, value);
                              ...
                              <select_1.SelectContent>
                                ...
                                <select_1.SelectItem value="false_positive">Falso Positivo</select_1.SelectItem>
                              </select_1.SelectContent>
                            </select_1.Select>
                          </div>
                        </div>
                ...
                locale: locale_1.ptBR
                            ...
                            {alert.user_id && (<span className="flex items-center">
                                <lucide_react_1.User className="h-4 w-4 mr-1"/>
                                {alert.user_id}
                              </span>)}
                            {alert.ip_address && (<span className="flex items-center">
                      ...
                      <card_1.CardContent>
                        ...
                        {alert.actions_taken.length > 0 && (<div className="mt-4">
                            <h4 className="text-sm font-medium mb-2">Ações Tomadas:</h4>
                            <ul className="text-sm text-muted-foreground space-y-1">
                              {alert.actions_taken.map(function (action, index) { return (<li key={index}>• {action}</li>); })}
                            </ul>
                          </div>)}
                      </card_1.CardContent>
                    </card_1.Card>); }))}
        ...
        <tabs_1.TabsContent value="reports" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {reports.loading ? (<div className="text-center py-8">Carregando relatórios...</div>) : reports.reports.length === 0 ? (<div className="text-center py-8">
                  ...
                  </div>) : (reports.reports.map(function (report) { return (<card_1.Card key={report.id}>
                      <card_1.CardHeader>
                        <div className="flex items-center justify-between">
                          <div>
                            <card_1.CardTitle className="text-lg">{report.title}</card_1.CardTitle>
                            <card_1.CardDescription>{report.description}</card_1.CardDescription>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            <button_1.Button variant="ghost" size="sm" onClick={function () { return reports.deleteReport(report.id); }}>
        ...
        <tabs_1.TabsContent value="statistics" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {statistics.loading ? (<div className="text-center py-8">Carregando estatísticas...</div>) : !statistics.statistics ? (<div className="text-center py-8">
                  ...
                  <button_1.Button className="mt-4" onClick={function () { return statistics.refresh(); }}>
                    Gerar Estatísticas
                  </button_1.Button>
                  ...
                  <div>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      {Object.entries(statistics.statistics.events_by_severity).map(function (_a) {
                var severity = _a[0], count = _a[1];
                return (<card_1.Card key={severity}>
                ...
                return (<div key={type} className="flex items-center justify-between p-3 border rounded">
                ...
                return (<div key={userId} className="flex items-center justify-between p-3 border rounded">
                          <div className="flex items-center">
                            <lucide_react_1.User className="h-4 w-4 mr-2"/>
