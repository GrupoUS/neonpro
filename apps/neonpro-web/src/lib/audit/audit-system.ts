...
export class createauditSystem {
   ...
   */
  async logEvent(event: Omit<AuditEvent, "id" | "timestamp" | "checksum">): Promise<void> {
    try {
      ...
      const { error } = await this.supabase.from("audit_logs").insert(auditEvent);
  ...
  async queryEvents(filters: AuditQueryFilters): Promise<AuditEvent[]> {
    try {
      ...
      return (
        data?.map((event) => ({
          ...event,
          old_values: event.old_values ? this.decryptSensitiveData(event.old_values) : undefined,
          new_values: event.new_values ? this.decryptSensitiveData(event.new_values) : undefined,
        })) || []
      );
    } catch (error) {
  ...
  async getStatistics(filters: Partial<AuditQueryFilters>): Promise<AuditStatistics> {
    try {
      ...
      events.forEach((event) => {
        ...
        // Por usuário
        if (event.user_id) {
          stats.events_by_user[event.user_id] = (stats.events_by_user[event.user_id] || 0) + 1;
        }
  ...
  async archiveOldLogs(retentionDays: number = 365): Promise<number> {
    try {
      ...
      if (selectError) {
        throw new Error(`Erro ao selecionar logs antigos: ${selectError.message}`);
      }
      ...
      const { error: insertError } = await this.supabase.from("audit_logs_archive").insert(oldLogs);
      ...
      if (deleteError) {
        throw new Error(`Erro ao deletar logs arquivados: ${deleteError.message}`);
      }
  ...
  private encryptSensitiveData(data: Record<string, any>): Record<string, any> {
    const sensitiveFields = ["cpf", "email", "phone", "address", "medical_data"];
    const encrypted = { ...data };

    Object.keys(encrypted).forEach((key) => {
      if (sensitiveFields.some((field) => key.toLowerCase().includes(field))) {
        const cipher = crypto.createCipher("aes-256-cbc", this.encryptionKey);
        let encryptedValue = cipher.update(JSON.stringify(encrypted[key]), "utf8", "hex");
