...
var SearchAnalytics = /** @class */ (function () {
     ...
     */
    SearchAnalytics.prototype.trackResultInteraction = function (sessionId, query, resultId, action, position) {
        return __awaiter(this, void 0, void 0, function () {
            var error, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                ...
                switch (_c.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.supabase
                                .from('search_result_interactions')
                                .select('*')
                                .in('session_id', (searchEvents === null || searchEvents === void 0 ? void 0 : searchEvents.map(function (e) { return e.session_id; })) || [])
                                .gte('created_at', startDate.toISOString())];
                    case 2:
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [2 /*return*/, (data === null || data === void 0 ? void 0 : data.map(function (alert) { return ({
                                id: alert.id,
                                type: alert.type,
    ...
    SearchAnalytics.prototype.calculateMetrics = function (searchEvents) {
        ...
        var successfulSearches = searchEvents.filter(function (e) { return e.success; }).length;
        var averageResponseTime = searchEvents.reduce(function (sum, e) { return sum + e.response_time; }, 0) / totalSearches;
        ...
        var searchTypes = Array.from(typeCount.entries())
            .map(function (_a) {
        ...
        var performanceBreakdown = searchEvents.reduce(function (acc, event) {
    ...
    SearchAnalytics.prototype.analyzeBehavior = function (searchEvents, interactions) {
        ...
        var averageSessionDuration = Array.from(sessions.values())
            .reduce(function (sum, session) { return sum + (session.end - session.start); }, 0) / sessions.size;
        ...
        // Click-through rate
        var clickedSessions = new Set(interactions.filter(function (i) { return i.action === 'click'; }).map(function (i) { return i.session_id; }));
        ...
        return {
            ...
            performancePreferences: {
                ...
                prefersFilters: searchEvents.some(function (e) { return e.filters && Object.keys(e.filters).length > 0; }),
                averageResultsViewed: interactions.filter(function (i) { return i.action === 'view'; }).length / totalSearches
            }
        };
    };
                    ...
                    case 0:
                        ...
                        if (event.responseTime > this.performanceThresholds.responseTime) {
                            alerts.push({
                                ...
                                message: "Slow search response: ".concat(event.responseTime, "ms for query \"").concat(event.query, "\""),
                        ...
                        if (!event.success) {
                            alerts.push({
                                ...
                                severity: 'medium',
                                message: "Search failed for query \"".concat(event.query, "\": ").concat(event.errorMessage),
                                metrics: { success: 0 },
                        ...
                        _loop_1 = function (alert_1) {
                            return __generator(this, function (_b) {
                                switch (_b.label) {
                                    case 0: return [4 /*yield*/, this_1.storeAlert(alert_1)];
                                    case 1:
                                        _b.sent();
                                        this_1.alertCallbacks.forEach(function (callback) { return callback(alert_1); });
                                        return [2 /*return*/];
                                }
                            });
                        };
    ...
    SearchAnalytics.prototype.updateRealTimeMetrics = function (event) {
        ...
        if (cached) {
            ...
            cached.data.averageResponseTime = ((cached.data.averageResponseTime * (cached.data.totalSearches - 1) + event.responseTime) /
                cached.data.totalSearches);
            if (event.success) {
                cached.data.successRate = ((cached.data.successRate * (cached.data.totalSearches - 1) + 1) /
                    cached.data.totalSearches);
            }
            else {
                cached.data.successRate = ((cached.data.successRate * (cached.data.totalSearches - 1)) /
                    cached.data.totalSearches);
            }
        }
    };
    SearchAnalytics.prototype.generateOptimizations = function (slowQueries) {
        ...
        slowQueries.forEach(function (query) {
            ...
            var pattern = query.query.toLowerCase().replace(/\d+/g, 'N').replace(/[^a-z\s]/g, '');
            ...
            if (count >= 5) { // Pattern appears in at least 5 slow queries
                optimizations.push({
                    ...
                    optimization: {
                        ...
                        implementation: "CREATE INDEX idx_search_".concat(pattern.replace(/\s+/g, '_'), " ON relevant_table (...)")
                    },
                    impact: {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.getSearchMetrics(__assign(__assign({}, options), { timeRange: { start: dayStart, end: dayEnd } }))];
                    case 2:
                        dayMetrics = _a.sent();
                        trends.push({
    ...
    SearchAnalytics.prototype.generateRecommendations = function (metrics, alerts, optimizations) {
        ...
        if (metrics.averageResponseTime > this.performanceThresholds.responseTime) {
            recommendations.push("Tempo de resposta m\u00E9dio (".concat(Math.round(metrics.averageResponseTime), "ms) est\u00E1 acima do limite recomendado. Considere implementar cache ou otimizar consultas."));
        }
        if (metrics.successRate < this.performanceThresholds.successRate) {
            recommendations.push("Taxa de sucesso (".concat(Math.round(metrics.successRate * 100), "%) est\u00E1 abaixo do esperado. Revise o tratamento de erros e valida\u00E7\u00E3o de consultas."));
        }
        // Alert-based recommendations
        var criticalAlerts = alerts.filter(function (a) { return a.severity === 'critical'; }).length;
        if (criticalAlerts > 0) {
            recommendations.push("".concat(criticalAlerts, " alertas cr\u00EDticos detectados. A\u00E7\u00E3o imediata necess\u00E1ria para manter a qualidade do servi\u00E7o."));
        }
        // Optimization recommendations
        if (optimizations.length > 0) {
            var highImpactOptimizations = optimizations.filter(function (o) { return o.impact.potentialSpeedup > 0.3; }).length;
            if (highImpactOptimizations > 0) {
                recommendations.push("".concat(highImpactOptimizations, " otimiza\u00E7\u00F5es de alto impacto identificadas. Implementa\u00E7\u00E3o pode melhorar performance significativamente."));
            }
        }
        // Usage pattern recommendations
        if (metrics.userEngagement.clickThroughRate < this.performanceThresholds.clickThroughRate) {
            recommendations.push("Taxa de cliques baixa (".concat(Math.round(metrics.userEngagement.clickThroughRate * 100), "%). Considere melhorar relev\u00E2ncia dos resultados ou interface de usu\u00E1rio."));
        }
...
var createsearchAnalytics = function () { return new SearchAnalytics(); };
