...
export class PatientSegmentation {
  ...
  async createSegment(
    ...
    try {
      ...
      const criteria: SegmentCriteria = {
        name,
        description:
          description || `Segmento criado a partir da consulta: "${naturalLanguageQuery}"`,
  ...
  private extractDemographicCriteria(
    ...
    const genderWords =
      genderKeywords[nlpResult.language as keyof typeof genderKeywords] || genderKeywords.pt;
  ...
  private extractFinancialCriteria(
    ...
    const insuranceTypes =
      insuranceKeywords[lang]?.filter((type) => queryLower.includes(type.toLowerCase())) || [];
    ...
    if (structuredCriteria.demographics) {
      ...
      if (demo.location && demo.location.length > 0) {
        const locationConditions = demo.location.map(
          (loc) => `(address ILIKE '%${loc}%' OR city ILIKE '%${loc}%' OR state ILIKE '%${loc}%')`,
        );
  ...
  async deleteSegment(segmentId: string): Promise<void> {
    try {
      const { error } = await this.supabase.from("patient_segments").delete().eq("id", segmentId);
