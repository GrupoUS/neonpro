...
class HealthcareQueryStrategies {
  ...
  public async executeQuery<T = any>(
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error(`Query timeout after ${strategy.timeout}ms`)), strategy.timeout)
      })
      
      // Execute query with timeout
      const queryPromise = this.executeWithRetry(queryFn, client, strategy.retryAttempts, context)
   ...
   */
  private getOptimalClient(context: QueryContext, strategy: QueryStrategy): SupabaseClient<Database> {
   ...
   */
  private verifyLGPDCompliance<T>(context: QueryContext, data: T): boolean {
   ...
   * Verify financial data compliance
   */
  private verifyFinancialCompliance<T>(context: QueryContext, data: T): boolean {
   ...
   */
  private generateOptimizations(queryType: HealthcareQueryType, strategy: QueryStrategy): string[] {
   ...
   */
  public updateStrategy(queryType: HealthcareQueryType, updates: Partial<QueryStrategy>): void {
...
export const HealthcareQueries = {
  patientData: (patientId: string) => createQueryContext(
    '', '', 'patient_standard', { patientId, lgpdSensitive: true }
  ),
  
  emergencyAccess: (patientId: string) => createQueryContext(
    '', '', 'patient_critical', { patientId, priority: 'emergency', lgpdSensitive: true }
  ),
  
  clinicalWorkflow: () => createQueryContext(
    '', '', 'clinical_workflow', { priority: 'high' }
  ),
  
  analytics: () => createQueryContext(
    '', '', 'analytics_readonly', { requiresAudit: false, lgpdSensitive: false }
  ),
  
  financial: () => createQueryContext(
    '', '', 'financial_sensitive', { priority: 'high', lgpdSensitive: true }
  )
}
