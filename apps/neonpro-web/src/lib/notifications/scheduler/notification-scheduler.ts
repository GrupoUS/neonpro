...
export class NotificationScheduler {
    ...
    try {
      for (const notification of notifications) {
        try {
          ...
          if (
            this.isInQuietHours(finalTime, notification.notification_schedules.options?.quietHours)
          ) {
  ...
  private async resolveCustomAudience(filters: any[], clinicId: string): Promise<string[]> {
    ...
    try {
      let query = this.supabase.from("profiles").select("id").eq("clinic_id", clinicId);
