...
export class NotificationAnalytics {
  ...
  private calculateMetrics(data: any[]): NotificationMetrics {
    ...
    // Métricas de tempo
    const deliveredNotifications = data.filter(n => n.delivered_at && n.sent_at);
    ...
    const respondedNotifications = data.filter(n => n.opened_at && n.delivered_at);
   ...
   */
  private calculateChannelBreakdown(data: any[]): Record<NotificationChannel, ChannelMetrics> {
    ...
    channels.forEach(channel => {
      ...
      const delivered = channelData.filter(n => n.status === 'delivered').length;
      const failed = channelData.filter(n => n.status === 'failed').length;
      const engaged = channelData.filter(n => n.opened_at || n.clicked_at).length;
      
      const engagementRate = delivered > 0 ? (engaged / delivered) * 100 : 0;
      const reliability = sent > 0 ? (delivered / sent) * 100 : 0;
      const avgCost = channelData.reduce((acc, n) => acc + (n.cost || 0), 0) / sent || 0;
   ...
   */
  async generateReport(query: AnalyticsQuery, clinicId: string): Promise<AnalyticsReport> {
    ...
    try {
      ...
      if (error) throw new Error(`Erro ao buscar dados históricos: ${error.message}`);
      ...
      const insights = await this.generatePredictiveInsights(historicalData || [], clinicId);
   ...
   */
  private generateTimeSeriesData(data: any[], groupBy: string): TimeSeriesData[] {
   ...
   */
  private async generatePredictiveInsights(data: any[], clinicId: string): Promise<PredictiveInsights> {
  ...
  private calculateBestSendTime(data: any[]): string {
    const hourlyEngagement = new Map<number, { sent: number; engaged: number }>();
  ...
  private calculateEngagementProbability(data: any[]): number {
    ...
    const engaged = recentData.filter(n => n.opened_at || n.clicked_at).length;
  ...
  private calculateChurnRisk(data: any[]): number {
    ...
    const previous = last60Days.filter(n => n.opened_at || n.clicked_at).length;
   ...
   */
  private generateRecommendations(metrics: NotificationMetrics, trends: TimeSeriesData[]): string[] {
    ...
    // Análise de canais
    Object.entries(metrics.channelBreakdown).forEach(([channel, channelMetrics]) => {
    ...
    const deliveryTrend = recentTrends.filter(t => t.metric === 'delivery_rate');
    
    if (deliveryTrend.length >= 2) {
      const isDecreasing = deliveryTrend[deliveryTrend.length - 1].value < deliveryTrend[0].value;
      if (isDecreasing) {
        recommendations.push(
