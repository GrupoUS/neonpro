      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 2:
            ...
            if (error) throw new Error("Erro ao buscar m\u00E9tricas: ".concat(error.message));
  ...
  NotificationAnalytics.prototype.generateRecommendations = function (metrics, trends) {
    ...
    if (metrics.deliveryRate < 90) {
      recommendations.push(
        "\uD83D\uDCC9 Taxa de entrega baixa (".concat(
          metrics.deliveryRate.toFixed(1),
          "%). Verifique configura\u00E7\u00F5es de DNS e reputa\u00E7\u00E3o do remetente.",
        ),
      );
    }
    if (metrics.openRate < 20) {
      recommendations.push(
        "\uD83D\uDCE7 Taxa de abertura baixa (".concat(
          metrics.openRate.toFixed(1),
          "%). Otimize assuntos e hor\u00E1rios de envio.",
        ),
      );
    }
    if (metrics.clickRate < 5) {
      recommendations.push(
        "\uD83C\uDFAF Taxa de clique baixa (".concat(
          metrics.clickRate.toFixed(1),
          "%). Melhore call-to-actions e relev\u00E2ncia do conte\u00FAdo.",
        ),
      );
    }
    // Análise de canais
    Object.entries(metrics.channelBreakdown).forEach(function (_a) {
      ...
      if (channelMetrics.reliability < 85) {
        recommendations.push(
          "\uD83D\uDD27 Canal "
            .concat(channel, " com baixa confiabilidade (")
            .concat(
              channelMetrics.reliability.toFixed(1),
              "%). Considere configura\u00E7\u00E3o ou provider alternativo.",
            ),
        );
      }
    });
    ...
    if (metrics.costPerDelivery > 0.5) {
      recommendations.push(
        "\uD83D\uDCB0 Custo por entrega alto (R$ ".concat(
          metrics.costPerDelivery.toFixed(2),
          "). Avalie otimiza\u00E7\u00E3o de canais e providers.",
        ),
      );
    }
    // Análise de ROI
    if (metrics.roi < 2) {
      recommendations.push(
        "\uD83D\uDCCA ROI baixo (".concat(
          ...
          "x). Foque em segmenta\u00E7\u00E3o e personaliza\u00E7\u00E3o para melhorar convers\u00F5es.",
        ),
      );
    }
