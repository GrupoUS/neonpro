...
var TemplateEngine = /** @class */ (function () {
        ...
        this.functions.set('formatPhone', function (context) {
            return function (phone) {
                ...
                if (cleaned.length === 11) {
                    return "(".concat(cleaned.slice(0, 2), ") ").concat(cleaned.slice(2, 7), "-").concat(cleaned.slice(7));
                }
                return phone;
            };
        });
    ...
    TemplateEngine.prototype.renderString = function (template, variables, context) {
        ...
        result = result.replace(/\{\?\s*([^}]+)\s*\}([\s\S]*?)\{\/\1\}/g, function (match, condition, content) {
            try {
    ...
    TemplateEngine.prototype.extractVariables = function (context) {
        return __assign(__assign({ 
            ...
            // Dados da clínica
            clinic: context.clinic }, context.data), { 
            // Dados de sistema
            timestamp: context.timestamp, locale: context.locale, 
            // Dados de data/hora
            now: new Date(), today: new Date().toISOString().split('T')[0], 
            // URLs úteis
            urls: {
                app: process.env.NEXT_PUBLIC_APP_URL || 'https://app.neonpro.com',
                unsubscribe: "".concat(process.env.NEXT_PUBLIC_APP_URL, "/unsubscribe?token=").concat(context.recipient.id),
                preferences: "".concat(process.env.NEXT_PUBLIC_APP_URL, "/preferences?token=").concat(context.recipient.id)
            } });
    };
    /**
    ...
    TemplateEngine.prototype.evaluateFunction = function (funcCall, context) {
        ...
        var funcName = match[1], argsStr = match[2];
        ...
        var args = argsStr ? argsStr.split(',').map(function (arg) {
    ...
    TemplateEngine.prototype.evaluateCondition = function (condition, variables, context) {
        ...
        for (var _i = 0, operators_1 = operators; _i < operators_1.length; _i++) {
            ...
            if (condition.includes(op)) {
                var _a = condition.split(op).map(function (s) { return s.trim(); }), left = _a[0], right = _a[1];
    ...
    TemplateEngine.prototype.getDefaultTemplate = function (type, channel) {
        ...
        var template = templates.find(function (t) { return t.type === type && t.channel === channel; });
        if (template) {
            return template;
        }
        // Template genérico
