...
export class ConflictDetectionEngine {
  ...
  async detectConflicts(appointmentId?: string): Promise<ConflictDetectionResponse> {
    ...
    try {
      ...
      const { data: conflicts, error } = await this.supabase.rpc("detect_scheduling_conflicts", {
  ...
  private async getSystemStatus(): Promise<SystemStatus> {
    ...
    return {
      isHealthy:
        (avgDetectionLatency || 0) <= this.config.performanceThresholds.maxDetectionLatencyMs,
