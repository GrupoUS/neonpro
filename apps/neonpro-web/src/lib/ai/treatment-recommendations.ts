...
export class AITreatmentRecommendationEngine {
  ...
  private async scoreTreatments(
    ...
    for (const treatment of treatments) {
      const score = await this.calculateTreatmentScore(treatment, patientProfile, riskAssessment);
  ...
  private async calculateTreatmentScore(
    ...
    const riskCompatibility = this.calculateRiskCompatibility(treatmentId, riskAssessment);
    ...
    const preferenceMatch = this.calculatePreferenceMatch(treatmentId, patientProfile);
  ...
  private async createDetailedRecommendation(
    ...
    const prerequisites = this.identifyPrerequisites(treatmentId, patientProfile, riskAssessment);
  ...
  private calculateEvidenceQuality(treatmentId: string): number {
    ...
    const avgConfidence =
      evidence.reduce((sum, e) => sum + e.confidence_level, 0) / evidence.length;
