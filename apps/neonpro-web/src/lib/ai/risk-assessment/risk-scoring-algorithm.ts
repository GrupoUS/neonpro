...
class RiskScoringAlgorithm {
   ...
   */
  private generateRecommendations(components: RiskScoreComponents, riskLevel: string, trends: any) {
  ...
  private async loadRiskFactors(): Promise<void> {
    try {
      const { data: factors } = await this.supabase.from("risk_factors").select("*");
