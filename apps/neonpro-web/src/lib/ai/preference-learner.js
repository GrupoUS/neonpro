...
var PatientPreferenceLearner = /** @class */ (function () {
    ...
    PatientPreferenceLearner.prototype.learnFromAppointmentHistory = function (patientId) {
        return __awaiter(this, void 0, void 0, function () {
            var appointmentHistory, timePatterns, staffPatterns, treatmentPatterns, satisfactionPatterns, updatedPreferences, confidenceImprovement, newPatterns, recommendations, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        updatedPreferences = this.generateUpdatedPreferences(patientId, timePatterns, staffPatterns, treatmentPatterns, satisfactionPatterns);
                        ...
                        recommendations = this.generateRecommendations(timePatterns, staffPatterns, satisfactionPatterns);
    ...
    PatientPreferenceLearner.prototype.analyzeTimePatterns = function (appointments) {
        ...
        var preferredDays = dayOfWeekCounts
            .map(function (count, index) { return ({ day: index, count: count, satisfaction: avgSatisfactionByDay[index] }); })
            .filter(function (item) { return item.count > 0; })
            .sort(function (a, b) { return (b.count * b.satisfaction) - (a.count * a.satisfaction); })
            .slice(0, 3)
            .map(function (item) { return item.day; });
        var preferredHours = hourOfDayCounts
            .map(function (count, index) { return ({ hour: index, count: count, satisfaction: avgSatisfactionByHour[index] }); })
            .filter(function (item) { return item.count > 0; })
            .sort(function (a, b) { return (b.count * b.satisfaction) - (a.count * a.satisfaction); })
            .slice(0, 4)
            .map(function (item) { return item.hour; });
        return {
            preferred_days: preferredDays,
            preferred_hours: preferredHours,
            avoid_early_morning: avgSatisfactionByHour.slice(6, 9).every(function (score) { return score < 3.5; }),
            avoid_late_evening: avgSatisfactionByHour.slice(18, 21).every(function (score) { return score < 3.5; }),
    ...
    PatientPreferenceLearner.prototype.analyzeStaffPatterns = function (appointments) {
        ...
        var preferredStaff = staffScores
            .filter(function (item) { return item.appointment_count >= 2; })
            .sort(function (a, b) { return b.score - a.score; })
            .slice(0, 3)
            .map(function (item) { return item.staff_id; });
        var avoidStaff = staffScores
            .filter(function (item) { return item.appointment_count >= 2 && item.avg_satisfaction < 3.0; })
            .map(function (item) { return item.staff_id; });
        return {
            preferred_staff_ids: preferredStaff,
            avoid_staff_ids: avoidStaff,
            staff_loyalty_score: preferredStaff.length > 0 ? 0.8 : 0.3,
            confidence: Math.min(1.0, staffScores.reduce(function (sum, item) { return sum + item.appointment_count; }, 0) / 15)
        };
    };
    PatientPreferenceLearner.prototype.analyzeTreatmentPatterns = function (appointments) {
        ...
        var preferredTreatments = treatmentAnalysis
            .filter(function (item) { return item.avg_satisfaction >= 4.0; })
            .sort(function (a, b) { return b.frequency - a.frequency; })
            .map(function (item) { return item.treatment_type; });
    ...
    PatientPreferenceLearner.prototype.analyzeSatisfactionPatterns = function (appointments) {
        var satisfactionData = appointments
            .filter(function (apt) { return apt.satisfaction_score; })
        ...
        var overallSatisfaction = satisfactionData.reduce(function (sum, item) { return sum + item.satisfaction; }, 0) / satisfactionData.length;
        // Identify satisfaction patterns
        var patterns = [];
        // Time-based satisfaction patterns
        var morningApts = satisfactionData.filter(function (item) { return item.hour_of_day < 12; });
        var afternoonApts = satisfactionData.filter(function (item) { return item.hour_of_day >= 12; });
        if (morningApts.length > 0 && afternoonApts.length > 0) {
            var morningAvg = morningApts.reduce(function (sum, item) { return sum + item.satisfaction; }, 0) / morningApts.length;
            var afternoonAvg = afternoonApts.reduce(function (sum, item) { return sum + item.satisfaction; }, 0) / afternoonApts.length;
            if (Math.abs(morningAvg - afternoonAvg) > 0.5) {
                patterns.push({
                    type: 'time_preference',
                    description: morningAvg > afternoonAvg ? 'Prefers morning appointments' : 'Prefers afternoon appointments',
                    confidence: Math.min(1.0, Math.abs(morningAvg - afternoonAvg))
                });
            }
        }
        // Wait time sensitivity
        var lowWaitApts = satisfactionData.filter(function (item) { return item.wait_time < 15; });
        var highWaitApts = satisfactionData.filter(function (item) { return item.wait_time >= 15; });
        if (lowWaitApts.length > 0 && highWaitApts.length > 0) {
            var lowWaitAvg = lowWaitApts.reduce(function (sum, item) { return sum + item.satisfaction; }, 0) / lowWaitApts.length;
            var highWaitAvg = highWaitApts.reduce(function (sum, item) { return sum + item.satisfaction; }, 0) / highWaitApts.length;
    ...
    PatientPreferenceLearner.prototype.generateUpdatedPreferences = function (patientId, timePatterns, staffPatterns, treatmentPatterns, satisfactionPatterns) {
        var combinedConfidence = (timePatterns.confidence * 0.3 +
        ...
        return {
            ...
            communication_preferences: {
                wait_sensitivity: satisfactionPatterns.patterns.some(function (p) { return p.type === 'wait_sensitivity'; }),
    ...
    PatientPreferenceLearner.prototype.calculateConfidenceImprovement = function (appointments) {
        ...
        var dataQuality = appointments.filter(function (apt) { return apt.satisfaction_score; }).length / appointments.length;
    ...
    PatientPreferenceLearner.prototype.checkDayConsistency = function (appointments) {
        var days = appointments.map(function (apt) { return new Date(apt.appointment_time).getDay(); });
    ...
    PatientPreferenceLearner.prototype.checkTimeConsistency = function (appointments) {
        var hours = appointments.map(function (apt) { return new Date(apt.appointment_time).getHours(); });
    ...
    PatientPreferenceLearner.prototype.generateRecommendations = function (timePatterns, staffPatterns, satisfactionPatterns) {
        var recommendations = [];
        if (timePatterns.confidence > 0.7) {
            if (timePatterns.preferred_hours.length > 0) {
                ...
                recommendations.push("Schedule appointments between ".concat(hourRanges, " for optimal satisfaction"));
            }
        ...
        if (satisfactionPatterns.patterns.some(function (p) { return p.type === 'wait_sensitivity'; })) {
    ...
    PatientPreferenceLearner.prototype.formatHourRanges = function (hours) {
        if (hours.length === 0)
            return '';
        var sortedHours = __spreadArray([], hours, true).sort(function (a, b) { return a - b; });
        var ranges = [];
        ...
        for (var i = 1; i < sortedHours.length; i++) {
            if (sortedHours[i] === end + 1) {
                ...
                ranges.push(start === end ? "".concat(start, ":00") : "".concat(start, ":00-").concat(end, ":00"));
    ...
    PatientPreferenceLearner.prototype.analyzeCancellationPatterns = function (appointments) {
        var cancelled = appointments.filter(function (apt) { return apt.status === 'cancelled'; });
        return {
