...
export class PatientInsightsIntegration {
  ...
  private async generateInsightsParallel(request: PatientInsightRequest): Promise<InsightResults> {
  ...
  private async generateInsightsSequential(
    ...
    try {
      ...
      if (this.configuration.enableBehaviorAnalysis) {
        results.behaviorAnalysis = await this.behaviorEngine.analyzeBehaviorPatterns(
          request.patientId,
        );
      }
