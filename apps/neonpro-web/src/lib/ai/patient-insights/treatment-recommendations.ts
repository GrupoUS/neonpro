...
export class TreatmentRecommendationEngine {
  ...
  async generateRecommendations(
    ...
    try {
      ...
      const safeRecommendations = this.filterContraindications(rankedRecommendations, riskAssessment)
  ...
  private async analyzeTreatment(
    ...
    const contraindications = this.checkContraindications(treatment, patientData, riskAssessment)
    ...
    const estimatedDuration = this.calculateEstimatedDuration(treatment, patientData)
  ...
  private calculateSuccessProbability(
    ...
    if (patientData.treatments?.length > 0) {
      const successfulTreatments = patientData.treatments.filter(t => t.outcome === 'successful').length
    ...
    const medicalRisks = riskAssessment.riskFactors.filter(rf => rf.category === 'medical')
  ...
  private assessTreatmentRisk(
    ...
    const highRiskFactors = riskAssessment.riskFactors.filter(rf => 
  ...
  private checkContraindications(
    ...
    const medicalConditions = patientData.medical_history?.map(mh => mh.condition_type) || []
    ...
    const medications = patientData.medications?.map(m => m.medication_name.toLowerCase()) || []
    ...
    const allergens = patientData.allergies?.map(a => a.allergen.toLowerCase()) || []
    ...
    const criticalRisks = riskAssessment.riskFactors.filter(rf => rf.severity === 'critical')
  ...
  private getEvidenceLevel(treatment: any, treatmentGoal?: string): TreatmentRecommendation['evidenceLevel'] {
    const evidence = this.evidenceDatabase.get(treatment.id) || this.evidenceDatabase.get(treatment.category)
  ...
  private calculateEstimatedCost(treatment: any, patientData: any): number {
    ...
    const complicatingConditions = patientData.medical_history?.filter(mh => 
  ...
  private getExpectedOutcome(treatment: any, successProbability: number): string {
    const baseOutcome = treatment.expected_outcome || 'Improvement in treated area'
  ...
  private rankRecommendations(recommendations: TreatmentRecommendation[]): TreatmentRecommendation[] {
    return recommendations.sort((a, b) => {
      ...
      const evidenceWeight = { 'very_strong': 4, 'strong': 3, 'moderate': 2, 'weak': 1 }
...
interface TreatmentEvidence {
  ...
  goalSpecificStudies: Record<string, {
    evidenceLevel: TreatmentRecommendation['evidenceLevel']
    studyCount: number
  }>
}