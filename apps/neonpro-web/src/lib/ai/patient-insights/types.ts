...
export interface RiskFactor {
  id: string
  name: string
  category: 'medical' | 'lifestyle' | 'genetic' | 'environmental' | 'behavioral'
...
export interface HealthPrediction {
  type: 'complication_risk' | 'treatment_success' | 'recovery_time' | 'satisfaction_score'
...
export interface SafetyAlert {
  ...
  type: 'contraindication' | 'drug_interaction' | 'allergy_risk' | 'medical_condition' | 'age_factor'
  message: string
...
export interface PatientBehaviorPattern {
  ...
  patterns: {
    ...
    communicationPreference: 'phone' | 'email' | 'sms' | 'whatsapp' | 'in_person'
...
export interface AIModelPerformance {
  modelId: string
  modelType: 'risk_assessment' | 'treatment_recommendation' | 'outcome_prediction' | 'behavior_analysis'
  version: string
  trainingDate: Date
  accuracy: number
  precision: number
