...
class OptimizationEngine {
  ...
  async optimizeScheduling(
    ...
    try {
      ...
      const metrics = this.calculateOptimizationMetrics(optimizedRecommendations, startTime);

      // 7. Determine confidence score
      const confidenceScore = this.calculateConfidenceScore(optimizedRecommendations, metrics);
