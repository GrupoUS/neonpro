...
var AISchedulingCore = /** @class */ (function () {
    function AISchedulingCore(config) {
        ...
        this.config = __assign({ maxLookAheadDays: 30, staffEfficiencyWeight: 0.25, patientPreferenceWeight: 0.20, revenueOptimizationWeight: 0.20, treatmentSequencingWeight: 0.15, workloadBalanceWeight: 0.20 }, config);
    }
    /**
    ...
    AISchedulingCore.prototype.generateSchedulingRecommendations = function (criteria) {
        return __awaiter(this, void 0, void 0, function () {
            var patientPreferences_1, availableSlots, staffEfficiencyData_1, scoredSlots, recommendations, error_1;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 6, , 7]);
                        return [4 /*yield*/, this.loadPatientPreferences(criteria.patientId)
                            // 2. Get available time slots
                        ];
                    case 1:
                        patientPreferences_1 = _a.sent();
                        return [4 /*yield*/, this.getAvailableTimeSlots(criteria.treatmentId, criteria.maxWaitDays)
                            // 3. Load staff efficiency patterns
                        ];
                    case 2:
                        availableSlots = _a.sent();
                        return [4 /*yield*/, this.loadStaffEfficiencyPatterns()
                            // 4. Calculate optimization scores for each slot
                        ];
                    case 3:
                        staffEfficiencyData_1 = _a.sent();
                        return [4 /*yield*/, Promise.all(availableSlots.map(function (slot) { return _this.calculateOptimizationScore(slot, criteria, patientPreferences_1, staffEfficiencyData_1); }))
                            // 5. Sort by optimization score and return top recommendations
                        ];
                    case 4:
                        scoredSlots = _a.sent();
                        recommendations = scoredSlots
                            .sort(function (a, b) { return b.optimizationScore - a.optimizationScore; })
     ...
     */
    AISchedulingCore.prototype.calculateOptimizationScore = function (slot, criteria, patientPreferences, staffEfficiency) {
        return __awaiter(this, void 0, void 0, function () {
            var scores, reasoning, staffId, staffPattern, _a, _b, _c, optimizationScore, confidence, estimatedRevenue, patientSatisfactionPrediction;
            var _d;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    ...
                    case 1:
                        staffId = _e.sent();
                        staffPattern = (_d = staffEfficiency.get(staffId)) === null || _d === void 0 ? void 0 : _d[0];
                        if (staffPattern) {
                            scores.staffEfficiency = this.calculateStaffEfficiencyScore(slot, staffPattern);
                            reasoning.push("Staff efficiency: ".concat((scores.staffEfficiency * 100).toFixed(1), "%"));
                        }
                        // 2. Patient Preference Score
                        scores.patientPreference = this.calculatePatientPreferenceScore(slot, patientPreferences);
                        reasoning.push("Patient preference match: ".concat((scores.patientPreference * 100).toFixed(1), "%"));
                    ...
                    case 2:
                        ...
                        reasoning.push("Revenue optimization: ".concat((scores.revenueOptimization * 100).toFixed(1), "%"));
                    ...
                    case 3:
                        ...
                        reasoning.push("Treatment sequencing: ".concat((scores.treatmentSequencing * 100).toFixed(1), "%"));
                        _e.label = 4;
                    case 4:
                    ...
                    case 5:
                        ...
                        reasoning.push("Workload balance: ".concat((scores.workloadBalance * 100).toFixed(1), "%"));
                        optimizationScore = (scores.staffEfficiency * this.config.staffEfficiencyWeight +
                    ...
                    case 6:
                        ...
                        patientSatisfactionPrediction = this.predictPatientSatisfaction(slot, patientPreferences, scores);
                        return [2 /*return*/, {
                                timeSlot: slot,
                                staffId: staffId,
    ...
    AISchedulingCore.prototype.learnPatientPreferences = function (patientId) {
        return __awaiter(this, void 0, void 0, function () {
            var appointments, dayPreferences, timePreferences, leadTimePattern, cancellationPattern, satisfactionScore, preferences, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
    ...
    AISchedulingCore.prototype.getAvailableTimeSlots = function (treatmentId, maxWaitDays) {
        return __awaiter(this, void 0, void 0, function () {
            var endDate, treatment, clinicHours, existingAppointments, availableSlots, currentDate, _loop_1, this_1, day, error_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 4, , 5]);
                    ...
                    case 3:
                        ...
                        _loop_1 = function (day) {
                            ...
                            var dayHours = clinicHours === null || clinicHours === void 0 ? void 0 : clinicHours.find(function (h) { return h.day_of_week === dayOfWeek; });
                            if (dayHours && dayHours.is_open) {
                                var slots = this_1.generateDaySlots(checkDate, dayHours, treatment.duration_minutes, existingAppointments || []);
     ...
     */
    AISchedulingCore.prototype.generateDaySlots = function (date, dayHours, durationMinutes, existingAppointments) {
        var slots = [];
        var slotInterval = 30; // 30-minute intervals
        var startTime = new Date(date);
        var _a = dayHours.start_time.split(':').map(Number), startHour = _a[0], startMinute = _a[1];
        startTime.setHours(startHour, startMinute, 0, 0);
        var endTime = new Date(date);
        var _b = dayHours.end_time.split(':').map(Number), endHour = _b[0], endMinute = _b[1];
    ...
    AISchedulingCore.prototype.calculatePatientPreferenceScore = function (slot, preferences) {
        ...
        var slotTime = "".concat(slotHour.toString().padStart(2, '0'), ":").concat(slotMinute.toString().padStart(2, '0'));
    ...
    AISchedulingCore.prototype.calculateConfidence = function (scores, patientPreferences, staffPattern) {
        ...
        if (patientPreferences.lastLearningUpdate) {
            var daysSinceUpdate = (Date.now() - patientPreferences.lastLearningUpdate.getTime()) / (1000 * 60 * 60 * 24);
    ...
    AISchedulingCore.prototype.analyzeDayPreferences = function (appointments) {
        ...
        return dayCount
            .map(function (count, day) { return ({ day: day, count: count }); })
            .filter(function (item) { return item.count >= maxCount * 0.3; })
    ...
    AISchedulingCore.prototype.analyzeTimePreferences = function (appointments) {
        ...
        var hours = appointments.map(function (apt) { return new Date(apt.start_time).getHours(); });
        var avgHour = hours.reduce(function (sum, hour) { return sum + hour; }, 0) / hours.length;
        return [{
                start: "".concat(Math.max(8, Math.floor(avgHour - 1)).toString().padStart(2, '0'), ":00"),
                end: "".concat(Math.min(18, Math.ceil(avgHour + 1)).toString().padStart(2, '0'), ":00")
            }];
    };
    AISchedulingCore.prototype.analyzeBookingLeadTime = function (appointments) {
        ...
        var leadTimes = appointments
            .filter(function (apt) { return apt.created_at && apt.start_time; })
            .map(function (apt) {
        ...
        return leadTimes.length > 0
            ? leadTimes.reduce(function (sum, days) { return sum + days; }, 0) / leadTimes.length
            : 7; // Default to 7 days
    };
    ...
    AISchedulingCore.prototype.calculateAverageSatisfaction = function (appointments) {
        ...
        var feedbackScores = appointments
            .filter(function (apt) { var _a; return ((_a = apt.appointment_feedback) === null || _a === void 0 ? void 0 : _a.length) > 0; })
            .map(function (apt) { return apt.appointment_feedback[0].satisfaction_score; })
            .filter(function (score) { return score !== null; });
        return feedbackScores.length > 0
            ? feedbackScores.reduce(function (sum, score) { return sum + score; }, 0) / feedbackScores.length
