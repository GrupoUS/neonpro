...
import type { SchedulingCriteria, SchedulingRecommendation } from "./ai-scheduling-core";
...
class ComplianceRulesEngine {
  ...
  async validateCompliance(
    ...
    try {
      ...
      const result: ComplianceValidationResult = {
        isCompliant:
          violations.filter((v) => v.severity === "critical" || v.severity === "error").length ===
          0,
        violations,
        warnings,
        requiredApprovals,
  ...
  async addComplianceRule(
    ...
    try {
      ...
      // Save to database
      await this.supabase.from("compliance_rules").insert({
        id: newRule.id,
        name: newRule.name,
        type: newRule.type,
