...
class AISchedulingCore {
    ...
    try {
      ...
      const scoredSlots = await Promise.all(
        availableSlots.map((slot) =>
          this.calculateOptimizationScore(slot, criteria, patientPreferences, staffEfficiencyData),
        ),
      );
  ...
  private async getAvailableTimeSlots(
    ...
    try {
      ...
      const { data: clinicHours } = await this.supabase.from("clinic_operating_hours").select("*");
