...
class AISchedulingCore {
  ...
  private staffEfficiencyCache = new Map<string, StaffEfficiencyPattern[]>()
  ...
  constructor(config: Partial<AISchedulingConfig> = {}) {
    this.config = {
      ...
      patientPreferenceWeight: 0.20,
      revenueOptimizationWeight: 0.20,
      treatmentSequencingWeight: 0.15,
      workloadBalanceWeight: 0.20,
      ...config
    }
  }
  ...
  async generateSchedulingRecommendations(
    ...
    try {
      ...
      const patientPreferences = await this.loadPatientPreferences(criteria.patientId)
      ...
      const scoredSlots = await Promise.all(
        availableSlots.map(slot => this.calculateOptimizationScore(
          slot,
          criteria,
          patientPreferences,
          staffEfficiencyData
  ...
  private async calculateOptimizationScore(
    ...
    if (staffPattern) {
      ...
      reasoning.push(`Staff efficiency: ${(scores.staffEfficiency * 100).toFixed(1)}%`)
    }
    
    // 2. Patient Preference Score
    scores.patientPreference = this.calculatePatientPreferenceScore(slot, patientPreferences)
    reasoning.push(`Patient preference match: ${(scores.patientPreference * 100).toFixed(1)}%`)
    
    // 3. Revenue Optimization Score
    scores.revenueOptimization = await this.calculateRevenueScore(slot, criteria)
    reasoning.push(`Revenue optimization: ${(scores.revenueOptimization * 100).toFixed(1)}%`)
    
    // 4. Treatment Sequencing Score
    if (criteria.isFollowUp || criteria.packageId) {
      scores.treatmentSequencing = await this.calculateSequencingScore(slot, criteria)
      reasoning.push(`Treatment sequencing: ${(scores.treatmentSequencing * 100).toFixed(1)}%`)
    }
    
    // 5. Workload Balance Score
    scores.workloadBalance = await this.calculateWorkloadBalanceScore(slot, staffId)
    reasoning.push(`Workload balance: ${(scores.workloadBalance * 100).toFixed(1)}%`)
    ...
    const confidence = this.calculateConfidence(scores, patientPreferences, staffPattern)
   ...
   */
  private async loadPatientPreferences(patientId: string): Promise<PatientPreference> {
    // Check cache first
   ...
   */
  private async learnPatientPreferences(patientId: string): Promise<PatientPreference> {
    try {
  ...
  private calculateStaffEfficiencyScore(slot: TimeSlot, pattern: StaffEfficiencyPattern): number {
  ...
  private calculatePatientPreferenceScore(slot: TimeSlot, preferences: PatientPreference): number {
    let score = 0
  ...
  private async calculateRevenueScore(slot: TimeSlot, criteria: SchedulingCriteria): Promise<number> {
    // Implementation for revenue optimization scoring
    // Consider peak hours, treatment pricing, package deals, etc.
    return 0.7 // Placeholder
  }

  private async calculateSequencingScore(slot: TimeSlot, criteria: SchedulingCriteria): Promise<number> {
    // Implementation for treatment sequencing optimization
    return 0.8 // Placeholder
  }

  private async calculateWorkloadBalanceScore(slot: TimeSlot, staffId: string): Promise<number> {
  ...
  private calculateConfidence(
    ...
    if (patientPreferences.lastLearningUpdate) {
      const daysSinceUpdate = (Date.now() - patientPreferences.lastLearningUpdate.getTime()) / (1000 * 60 * 60 * 24)
  ...
  private async estimateSlotRevenue(slot: TimeSlot, criteria: SchedulingCriteria): Promise<number> {
  ...
  private async findOptimalStaff(slot: TimeSlot, treatmentId: string): Promise<string> {
  ...
  private analyzeTimePreferences(appointments: any[]): { start: string; end: string }[] {
    // Analyze preferred time ranges
    const hours = appointments.map(apt => new Date(apt.start_time).getHours())
    const avgHour = hours.reduce((sum, hour) => sum + hour, 0) / hours.length
    
    return [{
      start: `${Math.max(8, Math.floor(avgHour - 1)).toString().padStart(2, '0')}:00`,
      end: `${Math.min(18, Math.ceil(avgHour + 1)).toString().padStart(2, '0')}:00`
    }]
  }
  ...
  private async loadStaffEfficiencyPatterns(): Promise<Map<string, StaffEfficiencyPattern[]>> {
