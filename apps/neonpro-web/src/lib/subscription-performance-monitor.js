...
var SubscriptionPerformanceMonitor = /** @class */ (function () {
    ...
    SubscriptionPerformanceMonitor.prototype.recordCacheOperation = function (hit, retrievalTime) {
        ...
        this.metrics.caching.averageRetrievalTime = this.updateAverage(this.metrics.caching.averageRetrievalTime, retrievalTime, this.metrics.subscriptionChecks.total);
    ...
    SubscriptionPerformanceMonitor.prototype.recordDatabaseOperation = function (queryTime) {
        ...
        this.metrics.database.averageQueryTime = this.updateAverage(this.metrics.database.averageQueryTime, queryTime, this.metrics.database.queryCount);
     ...
     */
    SubscriptionPerformanceMonitor.prototype.recordRealtimeOperation = function (latency, connectionsCount) {
        this.metrics.realtime.averageLatency = this.updateAverage(this.metrics.realtime.averageLatency, latency, connectionsCount);
    ...
    SubscriptionPerformanceMonitor.prototype.updateResponseTimeMetrics = function () {
        if (this.responseTimeHistory.length === 0)
            return;
        var sorted = __spreadArray([], this.responseTimeHistory, true).sort(function (a, b) { return a - b; });
        ...
        this.metrics.subscriptionChecks.averageResponseTime =
            this.responseTimeHistory.reduce(function (a, b) { return a + b; }, 0) / this.responseTimeHistory.length;
     ...
     */
    SubscriptionPerformanceMonitor.prototype.updateAverage = function (currentAverage, newValue, count) {
    ...
    SubscriptionPerformanceMonitor.prototype.getAlertMessage = function (metric, value, threshold) {
        var messages = {
            responseTime: "Subscription response time (".concat(value.toFixed(2), "ms) exceeds threshold (").concat(threshold, "ms)"),
            cacheHitRate: "Cache hit rate (".concat((value * 100).toFixed(2), "%) below threshold (").concat((threshold * 100).toFixed(2), "%)"),
            dbQueryTime: "Database query time (".concat(value.toFixed(2), "ms) exceeds threshold (").concat(threshold, "ms)"),
            realtimeLatency: "Realtime latency (".concat(value.toFixed(2), "ms) exceeds threshold (").concat(threshold, "ms)"),
        };
     ...
     */
    SubscriptionPerformanceMonitor.prototype.generateRecommendations = function (metric, value, threshold) {
        var recommendations = {
            responseTime: {
        ...
        if (recommendation && !this.recommendations.find(function (r) { return r.component === recommendation.component; })) {
    ...
    SubscriptionPerformanceMonitor.prototype.getPerformanceReport = function () {
        ...
        var criticalAlerts = activeAlerts.filter(function (a) { return a.type === 'critical'; });
        var warningAlerts = activeAlerts.filter(function (a) { return a.type === 'warning'; });
        ...
        var primaryIssues = activeAlerts
            .slice(0, 3)
            .map(function (alert) { return alert.message; });
        var topRecommendations = this.recommendations
            .filter(function (r) { return r.priority === 'high'; })
            .slice(0, 3)
            .map(function (r) { return r.recommendation; });
        return {
