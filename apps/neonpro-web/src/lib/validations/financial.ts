...
export const nfseRequestSchema = z.object({
  invoice_id: uuidSchema,
  service_code: z.string().regex(/^\d{2,10}$/, "Service code must be 2-10 digits"),
  ...
  taker_name: z.string().min(1, "Taker name is required").max(100, "Taker name too long"),
...
export const invoiceQuerySchema = z
  .object({
    ...
    sort: z
      .enum(["created_at", "issue_date", "due_date", "total_amount", "invoice_number"])
      .default("created_at"),
