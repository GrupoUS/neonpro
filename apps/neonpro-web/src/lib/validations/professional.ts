...
const dateSchema = z.string().datetime().or(z.date().transform(d => d.toISOString()));
...
const employmentStatusSchema = z.enum(['full_time', 'part_time', 'contractor', 'locum_tenens', 'retired']);
const professionalStatusSchema = z.enum(['active', 'inactive', 'suspended', 'pending_verification']);
const credentialTypeSchema = z.enum(['license', 'certification', 'board_certification', 'fellowship', 'residency', 'degree', 'cme', 'training']);
const verificationStatusSchema = z.enum(['pending', 'in_progress', 'verified', 'expired', 'rejected', 'requires_update']);
const metricTypeSchema = z.enum(['quality', 'safety', 'efficiency', 'patient_satisfaction', 'productivity', 'compliance', 'availability']);
const workflowStatusSchema = z.enum(['pending', 'in_progress', 'requires_documents', 'under_review', 'approved', 'rejected', 'expired']);
const alertSeveritySchema = z.enum(['low', 'medium', 'high', 'critical']);
const alertTypeSchema = z.enum(['expiration', 'renewal_required', 'compliance_check', 'performance_review', 'document_missing', 'license_status']);
const serviceTypeSchema = z.enum(['consultation', 'procedure', 'surgery', 'diagnostic', 'therapy', 'emergency', 'telemedicine', 'administrative']);
const specialtyCategorySchema = z.enum(['primary_care', 'specialty', 'surgical', 'emergency', 'diagnostic', 'mental_health', 'perioperative', 'administrative']);
...
export const professionalUpdateSchema = professionalCreateSchema.partial().omit({
...
export const medicalSpecialtyUpdateSchema = medicalSpecialtyCreateSchema.partial().omit({
...
export const professionalCredentialUpdateSchema = professionalCredentialCreateSchema.partial().omit({
...
export const professionalServiceCreateSchema = z.object({
  ...
  // Availability
  is_active: z.boolean().default(true),
  available_from: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(), // HH:MM format
  available_to: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(),
  ...
  // Financial
  base_fee: z.number().min(0).max(10000).optional(),
...
export const professionalServiceUpdateSchema = professionalServiceCreateSchema.partial().omit({
  professional_id: true
});
...
export const performanceMetricCreateSchema = z.object({
  ...
  measurement_frequency: z.enum(['daily', 'weekly', 'monthly', 'quarterly', 'annually']).optional(),
...
export const performanceMetricUpdateSchema = performanceMetricCreateSchema.partial().omit({
  professional_id: true,
...
export const professionalDevelopmentCreateSchema = z.object({
  ...
  // Activity details
  activity_type: z.enum(['cme', 'conference', 'workshop', 'certification', 'course', 'simulation', 'research']),
  ...
  completion_status: z.enum(['registered', 'in_progress', 'completed', 'cancelled']).default('registered'),
  ...
  // Cost and approval
  cost: z.number().min(0).max(50000).optional(),
...
export const professionalDevelopmentUpdateSchema = professionalDevelopmentCreateSchema.partial().omit({
  professional_id: true,
  activity_type: true,
  start_date: true
});
...
export const credentialingWorkflowCreateSchema = z.object({
  ...
  workflow_type: z.enum(['initial_credentialing', 'recredentialing', 'privilege_request', 'status_change', 'incident_review']),
  ...
  // Decision
  decision: z.enum(['approved', 'denied', 'conditional', 'deferred']).optional(),
...
export const credentialingWorkflowUpdateSchema = credentialingWorkflowCreateSchema.partial().omit({
  professional_id: true,
  workflow_type: true,
  initiated_date: true
});
...
export const credentialingAlertUpdateSchema = credentialingAlertCreateSchema.partial().omit({
  professional_id: true,
  alert_type: true,
  auto_generated: true
});
...
export const professionalSearchSchema = z.object({
  ...
  // Sorting
  sort_by: z.enum(['last_name', 'first_name', 'hire_date', 'status', 'primary_specialty']).default('last_name'),
...
export const credentialSearchSchema = z.object({
  ...
  page: z.number().int().min(1).default(1),
  limit: z.number().int().min(1).max(100).default(20),
  sort_by: z.enum(['expiration_date', 'issue_date', 'credential_name', 'verification_status']).default('expiration_date'),
  sort_order: z.enum(['asc', 'desc']).default('asc')
});
...
export const performanceMetricSearchSchema = z.object({
  ...
  page: z.number().int().min(1).default(1),
  limit: z.number().int().min(1).max(100).default(20),
  sort_by: z.enum(['measurement_period_start', 'metric_value', 'metric_name']).default('measurement_period_start'),
...
export const credentialAnalyticsFiltersSchema = z.object({
  ...
  credential_types: z.array(credentialTypeSchema).optional(),
  departments: z.array(z.string()).optional(),
  risk_level: z.enum(['all', 'low', 'medium', 'high', 'critical']).default('all')
});
...
export type CredentialCreateInput = z.infer<typeof professionalCredentialCreateSchema>;
export type CredentialUpdateInput = z.infer<typeof professionalCredentialUpdateSchema>;
export type CredentialSearchInput = z.infer<typeof credentialSearchSchema>;
export type ServiceCreateInput = z.infer<typeof professionalServiceCreateSchema>;
export type ServiceUpdateInput = z.infer<typeof professionalServiceUpdateSchema>;
export type MetricCreateInput = z.infer<typeof performanceMetricCreateSchema>;
export type DevelopmentCreateInput = z.infer<typeof professionalDevelopmentCreateSchema>;
export type WorkflowCreateInput = z.infer<typeof credentialingWorkflowCreateSchema>;
