...
const nonEmptyStringSchema = z.string().min(1, "Campo é obrigatório").trim();
...
const treatmentPlanStatusSchema = z.enum(
  ["draft", "active", "on-hold", "revoked", "completed", "entered-in-error", "unknown"] as const,
  {
...
export const treatmentPlanFormSchema = z
  .object({
    title: nonEmptyStringSchema.max(255, "Título deve ter no máximo 255 caracteres"),
    description: z.string().max(2000, "Descrição deve ter no máximo 2000 caracteres").optional(),
    patient_id: uuidSchema,
...
const clinicalNoteStatusSchema = z.enum(["current", "superseded", "entered-in-error"] as const, {
  errorMap: () => ({ message: "Status da nota clínica inválido" }),
});

const confidentialityLevelSchema = z.enum(["U", "L", "M", "N", "R", "V"] as const, {
