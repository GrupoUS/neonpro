...
const PredictionPeriodTypeSchema = z.enum(["daily", "weekly", "monthly", "quarterly", "annual"]);

const ModelTypeSchema = z.enum(["linear_regression", "arima", "lstm", "ensemble"]);

const AlgorithmTypeSchema = z.enum(["statistical", "machine_learning", "deep_learning"]);

export const ScenarioTypeSchema = z.enum(["optimistic", "realistic", "pessimistic", "custom"]);
...
export const ErrorMagnitudeSchema = z.enum(["low", "medium", "high", "critical"]);
