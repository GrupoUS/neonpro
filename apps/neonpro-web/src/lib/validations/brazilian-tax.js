...
exports.nfeCreateRequestSchema = exports.bulkTaxCalculationRequestSchema = exports.serviceTaxCodeSchema = exports.taxComplianceReportSchema = exports.spedEntrySchema = exports.nfeGenerationRequestSchema = exports.taxCalculationRequestSchema = exports.nfeDocumentSchema = exports.customerInfoSchema = exports.serviceItemSchema = exports.taxConfigurationSchema = exports.brazilianAddressSchema = exports.ufSchema = exports.cepSchema = exports.cnpjCpfSchema = exports.cpfSchema = exports.cnpjSchema = void 0;
...
exports.cnpjSchema = zod_1.z.string()
    ...
    if (digits.split('').every(function (digit) { return digit === digits[0]; })) {
        return false;
    }
    ...
    if (parseInt(digits[12]) !== firstCheck)
        return false;
    // Second check digit
    sum = 0;
    weight = 2;
...
exports.cpfSchema = zod_1.z.string()
    ...
    if (parseInt(digits[9]) !== firstCheck)
        return false;
    sum = 0;
    for (var i = 0; i < 10; i++) {
...
exports.cnpjCpfSchema = zod_1.z.string()
    ...
    var digits = value.replace(/[^\d]/g, '');
    if (digits.length === 11) {
        return exports.cpfSchema.safeParse(value.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')).success;
    }
    else {
        return exports.cnpjSchema.safeParse(value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')).success;
    }
}, 'CNPJ ou CPF inválido');
exports.cepSchema = zod_1.z.string()
    .regex(/^\d{5}-\d{3}$/, 'CEP deve estar no formato XXXXX-XXX');
exports.ufSchema = zod_1.z.string()
...
exports.brazilianAddressSchema = zod_1.z.object({
    logradouro: zod_1.z.string().min(1, 'Logradouro é obrigatório').max(100, 'Logradouro muito longo'),
...
exports.taxConfigurationSchema = zod_1.z.object({
    ...
    regime_tributario: zod_1.z.enum(['simples_nacional', 'lucro_presumido', 'lucro_real'], {
        errorMap: function () { return ({ message: 'Regime tributário inválido' }); }
    }),
    optante_simples_nacional: zod_1.z.boolean(),
    // Tax Rates (0-100%)
    icms_rate: zod_1.z.number().min(0, 'Taxa ICMS não pode ser negativa').max(100, 'Taxa ICMS não pode exceder 100%'),
    iss_rate: zod_1.z.number().min(0, 'Taxa ISS não pode ser negativa').max(25, 'Taxa ISS não pode exceder 25%'),
    pis_rate: zod_1.z.number().min(0, 'Taxa PIS não pode ser negativa').max(10, 'Taxa PIS não pode exceder 10%'),
    cofins_rate: zod_1.z.number().min(0, 'Taxa COFINS não pode ser negativa').max(15, 'Taxa COFINS não pode exceder 15%'),
    irpj_rate: zod_1.z.number().min(0, 'Taxa IRPJ não pode ser negativa').max(40, 'Taxa IRPJ não pode exceder 40%'),
    csll_rate: zod_1.z.number().min(0, 'Taxa CSLL não pode ser negativa').max(15, 'Taxa CSLL não pode exceder 15%'),
    // Simples Nacional
    simples_nacional_anexo: zod_1.z.enum(['I', 'II', 'III', 'IV', 'V'], {
        errorMap: function () { return ({ message: 'Anexo do Simples Nacional inválido' }); }
    }),
    simples_nacional_rate: zod_1.z.number().min(0, 'Taxa Simples Nacional não pode ser negativa').max(35, 'Taxa Simples Nacional não pode exceder 35%'),
    // Address
...
exports.serviceItemSchema = zod_1.z.object({
    codigo: zod_1.z.string().min(1, 'Código do serviço é obrigatório').max(20, 'Código muito longo'),
...
exports.nfeDocumentSchema = zod_1.z.object({
    ...
    // NFe Identification
    numero_nfe: zod_1.z.number().int('Número NFe deve ser inteiro').positive('Número NFe deve ser positivo'),
    serie_nfe: zod_1.z.number().int('Série NFe deve ser inteiro').positive('Série NFe deve ser positiva').default(1),
    // Document Information
    tipo_documento: zod_1.z.enum(['nfe', 'nfce', 'nfse'], {
        errorMap: function () { return ({ message: 'Tipo de documento inválido' }); }
    }),
    modelo_documento: zod_1.z.enum(['55', '65', '57'], {
        errorMap: function () { return ({ message: 'Modelo de documento inválido' }); }
    }),
    natureza_operacao: zod_1.z.string().min(1, 'Natureza da operação é obrigatória').max(100, 'Natureza da operação muito longa'),
    // Tax Information
    valor_total: zod_1.z.number().positive('Valor total deve ser positivo'),
    valor_base_calculo: zod_1.z.number().nonnegative('Base de cálculo não pode ser negativa').optional(),
    ...
    // Status
    status: zod_1.z.enum(['draft', 'authorized', 'cancelled', 'rejected'], {
...
exports.taxCalculationRequestSchema = zod_1.z.object({
    ...
    tipo_servico: zod_1.z.string().min(1, 'Tipo de serviço é obrigatório').max(100, 'Tipo de serviço muito longo'),
    codigo_servico: zod_1.z.string().max(10, 'Código de serviço muito longo').optional(),
...
// SPED Entry Schema
exports.spedEntrySchema = zod_1.z.object({
    clinic_id: zod_1.z.string().uuid('ID da clínica inválido'),
    // SPED Information
    periodo_apuracao: zod_1.z.string().regex(/^\d{4}-\d{2}-01$/, 'Período deve estar no formato YYYY-MM-01'),
    tipo_escrituracao: zod_1.z.enum(['ECD', 'ECF', 'EFD'], {
        errorMap: function () { return ({ message: 'Tipo de escrituração inválido' }); }
    }),
    // Entry Details
    codigo_conta: zod_1.z.string().min(1, 'Código da conta é obrigatório').max(20, 'Código da conta muito longo'),
    descricao_conta: zod_1.z.string().min(1, 'Descrição da conta é obrigatória').max(200, 'Descrição muito longa'),
    ...
    // Classification
    natureza_operacao: zod_1.z.string().max(100, 'Natureza da operação muito longa').optional(),
    origem_lancamento: zod_1.z.enum(['system', 'manual', 'imported'], {
        errorMap: function () { return ({ message: 'Origem do lançamento inválida' }); }
    }).default('system')
}).refine(function (data) {
...
exports.taxComplianceReportSchema = zod_1.z.object({
    ...
    tipo_relatorio: zod_1.z.enum(['sped_ecd', 'sped_ecf', 'livro_registro', 'dctf', 'defis'], {
        errorMap: function () { return ({ message: 'Tipo de relatório inválido' }); }
    }),
    periodo_referencia: zod_1.z.string().regex(/^\d{4}-\d{2}-01$/, 'Período deve estar no formato YYYY-MM-01'),
    descricao: zod_1.z.string().max(200, 'Descrição muito longa').optional(),
    // Report Data
    conteudo_relatorio: zod_1.z.record(zod_1.z.any()),
    // Submission
    status_envio: zod_1.z.enum(['pending', 'sent', 'accepted', 'rejected'], {
        errorMap: function () { return ({ message: 'Status de envio inválido' }); }
    }).default('pending')
});
// Service Tax Code Schema
exports.serviceTaxCodeSchema = zod_1.z.object({
    codigo_servico: zod_1.z.string().regex(/^\d{2}\.\d{2}$/, 'Código deve estar no formato XX.XX'),
    descricao_servico: zod_1.z.string().min(1, 'Descrição é obrigatória').max(500, 'Descrição muito longa'),
    categoria: zod_1.z.string().max(100, 'Categoria muito longa').optional(),
    // Tax Configuration
    iss_aliquota_minima: zod_1.z.number().min(0, 'Alíquota mínima não pode ser negativa').max(25, 'Alíquota mínima não pode exceder 25%'),
    iss_aliquota_maxima: zod_1.z.number().min(0, 'Alíquota máxima não pode ser negativa').max(25, 'Alíquota máxima não pode exceder 25%'),
    tributacao_pis_cofins: zod_1.z.enum(['cumulativo', 'nao_cumulativo'], {
        errorMap: function () { return ({ message: 'Tributação PIS/COFINS inválida' }); }
    }),
    // Healthcare Specific
...
exports.bulkTaxCalculationRequestSchema = zod_1.z.object({
    ...
    })).min(1, 'Pelo menos um cálculo é necessário').max(100, 'Máximo de 100 cálculos por lote')
});
...
exports.nfeCreateRequestSchema = zod_1.z.object({
    ...
    payment_method: zod_1.z.enum(['cash', 'card', 'bank_transfer', 'pix', 'check'], {
        errorMap: function () { return ({ message: 'Método de pagamento inválido' }); }
    }).optional(),
