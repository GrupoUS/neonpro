...
function compareFingerprintSimilarity(fp1, fp2) {
    ...
    // Screen resolution
    total++;
    if (((_a = fp1.screen) === null || _a === void 0 ? void 0 : _a.width) === ((_b = fp2.screen) === null || _b === void 0 ? void 0 : _b.width) &&
        ((_c = fp1.screen) === null || _c === void 0 ? void 0 : _c.height) === ((_d = fp2.screen) === null || _d === void 0 ? void 0 : _d.height)) {
        matches++;
    }
    // Timezone
    total++;
    ...
    if (fp1.plugins && fp2.plugins) {
        var intersection = fp1.plugins.filter(function (p) { return fp2.plugins.includes(p); });
        var union = __spreadArray([], new Set(__spreadArray(__spreadArray([], fp1.plugins, true), fp2.plugins, true)), true);
        if (intersection.length / union.length > 0.7)
            matches++;
    }
    // Canvas fingerprint
    total++;
 ...
 */
function isLocationSuspicious(currentLocation, previousLocations, timeWindow // 24 hours
) {
    if (timeWindow === void 0) { timeWindow = 24 * 60 * 60 * 1000; }
    if (!currentLocation.latitude || !currentLocation.longitude) {
        return false;
    }
    var recentLocations = previousLocations.filter(function (loc) { return Date.now() - new Date(loc.timestamp).getTime() < timeWindow; });
    ...
    for (var _i = 0, recentLocations_1 = recentLocations; _i < recentLocations_1.length; _i++) {
        ...
        if (!prevLoc.latitude || !prevLoc.longitude)
            continue;
        var distance = calculateDistance({ latitude: currentLocation.latitude, longitude: currentLocation.longitude }, { latitude: prevLoc.latitude, longitude: prevLoc.longitude });
...
function needsRenewal(session, renewalThreshold) {
    if (renewalThreshold === void 0) { renewalThreshold = 0.25; }
    var now = Date.now();
...
function detectSessionHijacking(currentSession, previousSessions) {
    ...
    if (currentSession.ipAddress !== lastSession.ipAddress) {
        ...
        if (currentSession.location && lastSession.location) {
            ...
            var timeDiff = new Date(currentSession.createdAt).getTime() -
    ...
    var fingerprintSimilarity = compareFingerprintSimilarity(currentSession.deviceFingerprint, lastSession.deviceFingerprint);
...
function encryptData(data, key) {
    var keyBuffer = Buffer.from(key.padEnd(ENCRYPTION_KEY_LENGTH, '0').slice(0, ENCRYPTION_KEY_LENGTH));
