...
export class SessionManager extends EventEmitter {
  ...
  async createSession(params: CreateSessionParams): Promise<UserSession> {
    try {
      ...
      if (dbError) {
        throw new SessionError('Failed to create session in database', 'SYSTEM_ERROR', { error: dbError });
      }
  ...
  async renewSession(sessionId: string): Promise<UserSession> {
    try {
      ...
      if (dbError) {
        throw new SessionError('Failed to renew session in database', 'SYSTEM_ERROR', { error: dbError });
      }
  ...
  async terminateSession(sessionId: string, reason?: string): Promise<void> {
    try {
      ...
      if (dbError) {
        throw new SessionError('Failed to terminate session in database', 'SYSTEM_ERROR', { error: dbError });
      }
  ...
  async cleanupExpiredSessions(): Promise<number> {
    try {
      ...
      if (fetchError) {
        throw new SessionError('Failed to fetch expired sessions', 'SYSTEM_ERROR', { error: fetchError });
      }
      ...
      if (updateError) {
        throw new SessionError('Failed to update expired sessions', 'SYSTEM_ERROR', { error: updateError });
      }
   ...
   */
  async recordActivity(sessionId: string, activity: Omit<SessionActivity, 'sessionId' | 'timestamp'>): Promise<void> {
    try {
      const session = await this.getSessionFromCache(sessionId);
      if (!session) {
  ...
  private async logSecurityEvent(event: Omit<SessionSecurityEvent, 'id' | 'timestamp' | 'resolved' | 'resolvedAt' | 'resolvedBy' | 'actions'>): Promise<void> {
