...
import { SecurityConfig, SecurityEvent, SecurityAlert, ThreatLevel } from '../types';
...
describe('SecurityMonitor', () => {
  ...
  describe('blockIP', () => {
    it('should block an IP address', async () => {
      const success = await securityMonitor.blockIP('192.168.1.100', 'Manual block', 60 * 60 * 1000);
      expect(success).toBe(true);
