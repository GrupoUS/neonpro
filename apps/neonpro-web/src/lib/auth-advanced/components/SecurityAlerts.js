...
function SecurityAlerts(_a) {
    var _b = _a.className, className = _b === void 0 ? '' : _b, _c = _a.maxAlerts, maxAlerts = _c === void 0 ? 10 : _c, _d = _a.showDismissed, showDismissed = _d === void 0 ? false : _d, _e = _a.autoHide, autoHide = _e === void 0 ? true : _e, _f = _a.compact, compact = _f === void 0 ? false : _f;
    var _g = (0, context_1.useSessionContext)(), securityAlerts = _g.securityAlerts, securityEvents = _g.securityEvents, clearSecurityAlerts = _g.clearSecurityAlerts;
    var _h = (0, react_1.useState)(new Set()), dismissedAlerts = _h[0], setDismissedAlerts = _h[1];
    var _j = (0, react_1.useState)(false), showAll = _j[0], setShowAll = _j[1];
    // Filter alerts based on settings
    var filteredAlerts = securityAlerts
        .filter(function (alert) { return showDismissed || !dismissedAlerts.has(alert.id); })
    ...
    if (filteredAlerts.length === 0) {
        ...
        return (<div className={"bg-green-50 border border-green-200 rounded-lg p-4 ".concat(className)}>
    ...
    if (compact) {
        ...
        {filteredAlerts.slice(0, 3).map(function (alert) {
                ...
                return (<div key={alert.id} className={"flex items-center justify-between p-2 rounded border ".concat(getAlertColor(alert), " ").concat(isDismissed ? 'opacity-50' : '')}>
              <div className="flex items-center space-x-2 flex-1 min-w-0">
                {getEventIcon(alert.event_type)}
                <span className={"text-sm font-medium truncate ".concat(getTextColor(alert))}>
                  {utils_1.AuthUtils.SecurityEvent.generateEventDescription(alert.event_type, alert.metadata)}
                </span>
              </div>
              
              <button onClick={function () { return dismissAlert(alert.id); }} className="flex-shrink-0 p-1 hover:bg-white/50 rounded" title="Dismiss alert">
                <lucide_react_1.X className="w-3 h-3"/>
              </button>
            </div>);
            })}
        
        {filteredAlerts.length > 3 && (<button onClick={function () { return setShowAll(!showAll); }} className="text-xs text-blue-600 hover:text-blue-800 font-medium">
    ...
    return (<div className={"bg-white rounded-lg border shadow-sm ".concat(className)}>
      ...
      <div className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center space-x-2">
          ...
          {filteredAlerts.length > 0 && (<span className="bg-red-100 text-red-800 text-xs font-medium px-2 py-1 rounded-full">
              {filteredAlerts.length}
            </span>)}
        </div>
        
        <div className="flex items-center space-x-2">
          <button onClick={function () { return setShowDismissed(!showDismissed); }} className="p-2 text-gray-400 hover:text-gray-600 rounded" title={showDismissed ? 'Hide dismissed' : 'Show dismissed'}>
            {showDismissed ? <lucide_react_1.EyeOff className="w-4 h-4"/> : <lucide_react_1.Eye className="w-4 h-4"/>}
          </button>
          
          {filteredAlerts.length > 0 && (<button onClick={handleClearAll} className="p-2 text-gray-400 hover:text-gray-600 rounded" title="Clear all alerts">
      ...
      <div className="divide-y">
        {filteredAlerts.map(function (alert) {
            var severity = utils_1.AuthUtils.SecurityEvent.classifyEventSeverity(alert.event_type);
            var riskScore = utils_1.AuthUtils.SecurityEvent.calculateEventRiskScore(alert.event_type, alert.metadata);
            var isDismissed = dismissedAlerts.has(alert.id);
            return (<div key={alert.id} className={"p-4 hover:bg-gray-50 transition-colors ".concat(isDismissed ? 'opacity-50' : '')}>
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className={"inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ".concat(severity === 'critical' ? 'bg-red-100 text-red-800' :
                    severity === 'high' ? 'bg-orange-100 text-orange-800' :
                      ...
                      {riskScore > 50 && (<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                          Risk: {riskScore}%
                        </span>)}
                    </div>
                    ...
                    {alert.metadata && Object.keys(alert.metadata).length > 0 && (<p className="text-xs text-gray-500">
                    ...
                    <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                      ...
                      {alert.ip_address && (<div className="flex items-center space-x-1">
                          <span>IP: {alert.ip_address}</span>
                        </div>)}
                      
                      {alert.session_id && (<div className="flex items-center space-x-1">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  <button onClick={function () { return dismissAlert(alert.id); }} className="p-1 text-gray-400 hover:text-gray-600 rounded" title="Dismiss alert">
      ...
      {/* Footer */}
      {securityAlerts.length > maxAlerts && !showAll && (<div className="p-4 border-t bg-gray-50">
          <button onClick={function () { return setShowAll(true); }} className="text-sm text-blue-600 hover:text-blue-800 font-medium">
            Show {securityAlerts.length - maxAlerts} more alerts
          </button>
        </div>)}
      
      {showAll && securityAlerts.length > maxAlerts && (<div className="p-4 border-t bg-gray-50">
          <button onClick={function () { return setShowAll(false); }} className="text-sm text-blue-600 hover:text-blue-800 font-medium">
            Show less
          </button>
        </div>)}
    </div>);
