...
export class SessionManagerExtended {
  ...
  async cleanupExpiredSessions(): Promise<number> {
    try {
      ...
      for (const session of expiredSessions) {
        await this.logSessionActivity(session.id, session.user_id, "session_expired", {
          cleanup_time: now,
  ...
  async handleSecurityEvent(event: Partial<SessionSecurityEvent>): Promise<void> {
    try {
      ...
      const { error } = await this.supabase.from("session_security_events").insert(securityEvent);
  ...
  private async logSessionActivity(
    ...
    try {
      ...
      const { error } = await this.supabase.from("session_audit_logs").insert(auditLog);
  ...
  private isPrivilegeEscalationAttempt(activity: any): boolean {
    ...
    return (
      ...
      activity.attempted_permissions.some(
        (perm: string) => perm.includes("admin") || perm.includes("owner"),
      )
    );
  }
