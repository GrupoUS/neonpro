...
var ReceiptInvoiceManager = /** @class */ (function () {
    ...
    ReceiptInvoiceManager.prototype.sendEmail = function (documentId, recipientEmail, customMessage) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, document_1, error, _b, pdfData, downloadError, pdfBuffer, _c, _d, template, recipient, mailOptions, result, error_3;
    ...
    ReceiptInvoiceManager.prototype.generatePDFContent = function (doc, data, options) {
        var colors = options.colors, fonts = options.fonts;
        // Header
        ...
        doc.fontSize(10).font(fonts.body).fillColor(colors.secondary)
            .text("".concat(this.companyInfo.address, ", ").concat(this.companyInfo.city, " - ").concat(this.companyInfo.state), 50, 80)
            .text("CEP: ".concat(this.companyInfo.zipCode, " | CNPJ: ").concat(this.companyInfo.cnpj), 50, 95);
        ...
        // Date
        doc.fontSize(10).font(fonts.body).fillColor(colors.secondary)
            .text("Data: ".concat((0, date_fns_1.format)(data.date, 'dd/MM/yyyy', { locale: locale_1.ptBR })), 400, 90);
        if (data.dueDate) {
            doc.text("Vencimento: ".concat((0, date_fns_1.format)(data.dueDate, 'dd/MM/yyyy', { locale: locale_1.ptBR })), 400, 105);
        }
        // Customer info
        ...
        doc.fontSize(10).font(fonts.body).fillColor('#000');
        if (data.taxTotal > 0) {
            doc.text("Subtotal: R$ ".concat(data.subtotal.toFixed(2)), 350, totalsY)
                .text("Impostos: R$ ".concat(data.taxTotal.toFixed(2)), 350, totalsY + 15);
        }
        doc.fontSize(12).font(fonts.header).fillColor(colors.primary)
            .text("TOTAL: R$ ".concat(data.total.toFixed(2)), 350, totalsY + (data.taxTotal > 0 ? 35 : 15));
        // Payment info
        ...
        if (data.paymentDate) {
            doc.text("Data do pagamento: ".concat((0, date_fns_1.format)(data.paymentDate, 'dd/MM/yyyy', { locale: locale_1.ptBR })), 50, totalsY + 65);
        }
        // Notes and terms
        if (data.notes) {
    ...
    ReceiptInvoiceManager.prototype.processEmailTemplate = function (template, data, customMessage) {
        var processed = template
            ...
            .replace(/{{date}}/g, (0, date_fns_1.format)(data.date, 'dd/MM/yyyy', { locale: locale_1.ptBR }))
