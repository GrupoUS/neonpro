...
export class RecurringPaymentProcessor {
  ...
  private async processStripePayment(subscription: any, plan: any): Promise<PaymentAttemptResult> {
    try {
  ...
  private async processManualPayment(subscription: any, plan: any): Promise<PaymentAttemptResult> {
    try {
      ...
      const defaultPaymentMethod =
        customer.payment_methods.find((pm: any) => pm.is_default) || customer.payment_methods[0];
  ...
  private async updateSubscriptionStatuses(): Promise<void> {
    try {
      ...
      const gracePeriodDate = new Date(
        now.getTime() - this.config.dunning_management.grace_period_days * 24 * 60 * 60 * 1000,
      );
      const suspensionDate = new Date(
        now.getTime() - this.config.dunning_management.suspension_after_days * 24 * 60 * 60 * 1000,
      );
