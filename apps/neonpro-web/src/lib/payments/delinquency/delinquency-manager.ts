...
export class DelinquencyManager {
  ...
  async processCollectionWorkflows(): Promise<void> {
    try {
      ...
      for (const payment of overduePayments) {
        const applicableRules = rules.filter((rule) => this.isRuleApplicable(rule, payment));
  ...
  async getDelinquencyStats(dateRange?: { from: Date; to: Date }): Promise<DelinquencyStats> {
    try {
      const { data, error } = await this.supabase.rpc("get_delinquency_statistics", {
  ...
  private processTemplate(template: string, customer: any, metadata?: Record<string, any>): string {
