...
var BankStatementProcessor = /** @class */ (() => {
   ...
   */
  BankStatementProcessor.prototype.getBatchSummary = (results) => ({
  ...
  BankStatementProcessor.prototype.detectBankParser = function (fileName, content) {
    ...
    for (var _i = 0, _a = this.parsers; _i < _a.length; _i++) {
      ...
      var matches = parser.patterns.some(
  ...
  BankStatementProcessor.prototype.parseCSVRow = (row, options) => {
  ...
  BankStatementProcessor.prototype.extractAccountNumber = (lines, bank) => {
  ...
  BankStatementProcessor.prototype.extractPeriodStart = (lines, bank) => {
    // Extract statement period start date
    return new Date().toISOString(); // Placeholder
  };
  BankStatementProcessor.prototype.extractPeriodEnd = (lines, bank) => {
    // Extract statement period end date
    return new Date().toISOString(); // Placeholder
  };
  BankStatementProcessor.prototype.findTransactionStartLine = (lines, bank) => {
  ...
  BankStatementProcessor.prototype.parseTransactionLine = (line, bank, options) => {
  ...
  BankStatementProcessor.prototype.parseAmount = (value) => {
  ...
  BankStatementProcessor.prototype.parseDate = (dateStr, format) => {
