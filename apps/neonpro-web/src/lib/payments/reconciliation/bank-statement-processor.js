...
var BankStatementProcessor = /** @class */ (function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.processStatement(parseResult.header, parseResult.transactions, fileName, processingOptions)];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, __assign(__assign({}, result), { warnings: __spreadArray(__spreadArray([], result.warnings, true), parseResult.warnings, true) })];
                    case 3:
                        error_1 = _a.sent();
    ...
    BankStatementProcessor.prototype.getBatchSummary = function (results) {
        return {
            ...
            failedFiles: results.filter(function (r) { return !r.success; }).length,
            totalTransactions: results.reduce(function (sum, r) { return sum + r.processedTransactions; }, 0),
            totalSkipped: results.reduce(function (sum, r) { return sum + r.skippedTransactions; }, 0),
        ...
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                ...
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        ...
                        reader.onerror = function (e) { return reject(new Error('Failed to read file')); };
    ...
    BankStatementProcessor.prototype.detectBankParser = function (fileName, content) {
        ...
        for (var _i = 0, _a = this.parsers; _i < _a.length; _i++) {
            var _b = _a[_i], key = _b[0], parser = _b[1];
            if (key === 'generic')
    ...
    BankStatementProcessor.prototype.parseBradescoStatement = function (content, options) {
        ...
        try {
            ...
            // Parse transactions
            var transactions = this.parseTransactionLines(lines.slice(this.findTransactionStartLine(lines, 'bradesco')), 'bradesco', options);
            return { header: header, transactions: transactions, errors: errors, warnings: warnings };
        }
        catch (error) {
            errors.push("Bradesco parsing error: ".concat(error instanceof Error ? error.message : 'Unknown error'));
            return {
                header: {},
                transactions: [],
                errors: errors,
    ...
    BankStatementProcessor.prototype.parseItauStatement = function (content, options) {
        ...
        try {
            ...
            var transactions = this.parseTransactionLines(lines.slice(this.findTransactionStartLine(lines, 'itau')), 'itau', options);
            return { header: header, transactions: transactions, errors: errors, warnings: warnings };
        }
        catch (error) {
            errors.push("Ita\u00FA parsing error: ".concat(error instanceof Error ? error.message : 'Unknown error'));
            return {
                header: {},
                transactions: [],
                errors: errors,
    ...
    BankStatementProcessor.prototype.parseSantanderStatement = function (content, options) {
        ...
        try {
            ...
            var transactions = this.parseTransactionLines(lines.slice(this.findTransactionStartLine(lines, 'santander')), 'santander', options);
            return { header: header, transactions: transactions, errors: errors, warnings: warnings };
        }
        catch (error) {
            errors.push("Santander parsing error: ".concat(error instanceof Error ? error.message : 'Unknown error'));
            return {
                header: {},
                transactions: [],
                errors: errors,
    ...
    BankStatementProcessor.prototype.parseGenericCSV = function (content, options) {
        ...
        try {
            ...
            if (parseResult.errors.length > 0) {
                errors.push.apply(errors, parseResult.errors.map(function (e) { return e.message; }));
            }
            ...
            for (var i = 0; i < data.length; i++) {
                try {
                    ...
                    warnings.push("Row ".concat(i + 1, ": ").concat(error instanceof Error ? error.message : 'Parse error'));
                }
            }
            return { header: header, transactions: transactions, errors: errors, warnings: warnings };
        }
        catch (error) {
            errors.push("CSV parsing error: ".concat(error instanceof Error ? error.message : 'Unknown error'));
            return {
                header: {},
                transactions: [],
                errors: errors,
    ...
    BankStatementProcessor.prototype.processStatement = function (header, transactions, fileName, options) {
        return __awaiter(this, void 0, void 0, function () {
            var errors, warnings, processedTransactions, skippedTransactions, totalCredits, totalDebits, validatedHeader, _a, statement, statementError, transactionInserts, _i, transactions_1, transaction, validatedTransaction, debitAmount, creditAmount, balance, transactionDate, transactionData, transactionError, updateError, expectedBalance, balanceCheck, error_2;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        errors = [];
                        ...
                        for (_i = 0, transactions_1 = transactions; _i < transactions_1.length; _i++) {
                            ...
                            try {
                                ...
                                if (!debitAmount && !creditAmount) {
                                    warnings.push("Transaction skipped - no amount: ".concat(validatedTransaction.description));
                                    skippedTransactions++;
                                    continue;
                                }
                                transactionDate = this.parseDate(validatedTransaction.date, options.dateFormat);
                                if (!transactionDate) {
                                    warnings.push("Transaction skipped - invalid date: ".concat(validatedTransaction.date));
                                    skippedTransactions++;
                                    continue;
                                }
                                transactionData = {
                                ...
                                warnings.push("Transaction validation error: ".concat(error instanceof Error ? error.message : 'Unknown error'));
                                skippedTransactions++;
                            }
                        }
                    ...
                    case 5:
                        ...
                        if (!balanceCheck) {
                            warnings.push("Balance mismatch: Expected ".concat(expectedBalance.toFixed(2), ", ") +
