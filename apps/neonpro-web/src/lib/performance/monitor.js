...
var PerformanceMonitor = /** @class */ (function () {
    ...
    PerformanceMonitor.prototype.getBenchmark = function (operation) {
        var operationMetrics = this.metrics.filter(function (m) { return m.operation === operation; });
        if (operationMetrics.length === 0)
            return null;
        var avgResponseTime = operationMetrics.reduce(function (sum, m) { return sum + m.duration; }, 0) / operationMetrics.length;
        var avgMemory = operationMetrics.reduce(function (sum, m) { return sum + m.memory; }, 0) / operationMetrics.length;
        var errorCount = operationMetrics.filter(function (m) { return !m.success; }).length;
    ...
    PerformanceMonitor.prototype.generateReport = function () {
        var _this = this;
        var report = {};
        var operations = __spreadArray([], new Set(this.metrics.map(function (m) { return m.operation; })), true);
