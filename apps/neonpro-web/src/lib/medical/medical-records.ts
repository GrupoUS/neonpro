...
export class MedicalRecordsManager {
  ...
  async uploadMedicalAttachment(
    ...
    try {
      ...
      // Get public URL
      const { data: urlData } = this.supabase.storage.from("medical-files").getPublicUrl(filePath);
  ...
  async deleteAttachment(
    ...
    try {
      ...
      await this.supabase.storage.from("medical-files").remove([filePath]);
  ...
  async verifySignature(
    ...
    try {
      ...
      const recordResult = await this.getMedicalRecord(signature.record_id, signature.signer_id);
  ...
  private async createRecordVersion(record: MedicalRecord, userId: string): Promise<void> {
    try {
      const versionData = {
        ...record,
        id: crypto.randomUUID(),
  ...
  private async generateThumbnail(filePath: string, file: File): Promise<string | undefined> {
    try {
      ...
      const { data } = this.supabase.storage.from("medical-files").getPublicUrl(thumbnailPath);
