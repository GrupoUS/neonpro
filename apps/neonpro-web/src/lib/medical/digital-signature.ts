...
export class DigitalSignatureManager {
  ...
  async signDocument(
    ...
    try {
      ...
      const consentCheck = await this.lgpdManager.checkConsent(signerId, "digital_signature");
      ...
      switch (options.signatureType) {
        ...
        case SignatureType.BIOMETRIC_SIGNATURE:
          signatureData = await this.createBiometricSignature(documentHash, options.biometricData);
          break;
  ...
  private async createCertificateSignature(
    ...
    try {
      ...
      const thumbprint = crypto.createHash("sha1").update(certificate).digest("hex");
  ...
  private async verifyElectronicSignature(signature: DigitalSignature): Promise<boolean> {
    try {
  ...
  private async verifyBiometricSignature(signature: DigitalSignature): Promise<boolean> {
    try {
  ...
  private async verifyPinSignature(signature: DigitalSignature): Promise<boolean> {
    try {
  ...
  async getSignatureStatistics(
    ...
    try {
      ...
      data?.forEach((signature) => {
        ...
        // Count by role
        stats.by_role[signature.signer_role] = (stats.by_role[signature.signer_role] || 0) + 1;
