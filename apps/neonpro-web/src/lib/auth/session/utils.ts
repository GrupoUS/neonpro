 ...
 */
export function generateDeviceFingerprint(deviceInfo: Partial<DeviceInfo>): DeviceFingerprint {
 ...
 */
export function isSessionNearTimeout(session: UserSession, thresholdMinutes: number = 5): boolean {
 ...
 */
export function formatTimestamp(timestamp: Timestamp, locale: string = 'pt-BR'): string {
 ...
 */
export function formatDuration(ms: number, locale: string = 'pt-BR'): string {
 ...
 */
export function extractDeviceInfo(userAgent: UserAgent, ipAddress?: IPAddress): DeviceInfo {
 ...
 */
export function calculateDeviceRiskScore(device: UserDevice, recentActivity?: any[]): {
  score: number
  level: RiskLevel
 ...
 */
export function detectAnomalousActivity(activities: any[], userBaseline?: any): {
  isAnomalous: boolean
  anomalies: string[]
  confidence: number
} {
  ...
  const timeSpan = new Date(activities[activities.length - 1].createdAt).getTime() - 
