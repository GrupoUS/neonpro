...
var SuspiciousActivityDetector = /** @class */ (function () {
     ...
     */
    SuspiciousActivityDetector.prototype.recordBehavior = function (userId, sessionId, behaviorType, metrics) {
        try {
            var pattern = {
                userId: userId,
                sessionId: sessionId,
     ...
     */
    SuspiciousActivityDetector.prototype.recordTypingBehavior = function (userId, sessionId, typingSpeed, keyPressIntervals) {
     ...
     */
    SuspiciousActivityDetector.prototype.recordMouseBehavior = function (userId, sessionId, mouseSpeed, clickPatterns) {
     ...
     */
    SuspiciousActivityDetector.prototype.recordNavigationBehavior = function (userId, sessionId, navigationPath) {
     ...
     */
    SuspiciousActivityDetector.prototype.recordApiUsage = function (userId, sessionId, endpoint, frequency) {
        var userPatterns = this.activePatterns.get(userId) || [];
        var recentApiPatterns = userPatterns
            .filter(function (p) { return p.patternType === 'api_usage' && Date.now() - p.timestamp < 300000; })
            .map(function (p) { return p.currentMetrics.apiEndpoints || []; })
            .flat();
    ...
    SuspiciousActivityDetector.prototype.performBehaviorAnalysis = function () {
        return __awaiter(this, void 0, void 0, function () {
            var _i, _a, _b, userId, patterns, recentPatterns, anomalies, _c, anomalies_1, anomaly, error_1;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    ...
                    case 1:
                        ...
                        recentPatterns = patterns.filter(function (p) { return Date.now() - p.timestamp < 300000; } // Last 5 minutes
                        );
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                            ...
                            return __generator(this, function (_c) {
                                switch (_c.label) {
                                    case 0:
                                        ...
                                        relevantPatterns = patterns.filter(function (p) { return rule.patternTypes.includes(p.patternType); });
                    ...
                    case 3:
                        _i++;
                        return [3 /*break*/, 1];
                    case 4: return [2 /*return*/, anomalies];
                }
            });
        });
    };
    ...
    SuspiciousActivityDetector.prototype.applyDetectionRule = function (userId, rule, patterns) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                try {
                    avgAnomalyScore = patterns.reduce(function (sum, p) { return sum + p.anomalyScore; }, 0) / patterns.length;
                    avgConfidence = patterns.reduce(function (sum, p) { return sum + p.confidence; }, 0) / patterns.length;
                    if (avgAnomalyScore >= rule.thresholds.deviation && avgConfidence >= rule.thresholds.confidence) {
                        ...
                        anomaly = {
                            ...
                            description: "".concat(rule.description, " (Score: ").concat(avgAnomalyScore.toFixed(1), ")"),
                            evidence: {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 3:
                        // Send notifications
                        _a.sent();
                        console.log("Anomaly detected: ".concat(anomaly.alertType, " for user ").concat(anomaly.userId, " (Risk: ").concat(anomaly.riskScore, ")"));
                        return [3 /*break*/, 5];
                    case 4:
                        error_2 = _a.sent();
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _a = anomaly.severity;
                        switch (_a) {
                            case 'critical': return [3 /*break*/, 1];
                            case 'high': return [3 /*break*/, 3];
                            case 'medium': return [3 /*break*/, 5];
                            case 'low': return [3 /*break*/, 7];
                        }
                        return [3 /*break*/, 9];
                    case 1: 
                    // Immediately suspend session
                    return [4 /*yield*/, this.suspendSession(anomaly.sessionId, 'Critical security anomaly detected')];
                    case 2:
                        // Immediately suspend session
                        _b.sent();
                        return [3 /*break*/, 9];
        ...
        switch (pattern.patternType) {
            case 'typing_speed':
                if (baseline.avgTypingSpeed > 0) {
                    var deviation = Math.abs(current.currentTypingSpeed - baseline.avgTypingSpeed) / baseline.avgTypingSpeed;
                    score = Math.min(100, deviation * 100);
                }
                break;
            case 'mouse_movement':
                if (baseline.avgMouseSpeed > 0) {
                    var deviation = Math.abs(current.currentMouseSpeed - baseline.avgMouseSpeed) / baseline.avgMouseSpeed;
                    score = Math.min(100, deviation * 100);
                }
                break;
            case 'time_pattern':
                var currentHour_1 = new Date().getHours();
                var isUsualTime = baseline.usualLoginTimes.some(function (hour) { return Math.abs(hour - currentHour_1) <= 2; });
            ...
            case 'interaction_frequency':
                if (baseline.interactionFrequency > 0) {
                    var deviation = Math.abs(current.interactionFrequency - baseline.interactionFrequency) / baseline.interactionFrequency;
    ...
    SuspiciousActivityDetector.prototype.getComparisonData = function (userId, patterns) {
        ...
        return {
            baseline: baseline,
            currentPatterns: patterns.map(function (p) { return p.currentMetrics; })
        };
    };
    SuspiciousActivityDetector.prototype.getContextualFactors = function (patterns) {
        ...
        if (patterns.some(function (p) { return p.anomalyScore > 70; }))
    ...
    SuspiciousActivityDetector.prototype.suspendSession = function (sessionId, reason) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('/api/session/suspend', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
    ...
    SuspiciousActivityDetector.prototype.requireReAuthentication = function (sessionId) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('/api/session/require-auth', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
    ...
    SuspiciousActivityDetector.prototype.updateUserRiskScore = function (userId, patterns) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        avgRiskScore = patterns.reduce(function (sum, p) { return sum + p.anomalyScore; }, 0) / patterns.length;
                        return [4 /*yield*/, fetch('/api/users/risk-score', {
                                method: 'PUT',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
    ...
    SuspiciousActivityDetector.prototype.getAnomalyAlerts = function (userId) {
        return Array.from(this.anomalyAlerts.values())
            .filter(function (alert) { return alert.userId === userId && !alert.resolved; });
    };
    ...
    SuspiciousActivityDetector.prototype.getUserRiskProfile = function (userId) {
        ...
        var recentPatterns = patterns.filter(function (p) { return Date.now() - p.timestamp < 86400000; }); // Last 24 hours
        var avgRiskScore = recentPatterns.length > 0
            ? recentPatterns.reduce(function (sum, p) { return sum + p.anomalyScore; }, 0) / recentPatterns.length
            : 0;
        var recentAnomalies = Array.from(this.anomalyAlerts.values())
            .filter(function (a) { return a.userId === userId && Date.now() - a.timestamp < 86400000; }).length;
        return {
            riskScore: Math.round(avgRiskScore),
            recentAnomalies: recentAnomalies,
            behaviorBaseline: this.getBehaviorBaseline(userId),
            lastAnalysis: recentPatterns.length > 0 ? Math.max.apply(Math, recentPatterns.map(function (p) { return p.timestamp; })) : 0
        };
    };
    ...
    SuspiciousActivityDetector.prototype.cleanup = function () {
        ...
        for (var _i = 0, _a = this.activePatterns.entries(); _i < _a.length; _i++) {
            var _b = _a[_i], userId = _b[0], patterns = _b[1];
            var recentPatterns = patterns.filter(function (p) { return now - p.timestamp < maxAge; });
            this.activePatterns.set(userId, recentPatterns);
        }
        // Clean up old anomalies
        for (var _c = 0, _d = this.anomalyAlerts.entries(); _c < _d.length; _c++) {
            var _e = _d[_c], id = _e[0], anomaly = _e[1];
