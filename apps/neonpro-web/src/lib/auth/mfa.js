...
exports.MFAUtils = exports.MFAService = exports.MFAVerificationSchema = exports.MFASetupSchema = void 0;
...
Promise < { isValid: boolean, delta: number } > {
    ...
    const: (_a = await this.supabase
        ...
        .eq('user_id', userId)
        .eq('method', 'totp')
        .single(), mfaData = _a.data, error = _a.error, _a),
    if: function (error) { }
} || !mfaData;
{
...
Promise < boolean > {
    const: (_b = await this.supabase
        ...
        .eq('user_id', userId)
        .eq('used', false)
        .single(), smsData = _b.data, error = _b.error, _b),
    if: function (error) { }
} || !smsData;
{
    return false;
...
Promise < boolean > {
    const: (_c = await this.supabase
        ...
        .eq('user_id', userId)
        .single(), mfaData = _c.data, error = _c.error, _c),
    if: function (error) { }
} || !mfaData;
{
    return false;
...
if (isValid) {
    ...
    var updatedBackupCodes = mfaData.backup_codes.filter(function (storedCode) { return storedCode !== hashedCode; });
    await this.supabase
        .from('user_mfa_settings')
        .update({
...
var auditLogId = await this.createAuditLog({
    ...
    metadata: {
        ...
        bypassCount: ((bypassCount === null || bypassCount === void 0 ? void 0 : bypassCount.length) || 0) + 1,
    },
});
...
Promise < string > {
    return: crypto_1.default.pbkdf2Sync(code, 'neonpro-mfa-salt', 100000, 64, 'sha512').toString('hex')
};
async;
hashRecoveryToken(token, string);
Promise < string > {
    return: crypto_1.default.pbkdf2Sync(token, 'neonpro-recovery-salt', 100000, 64, 'sha512').toString('hex')
};
async;
...
{
    var lastFailure = attempts.find(function (attempt) { return attempt.result === 'failure'; });
    var lockedUntil = new Date(new Date(lastFailure.timestamp).getTime() + RATE_LIMIT_CONFIG.lockoutMinutes * 60 * 1000);
...
Promise < string > {
    const: (_k = await this.supabase
        ...
        .select('id')
        .single(), data = _k.data, error = _k.error, _k),
    if: function (error) {
...
exports.MFAUtils = {
     ...
     */
    generateQRCodeDataURL: function (uri) { return __awaiter(void 0, void 0, void 0, function () {
