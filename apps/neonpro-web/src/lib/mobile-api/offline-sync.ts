...
export class OfflineSync {
  ...
  async performSync(
    options: SyncOptions & { userId: string; clinicId: string },
  ): Promise<SyncResult> {
  ...
  private async createServerRecord(entity: string, data: any): Promise<void> {
    const { error } = await this.supabase.from(entity).insert(data);
  ...
  private async updateServerRecord(entity: string, entityId: string, data: any): Promise<void> {
    const { error } = await this.supabase.from(entity).update(data).eq("id", entityId);
  ...
  private async deleteServerRecord(entity: string, entityId: string): Promise<void> {
    const { error } = await this.supabase.from(entity).delete().eq("id", entityId);
  ...
  private async saveOfflineStorage(): Promise<void> {
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        localStorage.setItem(`offline-storage-${this.userId}`, JSON.stringify(this.offlineStorage));
      }
    } catch (error) {
  ...
  private async saveOperationQueue(): Promise<void> {
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        localStorage.setItem(`operation-queue-${this.userId}`, JSON.stringify(this.operationQueue));
      }
    } catch (error) {
  ...
  private async saveConflictQueue(): Promise<void> {
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        localStorage.setItem(`conflict-queue-${this.userId}`, JSON.stringify(this.conflictQueue));
      }
    } catch (error) {
  ...
  private filterAppointments(params: any): OfflineAppointment[] {
    ...
    if (params?.date) {
      ...
      appointments = appointments.filter((a) => new Date(a.scheduledAt).toDateString() === date);
    }
  ...
  private updateAppointmentRecord(data: any): void {
    ...
    if (index > -1) {
      this.offlineStorage.appointments[index] = {
        ...this.offlineStorage.appointments[index],
        ...data,
      };
    } else {
