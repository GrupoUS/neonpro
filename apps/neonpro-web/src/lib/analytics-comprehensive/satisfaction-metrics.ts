...
export class PatientSatisfactionMetricsEngine {
  ...
  private processResponses(
    responses: Partial<Record<SatisfactionDimension, number>>,
  ): Record<SatisfactionDimension, number> {
  ...
  private async checkForImmediateActions(
    ...
    const criticalDimensions = Object.keys(scores).filter(
      (key) => scores[key as SatisfactionDimension] < criticalThreshold,
    );
  ...
  private calculateCES(surveys: any[]): number {
    const cesScores = surveys.map((s) => s.effort_score).filter((score) => score > 0);
  ...
  private calculateCSAT(surveys: any[]): number {
    const csatScores = surveys.map((s) => s.overall_score).filter((score) => score > 0);
  ...
  private calculateBenchmarkComparison(averageScores: Record<SatisfactionDimension, number>) {
    const variance: Record<SatisfactionDimension, number> = {} as Record<
  ...
  private calculateBenchmarkingInsights(surveys: any[]) {
    const latestSurvey = surveys[surveys.length - 1];
    const performanceVsIndustry: Record<SatisfactionDimension, "above" | "at" | "below"> =
      {} as any;
