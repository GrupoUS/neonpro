...
export class AdvancedAnalyticsManager {
  ...
  async getComprehensiveAnalytics(filter: AnalyticsFilter): Promise<ComprehensiveAnalytics> {
    ...
    try {
      ...
      const [schedulingMetrics, schedulingDashboard, performanceMetrics, alertDashboard] =
        await Promise.all([
      ...
      return {
        ...
        alerts: {
          ...
          criticalCount: alertDashboard.activeAlerts.filter((a) => a.severity === "critical")
            .length,
        },
        insights,
        summary,
      ...
      const keyMetrics = [
        {
          ...
          status:
            performanceMetrics.efficiency.appointmentCompletionRate >= 90 ? "good" : "warning",
        },
        {
          name: "Patient Satisfaction",
          value: performanceMetrics.satisfaction.overallSatisfactionScore,
          status:
            performanceMetrics.satisfaction.overallSatisfactionScore >= 4.0 ? "good" : "warning",
        },
        {
  ...
  ): ComprehensiveAnalytics["insights"] {
    const keyFindings = [];
    const actionItems = [];
    const trends = [];
    const riskFactors = [];
