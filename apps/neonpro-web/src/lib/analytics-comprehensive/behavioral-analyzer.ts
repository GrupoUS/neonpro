...
export interface BehavioralSegment {
  ...
  defining_characteristics: Record<
    BehavioralDimension,
    {
      importance: number;
...
export class BehavioralAnalysisSystem {
  ...
  async generateBehavioralPredictions(
    ...
    try {
      ...
      const lifecyclePredictions = await this.predictLifecycleMilestones(patientId, historicalData);

      // Assess risks
      const riskPredictions = await this.assessBehavioralRisks(patientId, historicalData);
   ...
   */
  async generatePersonalizedRecommendations(patientId: string): Promise<
    Array<{
  ...
  private async generateEngagementForecast(
    ...
    return {
      ...
      confidence_interval: [Math.max(0, forecastScore - 0.15), Math.min(1, forecastScore + 0.15)],
  ...
  private async saveBehavioralPrediction(prediction: BehavioralPrediction): Promise<void> {
    // Save prediction for future validation
    const { error } = await this.supabase.from("behavioral_predictions").insert(prediction);
  ...
  private async performBehavioralSegmentation(
  ...
  private async enrichSegmentCharacteristics(
  ...
  private async detectImmediateChanges(
    patientId: string,
    events: BehavioralEvent[],
  ): Promise<void> {
  ...
  private async calculateDimensionTrend(
    dimension: BehavioralDimension,
    events: BehavioralEvent[],
  ): Promise<"improving" | "stable" | "declining"> {
