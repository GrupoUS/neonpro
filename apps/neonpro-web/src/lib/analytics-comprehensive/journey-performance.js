...
exports.PerformanceUtils = exports.JourneyPerformanceAnalytics = exports.INDUSTRY_BENCHMARKS = exports.DEFAULT_KPIS = void 0;
...
var JourneyPerformanceAnalytics = /** @class */ (function () {
    function JourneyPerformanceAnalytics(config) {
        ...
        this.kpis = new Map();
        this.config = __assign({ enabledKPIs: exports.DEFAULT_KPIS.map(function (kpi) { return kpi.id; }), customKPIs: [], defaultPeriod: 'monthly', autoAnalysis: true, analysisFrequency: 24, criticalThresholds: {}, warningThresholds: {}, notificationSettings: {
                email: true,
                slack: false,
                dashboard: true,
                criticalOnly: false
            }, seasonalityDetection: true, trendAnalysis: true, forecastEnabled: true, benchmarkComparison: true }, config);
        this.initializeKPIs();
    }
    /**
    ...
    JourneyPerformanceAnalytics.prototype.analyzePerformance = function (clinicId_1) {
        return __awaiter(this, arguments, void 0, function (clinicId, options) {
            var _a, period, _b, startDate, _c, endDate, patientId, _d, includeForecasting, _e, includeBenchmarking, metrics, kpis, bottlenecks, opportunities, trends, benchmarkComparison, _f, industryPosition, _g, forecast, _h, dataQuality, analysis, error_1;
            if (options === void 0) { options = {}; }
            return __generator(this, function (_j) {
                switch (_j.label) {
                    case 0:
                        _j.trys.push([0, 17, , 18]);
                        _a = options.period, period = _a === void 0 ? this.config.defaultPeriod : _a, _b = options.startDate, startDate = _b === void 0 ? this.getDefaultStartDate(period) : _b, _c = options.endDate, endDate = _c === void 0 ? new Date() : _c, patientId = options.patientId, _d = options.includeForecasting, includeForecasting = _d === void 0 ? this.config.forecastEnabled : _d, _e = options.includeBenchmarking, includeBenchmarking = _e === void 0 ? this.config.benchmarkComparison : _e;
                        ...
                        return [4 /*yield*/, this.calculatePerformanceMetrics(clinicId, startDate, endDate, patientId)
                            // Calculate KPI results
                        ];
                    case 1:
                        metrics = _j.sent();
                        return [4 /*yield*/, this.calculateKPIResults(clinicId, metrics, startDate, endDate)
                            // Identify bottlenecks
                        ];
                    case 2:
                        kpis = _j.sent();
                        return [4 /*yield*/, this.identifyBottlenecks(clinicId, startDate, endDate, patientId)
                            // Find improvement opportunities
                        ];
                    case 3:
                        bottlenecks = _j.sent();
                        return [4 /*yield*/, this.findImprovementOpportunities(metrics, bottlenecks, kpis)
                            // Analyze trends
                        ];
                    case 4:
                        opportunities = _j.sent();
                        return [4 /*yield*/, this.analyzeTrends(clinicId, startDate, endDate, period)
     ...
     */
    JourneyPerformanceAnalytics.prototype.calculatePerformanceMetrics = function (clinicId, startDate, endDate, patientId) {
        return __awaiter(this, void 0, void 0, function () {
            var query, _a, journeys, error, totalJourneys, completedJourneys, droppedJourneys, conversionRate, completionRate, dropoffRate, journeyTimes, averageJourneyTime, stageConversions_1, stageDurations_1, stageDropoffs_1, revenueData, satisfactionData, metrics, error_2;
            var _b;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 1:
                        ...
                        if (error)
                            throw error;
                        totalJourneys = (journeys === null || journeys === void 0 ? void 0 : journeys.length) || 0;
                        completedJourneys = (journeys === null || journeys === void 0 ? void 0 : journeys.filter(function (j) { return j.status === 'completed'; }).length) || 0;
                        droppedJourneys = (journeys === null || journeys === void 0 ? void 0 : journeys.filter(function (j) { return j.status === 'dropped'; }).length) || 0;
                        ...
                        journeyTimes = (journeys === null || journeys === void 0 ? void 0 : journeys.filter(function (j) { return j.completed_at; }).map(function (j) {
                            var start = new Date(j.started_at);
                            var end = new Date(j.completed_at);
                            return (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24); // days
                        })) || [];
                        averageJourneyTime = journeyTimes.length > 0
                            ? journeyTimes.reduce(function (sum, time) { return sum + time; }, 0) / journeyTimes.length
                            : 0;
                        stageConversions_1 = {};
                        stageDurations_1 = {};
                        stageDropoffs_1 = {};
                        journeys === null || journeys === void 0 ? void 0 : journeys.forEach(function (journey) {
                            var _a;
                            (_a = journey.stages) === null || _a === void 0 ? void 0 : _a.forEach(function (stage) {
                                var stageName = stage.stage_name;
                                // Conversions
                                if (!stageConversions_1[stageName])
                                    stageConversions_1[stageName] = 0;
                                if (stage.status === 'completed')
                                    stageConversions_1[stageName]++;
                                // Durations
                                if (!stageDurations_1[stageName])
                                    stageDurations_1[stageName] = 0;
                                if (stage.duration_minutes) {
                                    stageDurations_1[stageName] += stage.duration_minutes / (60 * 24); // days
                                }
                                // Dropoffs
                                if (!stageDropoffs_1[stageName])
                                    stageDropoffs_1[stageName] = 0;
                                if (stage.status === 'dropped')
                                    stageDropoffs_1[stageName]++;
                            });
                        });
                    ...
                    case 5:
                        metrics = (_b.resourceUtilization = _c.sent(),
                            // Financial Metrics
     ...
     */
    JourneyPerformanceAnalytics.prototype.calculateKPIResults = function (clinicId, metrics, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function () {
            var results, _i, _a, _b, kpiId, kpi, currentValue, previousValue, change, changeDirection, status_1, achievementRate, trendData, insights, error_3;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        results = [];
     ...
     */
    JourneyPerformanceAnalytics.prototype.identifyBottlenecks = function (clinicId, startDate, endDate, patientId) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 4:
                        ...
                        // Sort by priority
                        bottlenecks.sort(function (a, b) { return b.priority - a.priority; });
     ...
     */
    JourneyPerformanceAnalytics.prototype.findImprovementOpportunities = function (metrics, bottlenecks, kpis) {
        return __awaiter(this, void 0, void 0, function () {
     ...
     */
    JourneyPerformanceAnalytics.prototype.analyzeTrends = function (clinicId, startDate, endDate, period) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        _a.trys.push([1, 6, , 7]);
                        metrics = ['conversion_rate', 'average_journey_time', 'satisfaction_score', 'revenue_per_journey'];
    ...
    JourneyPerformanceAnalytics.prototype.performBenchmarkComparison = function (metrics, industry) {
        return __awaiter(this, void 0, void 0, function () {
            var benchmarks, results, metricMappings, _i, _a, _b, metricKey, benchmarkKey, currentValue, benchmarkValue, difference, status_2;
            return __generator(this, function (_c) {
                ...
                for (_i = 0, _a = Object.entries(metricMappings); _i < _a.length; _i++) {
                    ...
                    if (currentValue !== undefined && benchmarkValue !== undefined) {
                        ...
                        status_2 = currentValue > benchmarkValue ? 'above' :
     ...
     */
    JourneyPerformanceAnalytics.prototype.generatePerformanceForecast = function (clinicId, metrics, trends) {
        return __awaiter(this, void 0, void 0, function () {
            var horizon, predictions, uncertaintyBounds, keyMetrics, _loop_1, this_1, _i, keyMetrics_1, metric, scenarios;
            return __generator(this, function (_a) {
                try {
                    horizon = 90 // 90 days
                    ;
                    ...
                    _loop_1 = function (metric) {
                        var trend = trends.find(function (t) { return t.metric === metric; });
     ...
     */
    JourneyPerformanceAnalytics.prototype.calculateEfficiencyScore = function (conversionRate, averageJourneyTime) {
    ...
    JourneyPerformanceAnalytics.prototype.determineKPIStatus = function (value, kpi) {
        if (value >= kpi.target)
            return 'excellent';
        if (value >= kpi.warningThreshold)
            return 'good';
        if (value >= kpi.criticalThreshold)
            return 'warning';
        return 'critical';
    };
    /**
     ...
     */
    JourneyPerformanceAnalytics.prototype.generateKPIInsights = function (kpi, value, change, status) {
        var insights = [];
        // Status insights
        if (status === 'excellent') {
            insights.push("".concat(kpi.name, " est\u00E1 superando a meta em ").concat(((value / kpi.target - 1) * 100).toFixed(1), "%"));
        }
        else if (status === 'critical') {
            insights.push("".concat(kpi.name, " est\u00E1 ").concat(((1 - value / kpi.target) * 100).toFixed(1), "% abaixo da meta cr\u00EDtica"));
        }
        // Change insights
        if (Math.abs(change) > 10) {
            var direction = change > 0 ? 'aumento' : 'redução';
            insights.push("".concat(direction, " significativo de ").concat(Math.abs(change).toFixed(1), "% em rela\u00E7\u00E3o ao per\u00EDodo anterior"));
        }
        // Benchmark insights
        if (value < kpi.benchmark) {
            insights.push("Abaixo do benchmark da ind\u00FAstria (".concat(kpi.benchmark, " ").concat(kpi.unit, ")"));
        }
        return insights;
    };
    /**
     ...
     */
    JourneyPerformanceAnalytics.prototype.calculateAnalysisConfidence = function (dataQuality, metrics) {
