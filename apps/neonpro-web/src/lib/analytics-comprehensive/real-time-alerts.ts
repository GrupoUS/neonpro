...
export interface AlertTriggerCondition {
  metric: string
  operator: 'equals' | 'not_equals' | 'greater_than' | 'less_than' | 'greater_equal' | 'less_equal' | 'contains' | 'not_contains'
...
export class RealTimeAlertSystem {
  ...
  private deliveryQueue: Map<DeliveryChannel, Array<{ alert: AlertInstance; config: AlertDeliveryConfig }>> = new Map()
   ...
   */
  async createAlertRule(ruleConfig: Omit<AlertRule, 'id' | 'created_at' | 'updated_at'>): Promise<{ success: boolean; rule_id?: string; error?: string }> {
    try {
  ...
  async getActiveAlerts(
    ...
    try {
      ...
      if (error) {
        ...
        return { alerts: [], total_count: 0, page_info: { current_page: 1, total_pages: 0, has_next: false } }
      }
  ...
  private async getApplicableAlertRules(dataType: string, data: Record<string, any>): Promise<AlertRule[]> {
  ...
  private async deliverEmailAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
    // Email delivery implementation
    return { success: true, delivery_id: `email_${Date.now()}` }
  }

  private async deliverSMSAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
    // SMS delivery implementation
    return { success: true, delivery_id: `sms_${Date.now()}` }
  }

  private async deliverPushAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
    // Push notification implementation
    return { success: true, delivery_id: `push_${Date.now()}` }
  }

  private async deliverInAppAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
    // In-app notification implementation
    return { success: true, delivery_id: `inapp_${Date.now()}` }
  }

  private async deliverWebhookAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
    // Webhook delivery implementation
    return { success: true, delivery_id: `webhook_${Date.now()}` }
  }

  private async deliverDashboardAlert(alert: AlertInstance, config: AlertDeliveryConfig): Promise<{ success: boolean; delivery_id?: string; error?: string }> {
  ...
  private async calculateDeliveryMetrics(alerts: AlertInstance[]): Promise<any> {
    ...
    const channelStats = new Map<DeliveryChannel, { total: number; successful: number; totalTime: number }>()
  ...
  private calculateEffectivenessMetrics(alerts: AlertInstance[]): any {
    return {
      ...
      preventive_actions_taken: alerts.filter(a => a.resolution_data?.prevention_measures?.length).length,
      business_impact_prevented: {
  ...
  private async notifyAlertAcknowledgment(alert: AlertInstance, acknowledgedBy: string, notes?: string): Promise<void> {
    // Notify stakeholders of acknowledgment
    logger.info('Alert acknowledgment notification sent', { alert_id: alert.id })
  }

  private async notifyAlertResolution(alert: AlertInstance, resolvedBy: string, action: string): Promise<void> {
    // Notify stakeholders of resolution
    logger.info('Alert resolution notification sent', { alert_id: alert.id })
  }

  private async sendEscalatedNotifications(alert: AlertInstance, escalationRule: any): Promise<void> {
    // Send escalated notifications to higher level
    logger.info('Escalated notifications sent', { alert_id: alert.id })
  }

  private async createFollowUpTasks(alert: AlertInstance, preventionMeasures: string[]): Promise<void> {
    // Create follow-up tasks for prevention measures
    logger.info('Follow-up tasks created', { alert_id: alert.id, measures_count: preventionMeasures.length })
  }
