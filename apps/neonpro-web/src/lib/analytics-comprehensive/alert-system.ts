...
export class AlertSystem {
  ...
  async createAlertRule(
    ...
    // Store in database
    const { error } = await this.supabase.from("alert_rules").insert(newRule);
  ...
  async updateAlertRule(ruleId: string, updates: Partial<AlertRule>): Promise<AlertRule | null> {
    ...
    // Update in database
    const { error } = await this.supabase.from("alert_rules").update(updatedRule).eq("id", ruleId);
  ...
  private calculateAlertsByCategory(
  ...
  private async calculateAlertTrends(): Promise<
    { date: string; count: number; severity: string }[]
  > {
