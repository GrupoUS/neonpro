...
export class PerformanceCalculator {
  ...
  private async calculateEfficiencyMetrics(filter: AnalyticsFilter): Promise<EfficiencyMetrics> {
    ...
    const totalDuration =
      appointments?.reduce((sum, a) => sum + (a.actual_duration || a.scheduled_duration || 0), 0) ||
      0;
    ...
    const scheduledVsActualDuration =
    ...
    const taskCompletionEfficiency =
      completedAppointments > 0 ? (onTimeCompletions / completedAppointments) * 100 : 0;
  ...
  private async calculateQualityMetrics(filter: AnalyticsFilter): Promise<QualityMetrics> {
    ...
    const successfulTreatments =
      treatmentsWithOutcomes.filter((t) => t.treatment_outcomes?.success) || [];
    const treatmentSuccessRate =
      treatmentsWithOutcomes.length > 0
        ? (successfulTreatments.length / treatmentsWithOutcomes.length) * 100
        : 0;

    // Complication rate
    const complicatedTreatments =
      treatmentsWithOutcomes.filter((t) => t.treatment_outcomes?.complications) || [];
    const complicationRate =
      treatmentsWithOutcomes.length > 0
        ? (complicatedTreatments.length / treatmentsWithOutcomes.length) * 100
        : 0;
  ...
  private async calculateUtilizationMetrics(filter: AnalyticsFilter): Promise<UtilizationMetrics> {
    ...
    const totalWorkedHours =
      staffSchedules?.reduce((sum, s) => {
        return (
          sum +
          (s.appointments?.reduce((appointmentSum: number, a: any) => {
    ...
    const staffUtilizationRate =
      totalScheduledHours > 0 ? (totalWorkedHours / totalScheduledHours) * 100 : 0;
    ...
    const capacityUtilizationRate =
      (staffUtilizationRate + equipmentUtilizationRate + roomUtilizationRate) / 3;
  ...
  private async calculateSatisfactionMetrics(
    ...
    const staffRatings = feedbackData
      .map((f) => f.staff_interaction_rating)
      .filter((r) => r !== null);
    const staffInteractionScore =
    ...
    const comparisons: BenchmarkComparison[] = [
      {
        ...
        percentileRank: this.calculatePercentileRank(
          ...
          this.INDUSTRY_BENCHMARKS.appointmentCompletionRate,
        ),
        performanceGap:
          metrics.efficiency.appointmentCompletionRate -
          this.INDUSTRY_BENCHMARKS.appointmentCompletionRate,
        improvementTarget: Math.max(
          this.INDUSTRY_BENCHMARKS.appointmentCompletionRate,
          metrics.efficiency.appointmentCompletionRate * 1.05,
        ),
        achievabilityScore: this.calculateAchievabilityScore(
          metrics.efficiency.appointmentCompletionRate,
          this.INDUSTRY_BENCHMARKS.appointmentCompletionRate,
        ),
      },
      {
        ...
        percentileRank: this.calculatePercentileRank(
          metrics.satisfaction.overallSatisfactionScore,
          this.INDUSTRY_BENCHMARKS.patientSatisfactionScore,
        ),
        performanceGap:
          metrics.satisfaction.overallSatisfactionScore -
          this.INDUSTRY_BENCHMARKS.patientSatisfactionScore,
        improvementTarget: Math.max(
          this.INDUSTRY_BENCHMARKS.patientSatisfactionScore,
          metrics.satisfaction.overallSatisfactionScore * 1.1,
        ),
        achievabilityScore: this.calculateAchievabilityScore(
          metrics.satisfaction.overallSatisfactionScore,
          this.INDUSTRY_BENCHMARKS.patientSatisfactionScore,
      ...
      },
      {
        ...
        percentileRank: this.calculatePercentileRank(
          metrics.utilization.staffUtilizationRate,
          this.INDUSTRY_BENCHMARKS.staffUtilizationRate,
        ),
        performanceGap:
          metrics.utilization.staffUtilizationRate - this.INDUSTRY_BENCHMARKS.staffUtilizationRate,
        improvementTarget: Math.max(
          this.INDUSTRY_BENCHMARKS.staffUtilizationRate,
          metrics.utilization.staffUtilizationRate * 1.05,
        ),
        achievabilityScore: this.calculateAchievabilityScore(
          metrics.utilization.staffUtilizationRate,
          this.INDUSTRY_BENCHMARKS.staffUtilizationRate,
      ...
      },
      {
        ...
        percentileRank: this.calculatePercentileRank(
          metrics.productivity.revenuePerHour,
          this.INDUSTRY_BENCHMARKS.revenuePerHour,
        ),
        performanceGap:
          metrics.productivity.revenuePerHour - this.INDUSTRY_BENCHMARKS.revenuePerHour,
        improvementTarget: Math.max(
          this.INDUSTRY_BENCHMARKS.revenuePerHour,
          metrics.productivity.revenuePerHour * 1.1,
        ),
        achievabilityScore: this.calculateAchievabilityScore(
          metrics.productivity.revenuePerHour,
          this.INDUSTRY_BENCHMARKS.revenuePerHour,
