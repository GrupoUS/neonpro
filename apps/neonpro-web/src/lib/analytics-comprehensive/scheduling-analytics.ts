...
export class SchedulingAnalytics {
  ...
  async getSchedulingMetrics(filter: AnalyticsFilter): Promise<SchedulingMetrics> {
    ...
    try {
      ...
      const avgDuration = appointments?.reduce((sum, a) => sum + (a.actual_duration || a.scheduled_duration || 0), 0) / totalAppointments || 0
      
      // Calculate utilization rate
      const totalScheduledHours = this.calculateTotalScheduledHours(filter)
      const totalUsedHours = appointments?.reduce((sum, a) => sum + ((a.actual_duration || a.scheduled_duration || 0) / 60), 0) || 0
      const utilizationRate = totalScheduledHours > 0 ? (totalUsedHours / totalScheduledHours) * 100 : 0

      // Calculate revenue
      const revenueGenerated = appointments?.reduce((sum, a) => {
  ...
  async getSeasonalTrends(filter: AnalyticsFilter): Promise<SeasonalTrend[]> {
    ...
    try {
      ...
      const result: SeasonalTrend[] = trends.map((trend, index) => {
        ...
        const averageVolume = trends.reduce((sum, t) => sum + t.appointmentVolume, 0) / trends.length
    ...
    try {
      ...
      const result: StaffPerformance[] = await Promise.all(
        (staffData || []).map(async (staff) => {
          const appointments = staff.appointments?.filter(a => {
          ...
          const totalDuration = appointments.reduce((sum, a) => sum + (a.actual_duration || a.scheduled_duration || 0), 0)
          ...
          const ratings = appointments.flatMap(a => a.patient_feedback?.map(f => f.rating) || [])
          const avgRating = ratings.length > 0 ? ratings.reduce((sum, r) => sum + r, 0) / ratings.length : 0
          
          // Calculate revenue generated
          const revenueGenerated = appointments.reduce((sum, a) => sum + (a.services?.price || 0), 0)
          
          // Calculate efficiency score (completed appointments / total scheduled)
          const efficiencyScore = appointments.length > 0 ? (completedAppointments.length / appointments.length) * 100 : 0
  ...
  private async calculateHourlyStaffUtilization(dayOfWeek: string, hour: number, filter: AnalyticsFilter): Promise<number> {
  ...
  private async calculateStaffUtilization(staffId: string, filter: AnalyticsFilter): Promise<number> {
    // Implementation for calculating staff utilization
    return 0 // Placeholder
  }

  private async calculateSkillUtilization(staffId: string, filter: AnalyticsFilter): Promise<number> {
    // Implementation for calculating skill utilization
    return 0 // Placeholder
  }

  private async calculateWorkloadBalance(staffId: string, filter: AnalyticsFilter): Promise<number> {
