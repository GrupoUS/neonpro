...
var JourneyStateMachine = /** @class */ (function () {
    ...
    JourneyStateMachine.STATE_TRANSITIONS = {
        ...
        consultation_completed: ['treatment_proposed', 'follow_up_scheduled', 'journey_completed', 'dormant'],
        ...
        treatment_started: ['treatment_ongoing', 'treatment_completed', 'treatment_paused', 'abandoned'],
...
var PatientJourneyMappingEngine = /** @class */ (function () {
    ...
    PatientJourneyMappingEngine.prototype.initializePatientJourney = function (patientId_1) {
        return __awaiter(this, arguments, void 0, function (patientId, initialState, config) {
            ...
            if (initialState === void 0) { initialState = 'lead_generated'; }
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 3, , 4]);
                        journeyConfig = __assign({ patient_id: patientId, tracking_enabled: true, auto_stage_progression: true, satisfaction_tracking: true, touchpoint_analysis: true, real_time_updates: true }, config);
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.trackJourneyEvent(patientId, {
    ...
    PatientJourneyMappingEngine.prototype.trackJourneyEvent = function (patientId, eventData) {
        return __awaiter(this, void 0, void 0, function () {
            var config, _a, included_types, excluded_types, minimum_quality_score, _b, eventRecord, error, error_2;
                ...
                switch (_c.label) {
                    case 0:
                        ...
                        if (config === null || config === void 0 ? void 0 : config.event_filters) {
                            _a = config.event_filters, included_types = _a.included_types, excluded_types = _a.excluded_types, minimum_quality_score = _a.minimum_quality_score;
                            ...
                            if (minimum_quality_score && eventData.metadata.quality_score &&
                        ...
                        return [4 /*yield*/, this.supabase
                                ...
                                metadata: __assign(__assign({}, eventData.metadata), { tracking_timestamp: new Date().toISOString() })
                            })
                                .select()
                                .single()];
                    case 1:
                        ...
                        if (!(config === null || config === void 0 ? void 0 : config.auto_stage_progression)) return [3 /*break*/, 3];
                        return [4 /*yield*/, this.checkAndProgressStage(patientId, eventData.event_type)];
                    case 2:
                        _c.sent();
                        _c.label = 3;
                    case 3:
                        if (!(config === null || config === void 0 ? void 0 : config.real_time_updates)) return [3 /*break*/, 5];
     ...
     */
    PatientJourneyMappingEngine.prototype.progressJourneyStage = function (patientId, newState, metadata) {
        return __awaiter(this, void 0, void 0, function () {
                ...
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.supabase
                                ...
                                status: 'completed',
                                duration_minutes: Math.round((new Date().getTime() - new Date(currentStage.started_at).getTime()) / (1000 * 60))
                            })
                    ...
                    case 3: return [4 /*yield*/, this.supabase
                            ...
                            started_at: new Date().toISOString(),
                            status: 'active',
                            metadata: __assign({ previous_stage: currentState, transition_timestamp: new Date().toISOString() }, metadata)
                        })
                            .select()
                            .single()];
                    case 4:
    ...
    PatientJourneyMappingEngine.prototype.analyzePatientJourney = function (patientId) {
        return __awaiter(this, void 0, void 0, function () {
            var stages, events, currentStage, currentState, journeyStart, journeyDuration, totalTouchpoints, completedStages, totalPossibleStages, completionScore, satisfactionScores, conversionProbability, churnRiskScore, bottlenecks, recommendations, insights, analysis, error_4;
            var _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                    ...
                    case 2:
                        events = (_b.sent()).data;
                        currentStage = stages.find(function (s) { return s.status === 'active'; });
                        currentState = (currentStage === null || currentStage === void 0 ? void 0 : currentStage.stage_name) || 'abandoned';
                        journeyStart = new Date(((_a = stages[0]) === null || _a === void 0 ? void 0 : _a.started_at) || new Date());
                        journeyDuration = Math.round((new Date().getTime() - journeyStart.getTime()) / (1000 * 60 * 60 * 24));
                        totalTouchpoints = (events === null || events === void 0 ? void 0 : events.length) || 0;
                        completedStages = stages.filter(function (s) { return s.status === 'completed'; }).length;
                        ...
                        satisfactionScores = stages
                            .filter(function (s) { return s.satisfaction_score !== null; })
                            .map(function (s) { return s.satisfaction_score; });
                        conversionProbability = this.calculateConversionProbability(currentState, stages);
                        return [4 /*yield*/, this.calculateChurnRiskScore(patientId, events || [], stages)
                    ...
                    case 3:
                        ...
                        analysis = {
                            ...
                            stage_progression: stages.map(function (stage) { return ({
                                id: stage.id,
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, this.progressJourneyStage(patientId, targetState, {
     ...
     */
    PatientJourneyMappingEngine.prototype.calculateConversionProbability = function (currentState, stages) {
        ...
        var averageDuration = stages.reduce(function (sum, stage) {
            return sum + (stage.duration_minutes || 0);
        }, 0) / stages.length;
        if (averageDuration < 1440) { // Less than 24 hours average
            probability *= 1.2; // Faster progression increases probability
        }
        else if (averageDuration > 10080) { // More than 7 days average
            probability *= 0.8; // Slower progression decreases probability
        }
        // Adjust based on satisfaction scores
        var avgSatisfaction = stages
            .filter(function (s) { return s.satisfaction_score; })
            .reduce(function (sum, s) { return sum + s.satisfaction_score; }, 0) /
            stages.filter(function (s) { return s.satisfaction_score; }).length;
     ...
     */
    PatientJourneyMappingEngine.prototype.calculateChurnRiskScore = function (patientId, events, stages) {
        return __awaiter(this, void 0, void 0, function () {
            var riskScore, lastEvent, daysSinceLastActivity, negativeEvents, currentStage, latestSatisfaction;
            var _a;
            return __generator(this, function (_b) {
                riskScore = 0;
                lastEvent = events[events.length - 1];
                if (lastEvent) {
                    daysSinceLastActivity = Math.round((new Date().getTime() - new Date(lastEvent.timestamp).getTime()) / (1000 * 60 * 60 * 24));
                ...
                negativeEvents = events.filter(function (e) {
                    return e.event_type === 'appointment_cancelled' ||
                ...
                currentStage = stages.find(function (s) { return s.status === 'active'; });
                if (currentStage && currentStage.duration_minutes > 20160) { // More than 14 days in current stage
                    riskScore += 0.2;
                }
                latestSatisfaction = (_a = stages
                    .filter(function (s) { return s.satisfaction_score; })
                    .pop()) === null || _a === void 0 ? void 0 : _a.satisfaction_score;
     ...
     */
    PatientJourneyMappingEngine.prototype.generateRecommendations = function (currentState, stages, events) {
        var recommendations = [];
        ...
        var daysSinceLastActivity = events.length > 0 ? Math.round((new Date().getTime() - new Date(events[events.length - 1].timestamp).getTime()) / (1000 * 60 * 60 * 24)) : 0;
    ...
    PatientJourneyMappingEngine.prototype.generateKeyInsights = function (stages, events) {
        ...
        // Timing insights
        var eventTimes = events.map(function (e) { return new Date(e.timestamp).getHours(); });
        var avgTime = eventTimes.reduce(function (sum, time) { return sum + time; }, 0) / eventTimes.length;
        if (avgTime > 17) {
    ...
    PatientJourneyMappingEngine.prototype.identifySuccessFactors = function (stages, events) {
        var successFactors = [];
        // Quick progression
        var avgStageDuration = stages.reduce(function (sum, stage) {
            return sum + (stage.duration_minutes || 0);
        }, 0) / stages.length;
        if (avgStageDuration < 2880) { // Less than 2 days average
        ...
        var uniqueChannels = new Set(events.map(function (e) { return e.channel; })).size;
        if (uniqueChannels >= 3) {
            successFactors.push('Engajamento multi-canal');
        }
        // Consistent activity
        var eventSpread = events.length > 1 ?
            (new Date(events[events.length - 1].timestamp).getTime() -
                new Date(events[0].timestamp).getTime()) / (1000 * 60 * 60 * 24) : 0;
        if (eventSpread > 0 && events.length / eventSpread > 0.5) { // More than 0.5 events per day
     ...
     */
    PatientJourneyMappingEngine.prototype.identifyOptimizationOpportunities = function (stages, events) {
        var opportunities = [];
        // Long duration in specific stages
        stages.forEach(function (stage) {
            if (stage.duration_minutes > 10080) { // More than 7 days
        ...
        var stagesWithoutSatisfaction = stages.filter(function (s) { return !s.satisfaction_score; }).length;
        ...
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                try {
                    ...
                    logger_1.logger.debug("Real-time update triggered for patient ".concat(patientId), { eventId: eventId });
                }
                catch (error) {
