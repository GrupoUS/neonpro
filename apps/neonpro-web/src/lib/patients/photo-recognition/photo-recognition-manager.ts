...
export class PhotoRecognitionManager {
  ...
  private async assessPhotoQuality(photoFile: File | Buffer): Promise<PhotoQualityAssessment> {
    try {
      ...
      const fileSize = photoFile instanceof File ? photoFile.size : Buffer.byteLength(photoFile as Buffer)
      const recommendations: string[] = []
   ...
   * Extract image dimensions
   */
  private async extractImageDimensions(photoFile: File | Buffer): Promise<{ width: number; height: number }> {
    try {
  ...
  async getRecognitionStats(patientId?: string): Promise<{
    ...
    try {
      ...
      const lastRecognition = photos
        ...
        .sort((a, b) => new Date(b.upload_date).getTime() - new Date(a.upload_date).getTime())[0]?.upload_date
