...
class ApprovalService {
  ...
  async createApprovalRequest(requestData: {
    ...
    try {
      ...
      const applicableLevels = levels.filter(level => 
      ...
      const isAutoApproved = firstLevel.auto_approve_below !== null && 
  ...
  private async sendApprovalNotification(
    ...
    try {
      ...
      console.log(`Notification: Request ${requestId} was ${action} by ${approverName}`)
    ...
    try {
      ...
      for (const level of levels) {
        ...
        if (levelUsers.length < level.required_approvers) {
          issues.push(`Nível ${level.level_name} requer ${level.required_approvers} aprovadores mas só tem ${levelUsers.length}`)
        }
      }
      ...
      for (let i = 0; i < levels.length; i++) {
        for (let j = i + 1; j < levels.length; j++) {
          ...
          const l1Max = level1.max_amount || Infinity
          const l2Min = level2.min_amount
          const l2Max = level2.max_amount || Infinity

          if ((l1Min <= l2Max && l1Max >= l2Min)) {
            issues.push(`Níveis ${level1.level_name} e ${level2.level_name} têm faixas de valores sobrepostas`)
          }
        }
      }
