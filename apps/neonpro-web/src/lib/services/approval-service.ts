...
class ApprovalService {
  ...
  async createApprovalLevel(levelData: Partial<ApprovalLevel>): Promise<ApprovalLevel> {
    try {
      const { data, error } = await this.supabase
        .from("approval_levels")
        .insert([
  ...
  async updateApprovalLevel(id: string, levelData: Partial<ApprovalLevel>): Promise<ApprovalLevel> {
    try {
      const { data, error } = await this.supabase
        .from("approval_levels")
        .update({
  ...
  async createApprovalUser(userData: Partial<ApprovalUser>): Promise<ApprovalUser> {
    try {
      const { data, error } = await this.supabase
        .from("approval_users")
        .insert([
  ...
  async updateApprovalUser(id: string, userData: Partial<ApprovalUser>): Promise<ApprovalUser> {
    try {
      const { data, error } = await this.supabase
        .from("approval_users")
        .update(userData)
  ...
  async createApprovalRequest(requestData: {
    ...
    try {
      ...
      const { error: stepsError } = await this.supabase.from("approval_steps").insert(steps);
  ...
  async processApprovalAction(
    ...
    try {
      ...
      const userPermission = await this.validateUserPermission(user.id);

      await this.recordApprovalAction(stepId, user, action, comments, userPermission);
      await this.updateStepStatus(step, action);

      this.sendApprovalNotification(step.approval_request_id, action, user.email || "Usuário");
    } catch (_error) {
  ...
  private async getApprovalStep(stepId: string) {
  ...
  private async validateUserPermission(userId: string) {
  ...
  private async recordApprovalAction(
    ...
    const { error: actionError } = await this.supabase.from("approval_actions").insert([
      {
  ...
  private async updateStepStatus(step: any, action: string) {
    const newApprovedCount = action === "approve" ? step.approved_count + 1 : step.approved_count;
    const stepStatus = this.calculateStepStatus(action, newApprovedCount, step.required_approvers);

    await this.updateStepInDatabase(step.id, newApprovedCount, stepStatus);

    if (stepStatus === "approved" || stepStatus === "rejected") {
      await this.updateRequestStatus(step, stepStatus);
    }
  }

  private calculateStepStatus(
    action: string,
  ...
  private async updateStepInDatabase(stepId: string, approvedCount: number, status: string) {
  ...
  private async updateRequestStatus(step: any, stepStatus: string) {
    const newCurrentLevel = this.calculateNewCurrentLevel(stepStatus, step);
    const newRequestStatus = this.calculateNewRequestStatus(stepStatus, step.level_order);
  ...
  private calculateNewCurrentLevel(stepStatus: string, step: any): number {
    if (stepStatus === "approved" && step.approval_requests.current_level < 3) {
      return step.approval_requests.current_level + 1;
    }
    return step.approval_requests.current_level;
  }

  private calculateNewRequestStatus(stepStatus: string, levelOrder: number): string {
    if (stepStatus === "rejected") {
      return "rejected";
    }
    if (stepStatus === "approved" && levelOrder === 3) {
      return "approved";
    }
    return "pending";
  }
  ...
  async validateApprovalHierarchy(): Promise<{
    ...
    try {
      ...
      for (const level of levels) {
        const levelUsers = users.filter((u) => u.approval_level_id === level.id);
