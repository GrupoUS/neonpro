...
export class createkpiCalculationService {
  ...
  async calculateClinicKPIs(
    ...
    try {
      ...
      for (const kpiDef of kpiDefinitions) {
        const result = await this.calculateSingleKPI(kpiDef, period.start, period.end);
        if (result) {
          results.push(result);
  ...
  async calculateSingleKPI(
    ...
    try {
      ...
      const currentValue = await this.executeKPICalculation(kpiDefinition, periodStart, periodEnd);
      ...
      const formattedPreviousValue =
        previousValue !== null ? this.formatKPIValue(previousValue, kpiDefinition.unit) : undefined;
  ...
  private calculateStatus(value: number, kpiDefinition: any): "normal" | "warning" | "critical" {
