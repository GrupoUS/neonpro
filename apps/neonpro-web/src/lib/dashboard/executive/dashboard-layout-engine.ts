...
export class DashboardLayoutEngine {
    ...
    try {
      ...
      if (layout.widgets.length > 0) {
        ...
        if (widgetsError) {
          ...
          await this.supabase.from("executive_dashboards").delete().eq("id", dashboardId);
          return null;
        }
      }
  ...
  async updateDashboardLayout(
    ...
    try {
      ...
      if (updates.widgets) {
        ...
        await this.supabase.from("dashboard_widgets").delete().eq("dashboard_id", dashboardId);
   ...
   */
  async updateWidgetPosition(widgetId: string, position: WidgetPosition): Promise<boolean> {
    try {
      const { error } = await this.supabase
        .from("dashboard_widgets")
        .update({
