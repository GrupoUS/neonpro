...
var RouteProtector = /** @class */ (function () {
    ...
    RouteProtector.prototype.checkAccess = function (req, context) {
        return __awaiter(this, void 0, void 0, function () {
            var startTime, route, method, permission, rateLimitResult, subscriptionResult, roleResult, featureResult, customResult, result, error_1, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        if (!permission) {
                            return [2 /*return*/, this.createResult(false, 'Route not found', '/dashboard', 'ROUTE_NOT_FOUND')];
                        }
                        // Check authentication requirement
                        if (permission.requiresAuth && !context.userId) {
                            return [2 /*return*/, this.createResult(false, 'Authentication required', '/login', 'AUTH_REQUIRED')];
                        }
                    ...
                    case 4:
                        error_1 = _a.sent();
                        result = this.createResult(false, 'System error during access check', '/dashboard', 'SYSTEM_ERROR');
    ...
    RouteProtector.prototype.findRoutePermission = function (route) {
        for (var _i = 0, ROUTE_PERMISSIONS_1 = ROUTE_PERMISSIONS; _i < ROUTE_PERMISSIONS_1.length; _i++) {
    ...
    RouteProtector.prototype.checkSubscription = function (context, permission) {
        ...
        if (permission.minimumTier) {
            ...
            if (currentLevel < requiredLevel) {
                return this.createResult(false, "Subscription tier ".concat(permission.minimumTier, " required"), '/dashboard/subscription', 'TIER_REQUIRED');
            }
        }
        ...
        if (context.subscriptionStatus !== 'active' && context.subscriptionStatus !== 'trialing') {
            ...
            return this.createResult(false, 'Active subscription required', '/dashboard/subscription', 'SUBSCRIPTION_REQUIRED');
        }
    ...
    RouteProtector.prototype.checkRolePermissions = function (context, permission) {
        ...
        if (permission.allowedRoles && !permission.allowedRoles.includes(context.userRole)) {
            return this.createResult(false, "Role ".concat(context.userRole, " not allowed for this resource"), '/dashboard', 'ROLE_NOT_ALLOWED');
        }
        // Check required permissions
        if (permission.requiredPermissions) {
            var hasRequiredPermissions = permission.requiredPermissions.every(function (required) { return context.permissions.includes(required); });
            if (!hasRequiredPermissions) {
                return this.createResult(false, 'Insufficient permissions', '/dashboard', 'INSUFFICIENT_PERMISSIONS');
            }
        }
    ...
    RouteProtector.prototype.checkFeatureFlags = function (context, permission) {
        ...
        if (missingFlags.length > 0) {
            return this.createResult(false, "Feature not available: ".concat(missingFlags.join(', ')), '/dashboard', 'FEATURE_DISABLED');
        }
    ...
    RouteProtector.prototype.logAccess = function (req, context, permission, result, duration) {
        ...
        var log = {
            ...
            duration: duration,
            metadata: __assign({ errorCode: result.errorCode, subscriptionTier: context.subscriptionTier, userRole: context.userRole }, (permission.auditLevel === 'detailed' && {
    ...
    RouteProtector.prototype.getAccessLogs = function (userId, limit) {
        ...
        if (userId) {
            logs = logs.filter(function (log) { return log.userId === userId; });
        }
        return logs
...
function calculateGracePeriodEnd(expiresAt, graceDays) {
    if (graceDays === void 0) { graceDays = 3; }
