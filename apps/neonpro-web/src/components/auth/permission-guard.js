...
function PermissionGuard(_a) {
    var _this = this;
    var permissions = _a.permissions, children = _a.children, fallback = _a.fallback, _b = _a.showLoading, showLoading = _b === void 0 ? true : _b, _c = _a.allowEmergencyOverride, allowEmergencyOverride = _c === void 0 ? false : _c, context = _a.context, ariaLabel = _a["aria-label"], ariaDescribedby = _a["aria-describedby"], className = _a.className, _d = _a.showDetails, showDetails = _d === void 0 ? false : _d, _e = _a.requireAll, requireAll = _e === void 0 ? true : _e, _f = _a.errorMessages, errorMessages = _f === void 0 ? {} : _f, onPermissionCheck = _a.onPermissionCheck, onEmergencyOverride = _a.onEmergencyOverride;
    var _g = (0, react_1.useState)(false), showEmergencyDialog = _g[0], setShowEmergencyDialog = _g[1];
    var _h = (0, react_1.useState)(''), emergencyPermission = _h[0], setEmergencyPermission = _h[1];
    // Normalize permissions to array
    var permissionArray = Array.isArray(permissions) ? permissions : [permissions];
    // Use permissions hook
    var _j = (0, use_permissions_1.usePermissions)(), checkPermissions = _j.checkPermissions, userRole = _j.userRole, isLoading = _j.isLoading, hasEmergencyOverride = _j.hasEmergencyOverride, requestEmergencyOverride = _j.requestEmergencyOverride;
    var _k = (0, react_1.useState)([]), permissionResults = _k[0], setPermissionResults = _k[1];
    var _l = (0, react_1.useState)(true), isCheckingPermissions = _l[0], setIsCheckingPermissions = _l[1];
    // Check permissions on mount and when dependencies change
    var checkPermissionsAsync = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        var results, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    onPermissionCheck === null || onPermissionCheck === void 0 ? void 0 : onPermissionCheck(results);
                    return [3 /*break*/, 5];
                case 3:
                    error_1 = _a.sent();
    ...
    var isAccessGranted = (0, react_1.useCallback)(function () {
        if (permissionResults.length === 0)
            return false;
        return requireAll
            ? permissionResults.every(function (result) { return result.granted; })
            : permissionResults.some(function (result) { return result.granted; });
    }, [permissionResults, requireAll]);
    // Handle emergency override request
    var handleEmergencyOverrideRequest = (0, react_1.useCallback)(function (permission) {
        if (!allowEmergencyOverride || !hasEmergencyOverride)
            return;
        setEmergencyPermission(permission);
        setShowEmergencyDialog(true);
    }, [allowEmergencyOverride, hasEmergencyOverride]);
    // Handle emergency override confirmation
    var handleEmergencyOverrideConfirm = (0, react_1.useCallback)(function (reason) { return __awaiter(_this, void 0, void 0, function () {
        var error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                ...
                case 1:
                    ...
                    onEmergencyOverride === null || onEmergencyOverride === void 0 ? void 0 : onEmergencyOverride(emergencyPermission);
    ...
    if (isLoading || isCheckingPermissions) {
        return showLoading ? (<div className={(0, utils_1.cn)("space-y-3", className)} aria-label={ariaLabel || "Checking permissions"} aria-describedby={ariaDescribedby}>
    ...
    if (isAccessGranted()) {
        return (<div className={className} aria-label={ariaLabel} aria-describedby={ariaDescribedby}>
        {children}
        {showDetails && (<PermissionDetails results={permissionResults} userRole={userRole} className="mt-4"/>)}
      </div>);
    }
    // Permission denied - show fallback or default error
    var deniedResults = permissionResults.filter(function (result) { return !result.granted; });
    var hasLicenseIssue = deniedResults.some(function (result) { return !result.license_valid; });
    var hasSpecialtyIssue = deniedResults.some(function (result) { return !result.specialty_match; });
    var hasCFMIssue = deniedResults.some(function (result) { return !result.cfm_compliant; });
    // Custom fallback provided
    if (fallback) {
        return (<div className={className} aria-label={ariaLabel || "Access denied"} aria-describedby={ariaDescribedby}>
        {fallback}
        {showDetails && (<PermissionDetails results={permissionResults} userRole={userRole} className="mt-4"/>)}
      </div>);
    }
    // Default permission denied UI
    return (<div className={(0, utils_1.cn)("max-w-md mx-auto", className)} aria-label={ariaLabel || "Access denied"} aria-describedby={ariaDescribedby}>
      <card_1.Card className="border-destructive/50">
        ...
        <card_1.CardContent className="space-y-4">
          ...
          {(context === null || context === void 0 ? void 0 : context.clinicId) && (<div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {deniedResults.map(function (result, index) { return (<badge_1.Badge key={index} variant="destructive" className="text-xs">
                  {result.permission}
                </badge_1.Badge>); })}
            </div>
          </div>
          ...
          {allowEmergencyOverride && hasEmergencyOverride && (<alert_1.Alert className="border-warning">
              ...
              <alert_1.AlertDescription>
                ...
                "Em caso de emergência médica, você pode solicitar acesso temporário."}
                  ...
                  <button_1.Button variant="outline" size="sm" onClick={function () { return handleEmergencyOverrideRequest(permissionArray[0]); }} className="w-full">
      ...
      <EmergencyOverrideDialog isOpen={showEmergencyDialog} onClose={function () { return setShowEmergencyDialog(false); }} onConfirm={handleEmergencyOverrideConfirm} permission={emergencyPermission} userRole={userRole}/>
    </div>);
}
// ============================================================================
// EMERGENCY OVERRIDE DIALOG
...
function EmergencyOverrideDialog(_a) {
    var _this = this;
    var isOpen = _a.isOpen, onClose = _a.onClose, onConfirm = _a.onConfirm, permission = _a.permission, userRole = _a.userRole;
      ...
      <card_1.Card className="w-full max-w-md">
        ...
        <card_1.CardContent className="space-y-4">
          <div>
            ...
            <textarea id="emergency-reason" value={reason} onChange={function (e) { return setReason(e.target.value); }} placeholder="Descreva a situação de emergência que justifica este acesso..." className="w-full min-h-[100px] p-3 border rounded-md resize-none focus:ring-2 focus:ring-primary focus:border-transparent" required/>
          </div>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button_1.Button variant="destructive" onClick={handleConfirm} disabled={!reason.trim() || isConfirming}>
...
function PermissionDetails(_a) {
    var results = _a.results, userRole = _a.userRole, className = _a.className;
    return (<card_1.Card className={(0, utils_1.cn)("border-muted", className)}>
      <card_1.CardHeader>
      ...
      <card_1.CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {results.map(function (result, index) { return (<div key={index} className="border rounded p-2 text-xs space-y-1">
              ...
              <div className="flex gap-2 flex-wrap">
                <badge_1.Badge variant={result.license_valid ? "success" : "destructive"} className="text-xs">
                  Licença: {result.license_valid ? "Válida" : "Inválida"}
                </badge_1.Badge>
                <badge_1.Badge variant={result.specialty_match ? "success" : "destructive"} className="text-xs">
                  Especialidade: {result.specialty_match ? "OK" : "Incompatível"}
                </badge_1.Badge>
                <badge_1.Badge variant={result.cfm_compliant ? "success" : "destructive"} className="text-xs">
                  CFM: {result.cfm_compliant ? "Conforme" : "Não conforme"}
                </badge_1.Badge>
              </div>

              {result.emergency_override && (<badge_1.Badge variant="warning" className="text-xs">
...
function ClinicalPermissionGuard(_a) {
    var children = _a.children, patientId = _a.patientId, _b = _a.allowEmergencyOverride, allowEmergencyOverride = _b === void 0 ? true : _b, props = __rest(_a, ["children", "patientId", "allowEmergencyOverride"]);
    return (<PermissionGuard permissions={props.permissions || ['patient.read.own', 'procedure.perform.general']} allowEmergencyOverride={allowEmergencyOverride} context={__assign({ patientId: patientId }, props.context)} errorMessages={{
...
function AdministrativePermissionGuard(_a) {
    ...
    return (<PermissionGuard permissions={props.permissions || ['scheduling.manage.clinic', 'billing.process.standard']} allowEmergencyOverride={false} errorMessages={{
...
function CompliancePermissionGuard(_a) {
    var children = _a.children, props = __rest(_a, ["children"]);
    return (<PermissionGuard permissions={props.permissions || ['audit.access.clinic', 'compliance.report.cfm']} allowEmergencyOverride={false} errorMessages={{
