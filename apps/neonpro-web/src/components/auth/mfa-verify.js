...
function MFAVerify(_a) {
    var _this = this;
    var userId = _a.userId, methods = _a.methods, onVerificationSuccess = _a.onVerificationSuccess, onVerificationError = _a.onVerificationError, _b = _a.allowEmergencyBypass, allowEmergencyBypass = _b === void 0 ? false : _b, _c = _a.showTrustedDeviceOption, showTrustedDeviceOption = _c === void 0 ? true : _c, className = _a.className, _d = _a.theme, theme = _d === void 0 ? 'light' : _d, _e = _a.locale, locale = _e === void 0 ? 'pt-BR' : _e;
    // Translations
    var t = useTranslations(locale);
    // MFA hook
    var _f = (0, use_mfa_1.useMFA)({ userId: userId }), mfaSettings = _f.mfaSettings, verifyMFA = _f.verifyMFA, sendSMSOTP = _f.sendSMSOTP, isLoading = _f.isLoading, error = _f.error;
    // Component state
    ...
    var handleMethodChange = (0, react_1.useCallback)(function (method) {
        setState(function (prev) { return (__assign(__assign({}, prev), { selectedMethod: method, token: '', showBackupCodeInput: method === 'backup' })); });
    }, []);
    /**
     * Handle token input change with validation
     */
    var handleTokenChange = (0, react_1.useCallback)(function (value) {
        ...
        setState(function (prev) { return (__assign(__assign({}, prev), { token: formattedValue })); });
    }, [state.selectedMethod]);
    /**
     * Send SMS OTP
     */
    var handleSendSMS = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        var err_1, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.sent();
                    // Start countdown
                    setState(function (prev) { return (__assign(__assign({}, prev), { smsCountdown: SMS_RESEND_COOLDOWN, canResendSMS: false })); });
     ...
     * Verify MFA token
     */
    var handleVerify = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    setState(function (prev) { return (__assign(__assign({}, prev), { isVerifying: true })); });
                    _d.label = 1;
                case 1:
                    _d.trys.push([1, 7, 8, 9]);
                    cleanToken = state.selectedMethod === 'backup'
                ...
                case 4:
                    _b = undefined;
                    _d.label = 5;
                case 5: return [4 /*yield*/, _a.apply(void 0, [(_c.deviceFingerprint = _b,
                            _c)])];
                case 6:
                    result_1 = _d.sent();
                    // Update remaining attempts
                    setState(function (prev) { return (__assign(__assign({}, prev), { remainingAttempts: result_1.remainingAttempts, lockedUntil: result_1.lockedUntil })); });
                    if (result_1.isValid) {
                        ...
                        setState(function (prev) { return (__assign(__assign({}, prev), { lockedUntil: result_1.lockedUntil })); });
                        (0, use_toast_1.toast)({
                            title: t.errors.accountLocked,
                            description: t.errors.accountLockedDescription.replace('{time}', result_1.lockedUntil.toLocaleTimeString(locale)),
                            variant: 'destructive',
                        });
                    }
                    else {
                        (0, use_toast_1.toast)({
                            ...
                            description: t.errors.invalidTokenDescription.replace('{attempts}', result_1.remainingAttempts.toString()),
                            variant: 'destructive',
                        });
                    }
                    return [3 /*break*/, 9];
                case 7:
                    err_2 = _d.sent();
                    error_2 = err_2 instanceof auth_1.MFAError ? err_2 : new Error(t.errors.verificationFailed);
                    ...
                    return [3 /*break*/, 9];
                case 8:
                    setState(function (prev) { return (__assign(__assign({}, prev), { isVerifying: false })); });
                    return [7 /*endfinally*/];
     ...
     */
    var handleEmergencyBypass = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, getUserIpAddress()];
                case 2: return [4 /*yield*/, _a.apply(void 0, [(_b.ipAddress = _c.sent(),
                ...
                case 4:
                    err_3 = _c.sent();
                    error_3 = err_3 instanceof auth_1.MFAError ? err_3 : new Error(t.errors.emergencyBypassFailed);
                    (0, use_toast_1.toast)({
     ...
     */
    var isValidTokenFormat = (0, react_1.useCallback)(function () {
        if (state.selectedMethod === 'backup') {
    ...
    return (<div className={(0, utils_1.cn)('w-full max-w-md mx-auto', className)}>
      {/* Header */}
      ...
      {isAccountLocked && (<alert_1.Alert variant="destructive" className="mb-6">
      ...
      {/* Error Display */}
      {error && !isAccountLocked && (<alert_1.Alert variant="destructive" className="mb-6">
          <lucide_react_1.AlertTriangle className="h-4 w-4"/>
          <alert_1.AlertDescription>{error.message}</alert_1.AlertDescription>
        </alert_1.Alert>)}

      {/* Remaining Attempts Warning */}
      {state.remainingAttempts <= 2 && state.remainingAttempts > 0 && !isAccountLocked && (<alert_1.Alert variant="destructive" className="mb-6">
      ...
      <card_1.Card>
        <card_1.CardContent className="p-6">
          {/* Method Selection */}
          {methods.length > 1 && (<div className="mb-6">
              <label_1.Label className="text-sm font-medium mb-3 block">{t.methodSelection}</label_1.Label>
              <tabs_1.Tabs value={state.selectedMethod} onValueChange={handleMethodChange}>
                <tabs_1.TabsList className="grid w-full grid-cols-2 lg:grid-cols-3">
                  {methods.includes('totp') && (<tabs_1.TabsTrigger value="totp" className="flex items-center gap-1 text-xs">
                      <lucide_react_1.Smartphone className="h-3 w-3"/>
                      {t.methods.totp}
                    </tabs_1.TabsTrigger>)}
                  {methods.includes('sms') && (<tabs_1.TabsTrigger value="sms" className="flex items-center gap-1 text-xs">
                      <lucide_react_1.Phone className="h-3 w-3"/>
                      {t.methods.sms}
                    </tabs_1.TabsTrigger>)}
                  {methods.includes('backup') && (<tabs_1.TabsTrigger value="backup" className="flex items-center gap-1 text-xs">
          ...
          <div className="space-y-4">
            {/* TOTP Method */}
            {state.selectedMethod === 'totp' && (<div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input_1.Input id="totp-token" type="text" inputMode="numeric" pattern="[0-9]*" placeholder="123456" value={state.token} onChange={function (e) { return handleTokenChange(e.target.value); }} className="text-center text-2xl font-mono tracking-widest" maxLength={6} disabled={isAccountLocked || state.isVerifying} autoFocus/>
                </div>
              </div>)}

            {/* SMS Method */}
            {state.selectedMethod === 'sms' && (<div className="space-y-4">
                ...
                <div className="flex justify-center">
                  <button_1.Button variant="outline" size="sm" onClick={handleSendSMS} disabled={!state.canResendSMS || isAccountLocked} className="flex items-center gap-2">
                    <lucide_react_1.RefreshCw className="h-4 w-4"/>
                    {state.canResendSMS ? t.buttons.resendSMS : "".concat(t.buttons.resendIn, " ").concat(state.smsCountdown, "s")}
                  </button_1.Button>
                </div>
                
                <div className="space-y-2">
                  <label_1.Label htmlFor="sms-token">{t.tokenLabel}</label_1.Label>
                  <input_1.Input id="sms-token" type="text" inputMode="numeric" pattern="[0-9]*" placeholder="123456" value={state.token} onChange={function (e) { return handleTokenChange(e.target.value); }} className="text-center text-2xl font-mono tracking-widest" maxLength={6} disabled={isAccountLocked || state.isVerifying}/>
                </div>
              </div>)}

            {/* Backup Code Method */}
            {state.selectedMethod === 'backup' && (<div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input_1.Input id="backup-token" type="text" placeholder="XXXX-XXXX-XX" value={state.token} onChange={function (e) { return handleTokenChange(e.target.value); }} className="text-center text-xl font-mono tracking-widest" disabled={isAccountLocked || state.isVerifying}/>
            ...
            {/* Trust Device Option */}
            {showTrustedDeviceOption && !isAccountLocked && (<div className="flex items-start space-x-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <checkbox_1.Checkbox id="trustDevice" checked={state.trustDevice} onCheckedChange={function (checked) { return setState(function (prev) { return (__assign(__assign({}, prev), { trustDevice: !!checked })); }); }}/>
            ...
            {/* Verify Button */}
            <button_1.Button onClick={handleVerify} disabled={!isValidTokenFormat() || state.isVerifying || isAccountLocked} className="w-full" size="lg">
              {state.isVerifying ? (<>
            ...
            {allowEmergencyBypass && !isAccountLocked && (<div className="mt-6 pt-6 border-t">
                {!state.showEmergencyBypass ? (<button_1.Button variant="ghost" size="sm" onClick={function () { return setState(function (prev) { return (__assign(__assign({}, prev), { showEmergencyBypass: true })); }); }} className="w-full text-red-600 hover:text-red-700 hover:bg-red-50">
                    ...
                    <div className="space-y-2">
                      <label_1.Label htmlFor="emergencyReason">{t.emergency.reasonLabel}</label_1.Label>
                      <input_1.Input id="emergencyReason" type="text" placeholder={t.emergency.reasonPlaceholder} value={state.emergencyReason} onChange={function (e) { return setState(function (prev) { return (__assign(__assign({}, prev), { emergencyReason: e.target.value })); }); }} maxLength={200}/>
                      <p className="text-xs text-gray-500">
                    ...
                    <div className="flex gap-2">
                      <button_1.Button variant="outline" size="sm" onClick={function () { return setState(function (prev) { return (__assign(__assign({}, prev), { showEmergencyBypass: false, emergencyReason: '' })); }); }} className="flex-1">
                        {t.buttons.cancel}
                      </button_1.Button>
                      <button_1.Button variant="destructive" size="sm" onClick={handleEmergencyBypass} disabled={!state.emergencyReason.trim() || state.isVerifying} className="flex-1">
                        <lucide_react_1.Zap className="h-4 w-4 mr-2"/>
                        {t.emergency.request}
                      </button_1.Button>
                    </div>
