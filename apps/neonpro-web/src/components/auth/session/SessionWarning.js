...
function SessionWarning(_a) {
  var className = _a.className,
    ...
    autoShow = _d === void 0 ? true : _d,
    _e = _a.showAsDialog,
    showAsDialog = _e === void 0 ? true : _e,
    _f = _a.showAsAlert,
    showAsAlert = _f === void 0 ? false : _f,
    onExtend = _a.onExtend,
    onLogout = _a.onLogout,
    onDismiss = _a.onDismiss;
  var _g = (0, auth_1.useSession)(),
    isAuthenticated = _g.isAuthenticated,
    isExpiringSoon = _g.isExpiringSoon,
    isExpired = _g.isExpired,
    extendSession = _g.extendSession,
    logout = _g.logout;
  var _h = (0, auth_1.useSessionTimeout)(),
    timeRemainingMinutes = _h.timeRemainingMinutes,
    timeRemainingFormatted = _h.timeRemainingFormatted;
      ...
      case "critical":
        return {
          ...
          description: "Your session will expire in "
            .concat(
              (timeRemainingFormatted === null || timeRemainingFormatted === void 0
                ? void 0
                : timeRemainingFormatted.minutes) || 0,
              " minutes and ",
            )
            .concat(
              (timeRemainingFormatted === null || timeRemainingFormatted === void 0
                ? void 0
                : timeRemainingFormatted.seconds) || 0,
              " seconds. Extend now to avoid losing your work.",
            ),
          variant: "destructive",
          icon: lucide_react_1.AlertTriangle,
          color: "text-red-600",
        ...
        return {
          ...
          description: "Your session will expire in ".concat(
            (timeRemainingFormatted === null || timeRemainingFormatted === void 0
              ? void 0
              : timeRemainingFormatted.minutes) || 0,
            " minutes. Consider extending your session to continue working.",
          ),
          variant: "default",
      ...
      <dialog_1.Dialog open={isVisible} onOpenChange={(open) => !open && handleDismiss()}>
        <dialog_1.DialogContent className={(0, utils_1.cn)("sm:max-w-md", className)}>
          ...
          <dialog_1.DialogFooter className="flex-col sm:flex-row gap-2">
            {config.showExtend && (
              <button_1.Button onClick={handleExtend} className="flex-1">
            ...
            {config.showLogout && (
              <button_1.Button variant="outline" onClick={handleLogout} className="flex-1">
            ...
            {warningLevel === "warning" && (
              <button_1.Button variant="ghost" onClick={handleDismiss} className="flex-1">
  ...
  if (showAsAlert) {
    return (
      <alert_1.Alert
        ...
        className={(0, utils_1.cn)("relative", config.bgColor, config.borderColor, className)}
      >
        ...
        <alert_1.AlertDescription className="mt-2">{config.description}</alert_1.AlertDescription>
        ...
        <div className="flex gap-2 mt-4">
          {config.showExtend && (
            <button_1.Button size="sm" onClick={handleExtend}>
              <lucide_react_1.RefreshCw className="h-4 w-4 mr-2" />
              Extend
            </button_1.Button>
          )}
          ...
          {config.showLogout && (
            <button_1.Button size="sm" variant="outline" onClick={handleLogout}>
              <lucide_react_1.LogOut className="h-4 w-4 mr-2" />
              Logout
            </button_1.Button>
          )}
          ...
          {warningLevel === "warning" && (
            <button_1.Button size="sm" variant="ghost" onClick={handleDismiss}>
              <lucide_react_1.X className="h-4 w-4 mr-2" />
              Dismiss
            </button_1.Button>
          )}
