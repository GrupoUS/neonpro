...
const SIGNATURE_STATUS = [
  { value: 'pending', label: 'Pendente', color: 'bg-yellow-100 text-yellow-800', icon: <Clock className="w-3 h-3" /> },
  { value: 'signed', label: 'Assinado', color: 'bg-green-100 text-green-800', icon: <CheckCircle className="w-3 h-3" /> },
  { value: 'rejected', label: 'Rejeitado', color: 'bg-red-100 text-red-800', icon: <X className="w-3 h-3" /> },
  { value: 'expired', label: 'Expirado', color: 'bg-gray-100 text-gray-800', icon: <AlertTriangle className="w-3 h-3" /> }
];
                ...
                <div className="border rounded-lg p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setNewSigner(prev => ({ ...prev, signatureType: value }))}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Tipo de assinatura" />
                        </SelectTrigger>
                        <SelectContent>
                          {SIGNATURE_TYPES.filter(type => allowedSignatureTypes.includes(type.value)).map(type => (
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setNewSigner(prev => ({ ...prev, isRequired: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <Button
                    type="button"
                    onClick={addSigner}
                    disabled={!newSigner.name || !newSigner.email || !newSigner.role || !newSigner.signatureType}
                    className="w-full"
                  >
                          ...
                          <div key={signer.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                ...
                                <Badge className={roleInfo.value === 'patient' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}>
                                  {roleInfo.label}
                                </Badge>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <Tabs value={selectedSignatureType} onValueChange={setSelectedSignatureType}>
                <TabsList className="grid w-full grid-cols-3">
                  {SIGNATURE_TYPES.filter(type => allowedSignatureTypes.includes(type.value)).slice(0, 3).map(type => (
                ...
                {SIGNATURE_TYPES.filter(type => allowedSignatureTypes.includes(type.value)).map(type => (
                  <TabsContent key={type.value} value={type.value} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => setSignatureData(prev => ({ ...prev, signerRole: value }))}
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => setSignatureData(prev => ({ ...prev, pinCode: e.target.value }))}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => setSignatureData(prev => ({ ...prev, otpCode: e.target.value }))}
                    ...
                    <div className="flex items-center justify-end space-x-4 pt-4">
                      ...
                      <Button
                        onClick={handleSign}
                        disabled={!signatureData.signerName || !signatureData.signerRole || isLoading}
                      >
                        {isLoading ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          <Badge className={signature.isValid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>{format(signature.timestamp, 'dd/MM/yyyy HH:mm', { locale: ptBR })}</span>
                          </div>
                          <div className="flex items-center space-x-1">
              ...
              {signatureRequests.map((request) => (
                <div key={request.id} className="border rounded-lg p-4">
                  ...
                  <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
                    <span>Criado em: {format(request.createdAt, 'dd/MM/yyyy HH:mm', { locale: ptBR })}</span>
                    {request.deadline && (
                      <span>Prazo: {format(request.deadline, 'dd/MM/yyyy HH:mm', { locale: ptBR })}</span>
                    )}
                  </div>
                </div>
              ))}
