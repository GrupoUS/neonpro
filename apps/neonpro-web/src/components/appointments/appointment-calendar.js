...
var formats = {
    ...
    dayRangeHeaderFormat: function (_a) {
        ...
        return "".concat((0, moment_1.default)(start).format('DD/MM'), " - ").concat((0, moment_1.default)(end).format('DD/MM/YYYY'));
    },
    ...
    agendaTimeRangeFormat: function (_a) {
        var start = _a.start, end = _a.end;
        return "".concat((0, moment_1.default)(start).format('HH:mm'), " - ").concat((0, moment_1.default)(end).format('HH:mm'));
    },
    timeGutterFormat: 'HH:mm',
    selectRangeFormat: function (_a) {
        var start = _a.start, end = _a.end;
        return "".concat((0, moment_1.default)(start).format('DD/MM HH:mm'), " - ").concat((0, moment_1.default)(end).format('DD/MM HH:mm'));
    },
};
function AppointmentCalendar(_a) {
    var appointments = _a.appointments, professionals = _a.professionals, view = _a.view, date = _a.date, onViewChange = _a.onViewChange, onDateChange = _a.onDateChange, onAppointmentDrop = _a.onAppointmentDrop, onAppointmentResize = _a.onAppointmentResize, onAppointmentSelect = _a.onAppointmentSelect, onSlotSelect = _a.onSlotSelect;
    // Convert appointments to calendar events
    var calendarEvents = (0, react_1.useMemo)(function () {
        return appointments.map(function (appointment) { return (__assign(__assign({}, appointment), { resource: appointment.professionalId })); });
    }, [appointments]);
    // Professional resources for resource view
    var resources = (0, react_1.useMemo)(function () {
        return professionals.map(function (prof) { return ({
            resourceId: prof.id,
    ...
    var eventStyleGetter = (0, react_1.useCallback)(function (event) {
        ...
        return {
            style: __assign({ backgroundColor: baseColor, borderColor: baseColor, color: 'white', border: '2px solid', borderRadius: '6px', fontSize: '12px', fontWeight: '500' }, statusStyle)
        };
    }, []);
    ...
    var handleEventDrop = (0, react_1.useCallback)(function (_a) {
        var event = _a.event, start = _a.start, end = _a.end;
        onAppointmentDrop(event, start, end);
    }, [onAppointmentDrop]);
    // Handle event resize
    var handleEventResize = (0, react_1.useCallback)(function (_a) {
        var event = _a.event, start = _a.start, end = _a.end;
    ...
    var handleSelectSlot = (0, react_1.useCallback)(function (slotInfo) {
    ...
    var components = (0, react_1.useMemo)(function () { return ({
        ...
        toolbar: function (props) { return (<div className="flex items-center justify-between mb-4 p-4 bg-muted/50 rounded-lg">
        <div className="flex items-center space-x-2">
          <button onClick={function () { return props.onNavigate('PREV'); }} className="px-3 py-1 text-sm bg-background hover:bg-muted rounded border">
            ← {messages.previous}
          </button>
          <button onClick={function () { return props.onNavigate('TODAY'); }} className="px-3 py-1 text-sm bg-primary text-primary-foreground hover:bg-primary/90 rounded">
            {messages.today}
          </button>
          <button onClick={function () { return props.onNavigate('NEXT'); }} className="px-3 py-1 text-sm bg-background hover:bg-muted rounded border">
            {messages.next} →
          </button>
        </div>
        ...
        <div className="flex items-center space-x-1">
          {[
                ...
                return (<button key={key} onClick={function () { return props.onView(key); }} className={(0, utils_1.cn)('px-3 py-1 text-sm rounded border', props.view === key
    ...
    return (<div className="h-[600px] w-full">
      ...
      <DnDCalendar localizer={localizer} events={calendarEvents} resources={view === 'day' ? resources : undefined} resourceIdAccessor="resourceId" resourceTitleAccessor="resourceTitle" startAccessor="start" endAccessor="end" titleAccessor="title" view={view} date={date} onView={function (newView) { return onViewChange(newView); }} onNavigate={onDateChange} onEventDrop={handleEventDrop} onEventResize={handleEventResize} onSelectEvent={handleSelectEvent} onSelectSlot={handleSelectSlot} selectable resizable draggableAccessor={function () { return true; }} eventPropGetter={eventStyleGetter} slotPropGetter={slotStyleGetter} dayPropGetter={dayPropGetter} messages={messages} formats={formats} components={components} min={new Date(2024, 0, 1, 8, 0)} // 8:00 AM
     max={new Date(2024, 0, 1, 18, 0)} // 6:00 PM
     step={15} // 15-minute intervals
     timeslots={4} // 4 slots per hour (15 min each)
     showMultiDayTimes popup popupOffset={{ x: 10, y: 10 }} className="rbc-calendar"/>
    </div>);
}
