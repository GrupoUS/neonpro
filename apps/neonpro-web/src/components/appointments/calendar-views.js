...
function CalendarViews(_a) {
    var currentView = _a.currentView, onViewChange = _a.onViewChange, currentDate = _a.currentDate, onDateChange = _a.onDateChange;
    // Navigation handlers
        ...
        switch (currentView) {
            ...
            case 'week':
                ...
                if (weekStart.month() === weekEnd.month()) {
                    return "".concat(weekStart.format('DD'), " - ").concat(weekEnd.format('DD'), " de ").concat(momentDate.format('MMMM YYYY'));
                }
                else {
    ...
    react_1.default.useEffect(function () {
        var handleKeyDown = function (event) {
            ...
            if (((_a = document.activeElement) === null || _a === void 0 ? void 0 : _a.tagName) === 'INPUT' ||
                ((_b = document.activeElement) === null || _b === void 0 ? void 0 : _b.tagName) === 'TEXTAREA') {
                return;
            }
            // View shortcuts
    ...
    return (<div className="flex items-center justify-between w-full">
      ...
      <div className="flex items-center space-x-2">
        <button_1.Button variant="outline" size="sm" onClick={handlePrevious} className="h-8 w-8 p-0" title={"Anterior (Shift + \u2190)"}>
          <lucide_react_1.ChevronLeft className="h-4 w-4"/>
        </button_1.Button>
        
        <button_1.Button variant="outline" size="sm" onClick={handleToday} className="h-8 px-3" title="Hoje (Home)">
          <lucide_react_1.RotateCcw className="h-3 w-3 mr-1"/>
          Hoje
        </button_1.Button>
        
        <button_1.Button variant="outline" size="sm" onClick={handleNext} className="h-8 w-8 p-0" title={"Pr\u00F3ximo (Shift + \u2192)"}>
      ...
      <div className="flex-1 text-center">
        ...
        {currentView === 'day' && (<p className="text-sm text-muted-foreground">
      ...
      <div className="flex items-center space-x-1 bg-muted rounded-lg p-1">
        {viewConfig.map(function (_a) {
            var key = _a.key, label = _a.label, Icon = _a.icon, shortcut = _a.shortcut;
            return (<button_1.Button key={key} variant={currentView === key ? 'secondary' : 'ghost'} size="sm" onClick={function () { return onViewChange(key); }} className={(0, utils_1.cn)('h-8 px-3 text-xs font-medium transition-all', currentView === key
                    ? 'bg-background shadow-sm'
                    : 'hover:bg-background/50')} title={"".concat(label, " (Ctrl/Cmd + ").concat(shortcut, ")")}>
...
function MobileCalendarViews(_a) {
    var currentView = _a.currentView, onViewChange = _a.onViewChange, currentDate = _a.currentDate, onDateChange = _a.onDateChange;
    var handlePrevious = function () {
        var unit = currentView === 'month' ? 'month' :
    ...
    var getShortDateLabel = function () {
        ...
        switch (currentView) {
            ...
            case 'week':
                return "".concat(momentDate.startOf('week').format('DD/MM'), " - ").concat(momentDate.endOf('week').format('DD/MM'));
            case 'day':
    ...
    return (<div className="flex flex-col space-y-3 md:hidden">
      {/* Date Navigation */}
      ...
      <div className="grid grid-cols-4 gap-1 bg-muted rounded-lg p-1">
        {viewConfig.map(function (_a) {
            ...
            return (<button_1.Button key={key} variant={currentView === key ? 'secondary' : 'ghost'} size="sm" onClick={function () { return onViewChange(key); }} className={(0, utils_1.cn)('h-8 text-xs', currentView === key && 'bg-background shadow-sm')}>
            <Icon className="h-3 w-3 mr-1"/>
            {label}
          </button_1.Button>);
        })}
