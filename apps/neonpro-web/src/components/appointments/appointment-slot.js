...
function AppointmentSlot(_a) {
  ...
  // Calculate duration
  var duration = (0, moment_1.default)(event.end).diff(
    (0, moment_1.default)(event.start),
    "minutes",
  );
  var durationText =
    duration >= 60
      ? ""
          .concat(Math.floor(duration / 60), "h")
          .concat(duration % 60 > 0 ? " ".concat(duration % 60, "min") : "")
      : "".concat(duration, "min");
  // Format time range
  var timeRange = ""
    .concat((0, moment_1.default)(event.start).format("HH:mm"), " - ")
    .concat((0, moment_1.default)(event.end).format("HH:mm"));
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={"Consulta de "
        .concat(event.patientName, " - ")
        .concat(serviceInfo.label, " - ")
        .concat(statusInfo.label)}
    >
          ...
          <div className="flex space-x-1">
            ...
            {event.equipmentNeeded.length > 2 && (
              <div
                ...
                title={"+".concat(event.equipmentNeeded.length - 2, " equipamentos")}
              >
                +{event.equipmentNeeded.length - 2}
              </div>
            )}
        ...
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            <div className="flex items-center space-x-2 mt-1">
              ...
              <badge_1.Badge
                variant="outline"
                className={(0, utils_1.cn)(statusInfo.color, "border-current")}
              >
          ...
          <div className="flex items-center space-x-2">
            ...
            {event.whatsappReminder && (
              <lucide_react_1.MessageCircle
                className="h-4 w-4 text-green-600"
                title="Lembrete WhatsApp ativo"
              />
            )}
          </div>
        </div>
