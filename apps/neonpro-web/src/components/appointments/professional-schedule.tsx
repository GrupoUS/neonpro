...
export function ProfessionalSchedule({
  ...
  const updateWorkingDay = (dayIndex: number, updates: Partial<WorkingDay>) => {
    setWorkingDays((prev) =>
      prev.map((day, index) => (index === dayIndex ? { ...day, ...updates } : day)),
    );
  };
  ...
  const toggleAvailability = (professionalId: string) => {
    onProfessionalsUpdate(
      professionals.map((p) =>
        p.id === professionalId ? { ...p, availability: !p.availability } : p,
      ),
    );
  };
                    ...
                    {workingDays.map((day, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <Label
                            className={cn("font-medium", !day.enabled && "text-muted-foreground")}
                          >
                            {day.dayName}
                          </Label>
                        </div>
                      ...
                      .map((day, index) => (
                        <div key={day.day} className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-4 pl-4">
                            ...
                            <div className="space-y-2">
                              <Label className="text-sm text-muted-foreground">Fim do Almoço</Label>
                              <Input
                                type="time"
                          ...
                          {index < workingDays.filter((d) => d.enabled).length - 1 && <Separator />}
                        </div>
                      ))}
                  </CardContent>
                </Card>
                    ...
                    {absences.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {absences.map((absence) => {
                          ...
                          const duration =
                            moment(absence.endDate).diff(moment(absence.startDate), "days") + 1;
