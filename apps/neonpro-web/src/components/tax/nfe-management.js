...
function NFEManagement(_a) {
    ...
    var _g = (0, react_1.useState)({
        limit: 20,
        offset: 0
    }), pagination = _g[0], setPagination = _g[1];
    ...
    var handleFilterChange = function (field, value) {
        ...
        setPagination(function (prev) { return (__assign(__assign({}, prev), { offset: 0 })); }); // Reset to first page
    };
    var handleAuthorizeNFE = function (nfeId) { return __awaiter(_this, void 0, void 0, function () {
        var response, errorData, err_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 5, , 6]);
                    return [4 /*yield*/, fetch("/api/tax/nfe/".concat(nfeId, "/authorize"), {
                            method: 'POST'
                        })];
                case 1:
                    response = _a.sent();
    ...
    var handleCancelNFE = function (nfeId) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.trys.push([1, 6, , 7]);
                    return [4 /*yield*/, fetch("/api/tax/nfe/".concat(nfeId, "/cancel"), {
                            method: 'POST',
                            headers: {
    ...
    var getStatusBadge = function (status) {
        var config = {
            draft: { color: 'bg-yellow-100 text-yellow-800', icon: lucide_react_1.Clock, label: 'Rascunho' },
            authorized: { color: 'bg-green-100 text-green-800', icon: lucide_react_1.CheckCircle, label: 'Autorizada' },
            cancelled: { color: 'bg-red-100 text-red-800', icon: lucide_react_1.XCircle, label: 'Cancelada' },
            rejected: { color: 'bg-red-100 text-red-800', icon: lucide_react_1.AlertTriangle, label: 'Rejeitada' }
        };
        var _a = config[status] || config.draft, color = _a.color, Icon = _a.icon, label = _a.label;
        return (<badge_1.Badge className={color}>
    ...
    return (<div className="space-y-6">
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <select_1.Select onValueChange={function (value) { return handleFilterChange('status', value); }}>
            ...
            <div>
              ...
              <input_1.Input placeholder="Nome do cliente" value={filters.customerName} onChange={function (e) { return handleFilterChange('customerName', e.target.value); }}/>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Data Início</label>
              <input_1.Input type="date" value={filters.startDate} onChange={function (e) { return handleFilterChange('startDate', e.target.value); }}/>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Data Fim</label>
              <input_1.Input type="date" value={filters.endDate} onChange={function (e) { return handleFilterChange('endDate', e.target.value); }}/>
            </div>
          </div>
        </card_1.CardContent>
      </card_1.Card>

      {error && (<card_1.Card className="border-red-200">
      ...
      <card_1.Card>
        <card_1.CardContent className="p-0">
          <table_1.Table>
            ...
            <table_1.TableBody>
              {documents.length === 0 ? (<table_1.TableRow>
                  <table_1.TableCell colSpan={6} className="text-center py-8 text-gray-500">
                    Nenhuma NFe encontrada
                  </table_1.TableCell>
                </table_1.TableRow>) : (documents.map(function (doc) { return (<table_1.TableRow key={doc.id}>
                    ...
                    <table_1.TableCell>{(0, utils_1.formatCurrency)(doc.valor_total)}</table_1.TableCell>
                    ...
                    <table_1.TableCell className="text-right">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        {doc.status === 'draft' && (<button_1.Button variant="ghost" size="sm" onClick={function () { return handleAuthorizeNFE(doc.id); }}>
                            <lucide_react_1.CheckCircle className="h-4 w-4"/>
                          </button_1.Button>)}
                        {doc.status === 'authorized' && (<button_1.Button variant="ghost" size="sm" onClick={function () { return handleCancelNFE(doc.id); }}>
      ...
      {/* Pagination */}
      {documents.length > 0 && (<div className="flex items-center justify-between">
          <div className="text-sm text-gray-500">
            Mostrando {pagination.offset + 1} a {Math.min(pagination.offset + pagination.limit, total)} de {total} registros
          </div>
          <div className="flex space-x-2">
            <button_1.Button variant="outline" size="sm" disabled={!hasPrevPage} onClick={function () { return setPagination(function (prev) { return (__assign(__assign({}, prev), { offset: Math.max(0, prev.offset - prev.limit) })); }); }}>
              Anterior
            </button_1.Button>
            <button_1.Button variant="outline" size="sm" disabled={!hasNextPage} onClick={function () { return setPagination(function (prev) { return (__assign(__assign({}, prev), { offset: prev.offset + prev.limit })); }); }}>
              Próximo
            </button_1.Button>
          </div>
        </div>)}
