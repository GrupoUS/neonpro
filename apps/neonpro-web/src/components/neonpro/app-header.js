...
var AppHeader = function (_a) {
    var onMenuToggle = _a.onMenuToggle, className = _a.className;
    ...
    return (<header className={(0, utils_1.cn)('sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700', 'backdrop-blur-sm bg-white/95 dark:bg-slate-900/95', className)}>
      <div className="flex items-center justify-between px-4 py-3">
        {/* Left Section */}
        <div className="flex items-center gap-4">
          {/* Mobile Menu Button */}
          <button_1.Button variant="ghost" size="sm" onClick={onMenuToggle} className="lg:hidden p-2">
            <lucide_react_1.Menu className="w-5 h-5"/>
          </button_1.Button>

          {/* Breadcrumbs */}
          <nav className="hidden sm:flex items-center space-x-2 text-sm">
            {breadcrumbs.map(function (item, index) { return (<react_1.default.Fragment key={index}>
                {index > 0 && (<lucide_react_1.ChevronRight className="w-4 h-4 text-slate-400"/>)}
                {item.href && !item.isActive ? (<link_1.default href={item.href} className="text-slate-600 hover:text-neon-primary dark:text-slate-400 dark:hover:text-neon-accent transition-colors">
                    {item.label}
                  </link_1.default>) : (<span className={(0, utils_1.cn)('font-medium', item.isActive
