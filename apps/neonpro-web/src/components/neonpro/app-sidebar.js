...
var AppSidebar = function (_a) {
    var _b = _a.isOpen, isOpen = _b === void 0 ? true : _b, onToggle = _a.onToggle, className = _a.className;
    ...
    return (<>
      ...
      {/* Sidebar */}
      <aside className={(0, utils_1.cn)('fixed left-0 top-0 h-full w-64 z-50', 'neon-sidebar transform transition-transform duration-300 ease-in-out', 'lg:translate-x-0 lg:static lg:z-auto', {
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          ...
          {/* Mobile Close Button */}
          <button onClick={onToggle} className="lg:hidden p-1 rounded-md hover:bg-slate-800 text-slate-400 hover:text-white transition-colors">
        ...
        <nav className="flex-1 p-4">
          <ul className="space-y-2">
            ...
            return (<li key={item.id}>
                  <link_1.default href={item.href} className={(0, utils_1.cn)('neon-sidebar-item group relative', {
                    'neon-sidebar-item-active': isActive,
                })}>
                    <Icon className="w-5 h-5 flex-shrink-0"/>
                    <span className="font-medium">{item.label}</span>
                    
                    {item.badge && (<span className="ml-auto px-2 py-0.5 bg-neon-accent text-neon-primary text-xs font-medium rounded-full">
                        {item.badge}
                      </span>)}
                    
                    {/* Active Indicator */}
                    {isActive && (<div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-neon-accent rounded-r-full"/>)}
                  </link_1.default>
                </li>);
        })}
          </ul>
