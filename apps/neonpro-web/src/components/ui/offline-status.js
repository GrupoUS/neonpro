...
function OfflineStatus(_a) {
  var className = _a.className,
    _b = _a.variant,
    variant = _b === void 0 ? "badge" : _b,
    _c = _a.showSyncQueue,
    showSyncQueue = _c === void 0 ? true : _c;
  var _d = (0, useNetworkStatus_1.useNetworkStatus)(),
    isOnline = _d.isOnline,
    isOffline = _d.isOffline,
    syncQueueCount = _d.syncQueueCount,
    isSyncing = _d.isSyncing,
    processSyncQueue = _d.processSyncQueue,
    clearSyncQueue = _d.clearSyncQueue;
  ...
  if (variant === "minimal") {
    return (
      <div className={(0, utils_1.cn)("flex items-center gap-1", className)}>
        {isOnline
          ? <lucide_react_1.Wifi className="w-4 h-4 text-green-600" />
          : <lucide_react_1.WifiOff className="w-4 h-4 text-red-600" />}
        {syncQueueCount > 0 && (
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          {isOffline
            ? <lucide_react_1.WifiOff className="w-4 h-4 text-destructive" />
              ...
              : syncQueueCount > 0
                ? <>
                    ...
                    <p className="text-sm text-blue-700">
                      {isSyncing
                        ? "Enviando..."
                        : "".concat(syncQueueCount, " a\u00E7\u00F5es pendentes")}
                    </p>
                  </>
                : null}
          </div>
        ...
        <card_1.Card className="w-80">
          ...
          <card_1.CardContent className="pt-0">
            <card_1.CardDescription>
              {isOnline
                ? syncQueueCount > 0
                  ? "Sincronizando ".concat(syncQueueCount, " a\u00E7\u00F5es pendentes...")
...
function SyncQueueDetails(_a) {
  var className = _a.className;
  var _b = (0, useNetworkStatus_1.useNetworkStatus)(),
    syncQueue = _b.syncQueue,
    isSyncing = _b.isSyncing,
    processSyncQueue = _b.processSyncQueue,
    clearSyncQueue = _b.clearSyncQueue;
  if (syncQueue.length === 0) {
    return (
      <div className={(0, utils_1.cn)("text-center p-4", className)}>
