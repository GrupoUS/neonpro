...
export const OptimizedTableRow = memo<OptimizedTableRowProps>(
  ({ id, data, columns, onEdit, onDelete, selected, onSelect }) => {
    ...
    return (
      <tr className={cn("hover:bg-muted/50 transition-colors", selected && "bg-muted")}>
        {onSelect && (
...
export const OptimizedListItem = memo<OptimizedListItemProps>(
  ({ id, title, subtitle, avatar, status, actions, onClick, className }) => {
