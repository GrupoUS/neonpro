...
function ComplianceAlertsSystem(_a) {
    ...
    var _e = (0, react_1.useState)('all'), selectedSeverity = _e[0], setSelectedSeverity = _e[1];
    var _f = (0, react_1.useState)('all'), selectedType = _f[0], setSelectedType = _f[1];
    var _g = (0, react_1.useState)(false), showResolved = _g[0], setShowResolved = _g[1];
    ...
    (0, react_1.useEffect)(function () {
        ...
        var channel = supabase
            ...
            setAlerts(function (prev) { return __spreadArray([newAlert], prev, true); });
    ...
    var markAsRead = function (alertId) { return __awaiter(_this, void 0, void 0, function () {
        var error, error_2;
                    ...
                    setAlerts(function (prev) {
                        return prev.map(function (alert) {
                            return alert.id === alertId
                                ? __assign(__assign({}, alert), { is_read: true }) : alert;
                        });
                    });
                    return [3 /*break*/, 3];
                case 2:
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    setAlerts(function (prev) { return prev.filter(function (alert) { return alert.id !== alertId; }); });
                    toast({
    ...
    var deleteAlert = function (alertId) { return __awaiter(_this, void 0, void 0, function () {
        var error, error_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    var getSeverityBadge = function (severity) {
        ...
        return (<badge_1.Badge variant={variants[severity]} className={colors[severity]}>
    ...
    var unreadCount = alerts.filter(function (alert) { return !alert.is_read; }).length;
    var criticalCount = alerts.filter(function (alert) { return alert.severity === 'critical' && !alert.resolved_at; }).length;
    if (loading) {
        return (<card_1.Card>
        <card_1.CardHeader>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <card_1.Card>
          <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <lucide_react_1.AlertTriangle className={"h-4 w-4 ".concat(criticalCount > 0 ? 'text-red-500' : 'text-green-500')}/>
          </card_1.CardHeader>
          <card_1.CardContent>
        ...
        <card_1.CardContent>
          <div className="flex gap-4 items-end mb-4">
            ...
            <div className="flex-1">
              ...
              <select_1.Select value={selectedType} onValueChange={setSelectedType}>
                ...
                <select_1.SelectContent>
                  ...
                  <select_1.SelectItem value="consent_expiring">Consent Expiring</select_1.SelectItem>
                  ...
                  <select_1.SelectItem value="regulatory_deadline">Regulatory Deadline</select_1.SelectItem>
            ...
            <div className="flex items-center space-x-2">
              <switch_1.Switch id="show-resolved" checked={showResolved} onCheckedChange={setShowResolved}/>
                ...
                <div className="space-y-4">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <switch_1.Switch id="email-notifications" checked={settings.email_enabled} onCheckedChange={function (checked) {
            return saveSettings(__assign(__assign({}, settings), { email_enabled: checked }));
        }}/>
                    </div>
                    <div className="flex items-center justify-between">
                      <label_1.Label htmlFor="push-notifications">Push Notifications</label_1.Label>
                      <switch_1.Switch id="push-notifications" checked={settings.push_enabled} onCheckedChange={function (checked) {
            return saveSettings(__assign(__assign({}, settings), { push_enabled: checked }));
        }}/>
                    </div>
                    <div className="flex items-center justify-between">
                      <label_1.Label htmlFor="sms-notifications">SMS Notifications</label_1.Label>
                      <switch_1.Switch id="sms-notifications" checked={settings.sms_enabled} onCheckedChange={function (checked) {
            return saveSettings(__assign(__assign({}, settings), { sms_enabled: checked }));
        }}/>
                    </div>
                    <div className="flex items-center justify-between">
                      <label_1.Label htmlFor="real-time">Real-time Updates</label_1.Label>
                      <switch_1.Switch id="real-time" checked={settings.real_time_enabled} onCheckedChange={function (checked) {
            return saveSettings(__assign(__assign({}, settings), { real_time_enabled: checked }));
        }}/>
                    </div>
                  </div>
                  <div>
                    <label_1.Label htmlFor="severity-threshold">Minimum Severity</label_1.Label>
                    <select_1.Select value={settings.severity_threshold} onValueChange={function (value) {
            return saveSettings(__assign(__assign({}, settings), { severity_threshold: value }));
        }}>
                      <select_1.SelectTrigger>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {alerts.length === 0 ? (<alert_1.Alert>
              ...
              <table_1.TableBody>
                {alerts.map(function (alert) { return (<table_1.TableRow key={alert.id} className={!alert.is_read ? 'bg-blue-50 dark:bg-blue-950/20' : ''}>
                    <table_1.TableCell>
                    ...
                    <table_1.TableCell>
                      <div className="flex items-center gap-2">
                        {!alert.is_read && (<badge_1.Badge variant="default" className="text-xs">
                            Unread
                          </badge_1.Badge>)}
                        {alert.resolved_at && (<badge_1.Badge variant="outline" className="text-xs">
                            Resolved
                          </badge_1.Badge>)}
                      </div>
                    </table_1.TableCell>
                    <table_1.TableCell>
                      <div className="flex items-center gap-2">
                        {!alert.is_read && (<button_1.Button variant="ghost" size="sm" onClick={function () { return markAsRead(alert.id); }}>
                            <lucide_react_1.Eye className="h-4 w-4"/>
                          </button_1.Button>)}
                        {!alert.resolved_at && (<button_1.Button variant="ghost" size="sm" onClick={function () { return resolveAlert(alert.id); }}>
                            <lucide_react_1.CheckCircle className="h-4 w-4"/>
                          </button_1.Button>)}
                        <button_1.Button variant="ghost" size="sm" onClick={function () { return deleteAlert(alert.id); }} className="text-red-600 hover:text-red-700">
