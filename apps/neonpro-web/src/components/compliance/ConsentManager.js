...
function ConsentManager(_a) {
    var _this = this;
    var clinicId = _a.clinicId, userId = _a.userId, _b = _a.className, className = _b === void 0 ? '' : _b, onConsentChange = _a.onConsentChange;
    var _c = (0, useLGPD_1.useLGPD)({
        clinicId: clinicId,
        onConsentChange: onConsentChange
    }), consents = _c.consents, activeConsents = _c.activeConsents, loading = _c.loading, error = _c.error, grantConsent = _c.grantConsent, withdrawConsent = _c.withdrawConsent, hasValidConsent = _c.hasValidConsent, getConsentByType = _c.getConsentByType, clearError = _c.clearError;
    var _d = (0, react_1.useState)(null), selectedConsent = _d[0], setSelectedConsent = _d[1];
    var _e = (0, react_1.useState)(false), isDetailsOpen = _e[0], setIsDetailsOpen = _e[1];
    // ============================================================================
    // HANDLERS
    // ============================================================================
    var handleConsentToggle = function (definition, granted) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, grantConsent(definition.type, {
    ...
    var renderConsentItem = function (definition) {
        ...
        var isExpired = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.expiresAt) &&
            new Date(existingConsent.expiresAt) < new Date();
        return (<card_1.Card key={definition.type} className="mb-4">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                {definition.required && (<badge_1.Badge variant="destructive" className="text-xs">
                    Obrigatório
                  </badge_1.Badge>)}
              </div>
            ...
            <div className="flex items-center gap-2 ml-4">
              {existingConsent && (<button_1.Button variant="ghost" size="sm" onClick={function () { return handleViewDetails(existingConsent); }}>
                  <lucide_react_1.Eye className="h-4 w-4"/>
                </button_1.Button>)}
              
              <switch_1.Switch checked={isGranted && !isExpired} onCheckedChange={function (checked) { return handleConsentToggle(definition, checked); }} disabled={loading || (definition.required && isGranted)}/>
            </div>
          </div>
        </card_1.CardContent>
      </card_1.Card>);
    ...
    var renderConsentDetails = function () {
        if (!selectedConsent)
            return null;
        var definition = CONSENT_DEFINITIONS.find(function (d) { return d.type === selectedConsent.consentType; });
        return (<dialog_1.Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p>{(definition === null || definition === void 0 ? void 0 : definition.title) || selectedConsent.consentType}</p>
                </div>
                <div>
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      <card_1.Card>
        ...
        <card_1.CardContent>
          {error && (<alert_1.Alert className="mb-4">
              <lucide_react_1.AlertTriangle className="h-4 w-4"/>
              <alert_1.AlertDescription>
                {error}
                <button_1.Button variant="link" size="sm" onClick={clearError} className="ml-2 p-0 h-auto">
                  Dispensar
                </button_1.Button>
              </alert_1.AlertDescription>
            </alert_1.Alert>)}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
              <div className="text-center p-2 bg-green-50 rounded">
                <div className="font-medium text-green-700">
                  {consents.filter(function (c) { return c.status === lgpd_1.ConsentStatus.GRANTED; }).length}
                </div>
                <div className="text-green-600">Concedidos</div>
              </div>
              <div className="text-center p-2 bg-red-50 rounded">
                <div className="font-medium text-red-700">
                  {consents.filter(function (c) { return c.status === lgpd_1.ConsentStatus.WITHDRAWN; }).length}
                </div>
                <div className="text-red-600">Revogados</div>
              </div>
              <div className="text-center p-2 bg-gray-50 rounded">
                <div className="font-medium text-gray-700">
                  {consents.filter(function (c) { return c.status === lgpd_1.ConsentStatus.EXPIRED; }).length}
                </div>
                <div className="text-gray-600">Expirados</div>
              </div>
              <div className="text-center p-2 bg-yellow-50 rounded">
                <div className="font-medium text-yellow-700">
                  {consents.filter(function (c) { return c.status === lgpd_1.ConsentStatus.PENDING; }).length}
                </div>
          ...
          <scroll_area_1.ScrollArea className="h-96">
            <div className="space-y-2">
              {CONSENT_DEFINITIONS.map(renderConsentItem)}
            </div>
