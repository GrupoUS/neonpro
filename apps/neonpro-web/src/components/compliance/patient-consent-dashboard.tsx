            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {services.filter(s => s.essential).map((service) => (
                    <div key={service.service} className="flex items-center justify-between">
                      <span className="text-sm">{service.service}</span>
                      <Badge variant={service.consentStatus === 'granted' ? 'default' : 'destructive'}>
                        {service.consentStatus === 'granted' ? <CheckCircle className="h-3 w-3" /> : <Lock className="h-3 w-3" />}
                      </Badge>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Badge variant={isGranted ? 'default' : 'outline'}>
              {isGranted ? <CheckCircle className="mr-1 h-3 w-3" /> : <Lock className="mr-1 h-3 w-3" />}
...
function ConsentHistoryItem({ activity }: { activity: any }) {
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'withdrawn': return <Lock className="h-4 w-4 text-red-500" />;
...
// Helper Functions
function calculateOverallStatus(services: ServiceConsent[]): 'fully_consented' | 'partially_consented' | 'missing_consent' {
