...
function CustomerManagement(_a) {
    var _b = _a.customers, customers = _b === void 0 ? [] : _b, _c = _a.appointments, appointments = _c === void 0 ? [] : _c, onCustomerSelect = _a.onCustomerSelect, onFollowUpSchedule = _a.onFollowUpSchedule, _d = _a.className, className = _d === void 0 ? '' : _d;
    ...
    var _h = (0, react_1.useState)('all'), filterLifecycle = _h[0], setFilterLifecycle = _h[1];
    // Filter and search customers
    var filteredCustomers = (0, react_1.useMemo)(function () {
        return customers.filter(function (customer) {
            var matchesSearch = customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    var customerAnalytics = (0, react_1.useMemo)(function () {
        var totalCustomers = customers.length;
        var activeCustomers = customers.filter(function (c) { return c.status === 'active'; }).length;
        var averageLifetimeValue = customers.reduce(function (sum, c) { return sum + c.totalSpent; }, 0) / totalCustomers;
        var averageSatisfaction = customers
            .filter(function (c) { return c.satisfactionRating; })
            .reduce(function (sum, c) { return sum + (c.satisfactionRating || 0); }, 0) /
            customers.filter(function (c) { return c.satisfactionRating; }).length;
        return {
    ...
    var getCustomerAppointments = function (customerId) {
        return appointments.filter(function (apt) { return apt.customerId === customerId; });
    };
    ...
    var handleScheduleFollowUp = function (customer, context) {
        ...
        onFollowUpSchedule === null || onFollowUpSchedule === void 0 ? void 0 : onFollowUpSchedule(customer.id, followUpDate, message);
    };
    ...
    var renderCustomerCard = function (customer) {
        ...
        return (<card_1.Card key={customer.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={function () { return handleCustomerSelect(customer); }}>
        <card_1.CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <avatar_1.Avatar>
                <avatar_1.AvatarFallback>{customer.name.split(' ').map(function (n) { return n[0]; }).join('')}</avatar_1.AvatarFallback>
              </avatar_1.Avatar>
              <div>
        ...
        <card_1.CardContent className="pt-0">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="flex items-center space-x-2">
              {churnRisk > 60 ? (<lucide_react_1.AlertTriangle className="h-4 w-4 text-red-600"/>) : (<lucide_react_1.Heart className="h-4 w-4 text-pink-600"/>)}
              <span>{churnRisk.toFixed(0)}% risk</span>
            </div>
          </div>
          
          {daysSinceLastVisit !== null && (<div className="mt-3 text-xs text-gray-500">
              Last visit: {daysSinceLastVisit} days ago
            </div>)}
          
          {customer.satisfactionRating && (<div className="mt-2 flex items-center space-x-1">
              {[1, 2, 3, 4, 5].map(function (star) { return (<lucide_react_1.Star key={star} className={"h-3 w-3 ".concat(star <= customer.satisfactionRating
    ...
    var renderCustomerDetails = function (customer) {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button_1.Button variant="outline" size="sm" onClick={function () { return handleScheduleFollowUp(customer, 'appointment'); }}>
              <lucide_react_1.MessageSquare className="h-4 w-4 mr-2"/>
              Schedule Follow-up
            </button_1.Button>
            <button_1.Button variant="outline" size="sm" onClick={function () { return handleScheduleFollowUp(customer, 'satisfaction'); }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <p className="text-xs text-gray-500">
                ...
                ? "Last visit: ".concat((0, utils_1.calculateDaysSinceLastVisit)(customer.lastVisitDate), " days ago")
                : 'No visits yet'}
              </p>
            </card_1.CardContent>
          </card_1.Card>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <progress_1.Progress value={churnRisk} className="mt-2" 
        // @ts-ignore
        style={{
          ...
          <tabs_1.TabsContent value="details" className="space-y-4">
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-3">
                ...
                {customer.tags.length > 0 && (<div>
                    <label_1.Label>Tags</label_1.Label>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {customer.tags.map(function (tag, index) { return (<badge_1.Badge key={index} variant="secondary">{tag}</badge_1.Badge>); })}
                    </div>
                  </div>)}
                {customer.notes && (<div>
          ...
          <tabs_1.TabsContent value="scoring" className="space-y-4">
            <card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Score factors contributing to overall lead quality</card_1.CardDescription>
              </card_1.CardHeader>
          ...
          <tabs_1.TabsContent value="appointments" className="space-y-4">
            <card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>{customerAppointments.length} total appointments</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2">
                  {customerAppointments.length === 0 ? (<p className="text-gray-500 text-center py-4">No appointments found</p>) : (customerAppointments.slice(0, 5).map(function (appointment) { return (<div key={appointment.id} className="flex items-center justify-between p-3 border rounded">
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <badge_1.Badge variant={appointment.status === 'completed' ? 'default' :
    ...
    return (<div className={"space-y-6 ".concat(className)}>
        ...
        <card_1.CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <input_1.Input id="search" placeholder="Search by name, email, or phone..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }}/>
            </div>
      ...
      {selectedCustomer ? (<div>
          <button_1.Button variant="outline" onClick={function () { return setSelectedCustomer(null); }} className="mb-4">
          ...
          {filteredCustomers.length === 0 && (<card_1.Card>
