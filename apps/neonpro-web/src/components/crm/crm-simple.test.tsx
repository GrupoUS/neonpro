...
describe("CRM Lifecycle Functions", () => {
  ...
  test("calculateDaysSinceLastVisit should throw error for invalid date", () => {
    ...
    expect(() => calculateDaysSinceLastVisit("invalid-date")).toThrow(
...
describe("CRM Follow-up Functions", () => {
  ...
  test("determineNextFollowUpDate should calculate correct dates for different customer types", () => {
    ...
    expect(new Date(newCustomerFollowUp).getTime()).toBeLessThan(
      new Date(atRiskCustomerFollowUp).getTime(),
    );
    expect(new Date(atRiskCustomerFollowUp).getTime()).toBeLessThan(
      new Date(activeCustomerFollowUp).getTime(),
    );
    expect(new Date(activeCustomerFollowUp).getTime()).toBeLessThan(
  ...
  test("determineNextFollowUpDate should throw error for invalid date", () => {
    ...
    expect(() => determineNextFollowUpDate("invalid-date", "active")).toThrow(
  ...
  test("generateFollowUpMessage should throw error for invalid customer", () => {
    // @ts-ignore - Testing invalid input
    expect(() => generateFollowUpMessage(null, "appointment")).toThrow(
      "Customer data with name is required",
    );
    expect(() => generateFollowUpMessage({} as Customer, "appointment")).toThrow(
      "Customer data with name is required",
    );
  });
});
  ...
  test("calculateRetentionRate should calculate retention metrics", () => {
    const customers: Customer[] = [
      { ...mockCustomer, id: "1", registrationDate: PAST_DATE, lastVisitDate: RECENT_DATE },
      {
        ...
        id: "2",
        registrationDate: PAST_DATE,
        lastVisitDate: "2024-12-01T12:00:00.000Z",
      },
      {
        ...mockCustomer,
        id: "3",
        registrationDate: PAST_DATE,
        lastVisitDate: "2024-09-01T12:00:00.000Z",
      },
    ];
