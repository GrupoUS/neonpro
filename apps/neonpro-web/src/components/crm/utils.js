...
function calculateCustomerLifetimeValue(appointments) {
    ...
    var completedAppointments = appointments.filter(function (apt) { return apt.status === 'completed'; });
    if (completedAppointments.length === 0) {
        return 0;
    }
    // Sum all completed appointment costs
    var totalValue = completedAppointments.reduce(function (sum, apt) { return sum + (apt.cost || 0); }, 0);
...
function rankCustomersByValue(customers) {
    ...
    return __spreadArray([], customers, true).sort(function (a, b) { return (b.totalSpent || 0) - (a.totalSpent || 0); });
}
// ============================================================================
...
function generateFollowUpMessage(customer, context) {
    ...
    switch (context) {
        case 'appointment':
            return "Hi ".concat(firstName, "! Hope you're doing well. It's been a while since your last visit. Would you like to schedule your next appointment?");
        case 'satisfaction':
            return "Hi ".concat(firstName, "! We'd love to hear about your recent experience with us. Your feedback helps us improve our services.");
        case 'retention':
            return "Hi ".concat(firstName, "! We miss you! As a valued client, we'd like to offer you a special discount on your next visit. Contact us to schedule!");
        case 'birthday':
            return "Happy Birthday ".concat(firstName, "! \uD83C\uDF89 We hope you have a wonderful day. Treat yourself to one of our services - contact us for a special birthday offer!");
        default:
            return "Hi ".concat(firstName, "! Just checking in to see how you're doing. Let us know if there's anything we can help you with!");
    }
}
// ============================================================================
...
function segmentCustomers(customers, criteria) {
    ...
    if (criteria.minTotalSpent) {
        var highValueCustomers = customers.filter(function (c) { return c.totalSpent >= criteria.minTotalSpent; });
    ...
    if (criteria.minAppointments) {
        var frequentCustomers = customers.filter(function (c) { return c.appointmentCount >= criteria.minAppointments; });
...
function calculateRetentionRate(customers, periodMonths) {
    ...
    var retentionRate = eligibleCustomers.length > 0
        ? (activeCustomers.length / eligibleCustomers.length) * 100
        : 0;
    var averageLifetimeValue = customers.length > 0
        ? customers.reduce(function (sum, c) { return sum + c.totalSpent; }, 0) / customers.length
        : 0;
    return {
