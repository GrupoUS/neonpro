...
function LeadTracking(_a) {
    var _b = _a.customers, customers = _b === void 0 ? [] : _b, _c = _a.appointments, appointments = _c === void 0 ? [] : _c, onLeadAction = _a.onLeadAction, _d = _a.className, className = _d === void 0 ? '' : _d;
    var _e = (0, react_1.useState)('all'), selectedPriority = _e[0], setSelectedPriority = _e[1];
    var _f = (0, react_1.useState)('all'), selectedLifecycle = _f[0], setSelectedLifecycle = _f[1];
    var _g = (0, react_1.useState)(''), searchTerm = _g[0], setSearchTerm = _g[1];
    var _h = (0, react_1.useState)('score'), sortBy = _h[0], setSortBy = _h[1];
    // Calculate lead scores for all customers
    var customerLeadScores = (0, react_1.useMemo)(function () {
        return customers.map(function (customer) {
            var customerAppointments = appointments.filter(function (apt) { return apt.customerId === customer.id; });
    ...
    var filteredLeads = (0, react_1.useMemo)(function () {
        var filtered = customerLeadScores.filter(function (lead) {
            var matchesSearch = lead.customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                lead.customer.email.toLowerCase().includes(searchTerm.toLowerCase());
            var matchesPriority = selectedPriority === 'all' || lead.leadScore.priority === selectedPriority;
        ...
        switch (sortBy) {
            case 'score':
                filtered.sort(function (a, b) { return b.leadScore.score - a.leadScore.score; });
                break;
            case 'value':
                filtered.sort(function (a, b) { return b.lifetimeValue - a.lifetimeValue; });
                break;
            case 'risk':
                filtered.sort(function (a, b) { return b.churnRisk - a.churnRisk; });
                break;
            case 'name':
                filtered.sort(function (a, b) { return a.customer.name.localeCompare(b.customer.name); });
                break;
            default:
                break;
        }
    ...
    var pipelineAnalytics = (0, react_1.useMemo)(function () {
        var totalLeads = customerLeadScores.length;
        var highPriorityLeads = customerLeadScores.filter(function (lead) { return lead.leadScore.priority === 'high'; }).length;
        var mediumPriorityLeads = customerLeadScores.filter(function (lead) { return lead.leadScore.priority === 'medium'; }).length;
        var lowPriorityLeads = customerLeadScores.filter(function (lead) { return lead.leadScore.priority === 'low'; }).length;
        var averageScore = totalLeads > 0
            ? customerLeadScores.reduce(function (sum, lead) { return sum + lead.leadScore.score; }, 0) / totalLeads
            : 0;
    ...
    var renderLeadCard = function (leadData) {
        var customer = leadData.customer, leadScore = leadData.leadScore, lifecycle = leadData.lifecycle, churnRisk = leadData.churnRisk, lifetimeValue = leadData.lifetimeValue;
        return (<card_1.Card key={customer.id} className="hover:shadow-md transition-shadow">
        ...
        <card_1.CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={"font-medium ".concat(churnRisk > 60 ? 'text-red-600' : churnRisk > 30 ? 'text-yellow-600' : 'text-green-600')}>
                {churnRisk.toFixed(0)}%
              </span>
            </div>
            <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2 pt-2">
            <button_1.Button size="sm" variant="default" onClick={function () { return handleLeadAction(customer.id, 'contact'); }} className="flex-1">
              Contact
            </button_1.Button>
            <button_1.Button size="sm" variant="outline" onClick={function () { return handleLeadAction(customer.id, 'schedule'); }} className="flex-1">
              Schedule
            </button_1.Button>
          </div>
        </card_1.CardContent>
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      {pipelineAnalytics.conversionOpportunities > 0 && (<card_1.Card className="border-green-200 bg-green-50">
          <card_1.CardHeader>
      ...
      <tabs_1.Tabs defaultValue="pipeline" className="space-y-4">
        ...
        <tabs_1.TabsContent value="pipeline" className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1">
                  ...
                  <div className="relative">
                    ...
                    <input_1.Input id="lead-search" placeholder="Search by name or email..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
                  </div>
                </div>
          ...
          {filteredLeads.length === 0 && (<card_1.Card>
        ...
        <tabs_1.TabsContent value="analytics" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <progress_1.Progress value={(pipelineAnalytics.highPriorityLeads / pipelineAnalytics.totalLeads) * 100} className="h-2"/>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-sm">Medium Priority</span>
                    <span className="text-sm font-medium">{pipelineAnalytics.mediumPriorityLeads}</span>
                  </div>
                  <progress_1.Progress value={(pipelineAnalytics.mediumPriorityLeads / pipelineAnalytics.totalLeads) * 100} className="h-2"/>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-sm">Low Priority</span>
                    <span className="text-sm font-medium">{pipelineAnalytics.lowPriorityLeads}</span>
                  </div>
                  <progress_1.Progress value={(pipelineAnalytics.lowPriorityLeads / pipelineAnalytics.totalLeads) * 100} className="h-2"/>
                </div>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {filteredLeads.slice(0, 5).map(function (lead, index) { return (<div key={lead.customer.id} className="flex items-center justify-between p-2 rounded bg-gray-50">
                      ...
                      <div className="text-right">
                        ...
                        <badge_1.Badge className={getPriorityColor(lead.leadScore.priority)} variant="outline">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredLeads
            .filter(function (lead) { return lead.leadScore.priority === 'high'; })
            .slice(0, 6)
            .map(renderLeadCard)}
              </div>
              {filteredLeads.filter(function (lead) { return lead.leadScore.priority === 'high'; }).length === 0 && (<p className="text-center text-gray-500 py-4">No high priority leads at this time</p>)}
            </card_1.CardContent>
          </card_1.Card>

          {/* At-Risk High Value Customers */}
          {pipelineAnalytics.atRiskHighValue > 0 && (<card_1.Card className="border-orange-200">
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>High-value customers with elevated churn risk</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredLeads
                .filter(function (lead) { return lead.churnRisk > 50 && lead.lifetimeValue > 500; })
                .slice(0, 4)
                .map(renderLeadCard)}
                </div>
              </card_1.CardContent>
