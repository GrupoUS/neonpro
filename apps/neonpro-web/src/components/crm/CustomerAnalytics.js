...
function CustomerAnalytics(_a) {
    var _b = _a.customers, customers = _b === void 0 ? [] : _b, _c = _a.appointments, appointments = _c === void 0 ? [] : _c, _d = _a.className, className = _d === void 0 ? '' : _d;
    ...
    var analytics = (0, react_1.useMemo)(function () {
        ...
        var customerLifecycles = customers.map(function (c) { return ({
            customer: c,
            lifecycle: (0, utils_1.determineCustomerLifecycle)(c),
            churnRisk: (0, utils_1.predictChurnRisk)(c)
        }); });
        var atRiskCustomers = customerLifecycles.filter(function (cl) { return cl.lifecycle === 'at-risk'; }).length;
        var churnedCustomers = customerLifecycles.filter(function (cl) { return cl.lifecycle === 'churned'; }).length;
        var totalRevenue = customers.reduce(function (sum, c) { return sum + c.totalSpent; }, 0);
        var averageLifetimeValue = totalRevenue / totalCustomers;
        var customersWithSatisfaction = customers.filter(function (c) { return c.satisfactionRating; });
        var averageSatisfaction = customersWithSatisfaction.length > 0
            ? customersWithSatisfaction.reduce(function (sum, c) { return sum + (c.satisfactionRating || 0); }, 0) / customersWithSatisfaction.length
            : 0;
        // Lifecycle distribution
        var lifecycleDistribution = {
            new: customerLifecycles.filter(function (cl) { return cl.lifecycle === 'new'; }).length,
            active: customerLifecycles.filter(function (cl) { return cl.lifecycle === 'active'; }).length,
            atRisk: customerLifecycles.filter(function (cl) { return cl.lifecycle === 'at-risk'; }).length,
            churned: customerLifecycles.filter(function (cl) { return cl.lifecycle === 'churned'; }).length
        };
    ...
    return (<div className={"space-y-6 ".concat(className)}>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <div className="flex items-center mt-1">
              {[1, 2, 3, 4, 5].map(function (star) { return (<lucide_react_1.Heart key={star} className={"h-3 w-3 ".concat(star <= Math.round(analytics.overview.averageSatisfaction)
      ...
      {/* Warning Cards */}
      {analytics.overview.atRiskCustomers > 0 && (<card_1.Card className="border-yellow-200 bg-yellow-50">
          <card_1.CardHeader>
      ...
      <tabs_1.Tabs defaultValue="overview" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Distribution of customers across lifecycle stages</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <progress_1.Progress value={(analytics.lifecycle.new / analytics.overview.totalCustomers) * 100} className="h-2"/>
                  ...
                  <progress_1.Progress value={(analytics.lifecycle.active / analytics.overview.totalCustomers) * 100} className="h-2"/>
                  ...
                  <progress_1.Progress value={(analytics.lifecycle.atRisk / analytics.overview.totalCustomers) * 100} className="h-2"/>
                  ...
                  <progress_1.Progress value={(analytics.lifecycle.churned / analytics.overview.totalCustomers) * 100} className="h-2"/>
                </div>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {analytics.topCustomers.slice(0, 5).map(function (customer, index) { return (<div key={customer.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                      ...
                      <div className="text-right">
                        ...
                        {customer.satisfactionRating && (<div className="text-xs text-gray-500">{customer.satisfactionRating}/5 ⭐</div>)}
                      </div>
                    </div>); })}
                </div>
              </card_1.CardContent>
        ...
        <tabs_1.TabsContent value="lifecycle" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <card_1.Card className="border-green-200">
              <card_1.CardHeader className="pb-2">
                <card_1.CardTitle className="text-sm text-green-700">Active Customers</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
            ...
            <card_1.Card className="border-yellow-200">
              <card_1.CardHeader className="pb-2">
                <card_1.CardTitle className="text-sm text-yellow-700">At-Risk Customers</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
            ...
            <card_1.Card className="border-red-200">
              <card_1.CardHeader className="pb-2">
                <card_1.CardTitle className="text-sm text-red-700">Churned Customers</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
        ...
        <tabs_1.TabsContent value="segments" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {analytics.segments.map(function (segment, index) { return (<card_1.Card key={index}>
                <card_1.CardHeader>
                ...
                <card_1.CardContent>
                  ...
                  <progress_1.Progress value={(segment.size / analytics.overview.totalCustomers) * 100} className="h-2"/>
                  
                  {segment.customers.slice(0, 3).map(function (customer) { return (<div key={customer.id} className="flex items-center justify-between mt-3 p-2 bg-gray-50 rounded text-sm">
                      <span>{customer.name}</span>
                      <span className="font-medium">{formatCurrency(customer.totalSpent)}</span>
                    </div>); })}
                  
                  {segment.customers.length > 3 && (<div className="text-xs text-gray-500 mt-2 text-center">
          ...
          {analytics.segments.length === 0 && (<card_1.Card>
        ...
        <tabs_1.TabsContent value="performance" className="space-y-4">
          ...
          {/* Risk Customers */}
          {analytics.retention.riskCustomers.length > 0 && (<card_1.Card className="border-orange-200">
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Customers who need immediate attention</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2">
                  {analytics.retention.riskCustomers.slice(0, 5).map(function (customer) { return (<div key={customer.id} className="flex items-center justify-between p-3 bg-orange-50 rounded">
                      <div>
