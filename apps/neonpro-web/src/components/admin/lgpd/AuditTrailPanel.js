...
function AuditTrailPanel(_a) {
    ...
    var _b = (0, useLGPD_1.useAuditTrail)(), events = _b.events, isLoading = _b.isLoading, error = _b.error, exportAuditTrail = _b.exportAuditTrail, refreshData = _b.refreshData;
    var _c = (0, react_1.useState)(''), searchTerm = _c[0], setSearchTerm = _c[1];
    var _d = (0, react_1.useState)('all'), actionFilter = _d[0], setActionFilter = _d[1];
    var _e = (0, react_1.useState)('all'), entityFilter = _e[0], setEntityFilter = _e[1];
    var _f = (0, react_1.useState)('all'), userFilter = _f[0], setUserFilter = _f[1];
    var _g = (0, react_1.useState)('7d'), dateRange = _g[0], setDateRange = _g[1];
    var _h = (0, react_1.useState)(null), selectedEvent = _h[0], setSelectedEvent = _h[1];
    // Filtrar eventos
    var filteredEvents = (events === null || events === void 0 ? void 0 : events.filter(function (event) {
        var _a, _b, _c, _d;
        var matchesSearch = ((_a = event.action) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_b = event.entity_type) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_c = event.details) === null || _c === void 0 ? void 0 : _c.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_d = event.user_id) === null || _d === void 0 ? void 0 : _d.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    var getActionBadge = function (action) {
        switch (action) {
            case 'create':
                return <badge_1.Badge variant="default" className="bg-green-600"><lucide_react_1.CheckCircle className="h-3 w-3 mr-1"/>Criar</badge_1.Badge>;
            case 'read':
            case 'view':
                return <badge_1.Badge variant="outline"><lucide_react_1.Eye className="h-3 w-3 mr-1"/>Visualizar</badge_1.Badge>;
            case 'update':
            case 'edit':
                return <badge_1.Badge variant="secondary"><lucide_react_1.Settings className="h-3 w-3 mr-1"/>Atualizar</badge_1.Badge>;
            case 'delete':
                return <badge_1.Badge variant="destructive"><lucide_react_1.XCircle className="h-3 w-3 mr-1"/>Excluir</badge_1.Badge>;
            case 'export':
                return <badge_1.Badge variant="outline" className="bg-blue-50 text-blue-700"><lucide_react_1.Download className="h-3 w-3 mr-1"/>Exportar</badge_1.Badge>;
            case 'login':
                return <badge_1.Badge variant="outline" className="bg-green-50 text-green-700"><lucide_react_1.User className="h-3 w-3 mr-1"/>Login</badge_1.Badge>;
            case 'logout':
                return <badge_1.Badge variant="outline" className="bg-gray-50 text-gray-700"><lucide_react_1.User className="h-3 w-3 mr-1"/>Logout</badge_1.Badge>;
            case 'consent_given':
                return <badge_1.Badge variant="default" className="bg-green-600"><lucide_react_1.CheckCircle className="h-3 w-3 mr-1"/>Consentimento</badge_1.Badge>;
            case 'consent_withdrawn':
                return <badge_1.Badge variant="destructive"><lucide_react_1.XCircle className="h-3 w-3 mr-1"/>Retirada</badge_1.Badge>;
            case 'data_request':
                return <badge_1.Badge variant="outline" className="bg-purple-50 text-purple-700"><lucide_react_1.FileText className="h-3 w-3 mr-1"/>Solicitação</badge_1.Badge>;
            case 'breach_reported':
                return <badge_1.Badge variant="destructive"><lucide_react_1.AlertTriangle className="h-3 w-3 mr-1"/>Violação</badge_1.Badge>;
            default:
                return <badge_1.Badge variant="secondary"><lucide_react_1.Activity className="h-3 w-3 mr-1"/>{action}</badge_1.Badge>;
        }
    };
    ...
    var stats = {
        ...
        today: (events === null || events === void 0 ? void 0 : events.filter(function (e) {
            var eventDate = new Date(e.timestamp);
            var today = new Date();
            return eventDate.toDateString() === today.toDateString();
        }).length) || 0,
        thisWeek: (events === null || events === void 0 ? void 0 : events.filter(function (e) {
            var eventDate = new Date(e.timestamp);
            var weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
            return eventDate >= weekAgo;
        }).length) || 0,
        uniqueUsers: new Set(events === null || events === void 0 ? void 0 : events.map(function (e) { return e.user_id; }).filter(Boolean)).size || 0
    };
    // Obter listas únicas para filtros
    var uniqueActions = __spreadArray([], new Set(events === null || events === void 0 ? void 0 : events.map(function (e) { return e.action; }).filter(Boolean)), true) || [];
    var uniqueEntities = __spreadArray([], new Set(events === null || events === void 0 ? void 0 : events.map(function (e) { return e.entity_type; }).filter(Boolean)), true) || [];
    var uniqueUsers = __spreadArray([], new Set(events === null || events === void 0 ? void 0 : events.map(function (e) { return e.user_id; }).filter(Boolean)), true) || [];
    if (isLoading) {
      ...
      <tabs_1.Tabs defaultValue="events" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input_1.Input id="search" placeholder="Ação, entidade, usuário..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-8"/>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select_1.Select value={actionFilter} onValueChange={setActionFilter}>
                    ...
                    <select_1.SelectContent>
                      ...
                      {uniqueActions.map(function (action) { return (<select_1.SelectItem key={action} value={action}>{action}</select_1.SelectItem>); })}
                    </select_1.SelectContent>
                  </select_1.Select>
                </div>
                ...
                <div>
                  ...
                  <select_1.Select value={entityFilter} onValueChange={setEntityFilter}>
                    ...
                    <select_1.SelectContent>
                      <select_1.SelectItem value="all">Todas</select_1.SelectItem>
                      {uniqueEntities.map(function (entity) { return (<select_1.SelectItem key={entity} value={entity}>{entity}</select_1.SelectItem>); })}
                    </select_1.SelectContent>
                  </select_1.Select>
                </div>
                ...
                <div>
                  ...
                  <select_1.Select value={userFilter} onValueChange={setUserFilter}>
                    ...
                    <select_1.SelectContent>
                      ...
                      {uniqueUsers.map(function (user) { return (<select_1.SelectItem key={user} value={user}>{user}</select_1.SelectItem>); })}
                    </select_1.SelectContent>
                  </select_1.Select>
                </div>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <table_1.Table>
                ...
                <table_1.TableBody>
                  {filteredEvents.map(function (event) { return (<table_1.TableRow key={event.id}>
                      <table_1.TableCell>
        ...
        <tabs_1.TabsContent value="analytics" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {Object.entries((events === null || events === void 0 ? void 0 : events.reduce(function (acc, event) {
            ...
            return (<div key={action} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
            ...
            return (<div key={entity} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
            ...
            return (<div key={user} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <lucide_react_1.User className="h-4 w-4 text-muted-foreground"/>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <badge_1.Badge variant="outline">
                      {(events === null || events === void 0 ? void 0 : events.filter(function (e) {
            var eventDate = new Date(e.timestamp);
