...
function ComplianceAssessmentPanel(_a) {
    ...
    var _c = (0, useLGPD_1.useComplianceAssessment)(), assessments = _c.assessments, isLoading = _c.isLoading, error = _c.error, createAssessment = _c.createAssessment, runAutomatedAssessment = _c.runAutomatedAssessment, exportAssessments = _c.exportAssessments, refreshData = _c.refreshData;
    ...
    var _g = (0, react_1.useState)(null), selectedAssessment = _g[0], setSelectedAssessment = _g[1];
    var _h = (0, react_1.useState)(false), isCreateOpen = _h[0], setIsCreateOpen = _h[1];
    var _j = (0, react_1.useState)(false), isRunningAutomated = _j[0], setIsRunningAutomated = _j[1];
    var _k = (0, react_1.useState)({
        name: '',
        description: '',
        type: 'manual'
    }), newAssessment = _k[0], setNewAssessment = _k[1];
    // Filtrar avaliações
    var filteredAssessments = (assessments === null || assessments === void 0 ? void 0 : assessments.filter(function (assessment) {
        var _a, _b;
        var matchesSearch = ((_a = assessment.name) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_b = assessment.description) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    var getStatusBadge = function (status) {
        switch (status) {
            ...
            case 'in_progress':
                return <badge_1.Badge variant="default"><lucide_react_1.RefreshCw className="h-3 w-3 mr-1"/>Em Execução</badge_1.Badge>;
            case 'completed':
                return <badge_1.Badge variant="outline"><lucide_react_1.CheckCircle className="h-3 w-3 mr-1"/>Concluída</badge_1.Badge>;
            case 'failed':
                return <badge_1.Badge variant="destructive"><lucide_react_1.XCircle className="h-3 w-3 mr-1"/>Falhou</badge_1.Badge>;
            default:
    ...
    var getScoreBadge = function (score) {
        if (score >= 90)
            return <badge_1.Badge variant="default" className="bg-green-600">Excelente</badge_1.Badge>;
        if (score >= 70)
            return <badge_1.Badge variant="secondary" className="bg-yellow-600">Bom</badge_1.Badge>;
        if (score >= 50)
            return <badge_1.Badge variant="secondary" className="bg-orange-600">Regular</badge_1.Badge>;
    ...
    var handleCreateAssessment = function () { return __awaiter(_this, void 0, void 0, function () {
        var error_1;
    ...
    var handleRunAutomatedAssessment = function () { return __awaiter(_this, void 0, void 0, function () {
        var error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    var stats = {
        ...
        completed: (assessments === null || assessments === void 0 ? void 0 : assessments.filter(function (a) { return a.status === 'completed'; }).length) || 0,
        pending: (assessments === null || assessments === void 0 ? void 0 : assessments.filter(function (a) { return a.status === 'pending'; }).length) || 0,
        averageScore: (assessments === null || assessments === void 0 ? void 0 : assessments.length) ?
            Math.round(assessments.reduce(function (sum, a) { return sum + (a.score || 0); }, 0) / assessments.length) : 0
    };
    var latestAssessment = assessments === null || assessments === void 0 ? void 0 : assessments.find(function (a) { return a.status === 'completed'; });
    if (isLoading) {
    ...
    return (<div className={className}>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button_1.Button variant="outline" onClick={handleRunAutomatedAssessment} disabled={isRunningAutomated}>
            {isRunningAutomated ? (<lucide_react_1.RefreshCw className="h-4 w-4 mr-2 animate-spin"/>) : (<lucide_react_1.Play className="h-4 w-4 mr-2"/>)}
      ...
      {/* Última avaliação */}
      {latestAssessment && (<card_1.Card className="mb-6">
          <card_1.CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className={"text-3xl font-bold ".concat(getScoreColor(latestAssessment.score || 0))}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {((_b = latestAssessment.recommendations) === null || _b === void 0 ? void 0 : _b.slice(0, 3).map(function (rec, index) { return (<div key={index} className="text-sm bg-muted p-2 rounded">
                      {rec}
      ...
      <tabs_1.Tabs defaultValue="assessments" className="space-y-4">
        ...
        <tabs_1.TabsContent value="assessments" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input_1.Input id="search" placeholder="Nome ou descrição..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-8"/>
                  </div>
                </div>
          ...
          <card_1.Card>
            <card_1.CardHeader>
              <card_1.CardTitle>Histórico de Avaliações ({filteredAssessments.length})</card_1.CardTitle>
            </card_1.CardHeader>
            <card_1.CardContent>
              <table_1.Table>
                <table_1.TableHeader>
                ...
                <table_1.TableBody>
                  {filteredAssessments.map(function (assessment) { return (<table_1.TableRow key={assessment.id}>
                      <table_1.TableCell>
                        <div>
                      ...
                      <table_1.TableCell>
                        <badge_1.Badge variant={assessment.type === 'automated' ? 'default' : 'outline'}>
                          {assessment.type === 'automated' ? 'Automatizada' : 'Manual'}
                        </badge_1.Badge>
                      </table_1.TableCell>
                      <table_1.TableCell>{getStatusBadge(assessment.status)}</table_1.TableCell>
                      <table_1.TableCell>
                        {assessment.score !== null ? (<div className="flex items-center gap-2">
                            <span className={"font-semibold ".concat(getScoreColor(assessment.score))}>
                              {assessment.score}%
                            </span>
                      ...
                      <table_1.TableCell>
                        {assessment.completed_at ? (new Date(assessment.completed_at).toLocaleDateString('pt-BR')) : (<span className="text-muted-foreground">-</span>)}
                      </table_1.TableCell>
                      <table_1.TableCell>
                        <dialog_1.Dialog>
                          ...
                          <dialog_1.DialogContent className="max-w-4xl">
                            ...
                            <div className="space-y-6">
                              ...
                              {/* Pontuação */}
                              {assessment.score !== null && (<div>
                                  <label_1.Label>Pontuação de Conformidade</label_1.Label>
                                  <div className="mt-2">
                                    <div className="flex items-center gap-2 mb-2">
                                      <span className={"text-2xl font-bold ".concat(getScoreColor(assessment.score))}>
                                        {assessment.score}%
                                      </span>
                                      {getScoreBadge(assessment.score)}
                                    </div>
                                    <progress_1.Progress value={assessment.score} className="mb-2"/>
                                  </div>
                                </div>)}
                              
                              {/* Recomendações */}
                              {assessment.recommendations && assessment.recommendations.length > 0 && (<div>
                                  <label_1.Label>Recomendações</label_1.Label>
                                  <div className="mt-2 space-y-2">
                                    {assessment.recommendations.map(function (rec, index) { return (<div key={index} className="text-sm bg-muted p-3 rounded">
                                        {rec}
                                      </div>); })}
                                  </div>
                                </div>)}
              ...
              {filteredAssessments.length === 0 && (<div className="text-center py-8 text-muted-foreground">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-4">
              <div>
                ...
                <input_1.Input id="name" value={newAssessment.name} onChange={function (e) { return setNewAssessment(__assign(__assign({}, newAssessment), { name: e.target.value })); }} placeholder="Ex: Avaliação Trimestral Q1 2024"/>
              </div>
              
              <div>
                <label_1.Label htmlFor="description">Descrição</label_1.Label>
                <textarea_1.Textarea id="description" value={newAssessment.description} onChange={function (e) { return setNewAssessment(__assign(__assign({}, newAssessment), { description: e.target.value })); }} placeholder="Descreva o objetivo e escopo desta avaliação..." rows={3}/>
              </div>
              
              <div>
                <label_1.Label htmlFor="type">Tipo de Avaliação</label_1.Label>
                <select_1.Select value={newAssessment.type} onValueChange={function (value) {
            ...
            : 'Avaliação que requer intervenção manual'}
              ...
              <div className="flex gap-2">
                ...
                <button_1.Button variant="outline" onClick={function () { return setNewAssessment({ name: '', description: '', type: 'manual' }); }}>
                  Limpar
                </button_1.Button>
              </div>
            </card_1.CardContent>
