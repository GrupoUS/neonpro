...
function SessionTimeoutConfig(_a) {
  ...
  var _b = (0, react_1.useState)(DEFAULT_CONFIGS),
    configs = _b[0],
    setConfigs = _b[1];
  var _c = (0, react_1.useState)("staff"),
    activeRole = _c[0],
    setActiveRole = _c[1];
  ...
  var handleConfigChange = function (role, field, value) {
    ...
    var newConfigs = __assign(
      __assign({}, configs),
      ((_a = {}),
      (_a[role] = __assign(__assign({}, configs[role]), ((_b = {}), (_b[field] = value), _b))),
      _a),
    );
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            return [
              4 /*yield*/,
              fetch("/api/admin/session-timeout-configs", {
                method: "POST",
                headers: {
          ...
          case 1:
            ...
            Object.entries(configs).forEach(function (_a) {
              ...
              onConfigUpdate === null || onConfigUpdate === void 0
                ? void 0
                : onConfigUpdate(role, sessionConfig);
            });
            setHasChanges(false);
            setValidationErrors({});
      ...
      >
        <tabs_1.TabsList className="grid w-full grid-cols-4">
          {Object.entries(ROLE_LABELS).map(function (_a) {
            ...
            return (
              <tabs_1.TabsTrigger key={role} value={role} className="flex flex-col gap-1">
                <span>{label}</span>
        ...
        {Object.entries(configs).map(function (_a) {
          var role = _a[0],
            config = _a[1];
          return (
            <tabs_1.TabsContent key={role} value={role}>
              <card_1.Card>
                <card_1.CardHeader>
                  ...
                  <card_1.CardDescription>{ROLE_DESCRIPTIONS[role]}</card_1.CardDescription>
                </card_1.CardHeader>
                <card_1.CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input_1.Input
                        id={"".concat(role, "-default-timeout")}
                        type="number"
                        min="1"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input_1.Input
                        id={"".concat(role, "-activity-extension")}
                        type="number"
                        min="1"
                  ...
                  <div className="bg-muted/50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div>
                        Extensão por atividade: {formatDuration(config.activityExtensionMinutes)}
                      </div>
                      <div>Período de graça: {formatDuration(config.gracePeriodMinutes)}</div>
                      <div>Avisos: {config.warningThresholds.join(", ")} min</div>
                      <div>
                        Auto-extensão: {config.autoExtendEnabled ? "Ativada" : "Desativada"}
                      </div>
                    </div>
                  </div>
                </card_1.CardContent>
