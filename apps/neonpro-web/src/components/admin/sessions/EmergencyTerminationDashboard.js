...
function EmergencyTerminationDashboard(_a) {
    var _this = this;
    var emergencyTermination = _a.emergencyTermination, currentUser = _a.currentUser;
    var _b = (0, react_1.useState)([]), activeSessions = _b[0], setActiveSessions = _b[1];
    var _c = (0, react_1.useState)([]), protocols = _c[0], setProtocols = _c[1];
    var _d = (0, react_1.useState)([]), auditLogs = _d[0], setAuditLogs = _d[1];
    var _e = (0, react_1.useState)(null), config = _e[0], setConfig = _e[1];
    var _f = (0, react_1.useState)(true), loading = _f[0], setLoading = _f[1];
    var _g = (0, react_1.useState)([]), selectedSessions = _g[0], setSelectedSessions = _g[1];
    var _h = (0, react_1.useState)(false), terminationInProgress = _h[0], setTerminationInProgress = _h[1];
    var _j = (0, react_1.useState)(false), showTerminationDialog = _j[0], setShowTerminationDialog = _j[1];
    var _k = (0, react_1.useState)(false), showProtocolDialog = _k[0], setShowProtocolDialog = _k[1];
    var _l = (0, react_1.useState)(''), terminationReason = _l[0], setTerminationReason = _l[1];
    var _m = (0, react_1.useState)('single'), terminationType = _m[0], setTerminationType = _m[1];
    var _o = (0, react_1.useState)(true), preserveData = _o[0], setPreserveData = _o[1];
    var _p = (0, react_1.useState)(true), notifyUsers = _p[0], setNotifyUsers = _p[1];
    var _q = (0, react_1.useState)(''), selectedProtocol = _q[0], setSelectedProtocol = _q[1];
    var _r = (0, react_1.useState)(null), refreshInterval = _r[0], setRefreshInterval = _r[1];
    var _s = (0, react_1.useState)({
        totalSessions: 0,
        highRiskSessions: 0,
        recentTerminations: 0,
        activeProtocols: 0
    }), stats = _s[0], setStats = _s[1];
    // Load initial data
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, emergencyTermination.getTerminationLogs({
                            limit: 50
                        })];
                case 1:
                    logs = _a.sent();
                    ...
                    setStats({
                        ...
                        highRiskSessions: mockSessions.filter(function (s) { return s.riskLevel === 'high' || s.riskLevel === 'critical'; }).length,
                        recentTerminations: logs.filter(function (log) {
                            return new Date(log.terminatedAt).getTime() > Date.now() - 24 * 60 * 60 * 1000;
                        }).length,
                        activeProtocols: protocolsList.filter(function (p) { return p.isActive; }).length
                    });
                    return [3 /*break*/, 4];
                case 2:
                    error_1 = _a.sent();
    ...
    var handleSessionSelection = function (sessionId, selected) {
        if (selected) {
            setSelectedSessions(function (prev) { return __spreadArray(__spreadArray([], prev, true), [sessionId], false); });
        }
        else {
            setSelectedSessions(function (prev) { return prev.filter(function (id) { return id !== sessionId; }); });
        }
    };
    var handleSelectAll = function () {
        if (selectedSessions.length === activeSessions.length) {
            ...
            setSelectedSessions(activeSessions.map(function (s) { return s.sessionId; }));
        }
    };
    var handleTerminateSession = function (sessionId) { return __awaiter(_this, void 0, void 0, function () {
        var result, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                ...
                case 1:
                    ...
                    return [4 /*yield*/, emergencyTermination.terminateSession(sessionId, currentUser.id, terminationReason, preserveData)];
                case 2:
                    result = _a.sent();
                ...
                case 3:
                    ...
                    return [3 /*break*/, 5];
                case 4:
                    sonner_1.toast.error("Falha ao terminar sess\u00E3o: ".concat(result.errors.join(', ')));
                    _a.label = 5;
    ...
    var handleBulkTermination = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    setTerminationInProgress(true);
                    return [4 /*yield*/, Promise.all(selectedSessions.map(function (sessionId) {
                            return emergencyTermination.terminateSession(sessionId, currentUser.id, terminationReason, preserveData);
                        }))];
                case 2:
                    results = _a.sent();
    ...
    var handleTerminateAll = function () { return __awaiter(_this, void 0, void 0, function () {
        var result, error_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                ...
                case 1:
                    _a.trys.push([1, 6, 7, 8]);
                    setTerminationInProgress(true);
                    return [4 /*yield*/, emergencyTermination.terminateAllSessions(currentUser.id, terminationReason, preserveData)];
                case 2:
                    result = _a.sent();
                    if (!result.success) return [3 /*break*/, 4];
                    sonner_1.toast.success("Todas as sess\u00F5es foram terminadas (".concat(result.totalSuccessful, " sess\u00F5es)"));
                    setSelectedSessions([]);
                    return [4 /*yield*/, loadDashboardData()];
                case 3:
                    _a.sent();
                    return [3 /*break*/, 5];
                case 4:
                    sonner_1.toast.error("Falha ao terminar todas as sess\u00F5es: ".concat(result.errors.join(', ')));
                    _a.label = 5;
                case 5: return [3 /*break*/, 8];
                case 6:
                    error_4 = _a.sent();
    ...
    var handleExecuteProtocol = function (protocolId) { return __awaiter(_this, void 0, void 0, function () {
        var result, error_5;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    setTerminationInProgress(true);
                    return [4 /*yield*/, emergencyTermination.executeProtocol(protocolId, currentUser.id, currentUser.role)];
                case 1:
                    result = _a.sent();
                    if (!result.success) return [3 /*break*/, 3];
                    sonner_1.toast.success("Protocolo executado com sucesso (".concat(result.totalSuccessful, " sess\u00F5es terminadas)"));
                    return [4 /*yield*/, loadDashboardData()];
                case 2:
                    _a.sent();
                    return [3 /*break*/, 4];
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="flex flex-wrap gap-2">
            {protocols.filter(function (p) { return p.isActive; }).map(function (protocol) { return (<ui_1.Button key={protocol.protocolId} variant={protocol.severity === 'critical' ? 'destructive' : 'outline'} size="sm" onClick={function () { return handleExecuteProtocol(protocol.protocolId); }} disabled={terminationInProgress} className="flex items-center">
            ...
            <ui_1.Dialog open={showTerminationDialog} onOpenChange={setShowTerminationDialog}>
              <ui_1.DialogTrigger asChild>
                <ui_1.Button variant="destructive" size="sm" disabled={terminationInProgress} onClick={function () {
          ...
          <card_1.Card>
            <card_1.CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {selectedSessions.length > 0 && (<ui_1.Dialog open={showTerminationDialog} onOpenChange={setShowTerminationDialog}>
                      <ui_1.DialogTrigger asChild>
                        <ui_1.Button variant="destructive" size="sm" disabled={terminationInProgress} onClick={function () {
            ...
            <card_1.CardContent>
              <div className="rounded-md border">
                <ui_1.Table>
                  <ui_1.TableHeader>
                    <ui_1.TableRow>
                      <ui_1.TableHead className="w-12">
                        <input type="checkbox" checked={selectedSessions.length === activeSessions.length && activeSessions.length > 0} onChange={handleSelectAll} className="rounded"/>
                      </ui_1.TableHead>
                  ...
                  <ui_1.TableBody>
                    {activeSessions.map(function (session) { return (<ui_1.TableRow key={session.sessionId}>
                        <ui_1.TableCell>
                          <input type="checkbox" checked={selectedSessions.includes(session.sessionId)} onChange={function (e) { return handleSessionSelection(session.sessionId, e.target.checked); }} className="rounded"/>
                        </ui_1.TableCell>
                        <ui_1.TableCell>
                          <div className="flex flex-col">
                            <span className="font-medium">{session.userName}</span>
                            <span className="text-sm text-muted-foreground">{session.userEmail}</span>
                            <ui_1.Badge className={"w-fit mt-1 ".concat(ROLE_COLORS[session.userRole])}>
                              {session.userRole}
                            </ui_1.Badge>
                          </div>
                        </ui_1.TableCell>
                        ...
                        <ui_1.TableCell>
                          <div className="flex flex-col">
                            <span className="text-sm">
                              Iniciada {(0, date_fns_1.formatDistanceToNow)(session.startedAt, { addSuffix: true, locale: locale_1.ptBR })}
                            </span>
                            <span className="text-sm text-muted-foreground">
                              Última atividade {(0, date_fns_1.formatDistanceToNow)(session.lastActivity, { addSuffix: true, locale: locale_1.ptBR })}
                            </span>
                          </div>
                        </ui_1.TableCell>
                        <ui_1.TableCell>
                        ...
                        <ui_1.TableCell>
                          <div className="flex items-center space-x-2">
                            <ui_1.Button variant="outline" size="sm" onClick={function () {
                ...
                sonner_1.toast.info('Funcionalidade em desenvolvimento');
                            ...
                            <ui_1.Dialog>
                              <ui_1.DialogTrigger asChild>
                                <ui_1.Button variant="destructive" size="sm" disabled={terminationInProgress}>
                              ...
                              <ui_1.DialogContent>
                                ...
                                <div className="space-y-4">
                                  <div>
                                    ...
                                    <ui_1.Textarea id="reason" placeholder="Descreva o motivo da terminação..." value={terminationReason} onChange={function (e) { return setTerminationReason(e.target.value); }}/>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <ui_1.Switch id="preserve-data" checked={preserveData} onCheckedChange={setPreserveData}/>
                                    <ui_1.Label htmlFor="preserve-data">Preservar dados da sessão</ui_1.Label>
                                  </div>
                                </div>
                                <ui_1.DialogFooter>
                                  <ui_1.Button variant="destructive" onClick={function () { return handleTerminateSession(session.sessionId); }} disabled={terminationInProgress || !terminationReason.trim()}>
                                    {terminationInProgress ? (<lucide_react_1.RefreshCw className="h-4 w-4 mr-2 animate-spin"/>) : (<lucide_react_1.Ban className="h-4 w-4 mr-2"/>)}
                                    Terminar Sessão
                                  </ui_1.Button>
                                </ui_1.DialogFooter>
                              </ui_1.DialogContent>
          ...
          <card_1.Card>
            <card_1.CardHeader>
              <div className="flex items-center justify-between">
                ...
                <ui_1.Button variant="outline" size="sm" onClick={function () { return setShowProtocolDialog(true); }}>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {protocols.map(function (protocol) { return (<card_1.Card key={protocol.protocolId} className="border-l-4 border-l-blue-500">
                    <card_1.CardHeader>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          ...
                          {protocol.automaticTrigger && (<ui_1.Badge variant="outline">
                              <lucide_react_1.Zap className="h-3 w-3 mr-1"/>
                              Auto
                            </ui_1.Badge>)}
                          {!protocol.isActive && (<ui_1.Badge variant="secondary">
                              <lucide_react_1.Pause className="h-3 w-3 mr-1"/>
                              Inativo
                            </ui_1.Badge>)}
                        </div>
                        <div className="flex items-center space-x-2">
                          <ui_1.Button variant="outline" size="sm" onClick={function () { return handleExecuteProtocol(protocol.protocolId); }} disabled={!protocol.isActive || terminationInProgress}>
                    ...
                    <card_1.CardContent>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <ul className="list-disc list-inside mt-1 text-muted-foreground">
                            {protocol.triggerConditions.map(function (condition, index) { return (<li key={index}>{condition}</li>); })}
                          </ul>
                        </div>
                      ...
                      {protocol.lastTriggered && (<div className="mt-4 pt-4 border-t">
                          <span className="text-sm text-muted-foreground">
                            Última execução: {(0, date_fns_1.format)(protocol.lastTriggered, 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
                          </span>
                        </div>)}
                    </card_1.CardContent>
                  </card_1.Card>); })}
            ...
            <card_1.CardContent>
              <div className="rounded-md border">
                <ui_1.Table>
                  ...
                  <ui_1.TableBody>
                    {auditLogs.map(function (log) { return (<ui_1.TableRow key={log.logId}>
                        <ui_1.TableCell>
                          {(0, date_fns_1.format)(log.terminatedAt, 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
                        </ui_1.TableCell>
                        <ui_1.TableCell>
                          <div className="flex flex-col">
                        ...
                        <ui_1.TableCell>
                          <div className="flex items-center space-x-2">
                            {log.success ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.XCircle className="h-4 w-4 text-red-600"/>)}
      ...
      <ui_1.Dialog open={showTerminationDialog} onOpenChange={setShowTerminationDialog}>
        <ui_1.DialogContent className="max-w-md">
          <ui_1.DialogHeader>
            ...
            terminationType === 'bulk' ? "Terminar ".concat(selectedSessions.length, " Sess\u00F5es") :
                'Terminar Sessão'}
            </ui_1.DialogTitle>
            <ui_1.DialogDescription>
              {terminationType === 'all' ?
            ...
            terminationType === 'bulk' ?
                "Esta a\u00E7\u00E3o ir\u00E1 terminar ".concat(selectedSessions.length, " sess\u00F5es selecionadas. Esta a\u00E7\u00E3o n\u00E3o pode ser desfeita.") :
          ...
          <div className="space-y-4">
            <div>
              ...
              <ui_1.Textarea id="termination-reason" placeholder="Descreva o motivo da terminação de emergência..." value={terminationReason} onChange={function (e) { return setTerminationReason(e.target.value); }} className="mt-1"/>
            </div>
            
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                <ui_1.Switch id="preserve-session-data" checked={preserveData} onCheckedChange={setPreserveData}/>
                <ui_1.Label htmlFor="preserve-session-data">Preservar dados das sessões</ui_1.Label>
              </div>
              
              <div className="flex items-center space-x-2">
                <ui_1.Switch id="notify-affected-users" checked={notifyUsers} onCheckedChange={setNotifyUsers}/>
          ...
          <ui_1.DialogFooter>
            <ui_1.Button variant="outline" onClick={function () {
            setShowTerminationDialog(false);
            setTerminationReason('');
        }} disabled={terminationInProgress}>
              Cancelar
            </ui_1.Button>
            <ui_1.Button variant="destructive" onClick={function () {
