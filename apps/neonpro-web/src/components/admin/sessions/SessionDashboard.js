...
function SessionDashboard() {
    var _a = (0, useSession_1.useSession)(), session = _a.session, isLoading = _a.isLoading, securityEvents = _a.securityEvents, devices = _a.devices, analytics = _a.analytics, terminateSession = _a.terminateSession, terminateAllSessions = _a.terminateAllSessions, refreshSession = _a.refreshSession, reportSuspiciousActivity = _a.reportSuspiciousActivity, trustDevice = _a.trustDevice, blockDevice = _a.blockDevice, error = _a.error;
    var _b = (0, react_1.useState)('overview'), activeTab = _b[0], setActiveTab = _b[1];
    var _c = (0, react_1.useState)(false), showSensitiveData = _c[0], setShowSensitiveData = _c[1];
    if (isLoading) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <button_1.Button variant="outline" size="sm" onClick={refreshSession} disabled={isLoading}>
            <lucide_react_1.RefreshCw className={"h-4 w-4 mr-2 ".concat(isLoading ? 'animate-spin' : '')}/>
            Atualizar
          </button_1.Button>
          <button_1.Button variant="destructive" size="sm" onClick={function () { return terminateSession('user_logout'); }}>
      ...
      {session.security_score < 50 && (<alert_1.Alert variant="destructive">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <tabs_1.TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <CurrentSessionCard session={session} showSensitiveData={showSensitiveData} onToggleSensitiveData={function () { return setShowSensitiveData(!showSensitiveData); }}/>
            <RecentSecurityEventsCard events={securityEvents.slice(0, 5)} onReportActivity={reportSuspiciousActivity}/>
          </div>
        </tabs_1.TabsContent>

        {/* Security Tab */}
        <tabs_1.TabsContent value="security" className="space-y-4">
          <SecurityEventsTable events={securityEvents} onReportActivity={reportSuspiciousActivity}/>
        </tabs_1.TabsContent>

        {/* Devices Tab */}
        <tabs_1.TabsContent value="devices" className="space-y-4">
          <DeviceManagementTable devices={devices} currentDeviceFingerprint={session.device_fingerprint} onTrustDevice={trustDevice} onBlockDevice={blockDevice}/>
        </tabs_1.TabsContent>
      ...
      <card_1.CardContent>
        ...
        <div className="space-y-2 mt-2">
          <div className="flex justify-between text-sm">
            ...
            <span>{(0, date_fns_1.formatDistanceToNow)(new Date(session.expires_at), { locale: locale_1.ptBR })}</span>
          </div>
...
function SecurityScoreCard(_a) {
    ...
    var getScoreIcon = function (score) {
        if (score >= 80)
            return <lucide_react_1.Shield className="h-4 w-4 text-green-600"/>;
        if (score >= 60)
    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent>
        <div className={"text-2xl font-bold ".concat(getScoreColor(score))}>
          {score}/100
        </div>
...
function ActiveDevicesCard(_a) {
    ...
    var activeDevices = devices.filter(function (d) { return !d.blocked; });
    var trustedDevices = activeDevices.filter(function (d) { return d.trusted; });
    return (<card_1.Card>
      <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
...
function SecurityEventsCard(_a) {
    ...
    var unresolvedEvents = events.filter(function (e) { return !e.resolved; });
    var criticalEvents = unresolvedEvents.filter(function (e) { return e.severity === 'critical'; });
    return (<card_1.Card>
      <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
...
function CurrentSessionCard(_a) {
    var session = _a.session, showSensitiveData = _a.showSensitiveData, onToggleSensitiveData = _a.onToggleSensitiveData;
      ...
      <card_1.CardHeader>
        <div className="flex items-center justify-between">
          ...
          <button_1.Button variant="ghost" size="sm" onClick={onToggleSensitiveData}>
            {showSensitiveData ? <lucide_react_1.EyeOff className="h-4 w-4"/> : <lucide_react_1.Eye className="h-4 w-4"/>}
          </button_1.Button>
        </div>
      </card_1.CardHeader>
      <card_1.CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {session.location ? "".concat(session.location.city, ", ").concat(session.location.country) : 'Desconhecida'}
            </p>
          </div>
          <div>
            <span className="font-medium">Criada em:</span>
            <p className="text-muted-foreground">
              {(0, date_fns_1.format)(new Date(session.created_at), 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
            </p>
          </div>
          <div>
...
function RecentSecurityEventsCard(_a) {
    ...
    var getSeverityBadge = function (severity) {
        ...
        return (<badge_1.Badge variant={variants[severity] || 'default'}>
    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent>
        {events.length === 0 ? (<p className="text-muted-foreground text-center py-4">
            Nenhum evento de segurança recente
          </p>) : (<div className="space-y-3">
            {events.map(function (event) { return (<div key={event.id} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <div className="flex items-center space-x-2">
                  {event.resolved ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.XCircle className="h-4 w-4 text-red-600"/>)}
                </div>
              </div>); })}
          </div>)}
      </card_1.CardContent>
