...
function AlertsPanel(_a) {
    ...
    var _c = (0, react_1.useState)(false), showAcknowledged = _c[0], setShowAcknowledged = _c[1];
    /**
     * Process alerts with additional metadata
     */
    var processedAlerts = (0, react_1.useMemo)(function () {
        return alerts.map(function (alert) {
            ...
            return __assign(__assign({}, alert), { categoryIcon: categoryIcon, severityColor: severityColor, timeAgo: timeAgo, isRecent: isRecent });
        });
    }, [alerts]);
    /**
     * Filter alerts
     ...
     */
    var alertSummary = (0, react_1.useMemo)(function () {
        return processedAlerts.reduce(function (summary, alert) { return ({
     ...
     */
    var alertsByCategory = (0, react_1.useMemo)(function () {
    ...
    var AlertCard = function (_a) {
        var alert = _a.alert;
        return (<card_1.Card className={"border-l-4 ".concat(alert.severityColor, " ").concat(alert.isRecent ? 'ring-2 ring-blue-100' : '')}>
      <card_1.CardContent className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {alert.isRecent && (<badge_1.Badge variant="secondary" className="text-xs">
                    New
                  </badge_1.Badge>)}
              </div>
              ...
              <div className="text-xs text-gray-500 space-y-1">
                ...
                {alert.affectedResources && alert.affectedResources.length > 0 && (<div>
                    Affected: {alert.affectedResources.slice(0, 2).join(', ')}
                    {alert.affectedResources.length > 2 && " +".concat(alert.affectedResources.length - 2, " more")}
                  </div>)}
              </div>

              {/* Resolution Steps */}
              {alert.resolutionSteps && alert.resolutionSteps.length > 0 && (<div className="mt-3 p-2 bg-gray-50 rounded text-xs">
                  <div className="font-medium text-gray-700 mb-1">Resolution Steps:</div>
                  <ol className="list-decimal list-inside space-y-0.5 text-gray-600">
                    {alert.resolutionSteps.slice(0, 2).map(function (step, index) { return (<li key={index}>{step}</li>); })}
                  </ol>
                </div>)}
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <badge_1.Badge variant={alert.severity === 'critical' ? 'destructive' : 'secondary'} className="text-xs">
              {alert.severity}
            </badge_1.Badge>
            
            {!alert.acknowledged && (<button_1.Button size="sm" variant="outline" onClick={function () { return handleAcknowledge(alert.id); }} className="text-xs">
                <lucide_react_1.CheckCircle className="w-3 h-3 mr-1"/>
                Acknowledge
              </button_1.Button>)}
            
            {alert.acknowledged && (<badge_1.Badge variant="secondary" className="text-xs">
                <lucide_react_1.CheckCircle className="w-3 h-3 mr-1"/>
                Acknowledged
              </badge_1.Badge>)}
          </div>
    ...
    if (isLoading) {
        ...
        <card_1.CardContent>
          <div className="animate-pulse space-y-4">
            {__spreadArray([], Array(3), true).map(function (_, i) { return (<div key={i} className="h-24 bg-gray-200 rounded"></div>); })}
          </div>
        </card_1.CardContent>
      </card_1.Card>);
    }
    return (<card_1.Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <card_1.CardTitle className="flex items-center gap-2">
              <lucide_react_1.Bell className="w-5 h-5 text-orange-600"/>
              System Alerts
              {alertSummary.total > 0 && (<badge_1.Badge variant={alertSummary.critical > 0 ? 'destructive' : 'secondary'}>
                  {alertSummary.total}
                </badge_1.Badge>)}
            </card_1.CardTitle>
          ...
          <div className="flex items-center gap-2">
            <button_1.Button variant="ghost" size="sm" onClick={function () { return setShowAcknowledged(!showAcknowledged); }}>
      ...
      <card_1.CardContent>
        ...
        {alertSummary.total === 0 ? (<div className="text-center py-8">
          ...
          </div>) : (<tabs_1.Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
            <tabs_1.TabsList className="grid w-full grid-cols-6">
              ...
              {alertsByCategory.slice(0, 5).map(function (_a) {
                var category = _a.category, count = _a.count, criticalCount = _a.criticalCount;
                return (<tabs_1.TabsTrigger key={category} value={category} className="flex items-center gap-1">
                  <span className="hidden lg:inline">
                    {category.charAt(0).toUpperCase() + category.slice(1)}
                  </span>
                  <badge_1.Badge variant={criticalCount > 0 ? 'destructive' : 'secondary'} className="text-xs">
                    {count}
                  </badge_1.Badge>
                </tabs_1.TabsTrigger>);
            })}
            ...
            <tabs_1.TabsContent value={selectedCategory} className="mt-6">
              <scroll_area_1.ScrollArea className="h-96">
                ...
                .map(function (alert, index) { return (<AlertCard key={"".concat(alert.id, "-").concat(index)} alert={alert}/>); }))}
                </div>
