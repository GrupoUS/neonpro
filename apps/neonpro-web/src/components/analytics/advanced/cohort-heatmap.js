...
function CohortHeatmap(_a) {
    var cohorts = _a.cohorts, _b = _a.loading, loading = _b === void 0 ? false : _b, _c = _a.className, className = _c === void 0 ? '' : _c, onCohortSelect = _a.onCohortSelect, onPeriodSelect = _a.onPeriodSelect;
    var _d = (0, react_1.useState)('retention'), selectedMetric = _d[0], setSelectedMetric = _d[1];
    var _e = (0, react_1.useState)('heatmap'), selectedView = _e[0], setSelectedView = _e[1];
    var _f = (0, react_1.useState)(null), hoveredCell = _f[0], setHoveredCell = _f[1];
    // Transform data for heatmap visualization
    var heatmapData = (0, react_1.useMemo)(function () {
        if (!cohorts.length)
            return [];
        var maxPeriods = Math.max.apply(Math, cohorts.map(function (c) { return c.periods.length; }));
        var data = [];
    ...
    var trendData = (0, react_1.useMemo)(function () {
        return cohorts.map(function (cohort) {
            var _a;
            var periods = cohort.periods.map(function (period, index) { return ({
                period: index,
            ...
            return {
                ...
                periods: periods,
                avgRetention: periods.reduce(function (sum, p) { return sum + p.retention; }, 0) / periods.length,
                totalRevenue: periods.reduce(function (sum, p) { return sum + p.revenue; }, 0),
                finalRetention: ((_a = periods[periods.length - 1]) === null || _a === void 0 ? void 0 : _a.retention) || 0
            };
        });
    }, [cohorts]);
    // Calculate cohort comparison metrics
    var comparisonData = (0, react_1.useMemo)(function () {
        return cohorts.map(function (cohort) {
            ...
            return {
                ...
                finalRetention: (finalPeriod === null || finalPeriod === void 0 ? void 0 : finalPeriod.retentionRate) || 0,
                totalRevenue: periods.reduce(function (sum, p) { return sum + p.revenue; }, 0),
                avgRevenuePerUser: periods.reduce(function (sum, p) { return sum + p.averageRevenuePerUser; }, 0) / periods.length,
                performanceScore: ((finalPeriod === null || finalPeriod === void 0 ? void 0 : finalPeriod.retentionRate) || 0) * 0.6 +
                    (periods.reduce(function (sum, p) { return sum + p.averageRevenuePerUser; }, 0) / periods.length) * 0.4
            };
        }).sort(function (a, b) { return b.performanceScore - a.performanceScore; });
    }, [cohorts]);
    ...
    var HeatmapTooltip = function (_a) {
        ...
        {payload.map(function (entry, index) { return (<div key={index} className="flex items-center gap-2 mt-1">
      ...
      <card_1.CardContent>
        <tabs_1.Tabs value={selectedView} onValueChange={function (value) { return setSelectedView(value); }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-900">
                    {cohorts.length > 0
            ? "".concat((trendData.reduce(function (sum, c) { return sum + c.avgRetention; }, 0) / trendData.length).toFixed(1), "%")
            : '0%'}
                  </p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-purple-900">
                    ${trendData.reduce(function (sum, c) { return sum + c.totalRevenue; }, 0).toLocaleString()}
                  </p>
                </div>
                <div className="bg-orange-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-orange-900">
                    {cohorts.length > 0
            ? Math.round(cohorts.reduce(function (sum, c) { return sum + c.periods.length; }, 0) / cohorts.length)
            : 0}
                  </p>
                </div>
              </div>
              ...
              <div className="h-96">
                <recharts_1.ResponsiveContainer width="100%" height="100%">
                  <recharts_1.AreaChart data={heatmapData}>
                    ...
                    <recharts_1.Legend />
                    {cohorts.map(function (cohort, index) { return (<recharts_1.Area key={cohort.cohortId} type="monotone" dataKey={cohort.cohortName} stackId="1" stroke={"hsl(".concat(index * 360 / cohorts.length, ", 70%, 50%)")} fill={"hsl(".concat(index * 360 / cohorts.length, ", 70%, 50%)")} fillOpacity={0.6}/>); })}
          ...
          <tabs_1.TabsContent value="trends" className="mt-6">
              ...
              <div className="h-96">
                <recharts_1.ResponsiveContainer width="100%" height="100%">
                  <recharts_1.LineChart>
                    ...
                    <recharts_1.Legend />
                    {trendData.map(function (cohort, index) { return (<recharts_1.Line key={cohort.cohortId} type="monotone" dataKey={selectedMetric} data={cohort.periods} stroke={"hsl(".concat(index * 360 / trendData.length, ", 70%, 50%)")} strokeWidth={2} name={cohort.cohortName}/>); })}
                  </recharts_1.LineChart>
                </recharts_1.ResponsiveContainer>
              </div>
            ...
            var _a;
            return (<card_1.Card key={cohort.cohortId} className="border-l-4 border-l-blue-500">
                    <card_1.CardHeader className="pb-2">
                      <card_1.CardTitle className="text-sm font-medium">{cohort.cohortName}</card_1.CardTitle>
                    ...
                    <card_1.CardContent className="pt-2">
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <badge_1.Badge variant={cohort.avgRetention >= 50 ? "default" : "secondary"}>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-1">
                            {cohort.finalRetention > ((_a = cohort.periods[0]) === null || _a === void 0 ? void 0 : _a.retention) ? (<lucide_react_1.TrendingUp className="h-3 w-3 text-green-500"/>) : (<lucide_react_1.TrendingDown className="h-3 w-3 text-red-500"/>)}
                            <span className="text-sm font-medium">
              ...
              <div className="h-96">
                <recharts_1.ResponsiveContainer width="100%" height="100%">
                  <recharts_1.BarChart data={comparisonData}>
                    <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                    <recharts_1.XAxis dataKey="cohortName" angle={-45} textAnchor="end" height={80}/>
                    <recharts_1.YAxis />
                    <recharts_1.Tooltip formatter={function (value, name) { return [
            name === 'finalRetention' ? "".concat(value.toFixed(1), "%") :
                name.includes('Revenue') ? "$".concat(value.toFixed(2)) :
                    value.toLocaleString(),
            name
        ]; }}/>
                    <recharts_1.Legend />
                    <recharts_1.Bar dataKey="finalRetention" fill="#3b82f6" name="Final Retention %"/>
                    <recharts_1.Bar dataKey="avgRevenuePerUser" fill="#10b981" name="Avg Revenue Per User"/>
                  </recharts_1.BarChart>
                </recharts_1.ResponsiveContainer>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {comparisonData.map(function (cohort, index) { return (<div key={cohort.cohortName} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
