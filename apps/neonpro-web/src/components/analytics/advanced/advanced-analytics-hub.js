...
function AdvancedAnalyticsHub(_a) {
    var _this = this;
    var _b, _c;
    var _d = _a.initialConfig, initialConfig = _d === void 0 ? {} : _d, _e = _a.className, className = _e === void 0 ? '' : _e, onConfigChange = _a.onConfigChange, onDataExport = _a.onDataExport;
    // State management
    var _f = (0, react_1.useState)(__assign({ dateRange: {
            start: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            end: new Date().toISOString().split('T')[0]
        }, refreshInterval: 300000, autoRefresh: false, selectedMetrics: ['subscriptions', 'revenue', 'churn_rate', 'mrr'], confidenceLevel: 95 }, initialConfig)), config = _f[0], setConfig = _f[1];
    var _g = (0, react_1.useState)({
        ...
        predictiveModels: []
    }), analyticsData = _g[0], setAnalyticsData = _g[1];
    var _h = (0, react_1.useState)(false), loading = _h[0], setLoading = _h[1];
    var _j = (0, react_1.useState)(null), error = _j[0], setError = _j[1];
    var _k = (0, react_1.useState)(null), lastRefresh = _k[0], setLastRefresh = _k[1];
    var _l = (0, react_1.useState)('overview'), selectedView = _l[0], setSelectedView = _l[1];
    // Fetch analytics data
    var fetchAnalyticsData = (0, react_1.useCallback)(function () {
        var args_1 = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            args_1[_i] = arguments[_i];
        }
        return __awaiter(_this, __spreadArray([], args_1, true), void 0, function (showLoading) {
            var cohortResponse, forecastResponse, statisticsResponse, _a, cohortData, forecastData, statisticsData, kpis, timeSeriesData, segmentationData, benchmarkData, err_1;
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, fetch('/api/analytics/advanced?type=cohort-analysis', {
                                method: 'POST',
                    ...
                    case 2:
                        ...
                        return [4 /*yield*/, fetch('/api/analytics/advanced?type=forecasting', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    metric: 'revenue',
                    ...
                    case 3:
                        ...
                        return [4 /*yield*/, fetch('/api/analytics/advanced?type=statistical-analysis', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                    ...
                    case 5:
                        _a = _b.sent(), cohortData = _a[0], forecastData = _a[1], statisticsData = _a[2];
    ...
    var analyticsSummary = (0, react_1.useMemo)(function () {
        var _a;
        ...
        return {
            ...
            bestModel: bestModel,
            outliers: ((_a = analyticsData.dataQuality.outliers) === null || _a === void 0 ? void 0 : _a.filter(function (o) { return o.isOutlier; }).length) || 0
        };
    }, [analyticsData]);
    // Event handlers
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              {Array.from({ length: 4 }).map(function (_, i) { return (<div key={i} className="h-24 bg-gray-100 rounded animate-pulse"/>); })}
            </div>
    ...
    return (<div className={"w-full space-y-6 ".concat(className)}>
        ...
        <card_1.CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {lastRefresh && (<badge_1.Badge variant="secondary">
        ...
        <card_1.CardContent>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <select_1.Select value={selectedView} onValueChange={function (value) { return setSelectedView(value); }}>
              ...
              <select_1.Select value={config.confidenceLevel.toString()} onValueChange={function (value) { return handleConfigUpdate({ confidenceLevel: parseInt(value) }); }}>
            ...
            <div className="flex items-center gap-2">
              <button_1.Button variant={config.autoRefresh ? "default" : "outline"} size="sm" onClick={function () { return handleConfigUpdate({ autoRefresh: !config.autoRefresh }); }}>
              ...
              <button_1.Button variant="outline" size="sm" onClick={function () { return handleExport('csv'); }}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-orange-900">
                  {(((_b = analyticsSummary.bestModel) === null || _b === void 0 ? void 0 : _b.accuracy) || 0).toFixed(1)}%
                </p>
                <p className="text-xs text-orange-700 capitalize">
                  {((_c = analyticsSummary.bestModel) === null || _c === void 0 ? void 0 : _c.modelType) || 'N/A'}
                </p>
              </div>
      ...
      {selectedView === 'overview' && (<div className="space-y-6">
          ...
          {/* Mini Dashboard */}
          <advanced_metrics_dashboard_1.AdvancedMetricsDashboard kpis={analyticsData.kpis} timeSeriesData={analyticsData.timeSeriesData} segmentationData={analyticsData.segmentationData} benchmarkData={analyticsData.benchmarkData} loading={loading} dateRange={config.dateRange} onDateRangeChange={handleDateRangeChange} onExport={handleExport} onRefresh={handleRefresh} className="h-96"/>
        </div>)}

      {selectedView === 'cohorts' && (<cohort_heatmap_1.CohortHeatmap cohortData={analyticsData.cohortData} loading={loading} onDateRangeChange={handleDateRangeChange} onExport={handleExport}/>)}

      {selectedView === 'forecasting' && (<forecasting_charts_1.ForecastingCharts metric="revenue" historicalData={analyticsData.timeSeriesData.map(function (d) { return ({ date: d.date, value: d.revenue }); })} forecastData={analyticsData.forecastData} loading={loading} onDateRangeChange={handleDateRangeChange}/>)}

      {selectedView === 'metrics' && (<advanced_metrics_dashboard_1.AdvancedMetricsDashboard kpis={analyticsData.kpis} timeSeriesData={analyticsData.timeSeriesData} segmentationData={analyticsData.segmentationData} benchmarkData={analyticsData.benchmarkData} loading={loading} dateRange={config.dateRange} onDateRangeChange={handleDateRangeChange} onExport={handleExport} onRefresh={handleRefresh}/>)}

      {selectedView === 'statistics' && (<statistical_insights_1.StatisticalInsights correlations={analyticsData.correlations} statisticalTests={analyticsData.statisticalTests} dataQuality={analyticsData.dataQuality} predictiveModels={analyticsData.predictiveModels} rawData={analyticsData.timeSeriesData} loading={loading} onDataRefresh={handleRefresh} onExportResults={handleExport}/>)}
      ...
      <card_1.Card>
        <card_1.CardContent className="p-4">
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <div className="flex items-center gap-2">
              {analyticsSummary.outliers > 0 && (<badge_1.Badge variant="destructive">
                  {analyticsSummary.outliers} outliers detected
                </badge_1.Badge>)}
              <badge_1.Badge variant={analyticsSummary.overallDataQuality >= 80 ? 'default' : 'secondary'}>
