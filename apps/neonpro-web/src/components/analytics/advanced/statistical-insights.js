...
var calculateCorrelationMatrix = function (data) {
    if (data.length === 0)
        return [];
    var keys = Object.keys(data[0]).filter(function (key) { return typeof data[0][key] === 'number'; });
    var matrix = [];
    keys.forEach(function (key1) {
        keys.forEach(function (key2) {
            var values1 = data.map(function (d) { return d[key1]; }).filter(function (v) { return !isNaN(v); });
            var values2 = data.map(function (d) { return d[key2]; }).filter(function (v) { return !isNaN(v); });
...
function StatisticalInsights(_a) {
    var _b, _c;
    var correlations = _a.correlations, regressionResults = _a.regressionResults, statisticalTests = _a.statisticalTests, dataQuality = _a.dataQuality, predictiveModels = _a.predictiveModels, rawData = _a.rawData, _d = _a.loading, loading = _d === void 0 ? false : _d, _e = _a.className, className = _e === void 0 ? '' : _e, onModelRetrain = _a.onModelRetrain, onDataRefresh = _a.onDataRefresh, onExportResults = _a.onExportResults;
    var _f = (0, react_1.useState)('correlations'), selectedView = _f[0], setSelectedView = _f[1];
    var _g = (0, react_1.useState)(null), selectedCorrelation = _g[0], setSelectedCorrelation = _g[1];
    var _h = (0, react_1.useState)(95), confidenceLevel = _h[0], setConfidenceLevel = _h[1];
    var _j = (0, react_1.useState)(false), showOnlySignificant = _j[0], setShowOnlySignificant = _j[1];
    // Calculate correlation matrix from raw data
    var correlationMatrix = (0, react_1.useMemo)(function () {
        return calculateCorrelationMatrix(rawData);
    }, [rawData]);
    // Filter significant correlations
    var significantCorrelations = (0, react_1.useMemo)(function () {
    ...
    var statisticalSummary = (0, react_1.useMemo)(function () {
        var strongCorrelations = correlations.filter(function (c) { return Math.abs(c.correlation) >= 0.6; }).length;
        var significantTests = statisticalTests.filter(function (t) { return t.result === 'reject'; }).length;
        var overallDataQuality = (dataQuality.completeness +
            dataQuality.accuracy +
            dataQuality.consistency +
            dataQuality.validity +
            dataQuality.uniqueness) / 5;
        var bestModel = predictiveModels.reduce(function (best, model) {
            return model.accuracy > ((best === null || best === void 0 ? void 0 : best.accuracy) || 0) ? model : best;
        }, predictiveModels[0]);
        return {
            ...
            bestModel: bestModel,
            totalOutliers: dataQuality.outliers.filter(function (o) { return o.isOutlier; }).length
        };
    ...
    if (loading) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map(function (_, i) { return (<card_1.Card key={i} className="animate-pulse">
    ...
    return (<div className={"w-full space-y-6 ".concat(className)}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-orange-900">
                  {(((_b = statisticalSummary.bestModel) === null || _b === void 0 ? void 0 : _b.accuracy) || 0).toFixed(1)}%
                </p>
                <p className="text-xs text-orange-700 capitalize">
                  {((_c = statisticalSummary.bestModel) === null || _c === void 0 ? void 0 : _c.modelType) || 'N/A'}
                </p>
              </div>
      ...
      <tabs_1.Tabs value={selectedView} onValueChange={function (value) { return setSelectedView(value); }}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button_1.Button variant={showOnlySignificant ? "default" : "outline"} size="sm" onClick={function () { return setShowOnlySignificant(!showOnlySignificant); }}>
            ...
            <button_1.Button variant="outline" size="sm" onClick={function () { return onExportResults === null || onExportResults === void 0 ? void 0 : onExportResults('csv'); }}>
        ...
        <tabs_1.TabsContent value="correlations" className="mt-6">
          <div className="space-y-6">
                  ...
                  <recharts_1.ResponsiveContainer width="100%" height="100%">
                    <recharts_1.ScatterChart>
                      ...
                      <recharts_1.Scatter data={correlationMatrix} fill="#8884d8">
                        {correlationMatrix.map(function (entry, index) { return (<recharts_1.Cell key={index} fill={entry.value > 0 ?
            ...
            return (<div key={"".concat(correlation.variable1, "-").concat(correlation.variable2)} className="p-4 border rounded-lg hover:bg-gray-50 cursor-pointer" onClick={function () { return setSelectedCorrelation(selectedCorrelation === "".concat(correlation.variable1, "-").concat(correlation.variable2)
                    ? null
                    : "".concat(correlation.variable1, "-").concat(correlation.variable2)); }}>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <badge_1.Badge variant={significance.level === 'ns' ? 'secondary' : 'default'} className={significance.color}>
                              {significance.level}
                            </badge_1.Badge>
                          </div>
                        </div>
                        
                        {selectedCorrelation === "".concat(correlation.variable1, "-").concat(correlation.variable2) && (<div className="mt-4 pt-4 border-t">
                        ...
                        : correlation.correlation > 0
                            ...
                            : "Negative relationship: as one variable increases, the other tends to decrease."}
                              ...
                              <div>
                                ...
                                <p className="text-gray-600 mt-1">
                                  {significance.description}. {correlation.pValue < 0.05
        ...
        <tabs_1.TabsContent value="regression" className="mt-6">
          {regressionResults ? (<div className="space-y-6">
              {/* Regression Summary */}
              <card_1.Card>
                <card_1.CardHeader>
                  <card_1.CardTitle className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-green-50 p-4 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-green-900 mt-1">
                        {regressionResults.coefficients.filter(function (c) { return c.significance; }).length}
                      </p>
              ...
              <card_1.Card>
                ...
                return (<div key={coeff.variable} className="flex items-center justify-between p-3 border rounded">
                          <div>
                          ...
                          <div className="text-right">
                            <badge_1.Badge variant={coeff.significance ? 'default' : 'secondary'} className={significance.color}>
                              {significance.level}
                            </badge_1.Badge>
                  ...
                  <div className="h-80">
                    <recharts_1.ResponsiveContainer width="100%" height="100%">
                      <recharts_1.ScatterChart data={regressionResults.predictions}>
                        <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                        <recharts_1.XAxis dataKey="predicted" name="Predicted" label={{ value: 'Predicted Values', position: 'insideBottom', offset: -5 }}/>
                        <recharts_1.YAxis dataKey="residual" name="Residual" label={{ value: 'Residuals', angle: -90, position: 'insideLeft' }}/>
        ...
        <tabs_1.TabsContent value="tests" className="mt-6">
          <div className="space-y-6">
            {statisticalTests.map(function (test, index) { return (<card_1.Card key={index}>
                <card_1.CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <badge_1.Badge variant={test.result === 'reject' ? 'default' : 'secondary'} className={test.result === 'reject' ? 'text-green-600' : 'text-gray-600'}>
            ...
            {statisticalTests.length === 0 && (<card_1.Card>
                <card_1.CardContent className="text-center py-12">
                  <lucide_react_1.Calculator className="h-12 w-12 text-gray-400 mx-auto mb-4"/>
        ...
        <tabs_1.TabsContent value="quality" className="mt-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {[
            { name: 'Completeness', value: dataQuality.completeness, icon: lucide_react_1.CheckCircle },
            { name: 'Accuracy', value: dataQuality.accuracy, icon: lucide_react_1.Target },
            { name: 'Consistency', value: dataQuality.consistency, icon: lucide_react_1.Layers },
            ...
            return (<card_1.Card key={metric.name}>
                    <card_1.CardContent className="p-6">
                      <div className="flex items-center justify-between mb-2">
                        <Icon className={"h-5 w-5 ".concat(getQualityColor(metric.value))}/>
                        <span className={"text-2xl font-bold ".concat(getQualityColor(metric.value))}>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                {dataQuality.outliers.length > 0 ? (<div className="space-y-3">
                    {dataQuality.outliers
                .filter(function (outlier) { return outlier.isOutlier; })
                .map(function (outlier, index) { return (<div key={index} className="flex items-center justify-between p-3 border rounded">
                          <div>
        ...
        <tabs_1.TabsContent value="models" className="mt-6">
          <div className="space-y-6">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {predictiveModels.map(function (model, index) { return (<div key={index} className="p-4 border rounded-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        {model.featureImportance.slice(0, 3).map(function (feature, idx) { return (<div key={idx} className="flex justify-between items-center text-sm">
                            <span className="text-gray-600">{feature.feature}</span>
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-2 bg-gray-200 rounded">
                                <div className="h-full bg-blue-500 rounded" style={{ width: "".concat(feature.importance * 100, "%") }}/>
                              </div>
                      ...
                      <div className="mt-3 flex gap-2">
                        <button_1.Button size="sm" variant="outline" onClick={function () { return onModelRetrain === null || onModelRetrain === void 0 ? void 0 : onModelRetrain(model.modelType); }}>
                          Retrain Model
                        </button_1.Button>
                      </div>
                    </div>); })}
            ...
            {predictiveModels.length === 0 && (<card_1.Card>
                <card_1.CardContent className="text-center py-12">
      ...
      {statisticalSummary.strongCorrelations > 0 && (<alert_1.Alert>
