...
var JourneyAnalyticsDashboard = function () {
    ...
    var _b = (0, react_1.useState)([]), journeyStages = _b[0], setJourneyStages = _b[1];
    var _c = (0, react_1.useState)([]), patientFlow = _c[0], setPatientFlow = _c[1];
    var _d = (0, react_1.useState)([]), churnRisks = _d[0], setChurnRisks = _d[1];
    var _e = (0, react_1.useState)([]), recommendations = _e[0], setRecommendations = _e[1];
    var _f = (0, react_1.useState)([]), satisfactionMetrics = _f[0], setSatisfactionMetrics = _f[1];
    var _g = (0, react_1.useState)([]), channelPerformance = _g[0], setChannelPerformance = _g[1];
    var _h = (0, react_1.useState)([]), realTimeEvents = _h[0], setRealTimeEvents = _h[1];
    ...
    var _m = (0, react_1.useState)('30d'), selectedTimeRange = _m[0], setSelectedTimeRange = _m[1];
    var _o = (0, react_1.useState)('all'), selectedStage = _o[0], setSelectedStage = _o[1];
    var _p = (0, react_1.useState)('all'), selectedChannel = _p[0], setSelectedChannel = _p[1];
    var _q = (0, react_1.useState)({
        from: (0, date_fns_1.subDays)(new Date(), 30),
        to: new Date()
    }), dateRange = _q[0], setDateRange = _q[1];
    var _r = (0, react_1.useState)('overview'), activeTab = _r[0], setActiveTab = _r[1];
    var _s = (0, react_1.useState)(''), searchTerm = _s[0], setSearchTerm = _s[1];
    var _t = (0, react_1.useState)({
        riskLevel: 'all',
        priority: 'all',
        satisfaction: 'all'
    }), filters = _t[0], setFilters = _t[1];
    // Refs for real-time updates
    var eventSourceRef = (0, react_1.useRef)(null);
    var refreshIntervalRef = (0, react_1.useRef)(null);
    // Memoized filtered data
    var filteredChurnRisks = (0, react_1.useMemo)(function () {
    ...
    var filteredRecommendations = (0, react_1.useMemo)(function () {
        return recommendations.filter(function (rec) {
            var matchesSearch = rec.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    var getPriorityBadge = function (priority) {
        switch (priority) {
            ...
            case 'MEDIUM':
                return <badge_1.Badge variant="secondary" className="bg-blue-100 text-blue-800">Média</badge_1.Badge>;
            case 'HIGH':
                return <badge_1.Badge variant="secondary" className="bg-orange-100 text-orange-800">Alta</badge_1.Badge>;
            case 'CRITICAL':
    ...
    var fetchDashboardData = (0, react_1.useCallback)(function () { return __awaiter(void 0, void 0, void 0, function () {
        var params, _a, metricsRes, stagesRes, flowRes, churnRes, recommendationsRes, satisfactionRes, channelsRes, _b, metricsData, stagesData, flowData, churnData, recommendationsData, satisfactionData, channelsData, err_1;
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    _a = _c.sent(), metricsRes = _a[0], stagesRes = _a[1], flowRes = _a[2], churnRes = _a[3], recommendationsRes = _a[4], satisfactionRes = _a[5], channelsRes = _a[6];
                    if (!metricsRes.ok)
                ...
                case 2:
                    _b = _c.sent(), metricsData = _b[0], stagesData = _b[1], flowData = _b[2], churnData = _b[3], recommendationsData = _b[4], satisfactionData = _b[5], channelsData = _b[6];
                    ...
                    setChannelPerformance(channelsData.map(function (channel) { return (__assign(__assign({}, channel), { icon: channelIcons[channel.channel] || <lucide_react_1.Globe className="h-4 w-4"/> })); }));
                    return [3 /*break*/, 4];
                case 3:
                    err_1 = _c.sent();
    ...
    var setupRealTimeUpdates = (0, react_1.useCallback)(function () {
        ...
        eventSourceRef.current.onmessage = function (event) {
            ...
            setRealTimeEvents(function (prev) { return __spreadArray([newEvent], prev.slice(0, 49), true); });
        };
    ...
    (0, react_1.useEffect)(function () {
        setLoading(true);
        fetchDashboardData().finally(function () { return setLoading(false); });
    }, [fetchDashboardData]);
    (0, react_1.useEffect)(function () {
    ...
    var handleExport = function (format) { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    return [4 /*yield*/, fetch('/api/analytics/journey/export', {
                            method: 'POST',
                ...
                case 2:
                    ...
                    a.href = url;
                    a.download = "journey-analytics-".concat(format, ".").concat(format === 'excel' ? 'xlsx' : format);
    ...
    if (error) {
        return (<alert_1.Alert variant="destructive" className="m-6">
    ...
    return (<div className="space-y-6 p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button_1.Button onClick={refreshData} disabled={refreshing} variant="outline" size="sm">
            <lucide_react_1.RefreshCw className={(0, utils_1.cn)("h-4 w-4", refreshing && "animate-spin")}/>
          </button_1.Button>
            ...
            <popover_1.PopoverContent>
              <div className="space-y-2">
                ...
                <div className="flex flex-col space-y-1">
                  <button_1.Button onClick={function () { return handleExport('pdf'); }} variant="ghost" size="sm">
                    Exportar PDF
                  </button_1.Button>
                  <button_1.Button onClick={function () { return handleExport('excel'); }} variant="ghost" size="sm">
                    Exportar Excel
                  </button_1.Button>
                  <button_1.Button onClick={function () { return handleExport('png'); }} variant="ghost" size="sm">
                    Exportar Imagem
                  </button_1.Button>
                </div>
              </div>
      ...
      {metrics && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                <span className={(0, utils_1.cn)("inline-flex items-center", metrics.trends.patients >= 0 ? "text-green-600" : "text-red-600")}>
                  {metrics.trends.patients >= 0 ? (<lucide_react_1.ArrowUpRight className="h-3 w-3 mr-1"/>) : (<lucide_react_1.ArrowDownRight className="h-3 w-3 mr-1"/>)}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                <span className={(0, utils_1.cn)("inline-flex items-center", metrics.trends.satisfaction >= 0 ? "text-green-600" : "text-red-600")}>
                  {metrics.trends.satisfaction >= 0 ? (<lucide_react_1.ArrowUpRight className="h-3 w-3 mr-1"/>) : (<lucide_react_1.ArrowDownRight className="h-3 w-3 mr-1"/>)}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                <span className={(0, utils_1.cn)("inline-flex items-center", metrics.trends.conversion >= 0 ? "text-green-600" : "text-red-600")}>
                  {metrics.trends.conversion >= 0 ? (<lucide_react_1.ArrowUpRight className="h-3 w-3 mr-1"/>) : (<lucide_react_1.ArrowDownRight className="h-3 w-3 mr-1"/>)}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                <span className={(0, utils_1.cn)("inline-flex items-center", metrics.trends.churn <= 0 ? "text-green-600" : "text-red-600")}>
                  {metrics.trends.churn <= 0 ? (<lucide_react_1.ArrowDownRight className="h-3 w-3 mr-1"/>) : (<lucide_react_1.ArrowUpRight className="h-3 w-3 mr-1"/>)}
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <tabs_1.TabsContent value="overview" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Distribuição de pacientes por estágio da jornada</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-4">
                  {patientFlow.map(function (stage, index) { return (<div key={index} className="flex items-center space-x-4">
                      ...
                      <div className={(0, utils_1.cn)("text-xs", stage.trend >= 0 ? "text-green-600" : "text-red-600")}>
            ...
            <card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Análise de performance por canal de comunicação</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-4">
                  {channelPerformance.map(function (channel, index) { return (<div key={index} className="flex items-center space-x-4 p-3 rounded-lg bg-muted/50">
                      ...
                      <div className={(0, utils_1.cn)("text-xs", channel.trend >= 0 ? "text-green-600" : "text-red-600")}>
          ...
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Últimas atividades da jornada dos pacientes</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <scroll_area_1.ScrollArea className="h-64">
                <div className="space-y-2">
                  {realTimeEvents.map(function (event, index) { return (<div key={event.id} className="flex items-start space-x-3 p-2 rounded-lg hover:bg-muted/50">
                      <div className={(0, utils_1.cn)("w-2 h-2 rounded-full mt-2", event.severity === 'ERROR' && "bg-red-500", event.severity === 'WARNING' && "bg-yellow-500", event.severity === 'INFO' && "bg-blue-500")}/>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                          <span className="font-medium text-sm">{event.patientName}</span>
                          <badge_1.Badge variant="outline" className="text-xs">{event.touchpoint}</badge_1.Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">{event.message}</p>
                        <p className="text-xs text-muted-foreground">
                          {(0, date_fns_1.format)(event.timestamp, "HH:mm:ss", { locale: locale_1.ptBR })}
                        </p>
                      </div>
                    </div>); })}
                </div>
        ...
        <tabs_1.TabsContent value="journey" className="space-y-4">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Análise detalhada de cada estágio da jornada</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-6">
                {journeyStages.map(function (stage, index) { return (<div key={stage.id} className="relative">
                    {index < journeyStages.length - 1 && (<div className="absolute left-6 top-12 w-px h-12 bg-border"/>)}
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {stage.touchpoints.map(function (touchpoint) { return (<div key={touchpoint.id} className="p-3 border rounded-lg">
                              <div className="flex items-center justify-between mb-2">
                                <span className="font-medium text-sm">{touchpoint.name}</span>
                                <badge_1.Badge variant="outline" className="text-xs">{touchpoint.channel}</badge_1.Badge>
                              </div>
        ...
        <tabs_1.TabsContent value="satisfaction" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {satisfactionMetrics.map(function (metric, index) { return (<card_1.Card key={index}>
                <card_1.CardHeader>
                  <card_1.CardTitle className="flex items-center justify-between">
                    <span>{metric.category}</span>
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold">{metric.score.toFixed(1)}</span>
                      <badge_1.Badge variant={metric.score >= metric.target ? "default" : "destructive"}>
                ...
                height: "".concat(Math.max(4, (metric.breakdown.terrible / metric.responses) * 40), "px")
            }}/>
                      </div>
                      <div className="space-y-1">
                ...
                height: "".concat(Math.max(4, (metric.breakdown.poor / metric.responses) * 40), "px")
            }}/>
                      </div>
                      <div className="space-y-1">
                ...
                height: "".concat(Math.max(4, (metric.breakdown.average / metric.responses) * 40), "px")
            }}/>
                      </div>
                      <div className="space-y-1">
                ...
                height: "".concat(Math.max(4, (metric.breakdown.good / metric.responses) * 40), "px")
            }}/>
                      </div>
                      <div className="space-y-1">
                ...
                height: "".concat(Math.max(4, (metric.breakdown.excellent / metric.responses) * 40), "px")
            }}/>
                      </div>
                    </div>
                  </div>
        ...
        <tabs_1.TabsContent value="churn" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <input_1.Input placeholder="Buscar paciente..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="w-64"/>
              <select_1.Select value={filters.riskLevel} onValueChange={function (value) { return setFilters(function (prev) { return (__assign(__assign({}, prev), { riskLevel: value })); }); }}>
                <select_1.SelectTrigger className="w-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredChurnRisks.map(function (risk) { return (<card_1.Card key={risk.patientId} className="relative">
                ...
                <card_1.CardContent className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {risk.factors.map(function (factor, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                          {factor}
                        </badge_1.Badge>); })}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm font-medium">
                      {(0, date_fns_1.format)(risk.lastActivity, "dd/MM/yyyy HH:mm", { locale: locale_1.ptBR })}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm font-medium text-red-600">
                      {(0, date_fns_1.format)(risk.predictedChurnDate, "dd/MM/yyyy", { locale: locale_1.ptBR })}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 mt-1">
                      {risk.recommendedActions.slice(0, 2).map(function (action, index) { return (<div key={index} className="text-xs p-2 bg-muted rounded text-muted-foreground">
                          • {action}
                        </div>); })}
                    </div>
                  </div>
        ...
        <tabs_1.TabsContent value="optimization" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <input_1.Input placeholder="Buscar recomendação..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="w-64"/>
              <select_1.Select value={filters.priority} onValueChange={function (value) { return setFilters(function (prev) { return (__assign(__assign({}, prev), { priority: value })); }); }}>
                <select_1.SelectTrigger className="w-32">
          ...
          <div className="space-y-4">
            {filteredRecommendations.map(function (rec) { return (<card_1.Card key={rec.id}>
                <card_1.CardHeader>
                ...
                <card_1.CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {rec.touchpoints.map(function (touchpoint, index) { return (<badge_1.Badge key={index} variant="secondary" className="text-xs">
                          {touchpoint}
                        </badge_1.Badge>); })}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {rec.implementation.map(function (step, index) { return (<div key={index} className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {rec.metrics.map(function (metric, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                          {metric}
                        </badge_1.Badge>); })}
                    </div>
                  </div>
