...
function SubscriptionStatusWidget(_a) {
    var className = _a.className, _b = _a.showUsageMetrics, showUsageMetrics = _b === void 0 ? true : _b, _c = _a.showQuickActions, showQuickActions = _c === void 0 ? true : _c, onUpgrade = _a.onUpgrade, onManageBilling = _a.onManageBilling, _d = _a.variant, variant = _d === void 0 ? "default" : _d;
    var _e = (0, react_1.useState)(false), showUpgradeModal = _e[0], setShowUpgradeModal = _e[1];
    var _f = (0, use_subscription_status_1.useSubscriptionStatus)(), status = _f.status, tier = _f.tier, features = _f.features, nextBilling = _f.nextBilling, gracePeriodEnd = _f.gracePeriodEnd, isLoading = _f.isLoading, error = _f.error, refresh = _f.refresh;
        ...
        <card_1.CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <subscription_status_indicator_1.SubscriptionStatusIndicator variant="minimal" className="scale-75"/>
              <div>
            ...
            {status !== "active" && (<button_1.Button size="sm" onClick={function () { return setShowUpgradeModal(true); }}>
    ...
    return (<>
        ...
        <card_1.CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <badge_1.Badge variant={status === "active" ? "default" : "secondary"} className={getStatusColor()}>
                {status.charAt(0).toUpperCase() + status.slice(1)}
              </badge_1.Badge>
            </div>
          </div>
          ...
          {/* Next Action Date */}
          {getNextActionDate() && (<div className="p-3 rounded-lg bg-muted/50">
              <div className="flex items-center justify-between">
          ...
          {/* Usage Metrics */}
          {showUsageMetrics && variant === "detailed" && (<div>
          ...
          {/* Features */}
          {variant === "detailed" && features.length > 0 && (<div>
              <h4 className="font-medium mb-3">Active Features</h4>
              <div className="flex flex-wrap gap-2">
                {features.slice(0, 6).map(function (feature, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                    {feature}
                  </badge_1.Badge>); })}
                {features.length > 6 && (<badge_1.Badge variant="outline" className="text-xs">
          ...
          {showQuickActions && (<>
              ...
              <div className="flex flex-col gap-2">
                {status !== "active" && (<button_1.Button onClick={function () { return setShowUpgradeModal(true); }} className="w-full">
      ...
      {/* Upgrade Modal */}
      <upgrade_prompt_1.UpgradePrompt isOpen={showUpgradeModal} onClose={function () { return setShowUpgradeModal(false); }} reason={status === "trialing" ? "trial_ended" : "expired"} currentPlan={tier || "Free"} suggestedPlan="Professional" onUpgrade={onUpgrade}/>
    </>);
}
