...
function SubscriptionNotificationProvider(_a) {
    var className = _a.className, _b = _a.position, position = _b === void 0 ? "top" : _b, _c = _a.autoHide, autoHide = _c === void 0 ? false : _c, _d = _a.hideAfter, hideAfter = _d === void 0 ? 10000 : _d, _e = _a.showProgress, showProgress = _e === void 0 ? true : _e;
    ...
    }), status = _f.status, tier = _f.tier, gracePeriodEnd = _f.gracePeriodEnd, nextBilling = _f.nextBilling, isLoading = _f.isLoading, error = _f.error, events = _f.events;
        ...
        var statusMessages = {
            ...
            past_due: {
                ...
                action: {
                    ...
                    onClick: function () { return console.log("Navigate to billing"); },
                },
            },
            cancelled: {
                type: "error",
                ...
                action: {
                    label: "Reativar",
                    onClick: function () { return console.log("Navigate to reactivate"); },
                },
            },
            incomplete: {
                type: "error",
                ...
                action: {
                    ...
                    onClick: function () { return console.log("Navigate to complete payment"); },
                },
            },
        };
    ...
    var addNotification = function (notification) {
        var newNotification = __assign(__assign({}, notification), { id: Math.random().toString(36).substr(2, 9), timestamp: new Date() });
        setNotifications(function (prev) { return __spreadArray([newNotification], prev.slice(0, 4), true); }); // Keep max 5 notifications
        if (autoHide) {
            setTimeout(function () {
        ...
        setNotifications(function (prev) {
            return prev.map(function (notification) {
                return notification.id === id
                    ? __assign(__assign({}, notification), { dismissed: true }) : notification;
            });
        });
    ...
    (0, react_1.useEffect)(function () {
        if (status === "trialing" && gracePeriodEnd) {
            var daysUntilExpiration = Math.ceil((new Date(gracePeriodEnd).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24));
            if (daysUntilExpiration <= 3 && daysUntilExpiration > 0) {
                addNotification({
                    type: "warning",
                    title: "Período de Teste Expirando",
                    description: "Seu per\u00EDodo de teste expira em ".concat(daysUntilExpiration, " ").concat(daysUntilExpiration === 1 ? "dia" : "dias", "."),
                    action: {
                        label: "Assinar Agora",
                        onClick: function () { return console.log("Navigate to subscription"); },
                    },
                });
            }
        }
        if (status === "active" && nextBilling) {
            var daysUntilBilling = Math.ceil((new Date(nextBilling).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24));
            if (daysUntilBilling <= 7 && daysUntilBilling > 0) {
                addNotification({
                    type: "info",
                    title: "Cobrança Próxima",
                    description: "Sua pr\u00F3xima cobran\u00E7a ser\u00E1 em ".concat(daysUntilBilling, " ").concat(daysUntilBilling === 1 ? "dia" : "dias", "."),
                });
            }
        }
    ...
    return (<div className={(0, utils_1.cn)("fixed z-50 w-full max-w-md space-y-2", position === "top" ? "top-4 right-4" : "bottom-4 right-4", className)}>
      {notifications
            .filter(function (notification) { return !notification.dismissed; })
            .map(function (notification) { return (<div key={notification.id} className={(0, utils_1.cn)("transform transition-all duration-300 ease-in-out", "translate-x-0 opacity-100", notification.dismissed && "translate-x-full opacity-0")}>
            <alert_1.Alert className={(0, utils_1.cn)("shadow-lg border-l-4", getNotificationStyles(notification.type))}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {notification.action && (<button_1.Button variant="outline" size="sm" className="mt-2" onClick={notification.action.onClick}>
                ...
                <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-transparent" onClick={function () { return dismissNotification(notification.id); }}>
                  <lucide_react_1.X className="h-4 w-4"/>
                </button_1.Button>
              </div>

              {showProgress && autoHide && (<div className="mt-3">
                  <div className="w-full bg-gray-200 rounded-full h-1">
                    <div className="bg-current h-1 rounded-full transition-all animate-pulse" style={{
                    width: "100%",
...
function SubscriptionAlert() {
    var _a = (0, use_subscription_status_1.useSubscriptionStatus)(), status = _a.status, gracePeriodEnd = _a.gracePeriodEnd, isLoading = _a.isLoading;
        ...
        switch (status) {
            case "past_due":
                return {
                    ...
                    description: gracePeriodEnd
                        ? "Atualize sua forma de pagamento. Acesso expira ".concat((0, date_fns_1.formatDistanceToNow)(new Date(gracePeriodEnd), { addSuffix: true, locale: locale_1.ptBR }), ".")
            ...
            case "canceled":
                return {
                    variant: "destructive",
                    title: "Assinatura Cancelada",
                    description: "Sua assinatura foi cancelada. Reative para continuar usando as funcionalidades premium.",
            ...
            case "incomplete":
                return {
                    variant: "destructive",
                    title: "Pagamento Incompleto",
                    description: "Complete seu pagamento para ativar sua assinatura e acessar todas as funcionalidades.",
    ...
    return (<alert_1.Alert variant={alertContent.variant} className="mb-6">
