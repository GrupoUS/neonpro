...
function SubscriptionStatusIndicator(_a) {
    var _b = _a.variant, variant = _b === void 0 ? 'default' : _b, _c = _a.showMetrics, showMetrics = _c === void 0 ? false : _c, _d = _a.showEvents, showEvents = _d === void 0 ? false : _d, _e = _a.showActions, showActions = _e === void 0 ? true : _e, className = _a.className, onUpgradeClick = _a.onUpgradeClick, onManageClick = _a.onManageClick, _f = _a.options, options = _f === void 0 ? {} : _f;
    var toast = (0, use_toast_1.useToast)().toast;
    var _g = (0, react_1.useState)(false), showDetails = _g[0], setShowDetails = _g[1];
    var subscription = (0, use_subscription_status_1.useSubscriptionStatus)(__assign({ autoConnect: true, enableLogging: process.env.NODE_ENV === 'development', onStatusChange: function (status, previous) {
            if (previous && status !== previous) {
                toast({
                    title: 'Subscription Status Updated',
                    description: "Your subscription status changed from ".concat(previous, " to ").concat(status),
    ...
    var status = subscription.status, tier = subscription.tier, features = subscription.features, gracePeriodEnd = subscription.gracePeriodEnd, nextBilling = subscription.nextBilling, isLoading = subscription.isLoading, isConnected = subscription.isConnected, lastUpdate = subscription.lastUpdate, error = subscription.error, isExpired = subscription.isExpired, isActive = subscription.isActive, canAccessFeature = subscription.canAccessFeature, refresh = subscription.refresh, clearError = subscription.clearError, metrics = subscription.metrics, events = subscription.events;
    ...
    if (!status && isLoading) {
        return (<card_1.Card className={(0, utils_1.cn)("animate-pulse", className)}>
    ...
    var gracePeriodProgress = gracePeriodEnd ?
        Math.max(0, Math.min(100, ((new Date(gracePeriodEnd).getTime() - Date.now()) / (3 * 24 * 60 * 60 * 1000)) * 100)) : 0;
    var renderMinimal = function () { return (<tooltip_1.TooltipProvider>
      <tooltip_1.Tooltip>
        <tooltip_1.TooltipTrigger>
          <div className={(0, utils_1.cn)("flex items-center space-x-1", className)}>
            <div className="relative">
              ...
              {isConnected ? (<lucide_react_1.Wifi className="absolute -bottom-1 -right-1 h-2 w-2 text-green-500"/>) : (<lucide_react_1.WifiOff className="absolute -bottom-1 -right-1 h-2 w-2 text-red-500"/>)}
            </div>
        ...
        <tooltip_1.TooltipContent>
          <div className="text-sm">
            ...
            {tier && (<p className="text-muted-foreground mt-1">
                Plan: {tierInfo.label}
              </p>)}
            {lastUpdate && (<p className="text-xs text-muted-foreground mt-1">
    ...
    var renderCompact = function () { return (<card_1.Card className={(0, utils_1.cn)("w-full", config.bgColor, config.borderColor, className)}>
      <card_1.CardContent className="p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                {tier && (<badge_1.Badge variant="outline" className={(0, utils_1.cn)("text-xs", tierInfo.color)}>
          ...
          <div className="flex items-center space-x-2">
            ...
            {showActions && (<div className="flex space-x-1">
                <button_1.Button variant="ghost" size="sm" onClick={refresh} disabled={isLoading}>
                  <lucide_react_1.RefreshCw className={(0, utils_1.cn)("h-3 w-3", { "animate-spin": isLoading })}/>
                </button_1.Button>
                {isExpired && onUpgradeClick && (<button_1.Button size="sm" onClick={onUpgradeClick}>
                    Upgrade
                  </button_1.Button>)}
              </div>)}
          </div>
    ...
    var renderDetailed = function () { return (<card_1.Card className={(0, utils_1.cn)("w-full", className)}>
      <card_1.CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <card_1.CardTitle className="flex items-center space-x-2">
            <StatusIcon className={(0, utils_1.cn)("h-5 w-5", config.color)}/>
            <span>Subscription Status</span>
            {isConnected ? (<lucide_react_1.Activity className="h-4 w-4 text-green-500"/>) : (<lucide_react_1.WifiOff className="h-4 w-4 text-red-500"/>)}
          </card_1.CardTitle>
          
          {showActions && (<div className="flex space-x-2">
              <button_1.Button variant="outline" size="sm" onClick={function () { return setShowDetails(!showDetails); }}>
                <lucide_react_1.Settings className="h-4 w-4 mr-1"/>
                Details
              </button_1.Button>
              <button_1.Button variant="outline" size="sm" onClick={refresh} disabled={isLoading}>
                <lucide_react_1.RefreshCw className={(0, utils_1.cn)("h-4 w-4", { "animate-spin": isLoading })}/>
              </button_1.Button>
            </div>)}
        </div>
      </card_1.CardHeader>
      ...
      <card_1.CardContent className="space-y-4">
        ...
        <div className={(0, utils_1.cn)("p-3 rounded-lg", config.bgColor, config.borderColor)}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              {tier && (<badge_1.Badge variant="outline" className={tierInfo.color}>
                  <TierIcon className="h-3 w-3 mr-1"/>
                  {tierInfo.label}
                </badge_1.Badge>)}
            </div>
          ...
          {gracePeriodEnd && gracePeriodProgress > 0 && (<div className="mt-3">
        ...
        {features.length > 0 && (<div>
            ...
            <div className="flex flex-wrap gap-1">
              {features.slice(0, showDetails ? features.length : 3).map(function (feature) { return (<badge_1.Badge key={feature} variant="secondary" className="text-xs">
                  {feature}
                </badge_1.Badge>); })}
              {!showDetails && features.length > 3 && (<badge_1.Badge variant="outline" className="text-xs">
        ...
        {error && (<div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <button_1.Button variant="ghost" size="sm" onClick={clearError} className="text-red-600 hover:text-red-700">
                Dismiss
              </button_1.Button>
            </div>
          </div>)}
        ...
        {showDetails && showEvents && events.length > 0 && (<>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {events.slice(0, 5).map(function (event, index) { return (<div key={"".concat(event.timestamp, "-").concat(index)} className="flex items-center justify-between text-xs">
        ...
        {showActions && (<>
            ...
            <div className="flex space-x-2">
              {isExpired && onUpgradeClick && (<button_1.Button onClick={onUpgradeClick} className="flex-1">
                  <lucide_react_1.Crown className="h-4 w-4 mr-2"/>
                  Upgrade Plan
                </button_1.Button>)}
              {onManageClick && (<button_1.Button variant="outline" onClick={onManageClick} className="flex-1">
