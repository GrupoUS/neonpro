...
function SubscriptionStatusIndicator(_a) {
    var _b = _a.showFullCard, showFullCard = _b === void 0 ? false : _b, className = _a.className;
    var _c = (0, use_subscription_status_1.useSubscriptionStatus)(), isActive = _c.isActive, isTrialing = _c.isTrialing, isPastDue = _c.isPastDue, isCanceled = _c.isCanceled, hasAccess = _c.hasAccess, inGracePeriod = _c.inGracePeriod, isLoading = _c.isLoading, status = _c.status, currentPeriodEnd = _c.currentPeriodEnd, cancelAtPeriodEnd = _c.cancelAtPeriodEnd, message = _c.message, error = _c.error;
    if (isLoading) {
    ...
    if (!showFullCard) {
        return (<div className={(0, utils_1.cn)("flex items-center gap-2", className)}>
        <StatusIcon className={(0, utils_1.cn)("h-4 w-4", statusConfig.color === "green" && "text-green-600", statusConfig.color === "blue" && "text-blue-600", statusConfig.color === "yellow" && "text-yellow-600", statusConfig.color === "red" && "text-red-600", statusConfig.color === "gray" && "text-gray-600")}/>
        <badge_1.Badge variant={statusConfig.variant}>{statusConfig.label}</badge_1.Badge>
        {inGracePeriod && (<badge_1.Badge variant="outline" className="text-xs">
            Grace Period
          </badge_1.Badge>)}
      </div>);
    }
    return (<card_1.Card className={className}>
      <card_1.CardHeader className="pb-3">
        <card_1.CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">
            <StatusIcon className={(0, utils_1.cn)("h-5 w-5", statusConfig.color === "green" && "text-green-600", statusConfig.color === "blue" && "text-blue-600", statusConfig.color === "yellow" && "text-yellow-600", statusConfig.color === "red" && "text-red-600", statusConfig.color === "gray" && "text-gray-600")}/>
            Subscription Status
          </span>
          <badge_1.Badge variant={statusConfig.variant}>{statusConfig.label}</badge_1.Badge>
        </card_1.CardTitle>
      ...
      <card_1.CardContent className="space-y-4">
        ...
        {currentPeriodEnd && (<div className="text-sm">
        ...
        {!hasAccess && (<div className="space-y-2">
            <alert_1.Alert>
              <lucide_react_1.AlertCircle className="h-4 w-4"/>
        ...
        {isPastDue && inGracePeriod && (<button_1.Button asChild className="w-full">
...
function SubscriptionBadge(_a) {
    ...
    var _b = (0, use_subscription_status_1.useSubscriptionStatus)(), hasAccess = _b.hasAccess, isLoading = _b.isLoading, status = _b.status;
    if (isLoading) {
        return (<div className={(0, utils_1.cn)("h-6 w-16 bg-gray-200 rounded animate-pulse", className)}/>);
    }
    if (!hasAccess) {
        return (<badge_1.Badge variant="outline" className={(0, utils_1.cn)("text-xs", className)}>
        <lucide_react_1.XCircle className="h-3 w-3 mr-1"/>
        Upgrade
      </badge_1.Badge>);
    }
    if (status === "trialing") {
        return (<badge_1.Badge variant="secondary" className={(0, utils_1.cn)("text-xs", className)}>
        <lucide_react_1.Clock className="h-3 w-3 mr-1"/>
        Trial
      </badge_1.Badge>);
    }
    return (<badge_1.Badge variant="default" className={(0, utils_1.cn)("text-xs", className)}>
