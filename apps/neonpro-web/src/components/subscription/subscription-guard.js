...
function SubscriptionGuard(_a) {
    var children = _a.children, fallback = _a.fallback, _b = _a.requireActive, requireActive = _b === void 0 ? false : _b, _c = _a.showUpgrade, showUpgrade = _c === void 0 ? true : _c, _d = _a.feature, feature = _d === void 0 ? "this feature" : _d, className = _a.className;
    var _e = (0, use_subscription_status_1.useSubscriptionStatus)(), hasAccess = _e.hasAccess, isActive = _e.isActive, isTrialing = _e.isTrialing, inGracePeriod = _e.inGracePeriod, isLoading = _e.isLoading, status = _e.status, message = _e.message, error = _e.error;
    // Show loading state
    if (isLoading) {
    ...
    return (<card_1.Card className={className}>
      <card_1.CardHeader className="text-center">
        ...
        <card_1.CardDescription>
          {requireActive
            ? "An active subscription is required to access ".concat(feature, ". ").concat(status === "trialing" ? "Trial users cannot access this feature." : "")
            : "Upgrade to NeonPro to unlock ".concat(feature, " and many other premium features.")}
        </card_1.CardDescription>
      </card_1.CardHeader>
      <card_1.CardContent className="space-y-4">
        {status === "trialing" && (<alert_1.Alert>
...
function PremiumFeatureGuard(_a) {
    var children = _a.children, feature = _a.feature, className = _a.className;
    return (<SubscriptionGuard requireActive={false} feature={feature || "premium features"} className={className}>
      {children}
    </SubscriptionGuard>);
}
function ActiveSubscriptionGuard(_a) {
    var children = _a.children, feature = _a.feature, className = _a.className;
    return (<SubscriptionGuard requireActive={true} feature={feature || "this advanced feature"} className={className}>
      {children}
    </SubscriptionGuard>);
}
// Hook for conditional rendering based on subscription
function useSubscriptionAccess() {
    var _a = (0, use_subscription_status_1.useSubscriptionStatus)(), hasAccess = _a.hasAccess, isActive = _a.isActive, isTrialing = _a.isTrialing, isLoading = _a.isLoading;
    return {
        ...
        isLoading: isLoading,
        // Helper functions
        renderIfPremium: function (content) { return (hasAccess ? content : null); },
        renderIfActive: function (content) { return (isActive ? content : null); },
        renderIfTrial: function (content) { return (isTrialing ? content : null); },
    };
}
