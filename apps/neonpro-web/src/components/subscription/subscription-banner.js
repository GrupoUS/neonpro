...
function SubscriptionBanner(_a) {
    var className = _a.className, onUpgrade = _a.onUpgrade, onDismiss = _a.onDismiss, _b = _a.dismissible, dismissible = _b === void 0 ? true : _b, _c = _a.showProgress, showProgress = _c === void 0 ? true : _c, _d = _a.compact, compact = _d === void 0 ? false : _d;
    var _e = (0, react_1.useState)(false), dismissed = _e[0], setDismissed = _e[1];
    var _f = (0, use_subscription_status_1.useSubscriptionStatus)(), status = _f.status, tier = _f.tier, gracePeriodEnd = _f.gracePeriodEnd, nextBilling = _f.nextBilling, isLoading = _f.isLoading;
        ...
        switch (status) {
            ...
            case "past_due": {
                ...
                return {
                    ...
                    message: daysRemaining > 0
                        ? "Update payment method within ".concat(daysRemaining, " days to avoid service interruption")
            ...
            case "canceled": {
                ...
                return {
                    ...
                    message: daysRemaining > 0
                        ? "Access will end in ".concat(daysRemaining, " days. Reactivate to continue using all features")
    ...
    if (compact) {
        return (<div className={(0, utils_1.cn)("flex items-center justify-between px-4 py-2 border rounded-lg", styles.container, className)}>
        ...
        <div className="flex items-center space-x-2">
          <button_1.Button size="sm" onClick={onUpgrade} className={(0, utils_1.cn)("text-xs", styles.button)}>
            {config.ctaText}
          </button_1.Button>
          {dismissible && (<button_1.Button variant="ghost" size="sm" onClick={handleDismiss} className="h-6 w-6 p-0 text-current opacity-70 hover:opacity-100">
              <lucide_react_1.X className="h-3 w-3"/>
            </button_1.Button>)}
        </div>
      </div>);
    }
    return (<div className={(0, utils_1.cn)("relative border rounded-lg p-4", styles.container, className)}>
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            ...
            {dismissible && (<button_1.Button variant="ghost" size="sm" onClick={handleDismiss} className="h-8 w-8 p-0 text-current opacity-70 hover:opacity-100">
          ...
          {showProgress && config.progress !== undefined && (<div className="mb-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            {status === "trialing" && (<button_1.Button variant="outline" size="sm" className="border-current/20">
