...
function SubscriptionDashboard(_a) {
    var _this = this;
    var _b, _c, _d;
    var onUpgrade = _a.onUpgrade, onManageBilling = _a.onManageBilling, onCancelSubscription = _a.onCancelSubscription;
    var _e = (0, react_1.useState)(null), subscription = _e[0], setSubscription = _e[1];
    var _f = (0, react_1.useState)(true), loading = _f[0], setLoading = _f[1];
    var _g = (0, react_1.useState)(null), error = _g[0], setError = _g[1];
    (0, react_1.useEffect)(function () {
        fetchSubscriptionData();
    }, []);
    var fetchSubscriptionData = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    return (<div className="space-y-6">
      {/* Status Alert */}
      {subscription.status_info.action_required && (<alert_1.Alert variant={subscription.status_info.status === 'trial_ending' ? 'default' : 'destructive'}>
          <lucide_react_1.AlertTriangle className="h-4 w-4"/>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {((_b = subscription.usage_stats.max_users) === null || _b === void 0 ? void 0 : _b.current) || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              de {((_c = subscription.usage_stats.max_users) === null || _c === void 0 ? void 0 : _c.limit) === 'Unlimited'
            ? 'ilimitados'
            : ((_d = subscription.usage_stats.max_users) === null || _d === void 0 ? void 0 : _d.limit) || 0}
            </p>
          </card_1.CardContent>
        </card_1.Card>
      </div>
        ...
        <card_1.CardContent>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            return (<div key={feature} className="flex items-center space-x-2">
                  {enabled ? (<lucide_react_1.Check className="h-4 w-4 text-green-500"/>) : (<lucide_react_1.X className="h-4 w-4 text-gray-400"/>)}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        {subscription.status === 'active' && !subscription.cancel_at_period_end && (<button_1.Button variant="destructive" onClick={onCancelSubscription} className="flex-1">
