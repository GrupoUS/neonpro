...
function SubscriptionStatusCard(_a) {
    var className = _a.className, _b = _a.variant, variant = _b === void 0 ? "default" : _b, _c = _a.showActions, showActions = _c === void 0 ? true : _c, _d = _a.showFeatures, showFeatures = _d === void 0 ? true : _d, _e = _a.showBilling, showBilling = _e === void 0 ? true : _e, onUpgrade = _a.onUpgrade, onManage = _a.onManage, onCancel = _a.onCancel;
    var _f = (0, use_subscription_status_1.useSubscriptionStatus)(), status = _f.status, tier = _f.tier, features = _f.features, gracePeriodEnd = _f.gracePeriodEnd, nextBilling = _f.nextBilling, isLoading = _f.isLoading, error = _f.error, refresh = _f.refresh;
    // Loading skeleton
    if (isLoading) {
    ...
    // Error state
    if (error) {
        return (<card_1.Card className={(0, utils_1.cn)("w-full border-destructive", className)}>
        <card_1.CardHeader>
    ...
    return (<card_1.Card className={(0, utils_1.cn)("w-full", config.borderColor, className)}>
      <card_1.CardHeader className={(0, utils_1.cn)("pb-3", config.bgColor)}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <card_1.CardDescription className="mt-1">
                {tier && (<span className="flex items-center gap-1">
          ...
          {variant !== "compact" && (<button_1.Button variant="ghost" size="sm" onClick={refresh}>
              Atualizar
            </button_1.Button>)}
        </div>

        {/* Trial progress */}
        {status === "trialing" && trialInfo && variant !== "compact" && (<div className="mt-4 space-y-2">
      ...
      <card_1.CardContent className="space-y-4">
        {/* Billing Information */}
        {showBilling && nextBilling && variant !== "compact" && (<div className="flex items-center justify-between">
        ...
        {gracePeriodEnd && status === "past_due" && (<div className="rounded-md bg-orange-50 p-3 border border-orange-200">
        ...
        {/* Features List */}
        {showFeatures && features.length > 0 && variant === "detailed" && (<div>
            ...
            <div className="pt-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {features.slice(0, 6).map(function (feature, index) { return (<div key={index} className="flex items-center gap-2 text-sm text-muted-foreground">
                    <lucide_react_1.CheckCircle2 className="h-3 w-3 text-green-600"/>
                    {feature}
                  </div>); })}
              </div>
              {features.length > 6 && (<p className="text-xs text-muted-foreground mt-2">
      ...
      {/* Actions */}
      {showActions && variant !== "compact" && (<card_1.CardFooter className="flex gap-2">
          {status === "trialing" && (<button_1.Button onClick={onUpgrade} className="flex-1">
              <lucide_react_1.Crown className="h-4 w-4 mr-2"/>
              Assinar Agora
            </button_1.Button>)}

          {status === "past_due" && (<button_1.Button onClick={onManage} variant="destructive" className="flex-1">
              <lucide_react_1.CreditCard className="h-4 w-4 mr-2"/>
              Atualizar Pagamento
            </button_1.Button>)}

          {status === "cancelled" && (<button_1.Button onClick={onUpgrade} className="flex-1">
          ...
          {status === "incomplete" && (<button_1.Button onClick={onManage} variant="destructive" className="flex-1">
              <lucide_react_1.CreditCard className="h-4 w-4 mr-2"/>
              Completar Pagamento
            </button_1.Button>)}
        </card_1.CardFooter>)}
