...
function SubscriptionDashboardWidget(_a) {
    var className = _a.className, _b = _a.variant, variant = _b === void 0 ? "default" : _b, _c = _a.showMetrics, showMetrics = _c === void 0 ? true : _c, _d = _a.showQuickActions, showQuickActions = _d === void 0 ? true : _d, onManage = _a.onManage, onUpgrade = _a.onUpgrade;
    var _e = (0, use_subscription_status_1.useSubscriptionStatus)(), status = _e.status, tier = _e.tier, gracePeriodEnd = _e.gracePeriodEnd, nextBilling = _e.nextBilling, features = _e.features, isLoading = _e.isLoading, metrics = _e.metrics;
    // Loading skeleton
    if (isLoading) {
    ...
    var getKeyMetrics = function () {
        if (!showMetrics || variant === "compact")
            return [];
        var keyMetrics = [
            {
    ...
    return (<card_1.Card className={(0, utils_1.cn)("w-full", className)}>
      <card_1.CardHeader className={(0, utils_1.cn)("pb-3", variant === "compact" && "pb-2")}>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <card_1.CardTitle className="text-sm font-medium">
                Assinatura
                {tier && (<span className="text-xs font-normal text-muted-foreground ml-1">
                    {tier}
                  </span>)}
              </card_1.CardTitle>
              {variant !== "compact" && (<card_1.CardDescription className="text-xs">
        ...
        {/* Trial Progress */}
        {status === "trialing" && trialInfo && variant !== "compact" && (<div className="mt-3 space-y-1">
      ...
      <card_1.CardContent className="space-y-3">
        {/* Key Metrics */}
        {keyMetrics.length > 0 && (<div className="grid grid-cols-3 gap-2">
            {keyMetrics.map(function (metric, index) { return (<div key={index} className="text-center">
        ...
        {/* Next Billing Info */}
        {nextBilling && variant === "detailed" && (<div className="text-xs text-muted-foreground flex items-center justify-between p-2 bg-gray-50 rounded">
        ...
        {gracePeriodEnd && status === "past_due" && (<div className="text-xs p-2 bg-orange-50 border border-orange-200 rounded text-orange-800">
        ...
        {/* Quick Actions */}
        {showQuickActions && variant !== "compact" && (<div className="flex gap-2 pt-2">
            {status === "trialing" && (<button_1.Button size="sm" onClick={onUpgrade} className="flex-1 text-xs">
                <lucide_react_1.Crown className="h-3 w-3 mr-1"/>
                Assinar
              </button_1.Button>)}

            {status === "active" && (<>
                <button_1.Button size="sm" variant="outline" onClick={onManage} className="flex-1 text-xs">
                  Gerenciar
                </button_1.Button>
            ...
            {(status === "past_due" || status === "incomplete") && (<button_1.Button size="sm" variant="destructive" onClick={onManage} className="flex-1 text-xs">
                <lucide_react_1.DollarSign className="h-3 w-3 mr-1"/>
                Pagar Agora
              </button_1.Button>)}

            {(status === "cancelled" || status === "canceled") && (<button_1.Button size="sm" onClick={onUpgrade} className="flex-1 text-xs">
                <lucide_react_1.TrendingUp className="h-3 w-3 mr-1"/>
                Reativar
              </button_1.Button>)}
          </div>)}
