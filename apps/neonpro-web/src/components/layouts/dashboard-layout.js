...
function DashboardLayout(_a) {
    var _this = this;
    var _b, _c;
    var children = _a.children, user = _a.user, _d = _a.breadcrumbs, breadcrumbs = _d === void 0 ? [] : _d, title = _a.title, subtitle = _a.subtitle;
    ...
    return (<div className="min-h-screen bg-background">
              ...
              <dropdown_menu_1.DropdownMenuTrigger asChild>
                <button_1.Button variant="ghost" className="relative h-8 w-8 rounded-full">
                  <avatar_1.Avatar className="h-8 w-8">
                    <avatar_1.AvatarImage src={(_b = user.user_metadata) === null || _b === void 0 ? void 0 : _b.avatar_url} alt={user.email}/>
                    <avatar_1.AvatarFallback>{getUserInitials(user.email || '')}</avatar_1.AvatarFallback>
                  </avatar_1.Avatar>
                </button_1.Button>
              </dropdown_menu_1.DropdownMenuTrigger>
              <dropdown_menu_1.DropdownMenuContent className="w-56" align="end" forceMount>
                <div className="flex flex-col space-y-1 p-2">
                  <p className="text-sm font-medium leading-none">{((_c = user.user_metadata) === null || _c === void 0 ? void 0 : _c.full_name) || user.email}</p>
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 lg:px-6 py-6">
          {/* Breadcrumbs */}
          {breadcrumbs.length > 0 && (<div className="mb-6">
              <breadcrumb_1.Breadcrumb>
                <breadcrumb_1.BreadcrumbList>
                  ...
                  {breadcrumbs.map(function (crumb, index) { return (<div key={index} className="flex items-center">
                      <breadcrumb_1.BreadcrumbSeparator />
                      <breadcrumb_1.BreadcrumbItem>
                        {crumb.href && index < breadcrumbs.length - 1 ? (<breadcrumb_1.BreadcrumbLink href={crumb.href}>
                            {crumb.title}
                          </breadcrumb_1.BreadcrumbLink>) : (<breadcrumb_1.BreadcrumbPage>{crumb.title}</breadcrumb_1.BreadcrumbPage>)}
                      </breadcrumb_1.BreadcrumbItem>
                    </div>); })}
                </breadcrumb_1.BreadcrumbList>
              </breadcrumb_1.Breadcrumb>
