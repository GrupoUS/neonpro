...
function OptimizationsList(_a) {
    ...
    var _b = (0, react_1.useState)([]), optimizations = _b[0], setOptimizations = _b[1];
    var _c = (0, react_1.useState)(true), loading = _c[0], setLoading = _c[1];
    var _d = (0, react_1.useState)(null), error = _d[0], setError = _d[1];
    (0, react_1.useEffect)(function () {
        fetchOptimizations();
    }, [clinicId]);
    var fetchOptimizations = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var calculateProgress = function (startDate, targetDate, completionDate) {
    ...
    if (loading) {
      ...
      {[1, 2, 3].map(function (i) { return (<div key={i} className="h-20 bg-gray-100 rounded-lg"/>); })}
    </div>;
    }
    if (error) {
        return (<card_1.Card>
        <card_1.CardContent className="flex items-center justify-center py-8">
          <div className="text-center">
            ...
            <button_1.Button variant="outline" size="sm" onClick={fetchOptimizations} className="mt-2">
              Try Again
            </button_1.Button>
          </div>
        </card_1.CardContent>
    ...
    return (<div className="space-y-3">
      {optimizations.map(function (optimization) {
            var StatusIcon = statusConfig[optimization.status].icon;
            var progress = calculateProgress(optimization.start_date, optimization.target_date, optimization.completion_date);
            return (<card_1.Card key={optimization.id} className="hover:shadow-md transition-shadow">
            <card_1.CardContent className="pt-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <badge_1.Badge variant="outline" className={statusConfig[optimization.status].color}>
                    <StatusIcon className="h-3 w-3 mr-1"/>
                    {optimization.status}
                  </badge_1.Badge>
                  
                  <badge_1.Badge variant="outline" className={priorityConfig[optimization.priority]}>
      ...
      {optimizations.length === 0 && (<card_1.Card>
          <card_1.CardContent className="flex items-center justify-center py-8">
            <div className="text-center">
