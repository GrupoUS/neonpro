...
function BudgetApprovalWorkflow(_a) {
    var _b = _a.clinicId, clinicId = _b === void 0 ? "default-clinic" : _b, onBudgetCreated = _a.onBudgetCreated, onApprovalActioned = _a.onApprovalActioned;
    var _c = (0, useBudgetApproval_1.useBudgetApproval)(), budgets = _c.budgets, budgetLoading = _c.budgetLoading, approvals = _c.approvals, pendingApprovals = _c.pendingApprovals, approvalLoading = _c.approvalLoading, recommendations = _c.recommendations, forecasts = _c.forecasts, validationResult = _c.validationResult, createBudget = _c.createBudget, updateBudget = _c.updateBudget, deleteBudget = _c.deleteBudget, createApproval = _c.createApproval, processApproval = _c.processApproval, generateOptimizationRecommendations = _c.generateOptimizationRecommendations, generateForecast = _c.generateForecast, validateBudget = _c.validateBudget, refreshBudgets = _c.refreshBudgets, refreshApprovals = _c.refreshApprovals;
    var _d = (0, react_1.useState)(false), showCreateBudget = _d[0], setShowCreateBudget = _d[1];
    var _e = (0, react_1.useState)(false), showCreateApproval = _e[0], setShowCreateApproval = _e[1];
    var _f = (0, react_1.useState)(null), selectedBudget = _f[0], setSelectedBudget = _f[1];
    ...
    var BudgetCard = function (_a) {
        ...
        var utilizationPercentage = budget.total_amount > 0
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={"font-medium ".concat(remainingAmount > 0 ? "text-green-600" : "text-red-600")}>
                ${remainingAmount.toLocaleString()}
              </span>
            </div>

            <div className="flex gap-2 mt-4">
              <button_1.Button variant="outline" size="sm" onClick={function () { return setSelectedBudget(budget); }}>
                <lucide_react_1.Eye className="h-4 w-4 mr-1"/>
                View Details
              </button_1.Button>
              <button_1.Button variant="outline" size="sm" onClick={function () { return generateForecast(budget.id); }}>
    ...
    var ApprovalCard = function (_a) {
        ...
        <card_1.CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <card_1.CardDescription>
                Amount: ${(_b = approval.amount) === null || _b === void 0 ? void 0 : _b.toLocaleString()}
              </card_1.CardDescription>
            </div>
        ...
        <card_1.CardContent>
          <div className="space-y-3">
            {approval.description && (<p className="text-sm text-muted-foreground">
            ...
            {approval.status === "pending" && (<div className="flex gap-2 mt-4">
                <button_1.Button size="sm" onClick={function () { return processApproval(approval.id, "approve"); }} className="bg-green-600 hover:bg-green-700">
                  <lucide_react_1.CheckCircle className="h-4 w-4 mr-1"/>
                  Approve
                </button_1.Button>
                <button_1.Button size="sm" variant="outline" onClick={function () { return processApproval(approval.id, "reject"); }} className="border-red-200 text-red-600 hover:bg-red-50">
    ...
    return (<div className="space-y-6">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              $
              {budgets
            .reduce(function (sum, b) { return sum + b.total_amount; }, 0)
            .toLocaleString()}
            </div>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              $
              {budgets
            .reduce(function (sum, b) { return sum + (b.spent_amount || 0); }, 0)
            .toLocaleString()}
            </div>
      ...
      <tabs_1.Tabs defaultValue="budgets" className="space-y-4">
        ...
        <tabs_1.TabsContent value="budgets" className="space-y-4">
          ...
          {budgetLoading ? (<div className="flex items-center justify-center h-32">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
            </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {budgets.map(function (budget) { return (<BudgetCard key={budget.id} budget={budget}/>); })}
            </div>)}
        </tabs_1.TabsContent>
        ...
        <tabs_1.TabsContent value="approvals" className="space-y-4">
          ...
          {approvalLoading ? (<div className="flex items-center justify-center h-32">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
            </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {approvals.map(function (approval) { return (<ApprovalCard key={approval.id} approval={approval}/>); })}
            </div>)}
        </tabs_1.TabsContent>
        ...
        <tabs_1.TabsContent value="forecasting" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {forecasts.length > 0 ? (<div className="grid gap-4">
                    {forecasts.map(function (forecast, index) { return (<div key={index} className="border rounded-lg p-4">
                        ...
                        {forecast.recommendations.length > 0 && (<div className="mt-3">
                            <label_1.Label>Recommendations</label_1.Label>
                            <ul className="list-disc list-inside text-sm text-muted-foreground">
                              {forecast.recommendations.map(function (rec, i) { return (<li key={i}>{rec}</li>); })}
                            </ul>
                          </div>)}
                      </div>); })}
        ...
        <tabs_1.TabsContent value="optimization" className="space-y-4">
          <card_1.Card>
            ...
            return generateOptimizationRecommendations(budgets.map(function (b) { return b.id; }));
        }} disabled={budgets.length === 0}>
                  Generate Optimization Recommendations
                </button_1.Button>

                {recommendations.length > 0 ? (<div className="grid gap-4">
                    {recommendations.map(function (rec, index) { return (<div key={index} className="border rounded-lg p-4">
                        <div className="flex justify-between items-center mb-2">
                          <h4 className="font-medium">{rec.type}</h4>
                          <badge_1.Badge variant={rec.priority === "high"
                    ? "destructive"
                    : rec.priority === "medium"
                        ? "default"
                        : "secondary"}>
