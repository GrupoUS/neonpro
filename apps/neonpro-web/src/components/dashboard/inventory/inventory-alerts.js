...
function InventoryAlerts() {
    ...
    var loadAlerts = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var markAsRead = function (alertId) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/inventory/alerts/".concat(alertId, "/read"), {
                            method: "PATCH",
                        })];
                case 1:
                    response = _a.sent();
                    ...
                    setAlerts(function (prev) {
                        return prev.map(function (alert) {
                            return alert.id === alertId ? __assign(__assign({}, alert), { is_read: true }) : alert;
                        });
                    });
                    return [3 /*break*/, 3];
                case 2:
    ...
    var dismissAlert = function (alertId) { return __awaiter(_this, void 0, void 0, function () {
        var response, err_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/inventory/alerts/".concat(alertId), {
                            method: "DELETE",
                        })];
                case 1:
                    response = _a.sent();
    ...
    var markAllAsRead = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/inventory/alerts/mark-all-read", {
                            method: "PATCH",
                        })];
                case 1:
                    response = _a.sent();
                    ...
                    setAlerts(function (prev) { return prev.map(function (alert) { return (__assign(__assign({}, alert), { is_read: true })); }); });
                    return [3 /*break*/, 3];
                case 2:
                    err_4 = _a.sent();
    ...
    var criticalCount = alerts.filter(function (alert) { return alert.severity === "critical" || alert.severity === "high"; }).length;
    // =====================================================================================
    // LOADING STATE
    // =====================================================================================
    if (loading) {
    ...
    return (<card_1.Card>
      <card_1.CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <card_1.CardTitle className="flex items-center gap-2">
              {unreadCount > 0 ? (<lucide_react_1.Bell className="h-5 w-5 text-orange-500"/>) : (<lucide_react_1.BellOff className="h-5 w-5 text-gray-400"/>)}
              Inventory Alerts
          ...
          {unreadCount > 0 && (<button_1.Button variant="outline" size="sm" onClick={markAllAsRead}>
        ...
        <div className="flex items-center space-x-2 pt-4">
          <button_1.Button variant={filter === "all" ? "default" : "outline"} size="sm" onClick={function () { return setFilter("all"); }}>
            All ({alerts.length})
          </button_1.Button>
          <button_1.Button variant={filter === "unread" ? "default" : "outline"} size="sm" onClick={function () { return setFilter("unread"); }}>
            Unread ({unreadCount})
          </button_1.Button>
          <button_1.Button variant={filter === "critical" ? "default" : "outline"} size="sm" onClick={function () { return setFilter("critical"); }}>
                ...
                return (<div key={alert.id} className={"\n                    p-4 border rounded-lg transition-all duration-200 hover:shadow-sm\n                    ".concat(config.bgColor, " ").concat(config.borderColor, "\n                    ").concat(!alert.is_read ? "border-l-4" : "", "\n                  ")}>
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {!alert.is_read && (<badge_1.Badge variant="default" className="bg-blue-100 text-blue-800">
                              New
                            </badge_1.Badge>)}
                        </div>
                        ...
                        <div className="flex items-center space-x-4 text-xs text-gray-500">
                          ...
                          {alert.current_quantity !== undefined && (<span>Current: {alert.current_quantity}</span>)}
                          {alert.expiry_date && (<span>
                              Expires:{" "}
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      {!alert.is_read && (<button_1.Button variant="ghost" size="sm" onClick={function () { return markAsRead(alert.id); }} className="text-gray-500 hover:text-gray-700">
                          <lucide_react_1.CheckCircle className="h-4 w-4"/>
                        </button_1.Button>)}
                      <button_1.Button variant="ghost" size="sm" onClick={function () { return dismissAlert(alert.id); }} className="text-gray-500 hover:text-red-600">
