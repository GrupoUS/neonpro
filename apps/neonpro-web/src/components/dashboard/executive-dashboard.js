...
var ExecutiveDashboard = function (_a) {
    ...
    // State management
    var _b = (0, react_1.useState)(null), dashboardData = _b[0], setDashboardData = _b[1];
    var _c = (0, react_1.useState)(true), loading = _c[0], setLoading = _c[1];
    var _d = (0, react_1.useState)(null), error = _d[0], setError = _d[1];
    var _e = (0, react_1.useState)('monthly'), periodType = _e[0], setPeriodType = _e[1];
    var _f = (0, react_1.useState)(false), refreshing = _f[0], setRefreshing = _f[1];
    var _g = (0, react_1.useState)(false), alertsExpanded = _g[0], setAlertsExpanded = _g[1];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    setLoading(true);
                    setError(null);
                    return [4 /*yield*/, fetch("/api/executive-dashboard?clinic_id=".concat(clinicId, "&period_type=").concat(periodType))];
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
    ...
    var handleAlertAction = function (alertId, action) { return __awaiter(void 0, void 0, void 0, function () {
        var response, result_1, updatedAlerts, err_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    return [4 /*yield*/, fetch("/api/executive-dashboard/alerts/".concat(alertId), {
                            method: 'PUT',
                            headers: {
                ...
                case 2:
                    ...
                    toast({
                        ...
                        description: "O alerta foi ".concat(action === 'acknowledge' ? 'reconhecido' : 'resolvido', " com sucesso."),
                    });
                    return [3 /*break*/, 4];
                case 3:
                    err_2 = _a.sent();
    ...
    var generateReport = function (reportName_1, reportType_1) {
        ...
        return __awaiter(void 0, __spreadArray([reportName_1, reportType_1], args_1, true), void 0, function (reportName, reportType, format) {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 4, , 5]);
                        return [4 /*yield*/, fetch('/api/executive-dashboard/reports', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                },
    ...
    var kpis = dashboardData.kpis, alerts = dashboardData.alerts, widgets = dashboardData.widgets, reports = dashboardData.reports, periodComparisons = dashboardData.periodComparisons; // Get key KPIs for display
    var keyKPIs = [
        { name: 'total_revenue', label: 'Receita Total', icon: lucide_react_1.DollarSign, formatter: formatCurrency },
        { name: 'total_appointments', label: 'Consultas Totais', icon: lucide_react_1.Calendar, formatter: function (value) { return value.toString(); } },
        { name: 'new_patients', label: 'Novos Pacientes', icon: lucide_react_1.Users, formatter: function (value) { return value.toString(); } },
        { name: 'patient_satisfaction', label: 'Satisfação', icon: lucide_react_1.BarChart3, formatter: formatPercentage }
    ];
    // Active alerts count by severity
    var activeAlerts = alerts.filter(function (alert) { return alert.is_active; });
    var criticalAlerts = activeAlerts.filter(function (alert) { return alert.severity === 'critical'; });
    var highAlerts = activeAlerts.filter(function (alert) { return alert.severity === 'high'; });
    return (<div className="space-y-6">
      {/* Header */}
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <dropdown_menu_1.DropdownMenu>
            ...
            <dropdown_menu_1.DropdownMenuContent>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return generateReport('Resumo Executivo', 'executive_summary', 'pdf'); }}>
                <lucide_react_1.Download className="h-4 w-4 mr-2"/>
                Resumo Executivo (PDF)
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return generateReport('Análise de KPIs', 'kpi_analysis', 'excel'); }}>
                <lucide_react_1.BarChart3 className="h-4 w-4 mr-2"/>
                Análise de KPIs (Excel)
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return generateReport('Análise de Tendências', 'trend_analysis', 'pdf'); }}>
      ...
      {/* Alerts Summary */}
      {activeAlerts.length > 0 && (<alert_1.Alert className={(0, utils_1.cn)("border-l-4", criticalAlerts.length > 0 ? "border-l-red-500 bg-red-50" :
          ...
          <alert_1.AlertDescription>
            <div className="flex items-center justify-between">
              <span>
                {criticalAlerts.length > 0 && (<span className="text-red-700 font-medium">
                    {criticalAlerts.length} alerta(s) crítico(s)
                  </span>)}
                {criticalAlerts.length > 0 && highAlerts.length > 0 && ' • '}
                {highAlerts.length > 0 && (<span className="text-orange-700 font-medium">
                    {highAlerts.length} alerta(s) importante(s)
                  </span>)}
                {activeAlerts.length - criticalAlerts.length - highAlerts.length > 0 && (<span className="text-yellow-700">
                    {' • '}{activeAlerts.length - criticalAlerts.length - highAlerts.length} outro(s)
                  </span>)}
              </span>
              <button_1.Button variant="ghost" size="sm" onClick={function () { return setAlertsExpanded(!alertsExpanded); }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {keyKPIs.map(function (kpi) {
            ...
            return (<card_1.Card key={kpi.name}>
              <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <card_1.CardContent>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <div className={(0, utils_1.cn)("flex items-center gap-1 px-2 py-1 rounded-full", trendDisplay.color, trendDisplay.bgColor)}>
                    {trendDisplay.icon}
      ...
      <tabs_1.Tabs defaultValue="overview" className="space-y-6">
        <tabs_1.TabsList>
          ...
          <tabs_1.TabsTrigger value="alerts">
            Alertas
            {activeAlerts.length > 0 && (<badge_1.Badge variant="destructive" className="ml-2 h-5 w-5 p-0 text-xs">
                {activeAlerts.length}
              </badge_1.Badge>)}
          </tabs_1.TabsTrigger>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {kpis.filter(function (kpi) { return !keyKPIs.some(function (k) { return k.name === kpi.kpi_name; }); }).map(function (kpi) { return (<div key={kpi.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {(0, date_fns_1.format)(new Date(kpi.period_start), 'dd/MM', { locale: locale_1.ptBR })} - {(0, date_fns_1.format)(new Date(kpi.period_end), 'dd/MM', { locale: locale_1.ptBR })}
                    </div>
                  </div>); })}
              </div>
            </card_1.CardContent>
        ...
        <tabs_1.TabsContent value="alerts" className="space-y-6">
          <div className="grid gap-4">
            {activeAlerts.length === 0 ? (<card_1.Card>
              ...
              </card_1.Card>) : (activeAlerts.map(function (alert) { return (<card_1.Card key={alert.id} className={(0, utils_1.cn)("border-l-4", {
                ...
                'border-l-blue-500': alert.severity === 'low',
                  ...
                  <card_1.CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <dropdown_menu_1.DropdownMenu>
                        ...
                        <dropdown_menu_1.DropdownMenuContent>
                          {!alert.acknowledged_at && (<dropdown_menu_1.DropdownMenuItem onClick={function () { return handleAlertAction(alert.id, 'acknowledge'); }}>
                              <lucide_react_1.Eye className="h-4 w-4 mr-2"/>
                              Reconhecer
                            </dropdown_menu_1.DropdownMenuItem>)}
                          <dropdown_menu_1.DropdownMenuItem onClick={function () { return handleAlertAction(alert.id, 'resolve'); }}>
                  ...
                  <card_1.CardContent>
                    <div className="flex items-center justify-between text-sm text-muted-foreground">
                      <span>
                        Criado em {(0, date_fns_1.format)(new Date(alert.created_at), 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
                      </span>
                      {alert.acknowledged_at && (<span className="text-blue-600">Reconhecido</span>)}
                    </div>
                  </card_1.CardContent>
                </card_1.Card>); }))}
          </div>
        ...
        <tabs_1.TabsContent value="reports" className="space-y-6">
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              ...
              <button_1.Button onClick={function () { return generateReport('Relatório Personalizado', 'custom', 'pdf'); }}>
                <lucide_react_1.Plus className="h-4 w-4 mr-2"/>
                Gerar Novo Relatório
              </button_1.Button>
            </div>
            
            {reports.length === 0 ? (<card_1.Card>
                <card_1.CardContent className="flex items-center justify-center h-64">
                  <div className="text-center">
              ...
              </card_1.Card>) : (reports.map(function (report) { return (<card_1.Card key={report.id}>
                  <card_1.CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        <card_1.CardTitle className="text-lg">{report.report_name}</card_1.CardTitle>
                        <card_1.CardDescription>
                          {report.report_type} • {report.format.toUpperCase()} • 
                          {(0, date_fns_1.format)(new Date(report.created_at), 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
                        </card_1.CardDescription>
                      </div>
                      <badge_1.Badge variant={report.status === 'completed' ? 'default' :
                  ...
                  <card_1.CardContent>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">
                        Período: {(0, date_fns_1.format)(new Date(report.period_start), 'dd/MM/yyyy', { locale: locale_1.ptBR })} - {(0, date_fns_1.format)(new Date(report.period_end), 'dd/MM/yyyy', { locale: locale_1.ptBR })}
                      </span>
                      {report.status === 'completed' && report.file_path && (<button_1.Button variant="outline" size="sm">
                          <lucide_react_1.Download className="h-4 w-4 mr-2"/>
                          Download
                        </button_1.Button>)}
                    </div>
