    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent>
        <div className="space-y-4">
          {mockAppointments.map(function (appointment, index) { return (<div key={index} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
              <div className="flex items-center space-x-4">
                <div className="text-sm font-mono font-medium text-primary">
                  {appointment.time}
                </div>
                <div className="flex items-center space-x-2">
                  <div className={"w-2 h-2 rounded-full ".concat(getRiskColor(appointment.noShowRisk))} title={"Risco de no-show: ".concat(appointment.noShowRisk)}/>
                  <div>
        ...
        {mockAppointments.length === 0 && (<div className="text-center py-8 text-muted-foreground">
