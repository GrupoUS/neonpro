...
function CommunicationAnalyticsDashboard(_a) {
    ...
    var _d = (0, react_1.useState)(null), dashboardData = _d[0], setDashboardData = _d[1];
    var _e = (0, react_1.useState)('30d'), selectedTimeframe = _e[0], setSelectedTimeframe = _e[1];
    var _f = (0, react_1.useState)({
        from: (0, date_fns_1.subDays)(new Date(), 30),
        to: new Date()
    }), dateRange = _f[0], setDateRange = _f[1];
    var _g = (0, react_1.useState)(['all']), selectedChannels = _g[0], setSelectedChannels = _g[1];
    var _h = (0, react_1.useState)('overview'), viewMode = _h[0], setViewMode = _h[1];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, analytics_engine_1.communicationAnalytics.getDashboardData(filter)];
                case 1:
                    data = _a.sent();
    ...
    var calculatedMetrics = (0, react_1.useMemo)(function () {
        if (!dashboardData)
            return null;
        var overview = dashboardData.overview, channelPerformance = dashboardData.channelPerformance, roiMetrics = dashboardData.roiMetrics, trends = dashboardData.trends;
        return {
    ...
    // Componente de erro
    if (error) {
        return (<card_1.Card className="border-red-200 bg-red-50">
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          ...
          {/* Date range picker */}
          <date_picker_with_range_1.DatePickerWithRange date={dateRange} onDateChange={setDateRange}/>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard title="Receita Total" value={calculatedMetrics.totalRevenue} format="currency" trend={dashboardData.trends.revenueGrowth} icon={<lucide_react_1.DollarSign className="w-4 h-4"/>} color="text-green-600"/>
        
        <MetricCard title="Custo Total" value={calculatedMetrics.totalCost} format="currency" trend={dashboardData.trends.costGrowth} icon={<lucide_react_1.Target className="w-4 h-4"/>} color="text-red-600"/>
        
        <MetricCard title="ROI Médio" value={calculatedMetrics.avgROI} format="percentage" trend={dashboardData.trends.roiGrowth} icon={<lucide_react_1.TrendingUp className="w-4 h-4"/>} color="text-blue-600"/>
        
        <MetricCard title="Mensagens Enviadas" value={calculatedMetrics.totalMessages} format="number" trend={dashboardData.trends.volumeGrowth} icon={<lucide_react_1.MessageCircle className="w-4 h-4"/>} color="text-purple-600"/>
      </div>
      ...
      <tabs_1.Tabs value={viewMode} onValueChange={function (value) { return setViewMode(value); }}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.LineChart data={dashboardData.timeSeriesData}>
                    ...
                    <recharts_1.XAxis dataKey="date" tickFormatter={function (date) { return (0, date_fns_1.format)(new Date(date), 'dd/MM', { locale: locale_1.ptBR }); }}/>
                    <recharts_1.YAxis tickFormatter={function (value) { return "R$ ".concat(value.toLocaleString()); }}/>
                    <recharts_1.Tooltip labelFormatter={function (date) { return (0, date_fns_1.format)(new Date(date), 'dd/MM/yyyy', { locale: locale_1.ptBR }); }} formatter={function (value) { return ["R$ ".concat(value.toLocaleString()), 'Receita']; }}/>
                    <recharts_1.Line type="monotone" dataKey="revenue" stroke="#3B82F6" strokeWidth={2} dot={{ fill: '#3B82F6' }}/>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.PieChart>
                    <recharts_1.Pie data={dashboardData.channelPerformance} cx="50%" cy="50%" labelLine={false} label={function (_a) {
        var channel = _a.channel, percentage = _a.percentage;
        return "".concat(channel, ": ").concat(percentage.toFixed(1), "%");
    }} outerRadius={80} fill="#8884d8" dataKey="totalMessages">
                      {dashboardData.channelPerformance.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={CHANNEL_COLORS[entry.channel] || CHART_COLORS[index % CHART_COLORS.length]}/>); })}
                    </recharts_1.Pie>
                    <recharts_1.Tooltip formatter={function (value) { return [value.toLocaleString(), 'Mensagens']; }}/>
                  </recharts_1.PieChart>
                </recharts_1.ResponsiveContainer>
              </card_1.CardContent>
            </card_1.Card>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {dashboardData.channelPerformance.map(function (channel) { return (<ChannelPerformanceCard key={channel.channel} performance={channel}/>); })}
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </tabs_1.TabsContent>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.BarChart data={dashboardData.channelPerformance}>
                    ...
                    <recharts_1.Bar dataKey="clickThroughRate" fill="#10B981" name="Taxa de Clique %"/>
                    <recharts_1.Bar dataKey="responseRate" fill="#F59E0B" name="Taxa de Resposta %"/>
                  </recharts_1.BarChart>
                </recharts_1.ResponsiveContainer>
              </card_1.CardContent>
            </card_1.Card>
        ...
        <tabs_1.TabsContent value="comparison" className="space-y-4">
          {dashboardData.benchmarkComparison && (<card_1.Card>
              <card_1.CardHeader>
          ...
          {/* Forecasting */}
          {dashboardData.forecasting && (<card_1.Card>
              <card_1.CardHeader>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.LineChart data={dashboardData.forecasting.nextMonth}>
                    ...
                    <recharts_1.XAxis dataKey="date" tickFormatter={function (date) { return (0, date_fns_1.format)(new Date(date), 'dd/MM', { locale: locale_1.ptBR }); }}/>
                    <recharts_1.YAxis />
                    <recharts_1.Tooltip labelFormatter={function (date) { return (0, date_fns_1.format)(new Date(date), 'dd/MM/yyyy', { locale: locale_1.ptBR }); }}/>
                    <recharts_1.Line type="monotone" dataKey="predictedRevenue" stroke="#8B5CF6" strokeDasharray="5 5" name="Receita Projetada"/>
                    <recharts_1.Line type="monotone" dataKey="predictedMessages" stroke="#10B981" strokeDasharray="5 5" name="Mensagens Projetadas"/>
                  </recharts_1.LineChart>
                </recharts_1.ResponsiveContainer>
              </card_1.CardContent>
            </card_1.Card>)}
        ...
        <card_1.CardContent>
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <progress_1.Progress value={dashboardData.overview.dataQualityScore} className="h-2"/>
            </div>
            <badge_1.Badge variant={dashboardData.overview.dataQualityScore >= 90 ? "default" :
            dashboardData.overview.dataQualityScore >= 70 ? "secondary" : "destructive"} className="ml-4">
...
function MetricCard(_a) {
    var title = _a.title, value = _a.value, format = _a.format, trend = _a.trend, icon = _a.icon, color = _a.color;
    ...
    var getTrendIcon = function (trendValue) {
        ...
        if (trendValue > 0)
            return <lucide_react_1.ArrowUpRight className="w-3 h-3 text-green-600"/>;
        if (trendValue < 0)
            return <lucide_react_1.ArrowDownRight className="w-3 h-3 text-red-600"/>;
        return <lucide_react_1.Minus className="w-3 h-3"/>;
    };
    return (<card_1.Card>
      ...
      <card_1.CardContent>
        ...
        {trend !== undefined && (<p className="text-xs text-muted-foreground flex items-center mt-1">
            {getTrendIcon(trend)}
            <span className="ml-1">
...
function ChannelPerformanceCard(_a) {
    ...
    var getChannelIcon = function (channel) {
        switch (channel) {
            ...
            case 'push': return <lucide_react_1.Send className="w-4 h-4"/>;
            case 'voice': return <lucide_react_1.Phone className="w-4 h-4"/>;
            default: return <lucide_react_1.MessageCircle className="w-4 h-4"/>;
        }
    };
    ...
    return (<div className="flex items-center justify-between p-4 border rounded-lg">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: "".concat(channelColor, "20"), color: channelColor }}>
...
function TrendsAnalysisDisplay(_a) {
    ...
    var getTrendBadge = function (value, label) {
        ...
        var icon = value > 0 ? <lucide_react_1.TrendingUp className="w-3 h-3"/> :
            value < 0 ? <lucide_react_1.TrendingDown className="w-3 h-3"/> :
...
function BenchmarkComparisonDisplay(_a) {
    ...
    return (<div className="space-y-4">
      <div className="text-center p-4 bg-blue-50 rounded-lg">
        ...
        <badge_1.Badge variant={comparison.industryRanking === 'excellent' ? "default" :
            comparison.industryRanking === 'good' ? "secondary" : "destructive"} className="mt-2">
      ...
      <div className="space-y-2">
        ...
        <ul className="space-y-1">
          {comparison.recommendations.map(function (rec, index) { return (<li key={index} className="text-sm text-muted-foreground flex items-start gap-2">
