...
function AlternativeSlotSuggestion(_a) {
    var _this = this;
    var clinicId = _a.clinicId, professionalId = _a.professionalId, serviceTypeId = _a.serviceTypeId, originalStartTime = _a.originalStartTime, originalEndTime = _a.originalEndTime, excludeAppointmentId = _a.excludeAppointmentId, onSlotSelected = _a.onSlotSelected, onClose = _a.onClose, _b = _a.maxSuggestions, maxSuggestions = _b === void 0 ? 6 : _b, _c = _a.searchWindowDays, searchWindowDays = _c === void 0 ? 14 : _c;
    var _d = (0, react_1.useState)(false), isLoading = _d[0], setIsLoading = _d[1];
    var _e = (0, react_1.useState)([]), suggestions = _e[0], setSuggestions = _e[1];
    var _f = (0, react_1.useState)(null), searchInfo = _f[0], setSearchInfo = _f[1];
    var _g = (0, react_1.useState)(null), selectedSlot = _g[0], setSelectedSlot = _g[1];
    // Calculate duration from original times
    var durationMinutes = Math.round(((0, date_fns_1.parseISO)(originalEndTime).getTime() -
    ...
    var searchAlternativeSlots = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var formatSlotTime = function (slot) {
        ...
        return "".concat((0, date_fns_1.format)(startDate, "EEEE, dd/MM", { locale: locale_1.ptBR }), " \u2022 ").concat((0, date_fns_1.format)(startDate, "HH:mm", { locale: locale_1.ptBR }), "-").concat((0, date_fns_1.format)(endDate, "HH:mm", { locale: locale_1.ptBR }));
    };
    ...
    return (<card_1.Card className="w-full max-w-2xl">
      ...
      <card_1.CardContent>
        ...
        {/* No Slots Found */}
        {!isLoading && !hasSlots && (<div className="text-center py-8">
        ...
        {/* Slots Display */}
        {!isLoading && hasSlots && (<div className="space-y-6">
            {/* Search Info */}
            {searchInfo && (<div className="bg-blue-50 rounded-lg p-3">
                ...
                return (<div key={date} className="space-y-3">
                  <div className="flex items-center gap-2">
                    <h4 className="font-medium text-gray-900">
                      {(0, date_fns_1.format)(dateObj, "EEEE, dd/MM/yyyy", { locale: locale_1.ptBR })}
                    </h4>
                    {isToday && (<badge_1.Badge variant="default" className="text-xs">
                        Hoje
                      </badge_1.Badge>)}
                    {daysFromOriginal > 0 && (<badge_1.Badge variant="secondary" className="text-xs">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {slots.map(function (slot, index) {
                        ...
                        return (<card_1.Card key={index} className={"cursor-pointer transition-all hover:shadow-md ".concat(isSelected
                                ? "ring-2 ring-blue-500 bg-blue-50"
                                : "hover:bg-gray-50")} onClick={function () { return handleSlotSelect(slot); }}>
                          <card_1.CardContent className="p-4">
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <lucide_react_1.Clock className="h-4 w-4 text-blue-600"/>
                                <span className="font-medium">
                                  {(0, date_fns_1.format)((0, date_fns_1.parseISO)(slot.start_time), "HH:mm")} -{" "}
                                  {(0, date_fns_1.format)((0, date_fns_1.parseISO)(slot.end_time), "HH:mm")}
                                </span>
                              </div>
                              {score > 0 && (<badge_1.Badge className={"text-xs ".concat(getSlotScoreColor(score))}>
                                  <lucide_react_1.Star className="h-3 w-3 mr-1"/>
                                  {getSlotScoreLabel(score)}
                                </badge_1.Badge>)}
                            </div>

                            {slot.reason && (<p className="text-xs text-muted-foreground mb-2">
                                {slot.reason}
                              </p>)}

                            {slot.conflicts && slot.conflicts.length > 0 && (<div className="space-y-1">
                                {slot.conflicts.map(function (conflict, cIndex) { return (<div key={cIndex} className={"text-xs p-2 rounded ".concat(conflict.severity === "warning"
                            ...
                            <div className="flex items-center justify-between mt-3">
                              ...
                              <lucide_react_1.ArrowRight className={"h-4 w-4 transition-transform ".concat(isSelected
            ...
            <div className="flex items-center gap-3">
              <button_1.Button onClick={function () { return searchAlternativeSlots(); }} variant="outline" size="sm" className="flex items-center gap-2">
