...
function BusinessRulesConfigPanel(_a) {
    var _this = this;
    var clinicId = _a.clinicId, onConfigurationChange = _a.onConfigurationChange;
    var _b = (0, react_1.useState)("schedules"), activeTab = _b[0], setActiveTab = _b[1];
    var _c = (0, react_1.useState)(false), hasUnsavedChanges = _c[0], setHasUnsavedChanges = _c[1];
    var _d = (0, react_1.useState)({
        professionalSchedules: 0,
        holidays: 0,
        serviceRules: 0,
    }), configStats = _d[0], setConfigStats = _d[1];
    var handleConfigurationUpdate = function (type, count) {
        setConfigStats(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[type === "schedules"
                ? "professionalSchedules"
                : type === "holidays"
                    ? "holidays"
                    : "serviceRules"] = count, _a)));
        });
        setHasUnsavedChanges(true);
        onConfigurationChange === null || onConfigurationChange === void 0 ? void 0 : onConfigurationChange();
    };
    var saveAllConfigurations = function () { return __awaiter(_this, void 0, void 0, function () {
        var response, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/clinic/".concat(clinicId, "/business-rules/refresh"), {
                            method: "POST",
                        })];
                case 1:
                    response = _a.sent();
    ...
    var getConfigurationSummary = function () {
        var totalConfigs = configStats.professionalSchedules +
        ...
        if (configStats.holidays === 0)
            missingConfigs.push("feriados");
        ...
        <card_1.CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <div className="flex items-center gap-2">
                  {configSummary.status === "success" && (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>)}
                  <badge_1.Badge variant={configSummary.status === "success"
            ? "default"
            : configSummary.status === "warning"
              ...
              {hasUnsavedChanges && (<button_1.Button onClick={saveAllConfigurations} size="sm">
          ...
          <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <div className="p-6">
              ...
              <tabs_1.TabsContent value="holidays" className="mt-0">
                <clinic_holiday_manager_1.ClinicHolidayManager clinicId={clinicId} onHolidaysChange={function (holidays) {
            return handleConfigurationUpdate("holidays", holidays.length);
        }}/>
              </tabs_1.TabsContent>

              <tabs_1.TabsContent value="rules" className="mt-0">
                <service_type_rule_manager_1.ServiceTypeRuleManager clinicId={clinicId} onRulesChange={function (rules) {
