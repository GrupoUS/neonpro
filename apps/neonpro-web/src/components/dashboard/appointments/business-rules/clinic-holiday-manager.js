...
function ClinicHolidayManager(_a) {
    var _this = this;
    var clinicId = _a.clinicId, onHolidaysChange = _a.onHolidaysChange;
    // State management
    ...
    var loadHolidays = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    data = _a.sent();
                    setHolidays(data);
                    onHolidaysChange === null || onHolidaysChange === void 0 ? void 0 : onHolidaysChange(data);
                    _a.label = 3;
    ...
    var openDialog = function (holiday) {
        if (holiday) {
    ...
    var saveHoliday = function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                case 2:
                    ...
                    if (editingHoliday) {
                        setHolidays(function (prev) {
                            return prev.map(function (h) { return (h.id === editingHoliday.id ? savedHoliday_1 : h); });
                        });
                        sonner_1.toast.success("Feriado atualizado com sucesso!");
                    }
                    else {
                        setHolidays(function (prev) { return __spreadArray(__spreadArray([], prev, true), [savedHoliday_1], false); });
    ...
    var deleteHoliday = function (holidayId) { return __awaiter(_this, void 0, void 0, function () {
        var response, error_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/clinic/".concat(clinicId, "/holidays/").concat(holidayId), {
                            method: "DELETE",
                        })];
                case 1:
                    response = _a.sent();
                    ...
                    setHolidays(function (prev) { return prev.filter(function (h) { return h.id !== holidayId; }); });
    ...
    var toggleHolidayStatus = function (holidayId, isActive) { return __awaiter(_this, void 0, void 0, function () {
                ...
                case 1:
                    ...
                    setHolidays(function (prev) {
                        return prev.map(function (h) {
                            return h.id === holidayId ? __assign(__assign({}, h), { is_active: isActive }) : h;
                        });
                    });
                    sonner_1.toast.success("Feriado ".concat(isActive ? "ativado" : "desativado", " com sucesso!"));
                    return [3 /*break*/, 3];
                case 2:
                    error_4 = _a.sent();
    ...
    var getHolidayDateRange = function (holiday) {
        ...
        return "".concat((0, date_fns_1.format)(startDate, "dd/MM/yyyy", { locale: locale_1.ptBR }), " - ").concat((0, date_fns_1.format)(endDate, "dd/MM/yyyy", { locale: locale_1.ptBR }));
    };
    var getHolidayTimeRange = function (holiday) {
        if (!holiday.start_time || !holiday.end_time) {
            return "Dia todo";
        }
        return "".concat(holiday.start_time, " - ").concat(holiday.end_time);
    };
    var getRecurrenceLabel = function (holiday) {
    ...
    return (<card_1.Card>
          ...
          <dialog_1.Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <dialog_1.DialogContent className="sm:max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input_1.Input id="holiday-name" value={formData.name} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { name: e.target.value })); });
                ...
                <div>
                  ...
                  <textarea_1.Textarea id="holiday-description" value={formData.description} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { description: e.target.value })); });
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input_1.Input id="start-date" type="date" value={formData.start_date} onChange={function (e) {
            setFormData(function (prev) { return (__assign(__assign({}, prev), { start_date: e.target.value, end_date: prev.end_date || e.target.value })); });
        }}/>
                  </div>
                  <div>
                    <label_1.Label htmlFor="end-date">Data final *</label_1.Label>
                    <input_1.Input id="end-date" type="date" value={formData.end_date} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { end_date: e.target.value })); });
        }}/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input_1.Input id="start-time" type="time" value={formData.start_time} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { start_time: e.target.value })); });
        }}/>
                  </div>
                  <div>
                    <label_1.Label htmlFor="end-time">Fim (opcional)</label_1.Label>
                    <input_1.Input id="end-time" type="time" value={formData.end_time} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { end_time: e.target.value })); });
        }}/>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <switch_1.Switch checked={formData.is_recurring} onCheckedChange={function (checked) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { is_recurring: checked, recurrence_type: checked ? "yearly" : undefined })); });
        }}/>
                    <label_1.Label>Recorrente</label_1.Label>
                  </div>

                  {formData.is_recurring && (<select_1.Select value={formData.recurrence_type} onValueChange={function (value) {
                return setFormData(function (prev) { return (__assign(__assign({}, prev), { recurrence_type: value })); });
            }}>
      ...
      <card_1.CardContent>
        <div className="space-y-4">
          {holidays.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
            ...
            </div>) : (holidays.map(function (holiday) { return (<card_1.Card key={holiday.id} className={!holiday.is_active ? "opacity-60" : ""}>
                <card_1.CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-medium">{holiday.name}</h4>
                        {holiday.is_recurring && (<badge_1.Badge variant="secondary" className="text-xs">
                            <lucide_react_1.RefreshCw className="h-3 w-3 mr-1"/>
                            {getRecurrenceLabel(holiday)}
                          </badge_1.Badge>)}
                        {!holiday.is_active && (<badge_1.Badge variant="outline" className="text-xs">
                            Inativo
                          </badge_1.Badge>)}
                      </div>
                    ...
                    <div className="flex items-center gap-1">
                      <switch_1.Switch checked={holiday.is_active} onCheckedChange={function (checked) {
                return toggleHolidayStatus(holiday.id, checked);
            }}/>
                      <button_1.Button variant="ghost" size="sm" onClick={function () { return openDialog(holiday); }}>
                        <lucide_react_1.Edit className="h-3 w-3"/>
                      </button_1.Button>
                      <button_1.Button variant="ghost" size="sm" onClick={function () { return deleteHoliday(holiday.id); }} className="text-red-600 hover:text-red-700">
        ...
        {holidays.length > 0 && (<div className="mt-4 p-3 bg-blue-50 rounded-lg">
