...
function ProfessionalScheduleManager(_a) {
    var _this = this;
    var professionalId = _a.professionalId, _b = _a.professionalName, professionalName = _b === void 0 ? "Professional" : _b, clinicId = _a.clinicId, onSave = _a.onSave;
    // State management
    ...
    var _f = (0, react_1.useState)("schedule"), activeTab = _f[0], setActiveTab = _f[1];
    ...
    var initializeSchedules = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    return [3 /*break*/, 4];
                case 3:
                    defaultSchedules = DAYS_OF_WEEK.map(function (day) { return ({
        ...
        setSchedules(function (prev) {
            return prev.map(function (schedule) {
                return schedule.day_of_week === dayOfWeek
                    ? __assign(__assign(__assign({}, schedule), updates), { updated_at: new Date().toISOString() }) : schedule;
            });
        });
    };
    var saveSchedules = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    config = {
                        professional_id: professionalId,
                        schedules: schedules.map(function (s) { return ({
    ...
    var copyScheduleToAll = function (sourceDay) {
        var sourceSchedule = schedules.find(function (s) { return s.day_of_week === sourceDay; });
        if (!sourceSchedule)
            return;
        setSchedules(function (prev) {
            return prev.map(function (schedule) { return (__assign(__assign({}, schedule), { start_time: sourceSchedule.start_time, end_time: sourceSchedule.end_time, break_start_time: sourceSchedule.break_start_time, break_end_time: sourceSchedule.break_end_time, max_appointments_per_hour: sourceSchedule.max_appointments_per_hour, buffer_minutes_between: sourceSchedule.buffer_minutes_between, updated_at: new Date().toISOString() })); });
        });
      ...
      <card_1.CardContent>
        <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <tabs_1.TabsContent value="schedule" className="space-y-4">
            ...
            var schedule = schedules.find(function (s) { return s.day_of_week === day.value; });
            if (!schedule)
                return null;
            return (<card_1.Card key={day.value} className={schedule.is_available ? "" : "opacity-60"}>
                    <card_1.CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <switch_1.Switch checked={schedule.is_available} onCheckedChange={function (checked) {
                    return updateSchedule(day.value, {
                        is_available: checked,
                    });
                }}/>
                          <button_1.Button variant="ghost" size="sm" onClick={function () { return copyScheduleToAll(day.value); }} title="Copiar para todos os dias">
                            <lucide_react_1.Plus className="h-3 w-3"/>
                          </button_1.Button>
                        </div>
                      </div>

                      {schedule.is_available && (<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                            <label_1.Label htmlFor={"start-".concat(day.value)}>Início</label_1.Label>
                            <input_1.Input id={"start-".concat(day.value)} type="time" value={schedule.start_time} onChange={function (e) {
                        return updateSchedule(day.value, {
                          ...
                          <div>
                            ...
                            <input_1.Input id={"end-".concat(day.value)} type="time" value={schedule.end_time} onChange={function (e) {
                        return updateSchedule(day.value, {
                          ...
                          <div>
                            ...
                            <input_1.Input id={"break-start-".concat(day.value)} type="time" value={schedule.break_start_time || ""} onChange={function (e) {
                        return updateSchedule(day.value, {
                          ...
                          <div>
                            ...
                            <input_1.Input id={"break-end-".concat(day.value)} type="time" value={schedule.break_end_time || ""} onChange={function (e) {
                        return updateSchedule(day.value, {
                ...
                <card_1.CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input_1.Input id="min-notice" type="number" min="0" max="168" value={globalSettings.min_booking_notice_hours} onChange={function (e) {
            return setGlobalSettings(function (prev) { return (__assign(__assign({}, prev), { min_booking_notice_hours: parseInt(e.target.value) || 0 })); });
        }}/>
                    </div>
                    <div>
                      <label_1.Label htmlFor="max-days">
                        Máximo de dias antecedência
                      </label_1.Label>
                      <input_1.Input id="max-days" type="number" min="1" max="365" value={globalSettings.max_booking_days_ahead} onChange={function (e) {
            return setGlobalSettings(function (prev) { return (__assign(__assign({}, prev), { max_booking_days_ahead: parseInt(e.target.value) || 1 })); });
        }}/>
                    </div>
                    <div>
                      ...
                      <input_1.Input id="buffer-minutes" type="number" min="0" max="120" value={globalSettings.buffer_minutes_between} onChange={function (e) {
            return setGlobalSettings(function (prev) { return (__assign(__assign({}, prev), { buffer_minutes_between: parseInt(e.target.value) || 0 })); });
        }}/>
                    </div>
                    <div>
                      <label_1.Label htmlFor="max-per-hour">
                        Máx. consultas por hora
                      </label_1.Label>
                      <input_1.Input id="max-per-hour" type="number" min="1" max="20" value={globalSettings.max_appointments_per_hour} onChange={function (e) {
            return setGlobalSettings(function (prev) { return (__assign(__assign({}, prev), { max_appointments_per_hour: parseInt(e.target.value) || 1 })); });
        }}/>
                    </div>
                  </div>
                </card_1.CardContent>
                ...
                <card_1.CardContent>
                  <div className="space-y-2 text-sm">
                    <p>
                      ...
                      {schedules.filter(function (s) { return s.is_available; }).length} de 7
                    </p>
                    <p>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button_1.Button onClick={saveSchedules} disabled={isSaving} className="min-w-[120px]">
            {isSaving ? (<lucide_react_1.Clock className="h-4 w-4 animate-spin mr-2"/>) : (<lucide_react_1.Save className="h-4 w-4 mr-2"/>)}
