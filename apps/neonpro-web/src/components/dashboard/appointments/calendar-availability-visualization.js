...
function CalendarAvailabilityVisualization(_a) {
    var _this = this;
    var professionalId = _a.professionalId, clinicId = _a.clinicId, _b = _a.selectedDate, selectedDate = _b === void 0 ? new Date() : _b, _c = _a.view, view = _c === void 0 ? "week" : _c, serviceTypeId = _a.serviceTypeId, onSlotClick = _a.onSlotClick, _d = _a.showHeatMap, showHeatMap = _d === void 0 ? true : _d;
    var _e = (0, react_1.useState)([]), availability = _e[0], setAvailability = _e[1];
    var _f = (0, react_1.useState)(false), isLoading = _f[0], setIsLoading = _f[1];
    var _g = (0, react_1.useState)(true), showDetails = _g[0], setShowDetails = _g[1];
    var _h = (0, react_1.useState)(null), hoveredSlot = _h[0], setHoveredSlot = _h[1];
    ...
    var dateRange = (0, react_1.useMemo)(function () {
        switch (view) {
            ...
            case "month":
                return {
                    start: (0, date_fns_1.startOfWeek)((0, date_fns_1.startOfDay)(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1))),
                    end: (0, date_fns_1.endOfWeek)((0, date_fns_1.endOfDay)(new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0))),
                };
        }
    }, [selectedDate, view]);
    ...
    var loadAvailabilityData = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    params = new URLSearchParams(__assign({ professional_id: professionalId, clinic_id: clinicId, start_date: dateRange.start.toISOString(), end_date: dateRange.end.toISOString() }, (serviceTypeId && { service_type_id: serviceTypeId })));
    ...
    var getAvailabilityLevel = function (slot) {
        if (!slot.available)
            return "blocked";
        var hasErrors = slot.conflicts.some(function (c) { return c.severity === "error"; });
        if (hasErrors)
            return "blocked";
        var hasWarnings = slot.conflicts.some(function (c) { return c.severity === "warning"; });
        if (hasWarnings)
            return "low";
    ...
    var getSlotTooltipContent = function (slot) {
        ...
        switch (level) {
            case "high":
                ...
                break;
            case "medium":
                content += "Parcialmente ocupado (".concat(slot.capacity.used, "/").concat(slot.capacity.maximum, ")");
                break;
            case "low":
                content += "Pouco dispon\u00EDvel (".concat(slot.capacity.used, "/").concat(slot.capacity.maximum, ")");
                if (slot.conflicts.length > 0) {
                    content +=
                        "\nAvisos: " + slot.conflicts.map(function (c) { return c.message; }).join(", ");
                }
                break;
            case "blocked":
                ...
                if (slot.conflicts.length > 0) {
                    content +=
                        "\nMotivo: " + slot.conflicts.map(function (c) { return c.message; }).join(", ");
                }
                break;
            case "none":
    ...
    var getDaySummary = function (day) {
        var availablePercent = Math.round((day.summary.available_slots / day.summary.total_slots) * 100);
        var level;
    ...
    var renderDayView = function () {
          ...
          {dayData.slots.map(function (slot, index) {
                ...
                <tooltip_1.Tooltip>
                  <tooltip_1.TooltipTrigger asChild>
                    <div className={(0, utils_1.cn)("h-8 rounded border cursor-pointer transition-all", getAvailabilityColor(level))} onClick={function () { return onSlotClick === null || onSlotClick === void 0 ? void 0 : onSlotClick(slot); }} onMouseEnter={function () { return setHoveredSlot(slot); }} onMouseLeave={function () { return setHoveredSlot(null); }}>
        ...
        <div className="grid grid-cols-12 gap-1 text-xs text-muted-foreground">
          {dayData.slots
                ...
                .map(function (slot, index) { return (<div key={index} className="col-span-2 text-center">
    ...
    var renderWeekView = function () {
        ...
        <div className="grid grid-cols-7 gap-2">
          {weekDays.map(function (_a, dayIndex) {
                var date = _a.date, data = _a.data;
                var summary = data
                    ? getDaySummary(data)
                    : { percent: 0, level: "none" };
                return (<div key={dayIndex} className="space-y-2">
                ...
                {showHeatMap && data && (<div className="space-y-1">
                    {data.slots.map(function (slot, slotIndex) {
                            var level = getAvailabilityLevel(slot);
                            return (<tooltip_1.TooltipProvider key={slotIndex}>
                          <tooltip_1.Tooltip>
                            <tooltip_1.TooltipTrigger asChild>
                              <div className={(0, utils_1.cn)("h-4 rounded border cursor-pointer transition-all", getAvailabilityColor(level))} onClick={function () { return onSlotClick === null || onSlotClick === void 0 ? void 0 : onSlotClick(slot); }}/>
                            </tooltip_1.TooltipTrigger>
                            <tooltip_1.TooltipContent>
                              <div className="text-xs whitespace-pre-line">
                                {getSlotTooltipContent(slot)}
    ...
    var renderMonthView = function () {
        ...
        for (var day = new Date(dateRange.start); day <= dateRange.end; day = (0, date_fns_1.addDays)(day, 1)) {
            _loop_2(day);
        }
        ...
        return (<div className="space-y-2">
        {weeks.map(function (week, weekIndex) { return (<div key={weekIndex} className="grid grid-cols-7 gap-1">
            {week.map(function (_a, dayIndex) {
                    ...
                    return (<tooltip_1.TooltipProvider key={dayIndex}>
                  <tooltip_1.Tooltip>
                    <tooltip_1.TooltipTrigger asChild>
                      <div className={(0, utils_1.cn)("h-12 rounded border flex flex-col items-center justify-center cursor-pointer transition-all", getAvailabilityColor(summary.level), !isCurrentMonth && "opacity-50")}>
                        <div className="text-xs font-medium">
                    ...
                    <tooltip_1.TooltipContent>
                      <div className="text-xs">
                        ...
                        <br />
                        {data
                            ? "".concat(data.summary.available_slots, "/").concat(data.summary.total_slots, " slots dispon\u00EDveis")
                            : "Sem dados"}
                      </div>
                    </tooltip_1.TooltipContent>
                  </tooltip_1.Tooltip>
    ...
    var renderLegend = function () { return (<div className="flex items-center gap-4 text-xs">
    ...
    return (<card_1.Card className="p-4">
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button_1.Button variant="ghost" size="sm" onClick={function () { return setShowDetails(!showDetails); }}>
              {showDetails ? (<lucide_react_1.Eye className="h-4 w-4"/>) : (<lucide_react_1.EyeOff className="h-4 w-4"/>)}
            </button_1.Button>
          </div>
        </div>
        ...
        {/* Current hover info */}
        {hoveredSlot && (<div className="border-t pt-4">
            <div className="text-sm">
              ...
              {hoveredSlot.conflicts.length > 0 && (<>
                  ...
                  {hoveredSlot.conflicts.map(function (c) { return c.message; }).join(", ")}
                </>)}
            </div>
          </div>)}
      </div>
