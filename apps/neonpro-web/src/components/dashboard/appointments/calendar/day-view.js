...
function DayView(_a) {
    ...
    var getAppointmentsForSlot = function (slotTime) {
        return dayAppointments.filter(function (appointment) {
            ...
            return ((appointmentStart >= slotTime && appointmentStart < slotEnd) ||
    ...
    return (<div className={(0, utils_1.cn)("flex-1 flex flex-col", className)}>
      ...
      <scroll_area_1.ScrollArea className="flex-1">
        <div className="relative">
          ...
          <div className="grid grid-cols-1 gap-px bg-border">
            ...
            return (<div key={index} className={(0, utils_1.cn)("relative bg-background border-r border-border", isHourMark && "border-t-2 border-t-border/50", "hover:bg-muted/50 cursor-pointer transition-colors")} style={{ minHeight: "48px" }} onClick={function () { return handleTimeSlotClick(slotTime); }}>
                  {/* Time label */}
                  ...
                  <div className="pl-2 pr-2 pt-1">
                    {slotAppointments.map(function (appointment, appointmentIndex) { return (<div key={"".concat(appointment.id, "-").concat(appointmentIndex)} className="mb-1" style={{
                        height: "".concat(getAppointmentHeight(appointment), "px"),
                        minHeight: "40px",
                    }}>
                        <appointment_card_1.AppointmentCard appointment={appointment} variant="compact" showTime={true} showPatient={true} showProfessional={false} showActions={true} onClick={function () { return onAppointmentClick === null || onAppointmentClick === void 0 ? void 0 : onAppointmentClick(appointment); }} onEdit={function () { return onAppointmentEdit === null || onAppointmentEdit === void 0 ? void 0 : onAppointmentEdit(appointment); }} onCancel={function () { return onAppointmentCancel === null || onAppointmentCancel === void 0 ? void 0 : onAppointmentCancel(appointment); }} onComplete={function () { return onAppointmentComplete === null || onAppointmentComplete === void 0 ? void 0 : onAppointmentComplete(appointment); }} className="h-full"/>
                      </div>); })}
                  </div>
                </div>);
        })}
      ...
      {/* Empty state */}
      {dayAppointments.length === 0 && (<div className="flex-1 flex items-center justify-center p-8">
...
function CurrentTimeIndicator(_a) {
    ...
    return (<div className="absolute left-0 right-0 z-10 flex items-center pointer-events-none" style={{ top: "".concat(position, "px") }}>
