...
function WeekView(_a) {
    var date = _a.date, appointments = _a.appointments, onAppointmentClick = _a.onAppointmentClick, onAppointmentEdit = _a.onAppointmentEdit, onAppointmentCancel = _a.onAppointmentCancel, onAppointmentComplete = _a.onAppointmentComplete, onTimeSlotClick = _a.onTimeSlotClick, className = _a.className;
    // Get week boundaries
    ...
    var generateTimeSlots = function () {
        ...
        for (var hour = startHour; hour < endHour; hour++) {
            for (var _i = 0, _a = [0, 15, 30, 45]; _i < _a.length; _i++) {
                var minutes = _a[_i];
                slots.push({ hour: hour, minutes: minutes, time: "".concat(hour.toString().padStart(2, '0'), ":").concat(minutes.toString().padStart(2, '0')) });
            }
        }
        return slots;
    };
    ...
    var getAppointmentsForSlot = function (day, slotHour, slotMinutes) {
        return weekAppointments.filter(function (appointment) {
            ...
            if (!(0, date_fns_1.isSameDay)(appointmentStart, day))
                return false;
    ...
    return (<div className={(0, utils_1.cn)("flex-1 flex flex-col", className)}>
      ...
      <div className="grid grid-cols-8 border-b bg-muted/50">
        ...
        {/* Day headers */}
        {weekDays.map(function (day, index) { return (<div key={index} className={(0, utils_1.cn)("p-2 border-r text-center", (0, date_fns_1.isToday)(day) && "bg-primary/10")}>
            <div className="text-xs text-muted-foreground">
              {(0, date_fns_1.format)(day, "EEE", { locale: locale_1.ptBR })}
            </div>
            <div className={(0, utils_1.cn)("text-lg font-semibold", (0, date_fns_1.isToday)(day) && "text-primary")}>
      ...
      <scroll_area_1.ScrollArea className="flex-1">
        <div className="grid grid-cols-8 gap-px bg-border min-h-full">
          ...
          {timeSlots.map(function (_a, slotIndex) {
            var hour = _a.hour, minutes = _a.minutes, time = _a.time;
            var isHourMark = minutes === 0;
            return (<react_1.default.Fragment key={slotIndex}>
                {/* Time label column */}
                <div className={(0, utils_1.cn)("bg-background border-r p-1 text-xs text-muted-foreground text-right pr-2", isHourMark && "border-t-2 border-t-border/50")} style={{ minHeight: "48px" }}>
                  {isHourMark && time}
                </div>
                ...
                {weekDays.map(function (day, dayIndex) {
                    ...
                    return (<div key={"".concat(slotIndex, "-").concat(dayIndex)} className={(0, utils_1.cn)("bg-background border-r hover:bg-muted/50 cursor-pointer transition-colors relative", isHourMark && "border-t-2 border-t-border/50", (0, date_fns_1.isToday)(day) && "bg-primary/5")} style={{ minHeight: "48px" }} onClick={function () { return handleTimeSlotClick(day, time); }}>
                      {/* Appointments in this slot */}
                      <div className="p-1">
                        {slotAppointments.map(function (appointment, appointmentIndex) { return (<div key={"".concat(appointment.id, "-").concat(appointmentIndex)} className="mb-1">
                            <appointment_card_1.AppointmentCard appointment={appointment} variant="compact" showTime={false} showPatient={true} showProfessional={false} showActions={false} onClick={function () { return onAppointmentClick === null || onAppointmentClick === void 0 ? void 0 : onAppointmentClick(appointment); }} className="text-xs"/>
                          </div>); })}
                      </div>
                    </div>);
                })}
      ...
      <div className="p-4 border-t bg-muted/50">
        <div className="flex items-center justify-between text-sm">
          <span>
            {(0, date_fns_1.format)(weekStart, "d 'de' MMM", { locale: locale_1.ptBR })} - {(0, date_fns_1.format)(weekEnd, "d 'de' MMM 'de' yyyy", { locale: locale_1.ptBR })}
          </span>
...
function WeekCurrentTimeIndicator(_a) {
    ...
    var _b = react_1.default.useState(null), currentTime = _b[0], setCurrentTime = _b[1];
    ...
    return (<div className="absolute z-10 flex items-center pointer-events-none" style={{
