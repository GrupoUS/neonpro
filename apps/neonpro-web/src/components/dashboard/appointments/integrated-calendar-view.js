...
function IntegratedCalendarView(_a) {
    var _this = this;
    var events = _a.events, onEventClick = _a.onEventClick, onSlotSelect = _a.onSlotSelect, _b = _a.view, view = _b === void 0 ? 'week' : _b, _c = _a.date, date = _c === void 0 ? new Date() : _c, selectedProfessionalId = _a.selectedProfessionalId, selectedServiceId = _a.selectedServiceId, _d = _a.enableAlternativeSlots, enableAlternativeSlots = _d === void 0 ? true : _d, _e = _a.conflictDetection, conflictDetection = _e === void 0 ? true : _e, _f = _a.showPerformanceMetrics, showPerformanceMetrics = _f === void 0 ? false : _f, _g = _a.mobileOptimized, mobileOptimized = _g === void 0 ? true : _g, className = _a.className;
    // State management with performance optimization
    var _h = (0, react_1.useState)(view), currentView = _h[0], setCurrentView = _h[1];
    var _j = (0, react_1.useState)(date), currentDate = _j[0], setCurrentDate = _j[1];
    var _k = (0, react_1.useState)(null), selectedSlot = _k[0], setSelectedSlot = _k[1];
    var _l = (0, react_1.useState)(false), showAlternativesDialog = _l[0], setShowAlternativesDialog = _l[1];
    var _m = (0, react_1.useState)(null), conflictingEvent = _m[0], setConflictingEvent = _m[1];
    ...
    var processedEvents = (0, react_1.useMemo)(function () {
        return events.map(function (event) { return (__assign(__assign({}, event), { 
    ...
    var detectConflict = (0, react_1.useCallback)(function (slotInfo) {
        if (!conflictDetection)
            return null;
        var start = slotInfo.start, end = slotInfo.end;
    ...
    var handleSlotSelect = (0, react_1.useCallback)(function (slotInfo) { return __awaiter(_this, void 0, void 0, function () {
        var enhancedSlotInfo, conflict, alternativeRequest;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    enhancedSlotInfo = __assign(__assign({}, slotInfo), { suggested: false, alternativeScore: 0 });
                    ...
                    if (!(enableAlternativeSlots && selectedProfessionalId && selectedServiceId)) return [3 /*break*/, 2];
                    alternativeRequest = {
                        professional_id: selectedProfessionalId,
                        service_type_id: selectedServiceId,
                        preferred_start_time: (0, dayjs_1.default)(slotInfo.start).format('YYYY-MM-DD HH:mm:ss'),
                        duration_minutes: (0, dayjs_1.default)(slotInfo.end).diff((0, dayjs_1.default)(slotInfo.start), 'minutes'),
                ...
                case 3:
                    ...
                    setSelectedSlot(enhancedSlotInfo);
                    onSlotSelect === null || onSlotSelect === void 0 ? void 0 : onSlotSelect(enhancedSlotInfo);
                    _a.label = 4;
    ...
    var eventStyleGetter = (0, react_1.useCallback)(function (event) {
        ...
        if (((_a = event.resource) === null || _a === void 0 ? void 0 : _a.professionalId) === selectedProfessionalId) {
    ...
    var mobileProps = (0, react_1.useMemo)(function () {
        if (!mobileOptimized)
            return {};
        return {
            popup: true,
    ...
    var components = (0, react_1.useMemo)(function () { return ({
        ...
        event: function (_a) {
            var event = _a.event;
            return (<div className="flex items-center gap-1 truncate">
        {event.isAlternativeSlot && (<lucide_react_1.Star className="h-3 w-3 text-yellow-400 flex-shrink-0"/>)}
        <span className="truncate text-sm">{event.title}</span>
        {event.alternativeScore && event.alternativeScore > 80 && (<lucide_react_1.Zap className="h-3 w-3 text-green-400 flex-shrink-0"/>)}
      </div>);
        },
    }); }, []);
    return (<div className={(0, utils_1.cn)('space-y-4', className)}>
      {/* Performance metrics display (research-based) */}
      {showPerformanceMetrics && alternativeSlots.performanceMetrics && (<card_1.Card className="border-blue-200 bg-blue-50/30">
      ...
      <card_1.Card className="overflow-hidden">
        ...
        <card_1.CardContent className="p-0">
          <div className="h-[600px] p-4">
            <react_big_calendar_1.Calendar localizer={localizer} events={processedEvents} startAccessor="start" endAccessor="end" titleAccessor="title" view={currentView} date={currentDate} onView={setCurrentView} onNavigate={setCurrentDate} onSelectEvent={function (event) { return onEventClick === null || onEventClick === void 0 ? void 0 : onEventClick(event); }} onSelectSlot={handleSlotSelect} eventPropGetter={eventStyleGetter} components={components} selectable popup step={15} timeslots={4} min={(0, dayjs_1.default)().hour(6).minute(0).toDate()} // 6:00 AM
     max={(0, dayjs_1.default)().hour(22).minute(0).toDate()} // 10:00 PM
     formats={{
            timeGutterFormat: 'HH:mm',
            eventTimeRangeFormat: function (_a) {
                var start = _a.start, end = _a.end;
                return "".concat((0, dayjs_1.default)(start).format('HH:mm'), " - ").concat((0, dayjs_1.default)(end).format('HH:mm'));
            },
            ...
            showMore: function (total) { return "+".concat(total, " mais"); },
        }} {...mobileProps}/>
          </div>
        </card_1.CardContent>
      </card_1.Card>
        ...
        <dialog_1.DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <tabs_1.Tabs defaultValue="alternatives" className="w-full">
            ...
            <tabs_1.TabsContent value="conflict" className="space-y-4">
              {conflictingEvent && (<card_1.Card className="border-red-200 bg-red-50/50">
                  <card_1.CardHeader>
                    <card_1.CardTitle className="text-red-700 text-lg">Agendamento Conflitante</card_1.CardTitle>
                  </card_1.CardHeader>
                  <card_1.CardContent>
                    <div className="space-y-3">
                      ...
                      {conflictingEvent.resource && (<div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
              ...
              {selectedSlot && (<card_1.Card className="border-blue-200 bg-blue-50/50">
                  <card_1.CardHeader>
                    <card_1.CardTitle className="text-blue-700 text-lg">Horário Solicitado</card_1.CardTitle>
                  </card_1.CardHeader>
                  <card_1.CardContent>
                    <div className="flex items-center gap-2">
            ...
            onEventClick === null || onEventClick === void 0 ? void 0 : onEventClick(alternativeEvent);
            setShowAlternativesDialog(false);
        }} showPerformanceMetrics={showPerformanceMetrics} showBookingProbability={true} enableRealtimeFiltering={true}/>
            </tabs_1.TabsContent>
          </tabs_1.Tabs>
          
          <div className="flex justify-end gap-2 pt-4 border-t">
            <button_1.Button variant="outline" onClick={function () { return setShowAlternativesDialog(false); }}>
              Cancelar
            </button_1.Button>
            <button_1.Button onClick={function () {
            // Handle force booking despite conflict
            if (selectedSlot) {
                onSlotSelect === null || onSlotSelect === void 0 ? void 0 : onSlotSelect(selectedSlot);
                setShowAlternativesDialog(false);
            }
        }}>
              Forçar Agendamento
