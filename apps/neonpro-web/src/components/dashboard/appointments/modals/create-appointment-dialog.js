...
function CreateAppointmentDialog(_a) {
    var _this = this;
    var open = _a.open, onOpenChange = _a.onOpenChange, onCreateSuccess = _a.onCreateSuccess, defaultDate = _a.defaultDate, defaultTime = _a.defaultTime, professionalId = _a.professionalId;
    var _b = (0, react_1.useState)([]), patients = _b[0], setPatients = _b[1];
    var _c = (0, react_1.useState)([]), professionals = _c[0], setProfessionals = _c[1];
    var _d = (0, react_1.useState)([]), services = _d[0], setServices = _d[1];
    var _e = (0, react_1.useState)([]), availableSlots = _e[0], setAvailableSlots = _e[1];
    var _f = (0, react_1.useState)(''), conflictError = _f[0], setConflictError = _f[1];
    var _g = (0, react_1.useState)(false), checkingConflict = _g[0], setCheckingConflict = _g[1];
    ...
    var _k = (0, react_1.useState)(''), selectedDate = _k[0], setSelectedDate = _k[1];
    ...
    (0, react_1.useEffect)(function () {
        if (!open)
            return;
        var loadReferenceData = function () { return __awaiter(_this, void 0, void 0, function () {
            var _a, patientsRes, professionalsRes, servicesRes, patientsData, professionalsData, servicesData, error_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        _a = _b.sent(), patientsRes = _a[0], professionalsRes = _a[1], servicesRes = _a[2];
    ...
    var loadAvailableSlots = function (date, professionalId, serviceId) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fetch("/api/appointments/available-slots?" + new URLSearchParams({
                            professional_id: professionalId,
    ...
    var checkConflicts = function (professionalId, startTime, serviceId) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    if (!selectedService)
                        return [2 /*return*/];
                    endTime = new Date(new Date(startTime).getTime() + selectedService.duration_minutes * 60000).toISOString();
                    return [4 /*yield*/, fetch('/api/appointments/check-conflicts', {
                            method: 'POST',
                ...
                case 3:
                    data = _b.sent();
                    if (data.has_conflict && ((_a = data.conflicting_appointments) === null || _a === void 0 ? void 0 : _a.length)) {
                        conflicts = data.conflicting_appointments.map(function (c) { return "".concat(c.patient_name, " (").concat((0, date_fns_1.format)(new Date(c.start_time), 'HH:mm', { locale: locale_1.ptBR }), ")"); }).join(', ');
    ...
    (0, react_1.useEffect)(function () {
        var professionalId = watchedValues[0], startTime = watchedValues[1], serviceId = watchedValues[2];
    ...
    var onSubmit = function (data) { return __awaiter(_this, void 0, void 0, function () {
        var selectedService, startTime, endTime, appointmentData, response, result, conflicts, error_4;
        var _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                ...
                case 1:
                    _b.trys.push([1, 4, 5, 6]);
                    setIsCreating(true);
                    selectedService = services.find(function (s) { return s.id === data.service_type_id; });
                    if (!selectedService) {
                ...
                case 3:
                    ...
                    if (!response.ok) {
                        if ((_a = result.conflicts) === null || _a === void 0 ? void 0 : _a.length) {
                            conflicts = result.conflicts.map(function (c) { return "".concat(c.patient_name, " (").concat((0, date_fns_1.format)(new Date(c.start_time), 'HH:mm', { locale: locale_1.ptBR }), ")"); }).join(', ');
      ...
      <dialog_1.DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        ...
        <form_1.Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            {/* Conflict Alert */}
            {(conflictError || checkingConflict) && (<alert_1.Alert variant="destructive">
                      ...
                      <select_1.SelectContent>
                        {professionals.map(function (professional) { return (<select_1.SelectItem key={professional.id} value={professional.id}>
                            <div>
                              ...
                              {professional.specialization && (<div className="text-xs text-muted-foreground">{professional.specialization}</div>)}
                            </div>
                          </select_1.SelectItem>); })}
                      </select_1.SelectContent>
                    </select_1.Select>
                      ...
                      {services.map(function (service) { return (<select_1.SelectItem key={service.id} value={service.id}>
                          <div className="flex items-center justify-between w-full">
                            ...
                            <div className="flex items-center gap-2 ml-4">
                              ...
                              {service.price && (<badge_1.Badge variant="outline" className="text-xs">
            ...
            <div className="space-y-3">
              ...
              <input_1.Input type="date" value={selectedDate} onChange={function (e) { return handleDateChange(e.target.value); }} min={(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')} max={getMaxDate()}/>
            </div>

            {/* Available Slots */}
            {selectedDate && form.watch('professional_id') && form.watch('service_type_id') && (<div className="space-y-3">
                <form_1.FormLabel>
                  Horários disponíveis para {(0, date_fns_1.format)(new Date(selectedDate), "dd 'de' MMMM", { locale: locale_1.ptBR })}
                </form_1.FormLabel>
                ...
                {loadingSlots ? (<div className="flex items-center justify-center py-8">
                  ...
                  </div>) : availableSlots.length > 0 ? (<div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 max-h-40 overflow-y-auto">
                    {availableSlots.map(function (slot, index) { return (<button_1.Button key={index} type="button" variant={form.watch('start_time') === (0, date_fns_1.format)(new Date(slot.start_time), "yyyy-MM-dd'T'HH:mm")
                        ? "default"
                        : slot.is_available
                            ? "outline"
                            : "secondary"} size="sm" disabled={!slot.is_available} onClick={function () { return handleSlotSelect(slot); }} className="relative">
                        {(0, date_fns_1.format)(new Date(slot.start_time), 'HH:mm')}
                        {slot.is_available && form.watch('start_time') === (0, date_fns_1.format)(new Date(slot.start_time), "yyyy-MM-dd'T'HH:mm") && (<lucide_react_1.CheckCircle className="h-3 w-3 absolute -top-1 -right-1"/>)}
                      </button_1.Button>); })}
            ...
            <form_1.FormField control={form.control} name="start_time" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem>
                  <form_1.FormLabel>Ou digite data e horário manualmente</form_1.FormLabel>
                  <form_1.FormControl>
                    <input_1.Input type="datetime-local" {...field} min={getMinDateTime()} max={(0, date_fns_1.format)((0, date_fns_1.addDays)(new Date(), 90), "yyyy-MM-dd'T'23:59")}/>
                  </form_1.FormControl>
                  <form_1.FormMessage />
                </form_1.FormItem>);
        }}/>
            ...
            return (<form_1.FormItem>
              ...
              <div className="space-y-4">
                <form_1.FormField control={form.control} name="send_confirmation" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem className="flex flex-row items-start space-x-3 space-y-0">
                ...
                <form_1.FormField control={form.control} name="send_reminder" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem className="flex flex-row items-start space-x-3 space-y-0">
                      <form_1.FormControl>
                        <checkbox_1.Checkbox checked={field.value} onCheckedChange={field.onChange}/>
            ...
            return (<form_1.FormItem>
                    ...
                    <form_1.FormControl>
                      <textarea_1.Textarea placeholder="Observações visíveis ao cliente..." {...field}/>
                    </form_1.FormControl>
                    <form_1.FormMessage />
                  </form_1.FormItem>);
        }}/>
            ...
            return (<form_1.FormItem>
                    ...
                    <form_1.FormControl>
                      <textarea_1.Textarea placeholder="Observações internas da equipe..." {...field}/>
                    </form_1.FormControl>
                    <form_1.FormMessage />
                  </form_1.FormItem>);
        }}/>
            ...
            <dialog_1.DialogFooter className="gap-2 sm:gap-0">
              <button_1.Button type="button" variant="outline" onClick={function () { return onOpenChange(false); }} disabled={isCreating}>
                Cancelar
              </button_1.Button>
              <button_1.Button type="submit" disabled={isCreating || !!conflictError || checkingConflict || !form.watch('start_time')}>
