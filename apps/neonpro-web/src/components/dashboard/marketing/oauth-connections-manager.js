...
function OAuthConnectionsManager() {
    var _this = this;
    var _a = (0, react_1.useState)([]), connections = _a[0], setConnections = _a[1];
    var _b = (0, react_1.useState)(null), stats = _b[0], setStats = _b[1];
    var _c = (0, react_1.useState)(true), loading = _c[0], setLoading = _c[1];
    var _d = (0, react_1.useState)(null), refreshing = _d[0], setRefreshing = _d[1];
    var _e = (0, react_1.useState)(null), selectedConnection = _e[0], setSelectedConnection = _e[1];
    var _f = (0, react_1.useState)(false), showDeleteDialog = _f[0], setShowDeleteDialog = _f[1];
    var _g = (0, react_1.useState)(null), connectingPlatform = _g[0], setConnectingPlatform = _g[1];
    ...
    var initiateOAuthFlow = function (platform) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch("/api/oauth/".concat(platform, "/auth"), {
                            method: 'POST',
                        })];
                case 2:
                    response = _a.sent();
                ...
                case 3:
                    ...
                    return [3 /*break*/, 5];
                case 4: throw new Error('Failed to initiate OAuth flow');
    ...
    var refreshConnection = function (connectionId, platform) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch("/api/oauth/".concat(platform, "/refresh"), {
                            method: 'POST',
    ...
    var toggleSync = function (connectionId, enabled) { return __awaiter(_this, void 0, void 0, function () {
        var response, error_5;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/social-media/accounts/".concat(connectionId), {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
                        ...
                        setConnections(function (prev) {
                            return prev.map(function (conn) {
                                return conn.id === connectionId
                                    ? __assign(__assign({}, conn), { sync_enabled: enabled }) : conn;
                            });
                        });
                        sonner_1.toast.success("Sync ".concat(enabled ? 'enabled' : 'disabled', " successfully"));
                    }
                    else {
    ...
    var deleteConnection = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.trys.push([1, 3, 4, 5]);
                    return [4 /*yield*/, fetch("/api/oauth/".concat(selectedConnection.platform, "/").concat(selectedConnection.id), {
                            method: 'DELETE',
                        })];
                case 2:
                    response = _a.sent();
    ...
    var getStatusBadge = function (status) {
        switch (status) {
            case 'active':
                return <badge_1.Badge variant="default" className="bg-green-100 text-green-800"><lucide_react_1.CheckCircle className="w-3 h-3 mr-1"/>Active</badge_1.Badge>;
            case 'needs_reauth':
                return <badge_1.Badge variant="destructive" className="bg-yellow-100 text-yellow-800"><lucide_react_1.AlertTriangle className="w-3 h-3 mr-1"/>Needs Reauth</badge_1.Badge>;
            case 'error':
                return <badge_1.Badge variant="destructive"><lucide_react_1.XCircle className="w-3 h-3 mr-1"/>Error</badge_1.Badge>;
            default:
    ...
    var getTokenExpirationStatus = function (expiresAt) {
        ...
        if (hoursUntilExpiration < 24) {
            return (<div className="text-sm text-yellow-600">
    ...
    var getPlatformConfig = function (platform) {
        return PLATFORM_CONFIGS.find(function (config) { return config.platform === platform; });
    };
    if (loading) {
        return (<div className="space-y-4">
        <div className="h-8 bg-gray-200 rounded animate-pulse"/>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {__spreadArray([], Array(4), true).map(function (_, i) { return (<div key={i} className="h-32 bg-gray-200 rounded animate-pulse"/>); })}
        </div>
      </div>);
    }
    return (<div className="space-y-6">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            var isConnected = connections.some(function (conn) { return conn.platform === config.platform; });
            var Icon = config.icon;
            return (<card_1.Card key={config.platform} className={"relative overflow-hidden ".concat(isConnected ? 'opacity-50' : 'hover:shadow-md')}>
                  <card_1.CardContent className="p-4">
                    <div className={"w-12 h-12 rounded-lg ".concat(config.color, " flex items-center justify-center mb-3")}>
                      <Icon className="w-6 h-6 text-white"/>
                    </div>
                    <h3 className="font-semibold text-sm mb-1">{config.name}</h3>
                    <p className="text-xs text-gray-600 mb-3">{config.description}</p>
                    
                    {isConnected ? (<badge_1.Badge variant="secondary" className="text-xs">Connected</badge_1.Badge>) : (<button_1.Button size="sm" onClick={function () { return initiateOAuthFlow(config.platform); }} disabled={connectingPlatform === config.platform} className="w-full">
                        {connectingPlatform === config.platform ? (<lucide_react_1.RefreshCw className="w-4 h-4 mr-2 animate-spin"/>) : (<lucide_react_1.ExternalLink className="w-4 h-4 mr-2"/>)}
                        Connect
                      </button_1.Button>)}
                  </card_1.CardContent>
                </card_1.Card>);
      ...
      {/* Connected Accounts */}
      {connections.length > 0 && (<card_1.Card>
          <card_1.CardHeader>
            ...
            <div className="space-y-4">
              {connections.map(function (connection) {
                ...
                if (!config)
                    return null;
                var Icon = config.icon;
                return (<div key={connection.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-4">
                      <div className={"w-10 h-10 rounded-lg ".concat(config.color, " flex items-center justify-center")}>
                      ...
                      <div className="space-y-1">
                        ...
                        {connection.account_username && (<p className="text-sm text-gray-600">@{connection.account_username}</p>)}
                        
                        <div className="flex items-center space-x-4 text-xs text-gray-500">
                          {connection.last_sync_at && (<span className="flex items-center">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <switch_1.Switch checked={connection.sync_enabled} onCheckedChange={function (enabled) { return toggleSync(connection.id, enabled); }}/>
                      </div>
                      ...
                      <dropdown_menu_1.DropdownMenu>
                        ...
                        <dropdown_menu_1.DropdownMenuContent align="end">
                          <dropdown_menu_1.DropdownMenuItem onClick={function () { return refreshConnection(connection.id, connection.platform); }} disabled={refreshing === connection.id}>
                            <lucide_react_1.RefreshCw className={"w-4 h-4 mr-2 ".concat(refreshing === connection.id ? 'animate-spin' : '')}/>
                            Refresh Token
        ...
        <dialog_1.DialogContent>
          <dialog_1.DialogHeader>
            ...
            <dialog_1.DialogDescription>
              ...
              <strong>{selectedConnection === null || selectedConnection === void 0 ? void 0 : selectedConnection.account_name}</strong>? This action cannot be undone.
            </dialog_1.DialogDescription>
          </dialog_1.DialogHeader>
          <dialog_1.DialogFooter>
            <button_1.Button variant="outline" onClick={function () { return setShowDeleteDialog(false); }}>
              Cancel
            </button_1.Button>
