...
function BackgroundJobsMonitor() {
    ...
    var _c = (0, react_1.useState)(null), queueHealth = _c[0], setQueueHealth = _c[1];
    ...
    var _g = (0, react_1.useState)(null), selectedJob = _g[0], setSelectedJob = _g[1];
    var _h = (0, react_1.useState)(true), isAutoRefresh = _h[0], setIsAutoRefresh = _h[1];
    ...
    var loadQueueHealth = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var retryJob = function (jobId) { return __awaiter(_this, void 0, void 0, function () {
    ...
    var cancelJob = function (jobId) { return __awaiter(_this, void 0, void 0, function () {
        var response, error_5;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/jobs/".concat(jobId, "/cancel"), {
                            method: 'POST',
                        })];
                case 1:
                    response = _a.sent();
    ...
    var deleteJob = function (jobId) { return __awaiter(_this, void 0, void 0, function () {
    ...
    var getStatusBadge = function (status) {
        switch (status) {
            case 'completed':
                return <badge_1.Badge className="bg-green-100 text-green-800"><lucide_react_1.CheckCircle className="w-3 h-3 mr-1"/>Completed</badge_1.Badge>;
            case 'running':
                return <badge_1.Badge className="bg-blue-100 text-blue-800"><lucide_react_1.RefreshCw className="w-3 h-3 mr-1 animate-spin"/>Running</badge_1.Badge>;
            case 'failed':
                return <badge_1.Badge variant="destructive"><lucide_react_1.XCircle className="w-3 h-3 mr-1"/>Failed</badge_1.Badge>;
            case 'cancelled':
                return <badge_1.Badge variant="secondary"><lucide_react_1.XCircle className="w-3 h-3 mr-1"/>Cancelled</badge_1.Badge>;
            default:
                return <badge_1.Badge variant="outline"><lucide_react_1.Clock className="w-3 h-3 mr-1"/>Pending</badge_1.Badge>;
        }
    };
    var getPriorityBadge = function (priority) {
        switch (priority) {
            case 'critical':
                return <badge_1.Badge variant="destructive" className="text-xs">Critical</badge_1.Badge>;
            case 'high':
                return <badge_1.Badge className="bg-orange-100 text-orange-800 text-xs">High</badge_1.Badge>;
            case 'normal':
                return <badge_1.Badge variant="secondary" className="text-xs">Normal</badge_1.Badge>;
            default:
                return <badge_1.Badge variant="outline" className="text-xs">Low</badge_1.Badge>;
        }
    };
    ...
    var formatRelativeTime = function (timestamp) {
        ...
        if (diffMs < 60000)
            return 'Just now';
        if (diffMs < 3600000)
            return "".concat(Math.round(diffMs / 60000), "m ago");
        if (diffMs < 86400000)
            return "".concat(Math.round(diffMs / 3600000), "h ago");
    ...
    return (<div className="space-y-6">
      {/* Stats Overview */}
      {stats && (<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <card_1.Card>
      ...
      <card_1.Card>
        <card_1.CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <card_1.CardDescription>Monitor and manage background job execution</card_1.CardDescription>
            </div>
            
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <button_1.Button variant={isAutoRefresh ? "default" : "outline"} size="sm" onClick={function () { return setIsAutoRefresh(!isAutoRefresh); }}>
                  {isAutoRefresh ? <lucide_react_1.Pause className="w-4 h-4 mr-2"/> : <lucide_react_1.Play className="w-4 h-4 mr-2"/>}
                  Auto Refresh
                </button_1.Button>
          ...
          <div className="flex items-center space-x-4">
            ...
            return (<select_1.SelectItem key={key} value={key}>{config.name}</select_1.SelectItem>);
        })}
              </select_1.SelectContent>
            </select_1.Select>
          </div>
        ...
        <card_1.CardContent>
          <table_1.Table>
            ...
            return (<table_1.TableRow key={job.id}>
                    <table_1.TableCell>
                      <div className="flex items-center space-x-3">
                        <div className={"w-8 h-8 rounded ".concat(config.color, " flex items-center justify-center")}>
                          <Icon className="w-4 h-4 text-white"/>
                        </div>
                        <div>
                          <p className="font-medium text-sm">{config.name}</p>
                          {job.platform && (<p className="text-xs text-gray-500">Platform: {job.platform}</p>)}
                        </div>
                      </div>
                    </table_1.TableCell>
                    ...
                    <table_1.TableCell className="text-sm">{formatRelativeTime(job.created_at)}</table_1.TableCell>
                      ...
                      <div className="flex items-center space-x-2">
                        <dialog_1.Dialog>
                          <dialog_1.DialogTrigger asChild>
                            <button_1.Button variant="ghost" size="sm" onClick={function () { return setSelectedJob(job); }}>
                        ...
                        {job.status === 'failed' && job.attempts < job.max_attempts && (<button_1.Button variant="ghost" size="sm" onClick={function () { return retryJob(job.id); }}>
                            <lucide_react_1.RefreshCw className="w-4 h-4"/>
                          </button_1.Button>)}
                        
                        {(job.status === 'pending' || job.status === 'running') && (<button_1.Button variant="ghost" size="sm" onClick={function () { return cancelJob(job.id); }}>
                            <lucide_react_1.XCircle className="w-4 h-4"/>
                          </button_1.Button>)}
                        
                        {(job.status === 'completed' || job.status === 'failed' || job.status === 'cancelled') && (<button_1.Button variant="ghost" size="sm" onClick={function () { return deleteJob(job.id); }}>
