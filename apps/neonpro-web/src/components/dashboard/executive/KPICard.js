...
var getTrendIcon = function (direction, iconSize) {
    var className = "".concat(iconSize, " ").concat(direction === 'up' ? 'text-green-600' :
...
var KPICard = function (_a) {
    var kpi = _a.kpi, _b = _a.size, size = _b === void 0 ? 'md' : _b, _c = _a.showTrend, showTrend = _c === void 0 ? true : _c, _d = _a.showTarget, showTarget = _d === void 0 ? true : _d, _e = _a.showActions, showActions = _e === void 0 ? true : _e, onClick = _a.onClick, onDrillDown = _a.onDrillDown, onAlert = _a.onAlert;
    ...
    // RENDER HELPERS
    // ============================================================================
    var renderHeader = function () { return (<card_1.CardHeader className={"".concat(sizeVariant.padding, " pb-2")}>
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <card_1.CardTitle className={"".concat(sizeVariant.titleClass, " font-medium text-gray-900 truncate")}>
            {kpi.name}
          </card_1.CardTitle>
          {kpi.description && size !== 'sm' && (<tooltip_1.TooltipProvider>
              <tooltip_1.Tooltip>
        ...
        <div className="flex items-center gap-2">
          ...
          {showActions && (<dropdown_menu_1.DropdownMenu>
              <dropdown_menu_1.DropdownMenuTrigger asChild>
                <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <lucide_react_1.MoreVertical className="h-3 w-3"/>
                </button_1.Button>
              </dropdown_menu_1.DropdownMenuTrigger>
              <dropdown_menu_1.DropdownMenuContent align="end">
                {onDrillDown && (<dropdown_menu_1.DropdownMenuItem onClick={handleDrillDown}>
                    Ver Detalhes
                  </dropdown_menu_1.DropdownMenuItem>)}
                {onAlert && kpi.status === 'critical' && (<dropdown_menu_1.DropdownMenuItem onClick={handleAlert}>
                    Criar Alerta
                  </dropdown_menu_1.DropdownMenuItem>)}
    ...
    var renderContent = function () { return (<card_1.CardContent className={"".concat(sizeVariant.padding, " pt-0")}>
      <div className="space-y-2">
        {/* Main Value */}
        <div className="flex items-baseline justify-between">
          <span className={"".concat(sizeVariant.valueClass, " font-bold ").concat(variant.textColor)}>
            {formattedValue}
          </span>
          
          {showTrend && kpi.trend && (<div className="flex items-center gap-1">
              {getTrendIcon(kpi.trend.direction, sizeVariant.iconSize)}
              {kpi.trend.changePercent && size !== 'sm' && (<span className={"text-xs ".concat(kpi.trend.direction === 'up' ? 'text-green-600' :
        ...
        <div className="flex items-center justify-between">
          ...
          {kpi.lastUpdated && size !== 'sm' && (<span className="text-xs text-gray-500">
        ...
        {/* Target Progress */}
        {showTarget && kpi.target && targetProgress !== null && size !== 'sm' && (<div className="space-y-1">
            <div className="flex items-center justify-between text-xs text-gray-600">
              <span>Meta: {formattedTarget}</span>
              <span>{targetProgress.toFixed(0)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div className={"h-1.5 rounded-full transition-all duration-300 ".concat(targetProgress >= 100 ? 'bg-green-500' :
                targetProgress >= 80 ? 'bg-blue-500' :
                    targetProgress >= 60 ? 'bg-yellow-500' : 'bg-red-500')} style={{ width: "".concat(Math.min(100, targetProgress), "%") }}/>
            </div>
          </div>)}
        
        {/* Trend Information */}
        {showTrend && kpi.trend && size === 'lg' && (<div className="pt-2 border-t border-gray-100">
            <tooltip_1.TooltipProvider>
              <tooltip_1.Tooltip>
                <tooltip_1.TooltipTrigger asChild>
    ...
    // MAIN RENDER
    // ============================================================================
    return (<card_1.Card className={"\n        group relative transition-all duration-200 cursor-pointer\n        ".concat(sizeVariant.cardClass, "\n        ").concat(variant.bgColor, "\n        ").concat(variant.borderColor, "\n        ").concat(isHovered ? 'shadow-md scale-105' : 'shadow-sm', "\n        ").concat(onClick ? 'hover:shadow-lg' : '', "\n      ")} onMouseEnter={function () { return setIsHovered(true); }} onMouseLeave={function () { return setIsHovered(false); }} onClick={handleCardClick}>
      {renderHeader()}
      {renderContent()}
      
      {/* Loading Overlay */}
      {kpi.isLoading && (<div className="absolute inset-0 bg-white/50 flex items-center justify-center rounded-lg">
          <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"/>
        </div>)}
      
      {/* Critical Alert Indicator */}
      {kpi.status === 'critical' && (<div className="absolute -top-1 -right-1">
