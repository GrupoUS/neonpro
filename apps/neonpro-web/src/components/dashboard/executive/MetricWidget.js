...
function MetricWidget(_a) {
    ...
    var config = __assign({ showTrend: true, showTarget: true, showStatus: true, showSparkline: false, colorScheme: 'default', size: 'md', layout: 'vertical' }, widget.config);
    // Fetch metric data
    ...
    var metricValue = (0, react_1.useMemo)(function () {
        if (!data)
            return null;
        return {
            current: data.value,
    ...
    var metricTrend = (0, react_1.useMemo)(function () {
        if (!metricValue || metricValue.previous === undefined)
            return null;
        var change = metricValue.current - metricValue.previous;
        var percentage = metricValue.previous !== 0
    ...
    var metricStatus = (0, react_1.useMemo)(function () {
        if (!metricValue || !data)
            return 'neutral';
        if (data.status)
            return data.status;
    ...
    var renderTrendIndicator = function () {
        if (!config.showTrend || !metricTrend)
            return null;
        var TrendIcon = metricTrend.direction === 'up' ? lucide_react_1.TrendingUp :
            metricTrend.direction === 'down' ? lucide_react_1.TrendingDown : lucide_react_1.Minus;
        var trendColor = metricTrend.isGood ? 'text-green-600' : 'text-red-600';
        return (<div className={"flex items-center gap-1 ".concat(trendColor)}>
    ...
    var renderTargetIndicator = function () {
        if (!config.showTarget || !(metricValue === null || metricValue === void 0 ? void 0 : metricValue.target))
            return null;
    ...
    var statusConfig = STATUS_CONFIG[metricStatus];
    return (<card_1.Card className={"h-full transition-all duration-200 hover:shadow-md ".concat(config.colorScheme !== 'default' ? statusConfig.bgColor : '', " ").concat(config.colorScheme !== 'default' ? statusConfig.borderColor : '')}>
      <card_1.CardHeader className="pb-2">
        <div className="flex items-center justify-between">
      ...
      <card_1.CardContent className="pt-0">
        <div className={"space-y-2 ".concat(config.layout === 'horizontal' ? 'flex items-center justify-between' : '')}>
          {/* Main Value */}
          <div className={"".concat(config.size === 'lg' ? 'text-3xl' :
            config.size === 'sm' ? 'text-xl' : 'text-2xl', " font-bold")}>
            {formatValue(metricValue.current, metricValue.format)}
            {metricValue.unit && (<span className="text-sm font-normal text-muted-foreground ml-1">
                {metricValue.unit}
              </span>)}
          </div>
          ...
          {metricValue.previous !== undefined && (<div className="text-xs text-muted-foreground">
