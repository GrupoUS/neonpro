...
function DashboardGrid(_a) {
    var layout = _a.layout, widgets = _a.widgets, _b = _a.isEditing, isEditing = _b === void 0 ? false : _b, onLayoutChange = _a.onLayoutChange, onWidgetAdd = _a.onWidgetAdd, onWidgetUpdate = _a.onWidgetUpdate, onWidgetDelete = _a.onWidgetDelete, onWidgetDuplicate = _a.onWidgetDuplicate, onSaveLayout = _a.onSaveLayout, onResetLayout = _a.onResetLayout, className = _a.className;
    var _c = (0, react_1.useState)({
        isOpen: false,
        mode: 'add'
    }), widgetDialog = _c[0], setWidgetDialog = _c[1];
    var _d = (0, react_1.useState)(''), selectedPreset = _d[0], setSelectedPreset = _d[1];
    var _e = (0, react_1.useState)('lg'), currentBreakpoint = _e[0], setCurrentBreakpoint = _e[1];
    ...
    var _h = (0, react_1.useState)(new Set()), hiddenWidgets = _h[0], setHiddenWidgets = _h[1];
    var _j = (0, react_1.useState)(new Set()), lockedWidgets = _j[0], setLockedWidgets = _j[1];
    // Convert widgets to grid layout format
    var gridLayouts = (0, react_1.useMemo)(function () {
        var layouts = {};
        Object.keys(BREAKPOINTS).forEach(function (breakpoint) {
            layouts[breakpoint] = widgets
                .filter(function (widget) { return !hiddenWidgets.has(widget.id); })
                .map(function (widget) { return ({
                i: widget.id,
    ...
    var handleDeleteWidget = (0, react_1.useCallback)(function (widgetId) {
        if (onWidgetDelete) {
            onWidgetDelete(widgetId);
        }
    }, [onWidgetDelete]);
    var handleDuplicateWidget = (0, react_1.useCallback)(function (widgetId) {
    ...
    // Apply layout preset
    var handleApplyPreset = (0, react_1.useCallback)(function (presetId) {
        var _a;
        var preset = LAYOUT_PRESETS.find(function (p) { return p.id === presetId; });
        if (preset && onLayoutChange) {
            // Convert preset to grid layout format
            var newLayout = preset.layout.areas.map(function (area) { return ({
                i: area.id,
                x: area.x,
                y: area.y,
                w: area.width,
    ...
    var renderWidget = (0, react_1.useCallback)(function (widget) {
        var commonProps = {
            widget: widget,
            isEditing: isEditing,
            onUpdate: function (updates) { return onWidgetUpdate === null || onWidgetUpdate === void 0 ? void 0 : onWidgetUpdate(widget.id, updates); }
        };
        switch (widget.widgetType) {
            ...
            default:
                return (<card_1.Card className="h-full flex items-center justify-center">
    ...
    return (<div className={"dashboard-grid ".concat(className || '')}>
      {/* Grid Controls */}
      {isEditing && (<div className="mb-4 flex items-center justify-between bg-muted/50 p-4 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <select_1.Select value={selectedPreset} onValueChange={setSelectedPreset}>
              ...
              <select_1.SelectContent>
                {LAYOUT_PRESETS.map(function (preset) { return (<select_1.SelectItem key={preset.id} value={preset.id}>
                    <div className="flex items-center gap-2">
            ...
            {selectedPreset && (<button_1.Button onClick={function () { return handleApplyPreset(selectedPreset); }} size="sm" variant="outline">
                Apply
              </button_1.Button>)}
          </div>
            ...
            <tooltip_1.TooltipProvider>
              <tooltip_1.Tooltip>
                <tooltip_1.TooltipTrigger asChild>
                  <button_1.Button onClick={function () { return setCurrentBreakpoint('xxs'); }} size="sm" variant={currentBreakpoint === 'xxs' ? 'default' : 'outline'}>
              ...
              <tooltip_1.Tooltip>
                <tooltip_1.TooltipTrigger asChild>
                  <button_1.Button onClick={function () { return setCurrentBreakpoint('sm'); }} size="sm" variant={currentBreakpoint === 'sm' ? 'default' : 'outline'}>
              ...
              <tooltip_1.Tooltip>
                <tooltip_1.TooltipTrigger asChild>
                  <button_1.Button onClick={function () { return setCurrentBreakpoint('lg'); }} size="sm" variant={currentBreakpoint === 'lg' ? 'default' : 'outline'}>
      ...
      {/* Responsive Grid */}
      <ResponsiveGridLayout className="layout" layouts={gridLayouts} breakpoints={BREAKPOINTS} cols={COLS} rowHeight={60} margin={[layout.gap || 16, layout.gap || 16]} containerPadding={[0, 0]} isDraggable={isEditing} isResizable={isEditing} onLayoutChange={handleLayoutChange} onBreakpointChange={setCurrentBreakpoint} onDragStart={function () { return setIsDragging(true); }} onDragStop={function () { return setIsDragging(false); }} onResizeStart={function () { return setIsResizing(true); }} onResizeStop={function () { return setIsResizing(false); }} useCSSTransforms preventCollision={false} compactType="vertical">
        {widgets
            .filter(function (widget) { return !hiddenWidgets.has(widget.id); })
            .map(function (widget) { return (<div key={widget.id} className="relative group">
              {/* Widget Controls */}
              {isEditing && (<div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="flex items-center gap-1 bg-background/90 backdrop-blur-sm rounded-md p-1 shadow-sm border">
                    <tooltip_1.TooltipProvider>
                      <tooltip_1.Tooltip>
                        <tooltip_1.TooltipTrigger asChild>
                          <button_1.Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={function () { return handleEditWidget(widget); }}>
                      ...
                      <tooltip_1.Tooltip>
                        <tooltip_1.TooltipTrigger asChild>
                          <button_1.Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={function () { return handleDuplicateWidget(widget.id); }}>
                      ...
                      <tooltip_1.Tooltip>
                        <tooltip_1.TooltipTrigger asChild>
                          <button_1.Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={function () { return handleToggleWidgetLock(widget.id); }}>
                            {lockedWidgets.has(widget.id) ? (<lucide_react_1.Lock className="h-3 w-3"/>) : (<lucide_react_1.Unlock className="h-3 w-3"/>)}
                          </button_1.Button>
                        </tooltip_1.TooltipTrigger>
                      ...
                      <tooltip_1.Tooltip>
                        <tooltip_1.TooltipTrigger asChild>
                          <button_1.Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={function () { return handleToggleWidgetVisibility(widget.id); }}>
                      ...
                      <tooltip_1.Tooltip>
                        <tooltip_1.TooltipTrigger asChild>
                          <button_1.Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-destructive hover:text-destructive" onClick={function () { return handleDeleteWidget(widget.id); }}>
              ...
              {isEditing && (<div className="absolute top-2 left-2 z-10 flex gap-1">
                  {lockedWidgets.has(widget.id) && (<badge_1.Badge variant="secondary" className="h-5 px-1 text-xs">
      ...
      {isEditing && hiddenWidgets.size > 0 && (<div className="mt-4 p-4 bg-muted/30 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {widgets
                .filter(function (widget) { return hiddenWidgets.has(widget.id); })
                .map(function (widget) { return (<badge_1.Badge key={widget.id} variant="outline" className="cursor-pointer hover:bg-muted" onClick={function () { return handleToggleWidgetVisibility(widget.id); }}>
      ...
      {/* Widget Dialog */}
      <WidgetConfigDialog isOpen={widgetDialog.isOpen} mode={widgetDialog.mode} widget={widgetDialog.widget} onClose={function () { return setWidgetDialog({ isOpen: false, mode: 'add' }); }} onSave={function (widget) {
...
function WidgetConfigDialog(_a) {
    var _b, _c, _d, _e, _f;
    var isOpen = _a.isOpen, mode = _a.mode, widget = _a.widget, onClose = _a.onClose, onSave = _a.onSave;
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input_1.Input id="title" value={formData.title || ''} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { title: e.target.value })); }); }} placeholder="Enter widget title"/>
            </div>
            
            <div className="space-y-2">
              <label_1.Label htmlFor="type">Widget Type</label_1.Label>
              <select_1.Select value={formData.widgetType} onValueChange={function (value) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { widgetType: value })); }); }}>
                <select_1.SelectTrigger>
                  <select_1.SelectValue />
                </select_1.SelectTrigger>
                <select_1.SelectContent>
                  {WIDGET_TYPES.map(function (type) { return (<select_1.SelectItem key={type.value} value={type.value}>
                      <div>
          ...
          <div className="space-y-2">
            ...
            <input_1.Input id="dataSource" value={formData.dataSource || ''} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { dataSource: e.target.value })); }); }} placeholder="Enter data source endpoint or query"/>
          </div>
          
          <div className="grid grid-cols-4 gap-4">
            <div className="space-y-2">
              <label_1.Label htmlFor="width">Width</label_1.Label>
              <input_1.Input id="width" type="number" min="1" max="12" value={((_b = formData.position) === null || _b === void 0 ? void 0 : _b.width) || 4} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { position: __assign(__assign({}, prev.position), { width: parseInt(e.target.value) }) })); }); }}/>
            </div>
            
            <div className="space-y-2">
              <label_1.Label htmlFor="height">Height</label_1.Label>
              <input_1.Input id="height" type="number" min="1" max="10" value={((_c = formData.position) === null || _c === void 0 ? void 0 : _c.height) || 3} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { position: __assign(__assign({}, prev.position), { height: parseInt(e.target.value) }) })); }); }}/>
            </div>
            
            <div className="space-y-2">
              <label_1.Label htmlFor="x">X Position</label_1.Label>
              <input_1.Input id="x" type="number" min="0" max="11" value={((_d = formData.position) === null || _d === void 0 ? void 0 : _d.x) || 0} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { position: __assign(__assign({}, prev.position), { x: parseInt(e.target.value) }) })); }); }}/>
            </div>
            
            <div className="space-y-2">
              <label_1.Label htmlFor="y">Y Position</label_1.Label>
              <input_1.Input id="y" type="number" min="0" value={((_e = formData.position) === null || _e === void 0 ? void 0 : _e.y) || 0} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { position: __assign(__assign({}, prev.position), { y: parseInt(e.target.value) }) })); }); }}/>
            </div>
          </div>
          
          <div className="space-y-2">
            <label_1.Label htmlFor="refreshInterval">Refresh Interval (seconds)</label_1.Label>
            <select_1.Select value={((_f = formData.refreshInterval) === null || _f === void 0 ? void 0 : _f.toString()) || '300'} onValueChange={function (value) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { refreshInterval: parseInt(value) })); }); }}>
              <select_1.SelectTrigger>
                <select_1.SelectValue />
              </select_1.SelectTrigger>
              <select_1.SelectContent>
