...
const getTrendIcon = (direction: TrendDirection, iconSize: string) => {
  const className = `${iconSize} ${
    direction === "up" ? "text-green-600" : direction === "down" ? "text-red-600" : "text-gray-400"
  }`;
          ...
          {showActions && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {onDrillDown && (
                  <DropdownMenuItem onClick={handleDrillDown}>Ver Detalhes</DropdownMenuItem>
                )}
                {onAlert && kpi.status === "critical" && (
                  <DropdownMenuItem onClick={handleAlert}>Criar Alerta</DropdownMenuItem>
                )}
                <DropdownMenuSeparator />
