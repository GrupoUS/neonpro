...
var CustomTooltip = function (_a) {
    ...
    if (active && payload && payload.length) {
        ...
        {payload.map(function (entry, index) { return (<div key={index} className="flex items-center gap-2 text-xs">
...
function ConversionCharts(_a) {
    var _this = this;
    var className = _a.className, _b = _a.timeRange, timeRange = _b === void 0 ? '30d' : _b;
    var _c = (0, react_1.useState)(null), data = _c[0], setData = _c[1];
    var _d = (0, react_1.useState)(true), loading = _d[0], setLoading = _d[1];
    var _e = (0, react_1.useState)(timeRange), selectedTimeRange = _e[0], setSelectedTimeRange = _e[1]; // Fetch chart data
    (0, react_1.useEffect)(function () {
        var fetchChartData = function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        setLoading(true);
                        return [4 /*yield*/, fetch("/api/analytics/charts?timeRange=".concat(selectedTimeRange))];
                    case 1:
                        response = _a.sent();
    ...
    return (<div className={(0, utils_1.cn)('space-y-6', className)}>
      ...
      <tabs_1.Tabs defaultValue="trend" className="space-y-4">
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.LineChart data={mockConversionTrend}>
                    ...
                    <recharts_1.XAxis dataKey="period" stroke="hsl(var(--muted-foreground))" fontSize={12}/>
                    <recharts_1.YAxis stroke="hsl(var(--muted-foreground))" fontSize={12}/>
                    <recharts_1.Tooltip content={<CustomTooltip />}/>
                    <recharts_1.Line type="monotone" dataKey="conversionRate" stroke={CHART_COLORS.chart1} strokeWidth={2} dot={{ fill: CHART_COLORS.chart1, r: 4 }}/>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.AreaChart data={mockConversionTrend}>
                    ...
                    <recharts_1.YAxis stroke="hsl(var(--muted-foreground))" fontSize={12}/>
                    <recharts_1.Tooltip content={<CustomTooltip />}/>
                    <recharts_1.Area type="monotone" dataKey="revenue" stroke={CHART_COLORS.success} fill={CHART_COLORS.success} fillOpacity={0.1}/>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {mockFunnelData.map(function (stage, index) { return (<div key={stage.stage} className="relative">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        {index > 0 && (<div className="flex items-center text-red-600">
                    ...
                    <div className="w-full bg-muted rounded-full h-3">
                      <div className="bg-gradient-to-r from-chart-1 to-chart-2 h-3 rounded-full transition-all duration-500" style={{
                width: "".concat(stage.percentage, "%"),
                background: "linear-gradient(to right, ".concat(CHART_COLORS.chart1, ", ").concat(CHART_COLORS.chart2, ")")
            }}/>
                    </div>
                  </div>); })}
              </div>
        ...
        </tabs_1.TabsContent>        {/* Traffic Sources */}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <card_1.Card>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.PieChart>
                    <recharts_1.Pie data={mockSourceData} cx="50%" cy="50%" outerRadius={80} dataKey="conversions" label={function (_a) {
            var name = _a.name, value = _a.value, percent = _a.percent;
            return "".concat(name, ": ").concat(value, " (").concat((percent * 100).toFixed(1), "%)");
        }}>
                      {mockSourceData.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.fill}/>); })}
                    </recharts_1.Pie>
                    <recharts_1.Tooltip content={<CustomTooltip />}/>
                  </recharts_1.PieChart>
                </recharts_1.ResponsiveContainer>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.BarChart data={mockSourceData}>
                    <recharts_1.CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted))"/>
                    <recharts_1.XAxis dataKey="source" stroke="hsl(var(--muted-foreground))" fontSize={12} angle={-45} textAnchor="end" height={80}/>
                    <recharts_1.YAxis stroke="hsl(var(--muted-foreground))" fontSize={12}/>
                    <recharts_1.Tooltip content={<CustomTooltip />}/>
                    <recharts_1.Bar dataKey="conversionRate" fill={CHART_COLORS.chart3} radius={[4, 4, 0, 0]}/>
                  </recharts_1.BarChart>
                </recharts_1.ResponsiveContainer>
              </card_1.CardContent>
            </card_1.Card>
