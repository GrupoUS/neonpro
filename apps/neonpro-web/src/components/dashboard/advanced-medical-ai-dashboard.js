...
function AIModelCard(_a) {
    ...
    return (<card_1.Card className="relative overflow-hidden">
...
function AdvancedMedicalAIDashboard() {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, getHealthcareMetrics()];
                case 1:
                    metrics = _a.sent();
                    return [2 /*return*/, (<div className="space-y-6 p-6 bg-gradient-to-br from-blue-50 to-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <card_1.Card>
          <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <card_1.CardTitle className="text-sm font-medium">Predições IA</card_1.CardTitle>
      ...
      <tabs_1.Tabs defaultValue="ai-models" className="space-y-4">
        ...
        <tabs_1.TabsContent value="ai-models" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {metrics.aiModels.map(function (model, index) { return (<AIModelCard key={index} model={model}/>); })}
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-green-600">
                    {(metrics.aiModels.reduce(function (acc, model) { return acc + model.accuracy; }, 0) / metrics.aiModels.length).toFixed(1)}%
                  </div>
                  <p className="text-xs text-muted-foreground">Acurácia Média</p>
                </div>
                <div>
                  <div className="text-2xl font-bold text-blue-600">
                    {Math.round(metrics.aiModels.reduce(function (acc, model) { return acc + model.inferenceTime; }, 0) / metrics.aiModels.length)}ms
                  </div>
                  <p className="text-xs text-muted-foreground">Tempo Médio</p>
                </div>
                <div>
                  <div className="text-2xl font-bold text-purple-600">
                    {metrics.aiModels.reduce(function (acc, model) { return acc + model.predictions; }, 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">Total Predições</p>
                </div>
                <div>
                  <div className="text-2xl font-bold text-orange-600">
                    {metrics.aiModels.filter(function (model) { return model.status === 'active'; }).length}/{metrics.aiModels.length}
                  </div>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <badge_1.Badge variant="secondary">{metrics.wellness.wearableConnections}</badge_1.Badge>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="space-y-2">
                  ...
                  <progress_1.Progress value={metrics.wellness.wellnessScore} className="h-2"/>
                </div>
              </card_1.CardContent>
            </card_1.Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <card_1.Card>
              ...
              <card_1.CardContent>
                ...
                <progress_1.Progress value={metrics.compliance.lgpdScore} className="mb-2"/>
                <p className="text-xs text-muted-foreground">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="flex items-center gap-2 mb-4">
                  {metrics.compliance.anvisaCompliance ? (<badge_1.Badge className="bg-green-500">Conforme</badge_1.Badge>) : (<badge_1.Badge variant="destructive">Não Conforme</badge_1.Badge>)}
                </div>
                <p className="text-xs text-muted-foreground">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="flex items-center gap-2 mb-4">
                  {metrics.compliance.cfmCompliance ? (<badge_1.Badge className="bg-green-500">Conforme</badge_1.Badge>) : (<badge_1.Badge variant="destructive">Não Conforme</badge_1.Badge>)}
                </div>
                <p className="text-xs text-muted-foreground">
        ...
        <card_1.CardContent className="pt-6">
          <div className="text-center space-y-2">
            ...
            <div className="flex justify-center gap-2 mt-3">
              <badge_1.Badge variant="outline" className="bg-green-50">Sequential ✓</badge_1.Badge>
              <badge_1.Badge variant="outline" className="bg-blue-50">Context7 ✓</badge_1.Badge>
              <badge_1.Badge variant="outline" className="bg-purple-50">Tavily ✓</badge_1.Badge>
              <badge_1.Badge variant="outline" className="bg-orange-50">Exa ✓</badge_1.Badge>
              <badge_1.Badge variant="outline" className="bg-gray-50">Desktop Commander ✓</badge_1.Badge>
              <badge_1.Badge variant="outline" className="bg-indigo-50">shadcn-ui ✓</badge_1.Badge>
            </div>
          </div>
        </card_1.CardContent>
      </card_1.Card>
