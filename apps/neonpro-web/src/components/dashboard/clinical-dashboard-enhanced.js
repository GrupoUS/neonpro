...
function ClinicalDashboardEnhanced(_a) {
    var className = _a.className, userRole = _a.userRole;
    ...
    ]), priorityPatients = _c[0], setPriorityPatients = _c[1];
    var announceToScreenReader = (0, accessibility_context_1.useAccessibility)().announceToScreenReader;
    var _d = (0, react_1.useState)('overview'), activeTab = _d[0], setActiveTab = _d[1];
    ...
    var calculatedMetrics = (0, react_1.useMemo)(function () {
        ...
        var satisfactionGrade = metrics.patientSatisfaction >= 4.5 ? 'Excelente' :
            metrics.patientSatisfaction >= 4.0 ? 'Bom' : 'Regular';
        var revenueGrowth = '+12%'; // Mock data
        return { occupancyRate: occupancyRate, satisfactionGrade: satisfactionGrade, revenueGrowth: revenueGrowth };
    }, [metrics]);
    // Announce important updates for screen readers
    (0, react_1.useEffect)(function () {
        if (metrics.emergencyAlerts > 0) {
            announceToScreenReader("Aten\u00E7\u00E3o: ".concat(metrics.emergencyAlerts, " alerta(s) m\u00E9dico(s) requer(em) aten\u00E7\u00E3o imediata"), 'assertive');
        }
    ...
    var getPriorityIcon = function (priority) {
        switch (priority) {
            ...
            case 'medium': return <lucide_react_1.Clock className="h-4 w-4"/>;
            case 'low': return <lucide_react_1.CheckCircle2 className="h-4 w-4"/>;
            default: return null;
        }
    };
    return (<div className={(0, utils_1.cn)('space-y-6 p-6', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        {metrics.emergencyAlerts > 0 && (<div className="flex items-center space-x-2">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <card_1.Card className="hover:shadow-md transition-shadow">
          <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <card_1.CardTitle className="text-sm font-medium">Tratamentos Concluídos</card_1.CardTitle>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-3">
                {priorityPatients.map(function (patient) { return (<div key={patient.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-accent/50 transition-colors">
                    <div className="flex items-center space-x-3">
                      <badge_1.Badge variant="outline" className={(0, utils_1.cn)("text-xs", getPriorityColor(patient.priority))}>
