...
function VendorPerformanceDashboard() {
    ...
    var _c = (0, react_1.useState)("all"), selectedVendor = _c[0], setSelectedVendor = _c[1];
    var _d = (0, react_1.useState)(true), loading = _d[0], setLoading = _d[1];
    var _e = (0, react_1.useState)("30"), period = _e[0], setPeriod = _e[1]; // days
    ...
    var loadPerformanceData = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            setLoading(true);
            try {
                mockPerformance = vendors.map(function (vendor) { return ({
                    vendor_id: vendor.id,
    ...
    var filteredPerformance = selectedVendor === "all"
        ? performance
        : performance.filter(function (p) { return p.vendor_id === selectedVendor; });
    var totalStats = performance.reduce(function (acc, p) { return ({
    ...
    var averagePaymentTime = performance.length > 0
        ? performance.reduce(function (sum, p) { return sum + p.average_payment_time; }, 0) /
            performance.length
        : 0;
    var averageOnTimeRate = performance.length > 0
        ? performance.reduce(function (sum, p) { return sum + p.on_time_payment_rate; }, 0) /
            performance.length
        : 0;
    if (loading) {
      ...
      <div className="flex gap-4 items-center">
        ...
        <select_1.Select value={selectedVendor} onValueChange={setSelectedVendor}>
          ...
          <select_1.SelectContent>
            ...
            {vendors.map(function (vendor) { return (<select_1.SelectItem key={vendor.id} value={vendor.id}>
                {vendor.company_name}
              </select_1.SelectItem>); })}
          </select_1.SelectContent>
        </select_1.Select>
        ...
        <card_1.CardContent>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filteredPerformance.map(function (vendor) { return (<tr key={vendor.vendor_id} className="border-b hover:bg-muted/50">
                    <td className="p-2">
                      <div>
                    ...
                    <td className="p-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center">
                          {vendor.quality_score >= 90 ? (<lucide_react_1.TrendingUp className="h-4 w-4 text-green-600"/>) : vendor.quality_score < 70 ? (<lucide_react_1.TrendingDown className="h-4 w-4 text-red-600"/>) : (<lucide_react_1.AlertTriangle className="h-4 w-4 text-yellow-600"/>)}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-2">
                      <div className="space-y-1">
                        ...
                        <progress_1.Progress value={vendor.on_time_payment_rate} className="h-2"/>
                      </div>
                    </td>
                    ...
                    <td className="p-2">
                      <div className="flex flex-col gap-1">
                        ...
                        {vendor.overdue_count > 0 && (<div className="flex items-center gap-2">
