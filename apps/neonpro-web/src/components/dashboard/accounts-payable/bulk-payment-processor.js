...
function BulkPaymentProcessor(_a) {
    ...
    var _c = (0, react_1.useState)(false), processing = _c[0], setProcessing = _c[1];
    var _d = (0, react_1.useState)(0), processedCount = _d[0], setProcessedCount = _d[1];
    ...
    var _g = (0, react_1.useState)("pix"), selectedPaymentMethod = _g[0], setSelectedPaymentMethod = _g[1];
    var _h = (0, react_1.useState)(0), maxPaymentAmount = _h[0], setMaxPaymentAmount = _h[1];
    ...
    ]), payableItems = _j[0], setPayableItems = _j[1];
    ...
    var filteredItems = (0, react_1.useMemo)(function () {
        ...
        switch (filterType) {
            case "overdue":
                items = items.filter(function (item) { return item.status === "overdue"; });
                break;
            case "due_today":
                var today_1 = new Date().toDateString();
                items = items.filter(function (item) { return new Date(item.due_date).toDateString() === today_1; });
                break;
            case "due_week":
                var weekFromNow_1 = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
                items = items.filter(function (item) { return new Date(item.due_date) <= weekFromNow_1; });
                break;
            case "high_priority":
                items = items.filter(function (item) { return item.priority === "high"; });
                break;
            case "high_amount":
                items = items.filter(function (item) { return item.remaining_balance > 1000; });
                break;
        }
        return items;
    }, [payableItems, searchTerm, filterType]);
    // Selected items calculations
    var selectedItems = filteredItems.filter(function (item) { return item.selected; });
    var totalSelectedAmount = selectedItems.reduce(function (sum, item) { return sum + item.suggested_payment; }, 0);
    ...
    var handleSelectAll = function (checked) {
        setPayableItems(function (items) {
            return items.map(function (item) {
                var isVisible = filteredItems.some(function (fi) { return fi.id === item.id; });
        ...
        setPayableItems(function (items) {
            return items.map(function (item) {
                return item.id === itemId
                    ? __assign(__assign({}, item), { suggested_payment: Math.min(amount, item.remaining_balance) }) : item;
            });
        });
    };
    var handleApplyMaxPayment = function () {
        ...
        setPayableItems(function (items) {
            return items.map(function (item) {
                if (!selectedItems.some(function (si) { return si.id === item.id; }))
                    return item;
    ...
    var handleProcessPayments = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 4:
                    ...
                    return [3 /*break*/, 2];
                case 5:
                    sonner_1.toast.success("".concat(selectedItems.length, " pagamentos processados com sucesso"));
                    onSuccess();
                    onOpenChange(false);
                    // Reset selections
                    setPayableItems(function (items) {
                        return items.map(function (item) { return (__assign(__assign({}, item), { selected: false })); });
                    });
                    return [3 /*break*/, 8];
                case 6:
                    error_1 = _a.sent();
    ...
    var handleExportSelected = function () {
        ...
        a.href = url;
        a.download = "pagamentos-lote-".concat((0, date_fns_1.format)(new Date(), "yyyy-MM-dd"), ".csv");
        a.click();
      ...
      <dialog_1.DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <card_1.Card>
              <card_1.CardContent className="flex items-center p-4">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {filteredItems.filter(function (i) { return i.status === "overdue"; }).length}
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input_1.Input placeholder="Invoice, fornecedor, categoria..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-9"/>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select_1.Select value={filterType} onValueChange={setFilterType}>
                    ...
                    <select_1.SelectContent>
                      {filterOptions.map(function (option) { return (<select_1.SelectItem key={option.value} value={option.value}>
                          {option.label}
                        </select_1.SelectItem>); })}
                    </select_1.SelectContent>
                  </select_1.Select>
                ...
                <div className="space-y-2">
                  ...
                  <select_1.Select value={selectedPaymentMethod} onValueChange={setSelectedPaymentMethod}>
                    <select_1.SelectTrigger>
                      <select_1.SelectValue />
                    </select_1.SelectTrigger>
                    <select_1.SelectContent>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    <input_1.Input type="number" step="0.01" value={maxPaymentAmount || ""} onChange={function (e) {
            return setMaxPaymentAmount(parseFloat(e.target.value) || 0);
        }} placeholder="Valor máximo para distribuir"/>
                  </div>
                  <button_1.Button type="button" variant="outline" onClick={handleApplyMaxPayment} disabled={maxPaymentAmount <= 0 || selectedCount === 0}>
                    <lucide_react_1.Calculator className="h-4 w-4 mr-2"/>
                    Aplicar
                  </button_1.Button>
                </div>
                {maxPaymentAmount > 0 && (<p className="text-xs text-muted-foreground">
            ...
            <card_1.CardHeader className="flex flex-row items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center space-x-2">
                  <checkbox_1.Checkbox id="select-all" checked={selectedCount === filteredItems.length &&
            filteredItems.length > 0} onCheckedChange={handleSelectAll}/>
              ...
              <div className="flex gap-2">
                <button_1.Button type="button" variant="outline" size="sm" onClick={handleExportSelected} disabled={selectedCount === 0}>
            ...
            <card_1.CardContent>
              <div className="rounded-md border">
                <table_1.Table>
                  ...
                  <table_1.TableBody>
                    {filteredItems.length === 0 ? (<table_1.TableRow>
                      ...
                      </table_1.TableRow>) : (filteredItems.map(function (item) { return (<table_1.TableRow key={item.id} className={(0, utils_1.cn)(item.selected && "bg-muted/50")}>
                          <table_1.TableCell>
                            <checkbox_1.Checkbox checked={item.selected} onCheckedChange={function (checked) {
                return handleSelectItem(item.id, checked);
            }}/>
                          </table_1.TableCell>
                          <table_1.TableCell className="font-medium">
                            {item.invoice_number}
                            {item.days_overdue && item.days_overdue > 0 && (<div className="text-xs text-red-600">
                ...
                locale: locale_1.ptBR,
                          ...
                          <table_1.TableCell>
                            <input_1.Input type="number" step="0.01" value={item.suggested_payment || ""} onChange={function (e) {
                return handleUpdateSuggestedPayment(item.id, parseFloat(e.target.value) || 0);
            }} className="w-24" max={item.remaining_balance} disabled={!item.selected}/>
                          </table_1.TableCell>
                          <table_1.TableCell>
                            <badge_1.Badge className={(0, utils_1.cn)("text-xs", getPriorityColor(item.priority))}>
                              {item.priority.toUpperCase()}
                            </badge_1.Badge>
                          </table_1.TableCell>
                          <table_1.TableCell>
                            <badge_1.Badge className={(0, utils_1.cn)("text-xs", getStatusColor(item.status))}>
                              {item.status === "overdue"
                ? "Em Atraso"
                : item.status === "partial"
                    ? "Parcial"
          ...
          {processing && (<card_1.Card>
              <card_1.CardContent className="p-4">
                <div className="space-y-2">
                  ...
                  <progress_1.Progress value={(processedCount / selectedCount) * 100} className="w-full"/>
                </div>
              </card_1.CardContent>
            </card_1.Card>)}
        </div>
        ...
        <dialog_1.DialogFooter>
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex gap-2">
              <button_1.Button type="button" variant="outline" onClick={function () { return onOpenChange(false); }} disabled={processing}>
                Cancelar
              </button_1.Button>
              <button_1.Button onClick={handleProcessPayments} disabled={selectedCount === 0 || processing}>
                {processing ? (<>
