...
function PaymentCalendar(_a) {
    ...
    var _d = (0, react_1.useState)(null), currentDate = _d[0], setCurrentDate = _d[1];
    ...
    var _g = (0, react_1.useState)("all"), priorityFilter = _g[0], setPriorityFilter = _g[1];
    (0, react_1.useEffect)(function () {
        loadPayments();
    ...
    var filteredPayments = (0, react_1.useMemo)(function () {
        ...
        if (statusFilter !== "all") {
            filtered = filtered.filter(function (payment) { return payment.status === statusFilter; });
        }
        if (priorityFilter !== "all") {
            filtered = filtered.filter(function (payment) { return payment.priority === priorityFilter; });
        }
        return filtered;
    }, [payments, statusFilter, priorityFilter]);
    // Organizar pagamentos por data
    var paymentsByDate = (0, react_1.useMemo)(function () {
        var grouped = {};
    ...
    var calendarDays = monthStart && monthEnd ? (0, date_fns_1.eachDayOfInterval)({ start: monthStart, end: monthEnd }) : [];
    var navigateMonth = function (direction) {
        setCurrentDate(function (prev) {
            if (!prev)
                return new Date();
            return direction === "prev" ? (0, date_fns_1.subMonths)(prev, 1) : (0, date_fns_1.addMonths)(prev, 1);
        });
    ...
    var getDayStatus = function (date) {
        ...
        if (hasOverdue)
            return "overdue";
        var hasUrgent = dayPayments.some(function (p) { return p.priority === "urgent"; });
        if (hasUrgent)
            return "urgent";
        var hasHigh = dayPayments.some(function (p) { return p.priority === "high"; });
        if (hasHigh)
            return "high";
        return "normal";
    };
    ...
    if (loading || !currentDate) {
        ...
        <div className="animate-pulse">
          ...
          <div className="grid grid-cols-7 gap-2">
            {Array.from({ length: 35 }).map(function (_, i) { return (<div key={i} className="h-20 bg-gray-300 rounded"></div>); })}
          </div>
        </div>
      </div>);
    }
    ...
    return (<div className="space-y-6">
      <card_1.Card>
        ...
        <card_1.CardContent>
          ...
          <div className="grid grid-cols-7 gap-2 mb-2">
            {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map(function (day) { return (<div key={day} className="text-center text-sm font-medium text-muted-foreground py-2">
                {day}
              </div>); })}
          </div>
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            return (<button key={day.toISOString()} onClick={function () { return setSelectedDate(day); }} className={"\n                    relative min-h-[80px] p-2 rounded-lg border text-left transition-colors\n                    ".concat(isSelected
                    ? "ring-2 ring-primary bg-primary/10"
                    : "hover:bg-muted/50", "\n                    ").concat(!(0, date_fns_1.isSameMonth)(day, currentDate)
                    ? "text-muted-foreground bg-muted/20"
                    : "", "\n                    ").concat((0, date_fns_1.isToday)(day)
                    ? "font-bold border-primary"
                    : "border-border", "\n                    ").concat((0, date_fns_1.isWeekend)(day) ? "bg-muted/10" : "", "\n                  ")}>
                  <div className="flex items-start justify-between">
                    <span className={"text-sm ".concat((0, date_fns_1.isToday)(day) ? "text-primary" : "")}>
                      {(0, date_fns_1.format)(day, "d")}
                    </span>

                    {dayPayments.length > 0 && (<div className="flex gap-1">
                        <span className={"w-2 h-2 rounded-full ".concat(getDayBadgeColor(dayStatus))}/>
                        {dayPayments.length > 1 && (<span className="text-xs text-muted-foreground">
                            +{dayPayments.length - 1}
                          </span>)}
                      </div>)}
                  </div>

                  {dayPayments.length > 0 && (<div className="mt-1 space-y-1">
                      {dayPayments.slice(0, 2).map(function (payment) { return (<div key={payment.id} className="text-xs p-1 rounded bg-background/80 border truncate">
                      ...
                      {dayPayments.length > 2 && (<div className="text-xs text-center text-muted-foreground">
      ...
      {selectedDate && (<card_1.Card>
          ...
          {selectedDatePayments.length > 0 && (<card_1.CardContent>
              <div className="space-y-3">
                {selectedDatePayments.map(function (payment) { return (<div key={payment.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h4 className="font-medium">{payment.vendor_name}</h4>
                        {payment.recurring && (<badge_1.Badge variant="outline" className="text-xs">
                            Recorrente
                          </badge_1.Badge>)}
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        payment.status !== "paid" && (<div className="flex items-center gap-1 text-red-600 text-sm">
