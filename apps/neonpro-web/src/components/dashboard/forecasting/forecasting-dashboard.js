...
function ForecastingDashboard(_a) {
    ...
    }), forecastParams = _g[0], setForecastParams = _g[1];
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 2:
                        result = _b.sent();
                        if (!result.success) {
                            throw new Error(((_a = result.error) === null || _a === void 0 ? void 0 : _a.message) || 'Failed to fetch forecasts');
                        }
    ...
    }), forecasts = _h.data, forecastsLoading = _h.isLoading, forecastsError = _h.error;
    ...
    var _j = (0, react_query_1.useQuery)({
        ...
        queryFn: function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('/api/forecasting/alerts')];
                    case 1:
                        response = _a.sent();
                        if (!response.ok) {
                            throw new Error("HTTP error! status: ".concat(response.status));
    ...
    }), alerts = _j.data, alertsLoading = _j.isLoading;
    // Query for resource allocation recommendations
    var _k = (0, react_query_1.useQuery)({
        queryKey: ['resource-allocations', forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts],
        queryFn: function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        if (!((_a = forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts) === null || _a === void 0 ? void 0 : _a.length))
                            return [2 /*return*/, []];
                        forecastIds = forecasts.forecasts.map(function (f) { return f.id; });
                        return [4 /*yield*/, fetch('/api/forecasting/resource-allocation', {
                                method: 'POST',
        ...
        enabled: !!((_b = forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts) === null || _b === void 0 ? void 0 : _b.length)
    }), resourceAllocations = _k.data, resourceLoading = _k.isLoading;
    ...
    var generateForecastMutation = (0, react_query_1.useMutation)({
        mutationFn: function (params) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0: return [4 /*yield*/, fetch('/api/forecasting', {
                            method: 'GET',
                            headers: {
                    ...
                    case 2:
                        result = _b.sent();
                        if (!result.success) {
                            throw new Error(((_a = result.error) === null || _a === void 0 ? void 0 : _a.message) || 'Failed to generate forecast');
                        }
                        return [2 /*return*/, result.data];
                }
            });
    ...
    // Calculate metrics
    var metrics = react_1.default.useMemo(function () {
        var _a, _b;
        if (!forecasts) {
            return {
                overall_accuracy: 0,
        ...
        var activeAlerts = (alerts === null || alerts === void 0 ? void 0 : alerts.filter(function (alert) { return alert.status === 'active'; }).length) || 0;
        var confidenceAvg = ((_a = forecasts.forecasts) === null || _a === void 0 ? void 0 : _a.length) > 0
            ? forecasts.forecasts.reduce(function (sum, f) { return sum + f.confidence_level; }, 0) / forecasts.forecasts.length
            : 0;
        return {
            overall_accuracy: forecasts.accuracy || 0,
            total_forecasts: ((_b = forecasts.forecasts) === null || _b === void 0 ? void 0 : _b.length) || 0,
            active_alerts: activeAlerts,
            confidence_average: confidenceAvg,
            trend_direction: forecasts.accuracy >= demand_forecasting_1.FORECASTING_CONSTANTS.MIN_ACCURACY_THRESHOLD ? 'up' : 'down'
        };
    }, [forecasts, alerts]);
    // Transform data for charts
    var chartData = react_1.default.useMemo(function () {
        if (!(forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts))
            return [];
        return forecasts.forecasts.map(function (forecast) { return ({
    ...
    var handleExportData = function () {
        ...
        var csvData = forecasts.forecasts.map(function (f) { return ({
            ID: f.id,
        ...
        ], csvData.map(function (row) { return Object.values(row).join(','); }), true).join('\n');
        var blob = new Blob([csv], { type: 'text/csv' });
        var url = URL.createObjectURL(blob);
        var link = document.createElement('a');
        link.href = url;
        link.download = "demand-forecasts-".concat((0, date_fns_1.format)(new Date(), 'yyyy-MM-dd'), ".csv");
        link.click();
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button_1.Button variant="outline" size="sm" onClick={handleExportData} disabled={!((_c = forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts) === null || _c === void 0 ? void 0 : _c.length)}>
            <lucide_react_1.Download className="h-4 w-4 mr-2"/>
            Export
          </button_1.Button>
          <button_1.Button size="sm" onClick={handleRegenerateForecast} disabled={generateForecastMutation.isPending}>
            <lucide_react_1.RefreshCw className={"h-4 w-4 mr-2 ".concat(generateForecastMutation.isPending ? 'animate-spin' : '')}/>
            Regenerate
          </button_1.Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {forecastsLoading ? (<skeleton_1.Skeleton className="h-8 w-16"/>) : ("".concat(Math.round(metrics.overall_accuracy * 100), "%"))}
            </div>
            <p className="text-xs text-muted-foreground">
              {metrics.overall_accuracy >= demand_forecasting_1.FORECASTING_CONSTANTS.MIN_ACCURACY_THRESHOLD ? (<span className="text-green-600">Above 80% threshold</span>) : (<span className="text-red-600">Below 80% threshold</span>)}
            </p>
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {forecastsLoading ? (<skeleton_1.Skeleton className="h-8 w-16"/>) : (metrics.total_forecasts)}
            </div>
            <p className="text-xs text-muted-foreground">
              Demand predictions
            </p>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {forecastsLoading ? (<skeleton_1.Skeleton className="h-8 w-16"/>) : ("".concat(Math.round(metrics.confidence_average * 100), "%"))}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <tabs_1.Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                {forecastsLoading ? (<skeleton_1.Skeleton className="h-[200px] w-full"/>) : (<recharts_1.ResponsiveContainer width="100%" height={200}>
                    <recharts_1.LineChart data={chartData}>
                      <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                      <recharts_1.XAxis dataKey="date"/>
                      <recharts_1.YAxis />
                      <recharts_1.Tooltip />
                      <recharts_1.Line type="monotone" dataKey="predicted_demand" stroke="#8884d8" strokeWidth={2}/>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                ...
                {metrics.overall_accuracy >= demand_forecasting_1.FORECASTING_CONSTANTS.MIN_ACCURACY_THRESHOLD ? (<badge_1.Badge variant="default" className="bg-green-500">
        ...
        <tabs_1.TabsContent value="forecasts" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {forecastsLoading ? (<div className="space-y-2">
                  {__spreadArray([], Array(5), true).map(function (_, i) { return (<skeleton_1.Skeleton key={i} className="h-12 w-full"/>); })}
                </div>) : ((_d = forecasts === null || forecasts === void 0 ? void 0 : forecasts.forecasts) === null || _d === void 0 ? void 0 : _d.length) > 0 ? (<div className="space-y-2">
                  {forecasts.forecasts.map(function (forecast) { return (<div key={forecast.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="space-y-1">
                        <p className="text-sm font-medium">
                          {forecast.forecast_type.replace('_', ' ').toUpperCase()}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {(0, date_fns_1.format)((0, date_fns_1.parseISO)(forecast.period_start), 'MMM dd, yyyy')} - 
                          {(0, date_fns_1.format)((0, date_fns_1.parseISO)(forecast.period_end), 'MMM dd, yyyy')}
                        </p>
                      </div>
                  ...
                  <button_1.Button className="mt-2" onClick={handleRegenerateForecast} disabled={generateForecastMutation.isPending}>
                    Generate Forecasts
                  </button_1.Button>
                </div>)}
            </card_1.CardContent>
        ...
        <tabs_1.TabsContent value="resources" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {resourceLoading ? (<div className="space-y-2">
                  {__spreadArray([], Array(3), true).map(function (_, i) { return (<skeleton_1.Skeleton key={i} className="h-16 w-full"/>); })}
                </div>) : ((_e = resourceAllocations === null || resourceAllocations === void 0 ? void 0 : resourceAllocations.recommendations) === null || _e === void 0 ? void 0 : _e.length) > 0 ? (<div className="space-y-3">
                  {resourceAllocations.recommendations.map(function (allocation) {
                var _a;
                return (<div key={allocation.forecast_id} className="p-4 border rounded-lg space-y-2">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium">
                          {((_a = allocation.staffing) === null || _a === void 0 ? void 0 : _a.required_staff_count) || 0} Staff Members
                        </h4>
                        <badge_1.Badge variant={allocation.priority_level === 'critical' ? 'destructive' :
        ...
        <tabs_1.TabsContent value="alerts" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {alertsLoading ? (<div className="space-y-2">
                  {__spreadArray([], Array(3), true).map(function (_, i) { return (<skeleton_1.Skeleton key={i} className="h-12 w-full"/>); })}
                </div>) : (alerts === null || alerts === void 0 ? void 0 : alerts.length) > 0 ? (<div className="space-y-2">
                  {alerts.map(function (alert) { return (<alert_1.Alert key={alert.id} variant={alert.severity === 'critical' ? 'destructive' : 'default'}>
                      <lucide_react_1.AlertTriangle className="h-4 w-4"/>
