...
function InvoicesManagement() {
    var _this = this;
    var _a, _b;
    var _c = (0, use_billing_1.useBilling)(), loading = _c.loading, invoices = _c.invoices, services = _c.services, fetchInvoices = _c.fetchInvoices, fetchServices = _c.fetchServices, createInvoice = _c.createInvoice, updateInvoice = _c.updateInvoice;
    ...
    var _g = (0, react_1.useState)(false), isCreateDialogOpen = _g[0], setIsCreateDialogOpen = _g[1];
    var _h = (0, react_1.useState)(false), isViewDialogOpen = _h[0], setIsViewDialogOpen = _h[1];
    var _j = (0, react_1.useState)({}), dateRange = _j[0], setDateRange = _j[1];
    // Add patients state
    var _k = (0, react_1.useState)([]), patients = _k[0], setPatients = _k[1];
    var _l = (0, react_1.useState)(false), loadingPatients = _l[0], setLoadingPatients = _l[1];
    ...
    var fetchPatients = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    (0, react_1.useEffect)(function () {
        if (dateRange.from || dateRange.to) {
            setFilters(function (prev) { return (__assign(__assign({}, prev), { date_range: dateRange })); });
        }
    }, [dateRange]);
    ...
    var filteredInvoices = invoices.filter(function (invoice) {
        ...
        return invoice.invoice_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ((_a = invoice.patient_id) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_b = invoice.notes) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase()));
    });
    ...
    var addInvoiceItem = function () {
        setFormData(function (prev) { return (__assign(__assign({}, prev), { items: __spreadArray(__spreadArray([], prev.items, true), [{ service_id: "", quantity: 1, unit_price: 0 }], false) })); });
    };
    ...
    var updateInvoiceItem = function (index, field, value) {
        setFormData(function (prev) { return (__assign(__assign({}, prev), { items: prev.items.map(function (item, i) {
                var _a;
                return i === index ? __assign(__assign({}, item), (_a = {}, _a[field] = value, _a)) : item;
            }) })); });
    };
    var calculateTotal = function () {
        var subtotal = formData.items.reduce(function (sum, item) { return sum + item.quantity * item.unit_price; }, 0);
            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (formData.items.length === 0 ||
                        formData.items.some(function (item) { return !item.service_id; })) {
                        sonner_1.toast.error("Adicione pelo menos um item à fatura");
                        return [2 /*return*/];
                    }
                    invoiceItems = formData.items.map(function (item) { return ({
    ...
    var handleStatusUpdate = function (invoice, status) { return __awaiter(_this, void 0, void 0, function () {
        var error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    var handleSendInvoice = function (invoice) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            try {
    ...
    var handleDownloadInvoice = function (invoice) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            try {
    ...
    var getStatusLabel = function (status) {
        var statusOption = billing_1.INVOICE_STATUSES.find(function (s) { return s.value === status; });
        return (statusOption === null || statusOption === void 0 ? void 0 : statusOption.label) || status;
    };
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            .reduce(function (sum, inv) { return sum + inv.total_amount; }, 0))}
            </div>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {invoices.filter(function (inv) { return inv.status === "paid"; }).length} faturas
            </p>
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {invoices.filter(function (inv) { return inv.status === "overdue"; }).length}
            </div>
            <p className="text-xs text-muted-foreground">
              {formatCurrency(invoices
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">
              {invoices.length > 0
            ? Math.round((invoices.filter(function (inv) { return inv.status === "paid"; }).length /
                invoices.length) *
                100)
            : 0}
              %
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input_1.Input placeholder="Buscar faturas..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
              </div>
            </div>

            <div className="flex gap-2">
              <select_1.Select value={((_a = filters.status) === null || _a === void 0 ? void 0 : _a[0]) || "all"} onValueChange={function (value) {
            return setFilters(function (prev) { return (__assign(__assign({}, prev), { status: value === "all" ? undefined : [value] })); });
        }}>
                ...
                <select_1.SelectContent>
                  ...
                  {billing_1.INVOICE_STATUSES.map(function (status) { return (<select_1.SelectItem key={status.value} value={status.value}>
                      {status.label}
                    </select_1.SelectItem>); })}
                </select_1.SelectContent>
              </select_1.Select>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {loading ? (<div className="space-y-3">
              {Array.from({ length: 5 }).map(function (_, i) { return (<div key={i} className="h-12 bg-gray-200 rounded animate-pulse"></div>); })}
              ...
              {!searchTerm && (<button_1.Button onClick={openCreateDialog}>
                  <lucide_react_1.Plus className="h-4 w-4 mr-2"/>
              ...
              <table_1.TableBody>
                {filteredInvoices.map(function (invoice) { return (<table_1.TableRow key={invoice.id}>
                    ...
                    <table_1.TableCell>
                      <div className="flex gap-1">
                        <button_1.Button variant="ghost" size="sm" onClick={function () { return openViewDialog(invoice); }}>
                          <lucide_react_1.Eye className="h-4 w-4"/>
                        </button_1.Button>

                        <button_1.Button variant="ghost" size="sm" onClick={function () { return handleDownloadInvoice(invoice); }}>
                          <lucide_react_1.Download className="h-4 w-4"/>
                        </button_1.Button>

                        {invoice.status !== "paid" && (<button_1.Button variant="ghost" size="sm" onClick={function () { return handleSendInvoice(invoice); }}>
                            <lucide_react_1.Send className="h-4 w-4"/>
                          </button_1.Button>)}

                        <select_1.Select value={invoice.status} onValueChange={function (value) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <select_1.Select value={formData.patient_id} onValueChange={function (value) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { patient_id: value })); });
        }}>
                  <select_1.SelectTrigger>
                    <select_1.SelectValue placeholder="Selecionar paciente"/>
                  </select_1.SelectTrigger>
                  <select_1.SelectContent>
                    {loadingPatients ? (<select_1.SelectItem value="" disabled>
                        Carregando pacientes...
                      </select_1.SelectItem>) : patients.length > 0 ? (patients.map(function (patient) { return (<select_1.SelectItem key={patient.id} value={patient.id}>
                          <div className="flex items-center gap-2">
                            <lucide_react_1.User className="h-4 w-4"/>
                            <div>
                              <div className="font-medium">{patient.full_name}</div>
                              {patient.email && (<div className="text-xs text-muted-foreground">
                                  {patient.email}
                                </div>)}
                            </div>
                          </div>
                        </select_1.SelectItem>); })) : (<select_1.SelectItem value="" disabled>
              ...
              <div className="space-y-2">
                ...
                <input_1.Input id="due_date" type="date" value={formData.due_date} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { due_date: e.target.value })); });
        }} required/>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {formData.items.map(function (item, index) { return (<div key={index} className="p-4 border rounded-lg space-y-4">
                  <div className="flex justify-between items-center">
                    <h4 className="font-medium">Item #{index + 1}</h4>
                    {formData.items.length > 1 && (<button_1.Button type="button" variant="ghost" size="sm" onClick={function () { return removeInvoiceItem(index); }}>
                        Remover
                      </button_1.Button>)}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <select_1.Select value={item.service_id} onValueChange={function (value) {
                var service = services.find(function (s) { return s.id === value; });
                ...
                if (service) {
                    updateInvoiceItem(index, "unit_price", service.base_price);
                        ...
                        <select_1.SelectContent>
                          {services.map(function (service) { return (<select_1.SelectItem key={service.id} value={service.id}>
                              {service.name} -{" "}
                    ...
                    <div className="space-y-2">
                      ...
                      <input_1.Input type="number" min="1" value={item.quantity} onChange={function (e) {
                return updateInvoiceItem(index, "quantity", parseInt(e.target.value));
                    ...
                    <div className="space-y-2">
                      ...
                      <input_1.Input type="number" step="0.01" min="0" value={item.unit_price} onChange={function (e) {
                return updateInvoiceItem(index, "unit_price", parseFloat(e.target.value));
            }}/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input_1.Input value={item.description || ""} onChange={function (e) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <input_1.Input id="discount_amount" type="number" step="0.01" min="0" value={formData.discount_amount} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { discount_amount: e.target.value })); });
        }} placeholder="0,00"/>
              </div>

              <div className="space-y-2">
                <label_1.Label htmlFor="tax_amount">Taxa/Imposto (R$)</label_1.Label>
                <input_1.Input id="tax_amount" type="number" step="0.01" min="0" value={formData.tax_amount} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { tax_amount: e.target.value })); });
        }} placeholder="0,00"/>
              </div>
            ...
            <div className="space-y-2">
              ...
              <textarea_1.Textarea id="notes" value={formData.notes} onChange={function (e) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { notes: e.target.value })); });
        }} placeholder="Observações adicionais para a fatura..." rows={3}/>
            </div>

            <div className="flex justify-end gap-2 pt-4">
              <button_1.Button type="button" variant="outline" onClick={function () { return setIsCreateDialogOpen(false); }}>
                Cancelar
              </button_1.Button>
      ...
      <dialog_1.Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <dialog_1.DialogContent className="max-w-2xl">
          <dialog_1.DialogHeader>
            <dialog_1.DialogTitle>Fatura #{selectedInvoice === null || selectedInvoice === void 0 ? void 0 : selectedInvoice.invoice_number}</dialog_1.DialogTitle>
          ...
          {selectedInvoice && (<div className="space-y-6">
              ...
              <div>
                ...
                {((_b = selectedInvoice.items) === null || _b === void 0 ? void 0 : _b.map(function (item, index) { return (<div key={index} className="flex justify-between items-center py-2">
                    <div>
              ...
              <div className="flex justify-end gap-2">
                <button_1.Button variant="outline" onClick={function () { return handleDownloadInvoice(selectedInvoice); }}>
                  <lucide_react_1.Download className="h-4 w-4 mr-2"/>
                  Download
                </button_1.Button>
                <button_1.Button onClick={function () { return handleSendInvoice(selectedInvoice); }}>
