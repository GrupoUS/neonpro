...
function TreatmentPredictionDashboard() {
    var _this = this;
    var _a = (0, react_1.useState)({}), patientData = _a[0], setPatientData = _a[1];
    var _b = (0, react_1.useState)([]), predictions = _b[0], setPredictions = _b[1];
    var _c = (0, react_1.useState)(false), isAnalyzing = _c[0], setIsAnalyzing = _c[1];
    ...
    var getRiskIcon = function (risk) {
        switch (risk) {
            case 'low': return <lucide_react_1.Heart className="h-4 w-4"/>;
            case 'medium': return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
            case 'high': return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
            default: return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
        }
    };
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs defaultValue="analysis" className="w-full">
            ...
            <card_1.CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <input_1.Input id="age" type="number" placeholder="Ex: 32" value={patientData.age || ''} onChange={function (e) { return setPatientData(function (prev) { return (__assign(__assign({}, prev), { age: parseInt(e.target.value) })); }); }}/>
                </div>

                <div className="space-y-2">
                  <label_1.Label htmlFor="skinType">Tipo de Pele</label_1.Label>
                  <select_1.Select onValueChange={function (value) { return setPatientData(function (prev) { return (__assign(__assign({}, prev), { skinType: value })); }); }}>
                ...
                <div className="space-y-2">
                  ...
                  <select_1.Select onValueChange={function (value) { return setPatientData(function (prev) { return (__assign(__assign({}, prev), { budget: value })); }); }}>
                    <select_1.SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <input_1.Input id="concerns" placeholder="Ex: rugas, flacidez, manchas, volume facial..." onChange={function (e) { return setPatientData(function (prev) { return (__assign(__assign({}, prev), { concerns: e.target.value.split(',').map(function (c) { return c.trim(); }) })); }); }}/>
              </div>

              <div className="space-y-2">
                <label_1.Label htmlFor="previous">Tratamentos Anteriores</label_1.Label>
                <input_1.Input id="previous" placeholder="Ex: botox, preenchimento, peeling..." onChange={function (e) { return setPatientData(function (prev) { return (__assign(__assign({}, prev), { previousTreatments: e.target.value.split(',').map(function (t) { return t.trim(); }) })); }); }}/>
              </div>

              <button_1.Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full" size="lg">
                {isAnalyzing ? (<>
        ...
        <tabs_1.TabsContent value="predictions" className="space-y-4">
          {predictions.length === 0 ? (<card_1.Card>
              ...
              {predictions.map(function (prediction, index) { return (<card_1.Card key={index} className="relative">
                  <card_1.CardHeader>
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <card_1.CardTitle className="text-lg">{prediction.treatment}</card_1.CardTitle>
                        <div className="flex items-center gap-2">
                          <badge_1.Badge variant="secondary" className="flex items-center gap-1">
                            <lucide_react_1.Target className="h-3 w-3"/>
                            {prediction.confidence}% Confiança
                          </badge_1.Badge>
                          <badge_1.Badge variant="outline" className={"flex items-center gap-1 ".concat(getRiskColor(prediction.riskLevel))}>
                  ...
                  <card_1.CardContent className="space-y-4">
                    <div>
                      <label_1.Label className="text-sm font-medium">Justificativa da IA</label_1.Label>
