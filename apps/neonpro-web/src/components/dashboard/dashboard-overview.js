...
function KPICard(_a) {
    var title = _a.title, value = _a.value, change = _a.change, Icon = _a.icon, description = _a.description;
    return (<card_1.Card className="medical-card">
      ...
      <card_1.CardContent>
        ...
        {change && (<p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
            {change.type === "increase" ? (<lucide_react_1.TrendingUp className="h-3 w-3 text-green-500"/>) : (<lucide_react_1.TrendingDown className="h-3 w-3 text-red-500"/>)}
...
function DashboardOverview() {
    var _a = (0, react_1.useState)("7d"), selectedPeriod = _a[0], setSelectedPeriod = _a[1];
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Total de Pacientes" value={kpiData.totalPatients.toLocaleString("pt-BR")} change={{
            type: "increase",
            value: "+12%",
            period: "vs. mês anterior",
        }} icon={lucide_react_1.Users} description="Pacientes ativos na base"/>

        <KPICard title="Receita Mensal" value={(0, utils_1.formatCurrency)(kpiData.monthlyRevenue)} change={{
            type: "increase",
            value: "+8.2%",
            period: "vs. mês anterior",
        }} icon={lucide_react_1.DollarSign} description="Faturamento do mês atual"/>

        <KPICard title="Agendamentos Hoje" value={kpiData.todayAppointments} icon={lucide_react_1.Calendar} description="Consultas e procedimentos"/>

        <KPICard title="Satisfação" value={"".concat(kpiData.patientSatisfaction, "/5.0")} change={{
            type: "increase",
            value: "+0.2",
            period: "vs. mês anterior",
        }} icon={lucide_react_1.Star} description="Avaliação média dos pacientes"/>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <card_1.Card className="medical-card">
            ...
            return (<div key={activity.id} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
                    ...
                    {activity.status && (<div className="flex-shrink-0">
        ...
        <div>
          <card_1.Card className="medical-card">
            ...
            <card_1.CardContent className="space-y-4">
              {upcomingAppointments.map(function (appointment) { return (<div key={appointment.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-muted/30 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {appointment.is_new_patient && (<badge_1.Badge variant="secondary" className="text-xs">
                          Novo
                        </badge_1.Badge>)}
                    </div>
                    <p className="text-sm text-muted-foreground">
