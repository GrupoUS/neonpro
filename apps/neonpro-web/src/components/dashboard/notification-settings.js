...
function NotificationSettings(_a) {
    ...
    var _b = (0, notification_context_1.useNotificationContext)(), preferences = _b.preferences, updatePreferences = _b.updatePreferences, requestPermission = _b.requestPermission, isLoading = _b.isLoading;
    var _c = (0, react_1.useState)(false), saving = _c[0], setSaving = _c[1];
    var _d = (0, react_1.useState)(typeof window !== 'undefined' && 'Notification' in window ? Notification.permission : 'denied'), permissionStatus = _d[0], setPermissionStatus = _d[1];
    var handlePreferenceChange = function (key, value) { return __awaiter(_this, void 0, void 0, function () {
        var error_1;
        var _a;
    ...
    var handleRequestPermission = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      {typeof window !== 'undefined' && 'Notification' in window && (<card_1.Card>
          <card_1.CardHeader>
          ...
          <card_1.CardContent>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {permissionStatus !== 'granted' && (<button_1.Button size="sm" onClick={handleRequestPermission} disabled={permissionStatus === 'denied'}>
                    Permitir
                  </button_1.Button>)}
              </div>
            </div>
      ...
      {/* Settings Sections */}
      {settingsSections.map(function (section, sectionIndex) { return (<card_1.Card key={section.title}>
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center gap-2">
          ...
          <card_1.CardContent className="space-y-6">
            {section.settings.map(function (setting, settingIndex) { return (<div key={setting.key}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    {setting.type === 'switch' && (<switch_1.Switch id={setting.key} checked={preferences[setting.key]} onCheckedChange={function (checked) { return handlePreferenceChange(setting.key, checked); }} disabled={saving}/>)}
                    
                    {setting.type === 'select' && setting.options && (<select_1.Select value={String(preferences[setting.key] || '')} onValueChange={function (value) { return handlePreferenceChange(setting.key, parseInt(value)); }}>
                        <select_1.SelectTrigger className="w-48">
                          <select_1.SelectValue />
                        </select_1.SelectTrigger>
                        <select_1.SelectContent>
                          {setting.options.map(function (option) { return (<select_1.SelectItem key={option.value} value={String(option.value)}>
                              {option.label}
                            </select_1.SelectItem>); })}
                        </select_1.SelectContent>
                      </select_1.Select>)}
                    
                    {setting.type === 'time' && (<input type="time" id={setting.key} value={preferences[setting.key] || ''} onChange={function (e) { return handlePreferenceChange(setting.key, e.target.value); }} className="px-3 py-2 border border-input bg-background rounded-md text-sm" disabled={saving}/>)}
                  </div>
                </div>
                
                {settingIndex < section.settings.length - 1 && (<separator_1.Separator className="mt-6"/>)}
              </div>); })}
          </card_1.CardContent>
        </card_1.Card>); })}
