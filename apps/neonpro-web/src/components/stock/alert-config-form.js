...
var AlertConfigForm = function (_a) {
    var onSubmit = _a.onSubmit, onCancel = _a.onCancel, initialData = _a.initialData, _b = _a.products, products = _b === void 0 ? [] : _b, _c = _a.categories, categories = _c === void 0 ? [] : _c, _d = _a.loading, loading = _d === void 0 ? false : _d;
    var _e = (0, react_1.useState)(null), submitError = _e[0], setSubmitError = _e[1];
    var _f = (0, react_1.useState)('global'), selectedScope = _f[0], setSelectedScope = _f[1];
    var form = (0, react_hook_form_1.useForm)({
        resolver: (0, zod_1.zodResolver)(stock_1.StockAlertConfigSchema.omit({ id: true, clinicId: true, createdAt: true, updatedAt: true })),
        defaultValues: __assign({ alertType: 'low_stock', thresholdValue: 10, thresholdUnit: 'quantity', severityLevel: 'medium', isActive: true, notificationChannels: ['in_app'] }, initialData)
    });
    // =====================================================
    // EFFECTS
    // =====================================================
    ...
    var handleNotificationChannelChange = function (channel, checked) {
        ...
        if (checked) {
            if (!currentChannels.includes(channel)) {
                form.setValue('notificationChannels', __spreadArray(__spreadArray([], currentChannels, true), [channel], false));
            }
        }
        else {
            form.setValue('notificationChannels', currentChannels.filter(function (c) { return c !== channel; }));
        }
    };
    // =====================================================
    // RENDER
      ...
      <card_1.CardContent>
        <form_1.Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="flex items-center space-x-2">
                  <input type="radio" id="scope-global" name="scope" value="global" checked={selectedScope === 'global'} onChange={function (e) { return e.target.checked && setSelectedScope('global'); }} className="radio"/>
                  <label_1.Label htmlFor="scope-global" className="cursor-pointer">
                    Global
                  </label_1.Label>
                </div>
                <div className="flex items-center space-x-2">
                  <input type="radio" id="scope-product" name="scope" value="product" checked={selectedScope === 'product'} onChange={function (e) { return e.target.checked && setSelectedScope('product'); }} className="radio"/>
                  <label_1.Label htmlFor="scope-product" className="cursor-pointer">
                    Produto
                  </label_1.Label>
                </div>
                <div className="flex items-center space-x-2">
                  <input type="radio" id="scope-category" name="scope" value="category" checked={selectedScope === 'category'} onChange={function (e) { return e.target.checked && setSelectedScope('category'); }} className="radio"/>
            ...
            {/* Product Selection */}
            {selectedScope === 'product' && (<form_1.FormField control={form.control} name="productId" render={function (_a) {
                var field = _a.field;
                return (<form_1.FormItem>
                    ...
                    <select_1.Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <select_1.SelectContent>
                        {products.map(function (product) { return (<select_1.SelectItem key={product.id} value={product.id}>
            ...
            {/* Category Selection */}
            {selectedScope === 'category' && (<form_1.FormField control={form.control} name="categoryId" render={function (_a) {
                var field = _a.field;
                return (<form_1.FormItem>
            ...
            {/* Alert Type */}
            <form_1.FormField control={form.control} name="alertType" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem>
            ...
            <div className="grid grid-cols-2 gap-4">
              <form_1.FormField control={form.control} name="thresholdValue" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem>
              ...
              <form_1.FormField control={form.control} name="thresholdUnit" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem>
            ...
            {/* Severity Level */}
            <form_1.FormField control={form.control} name="severityLevel" render={function (_a) {
            var field = _a.field;
            return (<form_1.FormItem>
            ...
            var _b;
            var channel = _a[0], label = _a[1];
            return (<div key={channel} className="flex items-center space-x-2">
                    <checkbox_1.Checkbox id={"channel-".concat(channel)} checked={(_b = form.getValues('notificationChannels')) === null || _b === void 0 ? void 0 : _b.includes(channel)} onCheckedChange={function (checked) {
                    return handleNotificationChannelChange(channel, !!checked);
                }}/>
                    <label_1.Label htmlFor={"channel-".concat(channel)} className="flex items-center gap-2 cursor-pointer">
              ...
              {form.formState.errors.notificationChannels && (<p className="text-sm text-red-500">
                  {form.formState.errors.notificationChannels.message}
                </p>)}
            </div>

            {/* Active Status */}
            <form_1.FormField control={form.control} name="isActive" render={function (_a) {
            var field = _a.field;
