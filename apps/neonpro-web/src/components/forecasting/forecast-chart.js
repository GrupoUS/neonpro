...
function ForecastChart(_a) {
    var forecasts = _a.forecasts, _b = _a.detailed, detailed = _b === void 0 ? false : _b, _c = _a.height, height = _c === void 0 ? 400 : _c, _d = _a.showConfidenceIntervals, showConfidenceIntervals = _d === void 0 ? true : _d, _e = _a.className, className = _e === void 0 ? "" : _e;
    var _f = (0, react_1.useState)('all'), selectedType = _f[0], setSelectedType = _f[1];
    var _g = (0, react_1.useState)('area'), chartMode = _g[0], setChartMode = _g[1];
    var _h = (0, react_1.useState)(true), showActual = _h[0], setShowActual = _h[1];
    // Process forecast data for chart display
    var chartData = (0, react_1.useMemo)(function () {
        if (!forecasts.length)
            return [];
        // Filter forecasts by selected type
        var filteredForecasts = selectedType === 'all'
            ? forecasts
            : forecasts.filter(function (f) { return f.forecast_type === selectedType; });
        ...
        return dataPoints.sort(function (a, b) { return a.dateObj.getTime() - b.dateObj.getTime(); });
    }, [forecasts, selectedType]);
    // Calculate trend and statistics
    var statistics = (0, react_1.useMemo)(function () {
        if (!chartData.length)
            return null;
        var values = chartData.map(function (d) { return d.predicted; });
        var actualValues = chartData.map(function (d) { return d.actual; }).filter(function (v) { return v !== undefined; });
        var avgPredicted = values.reduce(function (sum, val) { return sum + val; }, 0) / values.length;
        var avgActual = actualValues.length > 0
            ? actualValues.reduce(function (sum, val) { return sum + val; }, 0) / actualValues.length
            : null;
        // Calculate trend (simplified linear regression slope)
        var trend = values.length > 1
            ? (values[values.length - 1] - values[0]) / values.length
            : 0;
        // Calculate accuracy if we have actual values
        var accuracy = avgActual !== null
        ...
        return {
            ...
            forecastTypes: __spreadArray([], new Set(chartData.map(function (d) { return d.forecastType; })), true)
        };
    }, [chartData]);
    ...
    var CustomTooltip = function (_a) {
        ...
        <p className="font-medium">{(0, date_fns_1.format)((0, date_fns_1.parseISO)(label), 'MMM dd, yyyy')}</p>
        <div className="space-y-1 mt-2">
          {payload.map(function (entry, index) { return (<div key={index} className="flex items-center justify-between space-x-4">
    ...
    var forecastTypes = (0, react_1.useMemo)(function () {
        var types = __spreadArray([], new Set(forecasts.map(function (f) { return f.forecast_type; })), true);
        return types.map(function (type) { return ({
            value: type,
            label: type.replace('_', ' ').replace(/\b\w/g, function (l) { return l.toUpperCase(); })
        }); });
    }, [forecasts]);
    if (!forecasts.length) {
    ...
    return (<card_1.Card className={className}>
      <card_1.CardHeader>
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          ...
          {detailed && (<div className="flex items-center space-x-2">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center space-x-2">
            ...
            <select_1.Select value={selectedType} onValueChange={setSelectedType}>
              ...
              <select_1.SelectContent>
                ...
                {forecastTypes.map(function (type) { return (<select_1.SelectItem key={type.value} value={type.value}>
                    {type.label}
                  </select_1.SelectItem>); })}
              </select_1.SelectContent>
            </select_1.Select>
          ...
          <div className="flex items-center space-x-2">
            ...
            <select_1.Select value={chartMode} onValueChange={function (value) { return setChartMode(value); }}>
          ...
          {statistics && (<div className="flex items-center space-x-4 ml-auto">
              <div className="flex items-center space-x-2">
                {statistics.trend > 0 ? (<lucide_react_1.TrendingUp className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.TrendingDown className="h-4 w-4 text-red-600"/>)}
                <span className="text-sm font-medium">
                  Avg: {statistics.avgPredicted}
                </span>
              </div>
              
              {statistics.accuracy && (<badge_1.Badge variant="secondary" className="bg-blue-100 text-blue-800">
      ...
      <card_1.CardContent>
        <div style={{ height: "".concat(height, "px") }}>
          <recharts_1.ResponsiveContainer width="100%" height="100%">
            {chartMode === 'area' ? (<recharts_1.AreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <recharts_1.CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                <recharts_1.XAxis dataKey="date" tickFormatter={function (value) { return (0, date_fns_1.format)((0, date_fns_1.parseISO)(value), 'MMM dd'); }} stroke="#666"/>
                ...
                {showConfidenceIntervals && (<>
                    ...
                    <recharts_1.Area type="monotone" dataKey="upperBound" stroke="none" fill="url(#confidenceGradient)" fillOpacity={0.3}/>
                    <recharts_1.Area type="monotone" dataKey="lowerBound" stroke="none" fill="white" fillOpacity={1}/>
                  </>)}

                {/* Main prediction area */}
                <recharts_1.Area type="monotone" dataKey="predicted" stroke="#3b82f6" strokeWidth={2} fill="#3b82f6" fillOpacity={0.2} name="Predicted Demand"/>

                {/* Actual values if available */}
                {showActual && (<recharts_1.Area type="monotone" dataKey="actual" stroke="#10b981" strokeWidth={2} fill="none" strokeDasharray="5 5" name="Actual Demand" connectNulls={false}/>)}
              </recharts_1.AreaChart>) : (<recharts_1.LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <recharts_1.CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                <recharts_1.XAxis dataKey="date" tickFormatter={function (value) { return (0, date_fns_1.format)((0, date_fns_1.parseISO)(value), 'MMM dd'); }} stroke="#666"/>
                <recharts_1.YAxis stroke="#666"/>
                <recharts_1.Tooltip content={<CustomTooltip />}/>
                ...
                {showConfidenceIntervals && (<>
                    <recharts_1.Line type="monotone" dataKey="upperBound" stroke="#94a3b8" strokeWidth={1} strokeDasharray="2 2" dot={false} name="Upper Bound"/>
                    <recharts_1.Line type="monotone" dataKey="lowerBound" stroke="#94a3b8" strokeWidth={1} strokeDasharray="2 2" dot={false} name="Lower Bound"/>
                  </>)}

                {/* Main prediction line */}
                <recharts_1.Line type="monotone" dataKey="predicted" stroke="#3b82f6" strokeWidth={3} dot={{ fill: '#3b82f6', strokeWidth: 2, r: 4 }} name="Predicted Demand"/>

                {/* Actual values if available */}
                {showActual && (<recharts_1.Line type="monotone" dataKey="actual" stroke="#10b981" strokeWidth={2} strokeDasharray="5 5" dot={{ fill: '#10b981', strokeWidth: 2, r: 3 }} name="Actual Demand" connectNulls={false}/>)}
              </recharts_1.LineChart>)}
          </recharts_1.ResponsiveContainer>
        </div>

        {/* Summary Statistics */}
        {detailed && statistics && (<div className="mt-6 grid gap-4 md:grid-cols-3 lg:grid-cols-5">
            ...
            {statistics.avgActual && (<div className="text-center">
            ...
            <div className="text-center">
              <div className={"text-2xl font-bold ".concat(statistics.trend > 0 ? 'text-green-600' : 'text-red-600')}>
                {statistics.trend > 0 ? '+' : ''}{statistics.trend.toFixed(1)}
              </div>
              <div className="text-sm text-muted-foreground">Daily Trend</div>
            </div>
            
            {statistics.accuracy && (<div className="text-center">
