...
var ALERT_TYPE_CONFIG = {
    ...
    resource_shortage: { label: 'Resource Shortage', icon: lucide_react_1.Users, color: 'text-orange-600' },
    model_drift: { label: 'Model Drift', icon: lucide_react_1.TrendingDown, color: 'text-purple-600' },
    capacity_warning: { label: 'Capacity Warning', icon: lucide_react_1.Monitor, color: 'text-yellow-600' }
};
function ForecastAlerts(_a) {
    var _this = this;
    var alerts = _a.alerts, onAcknowledge = _a.onAcknowledge, _b = _a.className, className = _b === void 0 ? "" : _b;
    ...
    var _d = (0, react_1.useState)(new Set()), selectedAlerts = _d[0], setSelectedAlerts = _d[1];
    ...
    var stats = (0, react_1.useMemo)(function () {
        ...
        var active = alerts.filter(function (a) { return !a.acknowledged; }).length;
        var critical = alerts.filter(function (a) { return a.severity === 'critical'; }).length;
        var acknowledged = alerts.filter(function (a) { return a.acknowledged; }).length;
        ...
        return { total: total, active: active, critical: critical, acknowledged: acknowledged, byType: byType, bySeverity: bySeverity };
    }, [alerts]);
    ...
    var groupedAlerts = (0, react_1.useMemo)(function () {
        var groups = {};
        ...
        Object.keys(groups).forEach(function (key) {
            groups[key].sort(function (a, b) { return new Date(b.created_at).getTime() - new Date(a.created_at).getTime(); });
        });
        return groups;
    }, [filteredAlerts]);
    // Handle bulk acknowledgment
    var handleBulkAcknowledge = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var renderSeverityBadge = function (severity) {
        ...
        return (<badge_1.Badge className={config.color}>
    ...
    var renderTypeBadge = function (type) {
        ...
        var Icon = config.icon;
        return (<div className={"flex items-center space-x-1 text-sm ".concat(config.color)}>
    ...
    var renderAlertCard = function (alert) {
        ...
        return (<div key={alert.id} className={"border rounded-lg p-4 ".concat(severityConfig.bgColor, " ").concat(severityConfig.borderColor, " ").concat(alert.acknowledged ? 'opacity-60' : '')}>
        <div className="flex items-start space-x-3">
          <checkbox_1.Checkbox checked={isSelected} onCheckedChange={function () { return toggleAlertSelection(alert.id); }} className="mt-1"/>
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  ...
                  {alert.acknowledged && (<badge_1.Badge variant="outline" className="bg-green-100 text-green-800">
                ...
                <div className="flex items-center space-x-4 text-xs text-muted-foreground">
                  ...
                  {alert.forecast_id && (<span className="flex items-center space-x-1">
              ...
              <div className="flex items-center space-x-2">
                {!alert.acknowledged && (<button_1.Button size="sm" variant="outline" onClick={function () { return onAcknowledge(alert.id); }}>
                    Acknowledge
                  </button_1.Button>)}
                
                <button_1.Button size="sm" variant="ghost" onClick={function () { return setShowDetails(isExpanded ? null : alert.id); }}>
            ...
            {/* Affected Resources */}
            {alert.affected_resources && alert.affected_resources.length > 0 && (<div className="text-xs">
            ...
            {/* Expanded Details */}
            {isExpanded && (<div className="mt-4 pt-4 border-t space-y-3">
                {alert.recommended_actions && alert.recommended_actions.length > 0 && (<div>
                    <h5 className="text-sm font-medium mb-2">Recommended Actions:</h5>
                    <ul className="text-sm space-y-1">
                      {alert.recommended_actions.map(function (action, index) { return (<li key={index} className="flex items-start space-x-2">
    ...
    return (<card_1.Card className={className}>
      ...
      <card_1.CardContent className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input_1.Input placeholder="Search alerts..." value={filters.search} onChange={function (e) { return setFilters(function (prev) { return (__assign(__assign({}, prev), { search: e.target.value })); }); }} className="pl-8"/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <select_1.Select value={filters.type} onValueChange={function (value) { return setFilters(function (prev) { return (__assign(__assign({}, prev), { type: value })); }); }}>
                  <select_1.SelectTrigger>
                    <select_1.SelectValue />
                  </select_1.SelectTrigger>
                  <select_1.SelectContent>
                    ...
                    <select_1.SelectItem value="resource_shortage">Resource Shortage</select_1.SelectItem>
                    <select_1.SelectItem value="model_drift">Model Drift</select_1.SelectItem>
                    <select_1.SelectItem value="capacity_warning">Capacity Warning</select_1.SelectItem>
                  </select_1.SelectContent>
                </select_1.Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button_1.Button variant="outline" className="w-full" onClick={function () { return setFilters({ severity: 'all', status: 'all', type: 'all', search: '' }); }}>
                  Clear Filters
                </button_1.Button>
              </div>
            </div>
        ...
        <div className="space-y-6">
          {Object.keys(groupedAlerts).length === 0 ? (<div className="text-center py-8">
            ...
            </div>) : (Object.entries(groupedAlerts).map(function (_a) {
            var dateGroup = _a[0], groupAlerts = _a[1];
            return (<div key={dateGroup} className="space-y-4">
