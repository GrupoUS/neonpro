...
function ModelPerformance(_a) {
    ...
    var _c = (0, react_1.useState)('all'), selectedModel = _c[0], setSelectedModel = _c[1];
    var _d = (0, react_1.useState)('accuracy'), activeMetric = _d[0], setActiveMetric = _d[1];
    ...
    var summary = (0, react_1.useMemo)(function () {
        ...
        var accuracies = metrics.map(function (m) { return m.test_metrics.accuracy_percentage; });
        var stabilities = metrics.map(function (m) { return m.stability_score; });
        var driftScores = metrics.map(function (m) { return m.drift_score; });
        var avgAccuracy = accuracies.reduce(function (sum, acc) { return sum + acc; }, 0) / accuracies.length;
        var avgStability = stabilities.reduce(function (sum, stab) { return sum + stab; }, 0) / stabilities.length;
        ...
        var healthyModels = metrics.filter(function (m) {
            return m.test_metrics.accuracy_percentage >= METRIC_THRESHOLDS.accuracy.good &&
                m.stability_score >= METRIC_THRESHOLDS.stability.good &&
                m.drift_score <= METRIC_THRESHOLDS.drift.medium;
        }).length;
        var driftingModels = metrics.filter(function (m) { return m.drift_score > METRIC_THRESHOLDS.drift.medium; }).length;
        // Simple trend calculation (would use historical data in production)
        var performanceTrend = avgAccuracy >= 85 ? 'improving' :
    ...
    var chartData = (0, react_1.useMemo)(function () {
        ...
        var comparison = metrics.map(function (m) { return ({
        ...
        var bestModelMetrics = metrics.find(function (m) { return m.model_id === summary.bestModel; });
        var features = bestModelMetrics ? Object.entries(bestModelMetrics.feature_importance).map(function (_a) {
            var name = _a[0], importance = _a[1];
            return ({
                feature: name.replace('_', ' ').replace(/\b\w/g, function (l) { return l.toUpperCase(); }),
                importance: importance * 100,
                fullName: name
            });
        }).sort(function (a, b) { return b.importance - a.importance; }) : [];
        // Performance metrics radar chart
        var performance = comparison.map(function (m) { return ({
            model: m.model,
            accuracy: m.accuracy,
        ...
        return { comparison: comparison, trends: trends, features: features, performance: performance };
    }, [metrics, summary]);
    ...
    var renderPerformanceStatus = function (value, type) {
        if (type === 'drift') {
            ...
            if (value <= METRIC_THRESHOLDS.drift.medium * 100)
                return <badge_1.Badge className="bg-yellow-100 text-yellow-800">Medium Drift</badge_1.Badge>;
    ...
    return (<card_1.Card className={className}>
      <card_1.CardHeader>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="text-center">
            <div className={"text-2xl font-bold ".concat(summary.performanceTrend === 'improving' ? 'text-green-600' :
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <select_1.Select value={selectedModel} onValueChange={setSelectedModel}>
            ...
            <select_1.SelectContent>
              ...
              {metrics.map(function (metric) { return (<select_1.SelectItem key={metric.model_id} value={metric.model_id}>
                  {metric.model_id}
                </select_1.SelectItem>); })}
            </select_1.SelectContent>
          </select_1.Select>
        ...
        {/* Performance Tabs */}
        <tabs_1.Tabs value={activeMetric} onValueChange={function (value) { return setActiveMetric(value); }}>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {(selectedModel === 'all' ? metrics : metrics.filter(function (m) { return m.model_id === selectedModel; })).map(function (metric) { return (<div key={metric.model_id} className="p-4 border rounded-lg">
                      ...
                      <div className="grid gap-4 md:grid-cols-3">
                        <div>
                          ...
                          <progress_1.Progress value={metric.test_metrics.accuracy_percentage} className="mt-1"/>
                        </div>
          ...
          <tabs_1.TabsContent value="stability" className="space-y-6">
            ...
            <card_1.Card>
              <card_1.CardHeader>
                <card_1.CardTitle className="text-lg">Performance Trends (30 Days)</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="h-[300px]">
                  <recharts_1.ResponsiveContainer width="100%" height="100%">
                    <recharts_1.LineChart data={chartData.trends}>
                      ...
                      <recharts_1.Tooltip content={<CustomTooltip />}/>
                      <recharts_1.Line type="monotone" dataKey="accuracy" stroke={COLORS.primary} strokeWidth={2} name="Accuracy"/>
                      <recharts_1.Line type="monotone" dataKey="stability" stroke={COLORS.secondary} strokeWidth={2} name="Stability"/>
                      <recharts_1.Line type="monotone" dataKey="drift" stroke={COLORS.warning} strokeWidth={2} name="Drift"/>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <card_1.Card>
                ...
                <card_1.CardContent>
                  <div className="space-y-4">
                    {metrics.map(function (metric) { return (<div key={metric.model_id} className="flex items-center justify-between">
          ...
          <tabs_1.TabsContent value="performance" className="space-y-6">
            ...
            <card_1.Card>
              <card_1.CardHeader>
                <card_1.CardTitle className="text-lg">Feature Importance (Best Model: {summary.bestModel})</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="h-[300px]">
                  <recharts_1.ResponsiveContainer width="100%" height="100%">
                    <recharts_1.BarChart data={chartData.features} layout="horizontal">
                      ...
                      <recharts_1.YAxis dataKey="feature" type="category" tick={{ fontSize: 12 }} width={100}/>
                      <recharts_1.Tooltip content={<CustomTooltip />}/>
                      <recharts_1.Bar dataKey="importance" fill={COLORS.success} radius={[0, 4, 4, 0]}/>
                    </recharts_1.BarChart>
                  </recharts_1.ResponsiveContainer>
                </div>
              </card_1.CardContent>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <card_1.Card>
                ...
                <card_1.CardContent>
                  <div className="space-y-4">
                    {metrics.map(function (metric) { return (<div key={metric.model_id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium">{metric.model_id}</span>
                          <badge_1.Badge variant="outline">{metric.model_size_mb.toFixed(1)}MB</badge_1.Badge>
                        </div>
                  ...
                  <div className="h-[250px]">
                    <recharts_1.ResponsiveContainer width="100%" height="100%">
                      <recharts_1.RadarChart data={chartData.performance}>
                        ...
                        <recharts_1.Radar name="Performance" dataKey="accuracy" stroke={COLORS.primary} fill={COLORS.primary} fillOpacity={0.2} strokeWidth={2}/>
...
// Add Label import
function Label(_a) {
    var children = _a.children, htmlFor = _a.htmlFor, className = _a.className;
