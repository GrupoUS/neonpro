...
export function ModelPerformance({ metrics, className = "" }: ModelPerformanceProps) {
  ...
  const [activeMetric, setActiveMetric] = useState<"accuracy" | "stability" | "performance">(
    "accuracy",
  );
  ...
  const summary = useMemo((): PerformanceSummary => {
    ...
    const healthyModels = metrics.filter(
      (m) =>
    ...
    const driftingModels = metrics.filter(
  ...
  const chartData = useMemo(() => {
    ...
    const features = bestModelMetrics
  ...
  const renderPerformanceStatus = (value: number, type: "accuracy" | "stability" | "drift") => {
    if (type === "drift") {
      if (value <= METRIC_THRESHOLDS.drift.low * 100)
        return <Badge className="bg-green-100 text-green-800">Low Drift</Badge>;
      if (value <= METRIC_THRESHOLDS.drift.medium * 100)
        return <Badge className="bg-yellow-100 text-yellow-800">Medium Drift</Badge>;
