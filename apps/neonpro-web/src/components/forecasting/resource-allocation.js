...
var RESOURCE_TYPES = [
    ...
    { id: 'equipment', label: 'Medical Equipment', icon: lucide_react_1.Monitor, color: COLORS.equipment },
    { id: 'budget', label: 'Budget Allocation', icon: lucide_react_1.DollarSign, color: COLORS.budget }
];
function ResourceAllocation(_a) {
    var _this = this;
    var _b;
    var plan = _a.plan, _c = _a.detailed, detailed = _c === void 0 ? false : _c, _d = _a.editable, editable = _d === void 0 ? false : _d, _e = _a.className, className = _e === void 0 ? "" : _e;
    var _f = (0, react_1.useState)('staff'), activeResourceType = _f[0], setActiveResourceType = _f[1];
    var _g = (0, react_1.useState)(false), editMode = _g[0], setEditMode = _g[1];
    var _h = (0, react_1.useState)({}), adjustments = _h[0], setAdjustments = _h[1];
    ...
    var summary = (0, react_1.useMemo)(function () {
        ...
        var staffAllocations = plan.allocations.filter(function (a) { return a.resource_type === 'staff'; });
        var roomAllocations = plan.allocations.filter(function (a) { return a.resource_type === 'room'; });
        var budgetAllocations = plan.allocations.filter(function (a) { return a.resource_type === 'budget'; });
        var totalStaff = staffAllocations.reduce(function (sum, a) { return sum + a.capacity; }, 0);
        var utilizedStaff = staffAllocations.reduce(function (sum, a) { return sum + a.allocated_capacity; }, 0);
        var totalRooms = roomAllocations.reduce(function (sum, a) { return sum + a.capacity; }, 0);
        var utilizedRooms = roomAllocations.reduce(function (sum, a) { return sum + a.allocated_capacity; }, 0);
        var totalBudget = budgetAllocations.reduce(function (sum, a) { return sum + a.capacity; }, 0);
        var allocatedBudget = budgetAllocations.reduce(function (sum, a) { return sum + a.allocated_capacity; }, 0);
        var efficiency = totalStaff > 0 ? (utilizedStaff / totalStaff) * 100 : 0;
        var recommendations = plan.allocations.filter(function (a) { var _a; return ((_a = a.optimization_suggestions) === null || _a === void 0 ? void 0 : _a.length) > 0; }).length;
        return {
    ...
    var chartData = (0, react_1.useMemo)(function () {
        ...
        var utilizationData = RESOURCE_TYPES.map(function (type) {
            var allocations = plan.allocations.filter(function (a) { return a.resource_type === type.id; });
            var total = allocations.reduce(function (sum, a) { return sum + a.capacity; }, 0);
            var utilized = allocations.reduce(function (sum, a) { return sum + a.allocated_capacity; }, 0);
            return {
                name: type.label,
                total: total,
                utilized: utilized,
        ...
        var departments = __spreadArray([], new Set(plan.allocations.map(function (a) { return a.resource_id.split('-')[0]; })), true);
        var distributionData = departments.map(function (dept) {
            var deptAllocations = plan.allocations.filter(function (a) { return a.resource_id.startsWith(dept); });
            var allocated = deptAllocations.reduce(function (sum, a) { return sum + a.allocated_capacity; }, 0);
            return {
                name: dept.replace('_', ' ').replace(/\b\w/g, function (l) { return l.toUpperCase(); }),
                value: allocated,
    ...
    var applyAdjustments = function () { return __awaiter(_this, void 0, void 0, function () {
        var error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fetch("/api/forecasting/allocation-plans/".concat(plan.id), {
                            method: 'PATCH',
    ...
    return (<card_1.Card className={className}>
      <card_1.CardHeader>
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          ...
          <div className="flex items-center space-x-2">
            {editable && (<button_1.Button variant={editMode ? "default" : "outline"} size="sm" onClick={function () { return setEditMode(!editMode); }}>
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {chartData.utilization.map(function (resource, index) { return (<div key={resource.name} className="space-y-2">
        ...
        <tabs_1.Tabs value={activeResourceType} onValueChange={setActiveResourceType}>
          <tabs_1.TabsList className="grid w-full grid-cols-4">
            ...
            var Icon = type.icon;
            return (<tabs_1.TabsTrigger key={type.id} value={type.id} className="flex items-center space-x-1">
          ...
          {RESOURCE_TYPES.map(function (type) { return (<tabs_1.TabsContent key={type.id} value={type.id} className="space-y-4">
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <card_1.Card>
                  ...
                  <card_1.CardContent>
                    <div className="space-y-4">
                      {plan.allocations
                .filter(function (allocation) { return allocation.resource_type === type.id; })
                .slice(0, detailed ? undefined : 5)
                .map(function (allocation, index) {
                var _a, _b;
                return (<div key={allocation.resource_id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div className="space-y-1">
                            <div className="font-medium">
                              {allocation.resource_id.replace('_', ' ').replace(/\b\w/g, function (l) { return l.toUpperCase(); })}
                            </div>
                            <div className="text-sm text-muted-foreground">
                              Period: {(0, date_fns_1.format)(new Date(allocation.period_start), 'MMM dd')} - {(0, date_fns_1.format)(new Date(allocation.period_end), 'MMM dd')}
                            </div>
                            {((_a = allocation.optimization_suggestions) === null || _a === void 0 ? void 0 : _a.length) > 0 && (<div className="text-xs text-blue-600">
                                {allocation.optimization_suggestions.length} optimization suggestions
                              </div>)}
                          </div>
                          <div className="text-right space-y-1">
                            {editMode ? (<input_1.Input type="number" value={(_b = adjustments[allocation.resource_id]) !== null && _b !== void 0 ? _b : allocation.allocated_capacity} onChange={function (e) { return handleAdjustment(allocation.resource_id, parseInt(e.target.value) || 0); }} className="w-20 text-right" min={0} max={allocation.capacity}/>) : (<div className="text-lg font-bold">
                    ...
                    <div className="h-[250px]">
                      <recharts_1.ResponsiveContainer width="100%" height="100%">
                        <recharts_1.LineChart data={chartData.timeline}>
                          ...
                          <recharts_1.Line type="monotone" dataKey={type.id === 'staff' ? 'staff' : type.id === 'rooms' ? 'rooms' : 'efficiency'} stroke={type.color} strokeWidth={2} dot={{ fill: type.color, strokeWidth: 2, r: 4 }}/>
        ...
        {/* Edit Mode Actions */}
        {editMode && Object.keys(adjustments).length > 0 && (<div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="text-sm">
              <span className="font-medium">{Object.keys(adjustments).length}</span> allocation adjustments pending
            </div>
            <div className="flex space-x-2">
              <button_1.Button variant="outline" size="sm" onClick={function () { return setAdjustments({}); }}>
                Cancel
              </button_1.Button>
        ...
        {plan.allocations.some(function (a) { var _a; return ((_a = a.optimization_suggestions) === null || _a === void 0 ? void 0 : _a.length) > 0; }) && (<div className="space-y-4">
            ...
            <div className="space-y-2">
              {plan.allocations
                .filter(function (a) { var _a; return ((_a = a.optimization_suggestions) === null || _a === void 0 ? void 0 : _a.length) > 0; })
                .slice(0, 3)
                .map(function (allocation, index) {
                var _a;
                return (<div key={index} className="flex items-start space-x-2 text-sm">
                  ...
                  <div>
                    ...
                    <span>{(_a = allocation.optimization_suggestions) === null || _a === void 0 ? void 0 : _a[0]}</span>
                  </div>
                </div>);
            })}
            </div>
