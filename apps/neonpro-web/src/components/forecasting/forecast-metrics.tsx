...
export function ForecastMetrics({ forecasts, className = "" }: ForecastMetricsProps) {
  ...
  const metrics = useMemo((): MetricsSummary => {
    ...
    // Model distribution
    const modelCounts = forecasts.reduce(
      (acc, forecast) => {
    ...
    const typeCounts = forecasts.reduce(
      (acc, forecast) => {
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Generate forecasts to view performance metrics
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-2">
            ...
            <div className="text-2xl font-bold">
              {Math.round(
                ((metrics.qualityIndicators.excellent + metrics.qualityIndicators.good) /
                  metrics.totalForecasts) *
                  100,
              )}
              %
            </div>
            <div className="text-xs text-muted-foreground">Excellent + Good quality forecasts</div>
          </div>
        </div>
          ...
          <div className="space-y-2">
            ...
            {metrics.qualityIndicators.poor > 0 && (
              <div className="flex items-start space-x-2 text-sm">
                ...
                <span>
                  {metrics.qualityIndicators.poor} forecasts have poor quality. Review data quality
                  and model selection.
                </span>
              </div>
            )}
