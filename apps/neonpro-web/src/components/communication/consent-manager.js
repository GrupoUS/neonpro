...
function ConsentManager(_a) {
    var _this = this;
    var patientId = _a.patientId, consents = _a.consents, onConsentUpdate = _a.onConsentUpdate, className = _a.className;
    var _b = (0, react_1.useState)(false), loading = _b[0], setLoading = _b[1];
    var _c = (0, react_1.useState)(null), showRevokeDialog = _c[0], setShowRevokeDialog = _c[1];
    ...
    var getConsentStatus = function (consentType) {
        return consents.find(function (c) { return c.consent_type === consentType; });
    };
    // Atualizar consentimento
    var updateConsent = function (consentType, consented, reason) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_f) {
            switch (_f.label) {
                ...
                case 2:
                    consentData = (_c.ip_address = _f.sent(),
                ...
                case 7: 
                // Log de auditoria
                return [4 /*yield*/, _b.apply(_a, [(_d.ip_address = _f.sent(),
                            _d.user_agent = navigator.userAgent,
                            _d.details = {
                                consent_type: consentType,
                                previous_status: existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.consented,
                ...
                case 8:
                    ...
                    toast({
                        ...
                        description: "".concat((_e = CONSENT_TYPES.find(function (t) { return t.key === consentType; })) === null || _e === void 0 ? void 0 : _e.label, " ").concat(consented ? 'ativado' : 'desativado', " com sucesso."),
                    });
                    return [3 /*break*/, 11];
                case 9:
                    error_1 = _f.sent();
    ...
    var getClientIP = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var renderConsentItem = function (consentType) {
        ...
        var isConsented = (consent === null || consent === void 0 ? void 0 : consent.consented) || false;
        var isRequired = consentType.required;
        return (<div key={consentType.key} className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex-1 space-y-1">
            <div className="flex items-center gap-2">
              {consentType.icon}
              <h4 className="font-medium">{consentType.label}</h4>
              {isRequired && (<badge_1.Badge variant="destructive" className="text-xs">
                  Obrigatório
                </badge_1.Badge>)}
            ...
            {consent && (<div className="flex items-center gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1">
                  <lucide_react_1.Clock className="w-3 h-3"/>
                  {isConsented ? 'Concedido' : 'Revogado'} {(0, date_fns_1.formatDistanceToNow)(new Date(isConsented ? consent.consented_at : consent.revoked_at), { addSuffix: true, locale: locale_1.ptBR })}
                </span>
              </div>)}
          </div>

          <div className="flex items-center gap-2">
            {isRequired && !isConsented && (<lucide_react_1.AlertCircle className="w-4 h-4 text-destructive"/>)}
            
            {isConsented ? (<dialog_1.Dialog open={showRevokeDialog === consentType.key} onOpenChange={function (open) { return setShowRevokeDialog(open ? consentType.key : null); }}>
                ...
                <dialog_1.DialogContent>
                  <dialog_1.DialogHeader>
                    ...
                    <dialog_1.DialogDescription>
                      ...
                      {isRequired && (<alert_1.Alert className="mt-3">
                          <lucide_react_1.AlertCircle className="w-4 h-4"/>
                          <alert_1.AlertDescription>
                            <strong>Atenção:</strong> Este consentimento é obrigatório para 
                            o funcionamento adequado dos nossos serviços. Revogar pode afetar 
                            sua experiência.
                          </alert_1.AlertDescription>
                        </alert_1.Alert>)}
                    </dialog_1.DialogDescription>
                  ...
                  <dialog_1.DialogFooter>
                    <button_1.Button variant="outline" onClick={function () { return setShowRevokeDialog(null); }}>
                      Cancelar
                    </button_1.Button>
                    <button_1.Button variant="destructive" onClick={function () { return updateConsent(consentType.key, false); }} disabled={loading}>
                      Confirmar Revogação
                    </button_1.Button>
                  </dialog_1.DialogFooter>
                </dialog_1.DialogContent>
              </dialog_1.Dialog>) : (<button_1.Button size="sm" onClick={function () { return updateConsent(consentType.key, true); }} disabled={loading}>
    ...
    return (<card_1.Card className={(0, utils_1.cn)('', className)}>
      <card_1.CardHeader>
        ...
        <alert_1.Alert>
          <lucide_react_1.AlertCircle className="w-4 h-4"/>
          <alert_1.AlertDescription>
            <strong>LGPD - Lei Geral de Proteção de Dados:</strong> Você tem o direito 
            de controlar como seus dados são utilizados. Pode revogar consentimentos 
            a qualquer momento.
          </alert_1.AlertDescription>
        </alert_1.Alert>
      </card_1.CardHeader>
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <alert_1.Alert>
          ...
          <alert_1.AlertDescription className="text-xs">
            <strong>Importante:</strong> Todas as ações de consentimento são registradas 
            para fins de auditoria e compliance. Os dados são armazenados de forma segura 
            conforme as normas da LGPD.
          </alert_1.AlertDescription>
        </alert_1.Alert>
      </card_1.CardContent>
    </card_1.Card>);
