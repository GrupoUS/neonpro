...
function CommunicationDashboard(_a) {
    var _this = this;
    var _b;
    var userId = _a.userId, clinicId = _a.clinicId, className = _a.className;
    ...
    var _f = (0, react_1.useState)([]), consents = _f[0], setConsents = _f[1];
    var _g = (0, react_1.useState)([]), notifications = _g[0], setNotifications = _g[1];
    var _h = (0, react_1.useState)(null), selectedConversation = _h[0], setSelectedConversation = _h[1];
    ...
    (0, react_1.useEffect)(function () {
        ...
        var conversationChannel = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'communication_conversations' }, function () { return loadConversations(); })
            .subscribe();
        var messageChannel = supabase
            .channel('dashboard-messages')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'communication_messages' }, function () { return loadStats(); })
            .subscribe();
        return function () {
    ...
    var loadStats = function () { return __awaiter(_this, void 0, void 0, function () {
        var today, _a, conversationsData, messagesData, notificationsData, consentsData, conversations_1, messages, notifications_1, consents_1, messagesSent, messagesReceived, emailsToday, smsToday, pushToday, totalConsents, consentedCount, consentRate, error_2;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    _a = _b.sent(), conversationsData = _a[0], messagesData = _a[1], notificationsData = _a[2], consentsData = _a[3];
                    ...
                    messagesSent = messages.filter(function (m) { return m.sender_id === userId; }).length;
                    messagesReceived = messages.length - messagesSent;
                    emailsToday = notifications_1.filter(function (n) { return n.type === 'email' && n.sent_at; }).length;
                    smsToday = notifications_1.filter(function (n) { return n.type === 'sms' && n.sent_at; }).length;
                    pushToday = notifications_1.filter(function (n) { return n.type === 'push' && n.sent_at; }).length;
                    totalConsents = consents_1.length;
                    consentedCount = consents_1.filter(function (c) { return c.consented; }).length;
                    consentRate = totalConsents > 0 ? (consentedCount / totalConsents) * 100 : 0;
                    setStats({
                        totalConversations: conversations_1.length,
                        activeConversations: conversations_1.filter(function (c) { return c.status === 'active'; }).length,
    ...
    var StatCard = function (_a) {
        var title = _a.title, value = _a.value, icon = _a.icon, change = _a.change, _b = _a.changeType, changeType = _b === void 0 ? 'neutral' : _b;
        return (<card_1.Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {change && (<badge_1.Badge variant={changeType === 'positive' ? 'default' :
    ...
    return (<div className={(0, utils_1.cn)('space-y-6', className)}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Conversas Ativas" value={stats.activeConversations} icon={<lucide_react_1.MessageSquare className="w-5 h-5"/>} change="+12%" changeType="positive"/>
        <StatCard title="Mensagens Hoje" value={stats.messagesSentToday + stats.messagesReceivedToday} icon={<lucide_react_1.Send className="w-5 h-5"/>} change="+8%" changeType="positive"/>
        <StatCard title="Taxa de Consentimento" value={"".concat(stats.consentRate, "%")} icon={<lucide_react_1.CheckCircle className="w-5 h-5"/>} change="+3%" changeType="positive"/>
        <StatCard title="Taxa de Resposta" value={"".concat(stats.responseRate, "%")} icon={<lucide_react_1.TrendingUp className="w-5 h-5"/>} change="+5%" changeType="positive"/>
      </div>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
          ...
          <card_1.Card>
            ...
            var _a, _b, _c;
            return (<div key={conversation.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {((_a = conversation.patient) === null || _a === void 0 ? void 0 : _a.name) || 'Paciente não identificado'}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {((_c = (_b = conversation.last_message) === null || _b === void 0 ? void 0 : _b.content) === null || _c === void 0 ? void 0 : _c.substring(0, 50)) || 'Sem mensagens'}...
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <badge_1.Badge variant={conversation.status === 'active' ? 'default' : 'secondary'}>
                        {conversation.status}
                      </badge_1.Badge>
        ...
        <tabs_1.TabsContent value="conversations">
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            var _a, _b;
            return (<button_1.Button key={conversation.id} variant={selectedConversation === conversation.id ? 'default' : 'ghost'} className="w-full justify-start h-auto p-3" onClick={function () { return setSelectedConversation(conversation.id); }}>
                    <div className="text-left">
                      <p className="font-medium">
                        {((_a = conversation.patient) === null || _a === void 0 ? void 0 : _a.name) || 'Paciente não identificado'}
                      </p>
                      <p className="text-xs text-muted-foreground truncate">
                        {((_b = conversation.last_message) === null || _b === void 0 ? void 0 : _b.content) || 'Sem mensagens'}
                      </p>
                    </div>
                  </button_1.Button>);
        })}
            ...
            <div className="lg:col-span-2">
              {selectedConversation ? (<staff_chat_1.StaffChat conversationId={selectedConversation} userId={userId} patientContext={(_b = conversations.find(function (c) { return c.id === selectedConversation; })) === null || _b === void 0 ? void 0 : _b.patient}/>) : (<card_1.Card className="h-[600px] flex items-center justify-center">
        ...
        <tabs_1.TabsContent value="templates">
          <template_manager_1.TemplateManager templates={templates} onTemplateUpdate={function (template) {
            setTemplates(function (prev) {
                var index = prev.findIndex(function (t) { return t.id === template.id; });
                if (index >= 0) {
            ...
            setTemplates(function (prev) { return prev.filter(function (t) { return t.id !== templateId; }); });
        }}/>
        </tabs_1.TabsContent>

        <tabs_1.TabsContent value="consents">
          <consent_manager_1.ConsentManager patientId={userId} // TODO: Implementar seleção de paciente
     consents={consents} onConsentUpdate={function (consent) {
            setConsents(function (prev) {
                var index = prev.findIndex(function (c) { return c.id === consent.id; });
                if (index >= 0) {
                    var updated = __spreadArray([], prev, true);
