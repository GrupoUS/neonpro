...
function TemplateManager(_a) {
    var _this = this;
    var templates = _a.templates, onTemplateUpdate = _a.onTemplateUpdate, onTemplateDelete = _a.onTemplateDelete, className = _a.className;
    ...
    var _d = (0, react_1.useState)(null), previewTemplate = _d[0], setPreviewTemplate = _d[1];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    templateData = __assign(__assign({}, formData), { variables: extractedVariables, clinic_id: 'clinic-id', updated_at: new Date().toISOString() });
                    response = void 0;
                ...
                case 3: return [4 /*yield*/, supabase
                        .from('communication_templates')
                        .insert(__assign(__assign({}, templateData), { created_at: new Date().toISOString() }))
                        .select()
                        .single()];
                case 4:
                    // Criar novo template
    ...
    // Deletar template
    var deleteTemplate = function (templateId) { return __awaiter(_this, void 0, void 0, function () {
        var error, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    var duplicateTemplate = function (template) {
        openEditor(__assign(__assign({}, template), { id: '', name: "".concat(template.name, " (C\u00F3pia)"), created_at: '', updated_at: '' }));
    };
    ...
    var insertVariable = function (variable) {
        ...
        if (textarea) {
            ...
            var newContent_1 = formData.content.substring(0, start_1) + variable + formData.content.substring(end);
            setFormData(function (prev) { return (__assign(__assign({}, prev), { content: newContent_1 })); });
    ...
    return (<div className={(0, utils_1.cn)('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <button_1.Button onClick={function () { return openEditor(); }}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {templates.map(function (template) {
            var _a, _b;
            return (<card_1.Card key={template.id} className="relative">
            <card_1.CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div>
                  <card_1.CardTitle className="text-lg">{template.name}</card_1.CardTitle>
                  <div className="flex items-center gap-2 mt-1">
                    {(_a = TEMPLATE_TYPES.find(function (t) { return t.value === template.type; })) === null || _a === void 0 ? void 0 : _a.icon}
                    <badge_1.Badge variant="outline">
                      {(_b = TEMPLATE_TYPES.find(function (t) { return t.value === template.type; })) === null || _b === void 0 ? void 0 : _b.label}
                    </badge_1.Badge>
              ...
              <div className="flex items-center gap-2">
                <dialog_1.Dialog>
                  <dialog_1.DialogTrigger asChild>
                    <button_1.Button variant="ghost" size="sm" onClick={function () { return setPreviewTemplate(template); }}>
                ...
                <button_1.Button variant="ghost" size="sm" onClick={function () { return openEditor(template); }}>
                  <lucide_react_1.Edit className="w-3 h-3"/>
                </button_1.Button>

                <button_1.Button variant="ghost" size="sm" onClick={function () { return duplicateTemplate(template); }}>
                  <lucide_react_1.Copy className="w-3 h-3"/>
                </button_1.Button>

                <button_1.Button variant="ghost" size="sm" onClick={function () { return deleteTemplate(template.id); }} disabled={loading}>
      ...
      <dialog_1.Dialog open={isEditing} onOpenChange={setIsEditing}>
        <dialog_1.DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input_1.Input id="template-name" value={formData.name} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { name: e.target.value })); }); }} placeholder="Ex: Lembrete de Consulta"/>
              </div>
            ...
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { type: value })); });
                    ...
                    <select_1.SelectContent>
                      {TEMPLATE_TYPES.map(function (type) { return (<select_1.SelectItem key={type.value} value={type.value}>
                          <div className="flex items-center gap-2">
                            {type.icon}
                            {type.label}
                          </div>
                ...
                <div>
                  ...
                  <select_1.Select value={formData.category} onValueChange={function (value) {
            return setFormData(function (prev) { return (__assign(__assign({}, prev), { category: value })); });
        }}>
                    <select_1.SelectTrigger>
                      <select_1.SelectValue />
                    </select_1.SelectTrigger>
                    <select_1.SelectContent>
                      {TEMPLATE_CATEGORIES.map(function (category) { return (<select_1.SelectItem key={category} value={category}>
                          {category.replace('_', ' ')}
                        </select_1.SelectItem>); })}
                    </select_1.SelectContent>
                  </select_1.Select>
              ...
              {formData.type === 'email' && (<div>
                  <label_1.Label htmlFor="template-subject">Assunto</label_1.Label>
                  <input_1.Input id="template-subject" value={formData.subject} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { subject: e.target.value })); }); }} placeholder="Ex: Lembrete: Consulta agendada para {{appointment_date}}"/>
                </div>)}

              <div>
                <label_1.Label htmlFor="template-content">Conteúdo</label_1.Label>
                <textarea_1.Textarea id="template-content" value={formData.content} onChange={function (e) { return setFormData(function (prev) { return (__assign(__assign({}, prev), { content: e.target.value })); }); }} placeholder="Digite o conteúdo do template..." rows={8}/>
              </div>

              <div>
                <label_1.Label>Variáveis Disponíveis</label_1.Label>
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {AVAILABLE_VARIABLES.map(function (variable) { return (<button_1.Button key={variable} variant="outline" size="sm" onClick={function () { return insertVariable(variable); }} type="button">
                      {variable}
                    </button_1.Button>); })}
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-2 p-4 bg-muted rounded-lg min-h-[200px]">
                  {formData.subject && (<div className="mb-4">
          ...
          <dialog_1.DialogFooter>
            ...
            <button_1.Button onClick={saveTemplate} disabled={loading || !formData.name || !formData.content}>
