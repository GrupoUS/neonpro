...
function PrivacyPreferences(_a) {
    var _this = this;
    var userId = _a.userId, _b = _a.showHeader, showHeader = _b === void 0 ? true : _b, className = _a.className;
    var _c = (0, useLGPD_1.useConsentBanner)(), purposes = _c.purposes, userConsents = _c.userConsents, isLoading = _c.isLoading, error = _c.error, giveConsent = _c.giveConsent, withdrawConsent = _c.withdrawConsent, refreshData = _c.refreshData;
    var _d = (0, react_1.useState)({}), preferences = _d[0], setPreferences = _d[1];
    ...
    var _g = (0, react_1.useState)(null), saveMessage = _g[0], setSaveMessage = _g[1];
    (0, react_1.useEffect)(function () {
        // Inicializar preferências com consentimentos existentes
        var initialPreferences = {};
        purposes === null || purposes === void 0 ? void 0 : purposes.forEach(function (purpose) {
            var existingConsent = userConsents === null || userConsents === void 0 ? void 0 : userConsents.find(function (c) { return c.purpose_id === purpose.id; });
            initialPreferences[purpose.id] = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.status) === 'given';
        });
    ...
    var handlePreferenceChange = function (purposeId, enabled) {
        setPreferences(function (prev) {
            ...
            var hasChanges = (purposes === null || purposes === void 0 ? void 0 : purposes.some(function (purpose) {
                var existingConsent = userConsents === null || userConsents === void 0 ? void 0 : userConsents.find(function (c) { return c.purpose_id === purpose.id; });
                var currentStatus = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.status) === 'given';
    ...
    var handleSavePreferences = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    _loop_1 = function (purposeId, enabled) {
                        ...
                        return __generator(this, function (_d) {
                            switch (_d.label) {
                                case 0:
                                    existingConsent = userConsents === null || userConsents === void 0 ? void 0 : userConsents.find(function (c) { return c.purpose_id === purposeId; });
                                    currentStatus = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.status) === 'given';
                                ...
                                case 1:
                                    _d.sent();
                                    return [3 /*break*/, 4];
                                case 2: return [4 /*yield*/, withdrawConsent(purposeId)];
                                case 3:
                                    _d.sent();
                                    _d.label = 4;
                ...
                case 8:
                    setIsSaving(false);
                    setTimeout(function () { return setSaveMessage(null); }, 3000);
                    return [7 /*endfinally*/];
    ...
    var handleResetPreferences = function () {
        ...
        purposes === null || purposes === void 0 ? void 0 : purposes.forEach(function (purpose) {
            var existingConsent = userConsents === null || userConsents === void 0 ? void 0 : userConsents.find(function (c) { return c.purpose_id === purpose.id; });
            resetPreferences[purpose.id] = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.status) === 'given';
        });
    ...
    var getConsentHistory = function (purposeId) {
        return (userConsents === null || userConsents === void 0 ? void 0 : userConsents.filter(function (c) { return c.purpose_id === purposeId; })) || [];
    };
    var essentialPurposes = (purposes === null || purposes === void 0 ? void 0 : purposes.filter(function (p) { return p.category === 'essential'; })) || [];
    var optionalPurposes = (purposes === null || purposes === void 0 ? void 0 : purposes.filter(function (p) { return p.category !== 'essential'; })) || [];
    if (isLoading) {
    ...
    return (<div className={className}>
      ...
      {saveMessage && (<alert_1.Alert className={"mb-4 ".concat(saveMessage.includes('sucesso')
                ? 'border-green-200 bg-green-50 text-green-800'
                : 'border-red-200 bg-red-50 text-red-800')}>
          {saveMessage.includes('sucesso') ? (<lucide_react_1.CheckCircle className="h-4 w-4"/>) : (<lucide_react_1.XCircle className="h-4 w-4"/>)}
      ...
      <tabs_1.Tabs defaultValue="preferences" className="space-y-6">
        ...
        <tabs_1.TabsContent value="preferences" className="space-y-6">
          {/* Cookies Essenciais */}
          {essentialPurposes.length > 0 && (<card_1.Card>
              <card_1.CardHeader>
              ...
              <card_1.CardContent className="space-y-4">
                {essentialPurposes.map(function (purpose) { return (<div key={purpose.id} className="flex items-start justify-between p-4 border rounded-lg bg-green-50">
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        {purpose.legal_basis && (<p className="text-xs text-gray-500">
          ...
          {/* Cookies Opcionais */}
          {optionalPurposes.length > 0 && (<card_1.Card>
              <card_1.CardHeader>
                <card_1.CardTitle className="flex items-center gap-2">
              ...
              <card_1.CardContent className="space-y-4">
                {optionalPurposes.map(function (purpose) { return (<div key={purpose.id} className={"p-4 border rounded-lg transition-colors ".concat(preferences[purpose.id] ? 'bg-blue-50 border-blue-200' : 'bg-gray-50')}>
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="font-medium">{purpose.name}</h4>
                            <badge_1.Badge variant="outline" className={getPurposeColor(purpose.category)}>
                              {purpose.category}
                            </badge_1.Badge>
                          </div>
                          <p className="text-sm text-gray-600 mb-2">{purpose.description}</p>
                          ...
                          <div className="space-y-1 text-xs text-gray-500">
                            {purpose.legal_basis && (<p><strong>Base Legal:</strong> {purpose.legal_basis}</p>)}
                            {purpose.data_retention_days && (<p><strong>Retenção:</strong> {purpose.data_retention_days} dias</p>)}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <switch_1.Switch id={"switch-".concat(purpose.id)} checked={preferences[purpose.id] || false} onCheckedChange={function (checked) {
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            <button_1.Button variant="outline" onClick={handleResetPreferences} disabled={!hasChanges || isSaving}>
              <lucide_react_1.RefreshCw className="h-4 w-4 mr-2"/>
              Resetar
            </button_1.Button>
            
            <button_1.Button onClick={handleSavePreferences} disabled={!hasChanges || isSaving} className="bg-blue-600 hover:bg-blue-700">
              {isSaving ? (<lucide_react_1.RefreshCw className="h-4 w-4 mr-2 animate-spin"/>) : (<lucide_react_1.Save className="h-4 w-4 mr-2"/>)}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {purposes === null || purposes === void 0 ? void 0 : purposes.map(function (purpose) {
            var history = getConsentHistory(purpose.id);
            var currentConsent = history.find(function (c) { return c.status === 'given'; });
            return (<div key={purpose.id} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          {getPurposeIcon(purpose.category)}
                          <h4 className="font-medium">{purpose.name}</h4>
                          <badge_1.Badge variant={currentConsent ? "default" : "outline"} className={currentConsent ? "bg-green-100 text-green-800" : ""}>
                      ...
                      {history.length > 0 ? (<div className="space-y-2">
                          {history.slice(0, 3).map(function (consent, index) { return (<div key={consent.id} className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded">
                              <div className="flex items-center gap-2">
                                {consent.status === 'given' ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.XCircle className="h-4 w-4 text-red-600"/>)}
                                <span>
                          ...
                          {history.length > 3 && (<p className="text-xs text-gray-500 text-center">
                              +{history.length - 3} registros anteriores
                            </p>)}
                        </div>) : (<p className="text-sm text-gray-500">Nenhum histórico disponível</p>)}
                    </div>);
        })}
              </div>
            </card_1.CardContent>
