...
function ConsentBanner(_a) {
    var _this = this;
    var _b = _a.position, position = _b === void 0 ? 'bottom' : _b, _c = _a.theme, theme = _c === void 0 ? 'light' : _c, _d = _a.showLogo, showLogo = _d === void 0 ? true : _d, _e = _a.companyName, companyName = _e === void 0 ? 'NeonPro' : _e, _f = _a.privacyPolicyUrl, privacyPolicyUrl = _f === void 0 ? '/privacy-policy' : _f, _g = _a.termsOfServiceUrl, termsOfServiceUrl = _g === void 0 ? '/terms-of-service' : _g;
    var _h = (0, useLGPD_1.useConsentBanner)(), purposes = _h.purposes, userConsents = _h.userConsents, isVisible = _h.isVisible, isLoading = _h.isLoading, giveConsent = _h.giveConsent, withdrawConsent = _h.withdrawConsent, acceptAll = _h.acceptAll, rejectAll = _h.rejectAll, hideConsentBanner = _h.hideConsentBanner;
    var _j = (0, react_1.useState)({}), selectedPurposes = _j[0], setSelectedPurposes = _j[1];
    var _k = (0, react_1.useState)(false), showDetails = _k[0], setShowDetails = _k[1];
    (0, react_1.useEffect)(function () {
        ...
        purposes === null || purposes === void 0 ? void 0 : purposes.forEach(function (purpose) {
            ...
            initialConsents[purpose.id] = (existingConsent === null || existingConsent === void 0 ? void 0 : existingConsent.status) === 'given';
        });
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 2:
                    _c.sent();
                    return [3 /*break*/, 5];
                case 3: return [4 /*yield*/, withdrawConsent(purposeId)];
                case 4:
                    _c.sent();
                    _c.label = 5;
                case 5:
    ...
    return (<>
      ...
      {/* Banner */}
      <div className={"fixed left-0 right-0 z-50 ".concat(position === 'top' ? 'top-0' : 'bottom-0')}>
        <card_1.Card className={"mx-4 mb-4 shadow-lg border-2 ".concat(theme === 'dark'
            ? 'bg-gray-900 border-gray-700 text-white'
            : 'bg-white border-gray-200')}>
          <card_1.CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                {showLogo && (<div className="flex items-center justify-center w-10 h-10 bg-blue-600 rounded-lg">
                ...
                <div>
                  ...
                  <p className={"text-sm ".concat(theme === 'dark' ? 'text-gray-300' : 'text-gray-600')}>
                    Respeitamos sua privacidade e seguimos a LGPD
                  </p>
                </div>
              </div>
              
              <button_1.Button variant="ghost" size="sm" onClick={function () { return hideConsentBanner(); }} className="text-gray-500 hover:text-gray-700">
            ...
            <div className="space-y-4">
              {/* Resumo */}
              <div className={"p-4 rounded-lg border ".concat(theme === 'dark'
            ...
            : 'bg-gray-50 border-gray-200')}>
              ...
              {/* Cookies essenciais */}
              {essentialPurposes.length > 0 && (<div className="space-y-2">
              ...
              {/* Cookies opcionais */}
              {optionalPurposes.length > 0 && (<div className="space-y-3">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-6">
                    {optionalPurposes.map(function (purpose) { return (<div key={purpose.id} className="flex items-center space-x-3">
                        <checkbox_1.Checkbox id={purpose.id} checked={selectedPurposes[purpose.id] || false} onCheckedChange={function (checked) {
              ...
              <div className="flex items-center gap-4 text-xs text-gray-600">
                <a href={privacyPolicyUrl} className="flex items-center gap-1 hover:text-blue-600 transition-colors" target="_blank" rel="noopener noreferrer">
                  <lucide_react_1.FileText className="h-3 w-3"/>
                  Política de Privacidade
                  <lucide_react_1.ExternalLink className="h-3 w-3"/>
                </a>
                <a href={termsOfServiceUrl} className="flex items-center gap-1 hover:text-blue-600 transition-colors" target="_blank" rel="noopener noreferrer">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t">
                <div className="flex gap-2 flex-1">
                  <button_1.Button onClick={handleAcceptAll} className="flex-1 bg-blue-600 hover:bg-blue-700" disabled={isLoading}>
                ...
                <div className="flex gap-2">
                  <button_1.Button variant="outline" onClick={handleSavePreferences} disabled={isLoading}>
                    ...
                    <dialog_1.DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                      ...
                      <div className="space-y-6">
                        <accordion_1.Accordion type="single" collapsible className="w-full">
                          {purposes === null || purposes === void 0 ? void 0 : purposes.map(function (purpose) { return (<accordion_1.AccordionItem key={purpose.id} value={purpose.id}>
                              <accordion_1.AccordionTrigger className="text-left">
                                <div className="flex items-center gap-3">
                                  {getPurposeIcon(purpose.category)}
                                  <div>
                                    <div className="font-medium">{purpose.name}</div>
                                    <badge_1.Badge variant="outline" className={"".concat(getPurposeColor(purpose.category), " text-xs")}>
                                      {purpose.category}
                                    </badge_1.Badge>
                                  </div>
                                </div>
                              ...
                              <accordion_1.AccordionContent className="space-y-3">
                                ...
                                <div className="flex items-center gap-2 pt-2">
                                  ...
                                  {purpose.category === 'essential' ? (<badge_1.Badge className="bg-green-100 text-green-800">
                                      <lucide_react_1.Shield className="h-3 w-3 mr-1"/>
                                      Sempre Ativo
                                    </badge_1.Badge>) : (<badge_1.Badge variant={selectedPurposes[purpose.id] ? "default" : "outline"}>
