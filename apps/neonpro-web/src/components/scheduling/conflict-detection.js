...
function ConflictDetection(_a) {
    var _this = this;
    var appointmentStart = _a.appointmentStart, appointmentEnd = _a.appointmentEnd, professionalId = _a.professionalId, treatmentType = _a.treatmentType, roomId = _a.roomId, equipmentIds = _a.equipmentIds, onConflictDetected = _a.onConflictDetected, onResolutionSelected = _a.onResolutionSelected, _b = _a.autoDetect, autoDetect = _b === void 0 ? true : _b;
    var _c = (0, react_1.useState)(null), detectionResult = _c[0], setDetectionResult = _c[1];
    var _d = (0, react_1.useState)(false), isLoading = _d[0], setIsLoading = _d[1];
    var _e = (0, react_1.useState)(null), error = _e[0], setError = _e[1];
    var _f = (0, react_1.useState)(null), selectedResolution = _f[0], setSelectedResolution = _f[1];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch('/api/scheduling/conflicts/detect', {
                            method: 'POST',
                            headers: {
    ...
    var getConflictTypeIcon = function (type) {
        switch (type) {
            case 'time': return <lucide_react_1.Clock className="w-4 h-4"/>;
            case 'staff': return <lucide_react_1.Users className="w-4 h-4"/>;
            case 'room': return <lucide_react_1.MapPin className="w-4 h-4"/>;
            case 'equipment': return <lucide_react_1.AlertTriangle className="w-4 h-4"/>;
            case 'business_rules': return <lucide_react_1.XCircle className="w-4 h-4"/>;
    ...
    return (<div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <card_1.CardTitle className="flex items-center gap-2">
                {isLoading ? (<lucide_react_1.RefreshCw className="w-5 h-5 animate-spin"/>) : (detectionResult === null || detectionResult === void 0 ? void 0 : detectionResult.hasConflicts) ? (<lucide_react_1.XCircle className="w-5 h-5 text-red-500"/>) : detectionResult && !detectionResult.hasConflicts ? (<lucide_react_1.CheckCircle className="w-5 h-5 text-green-500"/>) : (<lucide_react_1.AlertTriangle className="w-5 h-5 text-gray-500"/>)}
                Conflict Detection
              </card_1.CardTitle>
              <card_1.CardDescription>
                {isLoading
            ? 'Analyzing potential scheduling conflicts...'
            : (detectionResult === null || detectionResult === void 0 ? void 0 : detectionResult.hasConflicts)
                ? "Found ".concat(detectionResult.conflicts.length, " conflict(s) that require attention")
            ...
            <button_1.Button onClick={detectConflicts} disabled={isLoading} variant="outline" size="sm">
              {isLoading ? (<lucide_react_1.RefreshCw className="w-4 h-4 animate-spin mr-2"/>) : (<lucide_react_1.RefreshCw className="w-4 h-4 mr-2"/>)}
              Re-check
            </button_1.Button>
          </div>
        </card_1.CardHeader>
        
        {detectionResult && (<card_1.CardFooter>
      ...
      {/* Conflicts List */}
      {(detectionResult === null || detectionResult === void 0 ? void 0 : detectionResult.hasConflicts) && (<card_1.Card>
          <card_1.CardHeader>
          ...
          <card_1.CardContent>
            <div className="space-y-4">
              {detectionResult.conflicts.map(function (conflict) { return (<div key={conflict.id} className="border rounded-lg p-4">
                  ...
                  {conflict.suggestedActions.length > 0 && (<div>
                      <span className="text-sm font-medium">Suggested actions:</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {conflict.suggestedActions.map(function (action, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                            {action.replace('_', ' ')}
                          </badge_1.Badge>); })}
                      </div>
                    </div>)}
      ...
      {/* Resolution Options */}
      {(detectionResult === null || detectionResult === void 0 ? void 0 : detectionResult.resolutionOptions) && detectionResult.resolutionOptions.length > 0 && (<card_1.Card>
          <card_1.CardHeader>
          ...
          <card_1.CardContent>
            <div className="space-y-4">
              {detectionResult.resolutionOptions.map(function (option) { return (<div key={option.id} className={"border rounded-lg p-4 cursor-pointer transition-all ".concat((selectedResolution === null || selectedResolution === void 0 ? void 0 : selectedResolution.id) === option.id
                    ? 'border-blue-500 bg-blue-50'
                    : 'hover:border-gray-300')} onClick={function () { return handleResolutionSelect(option); }}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                  ...
                  {option.alternativeSlots && option.alternativeSlots.length > 0 && (<div className="text-sm">
                  ...
                  {option.resourceAlternatives && option.resourceAlternatives.length > 0 && (<div className="text-sm">
          ...
          {selectedResolution && (<card_1.CardFooter>
              <button_1.Button onClick={function () { return handleResolutionSelect(selectedResolution); }} className="w-full">
      ...
      {/* Success State */}
      {detectionResult && !detectionResult.hasConflicts && (<card_1.Card>
