...
function WaitlistManagement(_a) {
    var _this = this;
    var treatmentType = _a.treatmentType, onPatientAdded = _a.onPatientAdded, onNotificationSent = _a.onNotificationSent, _b = _a.showAddForm, showAddForm = _b === void 0 ? true : _b;
    var _c = (0, react_1.useState)([]), waitlistEntries = _c[0], setWaitlistEntries = _c[1];
    var _d = (0, react_1.useState)(false), isLoading = _d[0], setIsLoading = _d[1];
    var _e = (0, react_1.useState)(null), error = _e[0], setError = _e[1];
    var _f = (0, react_1.useState)(false), showForm = _f[0], setShowForm = _f[1];
    // Form state
    ...
    var loadWaitlistEntries = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var addToWaitlist = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch('/api/scheduling/waitlist', {
                            method: 'POST',
                            headers: {
    ...
    var sendNotification = function (entryId) { return __awaiter(_this, void 0, void 0, function () {
    ...
    return (<div className="space-y-6">
      ...
      <card_1.Card>
        <card_1.CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button_1.Button onClick={loadWaitlistEntries} disabled={isLoading} variant="outline" size="sm">
                <lucide_react_1.RefreshCw className={"w-4 h-4 mr-2 ".concat(isLoading ? 'animate-spin' : '')}/>
                Refresh
              </button_1.Button>
              {showAddForm && (<button_1.Button onClick={function () { return setShowForm(!showForm); }} size="sm">
        ...
        <card_1.CardFooter>
          <div className="flex items-center gap-4 text-sm text-gray-600">
            ...
            <span>
              {waitlistEntries.filter(function (e) { return e.urgencyLevel === 'urgent' || e.urgencyLevel === 'emergency'; }).length} urgent case{waitlistEntries.filter(function (e) { return e.urgencyLevel === 'urgent' || e.urgencyLevel === 'emergency'; }).length !== 1 ? 's' : ''}
            </span>
          </div>
        </card_1.CardFooter>
      </card_1.Card>
          ...
          <card_1.CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input_1.Input id="patientId" value={formData.patientId} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { patientId: e.target.value })); }} placeholder="Enter patient ID"/>
              </div>
              <div>
                <label_1.Label htmlFor="treatmentType">Treatment Type</label_1.Label>
                <input_1.Input id="treatmentType" value={formData.treatmentType} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { treatmentType: e.target.value })); }} placeholder="Enter treatment type"/>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input_1.Input id="startDate" type="date" value={formData.preferredStartDate} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { preferredStartDate: e.target.value })); }}/>
              </div>
              <div>
                <label_1.Label htmlFor="endDate">Preferred End Date</label_1.Label>
                <input_1.Input id="endDate" type="date" value={formData.preferredEndDate} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { preferredEndDate: e.target.value })); }}/>
              </div>
            </div>
            
            <div>
              <label_1.Label htmlFor="urgencyLevel">Urgency Level</label_1.Label>
              <select_1.Select value={formData.urgencyLevel} onValueChange={function (value) { return setFormData(__assign(__assign({}, formData), { urgencyLevel: value })); }}>
            ...
            <div>
              <label_1.Label htmlFor="specialRequirements">Special Requirements (JSON)</label_1.Label>
              <textarea_1.Textarea id="specialRequirements" value={formData.specialRequirements} onChange={function (e) { return setFormData(__assign(__assign({}, formData), { specialRequirements: e.target.value })); }} placeholder='{"accessibility": true, "language": "spanish"}' rows={3}/>
            </div>
          </card_1.CardContent>
          <card_1.CardFooter className="flex gap-2">
            <button_1.Button onClick={addToWaitlist} disabled={isLoading}>
              {isLoading ? (<lucide_react_1.RefreshCw className="w-4 h-4 mr-2 animate-spin"/>) : (<lucide_react_1.Plus className="w-4 h-4 mr-2"/>)}
              Add to Waitlist
            </button_1.Button>
            <button_1.Button variant="outline" onClick={function () { return setShowForm(false); }}>
              Cancel
            </button_1.Button>
          </card_1.CardFooter>
        </card_1.Card>)}
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {isLoading && waitlistEntries.length === 0 ? (<div className="text-center py-8">
            ...
            </div>) : waitlistEntries.length === 0 ? (<div className="text-center py-8">
              <lucide_react_1.Users className="w-8 h-8 mx-auto mb-4 text-gray-400"/>
              <p className="text-gray-500">No patients on waitlist</p>
            </div>) : (<scroll_area_1.ScrollArea className="h-96">
              <div className="space-y-4">
                {waitlistEntries.map(function (entry) { return (<div key={entry.id} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      {entry.preferredTimeSlots && entry.preferredTimeSlots.length > 0 && (<div>
                          <span className="font-medium">Preferred times:</span>
                          <div className="text-gray-600">
                            {entry.preferredTimeSlots.map(function (slot, index) { return (<div key={index}>
                                {slot.dayOfWeek !== undefined && "".concat(getDayName(slot.dayOfWeek), " ")}
                    ...
                    {Object.keys(entry.specialRequirements).length > 0 && (<div className="mt-3">
                        ...
                        return (<badge_1.Badge key={key} variant="outline" className="mr-1 mt-1">
                              {key}: {String(value)}
                            </badge_1.Badge>);
                    })}
                        </div>
                    ...
                    <div className="flex justify-between items-center mt-4 pt-3 border-t">
                      <div className="text-xs text-gray-500">
                        {entry.lastNotificationAt && (<>Last notified: {formatDate(entry.lastNotificationAt.toString())}</>)}
                      </div>
                      
                      {entry.status === 'active' && (<button_1.Button size="sm" variant="outline" onClick={function () { return sendNotification(entry.id); }} disabled={isLoading}>
