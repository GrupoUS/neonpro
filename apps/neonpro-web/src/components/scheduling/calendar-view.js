...
var CalendarView = function (_a) {
    var onAppointmentClick = _a.onAppointmentClick, onTimeSlotClick = _a.onTimeSlotClick, onCreateAppointment = _a.onCreateAppointment;
    ...
    var _d = (0, react_1.useState)("week"), viewMode = _d[0], setViewMode = _d[1];
    var _e = (0, react_1.useState)("all"), selectedProfessional = _e[0], setSelectedProfessional = _e[1];
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        endDate = viewMode === "month" ? (0, date_fns_1.endOfWeek)(selectedDate) : (0, date_fns_1.endOfWeek)(currentWeek);
                        query = supabase
                            .from("appointments")
    ...
    }), _j = _h.data, appointments = _j === void 0 ? [] : _j, appointmentsLoading = _h.isLoading, appointmentsError = _h.error;
    // Fetch professionals
    ...
    }).data, professionals = _k === void 0 ? [] : _k;
    // Fetch service types
    ...
    }).data, serviceTypes = _l === void 0 ? [] : _l;
    // Week navigation
    ...
    var getAppointmentsForSlot = function (date, time, professionalId) {
        ...
        return appointments.filter(function (appointment) {
            ...
            var hasTimeOverlap = (appointmentStart <= slotDateTime && appointmentEnd > slotDateTime) ||
    ...
    var getPriorityIcon = function (priority) {
        if (priority >= 4)
            return <lucide_react_1.AlertTriangle className="w-3 h-3 text-red-500"/>;
        if (priority >= 3)
            return <lucide_react_1.Clock className="w-3 h-3 text-yellow-500"/>;
        return null;
    };
    // Appointment Card Component
    var AppointmentCard = function (_a) {
        var appointment = _a.appointment;
        return (<div className={"p-2 rounded-md text-xs cursor-pointer transition-all hover:shadow-md border ".concat(getStatusColor(appointment.status))} onClick={function () { return onAppointmentClick === null || onAppointmentClick === void 0 ? void 0 : onAppointmentClick(appointment); }} style={{
                borderLeftWidth: "4px",
    ...
    // Week View Component
    var WeekView = function () { return (<div className="overflow-x-auto">
      <div className="min-w-[800px]">
        {/* Header with days */}
        <div className="grid grid-cols-8 gap-1 mb-4">
          <div className="p-2 font-medium text-sm text-gray-600">Horário</div>
          {weekDays.map(function (day) { return (<div key={day.toISOString()} className="p-2 text-center">
              <div className="font-medium text-sm">
                {(0, date_fns_1.format)(day, "EEEEEE", { locale: locale_1.ptBR })}
              </div>
              <div className={"text-lg ".concat((0, date_fns_1.isSameDay)(day, new Date()) ? "text-blue-600 font-bold" : "")}>
        ...
        <div className="space-y-1">
          {timeSlots.map(function (time) { return (<div key={time} className="grid grid-cols-8 gap-1">
              <div className="p-2 text-sm text-gray-600 font-mono border-r">
                {time}
              </div>
              {weekDays.map(function (day) {
                var slotAppointments = getAppointmentsForSlot(day, time);
                return (<div key={"".concat(day.toISOString(), "-").concat(time)} className="min-h-[60px] border border-gray-200 p-1 hover:bg-gray-50 cursor-pointer transition-colors" onClick={function () { return onTimeSlotClick === null || onTimeSlotClick === void 0 ? void 0 : onTimeSlotClick(day, time); }}>
                    <div className="space-y-1">
                      {slotAppointments.map(function (appointment) { return (<AppointmentCard key={appointment.id} appointment={appointment}/>); })}
                    </div>
                  </div>);
            })}
            </div>); })}
    ...
    var MonthView = function () { return (<div className="space-y-4">
      <calendar_1.Calendar mode="single" selected={selectedDate} onSelect={function (date) { return date && setSelectedDate(date); }} className="rounded-md border" locale={locale_1.ptBR}/>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-2">
            ...
            .map(function (appointment) { return (<AppointmentCard key={appointment.id} appointment={appointment}/>); })}
            {appointments.filter(function (appointment) {
            return (0, date_fns_1.isSameDay)(new Date(appointment.start_time), selectedDate);
        }).length === 0 && (<p className="text-gray-500 text-center py-4">
    ...
    return (<div className="space-y-6">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            {/* Navigation Controls */}
            {viewMode === "week" && (<div className="flex items-center gap-2">
                <button_1.Button onClick={goToPreviousWeek} variant="outline" size="sm">
                  <lucide_react_1.ChevronLeft className="w-4 h-4"/>
                </button_1.Button>
                <span className="px-4 py-2 text-sm font-medium">
                  {(0, date_fns_1.format)((0, date_fns_1.startOfWeek)(currentWeek, { weekStartsOn: 1 }), "dd/MM", { locale: locale_1.ptBR })}{" "}
                  -{" "}
                  {(0, date_fns_1.format)((0, date_fns_1.endOfWeek)(currentWeek, { weekStartsOn: 1 }), "dd/MM/yyyy", { locale: locale_1.ptBR })}
                </span>
        ...
        }} variant="outline" size="sm" disabled={appointmentsLoading}>
                <lucide_react_1.RefreshCw className={"w-4 h-4 ".concat(appointmentsLoading ? "animate-spin" : "")}/>
              </button_1.Button>
            </div>
          </div>
        </card_1.CardContent>
      ...
      {/* Error Display */}
      {appointmentsError && (<alert_1.Alert variant="destructive">
      ...
      <card_1.Card>
        <card_1.CardContent className="p-6">
          {appointmentsLoading ? (<div className="flex items-center justify-center py-12">
              <lucide_react_1.RefreshCw className="w-6 h-6 animate-spin mr-2"/>
              Carregando agendamentos...
            </div>) : (<tabs_1.Tabs value={viewMode} className="w-full">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {appointments.filter(function (apt) { return apt.status === "confirmed"; })
            .length}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600">
                  {appointments.filter(function (apt) { return apt.status === "in_progress"; })
            .length}
                </p>
              </div>
