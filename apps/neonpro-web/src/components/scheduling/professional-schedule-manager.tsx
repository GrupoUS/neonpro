...
const ProfessionalScheduleManager: React.FC<ProfessionalScheduleManagerProps> = ({
  ...
  const [editingAvailability, setEditingAvailability] = useState<ProfessionalAvailability | null>(
    null,
  );
  ...
  const createDefaultSchedule = async () => {
    ...
    const defaultSchedule = daysOfWeek
      .filter(
        (day) => currentProfessional?.can_work_weekends || (day.value !== 0 && day.value !== 6),
      )
      .map((day) => ({
    ...
    try {
      const { error } = await supabase.from("professional_availability").insert(defaultSchedule);
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <Select onValueChange={(value) => handleBulkUpdate({ start_time: value })}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select onValueChange={(value) => handleBulkUpdate({ end_time: value })}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select onValueChange={(value) => handleBulkUpdate({ break_end_time: value })}>
                      <SelectTrigger>
