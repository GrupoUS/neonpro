...
function HealthcareProfile() {
    var _a;
    var _b = (0, hooks_1.useHealthcareAuth)(), user = _b.user, isLoading = _b.isLoading, error = _b.error, isAuthenticated = _b.isAuthenticated, isMedicalProfessional = _b.isMedicalProfessional, isAdmin = _b.isAdmin, isLGPDCompliant = _b.isLGPDCompliant, canAccessPatientData = _b.canAccessPatientData;
    var _c = (0, hooks_1.useLGPDConsent)(), grantConsent = _c.grantConsent, revokeConsent = _c.revokeConsent, consentLoading = _c.isLoading, consentError = _c.error;
    if (isLoading) {
    ...
    if (error) {
        return (<alert_1.Alert variant="destructive" className="w-full max-w-2xl mx-auto">
    ...
    if (!isAuthenticated || !user) {
        return (<alert_1.Alert className="w-full max-w-2xl mx-auto">
        ...
        <card_1.CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm">{((_a = user.profile) === null || _a === void 0 ? void 0 : _a.full_name) || 'Não informado'}</p>
            </div>
            <div>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              {isLGPDCompliant ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.AlertCircle className="h-4 w-4 text-red-600"/>)}
          ...
          <div className="flex space-x-2">
            {!isLGPDCompliant && (<button_1.Button onClick={grantConsent} disabled={consentLoading} size="sm">
                {consentLoading ? 'Processando...' : 'Conceder Consentimento LGPD'}
              </button_1.Button>)}
          </div>

          {consentError && (<alert_1.Alert variant="destructive">
              <lucide_react_1.AlertCircle className="h-4 w-4"/>
              <alert_1.AlertDescription>
