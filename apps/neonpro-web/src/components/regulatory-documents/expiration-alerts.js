...
function ExpirationAlerts(_a) {
    var _this = this;
    var onViewDocument = _a.onViewDocument, onEditDocument = _a.onEditDocument, _b = _a.compact, compact = _b === void 0 ? false : _b, _c = _a.showSettings, showSettings = _c === void 0 ? true : _c;
    ...
    var _g = (0, react_1.useState)(false), showAcknowledged = _g[0], setShowAcknowledged = _g[1];
    ...
    var acknowledgeAlert = function (alertId) { return __awaiter(_this, void 0, void 0, function () {
        var response, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/regulatory-documents/alerts/".concat(alertId, "/acknowledge"), {
                            method: 'PATCH'
                        })];
                case 1:
                    response = _a.sent();
                    ...
                    setAlerts(function (prev) { return prev.map(function (alert) {
                        return alert.id === alertId
                            ? __assign(__assign({}, alert), { acknowledged: true }) : alert;
                    }); });
    ...
    var filteredAlerts = showAcknowledged
        ? alerts
        : alerts.filter(function (alert) { return !alert.acknowledged; });
    if (loading) {
        return (<card_1.Card>
        <card_1.CardHeader>
    ...
    return (<card_1.Card>
      <card_1.CardHeader>
        <div className="flex items-center justify-between">
          <card_1.CardTitle className="flex items-center gap-2">
            <lucide_react_1.Bell className="h-5 w-5"/>
            Alertas de Compliance
            {filteredAlerts.length > 0 && (<badge_1.Badge variant="secondary">{filteredAlerts.length}</badge_1.Badge>)}
          </card_1.CardTitle>
          
          {showSettings && (<div className="flex items-center gap-2">
              <button_1.Button variant="outline" size="sm" onClick={function () { return setShowAcknowledged(!showAcknowledged); }}>
      ...
      <card_1.CardContent className="space-y-3">
        {filteredAlerts.map(function (alert) {
            var _a, _b, _c;
            return (<card_1.Card key={alert.id} className={"".concat(getSeverityColor(alert.severity), " ").concat(alert.acknowledged ? 'opacity-60' : '')}>
            <card_1.CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-medium mb-1">
                      {(_a = alert.document) === null || _a === void 0 ? void 0 : _a.document_type}
                      {((_b = alert.document) === null || _b === void 0 ? void 0 : _b.document_number) && " (".concat(alert.document.document_number, ")")}
                    </p>
                ...
                <div className="flex items-center gap-1 ml-3">
                  {onViewDocument && (<button_1.Button variant="ghost" size="sm" onClick={function () { return onViewDocument(alert.document_id); }} title="Visualizar documento">
                      <lucide_react_1.Eye className="h-4 w-4"/>
                    </button_1.Button>)}
                  
                  {onEditDocument && (<button_1.Button variant="ghost" size="sm" onClick={function () { return onEditDocument(alert.document_id); }} title="Editar documento">
                      <lucide_react_1.Edit className="h-4 w-4"/>
                    </button_1.Button>)}
                  
                  {!alert.acknowledged && (<button_1.Button variant="ghost" size="sm" onClick={function () { return acknowledgeAlert(alert.id); }} title="Reconhecer alerta">
