...
function ResourceManagement(_a) {
    ...
    var _c = (0, react_1.useState)(null), selectedResource = _c[0], setSelectedResource = _c[1];
    var _d = (0, react_1.useState)([]), allocations = _d[0], setAllocations = _d[1];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    setLoading(true);
                    params = new URLSearchParams(__assign(__assign(__assign({ clinic_id: clinicId }, (filters.type !== 'all' && { type: filters.type })), (filters.status !== 'all' && { status: filters.status })), (filters.category !== 'all' && { category: filters.category })));
    ...
    var fetchAllocations = function (resourceId) { return __awaiter(_this, void 0, void 0, function () {
    ...
    // Resource Actions
    // =====================================================
    var updateResourceStatus = function (resourceId, newStatus) { return __awaiter(_this, void 0, void 0, function () {
    ...
    var getResourceIcon = function (type) {
        switch (type) {
            case 'room': return <outline_1.MapPinIcon className="h-5 w-5"/>;
            case 'equipment': return <outline_1.WrenchIcon className="h-5 w-5"/>;
            case 'staff': return <outline_1.UserIcon className="h-5 w-5"/>;
            default: return <outline_1.SettingsIcon className="h-5 w-5"/>;
        }
    };
    ...
    var ResourceCard = function (_a) {
        ...
        return (<card_1.Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={function () {
      ...
      <card_1.CardContent>
        <div className="space-y-2 text-sm">
          {resource.location && (<div className="flex items-center text-gray-600">
              <outline_1.MapPinIcon className="h-4 w-4 mr-1"/>
              {resource.location}
            </div>)}
          {resource.capacity && (<div className="flex items-center text-gray-600">
              <outline_1.UserIcon className="h-4 w-4 mr-1"/>
              Capacity: {resource.capacity}
            </div>)}
          {resource.cost_per_hour && (<div className="flex items-center text-gray-600">
              <outline_1.ClockIcon className="h-4 w-4 mr-1"/>
              ${resource.cost_per_hour}/hour
            </div>)}
          {resource.next_maintenance && (<div className="flex items-center text-yellow-600">
              <outline_1.AlertTriangleIcon className="h-4 w-4 mr-1"/>
              Maintenance due: {new Date(resource.next_maintenance).toLocaleDateString()}
            </div>)}
        </div>
        
        {userRole !== 'patient' && (<div className="mt-4 flex space-x-2">
    ...
    var AllocationsList = function () { return (<div className="space-y-3">
      {allocations.length === 0 ? (<p className="text-gray-500 text-center py-4">No allocations for today</p>) : (allocations.map(function (allocation) { return (<card_1.Card key={allocation.id} className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {allocation.notes && (<div className="text-sm text-gray-500 mt-1">{allocation.notes}</div>)}
              </div>
              <badge_1.Badge className={allocation.status === 'confirmed' ? 'bg-green-100 text-green-800' :
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {userRole !== 'patient' && (<dialog_1.Dialog>
            ...
            <div>
              ...
              <select_1.Select value={filters.category} onValueChange={function (value) { return setFilters(function (prev) { return (__assign(__assign({}, prev), { category: value })); }); }}>
                ...
                <select_1.SelectContent>
                  ...
                  <select_1.SelectItem value="consultation_room">Consultation Room</select_1.SelectItem>
                  <select_1.SelectItem value="laser_equipment">Laser Equipment</select_1.SelectItem>
                  <select_1.SelectItem value="aesthetic_device">Aesthetic Device</select_1.SelectItem>
                </select_1.SelectContent>
              </select_1.Select>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {loading ? (<div className="text-center py-8">Loading resources...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {resources.map(function (resource) { return (<ResourceCard key={resource.id} resource={resource}/>); })}
                </div>)}
            </card_1.CardContent>
          </card_1.Card>
        </div>
        ...
        <div>
          <card_1.Card>
            ...
            <card_1.CardContent>
              {selectedResource ? (<tabs_1.Tabs defaultValue="allocations" className="w-full">
                  ...
                  <tabs_1.TabsContent value="details" className="mt-4">
                    <div className="space-y-3 text-sm">
                      ...
                      <div>
                        ...
                        <badge_1.Badge className={"ml-2 ".concat(getStatusColor(selectedResource.status))}>
                          {selectedResource.status}
                        </badge_1.Badge>
                      </div>
                      {selectedResource.location && (<div>
                          <strong>Location:</strong> {selectedResource.location}
                        </div>)}
                      {selectedResource.capacity && (<div>
                          <strong>Capacity:</strong> {selectedResource.capacity}
                        </div>)}
                      {selectedResource.cost_per_hour && (<div>
                          <strong>Cost per hour:</strong> ${selectedResource.cost_per_hour}
                        </div>)}
                      {selectedResource.skills && selectedResource.skills.length > 0 && (<div>
                          <strong>Skills:</strong>
                          <div className="flex flex-wrap gap-1 mt-1">
                            {selectedResource.skills.map(function (skill) { return (<badge_1.Badge key={skill} variant="secondary" className="text-xs">
                                {skill}
                              </badge_1.Badge>); })}
                          </div>
                        </div>)}
                ...
                </tabs_1.Tabs>) : (<div className="text-center py-8 text-gray-500">
