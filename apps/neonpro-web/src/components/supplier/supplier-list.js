...
function SupplierList(_a) {
    ...
    // STATE MANAGEMENT
    // ============================================================================
    var clinicId = _a.clinicId, onSupplierSelect = _a.onSupplierSelect, onSupplierCreate = _a.onSupplierCreate, onSupplierEdit = _a.onSupplierEdit, onSupplierDelete = _a.onSupplierDelete, _b = _a.selectable, selectable = _b === void 0 ? false : _b, _c = _a.compactView, compactView = _c === void 0 ? false : _c;
    var _d = (0, react_1.useState)([]), selectedSuppliers = _d[0], setSelectedSuppliers = _d[1];
    var _e = (0, react_1.useState)('name'), sortBy = _e[0], setSortBy = _e[1];
    var _f = (0, react_1.useState)('asc'), sortOrder = _f[0], setSortOrder = _f[1];
    var _g = (0, react_1.useState)('table'), viewMode = _g[0], setViewMode = _g[1];
    // Search and filtering
    var _h = (0, use_supplier_1.useSupplierSearch)(), searchTerm = _h.searchTerm, setSearchTerm = _h.setSearchTerm, selectedCategories = _h.selectedCategories, setSelectedCategories = _h.setSelectedCategories, selectedStatuses = _h.selectedStatuses, setSelectedStatuses = _h.setSelectedStatuses, selectedRiskLevels = _h.selectedRiskLevels, setSelectedRiskLevels = _h.setSelectedRiskLevels, filters = _h.filters, clearFilters = _h.clearFilters, hasActiveFilters = _h.hasActiveFilters;
    // Data fetching
    var _j = (0, use_supplier_1.useSuppliers)(clinicId, filters), suppliers = _j.suppliers, isLoading = _j.isLoading, error = _j.error, refetch = _j.refetch, createSupplier = _j.createSupplier, updateSupplier = _j.updateSupplier, deleteSupplier = _j.deleteSupplier, toggleSupplierStatus = _j.toggleSupplierStatus, isCreating = _j.isCreating, isUpdating = _j.isUpdating, isDeleting = _j.isDeleting, isTogglingStatus = _j.isTogglingStatus;
    // ============================================================================
    // COMPUTED VALUES
    // ============================================================================
    // Sorted and filtered suppliers
    var sortedSuppliers = (0, react_1.useMemo)(function () {
    ...
    var statistics = (0, react_1.useMemo)(function () {
        ...
        var active = suppliers.filter(function (s) { return s.status === supplier_1.SupplierStatus.ACTIVE; }).length;
        var averagePerformance = suppliers.reduce(function (sum, s) { return sum + (s.performance_score || 0); }, 0) / total;
        var highRisk = suppliers.filter(function (s) {
            return s.risk_level === supplier_1.RiskLevel.HIGH || s.risk_level === supplier_1.RiskLevel.CRITICAL;
        }).length;
        return { total: total, active: active, averagePerformance: averagePerformance, highRisk: highRisk };
    }, [suppliers]);
    // ============================================================================
    // EVENT HANDLERS
    // ============================================================================
    ...
    var handleSelectSupplier = function (supplierId) {
        ...
        setSelectedSuppliers(function (prev) {
            return prev.includes(supplierId)
                ? prev.filter(function (id) { return id !== supplierId; })
    ...
    var handleSelectAll = function () {
        ...
        if (selectedSuppliers.length === sortedSuppliers.length) {
            ...
            setSelectedSuppliers(sortedSuppliers.map(function (s) { return s.id; }));
        }
    };
    ...
    var renderSupplierCard = function (supplier) {
        var _a;
        return (<card_1.Card key={supplier.id} className={(0, utils_1.cn)("transition-all hover:shadow-md cursor-pointer", selectedSuppliers.includes(supplier.id) && "ring-2 ring-blue-500")} onClick={function () { return onSupplierSelect === null || onSupplierSelect === void 0 ? void 0 : onSupplierSelect(supplier); }}>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <badge_1.Badge variant="secondary" className={(0, utils_1.cn)("text-xs", getStatusColor(supplier.status))}>
                  {getStatusIcon(supplier.status)}
                  <span className="ml-1">{supplier.status}</span>
                </badge_1.Badge>
                <badge_1.Badge variant="outline" className={(0, utils_1.cn)("text-xs", getRiskColor(supplier.risk_level))}>
                  {supplier.risk_level}
                </badge_1.Badge>
              </div>
            </div>
          ...
          <dropdown_menu_1.DropdownMenu>
            ...
            <dropdown_menu_1.DropdownMenuContent align="end">
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onSupplierSelect === null || onSupplierSelect === void 0 ? void 0 : onSupplierSelect(supplier); }}>
                <lucide_react_1.Eye className="h-4 w-4 mr-2"/>
                Visualizar
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onSupplierEdit === null || onSupplierEdit === void 0 ? void 0 : onSupplierEdit(supplier); }}>
                <lucide_react_1.Edit className="h-4 w-4 mr-2"/>
                Editar
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuSeparator />
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onSupplierDelete === null || onSupplierDelete === void 0 ? void 0 : onSupplierDelete(supplier); }} className="text-red-600">
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>{((_a = supplier.primary_contact) === null || _a === void 0 ? void 0 : _a.phone) || 'N/A'}</span>
            </div>
          </div>
        </div>
      </card_1.CardContent>
    ...
    var renderSupplierRow = function (supplier) {
        var _a, _b;
        return (<table_1.TableRow key={supplier.id} className={(0, utils_1.cn)("cursor-pointer hover:bg-gray-50", selectedSuppliers.includes(supplier.id) && "bg-blue-50")} onClick={function () { return onSupplierSelect === null || onSupplierSelect === void 0 ? void 0 : onSupplierSelect(supplier); }}>
      {selectable && (<table_1.TableCell>
          <checkbox_1.Checkbox checked={selectedSuppliers.includes(supplier.id)} onCheckedChange={function () { return handleSelectSupplier(supplier.id); }} onClick={function (e) { return e.stopPropagation(); }}/>
        </table_1.TableCell>)}
        ...
        <div className="text-sm space-y-1">
          <div className="flex items-center gap-1 text-gray-600">
            ...
            <span className="truncate max-w-[120px]">
              {((_a = supplier.primary_contact) === null || _a === void 0 ? void 0 : _a.email) || 'N/A'}
            </span>
          </div>
          <div className="flex items-center gap-1 text-gray-600">
            <lucide_react_1.Phone className="h-3 w-3"/>
            <span>{((_b = supplier.primary_contact) === null || _b === void 0 ? void 0 : _b.phone) || 'N/A'}</span>
          </div>
        </div>
      </table_1.TableCell>
      ...
      <table_1.TableCell>
        <dropdown_menu_1.DropdownMenu>
          <dropdown_menu_1.DropdownMenuTrigger asChild onClick={function (e) { return e.stopPropagation(); }}>
            <button_1.Button variant="ghost" size="sm">
              <lucide_react_1.MoreVertical className="h-4 w-4"/>
            </button_1.Button>
          </dropdown_menu_1.DropdownMenuTrigger>
    ...
    if (error) {
        ...
        <card_1.CardContent className="p-6 text-center">
          ...
          <button_1.Button onClick={function () { return refetch(); }} variant="outline">
    ...
    return (<div className="space-y-6">
        ...
        <card_1.CardContent className="p-4">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="relative flex-1">
              ...
              <input_1.Input placeholder="Buscar fornecedores..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
            </div>
            
            {/* Filters */}
            <div className="flex gap-2">
              <select_1.Select value={selectedCategories[0] || 'all'} onValueChange={function (value) {
            return setSelectedCategories(value === 'all' ? [] : [value]);
              ...
              <select_1.Select value={selectedStatuses[0] || 'all'} onValueChange={function (value) {
            return setSelectedStatuses(value === 'all' ? [] : [value]);
                ...
                <select_1.SelectContent>
                  ...
                  {Object.values(supplier_1.SupplierStatus).map(function (status) { return (<select_1.SelectItem key={status} value={status}>
                      {getStatusIcon(status)} {status}
                    </select_1.SelectItem>); })}
                </select_1.SelectContent>
              </select_1.Select>
              
              <select_1.Select value={selectedRiskLevels[0] || 'all'} onValueChange={function (value) {
            return setSelectedRiskLevels(value === 'all' ? [] : [value]);
                ...
                <select_1.SelectContent>
                  <select_1.SelectItem value="all">Todos</select_1.SelectItem>
                  {Object.values(supplier_1.RiskLevel).map(function (risk) { return (<select_1.SelectItem key={risk} value={risk}>
                      {risk}
                    </select_1.SelectItem>); })}
                </select_1.SelectContent>
              </select_1.Select>
              
              {hasActiveFilters && (<button_1.Button variant="outline" onClick={clearFilters} size="sm">
                  Limpar
                </button_1.Button>)}
            </div>
            ...
            {/* Actions */}
            <div className="flex gap-2">
              <button_1.Button onClick={function () { return refetch(); }} variant="outline" size="sm">
                <lucide_react_1.RefreshCw className="h-4 w-4 mr-2"/>
                Atualizar
              </button_1.Button>
              
              {onSupplierCreate && (<button_1.Button onClick={onSupplierCreate} size="sm">
          ...
          {/* Bulk Actions */}
          {selectable && selectedSuppliers.length > 0 && (<div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-blue-900">
                  {selectedSuppliers.length} fornecedor(es) selecionado(s)
                </span>
                <div className="flex gap-2">
                  <button_1.Button size="sm" variant="outline" onClick={function () { return handleBulkAction('activate'); }}>
                    Ativar
                  </button_1.Button>
                  <button_1.Button size="sm" variant="outline" onClick={function () { return handleBulkAction('deactivate'); }}>
                    Desativar
                  </button_1.Button>
                  <button_1.Button size="sm" variant="outline" onClick={function () { return handleBulkAction('delete'); }} className="text-red-600 hover:text-red-700">
                    Excluir
                  </button_1.Button>
                </div>
              </div>
      ...
      {viewMode === 'cards' ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <table_1.Table>
            <table_1.TableHeader>
              <table_1.TableRow>
                {selectable && (<table_1.TableHead className="w-12">
                    <checkbox_1.Checkbox checked={selectedSuppliers.length === sortedSuppliers.length && sortedSuppliers.length > 0} onCheckedChange={handleSelectAll}/>
                  </table_1.TableHead>)}
                <table_1.TableHead className="cursor-pointer hover:bg-gray-50" onClick={function () { return handleSort('name'); }}>
                  <div className="flex items-center gap-1">
                    Fornecedor
                    {sortBy === 'name' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </div>
                </table_1.TableHead>
                <table_1.TableHead className="cursor-pointer hover:bg-gray-50" onClick={function () { return handleSort('status'); }}>
                  <div className="flex items-center gap-1">
                    Status
                    {sortBy === 'status' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </div>
                </table_1.TableHead>
                <table_1.TableHead className="cursor-pointer hover:bg-gray-50" onClick={function () { return handleSort('risk'); }}>
                  <div className="flex items-center gap-1">
                    Risco
                    {sortBy === 'risk' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </div>
                </table_1.TableHead>
                <table_1.TableHead className="cursor-pointer hover:bg-gray-50" onClick={function () { return handleSort('performance'); }}>
                  <div className="flex items-center gap-1">
                    Performance
      ...
      {/* Empty State */}
      {sortedSuppliers.length === 0 && !isLoading && (<card_1.Card>
          <card_1.CardContent className="p-12 text-center">
            ...
            {!hasActiveFilters && onSupplierCreate && (<button_1.Button onClick={onSupplierCreate}>
                <lucide_react_1.Plus className="h-4 w-4 mr-2"/>
