...
function LazyLoader(_a) {
    var children = _a.children, fallback = _a.fallback, errorFallback = _a.errorFallback, className = _a.className, _b = _a.minLoadingTime, minLoadingTime = _b === void 0 ? 200 : _b, _c = _a.retryable, retryable = _c === void 0 ? true : _c, onError = _a.onError, onLoad = _a.onLoad;
    ...
    var _f = (0, react_1.useState)(0), retryCount = _f[0], setRetryCount = _f[1];
    ...
    if (hasError) {
        return (<ErrorFallback error={new Error('Component failed to load')} onRetry={retryable ? handleRetry : undefined} retryCount={retryCount} customFallback={errorFallback} className={className}/>);
    }
    return (<div className={className}>
      <ErrorBoundary onError={handleError}>
        <react_1.Suspense fallback={fallback || (<LoadingFallback minLoadingTime={minLoadingTime} onLoadComplete={handleLoad}/>)}>
          {children}
        </react_1.Suspense>
      </ErrorBoundary>
    </div>);
...
function LoadingFallback(_a) {
    var _b = _a.minLoadingTime, minLoadingTime = _b === void 0 ? 200 : _b, onLoadComplete = _a.onLoadComplete, _c = _a.message, message = _c === void 0 ? 'Loading component...' : _c, _d = _a.showProgress, showProgress = _d === void 0 ? false : _d;
    ...
    return (<card_1.Card className="animate-pulse">
      <card_1.CardContent className="p-8">
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="text-center">
            ...
            {showProgress && (<div className="mt-2 w-32 h-2 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all duration-300 ease-out" style={{ width: "".concat(progress, "%") }}/>
              </div>)}
          </div>
        </div>
      </card_1.CardContent>
...
function ErrorFallback(_a) {
    var error = _a.error, onRetry = _a.onRetry, _b = _a.retryCount, retryCount = _b === void 0 ? 0 : _b, customFallback = _a.customFallback, className = _a.className;
    if (customFallback) {
        return <div className={className}>{customFallback}</div>;
    }
    return (<card_1.Card className={(0, utils_1.cn)('border-red-200 bg-red-50', className)}>
      <card_1.CardContent className="p-8">
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="text-center">
            ...
            {retryCount > 0 && (<p className="text-xs text-red-600 mb-4">
                Retry attempts: {retryCount}
              </p>)}
            {onRetry && (<button_1.Button onClick={onRetry} variant="outline" size="sm" className="border-red-300 text-red-700 hover:bg-red-100">
...
function createLazyComponent(importFn, options) {
    ...
    var fallback = options.fallback, errorFallback = options.errorFallback, _a = options.minLoadingTime, minLoadingTime = _a === void 0 ? 200 : _a, _b = options.retryable, retryable = _b === void 0 ? true : _b, _c = options.preload, preload = _c === void 0 ? false : _c, chunkName = options.chunkName;
    ...
    var EnhancedComponent = react_1.default.forwardRef(function (props, ref) {
        var _a = (0, react_1.useState)(false), shouldPreload = _a[0], setShouldPreload = _a[1];
        ...
        <LazyLoader fallback={fallback} errorFallback={errorFallback} minLoadingTime={minLoadingTime} retryable={retryable}>
    ...
    // Add preload method
    EnhancedComponent.preload = function () { return importFn(); };
    return EnhancedComponent;
}
function IntersectionLazyLoader(_a) {
    var children = _a.children, fallback = _a.fallback, _b = _a.rootMargin, rootMargin = _b === void 0 ? '50px' : _b, _c = _a.threshold, threshold = _c === void 0 ? 0.1 : _c, _d = _a.triggerOnce, triggerOnce = _d === void 0 ? true : _d, className = _a.className;
    var _e = (0, react_1.useState)(false), isVisible = _e[0], setIsVisible = _e[1];
    var _f = (0, react_1.useState)(false), hasTriggered = _f[0], setHasTriggered = _f[1];
    ...
    return (<div ref={elementRef} className={className}>
      {isVisible ? (children) : (fallback || (<div className="h-32 flex items-center justify-center">
...
function useLazyPreload(importFn) {
    var _this = this;
    var _a = (0, react_1.useState)(false), isPreloaded = _a[0], setIsPreloaded = _a[1];
    var _b = (0, react_1.useState)(false), isPreloading = _b[0], setIsPreloading = _b[1];
    var preload = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        var error_1;
...
var SkeletonCard = function () { return (<card_1.Card className="animate-pulse">
...
var SkeletonTable = function () { return (<div className="space-y-3">
    {Array.from({ length: 5 }).map(function (_, i) { return (<div key={i} className="flex space-x-4 animate-pulse">
...
var SkeletonForm = function () { return (<div className="space-y-4 animate-pulse">
    {Array.from({ length: 4 }).map(function (_, i) { return (<div key={i} className="space-y-2">
        <div className="h-4 bg-gray-200 rounded w-1/4"></div>
