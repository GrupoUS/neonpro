...
function LazyLoader(_a) {
  ...
  var handleRetry = (0, react_1.useCallback)(() => {
    setHasError(false);
    setIsLoading(true);
...
function createLazyComponent(importFn, options) {
  ...
  var LazyComponent = (0, react_1.lazy)(() => {
    // Add artificial delay for minimum loading time
    var loadPromise = importFn();
    if (minLoadingTime > 0) {
      var delayPromise = new Promise((resolve) => setTimeout(resolve, minLoadingTime));
      return Promise.all([loadPromise, delayPromise]).then((_a) => {
        var module = _a[0];
        return module;
      });
    }
  ...
  var EnhancedComponent = react_1.default.forwardRef((props, ref) => {
    var _a = (0, react_1.useState)(false),
      shouldPreload = _a[0],
      setShouldPreload = _a[1];
    var preloadTriggered = (0, react_1.useRef)(false);
    var handlePreload = (0, react_1.useCallback)(() => {
  ...
  // Add preload method
  EnhancedComponent.preload = () => importFn();
