...
import { FinancialKPIs } from '@/lib/financial';
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${className}`}>
      {kpiCards.map((kpi, index) => {
        ...
        const progressValue = kpi.target ? Math.min((Number(kpi.value) / kpi.target) * 100, 100) : undefined;
        
        return (
          <Card key={index} className={`transition-all duration-200 hover:shadow-md ${colorClasses.border}`}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
