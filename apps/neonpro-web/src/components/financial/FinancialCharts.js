...
function FinancialCharts(_a) {
    var charts = _a.charts, analytics = _a.analytics, timeframe = _a.timeframe, _b = _a.loading, loading = _b === void 0 ? false : _b, _c = _a.detailed, detailed = _c === void 0 ? false : _c, _d = _a.className, className = _d === void 0 ? '' : _d;
    var _e = (0, react_1.useState)('revenue'), selectedChart = _e[0], setSelectedChart = _e[1];
    var _f = (0, react_1.useState)('overview'), chartView = _f[0], setChartView = _f[1];
    var _g = (0, react_1.useState)('30'), dateRange = _g[0], setDateRange = _g[1];
    ...
    var generateMockData = function (type, days) {
        ...
        for (var i = days - 1; i >= 0; i--) {
            ...
            var baseValue = type === 'revenue' ? 15000 :
                type === 'patients' ? 25 :
                    type === 'treatments' ? 12 :
    ...
    var chartConfigs = (0, react_1.useMemo)(function () { return ({
        revenue: {
    ...
    var CustomTooltip = function (_a) {
        var active = _a.active, payload = _a.payload, label = _a.label, format = _a.format;
        if (active && payload && payload.length) {
            var value = payload[0].value;
            var formattedValue = format === 'currency' ? formatCurrency(value) :
                format === 'percentage' ? formatPercentage(value) :
                    value.toLocaleString('pt-BR');
            return (<div className="bg-white p-3 border rounded-lg shadow-lg">
          <p className="font-medium">{label}</p>
          <p className="text-blue-600">
            {"".concat(payload[0].name, ": ").concat(formattedValue)}
          </p>
        </div>);
        }
        return null;
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          {detailed && (<div className="flex items-center gap-2">
              <label className="text-sm font-medium">Visualização:</label>
              <select_1.Select value={chartView} onValueChange={function (value) { return setChartView(value); }}>
                <select_1.SelectTrigger className="w-32">
                  <select_1.SelectValue />
                </select_1.SelectTrigger>
                <select_1.SelectContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.AreaChart data={chartConfigs.revenue.data}>
                ...
                <recharts_1.Tooltip content={<CustomTooltip format={chartConfigs.revenue.format}/>}/>
                <recharts_1.Area type="monotone" dataKey={chartConfigs.revenue.yKey} stroke={chartConfigs.revenue.color} fill={chartConfigs.revenue.color} fillOpacity={0.3}/>
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.BarChart data={chartConfigs.patients.data}>
                ...
                <recharts_1.YAxis />
                <recharts_1.Tooltip content={<CustomTooltip format={chartConfigs.patients.format}/>}/>
                <recharts_1.Bar dataKey={chartConfigs.patients.yKey} fill={chartConfigs.patients.color}/>
              </recharts_1.BarChart>
            </recharts_1.ResponsiveContainer>
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.PieChart>
                <recharts_1.Pie data={treatmentDistribution} cx="50%" cy="50%" labelLine={false} label={function (_a) {
        var name = _a.name, value = _a.value;
        return "".concat(name, ": ").concat(value, "%");
    }} outerRadius={80} fill="#8884d8" dataKey="value">
                  {treatmentDistribution.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                </recharts_1.Pie>
                <recharts_1.Tooltip />
              </recharts_1.PieChart>
            </recharts_1.ResponsiveContainer>
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.LineChart data={chartConfigs.satisfaction.data}>
                ...
                <recharts_1.Tooltip content={<CustomTooltip format={chartConfigs.satisfaction.format}/>}/>
                ...
                <recharts_1.Line type="monotone" dataKey={chartConfigs.satisfaction.yKey} stroke={chartConfigs.satisfaction.color} strokeWidth={2} dot={{ fill: chartConfigs.satisfaction.color }}/>
      ...
      {detailed && chartView === 'detailed' && (<div className="space-y-6">
          ...
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Análise de receita por categoria de tratamento</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-4">
                {revenueByTreatment.map(function (item, index) { return (<div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className="w-4 h-4 rounded-full" style={{ backgroundColor: Object.values(colors)[index] }}/>
