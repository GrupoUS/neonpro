...
function FinancialKPICards(_a) {
    var kpis = _a.kpis, timeframe = _a.timeframe, _b = _a.loading, loading = _b === void 0 ? false : _b, _c = _a.className, className = _c === void 0 ? '' : _c;
    ...
    var kpiCards = [
        ...
        },
        {
            ...
            color: kpis.patient_satisfaction >= 4.0 ? 'green' : kpis.patient_satisfaction >= 3.0 ? 'yellow' : 'red',
            format: 'number'
        },
        {
            ...
            color: kpis.average_wait_time <= 15 ? 'green' : kpis.average_wait_time <= 30 ? 'yellow' : 'red',
            format: 'number'
        }
    ];
    // Render loading state
    if (loading) {
        return (<div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ".concat(className)}>
        {__spreadArray([], Array(8), true).map(function (_, i) { return (<card_1.Card key={i}>
    ...
    return (<div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ".concat(className)}>
      {kpiCards.map(function (kpi, index) {
            ...
            var progressValue = kpi.target ? Math.min((Number(kpi.value) / kpi.target) * 100, 100) : undefined;
            return (<card_1.Card key={index} className={"transition-all duration-200 hover:shadow-md ".concat(colorClasses.border)}>
            <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <card_1.CardContent>
              <div className="space-y-2">
                ...
                {/* Change Indicator */}
                {kpi.change !== undefined && (<div className={"flex items-center gap-1 text-xs ".concat(trendDisplay.color)}>
                    {trendDisplay.icon}
                ...
                {kpi.target && progressValue !== undefined && (<div className="space-y-1">
                ...
                {/* Status Badge */}
                {kpi.target && (<div className="flex justify-end">
                    {Number(kpi.value) >= kpi.target ? (<badge_1.Badge variant="default" className="text-xs bg-green-100 text-green-700">
                        <lucide_react_1.CheckCircle className="h-3 w-3 mr-1"/>
                        Meta atingida
                      </badge_1.Badge>) : Number(kpi.value) >= kpi.target * 0.8 ? (<badge_1.Badge variant="secondary" className="text-xs bg-yellow-100 text-yellow-700">
                        <lucide_react_1.AlertTriangle className="h-3 w-3 mr-1"/>
                        Próximo da meta
                      </badge_1.Badge>) : (<badge_1.Badge variant="destructive" className="text-xs">
                        <lucide_react_1.AlertTriangle className="h-3 w-3 mr-1"/>
                        Abaixo da meta
                      </badge_1.Badge>)}
                  </div>)}
