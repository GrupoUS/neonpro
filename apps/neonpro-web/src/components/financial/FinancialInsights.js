...
function FinancialInsights(_a) {
    var insights = _a.insights, analytics = _a.analytics, _b = _a.loading, loading = _b === void 0 ? false : _b, _c = _a.timeframe, timeframe = _c === void 0 ? 'month' : _c, _d = _a.className, className = _d === void 0 ? '' : _d;
    ...
    var filteredInsights = selectedCategory === 'all'
        ? mockInsights
        : mockInsights.filter(function (insight) { return insight.category === selectedCategory; });
    // Get priority color
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={function (value) { return setActiveTab(value); }}>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {mockInsights
            .filter(function (insight) { return insight.priority === 'critical'; })
            .map(function (insight) {
            var Icon = getInsightIcon(insight.type);
            return (<alert_1.Alert key={insight.id} className="border-red-200 bg-red-50">
                    <Icon className="h-4 w-4 text-red-600"/>
                    <alert_1.AlertTitle className="text-red-800">{insight.title}</alert_1.AlertTitle>
                    <alert_1.AlertDescription className="text-red-700">
                      {insight.description}
                      {insight.actionable && insight.actions && (<div className="mt-2">
                          <strong>Ações recomendadas:</strong>
                          <ul className="list-disc list-inside mt-1">
                            {insight.actions.slice(0, 2).map(function (action, index) { return (<li key={index} className="text-sm">{action}</li>); })}
                          </ul>
                        </div>)}
          ...
          <div className="space-y-4">
            ...
            .map(function (insight) {
            var Icon = getInsightIcon(insight.type);
            return (<card_1.Card key={insight.id} className="hover:shadow-md transition-shadow">
                      <card_1.CardHeader>
        ...
        <tabs_1.TabsContent value="trends" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            var _a = getTrendDisplay(trend.direction, trend.change), TrendIcon = _a.icon, color = _a.color, prefix = _a.prefix;
            return (<card_1.Card key={trend.id}>
                  <card_1.CardHeader>
                    <card_1.CardTitle className="flex items-center justify-between">
                      <span>{trend.metric}</span>
                      <badge_1.Badge variant={trend.significance === 'high' ? 'default' : 'outline'}>
                        {trend.significance}
                      </badge_1.Badge>
                    </card_1.CardTitle>
                  </card_1.CardHeader>
                  <card_1.CardContent>
                    <div className="space-y-4">
                      ...
                      {trend.forecast && (<div className="border-t pt-3">
        ...
        <tabs_1.TabsContent value="recommendations" className="space-y-6">
          <div className="space-y-6">
            {mockRecommendations.map(function (rec) { return (<card_1.Card key={rec.id}>
                <card_1.CardHeader>
                  <div className="flex items-start justify-between">
                    <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                        {rec.steps.map(function (step, index) { return (<li key={index}>{step}</li>); })}
                      </ol>
                    </div>
        ...
        <tabs_1.TabsContent value="alerts" className="space-y-6">
          <div className="space-y-4">
            {mockInsights
            .filter(function (insight) { return insight.type === 'alert' || insight.priority === 'critical'; })
            .map(function (insight) {
            var Icon = getInsightIcon(insight.type);
            return (<alert_1.Alert key={insight.id} className={"border-l-4 ".concat(insight.priority === 'critical' ? 'border-l-red-500 bg-red-50' :
                    ...
                    <alert_1.AlertDescription>
                      <div className="space-y-3">
                        ...
                        {insight.metrics && (<div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        {insight.actionable && insight.actions && (<div>
                            <div className="font-medium mb-2">Ações recomendadas:</div>
                            <ul className="list-disc list-inside space-y-1">
                              {insight.actions.map(function (action, index) { return (<li key={index} className="text-sm">{action}</li>); })}
                            </ul>
                          </div>)}
