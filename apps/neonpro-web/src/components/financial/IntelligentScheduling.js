...
function IntelligentScheduling(_a) {
    var _this = this;
    var patientId = _a.patientId, serviceId = _a.serviceId, onAppointmentScheduled = _a.onAppointmentScheduled;
    var _b = (0, react_1.useState)(), selectedDate = _b[0], setSelectedDate = _b[1];
    var _c = (0, react_1.useState)(null), selectedPatient = _c[0], setSelectedPatient = _c[1];
    var _d = (0, react_1.useState)(null), selectedProfessional = _d[0], setSelectedProfessional = _d[1];
    var _e = (0, react_1.useState)(null), selectedService = _e[0], setSelectedService = _e[1];
    var _f = (0, react_1.useState)([]), availableSlots = _f[0], setAvailableSlots = _f[1];
    ...
    var _j = (0, react_1.useState)([]), aiRecommendations = _j[0], setAiRecommendations = _j[1];
    var _k = (0, react_1.useState)([]), conflicts = _k[0], setConflicts = _k[1];
    ...
    var analyzeOptimalSlots = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    scoredSlots = slots.map(function (slot) { return (__assign(__assign({}, slot), { aiScore: calculateAIScore(slot, selectedPatient, selectedProfessional, selectedService), reasons: generateReasons(slot, selectedPatient, selectedProfessional) })); });
                    availableScored = scoredSlots
                        .filter(function (slot) { return slot.available; })
                        .sort(function (a, b) { return (b.aiScore || 0) - (a.aiScore || 0); });
    ...
    var generateTimeSlots = function (date, professional, service) {
        ...
        var _a = professional.workingHours.start.split(':').map(Number), startHour = _a[0], startMinute = _a[1];
        var _b = professional.workingHours.end.split(':').map(Number), endHour = _b[0], endMinute = _b[1];
    ...
    var detectConflicts = function () { return __awaiter(_this, void 0, void 0, function () {
        var detected, resourceConflicts, professionalConflicts, patientConflicts;
        return __generator(this, function (_a) {
            if (!selectedSlot || !selectedProfessional || !selectedService)
                return [2 /*return*/];
            detected = [];
    ...
    var scheduleAppointment = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 3:
                    ...
                    onAppointmentScheduled === null || onAppointmentScheduled === void 0 ? void 0 : onAppointmentScheduled(appointmentId);
                    return [3 /*break*/, 5];
                case 4:
                    error_2 = _a.sent();
                    toast({
        ...
        <card_1.CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select_1.Select onValueChange={function (value) {
            var professional = professionals.find(function (p) { return p.id === value; });
                ...
                <select_1.SelectContent>
                  {professionals.map(function (prof) { return (<select_1.SelectItem key={prof.id} value={prof.id}>
                      <div className="flex items-center gap-2">
                        {prof.name}
                        <badge_1.Badge variant={prof.currentLoad > 80 ? 'destructive' : 'secondary'}>
                          {prof.currentLoad}%
                        </badge_1.Badge>
                      </div>
                    </select_1.SelectItem>); })}
            ...
            <popover_1.Popover>
              <popover_1.PopoverTrigger asChild>
                <button_1.Button variant="outline" className="w-full justify-start text-left">
                  ...
                  {selectedDate ? (0, date_fns_1.format)(selectedDate, "PPP", { locale: locale_1.pt }) : "Selecionar data"}
                </button_1.Button>
              </popover_1.PopoverTrigger>
              <popover_1.PopoverContent className="w-auto p-0">
                <calendar_1.Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} disabled={function (date) { return date < new Date(); }} initialFocus/>
              </popover_1.PopoverContent>
            </popover_1.Popover>
          </div>

          {/* Analysis Button */}
          <button_1.Button onClick={analyzeOptimalSlots} disabled={!selectedDate || !selectedProfessional || !selectedService || isAnalyzing} className="w-full">
            <lucide_react_1.Brain className="mr-2 h-4 w-4"/>
            {isAnalyzing ? 'Analisando Horários...' : 'Analisar Horários Disponíveis'}
          </button_1.Button>

          {/* AI Recommendations */}
          {aiRecommendations.length > 0 && (<div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                var _a;
                return (<card_1.Card key={slot.id} className={"cursor-pointer transition-colors ".concat((selectedSlot === null || selectedSlot === void 0 ? void 0 : selectedSlot.id) === slot.id ? 'ring-2 ring-blue-500' : '')} onClick={function () { return setSelectedSlot(slot); }}>
                    <card_1.CardContent className="p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <lucide_react_1.Clock className="h-4 w-4"/>
                          <span className="font-medium">
                            {(0, date_fns_1.format)(slot.start, 'HH:mm')} - {(0, date_fns_1.format)(slot.end, 'HH:mm')}
                          </span>
                        </div>
                      ...
                      <div className="space-y-1">
                        {(_a = slot.reasons) === null || _a === void 0 ? void 0 : _a.map(function (reason, index) { return (<p key={index} className="text-xs text-gray-600">
                            • {reason}
                          </p>); })}
                      </div>
                    </card_1.CardContent>
          ...
          {availableSlots.length > 0 && (<div className="space-y-4">
              <label_1.Label>Todos os Horários Disponíveis</label_1.Label>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                {availableSlots.map(function (slot) { return (<button_1.Button key={slot.id} variant={(selectedSlot === null || selectedSlot === void 0 ? void 0 : selectedSlot.id) === slot.id ? "default" : "outline"} size="sm" onClick={function () { return setSelectedSlot(slot); }} className="h-auto p-2 flex flex-col items-center">
                    <span className="text-sm font-medium">
                      {(0, date_fns_1.format)(slot.start, 'HH:mm')}
                    </span>
                    {slot.aiScore && (<badge_1.Badge variant="secondary" className="text-xs mt-1">
                        {slot.aiScore}%
                      </badge_1.Badge>)}
                  </button_1.Button>); })}
              </div>
          ...
          {/* Conflicts Alert */}
          {conflicts.length > 0 && (<alert_1.Alert>
              <lucide_react_1.AlertTriangle className="h-4 w-4"/>
              <alert_1.AlertTitle>Conflitos Detectados</alert_1.AlertTitle>
              <alert_1.AlertDescription>
                <ul className="list-disc list-inside space-y-1 mt-2">
                  {conflicts.map(function (conflict, index) { return (<li key={index}>{conflict}</li>); })}
                </ul>
              </alert_1.AlertDescription>
            </alert_1.Alert>)}

          {/* Schedule Button */}
          <button_1.Button onClick={scheduleAppointment} disabled={!selectedSlot || conflicts.length > 0} className="w-full" size="lg">
