...
function FinancialDashboard(_a) {
    var _this = this;
    var clinicId = _a.clinicId, supabaseUrl = _a.supabaseUrl, supabaseKey = _a.supabaseKey, _b = _a.className, className = _b === void 0 ? '' : _b;
    // State Management
    var _c = (0, react_1.useState)(null), financialSystem = _c[0], setFinancialSystem = _c[1];
    ...
    var _k = (0, react_1.useState)('month'), timeframe = _k[0], setTimeframe = _k[1];
    var _l = (0, react_1.useState)(true), autoRefresh = _l[0], setAutoRefresh = _l[1];
    var _m = (0, react_1.useState)(300000), refreshInterval = _m[0], setRefreshInterval = _m[1]; // 5 minutes
    // Initialize Financial System
    var initializeSystem = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    setLoading(true);
                    setError(null);
                    config = (0, financial_1.getDefaultFinancialConfig)(clinicId, supabaseUrl, supabaseKey);
    ...
    // Load Dashboard Data
    var loadDashboardData = (0, react_1.useCallback)(function (system) { return __awaiter(_this, void 0, void 0, function () {
    ...
    // Check System Health
    var checkSystemHealth = (0, react_1.useCallback)(function (system) { return __awaiter(_this, void 0, void 0, function () {
    ...
    (0, react_1.useEffect)(function () {
        if (!autoRefresh || !financialSystem)
            return;
    ...
    var getHealthIcon = function (status) {
        switch (status) {
            case 'healthy': return <lucide_react_1.CheckCircle className="h-4 w-4"/>;
            case 'degraded': return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
            case 'unhealthy': return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {systemHealth && (<badge_1.Badge variant={systemHealth.status === 'healthy' ? 'default' : 'destructive'} className={"".concat(getHealthColor(systemHealth.status), " flex items-center gap-1")}>
              {getHealthIcon(systemHealth.status)}
              {systemHealth.status} ({systemHealth.overall_score.toFixed(0)}%)
            </badge_1.Badge>)}
          
          {/* Last Updated */}
          {dashboardData && (<div className="flex items-center gap-1 text-sm text-muted-foreground">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select_1.Select value={timeframe} onValueChange={function (value) { return setTimeframe(value); }}>
          ...
          {/* Auto Refresh Toggle */}
          <div className="flex items-center gap-2">
            <button_1.Button onClick={function () { return setAutoRefresh(!autoRefresh); }} variant={autoRefresh ? 'default' : 'outline'} size="sm">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
          {/* KPI Cards */}
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.kpis) && (<FinancialKPICards_1.FinancialKPICards kpis={dashboardData.kpis} timeframe={timeframe} loading={refreshing}/>)}
          
          {/* Charts Grid */}
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.charts) && (<FinancialCharts_1.FinancialCharts charts={dashboardData.charts} timeframe={timeframe} loading={refreshing}/>)}
          
          {/* Quick Insights */}
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.insights) && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <card_1.Card>
                <card_1.CardHeader>
                ...
                <card_1.CardContent>
                  <div className="space-y-3">
                    {dashboardData.insights.slice(0, 3).map(function (insight, index) { return (<div key={index} className="flex items-start gap-3 p-3 rounded-lg border">
                        <div className={"p-1 rounded-full ".concat(insight.type === 'positive' ? 'bg-green-100 text-green-600' :
                    insight.type === 'warning' ? 'bg-yellow-100 text-yellow-600' :
                        'bg-red-100 text-red-600')}>
                          {insight.type === 'positive' ? <lucide_react_1.TrendingUp className="h-4 w-4"/> :
                    insight.type === 'warning' ? <lucide_react_1.AlertTriangle className="h-4 w-4"/> :
                        <lucide_react_1.TrendingDown className="h-4 w-4"/>}
                        </div>
                        <div className="flex-1">
                          <h4 className="font-medium">{insight.title}</h4>
                          <p className="text-sm text-muted-foreground">{insight.description}</p>
                          {insight.recommendation && (<p className="text-xs text-blue-600 mt-1">{insight.recommendation}</p>)}
                        </div>
                      </div>); })}
                  </div>
                </card_1.CardContent>
              ...
              <card_1.Card>
                ...
                <card_1.CardContent>
                  {systemHealth && (<div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Overall Health</span>
                        <badge_1.Badge variant={systemHealth.status === 'healthy' ? 'default' : 'destructive'}>
                          {systemHealth.status}
                        </badge_1.Badge>
                      </div>
                    ...
                    var module = _a[0], status = _a[1];
                    return (<div key={module} className="flex items-center justify-between text-sm">
                            <span className="capitalize">{module.replace('_', ' ')}</span>
                            <div className={"flex items-center gap-1 ".concat(getHealthColor(status.status))}>
        ...
        <tabs_1.TabsContent value="analytics" className="space-y-6">
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.analytics) && (<FinancialCharts_1.FinancialCharts charts={dashboardData.charts} analytics={dashboardData.analytics} timeframe={timeframe} loading={refreshing} detailed={true}/>)}
        </tabs_1.TabsContent>

        {/* Reports Tab */}
        <tabs_1.TabsContent value="reports" className="space-y-6">
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.reports) && financialSystem && (<FinancialReports_1.FinancialReports reports={dashboardData.reports} financialSystem={financialSystem} timeframe={timeframe} loading={refreshing}/>)}
        </tabs_1.TabsContent>

        {/* Insights Tab */}
        <tabs_1.TabsContent value="insights" className="space-y-6">
          {(dashboardData === null || dashboardData === void 0 ? void 0 : dashboardData.insights) && (<FinancialInsights_1.FinancialInsights insights={dashboardData.insights} analytics={dashboardData.analytics} kpis={dashboardData.kpis} timeframe={timeframe} loading={refreshing}/>)}
        </tabs_1.TabsContent>

        {/* Settings Tab */}
        <tabs_1.TabsContent value="settings" className="space-y-6">
          {financialSystem && (<FinancialSettings_1.FinancialSettings financialSystem={financialSystem} autoRefresh={autoRefresh} refreshInterval={refreshInterval} onAutoRefreshChange={setAutoRefresh} onRefreshIntervalChange={setRefreshInterval} onSystemReset={initializeSystem}/>)}
        </tabs_1.TabsContent>
      </tabs_1.Tabs>
    </div>);
}
