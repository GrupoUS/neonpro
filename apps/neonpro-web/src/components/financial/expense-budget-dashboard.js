...
function ExpenseBudgetDashboard() {
    ...
    var _b = (0, react_1.useState)('current_month'), period = _b[0], setPeriod = _b[1];
    var _c = (0, react_1.useState)([]), categories = _c[0], setCategories = _c[1];
    var _d = (0, react_1.useState)([]), alerts = _d[0], setAlerts = _d[1];
    var _e = (0, react_1.useState)([]), vendors = _e[0], setVendors = _e[1];
    var _f = (0, react_1.useState)([]), trends = _f[0], setTrends = _f[1];
    var _g = (0, react_1.useState)([]), insights = _g[0], setInsights = _g[1];
    var _h = (0, react_1.useState)([]), costCenters = _h[0], setCostCenters = _h[1];
    var expenseEngine = new expense_budget_engine_1.ExpenseBudgetEngine();
    (0, react_1.useEffect)(function () {
        loadExpenseData();
    }, [period]);
    var loadExpenseData = function () { return __awaiter(_this, void 0, void 0, function () {
        var clinicId, dateRange, _a, budgetReport, vendorData, trendData, optimizationInsights, costCenterData, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    _a = _b.sent(), budgetReport = _a[0], vendorData = _a[1], trendData = _a[2], optimizationInsights = _a[3], costCenterData = _a[4];
    ...
    var totalBudget = categories.reduce(function (sum, cat) { return sum + cat.budgetAllocation; }, 0);
    var totalActual = categories.reduce(function (sum, cat) { return sum + cat.actualSpending; }, 0);
    ...
    return (<div className="space-y-6">
      ...
      {/* Budget Alerts */}
      {alerts.filter(function (alert) { return !alert.isResolved; }).length > 0 && (<div className="space-y-2">
          {alerts.filter(function (alert) { return !alert.isResolved; }).slice(0, 3).map(function (alert) { return (<alert_1.Alert key={alert.alertId} className={getAlertColor(alert.alertType)}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className={"text-2xl font-bold ".concat(totalVariance > 0 ? 'text-red-600' : 'text-green-600')}>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">{alerts.filter(function (alert) { return !alert.isResolved; }).length}</div>
            <p className="text-xs text-muted-foreground">
              Requerem atenção
            </p>
          </card_1.CardContent>
      ...
      <tabs_1.Tabs defaultValue="budget" className="space-y-4">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {categories.map(function (category) { return (<div key={category.categoryId} className="space-y-2 p-4 border rounded-lg">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold">{category.categoryName}</h4>
                      <div className="flex items-center space-x-2">
                        {category.isOverBudget && (<badge_1.Badge variant="destructive">Excedido</badge_1.Badge>)}
                        <badge_1.Badge variant={Math.abs(category.variancePercent) < 5 ? 'default' :
                Math.abs(category.variancePercent) < 10 ? 'secondary' : 'destructive'}>
                    ...
                    <div className="space-y-2">
                      ...
                      <progress_1.Progress value={Math.min(100, (category.actualSpending / category.budgetAllocation) * 100)} className="h-2"/>
        ...
        <tabs_1.TabsContent value="vendors" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {vendors.map(function (vendor) { return (<div key={vendor.vendorId} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold">{vendor.vendorName}</h4>
                        <div className="flex items-center space-x-2">
                          <badge_1.Badge variant="outline">{vendor.category}</badge_1.Badge>
                          <badge_1.Badge variant={vendor.performanceScore > 8 ? 'default' :
        ...
        <tabs_1.TabsContent value="trends" className="space-y-4">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Análise temporal e identificação de padrões</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-4">
                {trends.map(function (trend, index) { return (<div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold">{trend.period}</h4>
                        <badge_1.Badge variant={trend.growthRate > 0 ? 'destructive' : 'default'}>
                          {trend.growthRate > 0 ? <lucide_react_1.TrendingUp className="h-3 w-3 mr-1"/> : <lucide_react_1.TrendingDown className="h-3 w-3 mr-1"/>}
        ...
        <tabs_1.TabsContent value="optimization" className="space-y-4">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Insights para redução de custos e melhoria de eficiência</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-4">
                {insights.map(function (insight, index) { return (<div key={index} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                ...
                insight.priority === 'medium' ? 'default' : 'secondary'}>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {insight.recommendations.map(function (rec, recIndex) { return (<badge_1.Badge key={recIndex} variant="outline" className="text-xs">
                            {rec}
                          </badge_1.Badge>); })}
                      </div>
                    </div>
        ...
        <tabs_1.TabsContent value="centers" className="space-y-4">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Alocação departamental e análise de eficiência</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-4">
                {costCenters.map(function (center) { return (<div key={center.costCenterId} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="grid grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <progress_1.Progress value={center.utilizationRate} className="flex-1"/>
