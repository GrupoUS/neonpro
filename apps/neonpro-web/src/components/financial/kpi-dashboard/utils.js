...
exports.kpiUtils = exports.generateDrillDownHierarchy = exports.calculateCorrelation = exports.detectAnomalies = exports.calculateStatistics = exports.generateMockTimeSeries = exports.filterAlertsBySeverity = exports.sortKPIsByPriority = exports.getComparisonPeriod = exports.getDateRangeDuration = exports.validateDateRange = exports.getDateRangePresets = exports.calculateCAGR = exports.calculateGrowthRate = exports.calculateMovingAverage = exports.getTrendColor = exports.getStatusBgColor = exports.getStatusColor = exports.determineKPIStatus = exports.calculatePercentageChange = exports.calculateTrend = exports.formatKPIValue = exports.formatRelativeDate = exports.formatDate = exports.formatCompactNumber = exports.formatNumber = exports.formatPercentage = exports.formatCurrency = void 0;
...
var formatRelativeDate = function (date) {
    ...
    if (diffInDays === 1)
        return 'Ontem';
    if (diffInDays < 7)
        return "".concat(diffInDays, " dias atr\u00E1s");
    if (diffInDays < 30)
        return "".concat(Math.floor(diffInDays / 7), " semanas atr\u00E1s");
    if (diffInDays < 365)
        return "".concat(Math.floor(diffInDays / 30), " meses atr\u00E1s");
...
var calculateMovingAverage = function (data, window) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var average = subset.reduce(function (sum, val) { return sum + val; }, 0) / subset.length;
        result.push(average);
    }
    return result;
};
...
var filterAlertsBySeverity = function (alerts, minSeverity) {
    ...
    return alerts.filter(function (alert) { return severityOrder[alert.severity] >= minLevel; });
};
...
var generateMockTimeSeries = function (baseValue, days, volatility, trend) {
    if (volatility === void 0) { volatility = 0.1; }
    if (trend === void 0) { trend = 0; }
    var data = [];
    var now = new Date();
...
var generateDrillDownHierarchy = function (data) {
    ...
    return Object.entries(hierarchy).map(function (_a) {
        ...
        return ({
            ...
            label: category,
            value: items.reduce(function (sum, item) { return sum + item.value; }, 0),
            change: items.reduce(function (sum, item) { return sum + item.change; }, 0) / items.length,
            trend: 'stable',
            category: category,
            details: {
                itemCount: items.length,
                avgValue: items.reduce(function (sum, item) { return sum + item.value; }, 0) / items.length
            },
            children: items
        });
    });
