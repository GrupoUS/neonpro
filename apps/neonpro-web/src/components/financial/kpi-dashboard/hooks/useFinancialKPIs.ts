...
export function useFinancialKPIs({
  ...
  const generateKPIs = useCallback(
    async (currentMetrics: KPIMetrics, previousMetrics: KPIMetrics): Promise<FinancialKPI[]> => {
  ...
  const getBenchmarkComparison = useCallback(() => {
    ...
    return {
      ...
      aboveIndustryAverage: benchmarkedKPIs.filter(
        (kpi) => kpi.value > (kpi.benchmarkData?.industryAverage || 0),
      ).length,
