...
var getDateRange = function (period) {
    var now = new Date();
    var start;
    switch (period) {
        case 'today':
...
function CashFlowDashboard(_a) {
    ...
    var _b = (0, react_1.useState)('today'), selectedPeriod = _b[0], setSelectedPeriod = _b[1];
    var _c = (0, react_1.useState)(), selectedRegisterId = _c[0], setSelectedRegisterId = _c[1];
    var _d = (0, react_1.useState)(false), showNewTransaction = _d[0], setShowNewTransaction = _d[1];
    var _e = (0, react_1.useState)('overview'), activeTab = _e[0], setActiveTab = _e[1];
    // Get date range for current period
    var _f = getDateRange(selectedPeriod), start = _f.start, end = _f.end;
    ...
    var _g = (0, useCashFlow_1.useCashRegisters)(clinicId), registers = _g.registers, registersLoading = _g.loading, refetchRegisters = _g.refetch;
    var _h = (0, useCashFlow_1.useCashFlowEntries)(clinicId, filters), entries = _h.entries, entriesLoading = _h.loading, refetchEntries = _h.refetch;
    var _j = (0, useCashFlow_1.useCashFlowAnalytics)(clinicId, filters), analytics = _j.analytics, analyticsLoading = _j.loading, refetchAnalytics = _j.refetch;
    ...
    var totalBalance = registers.reduce(function (sum, register) { return sum + register.current_balance; }, 0);
    var activeRegisters = registers.filter(function (r) { return r.is_active; }).length;
    var lowBalanceRegisters = registers.filter(function (r) { return r.is_active && r.current_balance < 500; }).length;
    // Recent transactions for overview
    var recentTransactions = entries.slice(0, 10).map(function (t) {
        var _a;
        return (__assign(__assign({}, t), { registerName: ((_a = registers.find(function (r) { return r.id === t.register_id; })) === null || _a === void 0 ? void 0 : _a.register_name) || 'Desconhecido' }));
    });
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button_1.Button size="sm" onClick={function () { return setShowNewTransaction(true); }}>
      ...
      <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          ...
          <select_1.Select value={selectedRegisterId || 'all'} onValueChange={function (value) { return setSelectedRegisterId(value === 'all' ? undefined : value); }}>
            ...
            <select_1.SelectContent>
              ...
              {registers.map(function (register) { return (<select_1.SelectItem key={register.id} value={register.id}>
                  {register.register_name} - {(0, calculations_1.formatCurrency)(register.current_balance)}
                </select_1.SelectItem>); })}
            </select_1.SelectContent>
          </select_1.Select>
        </div>

        {lowBalanceRegisters > 0 && (<badge_1.Badge variant="outline" className="text-yellow-600 border-yellow-200">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            {lowBalanceRegisters > 0 && (<div className="mt-2">
                <progress_1.Progress value={(activeRegisters - lowBalanceRegisters) / activeRegisters * 100} className="h-1"/>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {entries.filter(function (e) { return e.transaction_type === 'receipt'; }).length} transações
            </p>
            {analytics && analytics.totalIncome > 0 && (<p className="text-xs text-green-600">
                Média: {(0, calculations_1.formatCurrency)(analytics.totalIncome / Math.max(1, entries.filter(function (e) { return e.transaction_type === 'receipt'; }).length))}
              </p>)}
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {entries.filter(function (e) { return e.transaction_type === 'payment'; }).length} transações
            </p>
            {analytics && analytics.totalExpenses > 0 && (<p className="text-xs text-red-600">
                Média: {(0, calculations_1.formatCurrency)(analytics.totalExpenses / Math.max(1, entries.filter(function (e) { return e.transaction_type === 'payment'; }).length))}
              </p>)}
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className={"text-2xl font-bold ".concat(analytics && analytics.netCashFlow >= 0 ? 'text-green-600' : 'text-red-600')}>
            ...
            <div className="flex items-center mt-2">
              {analytics && analytics.netCashFlow >= 0 ? (<lucide_react_1.CheckCircle className="h-3 w-3 text-green-600 mr-1"/>) : (<lucide_react_1.AlertTriangle className="h-3 w-3 text-red-600 mr-1"/>)}
              <span className={"text-xs ".concat(analytics && analytics.netCashFlow >= 0 ? 'text-green-600' : 'text-red-600')}>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {recentTransactions.map(function (transaction) { return (<div key={transaction.id} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={"h-2 w-2 rounded-full ".concat(transaction.transaction_type === 'receipt' ? 'bg-green-500' : 'bg-red-500')}/>
                        <div>
                      ...
                      <div className={"text-sm font-medium ".concat(transaction.transaction_type === 'receipt' ? 'text-green-600' : 'text-red-600')}>
                        {transaction.transaction_type === 'receipt' ? '+' : '-'}
                        {(0, calculations_1.formatCurrency)(transaction.amount)}
                      </div>
                    </div>); })}
                  {recentTransactions.length === 0 && (<div className="text-center py-6 text-muted-foreground">
                      <div className="mb-2">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {registers.map(function (register) { return (<div key={register.id} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={"h-2 w-2 rounded-full ".concat(register.is_active ? 'bg-green-500' : 'bg-gray-400')}/>
                        <div>
                  ...
                  {registers.length === 0 && (<div className="text-center py-6 text-muted-foreground">
                      <div className="mb-2">
      ...
      {showNewTransaction && (<transaction_entry_form_1.TransactionEntryForm clinicId={clinicId} userId={userId} registers={registers} onClose={function () { return setShowNewTransaction(false); }} onSuccess={function () {
