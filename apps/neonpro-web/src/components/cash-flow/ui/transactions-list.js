...
function TransactionsList(_a) {
    var clinicId = _a.clinicId, initialFilters = _a.initialFilters;
    var _b = (0, react_1.useState)(''), searchTerm = _b[0], setSearchTerm = _b[1];
    var _c = (0, react_1.useState)(initialFilters || {}), filters = _c[0], setFilters = _c[1];
    var registers = (0, use_cash_registers_1.useCashRegisters)(clinicId).registers;
    var _d = (0, use_cash_flow_1.useCashFlow)(clinicId, __assign(__assign({}, filters), { search: searchTerm })), entries = _d.entries, loading = _d.loading, totalPages = _d.totalPages, currentPage = _d.currentPage, loadEntries = _d.loadEntries, updateEntry = _d.updateEntry, deleteEntry = _d.deleteEntry;
    ...
    var getRegisterName = function (registerId) {
        var register = registers.find(function (r) { return r.id === registerId; });
    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent>
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input_1.Input placeholder="Buscar transações..." value={searchTerm} onChange={function (e) { return handleSearch(e.target.value); }} className="pl-10"/>
            </div>
          ...
          <div className="flex gap-2">
            <select_1.Select value={filters.transactionType || ''} onValueChange={function (value) { return handleFilterChange('transactionType', value || undefined); }}>
            ...
            <select_1.Select value={filters.paymentMethod || ''} onValueChange={function (value) { return handleFilterChange('paymentMethod', value || undefined); }}>
              <select_1.SelectTrigger className="w-[180px]">
            ...
            <select_1.Select value={filters.registerId || ''} onValueChange={function (value) { return handleFilterChange('registerId', value || undefined); }}>
              <select_1.SelectTrigger className="w-[180px]">
            ...
            <table className="w-full">
              ...
              <tbody>
                {loading ? (<tr>
                  ...
                  </tr>) : entries.length === 0 ? (<tr>
                    <td colSpan={9} className="text-center py-8 text-muted-foreground">
                      Nenhuma transação encontrada
                    </td>
                  </tr>) : (entries.map(function (entry) { return (<TransactionRow key={entry.id} entry={entry} registerName={getRegisterName(entry.register_id || '')} onUpdate={updateEntry} onDelete={deleteEntry}/>); }))}
              </tbody>
            </table>
          </div>
        </div>
        ...
        {totalPages > 1 && (<div className="flex items-center justify-between mt-4">
            ...
            <div className="flex gap-2">
              <button_1.Button variant="outline" size="sm" disabled={currentPage === 1} onClick={function () { return loadEntries(filters, currentPage - 1); }}>
                Anterior
              </button_1.Button>
              <button_1.Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={function () { return loadEntries(filters, currentPage + 1); }}>
                Próxima
              </button_1.Button>
            </div>
          </div>)}
...
function TransactionRow(_a) {
    var entry = _a.entry, registerName = _a.registerName, onUpdate = _a.onUpdate, onDelete = _a.onDelete;
    return (<tr className="border-b hover:bg-muted/50">
      <td className="p-4">
        <div className="text-sm">
          {(0, calculations_1.formatDateTime)(entry.created_at)}
        </div>
      </td>
      <td className="p-4">
        <badge_1.Badge variant="outline" className={(0, calculations_1.getTransactionTypeColor)(entry.transaction_type)}>
      ...
      <td className="p-4">
        ...
        {entry.reference_number && (<div className="text-xs text-muted-foreground">
      ...
      <td className="p-4">
        <span className={"font-medium ".concat(['receipt', 'opening_balance'].includes(entry.transaction_type)
            ? 'text-green-600'
            : 'text-red-600')}>
      ...
      <td className="p-4">
        <div className="flex items-center gap-2">
          <span className="text-lg">{(0, calculations_1.getPaymentMethodIcon)(entry.payment_method)}</span>
          <span className="text-sm">{(0, calculations_1.getPaymentMethodDisplayName)(entry.payment_method)}</span>
        </div>
      </td>
      <td className="p-4">
