...
var SessionAnalyticsComponent = function (_a) {
    var _b, _c, _d;
    var userId = _a.userId, _e = _a.className, className = _e === void 0 ? '' : _e;
    var _f = (0, react_1.useState)('7d'), timeframe = _f[0], setTimeframe = _f[1];
    var _g = (0, react_1.useState)(0), refreshKey = _g[0], setRefreshKey = _g[1];
    var _h = (0, use_session_1.useSessionAnalytics)(userId, timeframe), analytics = _h.analytics, loading = _h.loading, error = _h.error, refreshAnalytics = _h.refreshAnalytics;
          ...
          <card_1.CardContent>
            <div className="animate-pulse space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map(function (i) { return (<div key={i} className="h-24 bg-muted rounded"/>); })}
              </div>
    ...
    var deviceData = analytics.deviceBreakdown ? Object.entries(analytics.deviceBreakdown).map(function (_a) {
    ...
    var securityData = analytics.securityEvents ? Object.entries(analytics.securityEvents).map(function (_a) {
        var type = _a[0], count = _a[1];
        return ({
            name: type.replace('_', ' '),
            value: count
    ...
    return (<div className={"space-y-6 ".concat(className)}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <p className={"text-2xl font-bold ".concat(getHealthScoreColor(analytics.healthScore || 0))}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            {deviceData.length > 0 ? (<recharts_1.ResponsiveContainer width="100%" height={300}>
                <recharts_1.PieChart>
                  <recharts_1.Pie data={deviceData} cx="50%" cy="50%" labelLine={false} label={function (_a) {
            var name = _a.name, percent = _a.percent;
            return "".concat(name, " ").concat((percent * 100).toFixed(0), "%");
        }} outerRadius={80} fill="#8884d8" dataKey="value">
                    {deviceData.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={COLORS[index % COLORS.length]}/>); })}
                  </recharts_1.Pie>
                  <recharts_1.Tooltip />
                </recharts_1.PieChart>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            {securityData.length > 0 ? (<recharts_1.ResponsiveContainer width="100%" height={300}>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <badge_1.Badge variant="destructive">
                  {((_b = analytics.securityEvents) === null || _b === void 0 ? void 0 : _b.FAILED_LOGIN) || 0}
                </badge_1.Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Suspicious Activity</span>
                <badge_1.Badge variant="secondary">
                  {((_c = analytics.securityEvents) === null || _c === void 0 ? void 0 : _c.SUSPICIOUS_ACTIVITY) || 0}
                </badge_1.Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">IP Changes</span>
                <badge_1.Badge variant="outline">
                  {((_d = analytics.securityEvents) === null || _d === void 0 ? void 0 : _d.IP_CHANGE) || 0}
                </badge_1.Badge>
              </div>
            </div>
          </card_1.CardContent>
