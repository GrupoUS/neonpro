    ...
    return (<sidebar_1.Sidebar variant="inset" {...props}>
      ...
      <sidebar_1.SidebarContent>
        ...
        <sidebar_1.SidebarGroup>
          ...
          <sidebar_1.SidebarGroupContent>
            ...
            return (<sidebar_1.SidebarMenuItem key={item.title}>
                  <sidebar_1.SidebarMenuButton asChild isActive={pathname === item.url} tooltip={item.title}>
                    <link_1.default href={item.url}>
                      <item.icon />
                      <span>{item.title}</span>
                    </link_1.default>
                  </sidebar_1.SidebarMenuButton>
                  {((_a = item.items) === null || _a === void 0 ? void 0 : _a.length) ? (<sidebar_1.SidebarMenuSub>
                      {item.items.map(function (subItem) { return (<sidebar_1.SidebarMenuSubItem key={subItem.title}>
                          <sidebar_1.SidebarMenuSubButton asChild isActive={pathname === subItem.url}>
                            <link_1.default href={subItem.url}>
                              <span>{subItem.title}</span>
                            </link_1.default>
                          </sidebar_1.SidebarMenuSubButton>
                        </sidebar_1.SidebarMenuSubItem>); })}
                    </sidebar_1.SidebarMenuSub>) : null}
                </sidebar_1.SidebarMenuItem>);
        })}
            </sidebar_1.SidebarMenu>
          </sidebar_1.SidebarGroupContent>
        ...
        <sidebar_1.SidebarGroup>
          ...
          <sidebar_1.SidebarGroupContent>
            <sidebar_1.SidebarMenu>
              {navigationData.navSecondary.map(function (item) { return (<sidebar_1.SidebarMenuItem key={item.title}>
                  <sidebar_1.SidebarMenuButton asChild isActive={pathname === item.url} tooltip={item.title}>
                    <link_1.default href={item.url}>
                      <item.icon />
                      <span>{item.title}</span>
        ...
        <sidebar_1.SidebarMenu>
          <sidebar_1.SidebarMenuItem>
            <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
              <avatar_1.Avatar className="h-8 w-8 rounded-lg">
                <avatar_1.AvatarImage src={(_b = user === null || user === void 0 ? void 0 : user.user_metadata) === null || _b === void 0 ? void 0 : _b.avatar_url} alt={((_c = user === null || user === void 0 ? void 0 : user.user_metadata) === null || _c === void 0 ? void 0 : _c.full_name) || (user === null || user === void 0 ? void 0 : user.email)}/>
                <avatar_1.AvatarFallback className="rounded-lg">
                  {((_d = user === null || user === void 0 ? void 0 : user.email) === null || _d === void 0 ? void 0 : _d.charAt(0).toUpperCase()) || "U"}
                </avatar_1.AvatarFallback>
              </avatar_1.Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-semibold">
                  {((_e = user === null || user === void 0 ? void 0 : user.user_metadata) === null || _e === void 0 ? void 0 : _e.full_name) ||
            ((_f = user === null || user === void 0 ? void 0 : user.email) === null || _f === void 0 ? void 0 : _f.split("@")[0]) ||
            "Usuário"}
                </span>
          ...
          <sidebar_1.SidebarMenuItem>
            <form action="/api/auth/signout" method="post" className="w-full">
              <button_1.Button type="submit" variant="ghost" size="sm" className="w-full justify-start gap-2">
