...
function RiskFactors(_a) {
    var riskProfiles = _a.riskProfiles, predictions = _a.predictions, onProfileUpdate = _a.onProfileUpdate;
    var _b = (0, react_1.useState)(null), selectedProfile = _b[0], setSelectedProfile = _b[1];
    var _c = (0, react_1.useState)('overview'), analysisView = _c[0], setAnalysisView = _c[1];
    var _d = (0, react_1.useState)('30d'), timeWindow = _d[0], setTimeWindow = _d[1];
    /**
    ...
    var riskAnalysis = (0, react_1.useMemo)(function () {
        ...
        var categoryBreakdown = Object.entries(categoryData).map(function (_a) {
            var _b;
            var _c, _d;
            var category = _a[0], data = _a[1];
            var avgRisk = data.risks.reduce(function (sum, risk) { return sum + risk; }, 0) / data.risks.length;
            var categoryConfig = (_b = {},
                ...
                _b);
            return {
                category: category.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, function (l) { return l.toUpperCase(); }),
                avgRisk: Math.round(avgRisk),
                count: data.count,
                color: ((_c = categoryConfig[category]) === null || _c === void 0 ? void 0 : _c.color) || '#6B7280',
                icon: ((_d = categoryConfig[category]) === null || _d === void 0 ? void 0 : _d.icon) || <lucide_react_1.Target className="h-4 w-4"/>
            };
        });
        ...
        var factorCorrelations = [
            ...
            { factor: 'Communication Response Rate', correlation: -0.68, significance: 0.82, trend: 'DOWN' },
        ...
        var patientSegments = [
            {
                ...
                count: riskProfiles.filter(function (p) { return p.overallRiskScore < 25; }).length,
                avgRisk: 15,
                characteristics: ['High communication response', 'Consistent attendance', 'Long relationship']
            },
            {
                segment: 'Moderate Risk',
                count: riskProfiles.filter(function (p) { return p.overallRiskScore >= 25 && p.overallRiskScore < 50; }).length,
                avgRisk: 35,
                characteristics: ['Occasional no-shows', 'Variable communication', 'Standard patterns']
            },
            {
                segment: 'High Risk',
                count: riskProfiles.filter(function (p) { return p.overallRiskScore >= 50 && p.overallRiskScore < 75; }).length,
                avgRisk: 62,
                characteristics: ['Frequent no-shows', 'Poor communication', 'Multiple risk factors']
            },
            {
                segment: 'Critical Risk',
                count: riskProfiles.filter(function (p) { return p.overallRiskScore >= 75; }).length,
                avgRisk: 85,
     ...
     */
    var getProfileRadarData = function (profile) {
        return [
    ...
    var formatTrend = function (trend) {
        switch (trend) {
            case 'IMPROVING':
                return { icon: <lucide_react_1.TrendingDown className="h-4 w-4"/>, color: 'text-green-600' };
            case 'DETERIORATING':
    ...
    return (<div className="space-y-6">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-green-600">
              {riskProfiles.filter(function (p) { return p.riskTrend === 'IMPROVING'; }).length}
            </div>
            <div className="text-xs text-muted-foreground">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-orange-600">
              {riskAnalysis.factorCorrelations.filter(function (f) { return f.correlation > 0.7; }).length}
            </div>
            <div className="text-xs text-muted-foreground">
              Strong correlations
            </div>
      ...
      {/* Main Analysis Tabs */}
      <tabs_1.Tabs value={analysisView} onValueChange={function (value) { return setAnalysisView(value); }}>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            name === 'avgRisk' ? 'Average Risk' : 'Count'
                    ...
                    <recharts_1.Bar dataKey="avgRisk" name="Average Risk">
                      {riskAnalysis.categoryBreakdown.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                    </recharts_1.Bar>
                  </recharts_1.BarChart>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {riskAnalysis.patientSegments.map(function (segment, index) { return (<div key={segment.segment} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <badge_1.Badge variant={segment.avgRisk >= 75 ? 'destructive' :
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {riskAnalysis.protectiveFactors.map(function (factor, index) { return (<div key={factor.factor} className="flex items-center justify-between">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {riskAnalysis.timePatterns.map(function (pattern, index) { return (<div key={pattern.timeframe} className="space-y-2">
                      <div className="flex items-center justify-between">
        ...
        <tabs_1.TabsContent value="categories" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            {riskAnalysis.categoryBreakdown.map(function (category) { return (<card_1.Card key={category.category}>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.BarChart data={riskAnalysis.categoryBreakdown} layout="horizontal" margin={{ left: 100 }}>
                  <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                  <recharts_1.XAxis type="number"/>
                  <recharts_1.YAxis dataKey="category" type="category"/>
                  <recharts_1.Tooltip formatter={function (value) { return ["".concat(value, "%"), 'Average Risk']; }}/>
        ...
        <tabs_1.TabsContent value="correlations" className="space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {riskAnalysis.factorCorrelations.map(function (correlation, index) {
            var trendDisplay = formatTrend(correlation.trend);
            var isPositive = correlation.correlation > 0;
            return (<div key={correlation.factor} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="text-right">
                          <span className={"font-medium ".concat(isPositive ? 'text-red-600' : 'text-green-600')}>
                      ...
                      <progress_1.Progress value={Math.abs(correlation.correlation) * 100} className="h-2"/>
                    </div>);
        })}
              </div>
            </card_1.CardContent>
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.ScatterChart>
                  <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                  <recharts_1.XAxis dataKey="correlation" domain={[-1, 1]} tickFormatter={function (value) { return "".concat((value * 100).toFixed(0), "%"); }}/>
                  <recharts_1.YAxis dataKey="significance" domain={[0, 1]} tickFormatter={function (value) { return "".concat((value * 100).toFixed(0), "%"); }}/>
                  <recharts_1.Tooltip formatter={function (value, name) { return [
            name === 'correlation' ? "".concat((value * 100).toFixed(1), "%") : "".concat((value * 100).toFixed(1), "%"),
        ...
        <tabs_1.TabsContent value="segments" className="space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {riskProfiles.slice(0, 10).map(function (profile) { return (<card_1.Card key={profile.patientId} className="cursor-pointer hover:bg-muted/50" onClick={function () { return setSelectedProfile(profile); }}>
                    <card_1.CardContent className="pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <badge_1.Badge className={(0, risk_scoring_1.getRiskLevelColor)(profile.riskLevel)}>
                            {profile.riskLevel}
                          </badge_1.Badge>
                      ...
                      {profile.topRiskFactors.length > 0 && (<div className="mt-3 text-xs text-muted-foreground">
                          Top risks: {profile.topRiskFactors.slice(0, 3).map(function (f) { return f.factorName; }).join(', ')}
                        </div>)}
                    </card_1.CardContent>
                  </card_1.Card>); })}
              </div>
      ...
      {selectedProfile && (<card_1.Card className="border-primary">
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center justify-between">
              <span>Risk Profile Details</span>
              <button_1.Button variant="outline" size="sm" onClick={function () { return setSelectedProfile(null); }}>
                Close
              </button_1.Button>
            </card_1.CardTitle>
          </card_1.CardHeader>
          <card_1.CardContent>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <badge_1.Badge className={(0, risk_scoring_1.getRiskLevelColor)(selectedProfile.riskLevel)} size="lg">
              ...
              <div>
                ...
                <recharts_1.ResponsiveContainer width="100%" height={250}>
                  <recharts_1.RadarChart data={getProfileRadarData(selectedProfile)}>
                    ...
                    <recharts_1.Radar name="Risk Score" dataKey="score" stroke="#EF4444" fill="#EF4444" fillOpacity={0.3} strokeWidth={2}/>
            ...
            {/* Top Risk Factors */}
            {selectedProfile.topRiskFactors.length > 0 && (<div className="mt-6">
                <h4 className="font-medium mb-4">Top Risk Factors</h4>
                <div className="space-y-3">
                  {selectedProfile.topRiskFactors.map(function (factor, index) { return (<alert_1.Alert key={index}>
                      <lucide_react_1.AlertTriangle className="h-4 w-4"/>
            ...
            {selectedProfile.recommendedInterventions.length > 0 && (<div className="mt-6">
                <h4 className="font-medium mb-4">Recommended Interventions</h4>
                <div className="space-y-2">
                  {selectedProfile.recommendedInterventions.map(function (intervention, index) { return (<div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg">
