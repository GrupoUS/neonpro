...
function PredictionOverview(_a) {
    var predictions = _a.predictions, riskProfiles = _a.riskProfiles, timeRange = _a.timeRange, onTimeRangeChange = _a.onTimeRangeChange, riskLevelFilter = _a.riskLevelFilter, onRiskLevelChange = _a.onRiskLevelChange;
    var _b = (0, react_1.useState)(''), searchTerm = _b[0], setSearchTerm = _b[1];
    var _c = (0, react_1.useState)('riskScore'), sortBy = _c[0], setSortBy = _c[1];
    var _d = (0, react_1.useState)('desc'), sortOrder = _d[0], setSortOrder = _d[1];
    var _e = (0, react_1.useState)(null), selectedPrediction = _e[0], setSelectedPrediction = _e[1];
    /**
    ...
    var filteredPredictions = (0, react_1.useMemo)(function () {
        ...
        if (riskLevelFilter !== 'ALL') {
            filtered = filtered.filter(function (p) { return p.riskLevel === riskLevelFilter; });
        }
        // Apply search filter
        if (searchTerm) {
            filtered = filtered.filter(function (p) {
                return p.patientId.toLowerCase().includes(searchTerm.toLowerCase()) ||
     ...
     */
    var summaryData = (0, react_1.useMemo)(function () {
        // Risk level distribution
        var riskDistribution = [
            {
                level: 'LOW',
                count: predictions.filter(function (p) { return p.riskLevel === 'LOW'; }).length,
                color: '#10B981'
            },
            {
                level: 'MEDIUM',
                count: predictions.filter(function (p) { return p.riskLevel === 'MEDIUM'; }).length,
                color: '#F59E0B'
            },
            {
                level: 'HIGH',
                count: predictions.filter(function (p) { return p.riskLevel === 'HIGH'; }).length,
                color: '#F97316'
            },
            {
                level: 'CRITICAL',
                count: predictions.filter(function (p) { return p.riskLevel === 'CRITICAL'; }).length,
                color: '#EF4444'
            }
        ];
        ...
        var timeDistribution = Array.from({ length: 24 }, function (_, hour) {
            ...
            var avgRisk = hourPredictions.length > 0
                ? hourPredictions.reduce(function (sum, p) { return sum + p.riskScore; }, 0) / hourPredictions.length
                : 0;
            return {
        ...
        var riskTrends = Array.from({ length: 7 }, function (_, dayIndex) {
            ...
            var avgRisk = dayPredictions.length > 0
                ? dayPredictions.reduce(function (sum, p) { return sum + p.riskScore; }, 0) / dayPredictions.length
                : 0;
            return {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            ? Math.round(predictions.reduce(function (sum, p) { return sum + p.riskScore; }, 0) / predictions.length)
            : 0}%
            </div>
            <div className="text-xs text-muted-foreground">
              Across all patients
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-orange-600">
              {predictions.filter(function (p) { return p.riskLevel === 'HIGH' || p.riskLevel === 'CRITICAL'; }).length}
            </div>
            <div className="text-xs text-muted-foreground">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {predictions.length > 0
            ? Math.round((predictions.reduce(function (sum, p) { return sum + p.confidence; }, 0) / predictions.length) * 100)
            : 0}%
            </div>
            <div className="text-xs text-muted-foreground">
              Average confidence
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.PieChart>
                <recharts_1.Pie data={summaryData.riskDistribution} cx="50%" cy="50%" outerRadius={80} dataKey="count" label={function (_a) {
            var level = _a.level, count = _a.count, percent = _a.percent;
            return "".concat(level, ": ").concat(count, " (").concat((percent * 100).toFixed(0), "%)");
        }}>
                  {summaryData.riskDistribution.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                </recharts_1.Pie>
                <recharts_1.Tooltip />
              </recharts_1.PieChart>
          ...
          <card_1.CardContent>
            <recharts_1.ResponsiveContainer width="100%" height={300}>
              <recharts_1.AreaChart data={summaryData.timeDistribution}>
                ...
                <recharts_1.YAxis />
                <recharts_1.Tooltip labelFormatter={function (hour) { return "Time: ".concat(hour); }} formatter={function (value, name) { return [
            value,
            name === 'predictions' ? 'Predictions' : 'Avg Risk %'
        ]; }}/>
                <recharts_1.Area type="monotone" dataKey="predictions" stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.3}/>
                <recharts_1.Line type="monotone" dataKey="avgRisk" stroke="#EF4444" strokeWidth={2}/>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            name === 'avgRisk' ? 'Avg Risk %' : 'Count'
                ...
                <recharts_1.Line type="monotone" dataKey="avgRisk" stroke="#EF4444" strokeWidth={3}/>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="space-y-3">
              {summaryData.topRiskFactors.slice(0, 8).map(function (factor, index) { return (<div key={factor.factor} className="flex items-center justify-between">
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-4">
            <div className="flex-1">
              <input_1.Input placeholder="Search by patient ID or appointment..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="w-full"/>
            </div>
            ...
            <button_1.Button variant="outline" onClick={function () { return setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc'); }}>
              {sortOrder === 'asc' ? '↑' : '↓'}
            </button_1.Button>
          </div>
        </card_1.CardHeader>
        <card_1.CardContent>
          ...
          <div className="rounded-md border">
            ...
            var topFactor = prediction.factors.sort(function (a, b) { return b.contribution - a.contribution; })[0];
            return (<table_1.TableRow key={prediction.appointmentId} className="cursor-pointer hover:bg-muted/50" onClick={function () { return setSelectedPrediction(prediction); }}>
                      ...
                      <table_1.TableCell>
                        <badge_1.Badge className={(0, no_show_prediction_1.getRiskBadgeColor)(prediction.riskLevel)}>
                      ...
                      <table_1.TableCell>
                        ...
                        {topFactor && (<div className="text-xs text-muted-foreground">
          ...
          {filteredPredictions.length === 0 && (<div className="text-center py-8">
          ...
          {filteredPredictions.length > 20 && (<div className="text-center mt-4">
      ...
      {selectedPrediction && (<card_1.Card className="border-primary">
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center justify-between">
              <span>Prediction Details</span>
              <button_1.Button variant="outline" size="sm" onClick={function () { return setSelectedPrediction(null); }}>
                Close
              </button_1.Button>
            </card_1.CardTitle>
          </card_1.CardHeader>
          <card_1.CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={"font-bold ".concat((0, no_show_prediction_1.getRiskColor)(selectedPrediction.riskLevel))}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {selectedPrediction.factors.map(function (factor, index) { return (<div key={index} className="flex items-center justify-between">
                      <div className="flex-1">
            ...
            {selectedPrediction.interventionRecommendations.length > 0 && (<div className="mt-6">
                <h4 className="font-medium mb-4">Recommended Interventions</h4>
                <div className="space-y-3">
                  {selectedPrediction.interventionRecommendations.map(function (intervention, index) { return (<alert_1.Alert key={index}>
                      ...
                      <alert_1.AlertDescription>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <badge_1.Badge variant={intervention.priority === 'URGENT' ? 'destructive' :
