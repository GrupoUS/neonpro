...
function WaitlistOptimization(_a) {
    var waitlistEntries = _a.waitlistEntries, onUpdateEntry = _a.onUpdateEntry, onContactPatient = _a.onContactPatient, onScheduleAppointment = _a.onScheduleAppointment;
    var _b = (0, react_1.useState)(null), selectedEntry = _b[0], setSelectedEntry = _b[1];
    var _c = (0, react_1.useState)('overview'), activeTab = _c[0], setActiveTab = _c[1];
    var _d = (0, react_1.useState)('ALL'), priorityFilter = _d[0], setPriorityFilter = _d[1];
    var _e = (0, react_1.useState)('ALL'), specialtyFilter = _e[0], setSpecialtyFilter = _e[1];
    var _f = (0, react_1.useState)('waitTime'), sortBy = _f[0], setSortBy = _f[1];
    var _g = (0, react_1.useState)(false), showOptimizations = _g[0], setShowOptimizations = _g[1];
    /**
     * Calculate waitlist analytics
     */
    var analytics = (0, react_1.useMemo)(function () {
        var activeEntries = waitlistEntries.filter(function (e) { return e.status === 'WAITING' || e.status === 'CONTACTED'; });
        // Basic metrics
        var averageWaitTime = activeEntries.reduce(function (sum, entry) { return sum + entry.estimatedWaitTime; }, 0) / activeEntries.length || 0;
        ...
        var priorityDistribution = Object.entries(priorityGroups).map(function (_a) {
            var priority = _a[0], entries = _a[1];
            return ({
                priority: PRIORITY_LABELS[priority],
                count: entries.length,
                avgWait: entries.reduce(function (sum, e) { return sum + e.estimatedWaitTime; }, 0) / entries.length,
        ...
        var specialtyBreakdown = Object.entries(specialtyGroups).map(function (_a) {
            var specialty = _a[0], entries = _a[1];
            return ({
                specialty: specialty,
     ...
     */
    var optimizationSuggestions = (0, react_1.useMemo)(function () {
        var suggestions = [];
        ...
        if (highRiskPatients.length > 0) {
            suggestions.push({
                ...
                description: "".concat(highRiskPatients.length, " high-risk patients need immediate contact to prevent dropout"),
                impact: 'HIGH',
        ...
        if (overCapacitySpecialties.length > 0) {
            suggestions.push({
                ...
                description: "Rebalance capacity for ".concat(overCapacitySpecialties.length, " over-capacity specialties"),
                impact: 'MEDIUM',
                estimatedReduction: 2,
                affectedPatients: overCapacitySpecialties.reduce(function (sum, s) { return sum + s.waiting; }, 0),
                confidence: 78,
        ...
        if (likelyCancellations.length > 0) {
            suggestions.push({
                ...
                description: "".concat(likelyCancellations.length, " patients likely to cancel based on engagement patterns"),
                impact: 'MEDIUM',
    ...
    var filteredEntries = (0, react_1.useMemo)(function () {
        var filtered = waitlistEntries.filter(function (entry) {
            if (priorityFilter !== 'ALL' && entry.priority !== priorityFilter)
                return false;
            if (specialtyFilter !== 'ALL' && entry.specialty !== specialtyFilter)
                return false;
            return true;
        });
    ...
    var formatWaitTime = function (days) {
        ...
        if (days === 1)
            return '1 day';
        if (days < 7)
            return "".concat(Math.round(days), " days");
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button_1.Button variant="outline" onClick={function () { return setShowOptimizations(!showOptimizations); }}>
      ...
      {/* Optimization Alerts */}
      {showOptimizations && optimizationSuggestions.length > 0 && (<div className="space-y-3">
          {optimizationSuggestions.slice(0, 3).map(function (suggestion, index) { return (<alert_1.Alert key={index} className={suggestion.impact === 'HIGH' ? 'border-red-200 bg-red-50' :
              ...
              <alert_1.AlertDescription>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right ml-4">
                    <badge_1.Badge variant={suggestion.impact === 'HIGH' ? 'destructive' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className={"text-2xl font-bold ".concat(getWaitTimeColor(analytics.averageWaitTime))}>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={function (value) { return setActiveTab(value); }}>
            ...
            <card_1.CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select_1.Select value={specialtyFilter} onValueChange={setSpecialtyFilter}>
                    ...
                    <select_1.SelectContent>
                      ...
                      {analytics.specialtyBreakdown.map(function (specialty) { return (<select_1.SelectItem key={specialty.specialty} value={specialty.specialty}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select_1.Select value={sortBy} onValueChange={function (value) { return setSortBy(value); }}>
                    <select_1.SelectTrigger className="w-32">
                      <select_1.SelectValue />
                    </select_1.SelectTrigger>
                    <select_1.SelectContent>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-3">
                {filteredEntries.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                    No waitlist entries match the current filters.
                  </div>) : (filteredEntries.map(function (entry) { return (<card_1.Card key={entry.id} className="cursor-pointer hover:bg-muted/50" onClick={function () { return setSelectedEntry(entry); }}>
                      <card_1.CardContent className="pt-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="text-right">
                            <div className={"font-bold ".concat(getWaitTimeColor(entry.estimatedWaitTime))}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                          ...
                          <div>
                            ...
                            <badge_1.Badge variant={entry.status === 'WAITING' ? 'default' :
                    ...
                    entry.status === 'SCHEDULED' ? 'outline' : 'destructive'}>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="text-xs text-muted-foreground">
                            ...
                            {entry.lastContact && (<span> • Last contact: {entry.lastContact.toLocaleDateString()}</span>)}
                          </div>
        ...
        <tabs_1.TabsContent value="optimization" className="space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {optimizationSuggestions.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                    No optimization suggestions at the moment. Your waitlist is well-managed!
                  </div>) : (optimizationSuggestions.map(function (suggestion, index) { return (<card_1.Card key={index} className="p-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            return (<div key={specialty.specialty} className="space-y-2">
                        <div className="flex items-center justify-between">
                        ...
                        <progress_1.Progress value={utilizationRate} className={"h-2 ".concat(utilizationRate > 80 ? 'bg-red-100' : utilizationRate > 60 ? 'bg-yellow-100' : 'bg-green-100')}/>
                        <div className="text-xs text-muted-foreground">
            ...
            name === 'projected' ? 'Current Trend' :
                name === 'historical' ? 'Historical' : 'Optimized'
                    ...
                    <recharts_1.Line type="monotone" dataKey="historical" stroke="#9CA3AF" strokeDasharray="5 5" name="historical"/>
                    <recharts_1.Line type="monotone" dataKey="projected" stroke="#EF4444" strokeWidth={2} name="projected"/>
                    <recharts_1.Line type="monotone" dataKey="optimized" stroke="#10B981" strokeWidth={2} name="optimized"/>
        ...
        <tabs_1.TabsContent value="analytics" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.BarChart data={analytics.priorityDistribution}>
                    ...
                    <recharts_1.Bar dataKey="count" name="Count">
                      {analytics.priorityDistribution.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                    </recharts_1.Bar>
                  </recharts_1.BarChart>
                </recharts_1.ResponsiveContainer>
              </card_1.CardContent>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {analytics.conversionRates.map(function (stage, index) { return (<div key={stage.stage} className="space-y-2">
                      <div className="flex items-center justify-between">
          ...
          <card_1.Card>
            ...
            name === 'projected' ? 'Projected' :
                name === 'historical' ? 'Historical' : 'Optimized'
                  ...
                  <recharts_1.Area type="monotone" dataKey="historical" stackId="1" stroke="#9CA3AF" fill="#F3F4F6" name="historical"/>
                  <recharts_1.Area type="monotone" dataKey="projected" stackId="2" stroke="#EF4444" fill="#FEE2E2" name="projected"/>
                  <recharts_1.Area type="monotone" dataKey="optimized" stackId="3" stroke="#10B981" fill="#DCFCE7" name="optimized"/>
      ...
      {selectedEntry && (<dialog_1.Dialog open={!!selectedEntry} onOpenChange={function () { return setSelectedEntry(null); }}>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className={"font-medium ".concat(getWaitTimeColor(selectedEntry.estimatedWaitTime))}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <progress_1.Progress value={selectedEntry.noShowRisk} className="h-2 flex-1"/>
                    </div>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">Status</label>
                    <badge_1.Badge variant={selectedEntry.status === 'WAITING' ? 'default' :
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedEntry.preferredTimeSlots.map(function (slot, index) { return (<badge_1.Badge key={index} variant="outline">
                      {slot}
                    </badge_1.Badge>); })}
                </div>
              </div>
              ...
              <div className="flex justify-between">
                <div className="flex gap-2">
                  <button_1.Button variant="outline" onClick={function () { return handleContactPatient(selectedEntry, 'SMS'); }}>
                    <lucide_react_1.MessageSquare className="h-4 w-4 mr-2"/>
                    Send SMS
                  </button_1.Button>
                  <button_1.Button variant="outline" onClick={function () { return handleContactPatient(selectedEntry, 'PHONE'); }}>
                    <lucide_react_1.Phone className="h-4 w-4 mr-2"/>
                    Call
                  </button_1.Button>
                  <button_1.Button variant="outline" onClick={function () { return handleContactPatient(selectedEntry, 'EMAIL'); }}>
                    <lucide_react_1.Mail className="h-4 w-4 mr-2"/>
                    Email
                  </button_1.Button>
                </div>
                <div className="flex gap-2">
                  <button_1.Button variant="outline" onClick={function () { return setSelectedEntry(null); }}>
                    Close
                  </button_1.Button>
                  <button_1.Button>
