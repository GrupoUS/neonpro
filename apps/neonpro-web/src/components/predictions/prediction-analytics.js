...
function PredictionAnalytics(_a) {
    var predictions = _a.predictions, riskProfiles = _a.riskProfiles, performanceMetrics = _a.performanceMetrics;
     ...
     */
    var analyticsData = (0, react_1.useMemo)(function () {
        // Calculate overall model accuracy
        var latestMetrics = performanceMetrics[performanceMetrics.length - 1];
        var overallAccuracy = (latestMetrics === null || latestMetrics === void 0 ? void 0 : latestMetrics.accuracy) || 0;
        // Accuracy by risk level
        var riskLevels = ['LOW', 'MEDIUM', 'HIGH', 'VERY_HIGH'];
        var byRiskLevel = riskLevels.map(function (level) {
            var levelPredictions = predictions.filter(function (p) { return p.riskLevel === level; });
            var correct = levelPredictions.filter(function (p) { return p.confidence > 0.8; }).length; // Mock accuracy calculation
            return {
        ...
        var predictionDistribution = [
            { riskRange: 'Low (0-25%)', count: predictions.filter(function (p) { return p.riskScore <= 25; }).length, accuracy: 92, color: RISK_COLORS['Low (0-25%)'] },
            { riskRange: 'Medium (26-50%)', count: predictions.filter(function (p) { return p.riskScore > 25 && p.riskScore <= 50; }).length, accuracy: 88, color: RISK_COLORS['Medium (26-50%)'] },
            { riskRange: 'High (51-75%)', count: predictions.filter(function (p) { return p.riskScore > 50 && p.riskScore <= 75; }).length, accuracy: 84, color: RISK_COLORS['High (51-75%)'] },
            { riskRange: 'Very High (76-100%)', count: predictions.filter(function (p) { return p.riskScore > 75; }).length, accuracy: 89, color: RISK_COLORS['Very High (76-100%)'] }
        ];
        // Feature importance analysis
        var featureImportance = [
            { feature: 'Historical No-Show Rate', importance: 0.34, impact: 'POSITIVE', category: 'Historical' },
            { feature: 'Appointment Day of Week', importance: 0.18, impact: 'POSITIVE', category: 'Temporal' },
            { feature: 'Time Since Last Visit', importance: 0.15, impact: 'POSITIVE', category: 'Behavioral' },
            { feature: 'Communication Response Rate', importance: 0.12, impact: 'NEGATIVE', category: 'Communication' },
            { feature: 'Distance from Clinic', importance: 0.09, impact: 'POSITIVE', category: 'Geographic' },
        ...
        // Time-based patterns
        var timePatterns = Array.from({ length: 24 }, function (_, hour) { return ({
            hour: hour,
    ...
    var getModelInsights = function () {
        var insights = [];
        var lowAccuracySegments = analyticsData.demographicInsights.filter(function (s) { return s.accuracy < 85; });
        if (lowAccuracySegments.length > 0) {
            insights.push({
                type: 'improvement',
                message: "Model accuracy is lower for ".concat(lowAccuracySegments.map(function (s) { return s.segment; }).join(', '), ". Consider adding more training data for these segments."),
                priority: 'HIGH'
            });
        }
        var highROIInterventions = analyticsData.interventionImpact.filter(function (i) { return i.roi > 500; });
        if (highROIInterventions.length > 0) {
            insights.push({
                type: 'optimization',
                message: "Scale up ".concat(highROIInterventions.map(function (i) { return i.intervention; }).join(', '), " - showing excellent ROI."),
                priority: 'MEDIUM'
            });
        }
        return insights;
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <select_1.Select value={timeRange} onValueChange={function (value) { return setTimeRange(value); }}>
      ...
      {modelInsights.length > 0 && (<div className="space-y-3">
          {modelInsights.map(function (insight, index) { return (<card_1.Card key={index} className={insight.priority === 'HIGH' ? 'border-red-200 bg-red-50' :
                    insight.priority === 'MEDIUM' ? 'border-yellow-200 bg-yellow-50' :
                        'border-blue-200 bg-blue-50'}>
              <card_1.CardContent className="pt-4">
                <div className="flex items-center gap-2">
                  {insight.type === 'improvement' ? (<lucide_react_1.AlertTriangle className="h-4 w-4 text-red-600"/>) : (<lucide_react_1.TrendingUp className="h-4 w-4 text-blue-600"/>)}
                  <span className="text-sm">{insight.message}</span>
                  <badge_1.Badge variant={insight.priority === 'HIGH' ? 'destructive' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className={"text-2xl font-bold ".concat(getMetricColor(analyticsData.modelAccuracy.overall))}>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-green-600">
              {predictions.filter(function (p) { return p.confidence > 0.8; }).length}
            </div>
            <div className="text-xs text-muted-foreground">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={function (value) { return setActiveTab(value); }}>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.PieChart>
                    <recharts_1.Pie data={analyticsData.predictionDistribution} cx="50%" cy="50%" outerRadius={100} dataKey="count" nameKey="riskRange">
                      {analyticsData.predictionDistribution.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.color}/>); })}
                    </recharts_1.Pie>
                    <recharts_1.Tooltip formatter={function (value, name) { return [value, 'Count']; }} labelFormatter={function (label) { return "Risk: ".concat(label); }}/>
                  </recharts_1.PieChart>
                </recharts_1.ResponsiveContainer>
                <div className="grid grid-cols-2 gap-2 mt-4">
                  {analyticsData.predictionDistribution.map(function (item, index) { return (<div key={index} className="flex items-center gap-2 text-xs">
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.LineChart data={analyticsData.modelAccuracy.overTime.slice(-14)}>
                    ...
                    <recharts_1.XAxis dataKey="date" tickFormatter={function (date) { return new Date(date).toLocaleDateString('pt-BR', { month: 'short', day: 'numeric' }); }}/>
                    <recharts_1.YAxis domain={[60, 100]}/>
                    <recharts_1.Tooltip labelFormatter={function (date) { return new Date(date).toLocaleDateString('pt-BR'); }} formatter={function (value, name) { return [
            "".concat(value.toFixed(1), "%"),
            name === 'accuracy' ? 'Accuracy' : 'Predictions'
        ]; }}/>
                    <recharts_1.Line type="monotone" dataKey="accuracy" stroke="#10B981" strokeWidth={2} name="accuracy"/>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {analyticsData.featureImportance.slice(0, 6).map(function (feature, index) { return (<div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">{feature.feature}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-sm">{(feature.importance * 100).toFixed(1)}%</span>
                          <badge_1.Badge variant={feature.impact === 'POSITIVE' ? 'destructive' : 'default'} size="sm">
                            {feature.impact}
                          </badge_1.Badge>
                        </div>
                      </div>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-3">
                  {analyticsData.demographicInsights.map(function (segment, index) { return (<div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">{segment.segment}</span>
                        <div className="text-right">
                          <div className={"text-sm font-medium ".concat(getMetricColor(segment.accuracy))}>
        ...
        <tabs_1.TabsContent value="accuracy" className="space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="mb-4">
                <select_1.Select value={selectedMetric} onValueChange={function (value) { return setSelectedMetric(value); }}>
              ...
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.BarChart data={analyticsData.modelAccuracy.byRiskLevel}>
                  ...
                  <recharts_1.YAxis />
                  <recharts_1.Tooltip formatter={function (value) { return ["".concat(value.toFixed(1), "%"), selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)]; }}/>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="grid grid-cols-2 gap-4">
                  {analyticsData.modelAccuracy.byRiskLevel.map(function (level, index) { return (<div key={level.level} className="space-y-3 p-4 bg-muted rounded-lg">
          ...
          <card_1.Card>
            ...
            name === 'accuracy' ? 'Accuracy' : 'Predictions'
                  ...
                  <recharts_1.Area type="monotone" dataKey="accuracy" stroke="#10B981" fill="#DCFCE7" name="accuracy"/>
        ...
        <tabs_1.TabsContent value="features" className="space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.BarChart data={analyticsData.featureImportance} layout="horizontal" margin={{ left: 120 }}>
                  <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                  <recharts_1.XAxis type="number" tickFormatter={function (value) { return "".concat((value * 100).toFixed(0), "%"); }}/>
                  <recharts_1.YAxis dataKey="feature" type="category"/>
                  <recharts_1.Tooltip formatter={function (value) { return ["".concat((value * 100).toFixed(1), "%"), 'Importance']; }}/>
                  <recharts_1.Bar dataKey="importance">
                    {analyticsData.featureImportance.map(function (entry, index) { return (<recharts_1.Cell key={"cell-".concat(index)} fill={entry.impact === 'POSITIVE' ? '#EF4444' : '#10B981'}/>); })}
                  </recharts_1.Bar>
                </recharts_1.BarChart>
              </recharts_1.ResponsiveContainer>
            </card_1.CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {__spreadArray([], new Set(analyticsData.featureImportance.map(function (f) { return f.category; })), true).map(function (category) {
            var categoryFeatures = analyticsData.featureImportance.filter(function (f) { return f.category === category; });
            var totalImportance = categoryFeatures.reduce(function (sum, f) { return sum + f.importance; }, 0);
                    ...
                    <recharts_1.Pie data={[
            {
                ...
                value: analyticsData.featureImportance.filter(function (f) { return f.impact === 'POSITIVE'; }).length,
                fill: '#EF4444'
            },
            {
                name: 'Risk Decreasing',
                value: analyticsData.featureImportance.filter(function (f) { return f.impact === 'NEGATIVE'; }).length,
                fill: '#10B981'
            }
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.AreaChart data={analyticsData.timePatterns}>
                  <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                  <recharts_1.XAxis dataKey="hour" tickFormatter={function (hour) { return "".concat(hour, ":00"); }}/>
                  <recharts_1.YAxis />
                  <recharts_1.Tooltip labelFormatter={function (hour) { return "".concat(hour, ":00"); }} formatter={function (value, name) { return [
            name === 'noShowRate' ? "".concat(value.toFixed(1), "%") :
                name === 'accuracy' ? "".concat(value.toFixed(1), "%") : value,
            name === 'noShowRate' ? 'No-Show Rate' :
                name === 'accuracy' ? 'Prediction Accuracy' : 'Predictions'
        ]; }}/>
                  <recharts_1.Area type="monotone" dataKey="noShowRate" stroke="#EF4444" fill="#FEE2E2" name="noShowRate"/>
                  <recharts_1.Area type="monotone" dataKey="accuracy" stroke="#10B981" fill="#DCFCE7" name="accuracy"/>
                </recharts_1.AreaChart>
              </recharts_1.ResponsiveContainer>
            </card_1.CardContent>
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.ScatterChart>
                  <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                  <recharts_1.XAxis dataKey="noShowRate" name="Actual No-Show Rate" tickFormatter={function (value) { return "".concat(value, "%"); }}/>
                  <recharts_1.YAxis dataKey="predictedRate" name="Predicted No-Show Rate" tickFormatter={function (value) { return "".concat(value, "%"); }}/>
                  <recharts_1.Tooltip formatter={function (value, name) { return [
            "".concat(value, "%"),
            name === 'noShowRate' ? 'Actual Rate' : 'Predicted Rate'
        ]; }} labelFormatter={function (label, payload) { var _a, _b; return ((_b = (_a = payload === null || payload === void 0 ? void 0 : payload[0]) === null || _a === void 0 ? void 0 : _a.payload) === null || _b === void 0 ? void 0 : _b.segment) || ''; }}/>
        ...
        <tabs_1.TabsContent value="impact" className="space-y-6">
            ...
            <card_1.CardContent>
              <recharts_1.ResponsiveContainer width="100%" height={400}>
                <recharts_1.BarChart data={analyticsData.interventionImpact}>
                  ...
                  <recharts_1.YAxis />
                  <recharts_1.Tooltip formatter={function (value, name) { return [
            name === 'roi' ? "".concat(value, "%") :
                name === 'cost' ? "R$ ".concat(value.toFixed(2)) :
                    name === 'effectiveness' ? "".concat(value, "%") : value,
            name === 'preventedNoShows' ? 'Prevented No-Shows' :
                name === 'roi' ? 'ROI' :
                    name === 'cost' ? 'Cost per Contact' : 'Effectiveness'
        ]; }}/>
                  <recharts_1.Bar dataKey="preventedNoShows" name="preventedNoShows" fill="#10B981"/>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            .map(function (intervention, index) { return (<div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                      ...
                      <progress_1.Progress value={Math.min(intervention.roi / 10, 100)} className="h-2"/>
                    </div>); })}
                </div>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <recharts_1.ResponsiveContainer width="100%" height={300}>
                  <recharts_1.ScatterChart>
                    <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                    <recharts_1.XAxis dataKey="cost" name="Cost per Contact" tickFormatter={function (value) { return "R$ ".concat(value); }}/>
                    <recharts_1.YAxis dataKey="effectiveness" name="Effectiveness %" tickFormatter={function (value) { return "".concat(value, "%"); }}/>
                    <recharts_1.Tooltip formatter={function (value, name) { return [
            name === 'cost' ? "R$ ".concat(value.toFixed(2)) : "".concat(value, "%"),
            name === 'cost' ? 'Cost per Contact' : 'Effectiveness'
        ]; }} labelFormatter={function (label, payload) { var _a, _b; return ((_b = (_a = payload === null || payload === void 0 ? void 0 : payload[0]) === null || _a === void 0 ? void 0 : _a.payload) === null || _b === void 0 ? void 0 : _b.intervention) || ''; }}/>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {analyticsData.interventionImpact.reduce(function (sum, i) { return sum + i.preventedNoShows; }, 0)}
                  </div>
                  <div className="text-xs text-muted-foreground">Total No-Shows Prevented</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    R$ {(analyticsData.interventionImpact.reduce(function (sum, i) { return sum + (i.preventedNoShows * i.cost); }, 0)).toFixed(0)}
                  </div>
                  <div className="text-xs text-muted-foreground">Total Investment</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {Math.round(analyticsData.interventionImpact.reduce(function (sum, i) { return sum + i.roi; }, 0) / analyticsData.interventionImpact.length)}%
                  </div>
                  <div className="text-xs text-muted-foreground">Average ROI</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">
                    {Math.round(analyticsData.interventionImpact.reduce(function (sum, i) { return sum + i.effectiveness; }, 0) / analyticsData.interventionImpact.length)}%
                  </div>
