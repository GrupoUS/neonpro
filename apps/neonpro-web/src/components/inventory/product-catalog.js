...
function ProductCatalog() {
    ...
    var _d = (0, react_1.useState)("table"), viewMode = _d[0], setViewMode = _d[1];
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input_1.Input placeholder="Buscar produtos por nome, marca ou NCM..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {categories.map(function (category) {
            var categoryCount = mockProducts.filter(function (p) { return p.category === category.id && p.status === "active"; }).length;
            return (<card_1.Card key={category.id} className={"cursor-pointer transition-colors hover:shadow-md ".concat(selectedCategory === category.id ? "ring-2 ring-primary" : "")} onClick={function () {
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="overflow-x-auto">
            ...
            return (<table_1.TableRow key={product.id} className="hover:bg-muted/50">
                      <table_1.TableCell>
                        <div>
                          ...
                          {product.anvisaRegistration && (<div className="text-xs text-blue-600">
                      ...
                      <table_1.TableCell>
                        <badge_1.Badge variant="outline" className={category === null || category === void 0 ? void 0 : category.color}>
                      ...
                      <table_1.TableCell>
                        <div className="flex items-center gap-1">
                          {product.temperatureControlled && (<lucide_react_1.Thermometer className="w-3 h-3 text-blue-500"/>)}
                          <span className="text-sm">{product.location}</span>
                        </div>
                      </table_1.TableCell>

                      <table_1.TableCell>
                        {product.expirationDate ? (<div className={"text-sm ".concat(expiringSoon ? "text-amber-600 font-medium" : "")}>
                            {expiringSoon && (<lucide_react_1.Clock className="w-3 h-3 inline mr-1"/>)}
                            {new Date(product.expirationDate).toLocaleDateString("pt-BR")}
                            {product.batchNumber && (<div className="text-xs text-muted-foreground">
                    ...
                    minimumFractionDigits: 2,
                      ...
                      <table_1.TableCell>
                        <div className="flex flex-col gap-1">
                          {product.controlledSubstance && (<badge_1.Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 text-xs">
                              Controlado
                            </badge_1.Badge>)}
                          {product.temperatureControlled && (<badge_1.Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 text-xs">
                              Refrigerado
                            </badge_1.Badge>)}
                          {product.anvisaRegistration && (<badge_1.Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs">
                              ANVISA
                            </badge_1.Badge>)}
                        </div>
                      </table_1.TableCell>
                      ...
                      <table_1.TableCell>
                        <badge_1.Badge variant={product.status === "active"
                    ? "default"
                    : "secondary"} className={product.status === "active"
            ...
            {filteredProducts.length === 0 && (<div className="text-center py-8 text-muted-foreground">
