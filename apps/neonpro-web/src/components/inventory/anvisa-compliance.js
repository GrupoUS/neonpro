...
function ANVISACompliance() {
    ...
    var _b = (0, react_1.useState)('all'), selectedClass = _b[0], setSelectedClass = _b[1];
    var _c = (0, react_1.useState)('all'), selectedCompliance = _c[0], setSelectedCompliance = _c[1];
    var _d = (0, react_1.useState)(false), isNewDeviceOpen = _d[0], setIsNewDeviceOpen = _d[1];
    var filteredDevices = (0, react_1.useMemo)(function () {
        return mockANVISADevices.filter(function (device) {
            var matchesSearch = device.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            var matchesCompliance = selectedCompliance === 'all' ||
    ...
    var complianceMetrics = (0, react_1.useMemo)(function () {
        ...
        var compliant = mockANVISADevices.filter(function (d) { return d.compliance.current; }).length;
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input_1.Input placeholder="Buscar por nome, modelo, registro ANVISA ou serial..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
        </div>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="overflow-x-auto">
            ...
            return (<table_1.TableRow key={device.id} className="hover:bg-muted/50">
                      <table_1.TableCell>
                      ...
                      <table_1.TableCell>
                        <div className="space-y-1">
                          ...
                          {device.calibrationDueDate && (<div className="text-xs text-blue-600">
                      ...
                      <table_1.TableCell>
                        <div className="space-y-2">
                          ...
                          <div className={"text-xs ".concat(daysUntilMaintenance < 0 ? 'text-red-600' :
                    ...
                    : "".concat(daysUntilMaintenance, " dias restantes")}
                      ...
                      <table_1.TableCell>
                        <div className="space-y-2">
                          <badge_1.Badge variant="outline" className={device.compliance.current
                    ...
                    : 'bg-red-100 text-red-800 border-red-200'}>
                          ...
                          {device.compliance.nextAuditDate && (<div className="text-xs text-blue-600">
                      ...
                      <table_1.TableCell>
                        <div className="flex flex-col gap-2">
                          <button_1.Button size="sm" variant="outline" onClick={function () { return handleScheduleMaintenance(device.id); }}>
            ...
            {filteredDevices.length === 0 && (<div className="text-center py-8 text-muted-foreground">
      ...
      {/* Compliance Alerts */}
      {complianceMetrics.nonCompliant > 0 && (<alert_1.Alert className="border-red-200 bg-red-50">
          <lucide_react_1.AlertTriangle className="h-4 w-4"/>
          <alert_1.AlertTitle className="text-red-800">Atenção - Equipamentos Não Conformes</alert_1.AlertTitle>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            return (<div key={device.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                      <div className="space-y-1">
                        <div className="font-medium text-sm">{device.name}</div>
                        <div className="text-xs text-muted-foreground">
                          {(maintenanceItem === null || maintenanceItem === void 0 ? void 0 : maintenanceItem.type) === 'preventive' ? 'Preventiva' :
                    (maintenanceItem === null || maintenanceItem === void 0 ? void 0 : maintenanceItem.type) === 'calibration' ? 'Calibração' : 'Corretiva'}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {formatDate(device.nextMaintenanceDate)}
                        </div>
                      ...
                      <div className="text-right">
                        <badge_1.Badge variant="outline" className={daysUntil <= 7 ? 'bg-amber-50 text-amber-700 border-amber-200' :
                    'bg-blue-50 text-blue-700 border-blue-200'}>
                          {daysUntil === 0 ? 'Hoje' : "".concat(daysUntil, " dias")}
                        </badge_1.Badge>
                        {(maintenanceItem === null || maintenanceItem === void 0 ? void 0 : maintenanceItem.cost) && (<div className="text-xs text-muted-foreground mt-1">
            ...
            var daysUntil = getDaysUntilMaintenance(device.nextMaintenanceDate);
            return daysUntil >= 0 && daysUntil <= 30;
        }).length === 0 && (<p className="text-sm text-muted-foreground text-center py-4">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            var deviceClass = _a[0], info = _a[1];
            var devicesInClass = mockANVISADevices.filter(function (d) { return d.deviceClass === deviceClass; });
            var compliantInClass = devicesInClass.filter(function (d) { return d.compliance.current; });
            var classComplianceRate = devicesInClass.length > 0
                ? (compliantInClass.length / devicesInClass.length) * 100
                : 0;
            if (devicesInClass.length === 0)
                return null;
            return (<div key={deviceClass} className="space-y-2">
              ...
              <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <ul className="space-y-1 text-sm text-blue-700">
                  ...
                  {complianceMetrics.complianceRate >= 90 ? (<li>• ✅ Excelente nível de conformidade ANVISA</li>) : complianceMetrics.complianceRate >= 70 ? (<li>• ⚠️ Nível de conformidade adequado - monitorar</li>) : (<li>• 🚨 Nível de conformidade crítico - ação imediata necessária</li>)}
                </ul>
              </div>
            </div>
          </card_1.CardContent>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            ...
            return device.usageLog.map(function (usage) { return (__assign(__assign({}, usage), { deviceName: device.name, deviceId: device.id })); });
        })
            .sort(function (a, b) { return new Date(b.date).getTime() - new Date(a.date).getTime(); })
            .slice(0, 5)
            .map(function (usage, index) { return (<div key={"".concat(usage.deviceId, "-").concat(index)} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  <div className="space-y-1">
            ...
            {mockANVISADevices.every(function (device) { return device.usageLog.length === 0; }) && (<p className="text-sm text-muted-foreground text-center py-4">
