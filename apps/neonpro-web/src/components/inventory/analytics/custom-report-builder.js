...
function CustomReportBuilder() {
    var _this = this;
    var _a = (0, react_1.useState)(AVAILABLE_SECTIONS), sections = _a[0], setSections = _a[1];
    var _b = (0, react_1.useState)(AVAILABLE_METRICS), metrics = _b[0], setMetrics = _b[1];
    var _c = (0, react_1.useState)(), dateRange = _c[0], setDateRange = _c[1];
    var _d = (0, react_1.useState)('category'), groupBy = _d[0], setGroupBy = _d[1];
    var _e = (0, react_1.useState)('pdf'), format = _e[0], setFormat = _e[1];
    var _f = (0, react_1.useState)(''), reportTitle = _f[0], setReportTitle = _f[1];
    var _g = (0, react_1.useState)(''), reportDescription = _g[0], setReportDescription = _g[1];
    var _h = (0, react_1.useState)(''), selectedTemplate = _h[0], setSelectedTemplate = _h[1];
    var _j = (0, use_reports_1.useReports)(), generateCustomReport = _j.generateCustomReport, exportReport = _j.exportReport, isGenerating = _j.isGenerating;
    var handleSectionToggle = (0, react_1.useCallback)(function (sectionId) {
        setSections(function (prev) { return prev.map(function (section) {
            return section.id === sectionId
                ? __assign(__assign({}, section), { enabled: !section.enabled }) : section;
        }); });
    }, []);
    var handleMetricToggle = (0, react_1.useCallback)(function (metricId) {
        setMetrics(function (prev) { return prev.map(function (metric) {
            return metric.id === metricId
                ? __assign(__assign({}, metric), { enabled: !metric.enabled }) : metric;
        }); });
    }, []);
    var handleTemplateSelect = (0, react_1.useCallback)(function (templateId) {
        var template = REPORT_TEMPLATES.find(function (t) { return t.id === templateId; });
        if (!template)
            return;
        ...
        // Update sections
        setSections(function (prev) { return prev.map(function (section) { return (__assign(__assign({}, section), { enabled: template.sections.includes(section.id) })); }); });
        // Update metrics
        setMetrics(function (prev) { return prev.map(function (metric) { return (__assign(__assign({}, metric), { enabled: template.metrics.includes(metric.id) })); }); });
        setReportTitle(template.name);
        setReportDescription(template.description);
        react_hot_toast_1.toast.success("Template \"".concat(template.name, "\" aplicado!"));
    }, []);
    var handleGenerateReport = (0, react_1.useCallback)(function () { return __awaiter(_this, void 0, void 0, function () {
        var enabledSections, enabledMetrics, reportData, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    enabledSections = sections.filter(function (s) { return s.enabled; }).map(function (s) { return s.id; });
                    enabledMetrics = metrics.filter(function (m) { return m.enabled; }).map(function (m) { return m.id; });
                ...
                case 1:
                    _a.trys.push([1, 5, , 6]);
                    return [4 /*yield*/, generateCustomReport({
    ...
    var enabledSectionsCount = sections.filter(function (s) { return s.enabled; }).length;
    var enabledMetricsCount = metrics.filter(function (m) { return m.enabled; }).length;
    return (<div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {REPORT_TEMPLATES.map(function (template) { return (<card_1.Card key={template.id} className={"cursor-pointer transition-colors hover:bg-accent ".concat(selectedTemplate === template.id ? 'ring-2 ring-primary' : '')} onClick={function () { return handleTemplateSelect(template.id); }}>
                    ...
                    <card_1.CardContent className="pt-0">
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <badge_1.Badge variant="outline">{template.format.toUpperCase()}</badge_1.Badge>
                      </div>
                    </card_1.CardContent>
                  </card_1.Card>); })}
              </div>
            ...
            <card_1.CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input_1.Input id="report-title" value={reportTitle} onChange={function (e) { return setReportTitle(e.target.value); }} placeholder="Digite o título do relatório"/>
                </div>
                
                <div>
                  <label_1.Label htmlFor="format">Formato de Exportação</label_1.Label>
                  <select_1.Select value={format} onValueChange={function (value) { return setFormat(value); }}>
              ...
              <div>
                ...
                <textarea_1.Textarea id="report-description" value={reportDescription} onChange={function (e) { return setReportDescription(e.target.value); }} placeholder="Descrição opcional do relatório" rows={2}/>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select_1.Select value={groupBy} onValueChange={function (value) { return setGroupBy(value); }}>
                    <select_1.SelectTrigger>
                      <select_1.SelectValue />
                    </select_1.SelectTrigger>
                    <select_1.SelectContent>
          ...
          </card_1.Card>          {/* Sections Selection */}
          <card_1.Card>
            <card_1.CardHeader>
              <card_1.CardTitle className="flex items-center justify-between">
                <span>Seções do Relatório</span>
                <badge_1.Badge variant="secondary">{enabledSectionsCount} selecionadas</badge_1.Badge>
              </card_1.CardTitle>
              <card_1.CardDescription>
            ...
            return (<div key={section.id} className="flex items-center space-x-3">
                      <checkbox_1.Checkbox id={section.id} checked={section.enabled} onCheckedChange={function () { return handleSectionToggle(section.id); }}/>
                      <div className="flex items-center space-x-2 flex-1">
                        <Icon className="h-4 w-4 text-muted-foreground"/>
                        <div>
                          <label_1.Label htmlFor={section.id} className="text-sm font-medium cursor-pointer">
                            {section.title}
                          </label_1.Label>
          ...
          <card_1.Card>
            <card_1.CardHeader>
              <card_1.CardTitle className="flex items-center justify-between">
                ...
                <badge_1.Badge variant="secondary">{enabledMetricsCount} selecionadas</badge_1.Badge>
              </card_1.CardTitle>
              <card_1.CardDescription>
            ...
            <card_1.CardContent>
              <div className="space-y-3">
                {metrics.map(function (metric) { return (<div key={metric.id} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <checkbox_1.Checkbox id={metric.id} checked={metric.enabled} onCheckedChange={function () { return handleMetricToggle(metric.id); }}/>
                      <div>
                        <label_1.Label htmlFor={metric.id} className="text-sm font-medium cursor-pointer">
                          {metric.name}
                        </label_1.Label>
                        <p className="text-xs text-muted-foreground">
                          {metric.description}
        ...
        <div className="space-y-6">
            ...
            <card_1.CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {sections.filter(function (s) { return s.enabled; }).map(function (section) { return (<div key={section.id} className="flex items-center space-x-2">
                      <lucide_react_1.CheckCircle className="h-3 w-3 text-green-500"/>
                      <span className="text-xs">{section.title}</span>
                    </div>); })}
                  {enabledSectionsCount === 0 && (<p className="text-xs text-muted-foreground">Nenhuma seção selecionada</p>)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {metrics.filter(function (m) { return m.enabled; }).map(function (metric) { return (<div key={metric.id} className="flex items-center space-x-2">
                      <lucide_react_1.CheckCircle className="h-3 w-3 text-blue-500"/>
                      <span className="text-xs">{metric.name}</span>
                    </div>); })}
                  {enabledMetricsCount === 0 && (<p className="text-xs text-muted-foreground">Nenhuma métrica selecionada</p>)}
                </div>
              </div>
            </card_1.CardContent>
          </card_1.Card>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-4">
              <button_1.Button onClick={handleGenerateReport} disabled={isGenerating || enabledSectionsCount === 0} className="w-full">
                {isGenerating ? (<>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <badge_1.Badge variant={enabledSectionsCount + enabledMetricsCount > 8 ? "destructive" :
