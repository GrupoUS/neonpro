...
function StockAlerts() {
    var _a = (0, react_1.useState)('all'), selectedSeverity = _a[0], setSelectedSeverity = _a[1];
    var _b = (0, react_1.useState)('all'), selectedType = _b[0], setSelectedType = _b[1];
    var alertCounts = (0, react_1.useMemo)(function () {
        var unresolved = mockAlerts.filter(function (alert) { return !alert.resolvedAt; });
        return {
            total: unresolved.length,
            critical: unresolved.filter(function (alert) { return alert.severity === 'critical'; }).length,
            warning: unresolved.filter(function (alert) { return alert.severity === 'warning'; }).length,
            actionRequired: unresolved.filter(function (alert) { return alert.actionRequired; }).length
        };
    }, []);
    var filteredAlerts = (0, react_1.useMemo)(function () {
        return mockAlerts.filter(function (alert) {
            if (alert.resolvedAt)
                return false; // Only show unresolved alerts
    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-4">
        {filteredAlerts.map(function (alert) {
            ...
            return (<card_1.Card key={alert.id} className={"border-l-4 ".concat(alert.severity === 'critical' ? 'border-l-red-500' : alert.severity === 'warning' ? 'border-l-amber-500' : 'border-l-blue-500')}>
              <card_1.CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <badge_1.Badge variant="outline" className={getSeverityBadgeColor(alert.severity)}>
                  ...
                  <div className="flex items-center gap-2">
                    {alert.actionRequired && (<badge_1.Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">
                        Ação Necessária
                      </badge_1.Badge>)}
              ...
              <card_1.CardContent className="pt-0">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    {alert.batchNumber && (<div>
                        <span className="font-medium text-muted-foreground">Lote:</span>
                        <br />
                        {alert.batchNumber}
                      </div>)}
                    
                    {alert.anvisaRegistration && (<div>
                        <span className="font-medium text-muted-foreground">ANVISA:</span>
                        <br />
                        <badge_1.Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs">
                          <lucide_react_1.Shield className="w-3 h-3 mr-1"/>
                          {alert.anvisaRegistration}
                        </badge_1.Badge>
                      </div>)}
                    
                    {alert.temperatureReading && (<div>
                        <span className="font-medium text-muted-foreground">Temperatura:</span>
                        <br />
                        <badge_1.Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 text-xs">
                          <lucide_react_1.Thermometer className="w-3 h-3 mr-1"/>
                          {alert.temperatureReading}°C
                        </badge_1.Badge>
                        {alert.targetTemperatureRange && (<div className="text-xs text-muted-foreground mt-1">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-muted">
                    ...
                    {alert.type === 'anvisa_compliance' && (<alert_1.Alert className="max-w-md">
