...
export function ANVISACompliance() {
  ...
  const complianceMetrics = useMemo(() => {
    ...
    const overdueMaintenances = mockANVISADevices.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Manutenções agendadas para os próximos 30 dias</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {mockANVISADevices
                ...
                .map((device) => {
                  const daysUntil = getDaysUntilMaintenance(device.nextMaintenanceDate);
                  const maintenanceItem = device.maintenanceSchedule.find(
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Status geral de compliance ANVISA</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {/* Compliance by Class */}
              {Object.entries(deviceClassInfo).map(([deviceClass, info]) => {
                const devicesInClass = mockANVISADevices.filter(
                  (d) => d.deviceClass === deviceClass,
                );
                const compliantInClass = devicesInClass.filter((d) => d.compliance.current);
                const classComplianceRate =
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Histórico de uso para auditoria e controle de qualidade</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {mockANVISADevices
