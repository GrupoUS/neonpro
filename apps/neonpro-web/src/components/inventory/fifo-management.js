...
function FIFOManagement(_a) {
    ...
    var _b = (0, react_1.useState)([]), fifoAnalysis = _b[0], setFifoAnalysis = _b[1];
    var _c = (0, react_1.useState)([]), expiryAlerts = _c[0], setExpiryAlerts = _c[1];
    var _d = (0, react_1.useState)(true), isLoading = _d[0], setIsLoading = _d[1];
    var _e = (0, react_1.useState)(null), selectedProductId = _e[0], setSelectedProductId = _e[1];
    var toast = (0, use_toast_1.useToast)().toast;
    var fifoManager = new inventory_1.FIFOManager();
    (0, react_1.useEffect)(function () {
        loadFIFOData();
    }, []);
    var loadFIFOData = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 3:
                    error_1 = _c.sent();
                    errorMessage = error_1 instanceof Error ? error_1.message : 'Erro ao carregar dados FIFO';
                    toast({
                        title: 'Erro',
    ...
    var handleBlockExpiringBatches = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    error_2 = _b.sent();
                    errorMessage = error_2 instanceof Error ? error_2.message : 'Erro ao bloquear lotes vencidos';
                    toast({
                        title: 'Erro',
                        description: errorMessage,
                        variant: 'destructive',
    ...
    var handleExecuteTransfer = function (alert) { return __awaiter(_this, void 0, void 0, function () {
        var _a, success, error, error_3, errorMessage;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fifoManager.executeBatchTransfer({
                            ...
                            motivo: "Transfer\u00EAncia autom\u00E1tica - produto vencendo em ".concat(alert.dias_para_vencer, " dias"),
                ...
                case 2:
                    error_3 = _b.sent();
                    errorMessage = error_3 instanceof Error ? error_3.message : 'Erro ao executar transferência';
                    toast({
                        title: 'Erro',
                        description: errorMessage,
                        variant: 'destructive',
    ...
    var getUrgencyIcon = function (urgency) {
        switch (urgency) {
            case 'critica': return <icons_1.Icons.AlertTriangle className="h-4 w-4 text-red-500"/>;
            case 'alta': return <icons_1.Icons.AlertCircle className="h-4 w-4 text-orange-500"/>;
            case 'media': return <icons_1.Icons.Info className="h-4 w-4 text-yellow-500"/>;
            default: return <icons_1.Icons.CheckCircle className="h-4 w-4 text-green-500"/>;
        }
    };
    ...
    var criticalAlerts = expiryAlerts.filter(function (alert) { return alert.prioridade === 'critica'; }).length;
    var totalWasteValue = expiryAlerts.reduce(function (sum, alert) { return sum + alert.valor_estimado; }, 0);
    var fifoComplianceScore = fifoAnalysis.length > 0
        ? Math.round(fifoAnalysis.reduce(function (sum, analysis) { return sum + (analysis.economia_fifo / 100); }, 0) / fifoAnalysis.length * 100)
        : 0;
    return (<div className={"space-y-6 ".concat(className)}>
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button_1.Button variant="destructive" onClick={handleBlockExpiringBatches} disabled={criticalAlerts === 0}>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <div className="flex items-center gap-2 mt-2">
              {criticalAlerts > 0 && (<badge_1.Badge variant="destructive">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold text-green-600">
              {formatCurrency(fifoAnalysis.reduce(function (sum, analysis) { return sum + analysis.desperdicioEvitado; }, 0))}
            </div>
      ...
      {criticalAlerts > 0 && (<alert_1.Alert variant="destructive">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {expiryAlerts.length > 0 ? (<table_1.Table>
              <table_1.TableHeader>
                <table_1.TableRow>
              ...
              <table_1.TableBody>
                {expiryAlerts.map(function (alert) { return (<table_1.TableRow key={alert.id}>
                    <table_1.TableCell className="font-medium">{alert.nome_produto}</table_1.TableCell>
                    ...
                    <table_1.TableCell>
                      <div className="flex gap-1">
                        <button_1.Button size="sm" variant="outline" onClick={function () { return handleExecuteTransfer(alert); }}>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {fifoAnalysis.length > 0 ? (<div className="space-y-6">
              {fifoAnalysis.slice(0, 5).map(function (analysis) { return (<div key={analysis.produto_id} className="border rounded-lg p-4">
                  ...
                  {/* Recommendations */}
                  {analysis.recomendacoes.length > 0 && (<div className="space-y-2">
                      <h4 className="font-medium text-sm">Recomendações:</h4>
                      {analysis.recomendacoes.slice(0, 3).map(function (rec, index) { return (<div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded text-sm">
                          <div className="flex items-center gap-2">
              ...
              {fifoAnalysis.length > 5 && (<div className="text-center">
