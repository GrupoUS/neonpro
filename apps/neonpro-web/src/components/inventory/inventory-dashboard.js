...
function InventoryDashboard(_a) {
    ...
    var _c = (0, react_1.useState)("items"), activeTab = _c[0], setActiveTab = _c[1];
    var _d = (0, react_1.useState)(false), showScanner = _d[0], setShowScanner = _d[1];
    var _e = (0, react_1.useState)("lookup"), scanMode = _e[0], setScanMode = _e[1];
    var _f = (0, react_1.useState)(null), selectedItem = _f[0], setSelectedItem = _f[1];
    var _g = (0, use_inventory_1.useInventory)({
        enableRealTime: true,
        autoLoadData: true,
    }), state = _g.state, isLoading = _g.isLoading, isUpdating = _g.isUpdating, loadInventoryItems = _g.loadInventoryItems, loadCategories = _g.loadCategories, loadLocations = _g.loadLocations, loadAlerts = _g.loadAlerts, refreshData = _g.refreshData, updateStock = _g.updateStock, scanBarcode = _g.scanBarcode, setLocationFilter = _g.setLocationFilter, setCategoryFilter = _g.setCategoryFilter, setSearchQuery = _g.setSearchQuery, connectionStatus = _g.connectionStatus, reconnect = _g.reconnect;
    // Handle barcode scan
    var handleBarcodeScan = function (result) { return __awaiter(_this, void 0, void 0, function () {
        var scanResult;
    ...
    var metrics = react_1.default.useMemo(function () {
        ...
        var totalValue = state.items.reduce(function (sum, item) { return sum + item.unit_cost * item.current_stock; }, 0);
        var activeAlerts = state.alerts.filter(function (alert) { return alert.status === "active"; }).length;
        return {
            totalItems: totalItems,
            lowStockItems: lowStockItems,
            totalValue: totalValue,
            activeAlerts: activeAlerts,
            stockPercentage: totalItems > 0
    ...
    return (<div className={"inventory-dashboard ".concat(className)}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Inventário</h1>
          <p className="text-gray-500">
            Gestão em tempo real do estoque e produtos
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Connection Status */}
          <div className={"flex items-center gap-1 text-sm ".concat(getConnectionStatusColor(connectionStatus))}>
          ...
          <dialog_1.Dialog open={showScanner} onOpenChange={setShowScanner}>
            ...
            <dialog_1.DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                <select_1.Select value={scanMode} onValueChange={function (value) { return setScanMode(value); }}>
                ...
                <barcode_scanner_1.BarcodeScanner onScan={handleBarcodeScan} continuous={true} showHistory={true} autoStart={true}/>
              </div>
      ...
      <card_1.Card className="mb-6">
        <card_1.CardContent className="p-6">
          ...
          <p className="text-sm text-gray-600 mt-2">
            {metrics.totalItems - metrics.lowStockItems} de {metrics.totalItems}{" "}
            itens com estoque adequado
          </p>
        </card_1.CardContent>
      </card_1.Card>

      {/* Active Alerts */}
      {state.alerts.length > 0 && (<card_1.Card className="mb-6">
          <card_1.CardHeader>
          ...
          <card_1.CardContent>
            <div className="space-y-2">
              {state.alerts.slice(0, 5).map(function (alert) { return (<alert_1.Alert key={alert.id} variant="destructive">
              ...
              {state.alerts.length > 5 && (<p className="text-sm text-gray-500 text-center">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <div className="flex flex-wrap gap-2">
                <div className="flex-1 min-w-64">
                  <input_1.Input placeholder="Buscar itens..." value={state.searchQuery || ""} onChange={function (e) { return setSearchQuery(e.target.value); }} className="w-full"/>
                </div>

                <select_1.Select value={state.selectedLocation || ""} onValueChange={setLocationFilter}>
                  ...
                  <select_1.SelectContent>
                    ...
                    {state.locations.map(function (location) { return (<select_1.SelectItem key={location.id} value={location.id}>
                        {location.name}
                      </select_1.SelectItem>); })}
                  </select_1.SelectContent>
                </select_1.Select>

                <select_1.Select value={state.selectedCategory || ""} onValueChange={setCategoryFilter}>
                  <select_1.SelectTrigger className="w-48">
                    <select_1.SelectValue placeholder="Filtrar por categoria"/>
                  </select_1.SelectTrigger>
                  <select_1.SelectContent>
                    <select_1.SelectItem value="">Todas as categorias</select_1.SelectItem>
                    {state.categories.map(function (category) { return (<select_1.SelectItem key={category.id} value={category.id}>
                        {category.name}
                      </select_1.SelectItem>); })}
                  </select_1.SelectContent>
                </select_1.Select>
            ...
            <card_1.CardContent>
              {isLoading ? (<div className="flex items-center justify-center py-8">
                  ...
                  {state.items.map(function (item) { return (<card_1.Card key={item.id} className="hover:shadow-md transition-shadow">
                      <card_1.CardContent className="p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="font-semibold truncate">
                            {item.name}
                          </h4>
                          <div className={"w-3 h-3 rounded-full ".concat(getStatusColor(item.status))}/>
                        </div>
                        ...
                        {item.barcode && (<div className="mt-2 text-xs text-gray-500 font-mono">
                            {item.barcode}
                          </div>)}
                      </card_1.CardContent>
                    </card_1.Card>); })}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {state.locations.map(function (location) { return (<card_1.Card key={location.id}>
                    <card_1.CardContent className="p-4">
        ...
        <tabs_1.TabsContent value="categories">
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {state.categories.map(function (category) { return (<card_1.Card key={category.id}>
                    <card_1.CardContent className="p-4">
