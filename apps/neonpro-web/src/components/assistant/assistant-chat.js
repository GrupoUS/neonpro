...
function AssistantChat(_a) {
    var _this = this;
    var conversationId = _a.conversationId, onConversationChange = _a.onConversationChange;
    var scrollAreaRef = (0, react_1.useRef)(null);
    var _b = (0, react_1.useState)('gpt4'), selectedModel = _b[0], setSelectedModel = _b[1];
    var _c = (0, react_2.useChat)({
        ...
        onResponse: function (response) { return __awaiter(_this, void 0, void 0, function () {
            var error;
    ...
    }), messages = _c.messages, input = _c.input, handleInputChange = _c.handleInputChange, handleSubmit = _c.handleSubmit, isLoading = _c.isLoading, setMessages = _c.setMessages;
    ...
    var loadConversationMessages = function (convId) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    data = _a.sent();
                    formattedMessages = data.messages.map(function (msg) { return ({
                        id: msg.id,
                        role: msg.role,
                        content: msg.content,
      ...
      <card_1.CardContent className="flex-1 flex flex-col p-0">
        ...
        <scroll_area_1.ScrollArea ref={scrollAreaRef} className="flex-1 px-4">
          <div className="space-y-4 py-4">
            {messages.length === 0 ? (<div className="text-center text-muted-foreground py-8">
              ...
              </div>) : (messages.map(function (message) { return (<div key={message.id} className={"flex gap-3 ".concat(message.role === 'user' ? 'justify-end' : 'justify-start')}>
                  {message.role === 'assistant' && (<avatar_1.Avatar className="h-8 w-8 mt-1">
                      <avatar_1.AvatarFallback className="bg-blue-100 text-blue-600">
                        <lucide_react_1.Bot className="h-4 w-4"/>
                      </avatar_1.AvatarFallback>
                    </avatar_1.Avatar>)}
                  
                  <div className={"max-w-[80%] rounded-lg px-4 py-2 ".concat(message.role === 'user'
                ...
                : 'bg-gray-100 text-gray-900')}>
                  ...
                  {message.role === 'user' && (<avatar_1.Avatar className="h-8 w-8 mt-1">
        ...
        <div className="border-t p-4">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input_1.Input value={input} onChange={handleInputChange} placeholder="Digite sua mensagem..." disabled={isLoading} className="flex-1" autoFocus/>
            <button_1.Button type="submit" disabled={isLoading || !input.trim()} size="sm" className="px-3">
