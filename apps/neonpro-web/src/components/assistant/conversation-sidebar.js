...
function ConversationSidebar(_a) {
    var _this = this;
    var selectedConversationId = _a.selectedConversationId, onSelectConversation = _a.onSelectConversation, onNewConversation = _a.onNewConversation;
    ...
    var createNewConversation = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    data_1 = _a.sent();
                    setConversations(function (prev) { return __spreadArray([data_1.conversation], prev, true); });
    ...
    var deleteConversation = function (conversationId) { return __awaiter(_this, void 0, void 0, function () {
        var response, error_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/assistant/conversations/".concat(conversationId), {
                            method: 'DELETE',
                        })];
                case 1:
                    response = _a.sent();
    ...
    return (<card_1.Card className="h-full flex flex-col">
      ...
      <card_1.CardContent className="flex-1 p-0">
        <scroll_area_1.ScrollArea className="h-full px-3">
          {loading ? (<div className="space-y-3 py-4">
              {__spreadArray([], Array(5), true).map(function (_, i) { return (<div key={i} className="bg-gray-100 rounded-lg h-16 animate-pulse"/>); })}
            </div>) : conversations.length === 0 ? (<div className="text-center text-muted-foreground py-8">
              <lucide_react_1.MessageSquare className="h-8 w-8 mx-auto mb-2"/>
              <p className="text-sm">Nenhuma conversa ainda</p>
              <p className="text-xs">Clique em + para começar</p>
            </div>) : (<div className="space-y-2 py-3">
              {conversations.map(function (conversation) { return (<div key={conversation.id} className={"group p-3 rounded-lg cursor-pointer transition-colors border ".concat(selectedConversationId === conversation.id
                    ? 'bg-blue-50 border-blue-200'
                    : 'hover:bg-gray-50 border-transparent')} onClick={function () { return onSelectConversation(conversation.id); }}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <badge_1.Badge variant="secondary" className={"text-xs ".concat(getModelBadgeColor(conversation.model_used))}>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {(0, date_fns_1.formatDistanceToNow)(new Date(conversation.updated_at), {
                    addSuffix: true,
                    ...
                    <dropdown_menu_1.DropdownMenu>
                      <dropdown_menu_1.DropdownMenuTrigger asChild>
                        <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={function (e) { return e.stopPropagation(); }}>
