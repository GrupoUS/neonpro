...
function SessionSecurityDemo() {
    var _this = this;
    var sessionId = (0, react_1.useState)(function () { return "session_".concat(Date.now(), "_").concat(Math.random().toString(36).substr(2, 9)); })[0];
    var _a = (0, react_1.useState)(0), activityCount = _a[0], setActivityCount = _a[1];
    // Use session security hooks
    var _b = (0, useSessionSecurity_1.useSessionSecurity)(sessionId), isSecurityActive = _b.isSecurityActive, securityStatus = _b.securityStatus, updateActivity = _b.updateActivity, extendSession = _b.extendSession, terminateSession = _b.terminateSession, securityError = _b.error;
    var _c = (0, csrf_protection_hooks_1.useCSRFToken)(sessionId), csrfToken = _c.csrfToken, csrfLoading = _c.isLoading, csrfError = _c.error, refreshToken = _c.refreshToken;
    var _d = (0, useSessionSecurity_1.useSessionTimeout)(sessionId), timeRemaining = _d.timeRemaining, warningLevel = _d.warningLevel, isExpired = _d.isExpired, extendTimeout = _d.extendTimeout, timeoutError = _d.error;
    // Simulate user activity
    var simulateActivity = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, updateActivity('user_interaction', {
                ...
                case 1:
                    _a.sent();
                    setActivityCount(function (prev) { return prev + 1; });
                    return [2 /*return*/];
            }
        });
    }); };
    ...
    return (<div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-2">
              {isSecurityActive ? (<lucide_react_1.CheckCircle className="h-5 w-5 text-green-500"/>) : (<lucide_react_1.XCircle className="h-5 w-5 text-red-500"/>)}
          ...
          {securityStatus && (<div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <h4 className="font-medium mb-2">Detalhes de Segurança:</h4>
              <div className="text-sm space-y-1">
                <p>Risk Score: <badge_1.Badge variant="outline">{securityStatus.riskScore || 'N/A'}</badge_1.Badge></p>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {csrfLoading ? (<badge_1.Badge variant="secondary">Carregando...</badge_1.Badge>) : csrfToken ? (<badge_1.Badge variant="default">{csrfToken.slice(0, 16)}...</badge_1.Badge>) : (<badge_1.Badge variant="destructive">Não disponível</badge_1.Badge>)}
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button_1.Button onClick={function () { return extendTimeout(30); }} disabled={isExpired} variant="outline">
                Estender 30min
              </button_1.Button>
              <button_1.Button onClick={function () { return extendSession(60); }} disabled={isExpired} variant="outline">
                Estender 1h
              </button_1.Button>
            </div>
            
            {timeoutError && (<alert_1.Alert>
                <lucide_react_1.AlertTriangle className="h-4 w-4"/>
                <alert_1.AlertDescription>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            <div className="flex gap-2">
              <button_1.Button onClick={simulateActivity}>
                Simular Atividade
              </button_1.Button>
