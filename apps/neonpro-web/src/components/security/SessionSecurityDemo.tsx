    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Monitoramento em tempo real da segurança da sessão</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {securityStatus && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div className="text-sm space-y-1">
                <p>
                  Risk Score: <Badge variant="outline">{securityStatus.riskScore || "N/A"}</Badge>
                </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Proteção contra ataques Cross-Site Request Forgery</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            {csrfError && (
              <Alert>
                ...
                <AlertDescription>Erro no CSRF: {csrfError}</AlertDescription>
              </Alert>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Controle automático de expiração de sessão</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {warningLevel && (
            ...
            {isExpired && (
              <Alert>
                ...
                <AlertDescription>Sessão expirada! Faça login novamente.</AlertDescription>
              </Alert>
            )}

            <div className="flex gap-2">
              <Button onClick={() => extendTimeout(30)} disabled={isExpired} variant="outline">
                Estender 30min
              </Button>
              <Button onClick={() => extendSession(60)} disabled={isExpired} variant="outline">
                Estender 1h
              </Button>
            </div>
            ...
            {timeoutError && (
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>Erro no timeout: {timeoutError}</AlertDescription>
              </Alert>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Teste as funcionalidades de rastreamento de atividade</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex gap-2">
              <Button onClick={simulateActivity}>Simular Atividade</Button>
              <Button onClick={terminateSession} variant="destructive">
                Terminar Sessão
              </Button>
            </div>
            ...
            {securityError && (
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>Erro de segurança: {securityError}</AlertDescription>
              </Alert>
            )}
          </div>
        </CardContent>
