...
function PatientProfileView(_a) {
    var _this = this;
    var patientId = _a.patientId, onProfileUpdate = _a.onProfileUpdate;
    ...
    var fetchPatientProfile = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        <card_1.CardHeader>
          <div className="flex items-start justify-between">
            ...
            .toUpperCase()}
              ...
              <div>
                <card_1.CardTitle className="text-2xl">{profile.demographics.name}</card_1.CardTitle>
            ...
            <div className="text-right space-y-2">
              {profile.risk_level && (<badge_1.Badge className={getRiskBadgeColor(profile.risk_level)}>
      ...
      <tabs_1.Tabs defaultValue="overview" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <card_1.Card>
              <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <card_1.CardTitle className="text-sm font-medium">Vital Statistics</card_1.CardTitle>
                <lucide_react_1.Activity className="h-4 w-4 text-muted-foreground"/>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2 text-sm">
                  {profile.bmi && (<div className="flex justify-between">
                      <span>BMI:</span>
                      <span className="font-medium">{profile.bmi}</span>
                    </div>)}
                  {profile.blood_type && (<div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {profile.risk_score ? "".concat(Math.round(profile.risk_score * 100), "%") : 'N/A'}
                    </span>
                  </div>
                </div>
              </card_1.CardContent>
            ...
            <card_1.Card>
              <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <card_1.CardTitle className="text-sm font-medium">Chronic Conditions</card_1.CardTitle>
                <lucide_react_1.Heart className="h-4 w-4 text-muted-foreground"/>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2">
                  {profile.chronic_conditions.length > 0 ? (profile.chronic_conditions.map(function (condition, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                        {condition}
                      </badge_1.Badge>); })) : (<p className="text-sm text-gray-500">No chronic conditions recorded</p>)}
                </div>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-2">
                  {profile.allergies.length > 0 ? (profile.allergies.map(function (allergy, index) { return (<badge_1.Badge key={index} variant="destructive" className="text-xs">
                        {allergy}
                      </badge_1.Badge>); })) : (<p className="text-sm text-gray-500">No known allergies</p>)}
                </div>
              </card_1.CardContent>
            </card_1.Card>
          </div>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-3">
                {profile.medical_history.length > 0 ? (profile.medical_history.map(function (event, index) { return (<div key={index} className="p-3 border rounded-lg">
                      <p className="text-sm">{event}</p>
                    </div>); })) : (<p className="text-gray-500">No medical history recorded</p>)}
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </tabs_1.TabsContent>
        ...
        <tabs_1.TabsContent value="medications" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-3">
                {profile.current_medications.length > 0 ? (profile.current_medications.map(function (medication, index) { return (<div key={index} className="flex justify-between items-center p-3 border rounded-lg">
                      <div>
                        <p className="font-medium">{medication.name}</p>
                        <p className="text-sm text-gray-600">{medication.dosage} • {medication.frequency}</p>
                      </div>
                    </div>); })) : (<p className="text-gray-500">No current medications</p>)}
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </tabs_1.TabsContent>
        ...
        <tabs_1.TabsContent value="contacts" className="space-y-4">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>People to contact in case of emergency</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-3">
                {profile.emergency_contacts && profile.emergency_contacts.length > 0 ? (profile.emergency_contacts.map(function (contact, index) { return (<div key={index} className="p-3 border rounded-lg">
                    ...
                    </div>); })) : (<p className="text-gray-500">No emergency contacts recorded</p>)}
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </tabs_1.TabsContent>
        ...
        <tabs_1.TabsContent value="insights" className="space-y-4">
          <card_1.Card>
            ...
            <card_1.CardContent>
              {profile.ai_insights ? (<div className="space-y-4">
                  <div className="text-center">
                    <button_1.Button onClick={function () { return window.location.href = "/patients/".concat(patientId, "/insights"); }}>
                  ...
                  <button_1.Button className="mt-4" onClick={function () {
                // Generate insights
                fetch("/api/patients/".concat(patientId, "/insights"))
                    .then(function () { return fetchPatientProfile(); });
            }}>
