...
function QuickAccess(_a) {
    ...
    var _b = (0, react_1.useState)('recent'), activeTab = _b[0], setActiveTab = _b[1];
    var _c = (0, react_1.useState)({}), data = _c[0], setData = _c[1];
    var _d = (0, react_1.useState)(false), isLoading = _d[0], setIsLoading = _d[1];
    // Load quick access data
    var loadQuickAccessData = function (type) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, fetch("/api/patients/integration/quick-access?type=".concat(type, "&limit=20"))];
                case 2:
                    response = _a.sent();
    ...
    var toggleFavorite = function (patientId, currentStatus) { return __awaiter(_this, void 0, void 0, function () {
        var response, result, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    return [4 /*yield*/, fetch('/api/patients/integration/quick-access', {
                            method: 'POST',
                            headers: {
                        ...
                        setData(function (prev) {
                            ...
                            Object.keys(updated).forEach(function (key) {
                                ...
                                if ((_a = updated[listType]) === null || _a === void 0 ? void 0 : _a.patients) {
                                    updated[listType] = __assign(__assign({}, updated[listType]), { patients: updated[listType].patients.map(function (p) {
                                            return p.id === patientId ? __assign(__assign({}, p), { isFavorite: !currentStatus }) : p;
                                        }) });
                                }
                            });
                            return updated;
                        });
                        sonner_1.toast.success(currentStatus
    ...
    var getTabIcon = function (type) {
        switch (type) {
            case 'recent': return <lucide_react_1.Clock className="h-4 w-4"/>;
            case 'favorites': return <lucide_react_1.Star className="h-4 w-4"/>;
            case 'high-risk': return <lucide_react_1.AlertTriangle className="h-4 w-4"/>;
            case 'upcoming-appointments': return <lucide_react_1.Calendar className="h-4 w-4"/>;
            case 'pending-verification': return <lucide_react_1.Shield className="h-4 w-4"/>;
            case 'frequent': return <lucide_react_1.TrendingUp className="h-4 w-4"/>;
            default: return null;
        }
    };
    var getTabLabel = function (type) {
        switch (type) {
            ...
            case 'high-risk': return 'Alto Risco';
            case 'upcoming-appointments': return 'Próximas Consultas';
            case 'pending-verification': return 'Verificação Pendente';
            case 'frequent': return 'Frequentes';
            default: return type;
        }
    };
    var renderPatientCard = function (patient) { return (<card_1.Card key={patient.id} className="hover:shadow-md transition-shadow cursor-pointer">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3">
            <avatar_1.Avatar className="h-10 w-10">
              ...
              <avatar_1.AvatarFallback>
                {patient.name.split(' ').map(function (n) { return n[0]; }).join('').toUpperCase()}
              </avatar_1.AvatarFallback>
            </avatar_1.Avatar>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2">
                ...
                <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={function (e) {
            e.stopPropagation();
            toggleFavorite(patient.id, patient.isFavorite);
        }}>
                  <lucide_react_1.Star className={"h-3 w-3 ".concat(patient.isFavorite
            ...
            : 'text-gray-400')}/>
              ...
              <div className="text-xs text-gray-500 space-y-1">
                <div>{patient.age} anos • {patient.gender === 'male' ? 'M' : 'F'}</div>
            ...
            patient.riskLevel === 'medium' ? 'Médio' : 'Baixo'}
                ...
                {activeTab === 'pending-verification' && (<badge_1.Badge className={getVerificationStatusColor(patient.verificationStatus)} variant="secondary">
                    {patient.verificationStatus === 'verified' ? 'Verificado' :
                patient.verificationStatus === 'pending' ? 'Pendente' : 'Falhou'}
                  </badge_1.Badge>)}
                
                {patient.hasPhotos && (<badge_1.Badge variant="outline" className="text-xs">
                    📸
                  </badge_1.Badge>)}
              </div>
            </div>
          ...
          <div className="text-right">
            <div className="text-xs text-gray-500">
              {activeTab === 'recent' && (<span>
                  Última visita: {(0, date_fns_1.formatDistanceToNow)(new Date(patient.lastVisit), {
                addSuffix: true,
                locale: locale_1.ptBR
            })}
                </span>)}
              
              {activeTab === 'upcoming-appointments' && patient.nextAppointment && (<span>
                  Próxima: {(0, date_fns_1.formatDistanceToNow)(new Date(patient.nextAppointment), {
                addSuffix: true,
                locale: locale_1.ptBR
            })}
                </span>)}
            ...
            <button_1.Button variant="outline" size="sm" className="mt-2" onClick={function () { return onPatientSelect === null || onPatientSelect === void 0 ? void 0 : onPatientSelect(patient); }}>
              Ver Perfil
            </button_1.Button>
          </div>
        </div>
    ...
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={function (value) { return setActiveTab(value); }}>
        ...
        {/* Tab Contents */}
        {['recent', 'favorites', 'high-risk', 'upcoming-appointments', 'pending-verification', 'frequent'].map(function (type) { return (<tabs_1.TabsContent key={type} value={type} className="space-y-4">
            {isLoading && !currentData ? (<div className="text-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
                <p className="mt-2 text-gray-500">Carregando pacientes...</p>
              </div>) : (currentData === null || currentData === void 0 ? void 0 : currentData.patients.length) > 0 ? (<>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">
                    {currentData.totalCount} pacientes • Atualizado {(0, date_fns_1.formatDistanceToNow)(new Date(currentData.lastUpdated), {
                    addSuffix: true,
                    locale: locale_1.ptBR
                })}
                  </span>
