...
function EnhancedPatientProfile(_a) {
    var _this = this;
    var _b;
    var patient = _a.patient, onPatientUpdate = _a.onPatientUpdate, onClose = _a.onClose;
    var _c = (0, react_1.useState)(false), loading = _c[0], setLoading = _c[1];
    var _d = (0, react_1.useState)([]), timelineEvents = _d[0], setTimelineEvents = _d[1];
    var _e = (0, react_1.useState)('overview'), activeTab = _e[0], setActiveTab = _e[1];
    var supabase = (0, client_1.createClient)();
    // Load timeline events
    (0, react_1.useEffect)(function () {
        var loadTimelineEvents = function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 2:
                        ...
                        if (error)
                            throw error;
                        eventsWithStaff = (data === null || data === void 0 ? void 0 : data.map(function (event) {
                            var _a, _b;
                            return (__assign(__assign({}, event), { staff_name: ((_b = (_a = event.profiles) === null || _a === void 0 ? void 0 : _a.raw_user_meta_data) === null || _b === void 0 ? void 0 : _b.full_name) || 'Sistema' }));
                        })) || [];
    ...
    var getBMICategory = function (bmi) {
        if (bmi < 18.5)
            return { category: 'Abaixo do peso', color: 'text-blue-600' };
        if (bmi < 25)
            return { category: 'Peso normal', color: 'text-green-600' };
        if (bmi < 30)
            return { category: 'Sobrepeso', color: 'text-yellow-600' };
    ...
    var getEventTypeIcon = function (type) {
        switch (type) {
            case 'appointment': return <lucide_react_1.Calendar className="h-4 w-4"/>;
            case 'treatment': return <lucide_react_1.Stethoscope className="h-4 w-4"/>;
            case 'procedure': return <lucide_react_1.Activity className="h-4 w-4"/>;
            case 'diagnosis': return <lucide_react_1.FileText className="h-4 w-4"/>;
            case 'medication': return <lucide_react_1.Heart className="h-4 w-4"/>;
            case 'test_result': return <lucide_react_1.TrendingUp className="h-4 w-4"/>;
            case 'follow_up': return <lucide_react_1.Clock className="h-4 w-4"/>;
    ...
    var renderOverview = function () {
        ...
        <card_1.CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            <avatar_1.Avatar className="h-16 w-16">
              <avatar_1.AvatarImage src={((_a = patient.raw_user_meta_data) === null || _a === void 0 ? void 0 : _a.profile_picture) ||
                ((_c = (_b = patient.patient_photos) === null || _b === void 0 ? void 0 : _b.find(function (p) { return p.is_primary; })) === null || _c === void 0 ? void 0 : _c.photo_url)}/>
              <avatar_1.AvatarFallback className="text-lg">
                {((_e = (_d = patient.raw_user_meta_data) === null || _d === void 0 ? void 0 : _d.full_name) === null || _e === void 0 ? void 0 : _e.charAt(0)) || 'P'}
              </avatar_1.AvatarFallback>
            </avatar_1.Avatar>
            <div>
              <h3 className="font-semibold text-lg">
                {((_f = patient.raw_user_meta_data) === null || _f === void 0 ? void 0 : _f.full_name) || 'Nome não informado'}
              </h3>
              <p className="text-muted-foreground">
                {((_g = patient.raw_user_meta_data) === null || _g === void 0 ? void 0 : _g.date_of_birth)
                ? "".concat(calculateAge(patient.raw_user_meta_data.date_of_birth), " anos")
                : 'Idade não informada'}
                {((_h = patient.raw_user_meta_data) === null || _h === void 0 ? void 0 : _h.gender) &&
                " \u2022 ".concat(patient.raw_user_meta_data.gender === 'male' ? 'Masculino' :
          ...
          <div className="space-y-2">
            {patient.phone && (<div className="flex items-center gap-2 text-sm">
          ...
          {((_j = patient.patient_profiles_extended) === null || _j === void 0 ? void 0 : _j.emergency_contact) && (<div className="border-t pt-3">
      ...
      <card_1.Card className="lg:col-span-1">
        ...
        <card_1.CardContent className="space-y-4">
          {/* Risk Assessment */}
          {((_k = patient.patient_profiles_extended) === null || _k === void 0 ? void 0 : _k.risk_level) && (<div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Nível de Risco</span>
                <badge_1.Badge className={getRiskColor(patient.patient_profiles_extended.risk_level)} variant="outline">
              ...
              {patient.patient_profiles_extended.risk_score && (<div className="flex items-center gap-2">
                  <progress_1.Progress value={patient.patient_profiles_extended.risk_score * 100} className="flex-1"/>
          ...
          {/* BMI */}
          {((_l = patient.patient_profiles_extended) === null || _l === void 0 ? void 0 : _l.height_cm) && ((_m = patient.patient_profiles_extended) === null || _m === void 0 ? void 0 : _m.weight_kg) && (<div className="space-y-2">
              <div className="grid grid-cols-3 gap-2 text-sm">
                ...
                <div>
                  ...
                  {(function () {
                    var bmi = calculateBMI(patient.patient_profiles_extended.height_cm, patient.patient_profiles_extended.weight_kg);
                    if (bmi) {
                        var category = getBMICategory(parseFloat(bmi));
                        return (<div>
                          <p className="font-medium">{bmi}</p>
                          <p className={"text-xs ".concat(category.color)}>{category.category}</p>
                        </div>);
                    }
          ...
          {/* Blood Type */}
          {((_o = patient.patient_profiles_extended) === null || _o === void 0 ? void 0 : _o.blood_type) && (<div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Tipo Sanguíneo</span>
              <badge_1.Badge variant="outline">{patient.patient_profiles_extended.blood_type}</badge_1.Badge>
            </div>)}

          {/* Profile Completeness */}
          {((_p = patient.patient_profiles_extended) === null || _p === void 0 ? void 0 : _p.profile_completeness_score) && (<div className="space-y-2">
              <div className="flex justify-between items-center">
              ...
              <progress_1.Progress value={patient.patient_profiles_extended.profile_completeness_score * 100} className="w-full"/>
            </div>)}
        </card_1.CardContent>
      </card_1.Card>
      ...
      <card_1.Card className="lg:col-span-1">
        ...
        <card_1.CardContent>
          {timelineEvents.length > 0 ? (<div className="space-y-3">
              {timelineEvents.slice(0, 5).map(function (event) { return (<div key={event.id} className="flex items-start gap-3">
                  <div className={"\n                    flex items-center justify-center w-8 h-8 rounded-full \n                    ".concat(getEventTypeColor(event.event_type), "\n                  ")}>
              ...
              {timelineEvents.length > 5 && (<button_1.Button variant="ghost" size="sm" className="w-full" onClick={function () { return setActiveTab('timeline'); }}>
    ...
    var renderMedicalInfo = function () {
      ...
      <card_1.Card>
        ...
        <card_1.CardContent className="space-y-4">
          ...
          <div>
            ...
            {((_b = (_a = patient.patient_profiles_extended) === null || _a === void 0 ? void 0 : _a.chronic_conditions) === null || _b === void 0 ? void 0 : _b.length) ? (<div className="flex flex-wrap gap-1">
                {patient.patient_profiles_extended.chronic_conditions.map(function (condition, index) { return (<badge_1.Badge key={index} variant="outline" className="text-xs">
                    {condition}
                  </badge_1.Badge>); })}
              </div>) : (<p className="text-sm text-muted-foreground">Nenhuma condição crônica registrada</p>)}
          </div>

          {/* Allergies */}
          <div>
            <h4 className="font-medium text-sm mb-2">Alergias</h4>
            {((_d = (_c = patient.patient_profiles_extended) === null || _c === void 0 ? void 0 : _c.allergies) === null || _d === void 0 ? void 0 : _d.length) ? (<div className="flex flex-wrap gap-1">
                {patient.patient_profiles_extended.allergies.map(function (allergy, index) { return (<badge_1.Badge key={index} variant="destructive" className="text-xs">
                    {allergy}
                  </badge_1.Badge>); })}
              </div>) : (<p className="text-sm text-muted-foreground">Nenhuma alergia registrada</p>)}
          </div>

          {/* Medications */}
          <div>
            <h4 className="font-medium text-sm mb-2">Medicações</h4>
            {((_f = (_e = patient.patient_profiles_extended) === null || _e === void 0 ? void 0 : _e.medications) === null || _f === void 0 ? void 0 : _f.length) ? (<div className="space-y-2">
                {patient.patient_profiles_extended.medications.map(function (medication, index) { return (<div key={index} className="flex items-center gap-2 text-sm">
                    <div className="w-2 h-2 bg-blue-500 rounded-full"/>
                    {medication}
                  </div>); })}
              </div>) : (<p className="text-sm text-muted-foreground">Nenhuma medicação registrada</p>)}
          </div>
        </card_1.CardContent>
      </card_1.Card>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          {((_g = patient.patient_profiles_extended) === null || _g === void 0 ? void 0 : _g.ai_insights) ? (<div className="space-y-3">
              ...
              {patient.patient_profiles_extended.last_assessment_date && (<div className="text-xs text-muted-foreground">
    ...
    var renderTimeline = function () { return (<card_1.Card>
      <card_1.CardHeader>
        <card_1.CardTitle className="flex items-center gap-2">
          <lucide_react_1.Clock className="h-5 w-5"/>
      ...
      <card_1.CardContent>
        {timelineEvents.length > 0 ? (<div className="space-y-6">
            {timelineEvents.map(function (event, index) { return (<div key={event.id} className="relative flex gap-4">
                {/* Timeline Line */}
                {index < timelineEvents.length - 1 && (<div className="absolute left-6 top-12 w-px h-16 bg-border"/>)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {event.description && (<p className="text-sm text-muted-foreground mt-2">
                      {event.description}
                    </p>)}

                  {event.notes && (<div className="mt-2 p-3 bg-gray-50 rounded-lg">
                      <h5 className="font-medium text-xs text-gray-700 mb-1">Observações</h5>
                      <p className="text-sm text-gray-600">{event.notes}</p>
                    </div>)}

                  {event.outcome_score && (<div className="mt-2 flex items-center gap-2">
                      <span className="text-xs text-muted-foreground">Resultado:</span>
                      <div className="flex items-center gap-1">
                        {event.outcome_score >= 0.8 ? (<lucide_react_1.TrendingUp className="h-4 w-4 text-green-500"/>) : event.outcome_score >= 0.5 ? (<lucide_react_1.Minus className="h-4 w-4 text-yellow-500"/>) : (<lucide_react_1.TrendingDown className="h-4 w-4 text-red-500"/>)}
                        <span className="text-sm font-medium">
                  ...
                  {event.follow_up_required && (<div className="mt-2 p-2 bg-orange-50 border border-orange-200 rounded-lg">
                      <div className="flex items-center gap-2 text-orange-800">
                        <lucide_react_1.Bell className="h-4 w-4"/>
                        <span className="text-sm font-medium">Retorno necessário</span>
                      </div>
                      {event.follow_up_date && (<p className="text-sm text-orange-600 mt-1">
                          Data sugerida: {new Date(event.follow_up_date).toLocaleDateString('pt-BR')}
                        </p>)}
                    </div>)}

                  {event.photos && event.photos.length > 0 && (<div className="mt-3">
                      <h5 className="font-medium text-xs text-gray-700 mb-2">Fotos</h5>
                      <div className="flex gap-2">
                        {event.photos.slice(0, 3).map(function (photoId, photoIndex) { return (<div key={photoIndex} className="w-16 h-16 bg-gray-100 rounded-lg cursor-pointer hover:bg-gray-200 transition-colors" onClick={function () { return setSelectedPhoto(photoId); }}>
                            <lucide_react_1.Camera className="w-full h-full p-4 text-gray-400"/>
                          </div>); })}
                        {event.photos.length > 3 && (<div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center">
    ...
    var renderPhotos = function () { return (<photo_upload_system_1.PhotoUploadSystem patientId={patient.id} onPhotosUploaded={function (photos) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">
            {((_b = patient.raw_user_meta_data) === null || _b === void 0 ? void 0 : _b.full_name) || 'Paciente'}
          </h2>
          <p className="text-muted-foreground">
