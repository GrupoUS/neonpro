...
function MedicalTimeline(_a) {
    var _this = this;
    var patientId = _a.patientId, className = _a.className;
    var _b = (0, react_1.useState)([]), events = _b[0], setEvents = _b[1];
    var _c = (0, react_1.useState)([]), milestones = _c[0], setMilestones = _c[1];
    var _d = (0, react_1.useState)(true), loading = _d[0], setLoading = _d[1];
    var _e = (0, react_1.useState)({}), filter = _e[0], setFilter = _e[1];
    var _f = (0, react_1.useState)("timeline"), activeTab = _f[0], setActiveTab = _f[1];
    ...
    var loadTimeline = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    setLoading(true);
                    params = new URLSearchParams(__assign(__assign(__assign(__assign(__assign(__assign(__assign(__assign({ patientId: patientId, action: "timeline" }, (filter.eventTypes && { eventTypes: filter.eventTypes.join(",") })), (filter.categories && { categories: filter.categories.join(",") })), (((_a = filter.dateRange) === null || _a === void 0 ? void 0 : _a.start) && {
                    ...
                    })), (filter.severity && { severity: filter.severity.join(",") })), (filter.includePhotos && { includePhotos: "true" })), (filter.includeAttachments && { includeAttachments: "true" })));
    ...
    var loadMilestones = function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var toggleEventExpansion = function (eventId) {
        setEvents(events.map(function (event) {
            return event.id === eventId ? __assign(__assign({}, event), { expanded: !event.expanded }) : event;
        }));
    };
    ...
    var formatRelativeDate = function (date) {
        ...
        if (days === 1)
            return "Ontem";
        if (days < 7)
            return "".concat(days, " dias atr\u00E1s");
        if (days < 30)
            return "".concat(Math.floor(days / 7), " semanas atr\u00E1s");
        if (days < 365)
            return "".concat(Math.floor(days / 30), " meses atr\u00E1s");
    ...
    var renderTimelineEvent = function (event, index) {
        ...
        <div className="relative flex items-start space-x-4">
          <div className={(0, utils_1.cn)("flex items-center justify-center w-12 h-12 rounded-full border-4 border-white shadow-sm", getSeverityColor(event.severity) || "bg-blue-500")}>
            <div className="text-white">{getEventIcon(event.eventType)}</div>
          </div>

          {/* Event content */}
          <div className="flex-1 min-w-0 pb-8">
            <card_1.Card className={(0, utils_1.cn)("cursor-pointer transition-all hover:shadow-md", event.expanded && "shadow-lg")} onClick={function () { return toggleEventExpansion(event.id); }}>
              <card_1.CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {event.severity && (<badge_1.Badge variant="outline" className="text-xs">
                        {event.severity}
                      </badge_1.Badge>)}
                  </div>
                </div>
              ...
              {event.expanded && (<card_1.CardContent className="space-y-4">
                  {/* Description */}
                  ...
                  {event.beforeAfterPhotos &&
                    event.beforeAfterPhotos.length > 0 && (<div>
                        <h4 className="text-sm font-medium mb-2">
                          Fotos Antes/Depois
                        </h4>
                        <div className="grid grid-cols-2 gap-4">
                          {event.beforeAfterPhotos.map(function (photo) { return (<div key={photo.id} className="space-y-2">
                              <div className="grid grid-cols-2 gap-2">
                                {photo.beforePhoto && (<div className="space-y-1">
                                    <p className="text-xs text-gray-500">
                                      Antes
                                    </p>
                                    <img src={photo.beforePhoto.thumbnailUrl} alt="Antes" className="w-full h-24 object-cover rounded"/>
                                  </div>)}
                                {photo.afterPhoto && (<div className="space-y-1">
                                    <p className="text-xs text-gray-500">
                                      Depois
                                    </p>
                                    <img src={photo.afterPhoto.thumbnailUrl} alt="Depois" className="w-full h-24 object-cover rounded"/>
                                  </div>)}
                              </div>
                              {photo.notes && (<p className="text-xs text-gray-600">
                                  {photo.notes}
                                </p>)}
                            </div>); })}
                        </div>
                  ...
                  {event.outcome && (<div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          {event.outcome.success ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-500"/>) : (<lucide_react_1.AlertCircle className="h-4 w-4 text-red-500"/>)}
                        ...
                        {event.outcome.patientFeedback && (<div>
                            <p className="text-xs text-gray-500">
                        ...
                        {event.outcome.professionalAssessment && (<div>
                            <p className="text-xs text-gray-500">
                  ...
                  {/* Progress Notes */}
                  {event.notes && event.notes.length > 0 && (<div>
                      <h4 className="text-sm font-medium mb-2">Observações</h4>
                      <div className="space-y-2">
                        {event.notes.map(function (note) { return (<div key={note.id} className="border-l-2 border-gray-200 pl-3">
                  ...
                  {/* Attachments */}
                  {event.attachments && event.attachments.length > 0 && (<div>
                      <h4 className="text-sm font-medium mb-2">Anexos</h4>
                      <div className="space-y-1">
                        {event.attachments.map(function (attachment) { return (<div key={attachment.id} className="flex items-center space-x-2 text-sm">
    ...
    var renderMilestones = function () {
        ...
        {milestones.map(function (tracking) { return (<card_1.Card key={tracking.id}>
            <card_1.CardHeader>
              <div className="flex items-center justify-between">
                <div>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {tracking.milestones.map(function (milestone) { return (<div key={milestone.id} className="flex items-start space-x-4">
                    <div className="flex-shrink-0">
                      {milestone.status === "completed" ? (<lucide_react_1.CheckCircle className="h-5 w-5 text-green-500"/>) : milestone.status === "in_progress" ? (<lucide_react_1.Timer className="h-5 w-5 text-blue-500"/>) : (<lucide_react_1.Clock className="h-5 w-5 text-gray-400"/>)}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium">{milestone.title}</h4>
                        <badge_1.Badge variant={milestone.status === "completed"
                        ? "default"
    ...
    return (<div className={(0, utils_1.cn)("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <button_1.Button variant="outline" size="sm" onClick={function () { return setShowFilters(!showFilters); }}>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <tabs_1.TabsContent value="timeline" className="space-y-4">
          {loading ? (<div className="flex items-center justify-center py-8">
            ...
            </div>) : events.length === 0 ? (<card_1.Card>
            ...
            </card_1.Card>) : (<div className="space-y-0">
              {events.map(function (event, index) { return renderTimelineEvent(event, index); })}
            </div>)}
        </tabs_1.TabsContent>

        <tabs_1.TabsContent value="milestones">
          {milestones.length === 0 ? (<card_1.Card>
              <card_1.CardContent className="flex items-center justify-center py-8">
                <div className="text-center">
