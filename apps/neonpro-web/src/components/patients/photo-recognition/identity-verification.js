...
function IdentityVerification(_a) {
    var _this = this;
    var patientId = _a.patientId, patientName = _a.patientName, onVerificationComplete = _a.onVerificationComplete, onVerificationError = _a.onVerificationError, _b = _a.requiredConfidence, requiredConfidence = _b === void 0 ? 0.8 : _b;
    var _c = (0, react_1.useState)(null), selectedFile = _c[0], setSelectedFile = _c[1];
    var _d = (0, react_1.useState)('identity_check'), verificationContext = _d[0], setVerificationContext = _d[1];
    var _e = (0, react_1.useState)(false), isVerifying = _e[0], setIsVerifying = _e[1];
    var _f = (0, react_1.useState)(0), verificationProgress = _f[0], setVerificationProgress = _f[1];
    var _g = (0, react_1.useState)(null), previewUrl = _g[0], setPreviewUrl = _g[1];
    var _h = (0, react_1.useState)(null), verificationResult = _h[0], setVerificationResult = _h[1];
    var _j = (0, react_1.useState)(false), isUsingCamera = _j[0], setIsUsingCamera = _j[1];
    var _k = (0, react_1.useState)(null), stream = _k[0], setStream = _k[1];
    ...
    var verifyIdentity = function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    progressInterval = setInterval(function () {
                        setVerificationProgress(function (prev) { return Math.min(prev + 15, 90); });
                    }, 300);
                    return [4 /*yield*/, fetch('/api/patients/photos/verify', {
                            method: 'POST',
                            headers: {
                ...
                case 5:
                    ...
                    onVerificationComplete === null || onVerificationComplete === void 0 ? void 0 : onVerificationComplete(result.data);
                    return [3 /*break*/, 8];
                case 6:
                    error_2 = _a.sent();
                    ...
                    onVerificationError === null || onVerificationError === void 0 ? void 0 : onVerificationError(errorMessage);
                    return [3 /*break*/, 8];
                case 7:
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <select_1.Select value={verificationContext} onValueChange={setVerificationContext}>
            ...
            <select_1.SelectContent>
              {VERIFICATION_CONTEXTS.map(function (context) { return (<select_1.SelectItem key={context.value} value={context.value}>
                  {context.label}
                </select_1.SelectItem>); })}
            </select_1.SelectContent>
          </select_1.Select>
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <button_1.Button type="button" variant={isUsingCamera ? 'default' : 'outline'} onClick={isUsingCamera ? stopCamera : startCamera} className="flex-1">
              <lucide_react_1.Camera className="h-4 w-4 mr-2"/>
              {isUsingCamera ? 'Parar Câmera' : 'Usar Câmera'}
            </button_1.Button>
            <button_1.Button type="button" variant="outline" onClick={function () { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }} className="flex-1">
              <lucide_react_1.Eye className="h-4 w-4 mr-2"/>
              Selecionar Arquivo
            </button_1.Button>
          </div>

          <input ref={fileInputRef} type="file" accept="image/jpeg,image/jpg,image/png,image/webp" onChange={handleFileInputChange} className="hidden"/>
        </div>

        {/* Camera View */}
        {isUsingCamera && (<div className="space-y-4">
        ...
        {/* Preview */}
        {previewUrl && !isUsingCamera && (<div className="space-y-2">
            <label_1.Label>Foto para Verificação</label_1.Label>
            <div className="relative max-w-md mx-auto">
              <img src={previewUrl} alt="Verification photo" className="w-full h-auto rounded-lg shadow-md"/>
            </div>
          </div>)}

        {/* Verification Progress */}
        {isVerifying && (<div className="space-y-2">
        ...
        {/* Verification Result */}
        {verificationResult && (<alert_1.Alert className={verificationResult.verified ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
            {verificationResult.verified ? (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-600"/>) : (<lucide_react_1.AlertTriangle className="h-4 w-4 text-red-600"/>)}
            <alert_1.AlertDescription>
              <div className="space-y-3">
                ...
                {verificationResult.matchedPhotos.length > 0 && (<div>
                    <p className="text-sm font-medium mb-2">Fotos correspondentes:</p>
                    <div className="space-y-1">
                      {verificationResult.matchedPhotos.map(function (photo, index) { return (<div key={photo.id} className="flex items-center justify-between text-sm">
                          <span>{photo.type} - {new Date(photo.uploadedAt).toLocaleDateString()}</span>
                          <badge_1.Badge variant="outline" className={getConfidenceColor(photo.confidence)}>
                ...
                {verificationResult.recommendations.length > 0 && (<div>
                    <p className="text-sm font-medium mb-1">Recomendações:</p>
                    <ul className="text-sm list-disc list-inside space-y-1">
                      {verificationResult.recommendations.map(function (rec, index) { return (<li key={index}>{rec}</li>); })}
                    </ul>
                  </div>)}
              </div>
            </alert_1.AlertDescription>
        ...
        <div className="flex gap-3">
          <button_1.Button onClick={verifyIdentity} disabled={!selectedFile || isVerifying} className="flex-1" variant={(verificationResult === null || verificationResult === void 0 ? void 0 : verificationResult.verified) ? 'default' : 'default'}>
            {isVerifying ? (<>
          ...
          {selectedFile && (<button_1.Button type="button" variant="outline" onClick={resetVerification} disabled={isVerifying}>
