...
function PatientCard(_a) {
    ...
    var patient = _a.patient, selected = _a.selected, onSelect = _a.onSelect, onAction = _a.onAction;
    // Utility functions
    ...
    return (<card_1.Card className={"relative transition-all duration-200 hover:shadow-md ".concat(selected ? 'ring-2 ring-blue-500 bg-blue-50' : '')}>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {calculateAge(patient.raw_user_meta_data.date_of_birth)} anos • {patient.raw_user_meta_data.gender === 'male' ? 'Masculino' :
          ...
          <dropdown_menu_1.DropdownMenu>
            ...
            <dropdown_menu_1.DropdownMenuContent align="end">
              ...
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onAction('view', patient.id); }}>
                <lucide_react_1.Eye className="mr-2 h-4 w-4"/>
                Ver Detalhes
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onAction('schedule', patient.id); }}>
                <lucide_react_1.Calendar className="mr-2 h-4 w-4"/>
                Agendar Consulta
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onAction('edit', patient.id); }}>
                <lucide_react_1.Edit className="mr-2 h-4 w-4"/>
                Editar
              </dropdown_menu_1.DropdownMenuItem>
              <dropdown_menu_1.DropdownMenuSeparator />
              <dropdown_menu_1.DropdownMenuItem onClick={function () { return onAction('archive', patient.id); }} className="text-red-600">
      ...
      <card_1.CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          {getRiskLevelBadge((_b = patient.patient_profiles_extended) === null || _b === void 0 ? void 0 : _b.risk_level)}
        </div>
        ...
        {/* Medical Alerts */}
        {(((_d = (_c = patient.patient_profiles_extended) === null || _c === void 0 ? void 0 : _c.chronic_conditions) === null || _d === void 0 ? void 0 : _d.length) > 0 ||
            ((_f = (_e = patient.patient_profiles_extended) === null || _e === void 0 ? void 0 : _e.allergies) === null || _f === void 0 ? void 0 : _f.length) > 0) && (<div className="space-y-1">
            <h4 className="text-xs font-medium text-gray-700">Alertas Médicos</h4>
            {(_g = patient.patient_profiles_extended.chronic_conditions) === null || _g === void 0 ? void 0 : _g.map(function (condition, index) { return (<div key={index} className="flex items-center text-xs text-red-600">
                <lucide_react_1.Heart className="h-3 w-3 mr-1"/>
                {condition}
              </div>); })}
            {(_h = patient.patient_profiles_extended.allergies) === null || _h === void 0 ? void 0 : _h.map(function (allergy, index) { return (<div key={index} className="flex items-center text-xs text-orange-600">
        ...
        {((_j = patient.patient_profiles_extended) === null || _j === void 0 ? void 0 : _j.profile_completeness_score) && (<div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium">Perfil Completo</span>
              <span className={"text-xs font-medium ".concat(getProfileCompletenessColor(patient.patient_profiles_extended.profile_completeness_score))}>
                {Math.round(patient.patient_profiles_extended.profile_completeness_score * 100)}%
              </span>
            </div>
            <progress_1.Progress value={patient.patient_profiles_extended.profile_completeness_score * 100} className="h-2"/>
          </div>)}
        ...
        <div className="flex space-x-2">
          <button_1.Button size="sm" variant="outline" className="flex-1 text-xs h-8" onClick={function () { return onAction('view', patient.id); }}>
            <lucide_react_1.Eye className="h-3 w-3 mr-1"/>
            Ver
          </button_1.Button>
          <button_1.Button size="sm" variant="outline" className="flex-1 text-xs h-8" onClick={function () { return onAction('schedule', patient.id); }}>
            <lucide_react_1.Calendar className="h-3 w-3 mr-1"/>
            Agendar
          </button_1.Button>
          <button_1.Button size="sm" variant="outline" className="flex-1 text-xs h-8" onClick={function () { return onAction('edit', patient.id); }}>
        ...
        {/* Last Assessment */}
        {((_k = patient.patient_profiles_extended) === null || _k === void 0 ? void 0 : _k.last_assessment_date) && (<div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t">
        ...
        {((_l = patient.patient_profiles_extended) === null || _l === void 0 ? void 0 : _l.risk_score) !== undefined && (<div className="flex items-center justify-between text-xs">
            <span className="text-muted-foreground">Score de Risco</span>
            <span className={"font-medium ".concat(getRiskLevelColor(patient.patient_profiles_extended.risk_level))}>
