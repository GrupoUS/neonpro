...
function DuplicateManagerClassic(_a) {
    var _this = this;
    var duplicates = _a.duplicates, onMerge = _a.onMerge, onDismiss = _a.onDismiss;
    ...
    var handleMerge = react_1.default.useCallback(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.trys.push([1, , 4, 5]);
                    group = duplicates.find(function (g) { return g.id === selectedGroup; });
                    if (!group) return [3 /*break*/, 3];
                    secondaryIds = group.patients
                        .filter(function (p) { return p.id !== selectedPrimary; })
    ...
    return (<div className="space-y-6">
      ...
      {duplicates.map(function (group) { return (<card_1.Card key={group.id} className="border-amber-200">
          <card_1.CardHeader>
          ...
          <card_1.CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {group.patients.map(function (patient) { return (<div key={patient.id} className={"p-4 border rounded-lg cursor-pointer transition-colors ".concat(selectedPrimary === patient.id
                    ? "border-blue-500 bg-blue-50"
                    : "border-gray-200 hover:border-gray-300")} onClick={function () { return setSelectedPrimary(patient.id); }}>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium">{patient.name}</h4>
                    {group.suggestedPrimary === patient.id && (<badge_1.Badge variant="default" className="text-xs">
                        Suggested Primary
                      </badge_1.Badge>)}
                  </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t">
              <button_1.Button variant="outline" onClick={function () { return onDismiss(group.id); }} disabled={processing}>
                Not a duplicate
              </button_1.Button>

              <div className="flex items-center gap-2">
                {selectedPrimary && (<div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                handleMerge();
            }} disabled={!selectedPrimary || processing} className="bg-blue-600 hover:bg-blue-700">
