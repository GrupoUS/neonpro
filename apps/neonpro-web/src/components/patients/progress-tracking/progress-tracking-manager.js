...
function ProgressTrackingManager() {
    var _a = (0, react_1.useState)([]), progressEntries = _a[0], setProgressEntries = _a[1];
    var _b = (0, react_1.useState)([]), comparisons = _b[0], setComparisons = _b[1];
    var _c = (0, react_1.useState)(null), selectedEntry = _c[0], setSelectedEntry = _c[1];
    var _d = (0, react_1.useState)(false), isAddingEntry = _d[0], setIsAddingEntry = _d[1];
    var _e = (0, react_1.useState)('timeline'), activeTab = _e[0], setActiveTab = _e[1];
    ...
    var _k = (0, react_1.useState)({
        ...
        consentGiven: true,
        anonymizationLevel: 'none'
    }), newEntry = _k[0], setNewEntry = _k[1];
    ...
    var filteredEntries = progressEntries.filter(function (entry) {
        ...
        var matchesSearch = searchTerm === '' ||
            ...
            ((_a = entry.description) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    var getEntryIcon = function (type) {
        switch (type) {
            case 'photo': return <lucide_react_1.Camera className="h-4 w-4"/>;
            case 'video': return <lucide_react_1.Video className="h-4 w-4"/>;
            case 'audio': return <lucide_react_1.Mic className="h-4 w-4"/>;
            case 'measurement': return <lucide_react_1.BarChart3 className="h-4 w-4"/>;
            case 'note': return <lucide_react_1.FileText className="h-4 w-4"/>;
    ...
    var getStatusIcon = function (status) {
        switch (status) {
            ...
            case 'active': return <lucide_react_1.Activity className="h-4 w-4 text-blue-500"/>;
          ...
          <dialog_1.Dialog open={isAddingEntry} onOpenChange={setIsAddingEntry}>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select_1.Select value={newEntry.entryType} onValueChange={function (value) {
            return setNewEntry(__assign(__assign({}, newEntry), { entryType: value }));
                  ...
                  <div className="space-y-2">
                    ...
                    <select_1.Select value={newEntry.category} onValueChange={function (value) {
            return setNewEntry(__assign(__assign({}, newEntry), { category: value }));
                ...
                <div className="space-y-2">
                  ...
                  <input_1.Input id="title" value={newEntry.title} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { title: e.target.value })); }} placeholder="Enter entry title"/>
                </div>
                
                <div className="space-y-2">
                  <label_1.Label htmlFor="description">Description</label_1.Label>
                  <textarea_1.Textarea id="description" value={newEntry.description} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { description: e.target.value })); }} placeholder="Enter detailed description" rows={3}/>
                </div>
                
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <label_1.Label htmlFor="pain-level">Pain Level (0-10)</label_1.Label>
                    <input_1.Input id="pain-level" type="number" min="0" max="10" value={newEntry.painLevel} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { painLevel: parseInt(e.target.value) })); }}/>
                  </div>
                  <div className="space-y-2">
                    <label_1.Label htmlFor="satisfaction">Satisfaction (0-10)</label_1.Label>
                    <input_1.Input id="satisfaction" type="number" min="0" max="10" value={newEntry.satisfactionLevel} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { satisfactionLevel: parseInt(e.target.value) })); }}/>
                  </div>
                  <div className="space-y-2">
                    <label_1.Label htmlFor="progress">Progress %</label_1.Label>
                    <input_1.Input id="progress" type="number" min="0" max="100" value={newEntry.progressPercentage} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { progressPercentage: parseInt(e.target.value) })); }}/>
                  </div>
                </div>
                
                <div className="space-y-2">
                  <label_1.Label htmlFor="clinical-notes">Clinical Notes</label_1.Label>
                  <textarea_1.Textarea id="clinical-notes" value={newEntry.clinicalNotes} onChange={function (e) { return setNewEntry(__assign(__assign({}, newEntry), { clinicalNotes: e.target.value })); }} placeholder="Enter clinical observations and notes" rows={4}/>
                </div>
                
                <div className="flex justify-end space-x-2">
                  <button_1.Button variant="outline" onClick={function () { return setIsAddingEntry(false); }}>
                    Cancel
                  </button_1.Button>
                  <button_1.Button onClick={handleAddEntry}>
                    Add Entry
                  </button_1.Button>
                </div>
              </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input_1.Input placeholder="Search entries..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-8"/>
              </div>
            </div>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <tabs_1.TabsContent value="timeline" className="space-y-4">
          <div className="grid gap-4">
            {filteredEntries.map(function (entry) { return (<card_1.Card key={entry.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={function () { return setSelectedEntry(entry); }}>
                <card_1.CardContent className="pt-6">
                  ...
                  {entry.progressPercentage > 0 && (<div className="mt-4">
                  ...
                  {entry.tags.length > 0 && (<div className="flex items-center space-x-2 mt-3">
                      {entry.tags.map(function (tag) { return (<badge_1.Badge key={tag} variant="secondary" className="text-xs">
                          {tag}
                        </badge_1.Badge>); })}
                    </div>)}
                </card_1.CardContent>
        ...
        <tabs_1.TabsContent value="gallery" className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredEntries
            .filter(function (entry) { return entry.entryType === 'photo' || entry.entryType === 'video'; })
            .map(function (entry) { return (<card_1.Card key={entry.id} className="cursor-pointer hover:shadow-md transition-shadow">
                  <card_1.CardContent className="p-4">
                    <div className="aspect-square bg-gray-100 rounded-lg mb-3 flex items-center justify-center">
                      {entry.entryType === 'photo' ? (<lucide_react_1.Image className="h-8 w-8 text-gray-400"/>) : (<lucide_react_1.Video className="h-8 w-8 text-gray-400"/>)}
                    </div>
        ...
        <tabs_1.TabsContent value="comparisons" className="space-y-4">
          <div className="grid gap-4">
            {comparisons.map(function (comparison) { return (<card_1.Card key={comparison.id}>
                <card_1.CardHeader>
                ...
                <card_1.CardContent>
                  ...
                  <div className="mt-6 space-y-4">
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <progress_1.Progress value={comparison.metrics.improvementPercentage} className="flex-1"/>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {comparison.metrics.significantChanges.map(function (change, index) { return (<li key={index} className="flex items-center">
                      ...
                      <div>
                        ...
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {comparison.metrics.recommendations.map(function (rec, index) { return (<li key={index} className="flex items-center">
      ...
      {selectedEntry && (<dialog_1.Dialog open={!!selectedEntry} onOpenChange={function () { return setSelectedEntry(null); }}>
          <dialog_1.DialogContent className="max-w-4xl">
            ...
            <div className="space-y-6">
              ...
              {selectedEntry.clinicalNotes && (<div>
              ...
              {selectedEntry.nextSteps.length > 0 && (<div>
                  <label_1.Label className="text-sm font-medium">Next Steps</label_1.Label>
                  <ul className="text-sm text-muted-foreground mt-1 space-y-1">
                    {selectedEntry.nextSteps.map(function (step, index) { return (<li key={index} className="flex items-center">
