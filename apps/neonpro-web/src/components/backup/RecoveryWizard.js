...
var RecoveryWizard = function (_a) {
    ...
    var _c = (0, react_1.useState)([]), backups = _c[0], setBackups = _c[1];
    ...
    var _e = (0, react_1.useState)({
        ...
        files_to_restore: [],
    }), recoveryOptions = _e[0], setRecoveryOptions = _e[1];
    var _f = (0, react_1.useState)([]), availableFiles = _f[0], setAvailableFiles = _f[1];
    var _g = (0, react_1.useState)(false), loading = _g[0], setLoading = _g[1];
    var _h = (0, react_1.useState)(false), recoveryInProgress = _h[0], setRecoveryInProgress = _h[1];
    var steps = [
        {
    ...
    var loadAvailableBackups = function () { return __awaiter(void 0, void 0, void 0, function () {
    ...
    var loadBackupFiles = function (backupId) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    var handleRecoveryTypeChange = function (type) {
        setRecoveryOptions(__assign(__assign({}, recoveryOptions), { type: type, files_to_restore: type === 'PARTIAL_RESTORE' ? [] : undefined, point_in_time: type === 'POINT_IN_TIME' ? new Date().toISOString() : undefined }));
    };
    var handleFileToggle = function (file, checked) {
        var currentFiles = recoveryOptions.files_to_restore || [];
        if (checked) {
            setRecoveryOptions(__assign(__assign({}, recoveryOptions), { files_to_restore: __spreadArray(__spreadArray([], currentFiles, true), [file], false) }));
        }
        else {
            setRecoveryOptions(__assign(__assign({}, recoveryOptions), { files_to_restore: currentFiles.filter(function (f) { return f !== file; }) }));
        }
    };
    var startRecovery = function () { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fetch('/api/backup/recovery', {
                            method: 'POST',
                            headers: {
        ...
        switch (currentStep) {
            ...
            case 1:
                if (recoveryOptions.type === 'PARTIAL_RESTORE') {
                    return (((_a = recoveryOptions.files_to_restore) === null || _a === void 0 ? void 0 : _a.length) || 0) > 0;
                }
                return true;
            case 2:
                return true;
    ...
    var renderStepContent = function () {
        ...
        switch (currentStep) {
            ...
            {loading ? (<div className="text-center py-8">
              ...
              </div>) : backups.length === 0 ? (<alert_1.Alert>
                <lucide_react_1.AlertTriangle className="h-4 w-4"/>
                <alert_1.AlertDescription>
                  Nenhum backup válido encontrado para restauração.
                </alert_1.AlertDescription>
              </alert_1.Alert>) : (<div className="space-y-2 max-h-64 overflow-y-auto">
                {backups.map(function (backup) { return (<card_1.Card key={backup.id} className={"cursor-pointer transition-colors ".concat((selectedBackup === null || selectedBackup === void 0 ? void 0 : selectedBackup.id) === backup.id
                                ? 'ring-2 ring-primary bg-accent'
                                : 'hover:bg-accent')} onClick={function () { return handleBackupSelect(backup); }}>
                    <card_1.CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {backup.size && (<div className="text-sm text-muted-foreground mt-1">
            ...
            <div>
              ...
              <select_1.Select value={recoveryOptions.type} onValueChange={handleRecoveryTypeChange}>
            ...
            {recoveryOptions.type === 'PARTIAL_RESTORE' && (<div>
                <label_1.Label>Arquivos para Restaurar</label_1.Label>
                <div className="mt-2 max-h-48 overflow-y-auto border rounded-md p-3">
                  {availableFiles.length === 0 ? (<p className="text-sm text-muted-foreground">
                      Carregando lista de arquivos...
                    </p>) : (<div className="space-y-2">
                      {availableFiles.map(function (file) { return (<div key={file} className="flex items-center space-x-2">
                          <checkbox_1.Checkbox id={file} checked={(recoveryOptions.files_to_restore || []).includes(file)} onCheckedChange={function (checked) { return handleFileToggle(file, checked); }}/>
            ...
            {recoveryOptions.type === 'POINT_IN_TIME' && (<div>
                <label_1.Label htmlFor="point-in-time">Data e Hora</label_1.Label>
                <input_1.Input id="point-in-time" type="datetime-local" value={(_a = recoveryOptions.point_in_time) === null || _a === void 0 ? void 0 : _a.split('.')[0]} onChange={function (e) { return setRecoveryOptions(__assign(__assign({}, recoveryOptions), { point_in_time: e.target.value + '.000Z' })); }} className="mt-2"/>
              </div>)}

            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <checkbox_1.Checkbox id="overwrite" checked={recoveryOptions.overwrite_existing} onCheckedChange={function (checked) { return setRecoveryOptions(__assign(__assign({}, recoveryOptions), { overwrite_existing: checked })); }}/>
                <label_1.Label htmlFor="overwrite">
                  Sobrescrever arquivos existentes
                </label_1.Label>
              </div>

              <div className="flex items-center space-x-2">
                <checkbox_1.Checkbox id="verify" checked={recoveryOptions.verify_integrity} onCheckedChange={function (checked) { return setRecoveryOptions(__assign(__assign({}, recoveryOptions), { verify_integrity: checked })); }}/>
                <label_1.Label htmlFor="verify">
            ...
            <div>
              ...
              <input_1.Input id="email" type="email" value={recoveryOptions.notification_email || ''} onChange={function (e) { return setRecoveryOptions(__assign(__assign({}, recoveryOptions), { notification_email: e.target.value })); }} placeholder="seu@email.com" className="mt-2"/>
            </div>
          </div>);
            case 2:
                return (<div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 text-sm text-muted-foreground">
                  {selectedBackup === null || selectedBackup === void 0 ? void 0 : selectedBackup.config_name} - {(0, utils_1.formatDate)(new Date((selectedBackup === null || selectedBackup === void 0 ? void 0 : selectedBackup.start_time) || ''))}
                </div>
              </div>
              ...
              {recoveryOptions.type === 'PARTIAL_RESTORE' && (<div>
                  <label_1.Label className="text-sm font-medium">
                    Arquivos Selecionados ({((_b = recoveryOptions.files_to_restore) === null || _b === void 0 ? void 0 : _b.length) || 0})
                  </label_1.Label>
                  <div className="mt-1 max-h-32 overflow-y-auto">
                    {(_c = recoveryOptions.files_to_restore) === null || _c === void 0 ? void 0 : _c.map(function (file) { return (<div key={file} className="text-sm text-muted-foreground">
                        {file}
                      </div>); })}
                  </div>
                </div>)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label_1.Label className="text-sm font-medium">Sobrescrever Existentes</label_1.Label>
                  <div className="mt-1 text-sm text-muted-foreground">
                    {recoveryOptions.overwrite_existing ? 'Sim' : 'Não'}
                  </div>
                </div>
                <div>
                  <label_1.Label className="text-sm font-medium">Verificar Integridade</label_1.Label>
                  <div className="mt-1 text-sm text-muted-foreground">
            ...
            <div className="mx-auto w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center">
              {recoveryInProgress ? (<lucide_react_1.RefreshCw className="h-8 w-8 text-primary animate-spin"/>) : (<lucide_react_1.CheckCircle className="h-8 w-8 text-green-500"/>)}
            </div>
    ...
    return (<dialog_1.Dialog open={isOpen} onOpenChange={!recoveryInProgress ? handleClose : undefined}>
      <dialog_1.DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          <stepper_1.Stepper value={currentStep} className="w-full">
            {steps.map(function (step, index) { return (<stepper_1.StepperItem key={index} value={index}>
        ...
        <dialog_1.DialogFooter>
          {currentStep > 0 && currentStep < 3 && (<button_1.Button variant="outline" onClick={function () { return setCurrentStep(currentStep - 1); }}>
              <lucide_react_1.ArrowLeft className="h-4 w-4 mr-2"/>
              Anterior
            </button_1.Button>)}
          
          {currentStep < 2 && (<button_1.Button onClick={function () { return setCurrentStep(currentStep + 1); }} disabled={!canProceedToNext()}>
              Próximo
              <lucide_react_1.ArrowRight className="h-4 w-4 ml-2"/>
            </button_1.Button>)}
          
          {currentStep === 2 && (<button_1.Button onClick={startRecovery} disabled={recoveryInProgress}>
              <lucide_react_1.Download className="h-4 w-4 mr-2"/>
              Iniciar Recuperação
            </button_1.Button>)}
          
          {currentStep === 3 && !recoveryInProgress && (<button_1.Button onClick={handleClose}>
              Fechar
            </button_1.Button>)}
        </dialog_1.DialogFooter>
