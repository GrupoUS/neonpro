...
var StorageMonitor = function () {
    ...
    var _d = (0, react_1.useState)([]), distribution = _d[0], setDistribution = _d[1];
    var _e = (0, react_1.useState)(''), selectedProvider = _e[0], setSelectedProvider = _e[1];
    ...
    var loadStorageData = function () { return __awaiter(void 0, void 0, void 0, function () {
        var error_1;
    ...
    var loadProviders = function () { return __awaiter(void 0, void 0, void 0, function () {
        var response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch('/api/backup/storage/providers')];
                case 1:
                    response = _a.sent();
    ...
    var loadMetrics = function () { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch('/api/backup/metrics/storage')];
                case 1:
                    response = _a.sent();
                    if (!response.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, response.json()];
    ...
    var loadHealth = function () { return __awaiter(void 0, void 0, void 0, function () {
        var response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch('/api/backup/storage/health')];
                case 1:
                    response = _a.sent();
                    if (!response.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, response.json()];
    ...
    var loadDistribution = function () { return __awaiter(void 0, void 0, void 0, function () {
        var response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch('/api/backup/metrics/distribution')];
                case 1:
                    response = _a.sent();
                    if (!response.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, response.json()];
    ...
    var handleTestConnection = function (providerId) { return __awaiter(void 0, void 0, void 0, function () {
        var response, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch("/api/backup/storage/providers/".concat(providerId, "/test"), {
                            method: 'POST',
                        })];
                case 1:
                    response = _a.sent();
    ...
    var filteredProviders = selectedProvider
        ? providers.filter(function (p) { return p.id === selectedProvider; })
        : providers;
    if (loading) {
    ...
    return (<div className="space-y-6">
        ...
        <div className="flex space-x-2">
          <select_1.Select value={selectedProvider} onValueChange={setSelectedProvider}>
            ...
            <select_1.SelectContent>
              ...
              {providers.map(function (provider) { return (<select_1.SelectItem key={provider.id} value={provider.id}>
                  {provider.name}
                </select_1.SelectItem>); })}
            </select_1.SelectContent>
          </select_1.Select>
          ...
          <card_1.CardContent>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                <div className={"h-3 w-3 rounded-full ".concat(health.overall_status === 'HEALTHY' ? 'bg-green-500' :
            ...
            {health.issues.length > 0 && (<div className="space-y-2">
                {health.issues.map(function (issue, index) { return (<alert_1.Alert key={index} className={issue.severity === 'CRITICAL' || issue.severity === 'HIGH'
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="flex-1">
                        <alert_1.AlertDescription>
                          ...
                          {issue.recommendation && (<div className="mt-1 text-sm">
      ...
      {metrics && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <card_1.Card>
            <card_1.CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-bold">{(0, utils_1.formatBytes)(metrics.total_size)}</p>
          ...
          <card_1.Card>
            <card_1.CardContent className="pt-6">
              <div className="flex items-center justify-between">
                ...
                {metrics.growth_rate_mb_per_day > 0 ? (<lucide_react_1.TrendingUp className="h-8 w-8 text-green-500"/>) : (<lucide_react_1.TrendingDown className="h-8 w-8 text-red-500"/>)}
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </div>)}
      ...
      {distribution.length > 0 && (<card_1.Card>
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center">
          ...
          <card_1.CardContent>
            <div className="space-y-4">
              {distribution.map(function (item) { return (<div key={item.provider_id} className="space-y-2">
                  <div className="flex justify-between items-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredProviders.map(function (provider) { return (<card_1.Card key={provider.id}>
            <card_1.CardHeader>
              <card_1.CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <div className={"h-2 w-2 rounded-full ".concat(getStatusColor(provider.status))}/>
              ...
              <card_1.CardDescription>
                {provider.connection_status === 'CONNECTED' ? (<span className="text-green-600 flex items-center">
                    <lucide_react_1.CheckCircle className="h-4 w-4 mr-1"/>
                    Conectado
                  </span>) : provider.connection_status === 'ERROR' ? (<span className="text-red-600 flex items-center">
                    <lucide_react_1.AlertTriangle className="h-4 w-4 mr-1"/>
                    Erro de conexão
                  </span>) : (<span className="text-gray-600 flex items-center">
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {/* Capacidade */}
                {provider.total_capacity && (<div>
                    <div className="flex justify-between text-sm mb-2">
                      <span>Capacidade</span>
                      <span>
                        {(0, utils_1.formatBytes)(provider.used_capacity || 0)} / {(0, utils_1.formatBytes)(provider.total_capacity)}
                      </span>
                    </div>
                    <progress_1.Progress value={((provider.used_capacity || 0) / provider.total_capacity) * 100} className="h-2"/>
                ...
                {provider.last_sync && (<div className="flex justify-between text-sm">
                ...
                {/* Ações */}
                <div className="flex space-x-2">
                  <button_1.Button variant="outline" size="sm" onClick={function () { return handleTestConnection(provider.id); }} className="flex-1">
                    <lucide_react_1.Zap className="h-4 w-4 mr-2"/>
                    Testar Conexão
                  </button_1.Button>
                  {provider.config && (<button_1.Button variant="outline" size="sm">
      ...
      {providers.length === 0 && (<card_1.Card>
