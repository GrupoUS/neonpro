...
function TimeSlotDisplay(_a) {
    var _this = this;
    var serviceId = _a.serviceId, selectedSlot = _a.selectedSlot, onSlotSelect = _a.onSlotSelect, professionalId = _a.professionalId, _b = _a.className, className = _b === void 0 ? "" : _b;
    var _c = (0, react_1.useState)([]), availableSlots = _c[0], setAvailableSlots = _c[1];
    var _d = (0, react_1.useState)(true), loading = _d[0], setLoading = _d[1];
    var _e = (0, react_1.useState)(''), error = _e[0], setError = _e[1];
    var _f = (0, react_1.useState)((0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')), selectedDate = _f[0], setSelectedDate = _f[1];
    (0, react_1.useEffect)(function () {
        if (serviceId) {
            fetchAvailableSlots();
        }
    }, [serviceId, professionalId, selectedDate]);
    var fetchAvailableSlots = function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            switch (_b.label) {
                ...
                case 1:
                    ...
                    slotsArray = Object.entries(groupedSlots_1).map(function (_a) {
                        ...
                        return ({
                            date: date,
                            slots: slots.sort(function (a, b) { return new Date(a.datetime).getTime() - new Date(b.datetime).getTime(); })
                        });
                    });
                    setAvailableSlots(slotsArray.sort(function (a, b) { return a.date.localeCompare(b.date); }));
                    return [3 /*break*/, 4];
                case 2:
                    err_1 = _b.sent();
    ...
    return (<div className={"space-y-6 ".concat(className)}>
      ...
      <div className="flex overflow-x-auto gap-2 pb-2">
        {generateDateOptions().slice(0, 7).map(function (option) { return (<button_1.Button key={option.value} variant={selectedDate === option.value ? "default" : "outline"} size="sm" onClick={function () { return setSelectedDate(option.value); }} className="whitespace-nowrap min-w-fit">
      ...
      <div className="space-y-6">
        {availableSlots.length === 0 ? (<card_1.Card className="p-6 text-center">
          ...
          </card_1.Card>) : (availableSlots.map(function (_a) {
            var date = _a.date, slots = _a.slots;
            return (<card_1.Card key={date} className="overflow-hidden">
              ...
              <card_1.CardContent className="p-6">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  {slots.filter(function (slot) { return slot.is_available; }).map(function (slot) { return (<button_1.Button key={slot.datetime} variant={(selectedSlot === null || selectedSlot === void 0 ? void 0 : selectedSlot.datetime) === slot.datetime
                        ? "default"
                        : "outline"} size="sm" onClick={function () { return onSlotSelect(slot); }} className={"flex flex-col items-center p-3 h-auto ".concat((selectedSlot === null || selectedSlot === void 0 ? void 0 : selectedSlot.datetime) === slot.datetime
                        ? 'bg-blue-600 text-white hover:bg-blue-700'
                        : 'hover:border-blue-300 hover:bg-blue-50')} aria-pressed={(selectedSlot === null || selectedSlot === void 0 ? void 0 : selectedSlot.datetime) === slot.datetime}>
                      <lucide_react_1.Clock className="h-4 w-4 mb-1"/>
                      <span className="font-medium">
                        {formatTimeSlot(slot.datetime)}
                      </span>
                      {slot.professional_name && (<span className="text-xs opacity-80 truncate w-full">
                          {slot.professional_name}
                        </span>)}
                    </button_1.Button>); })}
                </div>
                
                {slots.filter(function (slot) { return slot.is_available; }).length === 0 && (<div className="text-center py-6 text-gray-500">
