...
export function TimeSlotDisplay({
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      const { data, error: fetchError } = await supabase.rpc("get_patient_available_slots", {
      ...
      <div className="space-y-6">
        {availableSlots.length === 0 ? (
          <Card className="p-6 text-center">
            ...
            <Button variant="outline" onClick={fetchAvailableSlots} className="mt-4">
              Atualizar Horários
            </Button>
          </Card>
        ) : (
          availableSlots.map(({ date, slots }) => (
            <Card key={date} className="overflow-hidden">
              <CardHeader className="bg-gray-50 py-4">
                <CardTitle className="text-lg capitalize">{formatDateHeader(date)}</CardTitle>
              </CardHeader>
