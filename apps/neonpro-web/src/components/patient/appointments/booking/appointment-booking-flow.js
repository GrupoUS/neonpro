...
function AppointmentBookingFlow(_a) {
    ...
    var _c = (0, react_1.useState)('service'), currentStep = _c[0], setCurrentStep = _c[1];
    var _d = (0, react_1.useState)(null), selectedService = _d[0], setSelectedService = _d[1];
    var _e = (0, react_1.useState)(null), selectedProfessional = _e[0], setSelectedProfessional = _e[1];
    var _f = (0, react_1.useState)(null), selectedTimeSlot = _f[0], setSelectedTimeSlot = _f[1];
    var _g = (0, react_1.useState)(''), appointmentNotes = _g[0], setAppointmentNotes = _g[1];
    var _h = (0, react_1.useState)([]), specialRequests = _h[0], setSpecialRequests = _h[1];
    var _j = (0, react_1.useState)(false), isSubmitting = _j[0], setIsSubmitting = _j[1];
    var _k = (0, react_1.useState)(null), bookingStartTime = _k[0], setBookingStartTime = _k[1];
    ...
    var handleConfirmBooking = function () { return __awaiter(_this, void 0, void 0, function () {
        var supabase, _a, availabilityCheck, availabilityError, appointmentData, _b, appointment, bookingError, bookingDuration, err_1;
            ...
            switch (_c.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, supabase
                            ...
                            p_professional_id: (selectedProfessional === null || selectedProfessional === void 0 ? void 0 : selectedProfessional.id) || null
                        })];
                case 2:
                    _a = _c.sent(), availabilityCheck = _a.data, availabilityError = _a.error;
                    if (availabilityError)
                        throw availabilityError;
                    if (!(availabilityCheck === null || availabilityCheck === void 0 ? void 0 : availabilityCheck.is_available)) {
                    ...
                    appointmentData = {
                        ...
                        professional_id: (selectedProfessional === null || selectedProfessional === void 0 ? void 0 : selectedProfessional.id) || null,
                ...
                case 3:
                    ...
                    router.push("/patient/appointments/booking/success?appointment_id=".concat(appointment.id));
                    return [3 /*break*/, 6];
                case 4:
                    err_1 = _c.sent();
    ...
    var renderStepContent = function () {
        switch (currentStep) {
            case 'service':
                return (<service_selection_1.ServiceSelection selectedService={selectedService} onServiceSelect={setSelectedService}/>);
            case 'professional':
                return (<professional_selection_1.ProfessionalSelection serviceId={selectedService.id} selectedProfessional={selectedProfessional} onProfessionalSelect={setSelectedProfessional} allowAnyProfessional={true}/>);
            case 'time':
                return (<RealTimeAvailability_1.RealTimeAvailability serviceId={selectedService.id} professionalId={selectedProfessional === null || selectedProfessional === void 0 ? void 0 : selectedProfessional.id} dateRange={{
                        ...
                        setSelectedTimeSlot({
                            ...
                            professional_name: selectedProfessional === null || selectedProfessional === void 0 ? void 0 : selectedProfessional.name
                        });
                    }} onSlotReserve={function (slotId) { return __awaiter(_this, void 0, void 0, function () {
                        ...
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    _a.trys.push([0, 2, , 3]);
                                    supabase = (0, client_1.createClient)();
                                    return [4 /*yield*/, supabase.rpc('confirm_appointment_booking', {
                                            slot_id: slotId,
                    ...
                    }); }} patientId="current_user" showAlternatives={true} maxAlternatives={3}/>);
            case 'notes':
                return (<appointment_notes_form_1.AppointmentNotesForm notes={appointmentNotes} onNotesChange={setAppointmentNotes} specialRequests={specialRequests} onSpecialRequestsChange={setSpecialRequests}/>);
            case 'confirmation':
                return (<booking_confirmation_1.BookingConfirmation service={selectedService} professional={selectedProfessional} timeSlot={selectedTimeSlot} notes={appointmentNotes} specialRequests={specialRequests} onConfirm={handleConfirmBooking} onBack={handleBack}/>);
            default:
                return null;
        }
    };
    return (<div className={"max-w-4xl mx-auto space-y-6 ".concat(className)}>
      {/* Header */}
      <card_1.Card>
        ...
        <card_1.CardContent>
          ...
          <div className="flex justify-between items-center mt-6">
            {steps.map(function (step, index) { return (<div key={step} className={"flex items-center ".concat(index < steps.length - 1 ? 'flex-1' : '')}>
                <div className={"flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium ".concat(index < currentStepIndex
                ? 'bg-green-100 text-green-600'
                : index === currentStepIndex
                    ? 'bg-blue-100 text-blue-600'
                    : 'bg-gray-100 text-gray-400')}>
                  {index < currentStepIndex ? (<lucide_react_1.CheckCircle2 className="h-4 w-4"/>) : (index + 1)}
                </div>
                {index < steps.length - 1 && (<div className={"flex-1 h-1 mx-2 ".concat(index < currentStepIndex
                    ? 'bg-green-200'
                    : 'bg-gray-200')}/>)}
              </div>); })}
          </div>
      ...
      {/* Navigation Buttons */}
      {currentStep !== 'confirmation' && (<div className="flex flex-col sm:flex-row gap-4 pt-6">
          <button_1.Button variant="outline" onClick={handleBack} disabled={!canGoBack() || isSubmitting} className="w-full sm:w-auto">
            <lucide_react_1.ArrowLeft className="mr-2 h-4 w-4"/>
            Voltar
          </button_1.Button>
          <button_1.Button onClick={handleNext} disabled={!canProceedToNext() || isSubmitting} className="w-full sm:flex-1">
            Próximo
