...
function UpcomingAppointments(_a) {
    var appointments = _a.appointments, onCancel = _a.onCancel, onReschedule = _a.onReschedule, onView = _a.onView, cancellationPolicies = _a.cancellationPolicies;
    ...
    var getStatusBadge = function (appointment) {
        switch (appointment.status) {
            case 'confirmed':
                return <badge_1.Badge variant="default" className="bg-green-100 text-green-800 hover:bg-green-100">Confirmado</badge_1.Badge>;
            case 'rescheduled':
    ...
    var getUrgencyIndicator = function (hoursUntil) {
        if (hoursUntil <= 24) {
            return (<div title="Menos de 24h - não pode cancelar">
          <lucide_react_1.AlertTriangle className="h-4 w-4 text-orange-500"/>
        </div>);
        }
        else if (hoursUntil <= 48) {
            return (<div title="Menos de 48h - não pode reagendar">
          <lucide_react_1.AlertTriangle className="h-4 w-4 text-yellow-500"/>
        </div>);
        }
        return (<div title="Pode cancelar e reagendar">
    ...
    return (<div className="space-y-4">
      {sortedDates.map(function (date) {
            ...
            {dayAppointments.map(function (appointment) {
                    var time = formatAppointmentDateTime(appointment.appointment_date, appointment.appointment_time).time;
                    return (<card_1.Card key={appointment.id} className="transition-shadow hover:shadow-md">
                  <card_1.CardContent className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          ...
                          <div>
                            ...
                            : "".concat(Math.abs(appointment.hours_until_appointment), "h atr\u00E1s")}
                          </div>
                        </div>

                        {/* Professional info */}
                        {appointment.professional_name && (<div className="flex items-center gap-2 text-sm">
                            <lucide_react_1.User className="h-4 w-4 text-muted-foreground"/>
                            <span>{appointment.professional_name}</span>
                          </div>)}

                        {/* Notes */}
                        {appointment.notes && (<div className="bg-muted/30 rounded-md p-3 text-sm">
                            <strong>Observações:</strong> {appointment.notes}
                          </div>)}

                        {/* Policy warnings */}
                        <div className="space-y-2">
                          {!appointment.can_cancel && appointment.hours_until_appointment < 24 && (<div className="flex items-start gap-2 text-xs text-orange-600 bg-orange-50 dark:bg-orange-950 p-2 rounded-md">
                              ...
                              <div>
                                ...
                                Mínimo necessário: {(cancellationPolicies === null || cancellationPolicies === void 0 ? void 0 : cancellationPolicies.minimum_hours) || 24}h.
                              </div>
                            </div>)}

                          {!appointment.can_reschedule && appointment.hours_until_appointment < 48 && appointment.can_cancel && (<div className="flex items-start gap-2 text-xs text-yellow-600 bg-yellow-50 dark:bg-yellow-950 p-2 rounded-md">
                              <lucide_react_1.AlertTriangle className="h-4 w-4 flex-shrink-0 mt-0.5"/>
                              <div>
                      ...
                      <div className="flex flex-col gap-2 ml-4">
                        <button_1.Button variant="outline" size="sm" onClick={function () { return onView(appointment.id); }} className="text-xs">
                          Detalhes
                        </button_1.Button>

                        {appointment.can_reschedule && (<button_1.Button variant="outline" size="sm" onClick={function () { return onReschedule(appointment.id); }} className="text-xs">
                            <lucide_react_1.RotateCcw className="h-3 w-3 mr-1"/>
                            Reagendar
                          </button_1.Button>)}

                        {appointment.can_cancel && (<button_1.Button variant="destructive" size="sm" onClick={function () { return onCancel(appointment.id); }} className="text-xs">
      ...
      {cancellationPolicies && (<div className="mt-6 p-4 bg-muted/30 rounded-lg text-xs text-muted-foreground">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <strong>Cancelamento:</strong> {cancellationPolicies.minimum_hours}h de antecedência
              {cancellationPolicies.fee_applies && (<div>Taxa por cancelamento tardio: R$ {cancellationPolicies.fee_amount.toFixed(2)}</div>)}
            </div>
            <div>
