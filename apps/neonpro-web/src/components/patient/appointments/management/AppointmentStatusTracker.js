...
function AppointmentStatusTracker(_a) {
    var upcomingCount = _a.upcomingCount, pastCount = _a.pastCount, noShowPattern = _a.noShowPattern, cancellationStats = _a.cancellationStats, cancellationPolicies = _a.cancellationPolicies;
    ...
    var getRecommendations = function () {
        ...
        if (noShowPattern.rate > 27) {
            recommendations.push({
                ...
                description: "Sua taxa de ".concat(noShowPattern.rate.toFixed(1), "% est\u00E1 acima da m\u00E9dia nacional (27%). Configure lembretes autom\u00E1ticos."),
    ...
    return (<div className="space-y-6">
          ...
          <card_1.CardContent>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                {attendanceRate >= 85 ? (<lucide_react_1.TrendingUp className="h-5 w-5 text-green-500"/>) : (<lucide_react_1.TrendingDown className="h-5 w-5 text-red-500"/>)}
              </div>
              <div className="text-sm text-muted-foreground">
      ...
      {/* Performance Badges */}
      {performanceBadges.length > 0 && (<card_1.Card>
          <card_1.CardHeader>
          ...
          <card_1.CardContent>
            <div className="flex flex-wrap gap-2">
              {performanceBadges.map(function (badge, index) { return (<badge_1.Badge key={index} variant={badge.variant} className={badge.className}>
                  {badge.label}
                </badge_1.Badge>); })}
            </div>
          </card_1.CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {cancellationStats.rate > 0 && (<card_1.Card>
            <card_1.CardHeader>
              <card_1.CardTitle className="text-lg flex items-center gap-2">
            ...
            <card_1.CardContent className="space-y-4">
              {Object.entries(cancellationStats.reasonBreakdown).map(function (_a) {
                var reason = _a[0], percentage = _a[1];
                return (<div key={reason} className="space-y-2">
        ...
        <card_1.Card>
          ...
          <card_1.CardContent className="space-y-4">
            ...
            {/* Policy reminder */}
            {cancellationPolicies && (<div className="mt-4 p-3 bg-blue-50 dark:bg-blue-950 rounded-lg text-xs">
                ...
                <div className="space-y-1 text-muted-foreground">
                  ...
                  {cancellationPolicies.fee_applies && (<div>• Taxa tardio: R$ {cancellationPolicies.fee_amount.toFixed(2)}</div>)}
                </div>
              </div>)}
          </card_1.CardContent>
        </card_1.Card>
      ...
      {recommendations.length > 0 && (<card_1.Card>
          ...
          <card_1.CardContent className="space-y-4">
            {recommendations.map(function (rec, index) {
                var Icon = rec.icon;
                var colorClass = rec.type === 'warning' ? 'text-orange-500' :
                    rec.type === 'info' ? 'text-blue-500' : 'text-green-500';
                return (<div key={index} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
