...
function AppointmentHistory(_a) {
    var appointments = _a.appointments, onView = _a.onView;
    ...
    var filteredAppointments = appointments
        ...
        var matchesSearch = searchTerm === '' ||
            ...
            ((_a = appointment.professional_name) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_b = appointment.notes) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase()));
        // Status filter  
    ...
    var getStatusBadge = function (status) {
        ...
        return (<badge_1.Badge variant={config.variant} className={config.className}>
    ...
    var stats = {
        ...
        noShow: appointments.filter(function (a) { return a.status === 'no_show'; }).length
    };
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input_1.Input placeholder="Buscar por serviço, profissional ou observações..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-9"/>
        </div>
        ...
        <select_1.Select value={sortOrder} onValueChange={function (value) { return setSortOrder(value); }}>
          <select_1.SelectTrigger className="w-full sm:w-48">
      ...
      {filteredAppointments.length === 0 ? (<div className="text-center py-8">
          ...
          {filteredAppointments.map(function (appointment) {
                var _a = formatAppointmentDateTime(appointment.appointment_date, appointment.appointment_time), date = _a.date, time = _a.time, fullDate = _a.fullDate;
                ...
                <card_1.CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-2">
                      ...
                      {/* Cancellation reason */}
                      {appointment.status === 'cancelled' && appointment.cancellation_reason && (<div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-md p-2 text-sm">
                          <strong>Motivo do cancelamento:</strong> {appointment.cancellation_reason}
                          {appointment.cancellation_date && (<div className="text-xs text-muted-foreground mt-1">
                              Cancelado em: {(0, date_fns_1.format)((0, date_fns_1.parseISO)(appointment.cancellation_date), 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR })}
                            </div>)}
                        </div>)}

                      {/* Notes */}
                      {appointment.notes && (<div className="bg-muted/30 rounded-md p-2 text-sm">
      ...
      {/* Summary footer */}
      {filteredAppointments.length > 0 && (<div className="mt-6 p-4 bg-muted/30 rounded-lg text-xs text-muted-foreground text-center">
          {stats.completed > 0 && (<span>Taxa de comparecimento: {Math.round((stats.completed / (stats.total - stats.cancelled)) * 100)}%</span>)}
          {stats.noShow > 0 && (<span className="ml-4">Taxa de faltas: {Math.round((stats.noShow / stats.total) * 100)}%</span>)}
        </div>)}
    </div>);
}
