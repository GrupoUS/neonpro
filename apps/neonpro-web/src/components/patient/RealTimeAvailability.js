...
function RealTimeAvailability(_a) {
    var _this = this;
    var professionalId = _a.professionalId, serviceId = _a.serviceId, dateRange = _a.dateRange, onSlotSelect = _a.onSlotSelect, onSlotReserve = _a.onSlotReserve, patientId = _a.patientId, className = _a.className, _b = _a.showAlternatives, showAlternatives = _b === void 0 ? true : _b, _c = _a.maxAlternatives, maxAlternatives = _c === void 0 ? 3 : _c;
    var _d = (0, react_1.useState)(null), selectedSlot = _d[0], setSelectedSlot = _d[1];
    var _e = (0, react_1.useState)(null), reservingSlot = _e[0], setReservingSlot = _e[1];
    ...
    }), slots = _f.slots, loading = _f.loading, error = _f.error, connectionStatus = _f.connectionStatus, optimisticUpdates = _f.optimisticUpdates, refetch = _f.refetch, reserveSlot = _f.reserveSlot, getAlternatives = _f.getAlternatives;
    /**
     * Handle slot selection with optimistic reservation
     */
    var handleSlotSelect = (0, react_1.useCallback)(function (slot) { return __awaiter(_this, void 0, void 0, function () {
                    ...
                    if (showAlternatives) {
                        ...
                        if (alternatives_1.length > 0) {
                            sonner_1.toast.error('Horário não disponível', {
                                description: "Encontramos ".concat(alternatives_1.length, " alternativa(s) similar(es)"),
                                action: {
                                    label: 'Ver alternativas',
                                    onClick: function () { return scrollToAlternatives(); }
                                }
                            });
                        }
                        else {
    ...
    var formatTimeSlot = function (date, time, duration) {
        ...
        return {
            ...
            time: "".concat(startTime.toLocaleTimeString('pt-BR', {
                hour: '2-digit',
                minute: '2-digit'
            }), " - ").concat(endTime.toLocaleTimeString('pt-BR', {
                hour: '2-digit',
                minute: '2-digit'
            }))
        };
    ...
    var getConnectionStatusIndicator = function () {
        switch (connectionStatus) {
            ...
            case 'connecting':
            case 'reconnecting':
                return <lucide_react_1.RefreshCw className="h-4 w-4 text-yellow-500 animate-spin" aria-label="Conectando"/>;
            case 'disconnected':
                return <lucide_react_1.WifiOff className="h-4 w-4 text-red-500" aria-label="Desconectado"/>;
            default:
                return null;
        }
    };
    ...
    var renderSlotCard = function (slot, isAlternative) {
        ...
        return (<card_1.Card key={slot.id} className={(0, utils_1.cn)("transition-all duration-200 cursor-pointer hover:shadow-md", isSelected && "ring-2 ring-primary", isAlternative && "border-amber-200 bg-amber-50", !slot.available && "opacity-50 cursor-not-allowed")} onClick={function () { return slot.available && handleSlotSelect(slot); }}>
        <card_1.CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              {slot.duration && (<p className="text-xs text-muted-foreground">
                  Duração: {slot.duration} minutos
                </p>)}
            </div>
            
            <div className="flex flex-col items-end gap-2">
              {isReserving ? (<button_1.Button size="sm" disabled>
                  <lucide_react_1.Loader2 className="h-4 w-4 animate-spin mr-2"/>
                  Reservando...
                </button_1.Button>) : (<button_1.Button size="sm" variant={isSelected ? "default" : "outline"} disabled={!slot.available}>
                  {isSelected ? "Selecionado" : "Selecionar"}
                </button_1.Button>)}
              
              {isOptimistic && (<badge_1.Badge variant="secondary" className="text-xs">
                  Aguardando confirmação
                </badge_1.Badge>)}
              
              {isAlternative && (<badge_1.Badge variant="outline" className="text-xs">
                  Alternativa
                </badge_1.Badge>)}
              
              {slot.reserved_until && (<badge_1.Badge variant="destructive" className="text-xs">
    ...
    var alternatives = selectedSlot && showAlternatives ?
        getAlternatives(slots.find(function (s) { return s.id === selectedSlot; }), maxAlternatives) : [];
    if (loading) {
    ...
    return (<div className={(0, utils_1.cn)("space-y-4", className)}>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent className="space-y-3">
          {slots.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
            ...
            </div>) : (slots.map(function (slot) { return renderSlotCard(slot); }))}
        </card_1.CardContent>
      </card_1.Card>

      {/* Alternative Suggestions */}
      {alternatives.length > 0 && (<card_1.Card id="alternatives-section">
          <card_1.CardHeader>
          ...
          <card_1.CardContent className="space-y-3">
            ...
            {alternatives.map(function (slot) { return renderSlotCard(slot, true); })}
          </card_1.CardContent>
        </card_1.Card>)}
    </div>);
}
