...
function PortalLayout(_a) {
    ...
    var NavigationContent = function () { return (<div className="space-y-6">
      {/* User Info Card */}
      <div className="p-4 rounded-lg bg-gradient-to-r from-primary/5 to-primary/10 border border-primary/20">
        ...
        {/* Next Appointment */}
        {currentUser.nextAppointment && (<div className="pt-3 border-t border-primary/20">
      ...
      <nav className="space-y-2">
        {navigationItems.map(function (item) {
            ...
            return (<link_1.default key={item.href} href={item.href}>
              <div className={(0, utils_1.cn)('flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 group', active
                    ? 'bg-primary text-primary-foreground shadow-md'
                    : 'text-gray-700 hover:bg-gray-100 hover:text-primary')}>
                <IconComponent className={(0, utils_1.cn)('h-5 w-5 transition-colors', active ? 'text-primary-foreground' : 'text-gray-500 group-hover:text-primary')}/>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{item.title}</span>
                    {item.badge && (<badge_1.Badge variant={active ? "secondary" : "outline"} className="text-xs">
                        {item.badge}
                      </badge_1.Badge>)}
                  </div>
                  {item.description && (<p className={(0, utils_1.cn)('text-xs mt-1', active ? 'text-primary-foreground/80' : 'text-gray-500')}>
                      {item.description}
                    </p>)}
                </div>
              </div>
      ...
      <div className="p-4 rounded-lg bg-red-50 border border-red-200">
        ...
        <div className="space-y-1">
          <button_1.Button variant="outline" size="sm" className="w-full justify-start text-left border-red-300 hover:bg-red-100">
          ...
          <div className="mt-auto pt-6">
            ...
            <dialog_1.Dialog>
              <dialog_1.DialogTrigger asChild>
                <button_1.Button variant="ghost" className="w-full justify-start text-red-600 hover:text-red-700 hover:bg-red-50">
              ...
              <dropdown_menu_1.DropdownMenu>
                <dropdown_menu_1.DropdownMenuTrigger asChild>
                  <button_1.Button variant="ghost" size="sm" className="relative">
                    ...
                    {currentUser.notifications > 0 && (<badge_1.Badge className="absolute -top-1 -right-1 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs">
                ...
                <dropdown_menu_1.DropdownMenuContent align="end" className="w-56">
                  ...
                  <dropdown_menu_1.DropdownMenuItem>
                    ...
                    Notificações
                    {currentUser.notifications > 0 && (<badge_1.Badge className="ml-auto h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs">
                        {currentUser.notifications}
                      </badge_1.Badge>)}
                  </dropdown_menu_1.DropdownMenuItem>
                  <dropdown_menu_1.DropdownMenuItem>
