...
function DocumentCenter() {
    ...
    var _e = (0, react_1.useState)("all"), filterCategory = _e[0], setFilterCategory = _e[1];
    var _f = (0, react_1.useState)(null), downloading = _f[0], setDownloading = _f[1];
    ...
    var filteredDocuments = documents.filter(function (doc) {
        var _a, _b;
        var matchesSearch = doc.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ((_a = doc.treatment) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ((_b = doc.doctor) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    var handleDownload = function (document) { return __awaiter(_this, void 0, void 0, function () {
                    ...
                    setDocuments(function (prev) {
                        return prev.map(function (doc) {
                            return doc.id === document.id
                                ? __assign(__assign({}, doc), { accessCount: doc.accessCount + 1 }) : doc;
                        });
                    });
                    setDownloading(null);
    ...
    var getStatusBadge = function (status) {
        switch (status) {
            case "ready":
                return (<badge_1.Badge variant="outline" className="text-green-700 border-green-300">
            Disponível
          </badge_1.Badge>);
            case "processing":
                return (<badge_1.Badge variant="outline" className="text-yellow-700 border-yellow-300">
            Processando
          </badge_1.Badge>);
            case "signed":
                return (<badge_1.Badge variant="outline" className="text-blue-700 border-blue-300">
            Assinado
          </badge_1.Badge>);
            case "pending_signature":
                return (<badge_1.Badge variant="outline" className="text-orange-700 border-orange-300">
    ...
    return (<div className="space-y-6">
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="relative">
              ...
              <input_1.Input placeholder="Buscar por título, tratamento ou médico..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-10"/>
            </div>
            ...
            <select_1.Select value={filterType} onValueChange={setFilterType}>
              ...
              <select_1.SelectContent>
                ...
                <select_1.SelectItem value="medical_report">
                  Relatórios Médicos
                </select_1.SelectItem>
      ...
      <div className="space-y-4">
        {filteredDocuments.length === 0 ? (<card_1.Card>
            <card_1.CardContent className="flex flex-col items-center justify-center py-12">
          ...
          </card_1.Card>) : (filteredDocuments.map(function (document) { return (<card_1.Card key={document.id} className="hover:shadow-md transition-shadow">
              <card_1.CardContent className="p-6">
                ...
                locale: locale_1.ptBR,
                          ...
                          {document.doctor && (<div className="flex items-center gap-1">
                              <lucide_react_1.User className="h-4 w-4"/>
                              {document.doctor}
                            </div>)}
                          {document.amount && (<div className="flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      ...
                      {document.encrypted && (<badge_1.Badge variant="outline" className="text-green-700 border-green-300">
                          <lucide_react_1.Shield className="h-3 w-3 mr-1"/>
                          Criptografado
                        </badge_1.Badge>)}
                      {document.lgpdCompliant && (<badge_1.Badge variant="outline" className="text-blue-700 border-blue-300">
                          <lucide_react_1.CheckCircle className="h-3 w-3 mr-1"/>
                          LGPD
                        </badge_1.Badge>)}
                      {document.expiryDate && (<badge_1.Badge variant="outline" className="text-orange-700 border-orange-300">
                    ...
                    {document.treatment && (<div className="bg-gray-50 rounded-lg p-3 mb-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    {/* Visualizar */}
                    <button_1.Button variant="outline" size="sm" onClick={function () { return handlePreview(document); }} className="flex items-center gap-2">
                      <lucide_react_1.Eye className="h-4 w-4"/>
                      Visualizar
                    </button_1.Button>

                    {/* Download */}
                    <button_1.Button size="sm" onClick={function () { return handleDownload(document); }} disabled={downloading === document.id ||
                document.status === "processing"} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700">
                ...
                {/* Avisos Especiais */}
                {document.requiresPassword && (<div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div className="flex items-start gap-3">
                ...
                {document.expiryDate &&
                document.expiryDate <
                    new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) && (<div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                      <div className="flex items-start gap-3">
