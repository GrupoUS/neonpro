...
function SelfBooking() {
    ...
    var _c = (0, react_1.useState)(null), selectedProfessional = _c[0], setSelectedProfessional = _c[1];
    var _d = (0, react_1.useState)(), selectedDate = _d[0], setSelectedDate = _d[1];
    var _e = (0, react_1.useState)(''), selectedTime = _e[0], setSelectedTime = _e[1];
    ...
    var _j = (0, react_1.useState)(false), isBooking = _j[0], setIsBooking = _j[1];
    ...
    var handleBookingConfirm = function () { return __awaiter(_this, void 0, void 0, function () {
        var error_1;
            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    sonner_1.toast.success('Agendamento realizado com sucesso!', {
                        description: "".concat(selectedService.name, " em ").concat((0, date_fns_1.format)(selectedDate, "d 'de' MMMM", { locale: locale_1.ptBR }), " \u00E0s ").concat(selectedTime)
                    });
                    // Reset form
                    setCurrentStep(1);
    ...
    return (<div className="max-w-4xl mx-auto space-y-8">
      ...
      <card_1.Card className="medical-card">
        <card_1.CardContent className="p-6">
          <div className="flex items-center justify-between">
            {bookingSteps.map(function (step, index) { return (<div key={step.step} className="flex items-center">
                <div className="flex flex-col items-center">
                  <div className={"w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium transition-colors ".concat(currentStep >= step.step
                ? 'bg-primary text-white'
                : 'bg-muted text-muted-foreground')}>
                    {currentStep > step.step ? (<lucide_react_1.CheckCircle className="w-5 h-5"/>) : (step.step)}
                  </div>
                ...
                {index < bookingSteps.length - 1 && (<div className={"flex-1 h-px mx-4 transition-colors ".concat(currentStep > step.step ? 'bg-primary' : 'bg-muted')}/>)}
              </div>); })}
          </div>
        </card_1.CardContent>
      </card_1.Card>
      ...
      {currentStep === 1 && (<div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {services.map(function (service) {
                ...
                return (<card_1.Card key={service.id} className="cursor-pointer hover:shadow-md transition-shadow medical-card" onClick={function () { return handleServiceSelect(service); }}>
                  <card_1.CardContent className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            ...
                            <div className="font-semibold text-foreground">
                              {service.price > 0 ? ("R$ ".concat(service.price.toLocaleString('pt-BR'))) : ('Gratuito')}
                            </div>
                          </div>
                        </div>
                        {service.requires_evaluation && (<div className="mt-3 flex items-center gap-2 text-xs text-amber-600">
      ...
      {currentStep === 2 && selectedService && (<div>
          <div className="flex items-center gap-2 mb-6">
            <button_1.Button variant="ghost" size="sm" onClick={function () { return setCurrentStep(1); }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {professionals
                .filter(function (prof) { return prof.available_services.includes(selectedService.id); })
                .map(function (professional) { return (<card_1.Card key={professional.id} className="cursor-pointer hover:shadow-md transition-shadow medical-card" onClick={function () { return handleProfessionalSelect(professional); }}>
                  <card_1.CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      <avatar_1.Avatar className="h-16 w-16">
                        ...
                        <avatar_1.AvatarFallback className="bg-primary/10 text-primary font-semibold">
                          {professional.name.split(' ').map(function (n) { return n[0]; }).join('')}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <div className="flex items-center">
                            {__spreadArray([], Array(5), true).map(function (_, i) { return (<lucide_react_1.Heart key={i} className={"w-4 h-4 ".concat(i < Math.floor(professional.rating)
      ...
      {currentStep === 3 && selectedService && selectedProfessional && (<div>
          <div className="flex items-center gap-2 mb-6">
            <button_1.Button variant="ghost" size="sm" onClick={function () { return setCurrentStep(2); }}>
              <lucide_react_1.ArrowLeft className="w-4 h-4 mr-2"/>
              Voltar
            </button_1.Button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <card_1.Card className="medical-card">
              ...
              <card_1.CardContent>
                <calendar_1.Calendar mode="single" selected={selectedDate} onSelect={handleDateSelect} disabled={function (date) { return !canSelectDates(date); }} locale={locale_1.ptBR} className="rounded-md border"/>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card className="medical-card">
              <card_1.CardHeader>
                ...
                {selectedDate && (<card_1.CardDescription>
                    {(0, date_fns_1.format)(selectedDate, "EEEE, d 'de' MMMM", { locale: locale_1.ptBR })}
              ...
              <card_1.CardContent>
                {!selectedDate ? (<div className="text-center py-8 text-muted-foreground">
                    ...
                    {availableSlots.map(function (slot) { return (<button_1.Button key={slot.time} variant={selectedTime === slot.time ? "default" : "outline"} disabled={!slot.available} onClick={function () { return handleTimeSelect(slot.time); }} className="h-12">
                        {slot.time}
                      </button_1.Button>); })}
                  </div>)}
              </card_1.CardContent>
      ...
      {/* Step 4: Confirmation */}
      {currentStep === 4 && selectedService && selectedProfessional && selectedDate && selectedTime && (<div>
          <div className="flex items-center gap-2 mb-6">
            <button_1.Button variant="ghost" size="sm" onClick={function () { return setCurrentStep(3); }}>
              <lucide_react_1.ArrowLeft className="w-4 h-4 mr-2"/>
              Voltar
            </button_1.Button>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">
                      {(0, date_fns_1.format)(selectedDate, "EEEE, d 'de' MMMM 'de' yyyy", { locale: locale_1.ptBR })}
                    </p>
            ...
            <div className="space-y-6">
              <card_1.Card className="medical-card">
                ...
                <card_1.CardContent>
                  ...
                  <textarea_1.Textarea id="notes" placeholder="Descreva sintomas, dúvidas ou informações relevantes..." value={notes} onChange={function (e) { return setNotes(e.target.value); }} className="mt-2" rows={4}/>
                </card_1.CardContent>
              </card_1.Card>
              ...
              <div className="flex gap-3">
                <button_1.Button onClick={handleBookingConfirm} disabled={isBooking} className="flex-1" size="lg">
