...
function LgpdPreferences() {
    var _a = (0, react_1.useState)(consentPreferences), preferences = _a[0], setPreferences = _a[1];
    var _b = (0, react_1.useState)(mockDataRequests), dataRequests = _b[0], setDataRequests = _b[1];
    var _c = (0, react_1.useState)(false), showDataExport = _c[0], setShowDataExport = _c[1];
    var _d = (0, react_1.useState)(false), showDeleteAccount = _d[0], setShowDeleteAccount = _d[1];
    var _e = (0, react_1.useState)(''), deleteReason = _e[0], setDeleteReason = _e[1];
    ...
    var handleDataRequest = function (type) {
        ...
        setDataRequests(function (prev) { return __spreadArray([newRequest], prev, true); });
    };
    ...
    return (<div className="space-y-6">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-6">
            {Object.entries(preferences.reduce(function (acc, pref) {
            if (!acc[pref.category])
                acc[pref.category] = [];
            acc[pref.category].push(pref);
            return acc;
        }, {})).map(function (_a) {
            var category = _a[0], prefs = _a[1];
            ...
            return (<div key={category} className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <badge_1.Badge className={getCategoryColor(category)}>
                      {prefs.filter(function (p) { return p.enabled; }).length}/{prefs.length}
                    </badge_1.Badge>
                  </div>
                  <div className="space-y-3 ml-7">
                    {prefs.map(function (pref) { return (<div key={pref.id} className="flex items-start justify-between p-4 rounded-lg border bg-card">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center gap-2">
                            <h4 className="font-medium text-gray-900">{pref.title}</h4>
                            {pref.required && (<badge_1.Badge variant="secondary" className="text-xs">
                                Obrigatório
                              </badge_1.Badge>)}
                          </div>
                        ...
                        <div className="ml-4">
                          <switch_1.Switch checked={pref.enabled} onCheckedChange={function (enabled) { return handlePreferenceChange(pref.id, enabled); }} disabled={pref.required}/>
                        </div>
                      </div>); })}
                  </div>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <button_1.Button variant="outline" className="h-auto p-4 flex flex-col items-center gap-2" onClick={function () { return handleDataRequest('access'); }}>
            ...
            <dialog_1.Dialog>
              <dialog_1.DialogTrigger asChild>
                <button_1.Button variant="outline" className="h-auto p-4 flex flex-col items-center gap-2">
              ...
              <dialog_1.DialogContent>
                ...
                <div className="space-y-4">
                  ...
                  <button_1.Button onClick={function () { return handleDataRequest('portability'); }} className="w-full">
            ...
            <button_1.Button variant="outline" className="h-auto p-4 flex flex-col items-center gap-2" onClick={function () { return handleDataRequest('correction'); }}>
            ...
            <dialog_1.Dialog>
              <dialog_1.DialogTrigger asChild>
                <button_1.Button variant="outline" className="h-auto p-4 flex flex-col items-center gap-2 border-red-200 hover:bg-red-50">
              ...
              <dialog_1.DialogContent>
                <dialog_1.DialogHeader>
                  <dialog_1.DialogTitle className="text-red-600">Exclusão de Conta</dialog_1.DialogTitle>
                  <dialog_1.DialogDescription>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label_1.Label htmlFor="delete-reason">Motivo da exclusão (opcional)</label_1.Label>
                    <textarea_1.Textarea id="delete-reason" placeholder="Conte-nos o motivo para nos ajudar a melhorar nossos serviços..." value={deleteReason} onChange={function (e) { return setDeleteReason(e.target.value); }}/>
                  </div>
                  <div className="flex gap-2">
                    <button_1.Button variant="destructive" onClick={function () { return handleDataRequest('deletion'); }} className="flex-1">
                      Confirmar Exclusão
                    </button_1.Button>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            {dataRequests.length === 0 ? (<div className="text-center py-8">
              ...
              </div>) : (dataRequests.map(function (request) {
            var IconComponent = getRequestIcon(request.type);
            return (<div key={request.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          ...
                          {request.completionDate && (<span>Concluído em {new Date(request.completionDate).toLocaleDateString('pt-BR')}</span>)}
                        </div>
                      </div>
                    </div>
                    {request.status === 'completed' && (<button_1.Button size="sm" variant="outline">
