...
function DocumentsManagement() {
    ...
    var _c = (0, react_1.useState)(0), uploadProgress = _c[0], setUploadProgress = _c[1];
    var _d = (0, react_1.useState)(false), isUploading = _d[0], setIsUploading = _d[1];
    var _e = (0, react_1.useState)(false), showLgpdDialog = _e[0], setShowLgpdDialog = _e[1];
    var _f = (0, react_1.useState)(false), lgpdConsent = _f[0], setLgpdConsent = _f[1];
            ...
            progressInterval = setInterval(function () {
                setUploadProgress(function (prev) {
                    if (prev >= 100) {
                        ...
                        setDocuments(function (prev) { return __spreadArray([newDoc_1], prev, true); });
                        return 100;
                    }
                    return prev + 10;
                });
    ...
    var handleDelete = function (docId) {
        setDocuments(function (prev) { return prev.filter(function (doc) { return doc.id !== docId; }); });
    };
    ...
    return (<div className="space-y-6">
        ...
        <dialog_1.DialogContent>
          <dialog_1.DialogHeader>
            ...
            <dialog_1.DialogDescription>
              Para fazer upload de documentos médicos, é necessário seu consentimento para processamento de dados pessoais.
            </dialog_1.DialogDescription>
          </dialog_1.DialogHeader>
          <div className="space-y-4">
            <alert_1.Alert>
              ...
              <alert_1.AlertDescription>
                Seus documentos serão criptografados e armazenados com segurança máxima, conforme LGPD e normas do CFM.
              </alert_1.AlertDescription>
            </alert_1.Alert>
            <div className="flex items-center space-x-2">
              <checkbox_1.Checkbox id="consent" checked={lgpdConsent} onCheckedChange={function (checked) { return setLgpdConsent(checked); }}/>
              <label_1.Label htmlFor="consent" className="text-sm">
                Consinto com o processamento dos meus dados médicos para fins de tratamento e acompanhamento clínico.
              </label_1.Label>
            </div>
            ...
            if (lgpdConsent && ((_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.files)) {
                handleFileUpload({ target: fileInputRef.current });
            }
        }} disabled={!lgpdConsent} className="flex-1">
                Confirmar e Continuar
              </button_1.Button>
              <button_1.Button variant="outline" onClick={function () { return setShowLgpdDialog(false); }} className="flex-1">
                Cancelar
              </button_1.Button>
            </div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button_1.Button onClick={function () { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }} className="bg-primary hover:bg-primary/90">
            <lucide_react_1.Upload className="h-4 w-4 mr-2"/>
            Upload Documento
          </button_1.Button>
          <input ref={fileInputRef} type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" onChange={handleFileUpload}/>
        </div>
      </div>

      {/* Upload Progress */}
      {isUploading && (<card_1.Card>
      ...
      <alert_1.Alert>
        ...
        <alert_1.AlertDescription>
          <strong>Segurança LGPD:</strong> Todos os documentos são criptografados e processados conforme regulamentações de proteção de dados pessoais e normas do CFM.
        </alert_1.AlertDescription>
      </alert_1.Alert>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            {filteredDocuments.length === 0 ? (<div className="text-center py-8">
              ...
              </div>) : (filteredDocuments.map(function (doc, index) {
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            {doc.isConfidential && (<lucide_react_1.Lock className="h-4 w-4 text-gray-500"/>)}
                          </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          <button_1.Button size="sm" variant="ghost" onClick={function () { return handleDownload(doc); }} disabled={doc.status !== 'processed'}>
                            <lucide_react_1.Eye className="h-4 w-4"/>
                          </button_1.Button>
                          <button_1.Button size="sm" variant="ghost" onClick={function () { return handleDownload(doc); }} disabled={doc.status !== 'processed'}>
                            <lucide_react_1.Download className="h-4 w-4"/>
                          </button_1.Button>
                          <button_1.Button size="sm" variant="ghost" onClick={function () { return handleDelete(doc.id); }} className="text-red-600 hover:text-red-700">
                            <lucide_react_1.X className="h-4 w-4"/>
                          </button_1.Button>
                        </div>
                      </div>
                    </div>
                    {index < filteredDocuments.length - 1 && <separator_1.Separator className="my-2"/>}
                  </div>);
        }))}
          </div>
        </card_1.CardContent>
