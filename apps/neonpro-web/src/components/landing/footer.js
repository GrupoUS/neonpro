...
function Footer() {
    ...
    var socialLinks = [
        ...
        { icon: lucide_react_1.Linkedin, href: "https://linkedin.com/company/neonpro", label: "LinkedIn" },
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="flex space-x-4">
              {socialLinks.map(function (social, index) { return (<a key={index} href={social.href} target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors" aria-label={social.label}>
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              {quickLinks.map(function (link, index) { return (<li key={index}>
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              {solutions.map(function (solution, index) { return (<li key={index}>
                  <a href={solution.href} className="text-slate-300 hover:text-white transition-colors text-sm">
                    {solution.title}
                  </a>
                </li>); })}
            </ul>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {complianceLinks.map(function (link, index) { return (<a key={index} href={link.href} className="flex items-start space-x-3 p-3 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors group">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <a href="/privacy-policy" className="text-xs text-sky-400 hover:text-sky-300 underline">
                  Política de Privacidade
                </a>
                <span className="text-slate-500">•</span>
                <a href="/data-processing" className="text-xs text-sky-400 hover:text-sky-300 underline">
                  Tratamento de Dados
                </a>
                <span className="text-slate-500">•</span>
                <a href="/patient-rights" className="text-xs text-sky-400 hover:text-sky-300 underline">
                  Direitos do Titular
                </a>
                <span className="text-slate-500">•</span>
                <a href="/dpo-contact" className="text-xs text-sky-400 hover:text-sky-300 underline">
                  Contato DPO
                </a>
              </div>
            </div>
