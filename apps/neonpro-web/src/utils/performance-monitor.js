...
var PerformanceMonitor = /** @class */ (function () {
    ...
    PerformanceMonitor.prototype.trackComponent = function (componentName, renderTime) {
        ...
        if (existing) {
            ...
            existing.averageRenderTime = ((existing.averageRenderTime * (existing.renderCount - 1) + renderTime) /
                existing.renderCount);
        }
        else {
    ...
    PerformanceMonitor.prototype.analyzePerformance = function () {
            ...
            categoryMetrics.forEach(function (metric) {
                if (metric.threshold && metric.value > metric.threshold) {
                    _this.addAlert({
                        ...
                        metric: metric.name,
                        message: "".concat(metric.name, " is ").concat(metric.value).concat(metric.unit || '', " (threshold: ").concat(metric.threshold).concat(metric.unit || '', ")"),
        ...
        this.components.forEach(function (comp) {
            if (comp.averageRenderTime > 16) { // 60fps = 16ms per frame
                _this.addAlert({
                    type: comp.averageRenderTime > 50 ? 'critical' : 'warning',
                    metric: 'component-render-time',
                    message: "Component ".concat(comp.componentName, " average render time: ").concat(comp.averageRenderTime.toFixed(2), "ms"),
    ...
    PerformanceMonitor.prototype.getSuggestion = function (metricName, value, threshold) {
        var suggestions = {
            'heap-used': 'Consider implementing memory cleanup, removing unused variables, or using WeakMap/WeakSet',
            'node-count': 'Reduce DOM complexity, implement virtualization for large lists, or use fragments',
            'listener-count': 'Remove unused event listeners, use event delegation, or implement cleanup in useEffect',
    ...
    PerformanceMonitor.prototype.addAlert = function (alert) {
        this.alerts.push(__assign(__assign({}, alert), { id: "alert-".concat(Date.now(), "-").concat(Math.random().toString(36).substr(2, 9)), timestamp: Date.now() }));
    };
    ...
    PerformanceMonitor.prototype.getAllMetrics = function () {
        ...
        return allMetrics.sort(function (a, b) { return b.timestamp - a.timestamp; });
    };
    PerformanceMonitor.prototype.getRecommendations = function () {
        var recommendations = [];
        if (this.alerts.some(function (a) { return a.metric === 'heap-used'; })) {
            recommendations.push('Implement memory management best practices');
        }
        if (this.alerts.some(function (a) { return a.metric === 'fps'; })) {
        ...
        if (this.alerts.some(function (a) { return a.metric === 'node-count'; })) {
...
function usePerformanceMetric(name, category) {
    var _this = this;
    if (category === void 0) { category = 'custom'; }
    var addMetric = (0, react_1.useCallback)(function (value, threshold, unit) {
    ...
    var measureAsync = (0, react_1.useCallback)(function (fn) { return __awaiter(_this, void 0, void 0, function () {
    ...
    var measureSync = (0, react_1.useCallback)(function (fn) {
