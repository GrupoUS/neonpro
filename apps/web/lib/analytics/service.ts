...
export class AnalyticsService {
  ...
  private async calculateFunnelAnalysis(_query: AnalyticsQuery) {
  ...
  private async calculateCohortAnalysis(_query: AnalyticsQuery) {
  ...
  private identifyRiskFactors(metrics: any[]): string[] {
    ...
    const avgActions =
      metrics.reduce((sum, m) => sum + (m.actionsCount || 0), 0) /
      metrics.length;
    const avgDaysActive =
      metrics.reduce((sum, m) => sum + (m.daysActive || 0), 0) / metrics.length;
