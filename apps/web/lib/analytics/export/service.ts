...
export class AnalyticsExportService {
  ...
  private async fetchAnalyticsData(
    ...
    try {
      switch (reportType) {
        case 'revenue':
          data.revenue = await this.analyticsService.getRevenueAnalytics(
            'month',
            startDate,
            endDate,
            filters
  ...
  private async generatePDF(
    ...
    const title =
      config.customization?.title ||
      `Analytics Report - ${config.reportType.toUpperCase()}`;
  ...
  private async addPDFContent(
    ...
    if (data.revenue) {
      yPosition = this.addRevenueSectionToPDF(
        doc,
        data.revenue,
        options,
        yPosition
    ...
    if (data.conversion) {
      yPosition = this.addConversionSectionToPDF(
        doc,
        data.conversion,
        options,
        yPosition
    ...
    if (data.cohorts) {
      yPosition = this.addCohortSectionToPDF(
        doc,
        data.cohorts,
        options,
        yPosition
  ...
  private addCohortSectionToPDF(
    ...
    cohorts.slice(0, 5).forEach((cohort, _index) => {
      doc.text(
