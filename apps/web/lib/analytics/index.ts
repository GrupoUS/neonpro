...
export const AnalyticsUtils = {
   ...
   */
  formatMetric: (value: number, unit: string, decimals = 2): string => {
    ...
    switch (unit) {
      ...
      case 'MB':
        return `${formatted}MB`;
      case 'GB':
        return `${(value / 1024).toFixed(decimals)}GB`;
      case 'TB':
  ...
  calculateTrendDirection: (
    ...
    const variance =
      changes.reduce((sum, change) => sum + (change - avgChange) ** 2, 0) /
      changes.length;
...
export const AnalyticsConstants = {
  ...
  DEFAULT_THRESHOLDS: {
    ...
    RESPONSE_TIME: { warning: 2000, critical: 5000, emergency: 10_000 },
  ...
  CACHE_TTL: {
    PREDICTIONS: 300_000, // 5 minutes
    METRICS: 60_000, // 1 minute
    REPORTS: 900_000, // 15 minutes
