...
export class CohortAnalyzer {
  ...
  async generateCohorts(
    ...
    let _tableName = 'subscription_metrics';

    if (validConfig.cohortType === 'trial') {
      dimension = 'trial_start';
      _tableName = 'trial_analytics';
    }
    ...
    for (const range of cohortRanges) {
      const { data: userCount } = await this.supabase.rpc(
        'get_cohort_user_count',
        {
  ...
  async calculateCohortRetention(
    ...
    for (const cohort of cohorts) {
      for (let period = 0; period <= config.periods; period++) {
        ...
        const { data: periodMetrics } = await this.supabase.rpc(
          'calculate_cohort_period_metrics',
          {
  ...
  private calculateRetentionTrends(
    ...
    for (const [period, metrics] of Object.entries(periodGroups)) {
      ...
      const avgRevenue =
        metrics.reduce((sum, m) => sum + m.averageRevenuePerUser, 0) /
        metrics.length;
  ...
  private analyzeSeasonality(metrics: CohortMetrics[]) {
    ...
    const monthlyPerformance = {} as Record<
      string,
      { retention: number[]; revenue: number[] }
    >;
  ...
  private performSignificanceTests(
    ...
    for (let i = 0; i < periods.length - 1; i++) {
      ...
      const currentAvgRetention =
        currentPeriod.reduce((sum, m) => sum + m.retentionRate, 0) /
        currentPeriod.length;
      const nextAvgRetention =
        nextPeriod.reduce((sum, m) => sum + m.retentionRate, 0) /
        nextPeriod.length;
  ...
  private calculatePerformanceScore(
    ...
    return (
      Math.round((retention * 0.6 + (lifetimeValue / 100) * 0.4) * 100) / 100
    );
  }
