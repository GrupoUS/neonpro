...
export type SubscriptionStatus = 'active' | 'canceled' | 'cancelled' | 'expired' | 'incomplete' | 'incomplete_expired' | 'past_due' | 'trialing' | 'unpaid'
...
class SubscriptionCache {
  ...
  set(key: string, data: SubscriptionValidationResult, customTTL?: number): void {
  ...
  getStats() {
    ...
    return {
      ...
      oldestEntry: entries.length > 0 ? Math.min(...entries.map(([_, entry]) => entry.created)) : null,
      newestEntry: entries.length > 0 ? Math.max(...entries.map(([_, entry]) => entry.created)) : null
    }
  }
}
...
export async function validateSubscriptionStatus(
  ...
  try {
    ...
    const gracePeriodEnd = currentPeriodEnd ? new Date(currentPeriodEnd.getTime() + (3 * 24 * 60 * 60 * 1000)) : null

    let result: SubscriptionValidationResult

    // Check trial status
    const isTrialActive = trialEnd && trialEnd > now && subscription.status === 'trialing'
    ...
    if (subscription.status === 'active' || isTrialActive) {
      ...
      const isNearExpiry = expiresAt && (expiresAt.getTime() - now.getTime()) < (7 * 24 * 60 * 60 * 1000) // 7 days
...
export function getPerformanceSummary(): {
  ...
  const averageTime = performanceMetrics.reduce((sum, m) => sum + m.validationTime, 0) / totalValidations
 ...
 */
export async function healthCheck(request: NextRequest): Promise<{
  healthy: boolean
  ...
  const healthy = performanceSummary.totalValidations === 0 || (
