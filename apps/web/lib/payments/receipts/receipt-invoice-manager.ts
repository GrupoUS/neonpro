...
export class ReceiptInvoiceManager {
  async generateReceipt(paymentId: string, data: unknown) {
  ...
  async generateInvoice(customerId: string, items: unknown[]) {
    // Mock implementation for build compatibility
    return {
  ...
  async updateReceipt(receiptId: string, data: unknown) {
    // Mock implementation for build compatibility
    return {
      id: receiptId,
      ...data,
