...
export class MFAService {
  ...
  async setupMFA(
    ...
    try {
      // Validate input
      const _validation = MFASetupSchema.parse({
        userId,
        method,
  ...
  async verifyMFA(
    ...
    try {
      // Validate input
      const _validation = MFAVerificationSchema.parse({
        userId,
        token,
        method,
      ...
      const _auditLogId = await this.createAuditLog({
        userId,
        action: 'verify',
        method,
        result: 'failure',
  ...
  async sendSMSOTP(
    ...
    try {
      ...
      const mfaSettings = await this.getUserMFASettings(userId);
      const smsMethod = mfaSettings?.methods.find(
        (m) => m.type === 'sms' && m.isEnabled,
      );
  ...
  private async hashBackupCode(code: string): Promise<string> {
    return crypto
      .pbkdf2Sync(code, 'neonpro-mfa-salt', 100_000, 64, 'sha512')
      .toString('hex');
  }
  ...
  private async hashRecoveryToken(token: string): Promise<string> {
    return crypto
      .pbkdf2Sync(token, 'neonpro-recovery-salt', 100_000, 64, 'sha512')
      .toString('hex');
  }
  ...
  private async checkRateLimit(
    ...
    if (failedAttempts >= RATE_LIMIT_CONFIG.maxAttempts) {
      const lastFailure = attempts.find(
        (attempt) => attempt.result === 'failure',
      );
    ...
    return {
      ...
      remainingAttempts: Math.max(
        0,
        RATE_LIMIT_CONFIG.maxAttempts - failedAttempts,
      ),
    };
  }
  ...
  private async createAuditLog(
    log: Omit<MFAAuditLog, 'id' | 'timestamp'>,
  ): Promise<string> {
    const { data, error } = await this.supabase
      .from('mfa_audit_logs')
      .insert({
        ...
        emergency_bypass: log.emergencyBypass,
   ...
   * Update last verified timestamp
   */
  private async updateLastVerified(
    userId: string,
    method: string,
  ): Promise<void> {
    await this.supabase
