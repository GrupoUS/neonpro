  ...
  .refine(
    (data) => {
      if (data.performed_period_start && data.performed_period_end) {
        return (
          new Date(data.performed_period_start) <=
          new Date(data.performed_period_end)
        );
      }
      return true;
    },
...
export const procedureFormSchema = z
  .object({
    ...
    outcome: z
      .string()
      .max(255, 'Resultado deve ter no máximo 255 caracteres')
      .optional(),
    data_consent_given: z.boolean({
      required_error: 'Consentimento de dados é obrigatório',
    }),
...
export const clinicalNoteSchema = z.object({
  ...
  authored_time: z
    .string()
    .datetime('Data de autoria deve estar no formato ISO 8601'),
