...
export const FHIRDateSchema = z.string().regex(
  /^\d{4}-\d{2}-\d{2}$/,
  'Must be a valid FHIR date (YYYY-MM-DD)'
);
...
export const CPFSchema = z.string().regex(
  /^\d{3}\.\d{3}\.\d{3}-\d{2}$/,
  'Must be a valid CPF (000.000.000-00)'
);

// Brazilian CEP validation
export const CEPSchema = z.string().regex(
  /^\d{5}-?\d{3}$/,
  'Must be a valid CEP (00000-000)'
);
...
export const FHIRPatientSchema = z.object({
  ...
  identifier: z.array(FHIRIdentifierSchema).min(1, 'At least one identifier is required'),
...
export const LGPDConsentSchema = z.object({
  ...
  processing_details: z.string().min(20, 'Processing details must be at least 20 characters'),
  third_party_sharing: z.array(z.object({
...
export const PatientRegistrationSchema = z.object({
  ...
  given_names: z.array(z.string().min(1))
...
export const PatientSearchSchema = z.object({
  ...
  sort_by: z.enum(['name', 'created_at', 'updated_at', 'birth_date']).default('created_at'),
