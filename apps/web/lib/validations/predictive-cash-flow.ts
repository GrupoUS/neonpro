...
const AccuracyPercentageSchema = z
  .number()
...
const AmountInCentavosSchema = z
  .number()
  .int('Amount must be an integer (centavos)')
  .min(-999_999_999_999, 'Amount too small')
...
export const CreatePredictionModelSchema = z
  .object({
    model_name: z
      .string()
  ...
  .refine(
    (data) => {
      if (data.training_period_start && data.training_period_end) {
        return (
          new Date(data.training_period_start) <=
          new Date(data.training_period_end)
        );
      }
      return true;
    },
...
export const CreateCashFlowPredictionSchema = z
  .object({
    model_id: UUIDSchema,
  ...
  .refine(
    (data) => {
      ...
      const calculated_net =
        data.predicted_inflow_amount - data.predicted_outflow_amount;
  ...
  .refine(
    (data) => {
      ...
      const interval_width =
        data.confidence_interval_upper - data.confidence_interval_lower;
...
export const CreateForecastingScenarioSchema = z
  .object({
    scenario_name: z
      .string()
  ...
  .refine(
    (data) => {
      return (
        new Date(data.forecast_start_date) <= new Date(data.forecast_end_date)
      );
    },
    {
    ...
    (data) => {
      if (
        ...
        const calculated_profit =
          data.total_predicted_revenue - data.total_predicted_expenses;
        const tolerance = Math.abs(calculated_profit * 0.01); // 1% tolerance
        return (
          Math.abs(data.total_predicted_profit - calculated_profit) <= tolerance
        );
      }
      return true;
    },
...
export const CreatePredictionAccuracySchema = z
  .object({
  ...
  .refine(
    (data) => {
      ...
      const calculated_net =
        data.actual_inflow_amount - data.actual_outflow_amount;
      const tolerance = Math.abs(calculated_net * 0.01); // 1% tolerance
...
export const CreatePredictionAlertSchema = z
  .object({
    ...
    threshold_period: z
      .string()
      .max(50, 'Threshold period too long')
      .optional(),
    ...
    alert_description: z
      .string()
      .max(1000, 'Alert description too long')
      .optional(),

    recommended_actions: z
      .array(z.string().max(200, 'Recommended action too long'))
  ...
  .refine(
    (data) => {
      ...
      if (data.alert_type === 'threshold_breach') {
        return (
          data.threshold_amount !== undefined ||
