...
import { LGPDDataType, type LGPDPurpose } from './consent-automation-manager';
  ...
  async checkExpiringData(clinicId?: string): Promise<number> {
    try {
      ...
      for (const record of records) {
        ...
        if (
          daysUntilExpiration <= policy.notificationDaysBefore &&
    ...
    try {
      ...
      Object.values(LGPDDataType).forEach((dataType) => {
        ...
        retentionByDataType[dataType] = {
          ...
          expiring: typeRecords.filter((r) => r.status === 'expiring_soon')
            .length,
