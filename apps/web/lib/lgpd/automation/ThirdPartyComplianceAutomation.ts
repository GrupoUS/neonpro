...
export class ThirdPartyComplianceAutomation {
  ...
  async registerThirdPartyProvider(
    ...
    try {
      ...
      const initialRiskAssessment =
        await this.performInitialRiskAssessment(providerData);
      ...
      const { data: assessment, error } = await this.supabase
        ...
        .insert({
          ...
          certification_recommended:
            assessmentResults.certification_recommended,
  ...
  private async validateProviderData(
    ...
    if (
      ...
      providerData.company_name.trim().length === 0
    ) {
    ...
    if (
      !(
        ...
        this.isValidEmail(providerData.contact_email)
      )
    ) {
    ...
    if (
      !(
        providerData.privacy_policy_url &&
        this.isValidUrl(providerData.privacy_policy_url)
      )
    ) {
  ...
  private async validateAgreementData(
    ...
    if (
      !agreementData.legal_basis ||
      agreementData.legal_basis.trim().length === 0
    ) {
      errors.push('Legal basis is required');
    }

    if (
      !agreementData.data_categories ||
      agreementData.data_categories.length === 0
    ) {
      errors.push('Data categories are required');
    }

    if (
      !agreementData.processing_purposes ||
      agreementData.processing_purposes.length === 0
    ) {
      errors.push('Processing purposes are required');
    }

    if (
      !agreementData.retention_period_days ||
      agreementData.retention_period_days <= 0
    ) {
    ...
    if (
      ...
      agreementData.agreement_end_date
    ) {
  ...
  private async validateDataTransfer(
    ...
    if (
      this.config.consent_verification_required &&
      !transferData.consent_verified
    ) {
  ...
  private async validateTransferCompliance(_transfer: any): Promise<void> {
