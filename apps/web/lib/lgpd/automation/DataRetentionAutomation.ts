...
export class DataRetentionAutomation {
   ...
   */
  async startAutomatedRetention(checkIntervalHours = 24): Promise<void> {
    try {
  ...
  async scheduleRetentionExecution(
    ...
    try {
      ...
      const executionDate =
        scheduledDate || this.calculateNextExecutionDate(policy);
  ...
  private isWithinExecutionWindow(date?: Date): boolean {
    ...
    const withinDays =
      this.config.execution_window.allowed_days.includes(dayName);
