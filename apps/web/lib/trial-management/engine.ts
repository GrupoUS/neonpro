...
export class TrialManagementEngine {
  ...
  async createTrial(userId: string, signupSource = 'website'): Promise<Trial> {
    const now = new Date();
  ...
  private async generateConversionRecommendations(
    ...
    if (trial.conversionProbability > 0.7) {
      recommendations.push({
        ...
        timeline: '1 day',
        reasoning:
          'High conversion probability warrants direct sales intervention',
      });
    }
  ...
  async trackEvent(
    ...
    await this.supabase.from('customer_lifecycle_events').insert({
      trial_id: trialId,
  ...
  private async updateTrialEngagement(trialId: string): Promise<void> {
    ...
    events.forEach((event) => {
      const ageHours =
        (now - new Date(event.created_at).getTime()) / (1000 * 60 * 60);
  ...
  private determineTrialStage(trial: Trial, journey: UserJourney): TrialStage {
    ...
    if (
      trial.status === 'onboarding' &&
      (featureUsageEvents >= 5 || uniqueFeatures >= 2)
    ) {
      return 'active';
    }
    ...
    if (trial.status === 'active') {
      ...
      if (trial.daysRemaining <= 3 && trial.conversionProbability < 0.3) {
        return 'at_risk';
      }
    }
  ...
  private generateJourneyMilestones(events: JourneyEvent[]) {
    const milestones = [
      ...
      },
      {
        ...
        completedAt:
          events.filter((e) => e.type === 'feature_usage').length >= 3
            ? events.filter((e) => e.type === 'feature_usage')[2]?.timestamp
            : undefined,
        importance: 0.95,
      ...
      },
      {
        ...
        completedAt: events.find((e) => e.type === 'support_contact')
          ?.timestamp,
        importance: 0.7,
        category: 'engagement' as const,
      },
  ...
  private calculateProgressScore(
    ...
    return totalImportance > 0
      ? (completedImportance / totalImportance) * 100
      : 0;
  }
