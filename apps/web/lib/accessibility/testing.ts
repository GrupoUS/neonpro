...
export class AccessibilityTester {
  ...
  testColorContrast(container: HTMLElement = document.body): AccessibilityTestResult[] {
    ...
    textElements.forEach(element => {
      ...
      if (foregroundHex && backgroundHex) {
        const contrast = ContrastChecker.calculateContrastRatio(foregroundHex, backgroundHex)
        const fontSize = parseInt(computedStyles.fontSize)
        const fontWeight = parseInt(computedStyles.fontWeight) || 400
        const isLargeText = fontSize >= 18 || (fontSize >= 14 && fontWeight >= 700)
        
        const meetsAA = ContrastChecker.meetsWCAGAA(foregroundHex, backgroundHex, isLargeText)
  ...
  testFocusManagement(container: HTMLElement = document.body): AccessibilityTestResult[] {
    const results: AccessibilityTestResult[] = []
    ...
    const elementsWithFocusStyle = Array.from(container.querySelectorAll(
      '*:focus, *:focus-visible, .focus\\:ring-2'
    )).length
  ...
  testAriaLabels(container: HTMLElement = document.body): AccessibilityTestResult[] {
    const results: AccessibilityTestResult[] = []
  ...
  testSemanticStructure(container: HTMLElement = document.body): AccessibilityTestResult[] {
    const results: AccessibilityTestResult[] = []
  ...
  auditAccessibility(container: HTMLElement = document.body): AccessibilityAuditReport {
  ...
  simulateScreenReader(element: HTMLElement): string {
    ...
    if (expanded) announcement += `, ${expanded === 'true' ? 'expandido' : 'recolhido'}`
    if (selected) announcement += `, ${selected === 'true' ? 'selecionado' : 'não selecionado'}`
    if (checked) announcement += `, ${checked === 'true' ? 'marcado' : 'desmarcado'}`
