...
const HEALTHCARE_POOL_CONFIGS: Record<string, PoolConfiguration> = {
  ...
  small: {
    ...
    queryTimeout: 30_000, // 30 seconds (patient safety)
  ...
  medium: {
    ...
    queryTimeout: 25_000, // 25 seconds
    healthCheckInterval: 45_000, // 45 seconds
  },
  ...
  large: {
    ...
    queryTimeout: 20_000, // 20 seconds
...
export class NeonProConnectionPoolManager {
    ...
    for (const [poolKey, client] of this.pools) {
      try {
        ...
        const healthCheck: HealthcheckResult = {
          ...
          poolUtilization:
            ((metrics?.activeConnections || 0) / this.config.maxClients) * 100,
   ...
   */
  private verifyANVISACompliance(_poolKey: string): boolean {
   ...
   */
  private verifyCFMCompliance(_poolKey: string): boolean {
  ...
  private async retryConnection(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const operationType = poolKey.includes('critical')
          ? 'critical'
          : 'standard';
  ...
  public getPoolAnalytics(): {
    ...
    const avgResponseTime =
      pools.reduce((acc, p) => acc + p.health.avgResponseTime, 0) /
      pools.length;
