...
export async function createTreatmentPlan(
  ...
  const treatmentPlan = {
    ...
    data_consent_date: data.data_consent_given
...
export async function searchProcedures(
  ...
  if (filters.procedure_code && filters.procedure_code.length > 0) {
    query = query.contains('code', {
      coding: [{ code: filters.procedure_code }],
    });
  }
...
export async function getTreatmentStatistics(): Promise<TreatmentStatistics> {
  ...
  const procedureCounts = (proceduresStats || []).reduce(
    (acc, proc) => {
      ...
      const display =
        proc.code?.coding?.[0]?.display || 'Procedimento não especificado';
  ...
  const avgDuration =
    completedPlans.length > 0
      ? completedPlans.reduce((acc, _plan) => {
  ...
  return {
    ...
    treatment_outcomes: {
      successful:
        outcomes?.filter((o) => o.outcome?.coding?.[0]?.code === 'successful')
          .length || 0,
      partial:
        outcomes?.filter((o) => o.outcome?.coding?.[0]?.code === 'partial')
          .length || 0,
      unsuccessful:
        outcomes?.filter((o) => o.outcome?.coding?.[0]?.code === 'unsuccessful')
          .length || 0,
    },
  };
}
