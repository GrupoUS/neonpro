  ...
  try {
    ...
    if (operationType === 'invoice_calculation') {
      await supabase
        .from('invoices')
        .update({
          shadow_validation_status: shadowResult.is_valid
            ? 'validated'
            : 'failed',
...
async function performShadowCalculation(
  ...
  const variancePercentage =
    original.total > 0 ? (variance / original.total) * 100 : 0;
  ...
  return {
    ...
    is_valid:
      variance <= tolerance && variancePercentage <= tolerancePercentage,
    tolerance_exceeded:
      variance > tolerance || variancePercentage > tolerancePercentage,
  };
}
...
export async function getFinancialSummary(
  ...
  try {
    ...
    if (invoices) {
      ...
      summary.total_pending = invoices
        .filter((inv) => inv.payment_status === 'pending')
        .reduce((sum, inv) => sum + inv.total_amount, 0);
      summary.total_overdue = invoices
