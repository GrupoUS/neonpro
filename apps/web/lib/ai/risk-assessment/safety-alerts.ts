...
class SafetyAlertsSystem {
      ...
      const alert: SafetyAlert = {
        ...
        metadata: {
          ...
          expiresAt: options?.expiresIn
            ? new Date(now.getTime() + options.expiresIn * 60_000)
            : undefined,
          escalationTime: options?.escalateIn
            ? new Date(now.getTime() + options.escalateIn * 60_000)
            : undefined,
        },
   ...
   */
  async getAlertStatistics(timeframe?: {
    start: Date;
    end: Date;
    ...
    try {
      ...
      stats.resolutionRate =
        alerts.length > 0 ? resolvedAlerts.length / alerts.length : 0;
      ...
      if (responseTimes.length > 0) {
        ...
        stats.responseTime.median =
          responseTimes[Math.floor(responseTimes.length / 2)];
        stats.responseTime.percentile95 =
    ...
    try {
      await this.monitorEquipmentStatus(equipment.id, {
        ...
        nextMaintenance: equipment.next_maintenance
          ...
          : undefined,
        errorCodes: equipment.error_codes
          ? JSON.parse(equipment.error_codes)
          : undefined,
  ...
  private async handleTreatmentUpdate(treatment: any): Promise<void> {
    try {
      ...
      if (
        treatment.status === 'emergency' ||
        treatment.status === 'complication'
      ) {
