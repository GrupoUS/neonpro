...
class MLRiskAssessmentEngine {
   ...
   */
  private async calculateCategoryRisks(input: RiskAssessmentInput): Promise<
    Array<{
      category: RiskCategory;
      severity: RiskSeverity;
      score: number;
  ...
  private calculateOverallRisk(
    ...
    categoryRisks.forEach((risk) => {
      ...
      if (
        this.severityToNumber(risk.severity) >
        this.severityToNumber(maxSeverity)
      ) {
    ...
    const finalSeverity =
      this.severityToNumber(maxSeverity) >
      this.severityToNumber(calculatedSeverity)
        ? maxSeverity
        : calculatedSeverity;
   ...
   */
  private getModelWeights(_category: RiskCategory): Record<string, number> {
