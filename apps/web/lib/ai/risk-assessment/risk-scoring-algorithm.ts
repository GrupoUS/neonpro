...
class RiskScoringAlgorithm {
  ...
  private async calculateComponents(
    ...
    components.historicalFactors.recovery =
      this.calculateRecoveryScore(historicalData);
    components.historicalFactors.adherence =
      this.calculateAdherenceScore(historicalData);
  ...
  private calculateMedicalScore(patientData: any): number {
    ...
    conditions.forEach((condition: string) => {
      ...
      if (highRiskConditions.some((risk) => conditionLower.includes(risk))) {
        ...
        mediumRiskConditions.some((risk) => conditionLower.includes(risk))
      ) {
        score += 10;
      } else {
  ...
  private normalizeScore(totalScore: number, treatmentType?: string): number {
    ...
    if (
      treatmentType &&
      this.config.adjustments.treatmentMultiplier[treatmentType]
    ) {
      adjustedScore *=
        this.config.adjustments.treatmentMultiplier[treatmentType];
    }
  ...
  private async analyzeTrends(patientId: string, currentScore: number) {
    try {
      ...
      return {
        direction:
          trend > 5 ? 'worsening' : trend < -5 ? 'improving' : 'stable',
   ...
   */
  private async calculateBenchmarks(inputData: any, _currentScore: number) {
    try {
      // Population average
      ...
      const treatmentTypeAverage =
        this.populationStats?.treatmentTypes?.[treatmentType] || 25;

      // Facility average
      const facilityType = inputData.environmentalFactors.facilityType;
      const facilityAverage =
        this.populationStats?.facilityTypes?.[facilityType] || 25;
