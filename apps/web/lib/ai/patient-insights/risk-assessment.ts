...
export class RiskAssessmentEngine {
  ...
  private assessMedicalConditionRisk(condition: any): RiskFactor {
    const riskMap: Record<
      string,
      { severity: RiskFactor['severity']; weight: number }
    > = {
    ...
    const riskInfo = riskMap[condition.condition_type] || {
  ...
  private assessLifestyleRisks(lifestyle: any): RiskFactor[] {
    ...
    if (lifestyle.alcohol_consumption === 'heavy') {
      risks.push({
        ...
        confidence: 0.8,
        description:
          'Heavy alcohol use affects healing and medication metabolism',
        evidenceLevel: 'moderate',
      });
    }
  ...
  private calculateOverallRiskScore(riskFactors: RiskFactor[]): number {
    ...
    const weightedScore = riskFactors.reduce((total, factor) => {
      ...
      return (
        total + factor.weight * severityMultiplier * factor.confidence * 25
      );
    }, 0);
  ...
  private async checkDrugInteractions(
    ...
    knownInteractions.forEach((interaction) => {
      if (
        medNames.includes(interaction.drug1) &&
        medNames.includes(interaction.drug2)
      ) {
        alerts.push({
  ...
  private calculateConfidenceScore(
    ...
    return (
      Math.round(((factorConfidence + predictionConfidence) / 2) * 100) / 100
    );
  }
