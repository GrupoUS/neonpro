...
export class DriftDetectionSystem {
	...
	async runDriftDetection(): Promise<DriftAlert[]> {
		...
		try {
			...
			const { data: models, error } = await this.supabase
				...
				.select("*")
				.eq("is_active", true)
				.not("drift_detection_config", "is", null);
	...
	private async analyzeModelDrift(model: AIModel): Promise<DriftMetrics> {
		...
		const dataShiftScore = await this.calculateDataShift(recentWindow, referenceWindow);
		const performanceDriftScore = await this.calculatePerformanceDrift(model.id);
		const featureDriftScores = await this.calculateFeatureDrift(recentWindow, referenceWindow);
		const predictionDistributionDrift = this.calculatePredictionDistributionDrift(recentWindow, referenceWindow);
	 ...
	 */
	private async calculateDataShift(recent: any[], reference: any[]): Promise<number> {
		if (!recent.length || !reference.length) return 0;
		...
		if (recent[0]?.risk_score !== undefined) {
			...
			const referenceAvgRisk = reference.reduce((sum, p) => sum + p.risk_score, 0) / reference.length;
			...
			const referenceAvgConfidence = reference.reduce((sum, p) => sum + p.confidence, 0) / reference.length;

			const confidenceShift = Math.abs(recentAvgConfidence - referenceAvgConfidence);

			return Math.max(riskShift, confidenceShift);
		}

		// For general metrics, use response time and accuracy shifts
		const recentAvgResponse = recent.reduce((sum, p) => sum + (p.response_time || 0), 0) / recent.length;
		const referenceAvgResponse = reference.reduce((sum, p) => sum + (p.response_time || 0), 0) / reference.length;

		return Math.abs(recentAvgResponse - referenceAvgResponse) / Math.max(referenceAvgResponse, 1);
	}
	...
	private async calculatePerformanceDrift(modelId: string): Promise<number> {
		...
		return Math.max(0, (currentErrorRate - baselineErrorRate) / baselineErrorRate);
	}

	/**
	 * Calculate feature-specific drift
	 */
	private async calculateFeatureDrift(recent: any[], reference: any[]): Promise<Record<string, number>> {
		const featureDrift: Record<string, number> = {};

		if (!recent.length || !reference.length) return featureDrift;
		...
		if (recent[0]?.factors) {
			...
			for (const factor in recentFactors) {
				if (referenceFactors[factor]) {
					const drift = Math.abs(recentFactors[factor] - referenceFactors[factor]);
	 ...
	 */
	private calculatePredictionDistributionDrift(recent: any[], reference: any[]): number {
		if (!recent.length || !reference.length) return 0;
		...
		if (recent[0]?.risk_level) {
			...
			for (const level in recentDist) {
				if (referenceDist[level] > 0) {
					divergence += recentDist[level] * Math.log(recentDist[level] / referenceDist[level]);
				}
			}
	...
	private calculateTemporalDrift(recent: any[]): number {
		if (recent.length < 10) return 0; // Need minimum samples
		...
		const variances = Object.values(timeSlots).map((slot) => this.calculateVariance(slot as any[]));
	 ...
	 */
	private async createDriftAlert(model: AIModel, metrics: DriftMetrics): Promise<DriftAlert> {
	...
	private getDriftSeverity(score: number): "low" | "medium" | "high" | "critical" {
		if (score >= this.DRIFT_THRESHOLDS.critical) return "critical";
		if (score >= this.DRIFT_THRESHOLDS.high) return "high";
		if (score >= this.DRIFT_THRESHOLDS.medium) return "medium";
		return "low";
	}

	private getDriftType(metrics: DriftMetrics): "data_drift" | "concept_drift" | "performance_drift" {
		if (metrics.performanceDriftScore > 0.1) return "performance_drift";
		if (metrics.predictionDistributionDrift > 0.1) return "concept_drift";
		return "data_drift";
	}
	...
	private estimateImpact(model: AIModel, driftScore: number) {
		...
		if (model.name === "no-show-predictor") {
			...
			const monthlyRevenue = 468750 / 12; // Monthly portion of annual ROI
		...
		return {
			...
			revenueAtRisk: Math.round(driftScore * 10000),
	 ...
	 */
	private async recordDriftDetection(modelId: string, metrics: DriftMetrics): Promise<void> {
		try {
			const { error } = await this.supabase.from("model_drift_monitoring").insert({
				model_id: modelId,
	...
	private async sendImmediateAlert(alert: DriftAlert): Promise<void> {
		...
		console.warn(`Revenue at Risk: $${alert.estimatedImpact.revenueAtRisk.toLocaleString()}`);
