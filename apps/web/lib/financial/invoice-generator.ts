...
class AutomatedInvoiceGenerator {
  ...
  async generateFromAppointment(
    ...
    try {
      ...
      if (
        options?.generateNFSe !== false &&
        this.config.nfseIntegration.enabled
      ) {
        try {
  ...
  private calculateTaxes(
    ...
    const totalTaxes =
      issAmount + pisAmount + cofinsAmount + irAmount + csllAmount;
  ...
  async getInvoiceAnalytics(filters?: {
    ...
    try {
      ...
      for (const invoice of invoices) {
        ...
        for (const payment of payments) {
          paymentMethods[payment.method] =
            (paymentMethods[payment.method] || 0) + 1;
        }
      }
      ...
      const monthlyData: Record<
        string,
        { invoices: number; amount: number; paidAmount: number }
      > = {};

      for (const invoice of invoices) {
        const month = new Date(invoice.issue_date)
          .toISOString()
  ...
  private async generateInvoiceNumber(
    ...
    const lastNumber = lastInvoice?.number
      ? Number.parseInt(lastInvoice.number, 10)
      : 0;
