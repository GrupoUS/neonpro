...
export class FinancialAnalyticsCore {
  ...
  async calculateFinancialKPIs(
    ...
    const returnOnEquity = balanceSheet.equity.total_equity > 0 ? 
      (profitLoss.net_profit / balanceSheet.equity.total_equity) * 100 : 0;

    // Calculate liquidity KPIs
    const currentRatio = balanceSheet.liabilities.current_liabilities.total_current_liabilities > 0 ?
    ...
    const quickRatio = balanceSheet.liabilities.current_liabilities.total_current_liabilities > 0 ?
      (balanceSheet.assets.current_assets.cash_and_equivalents + balanceSheet.assets.current_assets.accounts_receivable) /
      balanceSheet.liabilities.current_liabilities.total_current_liabilities : 0;

    const cashRatio = balanceSheet.liabilities.current_liabilities.total_current_liabilities > 0 ?
      balanceSheet.assets.current_assets.cash_and_equivalents /
      balanceSheet.liabilities.current_liabilities.total_current_liabilities : 0;

    // Calculate efficiency KPIs
    const assetTurnover = balanceSheet.assets.total_assets > 0 ?
      profitLoss.revenue.total_revenue / balanceSheet.assets.total_assets : 0;

    const receivablesTurnover = balanceSheet.assets.current_assets.accounts_receivable > 0 ?
      profitLoss.revenue.total_revenue / balanceSheet.assets.current_assets.accounts_receivable : 0;

    const daysInAccountsReceivable = receivablesTurnover > 0 ? 365 / receivablesTurnover : 0;

    // Calculate leverage KPIs
    const debtToAssets = balanceSheet.assets.total_assets > 0 ?
      balanceSheet.liabilities.total_liabilities / balanceSheet.assets.total_assets : 0;

    const debtToEquity = balanceSheet.equity.total_equity > 0 ?
      balanceSheet.liabilities.total_liabilities / balanceSheet.equity.total_equity : 0;

    const equityMultiplier = balanceSheet.equity.total_equity > 0 ?
      balanceSheet.assets.total_assets / balanceSheet.equity.total_equity : 0;

    // Calculate cash flow KPIs
    const operatingCashFlowRatio = balanceSheet.liabilities.current_liabilities.total_current_liabilities > 0 ?
      cashFlow.operating_activities.net_cash_from_operations / 
      balanceSheet.liabilities.current_liabilities.total_current_liabilities : 0;

    const cashFlowToDebt = balanceSheet.liabilities.total_liabilities > 0 ?
      cashFlow.operating_activities.net_cash_from_operations / balanceSheet.liabilities.total_liabilities : 0;

    const freeCashFlow = cashFlow.operating_activities.net_cash_from_operations + 
  ...
  async calculatePerformanceMetrics(
    ...
    const revenueGrowthRate = await this.calculateGrowthRate(
      clinicId, 'revenue', period_start, period_end
    );
    const averageTransactionValue = revenueData.total_revenue / (appointmentData.total_appointments || 1);
    const revenuePerSquareMeter = 0; // TODO: Implement clinic space tracking

    // Calculate operational metrics
    const patientRetentionRate = await this.calculatePatientRetentionRate(clinicId, parameters);
    ...
    const costGrowthRate = await this.calculateGrowthRate(
      clinicId, 'expenses', period_start, period_end
    );
    const costPerPatient = expenseData.cost_per_patient;
    const costPerAppointment = expenseData.total_expenses / (appointmentData.total_appointments || 1);
    ...
    return {
      ...
      benchmarks: {
        ...
        historical_performance: await this.getHistoricalPerformance(clinicId, parameters)
      }
    };
  }
   ...
   */
  async generateDashboardData(clinicId: string): Promise<FinancialDashboardData> {
    const now = new Date();
    const currentMonth = {
    ...
    const alerts = this.generateFinancialAlerts(currentKPIs, performanceMetrics);
    const recommendations = this.generateRecommendations(currentKPIs, performanceMetrics);
    ...
    return {
      ...
      charts_data: {
        ...
        profitability_analysis: await this.getProfitabilityAnalysisData(clinicId),
        kpi_dashboard: {
  ...
  private async fetchAppointmentMetrics(
    ...
    if (error) throw new Error(`Appointment metrics fetch failed: ${error.message}`);

    const totalAppointments = appointmentData?.length || 0;
    const completedAppointments = appointmentData?.filter(apt => apt.status === 'completed').length || 0;
   ...
   */
  private calculateKPIChanges(current: KPICalculation, previous: KPICalculation) {
    return {
   ...
   */
  private generateFinancialAlerts(kpis: KPICalculation, performance: PerformanceMetrics) {
    const alerts: Array<{
   ...
   */
  private generateRecommendations(kpis: KPICalculation, performance: PerformanceMetrics) {
   ...
   */
  private async getHistoricalPerformance(clinicId: string, parameters: ReportParameters) {
   ...
   */
  private async getRevenueTrendData(clinicId: string) {
   ...
   */
  private async getCashFlowTimelineData(clinicId: string) {
   ...
   */
  private async getProfitabilityAnalysisData(clinicId: string) {
