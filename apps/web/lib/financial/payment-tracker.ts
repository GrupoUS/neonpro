...
class PaymentTracker {
  ...
  async processWebhook(
    ...
    try {
      ...
      if (
        signature &&
        !(await this.verifyWebhookSignature(gateway, webhookData, signature))
      ) {
    ...
    try {
      ...
      const reconciliation: PaymentReconciliation = {
        ...
        reconciledTransactions:
          ourPayments.length - discrepancies.missingPayments.length,
  ...
  private async handleStripeWebhook(_webhook: PaymentWebhook): Promise<void> {
    // Stripe webhook handling
  }

  private async handlePagarmeWebhook(_webhook: PaymentWebhook): Promise<void> {
  ...
  private calculatePaymentAnalytics(
    ...
    const _failedPayments = payments.filter((p) => p.status === 'failed');
