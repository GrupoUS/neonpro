...
export class RevenueAnalyticsEngine {
  ...
  async analyzeRevenueByService(
    ...
    const { data: serviceData, error } = await this.supabase.rpc(
      'analyze_service_revenue',
      {
    ...
    const enrichedData = await Promise.all(
      serviceData.map(async (service: any) => {
        ...
        return {
          ...
          totalRevenue: Number.parseFloat(service.total_revenue),
          transactionCount: Number.parseInt(service.transaction_count, 10),
          averageValue: Number.parseFloat(service.average_value),
          costAllocation,
          profitMargin:
            ((Number.parseFloat(service.total_revenue) - costAllocation) /
              Number.parseFloat(service.total_revenue)) *
            100,
          growthRate,
          seasonalIndex,
        };
  ...
  async calculateServiceProfitability(
    ...
    const { data: overheadData } = await this.supabase.rpc(
      'calculate_overhead_allocation',
      {
  ...
  async analyzeProviderPerformance(
    ...
    const { data: providerData, error } = await this.supabase.rpc(
      'analyze_provider_revenue',
      {
        p_clinic_id: clinicId,
        p_start_date: dateRange.start.toISOString(),
        p_end_date: dateRange.end.toISOString(),
  ...
  async generateTimePeriodAnalysis(
    ...
    return timeSeriesData.map((period: any) => ({
      ...
      revenue: Number.parseFloat(period.revenue),
      transactions: Number.parseInt(period.transactions, 10),
      averageValue: Number.parseFloat(period.average_value),
      growthRate: Number.parseFloat(period.growth_rate),
      seasonalIndex: Number.parseFloat(period.seasonal_index),
  ...
  async calculatePatientLifetimeValue(
    ...
    return ltvData.map((patient: any) => ({
      ...
      totalLifetimeValue: Number.parseFloat(patient.total_ltv),
      averageVisitValue: Number.parseFloat(patient.avg_visit_value),
      visitFrequency: Number.parseFloat(patient.visit_frequency),
      retentionRate: Number.parseFloat(patient.retention_rate),
      churnRisk: Number.parseFloat(patient.churn_risk),
      nextVisitProbability: Number.parseFloat(patient.next_visit_probability),
  ...
  async generateRevenueForecast(
    ...
    return forecastData.map((forecast: any) => ({
      ...
      forecastedRevenue: Number.parseFloat(forecast.forecasted_revenue),
      confidenceInterval: {
      ...
      growthRate: Number.parseFloat(forecast.growth_rate),
    }));
  }
