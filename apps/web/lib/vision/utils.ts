...
import { QUALITY_THRESHOLDS, VISION_CONFIG } from './config';
import type {
  AnalysisResult,
  Coordinates,
  ErrorCode,
...
export class AnalysisUtils {
  ...
  static formatProcessingTime(timeMs: number): string {
    ...
    const seconds = Math.round((timeMs % 60_000) / 1000);
   ...
   */
  static formatMeasurement(value: number, unit: string, precision = 2): string {
  ...
  static validateAnalysisResult(
    ...
    if (result.analysisData) {
      ...
      if (
        analysisData.accuracyScore <
        VISION_CONFIG.PERFORMANCE.MIN_ACCURACY_THRESHOLD
      ) {
        warnings.push({
      ...
      if (
        ...
        VISION_CONFIG.PERFORMANCE.MIN_CONFIDENCE_THRESHOLD
      ) {
        warnings.push({
