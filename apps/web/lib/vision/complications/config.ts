...
const DETECTION_MODELS: Record<string, ModelConfig> = {
  infection_detector: {
    ...
    weightsUrl:
      '/models/complication-detection/infection-detector-v2.1-weights.bin',
  ...
  adverse_reaction_detector: {
    ...
    weightsUrl:
      '/models/complication-detection/adverse-reaction-detector-v1.8-weights.bin',
    inputShape: [224, 224, 3],
    outputClasses: [
      'no_reaction',
  ...
  healing_issue_detector: {
    ...
    weightsUrl:
      '/models/complication-detection/healing-issue-detector-v2.0-weights.bin',
    inputShape: [224, 224, 3],
    outputClasses: [
      'normal_healing',
      'delayed_healing',
  ...
  procedural_complication_detector: {
    ...
    weightsUrl:
      '/models/complication-detection/procedural-complication-detector-v1.5-weights.bin',
    inputShape: [224, 224, 3],
    outputClasses: [
      'normal_result',
      'asymmetry',
...
const COMPLICATION_RISK_WEIGHTS: Record<ComplicationCategory, number> = {
  ...
  device_malfunction: 0.8, // High risk - equipment safety
  other: 0.5, // Variable risk - case-by-case
};

// Processing Configuration
const PROCESSING_CONFIG = {
  maxProcessingTime: 30_000, // 30 seconds maximum
...
const QUALITY_CONFIG = {
  ...
  validationTimeout: 86_400_000, // 24 hours for validation
  minimumAccuracy: 0.9, // Story requirement: ≥90% accuracy
  ...
  performanceMonitoringInterval: 3_600_000, // 1 hour
  qualityReportInterval: 86_400_000, // Daily quality reports
};
...
const ALERT_CONFIG = {
  ...
  escalationTimeouts: {
    none: 0,
    low: 14_400_000, // 4 hours
    medium: 3_600_000, // 1 hour
    high: 900_000, // 15 minutes
  ...
  maxRetryAttempts: 5,
  retryDelayMs: 30_000, // 30 seconds between notification retries
  acknowledgmentTimeout: 1_800_000, // 30 minutes to acknowledge critical alerts
  autoEscalationEnabled: true,
  escalationDelayMs: 900_000, // 15 minutes before auto-escalation
};

// Model Performance Benchmarks
const PERFORMANCE_BENCHMARKS = {
  minimumAccuracy: 0.9, // Story requirement
  targetAccuracy: 0.95, // Target for excellence
  minimumPrecision: 0.88,
  minimumRecall: 0.92,
  minimumF1Score: 0.9,
  maximumProcessingTime: 30_000, // 30 seconds
  targetProcessingTime: 15_000, // 15 seconds target
...
const SECURITY_CONFIG = {
  ...
  requireMFA: true,
  sessionTimeoutMs: 3_600_000, // 1 hour session timeout
...
export function validateConfiguration(): {
  ...
  if (PROCESSING_CONFIG.maxProcessingTime > 60_000) {
