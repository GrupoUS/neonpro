...
export const ANVISASaMDRegistrationSchema = z.object({
  ...
  situacao_registro: z.enum(['em_analise', 'deferido', 'indeferido', 'suspenso']),
...
export type ANVISASaMDRegistration = z.infer<typeof ANVISASaMDRegistrationSchema>;
...
export const AdverseEventSchema = z.object({
  ...
  // Investigation
  investigation_status: z.enum(['pending', 'investigating', 'completed', 'escalated']),
 ...
 */export class ANVISASaMDComplianceManager {
  ...
  async implementCybersecurityControls(request: {
    ...
    try {
      ...
      const controlsImplementation = await this.validateCybersecurityImplementation(
        request.clinic_id,
        requiredControls
      );

      // 📊 Calculate compliance score
      const complianceScore = this.calculateCybersecurityCompliance(controlsImplementation);
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Cybersecurity implementation failed' 
      };
    }
  }
  ...
  async reportAdverseEvent(request: {
    ...
    try {
      ...
      const notificationRequired = this.isANVISANotificationRequired(request.severity);
      ...
      if (notificationRequired) {
        const notificationResult = await this.sendANVISANotification(adverseEvent as AdverseEvent);
  ...
  async implementHL7FHIRCompliance(request: {
    ...
    try {
      ...
      const fhirValidation = await this.validateFHIREndpoints(request.endpoints);
      ...
      const fhirCompliance = {
        ...
        next_validation_due: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toISOString() // 6 months
      };
      ...
      const interoperabilityScore = this.calculateInteroperabilityScore(fhirCompliance);
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'HL7 FHIR implementation failed' 
      };
    }
  }
  ...
  async setupPostMarketMonitoring(request: {
    ...
    try {
      ...
      const monitoringPlan = {
        ...
        monitoring_protocols: this.getMonitoringProtocols(request.monitoring_types),
        
        compliance_officer: request.compliance_officer,
        start_date: new Date().toISOString(),
        next_review_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() // 1 month
      };
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Post-market monitoring setup failed' 
      };
    }
  }  /**
  ...
  async generateANVISAComplianceReport(request: {
    clinic_id: string;
    report_type: 'cybersecurity' | 'adverse_events' | 'post_market' | 'comprehensive';
    date_from: Date;
    date_to: Date;
    compliance_officer: string;
    ...
    try {
      ...
      const report = {
        ...
        compliance_summary: {
          ...
          cybersecurity_compliance: complianceData.cybersecurity?.compliance_score || 0,
          adverse_events_handled: complianceData.adverse_events?.length || 0,
          post_market_monitoring_active: complianceData.post_market?.active || false
        },
      ...
      // 💾 Store report
      await this.supabase
        .from('anvisa_compliance_reports')
        .insert({
          ...report,
      ...
      await this.auditANVISAActivity({
        ...
        metadata: {
          ...
          period_days: Math.ceil((request.date_to.getTime() - request.date_from.getTime()) / (1000 * 60 * 60 * 24))
        }
      });
  ...
  private getCybersecurityControlsForClass(
    ...
    if (samdClass === ANVISASaMDClass.CLASSE_III || cybersecurityLevel === CybersecurityLevel.CRITICO) {
      return {
        ...baseControls,
    ...
    if (samdClass === ANVISASaMDClass.CLASSE_IIB || cybersecurityLevel === CybersecurityLevel.ELEVADO) {
      return {
        ...baseControls,
        vulnerability_management: true,
        threat_modeling: true,
    ...
    for (const [control, required] of Object.entries(requiredControls)) {
      if (required) {
        ...
        implementation[control] = {
          ...
          compliance_status: 'compliant',
          next_review: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString() // 3 months
        };
      }
    }
   ...
   */
  private calculateCybersecurityCompliance(implementation: Record<string, any>): number {
   ...
   */
  private isANVISANotificationRequired(severity: AdverseEventSeverity): boolean {
    return [
   ...
   */
  private async triggerCriticalEventAlert(eventId: string, clinicId: string): Promise<void> {
    try {
      const criticalAlert = {
        id: crypto.randomUUID(),
        ...
        clinic_id: clinicId,
        event_id: eventId,
        message: 'Critical adverse event reported - immediate investigation required',
   ...
   */
  private async validateFHIREndpoints(endpoints: string[]): Promise<{ valid: boolean; error?: string }> {
    try {
      for (const endpoint of endpoints) {
        // 🔍 Basic URL validation
        if (!endpoint.startsWith('https://')) {
          return { valid: false, error: `Endpoint must use HTTPS: ${endpoint}` };
        }

        // 🏥 FHIR endpoint validation (simplified)
        if (!endpoint.includes('/fhir/')) {
          return { valid: false, error: `Invalid FHIR endpoint format: ${endpoint}` };
        }
      }
   ...
   */
  private getMonitoringProtocols(monitoringTypes: PostMarketMonitoringType[]): Record<string, any> {
    ...
    monitoringTypes.forEach(type => {
      switch (type) {
        case PostMarketMonitoringType.DESEMPENHO_CLINICO:
          protocols[type] = {
            metrics: ['diagnostic_accuracy', 'treatment_effectiveness', 'clinical_outcomes'],
            frequency: 'monthly',
        ...
        case PostMarketMonitoringType.SEGURANCA:
          protocols[type] = {
            metrics: ['security_incidents', 'vulnerabilities', 'access_violations'],
            frequency: 'weekly',
        ...
        case PostMarketMonitoringType.CIBERSEGURANCA:
          protocols[type] = {
            metrics: ['threat_detection', 'intrusion_attempts', 'data_breaches'],
            frequency: 'daily',
        ...
        case PostMarketMonitoringType.INTEROPERABILIDADE:
          protocols[type] = {
            metrics: ['fhir_compliance', 'data_exchange_success', 'integration_errors'],
            frequency: 'monthly',
   ...
   */
  private async scheduleMonitoringAlerts(monitoringId: string, clinicId: string): Promise<void> {
    try {
      const scheduledAlerts = {
        ...
        next_alert_due: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
        created_at: new Date().toISOString()
      };
  ...
  private calculateANVISAMetrics(complianceData: any): any {
    return {
      cybersecurity_score: complianceData.cybersecurity?.[0]?.compliance_score || 0,
      adverse_events_count: complianceData.adverse_events?.length || 0,
      critical_events_count: complianceData.adverse_events?.filter(
        (event: any) => event.severity === AdverseEventSeverity.CRITICO
      ).length || 0,
      post_market_monitoring_active: complianceData.post_market?.length > 0,
      anvisa_notifications_sent: complianceData.adverse_events?.filter(
  ...
  private determineOverallComplianceStatus(complianceData: any): string {
    const cybersecurityScore = complianceData.cybersecurity?.[0]?.compliance_score || 0;
    const criticalEvents = complianceData.adverse_events?.filter(
      (event: any) => event.severity === AdverseEventSeverity.CRITICO
    ).length || 0;
  ...
  private assessRDC657Compliance(complianceData: any): any {
    return {
      cybersecurity_controls_implemented: complianceData.cybersecurity?.[0]?.compliance_status === 'compliant',
      adverse_event_reporting_active: complianceData.adverse_events !== undefined,
      post_market_monitoring_established: complianceData.post_market?.length > 0,
      overall_rdc_657_compliance: this.determineOverallComplianceStatus(complianceData) !== 'NON_COMPLIANT'
    };
  }
