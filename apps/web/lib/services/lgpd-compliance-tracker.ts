...
export class LGPDComplianceTracker {
  ...
  async calculateComplianceScore(
    ...
    try {
      ...
      const remediation_actions = await this.generateRemediationActions(
        tenantId,
        violations,
      );
  ...
  private async evaluateBreachResponse(tenantId: string) {
    ...
    if (breaches) {
      const unreportedBreaches = breaches.filter(
        (b) => !b.reported_to_authority && b.severity in ["high", "critical"],
      );
  ...
  private async storeComplianceScore(
    tenantId: string,
    score: LGPDComplianceScore,
  ) {
