...
class VoiceNavigationService {
  private recognition: SpeechRecognition | null = undefined;
      ...
      switch (voiceCommand.action) {
        ...
        case "search_patient": {
          if (parameters[1]) {
            ...
            return {
              success: true,
              message: `Buscando paciente ${patientName}`,
            };
          }
        ...
        case "show_patient_history": {
          if (parameters[1]) {
            ...
            return {
              success: true,
              message: `Abrindo histórico de ${parameters[1]}`,
            };
          }
          return { success: false, error: "Nome do paciente não especificado" };
        }
        ...
        case "check_inventory": {
          if (parameters[1]) {
            ...
            return {
              success: true,
              message: `Verificando estoque de ${parameters[1]}`,
            };
          }
  ...
  private async logUnrecognizedCommand(
    command: string,
    confidence: number,
  ): Promise<void> {
    await supabase.from("assistant_logs").insert({
