...
export class ConfigurationService {
	...
	async getConfiguration(tenantId: string, key: string): Promise<{ value?: unknown; error?: string }> {
		try {
	...
	async deleteConfiguration(tenantId: string, key: string): Promise<{ success?: boolean; error?: string }> {
		try {
			const { error } = await supabase.from("tenant_configurations").delete().eq("tenant_id", tenantId).eq("key", key);
