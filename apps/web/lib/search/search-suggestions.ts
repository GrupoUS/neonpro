...
export class SearchSuggestions {
    ...
    try {
      ...
      const [
        ...
        options.includeHistory
          ? this.getHistoricalSuggestions(query, context)
          : [],
        options.includePopular
          ? this.getPopularSuggestions(query, context)
          : [],
        options.includeContextual
          ? this.getContextualSuggestions(query, context)
          : [],
        options.includePersonalized
          ? this.getPersonalizedSuggestions(query, context)
          : [],
  ...
  private getPrefixCompletions(query: string): QueryCompletion[] {
    ...
    for (const [popularQuery, frequency] of this.popularQueries.entries()) {
      if (
        popularQuery.toLowerCase().startsWith(lowerQuery) &&
        popularQuery !== query
      ) {
        completions.push({
  ...
  private async getSemanticCompletions(
    ...
    try {
      ...
      if (
        nlpResult.intent.name === 'find_patient' &&
        nlpResult.intent.confidence > 0.6
      ) {
        completions.push({
  ...
  private async getContextualSuggestions(
    ...
    try {
      ...
      contextualQueries.forEach((contextQuery) => {
        if (contextQuery.toLowerCase().includes(query.toLowerCase())) {
          suggestions.push({
            ...
            metadata: {
              source: 'page_context',
              page: context.sessionContext.currentPage,
            },
          });
        }
      });
      ...
      context.sessionContext.previousSearches.forEach((prevSearch) => {
        if (
          prevSearch.toLowerCase().includes(query.toLowerCase()) &&
          prevSearch !== query
        ) {
          suggestions.push({
  ...
  async learnFromInteraction(data: LearningData): Promise<void> {
    try {
      ...
      await this.supabase.from('search_analytics').upsert(
        {
          query: data.query,
          frequency: 1,
   ...
   */
  private async updateUserPreferences(_data: LearningData): Promise<void> {
