...
export class UnifiedSearchSystem {
  ...
  async search(query: SearchQuery): Promise<SearchResponse> {
    ...
    try {
      ...
      const suggestions =
        nlpAnalysis?.suggestions || this.generateSearchSuggestions(query.term);
  ...
  private async searchPatients(query: SearchQuery): Promise<SearchResult[]> {
    try {
      ...
      for (const patient of mockPatients) {
        ...
        if (
          ageFilter &&
          patient.age >= (ageFilter.min || 0) &&
          patient.age <= (ageFilter.max || 120)
        ) {
          matches = true;
          matchScore += 0.2;
  ...
  private async searchInsights(query: SearchQuery): Promise<SearchResult[]> {
    try {
      ...
      for (const insight of mockInsights) {
        if (
          this.matchesSearchTerm(query.term, insight.title, insight.description)
        ) {
          results.push({
            id: insight.id,
            type: 'insights',
  ...
  private async searchDuplicates(query: SearchQuery): Promise<SearchResult[]> {
    try {
      ...
      for (const duplicate of duplicates) {
        if (
          this.matchesSearchTerm(query.term, duplicate.id, duplicate.status)
        ) {
          results.push({
            id: duplicate.id,
            type: 'duplicates',
   ...
   * Busca documentos
   */
  private async searchDocuments(_query: SearchQuery): Promise<SearchResult[]> {
    try {
  ...
  private generateSearchFacets(
    ...
    facets.type = Object.entries(typeCount).map(([value, count]) => ({
      value,
      count,
    }));
   ...
   */
  async getSearchStatistics(_timeframe = '30days'): Promise<GlobalSearchStats> {
    try {
