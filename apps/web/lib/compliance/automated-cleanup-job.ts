...
export class AutomatedCleanupJob {
  ...
  async execute(): Promise<CleanupJobResult> {
    ...
    try {
      ...
      console.log(`⚙️ Config: batch_size=${this.config.batch_size}, dry_run=${this.config.dry_run}`);
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(new Error(`Job timeout after ${this.config.max_runtime_minutes} minutes`));
      ...
      console.log(`📊 Processados: ${result.total_processed}, Anonimizados: ${result.anonymized_count}, Deletados: ${result.deleted_count}`);
      ...
      if (this.config.notify_on_completion && this.config.notification_emails.length > 0) {
  ...
  private async performDryRun(result: CleanupJobResult): Promise<void> {
    ...
    const tables = ['patients', 'consultations', 'transactions', 'appointments', 'invoices'];
    
    for (const table of tables) {
      try {
        const complianceStatus = await this.retentionEngine.checkComplianceStatus(table);
        
        console.log(`📊 ${table}: ${complianceStatus.total_records} registros, ${complianceStatus.records_need_anonymization} precisam anonimização, ${complianceStatus.records_need_deletion} precisam deleção`);
  ...
  private async performRealCleanup(result: CleanupJobResult): Promise<void> {
    console.log('⚠️ Executando limpeza REAL - Dados serão modificados permanentemente');
    
    try {
      // Executar políticas de retenção
      const complianceRecords = await this.retentionEngine.executeRetentionPolicies();
      
      // Processar resultados
      result.total_processed = complianceRecords.length;
      result.anonymized_count = complianceRecords.filter(r => r.action === 'anonymized').length;
      result.deleted_count = complianceRecords.filter(r => r.action === 'deleted').length;
      
      console.log(`📊 Limpeza concluída: ${result.total_processed} ações executadas`);
   ...
   */
  private async sendCompletionNotification(result: CleanupJobResult): Promise<void> {
    try {
      const isSuccess = result.status === 'completed' && result.errors.length === 0;
      ...
      console.log(`📧 Enviando notificação para: ${this.config.notification_emails.join(', ')}`);
   ...
   */
  private generateNotificationBody(result: CleanupJobResult, isSuccess: boolean): string {
...
Para mais detalhes, acesse o dashboard de compliance.
   ...
   */
  async getJobHistory(limit: number = 10): Promise<CleanupJobResult[]> {
    try {
