...
export class HealthcareComplianceManager {
  ...
  private async validateStandardCompliance(
    ...
    return {
      ...
      nonComplianceIssues:
        await this.identifyNonComplianceIssues(validationResults),
    };
  }
  ...
  async generateComplianceReport(
    ...
    try {
      const components = componentId
        ? [componentId]
        : Array.from(this.complianceCache.keys());
  ...
  private isValidationCurrent(compliance: MedicalDeviceCompliance): boolean {
    const validationAge =
      Date.now() - new Date(compliance.lastAssessment).getTime();
  ...
  private getStandardExpirationDate(
    ...
    return expirationPeriod
      ? new Date(Date.now() + expirationPeriod).toISOString()
      : undefined;
  }
