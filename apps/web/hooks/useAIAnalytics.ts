...
export function useAIAnalytics(config: AIAnalyticsConfig = {}) {
  ...
  const startAISession = useCallback((featureType: AIFeatureType): string => {
  ...
  const trackAIRecommendation = useCallback(async (
    sessionId: string,
    accepted: boolean,
    userSatisfaction?: number
  ) => {
  ...
  const loadAIUsageStats = useCallback(async () => {
    try {
      ...
      if (recentAnalytics && recentAnalytics.length > 0) {
        ...
        const successfulQueries = recentAnalytics.filter(a => 
