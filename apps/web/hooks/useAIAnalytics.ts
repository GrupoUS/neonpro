...
export function useAIAnalytics(config: AIAnalyticsConfig = {}) {
	...
	const [currentSessions, setCurrentSessions] = useState<Map<string, SessionContext>>(new Map());
		...
		async (
			...
			try {
				const analytic: Omit<AIUsageAnalytic, "id" | "created_at"> = {
					...
					success_rate: metadata.success !== undefined ? (metadata.success ? 100 : 0) : undefined,
					...
					business_impact: {
						estimated_roi: calculateFeatureROI(featureType, actionType, metadata.success),
				...
				if (analyticsBuffer.current.length >= batchSize || timeSinceLastFlush > flushInterval) {
	...
	const calculateFeatureROI = useCallback(
		(featureType: AIFeatureType, actionType: AIActionType, success?: boolean): number => {
			if (!success || actionType !== "session_completed") return 0;
	...
	const trackAIQuery = useCallback(
		async (sessionId: string, success: boolean, responseTime?: number, accuracy?: number) => {
			const session = currentSessions.get(sessionId);
			if (!session) return;

			// Update session context
			session.queryCount++;
			if (success) session.successCount++;
	...
	const trackAIRecommendation = useCallback(
		async (sessionId: string, accepted: boolean, userSatisfaction?: number) => {
			...
			const actionType = accepted ? "recommendation_accepted" : "recommendation_rejected";
	...
	const endAISession = useCallback(
		async (sessionId: string, userSatisfaction?: number) => {
			...
			const successRate = session.queryCount > 0 ? (session.successCount / session.queryCount) * 100 : 0;
	...
	const flushAnalyticsBuffer = useCallback(async () => {
		...
		try {
			...
			const { error } = await supabase.from("ai_usage_analytics").insert(analytics);
	...
	const loadAIUsageStats = useCallback(async () => {
		try {
			...
			if (recentAnalytics && recentAnalytics.length > 0) {
				...
				const sessions = new Set(recentAnalytics.map((a) => a.session_id).filter(Boolean));
				const activeFeatures = new Set(recentAnalytics.map((a) => a.feature_type));

				const successfulQueries = recentAnalytics.filter(
					(a) => a.action_type === "query_submitted" && (a.success_rate || 0) > 0
				);
				const totalQueries = recentAnalytics.filter((a) => a.action_type === "query_submitted");

				const avgSuccessRate = totalQueries.length > 0 ? (successfulQueries.length / totalQueries.length) * 100 : 0;
				...
				for (const featureType of activeFeatures) {
					const featureAnalytics = recentAnalytics.filter((a) => a.feature_type === featureType);
					const featureSessions = new Set(featureAnalytics.map((a) => a.session_id).filter(Boolean));
					const featureQueries = featureAnalytics.filter((a) => a.action_type === "query_submitted");
					const featureSuccesses = featureQueries.filter((a) => (a.success_rate || 0) > 0);

					const avgQueries = featureSessions.size > 0 ? featureQueries.length / featureSessions.size : 0;

					const featureSuccessRate =
						featureQueries.length > 0 ? (featureSuccesses.length / featureQueries.length) * 100 : 0;
					...
					const avgSatisfaction =
						userSatisfactions.length > 0
							? userSatisfactions.reduce((sum, s) => sum + s, 0) / userSatisfactions.length
							: 0;

					// Calculate clinic breakdown (would need clinic data in analytics)
					const clinicBreakdown: Array<{
						clinic_id: string;
						clinic_name: string;
					...
					featuresBreakdown.push({
						...
						active_users: new Set(featureAnalytics.map((a) => a.user_id).filter(Boolean)).size,
						...
						revenue_impact: featureAnalytics.reduce((sum, a) => sum + (a.business_impact?.estimated_roi || 0), 0),
	...
	return {
		...
		aiUsageStats,
		loading,
		currentSessions: Array.from(currentSessions.values()),
