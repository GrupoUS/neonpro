...
export function useSystemHealth(config: SystemHealthConfig = {}) {
  ...
  const checkGenericService = useCallback(async (
    component: SystemComponent,
    endpoint: string
  ...
  const checkComponentHealth = useCallback(async (component: SystemComponent): Promise<HealthCheckResult> => {
    switch (component) {
  ...
  const recordHealthCheck = useCallback(async (result: HealthCheckResult) => {
    try {
  ...
  const performHealthCheck = useCallback(async (components?: SystemComponent[]) => {
  ...
  const loadHealthSummary = useCallback(async () => {
    try {
      ...
      if (healthChecks && healthChecks.length > 0) {
        ...
        const overallUptime = latestChecksArray
          .reduce((sum, c) => sum + (c.uptime_percentage || 100), 0) / totalComponents;
