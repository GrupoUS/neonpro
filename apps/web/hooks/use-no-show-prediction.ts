...
export interface UseNoShowPredictionReturn {
  ...
  getPredictionForAppointment: (appointmentId: string) => NoShowPrediction | undefined;
...
export function useNoShowPrediction(options: UseNoShowPredictionOptions = {}): UseNoShowPredictionReturn {
  ...
  const getPredictionForAppointment = useCallback((appointmentId: string) => {
    return predictions.find(p => p.appointmentId === appointmentId);
  }, [predictions]);

  const calculateRiskLevel = useCallback((prediction: NoShowPrediction): RiskLevel => {
  ...
  useEffect(() => {
    ...
    const loadAppointments = async () => {
      ...
      const enhancedAppointments = mockAppointments.map(appointment => {
        const prediction = predictions.find(p => p.appointmentId === appointment.id);
        const riskLevel = prediction ? calculateRiskLevel(prediction) : undefined;
