...
export function useUpdateAppointment(id: string) {
  ...
  return queryUtils.createMutation({
    ...
    ...queryUtils.createOptimisticUpdate<AppointmentBase>(
      QueryKeys.appointments.detail(id),
      (oldData) => oldData ? { ...oldData, ...appointmentData } : oldData as AppointmentBase
    ),
...
export function useCheckInAppointment() {
  ...
  return queryUtils.createMutation({
    mutationFn: async ({ id, checkInData }: { 
      id: string; 
...
export function useRescheduleAppointment() {
  ...
  return queryUtils.createMutation({
    mutationFn: async ({ id, newDate, newTime, reason }: { 
      id: string; 
      newDate: string; 
      newTime: string; 
      reason?: string; 
...
export function useAvailableSlots() {
  ...
  return useCallback((professionalId: string, date: string, serviceId?: string) => {
    return queryUtils.createQuery({
      ...
      queryFn: async () => {
        const response = await apiClient.api.v1.appointments['available-slots'].$get({
          query: { professional_id: professionalId, date, service_id: serviceId }
        });
        return await response.json();
      },
      validator: (data: unknown) => {
...
export function useAppointmentUtils() {
  ...
  return useMemo(() => ({
    ...
    getAppointmentRevenue: (appointment: AppointmentBase): number => {
      return appointment.services_performed.reduce((total, service) => 
        total + service.final_price, 0);
    },
