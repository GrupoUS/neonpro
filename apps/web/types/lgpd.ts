...
export interface LGPDDataSubject {
  ...
  birthDate?: Date;
  address?: LGPDAddress;
  consentRecords: LGPDConsent[];
  dataProcessingRecords: LGPDDataProcessing[];
...
export interface LGPDConsentEvidence {
  ...
  method: string; // e.g., 'website_form', 'signed_document', 'phone_call'
  ipAddress?: string;
  userAgent?: string;
  timestamp: Date;
  witness?: string;
...
export interface LGPDDataProcessing {
  ...
  legalBasis: LGPDLegalBasis;
  categories: LGPDDataCategory[];
  processingActivities: LGPDProcessingActivity[];
  retentionPeriod: LGPDRetentionPeriod;
  thirdParties?: LGPDThirdParty[];
  internationalTransfers?: LGPDInternationalTransfer[];
  securityMeasures: LGPDSecurityMeasure[];
  startDate: Date;
  endDate?: Date;
...
export interface LGPDDataInventory {
  ...
  legalBases: LGPDLegalBasis[];
  sources: LGPDDataSource[];
  recipients: LGPDDataRecipient[];
  retentionPeriod: LGPDRetentionPeriod;
  securityMeasures: LGPDSecurityMeasure[];
  internationalTransfers?: LGPDInternationalTransfer[];
...
export interface LGPDRetentionPeriod {
  ...
  startEvent: string; // e.g., 'data_collection', 'contract_end', 'consent_withdrawal'
  endEvent: string; // e.g., 'purpose_fulfilled', 'legal_obligation_end'
  exceptions?: string[];
}
...
export interface LGPDRiskAssessment {
  ...
  threats: LGPDThreat[];
  vulnerabilities: LGPDVulnerability[];
  mitigationMeasures: LGPDMitigationMeasure[];
  residualRisk: number;
  assessmentDate: Date;
  nextAssessment: Date;
  assessor: string;
...
export interface LGPDIncidentResponse {
  containmentActions: LGPDResponseAction[];
  investigationActions: LGPDResponseAction[];
  recoveryActions: LGPDResponseAction[];
  communicationActions: LGPDResponseAction[];
  preventionActions: LGPDResponseAction[];
}
...
export interface LGPDComplianceStatus {
  ...
  nextAssessment: Date;
  assessor: string;
  recommendations: LGPDRecommendation[];
}
...
// Automation Types
export interface LGPDAutomationRule {
  id: string;
  name: string;
  description: string;
  trigger: LGPDAutomationTrigger;
  conditions: LGPDAutomationCondition[];
  actions: LGPDAutomationAction[];
  enabled: boolean;
  priority: number;
  lastExecuted?: Date;
