...
export function RouterProvider({ children, }: RouterProviderProps,) {
  ...
  // Handle middleware redirect targets (enforce TanStack as routing authority)
  React.useEffect(() => {
    if (typeof window === 'undefined') return
    const params = new URLSearchParams(window.location.search,)
    const toParam = params.get('__tsr_to',)
    if (toParam) {
      const target = decodeURIComponent(toParam,)
      // Navigate via TanStack router, then clean URL
      router.navigate({ to: target, },).finally(() => {
