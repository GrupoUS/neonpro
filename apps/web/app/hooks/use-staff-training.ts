...
interface UseStaffTrainingReturn {
  ...
  saveProgress: (moduleId: string, data: Partial<TrainingProgress>) => Promise<void>;
...
export function useStaffTraining({
  ...
  const startModule = useCallback(async (moduleId: string) => {
    ...
    try {
      ...
      if (unmetPrerequisites.length > 0) {
        throw new Error('Prerequisites not met. Please complete required modules first.');
      }
  ...
  const completeSection = useCallback(async (sectionId: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const currentIndex = currentModule.sections.findIndex(s => s.id === sectionId);
    ...
    try {
      ...
      if (data.success) {
        ...
        setUserProgress(prev => ({
          ...
          [currentModule.id]: {
            ...prev[currentModule.id],
            quizAttempts: [...(prev[currentModule.id]?.quizAttempts || []), attempt],
          },
        }));
  ...
  const getOverallProgress = useCallback((): number => {
    ...
    const completedCount = requiredModules.filter(moduleId => 
      userProgress[moduleId]?.status === 'completed'
    ).length;
    
    return requiredModules.length > 0 ? (completedCount / requiredModules.length) * 100 : 0;
  ...
  const downloadModuleForOffline = useCallback(async (moduleId: string) => {
    try {
      const response = await fetch(`/api/staff-training/offline-download/${moduleId}`);
