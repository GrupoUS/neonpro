...
interface UsePhase4ValidationReturn {
  ...
  validateEntity: (type: ValidationType, entityId: string, data: Record<string, any>) => Promise<ValidationSession>;
  ...
  updateRule: (id: string, updates: Partial<ValidationRule>) => Promise<ValidationRule>;
  ...
  updateConfig: (updates: Partial<ValidationConfig>) => Promise<ValidationConfig>;
  ...
  // Export functions
  exportResults: (sessionIds: string[], format: 'pdf' | 'excel' | 'csv') => Promise<Blob>;
  generateReport: (type: 'compliance' | 'performance' | 'summary', filters: any) => Promise<Blob>;
...
export function usePhase4Validation(options: UsePhase4ValidationOptions): UsePhase4ValidationReturn {
  // State management
  ...
  const activeSessions = useMemo(() => 
    sessions.filter(session => session.status === 'validating' || session.status === 'pending'),
    [sessions]
  );
  ...
  const updateSessionInState = useCallback((updatedSession: ValidationSession) => {
    setSessions(prev => 
      prev.map(session => 
  ...
  const getValidationResult = useCallback((sessionId: string): ValidationSession | null => {
    return sessions.find(session => session.id === sessionId) || null;
  }, [sessions]);

  // Retry validation
  const retryValidation = useCallback(async (sessionId: string): Promise<ValidationSession> => {
  ...
  // Load rules
  const loadRules = useCallback(async (type?: ValidationType) => {
    setLoading(true);
    try {
      ...
      setError(error instanceof Error ? error.message : 'Erro ao carregar regras');
    } finally {
      setLoading(false);
    }
  }, [options.clinic_id]);
  ...
  // Toggle rule
  const toggleRule = useCallback(async (id: string, enabled: boolean): Promise<void> => {
  ...
  const loadSessions = useCallback(async () => {
    try {
      ...
      setError(error instanceof Error ? error.message : 'Erro ao carregar sessões');
    }
  }, [options.clinic_id]);

  // Cancel session
  const cancelSession = useCallback(async (sessionId: string): Promise<void> => {
    const response = await fetch(`/api/validation/sessions/${sessionId}/cancel`, {
      method: 'POST',
    });
  ...
  const validateCPF = useCallback((cpf: string): boolean => {
    return BrazilianHealthcareValidationPresets.cpf_validation.pattern.test(cpf);
  }, []);

  const validateCNPJ = useCallback((cnpj: string): boolean => {
    return BrazilianHealthcareValidationPresets.cnpj_validation.pattern.test(cnpj);
  }, []);

  const validateCRM = useCallback((crm: string): boolean => {
    return BrazilianHealthcareValidationPresets.crm_validation.pattern.test(crm);
  }, []);

  const validatePhone = useCallback((phone: string): boolean => {
    return BrazilianHealthcareValidationPresets.phone_validation.pattern.test(phone);
  }, []);
  ...
  const getValidationStatusColor = useCallback((status: ValidationStatus): string => {
    switch (status) {
