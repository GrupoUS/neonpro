...
interface UseStaffAlertsReturn {
  ...
  // Interventions
  createIntervention: (alertId: string, intervention: Partial<Intervention>) => Promise<void>;
  updateIntervention: (interventionId: string, updates: Partial<Intervention>) => Promise<void>;
  // Filters and search
...
export function useStaffAlerts({
  ...
  const assignAlert = useCallback(async (alertId: string, staffMemberId: string) => {
    try {
  ...
  const createIntervention = useCallback(async (alertId: string, intervention: Partial<Intervention>) => {
    try {
  ...
  const updateIntervention = useCallback(async (interventionId: string, updates: Partial<Intervention>) => {
    try {
      const response = await fetch(`/api/staff-alerts/interventions/${interventionId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
