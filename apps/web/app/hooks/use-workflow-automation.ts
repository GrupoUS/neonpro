  ...
  const setupWebSocket = useCallback(() => {
    ...
    const scheduleReconnect = () => {
      ...
      if (
        ...
        console.log('Workflow automation WebSocket: Max retries exceeded or component unmounted',)
        return
      }
      ...
      console.log(
        `Workflow automation WebSocket: Scheduling reconnect attempt ${
    ...
    const handleClose = (event: CloseEvent,) => {
      console.log(
        `Workflow automation WebSocket closed: code=${event.code}, reason=${event.reason}`,
      )
      isConnectingRef.current = false
      wsRef.current = null
