...
interface UseWorkflowAutomationReturn {
  ...
  // Template operations
  createFromTemplate: (templateId: string, config: any) => Promise<WorkflowRule>;
  getTemplates: (category?: string) => Promise<WorkflowTemplate[]>;
  
  // Monitoring and analytics
  getExecutionStats: (period: 'hour' | 'day' | 'week' | 'month') => Promise<any>;
  getQueueMetrics: () => Promise<WorkflowQueue[]>;
  
  // Testing and validation
  testRule: (rule: Partial<WorkflowRule>, testData: any) => Promise<any>;
  validateRule: (rule: Partial<WorkflowRule>) => Promise<{ isValid: boolean; errors: string[] }>;
...
export function useWorkflowAutomation({
  ...
  const createRule = useCallback(async (rule: Partial<WorkflowRule>): Promise<WorkflowRule> => {
    try {
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to create workflow rule');
      throw err;
    }
  }, [clinicId]);

  const updateRule = useCallback(async (ruleId: string, updates: Partial<WorkflowRule>) => {
    try {
      setError(null);
  ...
  const triggerRule = useCallback(async (ruleId: string, inputData: any = {}): Promise<WorkflowExecution> => {
    try {
      setError(null);
      
      const response = await fetch(`/api/workflow-automation/rules/${ruleId}/trigger`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setError(err instanceof Error ? err.message : 'Failed to trigger workflow rule');
      throw err;
    }
  }, []);
  ...
  const cancelExecution = useCallback(async (executionId: string) => {
    try {
      ...
      const response = await fetch(`/api/workflow-automation/executions/${executionId}/cancel`, {
        method: 'POST',
      });
  ...
  const retryExecution = useCallback(async (executionId: string): Promise<WorkflowExecution> => {
    try {
      ...
      const response = await fetch(`/api/workflow-automation/executions/${executionId}/retry`, {
        method: 'POST',
      });
  ...
  const createFromTemplate = useCallback(async (templateId: string, config: any): Promise<WorkflowRule> => {
    try {
      setError(null);
      
      const response = await fetch('/api/workflow-automation/templates/create-rule', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          templateId,
      ...
      if (!response.ok) {
        throw new Error(`Failed to create rule from template: ${response.statusText}`);
      }
      ...
      if (data.success) {
        ...
        throw new Error(data.message || 'Failed to create rule from template');
      }
    } catch (err) {
  ...
  const getExecutionStats = useCallback(async (period: 'hour' | 'day' | 'week' | 'month') => {
    try {
      const response = await fetch(`/api/workflow-automation/stats?period=${period}&clinicId=${clinicId}`);
  ...
  const testRule = useCallback(async (rule: Partial<WorkflowRule>, testData: any) => {
    try {
      setError(null);
  ...
  const bulkToggleRules = useCallback(async (ruleIds: string[], active: boolean) => {
    try {
      setError(null);
      
      const response = await fetch('/api/workflow-automation/rules/bulk-toggle', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error(`Failed to bulk toggle rules: ${response.statusText}`);
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to bulk toggle rules');
    }
  }, []);
  ...
  const bulkDeleteRules = useCallback(async (ruleIds: string[]) => {
    try {
      ...
      const response = await fetch('/api/workflow-automation/rules/bulk-delete', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      ...
      setError(err instanceof Error ? err.message : 'Failed to bulk delete rules');
    }
  }, []);
