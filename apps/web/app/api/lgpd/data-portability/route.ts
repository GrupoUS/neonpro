...
const PortabilityRequestSchema = z.object({
  ...
  standard: z
    .enum(['generic', 'fhir', 'hl7', 'proprietary'])
    .default('generic'),
  categories: z
    .array(
      z.enum([
...
async function extractPortabilityData(
  supabase: any,
  userId: string,
  categories: string[],
  format: string,
...
async function logPortabilityRequest(
  supabase: any,
  userId: string,
  exportData: any,
  success: boolean
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract user data
    const exportData = await extractPortabilityData(
      supabase,
      user.id,
      categories,
      format,
    ...
    switch (format) {
      ...
      case 'fhir':
        ...
        responseData = {
          ...
          entry: Object.values(exportData.data)
            .flat()
            .map((resource: any) => ({
              resource,
            })),
