...
const ProfileUpdateSchema = z.object({
  ...
  emergency_contact_phone: z.string().regex(/^\+?[\d\s-()]+$/).optional(),
...
function validateCPF(cpf: string): boolean {
  ...
  const invalidPatterns = ['00000000000', '11111111111', '22222222222', '33333333333', 
...
async function getCurrentValues(supabase: any, userId: string, requestType: string) {
  ...
  switch (requestType) {
    case 'profile':
      const { data: profile } = await supabase
        .from('users')
        .select('full_name, phone, birth_date, gender, address, city, state, zip_code, emergency_contact_name, emergency_contact_phone, preferred_language, communication_preferences')
        .eq('id', userId)
        .single();
...
async function applyChanges(supabase: any, userId: string, requestType: string, changes: Record<string, any>) {
  let result;
...
function requiresApproval(requestType: string, changes: Record<string, any>): boolean {
  ...
  const sensitiveFields = ['birth_date', 'medical_conditions', 'allergies', 'medications'];
  return Object.keys(changes).some(field => sensitiveFields.includes(field));
}

// Create approval request
async function createApprovalRequest(supabase: any, rectificationId: string, userId: string, requestType: string, changes: Record<string, any>, reason: string) {
...
async function logRectificationRequest(supabase: any, rectificationResult: RectificationResult) {
  try {
    await supabase
...
export async function POST(request: NextRequest) {
  try {
    ...
    const originalValues = await getCurrentValues(supabase, user.id, requestType);
    ...
    if (needsApproval) {
      // Create approval request
      await createApprovalRequest(supabase, rectificationId, user.id, requestType, changes, reason);
      ...
      return NextResponse.json({
        ...rectificationResult,
        message: 'Rectification request submitted for approval. You will be notified when reviewed.',
      ...
      availableRequestTypes: [
        {
          ...
          fields: ['full_name', 'phone', 'birth_date', 'gender', 'address', 'city', 'state', 'zip_code', 'emergency_contact_name', 'emergency_contact_phone', 'preferred_language'],
        ...
        },
        {
          ...
          fields: ['notification_preferences', 'privacy_settings', 'accessibility_settings', 'theme_preference'],
          requiresApproval: false
        },
        {
          type: 'contact',
