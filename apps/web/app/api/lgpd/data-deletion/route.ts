...
function checkRetentionRequirements(record: any, recordType: string): boolean {
  const now = Date.now();
  const recordDate = new Date(record.created_at || record.date || record.timestamp).getTime();
  const retentionPeriod = RETENTION_REQUIREMENTS[recordType as keyof typeof RETENTION_REQUIREMENTS];
...
// Delete user profile
async function deleteUserProfile(supabase: any, userId: string, anonymize = false) {
  if (anonymize) {
...
// Delete appointments
async function deleteUserAppointments(supabase: any, userId: string, retainLegal = true, anonymize = false) {
  let deleted = 0;
  let retained = 0;
  let anonymized = 0;
  ...
  if (appointments) {
    for (const appointment of appointments) {
      ...
      if (retainLegal && checkRetentionRequirements(appointment, 'medical_records')) {
        if (anonymize) {
...
// Delete treatments
async function deleteUserTreatments(supabase: any, userId: string, retainLegal = true, anonymize = false) {
  let deleted = 0;
  let retained = 0;
  let anonymized = 0;
  ...
  if (treatments) {
    for (const treatment of treatments) {
      if (retainLegal && checkRetentionRequirements(treatment, 'medical_records')) {
        if (anonymize) {
...
// Delete payments
async function deleteUserPayments(supabase: any, userId: string, retainLegal = true, anonymize = false) {
  let deleted = 0;
  let retained = 0;
  let anonymized = 0;
  ...
  if (payments) {
    for (const payment of payments) {
      if (retainLegal && checkRetentionRequirements(payment, 'payment_records')) {
        if (anonymize) {
          const anonymizedRecord = anonymizeRecord(payment);
          await supabase
            .from('anonymized_payments')
            .insert([anonymizedRecord]);
          anonymized++;
        } else {
          retained++;
...
async function deleteUserCommunications(supabase: any, userId: string, anonymize = false) {
  let deleted = 0;
  let anonymized = 0;
...
async function scheduleDeletion(supabase: any, deletionRequest: any): Promise<string> {
...
async function executeDeletion(supabase: any, userId: string, request: any) {
  const { deletionType, categories, alternativeAction, retainLegalData } = request;
  ...
  if (deletionType === 'complete' || categories?.includes('appointments')) {
    const result = await deleteUserAppointments(supabase, userId, retainLegalData, anonymize);
  ...
  if (deletionType === 'complete' || categories?.includes('treatments')) {
    const result = await deleteUserTreatments(supabase, userId, retainLegalData, anonymize);
    totalDeleted += result.deleted;
    totalRetained += result.retained;
    totalAnonymized += result.anonymized;
  ...
  if (deletionType === 'complete' || categories?.includes('payments')) {
    const result = await deleteUserPayments(supabase, userId, retainLegalData, anonymize);
    totalDeleted += result.deleted;
    totalRetained += result.retained;
    totalAnonymized += result.anonymized;
  ...
  if (deletionType === 'complete' || categories?.includes('profile') || categories?.includes('account')) {
...
async function logDeletionRequest(supabase: any, userId: string, request: any, success: boolean, deletionId?: string) {
  try {
    await supabase
  ...
  try {
    ...
    if (gracePeriod) {
      ...
      deletionResult = {
        ...
        summary: { recordsDeleted: 0, recordsAnonymized: 0, recordsRetained: 0 }
      };
      
      await logDeletionRequest(supabase, user.id, deletionRequest, true, deletionId);
      
      return NextResponse.json({
        ...deletionResult,
        message: 'Data deletion scheduled successfully. You have 30 days to cancel this request.',
      ...
      return NextResponse.json({
        ...
        warning: 'This action is irreversible. Your data has been permanently deleted.'
      });
    }
