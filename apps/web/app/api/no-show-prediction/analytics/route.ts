...
import { GetAnalyticsQuerySchema } from '@/app/lib/validations/no-show-prediction';
  ...
  try {
    ...
    const parsedQuery = GetAnalyticsQuerySchema.parse({
      ...
      min_accuracy: queryParams.min_accuracy
        ? Number.parseFloat(queryParams.min_accuracy)
        : undefined,
    });

    // Build analytics query
    let query = supabase.from('no_show_analytics').select('*');
