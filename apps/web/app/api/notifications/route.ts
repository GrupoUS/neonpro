...
import { AnalyticsNotificationService } from '@/lib/services/analytics-notification-service'
import { z } from 'zod'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get notifications
    const notifications = await AnalyticsNotificationService.getUserNotifications(
      user.id,
      {
    ...
    switch (action) {
      ...
      case 'sendCustom': {
        ...
        if (validatedData.clinicId) {
          const hasAccess = userClinics?.some(uc => 
