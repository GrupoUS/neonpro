...
export async function POST(request: NextRequest) {
	try {
		...
		if (autoSchedule) {
			result = await regulatoryReportingService.scheduleAutomaticReports(user.id);
		} else {
			switch (reportType) {
				case "anvisa_quarterly":
					result = await regulatoryReportingService.generateANVISAReport(user.id, periodStart, periodEnd);
					break;
				case "cfm_annual":
					result = await regulatoryReportingService.generateCFMReport(user.id, periodStart, periodEnd);
					break;
				case "lgpd_compliance":
					result = await regulatoryReportingService.generateLGPDReport(user.id, periodStart, periodEnd);
					break;
				default:
