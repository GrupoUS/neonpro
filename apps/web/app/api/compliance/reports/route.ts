...
import { AuditEventType, LGPDAuditTrail } from '@/lib/compliance/audit-trail';
...
const ReportListQuerySchema = z.object({
  ...
  limit: z
    ...
    .default('20'),
  type: z
    .enum(['compliance', 'audit', 'consent', 'data_subject', 'security'])
    .optional(),
...
async function generateSecurityReport(
  ...
  return {
    ...
    summary: {
      totalSecurityEvents:
        securityEvents.events.length + breachEvents.events.length,
      ...
      await supabase
        ...
        .update({
          ...
          downloadUrl: reportData.filename
            ? `/api/compliance/reports/${reportId}/download`
            : null,
        })
        .eq('id', reportId);
