  ...
  try {
    ...
    if (!hasAccess) {
      await logSecurityEvent({
        ...
        severity: 'medium',
        description:
          'Usuário sem permissões suficientes para acessar logs de auditoria',
        user_id: user.id,
        ip_address: clientIP,
        user_agent: request.headers.get('user-agent') || undefined,
      });
  ...
  try {
    ...
    if (!csrfValid) {
      await logSecurityEvent({
        ...
        severity: 'high',
        description:
          'Falha na validação CSRF para criação de evento de auditoria',
        user_id: user.id,
        ip_address: clientIP,
        user_agent: request.headers.get('user-agent') || undefined,
      });
    ...
    const event = await logAuditEvent({
      ...
      ip_address: clientIP,
      user_agent:
        eventData.user_agent || request.headers.get('user-agent') || undefined,
    });
