...
export async function GET(request: NextRequest) {
  try {
    ...
    const forecastType =
      (searchParams.get('type') as MarketingForecastType) || 'roi';
    const period =
      (searchParams.get('period') as MarketingForecastPeriod) || '90d';
    ...
    const requestData: MarketingForecastRequest = {
      ...
      includeConfidenceInterval:
        searchParams.get('includeConfidenceInterval') === 'true',
      includeScenarios: searchParams.get('includeScenarios') === 'true',
    };

    const forecast =
      await marketingROIService.generateMarketingForecast(requestData);
