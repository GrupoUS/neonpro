...
const _BulkConfirmationSchema = z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    return await handleLegacyConfirmation(
      supabase,
      validatedData,
      appointment,
      user
...
export async function PUT(request: NextRequest) {
  try {
    ...
    try {
      const _communicationService = new CommunicationService();

      let _notificationMessage = '';
      switch (validatedData.response) {
        case 'confirmed':
          _notificationMessage = `✅ Consulta confirmada: ${confirmation.patients.name} - ${new Date(confirmation.appointments.date).toLocaleDateString('pt-BR')}`;
          break;
        case 'cancelled':
          _notificationMessage = `❌ Consulta cancelada: ${confirmation.patients.name} - ${new Date(confirmation.appointments.date).toLocaleDateString('pt-BR')}`;
          break;
        case 'reschedule':
          _notificationMessage = `🔄 Reagendamento solicitado: ${confirmation.patients.name} - ${new Date(confirmation.appointments.date).toLocaleDateString('pt-BR')}`;
          if (validatedData.rescheduleDate) {
            _notificationMessage += ` → Nova data: ${validatedData.rescheduleDate}`;
          }
          break;
      }
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('appointment_confirmations').select(`
        *,
        appointments(*),
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    let query = supabase.from('appointment_confirmations').update({
      status: 'expired',
      expired_by: user.id,
