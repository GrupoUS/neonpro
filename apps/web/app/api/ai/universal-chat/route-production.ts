...
export async function PUT(request: NextRequest) {
	try {
		...
		switch (action) {
			case "create": {
				const session = await createChatSession(userId, data.clinicId, data.interface || "external");
				return NextResponse.json({
					sessionId: session.id,
					status: "active",
...
async function getChatSession(sessionId: string): Promise<ChatSession> {
	const { data, error } = await supabase.from("ai_chat_sessions").select("*").eq("id", sessionId).single();
...
async function createChatSession(userId: string, clinicId: string, interfaceType: string): Promise<ChatSession> {
...
async function updateChatSession(sessionId: string, updates: any): Promise<void> {
	const { error } = await supabase.from("ai_chat_sessions").update(updates).eq("id", sessionId);
