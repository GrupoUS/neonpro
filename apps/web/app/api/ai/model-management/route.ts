...
export async function GET(request: NextRequest) {
	try {
		...
		switch (action) {
			...
			case "metrics": {
				if (!modelId) {
					return NextResponse.json({ error: "Model ID required for metrics" }, { status: 400 });
				}
			...
			case "drift-check": {
				if (!modelId) {
					return NextResponse.json({ error: "Model ID required for drift check" }, { status: 400 });
				}
			...
			default: {
				...
				const systemStatus = {
					...
					totalPredictions: activeModels.reduce((sum, model) => sum + (model.predictions_count || 0), 0),
...
export async function POST(request: NextRequest) {
	try {
		...
		switch (action) {
			case "register": {
				const { name, description, modelType, version, config, performanceMetrics } = body;
			...
			case "update-performance": {
				...
				if (!perfModelId || !metrics) {
					return NextResponse.json({ error: "Model ID and metrics required" }, { status: 400 });
				}
