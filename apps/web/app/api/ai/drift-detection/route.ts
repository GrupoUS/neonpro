...
import { driftDetector, DriftAlert } from '@/lib/ai/drift-detection';
    ...
    switch (action) {
      ...
      case 'history':
        ...
        const historyStats = {
          ...
          averageDriftScore: driftHistory?.length 
            ? (driftHistory.reduce((sum, d) => sum + (d.drift_score || 0), 0) / driftHistory.length * 100).toFixed(2) + '%'
            : '0%',
      ...
      case 'model-status':
        ...
        const modelStatus = latestDrift ? {
          ...
          status: (latestDrift.drift_score || 0) > 0.1 ? 'HIGH_DRIFT' : 
                 (latestDrift.drift_score || 0) > 0.05 ? 'MEDIUM_DRIFT' : 'STABLE',
        ...
        const systemHealth = {
          ...
          last24Hours: {
            ...
            averageDrift: recentActivity?.length 
              ? (recentActivity.reduce((sum, a) => sum + (a.drift_score || 0), 0) / recentActivity.length * 100).toFixed(2) + '%'
              : '0%'
          },
          monitoring: {
