...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (sessionError || !session?.user) {
      Logger.warn(
        'WhatsApp OAuth token refresh attempted without valid session',
        {
          requestId,
          provider: 'whatsapp',
    ...
    if (connection.status !== 'connected') {
      Logger.warn(
        'Attempted to refresh token for disconnected WhatsApp account',
        {
          requestId,
          provider: 'whatsapp',
          userId: session.user.id,
    ...
    if (updateError) {
      Logger.error(
        'Failed to update WhatsApp connection with refreshed token',
        {
          requestId,
          provider: 'whatsapp',
          userId: session.user.id,
