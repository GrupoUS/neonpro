  ...
  try {
    ...
    if (!recentCashFlow || recentCashFlow.length === 0) {
      alerts.push({
        id: `no-data-${Date.now()}`,
        type: 'warning',
        message: 'Nenhum dado financeiro encontrado nos últimos 30 dias. Verifique a integração de pagamentos.',
    ...
    const negativeFlowDays = recentCashFlow
      .slice(0, 7)
      .filter(record => record.net_cash_flow < 0)
      .length
    ...
    if (recentCashFlow.length >= 14) {
      ...
      if (previousWeekInflow > 0) {
        const revenueChange = ((lastWeekInflow - previousWeekInflow) / previousWeekInflow) * 100
    ...
    const recordsWithPredictions = recentCashFlow.filter(record => 
      record.prediction_accuracy !== null && record.prediction_accuracy !== undefined
    )
    
    if (recordsWithPredictions.length >= 7) {
      const averagePredictionAccuracy = recordsWithPredictions
        .reduce((sum, record) => sum + (record.prediction_accuracy || 0), 0) / recordsWithPredictions.length
    ...
    const highOutflowDays = recentCashFlow
      .slice(0, 7)
      .filter(record => Math.abs(record.outflow || 0) > 3000)
      .length
    ...
    alerts.push({
      ...
      type: 'warning',
      message: 'Erro ao gerar alertas financeiros. Entre em contato com o suporte se o problema persistir.',
      timestamp: new Date().toISOString(),
      resolved: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const alerts = await generateFinancialAlerts(supabase, validatedParams.clinic_id)
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        critical_alerts: filteredAlerts.filter(a => a.type === 'critical').length,
        warning_alerts: filteredAlerts.filter(a => a.type === 'warning').length,
