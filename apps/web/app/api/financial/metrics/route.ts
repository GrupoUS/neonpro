...
async function calculateFinancialMetrics(
  ...
  try {
    ...
    const totalInflow = currentPeriodData.reduce((sum, record) => sum + (record.inflow || 0), 0)
    const totalOutflow = Math.abs(currentPeriodData.reduce((sum, record) => sum + (record.outflow || 0), 0))
    const netCashFlow = totalInflow - totalOutflow
    
    const previousTotalInflow = previousPeriodData.reduce((sum, record) => sum + (record.inflow || 0), 0)
    ...
    const monthly_revenue = timeRange === '30d' ? totalInflow : (totalInflow / daysInPeriod) * 30
    const annual_revenue = (totalInflow / daysInPeriod) * 365

    // Calcular crescimento
    const growth_rate = previousTotalInflow > 0 
      ? ((totalInflow - previousTotalInflow) / previousTotalInflow) * 100 
      : 0

    // Calcular margem de lucro
    const profit_margin = totalInflow > 0 ? (netCashFlow / totalInflow) * 100 : 0
    ...
    if (patientData && patientData.length > 0) {
      const completedAppointments = patientData.flatMap(patient => 
        patient.appointments?.filter(apt => apt.status === 'completed') || []
      )
      
      if (completedAppointments.length > 0) {
        const totalTreatmentValue = completedAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0)
        average_treatment_value = totalTreatmentValue / completedAppointments.length
        ...
        patient_acquisition_cost = patientData.length > 0 ? marketingCost / patientData.length : 0
        
        // LTV simplificado (valor médio * frequência estimada * tempo de vida)
        const averageVisitsPerPatient = completedAppointments.length / patientData.length
    ...
    const break_even_point = totalOutflow > 0 ? Math.ceil((totalOutflow / daily_revenue)) : 0
