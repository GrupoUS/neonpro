...
export async function GET(request: NextRequest) {
  try {
    ...
    const { limit, offset, severity, type, startDate, endDate, resolved } =
      validation.data;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (eventId) {
      ...
      const success =
        await sessionSystem.securityEventLogger.deleteEvent(eventId);
    ...
    if (clearAll) {
      ...
      const deletedCount =
        await sessionSystem.securityEventLogger.clearUserEvents(user.id);
