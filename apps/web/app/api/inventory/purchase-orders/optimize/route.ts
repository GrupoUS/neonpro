...
export async function POST(request: NextRequest) {
  try {
    ...
    const enhancedRecommendations = optimizationResults.recommendations.map(
      (rec) => {
        ...
        return {
          ...
          potential_savings_percentage: itemData
            ? (rec.costSavings / (itemData.unit_cost * rec.currentQuantity)) *
              100
            : 0,
        };
      }
    ...
    const seasonalRecommendations =
      validatedData.analysis_type === 'seasonal' ||
    ...
    for (const [itemId, monthlyData] of Object.entries(monthlyConsumption)) {
      ...
      if (maxConsumption > avgConsumption * 1.5) {
        ...
        seasonalRecommendations.push({
          ...
          category: itemData?.category,
          peak_month: peakMonth ? Number.parseInt(peakMonth, 10) : null,
          peak_consumption: maxConsumption,
          average_consumption: Math.round(avgConsumption),
          seasonality_factor:
            Math.round((maxConsumption / avgConsumption) * 100) / 100,
          recommendation: `Consider increasing stock before month ${peakMonth ? Number.parseInt(peakMonth, 10) + 1 : 'N/A'} due to ${Math.round((maxConsumption / avgConsumption - 1) * 100)}% higher consumption`,
