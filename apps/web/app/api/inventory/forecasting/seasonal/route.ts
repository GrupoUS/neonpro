...
export async function POST(request: NextRequest) {
  try {
    ...
    const consumptionByType = historicalData.reduce((acc, item) => {
    ...
    const variance = consumptionValues.reduce((sum, val) => {
    ...
    const analysis = {
      ...
      appointmentCorrelation: {
        ...
        averageConsumptionPerAppointment: appointmentDemand.length > 0 
          ? appointmentDemand.reduce((sum, apt) => sum + (apt.actualConsumption || apt.expectedConsumption), 0) / appointmentDemand.length
          : 0
      }
    };
