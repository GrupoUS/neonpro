...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: historicalAlertsData, error: historicalError } =
      await supabase
        .from('stock_alerts')
    ...
    const criticalAlerts =
      alertsData?.filter((alert) => alert.severity_level === 'critical')
        .length || 0;
    const highAlerts =
      alertsData?.filter((alert) => alert.severity_level === 'high').length ||
      0;
