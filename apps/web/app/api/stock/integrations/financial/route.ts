...
export async function POST(request: NextRequest) {
  try {
    ...
    stockMovements?.forEach((movement) => {
      ...
      switch (movement.movement_type) {
        ...
        case 'consumption':
          ...
          financialMetrics.consumption.totalQuantity +=
    ...
    const _totalReturns = financialMetrics.consumption.totalCost; // Value consumed in procedures
    const wastePercentage =
      totalInvestment > 0
        ? (financialMetrics.waste.totalCost / totalInvestment) * 100
        : 0;
    const turnoverRate =
      currentInventoryValue > 0
        ? financialMetrics.consumption.totalCost / currentInventoryValue
        : 0;
      ...
      projections = {
        ...
        recommendations: [
          ...
          },
          {
            ...
            priority:
              budgetComparison?.status === 'over_budget' ? 'critical' : 'low',
          },
        ],
      };
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          netCashFlow:
            totalInvestment -
            financialMetrics.consumption.totalCost -
            financialMetrics.waste.totalCost,
        },
