...
export const GET = withErrorMonitoring(async (request: NextRequest) => {
  ...
  try {
    ...
    const avgProcessingTime = totalAnalyses > 0
      ? performanceData.reduce((sum, item) => sum + item.processing_time_ms, 0) / totalAnalyses
      : 0;
    const avgConfidence = totalAnalyses > 0
      ? performanceData.reduce((sum, item) => sum + item.confidence_score, 0) / totalAnalyses
      : 0;

    // Performance targets compliance
    const accuracyTarget = 0.95;
    const timeTarget = 30000; // 30 seconds
    
    const accuracyCompliance = totalAnalyses > 0
      ? performanceData.filter(item => item.accuracy_score >= accuracyTarget).length / totalAnalyses
      : 0;
    const timeCompliance = totalAnalyses > 0
      ? performanceData.filter(item => item.processing_time_ms <= timeTarget).length / totalAnalyses
      : 0;
    ...
    if (validatedParams.includeDetails) {
      ...
      timeSeriesData = Array.from(groupedData.values()).map(group => ({
    ...
    const treatmentStats = Array.from(treatmentBreakdown.entries()).map(([type, stats]) => ({
      treatmentType: type,
      count: stats.count,
