...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`📊 Results: ${criticalAlerts.length} critical, ${highAlerts.length} high, ${mediumAlerts.length} medium, ${lowAlerts.length} low`);
    
    if (alerts.length > 0) {
      const totalRevenue = alerts.reduce((sum, a) => sum + a.estimatedImpact.revenueAtRisk, 0);
      console.log(`💰 Total revenue at risk: $${totalRevenue.toLocaleString()}`);
    }
    ...
    return NextResponse.json({
      ...
      results: {
        ...
        impact: {
          modelsAffected: [...new Set(alerts.map(a => a.modelName))].length,
          totalRevenueAtRisk: alerts.reduce((sum, a) => sum + a.estimatedImpact.revenueAtRisk, 0),
      ...
      monitoring: {
        ...
        systemHealth: alerts.length < 5 ? 'good' : alerts.length < 10 ? 'warning' : 'critical'
      }
    });
    ...
    try {
      ...
      console.error('Failed to log drift monitoring failure:', logError);
    }
