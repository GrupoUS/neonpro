  ...
  if (data.statistics) {
    ...
    const statsData = [
      [
        ...
        `${(data.statistics.averageRetention || 0).toFixed(2)}%`,
      ],
...
export function generateForecastPDF(
  ...
  if (data.evaluation) {
    ...
    const evalData = [
      [
        ...
        `${(data.evaluation.mape || 0).toFixed(4)}`,
      ],
      [
        'RMSE (Root Mean Square Error)',
        `${(data.evaluation.rmse || 0).toFixed(4)}`,
      ],
      [
        'R² (Coefficient of Determination)',
        `${(data.evaluation.r2 || 0).toFixed(4)}`,
      ],
  ...
  if (data.scenarios) {
    ...
    const scenarioData = Object.entries(data.scenarios).map(
...
export function generateInsightsPDF(
  ...
  if (data.correlations && data.correlations.length > 0) {
    ...
    doc.autoTable({
      startY: yPosition,
      head: [
        ['Metric 1', 'Metric 2', 'Correlation', 'Significance', 'Strength'],
      ],
      body: corrData,
      theme: 'grid',
      styles: { fontSize: 9 },
...
export function generateDashboardPDF(
  ...
  if (data.kpis) {
    ...
    const kpiData = Object.entries(data.kpis).map(
    ...
    if (data[section.key] && data[section.key].length > 0) {
      ...
      if (section.key === 'insights') {
        ...
        data[section.key]
          ...
          .forEach((insight: any, _index: number) => {
            const text =
