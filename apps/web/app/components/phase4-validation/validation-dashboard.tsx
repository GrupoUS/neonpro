...
export function ValidationDashboard({ clinic_id }: ValidationDashboardProps) {
  ...
  const getTypeIcon = (type: ValidationType) => {
    switch (type) {
      case 'patient_data': return <Users className="h-4 w-4" />;
      ...
      case 'equipment': return <Settings2 className="h-4 w-4" />;
  ...
  const getStatusIcon = (status: ValidationStatus) => {
    switch (status) {
      ...
      case 'requires_review': return <AlertCircle className="h-4 w-4 text-orange-500" />;
  ...
  const handleValidateEntity = async (type: ValidationType, entityId: string, data: any) => {
    try {
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Badge 
                  variant={validation.systemHealth.status === 'healthy' ? 'default' : 'destructive'}
                >
                  {ValidationLabels.system_health[validation.systemHealth.status]}
                </Badge>
              ...
              <Button onClick={validation.checkSystemHealth} size="sm" variant="outline">
        ...
        <TabsContent value="dashboard" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {validation.dashboard?.recent_activities?.slice(0, 5).map(activity => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {validation.dashboard?.recommendations?.slice(0, 3).map((rec, index) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button onClick={() => handleExportResults('excel')} size="sm" variant="outline">
                    <Download className="h-4 w-4 mr-2" />
                    Exportar
                  </Button>
                  <Button onClick={validation.loadSessions} size="sm" variant="outline">
                    <RefreshCw className="h-4 w-4" />
                  </Button>
                </div>
              </CardTitle>
                      ...
                      <TableRow key={session.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            {ValidationLabels.validation_types[session.entity_type]}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <Switch 
                          ...
                          onCheckedChange={(enabled) => validation.toggleRule(rule.id, enabled)}
                        />
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => validation.deleteRule(rule.id)}>
                ...
                {validation.stats && (
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{validation.stats.total_validations}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Taxa de Sucesso:</span>
                      <span className="font-medium">{validation.stats.success_rate}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Score Médio:</span>
                      <span className="font-medium">{validation.stats.average_score}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Score de Conformidade:</span>
                      <span className="font-medium">{validation.stats.compliance_score}%</span>
                    </div>
                  </div>
                )}
              </CardContent>
                ...
                {validation.stats?.common_errors && (
                  <div className="space-y-3">
                    {validation.stats.common_errors.slice(0, 5).map(error => (
                      <div key={error.error_code} className="flex justify-between items-center">
                        <div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{ValidationLabels.validation_types[selectedSession.entity_type]}</p>
                </div>
                <div>
                  <Label>Status</Label>
                  <div className="flex items-center gap-2">
                    {getStatusIcon(selectedSession.status)}
                    <span>{ValidationLabels.validation_status[selectedSession.status]}</span>
                  </div>
                </div>
                <div>
                  <Label>Score Geral</Label>
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{selectedSession.overall_score}%</span>
                    <Progress value={selectedSession.overall_score} className="flex-1" />
                  </div>
                </div>
                <div>
                  <Label>Duração</Label>
                  <p className="font-medium">{selectedSession.duration ? `${selectedSession.duration}ms` : 'Em andamento'}</p>
                </div>
              </div>
                    ...
                    <div key={result.id} className="border rounded p-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">Regra {result.rule_id}</span>
                        </div>
