...
export interface StaffAlert {
  ...
  status: 'pending' | 'acknowledged' | 'assigned' | 'in_progress' | 'resolved' | 'dismissed';
...
export interface Intervention {
  id: string;
  alertId: string;
  type: 'call' | 'whatsapp' | 'email' | 'sms' | 'in_person' | 'reschedule' | 'discount';
...
export interface AlertAuditEntry {
  id: string;
  alertId: string;
  action: 'created' | 'acknowledged' | 'assigned' | 'escalated' | 'resolved' | 'dismissed' | 'updated';
  performedBy: string;
  performedAt: Date;
  details: string;
...
export interface EscalationCondition {
  type: 'time_elapsed' | 'risk_score' | 'appointment_time' | 'patient_history' | 'staff_availability';
...
export interface EscalationAction {
  type: 'notify_staff' | 'reassign' | 'escalate_level' | 'trigger_intervention' | 'schedule_followup';
