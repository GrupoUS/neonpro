...
import { reportData, formatCurrency, formatPercentage, formatDate, formatCPF, formatCNPJ } from './healthcare-data';
...
export class HealthcarePDFGenerator {
  ...
  private addFooter() {
    ...
    doc.text('Este relatório está em conformidade com a LGPD e regulamentações ANVISA/CFM', 
    ...
    doc.text('Assinatura Digital: NeonPro Healthcare System', 
             PDF_CONFIG.margin, footerY + 10);
  }
  ...
  generateLGPDReport(): Uint8Array {
    ...
    this.addHeader('Relatório de Conformidade LGPD', 
                   'Análise Completa de Proteção de Dados');
    
    // Overview section
    this.addDataTable('Visão Geral da Conformidade', [
      { label: 'Total de Titulares', value: data.overview.totalDataSubjects.toLocaleString('pt-BR') },
      { label: 'Consentimentos Ativos', value: data.overview.activeConsents.toLocaleString('pt-BR') },
      { label: 'Consentimentos Retirados', value: data.overview.withdrawnConsents.toLocaleString('pt-BR') },
      { label: 'Solicitações Pendentes', value: data.overview.pendingRequests.toString() },
      { label: 'Score de Conformidade', value: `${data.overview.complianceScore}%` },
      { label: 'Última Auditoria', value: formatDate(data.overview.lastAudit) },
      { label: 'Próxima Auditoria', value: formatDate(data.overview.nextAudit) },
    ]);
    ...
    this.addDataTable('Métricas de Consentimento', [
      { label: 'Marketing - Concedidos', value: data.consentMetrics.marketing.granted.toLocaleString('pt-BR') },
      { label: 'Analytics - Concedidos', value: data.consentMetrics.analytics.granted.toLocaleString('pt-BR') },
      { label: 'Terceiros - Concedidos', value: data.consentMetrics.thirdParty.granted.toLocaleString('pt-BR') },
      { label: 'Pesquisa - Concedidos', value: data.consentMetrics.research.granted.toLocaleString('pt-BR') },
    ]);
    ...
    data.dataRequests.forEach((request) => {
      ...
      if (request.completionDate) {
        requestData.push({ label: 'Data de Conclusão', value: formatDate(request.completionDate) });
      }
  ...
  generateFinancialReport(): Uint8Array {
    ...
    this.addDataTable('Receitas', [
      ...
      { label: 'Receita Trimestral', value: formatCurrency(data.revenue.quarterly) },
      { label: 'Receita Anual', value: formatCurrency(data.revenue.yearly) },
      { label: 'Crescimento Mensal', value: `${data.revenue.growth.monthly}%` },
      { label: 'Crescimento Trimestral', value: `${data.revenue.growth.quarterly}%` },
    ...
    this.addDataTable('Métodos de Pagamento', [
      { label: 'PIX', value: `${formatCurrency(data.paymentMethods.pix.amount)} (${data.paymentMethods.pix.percentage}%)` },
      { label: 'Cartão de Crédito', value: `${formatCurrency(data.paymentMethods.creditCard.amount)} (${data.paymentMethods.creditCard.percentage}%)` },
      { label: 'Cartão de Débito', value: `${formatCurrency(data.paymentMethods.debitCard.amount)} (${data.paymentMethods.debitCard.percentage}%)` },
      { label: 'Dinheiro', value: `${formatCurrency(data.paymentMethods.cash.amount)} (${data.paymentMethods.cash.percentage}%)` },
    ]);
  ...
  generateClinicalReport(): Uint8Array {
    ...
    this.addHeader('Relatório Clínico', 
                   'Resultados de Tratamento e Satisfação do Paciente');
    
    // Treatment outcomes
    this.addDataTable('Resultados de Tratamento', [
      { label: 'Taxa de Sucesso', value: `${data.treatmentOutcomes.successRate}%` },
      { label: 'Taxa de Complicações', value: `${data.treatmentOutcomes.complicationRate}%` },
      { label: 'Satisfação do Paciente', value: `${data.treatmentOutcomes.patientSatisfaction}/5.0` },
      { label: 'Taxa de Conclusão', value: `${data.treatmentOutcomes.treatmentCompletionRate}%` },
      { label: 'Adesão ao Follow-up', value: `${data.treatmentOutcomes.followUpCompliance}%` },
    ]);
    ...
    this.addDataTable('Satisfação do Paciente - Detalhada', [
      { label: 'Satisfação Geral', value: `${data.patientSatisfaction.overall}/5.0` },
      { label: 'Tratamento', value: `${data.patientSatisfaction.breakdown.treatment}/5.0` },
      { label: 'Atendimento', value: `${data.patientSatisfaction.breakdown.service}/5.0` },
      { label: 'Instalações', value: `${data.patientSatisfaction.breakdown.facilities}/5.0` },
      { label: 'Equipe', value: `${data.patientSatisfaction.breakdown.staff}/5.0` },
      { label: 'Agendamento', value: `${data.patientSatisfaction.breakdown.scheduling}/5.0` },
    ...
    if (data.adverseEvents.length > 0) {
      ...
      data.adverseEvents.forEach((event) => {
        this.addDataTable(`Evento ${event.id}`, [
          ...
          { label: 'Reportado ANVISA', value: event.reportedToAnvisa ? 'Sim' : 'Não' },
        ]);
        this.checkNewPage();
      });
    }
...
export class HealthcareExcelExporter {
  ...
  static generateComprehensiveReport(): Uint8Array {
    ...
    const lgpdData = this.prepareLGPDData();
    ...
    // Clinical Data Sheet
    const clinicalData = this.prepareClinicalData();
    ...
    const demographicsData = this.prepareDemographicsData();
  ...
  private static prepareLGPDData(): any[][] {
    ...
    return [
      ...
      ['Marketing', data.consentMetrics.marketing.granted, data.consentMetrics.marketing.withdrawn, data.consentMetrics.marketing.pending],
      ['Analytics', data.consentMetrics.analytics.granted, data.consentMetrics.analytics.withdrawn, data.consentMetrics.analytics.pending],
      ['Terceiros', data.consentMetrics.thirdParty.granted, data.consentMetrics.thirdParty.withdrawn, data.consentMetrics.thirdParty.pending],
      ['Pesquisa', data.consentMetrics.research.granted, data.consentMetrics.research.withdrawn, data.consentMetrics.research.pending],
      [],
  ...
  private static prepareFinancialData(): any[][] {
    ...
    return [
      ...
      ['Cartão de Crédito', data.paymentMethods.creditCard.amount, data.paymentMethods.creditCard.percentage, data.paymentMethods.creditCard.transactions],
      ['Cartão de Débito', data.paymentMethods.debitCard.amount, data.paymentMethods.debitCard.percentage, data.paymentMethods.debitCard.transactions],
      ['Dinheiro', data.paymentMethods.cash.amount, data.paymentMethods.cash.percentage, data.paymentMethods.cash.transactions],
      [],
      ['ANÁLISE DE SERVIÇOS'],
...
export const generateSecureShareLink = (reportId: string, expirationHours: number = 24): string => {
