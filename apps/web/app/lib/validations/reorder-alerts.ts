...
// Base schemas
export const reorderThresholdSchema = z
  .object({
    id: z.string().uuid(),
    ...
    critical_threshold_percentage: z
      .number()
      .int()
      .min(101)
      .max(500)
      .optional(),
    emergency_threshold_percentage: z
      .number()
      .int()
      .min(101)
      .max(1000)
  ...
  .refine(
    (data) => {
      ...
      if (data.requested_delivery_date && data.order_date) {
        return (
          new Date(data.requested_delivery_date) >= new Date(data.order_date)
        );
      }
      return true;
    },
...
export const purchaseOrderItemSchema = z
  ...
  .refine(
    (data) => {
      ...
      return (
        Math.abs(data.total_price - data.quantity * data.unit_price) < 0.01
      );
    },
    {
...
export const supplierLeadTimeSchema = z
  .object({
    id: z.string().uuid(),
    supplier_id: z.string().uuid(),
    item_id: z.string().uuid(),
...
export const createReorderThresholdSchema = z
  .object({
    item_id: z.string().uuid(),
    clinic_id: z.string().uuid(),
    reorder_point: z.number().int().min(0),
...
export const alertFilterSchema = z.object({
  status: z
    ...
    .optional(),
  priority: z
    .array(z.enum(['low', 'medium', 'high', 'critical', 'emergency']))
    .optional(),
  alert_type: z
    .array(z.enum(['warning', 'critical', 'emergency', 'reorder', 'overstock']))
    .optional(),
  date_range: z
    .object({
      start: z.string().date(),
...
export const purchaseOrderFilterSchema = z.object({
  ...
  order_type: z
    .array(z.enum(['reorder', 'emergency', 'bulk', 'special']))
    .optional(),
  date_range: z
    .object({
      start: z.string().date(),
...
export type CreateReorderThresholdRequest = z.infer<
  typeof createReorderThresholdSchema
>;
export type UpdateReorderThresholdRequest = z.infer<
  typeof updateReorderThresholdSchema
>;
export type CreateReorderAlertRequest = z.infer<
  typeof createReorderAlertSchema
>;
export type UpdateReorderAlertRequest = z.infer<
  typeof updateReorderAlertSchema
>;
