import { z } from 'zod';
import { RiskLevel, RiskFactorCategory, EscalationPriority } from '@/app/types/risk-assessment-automation';
...
const medicalHistorySchema = z.object({
  ...
  immunizationStatus: z.record(z.object({
 ...
 */
const procedureSpecificSchema = z.object({
  plannedProcedure: z.object({
...
export const professionalOversightSchema = z.object({
  ...
  specialistConsultation: z.object({
...
export const riskAssessmentConfigSchema = z.object({
  ...
  riskThresholds: z.object({
    ...
    return data.low.max < data.medium.min && 
  ...
  weightings: z.object({
    ...
    return Math.abs(sum - 1.0) < 0.001; // Account for floating point precision
  }, {
