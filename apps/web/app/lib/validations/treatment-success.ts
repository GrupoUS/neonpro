...
export const createSuccessMetricsSchema = z
  .object({
    treatment_type: z
      .string()
      .min(1, 'Tipo de tratamento é obrigatório')
    ...
    time_period: z.enum(['monthly', 'quarterly', 'yearly'], {
      errorMap: () => ({
        message: 'Período deve ser monthly, quarterly ou yearly',
      }),
    }),
    period_start: z
      .string()
    ...
    total_treatments: z
      .number()
      .int()
      .min(0, 'Total de tratamentos deve ser positivo'),
    successful_treatments: z
      .number()
      .int()
...
export const createProviderPerformanceSchema = z.object({
  ...
  period_end: z
    .string()
    .refine((date) => !Number.isNaN(Date.parse(date)), 'Data de fim inválida'),
  overall_success_rate: z
    .number()
    .min(0)
    .max(1, 'Taxa de sucesso deve estar entre 0 e 1')
    .optional(),
  patient_satisfaction_avg: z
    .number()
    .min(0)
    .max(1, 'Satisfação média deve estar entre 0 e 1')
    .optional(),
...
export const updateProtocolOptimizationSchema = z.object({
  ...
  approved_by: z
    .string()
    .uuid('ID do aprovador deve ser um UUID válido')
    .optional(),
...
export const createQualityBenchmarkSchema = z.object({
  ...
  benchmark_type: z.enum(
    ['industry_standard', 'clinic_target', 'best_practice'],
    {
      errorMap: () => ({
        message:
...
export const updateQualityBenchmarkSchema = z.object({
  benchmark_type: z
    .enum(['industry_standard', 'clinic_target', 'best_practice'])
    .optional(),
  metric_name: z
    .string()
...
export const updateComplianceReportSchema = z.object({
  ...
  reviewed_by: z
    .string()
    .uuid('ID do revisor deve ser um UUID válido')
    .optional(),
});
...
export const treatmentSuccessQuerySchema = z.object({
  ...
  date_from: z
    .string()
    .refine((date) => !Number.isNaN(Date.parse(date)), 'Data inicial inválida')
    .optional(),
  date_to: z
    .string()
    .refine((date) => !Number.isNaN(Date.parse(date)), 'Data final inválida')
    .optional(),
  success_rate_min: z
    .string()
    .transform(Number)
