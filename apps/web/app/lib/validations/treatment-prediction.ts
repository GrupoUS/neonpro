...
export const ConfidenceIntervalSchema = z
  .object({
    lower: z
      ...
      .min(0)
      .max(1)
      .describe('Lower bound of confidence interval'),
    upper: z
      .number()
      .min(0)
      .max(1)
      .describe('Upper bound of confidence interval'),
    confidence_level: z
      .number()
      .min(0)
      .max(1)
...
// Update Schemas
const UpdateTreatmentPredictionSchema =
  TreatmentPredictionSchema.partial().omit({
    id: true,
    created_at: true,
    updated_at: true,
  });
...
const UpdateTreatmentCharacteristicsSchema =
  TreatmentCharacteristicsSchema.partial().omit({
    id: true,
    created_at: true,
    updated_at: true,
  });
