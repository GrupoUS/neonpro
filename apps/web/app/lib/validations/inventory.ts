...
// Enum Schemas
export const ItemTypeSchema = z.enum(['supply', 'medication', 'equipment', 'consumable']);
export const StorageTypeSchema = z.enum(['room', 'cabinet', 'refrigerator', 'freezer', 'controlled']);
export const TransactionTypeSchema = z.enum(['receive', 'issue', 'transfer', 'adjustment', 'count', 'expire', 'return']);
export const ReferenceTypeSchema = z.enum(['purchase_order', 'treatment', 'appointment', 'adjustment', 'transfer']);
export const AlertTypeSchema = z.enum(['low_stock', 'expired', 'expiring', 'overstock', 'zero_stock']);
export const AlertLevelSchema = z.enum(['info', 'warning', 'critical', 'urgent']);
export const AlertStatusSchema = z.enum(['active', 'acknowledged', 'resolved', 'dismissed']);
export const StockStatusSchema = z.enum(['active', 'quarantine', 'expired', 'recalled']);
export const VerificationStatusSchema = z.enum(['pending', 'verified', 'rejected']);
export const ScanTypeSchema = z.enum(['barcode', 'qr_code']);
export const ScanFormatSchema = z.enum(['code128', 'code39', 'ean13', 'qr', 'datamatrix']);
export const ScanPurposeSchema = z.enum(['stock_in', 'stock_out', 'count', 'lookup', 'transfer']);
export const ScanStatusSchema = z.enum(['success', 'not_found', 'error', 'duplicate']);
export const DeviceTypeSchema = z.enum(['mobile', 'scanner', 'tablet', 'desktop']);
export const SyncStatusSchema = z.enum(['pending', 'synced', 'error', 'conflict']);
export const SyncOperationSchema = z.enum(['create', 'update', 'delete']);
export const ConflictResolutionSchema = z.enum(['server_wins', 'client_wins', 'merge', 'manual']);

// Common Field Schemas
export const UuidSchema = z.string().uuid();
export const SkuSchema = z.string().min(1).max(100).regex(/^[A-Z0-9\-_]+$/, 'SKU must contain only uppercase letters, numbers, hyphens, and underscores');
export const BarcodeSchema = z.string().min(1).max(128).optional();
export const QuantitySchema = z.number().int().min(0);
export const CostSchema = z.number().min(0).max(999999.99);
export const LocationCodeSchema = z.string().min(1).max(50).regex(/^[A-Z0-9\-_]+$/, 'Location code must contain only uppercase letters, numbers, hyphens, and underscores');
...
export const InventoryTransactionSchema = z.object({
  ...
  // Cost Information
  unit_cost: CostSchema.optional(),
  total_cost: z.number().min(0).max(9999999999.99).optional(),
...
export const StockUpdateRequestSchema = z.object({
  item_id: UuidSchema,
  location_id: UuidSchema,
  quantity_change: z.number().int().min(-999999).max(999999),
  transaction_type: TransactionTypeSchema,
...
export const InventorySearchParamsSchema = z.object({
  ...
  expiry_status: z.enum(['all', 'expired', 'expiring_soon', 'expiring_this_month']).default('all'),
  item_type: ItemTypeSchema.optional(),
  page: z.number().int().min(1).default(1),
  limit: z.number().int().min(1).max(100).default(20),
  sort_by: z.enum(['name', 'sku', 'quantity', 'expiry_date', 'last_updated']).default('name'),
...
export const InventoryReportParamsSchema = z.object({
  report_type: z.enum(['stock_summary', 'low_stock', 'expiring_items', 'transaction_history', 'usage_analysis']),
  period: z.enum(['today', 'this_week', 'this_month', 'this_quarter', 'this_year', 'custom']),
...
export type CreateInventoryItemForm = z.infer<typeof CreateInventoryItemFormSchema>;
