...
export function useComplianceMonitoring(): UseComplianceMonitoringResult {
  ...
  const handleStatusUpdate = useCallback(
    (newStatus: RealTimeComplianceStatus) => {
      setStatus(newStatus);
      setIsLoading(false);
    },
    []
...
export function useComplianceMetrics() {
  ...
  const getComplianceLevelColor = useCallback(
    (level: ComplianceLevel): string => {
      switch (level) {
...
export function useComplianceViolations() {
  const { status, isLoading, reportViolation, resolveViolation } =
