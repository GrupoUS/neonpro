...
class SecurityAuditLogger {
  ...
  async logEvent(
    ...
    try {
      ...
      if (event.severity === AuditSeverity.CRITICAL || this.eventQueue.length >= 10) {
   ...
   */
  async logLoginSuccess(userId: string, sessionId: string, method: string = 'oauth'): Promise<void> {
   ...
   */
  async logLoginFailure(email: string, reason: string, method: string = 'oauth'): Promise<void> {
  ...
  async logSessionCreated(userId: string, sessionId: string): Promise<void> {
    await this.logEvent(AuditEventType.SESSION_CREATED, {
   ...
   */
  async logSessionTerminated(userId: string, sessionId: string, reason: string): Promise<void> {
    await this.logEvent(AuditEventType.SESSION_TERMINATED, {
      reason,
   ...
   */
  async logOAuthFlow(stage: 'start' | 'success' | 'failure', details: Record<string, any>): Promise<void> {
    const eventType = stage === 'start' ? AuditEventType.OAUTH_FLOW_START :
  ...
  async logPermissionDenied(userId: string, resource: string, action: string): Promise<void> {
    await this.logEvent(AuditEventType.PERMISSION_DENIED, {
      resource,
      action,
  ...
  async getSecurityMetrics(hoursBack: number = 24): Promise<SecurityMetrics> {
    try {
      ...
      const events = this.getStoredEvents().filter(event => event.timestamp > cutoffTime)
      ...
      const metrics: SecurityMetrics = {
        ...
        suspiciousActivities: events.filter(e => e.type === AuditEventType.SUSPICIOUS_ACTIVITY).length,
  ...
  async detectSuspiciousActivity(event: AuditEvent): Promise<void> {
    try {
      ...
      const rapidAttempts = recentEvents.filter(e =>
        (e.type === AuditEventType.LOGIN_ATTEMPT || e.type === AuditEventType.LOGIN_FAILURE) &&
      ...
      if (rapidAttempts.length >= SUSPICIOUS_PATTERNS.RAPID_LOGIN_ATTEMPTS.threshold) {
        suspiciousPatterns.push({
          ...
          affectedUsers: [...new Set(rapidAttempts.map(e => e.email).filter(Boolean))],
          timeframe: {
   ...
   * Get security report
   */
  async getSecurityReport(hoursBack: number = 24): Promise<{
    ...
    const recommendations = this.generateRecommendations(metrics, suspiciousPatterns)
  ...
  private determineSeverity(type: AuditEventType, details: Record<string, any>): AuditSeverity {
    switch (type) {
  ...
  private assessRiskLevel(type: AuditEventType, details: Record<string, any>): RiskLevel {
  ...
  private determineOutcome(type: AuditEventType, details: Record<string, any>): 'success' | 'failure' | 'pending' {
  ...
  private calculateRiskDistribution(events: AuditEvent[]): Record<RiskLevel, number> {
    const distribution = {
      [RiskLevel.LOW]: 0,
      [RiskLevel.MEDIUM]: 0,
      [RiskLevel.HIGH]: 0,
  ...
  private async getSuspiciousPatterns(hoursBack: number): Promise<SuspiciousPattern[]> {
...
export async function getSecurityMetrics(hoursBack?: number): Promise<SecurityMetrics> {
