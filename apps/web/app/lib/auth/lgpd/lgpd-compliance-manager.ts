...
class LGPDComplianceManager {
  ...
  async recordConsent(
    ...
    try {
      const consent: ConsentRecord = {
        ...
        withdrawnAt: granted ? undefined : Date.now(),
        version,
   ...
   */
  async deleteUserData(userId: string, retainMedical = true): Promise<void> {
    try {
    ...
    try {
      ...
      switch (request.type) {
        case LGPDRights.ACCESS: {
          const _userData = await this.exportUserData(request.userId);
        ...
        case LGPDRights.RECTIFICATION:
          response =
            'Solicitação de retificação registrada. Entre em contato para prosseguir.';
          break;

        case LGPDRights.DELETION:
          response =
            'Dados serão excluídos conforme política de retenção médica';
          break;
        ...
        case LGPDRights.INFORMATION:
          response =
            'Informações sobre processamento disponíveis na política de privacidade';
          break;
      }
  ...
  private async getPersonalData(_userId: string): Promise<any> {
    return { message: 'Dados pessoais seriam recuperados do banco de dados' };
  }

  private async getMedicalData(_userId: string): Promise<any> {
    return { message: 'Dados médicos seriam recuperados do banco de dados' };
  }

  private async getAppointments(_userId: string): Promise<any> {
  ...
  private async getUserAuditLog(_userId: string): Promise<any> {
  ...
  private async getPendingDataRequests(): Promise<number> {
    return this.getStoredRequests().filter((r) => r.status === 'pending')
      .length;
  }
