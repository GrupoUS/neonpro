...
export class AutomatedProtocolOptimizationService {
  ...
  async getProtocolVersions(filters?: {
    ...
    let query = this.supabase.from('protocol_versions').select(`
        *,
  ...
  private async performOptimizationAnalysis(
    ...
    const successRate =
      outcomes.length > 0
        ? outcomes.reduce((sum, o) => sum + o.success_score, 0) /
          outcomes.length
        : 0;
    ...
    if (complicationRate > 10) {
      recommendations.push({
        ...
        priority: 'critical',
        suggestion:
          'Implement additional safety measures to reduce complications',
  ...
  async analyzeExperimentResults(
    ...
    const [controlOutcomes, testOutcomes] = await Promise.all([
      ...
      experiment.test_protocol_id
        ? this.getProtocolOutcomes({
            protocol_version_id: experiment.test_protocol_id,
          })
        : { data: [], total: 0 },
    ]);
    ...
    const _pooledSampleSize =
      controlOutcomes.data.length + testOutcomes.data.length;
  ...
  async getProtocolOptimizationAnalysis(
    ...
    return {
      ...
      comparative_analysis: {
        peer_protocols:
          'Analysis would include comparison with similar protocols',
