...
export class AISchedulingEngine {
  ...
  private predictTreatmentDuration(
    ...
    const weights = history.map((_: any, index: number) => 0.8 ** (history.length - index - 1));
    const weightedSum = history.reduce((sum: number, duration: number, index: number) => 
      sum + duration * weights[index], 0
    );
    const weightSum = weights.reduce((sum: number, weight: number) => sum + weight, 0);
  ...
  private predictDemandConflict(slot: AppointmentSlot): number {
    ...
    if ([4, 5, 11].includes(month)) { // May, June, December
  ...
  private calculateTimePreference(slot: AppointmentSlot, request: SchedulingRequest): number {
    ...
    const isPreferred = request.preferredTimeRanges.some(range => 
  ...
  private calculateSatisfactionScore(slot: AppointmentSlot, patient: Patient, staff: Staff): number {
    ...
    const timeMatch = patient.preferences.preferredTimeSlots.some(pref => 
      slot.start >= pref.start && slot.end <= pref.end
    );
  ...
  private calculateSatisfactionRisk(slot: AppointmentSlot, request: SchedulingRequest): number {
    ...
    const isPreferredTime = request.preferredTimeRanges.some(range => 
      slot.start >= range.start && slot.end <= range.end
    );
