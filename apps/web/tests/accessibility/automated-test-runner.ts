...
interface ComponentDiscovery {
  ...
  category: 'telemedicine' | 'patient' | 'admin' | 'ui' | 'accessibility' | 'common';
  hasTests: boolean;
...
interface AutomatedTestReport extends AccessibilityReport {
  ...
  componentCategories: Record<string, {
    count: number;
    violations: number;
...
class AccessibilityTestRunner {
  ...
  private async analyzeComponent(filePath: string, content: string): Promise<ComponentDiscovery | null> {
    ...
    else if (relativePath.includes('/accessibility/')) category = 'accessibility';
    ...
    if (lineCount > 200 || (hasState && hasEffects && hasComplexLogic)) {
      complexity = 'complex';
    } else if (lineCount > 100 || hasState || hasEffects || hasAsyncOperations) {
      complexity = 'medium';
    }
