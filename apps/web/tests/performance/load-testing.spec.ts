...
class LoadTester {
  ...
  async loginAllUsers() {
    const loginPromises = this.pages.map(async (page, index) => {
      ...
      await page.fill('[data-testid="crm-input"]', `CRM-SP-${String(index + 100000).padStart(6, '0')}`);
  ...
  private async performAction(page: any, action: string, userIndex: number) {
    switch (action) {
      ...
      case 'update_patient_info':
        ...
        await page.fill('[data-testid="phone-input"]', `(11) 9${String(Math.floor(Math.random() * 100000000)).padStart(8, '0')}`);
...
test.describe('Patient Dashboard Load Testing', () => {
  ...
  test('should handle small clinic load (3 concurrent doctors)', async () => {
    ...
    // Analyze results
    const avgPageLoadTime = metrics.reduce((sum, metric) => sum + metric.navigation.loadEventEnd, 0) / metrics.length;
    const avgMemoryUsage = metrics.reduce((sum, metric) => 
      sum + (metric.memory ? metric.memory.usedJSHeapSize / 1024 / 1024 : 0), 0
    ) / metrics.length;
  ...
  test('should handle medium clinic load (8 concurrent doctors)', async () => {
    ...
    const avgPageLoadTime = metrics.reduce((sum, metric) => sum + metric.navigation.loadEventEnd, 0) / metrics.length;
    const maxMemoryUsage = Math.max(...metrics.map(metric => 
      metric.memory ? metric.memory.usedJSHeapSize / 1024 / 1024 : 0
    ));
    
    console.log(`Medium clinic - Avg page load: ${avgPageLoadTime}ms, Max memory: ${maxMemoryUsage}MB`);
  ...
  test('should handle large clinic load (20 concurrent doctors)', async () => {
    ...
    console.log(`Large clinic - Avg load: ${avgPageLoadTime}ms, P95 load: ${p95PageLoadTime}ms, Avg memory: ${avgMemoryUsage}MB`);
  ...
  test('should test API response times under concurrent requests', async () => {
    ...
    for (const requestCount of concurrentRequests) {
      ...
      const requestPromises = Array.from({ length: requestCount }, async (_, index) => {
        ...
        await requestPage.fill('[data-testid="crm-input"]', `CRM-SP-${String(index + 200000).padStart(6, '0')}`);
      ...
      if (apiResponseTimes.length > 0) {
        ...
        expect(avgResponseTime).toBeLessThan(PERFORMANCE_THRESHOLDS.API_RESPONSE * (1 + requestCount / 50));
