...
class PerformanceTestUtils {
  ...
  static recordError(testName: string, error: Error): void {
    if (!this.errors.has(testName)) {
      this.errors.set(testName, []);
    }
    
    this.errors.get(testName)?.push(error);
  }
  ...
  static getStatistics(testName: string): {
    ...
    const errors = this.errors.get(testName) || [];
  ...
  static async simulateLoad(
    ...
    for (let i = 0; i < concurrentUsers; i++) {
      setTimeout(async () => {
        ...
        while (Date.now() < endTime) {
          const stopTimer = this.startMeasurement(testName);
          ...
          try {
            ...
            stopTimer();
            this.recordError(testName, error as Error);
          }
    ...
    await new Promise(resolve => 
      setTimeout(resolve, (durationMinutes * 60 * 1000) + (rampUpSeconds * 1000) + 5000)
    );
  }
}
...
class MockAIServiceClient {
  ...
  async sendChatMessage(sessionId: string, message: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/ai/universal-chat/message`, {
      method: "POST",
      headers: {
  ...
  async analyzeConversation(sessionId: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/ai/conversation/analyze`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer test-jwt-token"
...
describe("AI Services Performance Tests", () => {
  ...
  describe("Universal AI Chat Performance", () => {
    ...
    it("should handle single chat requests within performance thresholds", async () => {
      ...
      const stopTimer = PerformanceTestUtils.startMeasurement("single_chat_request");
      ...
      try {
        ...
        expect(duration).toBeLessThan(PERF_TEST_CONFIG.thresholds.chat_response_p95);
        
        console.log(`Single chat request completed in ${duration.toFixed(2)}ms`);
      } catch (error) {
    ...
    it("should maintain performance under light load", async () => {
      ...
      expect(stats.avg).toBeLessThan(PERF_TEST_CONFIG.thresholds.chat_response_avg);
      expect(stats.p95).toBeLessThan(PERF_TEST_CONFIG.thresholds.chat_response_p95);
      expect(stats.errorRate).toBeLessThan(PERF_TEST_CONFIG.thresholds.error_rate_max_percent);
    ...
    it("should handle normal production load", async () => {
      ...
      expect(stats.avg).toBeLessThan(PERF_TEST_CONFIG.thresholds.chat_response_avg * 1.5); // Allow 50% degradation under load
      expect(stats.p95).toBeLessThan(PERF_TEST_CONFIG.thresholds.chat_response_p95 * 1.3); // Allow 30% degradation for p95
      expect(stats.errorRate).toBeLessThan(PERF_TEST_CONFIG.thresholds.error_rate_max_percent);
    }, 400000); // 6-7 minutes timeout
  });
  ...
  describe("Compliance Automation Performance", () => {
    ...
    it("should process compliance validations quickly", async () => {
      const testData = complianceTestData[Math.floor(Math.random() * complianceTestData.length)];
      
      const stopTimer = PerformanceTestUtils.startMeasurement("single_compliance_check");
      ...
      try {
        ...
        expect(result.success).toBe(true);
        expect(duration).toBeLessThan(PERF_TEST_CONFIG.thresholds.compliance_check_p95);
        
        console.log(`Single compliance check completed in ${duration.toFixed(2)}ms`);
      } catch (error) {
        PerformanceTestUtils.recordError("single_compliance_check", error as Error);
        throw error;
      }
    });
    ...
    it("should maintain high throughput for compliance checks", async () => {
      ...
      const stats = PerformanceTestUtils.getStatistics("compliance_normal_load");
      ...
      expect(stats.avg).toBeLessThan(PERF_TEST_CONFIG.thresholds.compliance_check_avg);
      expect(stats.p95).toBeLessThan(PERF_TEST_CONFIG.thresholds.compliance_check_p95);
      expect(throughput).toBeGreaterThan(PERF_TEST_CONFIG.thresholds.compliance_throughput_min);
      expect(stats.errorRate).toBeLessThan(PERF_TEST_CONFIG.thresholds.error_rate_max_percent);
    }, 400000);
  });
  ...
  describe("Conversation Analysis Performance", () => {
    it("should analyze conversations within time limits", async () => {
      ...
      const stopTimer = PerformanceTestUtils.startMeasurement("single_conversation_analysis");
      ...
      try {
        ...
        expect(result.success).toBe(true);
        expect(duration).toBeLessThan(PERF_TEST_CONFIG.thresholds.conversation_analysis_p95);
        
        console.log(`Conversation analysis completed in ${duration.toFixed(2)}ms`);
      } catch (error) {
        PerformanceTestUtils.recordError("single_conversation_analysis", error as Error);
        throw error;
      }
    }, 30000);
    ...
    it("should handle concurrent conversation analyses", async () => {
      ...
      await PerformanceTestUtils.simulateLoad(
        async () => {
          ...
          await client.sendChatMessage(testSessionId, "Preciso de ajuda médica");
      ...
      const stats = PerformanceTestUtils.getStatistics("conversation_analysis_load");
      ...
      expect(stats.avg).toBeLessThan(PERF_TEST_CONFIG.thresholds.conversation_analysis_avg * 1.5);
      expect(stats.p95).toBeLessThan(PERF_TEST_CONFIG.thresholds.conversation_analysis_p95 * 1.3);
      expect(stats.errorRate).toBeLessThan(PERF_TEST_CONFIG.thresholds.error_rate_max_percent);
    }, 300000);
  });
  ...
  describe("End-to-End Performance Scenarios", () => {
    it("should handle complete healthcare workflow efficiently", async () => {
      ...
      try {
        ...
        const chatResult = await client.sendChatMessage(sessionId, 
