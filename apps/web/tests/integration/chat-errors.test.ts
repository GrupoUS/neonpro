...
describe('Chat error handling', () => {
  ...
  it('maps provider HTTP error', async () => {
    ...
    await expect(streamAestheticResponse([{ role: 'user', content: 'oi' }])).rejects.toThrow(/HTTP 500/)
  })

  it('maps rate limit (429)', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(null, { status: 429 })))
    await expect(streamAestheticResponse([{ role: 'user', content: 'oi' }])).rejects.toThrow(/HTTP 429/)
  })

  it('maps timeout when body is missing', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(null, { status: 200 })))
    await expect(streamAestheticResponse([{ role: 'user', content: 'oi' }])).rejects.toThrow(/missing body/)
  })
})
