...
describe('Offline Functionality Tests (T029)', () => {
  ...
  describe('Offline Data Storage and Retrieval', () => {
    test('should store patient data offline for access', async ({ context, page }) => {
      ...
      await expect(firstPatient.locator('[data-testid="patient-name"]')).toContainText(
        'Ana Carolina Silva',
      );
      await expect(firstPatient.locator('[data-testid="patient-cpf"]')).toContainText(
        '123.456.789-09',
      );
    });
    ...
    test('should maintain medical records offline', async ({ context, page }) => {
      ...
      await expect(firstRecord.locator('[data-testid="record-diagnosis"]')).toContainText(
        'Dermatite seborreica',
      );
      await expect(firstRecord.locator('[data-testid="record-treatment"]')).toContainText(
        'Shampoo antisseborreico',
      );
    });
  ...
  describe('Offline Action Queuing and Synchronization', () => {
    test('should queue patient creation when offline', async ({ context, page }) => {
      ...
      const queueMessage = await page.locator('[data-testid="action-queued-message"]')
        .textContent();
    ...
    test('should handle sync conflicts and resolution', async ({ context, page }) => {
      ...
      await expect(page.locator('[data-testid="online-version"]')).toContainText(
        'Ana Carolina Silva',
      );
      await expect(page.locator('[data-testid="offline-version"]')).toContainText(
  ...
  describe('Offline Form Validation and Data Integrity', () => {
    ...
    test('should validate prescription data offline', async ({ context, page }) => {
      ...
      const queuedPrescriptions = await page.evaluate(() => {
        const queue = localStorage.getItem('neonpro_pending_actions');
        return queue
          ? JSON.parse(queue).filter(action => action.type === 'CREATE_PRESCRIPTION')
          : [];
      });
  ...
  describe('Healthcare-Specific Offline Features', () => {
    test('should support emergency offline access to patient data', async ({ context, page }) => {
      ...
      const emergencyMode = await page.locator('[data-testid="emergency-offline-mode"]')
        .textContent();
    ...
    test('should maintain prescription history offline', async ({ context, page }) => {
      ...
      await expect(activePrescription.locator('[data-testid="prescription-medication"]'))
        .toContainText('Losartana');
      await expect(activePrescription.locator('[data-testid="prescription-physician"]'))
        .toContainText('Dr. Ana Santos');
      await expect(activePrescription.locator('[data-testid="prescription-status"]')).toContainText(
        'active',
      );
    });
