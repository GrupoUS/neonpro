...
describe('Offline Functionality Tests (T029)', () => {
  ...
  describe('Offline Detection and UI State', () => {
    test('should detect offline state and show appropriate UI', async ({ context, page }) => {
      ...
      await expect(page.locator('[data-testid="connection-status"]')).toHaveText(/online|conectado/i);
      ...
      await expect(page.locator('[data-testid="connection-status"]')).toHaveText(/offline|desconectado/i);
      ...
      const onlineOnlyButtons = page.locator('[data-testid*="sync"], [data-testid*="upload"], [data-testid*="share"]');
    ...
    test('should show queued actions count when offline', async ({ context, page }) => {
      ...
      await expect(page.locator('[data-testid="queued-actions-count"]')).toBeVisible();
      const queueCount = await page.locator('[data-testid="queued-actions-count"]').textContent();
      ...
      const updatedCount = await page.locator('[data-testid="queued-actions-count"]').textContent();
  ...
  describe('Offline Data Storage and Retrieval', () => {
    test('should store patient data offline for access', async ({ context, page }) => {
      ...
      await page.evaluate((patients) => {
        localStorage.setItem('neonpro_offline_patients', JSON.stringify(patients));
      }, initialPatients);
      ...
      await expect(firstPatient.locator('[data-testid="patient-name"]')).toContainText('Ana Carolina Silva');
      await expect(firstPatient.locator('[data-testid="patient-cpf"]')).toContainText('123.456.789-09');
    });
    ...
    test('should maintain medical records offline', async ({ context, page }) => {
      ...
      await page.evaluate((records) => {
        localStorage.setItem('neonpro_offline_records_1', JSON.stringify(records));
      }, medicalRecords);
      ...
      await expect(firstRecord.locator('[data-testid="record-diagnosis"]')).toContainText('Dermatite seborreica');
      await expect(firstRecord.locator('[data-testid="record-treatment"]')).toContainText('Shampoo antisseborreico');
    });
    ...
    test('should cache images and documents offline', async ({ context, page }) => {
      ...
      await page.evaluate(() => {
        ...
        localStorage.setItem('neonpro_offline_media_1', JSON.stringify(cacheData));
      });
      ...
      await expect(page.locator('[data-testid="cached-documents"]')).toBeVisible();
  ...
  describe('Offline Action Queuing and Synchronization', () => {
    test('should queue patient creation when offline', async ({ context, page }) => {
      ...
      await page.fill('[data-testid="patient-birth-date"]', testData.patient.birthDate);
      ...
      const queueMessage = await page.locator('[data-testid="action-queued-message"]').textContent();
    ...
    test('should sync queued actions when coming back online', async ({ context, page }) => {
      ...
      await page.reload();
      await page.waitForLoadState('domcontentloaded');
      await expect(page.locator('[data-testid="queued-actions-count"]')).toContainText('2');
      ...
      await expect(page.locator('[data-testid="sync-in-progress"]')).toBeVisible({ timeout: 3000 });
      const syncMessage = await page.locator('[data-testid="sync-message"]').textContent();
    ...
    test('should handle sync conflicts and resolution', async ({ context, page }) => {
      ...
      await expect(page.locator('[data-testid="sync-conflict-detected"]')).toBeVisible({ timeout: 5000 });
      
      // Should show conflict resolution dialog
      await expect(page.locator('[data-testid="conflict-resolution-dialog"]')).toBeVisible();
      
      // Should show both versions
      await expect(page.locator('[data-testid="online-version"]')).toContainText('Ana Carolina Silva');
      await expect(page.locator('[data-testid="offline-version"]')).toContainText('Ana Carolina Silva Santos');
    ...
    test('should handle partial sync failures with retry logic', async ({ context, page }) => {
      ...
      // Queue multiple actions
      const actions = [
        { id: 'action-1', type: 'CREATE_PATIENT', data: testData.patient, status: 'pending', retries: 0 },
        { id: 'action-2', type: 'CREATE_APPOINTMENT', data: testData.appointment, status: 'pending', retries: 0 },
        { id: 'action-3', type: 'CREATE_PRESCRIPTION', data: testData.prescription, status: 'pending', retries: 0 },
      ];

      await page.evaluate((queuedActions) => {
        localStorage.setItem('neonpro_pending_actions', JSON.stringify(queuedActions));
      }, actions);
      ...
      await page.evaluate(() => {
        ...
        const updatedActions = [
          { id: 'action-2', type: 'CREATE_APPOINTMENT', status: 'failed', retries: 1, error: 'Network timeout' },
          { id: 'action-3', type: 'CREATE_PRESCRIPTION', status: 'pending', retries: 0 },
        ];
        localStorage.setItem('neonpro_pending_actions', JSON.stringify(updatedActions));
        
        window.dispatchEvent(new CustomEvent('sync-partial-failure', { 
          detail: { succeeded: 1, failed: 1, pending: 1 }
        }));
      });

      // Should show partial sync status
      await expect(page.locator('[data-testid="sync-partial-failure"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="retry-failed-actions"]')).toBeVisible();
  ...
  describe('Offline Form Validation and Data Integrity', () => {
    test('should validate Brazilian data formats offline', async ({ context, page }) => {
      ...
      const cpfError = await page.locator('[data-testid="cpf-error"]').textContent();
    ...
    test('should maintain LGPD consent state offline', async ({ context, page }) => {
      ...
      const consentError = await page.locator('[data-testid="consent-error"]').textContent();
    ...
    test('should validate prescription data offline', async ({ context, page }) => {
      ...
      // Add medication
      await page.fill('[data-testid="medication-name"]', 'Losartana Potássica 50mg');
      await page.fill('[data-testid="medication-dosage"]', '1 comprimido pela manhã');
      ...
      await expect(page.locator('[data-testid="prescription-queued"]')).toBeVisible();
      
      // Verify prescription data integrity in queue
      const queuedPrescriptions = await page.evaluate(() => {
        const queue = localStorage.getItem('neonpro_pending_actions');
        return queue ? JSON.parse(queue).filter(action => action.type === 'CREATE_PRESCRIPTION') : [];
      });
  ...
  describe('Performance and Storage Management', () => {
    test('should manage offline storage size and cleanup', async ({ context, page }) => {
      ...
      const postCleanupStorage = await page.evaluate(() => {
        const keys = ['neonpro_offline_patients', 'neonpro_offline_records', 'neonpro_offline_media'];
        return keys.map(key => localStorage.getItem(key)).filter(Boolean).length;
      });
    ...
    test('should optimize data synchronization performance', async ({ context, page }) => {
      ...
      await expect(page.locator('[data-testid="batch-sync-progress"]')).toBeVisible({ timeout: 3000 });
      ...
      await page.evaluate(() => {
        localStorage.removeItem('neonpro_pending_actions');
        localStorage.setItem('neonpro_last_sync', new Date().toISOString());
        window.dispatchEvent(new CustomEvent('batch-sync-completed', {
    ...
    test('should handle storage quota exceeded scenarios', async ({ page }) => {
      ...
      const quotaError = await page.locator('[data-testid="storage-quota-error"]').textContent();
      ...
      await expect(page.locator('[data-testid="cleanup-options"]')).toBeVisible();
  ...
  describe('Healthcare-Specific Offline Features', () => {
    test('should support emergency offline access to patient data', async ({ context, page }) => {
      ...
      await page.evaluate((patients) => {
        localStorage.setItem('neonpro_emergency_patients', JSON.stringify(patients));
      }, emergencyPatients);
      ...
      await page.fill('[data-testid="emergency-patient-search"]', '123.456.789-09');
      await page.click('[data-testid="emergency-search-button"]');

      // Should find patient in offline emergency cache
      await expect(page.locator('[data-testid="emergency-patient-found"]')).toBeVisible();
      
      // Should display critical information
      await expect(page.locator('[data-testid="patient-blood-type"]')).toContainText('O+');
      await expect(page.locator('[data-testid="patient-allergies"]')).toContainText('Penicilina');
      await expect(page.locator('[data-testid="emergency-contact"]')).toContainText('Maria Silva');
      await expect(page.locator('[data-testid="critical-medications"]')).toContainText('Losartana');

      // Should show offline emergency mode indicator
      await expect(page.locator('[data-testid="emergency-offline-mode"]')).toBeVisible();
      const emergencyMode = await page.locator('[data-testid="emergency-offline-mode"]').textContent();
    ...
    test('should maintain prescription history offline', async ({ context, page }) => {
      ...
      const prescriptionHistory = [
        ...
        },
        {
          ...
          medications: [
            { name: 'Protetor Solar FPS 60', dosage: 'aplicar antes da exposição', duration: 'uso contínuo' },
          ],
          status: 'completed',
        },
      ];
      ...
      await page.evaluate((prescriptions) => {
        localStorage.setItem('neonpro_offline_prescriptions_1', JSON.stringify(prescriptions));
      }, prescriptionHistory);
      ...
      await expect(page.locator('[data-testid="prescription-history"]')).toBeVisible();
      ...
      await expect(activePrescription.locator('[data-testid="prescription-medication"]')).toContainText('Losartana');
      await expect(activePrescription.locator('[data-testid="prescription-physician"]')).toContainText('Dr. Ana Santos');
      await expect(activePrescription.locator('[data-testid="prescription-status"]')).toContainText('active');
    });
    ...
    test('should support offline appointment modifications with conflict resolution', async ({ context, page }) => {
      ...
      await page.evaluate((appointment) => {
        localStorage.setItem('neonpro_offline_appointments', JSON.stringify([appointment]));
      }, existingAppointment);
      ...
      await page.fill('[data-testid="appointment-notes"]', 'Horário alterado pelo paciente - offline');
      await page.click('[data-testid="save-appointment"]');

      // Should queue appointment update
      await expect(page.locator('[data-testid="appointment-update-queued"]')).toBeVisible();
      ...
      const appointmentUpdate = queuedActions.find(action => action.type === 'UPDATE_APPOINTMENT');
      ...
      await expect(page.locator('[data-testid="appointment-sync-in-progress"]')).toBeVisible();
      
      // Simulate conflict (appointment was also modified online)
      await page.evaluate(() => {
        window.dispatchEvent(new CustomEvent('appointment-conflict', {
          detail: {
            appointmentId: 'apt-1',
            onlineVersion: { time: '16:00', notes: 'Reagendado pela clínica' },
            offlineVersion: { time: '15:30', notes: 'Horário alterado pelo paciente - offline' },
          }
        }));
      });

      // Should show conflict resolution for appointment
      await expect(page.locator('[data-testid="appointment-conflict-dialog"]')).toBeVisible();
      await expect(page.locator('[data-testid="conflict-online-version"]')).toContainText('16:00');
      await expect(page.locator('[data-testid="conflict-offline-version"]')).toContainText('15:30');
      ...
      await expect(page.locator('[data-testid="appointment-sync-success"]')).toBeVisible();
    });
  });
});