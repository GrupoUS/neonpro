...
vi.mock('../../lib/services/lgpd-service', () => ({
...
describe('LGPD Compliance Flow Integration Tests', () => {
  ...
  describe('Consent Management', () => {
    ...
    it('should handle consent revocation with immediate effect', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.revoked_consent.granted).toBe(false);
      expect(result.revoked_consent.revoked_at).toBeDefined();
      expect(result.data_processing_stopped).toBe(true);
    });
  });
  describe('Data Subject Rights', () => {
    ...
    it('should process data portability requests (Right to Data Portability)', async () => {
      ...
      expect(result.portable_data.structured_data).toBe(true);
      expect(result.machine_readable).toBe(true);
    });
  });
  ...
  describe('Data Retention and Lifecycle Management', () => {
    ...
    it('should handle automated data anonymization on retention expiry', async () => {
      ...
      expect(result.medical_data_aggregated).toBe(true);
    });
  });
  ...
  describe('Brazilian Healthcare Specific Compliance', () => {
    ...
    it('should enforce professional council registration validation', async () => {
      ...
      expect(accessValidation.professional_verified).toBe(true);
      expect(accessValidation.council_registration_active).toBe(true);
      expect(accessValidation.access_granted).toBe(true);
      expect(accessValidation.audit_required).toBe(true);
    });
    ...
    it('should handle ANVISA reporting compliance for medication data', async () => {
      ...
      expect(anvisaReporting.controlled_substances_found).toBe(true);
      expect(anvisaReporting.reporting_required).toBe(true);
      expect(anvisaReporting.controlled_medications).toContain('Rivotril 2mg');
      expect(anvisaReporting.automated_reporting).toBe(true);
    });
  });
});
