...
test.skip('T023: Integration test LGPD consent flow', async () => {
  ...
  expect(
    screen.getByLabelText(/Concordo com o tratamento de dados/),
  ).toBeInTheDocument();
  expect(
    screen.getByLabelText(/Autorizo compartilhamento/),
  ).toBeInTheDocument();

  // Test consent flow
  const consentCheckbox = screen.getByLabelText(
    /Concordo com o tratamento de dados/,
  );
  ...
  await waitFor(() => {
    expect(
      screen.getByText('Consentimento registrado com sucesso'),
    ).toBeInTheDocument();
  });
