...
test.describe('AI Chat E2E Flow', () => {
  ...
  test('should handle consent flow correctly', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="chat-message-user"]')).toContainText('Hello, I need medical help');
  ...
  test('should complete full chat conversation flow', async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem('neonpro_consent', JSON.stringify({
        dataProcessing: true,
        aiInteraction: true,
    ...
    for (const [index, message] of conversation.entries()) {
      ...
      await expect(page.locator(`[data-testid="chat-message-user"]:nth-child(${(index + 1) * 2 - 1})`))
        .toContainText(message);
      
      // Wait for and verify assistant response
      await waitForChatResponse(page);
      await expect(page.locator(`[data-testid="chat-message-assistant"]:nth-child(${(index + 1) * 2})`))
        .toBeVisible();
  ...
  test('should handle streaming responses correctly', async ({ page }) => {
    ...
    await typeMessage(page, '[data-testid="chat-input"]', 'Explain hypertension treatment in detail');
    await page.click('[data-testid="chat-send-button"]');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const currentText = await page.locator('[data-testid="chat-message-assistant"]:last-child').textContent();
    ...
    await expect(page.locator('[data-testid="chat-streaming-indicator"]')).not.toBeVisible({ timeout: 15000 });
    
    // Verify final response
    const finalResponse = await page.locator('[data-testid="chat-message-assistant"]:last-child').textContent();
