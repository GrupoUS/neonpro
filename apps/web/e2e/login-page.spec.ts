...
const DEPLOY_URL = process.env.DEPLOY_URL || 'https://neonpro-v2-qedz6fw09-grupous-projects.vercel.app'
...
test('login page renders styled UI and no console errors', async ({ page }, testInfo) => {
  ...
  await testInfo.attach('login-screenshot', { body: screenshot, contentType: 'image/png' })

  // Log console output for debugging
  if (consoleMessages.length) {
    testInfo.attach('console', { body: consoleMessages.join('\n'), contentType: 'text/plain' })
  }

  // No severe console errors
  const hasError = consoleMessages.some(m => m.startsWith('[error]') || m.toLowerCase().includes('uncaught'))
