...
test.describe("SignupFormDemo - Form Validation", () => {
  ...
  test("should display the NeonPro login form correctly", async ({ page }) => {
    ...
    await expect(
      page.locator("text=AI-First SaaS • Clínicas de Estética"),
    ).toBeVisible();
  ...
  test("should validate required fields", async ({ page }) => {
    ...
    await expect(
      page.locator("#firstname").locator("..").locator("p.text-destructive"),
    ).toContainText("Nome deve ter pelo menos 2 caracteres");
    await expect(
      page.locator("#lastname").locator("..").locator("p.text-destructive"),
    ).toContainText("Sobrenome deve ter pelo menos 2 caracteres");
    await expect(
      page.locator("#email").locator("..").locator("p.text-destructive"),
    ).toContainText("E-mail é obrigatório");
    await expect(
      page.locator("#password").locator("..").locator("p.text-destructive"),
  ...
  test("should validate email format", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Formato de e-mail inválido"),
    ).not.toBeVisible();
  });
  ...
  test("should validate CRM format", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=Formato de CRM inválido. Use: CRM/UF 123456 (ex: CRM/SP 123456)",
      ),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Formato de CRM inválido"),
    ).not.toBeVisible();
  });

  test("should validate name fields with special characters", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=Nome deve conter apenas letras e espaços"),
    ).toBeVisible();
