...
export default async function handler(req: NextRequest) {
  ...
  try {
    ...
    if (page_info.device_type === 'desktop' && page_info.connection === 'wifi') {
      budget = performanceBudgets.desktop_wifi
    } else if (page_info.device_type === 'mobile' && page_info.connection === '4g') {
    ...
    return NextResponse.json(
      {
        ...
        performance_analysis: {
          ...
          grade: performance_score >= 90 ? 'A' : 
                 performance_score >= 80 ? 'B' : 
                 performance_score >= 70 ? 'C' : 'D',
    ...
    return NextResponse.json(
      { error: 'Erro na análise de performance', code: 'BUNDLE_ANALYSIS_ERROR' },
      { status: 500, headers }
    )
  }
}