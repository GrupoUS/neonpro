...
test.describe("AI Chat E2E Flow", () => {
  ...
  test("should handle consent flow correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="consent-ai-interaction"]'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="consent-modal"]'),
    ).not.toBeVisible();
  ...
  test("should complete full chat conversation flow", async ({ page }) => {
    ...
    for (const [index, message] of conversation.entries()) {
      ...
      await expect(
        page.locator(
          `[data-testid="chat-message-user"]:nth-child(${(index + 1) * 2 - 1})`,
        ),
      ...
      await expect(
        page.locator(
          `[data-testid="chat-message-assistant"]:nth-child(${(index + 1) * 2})`,
        ),
      ).toBeVisible();
  ...
  test("should handle streaming responses correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="chat-streaming-indicator"]'),
    ).toBeVisible();
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const currentText = await page
        .locator('[data-testid="chat-message-assistant"]:last-child')
        .textContent();
  ...
  test("should handle error states gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="chat-error-message"]'),
    ...
    // Should allow retry
    await expect(
      page.locator('[data-testid="chat-retry-button"]'),
    ).toBeVisible();
  ...
  test("should support LGPD compliance features", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="consent-decline-button"]'),
    ).toBeVisible();
