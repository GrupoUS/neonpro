...
describe("Security System Integration Tests", () => {
  ...
  describe("Authentication Security", () => {
    ...
    it("should record security events for suspicious activities", async () => {
      ...
      const { events } = await monitoringService.getSecurityEvents(testTenantId, {
  ...
  describe("Data Access Security", () => {
    ...
    it("should prevent unauthorized data access across tenants", async () => {
      ...
      const { data } = await supabase.from("patients").select("*").eq("tenant_id", "tenant-2");
    ...
    it("should encrypt sensitive patient data", async () => {
      ...
      expect(retrievedData?.medical_history).toBe(sensitiveData.medical_history);
    });
  });
  ...
  describe("LGPD Compliance Security", () => {
    it("should enforce consent requirements for data processing", async () => {
      ...
      const { error } = await supabase.from("patients").insert(patientWithoutConsent);
    ...
    it("should validate data retention policies", async () => {
      const { compliance } = await complianceService.checkLGPDCompliance(testTenantId);

      expect(compliance).toBeDefined();
      expect(compliance?.status).toBeDefined();

      if (compliance?.status === "compliant") {
        expect(compliance.details).toHaveProperty("data_retention_compliant", true);
      }
    });
  });
  ...
  describe("Healthcare Data Security", () => {
    ...
    it("should enforce professional access controls", async () => {
      ...
      expect(unauthorizedAccess.data?.length === 0 || unauthorizedAccess.error).toBeTruthy();
    });
  });
  ...
  async function cleanupTestData() {
    ...
    await supabase.from("security_events").delete().eq("tenant_id", testTenantId);

    await supabase.from("compliance_checks").delete().eq("tenant_id", testTenantId);
  }
});
