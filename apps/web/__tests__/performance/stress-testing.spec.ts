...
class StressTester {
    ...
    for (let batch = 0; batch < userBatches; batch++) {
      ...
      console.log(
        `Adding batch ${batch + 1}/${userBatches}: ${usersThisBatch} users (Total: ${
          this.browsers.length + usersThisBatch
        })`,
      );
  ...
  async executeStressScenario(
    ...
    console.log(
      `Starting stress scenario: ${this.pages.length} users for ${durationMs / 1000} seconds`,
    );

    this.pages.forEach((page, userIndex) => {
      const userPromise = this.executeUserStressActions(
        page,
        userIndex,
        endTime,
        actionFrequencyMs,
  ...
  private async executeUserStressActions(
    page: any,
    userIndex: number,
    endTime: number,
    ...
    for (const userCount of STRESS_TEST_CONFIG.RAMP_UP_STEPS) {
      ...
      try {
        ...
        if (!systemHealthy) {
          ...
          console.log(
            `- P95 response time: ${
              results.p95ResponseTime.toFixed(
                0,
              )
