...
class LoadTester {
  ...
  async simulateUserActions(durationMs: number) {
    ...
    this.pages.forEach((page, userIndex) => {
      const userActionPromise = this.simulateIndividualUserActions(
        page,
        userIndex,
        endTime,
      );
  ...
  private async simulateIndividualUserActions(
    page: any,
    userIndex: number,
    endTime: number,
  ) {
    while (Date.now() < endTime) {
      ...
      try {
        ...
        console.warn(
          `User ${userIndex} action ${selectedAction} failed:`,
          error,
        );
      }
    }
  ...
  private async performAction(page: any, action: string, userIndex: number) {
    switch (action) {
      ...
      case 'view_patient_details':
        ...
        if (cardCount > 0) {
          const randomCard = patientCards.nth(
...
test.describe('Patient Dashboard Load Testing', () => {
  ...
  test('should handle medium clinic load (8 concurrent doctors)', async () => {
    ...
    console.log(
      `Starting medium load test with ${userCount} concurrent users...`,
    );
    ...
    expect(avgPageLoadTime).toBeLessThan(
      PERFORMANCE_THRESHOLDS.PAGE_LOAD * 1.5,
    ); // Allow 50% degradation
    expect(maxMemoryUsage).toBeLessThan(
      PERFORMANCE_THRESHOLDS.MEMORY_USAGE * 1.2,
  ...
  test('should handle large clinic load (20 concurrent doctors)', async () => {
    ...
    console.log(
      `Starting large load test with ${userCount} concurrent users...`,
    );
    ...
    expect(avgMemoryUsage).toBeLessThan(
      PERFORMANCE_THRESHOLDS.MEMORY_USAGE * 1.5,
    ...
    for (const patientCount of patientCounts) {
      ...
      if (patientCount <= 500) {
        ...
        expect(searchTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.SEARCH_RESPONSE * 2,
        );
      }
    }
  });
  ...
  test('should test API response times under concurrent requests', async () => {
    ...
    for (const requestCount of concurrentRequests) {
      console.log(
        `Testing API performance with ${requestCount} concurrent requests...`,
      );
      ...
      console.log(
        `${requestCount} concurrent requests completed in ${totalTime}ms`,
      );
      ...
      if (apiResponseTimes.length > 0) {
        ...
        console.log(
          `Avg API response: ${avgResponseTime}ms, Max: ${maxResponseTime}ms`,
        );
        ...
        expect(maxResponseTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.API_RESPONSE * 3,
        );
      }
  ...
  test('should monitor memory leaks during extended usage', async ({ page }) => {
    ...
    console.log(
      `Initial memory usage: ${(initialMemory / 1024 / 1024).toFixed(2)}MB`,
    );
    ...
    for (let cycle = 0; cycle < cycles; cycle++) {
      ...
      if (cycle % 5 === 0) {
        ...
        console.log(
          `Cycle ${cycle}: Memory usage: ${(currentMemory / 1024 / 1024).toFixed(2)}MB`,
        );
      }
    }
    ...
    expect(finalMemory / 1024 / 1024).toBeLessThan(
    ...
    const isStableOrDecreasing = recentGrowth.every(
      (memory, index) => index === 0 || memory <= recentGrowth[index - 1] * 1.1,
    );
