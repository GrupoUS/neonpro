...
export function createRiskAssessmentService(
  ...
  const service = {
    assessPatientRisk: vi.fn().mockImplementation(async (patientData: any) => {
      ...
      if (
        patientData.idade >= 72 &&
        patientData.condicoes_medicas?.length >= 4
      ) {
        riskLevel = 'critical';
        score = 0.95;
      ...
      .mockImplementation(
        async (patientData: any, doctorId: string, options?: any) => {
          ...
          if (options?.simulateFailure) {
            throw new Error(
              'Database connection failed: Unable to process risk assessment'
            );
          }
                ...
                riskAssessment.riskLevel === 'low'
                  ...
                  : riskAssessment.riskLevel === 'moderate'
                    ...
                    : riskAssessment.riskLevel === 'high'
                      ...
                      : 'SENIOR_PHYSICIAN',
              level:
                riskAssessment.riskLevel === 'high' ? 'PHYSICIAN' : 'NURSE',
            },
            scoreBreakdown: {
              overallScore: riskAssessment.score * 100,
              riskLevel: riskAssessment.riskLevel.toUpperCase(),
              criticalFactors:
                riskAssessment.riskLevel === 'high' ? ['idade avançada'] : [],
            },
            recommendations:
              riskAssessment.riskLevel === 'low'
                ? [
