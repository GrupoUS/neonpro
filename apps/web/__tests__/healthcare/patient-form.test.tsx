...
describe("PatientForm - Healthcare Component Testing", () => {
  ...
  const testUser = healthcareTestHelpers.setupTestUser("healthcare_professional", testClinicId);
  ...
  describe("LGPD Compliance Validation", () => {
    it("should display LGPD consent checkbox and privacy notice", () => {
      render(
        <PatientForm onSubmit={mockOnSubmit} onCancel={mockOnCancel} clinicId={testClinicId} />,
      );
      ...
      expect(privacyNotice).toHaveAttribute("href", expect.stringContaining("privacy"));
    });
    ...
    it("should create valid LGPD consent record on form submission", async () => {
      ...
      const validationResult = lgpdValidators.validateConsent(submittedData.lgpd_consent);
  ...
  describe("Medical Data Validation", () => {
    it("should validate CPF format according to Brazilian standards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/cpf.*invalid|cpf.*inválido/i)).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(screen.queryByText(/cpf.*invalid|cpf.*inválido/i)).not.toBeInTheDocument();
      });
    });
    ...
    it("should validate email format for patient communication", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/email.*invalid|email.*inválido/i)).toBeInTheDocument();
      });
    });
    ...
    it("should validate Brazilian phone number format", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/phone.*invalid|telefone.*inválido/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Error Handling and Recovery", () => {
    it("should handle form submission errors gracefully", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/error.*occurred|erro.*ocorreu/i)).toBeInTheDocument();
        expect(
  ...
  describe("Performance and User Experience", () => {
    ...
    it("should show loading states during form submission", async () => {
      ...
      expect(screen.getByRole("button", { name: /submit|enviar/i })).toBeDisabled();
    });
  });

  describe("Integration Testing", () => {
    it("should integrate with healthcare context provider", () => {
      const testScenario = healthcareTestHelpers.createHealthcareTestScenario("basic", {
      ...
      expect(screen.getByDisplayValue(testScenario.patient.name)).toBeInTheDocument();
      expect(screen.getByDisplayValue(testScenario.patient.email)).toBeInTheDocument();
    });
