...
export interface CognitiveContextValue {
  ...
  addMemoryAid: (aid: Omit<MemoryAid, 'id' | 'created_at' | 'last_shown' | 'effectiveness_score'>) => void
...
export function CognitiveAccessibilityProvider({ children }: { children: React.ReactNode }) {
  ...
  // Add memory aid
  const addMemoryAid = useCallback((aid: Omit<MemoryAid, 'id' | 'created_at' | 'last_shown' | 'effectiveness_score'>) => {
  ...
  const logInteraction = useCallback((interaction: string, success: boolean) => {
    ...
    setAnalytics(prev => {
      ...
      if (!success) {
        newMetrics.error_rate = (newMetrics.error_rate * (newMetrics.interaction_count - 1) + 1) / newMetrics.interaction_count
      } else {
        newMetrics.error_rate = (newMetrics.error_rate * (newMetrics.interaction_count - 1)) / newMetrics.interaction_count
      }
    ...
    const exportData = {
      ...
      memory_aids_data: {
        ...
        most_effective_types: Object.keys(analytics.memory_aid_effectiveness)
          .sort((a, b) => analytics.memory_aid_effectiveness[b] - analytics.memory_aid_effectiveness[a])
          .slice(0, 3)
      },
      lgpd_compliance: {
    ...
    memoryAidReminderRef.current = setInterval(() => {
      ...
      memoryAids.forEach(aid => {
        ...
        if (minutesSinceLastShown >= aid.frequency_minutes) {
          ...
          setMemoryAids(prev => prev.map(a => 
            a.id === aid.id ? { ...a, last_shown: now } : a
          ))
        }
      })
...
export function CognitiveAccessibilitySettings() {
  ...
  const handleAddMemoryAid = () => {
    const randomAid = MEMORY_AIDS_TEMPLATES[Math.floor(Math.random() * MEMORY_AIDS_TEMPLATES.length)]
    if (randomAid) {
      addMemoryAid({
        ...randomAid,
        ...
        {settings.enabled && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <Card className="p-4">
              ...
              <Badge variant={
                currentLoad === 'overload' ? 'destructive' :
                currentLoad === 'high' ? 'secondary' :
                currentLoad === 'medium' ? 'outline' : 'default'
              }>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(COGNITIVE_IMPAIRMENTS_PT).map(([key, value]) => (
                      <label key={key} className="flex items-center space-x-2 text-sm">
                        <input
                          type="checkbox"
                          checked={settings.profile.impairment_types.includes(key as CognitiveImpairmentType)}
                          onChange={(e) => {
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {MEDICAL_CONTEXTS_PT[template.medical_context as keyof typeof MEDICAL_CONTEXTS_PT]}
                        </p>
                      </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex flex-col gap-1 items-end">
                        <Badge variant={
                          task.cognitive_load_level === 'low' ? 'default' :
                          task.cognitive_load_level === 'medium' ? 'secondary' : 'destructive'
                        }>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.session_metrics.interaction_count}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Taxa de Erro:</span>
                      <span className="font-semibold">{Math.round(analytics.session_metrics.error_rate * 100)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Pausas Tomadas:</span>
                      <span className="font-semibold">{analytics.session_metrics.attention_breaks_taken}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.attention_patterns.average_focus_duration}s</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Distrações:</span>
                      <span className="font-semibold">{analytics.attention_patterns.distraction_frequency}</span>
                    </div>
                    <div>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.improvement_indicators.task_completion_rate}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Redução de Erros:</span>
                      <span className="font-semibold">{analytics.improvement_indicators.error_reduction_percentage}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Crescimento da Independência:</span>
                      <span className="font-semibold">{analytics.improvement_indicators.independence_growth}%</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.session_metrics.help_requests}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Indicadores de Confusão:</span>
                      <span className="font-semibold">{analytics.session_metrics.confusion_indicators}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Capacidade Restante:</span>
                      <span className="font-semibold">{analytics.session_metrics.estimated_remaining_capacity}%</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div className="text-center">
              ...
              <div className="text-sm font-semibold">
                {MEDICAL_CONTEXTS_PT[settings.profile.medical_context as keyof typeof MEDICAL_CONTEXTS_PT]}
              </div>
            </div>
          </div>
        </CardContent>
              ...
              {currentTask.steps.map((step, index) => (
                <div key={index} 
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleCompleteStep(currentTask.id, index)}
                  >
                    <CheckCircle2 className="h-4 w-4" />
                  </Button>
                </div>
