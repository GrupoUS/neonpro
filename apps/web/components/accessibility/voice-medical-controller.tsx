...
// Speech recognition types are declared in types/shims.d.ts
...
export function VoiceMedicalProvider({ children, }: { children: React.ReactNode },) {
    ...
    if (recognition.current) {
      ...
      const handleError = (event: any,) => {
        ...
        if (event.error === 'not-allowed') {
          speak(
            'Permissão de microfone negada. Por favor, permita o acesso para usar comandos de voz.',
            'high',
          )
  ...
  const processMedicalTerminology = useCallback(
    (transcript: string, confidence: number,): MedicalTerm[] => {
      const recognizedTerms: MedicalTerm[] = []

      for (const vocabulary of vocabularies) {
        // Skip if specialty focus is set and doesn't match
        if (
          settings.specialty_focus !== 'all' && vocabulary.specialty !== settings.specialty_focus
        ) {
          continue
        }
  ...
  const processVoiceCommands = useCallback(
    (transcript: string, confidence: number,): VoiceCommand | null => {
      const allCommands = [
        ...registeredCommands,
  ...
  const executeVoiceCommand = useCallback((command: VoiceCommand, originalTranscript: string,) => {
    ...
    if (command.lgpd_sensitive && !consentGiven.current) {
      speak(
        'Esta ação requer consentimento LGPD. Por favor, confirme no painel de configurações.',
        'high',
      )
      return
    }
  ...
  const speak = useCallback(
    (text: string, priority: 'normal' | 'high' | 'emergency' = 'normal',) => {
      ...
      if (
        synthesis.current && currentUtterance.current
        && (priority === 'emergency'
          || (priority === 'high' && currentUtterance.current.text !== text))
      ) {
...
export function VoiceMedicalController({
  ...
  const activeVocabulary = vocabularies.find(v => v.specialty === settings.specialty_focus)
    || vocabularies[0]
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-mono">
                      {current_session.confidence_scores.length > 0
                        ? Math.round(
                          current_session.confidence_scores.reduce((a, b,) => a + b, 0,)
                            / current_session.confidence_scores.length * 100,
                        ) + '%'
                        : 'N/A'}
                    </span>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked,) =>
                  updateSettings({ medical_vocabulary_enabled: checked, },)}
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked,) =>
                  updateSettings({ pronunciation_assistance: checked, },)}
              />
            </div>
          ...
          <div className="pt-4 border-t">
            ...
            <div className="mt-4 p-3 bg-green-50 rounded-lg">
              <p className="text-sm text-green-700">
                ...
                Dados de voz são processados localmente quando possível. Consentimento explícito é
                solicitado para funcionalidades que requerem dados sensíveis.
              </p>
            </div>
          </div>
        )}
