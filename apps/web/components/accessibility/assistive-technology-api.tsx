...
export function useAssistiveTechnologyAPI() {
  ...
  if (context === undefined) {
    throw new Error('useAssistiveTechnologyAPI must be used within an AssistiveTechnologyAPIProvider')
  }
  return context
}
...
export function AssistiveTechnologyAPIProvider({ children }: { children: React.ReactNode }) {
    ...
    const promises = connectedDevices
      ...
      .map(async (device) => {
        try {
          ...
          if (device.capabilities.includes('text_to_speech') && message.type === 'command_feedback') {
  ...
  }, [settings.emergency_override, visualAccessibility, cognitiveAccessibility, oneHandedOperation, broadcastToDevices])
        ...
        {settings.enabled && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="p-4">
              ...
              <div className="text-2xl font-bold">{Math.round(analytics.average_response_time)}ms</div>
              <div className="text-xs text-muted-foreground">
                tempo médio
              </div>
            </Card>
            ...
            <Card className="p-4">
              ...
              <div className="flex items-center gap-1">
                <span className="text-lg font-bold">{analytics.accessibility_improvement_score}%</span>
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label className="text-sm font-medium">Habilitar API de Tecnologias Assistivas</label>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    onValueChange={([data_sync_interval]) => 
                      updateSettings({ data_sync_interval })}
                    min={100}
                    max={5000}
                    step={100}
                    className="w-full"
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            {device.medical_certified && (
                              <Badge variant="secondary">Médico</Badge>
                            )}
                            {device.brazilian_certified && (
                        ...
                        <div className="flex flex-col gap-2 items-end">
                          <Badge variant={
                            isConnected ? "default" : 
                            isConnecting ? "secondary" : "outline"
                          }>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={
                            command.priority_level === 'emergency' ? 'destructive' :
                            command.priority_level === 'high' ? 'secondary' : 'outline'
                          }>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.medical_priorities_triggered}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{analytics.accessibility_improvement_score}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Taxa de Erro:</span>
                      <span className="font-semibold">
                        {Math.round((analytics.errors_encountered / Math.max(analytics.commands_executed, 1)) * 100)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tempo de Resposta:</span>
                      <span className="font-semibold">{Math.round(analytics.average_response_time)}ms</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-1">
                    ...
                    {connectedDevices.filter(d => d.status === 'connected').length === 0 && (
                      <div className="text-sm text-muted-foreground">Nenhum dispositivo conectado</div>
                    )}
                  </div>
                </CardContent>
              </Card>
    ...
    <div className="space-y-4">
          ...
          <div className="space-y-2">
            {availableCommands.slice(0, 4).map((command) => (
              <div key={command.id} 
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={command.priority_level === 'emergency' ? 'destructive' : 'outline'}>
          ...
          <AlertDescription>
            <div className="space-y-1">
              ...
              <div className="text-xs">
                Tempo de execução: {commandResult.execution_time_ms}ms •
                Segurança médica: {commandResult.medical_safety_checked ? '✓' : '⚠️'}
              </div>
            </div>
          </AlertDescription>
        </Alert>
      ...
      {activeDemo === 'device_connected' && (
        <Alert>
          ...
          <AlertDescription>
            NVDA Screen Reader conectado com sucesso. Comandos de voz e navegação por teclado agora estão disponíveis.
          </AlertDescription>
        </Alert>
      )}
                ...
                <div key={device.id} className="flex items-center justify-between p-2 border rounded">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${
                      ...
                      device.status === 'connecting' ? 'bg-yellow-500' : 'bg-gray-500'
                    }`} />
      ...
      <Card className="bg-muted/50">
        <CardContent className="p-3">
          <div className="text-xs space-y-1">
            <div><strong>Endpoints Disponíveis:</strong> {AT_API_ENDPOINTS.length}</div>
            <div><strong>Protocolos Suportados:</strong> JSON, SSML, ARIA, WebSocket</div>
            <div><strong>Certificações:</strong> ANVISA, LGPD, WCAG 2.1 AA+</div>
            <div><strong>Idiomas:</strong> Português (BR), Inglês, Espanhol</div>
          </div>
        </CardContent>
      </Card>
    </div>
