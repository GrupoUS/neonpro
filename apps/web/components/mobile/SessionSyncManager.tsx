...
const useRealTimeSync = (userId: string, enabled: boolean = true) => {
  ...
  const updateStats = (deviceSessions: DeviceSession[]) => {
    ...
    const activeDevices = deviceSessions.filter(d => 
      now - d.lastActive < activeThreshold
    ).length;

    const syncedDevices = deviceSessions.filter(d => 
      d.syncStatus === 'synced'
    ).length;

    const conflictingDevices = deviceSessions.filter(d => 
      d.syncStatus === 'conflict'
    ).length;
...
export default function SessionSyncManager({
  ...
  const getSyncStatusDisplay = (status: string) => {
    switch (status) {
      ...
      case 'syncing':
        return { color: 'text-blue-600', icon: <RefreshCw className="h-4 w-4 animate-spin" />, bg: 'bg-blue-50' };
      case 'conflict':
        return { color: 'text-amber-600', icon: <AlertTriangle className="h-4 w-4" />, bg: 'bg-amber-50' };
      case 'offline':
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge variant={networkQuality === 'excellent' ? 'default' : 'secondary'} className="text-xs">
              {networkQuality === 'offline' ? <WifiOff className="h-3 w-3" /> : <Wifi className="h-3 w-3" />}
      ...
      <CardContent className="space-y-3">
          ...
          <div className={cn(
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className={cn("text-xs", getSyncStatusDisplay(currentDevice.syncStatus).color)}>
        ...
        {stats.conflictingDevices > 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {stats.conflictingDevices} device{stats.conflictingDevices > 1 ? 's have' : ' has'} sync conflicts that need resolution.
            </AlertDescription>
          </Alert>
        )}
              ...
              {devices.length === 0 ? (
                ...
                <div className="space-y-2">
                  {devices.map((device) => {
                    ...
                    return (
                      <div key={device.id} className="flex items-center justify-between p-2 rounded-md bg-muted/30">
                        <div className="flex items-center gap-2">
