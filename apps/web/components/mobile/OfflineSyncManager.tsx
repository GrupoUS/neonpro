...
const useOfflineSyncManager = () => {
  ...
  const initializeCache = useCallback(async () => {
    try {
      ...
      console.log(`Emergency cache initialized: ${mockPatients.length} patients, ${(totalSize / 1024).toFixed(1)}KB`)
    } catch (error) {
...
export function OfflineSyncManager({ className, emergencyMode = false }: OfflineSyncManagerProps) {
  ...
  const getNetworkIcon = () => {
    switch (syncStats.networkStatus) {
      case 'online': return <Wifi className="h-5 w-5" />
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={syncStats.networkStatus !== 'online' || syncStats.syncInProgress || syncStats.pendingActions === 0}
            variant="outline"
            size="sm"
            className="flex-1"
          >
