					...
					<>
						<CardHeader className="text-center">
							...
							<CardDescription className="text-base">
								Em conformidade com a <strong>Lei Geral de Proteção de Dados (LGPD)</strong>, precisamos do seu
						...
						<CardContent className="space-y-6">
							<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
								<div className="flex items-start space-x-3">
									...
									<div>
										<h3 className="mb-2 font-semibold text-blue-900">Seus Direitos LGPD</h3>
							...
							<div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
								<div className="flex items-start space-x-3">
									...
									<div>
										<h3 className="mb-2 font-semibold text-amber-900">Dados Médicos</h3>
										<p className="text-amber-800 text-sm">
											Como sistema de saúde, alguns dados são <strong>obrigatórios por lei</strong> para prestação de
											cuidados médicos e podem ter períodos de retenção específicos conforme regulamentações sanitárias.
										</p>
									</div>
								</div>
							</div>
							...
							<div className="flex flex-col gap-3 sm:flex-row">
								...
								<Button className="flex-1" onClick={handleAcceptEssential} size="lg" variant="outline">
									<Shield className="mr-2 h-4 w-4" />
									Apenas Essenciais
								</Button>
								<Button className="flex-1" onClick={handleCustomizeConsents} size="lg" variant="outline">
				...
				{step === "detailed" && (
					<>
						<CardHeader>
							...
							<CardDescription>
								Escolha exatamente quais dados você permite que processemos. Você pode alterar essas preferências a
								qualquer momento.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							{consentOptions.map((option) => (
								<Card className="border-2" key={option.id}>
											...
											<div className="flex items-start space-x-3">
												...
												<div className="flex-1">
													<CardTitle className="flex items-center space-x-2 text-lg">
														...
														{option.required && (
															<span className="rounded bg-red-100 px-2 py-1 text-red-800 text-xs">OBRIGATÓRIO</span>
														)}
													</CardTitle>
									...
									<CardContent>
										<div>
											<h4 className="mb-2 font-semibold text-sm">Exemplos de dados processados:</h4>
							...
							<div className="flex flex-col gap-3 pt-4 sm:flex-row">
								...
								<Button className="flex-1" onClick={() => setStep("banner")} variant="outline">
									Voltar
								</Button>
							</div>
						</CardContent>
				...
				{step === "confirmation" && (
					<>
						<CardHeader className="text-center">
							...
							<CardDescription>Revise suas escolhas antes da confirmação final</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="rounded-lg border border-green-200 bg-green-50 p-4">
								<h3 className="mb-3 font-semibold text-green-900">Consentimentos Concedidos:</h3>
								<div className="space-y-2">
									{consentOptions
										.filter((option) => consents[option.id])
										.map((option) => (
											<div className="flex items-center space-x-3 text-sm" key={option.id}>
							...
							{consentOptions.some((option) => !(consents[option.id] || option.required)) && (
								<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
									<h3 className="mb-3 font-semibold text-gray-900">Consentimentos Negados:</h3>
									<div className="space-y-2">
										{consentOptions
							...
							<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
								<div className="flex items-start space-x-3">
									...
									<div className="text-blue-800 text-sm">
										<p className="mb-2">
											<strong>Lembre-se:</strong> Você pode alterar estes consentimentos a qualquer momento através do
											Painel de Conformidade LGPD na sua conta.
										</p>
										<p>
							...
							<div className="flex flex-col gap-3 pt-4 sm:flex-row">
								<Button className="flex-1" onClick={handleFinalConfirmation} size="lg">
									<Check className="mr-2 h-4 w-4" />
									Confirmar e Continuar
								</Button>
								<Button className="flex-1" onClick={() => setStep("detailed")} variant="outline">
									Alterar Seleção
								</Button>
							</div>
						</CardContent>
