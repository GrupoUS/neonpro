...
export function RiskIndicator({ risk, className, showTooltip = true, compact = false }: RiskIndicatorProps) {
  ...
  const indicator = (
    ...
    >
      ...
      {compact ? risk.score.toFixed(0) : `${config.label} (${risk.score.toFixed(1)}%)`}
    </Badge>
  );
...
export function RiskIndicatorList({ risks, onRiskClick }: RiskIndicatorListProps) {
  return (
    <div className="space-y-2">
      {risks.map((item) => (
        <div
        ...
        >
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground">{item.appointmentDate}</div>
          </div>
...
export function useRiskCalculation() {
  const calculateRisk = React.useCallback((factors: {
    ...
    if (factors.previousNoShows > 0) {
      ...
      riskFactors.push(`${factors.previousNoShows} não comparecimento(s) anterior(es)`);
    }
    ...
    // Horário da consulta
    if (factors.timeOfDay === 'early_morning' || factors.timeOfDay === 'late_afternoon') {
      score += 8;
