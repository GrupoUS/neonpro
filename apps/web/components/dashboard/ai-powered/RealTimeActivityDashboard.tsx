...
interface ActivityEvent {
	id: string;
	type: "appointment" | "consultation" | "prescription" | "report" | "message" | "login" | "audit";
	title: string;
	description: string;
	user: {
		name: string;
		...
		const interval = setInterval(() => {
			if (Math.random() > 0.7) {
				const newActivity: ActivityEvent = {
					...
					type: ["appointment", "consultation", "message"][Math.floor(Math.random() * 3)] as any,
		...
		>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
				<Card className="neonpro-card group">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<div className="text-2xl font-bold text-blue-600">{liveStats.activeUsers}</div>
				...
				<Card className="neonpro-card group">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<div className="text-2xl font-bold text-green-600">{liveStats.ongoingConsultations}</div>
								<div className="text-xs text-muted-foreground">Consultas Ativas</div>
							</div>
				...
				<Card className="neonpro-card group">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<div className="text-2xl font-bold text-red-600">{liveStats.systemAlerts}</div>
								<div className="text-xs text-muted-foreground">Alertas Sistema</div>
							</div>
				...
				<Card className="neonpro-card group">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<div className="text-2xl font-bold text-purple-600">{liveStats.dailyAppointments}</div>
								<div className="text-xs text-muted-foreground">Agendamentos Hoje</div>
							</div>
				...
				<Card className="neonpro-card group">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<div className="text-2xl font-bold text-teal-600">{liveStats.completedToday}</div>
			...
			<Card className="neonpro-card">
				<CardHeader>
					...
					<CardDescription>
						Eventos e ações em tempo real • Última atualização: {new Date().toLocaleTimeString("pt-BR")}
					</CardDescription>
				</CardHeader>
				<CardContent>
					<ScrollArea className="h-96">
						<div className="space-y-4">
							{activities.map((activity) => (
								...
								>
									{/* Icon */}
									<div className={`p-2 rounded-lg ${getActivityColor(activity.type, activity.priority)}`}>
