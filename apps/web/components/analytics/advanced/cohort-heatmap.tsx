...
const _getIntensity = (rate: number) => {
...
export function CohortHeatmap({
  ...
  const [selectedMetric, setSelectedMetric] = useState<
    'retention' | 'revenue' | 'churn'
  >('retention');
  const [selectedView, setSelectedView] = useState<
    'heatmap' | 'trends' | 'comparison'
  >('heatmap');
  const [_hoveredCell, _setHoveredCell] = useState<{
    cohort: string;
    period: number;
  } | null>(null);
    ...
    return cohorts
      .map((cohort) => {
        ...
        return {
          ...
          totalRevenue: periods.reduce((sum, p) => sum + p.revenue, 0),
          avgRevenuePerUser:
            periods.reduce((sum, p) => sum + p.averageRevenuePerUser, 0) /
            periods.length,
          performanceScore:
            (finalPeriod?.retentionRate || 0) * 0.6 +
            (periods.reduce((sum, p) => sum + p.averageRevenuePerUser, 0) /
              periods.length) *
              0.4,
        };
    ...
    <Card className={`w-full ${className}`}>
      ...
      <CardContent>
        ...
        >
          ...
          <TabsContent className="mt-6" value="heatmap">
          ...
          <TabsContent className="mt-6" value="trends">
            <div className="space-y-6">
              {/* Trend Chart */}
              <div className="h-96">
                <ResponsiveContainer height="100%" width="100%">
          ...
          <TabsContent className="mt-6" value="comparison">
            <div className="space-y-6">
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-medium">
                          Score: {cohort.performanceScore.toFixed(1)}
                        </p>
                        <p className="text-gray-600 text-sm">
