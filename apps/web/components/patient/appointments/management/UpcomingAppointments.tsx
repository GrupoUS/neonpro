...
export function UpcomingAppointments({
    ...
    return (
      <div className="py-12 text-center">
        ...
        <p className="mx-auto max-w-sm text-muted-foreground text-sm">
          Você não tem agendamentos futuros. Que tal agendar um novo
          procedimento?
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="space-y-2">
                          {!appointment.can_cancel &&
                            appointment.hours_until_appointment < 24 && (
                              <div className="flex items-start gap-2 rounded-md bg-orange-50 p-2 text-orange-600 text-xs dark:bg-orange-950">
                                <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" />
                                <div>
                                  <strong>Cancelamento não permitido</strong>
                                  <br />
                                  Resta
                                  {appointment.hours_until_appointment > 1
                                    ? 'm'
                                    : ''}{' '}
                                  apenas {appointment.hours_until_appointment}h.
                                  Mínimo necessário:{' '}
                          ...
                          {!appointment.can_reschedule &&
                            ...
                            appointment.can_cancel && (
                              <div className="flex items-start gap-2 rounded-md bg-yellow-50 p-2 text-xs text-yellow-600 dark:bg-yellow-950">
                                <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" />
                                <div>
                                  <strong>Reagendamento não permitido</strong>
                                  <br />
                                  Solicitações de reagendamento precisam de 48h
                                  de antecedência.
                                </div>
                              </div>
                            )}
      ...
      {cancellationPolicies && (
        <div className="mt-6 rounded-lg bg-muted/30 p-4 text-muted-foreground text-xs">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              {cancellationPolicies.fee_applies && (
                <div>
                  Taxa por cancelamento tardio: R${' '}
                  {cancellationPolicies.fee_amount.toFixed(2)}
                </div>
              )}
            </div>
            <div>
              <strong>Reagendamento:</strong> 48h de antecedência para
              solicitação
