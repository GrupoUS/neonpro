	...
	const [campaigns, _setCampaigns] = useState<CampaignPerformance[]>([
		{
			...
			behavioralTargeting: ["Alto engagement", "Segmento leal", "Consulta em atraso"],
		},
		{
			id: "2",
			...
			behavioralTargeting: ["Alto risco", "Baixo engagement", "No-show recente"],
		},
	]);

	const [loading, setLoading] = useState(false);
	const [selectedPatient, setSelectedPatient] = useState<PatientBehaviorProfile | null>(patients[0]);
		...
		<div className="space-y-6 p-6">
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center space-x-4">
					...
					<Button onClick={handleAnalyzeBehavior} disabled={loading} className="flex items-center space-x-2">
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
						...
						<Card className="lg:col-span-1">
							<CardHeader>
								...
								<CardDescription>Lista com análise comportamental</CardDescription>
							</CardHeader>
								...
								{patients.map((patient) => (
									<div
										...
										className={`border rounded-lg p-4 cursor-pointer transition-colors ${
											selectedPatient?.id === patient.id ? "bg-primary/5 border-primary" : "hover:bg-muted/50"
										}`}
									...
									>
										<div className="flex items-center justify-between">
											...
											<div className="flex items-center space-x-2">
												<div className={`w-3 h-3 rounded-full ${getSegmentColor(patient.segment)}`} />
									...
									<div className="flex items-center justify-between">
										<div>
											<CardTitle className="flex items-center space-x-2">
												...
												<Badge variant="outline" className={`${getSegmentColor(selectedPatient.segment)} text-white`}>
													{selectedPatient.segment.toUpperCase()}
												</Badge>
											</CardTitle>
											<CardDescription>Perfil comportamental detalhado</CardDescription>
										</div>
										<Button
								...
								<CardContent className="space-y-6">
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
										<div className="text-center">
											<div className="text-2xl font-bold text-primary">{selectedPatient.engagementScore}%</div>
											<p className="text-sm text-muted-foreground">Engagement</p>
										</div>
										<div className="text-center">
											<div className="text-2xl font-bold text-green-500">{selectedPatient.loyaltyIndex}%</div>
											<p className="text-sm text-muted-foreground">Lealdade</p>
										</div>
										<div className="text-center">
											<div className="text-2xl font-bold text-blue-500">{selectedPatient.satisfactionScore}%</div>
									...
									<Card>
										...
										<CardContent>
											<div className="space-y-4">
												{selectedPatient.predictedActions.map((action, index) => (
													<div key={index} className="flex items-center justify-between border rounded-lg p-3">
														...
														<div className="text-right">
															<div className="text-lg font-bold text-primary">{action.probability}%</div>
									...
									<Card>
										...
										<CardContent>
											<div className="space-y-4">
												{selectedPatient.recommendedStrategies.map((strategy, index) => (
													<div key={index} className="border rounded-lg p-4">
														<div className="flex items-center justify-between mb-2">
															<Badge variant="outline">{strategy.type}</Badge>
															<span className="text-sm font-medium">{strategy.confidence}% confiança</span>
														</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-5 gap-6">
						<Card>
							...
							<CardContent>
								...
								<p className="text-sm text-muted-foreground">Alto valor • LTV R$ 15K+</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								...
								<p className="text-sm text-muted-foreground">Engajamento alto • 2+ anos</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								...
								<p className="text-sm text-muted-foreground">Baixo engagement • Ação necessária</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								...
								<p className="text-sm text-muted-foreground">&lt; 3 meses • Onboarding</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								...
								<p className="text-sm text-muted-foreground">6+ meses • Reativação</p>
							</CardContent>
						</Card>
					</div>
				</TabsContent>
				...
				<TabsContent value="campaigns" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>Campanhas automatizadas baseadas em comportamento</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-4">
								{campaigns.map((campaign) => (
									<div key={campaign.id} className="border rounded-lg p-4">
										...
										<div className="grid grid-cols-4 gap-4 text-center">
											...
											<div>
												<div className="text-lg font-semibold">{campaign.metrics.opened}</div>
												<p className="text-xs text-muted-foreground">Abertas</p>
											</div>
											<div>
												<div className="text-lg font-semibold">{campaign.metrics.clicked}</div>
												<p className="text-xs text-muted-foreground">Cliques</p>
											</div>
											<div>
												<div className="text-lg font-semibold">{campaign.metrics.converted}</div>
												<p className="text-xs text-muted-foreground">Conversões</p>
											</div>
										</div>

										<div className="mt-4">
											<p className="text-sm text-muted-foreground mb-2">Segmentação comportamental:</p>
				...
				<TabsContent value="analytics" className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<Card>
							...
							<CardContent>
								<div className="text-3xl font-bold text-success mb-2">R$ 104,167</div>
								<p className="text-sm text-muted-foreground">Projetado para R$ 1.25M/ano</p>
