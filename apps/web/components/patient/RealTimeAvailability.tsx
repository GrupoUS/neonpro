...
export function RealTimeAvailability({
  ...
  const getConnectionStatusIndicator = () => {
    switch (connectionStatus) {
      case 'connected':
        return (
          <Wifi aria-label="Conectado" className="h-4 w-4 text-green-500" />
        );
      case 'connecting':
      case 'reconnecting':
        return (
      ...
      case 'disconnected':
        return (
          <WifiOff aria-label="Desconectado" className="h-4 w-4 text-red-500" />
        );
      default:
        return null;
    }
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <p className="font-medium text-sm">{formatted.date}</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground text-sm">
            ...
            {connectionStatus === 'connecting' &&
              'Conectando às atualizações...'}
            {connectionStatus === 'reconnecting' && 'Reconectando...'}
            {connectionStatus === 'disconnected' &&
              'Atualizações desconectadas'}
          </span>
        </div>
        ...
        <CardContent className="space-y-3">
          {slots.length === 0 ? (
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p className="text-sm">
                Tente selecionar outras datas ou profissionais
              </p>
            </div>
          ) : (
      ...
      {alternatives.length > 0 && (
        <Card id="alternatives-section">
          ...
          <CardContent className="space-y-3">
            <Alert>
              <AlertDescription>
                Encontramos {alternatives.length} horário
                {alternatives.length !== 1 ? 's' : ''}
                similar{alternatives.length !== 1 ? 'es' : ''} que podem
                interessar
              </AlertDescription>
            </Alert>
