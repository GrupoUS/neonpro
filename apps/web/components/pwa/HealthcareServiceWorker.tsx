...
export function useHealthcareServiceWorker() {
	const [state, setState] = useState<ServiceWorkerState>({
		...
		lastSyncTime: null,
		cacheStatus: {
			patientData: { cached: 0, total: 0, lastSync: new Date(), criticalDataAvailable: false },
			emergencyContacts: { cached: 0, total: 0, lastSync: new Date() },
			medicalRecords: { cached: 0, total: 0, lastSync: new Date(), priority: "high" },
			medications: { cached: 0, total: 0, lastSync: new Date(), alertsAvailable: false },
...
export function PWAStatusCard() {
	...
	const getTotalCachePercentage = () => {
		const totalCached = Object.values(sw.cacheStatus).reduce((acc, status) => acc + status.cached, 0);
		const totalItems = Object.values(sw.cacheStatus).reduce((acc, status) => acc + status.total, 0);
		...
		<Card>
			<CardHeader>
				...
				<CardDescription>
					...
					{sw.lastSyncTime && ` • Última sync: ${sw.lastSyncTime.toLocaleTimeString()}`}
				</CardDescription>
			</CardHeader>
			...
			<CardContent className="space-y-4">
				...
				<div className="p-3 bg-muted/50 rounded-lg space-y-2">
					...
					<div className="grid grid-cols-3 gap-2 text-xs">
						...
						<div className="text-center">
							<p className="font-medium">{sw.dataUsage.downloaded.toFixed(1)} MB</p>
							<p className="text-muted-foreground">Baixados</p>
						</div>
						<div className="text-center">
							<p className="font-medium">{sw.dataUsage.uploaded.toFixed(1)} MB</p>
				...
				<div className="text-xs text-muted-foreground p-2 bg-muted/30 rounded border-l-2 border-blue-500">
					...
					Dados armazenados localmente conforme LGPD. Cache criptografado para proteção de informações médicas.
				</div>
			</CardContent>
		</Card>
	);
