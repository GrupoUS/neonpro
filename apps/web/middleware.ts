...
// TanStack-managed route prefixes (enforce single authority)
const TANSTACK_ROUTE_PREFIXES = [
  '/dashboard',
  '/patients',
  '/appointments',
  '/login',
...
export async function middleware(request: NextRequest,) {
  ...
  // Enforce TanStack as routing authority: redirect direct route hits to root with target param
  const isTanStackManaged = TANSTACK_ROUTE_PREFIXES.some((p,) =>
    pathname === p || pathname.startsWith(`${p}/`,)
  )

  const isRoot = pathname === '/'
  const hasTarget = searchParams.get('__tsr_to',)

  if (!isRoot && isTanStackManaged) {
    const to = encodeURIComponent(`${pathname}${request.nextUrl.search || ''}`,)
    const url = new URL(request.nextUrl,)
    url.pathname = '/'
    url.search = `__tsr_to=${to}`
