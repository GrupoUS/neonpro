...
async function main() {
  ...
  const tenant1 = await prisma.tenant.upsert({
    ...
    create: {
      ...
      description: 'Clínica de estética avançada especializada em tratamentos faciais e corporais',
  ...
  const tenant2 = await prisma.tenant.upsert({
    ...
    create: {
      ...
      description: 'Centro de estética com tecnologia de ponta para rejuvenescimento',
  ...
  await prisma.product.createMany({
    data: [
      ...
      },
      {
        ...
        description: 'Renovação celular com ácidos para rejuvenescimento facial',
        price: 280.00,
        tenant_id: tenant1.id
      },
      {
  ...
  console.log(`📊 Criados: ${await prisma.tenant.count()} tenants e ${await prisma.product.count()} produtos`)
}
