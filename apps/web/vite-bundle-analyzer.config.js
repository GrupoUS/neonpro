    ...
    writeBundle(options, bundle) {
      ...
      Object.entries(chunks).forEach(([chunkName, size]) => {
        ...
        if (budget && sizeKB > budget) {
          budgetViolations++;
          console.log(`  ${chunkName}: ${sizeKB} KB / ${budget} KB ${status} (+${sizeKB - budget} KB)`);
        } else if (budget) {
      ...
      const totalBudgetStatus = toKB(totalSize) > PERFORMANCE_BUDGETS.total ? '❌ OVER BUDGET' : '✅ OK';
      const cssBudgetStatus = toKB(cssSize) > PERFORMANCE_BUDGETS.css ? '❌ OVER BUDGET' : '✅ OK';
      const imageBudgetStatus = toKB(imageSize) > PERFORMANCE_BUDGETS.images ? '❌ OVER BUDGET' : '✅ OK';
      
      console.log('🎯 Performance Budget Summary:');
      console.log(`  Total: ${toKB(totalSize)} KB / ${PERFORMANCE_BUDGETS.total} KB ${totalBudgetStatus}`);
      console.log(`  CSS: ${toKB(cssSize)} KB / ${PERFORMANCE_BUDGETS.css} KB ${cssBudgetStatus}`);
      console.log(`  Images: ${toKB(imageSize)} KB / ${PERFORMANCE_BUDGETS.images} KB ${imageBudgetStatus}`);
      ...
      if (budgetViolations > 0) {
        ...
        console.log('   For healthcare platforms, performance is critical for user experience.');
      }
      ...
      if (chunks['feature-telemedicine'] && toKB(chunks['feature-telemedicine']) > 200) {
        console.log('   • Telemedicine chunk is large - consider lazy loading video components');
      }
      
      if (chunks['vendor-healthcare'] && toKB(chunks['vendor-healthcare']) > 250) {
        console.log('   • Healthcare libraries are large - consider splitting FHIR/HL7 components');
      }
      
      if (toKB(totalSize) > 2000) {
        console.log('   • Total bundle is large for healthcare - consider more aggressive code splitting');
      }
      
      if (chunks['vendor-react'] && toKB(chunks['vendor-react']) > 130) {
        console.log('   • React vendor chunk is large - ensure tree-shaking is working properly');
      }
