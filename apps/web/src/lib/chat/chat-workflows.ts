...
export class ChatWorkflowSystem {
  ...
  async processMessage(
    ...
    try {
      ...
      if (activeExecution) {
        ...
        const triggeredWorkflow = await this.checkWorkflowTriggers(message, conversation);
        ...
        if (triggeredWorkflow) {
          ...
          const result = await this.executeWorkflowStep(execution, execution.current_step_id, message);
          responses.push(...result.responses);
          escalationRequired = result.escalation_required;
      ...
      if (this.detectEmergency(message)) {
        const emergencyResult = await this.triggerEmergencyWorkflow(message, conversation);
  ...
  private async checkWorkflowTriggers(
    ...
    const messageText = this.extractTextFromContent(message.content).toLowerCase();
    
    for (const [workflowId, workflow] of this.workflows) {
      for (const trigger of workflow.trigger_conditions) {
        if (messageText.includes(trigger.toLowerCase())) {
          // Additional context checks
          if (workflow.healthcare_context_required && !conversation.healthcare_context) {
            continue;
          }
  ...
  private async executeWorkflowStep(
    ...
    switch (step.type) {
      ...
      case 'question':
        const questionContent = this.processTemplate(step.content, execution.collected_data);
      ...
      case 'action':
        const actionResult = await this.executeWorkflowAction(step, execution, triggerMessage);
  ...
  private async scheduleAppointment(
    ...
    const availableSlots = await this.checkAvailability(preferredDate, specialty);
  ...
  private async assessSymptoms(
    ...
    assessment.requires_immediate_attention = assessment.risk_level === 'critical' || assessment.risk_level === 'high';
    
    if (assessment.requires_immediate_attention) {
      assessment.recommended_actions.unshift('Buscar atendimento médico imediato');
    }
  ...
  private detectEmergency(message: ChatMessage): boolean {
    ...
    const emergencyKeywords = [
      'emergência', 'socorro', 'urgente', 'grave', 'não consigo respirar',
      'dor no peito forte', 'sangramento intenso', 'desmaio', 'convulsão',
      'infarto', 'derrame', 'avc', 'parada cardíaca'
    ];
  ...
  private initializeDefaultWorkflows(): void {
    ...
    this.workflows.set('appointment_booking', {
      ...
      steps: [
        ...
        },
        {
          ...
          options: ['Clínica Geral', 'Dermatologia', 'Cardiologia', 'Neurologia', 'Outro'],
          next_steps: { 
    ...
    this.workflows.set('symptom_assessment', {
      ...
      trigger_conditions: ['sintoma', 'dor', 'mal estar', 'não me sinto bem', 'symptoms'],
      healthcare_context_required: false,
      ...
      steps: [
        ...
        },
        {
          ...
          options: ['Menos de 24 horas', '1-3 dias', '1 semana', 'Mais de 1 semana'],
        ...
        },
        {
          ...
          type: 'message',
          content: 'Com base na avaliação, as recomendações foram geradas. Um profissional revisará e entrará em contato.',
          next_steps: {}
        }
      ]
    });
    ...
    this.workflows.set('emergency_protocol', {
      ...
      completion_actions: ['notify_emergency_contacts', 'create_incident_report'],
      steps: [
        {
  ...
  private determineNextStep(
    ...
    if (currentStep.options && currentStep.options.includes(userResponse)) {
      return currentStep.next_steps[userResponse] || currentStep.next_steps['default'];
    }
  ...
  private async completeWorkflow(execution: WorkflowExecution): Promise<void> {
    ...
    console.log(`✅ Completed workflow: ${workflow?.name} for user: ${execution.user_id}`);
  }
  ...
  private async checkAvailability(date: string, specialty: string): Promise<any[]> {
    // Mock availability - would integrate with actual scheduling system
    return [
      { date: '15/12/2024', time: '14:00', specialty, available: true }
    ];
  }
  ...
  private calculateSeverityScores(symptoms: string[], data: Record<string, any>): Record<string, number> {
    const scores: Record<string, number> = {};
    symptoms.forEach(symptom => {
      scores[symptom] = Math.min(10, Math.max(1, parseInt(data['severity_score']) || 5));
    });
    return scores;
  }

  private calculateRiskLevel(symptoms: string[], data: Record<string, any>): 'low' | 'medium' | 'high' | 'critical' {
    const severity = parseInt(data['severity_score']) || 5;
    const duration = data['symptom_duration'] || '';
    
    if (severity >= 9 || symptoms.includes('dor no peito') || symptoms.includes('dificuldade respirar')) {
      return 'critical';
  ...
  private suggestMedicalSpecialty(symptoms: string[]): string | undefined {
    if (symptoms.includes('dor no peito') || symptoms.includes('palpitação')) {
      return 'Cardiologia';
    } else if (symptoms.includes('dor de cabeça') || symptoms.includes('tontura')) {
      return 'Neurologia';
    } else if (symptoms.includes('dor abdominal') || symptoms.includes('náusea')) {
  ...
  private async collectVitalSigns(execution: WorkflowExecution): Promise<{ response?: MessageContent; escalation_required: boolean }> {
    return {
      response: {
  ...
  private async sendTreatmentReminder(execution: WorkflowExecution): Promise<{ response?: MessageContent; escalation_required: boolean }> {
    return {
      response: {
  ...
  private async updateMedicalRecord(execution: WorkflowExecution): Promise<{ response?: MessageContent; escalation_required: boolean }> {
    return {
      response: {
  ...
  private async calculateRiskScore(execution: WorkflowExecution): Promise<{ response?: MessageContent; escalation_required: boolean }> {
  ...
  private async executeCompletionAction(action: string, execution: WorkflowExecution): Promise<void> {
    console.log(`Executing completion action: ${action} for workflow: ${execution.workflow_id}`);
    // Implement completion actions
  }

  private async escalateToHealthcareProfessional(execution: WorkflowExecution, reason: string): Promise<void> {
    console.log(`Escalating to healthcare professional: ${reason}`);
    // Implement professional escalation
  }

  private async triggerEmergencyProtocol(execution: WorkflowExecution): Promise<void> {
    console.log(`Triggering emergency protocol for execution: ${execution.id}`);
    execution.emergency_escalated = true;
    // Implement emergency protocol
  }

  private async requestProfessionalReview(execution: WorkflowExecution, step: WorkflowStep): Promise<void> {
    console.log(`Requesting professional review for step: ${step.id}`);
    // Implement professional review request
  }

  private async notifyHealthcareProfessional(message: ChatMessage, conversation: ChatConversation): Promise<void> {
    console.log(`Notifying healthcare professional for conversation: ${conversation.id}`);
    // Implement professional notification
  }

  private async notifyEmergencyContacts(message: ChatMessage, conversation: ChatConversation): Promise<void> {
    console.log(`Notifying emergency contacts for conversation: ${conversation.id}`);
...
export function getChatWorkflowSystem(config?: WorkflowConfig): ChatWorkflowSystem {
  if (!workflowSystemInstance && config) {
    workflowSystemInstance = new ChatWorkflowSystem(config);
  }
  
  if (!workflowSystemInstance) {
    throw new Error('Chat Workflow System not initialized. Please provide config.');
  }
