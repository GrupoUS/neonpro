...
export class EmergencyMedicalProtocolsService {
   ...
   */
  public async triggerEmergencyResponse(emergencyData: {
    patientId: string;
    patientName: string;
    patientAge: number;
    symptoms: string[];
    ...
    try {
      ...
      const classification = await this.classifyEmergency(
   ...
   * Classify emergency based on symptoms
   */
  private async classifyEmergency(symptoms: string[]): Promise<
    ValidationResponse<{
  ...
  > {
    ...
    return {
      isValid: false,
      errors: [
        "Nenhum protocolo de emergência encontrado para os sintomas informados",
      ],
    };
  }
  ...
  private async sendEmergencyNotifications(
    ...
    console.log(
      `Emergency notifications sent to ${notifications.length} team members`,
    );
  }
  ...
  public generateEmergencyStatistics(
    ...
    try {
      ...
      const emergenciesByCategory = emergencies.reduce(
        (acc, e) => {
      ...
      const emergenciesByPriority = emergencies.reduce(
        (acc, e) => {
      ...
      const protocolCounts = emergencies.reduce(
        (acc, e) => {
