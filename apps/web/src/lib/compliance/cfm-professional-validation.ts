...
const cfmValidationCache = new Map<string, { data: CFMValidationResult; timestamp: number }>();
...
export class CFMValidationService {
   ...
   */
  private parseCRMNumber(crmNumber: string): { state: CFMState; number: string } | null {
    const crmRegex = /^CRM[-\s]?([A-Z]{2})\s*(\d{4,6})$/i;
    const match = crmNumber.trim().toUpperCase().match(crmRegex);
    
    if (!match) {
      throw new Error('Formato de CRM inválido. Use: CRM-SP 123456 ou CRM/SP 123456');
    }
   ...
   */
  private setCachedValidation(cacheKey: string, result: CFMValidationResult): void {
   ...
   */
  private async callCFMAPI(state: CFMState, number: string): Promise<CFMProfessional | null> {
   ...
   */
  public async validateLicense(crmNumber: string): Promise<ValidationResponse<CFMValidationResult>> {
    try {
      // Parse CRM number
      ...
      return {
        ...
        warnings: result.restrictions.length > 0 ? ['Profissional possui restrições'] : [],
        timestamp: new Date(),
        source: 'cfm-api'
      };
   ...
   */
  public async validateMultipleLicenses(crmNumbers: string[]): Promise<ValidationResponse<CFMValidationResult[]>> {
    ...
    for (const crmNumber of crmNumbers) {
      try {
        ...
        errors.push(`Erro ao validar ${crmNumber}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      }
    }
  ...
  public getValidationStats(): {
    ...
    const auditEntries = this.auditTrail.filter(entry => entry.action === 'license-validated');
    const validLicenses = auditEntries.filter(entry => entry.description.includes('VALID')).length;
   ...
   */
  public async checkExpirationWarnings(days: number = 30): Promise<CFMValidationResult[]> {
...
export const cfmUtils = {
  ...
  formatCRMNumber: (crmNumber: string): string => {
    const parsed = crmNumber.trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
