...
class WorkflowAutomationService {
  ...
  public async processRiskScore(
    ...
    console.log(`🤖 Processing risk score for appointment ${appointmentId}: ${Math.round(riskData.noShowProbability * 100)}%`);
    ...
    for (const rule of applicableRules) {
      ...
      const executionId = await this.executeWorkflow(rule, appointmentId, patientId, riskData, staffMembers);
      if (executionId) {
  ...
  private async executeWorkflow(
    ...
    try {
      ...
      for (let i = 0; i < rule.actions.length; i++) {
        ...
        try {
          const result = await this.executeAction(action, appointmentId, patientId, riskData, staffMembers);
  ...
  private async sendStaffAlert(
    ...
    for (const staff of targetStaff) {
      const alert: StaffAlert = {
        ...
        type: alertType,
        priority: riskData.riskCategory === 'critical' ? 'critical' : 
                 riskData.riskCategory === 'high' ? 'high' : 'medium',
  ...
  private async scheduleIntervention(
    ...
    console.log(`📅 Scheduling ${interventionType} intervention for appointment ${appointmentId} in ${delayMinutes} minutes`);
  ...
  private async assignStaff(action: WorkflowAction, appointmentId: string, staffMembers: StaffMember[]): Promise<any> {
    const targetStaff = this.findTargetStaff(action.config, staffMembers);
  ...
  private async updateAppointmentStatus(action: WorkflowAction, appointmentId: string): Promise<any> {
  ...
  private findTargetStaff(config: any, staffMembers: StaffMember[]): StaffMember[] {
    ...
    if (config.staffRole && Array.isArray(config.staffRole)) {
      return staffMembers.filter(staff => 
        config.staffRole.includes(staff.role) && staff.online
      );
    }
  ...
  private generateAlertMessage(alertType: string, riskData: RiskScoreData, appointmentId: string): string {
    const riskPercentage = Math.round(riskData.noShowProbability * 100);
    ...
    return `Consulta ${appointmentId} apresenta ${riskPercentage}% de probabilidade de falta (confiança: ${confidence}%). ${
      alertType === 'critical_risk' ? 'Ação imediata recomendada.' :
      alertType === 'high_risk' ? 'Considere entrar em contato com o paciente.' :
  ...
  private generateAlertActions(alertType: string, appointmentId: string): any[] {
    const baseActions = [
      ...
      { id: 'view_details', label: 'Ver Detalhes', type: 'primary', handler: 'view_appointment_details' }
    ];

    if (alertType === 'critical_risk' || alertType === 'high_risk') {
      baseActions.push(
        { id: 'contact_patient', label: 'Contatar Paciente', type: 'primary', handler: 'contact_patient' },
        { id: 'schedule_intervention', label: 'Agendar Intervenção', type: 'primary', handler: 'schedule_intervention' }
      );
    }
