...
interface AIInsight {
  id: string
  type: 'health_risk' | 'treatment_optimization' | 'prevention' | 'compliance' | 'operational'
  title: string
  description: string
  confidence: number
...
export function AIInsightsPage() {
      ...
      try {
        ...
        const mockInsights: AIInsight[] = [
          ...
          },
          {
            ...
            description: 'IA identifica padrão de consumo que permite otimização de estoque com redução de custos de 23%',
            confidence: 87,
            priority: 'medium',
          ...
          },
          {
            ...
            description: '128 pacientes elegíveis para programas preventivos baseados em histórico e fatores de risco',
            confidence: 85,
            priority: 'medium',
          ...
          },
          {
            ...
            description: 'Recomendação de redistribuição de horários para reduzir tempo de espera em 40%',
            confidence: 89,
            priority: 'high',
        ...
        const mockTrends: PredictionTrend[] = [
          { date: '2024-01-09', accuracy: 89.5, predictions: 1520, impact: 12.3 },
          { date: '2024-01-10', accuracy: 90.2, predictions: 1645, impact: 13.8 },
          { date: '2024-01-11', accuracy: 91.1, predictions: 1589, impact: 14.2 },
          { date: '2024-01-12', accuracy: 90.8, predictions: 1723, impact: 15.1 },
          { date: '2024-01-13', accuracy: 91.5, predictions: 1687, impact: 16.4 },
          { date: '2024-01-14', accuracy: 92.1, predictions: 1754, impact: 17.8 },
          { date: '2024-01-15', accuracy: 92.3, predictions: 1820, impact: 18.2 }
        ]
  ...
  const getPriorityBadge = (priority: string) => {
    const variants = {
      ...
      high: { variant: 'secondary' as const, label: 'Alta', icon: AlertTriangle },
  ...
  const getModelStatusBadge = (status: string) => {
    const variants = {
      ...
      training: { variant: 'secondary' as const, label: 'Treinando', color: 'bg-blue-500' },
      maintenance: { variant: 'outline' as const, label: 'Manutenção', color: 'bg-yellow-500' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{calculateImpactScore()}%</p>
              </div>
              <TrendingUp className="h-8 w-8 text-green-500" />
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              +2.3% vs semana anterior
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Pacientes Impactados</p>
                <p className="text-2xl font-bold">{calculateTotalPatientsImpacted()}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{getAverageAccuracy()}%</p>
              </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <Select value={selectedCategory} onValueChange={setSelectedCategory}>
          ...
          <SelectContent>
            ...
            <SelectItem value="Predição de Comparecimento">Comparecimento</SelectItem>
            <SelectItem value="Gestão de Estoque">Estoque</SelectItem>
            <SelectItem value="Medicina Preventiva">Preventivo</SelectItem>
          ...
          {filteredInsights.length > 0 ? (
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          ...
                          <span>•</span>
                          <span>{format(new Date(insight.createdAt), "dd/MM/yyyy HH:mm", { locale: ptBR })}</span>
                          {insight.expiresAt && (
                            <>
                              <span>•</span>
                              <span>Expira: {format(new Date(insight.expiresAt), "dd/MM/yyyy", { locale: ptBR })}</span>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                        {insight.data.labels.map((label, index) => (
                          <div key={index} className="text-center">
                            ...
                            <div className="text-sm font-medium">{insight.data.values[index]}</div>
                          </div>
                        ))}
                      </div>
            ...
            <Card>
              <CardContent className="text-center py-8">
                ...
                <p className="text-muted-foreground">Nenhum insight encontrado com os filtros selecionados</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold">{model.predictionsCount.toLocaleString()}</p>
                      </div>
                      <div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold">
                      {Math.round(trends.reduce((sum, trend) => sum + trend.impact, 0) / trends.length)}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Melhor Dia</span>
                    <span className="text-lg font-bold text-green-600">
                      {trends.length > 0 ? format(new Date(trends[trends.length - 1].date), "dd/MM", { locale: ptBR }) : '-'}
                    </span>
                  </div>
                </div>
              </CardContent>
