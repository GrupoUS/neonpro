...
const insuranceProviders = [
  'Unimed', 'Amil', 'Bradesco Saúde', 'SulAmérica', 'Porto Seguro', 'Notredame', 'Intermédica', 'Outra'
]
...
export function PatientRegister() {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!/^\d{5}-?\d{3}$/.test(formData.address_zip_code.replace(/\D/g, '').replace(/(\d{5})(\d{3})/, '$1-$2'))) {
  ...
  const sections = [
    {
      ...
      fields: [
        ...
        { name: 'blood_type', label: 'Tipo Sanguíneo', type: 'select', options: bloodTypes.map(type => ({ value: type, label: type })) },
      ]
    },
    {
      title: 'Endereço',
      fields: [
        ...
        { name: 'address_state', label: 'Estado', type: 'select', required: true, options: brazilianStates.map(state => ({ value: state, label: state })) },
        { 
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'emergency_relationship', label: 'Relação com o Paciente', type: 'text', required: true },
      ]
    },
    {
      title: 'Plano de Saúde',
      fields: [
        { name: 'insurance_provider', label: 'Operadora', type: 'select', options: [
          ...insuranceProviders.map(provider => ({ value: provider, label: provider })),
          { value: '', label: 'Particular' }
        ]},
        { name: 'insurance_plan_type', label: 'Tipo de Plano', type: 'select', options: [
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card key={section.title}>
            <CardHeader>
              ...
              <CardDescription>
                ...
                {section.title === 'Contato de Emergência' && 'Informações para contato em caso de emergência'}
                {section.title === 'Plano de Saúde' && 'Informações sobre o plano de saúde (opcional)'}
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              De acordo com a Lei Geral de Proteção de Dados, necessitamos do seu consentimento para processar os dados do paciente
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <div className="flex items-start space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleInputChange('lgpd_data_processing', checked as boolean)}
                />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="lgpd_data_processing" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
              ...
              <div className="flex items-start space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleInputChange('lgpd_communication', checked as boolean)}
                />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="lgpd_communication" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    Comunicação
                  </Label>
                  <p className="text-sm text-muted-foreground">
              ...
              <div className="flex items-start space-x-3">
                ...
                />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="lgpd_storage" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    Armazenamento
                  </Label>
                  <p className="text-sm text-muted-foreground">
              ...
              <div className="flex items-start space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleInputChange('lgpd_ai_processing', checked as boolean)}
                />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="lgpd_ai_processing" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            ...
            <div className="text-xs text-muted-foreground">
              Data do consentimento: {format(new Date(formData.lgpd_consent_date), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
            </div>
          </CardContent>
        </Card>
