...
export type DocumentationFormat = typeof DOCUMENTATION_FORMATS[keyof typeof DOCUMENTATION_FORMATS];
...
export const DocumentationConfigSchema = z.object({
  ...
  formats: z.array(z.nativeEnum(DOCUMENTATION_FORMATS)).default([
    DOCUMENTATION_FORMATS.MARKDOWN,
    DOCUMENTATION_FORMATS.HTML,
  ]),
  languages: z.array(z.nativeEnum(DOCUMENTATION_LANGUAGES)).default([
    DOCUMENTATION_LANGUAGES.PORTUGUESE_BR,
    DOCUMENTATION_LANGUAGES.ENGLISH,
  ]),
  includeTypes: z.array(z.nativeEnum(DOCUMENTATION_TYPES)).default([
  ...
  healthcareCategories: z.array(z.nativeEnum(HEALTHCARE_DOC_CATEGORIES)).default([
...
export interface DocumentationReport {
  ...
  errors: Array<{
    section: string;
    type: 'content' | 'translation' | 'example' | 'compliance' | 'accessibility';
    message: string;
...
export class DocumentationGeneratorService {
  ...
  async generateDocumentation(): Promise<DocumentationReport> {
    ...
    if (this.config.includeTypes.includes(DOCUMENTATION_TYPES.DEVELOPER_GUIDE)) {
      await this.generateDeveloperGuideDocumentation();
    }

    // Generate healthcare compliance documentation
    if (this.config.includeTypes.includes(DOCUMENTATION_TYPES.HEALTHCARE_COMPLIANCE)) {
  ...
  private async generateArchitectureDocumentation(): Promise<void> {
    const architectureSection: DocumentationSection = {
      ...
      metadata: {
        ...
        version: this.config.version,
        tags: ['architecture', 'system-design', 'compliance', 'performance', 'healthcare'],
        wcagCompliance: true,
        healthcareCompliance: true,
        mobileOptimized: true,
      },
