...
export class ExportService {
  ...
  static async exportFinancialMetrics(
    ...
    const completeMetadata = this.buildMetadata(metadata);    try {
      let blob: Blob;
      let filename: string;
  ...
  private static async exportToCsv(
    ...
    const metadataRows = [
      ...
      [`# Conformidade LGPD: ${metadata.lgpdCompliance.anonymized ? 'Dados Anonimizados' : 'Dados Identificados'}`],
      [""], // Empty row separator
    ];
  ...
  private static async exportToPdf(
    ...
    metrics.forEach(metric => {
      ...
      content += `  Mudança: ${this.formatCurrency(metric.change, locale)} (${this.formatPercentage(metric.changePercentage, locale)})\n`;
  ...
  private static async exportDashboardToPdf(
    ...
    content += "=" + "=".repeat(30) + "\n";
    content += `Receita Total: ${this.formatCurrency(dashboardData.aggregates.totalRevenue, locale)}\n`;
    content += `Despesas Totais: ${this.formatCurrency(dashboardData.aggregates.totalExpenses, locale)}\n`;
    content += `Lucro Líquido: ${this.formatCurrency(dashboardData.aggregates.netProfit, locale)}\n`;
    ...
    content += "TENDÊNCIAS\n";
    content += "=" + "=".repeat(20) + "\n";
    content += `Crescimento da Receita: ${this.formatPercentage(dashboardData.trends.revenueGrowth, locale)}\n`;
    content += `Crescimento das Despesas: ${this.formatPercentage(dashboardData.trends.expenseGrowth, locale)}\n`;
    content += `Crescimento do Lucro: ${this.formatPercentage(dashboardData.trends.profitGrowth, locale)}\n`;
    content += `Crescimento de Pacientes: ${this.formatPercentage(dashboardData.trends.patientGrowth, locale)}\n\n`;
    ...
    content += `Última atualização: ${this.formatDateTime(dashboardData.realTimeData.lastUpdated, locale)}\n`;
