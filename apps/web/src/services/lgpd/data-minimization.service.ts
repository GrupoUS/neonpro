...
import type { MinimizedCalendarAppointment, DataMinimizationLevel } from './calendar-consent.service';
...
export class CalendarDataMinimizationService {
  ...
  async minimizeAppointmentWithCompliance(
    ...
    try {
      ...
      const config = this.getMinimizationConfig(consentLevel, userRole, sensitivity);
      
      // Apply minimization rules
      const minimizedData = this.applyMinimizationRules(appointment, config, context);
      
      // Assess compliance
      const complianceScore = this.calculateComplianceScore(config, sensitivity, context);
      
      // Identify risks
      const risksIdentified = this.identifyRisks(config, sensitivity, context);
      
      // Generate recommendations
      const recommendations = this.generateRecommendations(complianceScore, risksIdentified);
      
      // Determine data categories shared
      const dataCategoriesShared = this.getDataCategoriesShared(config, appointment);
  ...
  async batchMinimizeAppointments(
    ...
    try {
      for (const appointment of appointments) {
        ...
        result.dataCategoriesShared.forEach(category => 
          dataCategoriesShared.add(category)
        );
      }
   ...
   */
  private assessDataSensitivity(appointment: CalendarAppointment): PatientDataSensitivity {
    let sensitivityScore = 0;

    // Check for sensitive medical information
    if (appointment.description?.toLowerCase().includes('hiv')) sensitivityScore += 3;
    if (appointment.description?.toLowerCase().includes('mental')) sensitivityScore += 2;
    if (appointment.description?.toLowerCase().includes('psychiatric')) sensitivityScore += 3;
    if (appointment.description?.toLowerCase().includes('addiction')) sensitivityScore += 2;
    
    // Check for sensitive procedures
    if (appointment.serviceName?.toLowerCase().includes('plastic')) sensitivityScore += 2;
    if (appointment.serviceName?.toLowerCase().includes('psychiatric')) sensitivityScore += 3;
    if (appointment.serviceName?.toLowerCase().includes('therapy')) sensitivityScore += 2;

    // Check appointment status
    if (appointment.status === 'emergency') sensitivityScore += 2;
    if (appointment.notes?.toLowerCase().includes('confidential')) sensitivityScore += 2;
  ...
  private sanitizeMedicalInfo(medicalInfo: string): string {
    ...
    const sensitiveTerms = [
      'hiv', 'aids', 'std', 'mental illness', 'psychiatric',
      'addiction', 'substance abuse', 'cancer diagnosis',
    ];
  ...
  private calculateComplianceScore(
    ...
    if (config.showPatientName && sensitivity === PatientDataSensitivity.LOW) score -= 10;
    if (config.showMedicalInfo && sensitivity === PatientDataSensitivity.MEDIUM) score -= 15;
    if (config.showMedicalInfo && sensitivity === PatientDataSensitivity.HIGH && context === 'view') score -= 20;

    // Deduct for inappropriate export permissions
    if (context === 'export' && config.allowExport && sensitivity === PatientDataSensitivity.HIGH) score -= 25;

    // Deduct for excessive retention
    if (config.retentionDays > 365 && sensitivity === PatientDataSensitivity.LOW) score -= 10;
    if (config.retentionDays > 2555 && sensitivity === PatientDataSensitivity.HIGH) score -= 15;
  ...
  private identifyRisks(
    ...
    if (context === 'export' && config.allowExport) {
      risks.push('Permissão de exportação pode violar princípios de minimização');
    }

    if (config.showMedicalInfo && !this.isHealthcareRole(config.level.toString())) {
      risks.push('Informações médicas acessíveis a não-profissionais de saúde');
    }

    if (config.retentionDays > 365 && sensitivity === PatientDataSensitivity.LOW) {
      risks.push('Período de retenção excessivo para dados de baixa sensibilidade');
    }
  ...
  private generateRecommendations(score: number, risks: string[]): string[] {
    ...
    if (score < 70) {
      recommendations.push('Revisar configurações de minimização de dados imediatamente');
    }

    if (risks.some(r => r.includes('sensíveis'))) {
      recommendations.push('Implementar controle de acesso baseado em sensibilidade dos dados');
    }

    if (risks.some(r => r.includes('exportação'))) {
      recommendations.push('Restringir permissões de exportação de dados sensíveis');
    }

    if (score < 50) {
      recommendations.push('Consultar equipe de compliance LGPD para revisão completa');
    }

    if (score >= 90) {
      recommendations.push('Manter configurações atuais e monitorar regularmente');
    }
  ...
  private getDataCategoriesShared(
    ...
    if (config.showMedicalInfo && this.assessDataSensitivity(appointment) === PatientDataSensitivity.HIGH) {
   ...
   */
  private getMinimalFallback(appointment: CalendarAppointment): MinimizedCalendarAppointment {
    return {
      id: appointment.id,
  ...
  private isHealthcareRole(userRole: string): boolean {
    const healthcareRoles = ['doctor', 'nurse', 'healthcare_professional', 'medical_staff'];
