...
describe('AccessibilityAuditService', () => {
  ...
  describe('performAudit', () => {
    it('should perform comprehensive accessibility audit', async () => {
      ...
      expect(result.level).toBeOneOf([WCAG_LEVELS.A, WCAG_LEVELS.AA, WCAG_LEVELS.AAA]);
    ...
    it('should use document.body when no element provided', async () => {
      const bodySpy = vi.spyOn(document.body, 'querySelectorAll').mockReturnValue([] as any);
  ...
  describe('Color Contrast Audit', () => {
    it('should detect color contrast issues', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('Contraste de cor insuficiente')))
        .toBe(true);
    });
    ...
    it('should pass with good contrast', async () => {
      ...
      expect(result.issues.filter(issue => issue.title.includes('Contraste de cor insuficiente')))
        .toHaveLength(0);
    });
  });
  ...
  describe('Keyboard Navigation Audit', () => {
    it('should detect keyboard navigation issues', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('não acessível via teclado'))).toBe(
        true,
      );
    });
    ...
    it('should detect missing focus indicators', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('Indicador de foco ausente'))).toBe(
        true,
      );
    });
  });
  ...
  describe('ARIA Labels Audit', () => {
    it('should detect missing ARIA labels', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('sem rótulo acessível'))).toBe(true);
    });
    ...
    it('should pass with proper ARIA labels', async () => {
      ...
      const mockInput = {
        ...
        id: 'labeled-input',
        className: 'form-input',
        hasAttribute: vi.fn().mockImplementation(attr => attr === 'aria-label'),
      ...
      expect(result.issues.filter(issue => issue.title.includes('sem rótulo acessível')))
        .toHaveLength(0);
    });
  });
  ...
  describe('Screen Reader Support Audit', () => {
    it('should detect images without alt text', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('sem texto alternativo'))).toBe(true);
    });
    ...
    it('should pass with proper alt text', async () => {
      ...
      expect(result.issues.filter(issue => issue.title.includes('sem texto alternativo')))
        .toHaveLength(0);
    });
  });
  ...
  describe('Healthcare Compliance Audit', () => {
    it('should detect missing LGPD consent indicators', async () => {
      ...
      mockElement.querySelectorAll = vi.fn().mockImplementation(selector => {
        if (selector.includes('lgpd') || selector.includes('consentimento')) return [];
        return [];
      });

      const result = await auditService.performAudit(mockElement);

      expect(result.issues.some(issue => issue.title.includes('consentimento LGPD ausentes'))).toBe(
        true,
      );
    ...
    it('should pass with LGPD consent indicators', async () => {
      ...
      expect(result.issues.filter(issue => issue.title.includes('consentimento LGPD ausentes')))
        .toHaveLength(0);
  ...
  describe('Brazilian Standards Audit', () => {
    it('should detect missing Portuguese language declaration', async () => {
      ...
      expect(result.issues.some(issue => issue.title.includes('português não declarado'))).toBe(
        true,
      );
    });
    ...
    it('should pass with Portuguese language declaration', async () => {
      ...
      expect(result.issues.filter(issue => issue.title.includes('português não declarado')))
        .toHaveLength(0);
    });
  });
  ...
  describe('Audit Result Generation', () => {
    ...
    it('should provide healthcare compliance assessment', async () => {
      ...
      expect(typeof result.healthcareCompliance.anvisaCompliant).toBe('boolean');
