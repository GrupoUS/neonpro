...
describe('Documentation Generator Service', () => {
  ...
  beforeEach(() => {
    config = {
      ...
      languages: [DOCUMENTATION_LANGUAGES.PORTUGUESE_BR, DOCUMENTATION_LANGUAGES.ENGLISH],
      includeTypes: [
  ...
  describe('Configuration Validation', () => {
    it('should validate valid documentation configuration', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.languages).toContain(DOCUMENTATION_LANGUAGES.PORTUGUESE_BR);
      }
    });
    ...
    it('should use default values for optional configuration', () => {
      ...
      expect(result.formats).toEqual([DOCUMENTATION_FORMATS.MARKDOWN, DOCUMENTATION_FORMATS.HTML]);
    ...
    it('should validate healthcare categories', () => {
      ...
      expect(result.healthcareCategories).toContain(HEALTHCARE_DOC_CATEGORIES.PATIENT_MANAGEMENT);
  ...
  describe('Documentation Generation', () => {
    ...
    it('should generate API documentation section', async () => {
      ...
      const apiSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.API);
      expect(apiSection).toBeDefined();
      expect(apiSection?.title).toBe('API Documentation');
      expect(apiSection?.titlePtBr).toBe(DOCUMENTATION_LABELS_PT_BR.apiReference);
      expect(apiSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.PATIENT_MANAGEMENT);
    ...
    it('should generate component documentation section', async () => {
      ...
      const componentService = new DocumentationGeneratorService(componentConfig);
      const report = await componentService.generateDocumentation();

      const componentSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.COMPONENT);
      ...
      expect(componentSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.ACCESSIBILITY_FEATURES);
    ...
    it('should generate architecture documentation section', async () => {
      ...
      const archSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.ARCHITECTURE);
      ...
      expect(archSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.COMPLIANCE_VALIDATION);
    });
    ...
    it('should generate deployment documentation section', async () => {
      ...
      const deploySection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.DEPLOYMENT);
      expect(deploySection).toBeDefined();
      expect(deploySection?.title).toBe('Deployment Guide');
      expect(deploySection?.titlePtBr).toBe(DOCUMENTATION_LABELS_PT_BR.deploymentGuide);
      expect(deploySection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.COMPLIANCE_VALIDATION);
    });
    ...
    it('should generate user guide documentation section', async () => {
      ...
      const userSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.USER_GUIDE);
      expect(userSection).toBeDefined();
      expect(userSection?.title).toBe('User Guide');
      expect(userSection?.titlePtBr).toBe('Guia do Usuário');
      expect(userSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.ACCESSIBILITY_FEATURES);
    });
    ...
    it('should generate developer guide documentation section', async () => {
      ...
      const devSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.DEVELOPER_GUIDE);
      ...
      expect(devSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.COMPLIANCE_VALIDATION);
    });
    ...
    it('should generate healthcare compliance documentation section', async () => {
      ...
      const complianceService = new DocumentationGeneratorService(complianceConfig);
      const report = await complianceService.generateDocumentation();

      const complianceSection = report.sections.find(s =>
        s.type === DOCUMENTATION_TYPES.HEALTHCARE_COMPLIANCE
      );
      ...
      expect(complianceSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.COMPLIANCE_VALIDATION);
    });
    ...
    it('should generate accessibility documentation section', async () => {
      ...
      const accessibilityService = new DocumentationGeneratorService(accessibilityConfig);
      const report = await accessibilityService.generateDocumentation();

      const accessibilitySection = report.sections.find(s =>
        s.type === DOCUMENTATION_TYPES.ACCESSIBILITY
      );
      expect(accessibilitySection).toBeDefined();
      expect(accessibilitySection?.title).toBe('Accessibility Documentation');
      expect(accessibilitySection?.titlePtBr).toBe('Documentação de Acessibilidade');
      expect(accessibilitySection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.ACCESSIBILITY_FEATURES);
    });
    ...
    it('should generate mobile documentation section', async () => {
      ...
      const mobileSection = report.sections.find(s => s.type === DOCUMENTATION_TYPES.MOBILE);
      ...
      expect(mobileSection?.category).toBe(HEALTHCARE_DOC_CATEGORIES.MOBILE_OPTIMIZATION);
    });
  });
  ...
  describe('Documentation Statistics', () => {
    it('should generate accurate statistics', async () => {
      ...
      expect(report.statistics.healthcareCategoriesCovered).toEqual(config.healthcareCategories);
  ...
  describe('Documentation Validation', () => {
    it('should validate documentation quality', async () => {
      ...
      expect(report.validationResults.translationCompleteness).toBeGreaterThan(80);
      ...
      expect(report.validationResults.accessibilityCompliance).toBeGreaterThan(80);
    ...
    it('should generate recommendations based on validation results', async () => {
      ...
      const positiveRecommendations = report.recommendations.filter(rec =>
        rec.includes('excelente') || rec.includes('manter')
      );
  ...
  describe('Brazilian Portuguese Localization', () => {
    it('should provide Brazilian Portuguese labels', () => {
      ...
      expect(DOCUMENTATION_LABELS_PT_BR.patientManagement).toBe('Gestão de Pacientes');
      expect(DOCUMENTATION_LABELS_PT_BR.appointmentScheduling).toBe('Agendamento de Consultas');
      expect(DOCUMENTATION_LABELS_PT_BR.lgpdCompliance).toBe('Conformidade LGPD');
      expect(DOCUMENTATION_LABELS_PT_BR.mobileAccessibility).toBe('Acessibilidade Móvel');
      expect(DOCUMENTATION_LABELS_PT_BR.codingStandards).toBe('Padrões de Codificação');
    });
  ...
  describe('Healthcare Compliance Integration', () => {
    it('should include healthcare compliance metadata', async () => {
      ...
      const healthcareSections = report.sections.filter(s =>
        s.category && Object.values(HEALTHCARE_DOC_CATEGORIES).includes(s.category)
      );
      ...
      healthcareSections.forEach(section => {
        ...
        const hasHealthcareTag = section.metadata.tags.some(tag =>
          tag.includes('healthcare') || tag.includes('patient') || tag.includes('medical')
        );
        expect(
          hasHealthcareTag || section.category === HEALTHCARE_DOC_CATEGORIES.COMPLIANCE_VALIDATION,
        ).toBe(true);
      });
    });
    ...
    it('should include accessibility compliance metadata', async () => {
      ...
      const accessibilitySections = report.sections.filter(s =>
        s.category === HEALTHCARE_DOC_CATEGORIES.ACCESSIBILITY_FEATURES
      );
    ...
    it('should include mobile optimization metadata', async () => {
      ...
      const hasMobileType = config.includeTypes.includes(DOCUMENTATION_TYPES.MOBILE);

      if (hasMobileType) {
        const mobileSections = report.sections.filter(s =>
          s.category === HEALTHCARE_DOC_CATEGORIES.MOBILE_OPTIMIZATION
        );
        ...
        const sectionsWithMobileOptimization = report.sections.filter(s =>
          s.metadata.mobileOptimized
        );
  ...
  describe('Documentation Examples', () => {
    it('should include interactive examples when enabled', async () => {
      ...
      const interactiveService = new DocumentationGeneratorService(interactiveConfig);
      const report = await interactiveService.generateDocumentation();

      const sectionsWithExamples = report.sections.filter(s => s.examples && s.examples.length > 0);
