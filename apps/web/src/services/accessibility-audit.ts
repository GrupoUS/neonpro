...
export class AccessibilityAuditService {
  ...
  private generateAuditResult(): AccessibilityAuditResult {
    const summary = {
      ...
      critical: this.issues.filter(i => i.severity === AUDIT_SEVERITY.CRITICAL).length,
      high: this.issues.filter(i => i.severity === AUDIT_SEVERITY.HIGH).length,
      medium: this.issues.filter(i => i.severity === AUDIT_SEVERITY.MEDIUM).length,
      low: this.issues.filter(i => i.severity === AUDIT_SEVERITY.LOW).length,
      info: this.issues.filter(i => i.severity === AUDIT_SEVERITY.INFO).length,
    };
    ...
    const healthcareCompliance = {
      ...
      cfmCompliant: this.testResults.screenReaderSupport && this.testResults.keyboardNavigation,
      issues: this.issues
