...
export type WCAGPrinciple =
  (typeof WCAG_PRINCIPLES)[keyof typeof WCAG_PRINCIPLES];
...
export class AccessibilityAuditService {
      ...
      if (
        ...
        if (
          ...
          this.addIssue({
            ...
            description: `Elemento não atende aos requisitos de contraste WCAG AA (${contrast.toFixed(
              2,
            )}:1)`,
            ...
            healthcareImpact:
              "Dificulta leitura de informações médicas críticas",
    ...
    interactiveElements.forEach((el) => {
      ...
      if (tabIndex === "-1" && !htmlEl.hasAttribute("aria-hidden")) {
        ...
        this.addIssue({
          ...
          selector: this.getElementSelector(htmlEl),
          recommendation:
            'Remover tabindex="-1" ou adicionar aria-hidden="true"',
          healthcareImpact:
            "Impede acesso a funcionalidades médicas via teclado",
          brazilianCompliance: false,
          autoFixable: true,
        });
      }
      ...
      if (!styles.outline && !styles.boxShadow && !styles.border) {
        ...
        this.addIssue({
          ...
          selector: this.getElementSelector(htmlEl),
          recommendation:
            "Adicionar estilos CSS para :focus com outline ou box-shadow",
    ...
    elementsNeedingLabels.forEach((el) => {
      ...
      if (!hasLabel) {
        ...
        this.addIssue({
          ...
          description:
            "Elemento de formulário sem aria-label, aria-labelledby ou label associado",
          severity: AUDIT_SEVERITY.HIGH,
          ...
          healthcareImpact:
            "Impede identificação de campos médicos por leitores de tela",
          brazilianCompliance: false,
          autoFixable: true,
        });
      }
    ...
    headings.forEach((heading) => {
      ...
      if (level > previousLevel + 1) {
        ...
        this.addIssue({
          ...
          recommendation:
            "Usar hierarquia sequencial de cabeçalhos (h1, h2, h3...)",
  ...
  private async auditHealthcareCompliance(element: HTMLElement): Promise<void> {
    ...
    if (consentElements.length === 0) {
      this.addIssue({
        ...
        description:
          "Não foram encontrados indicadores de consentimento LGPD acessíveis",
        severity: AUDIT_SEVERITY.MEDIUM,
        wcagLevel: WCAG_LEVELS.AA,
        principle: WCAG_PRINCIPLES.UNDERSTANDABLE,
        successCriteria: "3.2.2",
        recommendation:
          "Adicionar indicadores visuais e acessíveis de consentimento LGPD",
        healthcareImpact:
          "Não conformidade com regulamentações brasileiras de dados de saúde",
        brazilianCompliance: false,
        autoFixable: false,
      });
    }
  ...
  private async auditBrazilianStandards(element: HTMLElement): Promise<void> {
    ...
    const hasLangPt =
      element.closest('[lang="pt"], [lang="pt-BR"]') ||
    ...
    if (!hasLangPt) {
      this.addIssue({
        ...
        recommendation:
          'Adicionar lang="pt-BR" ao elemento html ou seções em português',
        healthcareImpact:
          "Pode afetar pronúncia de termos médicos por leitores de tela",
        brazilianCompliance: false,
        autoFixable: true,
      });
    }
  ...
  private generateAuditResult(): AccessibilityAuditResult {
    ...
    const healthcareCompliance = {
      ...
      cfmCompliant:
        this.testResults.screenReaderSupport &&
