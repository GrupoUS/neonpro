...
const TREATMENT_DATABASE = {
  acne: [
    ...
    },
    {
      ...
      category: "peeling",
      description:
        "Peeling médio com ácido tricloroacético para cicatrizes de acne",
      estimatedSessions: 3,
      intervalWeeks: 4,
      confidence: 0.8,
  ...
  pigmentation: [
    ...
    },
    {
      ...
      category: "clareamento",
      description:
        "Tratamento com ácido azelaico para melasma e hiperpigmentação",
...
export class AestheticAIAnalysisService {
    ...
    analysis.concerns.forEach((concern) => {
      ...
      if (concernLower.includes("acne") || analysis.conditions.acne) {
        const acneTreatments = TREATMENT_DATABASE.acne.filter((treatment) => {
          const severityMatch =
            analysis.severity.acne >= 5 || treatment.priority === "high";
  ...
  async analyzeMultiplePhotos(imageUrls: string[]): Promise<{
    ...
    const averageSeverity =
      analyses.reduce((sum, a) => sum + a.severity.overall, 0) /
      analyses.length;

    // Generate combined treatment suggestions
    const allSuggestions = analyses.flatMap((a) =>
      this.generateTreatmentSuggestions(a),
    );
