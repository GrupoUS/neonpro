...
export class AestheticAIAnalysisService {
   ...
   */
  generateTreatmentSuggestions(analysis: AestheticAnalysis): TreatmentSuggestion[] {
  ...
  private validateAnalysis(result: any): AestheticAnalysis {
    ...
    return {
      skinType: validSkinTypes.includes(result.skinType) ? result.skinType : 'normal',
      ...
      severity: {
        ...
        pigmentation: Math.max(1, Math.min(10, Number(result.severity?.pigmentation) || 5)),
        wrinkles: Math.max(1, Math.min(10, Number(result.severity?.wrinkles) || 5)),
      },
   ...
   */
  private getMockAnalysis(analysisType: keyof typeof ANALYSIS_PROMPTS): AestheticAnalysis {
    const baseAnalysis = {
      skinType: 'mista',
    ...
    switch (analysisType) {
      ...
      case 'pigmentation':
        return {
          ...
          conditions: { ...baseAnalysis.conditions, melasma: true, sunDamage: 'moderate' },
      ...
      case 'aging':
        return {
          ...
          conditions: { ...baseAnalysis.conditions, wrinkles: true, sunDamage: 'moderate' },
