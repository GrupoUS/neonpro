...
class ServiceCategoriesService {
  ...
  async getServiceCategories(
    ...
    try {
      ...
      const { data, error } = (await _query) as {
        data: any[] | null;
        error: any | null;
      };
    ...
    try {
      const insertResult: any = await supabase
        ...
        .insert({
          ...
          is_active: categoryData.is_active !== undefined
  ...
  async deleteServiceCategory(categoryId: string): Promise<void> {
    try {
      ...
      if (servicesError) {
        ...
        throw new Error(
          `Failed to check category services: ${servicesError.message}`,
        );
      }
   ...
   */
  async getServiceCategoriesWithServices(
    clinicId: string,
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `Failed to fetch categories with services: ${error.message}`,
        );
      }
      ...
      return (data || []).map(category => ({
        ...
        created_at: category.created_at,
        updated_at: category.updated_at,
        services: (category.service_types || []).map((_service: any) => ({
          id: service.id,
          name: service.name,
  ...
  async initializeDefaultCategories(
    ...
    try {
      const categoriesData = (DEFAULT_SERVICE_CATEGORIES as any[]).map(
        (category: any) => ({
          ...category,
          clinic_id: clinicId,
        }),
