...
function ARFallback({ modelUrl, simulationId }: { modelUrl: string; simulationId: string }) {
				...
				<div className="aspect-video bg-gradient-to-br from-orange-50 to-red-50 rounded-lg flex items-center justify-center">
					{isViewing ? (
						<div className="text-center">
							<div className="w-24 h-24 bg-orange-200 rounded-full animate-pulse mb-4 mx-auto"></div>
							<div className="text-lg font-semibold">Interactive 360° Model</div>
							<div className="text-sm text-gray-600 mt-2">Pinch to zoom, drag to rotate</div>
						</div>
					) : (
						<Button onClick={() => setIsViewing(true)} className="bg-orange-600 hover:bg-orange-700">
...
export default function WebARViewer({ simulationId, modelUrl, onARSession, className = "" }: WebARViewerProps) {
				...
				<CardContent>
					...
					<div className="mt-4 aspect-video bg-blue-50 rounded-lg flex items-center justify-center">
						<div className="text-center">
							...
							<div className="text-lg font-semibold text-blue-800">AR Mode Available</div>
							<div className="text-sm text-blue-600">Open on mobile to access</div>
						</div>
					</div>
				</CardContent>
			</Card>
	...
	return (
		<div className={`relative w-full ${className}`}>
			...
			<canvas
				...
				className={`w-full h-96 bg-black rounded-lg ${arSession.isARActive ? "fixed inset-0 z-50" : ""}`}
			/>
					...
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								<span className="text-sm text-gray-600">{capabilities.webXRSupported ? "WebXR" : "WebGL"} Enabled</span>
							</div>
						...
						<CardContent className="p-4">
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									...
									<Button variant="destructive" size="sm" onClick={arSession.endARSession}>
										Exit AR
									</Button>
								</div>
							</div>
				...
				<Card className="absolute top-4 right-4 w-80 z-10">
					...
					<CardContent className="space-y-3">
						...
						<div className="flex items-start gap-2">
							...
							<div className="text-sm">Tap to place the 3D model in your space</div>
						</div>
						<div className="flex items-start gap-2">
							<div className="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-sm font-bold">
								4
