...
interface PatientAIInsight {
  id: string;
  type: 'health_risk' | 'treatment_optimization' | 'prevention' | 'compliance' | 'engagement' | 'no_show_prediction';
  title: string;
  description: string;
  confidence: number;
...
interface PatientPrediction {
  patientId: string;
  patientName: string;
  predictionType: 'no_show' | 'treatment_response' | 'health_risk' | 'engagement';
  probability: number;
  confidence: number;
  factors: string[];
  ...
  const mockInsights: PatientAIInsight[] = [
    {
      ...
      description: 'Análise preditiva identifica 23 pacientes com probabilidade >85% de não comparecer às consultas desta semana',
      confidence: 0.89,
      priority: 'high',
    ...
    },
    {
      ...
      description: 'IA identifica 15 pacientes com fatores de risco que requerem monitoramento mais frequente',
      confidence: 0.94,
      priority: 'critical',
    ...
    },
    {
      ...
      description: 'Análise de 89 pacientes indica oportunidade de melhoria de 32% na eficácia dos tratamentos faciais',
      confidence: 0.87,
      priority: 'medium',
    ...
    },
    {
      ...
      description: '134 pacientes elegíveis para programas de manutenção e acompanhamento preventivo',
      confidence: 0.82,
      priority: 'medium',
  ...
  const mockPredictions: PatientPrediction[] = [
    {
      ...
      probability: 0.87,
      confidence: 0.92,
      factors: ['Histórico de faltas', 'Consulta agendada há 60 dias', 'Sem confirmação'],
    ...
    },
    {
      ...
      probability: 0.78,
      confidence: 0.85,
      factors: ['Tipo de pele', 'Resposta histórica positiva', 'Idade adequada'],
  ...
  return (
    <div className='container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl'>
      ...
      <section aria-labelledby='model-status-heading'>
        <h2 id='model-status-heading' className='sr-only'>Status dos modelos de IA</h2>
        <div className='grid gap-4 grid-cols-1 md:grid-cols-3 lg:grid-cols-4'>
          {models.map(model => (
            <Card key={model.modelId} className='transition-all hover:shadow-lg'>
              ...
              <CardContent className='pt-2 space-y-2'>
                ...
                <div className='flex justify-between text-sm'>
                  ...
                  <span className='font-medium'>{(model.avgConfidence * 100).toFixed(1)}%</span>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Tabs defaultValue='insights' className='space-y-4'>
              ...
              <div className='flex flex-col sm:flex-row gap-4'>
                <div className='flex-1'>
                  ...
                  >
                    ...
                    <option value='Otimização Clínica'>Otimização Clínica</option>
                    <option value='Engajamento e Retenção'>Engajamento e Retenção</option>
                    <option value='Conformidade e Documentação'>Conformidade</option>
                  </select>
                </div>
                <div className='flex-1'>
            ...
            {filteredInsights.map(insight => (
              <Card
                ...
                style={{
                  borderLeftColor: 
                    ...
                    insight.priority === 'medium' ? '#eab308' : '#6b7280'
                }}
              ...
              >
                <CardHeader className='pb-3'>
                  <div className='flex items-center justify-between'>
                    <CardTitle className='text-base font-medium flex items-center gap-2'>
                      {insight.type === 'no_show_prediction' && <Clock className='h-4 w-4 text-orange-600' />}
                      {insight.type === 'health_risk' && <Heart className='h-4 w-4 text-red-600' />}
                      {insight.type === 'treatment_optimization' && <Target className='h-4 w-4 text-blue-600' />}
                      {insight.type === 'engagement' && <Users className='h-4 w-4 text-green-600' />}
                    ...
                    <div className='flex gap-2'>
                      ...
                      <Badge variant={getStatusColor(insight.status)}>
                        {insight.status === 'active' ? 'Ativo' : insight.status === 'resolved' ? 'Resolvido' : 'Ignorado'}
                      </Badge>
                    </div>
                  </div>
                ...
                <CardContent className='pt-2 space-y-3'>
                  <div className='grid grid-cols-2 gap-4 text-sm'>
                    ...
                    {insight.patientCount && (
                      <div>
                        ...
                        <span className='ml-1 font-medium'>{insight.patientCount}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className='text-xs text-gray-500'>
                    Criado há {formatDistanceToNow(insight.timestamp, { addSuffix: true, locale: ptBR })}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value='predictions' className='space-y-4'>
          <Card>
            <CardHeader>
              <CardTitle className='text-lg font-semibold'>Predições por Paciente</CardTitle>
              <CardDescription>
            ...
            <CardContent>
              <div className='space-y-4'>
                {predictions.map(prediction => (
                  <div key={prediction.patientId} className='border rounded-lg p-4'>
                    <div className='flex items-center justify-between mb-3'>
                      <div>
                        <h3 className='font-medium'>{prediction.patientName}</h3>
                        <p className='text-sm text-gray-600'>{prediction.predictionType}</p>
                      </div>
                      <Badge variant={prediction.probability > 0.8 ? 'destructive' : 'warning'}>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm'>
                      <div>
                        <span className='text-gray-600'>Confiança:</span>
                        <span className='ml-1 font-medium'>{(prediction.confidence * 100).toFixed(0)}%</span>
                      </div>
                      <div>
                        <span className='text-gray-600'>Prazo:</span>
                        <span className='ml-1 font-medium'>{prediction.timeframe}</span>
                      </div>
                    </div>
                    <div className='mt-3'>
        ...
        <TabsContent value='treatments' className='space-y-4'>
          <Card>
            <CardHeader>
              <CardTitle className='text-lg font-semibold'>Eficácia de Tratamentos</CardTitle>
              <CardDescription>
            ...
            <CardContent>
              <div className='space-y-4'>
                {treatments.map(treatment => (
                  <div key={treatment.treatmentId} className='border rounded-lg p-4'>
                    <div className='flex items-center justify-between mb-3'>
                      <div>
                        <h3 className='font-medium'>{treatment.treatmentName}</h3>
                        <p className='text-sm text-gray-600'>{treatment.patientCount} pacientes</p>
                      </div>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-4 gap-4 text-sm'>
                      <div>
                        ...
                        <span className='ml-1 font-medium'>{treatment.duration}</span>
                      </div>
                      <div>
                        <span className='text-gray-600'>Custo:</span>
                        <span className='ml-1 font-medium'>R$ {treatment.cost.toLocaleString('pt-BR')}</span>
                      </div>
                    </div>
        ...
        <TabsContent value='analytics' className='space-y-4'>
          <div className='grid gap-4 grid-cols-1 lg:grid-cols-2'>
            <Card>
              <CardHeader>
                <CardTitle className='text-lg font-semibold'>Distribuição de Insights</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className='text-lg font-semibold'>Tendências de Previsões</CardTitle>
              </CardHeader>
              <CardContent>
                <div className='h-64 flex items-center justify-center text-gray-500'>
                  <div className='text-center'>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <div className='space-y-1 text-sm'>
                  <div><span className='text-gray-600'>Confiança:</span> {(selectedInsight.confidence * 100).toFixed(0)}%</div>
                  <div><span className='text-gray-600'>Prioridade:</span> {getPriorityLabel(selectedInsight.priority)}</div>
                  <div><span className='text-gray-600'>Impacto:</span> {selectedInsight.impact}</div>
                  {selectedInsight.patientCount && (
                    <div><span className='text-gray-600'>Pacientes afetados:</span> {selectedInsight.patientCount}</div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className='space-y-1 text-sm'>
                  <div><span className='text-gray-600'>Criado:</span> {format(selectedInsight.timestamp, 'dd/MM/yyyy HH:mm', { locale: ptBR })}</div>
                  {selectedInsight.expiresAt && (
                    <div><span className='text-gray-600'>Expira:</span> {format(selectedInsight.expiresAt, 'dd/MM/yyyy HH:mm', { locale: ptBR })}</div>
                  )}
                </div>
              </div>
            </div>
                ...
                <div className='space-y-2'>
                  {selectedInsight.actionItems.map((action, index) => (
                    <div key={index} className='border rounded p-3'>
                      ...
                      {action.dueDate && (
                        <p className='text-xs text-gray-500'>Prazo: {format(new Date(action.dueDate), 'dd/MM/yyyy', { locale: ptBR })}</p>
                      )}
                    </div>
                  ))}
                </div>
