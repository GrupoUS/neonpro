    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Link
            to={`/inventory/product/${productId}/edit`}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">{product.anvisa_registration || "Não informado"}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Código de Barras</p>
                    <p className="text-sm font-medium text-gray-900">{product.barcode || "Não informado"}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">{product.min_stock_level} {product.unit_of_measure}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Estoque Máximo</p>
                    <p className="text-sm font-medium text-gray-900">{product.max_stock_level} {product.unit_of_measure}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Ponto de Reposição</p>
                    <p className="text-sm font-medium text-gray-900">{product.reorder_point} {product.unit_of_measure}</p>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "batches" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Link
                  to={`/inventory/product/${productId}/add-batch`}
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
                >
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getBatchStatusColor(batch.status)}`}>
                          {batch.status === "active" ? "Ativo" :
                           batch.status === "expiring_soon" ? "Vencendo em Breve" :
                      ...
                      <div className="flex items-center space-x-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getTransactionTypeColor(transaction.transaction_type)}`}>
                          {transaction.transaction_type === "in" ? "Entrada" :
                           transaction.transaction_type === "out" ? "Saída" : "Ajuste"}
                        </span>
                        <div>
                          <p className="text-sm font-medium text-gray-900">{transaction.reason}</p>
                          <p className="text-sm text-gray-500">
                            {new Date(transaction.created_at).toLocaleDateString('pt-BR')} às {new Date(transaction.created_at).toLocaleTimeString('pt-BR')}
                          </p>
                          <p className="text-sm text-gray-500">Por: {transaction.user_name}</p>
                          {transaction.batch_number && (
                            <p className="text-sm text-gray-500">Lote: {transaction.batch_number}</p>
                          )}
                        </div>
                      </div>
                      <div className="text-right">
                        <p className={`text-sm font-medium ${
                          ...
                          transaction.transaction_type === "out" ? "text-blue-600" : "text-yellow-600"
                        }`}>
