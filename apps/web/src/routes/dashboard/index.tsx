  ...
  const { data: showRate = 0, isLoading: showRateLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!aErr && analytics && analytics.length > 0) {
        ...
        const noshow = analytics.reduce(
          (acc: number, r: any) => acc + (r.is_no_show ? 1 : 0),
          0,
        );
    ...
    queryFn: async () => {
      ...
      const [apptRes, patientRes] = await Promise.all([
        supabase
          .from("appointments")
          .select(
            "id, start_time, status, created_at, patient_id, service_type_id",
          )
        ...
        <DashboardLayout>
          ...
          <DashboardCard>
            <BentoGridItem
              ...
              description={
                loadingRevenue ? "Carregando..." : `Receita total do mês atual`
              }
