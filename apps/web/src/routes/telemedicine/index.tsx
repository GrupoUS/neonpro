...
import type { Session, SessionStats, ScheduledSession } from '@neonpro/types';
...
interface TelemedicineDashboardState {
  ...
  filterType: 'all' | 'consultation' | 'follow_up' | 'emergency' | 'second_opinion';
...
function TelemedicineDashboard() {
  ...
  } = trpc.telemedicine.getActiveSessions.useQuery();

  const { 
    data: complianceStatus 
  } = trpc.telemedicine.getComplianceStatus.useQuery();

  const { 
    data: systemHealth,
    isLoading: healthLoading 
  } = trpc.telemedicine.getSystemHealth.useQuery();
  ...
  trpc.telemedicine.onSessionUpdate.useSubscription(undefined, {
    onData: (data) => {
      if (data.type === 'session_started' || data.type === 'session_ended' || data.type === 'session_updated') {
        refetchSessions();
        refetchStats();
  ...
  // Handle session end
  const handleEndSession = async (sessionId: string, reason: string = 'completed') => {
    try {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">
                Sistema {systemHealth?.status === 'healthy' ? 'Operacional' : 'Instável'}
              </span>
            </div>
            ...
            >
              <RefreshCw className={`h-4 w-4 mr-2 ${(sessionsLoading || statsLoading) ? 'animate-spin' : ''}`} />
              Atualizar
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Agendadas Hoje</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600">
                    {statsLoading ? '...' : `${sessionStats?.avgDuration || 0}min`}
                  </p>
                </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {/* Filter */}
                    <Select value={state.filterStatus} onValueChange={(value: any) => setState(prev => ({ ...prev, filterStatus: value }))}>
              ...
              <CardContent>
                {sessionsLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600">
                      {state.searchQuery ? 'Tente ajustar os filtros de busca.' : 'Não há sessões programadas no momento.'}
                    </p>
                  </div>
                ) : (
                    ...
                    <div className="space-y-4">
                      {filteredSessions.map((session) => (
                        <div key={session.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                          ...
                          <div className="flex items-center space-x-3">
                            {/* Urgency indicator */}
                            {session.urgencyLevel && session.urgencyLevel !== 'low' && (
                              <AlertTriangle className={`h-4 w-4 ${getUrgencyColor(session.urgencyLevel)}`} />
                            )}
                      ...
                      <div key={session.id} className="flex items-center space-x-3 text-sm">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-gray-600">
                            {session.status === 'active' ? 'Em andamento' : 'Finalizada'} • {formatTime(session.startTime)}
                          </div>
                        </div>
                      </div>
                    )) || (
