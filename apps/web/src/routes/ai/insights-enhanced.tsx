...
const generateMockInsights = (): AIInsight[] => [
  ...
  },
  {
    ...
    description:
      "Padrões de agendamento indicam oportunidade de redução de 18% no tempo de espera",
    confidence: 89,
    priority: "medium",
...
const generateMockModels = (): AIModel[] => [
  {
    ...
    description:
      "Modelo de machine learning para prever não comparecimento a consultas",
    features: [
      "patient_age",
      "appointment_time",
      "day_of_week",
...
function AIInsightsPage() {
  ...
  const [populationMetrics, setPopulationMetrics] =
    useState<PopulationHealthMetrics | null>(null);
              ...
              >
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    {insight.category} • {formatPercentage(insight.confidence)}{" "}
                    confiança
                    ...
                    <ul className="text-xs text-gray-600 space-y-1">
                      ...
                      {insight.recommendations.length > 2 && (
                        <li className="text-xs text-purple-600">
                          +{insight.recommendations.length - 2} mais
                          recomendações
                        </li>
                      )}
                    </ul>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    {Object.entries(populationMetrics.riskDistribution).map(
                      ([level, count]) => {
                        const percentage =
                          (count / populationMetrics.totalPatients) * 100;
                        const color =
                          level === "critical"
                            ? "red"
                            : level === "high"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-xs">
                {models.filter((m) => m.status === "active").length} modelos
                ativos
              </Badge>
              <Badge variant="outline" className="text-xs">
                {models
