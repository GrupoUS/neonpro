...
export const Route = createFileRoute('/healthcare-test')({
...
function HealthcareTestComponent() {
  ...
  const { data: patient, isLoading: patientLoading, error: patientError } = usePatient(testPatientId);
  const { data: appointments, isLoading: appointmentsLoading } = usePatientAppointments(testPatientId);
  ...
  const handleUpdatePatient = async () => {
    ...
    try {
      await updatePatientMutation.mutateAsync({
        ...
        updates: {
          patient_notes: `Atualizado em ${new Date().toLocaleString('pt-BR')} - Teste de integração healthcare`,
        },
      });
    ...
    <div className="container mx-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <p><strong>Consentimento LGPD:</strong> 
                  <Badge variant={patient.lgpd_consent_given ? 'default' : 'destructive'} className="ml-2">
                    {patient.lgpd_consent_given ? '✅ Concedido' : '❌ Não concedido'}
                  </Badge>
                </p>
                <p><strong>Status dos Dados:</strong> {patient.data_consent_status || 'Não definido'}</p>
            ...
            <div className="space-y-3">
              {appointments.map((appointment: any) => (
                <div key={appointment.id} className="border-l-4 border-blue-500 pl-4 py-2">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold">{appointment.procedure_type}</h4>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-4">
            <Button
              ...
              variant="outline"
            >
              {isUpdating ? 'Atualizando...' : '📝 Atualizar Observações do Paciente'}
            </Button>
            ...
            <Button
              ...
              variant="outline"
            >
              {isCreatingAppointment ? 'Criando...' : '📅 Criar Agendamento de Teste'}
            </Button>
          </div>
          
          <div className="text-sm text-gray-600 space-y-1">
            <p>• <strong>Atualizar Observações:</strong> Testa a mutação de atualização com logging LGPD</p>
            <p>• <strong>Criar Agendamento:</strong> Testa a criação de agendamento com validações healthcare</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="mt-8 p-4 bg-gray-50 rounded-lg">
        <p className="text-xs text-gray-600 text-center">
          ...
          Teste de integração healthcare com Supabase • ID do Paciente: {testPatientId}
        </p>
      </div>
    </div>
  );
