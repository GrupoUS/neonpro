...
describe('API Documentation Generator', () => {
  ...
  describe('API Endpoint Schema Validation', () => {
    ...
    it('should validate endpoint parameters', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.parameters![0].healthcareContext).toBe('Patient identification');
      }
    });
    ...
    it('should validate request body schema', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.requestBody!.healthcareFields).toContain('patientId');
  ...
  describe('Healthcare API Endpoints', () => {
    it('should generate report with healthcare endpoints', () => {
      ...
      expect(report.categories).toContain(API_CATEGORIES.APPOINTMENT_SCHEDULING);
      expect(report.categories).toContain(API_CATEGORIES.AI_FEATURES);
    });

    it('should include patient management endpoint', () => {
      const report = generator.generateReport();
      const patientEndpoint = report.endpoints.find(e => e.id === 'get-patient');
    ...
    it('should include appointment scheduling endpoint', () => {
      const report = generator.generateReport();
      const appointmentEndpoint = report.endpoints.find(e => e.id === 'create-appointment');
      ...
      expect(appointmentEndpoint?.category).toBe(API_CATEGORIES.APPOINTMENT_SCHEDULING);
      expect(appointmentEndpoint?.title).toBe('Create Appointment');
      expect(appointmentEndpoint?.titlePtBr).toBe('Criar Consulta');
      expect(appointmentEndpoint?.requestBody?.healthcareFields).toContain('accessibility');
    });

    it('should include AI features endpoint', () => {
      const report = generator.generateReport();
      const aiEndpoint = report.endpoints.find(e => e.id === 'ai-no-show-prediction');
  ...
  describe('Healthcare Compliance Features', () => {
    it('should track LGPD compliance', () => {
      ...
      const lgpdEndpoints = report.endpoints.filter(e => e.metadata.compliance?.lgpd);
      expect(lgpdEndpoints.length).toBe(report.complianceFeatures.lgpdCompliant);
    ...
    it('should track ANVISA compliance', () => {
      ...
      const anvisaEndpoints = report.endpoints.filter(e => e.metadata.compliance?.anvisa);
      expect(anvisaEndpoints.length).toBe(report.complianceFeatures.anvisaCompliant);
    });
    ...
    it('should track CFM compliance', () => {
      ...
      const cfmEndpoints = report.endpoints.filter(e => e.metadata.compliance?.cfm);
    ...
    it('should track WCAG compliance', () => {
      ...
      const wcagEndpoints = report.endpoints.filter(e => e.metadata.compliance?.wcag);
      expect(wcagEndpoints.length).toBe(report.complianceFeatures.wcagCompliant);
    });
  });
  ...
  describe('Mobile Optimization', () => {
    it('should track mobile-optimized endpoints', () => {
      ...
      const mobileEndpoints = report.endpoints.filter(e => e.metadata.mobileOptimized);
    ...
    it('should include mobile notes in examples', () => {
      ...
      const endpointsWithExamples = report.endpoints.filter(e =>
    ...
    it('should include mobile optimization in response examples', () => {
      ...
      expect(patientEndpoint?.responses[0].example.accessibility.mobileOptimized).toBe(true);
    });
  });
  ...
  describe('Authentication and Security', () => {
    it('should track authentication methods', () => {
      ...
      const authEndpoints = report.endpoints.filter(e => e.authentication.required);
    ...
    it('should include LGPD consent requirements', () => {
      ...
      const lgpdEndpoints = report.endpoints.filter(e => e.authentication.lgpdConsent);
    ...
    it('should include rate limiting information', () => {
      ...
      const rateLimitedEndpoints = report.endpoints.filter(e => e.metadata.rateLimit);
  ...
  describe('Error Handling', () => {
    it('should include comprehensive error documentation', () => {
      ...
      const endpointsWithErrors = report.endpoints.filter(e => e.errors && e.errors.length > 0);
    ...
    it('should include healthcare-specific errors', () => {
      ...
      const lgpdError = patientEndpoint?.errors?.find(e => e.code === 'LGPD_CONSENT_REQUIRED');
    ...
    it('should include appointment-specific errors', () => {
      ...
      const conflictError = appointmentEndpoint?.errors?.find(e =>
        e.code === 'APPOINTMENT_CONFLICT'
      );
      expect(conflictError).toBeDefined();
      expect(conflictError?.statusCode).toBe(409);
      expect(conflictError?.messagePtBr).toBe('Conflito de horário da consulta');
    });
  });
  ...
  describe('Brazilian Portuguese Localization', () => {
    it('should provide Portuguese API labels', () => {
      ...
      expect(API_LABELS_PT_BR[API_CATEGORIES.PATIENT_MANAGEMENT]).toBe('Gestão de Pacientes');
      expect(API_LABELS_PT_BR[API_CATEGORIES.APPOINTMENT_SCHEDULING]).toBe(
        'Agendamento de Consultas',
      );
      expect(API_LABELS_PT_BR[API_CATEGORIES.AI_FEATURES]).toBe('Recursos de IA');
  ...
  describe('Healthcare Context and Accessibility', () => {
    it('should include healthcare context in parameters', () => {
      ...
      expect(patientEndpoint?.parameters![0].healthcareContext).toContain('Patient identification');
    });
  ...
  describe('Utility Methods', () => {
    it('should get endpoints by category', () => {
      const patientEndpoints = generator.getEndpointsByCategory(API_CATEGORIES.PATIENT_MANAGEMENT);
