...
describe('Component Documentation Generator', () => {
  ...
  describe('Component Documentation Schema Validation', () => {
    ...
    it('should validate component props', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.props![0].healthcareContext).toBe('Patient information display');
        expect(result.data.props![0].accessibilityNotes).toBe('Screen reader compatible');
        expect(result.data.props![0].mobileNotes).toBe('Touch-friendly interface');
      }
    });
  });
  ...
  describe('Healthcare Component Documentation', () => {
    ...
    it('should include Patient Card component', () => {
      const report = generator.generateReport();
      const patientCard = report.components.find(c => c.id === 'patient-card');
    ...
    it('should include Appointment Scheduler component', () => {
      const report = generator.generateReport();
      const scheduler = report.components.find(c => c.id === 'appointment-scheduler');
      ...
      expect(scheduler?.titlePtBr).toBe('Componente de Agendamento de Consultas');
    ...
    it('should include Mobile Accessibility Hook', () => {
      const report = generator.generateReport();
      const hook = report.components.find(c => c.id === 'use-mobile-accessibility');
  ...
  describe('Accessibility Compliance', () => {
    it('should track WCAG compliance levels', () => {
      ...
      expect(report.accessibilityCompliance.screenReaderSupport).toBeGreaterThan(0);
      expect(report.accessibilityCompliance.keyboardNavigation).toBeGreaterThan(0);
      expect(report.accessibilityCompliance.mobileAccessible).toBeGreaterThan(0);

      const wcagAAComponents = report.components.filter(c =>
        c.accessibility.level === ACCESSIBILITY_LEVELS.AA
      );
      expect(wcagAAComponents.length).toBe(report.accessibilityCompliance.wcagAA);
    });
    ...
    it('should include accessibility features in components', () => {
      ...
      expect(patientCard?.accessibility.ariaLabels).toContain('emergency-contact');
    ...
    it('should include accessibility notes in props', () => {
      ...
      const accessibilityProp = patientCard?.props?.find(p => p.name === 'accessibilityLevel');
      expect(accessibilityProp).toBeDefined();
      expect(accessibilityProp?.accessibilityNotes).toBeDefined();
      expect(accessibilityProp?.accessibilityNotes).toContain('accessibility enhancements');
    });
  });
  ...
  describe('Healthcare Compliance', () => {
    it('should track healthcare compliance features', () => {
      ...
      expect(report.healthcareCompliance.patientDataHandling).toBeGreaterThan(0);

      const lgpdComponents = report.components.filter(c => c.healthcare.lgpdCompliant);
      expect(lgpdComponents.length).toBe(report.healthcareCompliance.lgpdCompliant);
    });
    ...
    it('should include healthcare context in props', () => {
      ...
      const emergencyProp = patientCard?.props?.find(p => p.name === 'onEmergencyContact');
  ...
  describe('Mobile Optimization', () => {
    it('should track mobile optimization features', () => {
      ...
      const responsiveComponents = report.components.filter(c => c.mobile.responsive);
      expect(responsiveComponents.length).toBe(report.mobileOptimization.responsive);
    });
    ...
    it('should include mobile notes in props', () => {
      ...
      const mobileOptimizedProp = patientCard?.props?.find(p => p.name === 'mobileOptimized');
  ...
  describe('Component Examples', () => {
    it('should include comprehensive examples', () => {
      ...
      const componentsWithExamples = report.components.filter(c =>
    ...
    it('should include emergency example', () => {
      ...
      const emergencyExample = patientCard?.examples?.find(e => e.id === 'emergency-patient-card');
      expect(emergencyExample).toBeDefined();
      expect(emergencyExample?.title).toBe('Emergency Patient Card');
      expect(emergencyExample?.titlePtBr).toBe('Cartão de Paciente de Emergência');
  ...
  describe('Brazilian Portuguese Localization', () => {
    it('should provide Portuguese component labels', () => {
      expect(COMPONENT_LABELS_PT_BR[COMPONENT_TYPES.FUNCTIONAL]).toBe('Componente Funcional');
      expect(COMPONENT_LABELS_PT_BR[COMPONENT_TYPES.HOOK]).toBe('Hook');
      expect(COMPONENT_LABELS_PT_BR[COMPONENT_CATEGORIES.HEALTHCARE]).toBe('Saúde');
      expect(COMPONENT_LABELS_PT_BR[COMPONENT_CATEGORIES.ACCESSIBILITY]).toBe('Acessibilidade');
      ...
      expect(COMPONENT_LABELS_PT_BR.touchOptimized).toBe('Otimizado para Toque');
    });
    ...
    it('should include Portuguese medical terminology', () => {
      const report = generator.generateReport();
      const healthcareComponents = report.components.filter(c => c.healthcare.brazilianPortuguese);
