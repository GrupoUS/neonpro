...
export const MOBILE_HEALTHCARE_RULES = {
  ...
  MEDICAL_FORM_TOUCH_TARGETS: {
    ...
    severity: 'serious',
    check: (element: Element) => {
      const medicalForms = element.querySelectorAll('form[data-medical="true"], .medical-form');
...
export class MobileAccessibilityOptimizer {
  ...
  private async testDevice(
    ...
    const responsiveIssues = await this.testResponsiveDesign(context, width, height);
  ...
  private async applyOptimizations(
    ...
    const interactiveElements = context.querySelectorAll('button, input, select, textarea, a');
  ...
  async testNetworkConditions(
    ...
    const condition = MOBILE_ACCESSIBILITY_REQUIREMENTS.NETWORK_CONDITIONS[networkCondition];
    ...
    if (nav.connection) {
      ...
      if (connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g') {
        issues.push('Detected very slow network: optimize all assets aggressively.');
        accessibilityScore = 80;
      } else if (connection.effectiveType === '3g') {
        issues.push('Detected 3G network: optimize images and reduce requests.');
      ...
      issues.push('Network Information API not available; using simulated data.');
    }
