...
import type { AxeResults, Result, Node } from 'axe-core';
...
export class HealthcareAccessibilityAuditor {
  ...
  private calculateAccessibilityScore(axeResults: AxeResults): number {
    const totalViolations = axeResults.violations.reduce((sum, violation) => 
      sum + violation.nodes.length, 0
    );
  ...
  private calculateHealthcareSpecificScore(issues: HealthcareAccessibilityIssue[]): number {
    ...
    const totalWeight = issues.reduce((sum, issue) => {
      return sum + (issue.emergencyRelevant ? 3 : 
                   issue.impact === 'critical' ? 3 :
                   issue.impact === 'serious' ? 2 : 1);
    }, 0);
    
    const resolvedWeight = issues.filter(issue => 
      issue.remediationPriority === 'low'
    ).reduce((sum, issue) => {
      return sum + (issue.emergencyRelevant ? 3 : 
                   issue.impact === 'critical' ? 3 :
                   issue.impact === 'serious' ? 2 : 1);
   ...
   */
  private generateRecommendations(issues: HealthcareAccessibilityIssue[]): HealthcareAccessibilityRecommendation[] {
  ...
  private getHighestPriority(issues: HealthcareAccessibilityIssue[]): 'immediate' | 'high' | 'medium' | 'low' {
    const priorityOrder = { immediate: 4, high: 3, medium: 2, low: 1 };
  ...
  private getImplementationSteps(ruleId: HealthcareAccessibilityRuleId): string[] {
    ...
    return stepsMap[ruleId] || ['Analyze issue', 'Implement fix', 'Test with assistive technologies'];
  }
