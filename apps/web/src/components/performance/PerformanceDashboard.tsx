...
export function PerformanceDashboard({ className, compact = false }: PerformanceDashboardProps) {
  ...
  const performanceMetrics = [
    ...
    },
    {
      ...
      status: getPerformanceStatus(metrics.mobileLoadTime, PERFORMANCE_THRESHOLDS.MOBILE_LOAD_TIME),
      ...
      {analytics && (
        <Card>
          ...
          <CardContent>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              <div className='space-y-2'>
                <h4 className='text-sm font-medium text-muted-foreground'>Busca (Média)</h4>
            ...
            {showDetails && (
              <div className='mt-6 pt-6 border-t space-y-4'>
                ...
                <div className='grid grid-cols-2 md:grid-cols-4 gap-4 text-sm'>
                  <div>
                    <span className='text-muted-foreground'>Tempo de Página:</span>
                    <p className='font-mono'>{formatTime(metrics.pageLoadTime)}</p>
                  </div>
                  <div>
                    <span className='text-muted-foreground'>Renderização:</span>
                    <p className='font-mono'>{formatTime(metrics.renderTime)}</p>
                  </div>
                  <div>
                    <span className='text-muted-foreground'>Memória:</span>
                    <p className='font-mono'>{metrics.memoryUsage.toFixed(1)}MB</p>
                  </div>
                  <div>
                    <span className='text-muted-foreground'>Rede:</span>
                    <p className='font-mono'>{formatTime(metrics.networkLatency)}</p>
                  </div>
                </div>
              </div>
            )}
