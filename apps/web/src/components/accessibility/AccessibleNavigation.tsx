...
export function AccessibleNavigation({
  ...
  const toggleSubmenu = useCallback((itemId: string) => {
    setOpenSubmenu(prev => {
      ...
      if (item) {
        announce(
          newState ? `Submenu ${item.label} aberto` : `Submenu ${item.label} fechado`,
          'polite'
        );
      }
      return newState;
              ...
              <ul
                className={`
                  ${isMobile 
                    ...
                    : `absolute left-0 top-full mt-1 w-48 bg-white shadow-lg rounded-md border z-50 ${isSubmenuOpen ? 'block' : 'hidden'}`
                  }
      ...
      >
        ...
        {isMobileMenuOpen && (
          <div
            ...
            className={`
              md:hidden border-t
              ${prefersHighContrast ? 'border-t-2 border-gray-900 bg-white' : 'border-t border-gray-200 bg-white'}
            `}
            role="menu"
