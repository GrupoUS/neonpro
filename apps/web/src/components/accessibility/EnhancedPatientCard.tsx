...
export function EnhancedPatientCard({
  ...
  const healthcareDataCategory = useMemo(() => {
    if (patient.isEmergencyCase || patient.status === 'emergency') return 'emergency';
    if (patient.status === 'critical') return 'critical';
    if (patient.requiresSpecialAttention) return 'special-care';
    if (patient.medication && patient.medication.length > 0) return 'under-treatment';
    return 'general';
  }, [patient]);
  ...
  const formatPatientForScreenReader = useCallback(() => {
    ...
    if (showSensitiveData && patient.phone) {
      parts.push(`Telefone: ${ScreenReaderUtils.formatPhoneNumber(patient.phone)}`);
    }
    ...
    if (showSensitiveData && patient.medication && patient.medication.length > 0) {
      parts.push(`Medicamentos: ${patient.medication.join(', ')}`);
    }

    // Appointment information
    if (patient.lastAppointment) {
      const formattedDate = ScreenReaderUtils.formatDateForScreenReader(patient.lastAppointment);
      parts.push(`Última consulta: ${formattedDate}`);
    }

    if (patient.nextAppointment) {
      const formattedDate = ScreenReaderUtils.formatDateForScreenReader(patient.nextAppointment);
    ...
    if (showSensitiveData && patient.lastVitalSigns) {
      ...
      if (heartRate) parts.push(`Frequência cardíaca: ${heartRate} batimentos por minuto`);
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      announceHealthcareData('Paciente selecionado via teclado', patient.name);
    }
  ...
  return (
    ...
    >
      ...
      <CardContent className='pt-0'>
        ...
        >
          {/* Emergency Information */}
          {showEmergencyInfo && (patient.isEmergencyCase || patient.status === 'emergency') && (
            <div
              className={`
          ...
          {/* Medications */}
          {showSensitiveData && patient.medication && patient.medication.length > 0 && (
            <div className='text-sm text-gray-600'>
              <span className='font-medium'>Medicamentos:</span>{' '}
              <span aria-label={`Medicamentos: ${patient.medication.join(', ')}`}>
          ...
          {/* Allergies */}
          {showSensitiveData && patient.allergies && patient.allergies.length > 0 && (
            <div
              className={`
            ...
            >
              <div className='flex items-center space-x-1'>
                ...
                <span aria-label={`Alergias: ${patient.allergies.join(', ')}`}>
          ...
          {showSensitiveData && patient.phone && (
            <div className='flex items-center space-x-2 text-sm text-gray-600'>
              ...
              <span aria-label={`Telefone: ${ScreenReaderUtils.formatPhoneNumber(patient.phone)}`}>
                {patient.phone}
              </span>
            </div>
          )}
            ...
            <div className='text-sm text-gray-500'>
              ...
              <span
                aria-label={`Próximo agendamento: ${
                  ScreenReaderUtils.formatDateForScreenReader(patient.nextAppointment)
                }`}
              >
