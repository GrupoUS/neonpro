...
import { useHealthcareAccessibilityAudit, useEmergencyAccessibilityAudit } from '../../hooks/useHealthcareAccessibilityAudit';
...
export function HealthcareAccessibilityAuditor({
  ...
  const [selectedJourney, setSelectedJourney] = useState<'registration' | 'appointment' | 'treatment' | 'follow-up' | 'emergency'>('registration');
    ...
    <div className={`healthcare-accessibility-auditor space-y-4 ${className}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            WCAG 2.1 AA+ accessibility audit with Brazilian healthcare compliance (LGPD/ANVISA/CFM)
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold">{result?.accessibilityScore || 0}%</div>
              <div className="text-sm text-muted-foreground">Accessibility</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{result?.healthcareSpecificScore || 0}%</div>
              <div className="text-sm text-muted-foreground">Healthcare</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{result?.complianceScore || 0}%</div>
              <div className="text-sm text-muted-foreground">Compliance</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${getStatusColor(complianceStatus)} text-white rounded px-2 py-1`}>
                {complianceStatus}
              </div>
          ...
          <TabsContent value="overview" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Last run: {lastRun?.toLocaleString('pt-BR')} | Audit #{auditCount}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                      <div><strong>Patient Journey:</strong> {result.context.patientJourney}</div>
                      <div><strong>Data Sensitivity:</strong> {result.context.dataSensitivity}</div>
                      <div><strong>Emergency Context:</strong> {result.context.emergencyContext ? 'Yes' : 'No'}</div>
                      <div><strong>User Disability:</strong> {result.context.userDisabilityProfile || 'General'}</div>
                      <div><strong>Critical Issues:</strong> {criticalIssuesCount}</div>
                      <div><strong>Emergency Issues:</strong> {emergencyIssuesCount}</div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <Progress value={result.accessibilityScore} className="h-2" />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                        ...
                        <Progress value={result.healthcareSpecificScore} className="h-2" />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      {result.context.regulatoryRequirements.map(standard => (
                        <Badge 
                          key={standard.id}
                          variant={isStandardCompliant(standard.id) ? 'default' : 'destructive'}
                ...
                <ScrollArea className="h-96">
                  <div className="space-y-4">
                    {result.issues.map((issue, index) => (
                      <Card key={index} className={issue.impact === 'critical' ? 'border-red-500' : ''}>
                        <CardContent className="pt-4">
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1">
                              <h4 className="font-semibold">{issue.healthcareRuleId}</h4>
                              <p className="text-sm text-muted-foreground">{issue.description}</p>
                            </div>
                            <Badge variant={getPriorityColor(issue.remediationPriority)}>
                          ...
                          <div className="text-sm space-y-1">
                            <div><strong>Patient Impact:</strong> {issue.patientImpact}</div>
                            <div><strong>Affected Flows:</strong> {issue.affectedPatientFlows.join(', ')}</div>
                            <div><strong>Compliance Standards:</strong> {issue.complianceStandards.map(s => s.name).join(', ')}</div>
                          </div>
          ...
          <TabsContent value="recommendations" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {result.recommendations.length} recommendations for improvement
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-96">
                        ...
                        <CardContent className="pt-4">
                          ...
                          <div className="text-sm space-y-2">
                            <div>
                              ...
                              <p className="text-muted-foreground">{rec.healthcareRelevance}</p>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {rec.complianceStandards.map(standard => (
                                  <Badge key={standard.id} variant="outline" className="text-xs">
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground">
                              ...
                              <span>Validation: {rec.validationCriteria.length} criteria</span>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                  ...
                  {result.context.regulatoryRequirements.map(standard => (
                    <div key={standard.id}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant={isStandardCompliant(standard.id) ? 'default' : 'destructive'}>
                          {isStandardCompliant(standard.id) ? 'Compliant' : 'Non-Compliant'}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground mb-2">{standard.description}</p>
                      <div className="text-xs space-y-1">
                        <div><strong>Regulatory Body:</strong> {standard.regulatoryBody}</div>
                        <div><strong>Severity:</strong> {standard.severity}</div>
                        <div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <ScrollArea className="h-96">
                    <pre ref={reportRef} className="text-xs bg-muted p-4 rounded overflow-auto">
                      {generateComplianceReport()}
                    </pre>
                  </ScrollArea>
                </div>
