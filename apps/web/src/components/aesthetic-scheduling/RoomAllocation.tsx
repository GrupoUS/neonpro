          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold">
                  {schedules ? Math.round((schedules.filter(s => s.appointments.length > 0).length / schedules.length) * 100) : 0}%
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className={`text-lg font-bold ${getEfficiencyColor(optimization?.efficiencyScore || 0)}`}>
                    ...
                    <div key={room.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{room.type} • Capacidade: {room.capacity}</div>
                        </div>
                      </div>
                      <Badge className={getRoomStatusColor(room.status)}>
                        {room.status === 'available' ? 'Disponível' :
                         ...
                         room.status === 'cleaning' ? 'Limpeza' :
                         room.status === 'reserved' ? 'Reservada' : room.status}
                      </Badge>
                    </div>
                  ))}
                </div>
                  ...
                  <div key={timeSlot.id} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-500">
                          {rooms?.filter(r => isTimeSlotAvailable(r.id, timeSlot)).length} salas disponíveis
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {rooms?.map((room) => {
                        ...
                        const appointment = schedules?.find(s => s.roomId === room.id)?.appointments.find(a => {
                        ...
                        <div className="mt-3">
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            ...
                            {room.equipment.length > 3 && (
                              <span className="text-xs text-gray-500">+{room.equipment.length - 3}</span>
                            )}
                          </div>
                        </div>
                      )}
