      ...
      >
        ...
        <div className="p-3">
          <p className="text-sm leading-relaxed whitespace-pre-wrap break-words">
            {isExpanded || !isLongContent ? displayContent : `${displayContent.substring(0, 300)}...`}
          </p>
...
export const NeonProAgentStatus: React.FC<AgentStatusProps> = ({
  ...
  const getStatusIcon = () => {
    switch (agent.status) {
      case 'thinking': return <Clock className="h-4 w-4 text-yellow-500" />;
      case 'responding': return <Stethoscope className="h-4 w-4 text-blue-500" />;
  ...
  const getStatusColor = () => {
    switch (agent.status) {
      case 'thinking': return 'bg-yellow-100 border-yellow-300 text-yellow-800';
      case 'responding': return 'bg-blue-100 border-blue-300 text-blue-800';
      ...
      <CardContent className="space-y-3">
        <Alert>
          ...
          <AlertDescription className="text-xs">
            Estes controles permitem gerenciar seus dados conforme a Lei Geral de Proteção de Dados (LGPD).
          </AlertDescription>
        </Alert>
