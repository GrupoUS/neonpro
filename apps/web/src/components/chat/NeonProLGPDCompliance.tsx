...
export const NeonProLGPDCompliance: React.FC<LGPDComplianceProps> = ({
  ...
  function calculateComplianceMetrics(): ComplianceMetrics {
    const totalConsents = mockDataSubjects.reduce((sum, subject) => 
      sum + subject.consentRecords.length, 0
    );
    
    const activeConsents = mockDataSubjects.reduce((sum, subject) => 
      sum + subject.consentRecords.filter(c => 
        !c.revokedAt && (!c.expiresAt || c.expiresAt > new Date())
      ).length, 0
    );

    const expiredConsents = mockDataSubjects.reduce((sum, subject) => 
      sum + subject.consentRecords.filter(c => c.expiresAt && c.expiresAt <= new Date()).length, 0
    );
  ...
  const handleGrantConsent = useCallback(async () => {
    ...
    try {
      ...
      logAuditEvent('consent_failed', 'data_consent', {
  ...
  const handleDataAccessRequest = useCallback(async (subjectId: string, purpose: string) => {
    try {
      ...
      logAuditEvent('data_access_failed', 'patient_data', {
        patientId: subjectId,
        error: error instanceof Error ? error.message : 'Unknown error'
      }, 'failure', 'high');
    }
  ...
  const handleDataExport = useCallback(async (subjectId: string) => {
    try {
      ...
      logAuditEvent('data_export_failed', 'patient_data', {
        patientId: subjectId,
        error: error instanceof Error ? error.message : 'Unknown error'
      }, 'failure', 'high');
    }
  ...
  const handleDataDeletion = useCallback(async (subjectId: string) => {
    try {
      ...
      logAuditEvent('data_deletion_failed', 'patient_data', {
        patientId: subjectId,
        error: error instanceof Error ? error.message : 'Unknown error'
      }, 'failure', 'critical');
    }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center">
              <Badge className={getRiskLevelColor(metrics.riskLevel)}>
                {metrics.riskLevel === 'low' ? 'Baixo Risco' :
                 metrics.riskLevel === 'medium' ? 'Médio Risco' : 'Alto Risco'}
              </Badge>
          ...
          <div className="space-y-4">
            {mockDataSubjects.map((subject) => (
              <div key={subject.id} className="border rounded-lg p-4">
                ...
                {/* Consent Records */}
                <div className="mt-3">
                  <h4 className="text-sm font-medium mb-2">Consentimentos ({subject.consentRecords.length})</h4>
                  {subject.consentRecords.map((consent) => (
                    <div key={consent.id} className="bg-gray-50 p-2 rounded text-xs">
                      <div className="flex justify-between">
                        ...
                        <Badge 
                          variant={!consent.revokedAt && (!consent.expiresAt || consent.expiresAt > new Date()) 
                            ? 'default' : 'secondary'
                          }
                          className="text-xs"
                        >
                          {!consent.revokedAt && (!consent.expiresAt || consent.expiresAt > new Date()) 
                            ? 'Ativo' : 'Expirado'}
                        </Badge>
                      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                {['personal_data', 'medical_data', 'financial_data', 'contact_data', 'treatment_data'].map((scope) => (
                  <label key={scope} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm">
                      {scope === 'personal_data' ? 'Dados Pessoais' :
                       scope === 'medical_data' ? 'Dados Médicos' :
                       scope === 'financial_data' ? 'Dados Financeiros' :
                       scope === 'contact_data' ? 'Dados de Contato' :
                       scope === 'treatment_data' ? 'Dados de Tratamento' : scope}
                    </span>
                  </label>
                ))}
              </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setConsentForm(prev => ({ ...prev, duration: parseInt(e.target.value) }))}
                min="1"
                max="3650"
              />
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => handleDataAccessRequest(selectedSubject.id, 'Access request for compliance')}
              >
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {auditLogs.length === 0 ? (
              <p className="text-gray-500 text-center py-4">Nenhum evento de auditoria registrado</p>
            ) : (
              auditLogs.slice(-20).reverse().map((log) => (
                <div key={log.id} className="flex items-center gap-3 p-3 border rounded">
                  <div className={`w-2 h-2 rounded-full ${
                    log.outcome === 'success' ? 'bg-green-500' :
                    log.outcome === 'failure' ? 'bg-red-500' : 'bg-yellow-500'
                  }`} />
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>LGPD Compliance:</strong> Este sistema está em conformidade com a Lei Geral de 
          Proteção de Dados (Lei nº 13.709/2018). Todas as operações com dados pessoais são 
          registradas, monitoradas e protegidas conforme as exigências legais.
        </AlertDescription>
      </Alert>
    </div>
  );
