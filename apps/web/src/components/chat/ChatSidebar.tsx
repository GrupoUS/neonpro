...
import type { 
  ChatConversation, 
  PresenceStatus, 
  ConversationType} from '@/types/chat';
import {
  SenderType 
} from '@/types/chat';
...
const PlusIcon = ({ className }: { className?: string }) => (
  <svg className={cn("w-4 h-4", className)} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <line strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} x1={12} y1={5} x2={12} y2={19} />
...
const FilterIcon = ({ className }: { className?: string }) => (
  <svg className={cn("w-4 h-4", className)} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <polygon strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} points="22,3 2,3 10,12.46 10,19 14,21 14,12.46 22,3" />
  </svg>
);
...
type FilterType = 'all' | 'ai_assistant' | 'patient_support' | 'emergency' | 'unread';
...
function ConversationItem({ 
  ...
  const onlineParticipants = conversation.participants.filter(p => 
    presenceStatus?.[p.user_id] === 'online' || p.user_type === 'ai_assistant'
  ).length;
    ...
    return (
      <button
        ...
        className={cn(
          ...
          emergencyMode && isActive && "bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400"
        )}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isEmergency && "animate-pulse",
        emergencyMode && isActive && "bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800"
      )}
    >
...
export default function ChatSidebar({
  ...
  const handleNewConversation = useCallback((type: ConversationType) => {
    onNewConversation?.(type);
  }, [onNewConversation]);

  const filterOptions = [
    { key: 'all', label: 'Todas', count: conversations.length },
    { key: 'ai_assistant', label: 'Assistente IA', count: conversations.filter(c => c.type === 'ai_assistant').length },
    { key: 'patient_support', label: 'Suporte', count: conversations.filter(c => c.type === 'patient_support').length },
    { key: 'emergency', label: 'Emergência', count: conversations.filter(c => c.type === 'emergency').length },
  ...
  return (
    <div className={cn(
        ...
        <div className="flex items-center justify-between">
          {!collapsed && (
            <h1 className={cn(
              ...
              emergencyMode ? "text-red-900 dark:text-red-100" : "text-gray-900 dark:text-gray-100"
            )}>
              Conversas
            </h1>
          )}
      ...
      {onNewConversation && (
        <div className={cn("p-4 border-t border-gray-200 dark:border-gray-700", collapsed && "p-2")}>
          {collapsed ? (
            <button
              onClick={() => handleNewConversation('ai_assistant')}
