...
const CheckCheckIcon = ({ className }: { className?: string }) => (
  <svg className={cn("w-3 h-3", className)} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="m21,4-3,3-1.5-1.5" />
  </svg>
);
...
export default function MessageBubble({
  ...
  }, [isSystemMessage, isEmergencyMessage, isAIMessage, isOwnMessage, emergencyMode]);
    ...
    >
      ...
      {showAvatar && (
        <div className={cn(
          ...
          isAIMessage && "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400",
          message.sender_type === 'doctor' && "bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400",
          message.sender_type === 'nurse' && "bg-teal-100 dark:bg-teal-900 text-teal-600 dark:text-teal-400",
          message.sender_type === 'staff' && "bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-400",
          message.sender_type === 'patient' && "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
        )}>
            ...
            {message.content.file_url && (
              <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-700 rounded border">
                <div className="flex items-center gap-2 text-sm">
                  <span>📎</span>
                  <span className="font-medium">{message.content.file_name}</span>
              ...
              <div className="mt-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-blue-700 dark:text-blue-300">
                    ...
                    <span>
                      IA • Confiança: {Math.round((message.content.ai_response.confidence_score || 0) * 100)}%
                    </span>
                  </div>
                  <button
                ...
                {message.content.ai_response.suggested_actions && message.content.ai_response.suggested_actions.length > 0 && (
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
                      Ações sugeridas:
                    </div>
                    {message.content.ai_response.suggested_actions.map((action, idx) => (
                      <button
                        key={idx}
                      ...
                      >
                        ...
                        {action.priority !== 'low' && (
                          <div className="text-blue-600 dark:text-blue-300">
                            Prioridade: {action.priority === 'urgent' ? 'Urgente' : action.priority === 'high' ? 'Alta' : 'Média'}
                          </div>
                        )}
                      </button>
                    ))}
                ...
                {expandedAI && (
                  <div className="text-xs space-y-2 pt-2 border-t border-blue-200 dark:border-blue-700">
                    ...
                    <div>
                      <strong>Validação Médica:</strong> {message.content.ai_response.medical_accuracy_validated ? '✅ Validado' : '⚠️ Não validado'}
                    </div>
                    {message.content.ai_response.escalation_recommendation?.should_escalate && (
                      <div className="p-2 bg-orange-100 dark:bg-orange-900 rounded border border-orange-300 dark:border-orange-700">
                        <strong>Recomendação:</strong> {message.content.ai_response.escalation_recommendation.reasoning}
                      </div>
                    )}
                  </div>
                )}
