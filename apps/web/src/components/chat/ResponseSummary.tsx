...
export function ResponseSummary({ summary, lastUpdated, onRefine }: ResponseSummaryProps) {
  const { t } = useI18n();
  return (
    <div>
      <p aria-label='summary-text'>{summary}</p>
      <p aria-label='freshness'>{timeSince(lastUpdated, t)}</p>
      <button onClick={() => onRefine?.()} aria-label={t('chat.refine')}>{t('chat.refine')}</button>
    </div>
  );
}
