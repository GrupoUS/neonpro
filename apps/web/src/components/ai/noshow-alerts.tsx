...
const updatePredictionStatus = async (data: {
  ...
  const response = await fetch(`/api/v2/appointments/predictions/${data.predictionId}`, {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('authToken')}`,
      'Content-Type': 'application/json',
...
const PredictionCard = ({
  ...
  const riskConfig = {
    low: { color: 'bg-green-100 text-green-800', label: 'Baixo', variant: 'default' as const },
    medium: {
        ...
        <div className='flex gap-2 pt-3 border-t'>
          {prediction.status === 'pending' && (
            <>
              <Button
                ...
                size='sm'
                onClick={() =>
                  onStatusUpdate(prediction.id, 'contacted', 'Paciente contatado por telefone')}
                className='flex-1'
              >
              ...
              <Button
                variant='outline'
                size='sm'
                onClick={() => onStatusUpdate(prediction.id, 'confirmed', 'Consulta confirmada')}
                className='flex-1'
              >
                Confirmar
              </Button>
          ...
          {prediction.status === 'contacted' && (
            <Button
              ...
              size='sm'
              onClick={() =>
                onStatusUpdate(prediction.id, 'confirmed', 'Consulta confirmada após contato')}
              className='flex-1'
            >
              Confirmar
            </Button>
...
export const NoShowAlerts = ({
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: preds.length,
      critical: preds.filter((p: NoShowPrediction) => p.riskLevel === 'critical').length,
      high: preds.filter((p: NoShowPrediction) => p.riskLevel === 'high').length,
      pending: preds.filter((p: NoShowPrediction) => p.status === 'pending').length,
      contacted: preds.filter((p: NoShowPrediction) => p.status === 'contacted').length,
    };
  }, [predictions]);
  ...
  return (
    <div className='space-y-6' data-testid={testId}>
      ...
      <div className='flex items-center justify-between'>
        <div>
          <h2 className='text-xl font-semibold tracking-tight'>Alertas de No-Show</h2>
        ...
        ? (
          <div className='space-y-4'>
            {predictions.data.predictions
              .sort((a: NoShowPrediction, b: NoShowPrediction) => b.riskScore - a.riskScore)
        ...
        : (
          <Card>
            <CardContent className='text-center py-8'>
              ...
              <p className='text-lg font-medium mb-2'>Nenhuma predição de no-show</p>
              <p className='text-muted-foreground'>
                {showHighRiskOnly
