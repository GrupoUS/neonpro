...
import { usePricingStats, usePricingRules } from '@/hooks/usePricingRules';
...
export function PricingRuleStats({ clinicId }: PricingRuleStatsProps) {
  ...
  const { data: rules = [], isLoading: rulesLoading } = usePricingRules(clinicId);
  ...
  const adjustmentTypeDistribution = rules.reduce((acc, rule) => {
    acc[rule.adjustment_type] = (acc[rule.adjustment_type] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {rules.length > 0 ? Math.round((activeRules.length / rules.length) * 100) : 0}% do total
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              R$ {stats?.total_savings ? stats.total_savings.toFixed(2) : '0,00'}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Quantidade de regras por categoria
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {[
              { label: 'Alta Prioridade (80-100)', min: 80, max: 100, color: 'bg-red-100 text-red-800' },
              { label: 'Média Prioridade (50-79)', min: 50, max: 79, color: 'bg-yellow-100 text-yellow-800' },
              { label: 'Baixa Prioridade (1-49)', min: 1, max: 49, color: 'bg-green-100 text-green-800' },
            ].map((range) => {
              ...
              return (
                <div key={range.label} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <Progress
                      value={rules.length > 0 ? (count / rules.length) * 100 : 0}
                      className="w-20"
                    />
                    <span className="text-sm font-medium">
                      {rules.length > 0 ? Math.round((count / rules.length) * 100) : 0}%
                    </span>
                  </div>
                </div>
              );
