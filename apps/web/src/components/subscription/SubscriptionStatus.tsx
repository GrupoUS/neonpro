...
export default function SubscriptionStatus({
  ...
  const getStatusColor = () => {
    if (hasPro)
      return "bg-gradient-to-r from-[#AC9469] to-[#294359] text-white";
    if (isOnTrial)
      return "bg-[#AC9469]/10 text-[#AC9469] border border-[#AC9469]/20";
  ...
  if (variant === "compact") {
    return (
      <div className={cn("flex items-center space-x-2", className)}>
        <Badge
          className={cn("px-3 py-1 text-xs font-medium", getStatusColor())}
        >
          {getStatusIcon()}
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="bg-white rounded-lg border border-[#D2D0C8] p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Badge
            className={cn("px-3 py-1 text-sm font-medium", getStatusColor())}
          >
            {getStatusIcon()}
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="font-medium text-[#112031]">
              {availableModels.filter((m) => m.available).length} de{" "}
      ...
      {showModels && (
        <div className="bg-white rounded-lg border border-[#D2D0C8] p-4">
          <h4 className="text-md font-semibold text-[#112031] mb-3">
            Modelos de IA
          </h4>
