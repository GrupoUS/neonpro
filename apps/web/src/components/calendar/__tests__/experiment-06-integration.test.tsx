  ...
  BigCalendar: vi.fn(({ events, onEventUpdate, onEventDelete }) => (
    <div data-testid="big-calendar">
      {events.map((event: any) => (
        <div key={event.id} data-event-id={event.id} className="calendar-event">
...
describe('Experiment06CalendarIntegration - Healthcare Compliance', () => {
  ...
  it('should audit all calendar modifications for compliance', async () => {
    const { logCalendarAction } = await import('@/utils/accessibility/healthcare-audit-utils');
    ...
    const updateButton = screen.getByRole('button', { name: 'Update Consulta Dr. Silva' });
  ...
  it('should validate deletion permissions for healthcare events', async () => {
    const { auditAppointmentAccess } = await import('@/utils/accessibility/healthcare-audit-utils');
    ...
    const deleteButton = screen.getByRole('button', { name: 'Delete Exame Cardiológico' });
  ...
  it('should maintain consistent healthcare color coding', () => {
    ...
    const firstEvent = screen.getByText('Consulta Dr. Silva').closest('.calendar-event');
    expect(firstEvent).toHaveStyle({ color: expect.stringContaining('#3b82f6') });

    const secondEvent = screen.getByText('Exame Cardiológico').closest('.calendar-event');
    expect(secondEvent).toHaveStyle({ color: expect.stringContaining('#f59e0b') });
  });
  ...
  it('should handle real-time updates from Supabase', async () => {
  ...
  it('should provide accessible new consultation button', () => {
    ...
    const newConsultationBtn = screen.getByRole('button', { name: /nova consulta/i });
