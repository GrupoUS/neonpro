...
export function Experiment06CalendarIntegration({
  ...
  const [minimizedAppointments, setMinimizedAppointments] = useState<MinimizedCalendarAppointment[]>([]);
  ...
  useEffect(() => {
    const processAppointmentsWithCompliance = async () => {
      if (!appointments.length) {
        ...
        setComplianceStatus(prev => ({ ...prev, isCompliant: true, hasIssues: false }));
        return;
      }
      ...
      try {
        ...
        setComplianceStatus({
          ...
          minimizationLevel: result.compliantAppointments[0]?.consentLevel || DataMinimizationLevel.MINIMAL,
          hasIssues,
          blockedAppointments,
        });
        ...
        if (result.auditLogId) {
          console.log('LGPD: Calendar appointments processed with audit ID:', result.auditLogId);
        }
        ...
        console.error('LGPD: Error processing appointments with compliance:', err);
  ...
  const calendarEvents = useMemo(() => {
    try {
      ...
      console.error('Error mapping minimized appointments to calendar events:', err);
      return [];
    }
  }, [minimizedAppointments]);

  // Handle event updates with LGPD compliance
  const handleEventUpdate = async (event: CalendarEvent, updates?: Partial<CalendarEvent>) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (!consentResult.isValid) {
        setError(`LGPD: ${consentResult.error || 'Consentimento não válido para atualização'}`);
        return;
      }
  ...
  const handleEventDelete = async (eventId: string) => {
    ...
    try {
      // Find the original appointment
      const originalAppointment = appointments.find(apt => apt.id === eventId);
      if (!originalAppointment) {
        throw new Error('Appointment not found');
      }
      ...
      if (!consentResult.isValid) {
        setError(`LGPD: ${consentResult.error || 'Consentimento não válido para exclusão'}`);
        return;
      }
        ...
        {complianceStatus.hasIssues && (
          <div className="mt-3 p-3 bg-red-100 rounded border border-red-200">
            <p className="text-red-800 text-sm font-medium">
              {complianceStatus.blockedAppointments.length} agendamento(s) bloqueado(s) por conformidade LGPD
            </p>
            <p className="text-red-700 text-xs mt-1">
              Verifique os consentimentos dos pacientes ou contate a equipe de compliance.
            </p>
          </div>
        )}
      </div>
  ...
  const ComplianceIndicator = () => (
    <div className={`p-2 rounded-md text-xs font-medium ${
      ...
      {complianceStatus.isCompliant ? (
        ...
        <span>⚠ {complianceStatus.blockedAppointments.length} bloqueado(s) por LGPD</span>
      )}
    </div>
  );
    ...
    <div className={className}>
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
        <p className="text-blue-800 text-sm">
          <strong>LGPD:</strong> Este calendário opera em conformidade com a Lei Geral de Proteção de Dados. 
          Dados dos pacientes são minimizados e processados apenas com consentimento válido.
        </p>
        {complianceStatus.hasIssues && (
          <p className="text-blue-700 text-xs mt-1">
            Alguns agendamentos podem estar restritos por falta de consentimento.
          </p>
        )}
      </div>
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">Processando com conformidade LGPD...</p>
          </div>
        </div>
      )}
      ...
      {complianceStatus.hasIssues && (
        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
          ...
          <div className="space-y-1">
            {complianceStatus.consentValidations.slice(0, 3).map((issue, index) => (
            ...
            {complianceStatus.consentValidations.length > 3 && (
              <div className="text-yellow-600 text-xs">
                + {complianceStatus.consentValidations.length - 3} problemas adicionais
              </div>
            )}
          </div>
        </div>
