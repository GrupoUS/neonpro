...
const maskCpf = (cpf: string, level: 'minimal' | 'standard' | 'full'): string => {
  if (level === 'minimal') return '***.***.***-**';
  if (level === 'standard') return cpf.replace(/(\d{3})\d{3}(\d{3})/, '$1.***.***-$3');
...
const RiskIndicator = ({ level }: { level: 'low' | 'medium' | 'high' }) => {
  ...
  return (
    <div className={cn('flex items-center gap-1 px-2 py-1 rounded-full border text-xs', color)}>
...
const ConsentStatus = ({ consentInfo }: { consentInfo: PatientConsentInfo }) => {
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-blue-600">LGPD Compliance</span>
      </div>
      <div className="text-xs text-muted-foreground">
        {activeConsents.length > 0 ? (
          <>
            ...
            <br />
            Atualizado: {format(consentInfo.lastUpdate, 'dd/MM/yyyy', { locale: ptBR })}
          </>
        ) : (
...
export function MobilePatientCard({
  ...
  const canViewSensitiveData = dataVisibilityLevel === 'full' && 
    (userRole === 'admin' || consentInfo.dataProcessing);

  const displayCpf = maskCpf(patient.cpf, dataVisibilityLevel);
  const displayPhone = canViewSensitiveData ? formatBRPhone(patient.phone) : '(11) *****-****';
        ...
        <div className="flex items-start gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuItem onClick={() => navigate({ to: `/patients/${patient.id}` })}>
          ...
          <div className="flex justify-between items-center mt-3">
            ...
            {healthInfo.nextAppointment && (
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                <span>
                  Próxima: {format(healthInfo.nextAppointment, 'dd/MM', { locale: ptBR })}
                </span>
              </div>
            )}
          </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  {healthInfo.bloodType && (
                    <p>Tipo sanguíneo: <span className="font-medium">{healthInfo.bloodType}</span></p>
                  )}
                  ...
                  {healthInfo.lastVisit && (
                    <p>
                      Última consulta: {format(healthInfo.lastVisit, 'dd/MM/yyyy', { locale: ptBR })}
                    </p>
                  )}
                </div>
              </div>
