...
const maskCpf = (cpf: string, visibilityLevel: 'minimal' | 'standard' | 'full'): string => {
  if (visibilityLevel === 'minimal') return '***.***.***-**';
  if (visibilityLevel === 'standard') return cpf.replace(/(\d{3})\d{3}(\d{3})(\d{2})/, '$1.***.***-$3');
  return cpf;
};
...
const StatusBadge = ({ status }: { status: MobilePatientData['status'] }) => {
  const statusConfig = {
    ...
    pending: { label: 'Pendente', variant: 'outline' as const, className: 'bg-yellow-100 text-yellow-800' },
  };
...
const ConsentIndicator = ({ consentStatus }: { consentStatus: MobilePatientData['consentStatus'] }) => {
  const consentConfig = {
    granted: { icon: Shield, color: 'text-green-600', label: 'Consentimento concedido' },
    pending: { icon: Info, color: 'text-yellow-600', label: 'Consentimento pendente' },
    withdrawn: { icon: EyeOff, color: 'text-red-600', label: 'Consentimento retirado' },
  };
...
const MobilePatientCard = ({ 
  ...
  userRole: string;
  onVisibilityToggle: (id: string, newLevel: MobilePatientData['dataVisibilityLevel']) => void;
}) => {
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem 
                onClick={() => navigate({ to: `/appointments/new`, search: { patientId: patient.id } })}
              >
...
export function MobilePatientList({
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | MobilePatientData['status']>('all');
  const [consentFilter, setConsentFilter] = useState<'all' | MobilePatientData['consentStatus']>('all');
  const [visibilityLevels, setVisibilityLevels] = useState<Record<string, MobilePatientData['dataVisibilityLevel']>>({});
  ...
  const handleVisibilityToggle = (patientId: string, newLevel: MobilePatientData['dataVisibilityLevel']) => {
          ...
          <Sheet>
            ...
            <SheetContent side="bottom" className="max-h-[80vh]">
              ...
              <div className="space-y-4 mt-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">Status do Paciente</label>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block">Status LGPD</label>
                  <Select value={consentFilter} onValueChange={(value: any) => setConsentFilter(value)}>
                    <SelectTrigger className="h-12">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">Todos os consentimentos</SelectItem>
                      <SelectItem value="granted">Consentimento concedido</SelectItem>
                      <SelectItem value="pending">Consentimento pendente</SelectItem>
                      <SelectItem value="withdrawn">Consentimento retirado</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
      ...
      <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-blue-800">
            ...
            <p className="mt-1">
              Os dados pessoais são exibidos com base no consentimento do paciente e nível de acesso do usuário.
              Dados sensíveis são mascarados por padrão para garantir a privacidade.
            </p>
          </div>
        </div>
      </div>
