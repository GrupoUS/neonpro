...
const MobilePatientCard = ({
  ...
  const canViewFullData =
    userRole === "admin" || patient.consentStatus === "granted";
...
export function MobilePatientList({
  ...
  const filteredPatients = useMemo(() => {
    return patients.filter((patient) => {
      ...
      const matchesConsent =
        consentFilter === "all" || patient.consentStatus === consentFilter;
  ...
  const getPatientWithVisibility = (patient: MobilePatientData) => ({
    ...patient,
    dataVisibilityLevel:
      visibilityLevels[patient.id] || patient.dataVisibilityLevel,
  });
      ...
      <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-blue-800">
            ...
            <p className="mt-1">
              Os dados pessoais são exibidos com base no consentimento do
              paciente e nível de acesso do usuário. Dados sensíveis são
              mascarados por padrão para garantir a privacidade.
            </p>
          </div>
        </div>
      </div>
