...
const multiColumnFilterFn: FilterFn<PatientTableData> = (row, _columnId, filterValue: string) => {
  const searchableRowContent = `${row.original.fullName} ${row.original.email || ''} ${row.original.phone || ''}`.toLowerCase();
  ...
  const columns: ColumnDef<PatientTableData>[] = useMemo(() => [
    {
      ...
      header: ({ table }) => (
        <Checkbox
          ...
          onCheckedChange={(value: boolean | "indeterminate") => table.toggleAllPageRowsSelected(Boolean(value))}
              ...
              {table
                ...
                .map((column) => {
                  return (
                    ...
                    >
                      {column.id === "contact" ? "Contato" : 
                       column.id === "lastVisit" ? "Última Visita" : 
                       column.id === "totalAppointments" ? "Consultas" : column.id}
                    </DropdownMenuCheckboxItem>
                  );
                })}
            </DropdownMenuContent>
              ...
              <TableRow>
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    {(searchFilter || selectedStatuses.length > 0) && (
                      <Button
                        variant="ghost"
                        onClick={() => {
