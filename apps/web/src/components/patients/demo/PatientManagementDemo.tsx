...
export function PatientManagementDemo() {
  ...
  const handlePatientSelect = (_patientId: [a-zA-Z][a-zA-Z]*) => {
  ...
  const handleConsentUpdate = async (_consents: [a-zA-Z][a-zA-Z]*) => {
  ...
  const handleDataExport = async (_patientId: [a-zA-Z][a-zA-Z]*) => {
    console.log('Data export requested for patient:', patientId);
    // Simulate data export
    await new Promise(resolve => setTimeout(resolve, 2000));
    alert(
      'Exportação de dados solicitada. Você receberá um e-mail em até 72 horas.',
    );
  };

  const handleDataErasure = async (_patientId: [a-zA-Z][a-zA-Z]*) => {
    console.log('Data erasure requested for patient:', patientId);
    // Simulate data erasure request
    await new Promise(resolve => setTimeout(resolve, 1000));
    alert(
      'Solicitação de exclusão de dados enviada. Processamento em até 15 dias úteis.',
    );
  };

  const handleSearchSelect = (_patient: [a-zA-Z][a-zA-Z]*) => {
          ...
          <div className='max-w-6xl mx-auto space-y-6'>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Demonstração dos componentes de interface para gestão de pacientes com compliance
                  LGPD
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                        <div>
                          ...
                          <div className='space-y-2 text-sm'>
                            <div>
                              123.456.789-00: {validateCpf('123.456.789-00')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                            <div>
                              111.111.111-11: {validateCpf('111.111.111-11')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                            <div>
                              041.767.406-84: {validateCpf('041.767.406-84')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <div className='space-y-2 text-sm'>
                            <div>
                              (11) 99999-9999: {validateBrazilianPhone('(11) 99999-9999')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                            <div>
                              (11) 8888-8888: {validateBrazilianPhone('(11) 8888-8888')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                            <div>
                              (11) 1234-5678: {validateBrazilianPhone('(11) 1234-5678')
                                ? '✅ Válido'
                                : '❌ Inválido'}
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <div className='space-y-2 text-sm'>
                            ...
                            <div>
                              Telefone: {formatters.phone('11999999999')}
                            </div>
