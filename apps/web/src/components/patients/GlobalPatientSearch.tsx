...
export function GlobalPatientSearch({
  ...
  const { measureSearch, searchResponseTime, searchStatus, isSearchHealthy } = useSearchPerformance();
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const mockResults = [
          ...
          patient.fullName.toLowerCase().includes(debouncedQuery.toLowerCase()) ||
        ...
        setAnalytics(prev => ({
          ...
          averageResults: prev 
            ? ((prev.averageResults * prev.totalSearches) + mockResults.length) / (prev.totalSearches + 1)
            : mockResults.length,
          averageTime: prev
        ...
        return mockResults.map(patient => ({
          ...
          highlightedFields: {
            ...
            phone: patient.phone ? highlightMatch(patient.phone, debouncedQuery) : undefined,
            email: patient.email ? highlightMatch(patient.email, debouncedQuery) : undefined,
          },
        }));
      } catch (error) {
  ...
  useKeyboardShortcuts({
    ...
    'Enter': (e) => {
      if (isOpen && selectedIndex >= 0 && searchResults && searchResults[selectedIndex]) {
        e.preventDefault();
  ...
  function useKeyboardShortcuts(shortcuts: Record<string, (e: KeyboardEvent) => void>, deps: any[] = []) {
    useEffect(() => {
  ...
  function updateTopQueries(current: Array<{ query: string; count: number }>, newQuery: string) {
  ...
  // Search result item
  const SearchResultItem = ({ patient, index }: { patient: PatientSearchResult; index: number }) => (
    <div
      className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <p 
            ...
            dangerouslySetInnerHTML={{ __html: patient.highlightedFields.name || patient.fullName }}
          />
          <Badge 
        ...
        {patient.matchReasons.length > 0 && (
          <div className="flex items-center space-x-1 mt-2">
            ...
            {patient.matchReasons.length > 3 && (
              <span className="text-xs text-gray-500">+{patient.matchReasons.length - 3}</span>
            )}
          </div>
        )}
      </div>
        ...
        >
          ...
          <ScrollArea className="max-h-[400px]">
            {isLoading && debouncedQuery.length >= 2 ? (
              ...
              <div className="p-4 text-center">
                ...
                <p className="text-sm text-gray-500">Erro na busca. Tente novamente.</p>
              </div>
            ) : query && query.length >= 2 ? (
              filteredResults.length > 0 ? (
                <div className="divide-y">
                  {filteredResults.map((patient, index) => (
                    <SearchResultItem key={patient.id} patient={patient} index={index} />
                  ))}
                </div>
              ) : (
                <div className="p-8 text-center">
                  <Search className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                  <h3 className="text-sm font-medium mb-2">Nenhum paciente encontrado</h3>
              ...
              <div className="divide-y">
                <div className="p-3">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-xs font-medium text-gray-500">Buscas recentes</h4>
                    <Button
                      variant="ghost"
                      size="sm"
              ...
              <div className="p-8 text-center">
                <Search className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                <h3 className="text-sm font-medium mb-2">Busca inteligente de pacientes</h3>
          ...
          <div className="border-t p-2 bg-gray-50 dark:bg-gray-800">
            <div className="flex items-center justify-between text-xs text-gray-500">
              <span>Pressione <kbd className="px-1 py-0.5 bg-white dark:bg-gray-700 rounded">/</kbd> para focar</span>
              <span><kbd className="px-1 py-0.5 bg-white dark:bg-gray-700 rounded">↑↓</kbd> navegar</span>
              <span><kbd className="px-1 py-0.5 bg-white dark:bg-gray-700 rounded">Enter</kbd> selecionar</span>
              <span><kbd className="px-1 py-0.5 bg-white dark:bg-gray-700 rounded">Esc</kbd> fechar</span>
            </div>
          </div>
        </PopoverContent>
      </Popover>
