...
export function AppointmentBooking({
  ...
  const generateTimeSlots = useCallback((): TimeSlot[] => {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      ...
      slots.push({
        ...
        riskLevel: existingAppointment?.noShowRisk?.level as 'low' | 'medium' | 'high'
      });
    }
  ...
  const handleBookAppointment = async (formData: AppointmentFormData) => {
    try {
      const appointmentData = {
        ...formData,
        startTime: new Date(`${format(formData.date, 'yyyy-MM-dd')}T${formData.time}`),
        endTime: new Date(`${format(formData.date, 'yyyy-MM-dd')}T${formData.time}`),
              ...
              {dayAppointments.slice(0, 3).map((apt, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    apt.noShowRisk?.level === 'high' ? 'bg-red-500' :
                    apt.noShowRisk?.level === 'medium' ? 'bg-yellow-500' :
                    'bg-green-500'
                  )}
                />
              ))}
  ...
  return (
    <div className={cn("space-y-6", className)}>
              ...
              {timeSlots.map((slot) => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedTime === slot.time && "bg-primary/10 border-primary"
                  )}
                ...
                >
                  ...
                  {!slot.available && slot.patientName && (
                    <div className="flex items-center gap-2">
                      {slot.riskLevel && (
                        <Badge variant={getRiskBadgeVariant(slot.riskLevel)} className="text-xs">
      ...
      {/* Real-time status */}
      {(appointmentsLoading || availabilityLoading || createAppointment.isPending) && (
        <Alert>
          <Clock className="h-4 w-4" />
          <AlertDescription>
