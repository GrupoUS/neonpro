...
import { format, parseISO, addMinutes, differenceInMinutes, isToday, isTomorrow, isPast } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import type { Appointment, NoShowRisk, TimeSlot, RiskLevel } from '../types';

/**
 * Format appointment time for Brazilian healthcare context
 */
export function formatAppointmentTime(date: Date | string, includeDate = false): string {
  ...
  for (let hour = startHour; hour < endHour; hour++) {
    for (let minute = 0; minute < 60; minute += slotDuration) {
      ...
      const existingAppointment = existingAppointments.find(apt => 
        formatAppointmentTime(apt.startTime) === time
      );
 ...
 */
export function getReminderTimingOptions(): Array<{ value: number; label: string }> {
  return [
 ...
 */
export function validateAppointmentTime(date: Date, time: string): {
  isValid: boolean;
  error?: string;
} {
...
export function getBrazilianHealthcareConsiderations(date: Date): {
  ...
  return {
    isHoliday: !!holiday,
    isCarnival,
    note: holiday ? `Feriado: ${holiday.name}` : 
          isCarnival ? 'Período de Carnaval - maior risco de faltas' : undefined
  };
}
