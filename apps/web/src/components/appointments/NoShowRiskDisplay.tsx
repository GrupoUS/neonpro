...
export function NoShowRiskDisplay({
  ...
  const { mutate: sendReminder, isPending: sendingReminder } =
    useSendAppointmentReminder();
  const { mutate: getInsights, isPending: gettingInsights } =
    useAIHealthcareInsights();
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                  ...
                  <AlertDescription>
                    <div className="space-y-1">
                      {riskData.weatherImpact && (
                        <div>
                          Impacto climático:{" "}
                          {Math.round(riskData.weatherImpact * 100)}%
                        </div>
                      )}
            ...
            <TabsContent value="insights" className="space-y-4">
              <div className="text-center py-8">
                ...
                <div className="text-sm text-gray-600">
                  Análise personalizada baseada no histórico do paciente e
                  padrões comportamentais brasileiros
                </div>
              </div>
            </TabsContent>
          </Tabs>
