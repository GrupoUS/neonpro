...
export interface RiskFactor {
  ...
  confidence: number;
  description: string;
  category: 'behavioral' | 'environmental' | 'medical' | 'logistical' | 'cultural';
}
...
export interface BrazilianCulturalFactor {
  factor: 'carnival_season' | 'football_match' | 'holiday' | 'rain_season' | 'traffic_strike';
  impact: number;
  description: string;
}
...
export function NoShowRiskDisplay({
  ...
  const handleSendIntervention = (recommendation: InterventionRecommendation) => {
    sendReminder({
      appointmentId,
    ...
    toast.success(`Intervenção ${recommendation.type} enviada via ${recommendation.channel}`);
  };

  const formatTimeUntilAppointment = () => {
    const now = new Date();
    const appointment = new Date(scheduledFor);
    const diffHours = Math.floor((appointment.getTime() - now.getTime()) / (1000 * 60 * 60));
      ...
      <Card className={cn("", className)}>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            >
              <RefreshCw className={cn("h-4 w-4", riskLoading && "animate-spin")} />
            </Button>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          {/* Risk Score Display */}
          <div className={cn("p-4 rounded-lg border", getRiskColor(riskData.riskLevel))}>
            ...
            <Progress 
              value={riskData.riskScore * 100} 
              className="h-2 mb-2"
            />
            
            <div className="flex justify-between text-xs text-gray-600">
              <span>Confiança: {Math.round(riskData.confidence * 100)}%</span>
              <span>Precisão histórica: {Math.round(riskData.historicalAccuracy * 100)}%</span>
            </div>
          </div>
          ...
          {riskData.riskFactors && riskData.riskFactors.length > 0 && (
            <div>
              <h4 className="text-sm font-medium mb-2">Principais Fatores de Risco:</h4>
              <div className="space-y-1">
                {riskData.riskFactors.slice(0, 3).map((factor, index) => (
                  <div key={index} className="flex items-center justify-between text-xs">
                    <span className="text-gray-600">{factor.description}</span>
                    <Badge 
                      variant={factor.impact > 0.5 ? "destructive" : factor.impact > 0.2 ? "warning" : "secondary"}
                      className="text-xs"
                    >
          ...
          {/* Quick Interventions */}
          {showInterventions && riskData.recommendations && riskData.recommendations.length > 0 && (
            <div>
              <h4 className="text-sm font-medium mb-2">Intervenções Recomendadas:</h4>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">Probabilidade de Falta</div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">Confiança do Modelo</div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">Precisão Histórica</div>
                    </div>
                  </CardContent>
                </Card>
              </div>
                  ...
                  <AlertDescription>
                    <div className="space-y-1">
                      {riskData.weatherImpact && (
                        <div>Impacto climático: {Math.round(riskData.weatherImpact * 100)}%</div>
                      )}
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className={cn(
                          "font-semibold",
                          factor.impact > 0.3 ? "text-red-600" :
                          factor.impact > 0.1 ? "text-yellow-600" : "text-green-600"
                        )}>
                          {factor.impact > 0 ? '+' : ''}{Math.round(factor.impact * 100)}%
                        </div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <Badge variant={
                            ...
                            rec.priority === 'média' ? 'warning' : 'secondary'
                          }>
                            {rec.priority.toUpperCase()}
                          </Badge>
                          <span className="font-medium capitalize">{rec.type}</span>
                          <span className="text-sm text-gray-600">via {rec.channel}</span>
                        </div>
                        <div className="text-sm text-gray-700 mb-2">{rec.message}</div>
                        <div className="text-xs text-gray-500">
                          Timing: {rec.timing} • Impacto esperado: {Math.round(rec.expectedImpact * 100)}%
                          {rec.cost > 0 && ` • Custo: R$ ${rec.cost.toFixed(2)}`}
                        </div>
                      </div>
                      <Button
                        onClick={() => handleSendIntervention(rec)}
            ...
            <TabsContent value="insights" className="space-y-4">
              <div className="text-center py-8">
                ...
                >
                  ...
                  {gettingInsights ? 'Gerando Insights...' : 'Gerar Insights com IA'}
                </Button>
                <div className="text-sm text-gray-600">
                  Análise personalizada baseada no histórico do paciente e padrões comportamentais brasileiros
                </div>
              </div>
            </TabsContent>
          </Tabs>
