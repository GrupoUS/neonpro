...
export function ProcedureRecommendations({
  ...
  const criticalContraindications = contraindications.filter(c => c.type === 'critical');
  const warningContraindications = contraindications.filter(c => c.type === 'warning');
  ...
  return (
    <div className='max-w-6xl mx-auto space-y-6'>
      ...
      <Card>
        ...
        <CardContent>
          <div className='flex flex-wrap gap-2'>
            ...
            <Badge variant='outline'>
              Fototipo: {patientProfile.skinType}
            </Badge>
      ...
      {(criticalContraindications.length > 0 || warningContraindications.length > 0) && (
        <div className='space-y-3'>
          {criticalContraindications.map((alert, index) => (
            <Alert key={index} variant='destructive'>
              <XCircle className='h-4 w-4' />
              <AlertTitle>Contraindicação Crítica: {alert.condition}</AlertTitle>
              <AlertDescription>
                <p className='mb-2'>{alert.description}</p>
                <ul className='list-disc list-inside space-y-1'>
                  {alert.restrictions.map((restriction, i) => (
                    <li key={i} className='text-sm'>{restriction}</li>
                  ))}
                </ul>
                {alert.alternatives && (
                  <div className='mt-2'>
                    <p className='font-medium text-sm'>Alternativas recomendadas:</p>
      ...
      <Tabs defaultValue='all' className='w-full'>
        ...
        {['facial', 'corporal', 'injetável', 'laser', 'peeling'].map(category => (
        ...
        >
          <CardHeader className='pb-2'>
            <div className='flex items-start justify-between'>
              ...
              <Badge className={`text-xs ${getSuitabilityColor(procedure.suitabilityScore)}`}>
          ...
          <CardContent className='space-y-3'>
            ...
            <div className='space-y-2'>
              <div className='flex justify-between text-sm'>
                ...
                <span className='font-medium'>{procedure.estimatedSessions}x</span>
              </div>
              <div className='flex justify-between text-sm'>
