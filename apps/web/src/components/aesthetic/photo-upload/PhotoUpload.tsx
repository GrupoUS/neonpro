...
export function PhotoUpload({
    ...
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        if (width > maxResolution || height > maxResolution) {
          ...
          resolve(`Resolução muito baixa. Mínimo: 512x512px para análise precisa`);
        } else {
          resolve(null);
        }
      };
  ...
  const analyzePhoto = async (photo: AestheticPhoto): Promise<AestheticAnalysis> => {
  ...
  const generateTreatmentSuggestions = (analysis: AestheticAnalysis): TreatmentSuggestion[] => {
      ...
      try {
        ...
        setUploadStates(prev => {
          ...
          newMap.set(photoId, {
            ...
            progress: 0,
            status: 'error',
            error: error instanceof Error ? error.message : 'Erro no upload/análise',
          });
          return newMap;
        });
  ...
  }, [disabled, validatePhoto, uploadPhoto, analyzePhoto, onPhotosUploaded, onAnalysisComplete]);
                ...
                <div className='p-3 space-y-2'>
                  ...
                  {photo.analysis && (
                    <div className='space-y-1'>
                      ...
                      <div className='flex items-center justify-between text-xs'>
                        <span>Gravidade:</span>
                        <span className='font-medium'>{photo.analysis.severity.overall}/10</span>
                      </div>
                    </div>
                  )}
