...
import { TreatmentSuggestions, TreatmentSuggestion } from '../TreatmentSuggestions';
...
describe('TreatmentSuggestions Component', () => {
  ...
  describe('Renderização', () => {
    it('deve renderizar o componente sem sugestões', () => {
      ...
      expect(screen.getByText('Nenhuma sugestão de tratamento disponível')).toBeInTheDocument();
    });
  ...
  describe('Detalhes do Tratamento', () => {
    it('deve exibir informações básicas do tratamento', () => {
      ...
      expect(screen.getByText('Toxina Botulínica')).toBeInTheDocument();
      expect(screen.getByText('Redução de rugas dinâmicas')).toBeInTheDocument();
    ...
    it('deve exibir considerações', () => {
      ...
      expect(screen.getByText('Evitar exercícios físicos por 24h')).toBeInTheDocument();
    });
  });
  ...
  describe('Interação com o Usuário', () => {
    it('deve permitir selecionar um tratamento', async () => {
      ...
      render(<TreatmentSuggestions suggestions={mockSuggestions} onTreatmentSelect={onTreatmentSelect} />);
    ...
    it('deve permitir expandir detalhes do tratamento', async () => {
      ...
      expect(screen.getByText('Toxina Botulínica, Ácido Hialurônico')).toBeInTheDocument();
    });
    ...
    it('deve permitir comparar tratamentos', async () => {
      ...
      const compareButton1 = screen.getAllByRole('button', { name: /comparar/i })[0];
      await userEvent.click(compareButton1);
      
      // Selecionar segundo tratamento para comparação
      const compareButton2 = screen.getAllByRole('button', { name: /comparar/i })[1];
      await userEvent.click(compareButton2);
      
      // Verificar se modo de comparação foi ativado
      expect(screen.getByText('Modo Comparação (2 selecionados)')).toBeInTheDocument();
    });

    it('deve permitir agendar consulta', async () => {
      const onScheduleConsultation = jest.fn();
      render(<TreatmentSuggestions suggestions={[mockSuggestions[0]]} onScheduleConsultation={onScheduleConsultation} />);
  ...
  describe('Acessibilidade', () => {
    it('deve não ter violações de acessibilidade', async () => {
      const { container } = render(<TreatmentSuggestions suggestions={mockSuggestions} />);
    ...
    it('deve ter atributos ARIA corretos', () => {
      ...
      expect(suggestionsList).toHaveAttribute('aria-label', 'Sugestões de tratamento');
    ...
    it('deve suportar navegação por teclado', async () => {
      ...
      const focusedTreatment = screen.getByTestId('treatment-suggestion-tx-001');
    ...
    it('deve ter descrições adequadas para leitores de tela', () => {
      ...
      const treatmentCard = screen.getByTestId('treatment-suggestion-tx-001');
      expect(treatmentCard).toHaveAttribute('aria-describedby', expect.any(String));
  ...
  describe('Considerações de Saúde', () => {
    it('deve exibir avisos de contraindicação', () => {
      ...
      expect(screen.getByText('Gravidez - Risco teórico para o feto')).toBeInTheDocument();
    });
    ...
    it('deve exibir avisos de precaução', () => {
      ...
      expect(screen.getByText('Alergias - Teste de sensibilidade necessário')).toBeInTheDocument();
    });

    it('deve permitir ver detalhes de saúde', async () => {
      render(<TreatmentSuggestions suggestions={[mockSuggestions[0]]} />);
      
      const healthDetailsButton = screen.getByRole('button', { name: /detalhes de saúde/i });
  ...
  describe('Edge Cases', () => {
    ...
    it('deve lidar com sugestões duplicadas', () => {
      ...
      const suggestionElements = screen.getAllByTestId(/treatment-suggestion-tx-001/);
