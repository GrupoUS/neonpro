...
import { LGPDConsentManager, LGPDConsentData } from '../LGPDConsentManager';
...
describe('LGPDConsentManager Component', () => {
  ...
  describe('Renderização Inicial', () => {
    it('deve renderizar o componente vazio', () => {
      ...
      expect(screen.getByText('Gerenciamento de Consentimento LGPD')).toBeInTheDocument();
      expect(screen.getByText(/Nenhum consentimento registrado/i)).toBeInTheDocument();
    });
    ...
    it('deve mostrar estado de loading', () => {
      ...
      expect(screen.getByText('Carregando dados de consentimento...')).toBeInTheDocument();
    });
    ...
    it('deve mostrar consentimento expirado', () => {
      ...
      expect(screen.getByText(/Seu consentimento expirou e precisa ser renovado/i)).toBeInTheDocument();
    });
    ...
    it('deve mostrar consentimento revogado', () => {
      ...
      expect(screen.getByText(/Você revogou seu consentimento/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Formulário de Consentimento', () => {
    it('deve mostrar formulário para novo consentimento', async () => {
      ...
      const newConsentButton = screen.getByRole('button', { name: /novo consentimento/i });
    ...
    it('deve permitir selecionar finalidade', async () => {
      ...
      await userEvent.click(screen.getByRole('button', { name: /novo consentimento/i }));
    ...
    it('deve permitir definir período de retenção', async () => {
      ...
      const retentionInput = screen.getByLabelText(/período de retenção \(dias\)/i);
  ...
  describe('Consentimento Granular', () => {
    ...
    it('deve mostrar explicações para cada tipo de consentimento', async () => {
      ...
      const infoButtons = screen.getAllByRole('button', { name: /informação/i });
      
      await userEvent.click(infoButtons[0]); // Info de armazenamento
      
      expect(screen.getByText(/Seus dados serão armazenados de forma segura/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Submissão do Formulário', () => {
    it('deve validar campos obrigatórios', async () => {
      ...
      const submitButton = screen.getByRole('button', { name: /salvar consentimento/i });
      await userEvent.click(submitButton);
      
      expect(screen.getByText(/finalidade é obrigatória/i)).toBeInTheDocument();
      expect(screen.getByText(/selecione pelo menos uma categoria de dados/i)).toBeInTheDocument();
    ...
    it('deve chamar onSaveConsent com dados corretos', async () => {
      ...
      await userEvent.selectOptions(screen.getByLabelText(/finalidade/i), 'photo_analysis');
      await userEvent.click(screen.getByLabelText(/fotos/i));
      await userEvent.click(screen.getByLabelText(/dados biométricos/i));
      await userEvent.type(screen.getByLabelText(/período de retenção \(dias\)/i), '365');
      await userEvent.selectOptions(screen.getByLabelText(/base legal/i), 'consent');
    ...
    it('deve mostrar confirmação antes de salvar', async () => {
      ...
      expect(screen.getByText(/tem certeza que deseja conceder consentimento/i)).toBeInTheDocument();
    });
    ...
    it('deve mostrar sucesso após salvar', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Consentimento salvo com sucesso!')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Gerenciamento de Consentimento Existente', () => {
    it('deve permitir visualizar detalhes do consentimento', async () => {
      ...
      expect(screen.getByText(mockConsentData.dataCategories.join(', '))).toBeInTheDocument();
    });

    it('deve permitir editar consentimento', async () => {
      render(<LGPDConsentManager consentData={mockConsentData} onUpdateConsent={mockUpdateConsent} />);
    ...
    it('deve permitir revogar consentimento', async () => {
      render(<LGPDConsentManager consentData={mockConsentData} onWithdrawConsent={mockWithdrawConsent} />);
      
      const withdrawButton = screen.getByRole('button', { name: /revogar consentimento/i });
      ...
      const confirmWithdrawButton = screen.getByRole('button', { name: /sim, revogar/i });
    ...
    it('deve permitir solicitar acesso aos dados', async () => {
      ...
      render(<LGPDConsentManager consentData={mockConsentData} onRequestDataAccess={onRequestDataAccess} />);
      
      const accessButton = screen.getByRole('button', { name: /acessar meus dados/i });
    ...
    it('deve permitir solicitar exclusão dos dados', async () => {
      ...
      render(<LGPDConsentManager consentData={mockConsentData} onRequestDataDeletion={onRequestDataDeletion} />);
      
      const deleteButton = screen.getByRole('button', { name: /excluir meus dados/i });
      ...
      const confirmDeleteButton = screen.getByRole('button', { name: /sim, excluir/i });
  ...
  describe('Direitos do Titular', () => {
    ...
    it('deve permitir exercer direitos através de botões', async () => {
      ...
      render(<LGPDConsentManager consentData={mockConsentData} onExerciseRight={onExerciseRight} />);
      
      const accessButton = screen.getByRole('button', { name: /solicitar acesso/i });
      await userEvent.click(accessButton);
      
      expect(onExerciseRight).toHaveBeenCalledWith('access', mockConsentData.id);
    });
  });
  ...
  describe('Audit Trail', () => {
    it('deve mostrar histórico de alterações', async () => {
      ...
      render(<LGPDConsentManager consentData={mockConsentData} auditTrail={auditTrail} />);
      ...
      expect(screen.getByText('Período de retenção alterado')).toBeInTheDocument();
    });
  ...
  describe('Criptografia e Segurança', () => {
    it('deve mostrar status de criptografia', () => {
      render(<LGPDConsentManager consentData={mockConsentData} isEncrypted={true} />);
    ...
    it('deve mostrar detalhes de criptografia', async () => {
      render(<LGPDConsentManager consentData={mockConsentData} encryptionDetails="AES-256" />);
      
      await userEvent.click(screen.getByRole('button', { name: /detalhes de segurança/i }));
    ...
    it('deve mostrar alerta se não criptografado', () => {
      render(<LGPDConsentManager consentData={mockConsentData} isEncrypted={false} />);
      
      expect(screen.getByText(/atenção: dados não criptografados/i)).toBeInTheDocument();
      expect(screen.getByText(/seus dados estão vulneráveis/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Acessibilidade', () => {
    ...
    it('deve ter landmarks ARIA corretos', () => {
      ...
      expect(screen.getByRole('complementary', { name: /direitos do titular/i })).toBeInTheDocument();
    });
    ...
    it('deve suportar navegação por teclado', async () => {
      ...
      const button = screen.getByRole('button', { name: /novo consentimento/i });
  ...
  describe('Performance', () => {
    it('deve carregar rapidamente com audit trail grande', () => {
      ...
      render(<LGPDConsentManager consentData={mockConsentData} auditTrail={largeAuditTrail} />);
  ...
  describe('Testes de Integração', () => {
    ...
    it('deve enviar eventos de analytics', () => {
      ...
      // Simular interação
      fireEvent.click(screen.getByRole('button', { name: /novo consentimento/i }));
    ...
    it('deve respeitar preferências de idioma', () => {
      render(<LGPDConsentManager consentData={mockConsentData} language="pt-BR" />);
  ...
  describe('Edge Cases', () => {
    ...
    it('deve lidar com falha na revogação', async () => {
      mockWithdrawConsent.mockRejectedValueOnce(new Error('Falha na revogação'));
      
      render(<LGPDConsentManager consentData={mockConsentData} onWithdrawConsent={mockWithdrawConsent} />);
      
      await userEvent.click(screen.getByRole('button', { name: /revogar consentimento/i }));
      await userEvent.click(screen.getByRole('button', { name: /sim, revogar/i }));
    ...
    it('deve lidar com consentimento sem data de expiração', () => {
      ...
      expect(screen.getByText(/período de retenção não definido/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Conformidade LGPD', () => {
    ...
    it('deve garantir que consentimento seja informado', async () => {
      ...
      expect(screen.getByText(/seus dados serão utilizados para/i)).toBeInTheDocument();
    });
    ...
    it('deve garantir que consentimento seja inequívoco', async () => {
      ...
      const consentCheckbox = screen.getByLabelText(/declaro que li e concordo/i);
      ...
      expect(screen.getByText(/você precisa concordar com os termos/i)).toBeInTheDocument();
    });
