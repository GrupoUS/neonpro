...
export const MobileResponsiveValidator: React.FC = () => {
  const [currentViewport, setCurrentViewport] = useState<keyof typeof VIEWPORT_BREAKPOINTS>(
    'mobile',
  );
  ...
  const [accessibilityResults, setAccessibilityResults] = useState<AccessibilityValidation | null>(
    null,
  );
  ...
  const validateTouchTargets = useCallback((): TouchTargetValidation[] => {
    ...
    interactiveElements.forEach(element => {
      ...
      if (
        htmlElement.hasAttribute('data-critical') || htmlElement.classList.contains('emergency')
      ) {
        category = 'critical';
      } else if (
        htmlElement.hasAttribute('data-healthcare') || htmlElement.closest('[data-healthcare]')
      ) {
  ...
  const validateHealthcareUX = useCallback((): HealthcareUXValidation => {
    ...
    const clinicalElements = document.querySelectorAll(
      '[data-clinical], .clinical-workflow, .patient-card',
    );
    ...
    const safetyElements = document.querySelectorAll(
      '[data-safety], .emergency-alert, .critical-action',
    );
    ...
    const privacyElements = document.querySelectorAll(
      '[data-privacy], .data-masked, .sensitive-info',
    );
      ...
      <CardContent>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
          {Object.entries(performanceMetrics).map(([metric, value]) => {
            ...
            const target =
              PERFORMANCE_TARGETS[metric.toUpperCase() as keyof typeof PERFORMANCE_TARGETS];
            ...
            <CardContent>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                <div className='p-3 border rounded'>
                  ...
                  <div className='text-sm text-muted-foreground'>
                    {result.touchTargets.filter(tt => tt.isValid).length} /{' '}
                    {result.touchTargets.length} valid
                  </div>
                </div>
                <div className='p-3 border rounded'>
                ...
                <div className='p-3 border rounded'>
                  ...
                  <div className='text-sm text-muted-foreground'>
                    {result.touchTargets.filter(tt => tt.category === 'recommended').length}{' '}
                    elements
                  </div>
                </div>
              </div>
            ...
            <Card>
              <CardContent className='pt-6 text-center'>
                <p className='text-muted-foreground'>
                  Run validation to see responsive design results
                </p>
              </CardContent>
            </Card>
          )}
