      ...
      <div className='max-w-md w-full mx-auto p-6'>
        <div className='text-center'>
          ...
          <div className='space-y-3'>
            <button onClick={resetError} className='w-full bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors'>
              Tentar novamente
            </button>
            <button onClick={() => window.location.reload()} className='w-full bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-200 transition-colors'>
              Recarregar página
            </button>
          </div>
          {process.env.NODE_ENV === 'development' && (
            <details className='mt-6 text-left'>
              <summary className='cursor-pointer text-sm text-gray-500 hover:text-gray-700'>Detalhes do erro (dev)</summary>
...
export class LocalErrorBoundary extends React.Component<Props, { hasError: boolean; error: Error | null }> {
