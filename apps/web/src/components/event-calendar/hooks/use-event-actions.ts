...
import type { CreateEventData, UpdateEventData, CalendarEventExtended } from "@/services/event.service";
...
export function useEventActions() {
  ...
  const duplicateEvent = useCallback(async (
    ...
    try {
      ...
      const newEvent: CreateEventData = {
        ...
        start: new Date(originalEvent.start.getTime() + (offsetDays * 24 * 60 * 60 * 1000) + (offsetHours * 60 * 60 * 1000)),
        end: new Date(originalEvent.end.getTime() + (offsetDays * 24 * 60 * 60 * 1000) + (offsetHours * 60 * 60 * 1000)),
  ...
  const bulkDeleteEvents = useCallback(async (
    ...
    try {
      if (requireConfirmation) {
        const confirmed = window.confirm(`Are you sure you want to delete ${eventIds.length} events?`);
        if (!confirmed) {
          return false;
        }
      }
