...
vi.mock(_'@/integrations/supabase/client',_() => {
  ...
  const supabase = {
    from: vi
      .fn()
      .mockImplementation((table: string) => mkChain(tables[table] ?? [])),
  };
  return { supabase };
});
...
describe(_'NotificationCard',_() => {
  test(_'renders aggregated notifications and navigates on click',_async () => {
