    ...
    complications: [
      {
        ...
        severity: 'minor',
        timeframe: 3,
        preventionStrategies: ["Compressas frias", "Elevação da área", "Anti-inflamatórios"],
        warningSignals: ["Vermelhidão excessiva", "Dor crescente", "Temperatura local"]
      },
      {
        ...
        severity: 'minor',
        timeframe: 14,
        preventionStrategies: ["Protetor solar FPS 50+", "Evitar exposição solar", "Hidratação"],
      ...
      },
      {
        ...
        severity: 'moderate',
        timeframe: 1,
        preventionStrategies: ["Teste de alergia prévio", "Anti-histamínicos", "Monitoramento"],
        warningSignals: ["Coceira intensa", "Urticária", "Dificuldade respiratória"]
      }
    ],
    recoveryTimeline: [
      {
  ...
  recommendations: {
    ...
    followUpSchedule: {
      appointments: [
        ...
        { day: 21, type: "final", importance: "critical", description: "Avaliação final dos resultados" }
      ]
    },
    riskMitigation: [
      {
...
export default function PatientOutcomePrediction({
  ...
  const riskLevelConfig = useMemo(() => {
    ...
    switch (risk) {
      case 'low':
        return { label: 'Baixo', color: 'text-green-600', bgColor: 'bg-green-100', icon: CheckCircle };
      case 'medium':
        return { label: 'Médio', color: 'text-yellow-600', bgColor: 'bg-yellow-100', icon: AlertTriangle };
      case 'high':
        return { label: 'Alto', color: 'text-orange-600', bgColor: 'bg-orange-100', icon: AlertTriangle };
      case 'critical':
        return { label: 'Crítico', color: 'text-red-600', bgColor: 'bg-red-100', icon: AlertTriangle };
      default:
        return { label: 'Médio', color: 'text-yellow-600', bgColor: 'bg-yellow-100', icon: AlertTriangle };
    }
  ...
  const highRiskComplications = useMemo(() => 
    prediction.predictions.complications.filter(comp => 
      comp.probability > 0.2 || comp.severity === 'severe' || comp.severity === 'critical'
    ),
  ...
  // ====== RENDER COMPONENTS ======
  const renderComplicationCard = (complication: ComplicationPrediction, index: number) => (
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={cn(
              "p-2 rounded-full",
              complication.severity === 'critical' ? 'bg-red-100 text-red-600' :
              complication.severity === 'severe' ? 'bg-orange-100 text-orange-600' :
              complication.severity === 'moderate' ? 'bg-yellow-100 text-yellow-600' :
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                <Badge variant={
                  complication.severity === 'critical' ? 'destructive' :
                  complication.severity === 'severe' ? 'destructive' :
                  complication.severity === 'moderate' ? 'secondary' : 'outline'
                }>
                  {complication.severity === 'critical' ? 'Crítico' :
                   complication.severity === 'severe' ? 'Grave' :
                   complication.severity === 'moderate' ? 'Moderado' : 'Leve'}
                </Badge>
      ...
      <CardContent className="pt-0">
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div>
              <div className="text-sm font-medium mb-1">Estratégias de Prevenção:</div>
            ...
            <div>
              <div className="text-sm font-medium mb-1 text-orange-600">Sinais de Alerta:</div>
              <ul className="text-sm text-muted-foreground space-y-1">
  ...
  const renderRecoveryMilestone = (milestone: RecoveryMilestone, index: number) => (
    <div key={index} className="flex items-center space-x-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center space-x-3 mt-1">
          ...
          <Progress value={milestone.probability * 100} className="h-1 flex-1 max-w-24" />
        </div>
  ...
  const renderAlternativeTreatment = (treatment: AlternativeTreatment) => (
    <Card key={treatment.treatmentId} className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center space-x-2 mt-1">
              ...
              <Badge variant={treatment.suitabilityScore >= 80 ? 'default' : 'secondary'}>
          ...
          <div className="text-right">
            <div className={cn(
              "text-sm font-medium",
              treatment.costDifference < 0 ? 'text-green-600' : 'text-red-600'
            )}>
      ...
      <CardContent className="pt-0">
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <Progress value={treatment.successProbability * 100} className="h-2 w-24" />
          </div>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge variant={
              treatment.riskProfile.overallRisk === 'low' ? 'outline' :
              treatment.riskProfile.overallRisk === 'medium' ? 'secondary' : 'destructive'
            }>
              {treatment.riskProfile.overallRisk === 'low' ? 'Baixo' :
               treatment.riskProfile.overallRisk === 'medium' ? 'Médio' : 'Alto'}
            </Badge>
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Predição de Resultados</h2>
        ...
        <CardContent className="flex items-center justify-between py-4">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-sm">
              Confiança: <span className={cn("font-bold", confidenceLevel.color)}>
                {(prediction.confidence * 100).toFixed(0)}%
              </span>
            </div>
            <Badge className={cn("text-white", confidenceLevel.color.replace('text-', 'bg-'))}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Outcome Score */}
        <Card className={cn("relative overflow-hidden", outcomeCategory.bgColor)}>
          ...
          <CardContent>
            <div className="text-center space-y-2">
              ...
              <Badge className={cn("text-white", outcomeCategory.color.replace('text-', 'bg-'))}>
        ...
        {/* Risk Level */}
        <Card className={cn("relative overflow-hidden", riskLevelConfig.bgColor)}>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center space-x-2">
              <riskLevelConfig.icon className={cn("h-5 w-5", riskLevelConfig.color)} />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="prediction" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {[
                    { factor: 'Histórico de tratamentos similares', impact: 85, positive: true },
                    { factor: 'Tipo de pele e resposta', impact: 78, positive: true },
                    { factor: 'Idade do paciente', impact: 65, positive: true },
                    { factor: 'Compliance com instruções', impact: 72, positive: true },
                    { factor: 'Fatores de risco identificados', impact: 45, positive: false }
                  ].map((item, index) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {prediction.recommendations.preventiveMeasures.map((measure, index) => (
                    <div key={index} className="flex items-start space-x-3 p-3 bg-muted/20 rounded-lg">
                      <div className={cn(
                        "p-1 rounded-full mt-1",
                        measure.priority === 'critical' ? 'bg-red-100 text-red-600' :
                        measure.priority === 'high' ? 'bg-orange-100 text-orange-600' :
                        'bg-blue-100 text-blue-600'
                      )}>
                        <CheckCircle className="h-3 w-3" />
                      </div>
                      <div className="flex-1">
                        <div className="font-medium text-sm">{measure.action}</div>
                        <div className="flex items-center space-x-2 mt-1">
                          <Badge 
                            variant={
                              measure.priority === 'critical' ? 'destructive' :
                              measure.priority === 'high' ? 'secondary' : 'outline'
                            }
                            className="text-xs"
                          >
                            {measure.priority === 'critical' ? 'Crítico' :
                             measure.priority === 'high' ? 'Alto' : 'Médio'}
                          </Badge>
        ...
        <TabsContent value="timeline" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {prediction.predictions.recoveryTimeline.map(renderRecoveryMilestone)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {prediction.recommendations.followUpSchedule.appointments.map((apt, index) => (
        ...
        <TabsContent value="complications" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {prediction.recommendations.riskMitigation.map((strategy, index) => (
                  <div key={index} className="flex items-start space-x-3 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 mt-2">
                        ...
                        <Badge variant={
                          strategy.impact === 'low' ? 'outline' :
                          strategy.impact === 'medium' ? 'secondary' : 'destructive'
                        }>
