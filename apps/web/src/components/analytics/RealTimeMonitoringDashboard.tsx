...
const mockMonitoringData: HealthcareMonitoring = {
  ...
  monitoringData: {
    ...
    medicationCompliance: {
      currentMedications: [
        { name: 'Losartana 50mg', dosage: '1x ao dia', lastTaken: new Date(Date.now() - 8 * 60 * 60 * 1000), nextDue: new Date(Date.now() + 16 * 60 * 60 * 1000) },
        { name: 'Sinvastatina 20mg', dosage: '1x à noite', lastTaken: new Date(Date.now() - 14 * 60 * 60 * 1000), nextDue: new Date(Date.now() + 10 * 60 * 60 * 1000) }
      ],
      adherenceRate: 0.92,
      missedDoses: [
        { medication: 'Sinvastatina 20mg', scheduledTime: new Date(Date.now() - 38 * 60 * 60 * 1000), actualTime: null, reason: 'Esqueceu' }
      ],
      sideEffects: [],
      drugInteractions: [],
    ...
    healthIndicators: {
      ...
      concerns: ['Blood pressure trending upward', 'Medication adherence needs improvement']
    },
    riskFactors: {
      cardiovascular: 0.35,
      diabetes: 0.12,
    ...
    warningAlerts: [
      {
        ...
        escalationLevel: 1,
        autoActions: [],
        requiredActions: ['Verificar medicação', 'Agendar consulta de seguimento'],
        dismissible: true,
...
export default function RealTimeMonitoringDashboard({
        ...
        monitoringData: {
          ...
          vitalSigns: {
            ...
            heartRate: {
              ...
              timestamps: [...prev.monitoringData.vitalSigns.heartRate.timestamps.slice(-4), new Date()],
              values: [...prev.monitoringData.vitalSigns.heartRate.values.slice(-4), 75 + Math.random() * 20]
            }
          }
        }
      }));
  ...
  const handleAlertAction = useCallback((alertId: string, action: 'acknowledge' | 'dismiss' | 'escalate') => {
  ...
  const filteredAlerts = useMemo(() => {
    const allAlerts = [
      ...monitoringData.alerts.criticalAlerts.map(a => ({ ...a, category: 'critical' as const })),
      ...monitoringData.alerts.warningAlerts.map(a => ({ ...a, category: 'warning' as const })),
  ...
  const renderPatientCard = (patient: typeof activePatientsMonitoring[0]) => (
    <Card 
      ...
      className={cn(
        ...
        patient.status === 'attention' ? "border-l-4 border-l-yellow-500" :
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex items-center justify-between">
            ...
            <span className={cn(
              "font-medium",
              patient.vitals.hr > 100 ? "text-red-600" :
              patient.vitals.hr > 90 ? "text-yellow-600" : "text-green-600"
            )}>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={cn(
              "font-medium",
              patient.vitals.spo2 < 95 ? "text-red-600" :
              patient.vitals.spo2 < 97 ? "text-yellow-600" : "text-green-600"
            )}>
              {patient.vitals.spo2}%
            </span>
          </div>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className={cn(
            ...
            patient.status === 'attention' ? "text-yellow-600" : "text-green-600"
          )}>
            <Activity className="h-3 w-3" />
            <span>
              {patient.status === 'critical' ? 'Crítico' :
               patient.status === 'attention' ? 'Atenção' : 'Estável'}
            </span>
          </div>
  ...
  const renderVitalSignCard = (title: string, current: string | number, previous: string | number, unit: string, trend: 'up' | 'down' | 'stable', status: 'normal' | 'warning' | 'critical') => (
    <Card>
      <CardContent className="p-4">
  ...
  const renderAlertCard = (alert: any, index: number) => (
    <Card key={alert.id || index} className={cn(
      "border-l-4",
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-muted-foreground">
              {alert.triggeredAt ? new Date(alert.triggeredAt).toLocaleTimeString('pt-BR', { 
                hour: '2-digit', 
                minute: '2-digit' 
              }) : 'Agora'}
            </span>
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center justify-between">
          <div className="flex space-x-2">
            ...
            <Button size="sm" variant="destructive" onClick={() => handleAlertAction(alert.id, 'escalate')}>
        ...
        <CardContent className="flex items-center justify-between py-4">
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">{systemHealth.connectedDevices} Pacientes Monitorados</span>
            </div>
            <div className="flex items-center space-x-2">
              <Bell className="h-5 w-5 text-yellow-600" />
              <span className="font-medium">{systemHealth.alertsCount} Alertas Ativos</span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">Uptime: {systemHealth.uptime}%</span>
            </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="text-sm">
              Qualidade dos Dados: <span className="font-bold text-green-600">{systemHealth.dataQuality}%</span>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Badge variant="outline">{activePatientsMonitoring.length}</Badge>
              </CardTitle>
            </CardHeader>
        ...
        <div className="lg:col-span-3 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-3">
                    <Badge variant={
                      ...
                      currentPatient.status === 'attention' ? 'secondary' : 'outline'
                    } className="text-base px-3 py-1">
                      {currentPatient.status === 'critical' ? 'Estado Crítico' :
                       currentPatient.status === 'attention' ? 'Requer Atenção' : 'Estável'}
                    </Badge>
                    <Button size="sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {renderVitalSignCard(
              ...
              " bpm", 
              "up", 
              (currentPatient?.vitals.hr || 0) > 100 ? "critical" : (currentPatient?.vitals.hr || 0) > 90 ? "warning" : "normal"
            )}
            {renderVitalSignCard(
              "Pressão Arterial", 
              ...
              "", 
              "up", 
              currentPatient?.vitals.bp.includes('160') ? "critical" : currentPatient?.vitals.bp.includes('145') ? "warning" : "normal"
            )}
            {renderVitalSignCard(
              "Temperatura", 
            ...
            {renderVitalSignCard(
              ...
              "%", 
              "down", 
              (currentPatient?.vitals.spo2 || 0) < 95 ? "critical" : (currentPatient?.vitals.spo2 || 0) < 97 ? "warning" : "normal"
            )}
          </div>
          ...
          <Card>
            ...
            <CardContent className="h-64 flex items-center justify-center text-muted-foreground">
              <div className="text-center">
                ...
                <p>Gráfico de tendências em tempo real será implementado aqui</p>
            ...
            <CardContent>
              {filteredAlerts.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Todos os pacientes estão dentro dos parâmetros normais</p>
                </div>
              )}
            </CardContent>
          </Card>
