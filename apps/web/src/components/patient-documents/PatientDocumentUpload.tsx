...
export function PatientDocumentUpload({
  ...
  const { user } = useAuth();
  const [uploadQueue, setUploadQueue] = useState<Map<string, UploadState>>(new Map());
  ...
  const validateFile = useCallback((file: File): string | null => {
    ...
    if (!acceptedTypes.includes(file.type as any)) {
      return `Tipo de arquivo não suportado. Aceito: ${
        acceptedTypes.map(type =>
          HEALTHCARE_FILE_TYPES[type as keyof typeof HEALTHCARE_FILE_TYPES]?.label || type
        ).join(', ')
      }`;
    }

    // Check file size with type-specific limits
    const typeInfo = HEALTHCARE_FILE_TYPES[file.type as keyof typeof HEALTHCARE_FILE_TYPES]
    ...
    if (
      category === 'identity' && !file.name.toLowerCase().includes('rg')
      && !file.name.toLowerCase().includes('cpf') && !file.name.toLowerCase().includes('cnh')
    ) {
      console.warn('Identity document should contain identifying keywords in filename');
    }

    return null;
  }, [acceptedTypes, maxFileSize, maxFiles, existingDocuments.length, uploadQueue.size, category]);

  // Upload file to backend API
  const uploadDocument = async (file: File, uploadId: string): Promise<PatientDocument> => {
    if (!user) {
  ...
  }, [disabled, user, validateFile, uploadDocument, onDocumentsUploaded, patientId, category]);
  ...
  const getFileTypeInfo = (mimeType: string) => {
    return HEALTHCARE_FILE_TYPES[mimeType as keyof typeof HEALTHCARE_FILE_TYPES]
      || HEALTHCARE_FILE_TYPES.default;
  };
    ...
    >
      ...
      >
        ...
        <div
          ...
          aria-label={canUpload ? 'Área de upload de documentos' : 'Limite de documentos atingido'}
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              <div className='flex items-center justify-center gap-4 text-xs text-muted-foreground'>
                ...
                <span>•</span>
                <span>{totalDocuments}/{maxFiles} documentos</span>
              </div>
            </div>
        ...
        {/* Upload Queue */}
        {uploadQueue.size > 0 && (
          <div className='space-y-3' role='region' aria-label='Progresso de upload'>
              ...
              return (
                ...
                >
                  ...
                  <div className='flex-1 min-w-0 space-y-1'>
                    ...
                    {state.status === 'completed' && (
                      <p className='text-xs text-green-600'>Upload concluído com sucesso!</p>
                    )}
                  </div>
        ...
        {showExisting && existingDocuments.length > 0 && (
          <div className='space-y-3' role='region' aria-label='Documentos existentes'>
            <h4 className='text-sm font-medium flex items-center gap-2'>
        ...
        {/* Empty state */}
        {showExisting && existingDocuments.length === 0 && uploadQueue.size === 0 && (
          <div className='text-center py-8 text-muted-foreground'>
            <IconFile className='mx-auto h-12 w-12 mb-4 opacity-50' />
            <p>Nenhum documento enviado ainda</p>
            <p className='text-sm'>Faça upload dos primeiros documentos do paciente</p>
          </div>
        )}
      </div>
