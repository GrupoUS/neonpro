...
export function SchedulingUIPreview() {
  const [currentScenario, setCurrentScenario] =
    React.useState<keyof typeof mockScenarios>("busy");
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          <p className="text-gray-600 mb-6">
            Interactive preview of the telemedicine scheduling interface. Test
            appointment creation, editing, and management features.
          </p>
          ...
          <div className="flex gap-4">
            {Object.entries(mockScenarios).map(([key, scenario]) => (
              <button
                key={key}
                onClick={() =>
                  setCurrentScenario(key as keyof typeof mockScenarios)
                }
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-md">
            <p className="text-sm text-gray-600">
              ...
              {mockScenarios[currentScenario].description}
            </p>
          </div>

          <div className="mt-4 p-4 bg-blue-50 rounded-md">
            <p className="text-sm text-blue-700">
              <strong>Interaction Demo:</strong> Click buttons in the component
              below to see console logs and alerts demonstrating the callback
              functions.
            </p>
          </div>
        </div>
