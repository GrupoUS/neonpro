...
export function RealTimeChat({
  ...
  const { complianceStatus, flagMessage, checkCompliance } =
    useChatCompliance(sessionId);

  const { auditLog, exportChatHistory, generateAuditReport } =
  ...
  useEffect(() => {
    const unreadMessages = messages.filter(
      (m) =>
        ...
        !m.auditTrail.seen?.some(
          (seen) => seen.toString() === new Date().toDateString(),
        ),
    );
    ...
    if (filterType !== "all") {
      filtered = filtered.filter((message) => {
        switch (filterType) {
          ...
          case "medical":
            return (
              message.messageType === "medical_note" ||
  ...
  const detectUrgencyLevel = useCallback(
    (text: string): "low" | "medium" | "high" | "critical" => {
      ...
      if (criticalKeywords.some((keyword) => lowerText.includes(keyword))) {
        ...
        mediumKeywords.some((keyword) => lowerText.includes(keyword))
      ) {
        return "medium";
      }
      ...
      try {
        await sendMessage(suggestion.content, {
          ...
          metadata: {
            ...
            urgencyLevel:
              suggestion.type === "emergency" ? "critical" : "medium",
          },
        });
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={setAutoAIAssist}
                  />
                </div>
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-green-800">
                Chat em conformidade com CFM e LGPD
              </span>
            </div>
            ...
            >
              <div
                className={`max-w-xs lg:max-w-md ${
                  message.senderType !== participantRole ? "order-2" : ""
                }`}
              >
                {/* Avatar and Name */}
                {message.senderType !== participantRole && (
                ...
                {/* Message Bubble */}
                <div
                  className={`rounded-lg p-3 ${getMessageBgColor(message)} relative group`}
                >
                        ...
                        message.metadata?.fileName && (
                          <div className="mt-2 p-2 bg-gray-100 rounded flex items-center space-x-2">
                            ...
                            <span className="text-xs text-gray-500">
                              (
                              {Math.round(
                                (message.metadata.fileSize || 0) / 1024,
                              )}{" "}
                              KB)
                            </span>
                          </div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <div className="flex items-center space-x-1">
                      ...
                      {message.metadata?.urgencyLevel &&
                        message.metadata.urgencyLevel !== "low" && (
                          <Badge
                            variant="outline"
                            className={`text-xs ${
                              message.metadata.urgencyLevel === "critical"
        ...
        {enableAI && (
          <div className="flex items-center space-x-2 mt-2">
            <Button
              ...
              size="sm"
              onClick={() =>
                generateSuggestion("Preciso de ajuda médica", "emergency")
              }
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                generateSuggestion("Quais são os sintomas?", "diagnosis")
              }
            >
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                generateSuggestion("Como devo proceder?", "treatment")
              }
            >
