...
export function VideoConsultation({
  ...
  // Hooks
  const { session, updateSession, endSession, isEnding } =
  ...
  const { messages, sendMessage, isSending } = useRealTimeChat({
    sessionId,
    enableAI: true,
  });
  ...
  const handleEmergencyEscalation = useCallback(
    async (emergencyType: "medical" | "technical") => {
      try {
  ...
  return (
    <div
      className={`h-screen flex flex-col bg-gray-900 text-white ${className}`}
    >
      {/* Session Header */}
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-300">
              ...
              <span>•</span>
              <div
                className={`flex items-center space-x-1 ${getQualityColor(networkQuality)}`}
              >
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-1">
            ...
            {session.lgpdCompliant && (
              <Badge
                variant="outline"
                className="text-xs bg-blue-900 text-blue-300 border-blue-700"
              >
                LGPD
              </Badge>
            )}
            {session.consentGiven && (
              <Badge
                variant="outline"
                className="text-xs bg-purple-900 text-purple-300 border-purple-700"
              >
                Consent
              </Badge>
            )}
            {isRecording && (
              <Badge
                variant="outline"
                className="text-xs bg-red-900 text-red-300 border-red-700 animate-pulse"
              >
                REC
              </Badge>
            )}
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 left-4 space-y-2">
          {participants.map((participant) => (
            <div
              key={participant.id}
              className="flex items-center space-x-2 bg-gray-800/80 backdrop-blur-sm rounded-lg p-2"
            >
              <div
          ...
          (networkQuality === "critical" && (
            <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-50">
              <Alert className="bg-orange-900 border-orange-700 text-orange-300">
                ...
                <AlertDescription>
                  Qualidade da conexão baixa. A experiência pode ser
                  comprometida.
                </AlertDescription>
              </Alert>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              onClick={toggleVideo}
              title={
                webrtcState.isVideoEnabled ? "Desligar câmera" : "Ligar câmera"
              }
            >
              {webrtcState.isVideoEnabled ? (
            ...
            <div className="flex space-x-1">
              <div
                className={`w-1 h-3 rounded ${
                  networkQuality === "critical" ? "bg-red-500" : "bg-gray-600"
                }`}
              />
              <div
                className={`w-1 h-3 rounded ${
                  ["poor", "good", "excellent"].includes(networkQuality)
                    ? "bg-yellow-500"
                    : "bg-gray-600"
                }`}
              />
              <div
                className={`w-1 h-3 rounded ${
                  ["good", "excellent"].includes(networkQuality)
                    ? "bg-green-500"
                    : "bg-gray-600"
                }`}
              />
