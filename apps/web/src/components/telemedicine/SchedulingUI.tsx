...
import { format, addDays, startOfWeek, endOfWeek, isSameDay, isPast, isToday } from 'date-fns';
...
interface SchedulingUIProps {
  ...
  onAppointmentCreate?: (appointment: Omit<TelemedicineAppointment, 'id'>) => void;
...
const getStatusBadge = (status: TelemedicineAppointment['status']) => {
  const variants = {
    scheduled: { variant: 'secondary' as const, icon: Calendar, text: 'Agendado' },
    confirmed: { variant: 'default' as const, icon: CheckCircle, text: 'Confirmado' },
    'in-progress': { variant: 'default' as const, icon: Video, text: 'Em Andamento' },
    completed: { variant: 'outline' as const, icon: CheckCircle, text: 'Concluído' },
    cancelled: { variant: 'destructive' as const, icon: AlertCircle, text: 'Cancelado' },
    'no-show': { variant: 'destructive' as const, icon: AlertCircle, text: 'Faltou' },
  };
...
const getAppointmentTypeIcon = (type: TelemedicineAppointment['appointmentType']) => {
  const icons = {
    video: Video,
    phone: Phone,
    hybrid: Monitor,
  ...
  const handleDateSelect = (date: Date) => {
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Agendamento de Telemedicina</h2>
...
function NewAppointmentForm({ 
  ...
  const [appointmentType, setAppointmentType] = useState<'video' | 'phone' | 'hybrid'>('video');
