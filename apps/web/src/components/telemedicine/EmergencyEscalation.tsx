...
export interface EmergencyEvent {
  timestamp: Date;
  type: 'activation' | 'contact' | 'dispatch' | 'update' | 'arrival' | 'transfer';
  description: string;
  actor: string;
}
...
export function EmergencyEscalation({
  ...
  const addTimelineEvent = (type: EmergencyEvent['type'], description: string) => {
  ...
  const calculateSeverity = (type: EmergencyType, symptoms: string): EmergencySeverity => {
    ...
    if (type.category === 'cardiac') {
      ...
      score = 10;
      timeLimit = 0;
      indicators.push('Alteração neurológica aguda', 'Janela terapêutica crítica');
    }
