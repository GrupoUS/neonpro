...
export const LGPDConsentManager: React.FC<LGPDConsentManagerProps> = ({ className }) => {
  const [consentRecords, setConsentRecords] = useState<LGPDConsentRecord[]>([]);
  const [processingActivities, setProcessingActivities] = useState<LGPDDataProcessingActivity[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<LGPDConsentStatus | 'all'>('all');
  ...
  const calculateComplianceScore = (
    ...
    const consentBasedActivities = activities.filter(a => a.legalBasis === 'consent');
    const activeConsents = consents.filter(c => c.status === 'given');
    
    consentBasedActivities.forEach(activity => {
      const hasActiveConsent = activeConsents.some(c => c.processingActivityId === activity.id);
      if (!hasActiveConsent) {
        score -= 10;
      }
    });
    ...
    try {
      ...
      if (result.isValid) {
        ...
        const updatedConsents = consentRecords.map(c => 
          c.id === selectedConsent.id ? { ...c, status: 'withdrawn' as LGPDConsentStatus } : c
        );
  ...
  const handleDataSubjectRequest = async (type: 'access' | 'portability' | 'deletion' | 'rectification') => {
    try {
      ...
      if (result.isValid) {
        toast.success(`Solicitação de ${type === 'access' ? 'acesso' : type === 'portability' ? 'portabilidade' : type === 'deletion' ? 'exclusão' : 'retificação'} processada com sucesso`);
      } else {
  ...
  const filteredConsentRecords = consentRecords.filter(consent => {
    const matchesSearch = consent.dataSubjectId.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || consent.status === statusFilter;
    const matchesActivity = activityFilter === 'all' || consent.processingActivityId === activityFilter;
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold tracking-tight">Gestão de Consentimento LGPD</h2>
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <div className={cn("text-2xl font-bold", getScoreColor(complianceScore))}>
              {complianceScore}%
            </div>
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid gap-6 py-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, dataSubjectName: e.target.value }))}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, dataSubjectId: e.target.value }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, dataSubjectEmail: e.target.value }))}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, dataSubjectPhone: e.target.value }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => setFormData(prev => ({ ...prev, processingActivityId: value }))}
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value: LGPDConsentType) => setFormData(prev => ({ ...prev, consentType: value }))}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Tipo de consentimento" />
                        </SelectTrigger>
                        <SelectContent>
                          {Object.entries(CONSENT_TYPE_CONFIG).map(([key, config]) => (
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setFormData(prev => ({
                                ...prev,
                                dataCategories: [...prev.dataCategories, key as any]
                              }));
                            } else {
                              setFormData(prev => ({
                                ...prev,
                                dataCategories: prev.dataCategories.filter(cat => cat !== key)
                              }));
                            }
                          }}
                        />
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              ...
                              setFormData(prev => ({
                                ...prev,
                                purposes: prev.purposes.filter(purpose => purpose !== key)
                              }));
                            }
                          }}
                        />
                ...
                <div className="space-y-4">
                  ...
                  <Select 
                    ...
                    onValueChange={(value: any) => setFormData(prev => ({ ...prev, communicationChannel: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  disabled={!formData.dataSubjectId || !formData.processingActivityId || formData.dataCategories.length === 0}
                >
                  Registrar Consentimento
                </Button>
              </div>
      ...
      <div className="flex gap-4">
        ...
        <Select value={statusFilter} onValueChange={(value: LGPDConsentStatus | 'all') => setStatusFilter(value)}>
      ...
      <Tabs defaultValue="consents" className="space-y-4">
              ...
              <Table>
                ...
                <TableBody>
                  {filteredConsentRecords.map((consent) => {
                    const activity = processingActivities.find(a => a.id === consent.processingActivityId);
                    ...
                    return (
                      <TableRow key={consent.id}>
                        <TableCell>
                          <div className="font-medium">ID: {consent.dataSubjectId}</div>
                        </TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium text-sm">{activity?.name || 'Atividade não encontrada'}</div>
                        ...
                        <TableCell>
                          <div className={cn("flex items-center gap-2 text-sm", statusConfig.text)}>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            {consent.expirationDate 
                              ? consent.expirationDate.toLocaleDateString('pt-BR')
                              : 'Indefinida'
                            }
                          </div>
                        </TableCell>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {processingActivities.map((activity) => {
              ...
              const activeConsents = consentRecords.filter(
                c => c.processingActivityId === activity.id && c.status === 'given'
              ).length;
                ...
                <Card key={activity.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant="outline" className={activity.isActive ? 'bg-green-50' : 'bg-red-50'}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="text-sm font-medium">Categorias de Dados:</div>
        ...
        <TabsContent value="requests" className="space-y-4">
          ...
          <Alert>
            ...
            <AlertDescription>
              Os titulares de dados têm direito ao acesso, correção, portabilidade e eliminação de seus dados pessoais,
              conforme previsto na LGPD. As solicitações devem ser atendidas em até 15 dias.
            </AlertDescription>
          </Alert>
        </TabsContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Consentimentos Retirados</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Consentimentos Expirados</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Alert>
                ...
                <AlertDescription>
                  Score de Conformidade Atual: <span className={cn("font-bold", getScoreColor(complianceScore))}>
                    {complianceScore}%
                  </span>
                  {complianceScore < 90 && (
                    <span className="block mt-2 text-sm">
                      • {consentRecords.filter(c => c.status === 'expired').length > 0 && 'Renovar consentimentos expirados'}
                      • {consentRecords.filter(c => c.status === 'given').length === 0 && 'Coletar consentimentos para atividades obrigatórias'}
                    </span>
                  )}
                </AlertDescription>
              </Alert>
      ...
      <Dialog open={isWithdrawalDialogOpen} onOpenChange={setIsWithdrawalDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="withdrawalReason">Motivo da Retirada (opcional)</Label>
              <Textarea
                id="withdrawalReason"
            ...
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                A retirada do consentimento será efetiva imediatamente e pode impactar a prestação de alguns serviços.
              </AlertDescription>
            </Alert>
          </div>
