...
export function TreatmentGuidelinesViewer({ 
  ...
  const categories = [
    ...
    { id: 'botulinum_toxin', label: 'Toxina Botulínica', count: guidelines?.guidelines.filter(g => g.category === 'botulinum_toxin').length || 0 },
    { id: 'fillers', label: 'Preenchimentos', count: guidelines?.guidelines.filter(g => g.category === 'fillers').length || 0 },
    { id: 'laser', label: 'Laser', count: guidelines?.guidelines.filter(g => g.category === 'laser').length || 0 },
    { id: 'chemical_peels', label: 'Peelings Químicos', count: guidelines?.guidelines.filter(g => g.category === 'chemical_peels').length || 0 },
    { id: 'thread_lift', label: 'Fios de Sustentação', count: guidelines?.guidelines.filter(g => g.category === 'thread_lift').length || 0 },
    { id: 'skin_care', label: 'Cuidados com a Pele', count: guidelines?.guidelines.filter(g => g.category === 'skin_care').length || 0 },
  ];
  ...
  const getEvidenceColor = (level: string) => {
    switch (level) {
      ...
      case 'expert_opinion': return 'bg-purple-100 text-purple-800';
              ...
              <div className="space-y-4">
                {guidelines.references.map((reference, index) => (
                  <div key={index} className="border-l-4 border-blue-500 pl-4">
                    ...
                    <p className="text-sm text-gray-500 mb-2">{reference.journal} • {reference.year}</p>
                    <div className="flex items-center gap-2">
      ...
      <CardContent className="pt-0">
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline" className={getEvidenceColor(guideline.evidenceLevel)}>
            {guideline.evidenceLevel === 'high' ? 'Alta' :
             guideline.evidenceLevel === 'moderate' ? 'Moderada' :
             guideline.evidenceLevel === 'low' ? 'Baixa' :
             guideline.evidenceLevel === 'expert_opinion' ? 'Opinião Especialista' : guideline.evidenceLevel}
          </Badge>
          <Badge variant="outline" className={getComplexityColor(guideline.complexity)}>
            {guideline.complexity === 'basic' ? 'Básico' :
             guideline.complexity === 'intermediate' ? 'Intermediário' :
             guideline.complexity === 'advanced' ? 'Avançado' : guideline.complexity}
          </Badge>
        ...
        <CardHeader>
          ...
          <div className="flex items-center gap-2 mt-4">
            <Badge variant="outline" className="bg-green-100 text-green-800">
              {guideline.evidenceLevel === 'high' ? 'Evidência Alta' :
               guideline.evidenceLevel === 'moderate' ? 'Evidência Moderada' :
               guideline.evidenceLevel === 'low' ? 'Evidência Baixa' :
               guideline.evidenceLevel === 'expert_opinion' ? 'Opinião de Especialista' : guideline.evidenceLevel}
            </Badge>
