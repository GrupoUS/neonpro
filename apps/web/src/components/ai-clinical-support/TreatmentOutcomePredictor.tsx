...
export function TreatmentOutcomePredictor({ 
  ...
  const { data: prediction, isLoading, error, refetch } = useQuery({
    queryKey: ['treatment-outcome-prediction', patientId, treatmentPlanId, procedureIds, timeHorizon[0]],
    queryFn: async () => {
    ...
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className={`text-lg font-bold ${getConfidenceColor(prediction.predictionConfidence)}`}>
      ...
      {prediction.predictionConfidence < confidenceThreshold[0] && (
        <Alert>
          ...
          <AlertDescription>
            A predição atual tem baixa confiança devido a limitações nos dados ou alta variabilidade nos resultados.
            Considere aumentar o limiar de confiança ou coletar mais informações do paciente.
          </AlertDescription>
        </Alert>
      )}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium capitalize">
                            {outcome.outcome === 'excellent' ? 'Excelente' :
                             outcome.outcome === 'good' ? 'Bom' :
                             outcome.outcome === 'moderate' ? 'Moderado' :
                             outcome.outcome === 'poor' ? 'Ruim' : outcome.outcome}
                          </span>
                        </div>
                  ...
                  {prediction.metrics.map((metric) => (
                    <div key={metric.name} className="space-y-2">
                      ...
                      <div className="text-xs text-gray-500">
                        {metric.minValue}{metric.unit} - {metric.maxValue}{metric.unit}
                      </div>
                    ...
                    {prediction.benchmarks.map((benchmark, index) => (
                      <div key={index} className="grid grid-cols-3 gap-4 items-center">
                        ...
                        <div className="text-center">
                          ...
                          <div className="font-medium">{benchmark.averageValue}{benchmark.unit}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-sm text-gray-600">Predição</div>
                          <div className={`font-medium ${benchmark.predictedValue > benchmark.averageValue ? 'text-green-600' : 'text-red-600'}`}>
                            {benchmark.predictedValue}{benchmark.unit}
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    {prediction.alternativeScenarios.map((scenario, index) => (
                      <div key={index} className="border rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge variant="outline" className={getOutcomeColor(scenario.expectedOutcome)}>
                            {scenario.expectedOutcome === 'excellent' ? 'Excelente' :
                             scenario.expectedOutcome === 'good' ? 'Bom' :
                             scenario.expectedOutcome === 'moderate' ? 'Moderado' :
                             scenario.expectedOutcome === 'poor' ? 'Ruim' : scenario.expectedOutcome}
                          </Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <span className="font-medium">{(scenario.successRate * 100).toFixed(1)}%</span>
                          </div>
                          <div>
