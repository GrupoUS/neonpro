          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className={`text-lg font-bold ${getSatisfactionColor(progress.averageSatisfaction)}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <Badge className={getStatusColor(progress.overallStatus)}>
                  {progress.overallStatus === 'on_track' ? 'No Caminho' :
                   progress.overallStatus === 'ahead' ? 'Adiantado' :
                   progress.overallStatus === 'behind' ? 'Atrasado' :
                   progress.overallStatus === 'at_risk' ? 'Em Risco' :
                   progress.overallStatus === 'paused' ? 'Pausado' : progress.overallStatus}
                </Badge>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={newUpdate.sessionId} onValueChange={(value) => setNewUpdate(prev => ({ ...prev, sessionId: value }))}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewUpdate(prev => ({ ...prev, progressPercentage: parseInt(e.target.value) || 0 }))}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewUpdate(prev => ({ ...prev, satisfaction: parseInt(e.target.value) || 5 }))}
              />
            </div>
                  ...
                  {progress.goals.map((goal) => (
                    <div key={goal.id} className="space-y-2">
                      ...
                      <div className="text-xs text-gray-500">
                        Meta: {goal.targetProgress}% • {goal.isAchieved ? '✅ Alcançado' : '🔄 Em andamento'}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {progress.timeline.map((event, index) => (
                  <div key={index} className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                      {event.type === 'session' ? <Calendar className="h-4 w-4 text-blue-600" /> :
                       event.type === 'update' ? <Edit className="h-4 w-4 text-green-600" /> :
                       event.type === 'milestone' ? <Award className="h-4 w-4 text-yellow-600" /> :
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-medium">{progress.efficacyAnalysis.overall}%</span>
                    </div>
                    ...
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <Badge variant="outline" className={`mt-1 ${
                          risk.level === 'high' ? 'border-red-200 text-red-800' :
                          risk.level === 'medium' ? 'border-yellow-200 text-yellow-800' :
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <div className="font-medium">
              {session.nextSession ? format(session.nextSession, 'dd/MM/yyyy', { locale: ptBR }) : '-'}
            </div>
          </div>
        </div>
