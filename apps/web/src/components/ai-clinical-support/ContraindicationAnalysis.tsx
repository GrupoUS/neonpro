...
export function ContraindicationAnalysis({ 
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'dermatological': return <Eye className="h-4 w-4" />;
      case 'medication': return <Pill className="h-4 w-4" />;
      case 'allergy': return <Leaf className="h-4 w-4" />;
      case 'lifestyle': return <Activity className="h-4 w-4" />;
      case 'procedural': return <Scissors className="h-4 w-4" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className={analysis.overallRiskLevel === 'critical' ? 'border-red-200 bg-red-50' : ''}>
      ...
      {analysis.safetyRecommendation && (
        <Alert className={analysis.isSafeForTreatment ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
          <AlertTriangle className="h-4 w-4" />
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      {measure.requiredTests.length > 0 && (
                        <div>
                          ...
                          <ul className="list-disc list-inside text-sm text-gray-600">
                            {measure.requiredTests.map((test, idx) => (
                              <li key={idx}>{test}</li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {measure.alternativeProcedures.length > 0 && (
                        <div>
                          <h4 className="font-medium text-sm mb-2">Procedimentos alternativos:</h4>
                          <ul className="list-disc list-inside text-sm text-gray-600">
...
function ContraindicationRiskCard({
  ...
  return (
    <Card className={`border-l-4 ${risk.severity === 'critical' ? 'border-l-red-500' : risk.severity === 'high' ? 'border-l-orange-500' : 'border-l-yellow-500'}`}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              {risk.isAbsolute && (
                <Badge variant="destructive">Contraindicação Absoluta</Badge>
              )}
            </div>
            <CardDescription>
              {risk.description}
