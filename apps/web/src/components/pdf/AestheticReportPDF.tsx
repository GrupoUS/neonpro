...
const CheckboxComponent: React.FC<{ checked: boolean; label: string }> = ({ checked, label }) => (
      ...
      <Page size='A4' style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{skinTypeLabels[patientData.skinType]}</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{skinConditionLabels[skinAnalysis.skinCondition]}</Text>
          </View>

          <View style={styles.row}>
            <Text style={styles.label}>Danos Solares:</Text>
            <Text style={styles.value}>{sunDamageLabels[skinAnalysis.sunDamage]}</Text>
          </View>
          ...
          {skinAnalysis.primaryConcerns.map((concern, index) => (
            <Text key={index} style={[styles.value, { marginLeft: 20, marginBottom: 4 }]}>
              • {concern}
            </Text>
          ))}
          ...
          <CheckboxComponent checked={skinAnalysis.acnePresent} label='Presença de Acne' />
          <CheckboxComponent checked={skinAnalysis.melasmaPresent} label='Presença de Melasma' />
          <CheckboxComponent checked={skinAnalysis.wrinklesPresent} label='Presença de Rugas' />
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={[styles.label, { marginBottom: 8 }]}>Condições Médicas:</Text>
          <CheckboxComponent checked={medicalHistory.isPregnant} label='Gravidez' />
          <CheckboxComponent checked={medicalHistory.isBreastfeeding} label='Amamentação' />
          <CheckboxComponent checked={medicalHistory.hasDiabetes} label='Diabetes' />
          <CheckboxComponent checked={medicalHistory.hasAutoimmune} label='Doença Autoimune' />

          {medicalHistory.currentMedications && (
            <View style={[styles.row, { marginTop: 10 }]}>
              <Text style={styles.label}>Medicações Atuais:</Text>
              <Text style={styles.value}>{medicalHistory.currentMedications}</Text>
            </View>
          )}
          ...
          {medicalHistory.previousTreatments && (
            <View style={styles.row}>
              ...
              <Text style={styles.value}>{medicalHistory.previousTreatments}</Text>
            </View>
          )}
        </View>{' '}
        ...
        <Text style={styles.footer}>
          ...
          {generatedAt.toLocaleDateString('pt-BR')} às {generatedAt.toLocaleTimeString('pt-BR')}.
          {'\n'}Documento confidencial - Manter sigilo médico conforme legislação vigente.
        </Text>
