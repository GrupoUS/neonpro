 ...
 */
export function useProfessionalsByServiceType(clinicId: string, serviceTypeId: string) {
  return useQuery({
    queryKey: ['professionals', 'by-service-type', clinicId, serviceTypeId],
    queryFn: () => professionalService.getProfessionalsByServiceType(clinicId, serviceTypeId),
