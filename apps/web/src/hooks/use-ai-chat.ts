...
export interface Conversation {
  ...
  updatedAt: Date;
  patientId?: string;
  context: 'general' | 'patient_analysis' | 'appointment_booking' | 'medical_consultation';
  language: 'pt' | 'en';
  totalCost: number;
  messageCount: number;
}
...
export interface AIInsight {
  type: 'no_show_prediction' | 'patient_analysis' | 'treatment_suggestion' | 'risk_assessment';
  content: string;
  confidence: number;
  evidence: string[];
...
export function useAIChat(conversationId?: string) {
  ...
  const sendMessage = trpc.ai.chat.useMutation({
    onMutate: async ({ message, conversationId: targetConvId, context }) => {
      ...
      if (convId) {
        await queryClient.cancelQueries({ queryKey: aiChatKeys.conversation(convId) });
      }
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (data.cost > 0.50) {
        toast.warning(`Consulta AI custou R$ ${data.cost.toFixed(2)}. Monitore o uso.`);
      }
    },
  ...
  return trpc.ai.predictNoShow.useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.setQueryData(
        aiChatKeys.predictions(),
        (old: any) => {
          const predictions = old || [];
          return [
            ...predictions.filter((p: any) => p.patientId !== variables.patientId),
            {
...
export function useAIProviderRouting() {
  const [currentProvider, setCurrentProvider] = React.useState<'openai' | 'anthropic'>('openai');
  ...
  const routeProvider = trpc.ai.routeProvider.useMutation({
    ...
    onSuccess: data => {
      ...
      if (failoverCount > 0) {
        setFailoverCount(0);
        toast.success(`Conectado ao provedor ${data.selectedProvider.toUpperCase()}`);
      }
    },
    ...
    onError: error => {
      ...
      if (failoverCount < 2) {
        toast.warning(`Tentando provedor alternativo: ${nextProvider.toUpperCase()}`);
      } else {
  ...
  const deleteConversation = trpc.ai.deleteConversation.useMutation({
    onSuccess: (data, variables) => {
      ...
      queryClient.setQueryData(aiChatKeys.conversations(), (old: any) => {
        if (!old) return old;
        return old.filter((conv: Conversation) => conv.id !== variables.conversationId);
      });
