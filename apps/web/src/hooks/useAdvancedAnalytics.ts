...
export function useAdvancedAnalytics(options: UseAdvancedAnalyticsOptions = {}) {
    ...
    return [
      {
        ...
        description: `Padrão identificado: ${Math.random() > 0.5 ? 'Terças' : 'Quintas'}-feiras às ${Math.random() > 0.5 ? '14h' : '16h'} apresentam ${(25 + Math.random() * 15).toFixed(0)}% mais cancelamentos.`,
      ...
      },
      {
        ...
        description: `${(45 + Math.random() * 15).toFixed(0)}% dos pacientes de ${Math.random() > 0.5 ? 'limpeza de pele' : 'hidratação facial'} retornam em ${(20 + Math.random() * 20).toFixed(0)} dias.`,
        confidence: 0.8 + Math.random() * 0.15,
        impact: 'high',
        recommendation: 'Criar pacotes promocionais para tratamentos recorrentes com desconto escalonado.',
        data: { 
      ...
      },
      {
        ...
        description: `${Math.random() > 0.5 ? 'Sextas' : 'Segundas'}-feiras ${Math.random() > 0.5 ? '8h-10h' : '17h-19h'} têm apenas ${(30 + Math.random() * 20).toFixed(0)}% de ocupação.`,
        confidence: 0.85 + Math.random() * 0.1,
        impact: 'medium',
        recommendation: 'Implementar campanhas promocionais direcionadas para horários de baixa demanda.',
        data: { 
  ...
  }, [generateMockInsights, generateMockMetrics, generateMockBrazilianKPIs, generateMockComplianceAudit]);
    ...
    if (enableRealTime) {
      const interval = setInterval(() => {
        ...
        setMetrics(prev => prev ? {
          ...prev,
          attendanceRate: Math.max(0, Math.min(1, prev.attendanceRate + (Math.random() - 0.5) * 0.02)),
  ...
  return {
    ...
    averageConfidence: insights.length > 0 
      ...
      : 0,
    isCompliant: complianceAudit?.lgpdCompliant && complianceAudit?.anvisaCompliant && complianceAudit?.cfmCompliant,
