...
export function useSubscription() {
  ...
  const chatModels = subscription
    ? getModelsForChat(subscription)
    : [];

  // Check if user has pro access
  const hasPro = subscription?.status === 'pro' || subscription?.status === 'trial';
  ...
  const checkModelAccess = useCallback((model: AIModel): boolean => {
  ...
  const subscriptionInfo = {
    ...
    isExpired,
    trialDaysRemaining,
    displayStatus: subscription?.status === 'free'
      ? 'Plano Gratuito'
