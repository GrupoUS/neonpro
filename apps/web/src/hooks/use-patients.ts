 ...
 */
export function usePatient(patientId: string, options?: {
  enabled?: boolean;
}) {
...
export function useWithdrawPatientConsent() {
  ...
  return trpc.patients.withdrawConsent.useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.removeQueries({ queryKey: patientKeys.detail(variables.patientId) });
      queryClient.invalidateQueries({ queryKey: patientKeys.lists() });
...
export function usePrefetchPatients() {
  ...
  const prefetchPatient = React.useCallback((patientId: string) => {
  ...
  const prefetchPatientsList = React.useCallback((options?: {
    page?: number;
    limit?: number;
  }) => {
    queryClient.prefetchQuery({
      queryKey: patientKeys.list(options),
      queryFn: () => trpc.patients.list.fetch({
        page: options?.page || 0,
...
export function useBulkPatientOperations() {
  ...
  const bulkUpdateConsent = trpc.patients.bulkUpdateConsent.useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      toast.success(`Consentimento atualizado para ${variables.patientIds.length} pacientes.`);
    },
    ...
    const unsubscribe = trpc.realtimeTelemedicine.subscribeToPatientUpdates.subscribe(
      { patientId },
      {
        onData: (update) => {
          ...
          if (update.type === 'appointment_scheduled' || update.type === 'appointment_cancelled') {
...
export function usePatientPerformanceMetrics() {
  ...
  React.useEffect(() => {
    ...
    if (performanceMonitor) {
      const patientMetrics = performanceMonitor.getMetrics()
        .filter((m: any) => m.operationType.includes('patient'));

      if (patientMetrics.length > 0) {
        const avgTime = patientMetrics.reduce((sum: number, m: any) => sum + m.duration, 0) / patientMetrics.length;
        const slowOps = patientMetrics.filter((m: any) => m.duration > 2000).length;
