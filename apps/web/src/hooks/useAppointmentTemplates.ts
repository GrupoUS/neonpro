 ...
 */
export function useAppointmentTemplatesByCategory(category: string, clinicId?: string) {
  return useQuery({
...
export function useCreateAppointmentTemplate() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return appointmentTemplatesService.createAppointmentTemplate(templateData, clinicId, userId);
    },
...
export function useUpdateAppointmentTemplate() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return appointmentTemplatesService.updateAppointmentTemplate(templateId, updateData, userId);
    },
    onSuccess: updatedTemplate => {
      ...
      queryClient.invalidateQueries({ queryKey: appointmentTemplateKeys.lists() });
...
export function useDeleteAppointmentTemplate() {
  ...
  return useMutation({
    ...
    onSuccess: (_, templateId) => {
      // Remove from cache
      queryClient.removeQueries({ queryKey: appointmentTemplateKeys.detail(templateId) });
 ...
 */
export function useSearchAppointmentTemplates(searchQuery: string, clinicId?: string) {
  return useQuery({
    queryKey: [...appointmentTemplateKeys.lists(), { search: searchQuery, clinicId }],
    queryFn: () =>
...
export function useAppointmentTemplateCategories(clinicId?: string) {
  const { data: templates } = useAppointmentTemplates({ clinicId, isActive: true });
