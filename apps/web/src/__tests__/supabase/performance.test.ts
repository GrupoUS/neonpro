...
describe("Supabase Performance Tests", () => {
  ...
  const recordMetrics = (
    ...
    const metric: PerformanceMetrics = {
      ...
      passed: HealthcareTestValidators.validatePerformance(
        responseTime,
        ...
        },
        {
          ...
          query: () =>
            testClient
              ...
              .in(
                "doctor_id",
                testDoctors.slice(0, 10).map((d) => d.id),
              )
              .gte("appointment_date", new Date().toISOString())
              .limit(100),
          expectedType: "general_query",
  ...
  describe("Connection Management Performance", () => {
    test("should validate connection pooling under load", async () => {
      ...
      for (const connectionCount of concurrentConnections) {
        ...
        console.log(
          `✅ ${connectionCount} concurrent connections: avg=${avgResponseTime.toFixed(2)}ms, max=${maxResponseTime.toFixed(
            2,
          )}ms`,
        );
      }
  ...
  describe("Caching Strategy Performance", () => {
    test("should validate query result caching", async () => {
      ...
      const cacheTests = [
        {
          ...
          query: () =>
            testClient.from("doctors").select("specialization").distinct(),
        },
        {
          name: "Organization Data Cache",
          query: () =>
            testClient.from("organizations").select("id, name, type, city"),
        },
        {
          name: "Appointment Status Options Cache",
          query: () =>
            testClient.from("appointments").select("status").distinct(),
        },
      ];
      ...
      for (const test of cacheTests) {
        ...
        recordMetrics(
          `${test.name} - Cache Hit 2`,
          responseTime3,
          "general_query",
        );

        console.log(
          `✅ ${test.name}: miss=${responseTime1.toFixed(2)}ms, hit1=${responseTime2.toFixed(
            2,
    ...
    test("should validate cache invalidation strategies", async () => {
      ...
      recordMetrics(
        "Doctor Data - Post Update",
        responseTime2,
        "general_query",
      );

      console.log(
        `✅ Cache Invalidation: initial=${responseTime1.toFixed(2)}ms, post-update=${responseTime2.toFixed(
          2,
        )}ms`,
      );
    });
  ...
  describe("Real-time Performance Monitoring", () => {
    test("should validate real-time subscription performance", async () => {
      ...
      for (const test of subscriptionTests) {
        ...
        console.log(
          `✅ ${test.name}: subscription setup in ${responseTime.toFixed(2)}ms`,
        );
      }
    });
    ...
    test("should validate real-time data synchronization latency", async () => {
      ...
      for (const test of latencyTests) {
        ...
        recordMetrics(
          `${test.name} - Sync Latency`,
          totalLatency,
          "critical_query",
        );

        console.log(
          `✅ ${test.name}: sync latency ${totalLatency.toFixed(2)}ms`,
        );
      }
    });
  ...
  describe("Load Testing Scenarios", () => {
    test("should validate peak hour simulation", async () => {
      ...
      const avgUserTime =
        results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(
        `✅ Peak Load Simulation: ${peakLoadSimulation.concurrentUsers} users, avg=${avgUserTime.toFixed(
          2,
        )}ms/user`,
      );
    });
