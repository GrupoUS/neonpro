...
function collectPerformanceMetrics() {
  ...
  if (performanceMetrics.memoryUsage.length > 0) {
    const avgMemory = performanceMetrics.memoryUsage.reduce((a, b) => a + b, 0) / performanceMetrics.memoryUsage.length;
    const peakMemory = Math.max(...performanceMetrics.memoryUsage);
    
    console.log(`📊 Memory Usage - Avg: ${avgMemory.toFixed(2)}MB, Peak: ${peakMemory.toFixed(2)}MB`);
  }
}
  ...
  if (performanceMetrics.renderTimes.length > 0) {
    ...
    if (avgRenderTime > baselineMetrics.catalogRenderTime * 1.2) {
      regressions.push({
        ...
        increase: ((avgRenderTime - baselineMetrics.catalogRenderTime) / baselineMetrics.catalogRenderTime) * 100
      });
    }
  }
...
export { performanceMetrics, baselineMetrics, PerformanceTestUtils };
...
expect.extend({
  toBePerformant(received: number, threshold: number = 200) {
    ...
    const message = () => 
      `Expected performance to be ${pass ? 'at most' : 'less than'} ${threshold}ms, but received ${received}ms`;
