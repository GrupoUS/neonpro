...
export function createMockUser(type: 'patient' | 'professional' = 'patient'): MockUser {
  const names = {
    ...
    professional: ['Dr. Carlos Roberto Silva', 'Dra. Maria Fernanda Costa', 'Dr. Pedro Henrique Santos']
  };
...
export function createMockAppointment(type: 'consultation' | 'treatment' | 'follow_up'): MockAppointment {
  ...
  return {
    ...
    notes: type === 'consultation' ? 'Avaliação inicial' : type === 'treatment' ? 'Sessão de tratamento' : 'Acompanhamento pós-tratamento',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString()
  };
}

export function createMockPhotoAssessment(type: 'facial_analysis' | 'body_analysis' | 'progress_tracking'): MockPhotoAssessment {
  const skinTypes = ['Normal', 'Seca', 'Oleosa', 'Mista', 'Sensível'];
  const concerns = [
    'Rugas finas', 'Linhas de expressão', 'Manchas', 'Poros dilatados',
    'Flacidez', 'Vermelhidão', 'Acne', 'Textura irregular'
  ];
  
  const recommendations = [
    'Hidratação diária', 'Protetor solar SPF 50+', 'Ácido retinoico',
    'Vitamina C', 'Peeling suave', 'Toxina botulínica', 'Preenchimento'
  ];
...
function generateProfessionalBio(specialty: string): string {
  const bios = {
    dermatologist: 'Médico dermatologista com 15 anos de experiência em tratamentos estéticos faciais e corporais. Especialista em toxina botulínica e preenchimentos.',
    plastic_surgeon: 'Cirurgião plástico especializado em procedimentos estéticos e reconstrutivos. Membro da Sociedade Brasileira de Cirurgia Plástica.',
    aesthetic_nurse: 'Enfermeira estética com foco em procedimentos minimamente invasivos e acompanhamento pós-operatório.'
  };
  
  return bios[specialty as keyof typeof bios] || 'Profissional da área estética com ampla experiência.';
}
