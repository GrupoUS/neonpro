...
describe("Backend API Contracts", () => {
      ...
      const tRPCRouterContract = {
        ...
        aiChat: {
          sendMessage: {
            ...
            output:
              "z.object({ response: z.string(), sources: z.array(z.string()) })",
            middleware: ["auth", "aiSafety", "contentFilter", "auditLog"],
          },
          getHistory: {
            input:
              "z.object({ sessionId: z.string(), limit: z.number().optional() })",
  ...
  describe("AI Chat API Contract", () => {
    ...
    test("should provide AI chat API endpoint contracts", () => {
      ...
      const sendMessageEndpoint = aiChatAPIContracts.find((e) =>
        e.path.includes("send-message"),
      );
      ...
      const escalateEndpoint = aiChatAPIContracts.find((e) =>
        e.path.includes("escalate"),
      );
      ...
      const safetyReportEndpoint = aiChatAPIContracts.find((e) =>
        e.path.includes("safety-report"),
      );
  ...
  describe("Authentication and Authorization Contract", () => {
    ...
    test("should provide API endpoint authorization contract", () => {
      ...
      const aiChatEndpoint = apiEndpointAuthContracts.find((e) =>
        e.endpoint.includes("ai-chat"),
      );
