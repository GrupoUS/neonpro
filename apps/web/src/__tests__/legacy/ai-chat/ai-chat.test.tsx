...
import { AIInputSearch, AILoading, AIPrompt, AIVoice } from '@/components/ui/ai-chat';
...
vi.mock('@/lib/ai/ai-chat-service', () => ({
  streamAestheticResponse: vi.fn(() =>
    Promise.resolve(
      new ReadableStream({
        start(controller) {
          controller.enqueue(
            new TextEncoder().encode('Olá! Como posso ajudar com seus tratamentos estéticos?'),
          );
          controller.close();
        },
      }),
  ...
  processVoiceInput: vi.fn(() => Promise.resolve('Gostaria de agendar uma consulta')),
