...
describe('PDF Export Utils', () => {
  describe('generatePDFFilename', () => {
    it('should generate correct filename for assessment', () => {
      const filename = generatePDFFilename('assessment', 'João Silva', new Date('2024-01-01'));
      expect(filename).toBe('avaliacao_estetica_joao_silva_2024-01-01.pdf');
    });

    it('should generate correct filename for treatment plan', () => {
      const filename = generatePDFFilename('treatment', 'Maria Santos', new Date('2024-02-15'));
    ...
    it('should use current date when not provided', () => {
      ...
      expect(filename).toMatch(/^avaliacao_estetica_test_user_\d{4}-\d{2}-\d{2}\.pdf$/);
    });
  });
});
