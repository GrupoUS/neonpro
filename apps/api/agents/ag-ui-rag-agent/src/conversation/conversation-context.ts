...
export class ConversationContextManager {
  ...
  async createConversation(params: ConversationCreateParams): Promise<ConversationContext> {
    try {
      // Validate session
  ...
  async getConversation(conversationId: string, userId: string): Promise<ConversationContext | null> {
    try {
  ...
  getMemoryUsage(): any {
    return {
      ...
      estimatedMemorySize: Array.from(this.activeContexts.values())
        .reduce((total, conv) => total + JSON.stringify(conv).length, 0)
    }
  }
}