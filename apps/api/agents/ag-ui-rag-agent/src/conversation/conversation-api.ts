...
import { ConversationService, ConversationRequest, ConversationResponse } from './conversation-service'
...
export interface ConversationAPIRequest {
  type: 'start_conversation' | 'continue_conversation' | 'get_history' | 'get_details' | 'delete' | 'search'
  payload: any
  requestId: string
}
...
export class ConversationAPI {
  ...
  async handleRequest(request: ConversationAPIRequest, ws?: WebSocket): Promise<ConversationAPIResponse> {
  ...
  private async checkConversationPermissions(userId: string, clinicId: string, action: string): Promise<boolean> {
    try {
