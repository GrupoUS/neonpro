...
import { ConversationContextManager, ConversationContext, ConversationMessage } from './conversation-context'
...
export class ConversationService {
  ...
  private async findExistingConversation(request: ConversationRequest): Promise<ConversationContext | null> {
    // Look for active conversations in the same session
    const conversations = await this.contextManager.getUserConversations(request.userId, request.clinicId)
