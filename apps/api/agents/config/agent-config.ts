...
export class AgentConfigManager {
  ...
  private createDefaultConfig(): AgentConfig {
    return {
      ...
      agui: {
        ...
        enabled: true,
        compression: true,
        heartbeatInterval: parseInt(process.env.AGUI_HEARTBEAT_INTERVAL || '30000'),
        connectionTimeout: parseInt(process.env.AGUI_CONNECTION_TIMEOUT || '10000'),
        maxMessageSize: parseInt(process.env.AGUI_MAX_MESSAGE_SIZE || '1048576'), // 1MB
      ...
      healthcare: {
        ...
        dataRetention: {
          conversationHistoryDays: parseInt(process.env.CONVERSATION_RETENTION_DAYS || '365'),
          auditLogDays: parseInt(process.env.AUDIT_LOG_RETENTION_DAYS || '1825'), // 5 years
        ...
        emergency: {
          enabled: true,
          escalationTimeout: parseInt(process.env.EMERGENCY_ESCALATION_TIMEOUT || '300000'), // 5 minutes
  ...
  private validateConfig(config: AgentConfig): void {
    ...
    if (!config.database.connectionString) errors.push('Database connection string is required')
    if (!config.ai.model) errors.push('AI model is required')
    if (config.ai.provider === 'openai' && !config.ai.apiKey) errors.push('OpenAI API key is required')

    // Validate ranges
    if (config.ai.maxTokens <= 0) errors.push('AI max tokens must be positive')
    if (config.ai.temperature < 0 || config.ai.temperature > 2) errors.push('AI temperature must be between 0 and 2')
    if (config.rag.chunkSize <= 0) errors.push('RAG chunk size must be positive')
    if (config.agui.heartbeatInterval <= 0) errors.push('AGUI heartbeat interval must be positive')
    if (config.security.sessionTimeout <= 0) errors.push('Session timeout must be positive')
  ...
  private applyEnvironmentOverrides(config: AgentConfig): void {
    ...
    if (process.env.AGENT_DESCRIPTION) config.description = process.env.AGENT_DESCRIPTION
    if (process.env.AGENT_ENVIRONMENT) config.environment = process.env.AGENT_ENVIRONMENT as 'development' | 'staging' | 'production'
  }
