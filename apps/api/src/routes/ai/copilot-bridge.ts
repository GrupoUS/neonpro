...
copilotBridge.post('/chat/completions', async (c) => {
  ...
  try {
    ...
    const { messages, stream = true, model, max_tokens } = body;
    ...
    const agentResponse = await callAGUIAgent(userQuery, userContext, requestId);
    ...
    return c.json({
      error: {
        type: 'internal_error',
        message: 'Ocorreu um erro ao processar sua solicitação. Tente novamente em alguns momentos.',
 ...
 */
function formatCopilotResponse(agentResponse: any, requestId: string, startTime: number) {
    ...
    data: [
      {
        ...
        description: 'NeonPro Healthcare Data Agent with LGPD compliance and Portuguese language support',
        capabilities: {
