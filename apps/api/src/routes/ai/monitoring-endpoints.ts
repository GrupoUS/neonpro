...
class MonitoringService {
  ...
  static async getServicesHealth(): Promise<ServiceHealthSummary[]> {
    ...
    for (const service of this.AI_SERVICES) {
      try {
        ...
        if (response.ok) {
          ...
          healthChecks.push({
            ...
            uptime_percentage: this.calculateUptimePercentage(service),
  ...
  static async getServiceMetrics(timeRange: string = '1h'): Promise<ServiceMetricsData[]> {
    try {
      ...
      const aggregatedMetrics = this.aggregateMetricsByTimeInterval(metricsData, minutes);
  ...
  private static aggregateMetricsByTimeInterval(metricsData: any[], totalMinutes: number): ServiceMetricsData[] {
    ...
    return Object.entries(intervals).map(([timestamp, metrics]) => {
      ...
      return {
        ...
        avg_response_time: serviceData.response_times.reduce((a: number, b: number) => a + b, 0) / serviceData.response_times.length,
  ...
  static async getComplianceAlerts(activeOnly: boolean = true): Promise<ComplianceAlert[]> {
    try {
  ...
  static async getSystemOverview(): Promise<any> {
    try {
      ...
      const errorRate = metrics.reduce((sum, m) => 
        sum + (m.error_count / (m.error_count + m.success_count) * 100), 0) / metrics.length || 0;
...
monitoring.post('/service-metrics', async (c) => {
  try {
    ...
    const responseTimeCount = metricsData
      .filter(m => m.metric_name === 'response_time').length;
