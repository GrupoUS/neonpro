...
describe("POST /api/v2/ai/chat endpoint (T051)", () => {
  beforeEach(async () => {
    ...
    mockAIChatService.sendMessage.mockResolvedValue({
      ...
      data: {
        ...
        response: {
          content:
            "Olá! Como posso ajudá-lo com questões relacionadas à estética e saúde hoje?",
          model: "gpt-4",
          confidence: 0.95,
          tokens: {
            input: 25,
  ...
  describe("Successful AI Chat Operations", () => {
    it("should send message to AI with default model", async () => {
      ...
      const chatData = {
        message:
          "Olá, preciso de informações sobre tratamentos de rejuvenescimento facial.",
        conversationId: "conv-123",
        context: {
          healthcareContext: true,
          language: "pt-BR",
    ...
    it("should include patient context when provided", async () => {
      ...
      const chatData = {
        message:
          "Com base no histórico da paciente, qual tratamento você recomendaria?",
      ...
      setServices({
        ...
        lgpdService: {
          ...
          maskSensitiveData: vi.fn().mockReturnValue({
            ...
            response: {
              content:
                "Informações sobre tratamento com dados sensíveis mascarados: ***",
              model: "gpt-4",
              confidence: 0.95,
