    ...
    try {
      ...
      if (ottomatorResponse.success && ottomatorResponse.response) {
        ...
        const formattedResponse = {
          id: crypto.randomUUID(),
          type:
            ottomatorResponse.response.type === "structured" ? "table" : "text",
          content: {
            title: "Resultado da Consulta",
            text: ottomatorResponse.response.content,
            data:
              ottomatorResponse.response.type === "structured"
                ? JSON.parse(ottomatorResponse.response.content)
                : null,
          },
          metadata: {
            processingTime:
              ottomatorResponse.metadata?.processingTimeMs ||
              Date.now() - startTime,
            confidence:
              ottomatorResponse.response.sources?.[0]?.confidence || 0.8,
        ...
        return {
          ...
          content: {
            ...
            data: data.map((client) => ({
              ...
              cadastrado_em: new Date(client.created_at).toLocaleDateString(
                "pt-BR",
              ),
            })),
            columns: [
