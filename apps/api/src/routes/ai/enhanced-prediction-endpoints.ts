...
import { 
  enhancedNoShowPredictionService,
  type PatientProfile,
  ...
  type ROIMetrics
} from '@neonpro/ai/services/enhanced-no-show-prediction-service';
...
const ExternalFactorsSchema = z.object({
...
const BulkPredictionSchema = z.object({
  ...
  })).min(1).max(100), // Limit bulk predictions for performance
});
...
const PredictionResponseSchema = z.object({
  ...
  ensemble_details: z.object({
    models_used: z.array(z.string()),
    ensemble_method: z.string(),
    calibrated: z.boolean(),
    prediction_intervals: z.object({
  ...
  async (c) => {
    ...
    try {
      ...
      if (processingTime > 200) {
        console.warn(`Prediction took ${processingTime.toFixed(2)}ms - over 200ms target`);
      }
      ...
      const response = {
        ...
        intervention_strategies: predictionResult.recommended_interventions
          .map(ri => ({
      ...
      console.error(`Enhanced prediction failed for request ${requestId}:`, error);
...
enhancedPredictionRoutes.post(
  ...
  zValidator('json', z.object({
    predictions: z.array(z.object({
      ...
      appointment_context: AppointmentContextSchema,
      external_factors: ExternalFactorsSchema,
    })).min(1).max(50), // Limit for performance
    options: z.object({
      priority: z.enum(['standard', 'high']).default('standard'),
  ...
  async (c) => {
    ...
    try {
      ...
      const predictionPromises = predictions.map(async (predRequest, index) => {
        try {
          const predictionResult = await enhancedNoShowPredictionService.getEnhancedPredictionWithROI(
          ...
          console.error(`Bulk prediction failed for appointment ${predRequest.appointment_id}:`, error);
      ...
      const avgProbability = results
        .filter(r => r.success)
        .reduce((sum, r) => sum + r.no_show_probability, 0) / Math.max(1, successCount);
      ...
      console.error(`Bulk prediction failed for batch ${batchId}:`, error);
 ...
 */
enhancedPredictionRoutes.get('/metrics',
  cache({
    ...
    try {
      ...
      return c.json({
        ...
        target_compliance: {
          ...
          roi_target_annual: 150000,
          roi_projected_annual: metrics.roi_summary.yearly_projection,
          roi_target_met: metrics.roi_summary.yearly_projection >= 150000,
        },
        health_status: {
          overall: 'healthy',
      ...
      console.error('Failed to get enhanced prediction metrics:', error);
 ...
 */
enhancedPredictionRoutes.get('/health',
  cache({
  ...
  async (c) => {
    ...
    try {
      ...
      console.error('Enhanced prediction health check failed:', error);
