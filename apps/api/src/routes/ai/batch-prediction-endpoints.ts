...
batchPredictionRoutes.use('*', async (c, next) => {
  ...
  console.log(`[BATCH-API] ${method} ${path} - ${responseStatus} - ${processingTime.toFixed(2)}ms`);

  // Log slow requests (>2000ms)
  if (processingTime > 2000) {
    console.warn(`[BATCH-API-SLOW] ${method} ${path} took ${processingTime.toFixed(2)}ms`);
  }
});
...
batchPredictionRoutes.post('/jobs',
...
batchPredictionRoutes.get('/jobs',
...
batchPredictionRoutes.get('/analytics', async (c) => {
  ...
  try {
    ...
    const totalPredictionsProcessed = completedJobs.reduce((sum, job) => 
      sum + job.progress.processed_appointments, 0);
    ...
    const analytics = {
      overview: {
        ...
        success_rate_percentage: recentJobs.length > 0 ? 
          Math.round((completedJobs.length / recentJobs.length) * 100) : 0
      },
      performance: {
        avg_processing_time_ms: Math.round(avgProcessingTime),
        total_predictions_processed: totalPredictionsProcessed,
        avg_predictions_per_job: completedJobs.length > 0 ? 
          Math.round(totalPredictionsProcessed / completedJobs.length) : 0,
        estimated_throughput_per_hour: totalPredictionsProcessed > 0 && avgProcessingTime > 0 ?
          Math.round((totalPredictionsProcessed / avgProcessingTime) * 3600000) : 0
      },
      job_types: {
...
batchPredictionRoutes.post('/jobs/bulk',
  ...
  async (c) => {
    ...
    try {
      ...
      const processingTime = performance.now() - startTime;
      console.error('[BATCH-BULK-CREATE-ERROR]', error);
