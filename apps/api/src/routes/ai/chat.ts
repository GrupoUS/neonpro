  ...
  async (c) => {
    ...
    try {
      ...
      if (requestData.patientId && requestData.context?.includePatientHistory) {
        const patientData =
          await currentServices.patientService.getPatientContext({
            patientId: requestData.patientId,
            userId: _user.id,
            includeHistory: true,
          });
      ...
      if (lgpdValidation.data?.accessLevel === "limited") {
        responseData =
          currentServices.lgpdService.maskSensitiveData(responseData);
      }
      ...
      if (!requestData.streaming) {
        ...
        if (responseData.metadata?.fallbackUsed) {
          ...
          if (responseData.metadata.originalModel) {
            responseHeaders["X-AI-Original-Model"] =
              responseData.metadata.originalModel;
          }
          if (responseData.metadata.fallbackReason) {
            responseHeaders["X-AI-Fallback-Reason"] =
              responseData.metadata.fallbackReason;
          }
        }
      }
...
app.post(
  ...
  async (c) => {
    ...
    try {
      ...
      const hasLGPDConsent =
        requestData.lgpdConsent === true ||
        ...
        return c.json(
          {
            success: false,
            error:
              "Consentimento LGPD obrigatório para processamento de dados de IA.",
      ...
      if (
        ...
        if (requestData.context?.healthcare?.specialty) {
          responseData.context.specialty =
            requestData.context.healthcare.specialty;
        }
      }
  ...
  async (c) => {
    ...
    try {
      ...
      const isValidSession =
        validTestSessions.includes(sessionId) ||
      ...
      if (requestData.attachments && requestData.attachments.length > 0) {
        for (const attachment of requestData.attachments) {
          ...
          const attachmentType =
            attachment.metadata?.mimeType || attachment.type;

          if (attachmentType) {
            const isSpecificTypeSupported =
              supportedTypes.includes(attachmentType);
            const isGenericTypeSupported =
              supportedGenericTypes.includes(attachmentType);
      ...
      const containsPortuguese =
        /português|brasil|anvisa|cfm|lgpd|toxina|botulínica/i.test(
          requestData.content,
        );
