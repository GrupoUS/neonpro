...
exportRouter.post('/export', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
...
exportRouter.get('/export/:jobId', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
    const userId = c.get('jwtPayload').sub
    if (!userId) {
      return c.json({ error: 'Usuário não autenticado' }, 401)
...
exportRouter.delete('/export/:jobId', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
    const userId = c.get('jwtPayload').sub
    if (!userId) {
      return c.json({ error: 'Usuário não autenticado' }, 401)
...
exportRouter.get('/export/:jobId/download', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
    const userId = c.get('jwtPayload').sub
    if (!userId) {
      return c.json({ error: 'Usuário não autenticado' }, 401)
...
exportRouter.get('/export', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
    const userId = c.get('jwtPayload').sub
    if (!userId) {
      return c.json({ error: 'Usuário não autenticado' }, 401)
...
exportRouter.get('/export/:jobId/metrics', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
    const userId = c.get('jwtPayload').sub
    if (!userId) {
      return c.json({ error: 'Usuário não autenticado' }, 401)
...
exportRouter.get('/export/meta/fields', jwt({ secret: process.env.JWT_SECRET || 'default-secret' }), async (c) => {
  try {
