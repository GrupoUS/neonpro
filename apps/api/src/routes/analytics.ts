...
export const analyticsRoutes = new Hono()
  ...
  .get(
    ...
    async (context) => {
      ...
      try {
        ...
        const prevCompletedAppointments = prevAppointments?.filter(apt => apt.status === 'completed') || [];
        const prevTotalRevenue = prevCompletedAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0);
        ...
        const prevAverageTicket = prevTotalAppointments > 0 ? prevTotalRevenue / prevCompletedAppointments.length : 0;

        // Calculate growth percentages
        const revenueGrowth = prevTotalRevenue > 0 ? ((totalRevenue - prevTotalRevenue) / prevTotalRevenue) * 100 : 0;
        const appointmentsGrowth = prevTotalAppointments > 0 ? ((totalAppointments - prevTotalAppointments) / prevTotalAppointments) * 100 : 0;
        const patientsGrowth = prevUniquePatients > 0 ? ((uniquePatients - prevUniquePatients) / prevUniquePatients) * 100 : 0;
        ...
        const recentActivity = recentAppointments?.map((apt, index) => ({
          ...
          type: apt.status === 'completed' ? 'appointment_completed' : 
                apt.status === 'scheduled' ? 'appointment_booked' : 'appointment_updated',
  ...
  .get("/revenue", zValidator("query", RevenueQuerySchema), async (context) => {
    ...
    try {
      ...
      const prevTotalRevenue = prevAppointments?.reduce((sum, apt) => sum + (apt.total_amount || 0), 0) || 0;
      const revenueGrowth = prevTotalRevenue > 0 ? ((totalRevenue - prevTotalRevenue) / prevTotalRevenue) * 100 : 0;
    ...
    async (context) => {
      ...
      try {
        ...
        const hourlyDistribution = appointments?.reduce((acc, apt) => {
          const hour = apt.appointment_time.split('T')[1].substring(0, 5);
        ...
        const overallDuration = durations.length > 0 ? Math.round(durations.reduce((sum, d) => sum + d, 0) / durations.length) : 0;
  ...
  .get("/patients", async (context) => {
    try {
      ...
      const returning = patients?.filter(patient => {
        const completedAppointments = patient.appointments?.filter(apt => apt.status === 'completed').length || 0;
      ...
      const averageLTV = lifetimeValues.length > 0 
        ? Math.round(lifetimeValues.reduce((sum, val) => sum + val, 0) / lifetimeValues.length * 100) / 100
        : 0;
          ...
          segments: [
            {
              ...
              average: premiumAvg.length > 0 ? Math.round(premiumAvg.reduce((sum, val) => sum + val, 0) / premiumAvg.length * 100) / 100 : 0
            },
            {
              segment: 'regular',
              count: regularCustomers,
              average: regularAvg.length > 0 ? Math.round(regularAvg.reduce((sum, val) => sum + val, 0) / regularAvg.length * 100) / 100 : 0
            },
            {
              segment: 'occasional',
              count: occasionalCustomers,
              average: occasionalAvg.length > 0 ? Math.round(occasionalAvg.reduce((sum, val) => sum + val, 0) / occasionalAvg.length * 100) / 100 : 0
            },
          ],
        },
      };
  ...
  .get("/performance", async (context) => {
    try {
      ...
      const completedAppointments = currentMonthAppointments.filter(apt => apt.status === 'completed');
      ...
      const patientSatisfaction = ratingsWithValues.length > 0 
        ? Math.round(ratingsWithValues.reduce((sum, apt) => sum + (apt.rating || 0), 0) / ratingsWithValues.length * 10) / 10
      ...
      const appointmentUtilization = Math.round((currentMonthAppointments.length / totalSlots) * 100 * 10) / 10;
      ...
      const conversionRate = consultationAppointments.length > 0 
        ? Math.round((treatmentAppointments.length / consultationAppointments.length) * 100 * 10) / 10
        : 0;
      ...
      const repeatCustomers = Object.values(patientAppointmentCounts).filter(count => count > 1).length;
      const repeatCustomerRate = Object.keys(patientAppointmentCounts).length > 0 
        ? Math.round((repeatCustomers / Object.keys(patientAppointmentCounts).length) * 100 * 10) / 10
        : 0;
      ...
      const bookingLeadTimes = currentMonthAppointments
        ...
        .map(apt => {
          ...
          return Math.abs(appointmentDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24);
        });
      ...
      const adherenceTimes = appointmentsWithTimes.filter(apt => {
        ...
        const actualDuration = (new Date(apt.actual_end_time!).getTime() - new Date(apt.actual_start_time!).getTime()) / (1000 * 60);
      ...
      const professionalUtilization = Math.round((completedAppointments.length / (professionals?.length || 1) / 22) * 100 * 10) / 10;

      const cancelledAppointments = currentMonthAppointments.filter(apt => apt.status === 'cancelled');
      ...
      const cancellationLeadTime = cancellationLeadTimes.length > 0 
        ? Math.round(cancellationLeadTimes.reduce((sum, hours) => sum + hours, 0) / cancellationLeadTimes.length * 10) / 10
        : 0;
      ...
      try {
        ...
        switch (reportType) {
          case 'revenue': {
            ...
            reportData = {
              totalRevenue,
              averageTicket: totalRecords > 0 ? Math.round((totalRevenue / totalRecords) * 100) / 100 : 0,
            ...
            reportData = {
              ...
              results: patientsData?.map(patient => {
                const totalSpent = patient.appointments?.reduce((sum, apt) => 
                  sum + (apt.status === 'completed' ? (apt.total_amount || 0) : 0), 0
                ) || 0;
          ...
          case 'services': {
            ...
            const serviceStats = servicesData?.map(service => {
              const completedAppointments = service.appointments?.filter(apt => apt.status === 'completed') || [];
              const totalRevenue = completedAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0);
              ...
              return {
                ...
                totalRevenue,
                averageRevenue: completedAppointments.length > 0 ? Math.round((totalRevenue / completedAppointments.length) * 100) / 100 : 0
              };
            }) || [];
          ...
          case 'professionals': {
            ...
            const professionalStats = professionalsData?.map(professional => {
              const completedAppointments = professional.appointments?.filter(apt => apt.status === 'completed') || [];
              const totalRevenue = completedAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0);
              const ratingsWithValues = completedAppointments.filter(apt => apt.rating && apt.rating > 0);
