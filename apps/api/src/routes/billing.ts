...
const CreateBillingSchema = z.object({
  ...
  paymentMethod: z.enum(['CASH', 'PIX', 'CREDIT_CARD', 'DEBIT_CARD', 'BANK_TRANSFER', 'HEALTH_PLAN', 'SUS']),
...
const PaymentProcessingSchema = z.object({
  ...
  paymentMethod: z.enum(['CASH', 'PIX', 'CREDIT_CARD', 'DEBIT_CARD', 'BANK_TRANSFER', 'HEALTH_PLAN']),
  amount: z.number().positive(),
 ...
 */
billing.post('/:id/payment',
  requireAuth,
  requirePermission('billing_payment'),
  async (c) => {
    try {
      const billingId = c.req.param('id')
      const userId = c.get('userId')
