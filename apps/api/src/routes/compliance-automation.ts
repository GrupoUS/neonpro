...
export const complianceAutomationRoutes = new Hono()
  ...
  .post("/execute", zValidator("json", ComplianceExecutionSchema), async (c) => {
    ...
    try {
      ...
      console.error('Compliance automation execution error:', error);
      return c.json(
        {
          success: false,
  ...
  .get("/status/:tenant_id", async (c) => {
    ...
    try {
      ...
      if (monitorError && monitorError.code !== "PGRST116") {
        console.warn('Monitor query error:', monitorError);
      }
      ...
      const response: ApiResponse<{
        ...
        data: {
          ...
          status_summary: {
            overall_compliant: latestAssessment?.overall_status === "compliant" || latestAssessment?.overall_score >= 9.9,
  ...
  .get("/history/:tenant_id", async (c) => {
    ...
    try {
      ...
      if (complianceHistory && complianceHistory.length >= 2) {
        ...
        if (trendPercentage > 1) trends.score_trend = "improving";
        else if (trendPercentage < -1) trends.score_trend = "declining";
      }
  ...
  .get("/alerts/:tenant_id", async (c) => {
    ...
    try {
      ...
      const alertsSummary = {
        ...
        critical: alerts?.filter(a => a.severity === "critical" || a.severity === "constitutional_violation").length || 0,
    ...
    try {
      ...
      const reportSummary = {
        ...
        compliance_overview: {
          ...
          average_score: reportData?.length ? reportData.reduce((sum, r) => sum + r.overall_score, 0) / reportData.length : 9.9,
          constitutional_compliance_rate: reportData?.length ? (reportData.filter(r => r.overall_score >= 9.9).length / reportData.length) * 100 : 100,
