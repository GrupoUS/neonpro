...
const ComplianceAlertInputSchema = z.object({
  clinicId: z.string(),
  alertType: z.enum(["consent_expiry", "license_expiry", "assessment_due", "data_breach", "compliance_violation"]),
  severityLevel: z.enum(["low", "medium", "high", "critical"]),
...
const ComplianceReportInputSchema = z.object({
  clinicId: z.string(),
  reportType: z.enum(["lgpd_summary", "anvisa_compliance", "license_status", "data_breach", "assessment_summary"]),
  ...
  getComplianceDashboard: {
    ...
    resolve: async ({ input, ctx }) => {
      try {
        ...
        const complianceScore = totalAssessments > 0 ? (passedAssessments / totalAssessments) * 100 : 0;
          ...
          if (!acc[body]) {
            acc[body] = {
              ...
              complianceScore: bodyRequirements.length > 0 
                ? (bodyAssessments.filter(a => a.status === 'passed').length / bodyRequirements.length) * 100 
                : 0
            };
          }
