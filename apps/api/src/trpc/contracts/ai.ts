...
export const aiRouter = router({
  ...
  chat: protectedProcedure
    .meta({
      description:
        "AI chat completion with healthcare context and LGPD compliance",
    ...
    .mutation(async ({ input, _ctx }) => {
      ...
      const aiRequest = {
        ...
        messages: [
          {
            _role: "system",
            content: `Healthcare AI Assistant for clinic ${input.clinicId}. User: ${ctx.userId}. Patient _context: ${
          ...
          return {
            ...
            lastMessage: lastMessageInfo
              ? {
                  ...
                  content:
                    lastMessageInfo.content.substring(0, 100) +
  ...
  healthAnalysis: protectedProcedure
    ...
    .mutation(async ({ input, _ctx }) => {
      ...
      if (!hasHealthAnalysisPermission(ctx.user._role)) {
        throw new HealthcareTRPCError(
          "FORBIDDEN",
      ...
      try {
        ...
        const analysisResult =
          healthAnalysisService.parseHealthAnalysisResponse(aiResponse.content);
