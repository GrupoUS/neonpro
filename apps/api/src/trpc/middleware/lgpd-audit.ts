...
const DATA_MINIMIZATION_RULES = {
  ...
  appointment: ['id', 'fullName', 'phone', 'email', 'emergencyContactName', 'emergencyContactPhone'],
  emergency: ['id', 'fullName', 'phone', 'bloodType', 'allergies', 'emergencyContactName', 'emergencyContactPhone', 'currentMedications']
} as const;
...
function classifyDataCategory(path: string, input: any): string {
  if (path.includes('patient')) {
    if (input?.bloodType || input?.allergies || input?.chronicConditions || input?.currentMedications) {
...
function applyDataMinimization(data: any, operationType: string, userRole?: string): any {
  ...
  const allowedFields = DATA_MINIMIZATION_RULES[operationType as keyof typeof DATA_MINIMIZATION_RULES] || 
...
function calculateRiskLevel(path: string, type: string, dataCategory: string): RiskLevel {
  if (dataCategory === LGPD_DATA_CATEGORIES.HEALTH || dataCategory === LGPD_DATA_CATEGORIES.SENSITIVE) {
