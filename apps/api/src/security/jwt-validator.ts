...
export class JWTSecurityValidator {
  ...
  async validateToken(token: string, _context?: Context): Promise<JWTValidationResult> {
    try {
      ...
      const healthcareResult = this.validateHealthcareRequirements(verifyResult.payload!);
   ...
   */
  private verifyTokenSignature(token: string, header: any): JWTValidationResult {
    try {
      const { kid } = header;
      const secret = kid ? this.keyStore.get(kid) : this.keyStore.get('default');
