...
class SQLSanitizer {
  ...
  private static readonly ALLOWED_TABLES: TableConfig[] = [
    {
      ...
      sensitiveColumns: ['cpf', 'rg', 'medical_record_number', 'phone', 'email'],
    ...
    },
    {
      ...
      allowedOperations: ['SELECT', 'INSERT', 'UPDATE'],
      sensitiveColumns: ['patient_id', 'diagnosis', 'treatment', 'notes', 'attachments'],
      requiresPatientConsent: true,
      lgpdProtected: true,
      auditRequired: true
    },
  ...
  private static readonly ALLOWED_FUNCTIONS = [
    ...
    'NOW', 'CURRENT_TIMESTAMP', 'DATE', 'TIME',
    'CAST', 'CONVERT'
  ];
  ...
  static validateOperation(operation: SQLOperation): SQLValidationResult {
    ...
    try {
      ...
      if (!columnValidation.isValid) {
        ...
        result.riskLevel = Math.max(result.riskLevel, columnValidation.riskLevel) as any;
      }
  ...
  private static validateTableAccess(operation: SQLOperation): SQLValidationResult {
    const result: SQLValidationResult = {
      isValid: false,
      errors: [],
      warnings: [],
    ...
    const tableConfig = this.ALLOWED_TABLES.find(t => t.name === operation.table);
    
    if (!tableConfig) {
      result.errors.push(`Table '${operation.table}' is not in the allowed tables list`);
      result.riskLevel = 'CRITICAL';
      return result;
    }
  ...
  private static validateOperationType(operation: SQLOperation): SQLValidationResult {
    const result: SQLValidationResult = {
      isValid: false,
      errors: [],
      warnings: [],
    ...
    if (!tableConfig.allowedOperations.includes(operation.type)) {
      result.errors.push(`Operation '${operation.type}' is not allowed on table '${operation.table}'`);
      result.riskLevel = 'HIGH';
      return result;
    }
  ...
  private static validateColumns(operation: SQLOperation): SQLValidationResult {
    ...
    operation.columns.forEach(column => {
      ...
      if (tableConfig.sensitiveColumns.includes(column)) {
        result.warnings.push(`Accessing sensitive column '${column}' - ensure LGPD compliance`);
  ...
  private static validateConditions(operation: SQLOperation): SQLValidationResult {
    const result: SQLValidationResult = {
      isValid: true,
      errors: [],
      warnings: [],
    ...
    Object.entries(operation.conditions).forEach(([key, value]) => {
      ...
      if (typeof value === 'string' && this.containsDangerousPatterns(value)) {
        result.errors.push(`Condition value for '${key}' contains dangerous patterns`);
        result.riskLevel = 'CRITICAL';
        result.isValid = false;
      }
    });
  ...
  private static validateLGPDCompliance(operation: SQLOperation): SQLValidationResult {
    const result: SQLValidationResult = {
      isValid: true,
      errors: [],
      warnings: [],
    ...
    if (tableConfig.requiresPatientConsent && !operation.patientId) {
      result.warnings.push('Operation on LGPD-protected data without patient ID - ensure consent validation');
      result.riskLevel = 'MEDIUM';
    }
  ...
  private static auditSQLOperation(operation: SQLOperation, result: SQLValidationResult): void {
    const tableConfig = this.ALLOWED_TABLES.find(t => t.name === operation.table)!;
  ...
  static isOperationAllowed(operationType: string, tableName: string): boolean {
    ...
    return tableConfig.allowedOperations.includes(operationType as SQLOperation['type']);
  }
...
export { SQLSanitizer, type SQLOperation, type SQLValidationResult, type TableConfig };