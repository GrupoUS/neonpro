...
export class EnhancedSessionManager {
   ...
   */
  private updateSessionValidationResult(session: EnhancedSessionMetadata, isValid: boolean): void {
    if (isValid) {
  ...
  private manageConcurrentSessions(userId: string, currentSessionId: string): {
    ...
    if (userSessions.length === this.config.maxConcurrentSessions) {
      ...
      const existingSession = userSessions.find(s => s.sessionId === currentSessionId);
      if (existingSession) {
  ...
  validateAndUpdateSession(
    ...
    let finalAction: 'allow' | 'warn' | 'block' | 'require_verification' | 'require_mfa' = 'allow';
    ...
    if (anomalyResult.hasAnomaly) {
      ...
      } else if (anomalyResult.recommendedAction === 'warn' && finalAction === 'allow') {
        finalAction = 'warn';
      }
  ...
  cleanExpiredSessions(maxInactiveHours?: number): number {
    const cutoffTime = new Date(Date.now() - (maxInactiveHours || 24) * 60 * 60 * 1000);
