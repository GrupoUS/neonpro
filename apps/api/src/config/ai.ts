...
export async function streamWithFailover(opts: {
  ...
  const { model, allowExperimental, messages, temperature = 0.7, maxOutputTokens = 1000, mock } = opts
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        const id = setInterval(() => {
          ...
          i++
          if (i >= chunks.length) { clearInterval(id); controller.close() }
        }, 10)
      }
    })
  ...
  try {
    ...
    const result = await streamText({ model: adapter(chosen), messages, temperature, maxOutputTokens })
    return result.toTextStreamResponse({ headers: { 'X-Chat-Model': `${MODEL_REGISTRY[chosen].provider}:${chosen}`, 'X-Data-Freshness': 'as-of-now' } })
  } catch (primaryError) {
    console.error('Primary AI provider failed:', primaryError)
    try {
      const { adapter } = resolveProvider(DEFAULT_SECONDARY)
      const result = await streamText({ model: adapter(DEFAULT_SECONDARY), messages, temperature, maxOutputTokens })
      return result.toTextStreamResponse({ headers: { 'X-Chat-Model': `google:${DEFAULT_SECONDARY}` } })
    } catch (fallbackError) {
