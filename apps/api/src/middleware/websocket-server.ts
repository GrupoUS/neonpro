...
import { createAguiService, AguiService } from '../services/agui-protocol';
...
export class WebSocketServerMiddleware {
  ...
  async getHealthStatus(): Promise<any> {
    try {
      ...
      return {
        websocketServer: { connectedClients: this.wss.clients.size, isInitialized: this.isInitialized },
  ...
  getMetrics(): any {
    const serverMetrics = {
      ...
      connectionUtilization: this.config.maxConnections ? 
        (this.wss.clients.size / this.config.maxConnections) * 100 : 0
    };
