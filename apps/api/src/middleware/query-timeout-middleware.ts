...
export class QueryTimeoutMiddleware {
  ...
  middleware = (req: Request, res: Response, next: NextFunction): void => {
    ...
    res.end = function(chunk?: any, encoding?: any) {
      ...
      if (duration > timeout * 0.8 && duration < timeout) {
        console.warn(`[QueryTimeout] Query approaching timeout: ${duration}ms / ${timeout}ms`, {
          queryId,
          route: req.path,
          method: req.method,
   ...
   */
  private handleQueryTimeout(queryId: string, req: Request, res: Response): void {
    ...
    // Log timeout event
    console.error(`[QueryTimeout] Query timeout exceeded: ${duration}ms > ${timeout}ms`, {
      queryId,
      route: req.path,
      method: req.method,
      userId: metrics.userId,
  ...
  getRealTimeMetrics() {
    return {
      activeQueries: Array.from(this.activeQueries.entries()).map(([id, query]) => ({
        queryId: id,
  ...
  private generateHealthRecommendations(stats: any): string[] {
    ...
    if (stats.averageResponseTime > 1500) {
      recommendations.push('Average response time is high - investigate query performance');
    }

    if (stats.currentActiveQueries > 50) {
      recommendations.push('High number of active queries - consider scaling or load balancing');
    }
