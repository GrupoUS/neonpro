...
class WebSocketManager {
  ...
  private setupConnectionHandlers(connectionId: string, ws: WebSocket, metadata: ConnectionMetadata) {
...
export function websocketMiddleware() {
  return async (c: Context, next: Next) => {
    ...
    c.set('sendToUser', (userId: string, message: WSMessage) => wsManager.sendToUser(userId, message));
    c.set('broadcast', (message: WSMessage, filter?: (metadata: ConnectionMetadata) => boolean) => 
