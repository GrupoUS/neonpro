...
describe("Caching Middleware", () => {
  ...
  describe("createCacheMiddleware", () => {
    it("should cache GET requests for public services", async () => {
      ...
      app.get("/api/services/list", (c) =>
        c.json({ services: ["service1", "service2"] }),
      );
    ...
    it("should not cache sensitive patient data", async () => {
      ...
      app.get("/api/patients/123", (c) =>
        c.json({ patient: { id: 123, name: "John Doe" } }),
      );
    ...
    it("should respect cache TTL", async () => {
      ...
      app.use("/api/services/*", middleware);
      app.get("/api/services/ttl-test", (c) =>
        c.json({ timestamp: Date.now() }),
      );
