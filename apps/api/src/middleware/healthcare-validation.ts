...
const relationshipTypes = [
  'pai', 'mae', 'irmao', 'irma', 'filho', 'filha', 'conjuge', 
  'companheiro', 'companheira', 'responsavel', 'outro'
];
...
export const PatientDataSchemas = {
  ...
  personalInfo: z.object({
    ...
    dateOfBirth: z.string()
      .refine(date => {
        ...
        const actualAge = monthDiff < 0 || (monthDiff === 0 && now.getDate() < parsedDate.getDate()) ? age - 1 : age;
    ...
    phone: z.string()
      .regex(brazilianPhoneRegex, 'Telefone deve estar no formato +55 (00) 90000-0000'),
      
    alternativePhone: z.string()
      .regex(brazilianPhoneRegex, 'Telefone alternativo deve estar no formato +55 (00) 90000-0000')
      .optional(),
      
    // Address (Brazilian format)
    address: z.object({
      street: z.string().min(5, 'Endereço muito curto').max(200, 'Endereço muito longo'),
      number: z.string().max(10, 'Número muito longo'),
      complement: z.string().max(100, 'Complemento muito longo').optional(),
      neighborhood: z.string().min(2, 'Bairro obrigatório').max(100, 'Bairro muito longo'),
    ...
    gender: z.enum(['masculino', 'feminino', 'outro', 'nao_informar'], {
      errorMap: () => ({ message: 'Gênero deve ser: masculino, feminino, outro, ou nao_informar' })
    }).optional(),
    ...
    // Nationality
    nationality: z.string().max(50, 'Nacionalidade muito longa').default('brasileira'),
  }),
  ...
  medicalInfo: z.object({
    ...
    allergies: z.array(z.object({
      ...
      severity: z.enum(['leve', 'moderada', 'grave'], {
        errorMap: () => ({ message: 'Gravidade deve ser: leve, moderada, ou grave' })
      }),
      reaction: z.string().max(200, 'Descrição da reação muito longa').optional(),
    ...
    medications: z.array(z.object({
      ...
      prescribingDoctor: z.string().max(100, 'Nome do médico muito longo').optional(),
    ...
    chronicConditions: z.array(z.object({
      ...
      status: z.enum(['ativa', 'controlada', 'remissao'], {
        errorMap: () => ({ message: 'Status deve ser: ativa, controlada, ou remissao' })
      }),
    ...
    surgeries: z.array(z.object({
      ...
      date: z.string(),
      hospital: z.string().max(150, 'Nome do hospital muito longo').optional(),
      surgeon: z.string().max(100, 'Nome do cirurgião muito longo').optional(),
      complications: z.string().max(300, 'Descrição das complicações muito longa').optional(),
    ...
    lifestyle: z.object({
      smoker: z.enum(['nunca', 'ex_fumante', 'fumante_ocasional', 'fumante_regular']).optional(),
      alcohol: z.enum(['nunca', 'ocasionalmente', 'moderadamente', 'frequentemente']).optional(),
      physicalActivity: z.enum(['sedentario', 'leve', 'moderada', 'intensa']).optional(),
      diet: z.enum(['normal', 'vegetariana', 'vegana', 'restricoes_medicas']).optional(),
    }).optional(),
  ...
  lgpdConsent: z.object({
    ...
    dataProcessing: z.literal(true, { 
      errorMap: () => ({ message: 'Consentimento para processamento de dados obrigatório (LGPD Art. 7º)' })
    }),
    medicalTreatment: z.literal(true, {
      errorMap: () => ({ message: 'Consentimento para tratamento médico obrigatório' })
    }),
    ...
    rightsAcknowledged: z.literal(true, {
      errorMap: () => ({ message: 'Reconhecimento dos direitos do titular obrigatório' })
    }),
  }),
  ...
  professionalInfo: z.object({
    ...
    licenseType: z.enum(['CRM', 'CRF', 'CREFITO', 'CRN', 'COREN', 'CRO', 'CRP'], {
    ...
    specializations: z.array(z.string().max(100, 'Especialização muito longa')).max(10, 'Máximo 10 especializações'),
    
    clinicAffiliations: z.array(z.string().uuid('ID da clínica inválido')).max(5, 'Máximo 5 afiliações'),
  }),
};
...
export const AppointmentDataSchemas = {
  booking: z.object({
    ...
    scheduledDate: z.string().refine(date => {
      ...
      return scheduledDate > now && scheduledDate < new Date(now.getFullYear() + 1, now.getMonth(), now.getDate());
...
export function validateHealthcareInput(schema: z.ZodSchema): MiddlewareHandler {
  return async (c, next) => {
    try {
...
async function validatePatientDataCompliance(data: any, c: Context): Promise<void> {
  ...
  if (user?.role === 'healthcare_provider' && !user?.professionalLicense) {
    throw createError.authorization('Licença profissional obrigatória para modificar dados de pacientes');
  }
}
...
export const healthcareValidationUtils = {
  ...
  // Professional license validation
  validateLicenseFormat: (license: string, type: keyof typeof licenseRegexes) => {
  ...
  sanitizePhone: (phone: string): string => {
    return phone.replace(/\D/g, '').replace(/^(\d{2})(\d{2})(\d{4,5})(\d{4})$/, '+55 ($1) $3-$4');
  },
...
export { validateBrazilianCPF, validateBrazilianRG, brazilianPhoneRegex, cepRegex };