  ...
  return async (c: Context, next: Next) => {
    try {
      ...
      if (shortWindowLimited || longWindowLimited) {
        ...
        const resetTimeShort = chatLimiter.getResetTime(`${rateLimitKey}:5m`, shortWindowMs);
        const resetTimeLong = chatLimiter.getResetTime(`${rateLimitKey}:1h`, longWindowMs);
        ...
        c.header('X-RateLimit-Reset-Short', Math.ceil(resetTimeShort / 1000).toString());
        
        c.header('X-RateLimit-Limit-Long', longWindowLimit.toString());
        c.header('X-RateLimit-Remaining-Long', remainingLong.toString());
        c.header('X-RateLimit-Reset-Long', Math.ceil(resetTimeLong / 1000).toString());
