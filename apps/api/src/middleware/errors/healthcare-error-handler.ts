 ...
 */
async function handleHealthcareError(error: unknown, c: Context): Promise<HealthcareError> {
  ...
  if (healthcareError.category === ErrorCategory.PATIENT_DATA || healthcareError.patientImpact) {
 ...
 */
async function logHealthcareError(error: HealthcareError, c: Context): Promise<void> {
  const logData = {
    errorId: error.id,
 ...
 */
async function triggerCriticalErrorAlert(error: HealthcareError): Promise<void> {
  ...
  logger.error('CRITICAL ALERT: Healthcare system error requires immediate attention', {
    errorId: error.id,
    category: error.category,
    severity: error.severity,
    context: error.context
 ...
 */
function sanitizeErrorMessage(message: string, category: ErrorCategory): string {
  ...
  switch (category) {
    ...
    case ErrorCategory.VALIDATION:
      ...
      return message.replace(/\b\d{11}\b/g, '***.***.***-**') // Hide CPF
