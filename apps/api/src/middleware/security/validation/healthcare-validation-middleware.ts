...
export class HealthcareInputValidator {
    ...
    try {
      ...
      if (!validationResult.success) {
        ...
        if (rule.auditRequired) {
          await this.logValidationEvent(context, rule.severity, false, validationResult.errors, options.userId);
        }
        return validationResult;
      }

      // Additional Brazilian healthcare-specific validations
      const additionalValidation = await this.performAdditionalValidations(context, validationResult.data);
   ...
   */
  private async checkCPFDuplication(cpf: string): Promise<{ isDuplicate: boolean; existingPatientId?: string }> {
    try {
   ...
   */
  private async queryPatientByCPF(cpf: string): Promise<Array<{ id: string; cpf: string }> | null> {
   ...
   */
  private validateAgeRestrictions(context: ValidationContext, dateOfBirth: string): { valid: boolean; message?: string } {
  ...
  private async logValidationEvent(
    ...
    const logEntry = {
      ...
      errors: success ? undefined : BrazilianHealthcareSanitizer.removeSensitiveDataForLogging(errors),
    };
