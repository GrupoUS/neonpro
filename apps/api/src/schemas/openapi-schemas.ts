...
export const ErrorResponseSchema = z
  .object({
    ...
    code: z.string().optional().openapi({
      description:
        "Error code for programmatic handling (e.g., LGPD_CONSENT_REQUIRED)",
...
export const AppointmentsListResponseSchema = z
  .object({
    items: z.array(AppointmentSchema).openapi({
      description:
        "List of appointments (max 10 items, LGPD consent validated)",
      example: [],
    }),
  })
...
export const CommonResponses = {
  ...
  403: {
    description:
      "Forbidden - LGPD consent required or insufficient permissions",
    content: {
      "application/json": {
        schema: ErrorResponseSchema,
        example: {
