...
export function unauthorized(c: Context, message = 'Unauthorized', details?: unknown) {
  return c.json<ApiError>({ success: false, error: { code: 'AUTHENTICATION_REQUIRED', message, details } }, 401)
}
...
export function serverError(c: Context, message = 'Internal Server Error', details?: unknown) {
  return c.json<ApiError>({ success: false, error: { code: 'INTERNAL_ERROR', message, details } }, 500)
}
