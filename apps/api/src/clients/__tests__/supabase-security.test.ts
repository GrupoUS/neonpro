  ...
  createAdminClient: vi.fn(() => ({
    rpc: vi.fn((functionName: string, params?: any) => {
      ...
      if (functionName === 'validate_phi_encryption') {
        return Promise.resolve({
          data: {
            ...
            tables_encrypted: params?.table_names || ['patients', 'medical_records', 'aesthetic_assessments'],
      ...
      if (functionName === 'validate_field_encryption') {
        return Promise.resolve({
          data: {
            encrypted_fields: params?.sensitive_fields || ['cpf', 'medical_diagnosis', 'treatment_notes'],
      ...
      if (functionName === 'lgpd_validate_consent') {
        return Promise.resolve({
          data: {
            ...
            purposes_covered: params?.processing_purpose ? [params.processing_purpose] : ['aesthetic_analysis'],
            data_categories_approved: params?.data_categories || ['health_data', 'biometric_data', 'personal_images'],
            legal_basis: 'consent'
          },
          error: null
        });
      ...
      if (functionName === 'validate_concurrent_sessions') {
        return Promise.resolve({
          data: {
            sessions_allowed: 3,
            current_sessions: 2,
            oldest_session_terminated: params?.new_session_request ? new Date().toISOString() : null
          },
          error: null
        });
      }
