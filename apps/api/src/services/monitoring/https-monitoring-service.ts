...
export interface MonitoringAlert {
  id: string;
  type: 'handshake_timeout' | 'degraded_performance' | 'protocol_deprecated' | 'cipher_weak';
...
export class HTTPSMonitoringService {
  ...
  private async escalateAlert(alert: MonitoringAlert): Promise<void> {
    logger.error(
      ...
      `Critical HTTPS monitoring alert: ${alert.message}`,
      {
        ...
        severity: alert.severity,
        serverName: alert.metrics.serverName,
        alertCount: this.alerts.filter(a => !a.resolved && a.type === alert.type).length
      },
      alert.metrics
    );
  ...
  getPerformanceSummary(serverName?: string): {
    ...
    const compliantHandshakes = allMetrics.filter(m => m.handshakeTimeMs <= this.config.maxHandshakeTimeMs);
   ...
   */
  getDetailedMetrics(serverName?: string, timeRange?: { start: Date; end: Date }): TLSHandshakeMetrics[] {
  ...
  resolveAlert(alertId: string): void {
    ...
    if (alert) {
      alert.resolved = true;
      logger.info('https_monitoring_alert_resolved', `Alert resolved: ${alert.message}`, {
        alertId,
        serverName: alert.metrics.serverName
      });
    }
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      for (const [serverName, metrics] of this.metrics.entries()) {
        const filteredMetrics = metrics.filter(m => m.clientHelloTime > cutoffTime);
      ...
      if (cleanedMetrics > 0) {
        logger.info('https_monitoring_cleanup', 'Cleaned up stale metrics', { cleanedMetrics });
      }
      ...
      logger.error('https_monitoring_health_check', 'Health check failed', { error: (error as Error).message });
    }
  }
