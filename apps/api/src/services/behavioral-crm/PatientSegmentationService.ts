...
import { BehavioralAnalysisService, PatientBehaviorProfile } from './BehavioralAnalysisService';
...
export class PatientSegmentationService {
  ...
  async createSegment(
    ...
    try {
      ...
      console.log(`✅ Created segment "${name}" with ${patientCount} patients`);
      return segment;
    } catch (error) {
  ...
  async getSegmentPatients(
    ...
    try {
      ...
      console.error('Error getting segment patients:', error);
      throw error;
    }
  }
  ...
  async refreshPatientSegmentation(patientId?: string): Promise<void> {
    try {
      ...
      console.log(`🔄 Re-segmenting ${patients.length} patients...`);
      ...
      for (const pid of patients) {
        ...
        await this.updatePatientSegments(pid, matchingSegments.map(s => s.id));
      }
      
      // Update segment statistics
      await this.refreshAllSegmentMetrics();
      
      console.log(`✅ Completed re-segmentation of ${patients.length} patients`);
    } catch (error) {
      console.error('Error refreshing segmentation:', error);
      throw error;
    }
  }
  ...
  async generateCampaignTargets(target: CampaignTarget): Promise<PatientBehaviorProfile[]> {
    try {
      ...
      // Remove duplicates
      candidates = candidates.filter((patient, index, self) => 
      ...
      console.log(`🎯 Generated ${candidates.length} campaign targets`);
      return candidates;
    } catch (error) {
      console.error('Error generating campaign targets:', error);
      throw error;
    }
  }
  ...
  async createSegmentABTest(
    ...
    try {
      ...
      console.log(`🧪 Created A/B test: ${controlGroup.length} control, ${testGroup.length} test`);
  ...
  async getTopPerformingSegments(
    ...
    try {
      ...
      console.error('Error getting top performing segments:', error);
      throw error;
    }
  }
  ...
  async initializeStandardSegments(): Promise<PatientSegment[]> {
    ...
    for (const template of templates) {
      try {
        ...
        console.error(`Error creating template segment ${template.name}:`, error);
      }
    }
  ...
  private async calculateSegmentMetrics(criteria: SegmentCriteria): Promise<{
    ...
    return {
      averageLTV: Math.floor(Math.random() * 15000) + 5000,
  ...
  private async findPatientsMatchingCriteria(
    ...
    try {
      ...
      console.error('Error finding patients matching criteria:', error);
      return [];
    }
  }
  ...
  private doesPatientMatchCriteria(
    ...
    if (criteria.scores) {
      for (const [scoreType, range] of Object.entries(criteria.scores)) {
        ...
        if (range.min && patientScore < range.min) return false;
        if (range.max && patientScore > range.max) return false;
      }
    }

    // Personality type criteria
    if (criteria.personalityTypes) {
      if (!criteria.personalityTypes.includes(patient.personalityType)) {
        return false;
      }
    }
  ...
  private async getAllActivePatientIds(): Promise<string[]> {
    try {
      ...
      console.error('Error getting active patient IDs:', error);
      return [];
    }
  }
  ...
  private async updatePatientSegments(patientId: string, segmentIds: string[]): Promise<void> {
    try {
      ...
      console.error('Error updating patient segments:', error);
      throw error;
    }
  }
  ...
  private async refreshAllSegmentMetrics(): Promise<void> {
    try {
      ...
      console.error('Error refreshing segment metrics:', error);
      throw error;
    }
  }
  ...
  private async storeABTest(test: any): Promise<void> {
    try {
      const { error } = await supabase
        .from('segment_ab_tests')
        .insert(test);

      if (error) throw error;
    } catch (error) {
      console.error('Error storing A/B test:', error);
      throw error;
    }
  }
}
