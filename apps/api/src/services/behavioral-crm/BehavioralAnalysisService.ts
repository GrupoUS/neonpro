...
export class BehavioralAnalysisService {
  ...
  async analyzePatientBehavior(patientId: string): Promise<PatientBehaviorProfile> {
    try {
      ...
      const appointments = await this.getAppointmentHistory(patientId);
      ...
      try {
        ...
        chunkResults.forEach((result, index) => {
          if (result.status === 'fulfilled') {
            ...
            console.error(`Failed to analyze patient ${chunk[index]}:`, result.reason);
          }
        });
      } catch (error) {
        console.error('Error in batch analysis chunk:', error);
      }
    }
  ...
  private async calculateBehavioralScores(
    ...
    const recentEvents = events.filter(e => 
      e.timestamp >= new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)
    );
    ...
    const proactiveInteractions = communicationEvents.filter(e => 
      e.metadata?.initiatedBy === 'patient'
    ).length;
    ...
    const noShows = appointmentEvents.filter(e => 
      e.eventSubtype === 'no_show'
    ).length;
    const latePayments = recentEvents.filter(e => 
      e.eventType === 'payment' && e.outcome === 'negative'
    ).length;
    const complaints = recentEvents.filter(e => 
      e.eventSubtype === 'complaint'
    ).length;
    ...
    const completedTreatments = treatmentEvents.filter(e => 
      e.outcome === 'positive'
    ).length;
  ...
  private analyzeCommunicationStyle(interactions: any[]): PatientBehaviorProfile['patterns']['communicationStyle'] {
    ...
    const avgMessageLength = interactions.reduce((sum, i) => 
      sum + (i.message?.length || 0), 0
    ) / interactions.length;
    
    const formalWords = interactions.filter(i => 
      i.message?.includes('Senhor') || i.message?.includes('Senhora')
    ).length;
  ...
  private analyzeResponseTime(interactions: any[]): PatientBehaviorProfile['patterns']['responseTime'] {
    ...
    if (responseTimes.length === 0) return 'delayed';
    
    const avgResponseTime = responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length;
    
    if (avgResponseTime < 1) return 'immediate';
    if (avgResponseTime < 24) return 'hours';
    if (avgResponseTime < 72) return 'days';
    return 'delayed';
  }
  ...
  private analyzeAppointmentBehavior(events: BehavioralEvent[]): PatientBehaviorProfile['patterns']['appointmentBehavior'] {
    const appointmentEvents = events.filter(e => e.eventType === 'appointment');
    
    const punctualCount = appointmentEvents.filter(e => 
      e.eventSubtype === 'attended_on_time'
    ).length;
    const earlyCount = appointmentEvents.filter(e => 
      e.eventSubtype === 'attended_early'
    ).length;
    const lateCount = appointmentEvents.filter(e => 
      e.eventSubtype === 'attended_late'  
    ).length;
    const rescheduleCount = appointmentEvents.filter(e => 
      e.eventSubtype === 'rescheduled'
    ).length;

    if (rescheduleCount > appointmentEvents.length * 0.3) return 'reschedules';
    if (punctualCount > appointmentEvents.length * 0.7) return 'punctual';
    if (earlyCount > appointmentEvents.length * 0.4) return 'early';
    return 'late';
  }
  ...
  private analyzeSeasonalTrends(events: BehavioralEvent[]): string[] {
    ...
    const monthlyActivity = events.reduce((acc, e) => {
  ...
  private async analyzePersonalityType(
    ...
    const quickDecisions = events.filter(e => 
      e.metadata?.decisionTime && e.metadata.decisionTime < 24
    ).length;
    ...
    const socialReferences = interactions.filter(i =>
    ...
    const casualTone = interactions.filter(i =>
      i.message?.includes('😊') || i.message?.includes('rs')
    ).length;
  ...
  private async calculateLifetimeValue(patientId: string, segment: string): Promise<number> {
    try {
      ...
      if (payments && payments.length > 0) {
        ...
        const lastPayment = new Date(payments[payments.length - 1].created_at);
    ...
    try {
      ...
      appointments?.forEach(apt => {
        events.push({
          ...
          outcome: apt.status === 'completed' ? 'positive' : 
                  apt.status === 'no_show' ? 'negative' : 'neutral',
      ...
      console.error('Error collecting behavioral events:', error);
      return [];
    }
  }

  private async getPatientInteractions(patientId: string): Promise<any[]> {
  ...
  private async storeBehavioralProfile(profile: PatientBehaviorProfile): Promise<void> {
    try {
      ...
      console.error('Error storing behavioral profile:', error);
      throw error;
    }
  }
  ...
  private calculateResponseRate(communicationEvents: BehavioralEvent[]): number {
    const sentMessages = communicationEvents.filter(e => 
      e.metadata?.direction === 'outbound'
    ).length;
    const responses = communicationEvents.filter(e => 
      e.metadata?.direction === 'inbound'
    ).length;
  ...
  private calculateAttendanceConsistency(appointmentEvents: BehavioralEvent[]): number {
    const completed = appointmentEvents.filter(e => 
      e.outcome === 'positive'
    ).length;
