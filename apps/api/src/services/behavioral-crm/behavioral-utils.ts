 ...
 */
export function categorizeResponseTime(avgHours: number): "immediate" | "hours" | "days" | "delayed" {
...
function filterRecentInteractions(interactions: readonly PatientInteraction[], days: number): PatientInteraction[] {
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - days);
