...
export class LGPDCompliantDataHandler {
  ...
  constructor(config: LGPDDataHandlerConfig) {
    ...
    this.encryptionKey =
      config.encryptionKey || process.env.ENCRYPTION_KEY || "default-key";
  ...
  private maskEmail(email: string): string {
    ...
    if (local && domain) {
      const maskedLocal =
        local.length > 3 ? local.substring(0, 2) + "***" : "***";
  ...
  private getDefaultPurpose(consentType: string): string {
    const purposes: Record<string, string> = {
      ...
      data_sharing:
        "Compartilhamento com profissionais de saúde envolvidos no tratamento",
