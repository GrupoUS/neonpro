...
import { whatsappReminderService, type AppointmentReminder } from './whatsapp-reminder-service'
...
export class ReminderScheduler {
  ...
  async startScheduler(intervalMinutes = 5): Promise<void> {
    ...
    console.log(`Starting automated reminder scheduler (${intervalMinutes}min intervals)`)
    this.isRunning = true
    ...
    try {
      ...
      for (const scheduledReminder of scheduledReminders || []) {
        ...
        try {
          ...
          console.error(`Error processing reminder ${scheduledReminder.id}:`, reminderError)
          ...
          await this.updateScheduledReminder(scheduledReminder.id, {
            ...
            last_attempt: new Date().toISOString(),
            attempts: scheduledReminder.attempts + 1,
            error_message: reminderError instanceof Error ? reminderError.message : 'Unknown error',
            updated_at: new Date().toISOString()
          })
          failed++
        }
      ...
      console.log(`Reminder processing completed: ${processed} processed, ${sent} sent, ${failed} failed, ${skipped} skipped (${processingTime}ms)`)
  ...
  async scheduleRemindersForAppointment(appointmentId: string): Promise<{
    ...
    try {
      ...
      if (schedulesError || !schedules || schedules.length === 0) {
        console.log(`No active reminder schedules found for clinic ${appointment.clinic_id}`)
        return { scheduled: 0, reminders: [] }
      }
      ...
      for (const schedule of schedules) {
        const reminderTime = new Date(
          appointmentDateTime.getTime() - (schedule.hours_before_appointment * 60 * 60 * 1000)
        )
      ...
      console.log(`Scheduled ${scheduledReminders.length} reminders for appointment ${appointmentId}`)
  ...
  async cancelScheduledReminders(appointmentId: string): Promise<number> {
    try {
      ...
      console.log(`Cancelled ${cancelledCount} scheduled reminders for appointment ${appointmentId}`)
   ...
   */
  async upsertReminderSchedule(schedule: Partial<ReminderSchedule> & {
    clinicId: string
    reminderType: string
    hoursBeforeAppointment: number
   ...
   */
  async getSchedulerStatistics(clinicId?: string, days = 7): Promise<{
    ...
    try {
      ...
      const averageProcessingTime = stats.processedCount > 0 ? 
        stats.totalProcessingTime / stats.processedCount : 0
