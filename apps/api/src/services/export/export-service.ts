import { ExportConfig, ExportJob, ExportFilter, ExportPagination, ExportMetrics, LGPDComplianceOptions, PatientExportField } from './types'
...
export class ExportService {
  ...
  private static async processExportJob(jobId: string, lgpdOptions: LGPDComplianceOptions): Promise<void> {
    const job = this.ACTIVE_JOBS.get(jobId)
    if (!job) return
  ...
  static async getExportMetrics(jobId: string): Promise<ExportMetrics | null> {
    ...
    return {
      ...
      averageSpeed: job.progress.processed > 0 ? job.progress.processed / ((job.updatedAt.getTime() - job.createdAt.getTime()) / 1000) : 0
    }
  }
