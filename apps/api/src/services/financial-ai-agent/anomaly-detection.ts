...
export class AnomalyDetectionService {
  ...
  private detectAmountAnomaly(
    ...
    if (zScore > this.statisticalConfig.zScoreThreshold) {
      return {
        ...
        description: `Valor de cobrança R$ ${billing.total.toFixed(2)} está ${zScore.toFixed(2)} desvios padrão acima da média (R$ ${mean.toFixed(2)})`,
  ...
  private detectIQROutlier(
    ...
    if (billing.total < lowerBound || billing.total > upperBound) {
      ...
      return {
        ...
        description: `Valor de cobrança R$ ${billing.total.toFixed(2)} está fora do intervalo normal (R$ ${lowerBound.toFixed(2)} - R$ ${upperBound.toFixed(2)})`,
        detectedAt: new Date().toISOString(),
        affectedEntities: [
          {
            type: 'billing',
        ...
        recommendedAction: 'Investigar cobrança que se desvia significativamente dos valores típicos',
        status: 'open',
      };
    }
  ...
  private detectTimeAnomaly(
    ...
    if (isOutsideHours || isWeekend) {
      ...
      const outsideHoursRate = similarBillings.length > 0 ? outsideHoursCount / similarBillings.length : 0;
      ...
      if (outsideHoursRate < 0.1 && similarBillings.length >= 5) {
        return {
          ...
          severity: 'medium',
          confidence: 0.8,
          description: `Cobrança criada fora do horário comercial (${hour}:${String(billingTime.getMinutes()).padStart(2, '0')})`,
          detectedAt: new Date().toISOString(),
          affectedEntities: [
            {
              type: 'billing',
  ...
  private extractFeatures(billing: Billing, historicalData: Billing[]): number[] {
    ...
    const professionalHistory = historicalData.filter(b => b.professionalId === billing.professionalId);
  ...
  private detectBillingVelocity(
    ...
    if (billingsLastHour > this.ruleConfig.velocityThresholds.billingPerHour) {
      return {
        ...
        type: 'billing',
        severity: 'high',
        confidence: Math.min(billingsLastHour / this.ruleConfig.velocityThresholds.billingPerHour, 1),
        description: `${billingsLastHour} cobranças na última hora (limite: ${this.ruleConfig.velocityThresholds.billingPerHour})`,
        detectedAt: new Date().toISOString(),
        affectedEntities: [
          {
            type: 'clinic',
    ...
    if (billingsLastDay > this.ruleConfig.velocityThresholds.billingPerDay) {
      return {
        ...
        description: `${billingsLastDay} cobranças nas últimas 24 horas (limite: ${this.ruleConfig.velocityThresholds.billingPerDay})`,
        detectedAt: new Date().toISOString(),
        affectedEntities: [
          {
            type: 'clinic',
  ...
  private detectDuplicateBilling(
    ...
    if (similarBillings.length > 0) {
      return {
        ...
        severity: 'high',
        confidence: 0.9,
        description: `Encontradas ${similarBillings.length} cobranças similares nas últimas ${timeWindow} horas`,
        detectedAt: new Date().toISOString(),
        affectedEntities: [
          {
            type: 'billing',
  ...
  private detectSuspiciousDiscounts(billing: Billing): AnomalyAlert | null {
    ...
    if (discountRate > maxDiscount) {
      return {
        ...
        description: `Desconto de ${(discountRate * 100).toFixed(1)}% aplicado (limite: ${(maxDiscount * 100).toFixed(1)}%)`,
        detectedAt: new Date().toISOString(),
        affectedEntities: [
          {
            type: 'billing',
  ...
  private async runFraudDetection(
    ...
    totalRiskScore = validIndicators.reduce((sum, ind) => sum + (ind.weight * (ind.triggered ? 1 : 0)), 0);
   ...
   */
  private checkVelocityRisk(billing: Billing, historicalData: Billing[]): FraudDetectionResult['indicators'][0] {
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
   ...
   */
  private checkAmountRisk(billing: Billing, historicalData: Billing[]): FraudDetectionResult['indicators'][0] {
    const patientHistory = historicalData.filter(b => b.patientId === billing.patientId);
   ...
   */
  private checkHistoryRisk(billing: Billing, historicalData: Billing[]): FraudDetectionResult['indicators'][0] {
    const patientHistory = historicalData.filter(b => b.patientId === billing.patientId);
   ...
   */
  private generateFraudDescription(indicators: FraudDetectionResult['indicators'], riskLevel: string): string {
    ...
    return `Risco ${riskLevel} de fraude detectado. Indicadores acionados: ${indicatorNames.join(', ')}`;
  }
  ...
  private async analyzeEntityBehavior(
    ...
    const avgFrequency = dailyBillings.reduce((sum, count) => sum + count, 0) / dailyBillings.length;
    const variance = dailyBillings.reduce((sum, count) => sum + Math.pow(count - avgFrequency, 2), 0) / dailyBillings.length;
    const stdDev = Math.sqrt(variance);
  ...
  private async analyzeNetworkPatterns(
    ...
    const anomalies = this.detectNetworkAnomalies(Array.from(nodes.values()), Array.from(edges.values()));
  ...
  private detectNetworkAnomalies(nodes: any[], edges: any[]): NetworkAnalysis['anomalies'] {
    ...
    const avgDegree = Array.from(nodeDegrees.values()).reduce((sum, deg) => sum + deg, 0) / nodeDegrees.size;
  ...
  private async sendAlertNotification(alert: AnomalyAlert): Promise<void> {
    ...
    if (escalationRule) {
      console.log(`Escalating alert: ${escalationRule.action} to ${escalationRule.recipients.join(', ')}`);
    }
  }
