...
describe("Healthcare Prisma Client Integration Tests", () => {
  ...
  describe("Brazilian Healthcare Validation", () => {
    ...
    test("should validate patient data completeness", () => {
      const validation =
        PatientDataHelper.validatePatientDataCompleteness(mockPatientData);
  ...
  describe("Healthcare Query Operations", () => {
    ...
    test("should find appointments for professional", async () => {
      const appointments =
        await prismaWithContext.findAppointmentsForProfessional(
          testProfessionalId,
          {
            status: "scheduled",
          },
  ...
  describe("Performance Optimization", () => {
    ...
    test("should get optimized dashboard metrics", async () => {
      const result =
        await queryOptimizer.getDashboardMetricsOptimized(testClinicId);
  ...
  describe("Healthcare Appointment Operations", () => {
    ...
    test("should check appointment conflicts", async () => {
      ...
      const conflicts =
        await HealthcareAppointmentHelper.checkAppointmentConflicts(
          prismaWithContext,
          testProfessionalId,
          startTime,
          endTime,
  ...
  describe("Data Anonymization", () => {
    test("should anonymize patient data for research", () => {
      const anonymized =
        PatientDataHelper.anonymizePatientData(mockPatientData);
    ...
    try {
      ...
      await prismaClient.patient.upsert({
        ...
        create: {
          ...
          clinicId: testClinicId,
          medicalRecordNumber:
            PatientDataHelper.generateMedicalRecordNumber(testClinicId),
          ...mockPatientData,
        },
      });
    } catch (error) {
