    ...
    it('SHOULD FAIL: Should not expose file paths in error messages', async () => {
      app.get('/file-error', async (c) => {
        try {
          ...
          throw new Error('ENOENT: no such file or directory, open \'/home/vibecode/neonpro/apps/api/src/config/secret-keys.json\'');
        } catch (error) {
    ...
    it('SHOULD FAIL: Should not expose environment variables', async () => {
      app.get('/config-error', async (c) => {
        try {
          ...
          throw new Error(`Missing required environment variable: SUPABASE_SERVICE_ROLE_KEY=${process.env.SUPABASE_SERVICE_ROLE_KEY}`);
        } catch (error) {
  ...
  describe('Database Error Information Leakage', () => {
    it('SHOULD FAIL: Should not expose database schema details', async () => {
      app.get('/db-schema-error', async (c) => {
        try {
          ...
          throw new Error('column "patients.social_security_number" does not exist in table "patients"');
        } catch (error) {
    ...
    it('SHOULD FAIL: Should not expose database connection details', async () => {
      app.get('/db-connection-error', async (c) => {
        try {
          ...
          throw new Error('could not connect to server: Connection refused (0x0000274D/10061)\n\tIs the server running on host "localhost" (127.0.0.1) and accepting\n\tTCP/IP connections on port 5432?');
        } catch (error) {
    ...
    it('SHOULD FAIL: Should not expose SQL query details', async () => {
      app.get('/sql-error', async (c) => {
        try {
          // Simulate SQL error
          throw new Error('syntax error at or near "WHERE" in query: SELECT * FROM patients WHERE name = \'John\' AND age > 25 ORDER BY created_at DESC');
        } catch (error) {
      ...
      app.use('/error-endpoint', async (c, next) => {
        ...
        if (ip === clientIp) {
          ...
          if (timeSinceLastError < 1000 && lastErrorTime > 0) {
            await new Promise(resolve => setTimeout(resolve, Math.min(timeSinceLastError * 2, 5000)));
          }
          lastErrorTime = now;
        }
