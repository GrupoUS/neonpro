      ...
      getAppointmentsByDate: async (date: string, userId: string) => {
        ...
        if (userId === 'admin-user' || userId === 'regular-user') {
          return {
            ...
            title: 'Agendamentos',
            data: [
              { id: 'apt-1', datetime: '2024-01-20T09:00:00Z', clientName: 'Maria Silva' }
            ],
            columns: []
          }
        } else {
      ...
      getFinancialSummary: async (period: string, userId: string) => {
        ...
        if (userId === 'admin-user') {
          return {
            ...
            data: [],
            summary: { totalRevenue: 5000, totalExpenses: 2000, netIncome: 3000 },
            columns: []
          }
        } else {
...
describe('Integration Tests: Access Control', () => {
  ...
  describe('Role-Based Access Control', () => {
    ...
    it('should respect Row Level Security (RLS) policies', async () => {
      ...
      expect(data.response.data.every((c: any) => !c.name.includes('clínica B'))).toBe(true)
    })
  })
