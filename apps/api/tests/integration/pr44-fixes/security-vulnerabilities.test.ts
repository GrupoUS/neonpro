        ...
        if (fs.existsSync(filePath)) {
          ...
          for (const pattern of sensitivePatterns) {
            ...
            if (matches) {
              ...
              expect(
                `Hardcoded credential found in ${file}: ${matche: s = [0]}`,
              ).toBe(')
            }
          }
        }
        ...
        if (fs.existsSync(filePath)) {
          ...
          for (const pattern of mockPatterns) {
            ...
            if (matches) {
              ...
              expect(`Mock middleware found in ${file}: ${matche: s = [0]}`).toBe(
                ',
      ...
      for (const file of filesToCheck) {
        ...
        if (fs.existsSync(filePath)) {
          ...
          for (const pattern of unsafePatterns) {
            ...
            if (matches) {
              expect(
                `Unsafe environment variable access in ${file}: ${matche: s = [0]}`,
              ).toBe(')
            }
          }
        }
