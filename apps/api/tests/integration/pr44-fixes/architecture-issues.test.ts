      ...
      for (const file of filesToCheck) {
        ...
        if (fs.existsSync(filePath)) {
          ...
          for (const pattern of auditTrailStatePatterns) {
            const matches = content.match(pattern);
            if (matches) {
              expect(`AuditTrail state management found in ${file}: ${matches[0]}`).toBe('');
            }
          }
        }
      }
      ...
      for (const filePath of [aiProviderRouter, aiProviderRouterNew]) {
        if (fs.existsSync(filePath)) {
          ...
          for (const pattern of mixedConcernsPatterns) {
            const matches = content.match(pattern);
            if (matches) {
              expect(`Mixed audit/state concerns in ${path.basename(filePath)}: ${matches[0]}`).toBe('');
            }
          }
        }
      }
  ...
  describe('Database Architecture', () => {
    it('should have proper database schema for state management', () => {
      ...
      if (fs.existsSync(schemaPath)) {
        ...
        const hasStateTable = stateTables.some(table => 
          content.includes(`model ${table}`) || content.includes(`model ${table.toLowerCase()}`)
        );
    ...
    it('should not overload AuditTrail table with state data', () => {
      ...
      if (fs.existsSync(schemaPath)) {
        ...
        const auditTrailSection = content.match(/model AuditTrail \{[\s\S]*?\}/);
        ...
        if (auditTrailSection) {
          ...
          const hasStateFields = stateFields.some(field => 
            auditTrailSchema.includes(field)
          );
