...
describe('Targeted Issue Tests', () => {
  ...
  describe('Actual Console.log Issues', () => {
    it('should not have console.log statements in ai-provider-router.ts', () => {
      // This test SHOULD fail because console.log statements exist
      ...
      if (fs.existsSync(filePath)) {
        ...
        // This should fail because there ARE console.log statements (7 found)
        expect(consoleLogCount).toBe(0);
      }
    });

    it('should not have console.log statements in ai-provider-router-new.ts', () => {
      // This test SHOULD fail because console.log statements exist
      ...
      if (fs.existsSync(filePath)) {
        ...
        // This should fail because there ARE console.log statements (3 found)
        expect(consoleLogCount).toBe(0);
      }
    });
  });
  ...
  describe('Actual File Size Issues', () => {
    it('should not have excessively large files', () => {
      // This test SHOULD fail because ai-provider-router-new.ts is too large
      const filePath = path.join(servicesPath, 'ai-provider-router-new.ts');
  ...
  describe('Duplicate AI Provider Router Files', () => {
    it('should not have both ai-provider-router.ts and ai-provider-router-new.ts', () => {
      ...
      const routerNewPath = path.join(servicesPath, 'ai-provider-router-new.ts');
