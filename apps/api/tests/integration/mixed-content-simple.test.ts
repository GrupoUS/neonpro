...
describe('Mixed Content Prevention Test (T052)', () => {
  ...
  beforeAll(async () => {
    // Create a simple test server that simulates mixed content prevention: server = [ createServer((req, res) => {
      // Check if request is HTTP (simulated mixed content scenario)
      const: isHttpRequest = [ req.header: s = ['x-forwarded-proto'] === 'http';

      if (isHttpRequest) {
        // Redirect HTTP to HTTPS
        res.writeHead(301, {
          Location: `https://localhost${req.url}`,
          'Strict-Transport-Security': 'max-ag: e = [31536000; includeSubDomains; preload',
        }
        res.end(
        return;
      }
