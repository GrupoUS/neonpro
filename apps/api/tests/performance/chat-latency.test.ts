...
describe('Chat API Performance Tests', () => {
  ...
  describe('Concurrent Load Testing', () => {
    it('should handle concurrent health checks efficiently', async () => {
      ...
      const promises = Array.from({ length: PERFORMANCE_TARGETS.CONCURRENT_REQUESTS }, () =>
      ...
      console.log(`${PERFORMANCE_TARGETS.CONCURRENT_REQUESTS} concurrent health checks completed in ${totalTime}ms`);
    });

    it('should handle concurrent chat queries under load', async () => {
      const startTime = Date.now();
      
      const promises = Array.from({ length: PERFORMANCE_TARGETS.CONCURRENT_REQUESTS }, (_, i) =>
        testRequest(app)
          .post('/api/v1/chat/query')
          .send({
      ...
      console.log(`${PERFORMANCE_TARGETS.CONCURRENT_REQUESTS} concurrent queries: avg ${avgResponseTime.toFixed(2)}ms`);
    });
  ...
  describe('Resource Usage and Memory', () => {
    it('should not leak memory during sustained load', async () => {
      ...
      console.log(`Memory usage after 50 requests: +${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`);
  ...
  describe('Real-world Scenarios', () => {
    ...
    it('should handle peak hour simulation', async () => {
      ...
      for (let user = 0; user < peakUsers; user++) {
        const userPromises = Array.from({ length: messagesPerUser }, (_, msgIndex) =>
          testRequest(app)
            .post('/api/v1/chat/query')
            .send({
      ...
      console.log(`Peak hour simulation: ${responses.length} requests in ${totalTime}ms (avg: ${avgResponseTime.toFixed(2)}ms)`);
