...
describe("Chat API Performance Tests", () => {
  ...
  describe("Concurrent Load Testing", () => {
    ...
    it("should handle concurrent chat queries under load", async () => {
      ...
      const avgResponseTime =
        totalTime / PERFORMANCE_TARGETS.CONCURRENT_REQUESTS;
      ...
      console.log(
        `${PERFORMANCE_TARGETS.CONCURRENT_REQUESTS} concurrent queries: avg ${avgResponseTime.toFixed(
          2,
        )}ms`,
      );
    });
  ...
  describe("Real-world Scenarios", () => {
    ...
    it("should handle peak hour simulation", async () => {
      ...
      console.log(
        `Peak hour simulation: ${responses.length} requests in ${totalTime}ms (avg: ${avgResponseTime.toFixed(
          2,
        )}ms)`,
      );
