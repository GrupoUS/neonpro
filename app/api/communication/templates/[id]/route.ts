...
export async function PUT(
  ...
  try {
    ...
    if (validatedData.name && validatedData.name !== existingTemplate.name) {
      const nameConflict = await communicationService.getTemplateByName(validatedData.name)
    ...
    // Update template
    const updatedTemplate = await communicationService.updateTemplate(templateId, {
      ...validatedData,
...
export async function DELETE(
  ...
  try {
    ...
    const templatesInUse = await communicationService.checkTemplatesInUse([templateId])
...
export async function POST(
  ...
  try {
    ...
    // Duplicate template
    const duplicatedTemplate = await communicationService.duplicateTemplate(templateId, name, session.user.id)
