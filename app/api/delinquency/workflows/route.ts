...
const GetWorkflowsQuerySchema = z.object({
  ...
  sortBy: z.enum(['created_at', 'next_action_date', 'priority', 'customer_name']).optional().default('next_action_date'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = GetWorkflowsQuerySchema.parse(Object.fromEntries(searchParams));
    ...
    const { data: stats, error: statsError } = await supabase.rpc('get_workflow_statistics');
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'bulk_assign': {
        const { workflowIds, assignedTo } = z.object({
          workflowIds: z.array(z.string().uuid()),
      ...
      case 'escalate_workflow': {
        const { workflowId, escalationReason, escalateTo } = z.object({
          workflowId: z.string().uuid(),
  ...
  try {
    ...
    switch (action) {
      case 'update_workflow': {
        const { workflowId, ...updates } = z.object({
          workflowId: z.string().uuid(),
        }).merge(UpdateWorkflowSchema).parse(data);
      ...
      case 'bulk_update': {
        ...
        if (validatedData.updates.assignedTo) updateData.assigned_to = validatedData.updates.assignedTo;
      ...
      case 'pause_workflow': {
        const { workflowId, reason } = z.object({
          workflowId: z.string().uuid(),
