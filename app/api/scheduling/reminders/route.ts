      ...
      try {
        const workflowConfig = {
          reminderSettings: {
            ...
            preferredChannel: validatedData.patientPreferences?.preferredChannel || validatedData.channels[0]
          }
        }
 ...
 */
async function handleImmediateReminder(supabase: any, data: z.infer<typeof ImmediateReminderSchema>, user: any) {
  ...
  if (!message) {
    ...
    if (template) {
      ...
      message = await schedulingTemplateEngine.renderTemplate(template, channel, variables)
    }
  }

  if (!message) {
    return NextResponse.json({ error: 'No message content available' }, { status: 400 })
  }
 ...
 */
async function handleLegacyReminders(supabase: any, data: z.infer<typeof ReminderConfigSchema>, user: any) {
