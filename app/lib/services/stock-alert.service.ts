...
export class StockAlertService {
  ...
  private async evaluateConfigAndGenerateAlerts(
    ...
    for (const product of products) {
      ...
      if (shouldAlert.generate) {
        const alert = await this.generateAlert(config, product, shouldAlert.message)
        alerts.push(alert)
      }
    }
  ...
  private async evaluateExpiringAlert(
    ...
    const shouldAlert = daysUntilExpiration <= config.thresholdValue && daysUntilExpiration > 0
   ...
   */
  private getCurrentValueForAlert(config: StockAlertConfig, product: any): number {
    switch (config.alertType) {
      case 'low_stock':
      case 'overstock':
  ...
  private async calculateDaysCoverage(productId: string): Promise<number> {
    ...
    const totalConsumption = data.reduce((sum, movement) => sum + Math.abs(movement.quantity), 0)
  ...
  private async validateUniqueAlertConfig(config: StockAlertConfig): Promise<void> {
    let query = this.supabase
      .from('stock_alert_configs')
      .select('id')
      .eq('clinic_id', this.clinicId)
  ...
  private async logStockEvent(event: Omit<StockEvent, 'id' | 'timestamp' | 'clinicId'>): Promise<void> {
    try {
      await this.supabase
        .from('stock_events')
        .insert([{
  ...
  private mapDbToAlert(dbData: any): StockAlert {
    return {
      ...
      acknowledgedAt: dbData.acknowledged_at ? new Date(dbData.acknowledged_at) : undefined,
 ...
 */
export async function createStockAlertService(clinicId: string): Promise<StockAlertService> {
