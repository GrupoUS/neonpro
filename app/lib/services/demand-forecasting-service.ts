...
interface ForecastResult {
  ...
  modelUsed: 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average';
}
...
export class DemandForecastingService {
  ...
  async getHistoricalConsumption(
    ...
    return transactions?.map(tx => ({
      ...
      type: this.categorizeConsumption(tx.transaction_context, tx.appointment_id),
  ...
  async getAppointmentBasedDemand(
    ...
    return appointments?.map(apt => ({
      ...
      expectedConsumption: this.getExpectedConsumptionForProcedure(apt.procedure_type, itemId),
      actualConsumption: apt.inventory_usage?.find((usage: any) => usage.item_id === itemId)?.quantity_used
   ...
   */
  analyzeSeasonalPatterns(consumptionData: ConsumptionData[]): SeasonalPattern[] {
  ...
  private analyzeWeeklyPattern(data: ConsumptionData[]): SeasonalPattern {
    ...
    const weeklyAverages = weeklyData.map(day => day.count > 0 ? day.total / day.count : 0);
    const overallAverage = weeklyAverages.reduce((sum, val) => sum + val, 0) / 7;
    
    // Calcular força do padrão (variação em relação à média)
    const variance = weeklyAverages.reduce((sum, val) => sum + Math.pow(val - overallAverage, 2), 0) / 7;
  ...
  private analyzeMonthlyPattern(data: ConsumptionData[]): SeasonalPattern {
    ...
    const monthlyAverages = monthlyData.map(month => month.count > 0 ? month.total / month.count : 0);
  ...
  private analyzeAnnualPattern(data: ConsumptionData[]): SeasonalPattern {
    ...
    const quarterlyData = new Array(4).fill(0).map(() => ({ total: 0, count: 0 }));
    ...
    const quarterlyAverages = quarterlyData.map(q => q.count > 0 ? q.total / q.count : 0);
    const overallAverage = quarterlyAverages.reduce((sum, val) => sum + val, 0) / 4;
    
    const variance = quarterlyAverages.reduce((sum, val) => sum + Math.pow(val - overallAverage, 2), 0) / 4;
    const strength = Math.min(Math.sqrt(variance) / overallAverage, 1);
  ...
  async generateDemandForecast(input: ForecastInput): Promise<ForecastResult> {
    ...
    const timeSeriesData = this.prepareTimeSeriesData(historicalData, input.forecastPeriod);
    
    // Aplicar diferentes algoritmos e escolher o melhor
    const forecasts = {
      exponentialSmoothing: this.exponentialSmoothingForecast(timeSeriesData, input.forecastPeriod),
      seasonalDecomposition: this.seasonalDecompositionForecast(timeSeriesData, seasonalPatterns, input.forecastPeriod),
      movingAverage: this.movingAverageForecast(timeSeriesData, input.forecastPeriod),
      linearRegression: this.linearRegressionForecast(timeSeriesData, input.forecastPeriod)
    };
  ...
  private exponentialSmoothingForecast(data: number[], periods: number) {
    ...
    return {
      ...
      lastPeriodAccuracy: this.calculateLastPeriodAccuracy(data, smoothedValues)
    };
  }

  private seasonalDecompositionForecast(data: number[], patterns: SeasonalPattern[], periods: number) {
    ...
    const errorEstimate = this.calculateDecompositionError(data, trend, seasonal);
  ...
  private movingAverageForecast(data: number[], periods: number) {
    ...
    return {
      ...
      lastPeriodAccuracy: 0.80 // Simplified
    };
  }
  ...
  private categorizeConsumption(context?: string, appointmentId?: string): ConsumptionData['type'] {
    if (appointmentId) return 'appointment_based';
    if (context?.includes('emergency')) return 'emergency';
    return 'routine';
  }

  private getExpectedConsumptionForProcedure(procedureType: string, itemId: string): number {
  ...
  private prepareTimeSeriesData(consumption: ConsumptionData[], forecastPeriod: number): number[] {
  ...
  private selectBestModel(forecasts: any, historicalData: number[]): 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average' {
    // Select model with lowest MAPE
    let bestModel: 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average' = 'moving_average';
    let lowestMAPE = Infinity;
    
    const modelMapping: Record<string, 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average'> = {
    ...
    Object.entries(forecasts).forEach(([model, forecast]: [string, any]) => {
      if (forecast.mape < lowestMAPE) {
        ...
        bestModel = modelMapping[model] || 'moving_average';
      }
    });
  ...
  private selectBestModelKey(forecasts: any, historicalData: number[]): keyof typeof forecasts {
    // Select model with lowest MAPE
  ...
  private mapModelKeyToEnum(modelKey: string): 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average' {
    const modelMapping: Record<string, 'exponential_smoothing' | 'seasonal_decomposition' | 'linear_regression' | 'moving_average'> = {
      exponentialSmoothing: 'exponential_smoothing',
      seasonalDecomposition: 'seasonal_decomposition',
      linearRegression: 'linear_regression',
  ...
  private calculateConfidenceInterval(prediction: number, errorEstimate: number, confidenceLevel: number) {
  ...
  private generateRecommendations(
    ...
    if (forecast.trend > 0.1) {
      recommendations.push('Increasing trend detected - consider higher safety stock levels');
    } else if (forecast.trend < -0.1) {
      recommendations.push('Decreasing trend detected - review minimum stock levels');
    }
    
    // Seasonal recommendations
    seasonalPatterns.forEach(pattern => {
      if (pattern.strength > 0.5) {
        recommendations.push(`Strong ${pattern.pattern} seasonality detected - adjust ordering patterns accordingly`);
      }
    });
    
    // Accuracy recommendations
    if (forecast.mape > 0.3) {
      recommendations.push('High forecast uncertainty - consider more frequent monitoring');
    }
  ...
  private calculateForecastError(actual: number[], predicted: number[]): number {
    const errors = actual.map((val, idx) => Math.abs(val - (predicted[idx] || 0)));
  ...
  private calculateRMSE(actual: number[], predicted: number[]): number {
    const squaredErrors = actual.map((val, idx) => Math.pow(val - (predicted[idx] || 0), 2));
  ...
  private calculateLastPeriodAccuracy(actual: number[], predicted: number[]): number {
  ...
  private extractSeasonal(data: number[], patterns: SeasonalPattern[]): number[] {
  ...
  private projectTrend(trend: number[], periods: number): number {
  ...
  private getSeasonalAdjustment(patterns: SeasonalPattern[], periods: number): number {
  ...
  private calculateDecompositionError(data: number[], trend: number[], seasonal: number[]): number {
  ...
  private calculateMovingAverageRMSE(data: number[], window: number): number {
    ...
    for (let i = window; i < data.length; i++) {
      const slice = data.slice(i - window, i);
      const average = slice.reduce((sum, val) => sum + val, 0) / slice.length;
      squaredErrors.push(Math.pow(data[i] - average, 2));
    }
