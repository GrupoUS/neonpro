...
export const barcodeDataSchema = z.object({
  ...
  barcode: z.string().min(8, 'Código de barras deve ter pelo menos 8 caracteres'),
...
export class BarcodeService {
  ...
  async generateBarcode(options: BarcodeGenerationOptions): Promise<{
    ...
    try {
      const validatedOptions = barcodeDataSchema.parse(options)
      
      // Generate barcode based on type
      const barcode = await this.createBarcodeNumber(options.barcode_type, options.item_id)
      ...
      if (options.include_qr) {
        const qrData: QRCodeData = await this.buildQRData(options.item_id, options)
  ...
  async scanBarcode(request: {
    ...
    try {
      const validatedRequest = scanRequestSchema.parse(request)
      ...
      // Log scan activity
      await this.logScanActivity(request.user_id, barcodeData.item_id, request.value, 'success')
      ...
      await this.logScanActivity(request.user_id, '', request.value, 'error', 
        error instanceof Error ? error.message : 'Erro desconhecido')
      
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Erro desconhecido no scan'
      }
    }
  }
  ...
  async startBulkScanOperation(options: {
    ...
    try {
      const validatedOptions = bulkScanSchema.parse({
        ...options,
        items: options.items || []
      })
   ...
   */
  async processBulkScan(operationId: string, scanValue: string, userId: string): Promise<ScanResult> {
    try {
      // Get operation
      ...
      const errors = scanResult.success ? operation.errors : [...operation.errors, scanResult.error || 'Scan failed']
      ...
      await this.supabase
        ...
        .update({
          ...
          errors,
          status: scannedItems >= operation.total_items ? 'completed' : 'in_progress',
          completed_at: scannedItems >= operation.total_items ? new Date().toISOString() : null
        })
        .eq('id', operationId)
   ...
   */
  validateBarcodeFormat(barcode: string, type: string): { valid: boolean; error?: string } {
    try {
      switch (type.toUpperCase()) {
        ...
        case 'CODE39':
          if (!/^[A-Z0-9\-. $\/+%]+$/.test(barcode)) {
            return { valid: false, error: 'CODE39 contém caracteres inválidos' }
          }
          break
        case 'QR':
          ...
          break
        default:
          return { valid: false, error: 'Tipo de código de barras não suportado' }
      }
  ...
  private async createBarcodeNumber(type: string, itemId: string): Promise<string> {
    switch (type) {
      case 'EAN13':
  ...
  private async buildQRData(itemId: string, options: BarcodeGenerationOptions): Promise<QRCodeData> {
    // Get item details
  ...
  private async scanQRCode(qrValue: string, userId: string): Promise<ScanResult> {
    try {
