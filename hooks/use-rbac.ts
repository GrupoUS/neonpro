...
export function useRBAC(options: UseRBACOptions = {}) {
  ...
  const loadUserRole = useCallback(async () => {
    ...
    try {
      ...
      if (roleDefError) {
        throw new Error(
          `Failed to load role definition: ${roleDefError.message}`
        );
      }
      ...
      setState((prev) => ({
        ...
        isLoading: false,
        error:
          error instanceof Error ? error.message : 'Failed to load user role',
      }));
    }
  ...
  const hasPermissions = useCallback(
    async (permissions: Permission[]): Promise<Record<Permission, boolean>> => {
  ...
  const computedValues = useMemo(
    () => ({
      ...
      isStaffOrAbove: state.roleDefinition
        ? state.roleDefinition.hierarchy <= 3
        : false,
      isManagerOrAbove: state.roleDefinition
        ? state.roleDefinition.hierarchy <= 2
        : false,
