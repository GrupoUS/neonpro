...
export function useForecasting(
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const evaluation = await forecastingEngine.evaluateModel(
          forecast,
          forecastConfig
        );
        ...
        const sensitivityAnalysis =
          await forecastingEngine.performSensitivityAnalysis(forecastConfig, [
            'growth_rate',
            'seasonal_factor',
            'trend_strength',
          ]);
        ...
        const chartData = forecastingUtils.formatForChart(
          forecast,
          config.granularity
        );
        const confidenceIntervals =
          forecastingUtils.calculateConfidenceIntervals(
  ...
  const updateConfig = useCallback(
    (newConfig: Partial<ForecastingHookConfig>) => {
    ...
    const subscription = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table:
            metric === 'revenue' ? 'subscription_revenue' : 'subscriptions',
        },
        (payload) => {
