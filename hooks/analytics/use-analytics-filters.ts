...
export function useAnalyticsFilters(
  ...
  const [appliedFilters, setAppliedFilters] = useState<AnalyticsFilters | null>(
    null
  );
    ...
    (name: string) => {
      ...
      try {
        localStorage.setItem(
          'neonpro-analytics-filters',
          JSON.stringify(newFilterSets)
        );
      } catch (error) {
  ...
  const getFilterSummary = useCallback(() => {
    ...
    if (filters.segments.length > 0) {
      parts.push(
        `${filters.segments.length} segment${filters.segments.length > 1 ? 's' : ''}`
      );
    }
