...
export function useSession(options: UseSessionOptions = {}): UseSessionReturn {
  ...
  const [_securityAlerts, setSecurityAlerts] = useState<SecurityAlert[]>([]);
  const [_lastActivity, setLastActivity] = useState<Date>(new Date());

  const router = useRouter();
  const refreshIntervalRef = useRef<NodeJS.Timeout | null>(null);
  const activityTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const warningTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const _sessionManagerRef = useRef<SessionManager | null>(null);
      ...
      try {
        ...
        if (result.valid && result.session) {
          ...
          if (result.warnings && result.warnings.length > 0) {
            const alerts: SecurityAlert[] = result.warnings.map(
              (warning, index) => ({
        ...
        const error =
          err instanceof Error ? err : new Error('Session validation failed');
        setError(error);
        setSession(null);
  ...
  const terminate = useCallback(async (): Promise<void> => {
    try {
      ...
      const error =
        err instanceof Error ? err : new Error('Session termination failed');
      setError(error);
  ...
  const extend = useCallback(
    async (minutes?: number): Promise<void> => {
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error('Session extension failed');
        setError(error);
  ...
  const registerDevice = useCallback(
    async (deviceName: string): Promise<DeviceRegistration> => {
      try {
        ...
        const error =
          err instanceof Error ? err : new Error('Device registration failed');
        setError(error);
  ...
  const getActiveSessions = useCallback(async (): Promise<UserSession[]> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to get active sessions: ${response.statusText}`
        );
      }
      ...
      const error =
        err instanceof Error ? err : new Error('Failed to get active sessions');
      setError(error);
      throw error;
    }
  }, []);
  ...
  const terminateSession = useCallback(
    async (sessionId: string): Promise<void> => {
      try {
        ...
        const error =
          err instanceof Error ? err : new Error('Failed to terminate session');
        setError(error);
        toast.error('Failed to terminate session');
        throw error;
      }
  ...
  // Get security events
  const getSecurityEvents = useCallback(async (): Promise<
  ...
  > => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to get security events: ${response.statusText}`
        );
      }
      ...
      const error =
        err instanceof Error ? err : new Error('Failed to get security events');
      setError(error);
      throw error;
    }
  }, []);
  ...
  const trackActivity = useCallback(() => {
    ...
    }, 30_000); // 30 seconds after activity
    ...
    async (deviceId: string) => {
      try {
        const response = await fetch(
          `${SESSION_API.devices}/${deviceId}/trust`,
          {
            method: 'POST',
          }
        );
