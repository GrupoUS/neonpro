...
export function useDeviceManagement(
  ...
  const initializeDeviceManagement = useCallback(async () => {
    ...
    try {
      ...
      const trustedDevices =
        await deviceManagerRef.current.getTrustedDevices(userId);
  ...
  const trustDevice = useCallback(
    async (deviceId: string): Promise<boolean> => {
      ...
      try {
        const result = await deviceManagerRef.current.trustDevice(
          userId,
          deviceId
        );
  ...
  const untrustDevice = useCallback(
    async (deviceId: string): Promise<boolean> => {
      ...
      try {
        const result = await deviceManagerRef.current.untrustDevice(
          userId,
          deviceId
        );
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to untrust device';
        if (showNotifications) {
          toast.error(errorMessage);
        }
        return false;
  ...
  const removeDevice = useCallback(
    async (deviceId: string): Promise<boolean> => {
      ...
      try {
        const result = await deviceManagerRef.current.removeDevice(
          userId,
          deviceId
        );
  ...
  const validateCurrentDevice =
    useCallback(async (): Promise<DeviceValidation | null> => {
      if (!(userId && deviceManagerRef.current && state.currentDevice)) {
        return null;
      }
  ...
  const reportSuspiciousDevice = useCallback(
    async (deviceId: string, reason: string): Promise<boolean> => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to report device';
        if (showNotifications) {
          toast.error(errorMessage);
        }
        return false;
  ...
  const deviceStats = {
    ...
    recentDevices: state.trustedDevices.filter((d) => {
      const daysSinceLastUsed =
        (Date.now() - d.lastUsedAt.getTime()) / (1000 * 60 * 60 * 24);
