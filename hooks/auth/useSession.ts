...
import { type SessionData, UnifiedSessionSystem } from '@/lib/auth/session';
...
export function useSession(options: UseSessionOptions = {}): UseSessionReturn {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      if (sessionId && sessionSystemRef.current) {
        await sessionSystemRef.current.terminateSession(
          sessionId,
          'user_logout'
        );
      }
  ...
  const isExpiringSoon =
    timeUntilExpiry !== null && timeUntilExpiry <= 5 * 60 * 1000; // 5 minutes
