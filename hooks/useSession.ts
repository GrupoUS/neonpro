...
export function useSession(): UseSessionReturn {
  ...
  const terminateAllSessions = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to terminate all sessions';
      setError(errorMessage);
    ...
    async (deviceId: string): Promise<void> => {
      try {
        ...
        const { data: deviceSessions } = await sessionManager.supabase
          ...
          .eq(
            'device_fingerprint',
            devices.find((d) => d.id === deviceId)?.device_fingerprint
          )
          .eq('is_active', true);
        ...
        if (deviceSessions) {
          await Promise.all(
            deviceSessions.map((s) =>
              sessionManager.terminateSession(s.id, 'device_blocked')
            )
          );
        }
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to block device';
        setError(errorMessage);
  ...
  const loadSecurityEvents = useCallback(
  ...
  const loadSessionAnalytics = useCallback(
      ...
      try {
        ...
        if (sessions) {
          const analytics: SessionAnalytics = {
            ...
            unique_devices: new Set(sessions.map((s) => s.device_fingerprint))
              .size,
...
export function useSecurityEvents(sessionId?: string): UseSecurityEventsReturn {
  ...
  const loadEvents = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const { data: securityEvents, error: loadError } =
        await sessionManager.supabase
          .from('session_security_events')
          .select('*')
          .eq('session_id', sessionId)
  ...
  const dismissEvent = useCallback(
    async (eventId: string): Promise<void> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to dismiss event';
        setError(errorMessage);
