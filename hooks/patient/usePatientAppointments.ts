...
export function usePatientAppointments(): UsePatientAppointmentsResult {
  ...
  const [cancellationPolicies, setCancellationPolicies] =
    useState<CancellationPolicies | null>(null);
    ...
    try {
      ...
      const processedAppointments = (appointments || []).map((apt) => {
        ...
        return {
          ...
          service_duration:
            apt.services?.duration_minutes || apt.duration || 60,
  ...
  const getNoShowPattern = useCallback(() => {
    ...
    const noShowRate =
      totalAppointments > 0 ? (noShows.length / totalAppointments) * 100 : 0;
  ...
  const getCancellationStats = useCallback(() => {
    const cancelled = pastAppointments.filter(
      (apt) => apt.status === 'cancelled'
    );
    const totalPast = pastAppointments.length;
    const cancellationRate =
      totalPast > 0 ? (cancelled.length / totalPast) * 100 : 0;
