...
interface UseConsentManagementReturn {
  ...
  updatePurpose: (
    id: string,
    updates: Partial<ConsentPurpose>
  ) => Promise<void>;
...
export function useConsentManagement(): UseConsentManagementReturn {
  ...
  const loadConsents = useCallback(async () => {
    try {
      ...
      const response: PaginatedResponse<ConsentRecord> =
        await complianceManager.getConsents({
          ...filters,
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Erro ao carregar consentimentos';
  ...
  const loadPurposes = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Erro ao carregar finalidades';
      setError(errorMessage);
      toast({
        title: 'Erro',
        description: errorMessage,
  ...
  const createPurpose = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Erro ao criar finalidade';
        setError(errorMessage);
        toast({
          title: 'Erro',
          description: errorMessage,
  ...
  const updatePurpose = useCallback(
    async (id: string, updates: Partial<ConsentPurpose>) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Erro ao atualizar finalidade';
        setError(errorMessage);
        toast({
          title: 'Erro',
          description: errorMessage,
  ...
  const deletePurpose = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Erro ao excluir finalidade';
        setError(errorMessage);
        toast({
          title: 'Erro',
          description: errorMessage,
  ...
  const withdrawConsent = useCallback(
    async (consentId: string, reason?: string) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Erro ao retirar consentimento';
        setError(errorMessage);
        toast({
          title: 'Erro',
          description: errorMessage,
  ...
  const exportConsents = useCallback(async () => {
    try {
      ...
      link.setAttribute(
        'download',
        `lgpd-consents-${new Date().toISOString().split('T')[0]}.csv`
      );
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Erro ao exportar consentimentos';
      setError(errorMessage);
      toast({
