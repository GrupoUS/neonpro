...
export function useComplianceAssessment(): UseComplianceAssessmentReturn {
  ...
  const createAssessment = useCallback(
    async (
      ...
      try {
        ...
        const newAssessment =
          await complianceManager.createComplianceAssessment(assessment);
  ...
  const runAutomatedAssessment = useCallback(async () => {
    ...
    try {
      ...
      const automatedAssessment: Omit<
        ComplianceAssessment,
      ...
      const newAssessment =
        await complianceManager.createComplianceAssessment(automatedAssessment);

      // Run automated assessment
      const result = await complianceManager.runAutomatedAssessment(
        newAssessment.id
      );
      ...
      setStatistics((prev) => ({
        ...
        averageScore:
          prev.completed > 0
            ? (prev.averageScore * (prev.completed - 1) + (result.score || 0)) /
              prev.completed
            : result.score || 0,
      }));
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Erro ao executar avaliação automatizada';
      setError(errorMessage);
      toast({
        title: 'Erro',
        description: errorMessage,
  ...
  const exportAssessments = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Erro ao exportar avaliações';
      setError(errorMessage);
      toast({
