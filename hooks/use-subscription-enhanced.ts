...
export function useSubscription(
  ...
  const fetchSubscription = useCallback(
    async (
      ...
      try {
            ...
            if (!isStale || backgroundUpdate) {
              ...
              setState((prev) => ({
                ...
                performance: {
                  ...
                  cacheHitRate:
                    (prev.performance.cacheHitRate *
                      prev.performance.totalFetches +
                      1) /
        ...
        const fetchPromise = subscriptionQueryOptimizer.getSubscriptionStatus(
          currentUserId,
          {
        ...
        setState((prev) => {
          ...
          const newAverageResponseTime =
            (prev.performance.averageResponseTime *
              prev.performance.totalFetches +
              duration) /
            newTotalFetches;
        ...
        if (performanceTimerRef.current) {
          subscriptionPerformanceMonitor.endTimer(
   ...
   */
  const refetch =
    useCallback(async (): Promise<SubscriptionValidationResult | null> => {
...
export function useSubscriptionAccess(userId?: string): {
  ...
  const { hasAccess, isLoading, canAccessFeature, refetch } = useSubscription(
    userId,
    {
      enableBackgroundSync: true,
      cacheStrategy: 'adaptive',
...
export function useSubscriptionLimits(userId?: string): {
  ...
  const { getUsageLimit, subscription, isLoading, refetch } = useSubscription(
    userId,
    {
      enableRealtime: true,
