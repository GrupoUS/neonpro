...
export interface UseAlternativeSlotsResult {
  suggestions: AlternativeSlot[];
  isLoading: boolean;
  error: string | null;
...
export function useAlternativeSlots(): UseAlternativeSlotsResult {
  ...
  const [selectedSuggestion, setSelectedSuggestion] =
    useState<AlternativeSlot | null>(null);
  const [searchMetadata, setSearchMetadata] = useState<
    AlternativeSlotsResponse['metadata'] | null
  >(null);
  const [_performanceMetrics, _setPerformanceMetrics] =
    useState<PerformanceMetrics | null>(null);

  // Enhanced suggestion retrieval with performance tracking
  const getSuggestions = useCallback(
    async (request: AlternativeSlotsRequest) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.details ||
              errorData.error ||
              'Failed to get alternative suggestions'
          );
        }
...
function _calculatePreferenceMatch(
  ...
  if (request.preferred_times && request.preferred_times.length > 0) {
    ...
    const timeMatch = request.preferred_times.some(
      (pref) =>
        Math.abs(
          dayjs(`1970-01-01 ${suggestionTime}`).diff(
            dayjs(`1970-01-01 ${pref}`),
            'minutes'
          )
        ) <= 30
    );
...
export function filterSuggestionsWithDayjs(
  ...
  return suggestions.filter((suggestion) => {
    const suggestionDayjs =
      suggestion.dayjs_instance || dayjs(suggestion.start_time);
