...
} from '@/hooks/use-realtime-availability';
...
export function useAvailabilityManager() {
  ...
  const { timeSlots, isConnected, isLoading, error, bookSlot, refetch } =
    useRealtimeAvailability({
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<AvailabilityFilters>) => {
      setFilters((prev) => ({
        ...prev,
        ...newFilters,
      }));
  ...
  const hasTimeConflict = useCallback(
    (
      newSlot: { date: string; start_time: string; end_time: string },
      ...
      return timeSlots.some((existingSlot) => {
        ...
        const existingStart = new Date(
          `${existingSlot.date}T${existingSlot.start_time}`
        );
        const existingEnd = new Date(
          `${existingSlot.date}T${existingSlot.end_time}`
        );
