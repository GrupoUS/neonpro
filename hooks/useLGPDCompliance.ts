...
export const useConsentManagement = (
  ...
  const refreshConsents = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to fetch consents';
      setError(errorMessage);
  ...
  const activeConsents = consents.filter(
    (consent) => consent.consent_status === 'active'
  );

  const checkConsent = useCallback(
    (consentType: LGPDConsentType): boolean => {
      return activeConsents.some(
        (consent) => consent.consent_type === consentType
      );
    },
    [activeConsents]
  );
  ...
  const grantConsent = useCallback(
    async (
      ...
      try {
        ...
        const consentData = lgpdUtils.generateConsentFormData(
          consentType,
          purpose
        );
...
export const useDataSubjectRights = (
  ...
  const downloadData = useCallback(async (): Promise<{
    success: boolean;
    data?: any;
  }> => {
    if (!(patientId && clinicId)) {
  ...
  const requestDeletion = useCallback(
    async (reason?: string): Promise<{ success: boolean }> => {
      return await submitRequest(
        'erasure',
        reason ||
          'Patient requests deletion of personal data under LGPD Article 18',
...
export const useLGPDComplianceStatus = (
  patientId?: string,
  clinicId?: string
) => {
