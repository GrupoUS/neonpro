...
export function useRealTimeAvailability({
  ...
  const fetchAvailability = useCallback(async () => {
    try {
      ...
      const filteredSlots = (slots || []).filter(slot => 
        !slot.reserved_until || slot.reserved_until < now
      )
   ...
   */
  const reserveSlotOptimistic = useCallback(async (slotId: string, patientId: string) => {
  ...
  const calculateSlotScore = (slot: TimeSlot, preferredDate: Date, preferredTime: Date) => {
