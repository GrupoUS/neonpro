  ...
  const fetchAvailability = useCallback(async () => {
    try {
      ...
      const filteredSlots = (slots || []).filter(
        (slot) => !slot.reserved_until || slot.reserved_until < now
      );
      ...
      channelRef.current = supabase
        ...
        .subscribe((status) => {
          ...
          if (status === 'SUBSCRIBED') {
            ...
            console.error(
              'Real-time subscription error, attempting reconnect...'
            );
  ...
  const reserveSlotOptimistic = useCallback(
    async (slotId: string, patientId: string) => {
      ...
      setState((prev) => ({
        ...prev,
        optimisticUpdates: new Map(
          prev.optimisticUpdates.set(slotId, optimisticSlot)
        ),
  ...
  const getAlternativeSuggestions = useCallback(
    (preferredSlot: TimeSlot, maxAlternatives = 3): TimeSlot[] => {
      ...
      const preferredTime = new Date(
        `${preferredSlot.date}T${preferredSlot.time}`
      );
