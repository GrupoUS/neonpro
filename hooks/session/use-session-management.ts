...
import { SessionStorage, SessionUtils } from '@/lib/auth/utils/session-utils';
import type {
  SecurityEvent,
  SessionAnalytics,
  SuspiciousActivity,
...
export function useSessionManagement(): SessionManagementState &
  ...
  const loadAnalytics = async (_userId: string, timeframe = '7d') => {
    try {
      const response = await fetch(
        `/api/auth/session/analytics?timeframe=${timeframe}`,
        {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  const registerDevice = useCallback(
    async (deviceInfo: Partial<UserDevice>): Promise<boolean> => {
      try {
