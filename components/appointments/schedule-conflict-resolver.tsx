...
export function ScheduleConflictResolver({
  ...
  const generateSuggestions = (conflictGroup: typeof conflictGroups[0]) => {
    ...
    const sortedAppointments = conflictGroup.appointments.sort((a, b) => a.start.getTime() - b.start.getTime())
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">
                    {moment(group.timeRange.start).format('DD/MM HH:mm')} - {moment(group.timeRange.end).format('HH:mm')}
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                            ...
                            <div className="mt-3 pt-3 border-t">
                              <div className="flex items-center space-x-2 text-sm">
                                ...
                                <span className="font-medium">
                                  {moment(resolution.newStart).format('DD/MM HH:mm')} - {moment(resolution.newEnd).format('HH:mm')}
                                </span>
                              </div>
                              {resolution.reason && (
                                <p className="text-xs text-muted-foreground mt-1">{resolution.reason}</p>
                              )}
                            </div>
                          )}
                        </div>
                          ...
                          <div className="pl-4 border-l-2 border-primary/20">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const suggestions = generateSuggestions(group)
                                  const suggestion = suggestions.find(s => s.appointmentId === appointment.id)
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() => updateResolution(appointment.id, { action: 'cancel' })}
                                className="justify-start"
                              >
