...
const ERROR_PATTERNS = {
  ...
  UNUSED_VARS: {
    pattern:
      /const ([a-zA-Z_][a-zA-Z0-9_]*) = ([^;]+);[\s]*\/\/ Variable '[^']+' is declared but never used/g,
    replacement: (match, varName, assignment) =>
      `const _${varName} = ${assignment};`,
  },
};
...
class ComprehensiveErrorFixer {
  ...
  hasUnusedImport(content, importName) {
    ...
    return (
      (matches.length === 1 && content.includes(`${importName},`)) ||
  ...
  async run() {
    ...
    const targetDirs = ["./apps/api/src", "./apps/web/src", "./packages"];
