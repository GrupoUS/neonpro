...
export function middleware(request: NextRequest) {
  ...
  response.headers.set('X-Content-Security-Policy', 'healthcare-data-protected');
  ...
  if (request.nextUrl.pathname.startsWith('/api/')) {
    response.headers.set('Access-Control-Allow-Origin', process.env.NEXT_PUBLIC_ALLOWED_ORIGINS || 'https://neonpro.healthcare');
  ...
  const sensitivePaths = ['/api/patient', '/api/medical', '/api/appointment', '/api/prescription'];
  if (sensitivePaths.some(path => request.nextUrl.pathname.startsWith(path))) {
    console.log(`[AUDIT] ${new Date().toISOString()} - ${request.method} ${request.nextUrl.pathname} - ${request.ip}`);
  }
