...
class AdvancedUnderscoreFixer {
  ...
  shouldProcessFile(filePath) {
    ...
    const skipDirs = ['node_modules', 'dist', 'build', '.git', '.next', '.turbo', '.tscache'];
  ...
  fixRemainingPatterns(content) {
    ...
    const patterns = [
      ...
      { regex: /\.listen\(_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*_\(\s*\)\s*=>/g, replacement: '.listen($1, () =>', count: 0 },
      
      // Async function declarations with underscores
      { regex: /setTimeout\(_async\s*\(\s*\)\s*=>/g, replacement: 'setTimeout(async () =>', count: 0 },
      { regex: /setInterval\(_async\s*\(\s*\)\s*=>/g, replacement: 'setInterval(async () =>', count: 0 },
      
      // Function calls with underscore parameters
      { regex: /Promise\.(_\s*resolve|_\s*reject|_resolve|_reject)\s*\(/g, replacement: 'Promise.$1(', count: 0 },
      
      // Mixed parameter lists with underscores
      { regex: /\(_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*\(\s*\)\s*=>/g, replacement: '($1, $2, () =>', count: 0 },
      
      // Array access with underscore strings
      { regex: /\[\s*_'([^']+)'\s*\]/g, replacement: "['$1']", count: 0 },
      
      // Object property access with underscores
      { regex: /\.(_[a-zA-Z_][a-zA-Z0-9_]*)\s*\./g, replacement: '.$1.', count: 0 },
      
      // Event handler parameters with mixed patterns
      { regex: /\.on\(_'([^']+)',\s*_\s*\(\s*([^)]+)\s*\)/g, replacement: ".on('$1', ($2)", count: 0 },
      
      // Function declarations with underscore prefixes
      { regex: /function\s+_([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/g, replacement: 'function $1(', count: 0 },
      
      // Variable declarations with underscore prefixes in certain contexts
      { regex: /const\s+_([a-zA-Z_][a-zA-Z0-9_]*)\s*=/g, replacement: 'const $1 =', count: 0 },
      
      // Catch parameters with underscores
      { regex: /catch\s*\(_([a-zA-Z_][a-zA-Z0-9_]*)\)/g, replacement: 'catch ($1)', count: 0 },
      
      // Arrow functions with single underscore parameter
      { regex: /_\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*=>/g, replacement: '$1 =>', count: 0 },
      
      // Template literals with underscore prefixes
      { regex: /_`([^`]+)`/g, replacement: '`$1`', count: 0 },
      
      // Method calls with underscore prefixes
      { regex: /(_[a-zA-Z_][a-zA-Z0-9_]*)\s*\.\s*([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: '$1.$2', count: 0 },
      
      // Import/export statements with underscores
      { regex: /import\s+_\s*([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: 'import $1', count: 0 },
      { regex: /export\s+_\s*([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: 'export $1', count: 0 },
      
      // Type annotations with underscores
      { regex: /:\s*_([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: ': $1', count: 0 },
      
      // Return statements with underscores
      { regex: /return\s+_([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: 'return $1', count: 0 },
      
      // New expressions with underscores
      { regex: /new\s+_([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: 'new $1', count: 0 },
      
      // Throw statements with underscores
      { regex: /throw\s+_([a-zA-Z_][a-zA-Z0-9_]*)/g, replacement: 'throw $1', count: 0 },
    ];
    ...
    for (const pattern of patterns) {
      ...
      if (matches) {
        ...
        if (matches.length > 0) {
          this.log(`Applied pattern: ${pattern.regex.toString()} (${matches.length} times)`, 'debug');
        }
      }
    }
  ...
  processFile(filePath) {
    try {
      ...
      if (result.fixes > 0) {
        ...
        this.log(`Fixed ${result.fixes} remaining issues in ${filePath}`, 'success');
  ...
  processDirectory(dirPath, maxFiles = 1000) {
    this.log(`Processing directory for remaining patterns: ${dirPath} (max ${maxFiles} files)`);
      ...
      try {
        ...
        for (const item of items) {
          ...
          try {
            ...
            this.log(`Cannot access ${fullPath}: ${statError.message}`, 'debug');
          }
        }
      } catch (readdirError) {
        // Skip directories that can't be read
        this.log(`Cannot read directory ${currentPath}: ${readdirError.message}`, 'debug');
      }
    };
  ...
  run() {
    ...
    try {
      ...
      this.log('详细报告已保存到: remaining-underscore-fix-report.json', 'success');
    } catch (saveError) {
