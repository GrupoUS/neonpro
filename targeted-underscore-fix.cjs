    ...
    this.targetPatterns = [
      {
        ...
        regex: /\.on\(_'([^']+)',_\s*\(([^)]+)\)/g,
        replacement: '.on(\'$1\', ($2)',
        files: ['apps/api/src/config/https-server.ts', 'apps/api/src/config/certificate-renewal.ts']
      },
      {
    ...
    try {
      ...
      for (const pattern of this.targetPatterns) {
        if (pattern.files.includes(filePath)) {
          ...
          if (matches) {
            ...
            this.log(`Applied ${pattern.name} to ${filePath}: ${matches.length} matches`, 'debug');
          }
        }
      }
      ...
      if (filePath.includes('https-server.ts')) {
        ...
        const fixes = [
          ...
          { from: '.on(_\'request\',_(req,_res) => {', to: '.on(\'request\', (req, res) => {' },
      ...
      if (filePath.includes('certificate-renewal.ts')) {
        ...
        for (const fix of fixes) {
          const matches = (content.match(new RegExp(fix.from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
          if (matches > 0) {
            content = content.replace(new RegExp(fix.from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), fix.to);
            fileFixes += matches;
          }
