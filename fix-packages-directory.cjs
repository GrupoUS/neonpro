...
class PackagesDirectoryFixer {
  ...
  fixUnderscorePatterns(content) {
    ...
    const patterns = [
      ...
      { regex: /\.on\(_'([^']+)',_\s*\(([^)]+)\)/g, replacement: '.on(\'$1\', ($2)', count: 0 },
      
      // Timer functions with underscore callbacks
      { regex: /(setInterval|setTimeout|setImmediate)\s*\(_\(\s*\)\s*=>/g, replacement: '$1(() =>', count: 0 },
      
      // Promise with underscore parameters
      { regex: /Promise\(_\(([^)]+)\)\)/g, replacement: 'Promise(($1))', count: 0 },
      
      // Process event handlers
      { regex: /process\.on\(_'([^']+)',_\s*\(\s*\)\s*=>/g, replacement: "process.on('$1', () =>", count: 0 },
      
      // HTTP createServer with underscore parameters
      { regex: /http\.createServer\(_\(([^)]+)\)\)/g, replacement: 'http.createServer(($1))', count: 0 },
      
      // Server listen with underscores
      { regex: /\.listen\(_([^,]+),\s*_([^,]+),\s*_\(\s*\)\s*=>/g, replacement: '.listen($1, $2, () =>', count: 0 },
      
      // Single parameter with underscore
      { regex: /\(_([a-zA-Z_][a-zA-Z0-9_]*)\)/g, replacement: '($1)', count: 0 },
      
      // Multiple parameters with underscores
      { regex: /\(_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*_([a-zA-Z_][a-zA-Z0-9_]*)\)/g, replacement: '($1, $2)', count: 0 },
      
      // Triple parameters with underscores
      { regex: /\(_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*_([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*_([a-zA-Z_][a-zA-Z0-9_]*)\)/g, replacement: '($1, $2, $3)', count: 0 },
      ...
      { regex: /_\s*\.\s*(on|listen|then|catch|finally)/g, replacement: '.$1', count: 0 },
    ];
  ...
  run() {
    this.log('Starting packages directory underscore fix...');
    ...
    const packagesDirs = [
      ...
      'packages/core-services/tests'
    ];

    for (const dir of packagesDirs) {
      if (fs.existsSync(dir)) {
        this.processDirectory(dir, 200); // Limit files per directory
      }
    }

    const report = this.generateReport();
    
    this.log('=== PACKAGES DIRECTORY 修复总结 ===', 'info');
    ...
    try {
      fs.writeFileSync(
        'packages-directory-fix-report.json', 
        JSON.stringify(report, null, 2)
      );
      this.log('详细报告已保存到: packages-directory-fix-report.json', 'success');
    } catch (saveError) {
...
// Run the fixer
const fixer = new PackagesDirectoryFixer();
fixer.run();
