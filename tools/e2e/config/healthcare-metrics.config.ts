 ...
 */
export const HEALTHCARE_PERFORMANCE_BUDGETS: Record<string, HealthcarePerformanceBudget> = {
...
export const HEALTHCARE_PERFORMANCE_ASSERTIONS = {
  ...
  patientSafety: {
    criticalDataLoad: {
      ...
      assertion: (metrics: any) => {
        const criticalMetrics = ['patientDataLoadTime', 'prescriptionLoadTime', 'medicalRecordAccessTime'];
        return criticalMetrics.every(metric => {
          const value = metrics[metric];
          const threshold = HEALTHCARE_CRITICAL_METRICS.find(m => m.name === metric)?.threshold;
    ...
    emergencyResponse: {
      ...
      assertion: (metrics: any, pageType: string) => {
        if (pageType !== 'emergency') {return true;}
  ...
  userExperience: {
    coreWebVitals: {
      ...
      assertion: (metrics: any, pageType: string = 'clinical') => {
        const budget = HEALTHCARE_PERFORMANCE_BUDGETS[pageType] || HEALTHCARE_PERFORMANCE_BUDGETS.clinical;
        return (
          metrics.lcp <= budget.lcp &&
 ...
 */
export function getHealthcarePerformanceBudget(pageType: string): HealthcarePerformanceBudget {
  return HEALTHCARE_PERFORMANCE_BUDGETS[pageType] || HEALTHCARE_PERFORMANCE_BUDGETS.clinical;
}
 ...
 */
export function getCriticalHealthcareMetrics(pageType: string): HealthcareMetric[] {
  if (isCriticalHealthcarePage(pageType)) {
    return HEALTHCARE_CRITICAL_METRICS;
  }
  return HEALTHCARE_CRITICAL_METRICS.filter(metric => metric.category === 'compliance');
}
...
export function validateHealthcareMetrics(
  ...
  criticalMetrics.forEach(metric => {
    ...
    if (metrics[metric.name] && budgetValue && metrics[metric.name] > budgetValue) {
      failures.push({
        metric: metric.name,
