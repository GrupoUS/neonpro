  ...
  try {
    for (const user of testUsers) {
      ...
      if (authError && !authError.message.includes('already registered')) {
        console.warn(`Aviso ao criar usuário ${user.email}:`, authError.message);
        continue;
      }
      ...
      if (authData.user) {
        ...
        if (profileError) {
          console.warn(`Aviso ao criar perfil para ${user.email}:`, profileError.message);
        }
      }
    }
...
async function verifyApplicationHealth() {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`⏳ Tentativa ${attempt}/${maxRetries} - Aguardando aplicação...`);
      
      if (attempt === maxRetries) {
        throw new Error(`Aplicação não está acessível em ${baseURL} após ${maxRetries} tentativas`);
      }
