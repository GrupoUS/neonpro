...
export class PatientsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.pageHeader = page.locator('[data-testid="patients-header"], .patients-header, h1');
    ...
    this.searchInput = page.locator('[data-testid="patients-search"], .patients-search, input[placeholder*="Search patients"]');
    this.searchButton = page.locator('[data-testid="search-button"], .search-button');
    this.clearSearchButton = page.locator('[data-testid="clear-search"], .clear-search');
    this.filterButton = page.locator('[data-testid="filter-button"], .filter-button');
    ...
    this.ageFilter = page.locator('[data-testid="age-filter"], select[name="age"]');
    this.genderFilter = page.locator('[data-testid="gender-filter"], select[name="gender"]');
    this.dateRangeFilter = page.locator('[data-testid="date-range-filter"], .date-range-filter');
    
    // Initialize action button locators
    this.addPatientButton = page.locator('[data-testid="add-patient-button"], .add-patient-button');
    this.importPatientsButton = page.locator('[data-testid="import-patients-button"], .import-patients-button');
    this.exportPatientsButton = page.locator('[data-testid="export-patients-button"], .export-patients-button');
    this.bulkActionsButton = page.locator('[data-testid="bulk-actions-button"], .bulk-actions-button');
    
    // Initialize table locators
    this.patientsTable = page.locator('[data-testid="patients-table"], .patients-table, table');
    this.patientsTableHeader = page.locator('[data-testid="patients-table-header"], thead');
    this.patientsTableBody = page.locator('[data-testid="patients-table-body"], tbody');
    this.patientRows = page.locator('[data-testid="patient-row"], .patient-row, tbody tr');
    this.selectAllCheckbox = page.locator('[data-testid="select-all-checkbox"], .select-all-checkbox');
    this.patientCheckboxes = page.locator('[data-testid="patient-checkbox"], .patient-checkbox');
    
    // Initialize column locators
    this.nameColumn = page.locator('[data-testid="name-column"], .name-column');
    this.emailColumn = page.locator('[data-testid="email-column"], .email-column');
    this.phoneColumn = page.locator('[data-testid="phone-column"], .phone-column');
    this.ageColumn = page.locator('[data-testid="age-column"], .age-column');
    this.statusColumn = page.locator('[data-testid="status-column"], .status-column');
    this.actionsColumn = page.locator('[data-testid="actions-column"], .actions-column');
    
    // Initialize pagination locators
    this.pagination = page.locator('[data-testid="pagination"], .pagination');
    this.previousPageButton = page.locator('[data-testid="previous-page"], .previous-page');
    ...
    this.totalItemsCount = page.locator('[data-testid="total-items"], .total-items');
    
    // Initialize action locators
    this.viewPatientButtons = page.locator('[data-testid="view-patient"], .view-patient');
    this.editPatientButtons = page.locator('[data-testid="edit-patient"], .edit-patient');
    this.deletePatientButtons = page.locator('[data-testid="delete-patient"], .delete-patient');
    this.patientMenuButtons = page.locator('[data-testid="patient-menu"], .patient-menu');
    
    // Initialize modal locators
    this.deleteConfirmModal = page.locator('[data-testid="delete-confirm-modal"], .delete-confirm-modal');
    this.confirmDeleteButton = page.locator('[data-testid="confirm-delete"], .confirm-delete');
    this.cancelDeleteButton = page.locator('[data-testid="cancel-delete"], .cancel-delete');
  ...
  async clearAllFilters() {
    const clearFiltersButton = this.filterPanel.locator('[data-testid="clear-filters"], .clear-filters');
  ...
  async getPatientDataFromRow(rowIndex: number): Promise<{
    ...
    return {
      name: await row.locator('.name-cell, [data-testid="name-cell"]').textContent() || '',
      email: await row.locator('.email-cell, [data-testid="email-cell"]').textContent() || '',
      phone: await row.locator('.phone-cell, [data-testid="phone-cell"]').textContent() || '',
      age: await row.locator('.age-cell, [data-testid="age-cell"]').textContent() || '',
      status: await row.locator('.status-cell, [data-testid="status-cell"]').textContent() || '',
    };
  }
  ...
  async performBulkAction(action: string) {
    ...
    const actionButton = this.page.locator(`[data-testid="bulk-${action}"], .bulk-${action}`);
  ...
  async exportPatients(format: 'csv' | 'excel' | 'pdf' = 'csv') {
    ...
    const exportButton = this.page.locator(`[data-testid="export-${format}"], .export-${format}`);
  ...
  async expectPatientRowsToBeVisible(minCount: number = 1) {
    ...
    if (rowCount < minCount) {
      throw new Error(`Expected at least ${minCount} patient rows, but found ${rowCount}`);
    }
  }
  ...
  async expectPatientNotInResults(patientName: string) {
    ...
    if (isVisible) {
      throw new Error(`Expected patient "${patientName}" not to be in results, but it was found`);
    }
  }

  // Sorting methods
  async sortByColumn(columnName: string) {
    const columnHeader = this.patientsTableHeader.locator(`text="${columnName}"`);
  ...
  async expectColumnToBeSorted(columnName: string, direction: 'asc' | 'desc') {
    const columnHeader = this.patientsTableHeader.locator(`text="${columnName}"`);
    const sortIndicator = columnHeader.locator(`.sort-${direction}, [data-sort="${direction}"]`);
