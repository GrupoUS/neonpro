...
export class AppointmentsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Botões
    this.newAppointmentBtn = page.locator('[data-testid="new-appointment-btn"]');
  ...
  async createAppointment(appointmentData: {
    ...
    await this.page.locator(`[data-value="${appointmentData.patientId}"]`).click();
    
    await this.doctorSelect.click();
    await this.page.locator(`[data-value="${appointmentData.doctorId}"]`).click();
    ...
    await this.page.locator(`[data-value="${appointmentData.duration}"]`).click();
  ...
  async editAppointment(appointmentId: string, updates: Record<string, string>): Promise<void> {
  ...
  async rescheduleAppointment(appointmentId: string, newDate: string, newTime: string): Promise<void> {
    await this.clickAppointment(appointmentId);
    await this.page.locator('[data-testid="reschedule-appointment-btn"]').click();
  ...
  async getAppointmentDetails(appointmentId: string): Promise<Record<string, string>> {
    const appointmentCard = this.page.locator(`[data-appointment-id="${appointmentId}"]`);
    
    return {
      patient: await appointmentCard.locator('[data-testid="patient-name"]').textContent() || '',
      doctor: await appointmentCard.locator('[data-testid="doctor-name"]').textContent() || '',
      date: await appointmentCard.locator('[data-testid="appointment-date"]').textContent() || '',
      time: await appointmentCard.locator('[data-testid="appointment-time"]').textContent() || '',
      status: await appointmentCard.locator('[data-testid="appointment-status"]').textContent() || ''
    };
  }
