...
export class PrescriptionPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.instructionsTextarea = page.locator('[data-testid="instructions-textarea"]');
    this.prescriberSelect = page.locator('[data-testid="prescriber-select"]');
    this.submitPrescriptionButton = page.locator('[data-testid="submit-prescription-button"]');
    this.prescriptionConfirmation = page.locator('[data-testid="prescription-confirmation"]');
    
    // Prescription history selectors
    this.prescriptionHistory = page.locator('[data-testid="prescription-history"]');
    this.activePrescritions = page.locator('[data-testid="active-prescriptions"]');
    this.pastPrescriptions = page.locator('[data-testid="past-prescriptions"]');
    this.prescriptionSearch = page.locator('[data-testid="prescription-search"]');
    this.prescriptionFilters = page.locator('[data-testid="prescription-filters"]');
    
    // Drug interaction selectors
    this.interactionChecker = page.locator('[data-testid="interaction-checker"]');
    this.interactionWarnings = page.locator('[data-testid="interaction-warnings"]');
    this.majorInteractionAlert = page.locator('[data-testid="major-interaction-alert"]');
    this.moderateInteractionWarning = page.locator('[data-testid="moderate-interaction-warning"]');
    this.minorInteractionInfo = page.locator('[data-testid="minor-interaction-info"]');
    this.contraindicationAlert = page.locator('[data-testid="contraindication-alert"]');
    
    // Medication database selectors
    this.medicationSearch = page.locator('[data-testid="medication-search"]');
    this.medicationSuggestions = page.locator('[data-testid="medication-suggestions"]');
    ...
    this.electronicPrescription = page.locator('[data-testid="electronic-prescription"]');
    this.pharmacySelect = page.locator('[data-testid="pharmacy-select"]');
    this.transmissionStatus = page.locator('[data-testid="transmission-status"]');
    
    // Audit selectors
    this.prescriptionAudit = page.locator('[data-testid="prescription-audit"]');
    this.controlledSubstanceLog = page.locator('[data-testid="controlled-substance-log"]');
  }
  ...
  async checkDrugInteractions(medications: string[]): Promise<DrugInteractionResult> {
    ...
    await this.page.evaluate((meds) => {
      window.dispatchEvent(new CustomEvent('check-drug-interactions', {
    ...
    const interactions = await this.page.evaluate(() => {
      const interactionElements = document.querySelectorAll('[data-testid^="interaction-"]');
      const results = [];
      
      interactionElements.forEach(element => {
        const severity = element.getAttribute('data-severity') as 'minor' | 'moderate' | 'major' | 'contraindicated';
  ...
  async fillPrescriptionForm(formData: PrescriptionFormData): Promise<void> {
    ...
    try {
      await this.medicationSuggestions.waitFor({ state: 'visible', timeout: 2000 });
      await this.page.click(`[data-testid="medication-suggestion-${formData.medication.toLowerCase().replace(/\s+/g, '-')}"]`);
    } catch {
  ...
  async getMedicationInfo(medicationName: string): Promise<{
    ...
    await this.page.click(`[data-testid="medication-${medicationName.toLowerCase().replace(/\s+/g, '-')}"]`);
    ...
    const allergyWarnings = await this.page.evaluate(() => {
      const warningElements = document.querySelectorAll('[data-testid="allergy-warning"]');
  ...
  async filterPrescriptions(filters: {
    ...
    if (filters.dateRange) {
      await this.page.fill('[data-testid="start-date-filter"]', filters.dateRange.start);
      await this.page.fill('[data-testid="end-date-filter"]', filters.dateRange.end);
    }
    
    if (filters.prescriber) {
      await this.page.selectOption('[data-testid="prescriber-filter"]', filters.prescriber);
    }
    
    if (filters.medication) {
      await this.page.fill('[data-testid="medication-filter"]', filters.medication);
    }
  ...
  async printPrescription(prescriptionId: string): Promise<void> {
    await this.page.click(`[data-testid="print-prescription-${prescriptionId}"]`);
   ...
   */
  async sendElectronicPrescription(prescriptionId: string, pharmacyId: string): Promise<void> {
    await this.page.click(`[data-testid="electronic-prescription-${prescriptionId}"]`);
   ...
   */
  async discontinuePrescription(prescriptionId: string, reason: string): Promise<void> {
    await this.page.click(`[data-testid="discontinue-prescription-${prescriptionId}"]`);
    ...
    await this.page.locator('[data-testid="discontinuation-confirmation"]').waitFor({ state: 'visible' });
  }
   ...
   */
  async renewPrescription(prescriptionId: string, renewalData: {
    duration: string;
    refills: number;
    instructions?: string;
  }): Promise<void> {
    await this.page.click(`[data-testid="renew-prescription-${prescriptionId}"]`);
    
    // Fill renewal information
    await this.page.fill('[data-testid="renewal-duration"]', renewalData.duration);
    await this.page.fill('[data-testid="renewal-refills"]', renewalData.refills.toString());
    
    if (renewalData.instructions) {
      await this.page.fill('[data-testid="renewal-instructions"]', renewalData.instructions);
    }
    ...
    await this.page.locator('[data-testid="renewal-confirmation"]').waitFor({ state: 'visible' });
  }
  ...
  async checkControlledSubstanceCompliance(prescriptionId: string): Promise<{
    ...
    return await this.page.evaluate(() => {
      const complianceData = document.querySelector('[data-testid="compliance-data"]');
      if (!complianceData) {
        return {
      ...
      return {
        ...
        complianceStatus: complianceData.getAttribute('data-status') as 'compliant' | 'warning' | 'violation',
        requirements: JSON.parse(complianceData.getAttribute('data-requirements') || '[]')
      };
    });
  }
}