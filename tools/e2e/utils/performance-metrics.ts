...
export class PerformanceMonitor {
   ...
   */
  async measurePatientDataLoad(selector: string = '[data-testid="patient-list"]'): Promise<number> {
   ...
   */
  async measureFormSubmission(submitSelector: string, successSelector: string): Promise<number> {
   ...
   */
  async measureNavigation(navigationAction: () => Promise<void>, expectedUrl?: string): Promise<number> {
   ...
   */
  async measureSearchResponse(searchInput: string, searchQuery: string, resultsSelector: string): Promise<number> {
   ...
   */
  validateBudget(metrics: PerformanceMetrics, budget: PerformanceBudget = DEFAULT_PERFORMANCE_BUDGET): {
    passed: boolean
...
<body>
    <div class="container">
        ...
        <div class="test-results">
            ${this.metricsHistory.map(metric => `
                <div class="test-item">
  ...
  generateSummary(): {
    ...
    const budgetViolations = this.metricsHistory.filter(metrics => {
      const monitor = new PerformanceMonitor({} as Page, {} as BrowserContext, '')
