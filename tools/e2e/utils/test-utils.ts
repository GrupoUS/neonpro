...
export class TestUtils {
  ...
  static async loginAndNavigateToDashboard(
    page: Page,
    credentials: { email: string; password: string } = TestUtils.getValidCredentials()
   ...
   */
  static async waitForPageLoad(page: Page, timeout: number = 10_000): Promise<void> {
    await Promise.all([
  ...
  static getCredentialsByUserType(userType: string) {
    const credentials = {
      ...
      receptionist: { email: 'receptionist@neonpro.com', password: 'receptionist123' }
    };
  ...
  private static generateCPF(): string {
    ...
    const digit2 = [...numbers, digit1].reduce((sum, num, index) => sum + num * (11 - index), 0) % 11;
    
    return [...numbers, digit1 < 2 ? 0 : 11 - digit1, digit2 < 2 ? 0 : 11 - digit2]
      .join('')
