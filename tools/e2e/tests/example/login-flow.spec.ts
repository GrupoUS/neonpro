...
test.describe('Login Flow', () => {
  ...
  test('should redirect to dashboard after successful login', async ({ page }) => {
    ...
    await page.waitForURL(/\/dashboard/, { timeout: TEST_CONSTANTS.TIMEOUTS.MEDIUM });
  ...
  test('should logout successfully', async ({ page }) => {
    ...
    // Assert
    await page.waitForURL(/\/login/, { timeout: TEST_CONSTANTS.TIMEOUTS.MEDIUM });
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    await page.waitForURL(/\/dashboard/, { timeout: TEST_CONSTANTS.TIMEOUTS.EXTRA_LONG });
  });
...
test.describe('Login API Integration', () => {
  ...
  test('should send correct login payload', async ({ page }) => {
    ...
    await page.route(TEST_CONSTANTS.API_ENDPOINTS.LOGIN, async (route, request) => {
      requestPayload = request.postDataJSON();
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ token: 'fake-token', user: { id: 1, email: credentials.email } })
      });
    });
