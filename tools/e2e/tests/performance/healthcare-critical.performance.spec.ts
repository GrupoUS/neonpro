...
test.describe('Healthcare Critical Performance Tests', () => {
  ...
  test.afterEach(async ({ page }) => {
    ...
    const report = performanceReporter.generateReport(metrics, 'healthcare-critical');
  ...
  test.describe('Emergency Scenarios - Critical Patient Safety', () => {
    test('Emergency patient data load - Life critical performance', async ({ page }) => {
      ...
      await performanceMonitor.recordCustomTiming('patientDataLoadTime', patientDataLoadTime);
      
      // Validate against emergency budget
      expect(patientDataLoadTime).toBeLessThanOrEqual(budget.patientDataLoadTime);
      ...
      await performanceMonitor.recordCustomTiming('prescriptionLoadTime', prescriptionLoadTime);
      expect(prescriptionLoadTime).toBeLessThanOrEqual(budget.prescriptionLoadTime);
      ...
      if (!validation.passed) {
        ...
        if (criticalFailures.length > 0) {
          throw new Error(`Critical healthcare metrics failed: ${JSON.stringify(criticalFailures, null, 2)}`);
        }
      }
      
      // Validate patient safety assertions
      expect(HEALTHCARE_PERFORMANCE_ASSERTIONS.patientSafety.criticalDataLoad.assertion(metrics)).toBe(true);
      expect(HEALTHCARE_PERFORMANCE_ASSERTIONS.patientSafety.emergencyResponse.assertion(metrics, 'emergency')).toBe(true);
    });
    ...
    test('Emergency diagnostic image load - Critical imaging performance', async ({ page }) => {
      ...
      const startTime = Date.now();
      await emergencyPage.loadDiagnosticImages('EMERGENCY-001', ['xray', 'ct-scan']);
      const diagnosticImageLoadTime = Date.now() - startTime;
      
      await performanceMonitor.recordCustomTiming('diagnosticImageLoadTime', diagnosticImageLoadTime);
      
      // Emergency diagnostic images must load within strict timeframe
      expect(diagnosticImageLoadTime).toBeLessThanOrEqual(budget.diagnosticImageLoadTime);
    ...
    test('Emergency form submission - Critical data entry performance', async ({ page }) => {
      ...
      await performanceMonitor.recordCustomTiming('formSubmissionTime', formSubmissionTime);
  ...
  test.describe('Clinical Workflows - Standard Patient Care', () => {
    test('Patient dashboard load - Standard clinical performance', async ({ page }) => {
      ...
      await performanceMonitor.recordCustomTiming('searchResponseTime', searchResponseTime);
    ...
    test('Medical record access - Clinical workflow performance', async ({ page }) => {
      ...
      await performanceMonitor.recordCustomTiming('medicalRecordAccessTime', medicalRecordAccessTime);
      expect(medicalRecordAccessTime).toBeLessThanOrEqual(budget.medicalRecordAccessTime);
      ...
      await performanceMonitor.recordCustomTiming('navigationTime', navigationTime);
  ...
  test.describe('Compliance and Security - Regulatory Requirements', () => {
    test('Authentication performance - Security compliance', async ({ page }) => {
      ...
      const startTime = Date.now();
      await page.fill('[data-testid="username"]', 'healthcare.provider@hospital.com');
      ...
      await performanceMonitor.recordCustomTiming('authenticationTime', authenticationTime);
      ...
      await performanceMonitor.recordCustomTiming('accessControlCheckTime', accessControlCheckTime);
    ...
    test('Audit trail performance - HIPAA compliance', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="audit-confirmed"]', { timeout: 1000 });
      const auditLogTime = Date.now() - auditStartTime;
      ...
      const metrics = await performanceMonitor.getMetrics();
      expect(HEALTHCARE_PERFORMANCE_ASSERTIONS.compliance.auditTrail.assertion(metrics)).toBe(true);
    });
    ...
    test('Data encryption performance - Security compliance', async ({ page }) => {
      ...
      await performanceMonitor.recordCustomTiming('encryptionTime', encryptionTime);
      ...
      // Validate compliance assertion
      const metrics = await performanceMonitor.getMetrics();
      expect(HEALTHCARE_PERFORMANCE_ASSERTIONS.compliance.dataEncryption.assertion(metrics)).toBe(true);
    });
  });
  ...
  test.describe('Performance Regression Detection', () => {
    test('Core Web Vitals regression - Healthcare standards', async ({ page }) => {
      ...
      expect(HEALTHCARE_PERFORMANCE_ASSERTIONS.userExperience.coreWebVitals.assertion(metrics, 'clinical')).toBe(true);
