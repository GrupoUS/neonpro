...
test.describe('Patient Dashboard Performance', () => {
  test.beforeAll(async () => {
    performanceReporter = new PerformanceReporter('./test-results/performance/patient-dashboard')
  })
  ...
  test('should load patient dashboard within performance budget', async ({ page, context }) => {
    ...
    console.log(`\n🏥 Testing: ${testName}`)
    console.log(`📋 Budget: LCP=${budget.lcp}ms, FID=${budget.fid}ms, CLS=${budget.cls}`)
    ...
    const patientDataLoadTime = await monitor.measurePatientDataLoad(PERFORMANCE_SELECTORS.patientList)
    ...
    if (!validation.passed) {
      ...
      validation.violations.forEach(violation => {
        console.log(`  ${violation.metric}: ${violation.actual} > ${violation.budget}`)
      })
    } else {
    ...
    if (config.thresholds.failOnBudgetViolation && !validation.passed) {
      ...
      if (criticalViolations.length > 0) {
        throw new Error(`Critical performance budget violations: ${criticalViolations.map(v => v.metric).join(', ')}`)
      }
    }
  })
  ...
  test('should handle patient form submission performance', async ({ page, context }) => {
    ...
    await page.waitForSelector(PERFORMANCE_SELECTORS.medicalForm, { state: 'visible' })
  ...
  test('should maintain performance during patient search', async ({ page, context }) => {
    ...
    await page.waitForSelector(PERFORMANCE_SELECTORS.patientList, { state: 'visible' })
    ...
    for (const query of searchQueries) {
      ...
      await expect(page.locator(PERFORMANCE_SELECTORS.searchResults)).toBeVisible()
    }
  ...
  test('should monitor memory usage during extended session', async ({ page, context }) => {
    ...
    for (const pagePath of pages) {
      ...
      console.log(`📄 ${pagePath}: ${(jsHeapSize / 1024 / 1024).toFixed(2)} MB`)
