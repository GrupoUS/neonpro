...
function generateRecommendations(
  ...
  if (scan.warnings.length > 0) {
    recommendations.push('Review scan warnings and address inaccessible directories or files.');
  }

  if (dependency.unusedFiles.length > 0) {
    recommendations.push('Consider removing or refactoring files that appear unused.');
  }

  if (dependency.totalImports === 0) {
    recommendations.push('No imports detected. Confirm that the project root is correct.');
  }
