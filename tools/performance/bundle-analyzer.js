...
function getChunkStatus(size) {
  if (size > exports.BUNDLE_THRESHOLDS.ERROR) return "error";
  if (size > exports.BUNDLE_THRESHOLDS.WARNING) return "warning";
  return "good";
}
...
function generateRecommendations(analysis) {
  ...
  if (largeChunks.length > 0) {
    recommendations.push(
      "\uD83D\uDD04 ".concat(
        largeChunks.length,
        " chunks exceed 250KB - split into smaller chunks",
      ),
    );
  }
  ...
  if (analysis.duplicates.length > 0) {
    ...
    recommendations.push(
      "\uD83D\uDCE6 "
        ...
        .concat(formatBytes(totalWasted), " - optimize imports"),
    );
  }
  ...
  if (analysis.largeModules.length > 0) {
    recommendations.push(
      "\uD83C\uDFAF ".concat(
        analysis.largeModules.length,
        " large modules found - consider lazy loading",
      ),
    );
  }
  ...
  if (analysis.duplicates.length > 0) {
    ...
    analysis.duplicates.slice(0, 10).forEach(function (dup) {
      report += "- **"
        ...
        .concat(dup.chunks.length, " chunks\n");
    });
  }
  return report;
}
