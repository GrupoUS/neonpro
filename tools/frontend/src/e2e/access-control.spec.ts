...
test.describe("Role-Based Access Control", () => {
  ...
  test.describe("Healthcare Provider Role Permissions", () => {
    ...
    test("should access medical records with restrictions", async ({
      request,
    }) => {
      const patientId = "test-patient-id";
    ...
    test("should use AI assistant for healthcare queries", async ({
      request,
    }) => {
      const aiRequest = {
  ...
  test.describe("Patient Role Permissions", () => {
    ...
    test("should use AI assistant for personal queries", async ({
      request,
    }) => {
      const aiRequest = {
    ...
    test("should be denied access to other patients data", async ({
      request,
    }) => {
    ...
    test("should be denied access to system administration", async ({
      request,
    }) => {
      const adminEndpoints = [
        "/v1/security/status",
  ...
  test.describe("Role-Specific Data Masking", () => {
    test("should apply different data masking based on role", async ({
      request,
    }) => {
      // Test as patient
      await request.post(`${baseUrl}/api/v1/auth/login`, {
