...
test.describe("Performance Requirements Validation", () => {
  ...
  test.describe("API Response Time Requirements", () => {
    test("should meet <2s response time for all critical endpoints", async ({
      request,
    }) => {
  ...
  test.describe("AI Agent Performance", () => {
    test("should meet AI query performance requirements", async ({
      request,
    }) => {
      const aiQueries = [
        {
        ...
        },
        {
          ...
          query:
            "Analyze appointment patterns for the last month and provide insights",
    ...
    test("should handle streaming AI responses efficiently", async ({
      request,
    }) => {
    ...
    test("should maintain AI performance with concurrent requests", async ({
      request,
    }) => {
      const concurrentQueries = 5;

      const queryPromises = Array(concurrentQueries)
        .fill(null)
        .map((_, index) =>
  ...
  test.describe("Performance Monitoring", () => {
    ...
    test("should track performance degradation", async () => {
      ...
      test.info().annotations.push({
        type: "note",
        description:
          "Performance degradation testing requires extended monitoring",
      });
    });
  });
