...
test.describe('AI Chat E2E - NeonPro Aesthetic Clinic', () => {
  ...
  test('complete AI chat workflow for aesthetic treatments', async ({ page }) => {
    ...
    // Test search input
    const searchInput = page.getByPlaceholder(/digite sua pergunta sobre tratamentos/i);
  ...
  test('LGPD compliance and privacy notices', async ({ page }) => {
    ...
    await expect(page.getByText(/respeitamos sua privacidade \(lgpd\)/i)).toBeVisible();
