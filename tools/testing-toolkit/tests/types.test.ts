...
describe('Types', () => {
  ...
  describe('TestEnvironment', () => {
    it('should accept valid test environments', () => {
      const environments: TestEnvironment[] = ['node', 'browser', 'both'];
    ...
    it('should work with type guards', () => {
      function isValidTestEnvironment(value: string): value is TestEnvironment {
        return ['node', 'browser', 'both'].includes(value);
      }

      expect(isValidTestEnvironment('node')).toBe(true);
      expect(isValidTestEnvironment('browser')).toBe(true);
      expect(isValidTestEnvironment('invalid')).toBe(false);
    });
  });

  describe('CoverageThreshold', () => {
    it('should accept valid coverage threshold objects', () => {
      ...
      expect(threshold.functions).toBe(85);
      expect(threshold.lines).toBe(90);
      expect(threshold.statements).toBe(88);
    });

    it('should accept partial coverage threshold objects', () => {
      const partialThreshold: CoverageThreshold = {
        lines: 75,
        functions: 80,
      };

      expect(partialThreshold.lines).toBe(75);
      expect(partialThreshold.functions).toBe(80);
      expect(partialThreshold.branches).toBeUndefined();
      expect(partialThreshold.statements).toBeUndefined();
    });

    it('should accept empty coverage threshold object', () => {
      const emptyThreshold: CoverageThreshold = {};
      expect(Object.keys(emptyThreshold).length).toBe(0);
    });
  });
