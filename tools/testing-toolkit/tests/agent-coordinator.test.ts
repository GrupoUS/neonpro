...
describe('AgentCoordinator', () => {
  ...
  it('stops sequential coordination when fail-fast gate triggers', async () => {
    ...
    expect(results[0]?.issues).toContain(
      'Quality gates not met for architect-review'
    );
  ...
  it('returns deterministic metrics for quality gates based on agent type', async () => {
    ...
    const codeMetrics = await (coordinator as any).validateQualityGates(
      'code-reviewer'
    );
  ...
  it('produces recommendations when specialties are below threshold', () => {
    ...
    const generateRecommendations = (
      coordinator as any
    ).generateRecommendations.bind(coordinator);
    ...
    expect(recommendations).toContain(
      'Improve quality metrics for code-reviewer'
    );
