...
export class AuditTrailValidator {
  ...
  static validateTrail(entries: AuditTrailEntry[]): {
    ...
    // Validate each entry
    const invalidEntries = entries.filter(entry => !this.validateEntry(entry));
    ...
    const presentActions = new Set(entries.map(entry => entry.action.toLowerCase()));
    ...
    const isChronological = entries.every((entry, index) =>
    ...
    const coverage = ((requiredActions.length - missingActions.length) / requiredActions.length)
      * 100;
  ...
  describe(`Audit Trail: ${testName}`, () => {
    ...
    it('should have valid entry structure', () => {
      auditTrail.forEach((entry, index) => {
        expect(AuditTrailValidator.validateEntry(entry), `Entry ${index} should be valid`).toBe(
          true,
        );
      });
    });
