...
export class LGPDValidator {
   ...
   */
  static validateDataMinimization(data: LGPDTestData, requiredFields: string[]): boolean {
    const dataKeys = Object.keys(data.personalData);
    return requiredFields.every(field => dataKeys.includes(field))
   ...
   */
  static validateCompliance(data: LGPDTestData, requiredFields: string[]): {
    isCompliant: boolean;
    violations: string[];
    ...
    if (!this.validateConsent(data)) {
      ...
      recommendations.push('Implement proper consent management with clear purpose');
    }

    if (!this.validateDataMinimization(data, requiredFields)) {
      violations.push('Data minimization principle violated');
      recommendations.push('Collect only necessary data for the specified purpose');
    }
...
export function createLGPDTestSuite(
  ...
  describe(`LGPD Compliance: ${testName}`, () => {
    ...
    it('should follow data minimization principle', () => {
      expect(LGPDValidator.validateDataMinimization(testData, requiredFields)).toBe(true);
    });
 ...
 */
export function createMockLGPDData(overrides: Partial<LGPDTestData> = {}): LGPDTestData {
  return {
    personalData: {
      name: 'João Silva',
