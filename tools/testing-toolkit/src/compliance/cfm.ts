...
export class CFMValidator {
  ...
  static validateCompliance(data: CFMTestData): {
    ...
    if (!this.validateProfessionalLicensing(data)) {
      ...
      recommendations.push('Ensure all medical professionals have valid CRM registration');
    }

    if (data.telemedicineCompliance.enabled && !this.validateTelemedicineCompliance(data)) {
    ...
    if (data.digitalPrescription.enabled && !this.validateDigitalPrescription(data)) {
      violations.push('Digital prescription requirements not met');
      recommendations.push('Implement digital signature and prescription validation systems');
    }

    if (!this.validatePatientConfidentiality(data)) {
      violations.push('Patient confidentiality requirements not met');
      recommendations.push('Implement comprehensive access controls and audit logging');
    }
  ...
  describe(`CFM Compliance: ${testName}`, () => {
    ...
    it('should comply with telemedicine requirements (if enabled)', () => {
      if (testData.telemedicineCompliance.enabled) {
        expect(CFMValidator.validateTelemedicineCompliance(testData)).toBe(true);
      }
    });
 ...
 */
export function createMockCFMData(overrides: Partial<CFMTestData> = {}): CFMTestData {
  return {
    professionalLicensing: {
