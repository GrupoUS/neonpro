...
export const alertsEndpoint: EndpointConfig = {
  ...
  handler: async (req, res) => {
    ...
    const alerts = Array.from({ length: Math.min(parseInt(limit as string), 50) }, (_, i) => generateAlert(i));
...
export const logsEndpoint: EndpointConfig = {
  ...
  handler: async (req, res) => {
    ...
    const logs = Array.from({ length: Math.min(parseInt(limit as string), 100) }, (_, i) => generateLog(i));
