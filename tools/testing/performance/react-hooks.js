  ...
  return (0, react_1.useCallback)(() => {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "\uD83D\uDC0C Slow callback "
            .concat(debugName || "unknown", ": ")
            .concat(duration.toFixed(2), "ms"),
        );
      }
      return result;
    } catch (error) {
      ...
      if (renderTime > PERFORMANCE_THRESHOLDS.RENDER_TIME_ERROR) {
        console.error(
          "\uD83D\uDEA8 Slow render "
            ...
            .concat(renderTime.toFixed(2), "ms (avg: ")
            .concat(avgRenderTime.toFixed(2), "ms)"),
        );
...
function useDebouncedState(initialValue, delay) {
  ...
  var _a = (0, react_1.useState)(initialValue),
    immediateValue = _a[0],
    setImmediateValue = _a[1];
  var _b = (0, react_1.useState)(initialValue),
    debouncedValue = _b[0],
...
function useOptimizedChartData(rawData, processor, deps) {
  return useOptimizedMemo(
    ...
    __spreadArray([rawData], deps, true),
    "chartDataProcessor",
  );
}
...
function usePerformanceProfiler(name, enabled) {
  ...
  var measure = (0, react_1.useCallback)(
    (startMark, endMark) => {
      ...
      try {
        performance.measure(
          "".concat(name, "-").concat(startMark, "-to-").concat(endMark),
          startName,
          endName,
        );
