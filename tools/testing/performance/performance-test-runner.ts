...
export class PerformanceTestRunner {
  ...
  private async runDatabaseTests(): Promise<any> {
    ...
    if (healthcareMetrics.emergencyDataAccessTime < 10000) {
  ...
  private async runBundleAnalysis(): Promise<any> {
    ...
    if (analysis.gzippedSize < 500 * 1024) { // 500KB
  ...
  private async runHealthcareTests(): Promise<any> {
    ...
    try {
      ...
      if (healthcareMetrics.emergencyAccessTime < 10000) {
  ...
  private generateSummary(report: ComprehensivePerformanceReport): PerformanceSummary {
    ...
    if (report.healthcare?.emergencyAccessTime > 10000) criticalIssues++;
  ...
  private generateCsvReport(report: ComprehensivePerformanceReport): string {
    const csv = [
      ...
      `Emergency Access (ms),${report.healthcare?.emergencyAccessTime || 0},10000,${(report.healthcare?.emergencyAccessTime || 0) < 10000 ? 'PASS' : 'FAIL'}`
    ];
