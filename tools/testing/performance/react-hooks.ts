  ...
  return useCallback((...args: Parameters<T>) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' && duration > PERFORMANCE_THRESHOLDS.INTERACTION_TIME_WARNING) {
        console.warn(`🐌 Slow callback ${debugName || 'unknown'}: ${duration.toFixed(2)}ms`)
      }
  ...
  return useMemo(() => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' && duration > PERFORMANCE_THRESHOLDS.RENDER_TIME_WARNING) {
        console.warn(`🧮 Expensive computation ${debugName || 'unknown'}: ${duration.toFixed(2)}ms`)
      }
      
      return result
    } catch (error) {
      console.error(`❌ Memo computation error in ${debugName || 'unknown'}:`, error)
      throw error
    }
  }, deps)
}
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (renderTime > PERFORMANCE_THRESHOLDS.RENDER_TIME_ERROR) {
        console.error(`🚨 Slow render ${componentName}: ${renderTime.toFixed(2)}ms (avg: ${avgRenderTime.toFixed(2)}ms)`)
      } else if (renderTime > PERFORMANCE_THRESHOLDS.RENDER_TIME_WARNING) {
        console.warn(`⚠️ Render warning ${componentName}: ${renderTime.toFixed(2)}ms (avg: ${avgRenderTime.toFixed(2)}ms)`)
      }
    }
  })
    ...
    if (process.env.NODE_ENV === 'development' && 'memory' in performance) {
      const checkMemory = () => {
        ...
        if (used > PERFORMANCE_THRESHOLDS.MEMORY_USAGE_WARNING) {
          console.warn(`🧠 High memory usage in ${componentName}: ${(used / 1024 / 1024).toFixed(2)}MB`)
        }
...
export function useOptimizedChartData<T>(
  ...
  }, [rawData, ...deps], 'chartDataProcessor')
}
...
export function usePerformanceProfiler(name: string, enabled: boolean = process.env.NODE_ENV === 'development') {
  ...
  const measure = useCallback((startMark: string, endMark: string) => {
    ...
    try {
      ...
      console.log(`⏱️ ${name}: ${startMark} → ${endMark} took ${duration.toFixed(2)}ms`)
