...
const mockQueryChain = {
  ...
  _updateData: undefined, // Store update data
  _insertData: undefined, // Store insert data
...
mockQueryChain.single.mockImplementation(async () => {
  ...
  if (insertData) {
    mockQueryChain._insertData = undefined; // Reset insert data
...
beforeEach(() => {
  ...
  mockQueryChain._updateData = undefined;
  mockQueryChain._insertData = undefined;
...
describe('gET /api/stock/alerts', () => {
  ...
  it('should return alerts with proper pagination', async () => {
    ...
    if (response?.status !== HTTP_STATUS.OK) {
      ...
      throw new Error(
        `Status ${response?.status}: ${JSON.stringify(responseData, undefined, 2)}`,
      );
    }
