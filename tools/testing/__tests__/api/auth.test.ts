...
const mockAuthService = {
  validateToken: (token: string) => {
    ...
    if (token.startsWith('valid-')) return { 
      valid: true, 
      user: { 
        id: '123', 
  ...
  validateHealthcareProfessional: (crm: string, state: string) => {
    if (!crm || !state) return { valid: false, error: 'CRM and state required' };
    if (!/^\d{4,6}$/.test(crm)) return { valid: false, error: 'Invalid CRM format' };
    if (!/^[A-Z]{2}$/.test(state)) return { valid: false, error: 'Invalid state format' };
