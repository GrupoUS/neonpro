...
describe('ProfessionalForm', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(createProfessional as jest.Mock).mockResolvedValue({ id: 'new-id' })
    ;(updateProfessional as jest.Mock).mockResolvedValue(undefined)
  })
  ...
  describe('Form Pre-population', () => {
    ...
    it('should change submit button text when editing', () => {
      ...
      expect(screen.queryByRole('button', { name: 'Cadastrar Profissional' })).not.toBeInTheDocument()
    })
  })
  ...
  describe('Form Validation', () => {
    it('should show validation errors for required fields', async () => {
      ...
      const submitButton = screen.getByRole('button', { name: 'Cadastrar Profissional' })
  ...
  describe('Credentials Management', () => {
    it('should allow adding new credential', async () => {
      ...
      const addCredentialButton = screen.getByRole('button', { name: 'Adicionar Credencial' })
    ...
    it('should validate credential expiry date is after issue date', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Data de expiração deve ser posterior à data de emissão')).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Services Management', () => {
    it('should allow adding new service', async () => {
      ...
      const addServiceButton = screen.getByRole('button', { name: 'Adicionar Serviço' })
  ...
  describe('Form Submission', () => {
    it('should create new professional successfully', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Sobrenome'), { target: { value: 'Santos' } })
      fireEvent.change(screen.getByLabelText('Email'), { target: { value: 'joao@email.com' } })
      fireEvent.change(screen.getByLabelText('Número da Licença'), { target: { value: 'CRM 789012' } })
      fireEvent.change(screen.getByLabelText('Qualificação'), { target: { value: 'Cardiologista' } })
    ...
    it('should update existing professional successfully', async () => {
      ...
      const submitButton = screen.getByRole('button', { name: 'Atualizar Profissional' })
      fireEvent.click(submitButton)
    ...
    it('should handle creation errors', async () => {
      ;(createProfessional as jest.Mock).mockRejectedValue(new Error('Database error'))
    ...
    it('should handle update errors', async () => {
      ;(updateProfessional as jest.Mock).mockRejectedValue(new Error('Update error'))
    ...
    it('should disable submit button during submission', async () => {
      ;(createProfessional as jest.Mock).mockImplementation(() => 
