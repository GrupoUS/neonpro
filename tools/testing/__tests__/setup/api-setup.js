...
global.Date = jest.fn(function () { return new originalDate('2025-01-24T10:00:00.000Z'); });
global.Date.now = jest.fn(function () { return new originalDate('2025-01-24T10:00:00.000Z').getTime(); });
global.Date.UTC = originalDate.UTC;
global.Date.parse = originalDate.parse;
// Mock console to suppress logs during tests
global.console = __assign(__assign({}, console), { log: jest.fn(), warn: jest.fn(), error: jest.fn(), debug: jest.fn(), info: jest.fn() });
// Mock crypto for security-related operations
Object.defineProperty(global, 'crypto', {
    value: {
        getRandomValues: jest.fn(function () { return new Uint8Array(32); }),
        randomUUID: jest.fn(function () { return '12345678-1234-5678-9012-123456789012'; }),
        subtle: {
            encrypt: jest.fn(),
            decrypt: jest.fn(),
            sign: jest.fn(),
...
global.URL = jest.fn().mockImplementation(function (url) { return ({
    href: url,
...
global.URLSearchParams = jest.fn().mockImplementation(function () { return ({
    get: jest.fn(),
    set: jest.fn(),
    has: jest.fn(),
    delete: jest.fn(),
...
process.env = __assign(__assign({}, process.env), { NODE_ENV: 'test', NEXT_PUBLIC_SUPABASE_URL: 'https://test.supabase.co', NEXT_PUBLIC_SUPABASE_ANON_KEY: 'test-anon-key', SUPABASE_SERVICE_ROLE_KEY: 'test-service-role-key', NEXTAUTH_SECRET: 'test-secret', NEXTAUTH_URL: 'http://localhost:3000', DATABASE_URL: 'postgresql://test:test@localhost:5432/test', STRIPE_SECRET_KEY: 'sk_test_123', STRIPE_PUBLISHABLE_KEY: 'pk_test_123' });
...
var MockNextResponse = /** @class */ (function () {
    function MockNextResponse(body, options) {
        ...
        this.text = jest.fn().mockResolvedValue(typeof this.body === 'string' ? this.body : JSON.stringify(this.body));
        this.clone = jest.fn();
...
// Mock Supabase SSR
jest.mock('@supabase/ssr', function () { return ({
    createServerClient: jest.fn().mockImplementation(function () { return ({
        auth: {
            getSession: jest.fn().mockResolvedValue({
                data: {
                    session: {
    ...
    createBrowserClient: jest.fn().mockImplementation(function () { return ({
        auth: {
            getSession: jest.fn().mockResolvedValue({
                data: {
                    session: {
...
jest.mock('date-fns', function () { return ({
    ...
    parseISO: jest.fn().mockImplementation(function (dateString) { return new Date(dateString); }),
...
// Mock React hooks
jest.mock('react', function () { return (__assign(__assign({}, jest.requireActual('react')), { useState: jest.fn(), useEffect: jest.fn(), useContext: jest.fn(), useReducer: jest.fn(), useCallback: jest.fn(), useMemo: jest.fn(), useRef: jest.fn(), useImperativeHandle: jest.fn(), useLayoutEffect: jest.fn(), useDebugValue: jest.fn() })); });
