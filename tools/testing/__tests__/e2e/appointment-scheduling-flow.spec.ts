...
test.describe('Complete Appointment Scheduling Flow', () => {
  ...
  test('should cancel appointment with proper workflow', async ({ page, },) => {
    ...
    await expect(page.locator(`[data-testid="appointment-${tomorrowStr}"]`,),).toHaveClass(
      /cancelled/,
    )
  })
  ...
  test('should manage appointment reminders', async ({ page, },) => {
    ...
    await expect(page.locator('[data-testid="active-reminders-list"]',),).toContainText(
      'SMS - 24h antes',
    )
    await expect(page.locator('[data-testid="active-reminders-list"]',),).toContainText(
      'Email - 2h antes',
  ...
  test('should handle appointment check-in process', async ({ page, },) => {
    ...
    await expect(page.locator('[data-testid="appointment-status"]',),).toContainText(
      'Paciente Presente',
    )
