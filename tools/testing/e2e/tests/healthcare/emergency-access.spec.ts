...
test.describe('🚨 Emergency Access - Critical E2E', () => {
  ...
  test.describe('⚡ Emergency Login (<10s)', () => {
    test('should complete emergency login within 10 seconds', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="emergency-mode"], .emergency-portal'),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("ACESSO DE EMERGÊNCIA")'),
      ).toBeVisible();

      // Emergency login with special credentials
      await page.fill(
        '[data-testid="emergency-code"], input[name="emergency_code"]',
        'EMRG2024',
      );
      await page.fill(
        '[data-testid="professional-id"], input[name="professional_id"]',
        'CRM12345',
      );
      await page.fill(
        '[data-testid="emergency-password"], input[type="password"]',
        'EmergencySecure123!',
      );

      // Submit emergency login
      await page.click(
        '[data-testid="emergency-login"], button:has-text("ACESSO EMERGENCIAL")',
      );
      ...
      await expect(
        page.locator('[data-testid="emergency-dashboard"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="patient-search-emergency"]'),
      ).toBeVisible();
    });
    ...
    test('should handle life-threatening emergency access bypass', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="emergency-timer"]'),
      ).toBeVisible();
    });
    ...
    test('should bypass normal authentication restrictions in emergency mode', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="emergency-password"]',
        'EmergencySecure123!',
      );
      ...
      await expect(
        page.locator('[data-testid="emergency-privileges"]'),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("MODO EMERGENCIAL ATIVO")'),
      ).toBeVisible();

      // Should have access to all patient data without normal role restrictions
      await expect(
        page.locator('[data-testid="unrestricted-access"]'),
      ).toBeVisible();

      // Emergency session indicator
      await expect(
        page.locator('[data-testid="emergency-session"]'),
    ...
    test('should validate emergency credentials and prevent abuse', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="incident-description"]',
        'Teste de validação com descrição adequada',
      );
      await page.click('[data-testid="emergency-access-confirm"]');
      ...
      await expect(
        page.locator('text=ID de emergência inválido'),
      ).toBeVisible();
  ...
  test.describe('🔍 Critical Data Access', () => {
    test('should provide immediate access to critical patient data', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="patient-search-emergency"]',
        'Maria Silva Santos',
      );
      ...
      const medicalAlerts = page.locator(
        '[data-testid="medical-alerts"], .medical-alert',
      );
    ...
    test('should display patient allergies and contraindications prominently', async ({ page }) => {
      ...
      const contraindications = page.locator(
        '[data-testid="contraindications"]',
      );
      ...
      await expect(
        page.locator('[data-testid="medication-warnings"]'),
      ).toBeVisible();
    });
    ...
    test('should show current medications and dosages', async ({ page }) => {
      ...
      await expect(medicationsSection).toContainText(
        /mg|ml|comprimidos|cápsulas/,
      );
      await expect(medicationsSection).toContainText(
        /manhã|tarde|noite|vezes ao dia/,
      );
      ...
      const interactionWarnings = page.locator(
        '[data-testid="drug-interactions"]',
      );
  ...
  test.describe('📋 Audit Trail Logging', () => {
    test('should log all emergency access events', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="audit-trail-active"]'),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("ACESSO REGISTRADO EM AUDITORIA")'),
      ).toBeVisible();
      ...
      if ((await auditEntries.count()) > 0) {
        ...
        await expect(auditEntries.first()).toContainText(
    ...
    test('should require justification for emergency access', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="emergency-justification-modal"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="justification-required"]'),
      ).toContainText('obrigatória');
      ...
      await expect(
        page.locator('[data-testid="justification-logged"]'),
    ...
    test('should maintain audit integrity and prevent tampering', async ({ page }) => {
      ...
      const editButton = page.locator(
        '[data-testid="edit-audit"], button:has-text("Editar")',
      );
      const deleteButton = page.locator(
        '[data-testid="delete-audit"], button:has-text("Excluir")',
      );
      ...
      const signatureField = page.locator(
        '[data-testid="audit-signature"], .audit-hash',
      );
  ...
  test.describe('⏱️ Performance & Reliability', () => {
    test('should handle concurrent emergency access requests', async ({ page, context }) => {
      ...
      await expect(
        page1.locator('[data-testid="emergency-dashboard"]'),
      ).toBeVisible();
      await expect(
        page2.locator('[data-testid="emergency-dashboard"]'),
      ).toBeVisible();
      await expect(
        page3.locator('[data-testid="emergency-dashboard"]'),
      ).toBeVisible();
    ...
    test('should maintain performance under emergency conditions', async ({ page }) => {
      ...
      for (const patient of patients) {
        ...
        if (
          (await page
            .locator('[data-testid="emergency-justification"]')
            .count()) > 0
        ) {
          await page.fill(
            '[data-testid="emergency-justification"]',
            `Emergência médica - ${patient}`,
          );
          await page.selectOption('[data-testid="emergency-type"]', 'other');
          await page.click('[data-testid="confirm-emergency-access"]');
        }
    ...
    test('should handle system failures gracefully in emergency mode', async ({ page }) => {
      ...
      await expect(
        page.locator(
          '[data-testid="retry-button"], button:has-text("Tentar Novamente")',
        ),
      ).toBeVisible();
  ...
  test.describe('🔒 Security & Compliance', () => {
    test('should enforce emergency access time limits', async ({ page }) => {
      ...
      const timerText = await page
        .locator('[data-testid="emergency-timer"]')
        .textContent();
    ...
    test('should automatically log out after emergency session expires', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="emergency-session-control"]'),
      ).toBeVisible();

      // Verify manual emergency logout option
      const emergencyLogout = page.locator(
        '[data-testid="emergency-logout"], button:has-text("ENCERRAR EMERGÊNCIA")',
      );
      ...
      await expect(
        page.locator('[data-testid="emergency-mode"]'),
      ).toBeVisible();
