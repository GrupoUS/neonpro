...
test.describe('🏥 Complete Healthcare Workflow E2E', () => {
  ...
  test('📅 Complete Appointment Booking Flow', async ({ page }) => {
    ...
    await test.step('Navigate to appointment booking', async () => {
      ...
      await expect(page.locator('h1')).toContainText(
        /(?:Novo Agendamento|Agendar Consulta)/
      );
    });
    ...
    await test.step('Select patient and service', async () => {
      ...
      await expect(page.locator('[name="serviceType"]')).toHaveValue(
        appointmentData.serviceType
      );
    });
    ...
    await test.step('Select date and time', async () => {
      ...
      await expect(
        page.locator(`[data-testid="time-slot-${appointmentData.time}"]`)
    ...
    await test.step('Add notes and confirm', async () => {
      ...
      await expect(
        page.locator('[data-testid="appointment-success"]')
      ).toContainText('Agendamento realizado');
    });
    ...
    await test.step('Verify appointment in calendar', async () => {
      ...
      await expect(
        page.locator(`text=${appointmentData.serviceType}`)
  ...
  test('⚕️ Professional Workflow - Medical Consultation', async ({ page }) => {
    ...
    await test.step('Access patient consultation', async () => {
      ...
      await expect(
        page.locator('[data-testid="consultation-interface"]')
      ).toBeVisible();
    });
    ...
    await test.step('Complete medical consultation', async () => {
      ...
      await page.fill(
        '[name="treatmentNotes"]',
        'Aplicação de 20U de botox na região frontal'
      );
    ...
    await test.step('Generate consultation report', async () => {
      ...
      await expect(
        page.locator('[data-testid="report-generated"]')
  ...
  test('🏢 Clinic Administration Workflow', async ({ page }) => {
    ...
    await test.step('Access analytics dashboard', async () => {
      ...
      await expect(
        page.locator('[data-testid="appointments-widget"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="patients-widget"]')
      ).toBeVisible();
    });
    ...
    await test.step('Review compliance dashboard', async () => {
      ...
      const complianceIndicators = page.locator(
  ...
  test('🔒 LGPD Compliance User Journey', async ({ page }) => {
    ...
    await test.step('Patient data subject rights access', async () => {
      ...
      await expect(
        page.locator('[data-testid="patient-data-view"]')
      ).toBeVisible();
      await expect(
    ...
    await test.step('Update consent preferences', async () => {
      ...
      await expect(
        page.locator('[data-testid="consent-updated"]')
      ).toBeVisible();
    });
    ...
    await test.step('Request data export', async () => {
      ...
      await expect(
        page.locator('text="Dados serão enviados por email"')
      ).toBeVisible();
    });
  });
