...
test.describe("👥 Patient Registration - Critical E2E", () => {
	...
	test.describe("📝 Complete Registration with CPF Validation", () => {
		...
		test("should validate required fields before proceeding", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="phone-required"], .field-error')).toContainText(
				"Telefone é obrigatório"
			);
			await expect(page.locator('[data-testid="birthdate-required"], .field-error')).toContainText(
	...
	test.describe("🔒 LGPD Consent Capture", () => {
		test("should capture comprehensive LGPD consent with detailed explanations", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="processing-explanation"]')).toContainText(
				"processamento dos dados pessoais"
			);
			await expect(page.locator('[data-testid="sharing-explanation"]')).toContainText(
		...
		test("should require mandatory LGPD consents for registration", async ({ page }) => {
			...
			await expect(page.locator('[data-testid="mandatory-consent-error"]')).toContainText(
	...
	test.describe("🏥 Medical History and Allergies", () => {
		...
		test("should handle patients with no known allergies properly", async ({ page }) => {
			...
			// Verify confirmation
			await expect(page.locator('[data-testid="no-allergies-confirmation"]')).toContainText(
				"Nenhuma alergia conhecida"
			);

			await page.click('[data-testid="save-allergies"]');
			await expect(page.locator('[data-testid="no-allergies-saved"]')).toContainText(
				"Informação de alergias registrada"
			);
		});
