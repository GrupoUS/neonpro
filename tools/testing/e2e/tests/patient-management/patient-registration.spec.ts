...
test.describe('👥 Patient Registration - Critical E2E', () => {
  ...
  test.describe('📝 Complete Registration with CPF Validation', () => {
    test('should complete full patient registration with valid data', async ({ page }) => {
      ...
      await expect(page.locator(':has-text("Cadastro de Paciente")')).toBeVisible();

      // Fill personal information
      await page.fill('[data-testid="patient-name"], input[name="name"]', 'Maria Silva Santos');
      await page.fill('[data-testid="patient-cpf"], input[name="cpf"]', '123.456.789-09');
      await page.fill('[data-testid="patient-rg"], input[name="rg"]', '12.345.678-9');
      await page.fill('[data-testid="patient-birthdate"], input[type="date"]', '1985-03-15');
      await page.selectOption('[data-testid="patient-gender"], select[name="gender"]', 'feminino');
      await page.selectOption('[data-testid="patient-marital-status"], select[name="marital_status"]', 'solteira');

      // Fill contact information
      await page.fill('[data-testid="patient-phone"], input[name="phone"]', '(11) 99999-8888');
      await page.fill('[data-testid="patient-email"], input[name="email"]', 'maria.santos@email.com');
      await page.fill('[data-testid="patient-emergency-contact"], input[name="emergency_contact"]', 'Ana Santos - (11) 98888-7777');

      // Fill address information
      await page.fill('[data-testid="patient-cep"], input[name="cep"]', '01234-567');
      // Wait for CEP lookup to populate address fields
      await page.waitForTimeout(1000);
      
      await page.fill('[data-testid="patient-street"], input[name="street"]', 'Rua das Flores, 123');
      await page.fill('[data-testid="patient-neighborhood"], input[name="neighborhood"]', 'Centro');
      await page.fill('[data-testid="patient-city"], input[name="city"]', 'São Paulo');
      await page.selectOption('[data-testid="patient-state"], select[name="state"]', 'SP');

      // Verify CPF validation
      const cpfValidation = page.locator('[data-testid="cpf-validation-status"]');
      await expect(cpfValidation).toContainText('CPF válido');
      await expect(cpfValidation).toHaveClass(/success|valid/);

      // Continue to next step
      await page.click('[data-testid="continue-to-medical"], button:has-text("Continuar")');
      
      // Verify navigation to medical information step
      await expect(page.locator('[data-testid="medical-information-step"]')).toBeVisible();
      await expect(page.locator(':has-text("Informações Médicas")')).toBeVisible();
    });
    ...
    test('should validate CPF format and prevent invalid CPF registration', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="cpf-error"], .cpf-error')).toBeVisible();
      await expect(page.locator('[data-testid="cpf-error"]')).toContainText('CPF inválido');
      
      // Submit button should be disabled
      const submitButton = page.locator('[data-testid="continue-to-medical"], button[type="submit"]');
      ...
      await expect(page.locator('[data-testid="cpf-validation-status"]')).toContainText('CPF válido');
    ...
    test('should prevent duplicate CPF registration', async ({ page }) => {
      ...
      const duplicateWarning = page.locator('[data-testid="cpf-duplicate"], .cpf-duplicate');
      if (await duplicateWarning.count() > 0) {
        ...
        await expect(page.locator('[data-testid="view-existing-patient"]')).toBeVisible();
      }
    });
    ...
    test('should validate required fields before proceeding', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="name-required"], .field-error')).toContainText('Nome é obrigatório');
      await expect(page.locator('[data-testid="cpf-required"], .field-error')).toContainText('CPF é obrigatório');
      await expect(page.locator('[data-testid="phone-required"], .field-error')).toContainText('Telefone é obrigatório');
      await expect(page.locator('[data-testid="birthdate-required"], .field-error')).toContainText('Data de nascimento é obrigatória');

      // Should remain on the same step
      expect(page.url()).toMatch(/\/patients\/register/);
      await expect(page.locator('[data-testid="medical-information-step"]')).not.toBeVisible();
    });
    ...
    test('should capture comprehensive LGPD consent with detailed explanations', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="lgpd-explanation"]')).toBeVisible();
      await expect(page.locator(':has-text("Lei Geral de Proteção de Dados")')).toBeVisible();

      // Check individual consent options
      await expect(page.locator('[data-testid="consent-data-processing"]')).toBeVisible();
      await expect(page.locator('[data-testid="consent-medical-treatment"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="consent-research"]')).toBeVisible();

      // Verify detailed explanations for each consent type
      await expect(page.locator('[data-testid="processing-explanation"]')).toContainText('processamento dos dados pessoais');
      await expect(page.locator('[data-testid="sharing-explanation"]')).toContainText('compartilhamento com profissionais');
      await expect(page.locator('[data-testid="marketing-explanation"]')).toContainText('comunicações promocionais');
      ...
      await expect(page.locator('[data-testid="consent-summary"]')).toContainText('3 de 5 consentimentos');

      // Continue registration
      await page.click('[data-testid="confirm-lgpd-consent"]');

      // Should show consent confirmation
      await expect(page.locator('[data-testid="consent-recorded"]')).toContainText('Consentimentos registrados');
    });
    ...
    test('should require mandatory LGPD consents for registration', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'carlos.mandatory@email.com');
      ...
      await expect(page.locator('[data-testid="mandatory-consent-error"]')).toBeVisible();
      await expect(page.locator('[data-testid="mandatory-consent-error"]')).toContainText('Consentimentos obrigatórios');

      // Highlight mandatory fields
      await expect(page.locator('[data-testid="consent-data-processing"]')).toHaveClass(/required|mandatory/);
      await expect(page.locator('[data-testid="consent-medical-treatment"]')).toHaveClass(/required|mandatory/);
      ...
      await page.click('[data-testid="confirm-lgpd-consent"]');
      await expect(page.locator('[data-testid="consent-recorded"]')).toBeVisible();
    });
    ...
    test('should provide consent withdrawal options', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'paula.withdrawal@email.com');
      ...
      await expect(page.locator('[data-testid="withdrawal-rights"]')).toBeVisible();
      await expect(page.locator('[data-testid="withdrawal-rights"]')).toContainText('revogar seu consentimento');
      await expect(page.locator('[data-testid="withdrawal-contact"]')).toContainText('dpo@neonpro.com.br');
      ...
      await expect(page.locator('[data-testid="consent-details"]')).toContainText('Direitos do titular');
    });
    ...
    test('should create audit trail for LGPD consent events', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'roberto.audit@email.com');
      ...
      await expect(page.locator('[data-testid="audit-created"]')).toContainText('Registro de auditoria criado');
      
      // Verify consent timestamp
      const consentTimestamp = page.locator('[data-testid="consent-timestamp"]');
      if (await consentTimestamp.count() > 0) {
        await expect(consentTimestamp).toContainText(new Date().toISOString().split('T')[0]);
      }
  ...
  test.describe('🏥 Medical History and Allergies', () => {
    test('should capture comprehensive medical history', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'laura.historia@email.com');
      ...
      await page.selectOption('[data-testid="blood-type"], select[name="blood_type"]', 'O+');
      ...
      await page.fill('[data-testid="surgery-description-0"]', 'Apendicectomia');
      await page.fill('[data-testid="surgery-date-0"]', '2010-08-15');
      await page.fill('[data-testid="surgery-hospital-0"]', 'Hospital São Paulo');

      // Chronic conditions
      await page.click('[data-testid="add-condition"]');
      await page.selectOption('[data-testid="condition-type-0"]', 'diabetes');
      await page.fill('[data-testid="condition-description-0"]', 'Diabetes tipo 2 controlada com medicação');
      ...
      await page.selectOption('[data-testid="family-condition-0"]', 'hipertensao');
      await page.fill('[data-testid="family-notes-0"]', 'Hipertensão arterial diagnosticada aos 50 anos');
      ...
      await expect(page.locator('[data-testid="medical-summary"]')).toContainText('Diabetes tipo 2');
      await expect(page.locator('[data-testid="medical-summary"]')).toContainText('Metformina');
      await expect(page.locator('[data-testid="medical-summary"]')).toContainText('Apendicectomia');

      await page.click('[data-testid="continue-to-allergies"]');
      await expect(page.locator('[data-testid="allergies-step"]')).toBeVisible();
    });
    ...
    test('should capture and validate allergy information with severity levels', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'marcos.alergia@email.com');
      ...
      await page.fill('[data-testid="allergy-reaction-0"]', 'Anafilaxia, dificuldade respiratória, urticária');
      ...
      await page.fill('[data-testid="allergy-reaction-1"]', 'Inchaço labial, coceira');
      ...
      await expect(page.locator('[data-testid="allergy-summary"]')).toContainText('3 alergias registradas');
      
      // Critical allergy warning should be highlighted
      const criticalAllergy = page.locator('[data-testid="critical-allergy-warning"]');
      await expect(criticalAllergy).toBeVisible();
      await expect(criticalAllergy).toContainText('ATENÇÃO: Alergia GRAVE a Penicilina');
      await expect(criticalAllergy).toHaveClass(/critical|danger|alert/);

      // Emergency allergy card option
      await expect(page.locator('[data-testid="emergency-card-option"]')).toBeVisible();
      await page.check('[data-testid="generate-emergency-card"]');

      await page.click('[data-testid="save-allergies"]');
      await expect(page.locator('[data-testid="allergies-saved"]')).toContainText('Alergias registradas com sucesso');
    });
    ...
    test('should handle patients with no known allergies properly', async ({ page }) => {
      ...
      // Verify confirmation
      await expect(page.locator('[data-testid="no-allergies-confirmation"]')).toContainText('Nenhuma alergia conhecida');

      await page.click('[data-testid="save-allergies"]');
      await expect(page.locator('[data-testid="no-allergies-saved"]')).toContainText('Informação de alergias registrada');
    });
    ...
    test('should validate critical allergy information completeness', async ({ page }) => {
      ...
      // Navigate to allergies step
      await page.fill('[data-testid="patient-name"]', 'Teste Validação Alergia');
      await page.fill('[data-testid="patient-cpf"]', '147.852.963-14');
      await page.fill('[data-testid="patient-phone"]', '(11) 90000-9999');
      await page.fill('[data-testid="patient-email"]', 'teste.validacao@email.com');
      ...
      await expect(page.locator('[data-testid="allergy-substance-error"]')).toContainText('Substância é obrigatória');
      await expect(page.locator('[data-testid="allergy-reaction-error"]')).toContainText('Reação é obrigatória');

      // Critical allergies should require all fields
      const criticalAllergyFields = page.locator('[data-testid="critical-allergy-fields"]');
      ...
      await page.fill('[data-testid="allergy-reaction-0"]', 'Choque anafilático');

      await page.click('[data-testid="save-allergies"]');
      await expect(page.locator('[data-testid="allergies-saved"]')).toBeVisible();
    });
    ...
    test('should complete end-to-end patient registration with all components', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-name"]', 'Complete Registration Test');
      ...
      await page.selectOption('[data-testid="patient-gender"]', 'feminino');
      await page.selectOption('[data-testid="patient-marital-status"]', 'casada');
      await page.fill('[data-testid="patient-phone"]', '(11) 99999-0001');
      await page.fill('[data-testid="patient-email"]', 'complete.test@email.com');
      await page.fill('[data-testid="patient-emergency-contact"]', 'João Test - (11) 99999-0002');

      // Address information
      await page.fill('[data-testid="patient-cep"]', '01234-567');
      await page.waitForTimeout(500); // CEP lookup
      await page.fill('[data-testid="patient-street"]', 'Rua Complete Test, 456');
      ...
      // Add medical history
      await page.click('[data-testid="add-condition"]');
      await page.selectOption('[data-testid="condition-type-0"]', 'hipertensao');
      await page.fill('[data-testid="condition-description-0"]', 'Hipertensão arterial leve');
      ...
      await page.fill('[data-testid="allergy-reaction-0"]', 'Gastrite, dor abdominal');
      ...
      await expect(page.locator('[data-testid="review-name"]')).toContainText('Complete Registration Test');
      await expect(page.locator('[data-testid="review-cpf"]')).toContainText('789.456.123-78');
      await expect(page.locator('[data-testid="review-medical"]')).toContainText('Hipertensão');
      await expect(page.locator('[data-testid="review-allergies"]')).toContainText('AAS');
      await expect(page.locator('[data-testid="review-consent"]')).toContainText('3 consentimentos');
      ...
      await expect(page.locator('[data-testid="registration-success"]')).toContainText('Paciente cadastrado com sucesso');
      ...
      await expect(page.locator('[data-testid="patient-detail-name"]')).toContainText('Complete Registration Test');
    ...
    test('should generate patient ID and medical record number', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'id.generation@email.com');
      ...
      const medicalRecordNumber = page.locator('[data-testid="medical-record-number"]');
    ...
    test('should integrate with appointment scheduling system', async ({ page }) => {
      ...
      await page.fill('[data-testid="patient-email"]', 'integration.test@email.com');
      ...
      const scheduleAppointmentButton = page.locator('[data-testid="schedule-first-appointment"]');
      if (await scheduleAppointmentButton.count() > 0) {
        ...
        const selectedPatient = page.locator('[data-testid="selected-patient-name"]');
    ...
    test('should handle registration errors gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="registration-error"]')).toContainText('Erro ao cadastrar paciente');

      // Should offer retry option
      await expect(page.locator('[data-testid="retry-registration"]')).toBeVisible();

      // Should preserve form data
      await page.click('[data-testid="back-to-review"]');
      await expect(page.locator('[data-testid="review-name"]')).toContainText('Error Test Patient');
    });
  });
  ...
  test.describe('🚀 Performance & Validation', () => {
    test('should complete registration within performance targets', async ({ page }) => {
      ...
      console.log(`Performance metrics - Page load: ${pageLoadTime}ms, CPF validation: ${cpfValidationTime}ms, Navigation: ${navigationTime}ms`);
    });
    ...
    test('should provide accessibility compliance in registration form', async ({ page }) => {
      ...
      const nameLabel = page.locator('label[for="patient-name"], label:has([data-testid="patient-name"])');
      await expect(nameLabel).toBeVisible();

      const cpfLabel = page.locator('label[for="patient-cpf"], label:has([data-testid="patient-cpf"])');
      await expect(cpfLabel).toBeVisible();

      // Check for ARIA attributes
      const form = page.locator('[data-testid="patient-registration-form"], form');
    ...
    test('should handle concurrent registration attempts', async ({ page, context }) => {
      ...
      const registerPatient = async (testPage: any, patientName: string, cpf: string) => {
        ...
        await testPage.fill('[data-testid="patient-email"]', `${patientName.toLowerCase()}@email.com`);
        await testPage.fill('[data-testid="patient-birthdate"]', '1990-01-01');
        await testPage.selectOption('[data-testid="patient-gender"]', 'masculino');
      ...
      await expect(page1.locator('[data-testid="registration-success"]')).toBeVisible();
      await expect(page2.locator('[data-testid="registration-success"]')).toBeVisible();
