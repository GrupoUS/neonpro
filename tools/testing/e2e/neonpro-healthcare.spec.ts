...
test.describe('NeonPro Healthcare System', () => {
  ...
  test('should navigate to login page', async ({ page }) => {
    ...
    try {
      ...
      const loginElements = await page.locator('input[type="email"], input[type="password"], [data-testid*="login"]').count();
      ...
      const hasLoginText = await page.locator('text=/login|entrar|acesso/i').isVisible().catch(() => false);
      ...
      const isErrorPage = await page.locator('text=/error|erro/i').isVisible().catch(() => false);
  ...
  test('should handle patient routes appropriately', async ({ page }) => {
    ...
    for (const route of patientRoutes) {
      try {
        ...
        const hasPatientContent = await page.locator('text=/patient|paciente/i').isVisible().catch(() => false);
        const hasTableOrList = await page.locator('table, [data-testid*="list"], .table').isVisible().catch(() => false);
        ...
        // Route might not exist, continue to next
        continue;
      }
    }
    ...
    if (!foundPatientRoute) {
      ...
      const isRedirectToAuth = currentUrl.includes('login') || currentUrl.includes('auth') || currentUrl.includes('dashboard');
