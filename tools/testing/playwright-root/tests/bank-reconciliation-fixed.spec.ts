...
test.describe("🏥 Healthcare Bank Reconciliation System E2E", () => {
  ...
  test("should import bank statement with healthcare data anonymization", async ({ page }) => {
    ...
    const totalTransactions = await page
      ...
      .textContent();
    expect(
      Number(totalTransactions?.replaceAll(/\D/g, "") || 0),
    ).toBeGreaterThan(0);
  });
  ...
  test("should perform intelligent transaction matching with healthcare validation", async ({ page }) => {
    ...
    const matchedCount = await page
      ...
      .textContent();
    expect(Number(matchedCount?.replaceAll(/\D/g, "") || 0)).toBeGreaterThan(0);
...
test("should handle large dataset with healthcare performance standards", async ({ page }) => {
  ...
  const totalCount = await page
    .getByTestId("total-transactions-count")
    .textContent();
  expect(Number(totalCount?.replaceAll(/\D/g, "") || 0)).toBeGreaterThan(1000);
