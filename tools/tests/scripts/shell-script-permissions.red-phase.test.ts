...
describe("Shell Script Permissions Validation (RED PHASE)", () => {
  ...
  describe("Execute Permissions Validation", () => {
    it("should ensure all shell scripts have execute permissions", () => {
      ...
      expect(missingPermissions.length).toBe(0)
      if (missingPermissions.length > 0) {
        console.warn(`The following scripts are missing execute permissions: ${missingPermissions.join(", ")}`)
      }
    })
    ...
    it("should validate secure permission settings (no world-writable)", () => {
      ...
      if (insecureScripts.length > 0) {
        console.warn(`The following scripts have insecure permissions: ${insecureScripts.join(", ")}`)
      }
    })
    ...
    it("should ensure scripts are not owned by root (security risk)", () => {
      ...
      expect(rootOwnedScripts.length).toBe(0)
      if (rootOwnedScripts.length > 0) {
        console.warn(`The following scripts are owned by root: ${rootOwnedScripts.join(", ")}`)
      }
    })
  })
  ...
  describe("Script Execution Safety", () => {
    it("should prevent execution of scripts without proper shebang", () => {
      ...
      expect(invalidShebangScripts.length).toBe(0)
      if (invalidShebangScripts.length > 0) {
        console.warn(`The following scripts lack proper shebang: ${invalidShebangScripts.join(", ")}`)
      }
    })
    ...
    it("should validate script syntax before execution", () => {
      ...
      expect(syntaxErrorScripts.length).toBe(0)
      if (syntaxErrorScripts.length > 0) {
        console.warn(`The following scripts have syntax errors: ${syntaxErrorScripts.join(", ")}`)
      }
    })
    ...
    it("should check for dangerous commands in scripts", () => {
      ...
      for (const script of TEST_SCRIPTS) {
        ...
        if (fs.existsSync(scriptPath)) {
          try {
            const content = fs.readFileSync(scriptPath, "utf8")
            const foundDangerousCommands = dangerousCommands.filter(cmd => content.includes(cmd))
      ...
      expect(scriptsWithDangerousCommands.length).toBe(0)
      if (scriptsWithDangerousCommands.length > 0) {
        console.warn(`The following scripts contain dangerous commands: ${scriptsWithDangerousCommands.join(", ")}`)
      }
    })
  })
  ...
  describe("Permission Consistency", () => {
    it("should ensure consistent permissions across deployment scripts", () => {
      ...
      expect(hasConsistentPermissions).toBe(
        true,
    ...
    it("should validate script group ownership", () => {
      ...
      expect(invalidGroupScripts.length).toBe(0)
      if (invalidGroupScripts.length > 0) {
        console.warn(`The following scripts have invalid group ownership: ${invalidGroupScripts.join(", ")}`)
      }
    })
  })
  ...
  describe("Permission Repair Functionality", () => {
    it("should provide functionality to fix missing execute permissions", () => {
      ...
      expect(repairFunctionExists).toBe(
        true,
    ...
    it("should provide functionality to secure script permissions", () => {
      ...
      expect(secureFunctionExists).toBe(true, "Should have a function to secure script permissions")
    })
    ...
    it("should validate permissions after repair", () => {
      ...
      expect(validationFunctionExists).toBe(
        true,
  ...
  describe("Integration with Deployment Pipeline", () => {
    it("should check permissions before deployment", () => {
      ...
      expect(preDeployCheckExists).toBe(
        true,
