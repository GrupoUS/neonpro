...
const secureLoggerModule = await import('/home/vibecode/neonpro/apps/api/src/utils/secure-logger.ts')
const { createLogger, SecureLogger } = secureLoggerModule
...
describe('Enhanced SecureLogger', () => {
  ...
  describe('Database Operation Logging', () => {
    it('should log database operations with performance tracking', () => {
      ...
      expect(logEntry.level).toBe('warn') // Duration > 1000ms would be warn, but 250ms is info
  ...
  describe('Metrics Collection', () => {
    it('should track basic metrics', () => {
      ...
      expect(metrics.logsCount).toBe(3)
    ...
    it('should reset metrics correctly', () => {
      ...
      expect(logger.getMetrics().logsCount).toBe(2)
      
      logger.resetMetrics()
      
      const resetMetrics = logger.getMetrics()
  ...
  describe('Sensitive Data Masking', () => {
    it('should mask sensitive data in messages', () => {
      ...
      expect(logEntry.message).toContain('sec***') // Password masked
      expect(logEntry.message).not.toContain('secret123')
      expect(logEntry.message).toContain('t***@***.com') // Email masked
    })
