...
export class ResultAggregator {
  async aggregateAgentResults(results: AgentResult[]): Promise<AggregatedResult> {
    ...
    const warningCount = resultArray.reduce((sum, r) => sum + ((r.warnings as any[])?.length || 0), 0);
    ...
    const totalDuration = resultArray.reduce((sum, r) => sum + (r.duration || 0), 0);
  ...
  async analyzeTrend(historicalResults: AggregatedResult[], _metric?: string): Promise<TrendAnalysis> {
    ...
    if (slope > 0.01) direction = "improving";  // More sensitive threshold
    else if (slope < -0.01) direction = "declining";  // More sensitive threshold
  ...
  async detectAnomaly(result: any, baseline: any[]): Promise<boolean> {
    ...
    const avgQuality = baseline.reduce((sum, r) => sum + (r.qualityScore || 0), 0) / baseline.length;
  ...
  async categorizeResults(aggregatedOrResults: AggregatedResult | AgentResult[]): Promise<any> {
    ...
    if ('results' in aggregatedOrResults) {
      ...
      return { success: [], failed: [], warnings: [], byType: {}, byQuality: {} };
    }
    
    const success = results.filter(r => r.success);
    const failed = results.filter(r => !r.success);
    const warnings = results.filter(r => r.warnings && (r.warnings as any[]).length > 0);
    ...
    const byQuality = {
      ...
      good: results.filter(r => (r.metrics as any)?.quality > 0.7 && (r.metrics as any)?.quality <= 0.9),
      fair: results.filter(r => (r.metrics as any)?.quality > 0.5 && (r.metrics as any)?.quality <= 0.7),
  ...
  async analyzeCompliance(aggregated: AggregatedResult): Promise<any> {
    const violations = aggregated.results.filter(r => 
      (r.metrics as any)?.compliance === false || aggregated.complianceScore < 0.8
    );
  ...
  async calculateResourceUtilization(aggregated: AggregatedResult): Promise<any> {
    const avgMemory = aggregated.results.reduce((sum, r) => sum + ((r.metrics as any)?.memoryUsage || 256), 0) / aggregated.results.length;
    const avgCpu = aggregated.results.reduce((sum, r) => sum + ((r.metrics as any)?.cpuUsage || 50), 0) / aggregated.results.length;
    
    return {
      averageMemoryUsage: avgMemory,
      averageCpuUsage: avgCpu,
      peakMemoryUsage: Math.max(...aggregated.results.map(r => (r.metrics as any)?.memoryUsage || 256)),
      peakCpuUsage: Math.max(...aggregated.results.map(r => (r.metrics as any)?.cpuUsage || 50))
    };
  }
  ...
  calculatePerformanceMetrics(aggregatedOrResults: AggregatedResult | AgentResult[]): any {
    let results: AgentResult[];
  ...
  private calculateCoverageScore(aggregated: AggregatedResult): number {
    ...
    return aggregated.results.length > 0 ? coverageSum / aggregated.results.length : 0;
  }
  ...
  private generateRecommendations(aggregated: AggregatedResult): string[] {
    ...
    if (aggregated.complianceScore < 0.8) {
      recommendations.push("Address compliance violations and regulatory requirements");
    }
