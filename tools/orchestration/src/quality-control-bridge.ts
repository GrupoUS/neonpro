...
export class QualityControlBridge {
  async executeQualityControl(
  ...
  ): Promise<QualityControlResult & { qualityScore?: number; complianceStatus?: any }> {
    ...
    return {
      ...
      complianceStatus: healthcareCompliance ? {
        required: true,
        lgpd: typeof healthcareCompliance.lgpd === 'boolean' ? healthcareCompliance.lgpd : true,
        anvisa: typeof healthcareCompliance.anvisa === 'boolean' ? healthcareCompliance.anvisa : true,
        cfm: typeof healthcareCompliance.cfm === 'boolean' ? healthcareCompliance.cfm : true,
      } : undefined
    };
  }
  ...
  private parseCommand(command: string): QualityControlContext {
    ...
    const context: QualityControlContext = {
      ...
      depth: command.includes('--depth=L5') ? 'L5' : command.includes('--depth=L4') ? 'L4' : 'L3'
    };
