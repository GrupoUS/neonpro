...
import { TOOL_WORKFLOWS, ToolWorkflow, OrchestrationStep } from './config';

const logContext = (operation: string, metadata: Record<string, unknown> = {}) => ({
...
const runCommand = (command: string[], cwd: string, dryRun: boolean): Promise<void> => {
  if (dryRun) {
    logger.debug(
      'Dry run command',
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on('close', code => {
      if (code === 0) {
        ...
        rejectPromise(new Error(`${command.join(' ')} exited with code ${code}`));
      }
    });
...
const runWithConcurrency = async (
  ...
  let index = 0;
  const workerCount = Math.min(Math.max(1, options.concurrency), workflows.length);
...
const summarize = (outcomes: WorkflowOutcome[]): void => {
  const totalDuration = outcomes.reduce((acc, outcome) => acc + outcome.duration, 0);
...
export const orchestrateTools = async (inputOptions: Partial<OrchestratorOptions> = {}): Promise<void> => {
  ...
  if (workflows.length === 0) {
    logger.warn('No workflows match the provided filters', logContext('no-workflows'));
    return;
  }
