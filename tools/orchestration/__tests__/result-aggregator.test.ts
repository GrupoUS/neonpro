...
describe('ResultAggregator', () => {
  ...
  describe('Result Analysis', () => {
    ...
    it('should provide improvement suggestions', async () => {
      ...
      expect(analysis.recommendations.length).toBeGreaterThan(0);
      expect(analysis.recommendations.every(r => typeof r === 'string')).toBe(true);
    });
  });
  ...
  describe('Anomaly Detection', () => {
    it('should detect performance anomalies', async () => {
      ...
      const isAnomaly = await aggregator.detectAnomaly(anomalousResult, normalResults);
    ...
    it('should not flag normal results as anomalies', async () => {
      ...
      const isAnomaly = await aggregator.detectAnomaly(normalResult, normalResults);
      ...
      const results: AgentResult[] = [
        {
          ...
          success: false,
          duration: 1500,
          result: { lgpd: 'non-compliant', anvisa: 'compliant', cfm: 'compliant' },
  ...
  describe('Performance Metrics', () => {
    ...
    it('should track resource utilization', async () => {
      ...
      const aggregated = await aggregator.aggregateAgentResults(results);
      const resourceMetrics = await aggregator.calculateResourceUtilization(aggregated);
