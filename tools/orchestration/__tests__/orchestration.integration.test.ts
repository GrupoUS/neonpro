  ...
  try {
    ...
    for (const command of commands) {
      try {
        ...
        console.log(`✅ Command "${command.split(' ')[0]}" executed: ${result.success}`);
        console.log(`   - Duration: ${result.duration}ms`);
        console.log(`   - Agents: ${result.orchestrationResult?.agentResults?.length || 0}`);
      } catch (error) {
    ...
    console.log(`   - Optimized agents: ${optimized.map(a => a.name).join(', ')}`);

    const groups = system.agentRegistry.getAgentCoordinationGroups(agents, 'parallel');
    console.log(`   - Coordination groups: ${groups.length}`);

    const plan = system.agentRegistry.createParallelExecutionPlan(agents, 'parallel');
    ...
    if ('snapshot' in metrics) {
      ...
      console.log(`   - Total executions: ${metrics.snapshot.orchestration.totalExecutions}`);
      console.log(`   - Quality score: ${metrics.snapshot.quality.overallQualityScore.toFixed(1)}`);
      console.log(`   - Average execution time: ${metrics.snapshot.performance.averageExecutionTime}ms`);
      console.log(`   - LGPD compliance: ${(metrics.snapshot.healthcare.lgpdCompliance * 100).toFixed(1)}%`);
    } else {
    ...
    if (system.complianceValidator) {
      ...
      try {
        const compliance = await system.validateCompliance(context, agentResults);
        console.log('✅ Healthcare compliance validation completed');
        console.log(`   - LGPD compliance: ${compliance.lgpd.compliant ? '✅' : '❌'} (${compliance.lgpd.score}/100)`);
        console.log(`   - ANVISA compliance: ${compliance.anvisa.compliant ? '✅' : '❌'} (${compliance.anvisa.score}/100)`);
        console.log(`   - CFM compliance: ${compliance.cfm.compliant ? '✅' : '❌'} (${compliance.cfm.score}/100)`);
      } catch (error) {
        console.log(`❌ Healthcare compliance validation failed: ${error.message}`);
      }
    } else {
    ...
    console.log(`   - Example commands: ${examples.examples.slice(0, 3).join(', ')}`);
