...
export class UnifiedLogger extends EventEmitter {
   ...
   */
  static getInstance(name: string, config?: Partial<LoggerConfig>): UnifiedLogger {
  ...
  createChild(context: LogContext): UnifiedLogger {
    const childConfig = { ...this.config, name: `${this.config.name}:${context.component || 'Child'}` };
  ...
  private formatError(error: Error): ErrorInfo {
    return {
      ...
      cause: (error as any).cause ? this.formatError((error as any).cause) : undefined,
    };
  }
  ...
  private writeToSpecializedFiles(entry: LogEntry): void {
    ...
    // Error log
    if (error && (entry.level === LogLevel.ERROR || entry.level === LogLevel.CRITICAL)) {
  ...
  private cleanupOldLogs(): void {
    try {
      const files = readdirSync(this.config.logDirectory)
        .filter(file => file.endsWith('.log') && file.startsWith(this.config.name))
        .map(file => ({
          name: file,
  ...
  private log(
    ...
    const entry = this.createLogEntry(level, message, context, error, constitutional);
...
export const createLogger = (name: string, config?: Partial<LoggerConfig>): UnifiedLogger => {
