...
export const TEST_CATEGORIES: Record<TestCategory, TestCategoryConfig> = {
  frontend: {
    ...
    testTypes: ['components', 'routes', 'e2e', 'accessibility', 'healthcare-ui'],
...
export class TestCategoryManager {
  ...
  static getHealthcareComplianceCategories(): TestCategoryConfig[] {
    return Object.values(TEST_CATEGORIES).filter(config => config.healthcareCompliance);
  }

  static generateTestCommand(category: TestCategory, testType?: string): string {
