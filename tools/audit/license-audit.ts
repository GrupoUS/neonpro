...
export async function performLicenseAudit(): Promise<LicenseAuditReport> {
  ...
  const depNames = Object.keys({ ...(rootPkg.dependencies || {}), ...(rootPkg.devDependencies || {}) })
  const packages: LicensePackageInfo[] = []
  for (const name of depNames) {
    try {
      const pkgPath = require.resolve(path.join(name, 'package.json'))
      const pkgJson = JSON.parse(fs.readFileSync(pkgPath, 'utf8'))
      const license = pkgJson.license || (Array.isArray(pkgJson.licenses) ? pkgJson.licenses.map((l: any) => l.type).join(',') : 'UNKNOWN')
