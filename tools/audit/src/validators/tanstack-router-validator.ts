...
export class TanStackRouterValidator extends EventEmitter {
  ...
  constructor(projectRoot: string, routesDirectory?: string) {
    super();
  ...
  private parseRouterConfig(viteConfigContent: string): TanStackRouterConfig | null {
    try {
      ...
      const routesDirectory = this.extractConfigValue(configText, 'routesDirectory') || './src/routes';
      const generatedRouteTree = this.extractConfigValue(configText, 'generatedRouteTree') || './src/routeTree.gen.ts';
  ...
  private isRouteFile(filename: string): boolean {
    // Skip ignored files
    if (this.config?.routeFileIgnorePrefix && filename.startsWith(this.config.routeFileIgnorePrefix)) {
      return false;
    }
   ...
   */
  private async analyzeRouteFile(filePath: string, routesBasePath: string): Promise<RouteDefinition | null> {
    try {
      ...
      const routeDefinition: RouteDefinition = {
        ...
        hasComponent: /export\s+(?:default|const|function)\s+(?:component|Component)/mi.test(content),
