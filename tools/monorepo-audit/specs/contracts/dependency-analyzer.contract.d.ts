...
export type NodeType = 'entry_point' | 'component' | 'service' | 'utility' | 'config' | 'test' | 'route' | 'type_definition' | 'external';
export type DependencyType = 'es6_import' | 'commonjs_require' | 'dynamic_import' | 'route_reference' | 'type_reference' | 'asset_reference' | 'config_reference';
export type CircularSeverity = 'low' | 'medium' | 'high' | 'critical';
export type ResolutionStrategy = 'extract_interface' | 'dependency_injection' | 'event_driven' | 'move_shared_code' | 'break_into_layers' | 'lazy_loading';
...
export interface IDependencyAnalyzer {
     ...
     */
    getTransitiveDependencies(assetPath: string, graph: DependencyGraph, maxDepth: number): string[];
    /**
...
export type ValidationIssueType = 'missing_dependency' | 'orphaned_asset' | 'circular_dependency' | 'invalid_import_path' | 'unused_import' | 'duplicate_dependency';
...
export declare const DEFAULT_ANALYZER_OPTIONS: AnalyzerOptions;
export declare const ANALYZER_PERFORMANCE_REQUIREMENTS: {
    /** Maximum analysis time for 10k files (ms) */
    readonly MAX_ANALYSIS_TIME_10K_FILES: 45000;
    /** Maximum memory usage during analysis (bytes) */
    readonly MAX_MEMORY_USAGE: 750000000;
    /** Maximum circular dependency detection time (ms) */
    readonly MAX_CIRCULAR_DETECTION_TIME: 15000;
};
//# sourceMappingURL=dependency-analyzer.contract.d.ts.map