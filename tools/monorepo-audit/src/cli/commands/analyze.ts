...
export const analyzeCommand = new Command('analyze')
  ...
  .action(async (path: string, options: any) => {
    ...
    try {
      ...
      const dependencyGraph = await analyzer.buildGraph(scanResult.assets, analyzeOptions);
      ...
      if (results.circularDependencies) {
        console.log(`  Circular dependencies: ${chalk.red(results.circularDependencies.length)}`);
        if (results.circularDependencies.length > 0) {
          console.log(chalk.red('\n  Circular Dependencies:'));
          results.circularDependencies.slice(0, 5).forEach((cycle: any, i: number) => {
      ...
      if (results.unusedAssets) {
        console.log(`  Unused assets: ${chalk.yellow(results.unusedAssets.length)}`);
      ...
      if (results.importanceScores) {
        ...
        results.importanceScores.slice(0, 10).forEach((item: any, i: number) => {
          console.log(`    ${i + 1}. ${item.assetPath} (score: ${item.score.toFixed(4)})`);
        });
      }
      ...
      if (options.graphViz) {
        ...
        console.log(chalk.gray('Generate SVG with: dot -Tsvg ' + dotPath + ' -o graph.svg'));
      }
    } catch (error) {
