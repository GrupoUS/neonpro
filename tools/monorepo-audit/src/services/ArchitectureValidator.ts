...
import { CodeAsset, ArchitectureDocument } from '../models/types.js'
import {
...
export class ArchitectureValidator implements IArchitectureValidator {
  ...
  public async generateComplianceReport(
    ...
    for (const violation of violations) {
      ...
      report += `**Location:** Line ${violation.location.line}, Column ${violation.location.column}\n`
  ...
  private validateHonoRouteFile(sourceFile: SourceFile, filePath: string): ArchitectureViolation[] {
    const violations: ArchitectureViolation[] = []

    // Check for Hono import
    const imports = sourceFile.getImportDeclarations()
    const hasHonoImport = imports.some(imp => 
      imp.getModuleSpecifierValue().includes('hono')
    )
  ...
  private validateTanStackRouteFile(sourceFile: SourceFile, filePath: string): ArchitectureViolation[] {
    const violations: ArchitectureViolation[] = []
  ...
  private checkImportOrder(sourceFile: SourceFile, filePath: string): ArchitectureViolation[] {
    ...
    return violations
  }  private checkNamingConventions(sourceFile: SourceFile, filePath: string): ArchitectureViolation[] {
    const violations: ArchitectureViolation[] = []
