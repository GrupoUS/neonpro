...
export class SecurityAuditorAgent extends BaseAgent {
    ...
    try {
      ...
      return {
        ...
        metrics: {
          ...
          securityIssues: findings.filter((f) => f.type.startsWith("security"))
            .length,
          vulnerabilities: findings.filter(
            (f) => f.severity === "critical" || f.severity === "high",
          ).length,
          complianceIssues: findings.filter((f) => f.type === "compliance")
            .length,
          securityRecommendations: recommendations.filter(
            (r) => r.type === "security-testing",
          ).length,
  ...
  private async executeRedPhase(
    ...
    if (
      context.domain.includes("payment") ||
      context.domain.includes("financial")
    ) {
      findings.push(
        this.createFinding(
          "security-implementation",
  ...
  private async executeRefactorPhase(
    ...
    if (
      context.domain.includes("crypto") ||
      context.domain.includes("encryption")
    ) {
      findings.push(
        this.createFinding(
          "security-implementation",
