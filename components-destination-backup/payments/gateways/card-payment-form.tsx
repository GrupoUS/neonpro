...
const PaymentForm: React.FC<CardPaymentFormProps> = ({
  ...
  const installmentAmount = formData.installments ? Math.ceil(amount / formData.installments!) : amount;
  const totalWithInterest = formData.installments && formData.installments > 1 
              ...
              >
                ...
                <SelectContent>
                  {Array.from({ length: 12 }, (_, i) => i + 1).map((installment) => {
                    const installmentAmountCalc = installment === 1 
                      ? amount 
