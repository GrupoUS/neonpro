...
test.describe('Bank Reconciliation System E2E', () => {
  ...
  test('should perform intelligent transaction matching', async ({ page }) => {
    ...
    await page.getByTestId('import-statement-button').click()
    await page.getByTestId('file-upload-input').setInputFiles('cypress/fixtures/bank-statement-sample.csv')
    await page.getByTestId('start-import-button').click()
    await expect(page.getByText('Import completed successfully')).toBeVisible({ timeout: 30000 })
    ...
    await expect(page.getByText('Matching completed')).toBeVisible({ timeout: 60000 })
  ...
  test('should handle large dataset performance', async ({ page }) => {
    ...
    await page.getByTestId('import-statement-button').click()
    await page.getByTestId('file-upload-input').setInputFiles('cypress/fixtures/large-bank-statement.csv')
    ...
    // Execute import
    await page.getByTestId('start-import-button').click()
    await expect(page.getByText('Import completed successfully')).toBeVisible({ timeout: 120000 })
    
    // Check performance metrics
    const importTime = Date.now() - startTime
    expect(importTime).toBeLessThan(60000) // Should complete within 60 seconds
    ...
    await page.getByTestId('start-matching-button').click()
    await page.getByTestId('execute-matching-button').click()
    await expect(page.getByText('Matching completed')).toBeVisible({ timeout: 180000 })
    
    const matchingTime = Date.now() - matchingStartTime
    expect(matchingTime).toBeLessThan(120000) // Should complete within 2 minutes
  })
  ...
  test('should handle errors gracefully', async ({ page }) => {
    ...
    await page.getByTestId('import-statement-button').click()
    await page.getByTestId('file-upload-input').setInputFiles('cypress/fixtures/invalid-format.txt')
    await page.getByTestId('start-import-button').click()
