...
test.describe('🌐 Healthcare Accessibility Testing - WCAG 2.1 AA+ & NBR 17225', () => {
  ...
  test('should validate NBR 17225 Brazilian accessibility standards', async ({ page }) => {
    ...
    const portugueseWords = [
      'Paciente', 'Agendamento', 'Tratamento', 'Privacidade', 
      'Dados', 'Consulta', 'Histórico', 'Contato'
    ];
    ...
    if (currencyElements > 0) {
      const currencyText = await page.locator('[data-testid*="currency"], [data-testid*="price"]').first().textContent();
