...
async function sessionIntelligence() {
	try {
		...
		utils.log(
			"INFO",
			"SESSION_HOOK",
			`Session intelligence hook executing for session: ${env.sessionId}`,
		);
		...
		utils.log(
			"SUCCESS",
			"SESSION_HOOK",
			"Session intelligence hook completed successfully",
		);
		...
		utils.log(
			"ERROR",
			"SESSION_HOOK",
			`Session intelligence hook error: ${error.message}`,
		);
...
async function analyzeSession(env) {
	try {
		...
		utils.log(
			"WARN",
			"SESSION_HOOK",
			`Session analysis error: ${error.message}`,
		);
	}
}
...
async function getSessionStatistics(env) {
	try {
		...
		utils.log(
			"WARN",
			"SESSION_HOOK",
			`Session statistics error: ${error.message}`,
		);
		return {
			toolCount: 0,
			duration: 0,
	...
	try {
		...
		if (sessionStats.errors > 0) {
			utils.log(
				"WARN",
				"SESSION_HOOK",
				`Session had ${sessionStats.errors} errors`,
			);
		}
		...
		if (sessionStats.toolCount > 50) {
			utils.log(
				"WARN",
				"SESSION_HOOK",
				`High tool usage detected: ${sessionStats.toolCount} tools used`,
			);
		}
		...
		utils.log(
			"WARN",
			"SESSION_HOOK",
			`Tool usage analysis error: ${error.message}`,
		);
	}
}
...
async function checkRepetitivePatterns(sessionStats) {
	try {
		...
		for (let i = 0; i < toolSequence.length - 5; i++) {
			...
			for (
				let j = i + 1;
				j < toolSequence.length && toolSequence[j] === tool;
				j++
			) {
				consecutive++;
			}
			...
			if (consecutive >= 5) {
				utils.log(
					"WARN",
					"SESSION_HOOK",
					`Potential loop detected: ${tool} used ${consecutive} times consecutively`,
				);
				break;
			}
		}
		...
		if (maxPairFreq > 5) {
			const mostCommonPair = Object.entries(pairFrequency).find(
				([, freq]) => freq === maxPairFreq,
			);

			utils.log(
				"INFO",
				"SESSION_HOOK",
				`Common tool pattern: ${mostCommonPair[0]} (${mostCommonPair[1]} times)`,
			);
		}
	} catch (error) {
		utils.log(
	...
	try {
		...
		if (durationMinutes > 30) {
			utils.log(
				"INFO",
				"SESSION_HOOK",
				`Long session detected: ${Math.round(durationMinutes)} minutes`,
			);
		}
		...
		if (tools.includes("bash") && tools.includes("todowrite")) {
			utils.log(
				"INFO",
				"SESSION_HOOK",
				"Development workflow pattern detected",
			);
		}
		...
		if (tools.some((tool) => tool.includes("archon"))) {
			utils.log(
				"INFO",
				"SESSION_HOOK",
				"Archon task management workflow detected",
			);
		}
	} catch (error) {
		utils.log(
			"WARN",
			"SESSION_HOOK",
			`Pattern detection error: ${error.message}`,
		);
	}
}
...
async function generateSessionSummary(env) {
	try {
		...
		utils.log(
			"INFO",
			"SESSION_HOOK",
			`Session summary saved: ${sessionStats.toolCount} tools, ${Math.round(
	...
	try {
		...
		if (cleanedFiles > 0) {
			utils.log(
				"INFO",
				"SESSION_HOOK",
				`Cleaned up ${cleanedFiles} old cache files`,
			);
		}
	} catch (error) {
	...
	try {
		...
		if (stats.size > maxSize) {
			...
			utils.log(
				"INFO",
				"SESSION_HOOK",
				`Log rotated: ${Math.round(stats.size / 1024 / 1024)}MB -> ${backupFile}`,
			);
		}
	} catch (error) {
...
async function cleanupTempFiles() {
	try {
		...
		utils.log(
			"WARN",
			"SESSION_HOOK",
			`Temp file cleanup error: ${error.message}`,
		);
	}
}
