...
const createMockQuery = () => {
  ...
  const chainMethods = ['select', 'insert', 'update', 'delete', 'eq', 'neq', 'gt', 'lt', 'gte', 'lte', 'order', 'limit'];
...
describe('SessionManager', () => {
  ...
  describe('terminateSession', () => {
    ...
    it('should terminate session with custom reason', async () => {
      await expect(sessionManager.terminateSession('session-123', 'admin_logout')).resolves.toBeUndefined();
      expect(mockSupabase.from).toHaveBeenCalledWith('user_sessions');
    });
  ...
  describe('Integration Tests', () => {
    it('should complete full session lifecycle', async () => {
      ...
      // Update session
      const updatedSession = await sessionManager.updateSession(session.id, mockUpdateSessionRequest);
