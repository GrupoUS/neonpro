...
describe("RevenueOptimizationEngine", () => {
  ...
  describe("💰 Dynamic Pricing Optimization", () => {
    ...
    test("should handle pricing optimization errors gracefully", async () => {
      ...
      jest
        .spyOn(mockEngine as any, "analyzeMarketDemand")
        .mockRejectedValue(new Error("Network error"));

      await expect(mockEngine.optimizePricing(mockClinicId)).rejects.toThrow(
  ...
  describe("🔧 Error Handling and Edge Cases", () => {
    ...
    test("should handle network errors gracefully", async () => {
      ...
      await expect(engine.optimizePricing(mockClinicId)).rejects.toThrow(
        /Failed to optimize pricing strategy/,
      );
    });
