...
import { GET as nfeGET, POST as nfePOST, PUT as nfePUT, DELETE as nfeDELETE } from '@/app/api/tax/nfe/route';
...
describe('NFE API - Story 5.5 AC1: Automated NFSe Generation and Submission', () => {
  ...
  describe('GET /api/tax/nfe - NFE Listing and Status', () => {
    it('should list NFEs for a clinic', async () => {
      const request = new NextRequest(`http://localhost/api/tax/nfe?clinic_id=${testClinicId}&action=list`);
    ...
    it('should get specific NFE details', async () => {
      const request = new NextRequest(`http://localhost/api/tax/nfe?nfe_id=${testNFEId}&action=details`);
    ...
    it('should check NFE status with municipal authority', async () => {
      const request = new NextRequest(`http://localhost/api/tax/nfe?nfe_id=${testNFEId}&action=status`);
    ...
    it('should download NFE XML', async () => {
      const request = new NextRequest(`http://localhost/api/tax/nfe?nfe_id=${testNFEId}&action=download&format=xml`);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle invalid NFE ID', async () => {
      const request = new NextRequest('http://localhost/api/tax/nfe?nfe_id=invalid-id&action=details');
    ...
    it('should handle NFE emission failures', async () => {
      ...
      jest.mocked(require('@/lib/services/tax/nfe-service').NFEIntegrationService)
  ...
  describe('Performance Requirements - AC1', () => {
    ...
    it('should handle concurrent NFE generations efficiently', async () => {
      ...
      const responses = await Promise.all(
        concurrentRequests.map(request => nfePOST(request))
      );
      const endTime = Date.now();
