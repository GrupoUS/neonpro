  ...
  createClient: jest.fn(() => ({
    from: jest.fn((table) => {
      ...
      if (table === 'executive_dashboard_metrics' || table === 'executive_dashboard_charts') {
        ...
        mockChain.order = jest.fn(() => Promise.resolve({
          data: [
            {
              id: '1',
              date: '2024-01-01',
...
describe('ExecutiveDashboardService Integration', () => {
  ...
  describe('Error Handling', () => {
    it('should handle authentication errors gracefully', async () => {
      ...
      await expect(service.getDashboardMetrics(filters))
    ...
    it('should handle insufficient permissions', async () => {
      ...
      const verifyAccessSpy = jest.spyOn(service as any, 'verifyExecutiveAccess')
