...
describe('Subscription System Performance', () => {
  ...
  describe('Response Time Performance', () => {
    ...
    it('should handle concurrent subscription checks efficiently', async () => {
      ...
      const promises = Array.from({ length: 100 }, (_, index) => {
        return Promise.resolve(createMockSubscription({ 
          id: `test-sub-${index}`,
          status: 'active' 
        }))
      })
  ...
  describe('Caching Performance', () => {
    it('should provide significant performance improvement with caching', () => {      // Simulate cache miss (first call)
  ...
  describe('Load Testing', () => {
    ...
    it('should maintain performance under sustained load', async () => {
      ...
      const variance = performanceResults.reduce((acc, time) => 
        acc + Math.pow(time - mean, 2), 0) / performanceResults.length
