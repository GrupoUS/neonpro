-- Migration: Create Additional Compliance Documentation Tables
-- Date: 2025-01-27
...
CREATE POLICY IF NOT EXISTS "Users can view consent forms from their clinic" ON consent_forms
    FOR SELECT USING (
        clinic_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can insert consent forms for their clinic" ON consent_forms
...
CREATE POLICY IF NOT EXISTS "Users can update consent forms from their clinic" ON consent_forms
    FOR UPDATE USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can view legal documents from their clinic" ON legal_documents
    FOR SELECT USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can insert legal documents for their clinic" ON legal_documents
    FOR INSERT WITH CHECK (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can update legal documents from their clinic" ON legal_documents
    FOR UPDATE USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can view compliance metrics from their clinic" ON compliance_metrics
    FOR SELECT USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can insert compliance metrics for their clinic" ON compliance_metrics
    FOR INSERT WITH CHECK (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can view compliance training from their clinic" ON compliance_training
    FOR SELECT USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can insert compliance training for their clinic" ON compliance_training
    FOR INSERT WITH CHECK (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can update compliance training from their clinic" ON compliance_training
    FOR UPDATE USING (
        clinic_id IN (
            SELECT clinic_id FROM profiles 
            WHERE id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Users can view their own training completions" ON training_completions
    FOR SELECT USING (
        profile_id = auth.uid()
        OR clinic_id IN (
            SELECT clinic_id FROM profiles 
...
CREATE POLICY IF NOT EXISTS "Users can insert their own training completions" ON training_completions
    FOR INSERT WITH CHECK (
        profile_id = auth.uid()
        AND clinic_id IN (
            SELECT clinic_id FROM profiles 
...
CREATE POLICY IF NOT EXISTS "Users can update their own training completions" ON training_completions
    FOR UPDATE USING (
        profile_id = auth.uid()
    );
...
INSERT INTO compliance_training (clinic_id, training_name, training_type, description, duration_minutes, mandatory, recurring, recurrence_period_days, regulatory_requirement, target_roles, learning_objectives)
SELECT 
    c.id,
