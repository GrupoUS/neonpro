...
async function testEmailAutomation() {
  try {
    ...
    if (
      ...
      const _confirmationResult = await NeonProAutomation.sendAppointmentConfirmation(
        testAppointmentData,
      );

      const _reminderResult = await NeonProAutomation.scheduleAppointmentReminder(
        testAppointmentData,
      );
