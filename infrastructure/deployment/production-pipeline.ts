...
class ProductionDeploymentPipeline {
  ...
  private async monitorCanaryDeployment(durationMs: number): Promise<void> {
    ...
    while (Date.now() < endTime) {
      ...
      if (metrics.error_rate > this.config.rollback_threshold_error_rate) {
        throw new Error(`Error rate ${metrics.error_rate}% exceeds threshold ${this.config.rollback_threshold_error_rate}%`);
      }
  ...
  private async runComprehensiveHealthChecks(): Promise<void> {
    ...
    const healthCheckPromises = Array.from(this.healthCheckEndpoints.entries()).map(
      ([service, endpoint]) => this.runHealthCheck(service, endpoint)
    );
  ...
  private async runHealthCheck(service: string, endpoint: string): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const status = response.ok && responseData.healthy ? 'healthy' : 
                    response.ok ? 'degraded' : 'unhealthy';
  ...
  private async validatePerformanceMetrics(): Promise<void> {
    ...
    if (metrics.avg_response_time_ms > thresholds.max_avg_response_time_ms) {
      failures.push(`Average response time ${metrics.avg_response_time_ms}ms exceeds threshold ${thresholds.max_avg_response_time_ms}ms`);
    }
  ...
  private async collectCurrentMetrics(): Promise<any> {
    try {
      ...
      return data.data?.aggregated_metrics || {
        avg_response_time_ms: 0,
        requests_per_second: 0,
        error_count: 0,
        success_rate: 100
  ...
  private async monitorDeploymentHealth(): Promise<void> {
    ...
    while (Date.now() - startTime < monitoringDuration) {
      try {
        const metrics = await this.collectCurrentMetrics();
        this.deploymentStatus.error_rate = (metrics.error_count / (metrics.error_count + metrics.success_count || 1)) * 100;
  ...
  private async sendDeploymentNotification(status: 'success' | 'failed' | 'rollback' | 'rollback-failed', details?: string): Promise<void> {
    const notification = {
...
export { DeploymentConfig, DeploymentStatus, HealthCheckResult };