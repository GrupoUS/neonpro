#!/usr/bin/env ts-node
import ProductionDeploymentPipeline, { DeploymentConfig } from './production-pipeline';
...
Healthcare Compliance Notes:
  ...
  private static async loadDeploymentConfig(args: CLIArguments): Promise<DeploymentConfig> {
    ...
    const finalConfig: DeploymentConfig = {
      ...
      version: args.version || await this.getCurrentGitCommit(),
      deployment_strategy: args.strategy || config.deployment_strategy || 'blue-green',
      health_check_timeout_ms: config.health_check_timeout_ms || 30000,
      ...
      post_deployment_tests: args.skipTests ? [] : config.post_deployment_tests || [
  ...
  static async run(args: string[]): Promise<void> {
    ...
    try {
      ...
      const cliArgs = this.parseArguments(args);
      
      // Load deployment configuration
      const config = await this.loadDeploymentConfig(cliArgs);
      
      // Validate environment for production
      this.validateEnvironmentForProduction(config);
      
      // Confirm production deployment if needed
      const confirmed = await this.confirmProductionDeployment(config);
      if (!confirmed) {
      ...
      console.log(`  Health Checks: ${result.health_checks.filter(hc => hc.status === 'healthy').length}/${result.health_checks.length} healthy`);
