  ...
  fixFile(filePath) {
    try {
      ...
      if (filePath.includes('https-server.ts')) {
        const replacements = [
          { from: 'http.createServer(_(req,_res) => {', to: 'http.createServer((req, res) => {' },
          { from: '.listen(_this.config.httpPort,_this.config.host,_() => {', to: '.listen(this.config.httpPort, this.config.host, () => {' },
          { from: '.listen(_this.config.port,_this.config.host,_() => {', to: '.listen(this.config.port, this.config.host, () => {' },
          { from: 'return new Promise(_((resolve, reject)) => {', to: 'return new Promise((resolve, reject) => {' },
          { from: 'res.on(_\'finish\',_() => {', to: 'res.on(\'finish\', () => {' },
          { from: 'process.on(_\'SIGTERM\',_() => {', to: 'process.on(\'SIGTERM\', () => {' },
          { from: 'process.on(_\'SIGINT\',_() => {', to: 'process.on(\'SIGINT\', () => {' },
        ];

        for (const replacement of replacements) {
          if (fixedContent.includes(replacement.from)) {
            fixedContent = fixedContent.replace(new RegExp(replacement.from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), replacement.to);
            fixes++;
          }
        }
      }
      ...
      if (filePath.includes('certificate-renewal.ts')) {
        const replacements = [
          { from: 'certbot.on(_\'close\',_(code: number) => {', to: 'certbot.on(\'close\', (code: number) => {' },
        ];
