...
class CleanupFixer {
  ...
  fixFile(filePath) {
    try {
      ...
      const replacements = [
        { from: "Promise.all(stopPromises).then(_() => {", to: "Promise.all(stopPromises).then(() => {" },
        { from: "process.on(_'SIGTERM',_() => shutdown('SIGTERM'));", to: "process.on('SIGTERM', () => shutdown('SIGTERM'));" },
        { from: "process.on(_'SIGINT',_() => shutdown('SIGINT'));", to: "process.on('SIGINT', () => shutdown('SIGINT'));" },
      ];
