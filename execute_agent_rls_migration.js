...
const migrationPath = path.join(__dirname, 'supabase', 'migrations', '20250921040936_agent_rls_system_policies.sql');
...
async function setupSQLExecution() {
  ...
  try {
    ...
    if (error) {
      console.log('⚠️ SQL execution function may not exist, trying to create it...');
      // Try direct SQL execution
      const { error: createError } = await supabase.from('pg_tables').select('*').limit(1);
      if (createError) {
