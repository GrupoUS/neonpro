...
export const mockCPFValidator = {
  validate: vi.fn((cpf: string) => {
    ...
    return validCPFs.includes(cpf.replace(/\D/g, '').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4'))
  }),
...
export const mockSupabaseHealthcare = {
  patients: {
    create: vi.fn(async (data: any) => ({ data: mockPatientData.createMockPatient(), error: null })),
    update: vi.fn(async (id: string, data: any) => ({ data: { ...mockPatientData.createMockPatient(), ...data }, error: null })),
    delete: vi.fn(async (id: string) => ({ data: null, error: null })),
    findById: vi.fn(async (id: string) => ({ data: mockPatientData.createMockPatient(), error: null })),
    findByCPF: vi.fn(async (cpf: string) => ({ data: mockPatientData.createMockPatient(), error: null }))
  },
  
  treatments: {
    create: vi.fn(async (data: any) => ({ data: mockPatientData.createMockTreatment(), error: null })),
    update: vi.fn(async (id: string, data: any) => ({ data: { ...mockPatientData.createMockTreatment(), ...data }, error: null })),
    findByPatient: vi.fn(async (patientId: string) => ({ data: [mockPatientData.createMockTreatment()], error: null }))
  },
  
  medicalRecords: {
    create: vi.fn(async (data: any) => ({ data: mockPatientData.createMockMedicalRecord(), error: null })),
    findByPatient: vi.fn(async (patientId: string) => ({ data: [mockPatientData.createMockMedicalRecord()], error: null }))
  }
}