  ...
  describe('Audit Trail Security', () => {
    ...
    it('should automatically log all data access events', async () => {
      const dataAccessEvents = [
        ...
        { action: 'create_treatment', user: 'doctor-123', resource: 'treatment-456' },
  ...
  describe('Data Encryption and Storage', () => {
    it('should encrypt sensitive patient data at rest', () => {
      const sensitiveFields = [
        'cpf', 'email', 'phone', 'address', 'medical_history',
        'allergies', 'medications', 'diagnosis', 'treatment_notes'
      ]
    ...
    it('should use field-level encryption for medical data', async () => {
      ...
      const encryptedData = await Promise.all([
        { field: 'diagnosis', encrypted: await mockMedicalEncryption.encryptPatientData(medicalData.diagnosis) },
        { field: 'treatment_plan', encrypted: await mockMedicalEncryption.encryptPatientData(medicalData.treatment_plan) },
        { field: 'medical_notes', encrypted: await mockMedicalEncryption.encryptPatientData(medicalData.medical_notes) }
      ])
