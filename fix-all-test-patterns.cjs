...
testDirs.forEach(dir => {
  ...
  files.forEach(file => {
    ...
    content = content.replace(/describe\(_'(.*?)'_,\s*\(\) => {/g, "describe('$1', () => {");
    content = content.replace(/describe\(_\"(.*?)\"_,\s*\(\) => {/g, 'describe("$1", () => {');
    
    // Fix ALL it patterns: it_'desc') -> it('desc')
    content = content.replace(/it\(_'(.*?)'_,\s*async \(\) => {/g, "it('$1', async () => {");
    content = content.replace(/it\(_'(.*?)'_,\s*\(\) => {/g, "it('$1', () => {");
    content = content.replace(/it\(_\"(.*?)\"_,\s*async \(\) => {/g, 'it("$1", async () => {');
    content = content.replace(/it\(_\"(.*?)\"_,\s*\(\) => {/g, 'it("$1", () => {');
    
    // Fix ALL test patterns: test_'desc') -> test('desc')
    content = content.replace(/test\(_'(.*?)'_,\s*\(\) => {/g, "test('$1', () => {");
    content = content.replace(/test\(_\"(.*?)\"_,\s*\(\) => {/g, 'test("$1", () => {');
