...
import { PIIDetectionEngine, BrazilianPIIPatterns, SecurityUtilities } from '../apps/api/src/lib/pii-redaction';
...
async function debugDetection() {
  ...
  cpfResults.forEach((r, i) => {
    console.log(`  ${i+1}. Type: ${r.type}, Value: "${r.value}", Confidence: ${r.confidence}`);
  });
  ...
  console.log(`CPF Context Pattern: ${BrazilianPIIPatterns.CPF_CONTEXT_PATTERN}`);
  const contextMatch = contextText.match(BrazilianPIIPatterns.CPF_CONTEXT_PATTERN);
  console.log(`Direct context pattern match: ${contextMatch ? 'FOUND' : 'NOT FOUND'}`);
  if (contextMatch) {
  ...
  console.log(`Engine detection on unicode: ${unicodeResults.length} items found`);
  
  const normalizedResults = engine.detectPII(normalizedText);
  console.log(`Engine detection on normalized: ${normalizedResults.length} items found`);
  ...
  console.log(`Engine detection with Base64 scanning: ${base64Results.length} items found`);
  base64Results.forEach((r, i) => {
    console.log(`  ${i+1}. Type: ${r.type}, Value: "${r.value}", Base64: ${r.metadata?.detectedInBase64}`);
  });
