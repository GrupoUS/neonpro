...
function fixFunctionParameters(content, filePath) {
  ...
  fixed = fixed.replace(/\.sort\(_\((.*?),_(.*?)\)\s*=>/g, (match, param1, param2) => {
    localFixes++;
  ...
  fixed = fixed.replace(/\.forEach\(_\((.*?),_(.*?)\)\s*=>/g, (match, param1, param2) => {
    localFixes++;
  ...
  fixed = fixed.replace(/(\w+)\(_\((.*?)\)\s*=>/g, (match, funcName, params) => {
