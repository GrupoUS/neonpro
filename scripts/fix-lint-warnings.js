...
const PATTERNS = [
  // Unused parameters - prefix with underscore
  {
    ...
    fix: (content, matches) => {
      matches.forEach(match => {
        ...
        const paramRegex = new RegExp(`\\b${paramName}\\b(?=\\s*[,)]|\\s*:)`, 'g');
  ...
  // Unused catch parameters
  {
    ...
    fix: (content, matches) => {
      matches.forEach(match => {
        const paramName = match[1];
        const catchRegex = new RegExp(`catch\\s*\\(\\s*${paramName}\\s*\\)`, 'g');
  ...
  // Remove unused imports completely
  {
    ...
    fix: (content, matches) => {
      matches.forEach(match => {
        ...
        const namedImportRegex = new RegExp(`\\s*,?\\s*${importName}\\s*,?`, 'g');
