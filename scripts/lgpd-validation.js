...
async function validateConsentManagement() {
  ...
  try {
    ...
    testConsents.forEach((consent, index) => {
      const statusValid = validStatuses.has(consent.status);
      const basisValid = validLegalBases.has(consent.legal_basis);
...
async function validateDataSubjectRights() {
  ...
  try {
    ...
    testRequests.forEach((request, index) => {
      const typeValid = validTypes.has(request.type);
      const statusValid = validStatuses.has(request.status);
