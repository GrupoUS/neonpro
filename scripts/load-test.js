...
console.log('🔥 NeonPro Load Testing Suite');
console.log('=============================\n');
...
function logResult(test, value, target, unit, passed) {
  const icon = passed ? '✅' : '❌';
  const color = passed ? '\x1b[32m' : '\x1b[31m';
  console.log(`${icon} ${test}: ${color}${value}${unit}\x1b[0m (target: ${target}${unit})`);
}
...
async function runLoadTests() {
  try {
    ...
    for (const userCount of config.concurrent_users) {
      ...
      if (!analysis.passed) allPassed = false;
      
      // Log results for this user count
      console.log(`\n📊 Results for ${userCount} users:`);
      logResult('95th Percentile Response', analysis.p95ResponseTime, config.targets.response_time_95th, 'ms', analysis.breakdown.p95Pass);
      logResult('Error Rate', (analysis.errorRate * 100).toFixed(1), (config.targets.error_rate * 100), '%', analysis.breakdown.errorPass);
      logResult('Throughput', analysis.throughput, config.targets.throughput_min, ' req/s', analysis.breakdown.throughputPass);
...
async function generateLoadReport() {
  ...
  console.log('User Count | Throughput | 95th %ile | Error Rate | Status');
  console.log('-----------|------------|-----------|------------|-------');
  
  testResults.forEach(result => {
    const status = result.passed ? '✅ PASS' : '❌ FAIL';
    console.log(`${result.userCount.toString().padStart(9)} | ${result.throughput.toString().padStart(9)} | ${result.p95ResponseTime.toString().padStart(8)} | ${(result.errorRate * 100).toFixed(1).padStart(9)}% | ${status}`);
  });
  
  if (allPassed) {
    console.log('\n🎉 System scales well under load! Ready for production traffic.');
  } else {
    console.log('\n⚠️  Performance degrades under load. Optimization needed.');
    
    const failedTests = testResults.filter(r => !r.passed);
    console.log('\nPerformance Bottlenecks:');
    failedTests.forEach(result => {
      console.log(`  - ${result.userCount} users: `, end='');
      if (!result.breakdown.p95Pass) console.log('Response time too high');
      if (!result.breakdown.errorPass) console.log('Error rate too high');
      if (!result.breakdown.throughputPass) console.log('Throughput too low');
    });
    
    console.log('\nScaling Recommendations:');
    console.log('  1. Implement horizontal scaling (load balancer)');
    console.log('  2. Add database connection pooling');
    console.log('  3. Implement response caching');
