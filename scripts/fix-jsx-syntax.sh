...
# Fix JSX syntax issues in test files
cd apps/web

echo "Fixing JSX syntax issues in test files..."

# Fix the calendar integration test
cat > src/services/__tests__/lgpd/calendar-integration.test.ts << 'EOF'
...
# Fix the UI components contract test - just the problematic MockButton part
sed -i '/const MockButton/,/);$/c\
...
};' src/__tests__/contracts/ui-components.test.ts
