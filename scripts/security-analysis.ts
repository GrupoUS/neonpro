...
interface SecurityIssue {
  type: 'hardcoded_secret' | 'hardcoded_salt' | 'sensitive_logging' | 'insecure_storage';
  ...
  lines.forEach((line, index) => {
    ...
    if (line.includes("'default-secret'") || line.includes("'default-key'")) {
      issues.push({
        ...
        line: lineNum,
        code: line.trim(),
        description: 'Hardcoded default secret detected. This compromises security if environment variables are not set.',
    ...
    if (line.includes("'salt'") && line.includes('scryptSync')) {
      issues.push({
        ...
        line: lineNum,
        code: line.trim(),
        description: 'Hardcoded salt in encryption function. This makes encryption predictable and vulnerable.',
        lgpdCompliance: false,
        cfmCompliance: false
      });
    }
    ...
    if (line.includes('console.log') && (
      ...
      issues.push({
        ...
        line: lineNum,
        code: line.trim(),
        description: 'Potentially sensitive data being logged. This may expose patient information.',
        lgpdCompliance: false,
        cfmCompliance: false
      });
    }
    ...
    if (line.includes('Map<string, TelemedicineSession>') ||
      ...
      issues.push({
        ...
        line: lineNum,
        code: line.trim(),
        description: 'Sensitive medical session data stored in unencrypted memory. Potential memory leak vulnerability.',
        lgpdCompliance: false,
        cfmCompliance: false
      });
    }
...
async function generateSecurityReport(): Promise<void> {
  ...
  console.log(`LGPD Violations: ${issues.filter(i => !i.lgpdCompliance).length}`);
  console.log(`CFM Violations: ${issues.filter(i => !i.cfmCompliance).length}`);
  console.log();
