...
class StoryValidator {
  ...
  parseStoryFile(content, filename) {
    ...
    const statusLine = lines.find(line => line.includes('Status:') || line.includes('**Status**:'));
  ...
  extractStatus(statusLine) {
    ...
    if (statusLine.includes('COMPLETED') || statusLine.includes('✅')) {
      return 'COMPLETED';
    } else if (statusLine.includes('IN_PROGRESS') || statusLine.includes('🔄')) {
      return 'IN_PROGRESS';
    } else if (statusLine.includes('PENDING') || statusLine.includes('⏳')) {
      return 'PENDING';
    }
  ...
  calculateFinalStatus(checks) {
    ...
    if (isImplemented && hasFiles && hasTests) {
      return 'COMPLETED';
    } else if (isImplemented || hasFiles) {
      return 'IN_PROGRESS';
    } else {
      return 'PENDING';
    }
      ...
      if (result) {
        ...
        switch (validation.finalStatus) {
          case 'COMPLETED':
            ...
            this.log(`  ✅ COMPLETED (Impl: ${validation.implementation.score}%, Files: ${validation.files.score}%, Tests: ${validation.tests.score}%)`, 'green');
            break;
          case 'IN_PROGRESS':
            this.results.inProgress.push(result);
            this.log(`  🔄 IN_PROGRESS (Impl: ${validation.implementation.score}%, Files: ${validation.files.score}%, Tests: ${validation.tests.score}%)`, 'yellow');
            break;
          case 'PENDING':
            this.results.pending.push(result);
            this.log(`  ⏳ PENDING (Impl: ${validation.implementation.score}%, Files: ${validation.files.score}%, Tests: ${validation.tests.score}%)`, 'red');
            break;
        }
        
        // Mostrar discrepâncias
        if (result.declaredStatus !== validation.finalStatus) {
          this.log(`  ⚠️  Status declarado (${result.declaredStatus}) difere do real (${validation.finalStatus})`, 'yellow');
        }
      }
    }
  ...
  generateReport() {
    ...
    const total = this.results.completed.length + this.results.inProgress.length + this.results.pending.length;
    
    this.log(`\n📈 RESUMO GERAL:`, 'blue');
    this.log(`  Total de stories: ${total}`);
    this.log(`  ✅ Completed: ${this.results.completed.length} (${((this.results.completed.length / total) * 100).toFixed(1)}%)`, 'green');
    this.log(`  🔄 In Progress: ${this.results.inProgress.length} (${((this.results.inProgress.length / total) * 100).toFixed(1)}%)`, 'yellow');
    this.log(`  ⏳ Pending: ${this.results.pending.length} (${((this.results.pending.length / total) * 100).toFixed(1)}%)`, 'red');
    ...
    const discrepancies = [...this.results.completed, ...this.results.inProgress, ...this.results.pending]
    ...
    if (needsAttention.length > 0) {
      ...
      needsAttention.forEach(story => {
        this.log(`  ${story.filename} (Files: ${story.validation.files.score}%, Missing: ${story.validation.files.missing.length})`);
      });
    }
  ...
  saveReportToFile() {
    ...
    const report = {
      ...
      summary: {
        total: this.results.completed.length + this.results.inProgress.length + this.results.pending.length,
