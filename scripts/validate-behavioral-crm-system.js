...
class BehavioralCrmValidator {
  ...
  async testPatientBehaviorAnalysis() {
    ...
    const requiredProfileFields = ['engagement_level', 'communication_preference', 'appointment_booking_pattern', 'loyalty_score', 'churn_risk'];
  ...
  async testPatientSegmentation() {
    ...
    return {
      ...
      lowest_score: data.segmented_patients.length > 0 ? data.segmented_patients[data.segmented_patients.length - 1].segment_score : 0
    };
  }
  ...
  async testPersonalizedMessageGeneration() {
    ...
    const messageTemplate = 'Hello {patient_name}, your {loyalty_level} status gives you priority booking at {preferred_time}. You last visited us {last_visit}.';
    ...
    const hasUnreplacedVariables = personalizedMessage.personalized_content.includes('{') && personalizedMessage.personalized_content.includes('}');
    if (hasUnreplacedVariables) {
      console.warn('⚠️  Warning: Some personalization variables may not have been replaced');
    }
  ...
  async testBehavioralInsights() {
    ...
    for (const insight of data.insights) {
      const requiredFields = ['insight_id', 'insight_type', 'title', 'description', 'key_findings', 'actionable_recommendations'];
      for (const field of requiredFields) {
        if (!insight[field]) {
    ...
    return {
      ...
      avg_recommendations_per_insight: data.insights.reduce((sum, i) => sum + i.actionable_recommendations.length, 0) / data.insights.length
    };
  }
  ...
  async testApiHealth() {
    ...
    const requiredEndpoints = ['behavioral_analytics', 'personalization_engine', 'campaign_automation', 'dashboard_analytics'];
  ...
  async testResponseTimePerformance() {
    ...
    if (!allWithinLimits) {
      console.warn('⚠️  Some endpoints exceeded expected response time limits');
    }
  ...
  async runAllValidations() {
    console.log('\n🚀 Starting Behavioral CRM System Validation');
    console.log('=' * 60);
    ...
    console.log('\n🎯 Personalization Engine Tests');
    console.log('-' * 30);
    await this.runTest('Personalized Message Generation', () => this.testPersonalizedMessageGeneration(), 'personalization_engine');
    await this.runTest('Personalization Rule Creation', () => this.testPersonalizationRuleCreation(), 'personalization_engine');

    // Campaign Automation Tests
    console.log('\n📧 Campaign Automation Tests');
    console.log('-' * 30);
    await this.runTest('Behavioral Campaign Creation', () => this.testBehavioralCampaignCreation(), 'campaign_automation');
  ...
  generateValidationReport() {
    ...
    console.log('\n' + '=' * 60);
    console.log('🎯 BEHAVIORAL CRM SYSTEM VALIDATION REPORT');
    console.log('=' * 60);
    ...
    if (overallSuccessRate >= 90) {
      console.log('\n🎉 BEHAVIORAL CRM SYSTEM VALIDATION: SUCCESS! 🎉');
      console.log('✨ The system is performing within expected parameters.');
    } else if (overallSuccessRate >= 70) {
      console.log('\n⚠️  BEHAVIORAL CRM SYSTEM VALIDATION: PARTIAL SUCCESS');
      console.log('🔧 Some components need attention but core functionality is working.');
    } else {
      console.log('\n🚨 BEHAVIORAL CRM SYSTEM VALIDATION: NEEDS ATTENTION');
      console.log('⚡ Multiple critical issues detected. System requires review.');
    }
