...
async function debugValidation() {
  ...
  try {
    ...
    console.log(`First check digit calculated: ${checkDigit1}, actual: ${digits[9]}`);
    ...
    console.log(`Second check digit calculated: ${checkDigit2}, actual: ${digits[10]}`);
  ...
  const contextPattern = /(?:cpf|cadastro\s+de\s+pessoas?\s+físicas?)\s*[:=]\s*(\d{3}\.?\d{3}\.?\d{3}-?\d{2})/gi;
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    console.log(`Pattern ${i+1}: ${match ? `FOUND "${match[1]}"` : 'NOT FOUND'}`);
  }
