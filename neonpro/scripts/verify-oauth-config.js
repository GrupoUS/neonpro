...
if (fs.existsSync(envPath)) {
  ...
  console.log(
    `${hasGoogleClientSecret ? "✅" : "❌"} GOOGLE_CLIENT_SECRET definido`
  );
  ...
  if (hasGoogleClientId && hasGoogleClientSecret) {
    ...
    const secretPlaceholder =
      clientSecretMatch &&
      clientSecretMatch[1].includes("your_google_client_secret_here");
...
if (fs.existsSync(configPath)) {
  ...
  const hasClientIdRef = configContent.includes(
    'client_id = "env(GOOGLE_CLIENT_ID)"'
  );
  const hasSecretRef = configContent.includes(
    'secret = "env(GOOGLE_CLIENT_SECRET)"'
  );

  console.log(`✅ Arquivo config.toml encontrado`);
  console.log(
    `${hasGoogleConfig ? "✅" : "❌"} Seção [auth.external.google] presente`
  );
  console.log(`${isGoogleEnabled ? "✅" : "❌"} Google OAuth habilitado`);
  console.log(
    `${hasClientIdRef ? "✅" : "❌"} client_id referenciando env var`
  );
