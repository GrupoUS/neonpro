...
export const createEquipmentSchema = z.object({
  ...
  model: z.string().max(255, 'Modelo deve ter no máximo 255 caracteres').optional(),
  manufacturer: z.string().max(255, 'Fabricante deve ter no máximo 255 caracteres').optional(),
  serial_number: z.string().max(255, 'Número serial deve ter no máximo 255 caracteres').optional(),
  ...
  department: z.string().max(100, 'Departamento deve ter no máximo 100 caracteres').optional(),
  room_number: z.string().max(50, 'Número da sala deve ter no máximo 50 caracteres').optional(),
  
  // Purchase and warranty
  purchase_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de compra deve estar no formato YYYY-MM-DD').optional(),
  purchase_cost: z.number().min(0, 'Custo de compra deve ser positivo').optional(),
  warranty_start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de início da garantia deve estar no formato YYYY-MM-DD').optional(),
  warranty_end_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de fim da garantia deve estar no formato YYYY-MM-DD').optional(),
  ...
  estimated_lifespan_years: z.number().int().min(1).max(50, 'Vida útil deve estar entre 1 e 50 anos').optional(),
  maintenance_frequency_days: z.number().int().min(1, 'Frequência de manutenção deve ser pelo menos 1 dia').optional(),
  usage_based_maintenance: z.boolean().default(false),
  usage_threshold_hours: z.number().min(0, 'Limite de horas deve ser positivo').optional(),
  usage_threshold_cycles: z.number().int().min(0, 'Limite de ciclos deve ser positivo').optional(),
  
  // Regulatory
  requires_calibration: z.boolean().default(false),
  calibration_frequency_days: z.number().int().min(1, 'Frequência de calibração deve ser pelo menos 1 dia').optional(),
...
export const updateEquipmentSchema = createEquipmentSchema.partial().omit({ clinic_id: true });
...
export const createMaintenanceScheduleSchema = z.object({
  ...
  frequency_days: z.number().int().min(1, 'Frequência em dias deve ser pelo menos 1').optional(),
  frequency_hours: z.number().min(0.1, 'Frequência em horas deve ser positiva').optional(),
  frequency_cycles: z.number().int().min(1, 'Frequência em ciclos deve ser pelo menos 1').optional(),
  
  // Scheduling preferences
  preferred_time_slot: z.string().max(20).optional(),
  estimated_duration_minutes: z.number().int().min(1, 'Duração estimada deve ser pelo menos 1 minuto').optional(),
  required_skills: z.array(z.string()).default([]),
  required_tools: z.array(z.string()).default([]),
  required_parts: z.array(z.string()).default([]),
  
  // Cost planning
  estimated_cost: z.number().min(0, 'Custo estimado deve ser positivo').optional(),
  budget_code: z.string().max(50, 'Código do orçamento deve ter no máximo 50 caracteres').optional(),
  
  // Notification settings
  alert_days_before: z.number().int().min(0).max(365, 'Alerta deve estar entre 0 e 365 dias').default(7),
  critical_alert_days_before: z.number().int().min(0).max(30, 'Alerta crítico deve estar entre 0 e 30 dias').default(3),
  notification_recipients: z.array(z.string().uuid('ID do destinatário deve ser um UUID válido')).default([])
}).refine((data) => {
...
export const updateMaintenanceScheduleSchema = z.object({
  ...
  // Notification settings
  alert_days_before: z.number().int().min(0).max(365, 'Alerta deve estar entre 0 e 365 dias').optional(),
  critical_alert_days_before: z.number().int().min(0).max(30, 'Alerta crítico deve estar entre 0 e 30 dias').optional(),
  notification_recipients: z.array(z.string().uuid('ID do destinatário deve ser um UUID válido')).optional(),
...
export const createWorkOrderSchema = z.object({
  equipment_id: z.string().uuid('ID do equipamento deve ser um UUID válido'),
  schedule_id: z.string().uuid('ID do cronograma deve ser um UUID válido').optional(),
  clinic_id: z.string().uuid('ID da clínica deve ser um UUID válido'),
  ...
  // Scheduling
  scheduled_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data agendada deve estar no formato YYYY-MM-DD').optional(),
  scheduled_start_time: z.string().regex(/^\d{2}:\d{2}$/, 'Horário deve estar no formato HH:MM').optional(),
  estimated_duration_minutes: z.number().int().min(1, 'Duração estimada deve ser pelo menos 1 minuto').optional(),
  
  // Assignment
  assigned_technician_id: z.string().uuid('ID do técnico deve ser um UUID válido').optional(),
  assigned_team_ids: z.array(z.string().uuid('ID da equipe deve ser um UUID válido')).default([]),
  external_vendor_id: z.string().uuid('ID do fornecedor deve ser um UUID válido').optional()
});
...
export const createUsageLogSchema = z.object({
  ...
  // Usage session
  session_start: z.string().datetime('Data/hora de início deve ser um datetime válido'),
  session_end: z.string().datetime('Data/hora de fim deve ser um datetime válido').optional(),
  usage_type: z.string().max(50).optional(),
  
  // Metrics
  cycles_performed: z.number().int().min(0, 'Ciclos executados deve ser positivo').optional(),
  power_consumption_kwh: z.number().min(0, 'Consumo de energia deve ser positivo').optional(),
  performance_metrics: z.record(z.any()).optional(),
  
  // Operator information
  operated_by: z.string().uuid('ID do operador deve ser um UUID válido').optional(),
  department: z.string().max(100).optional(),
  patient_id: z.string().uuid('ID do paciente deve ser um UUID válido').optional(),
  procedure_type: z.string().max(100).optional(),
  
  // Environmental conditions
  ambient_temperature: z.number().min(-50).max(100, 'Temperatura deve estar entre -50°C e 100°C').optional(),
  humidity_percentage: z.number().min(0).max(100, 'Umidade deve estar entre 0% e 100%').optional(),
  environmental_conditions: z.record(z.any()).optional(),
  
  // Performance assessment
  performance_rating: z.number().int().min(1).max(10, 'Avaliação deve estar entre 1 e 10').optional(),
...
export const createMaintenanceAlertSchema = z.object({
  equipment_id: z.string().uuid('ID do equipamento deve ser um UUID válido'),
  schedule_id: z.string().uuid('ID do cronograma deve ser um UUID válido').optional(),
  work_order_id: z.string().uuid('ID da ordem de serviço deve ser um UUID válido').optional(),
  clinic_id: z.string().uuid('ID da clínica deve ser um UUID válido'),
  ...
  // Timing
  trigger_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data do trigger deve estar no formato YYYY-MM-DD'),
  due_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de vencimento deve estar no formato YYYY-MM-DD').optional(),
  
  // Notification settings
  notification_recipients: z.array(z.string().uuid('ID do destinatário deve ser um UUID válido')).default([]),
  delivery_methods: z.array(z.enum(['email', 'sms', 'push', 'dashboard'])).default(['dashboard'])
});
...
export const createVendorServiceContractSchema = z.object({
  equipment_id: z.string().uuid('ID do equipamento deve ser um UUID válido'),
  ...
  contract_type: z.enum(['warranty', 'service_agreement', 'maintenance_contract']),
  service_level: z.enum(['basic', 'standard', 'premium', 'full_service']).optional(),
  
  // Coverage
  coverage_type: z.enum(['parts_only', 'labor_only', 'parts_and_labor', 'full_coverage']),
  covered_services: z.array(z.string()).default([]),
  excluded_services: z.array(z.string()).default([]),
  response_time_hours: z.number().int().min(1, 'Tempo de resposta deve ser pelo menos 1 hora').optional(),
  resolution_time_hours: z.number().int().min(1, 'Tempo de resolução deve ser pelo menos 1 hora').optional(),
  
  // Contract terms
  start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de início deve estar no formato YYYY-MM-DD'),
  end_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de fim deve estar no formato YYYY-MM-DD'),
  auto_renewal: z.boolean().default(false),
  contract_value: z.number().min(0, 'Valor do contrato deve ser positivo').optional(),
  payment_schedule: z.enum(['monthly', 'quarterly', 'annually', 'one_time']).optional(),
  
  // Performance guarantees
  uptime_guarantee_percentage: z.number().min(0).max(100, 'Garantia de disponibilidade deve estar entre 0% e 100%').optional(),
  ...
  vendor_contact_email: z.string().email('Email de contato deve ser válido').optional(),
  emergency_contact_phone: z.string().max(20).optional(),
  
  // Documentation
  contract_document_url: z.string().url('URL do documento deve ser válida').optional(),
  terms_and_conditions_url: z.string().url('URL dos termos deve ser válida').optional()
}).refine((data) => {
...
export const createComplianceRecordSchema = z.object({
  ...
  requirement_type: z.enum(['calibration', 'inspection', 'testing', 'certification']),
  compliance_standard: z.string().max(100).optional(),
  
  // Timing
  last_compliance_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de última conformidade deve estar no formato YYYY-MM-DD').optional(),
  next_compliance_due_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de vencimento deve estar no formato YYYY-MM-DD').optional(),
  compliance_frequency_days: z.number().int().min(1, 'Frequência deve ser pelo menos 1 dia').optional(),
  
  // Documentation
  certificate_number: z.string().max(100).optional(),
  certificate_url: z.string().url('URL do certificado deve ser válida').optional(),
  compliance_report_url: z.string().url('URL do relatório deve ser válida').optional(),
  test_results: z.record(z.any()).optional(),
  
  // Responsible parties
  compliance_officer_id: z.string().uuid('ID do oficial de compliance deve ser um UUID válido').optional(),
  testing_organization: z.string().max(255).optional(),
  certified_by: z.string().max(255).optional(),
  
  // Notification settings
  reminder_days_before: z.number().int().min(1).max(365, 'Lembrete deve estar entre 1 e 365 dias').default(30),
  critical_reminder_days_before: z.number().int().min(1).max(30, 'Lembrete crítico deve estar entre 1 e 30 dias').default(7),
  notification_recipients: z.array(z.string().uuid('ID do destinatário deve ser um UUID válido')).default([])
});
...
export const analyticsQuerySchema = z.object({
  clinic_id: z.string().uuid(),
  period_start: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de início deve estar no formato YYYY-MM-DD'),
  period_end: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de fim deve estar no formato YYYY-MM-DD'),
