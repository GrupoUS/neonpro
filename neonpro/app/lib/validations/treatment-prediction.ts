...
export const ConfidenceIntervalSchema = z.object({
  ...
  confidence_level: z.number().min(0).max(1).default(0.95).describe('Confidence level (e.g., 0.95 for 95%)')
...
export const PredictionModelSchema = z.object({
  ...
  algorithm_type: z.enum(['ensemble', 'neural_network', 'random_forest', 'gradient_boosting', 'svm']),
  accuracy: z.number().min(0).max(1).refine(val => val >= 0.85, {
...
export const ModelFiltersSchema = z.object({
  ...
  algorithm_type: z.enum(['ensemble', 'neural_network', 'random_forest', 'gradient_boosting', 'svm']).optional(),
...
export {
    UpdatePatientFactorsSchema, UpdatePredictionModelSchema, UpdateTreatmentCharacteristicsSchema, UpdateTreatmentPredictionSchema
};
