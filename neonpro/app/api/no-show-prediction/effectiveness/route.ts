...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: predictions, error: predictionsError } = await effectivenessQuery;

    if (predictionsError) {
      console.error('Database error:', predictionsError);
      return NextResponse.json({ error: 'Failed to fetch effectiveness data' }, { status: 500 });
    }
    ...
    const interventionEffectiveness = predictions?.reduce((acc, prediction) => {
    ...
    const modelPerformance = predictions?.reduce((acc, prediction) => {
