...
export async function GET(request: NextRequest) {
  try {
    ...
    const { page = 1, limit = 10, category, status, search } = ListDocumentsSchema.parse(queryParams)
    ...
    if (search) {
      query = query.or(`document_type.ilike.%${search}%,document_number.ilike.%${search}%,file_name.ilike.%${search}%`)
    }
