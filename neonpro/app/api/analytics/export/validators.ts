...
export function validateType(type: string): ValidationResult {
  ...
  if (!SUPPORTED_TYPES.includes(type as ExportType)) {
    return {
      valid: false,
      errors: [`Unsupported type: ${type}. Supported types: ${SUPPORTED_TYPES.join(', ')}`]
    }
  }
...
export function validateOptions(options: any): ValidationResult {
  ...
  if (options && typeof options === 'object') {
    ...
    const booleanOptions = ['includeCharts', 'includeHeader', 'includeFooter', 'compression']
  ...
  if (!data.metrics || !Array.isArray(data.metrics)) {
    ...
    data.metrics.forEach((metric: any, index: number) => {
      ...
      if (typeof metric.totalUsers !== 'number' || metric.totalUsers < 0) {
        errors.push(`Metric ${index}: totalUsers must be a non-negative number`)
      }
      if (typeof metric.retentionRate !== 'number' || metric.retentionRate < 0 || metric.retentionRate > 100) {
        errors.push(`Metric ${index}: retentionRate must be a number between 0 and 100`)
      }
      ...
      if (typeof metric.churnRate !== 'number' || metric.churnRate < 0 || metric.churnRate > 100) {
        errors.push(`Metric ${index}: churnRate must be a number between 0 and 100`)
      }
    })
  }
    ...
    data.predictions.forEach((prediction: any, index: number) => {
      ...
      if (prediction.confidence !== undefined) {
        if (typeof prediction.confidence !== 'number' || prediction.confidence < 0 || prediction.confidence > 1) {
          errors.push(`Prediction ${index}: confidence must be a number between 0 and 1`)
        }
      }
    })
  }
  ...
  if (data.correlations && !Array.isArray(data.correlations)) {
    ...
    data.correlations.forEach((corr: any, index: number) => {
      if (!corr.metric1 || !corr.metric2) {
        errors.push(`Correlation ${index}: both metric1 and metric2 are required`)
      }
      if (typeof corr.correlation !== 'number' || corr.correlation < -1 || corr.correlation > 1) {
        errors.push(`Correlation ${index}: correlation must be a number between -1 and 1`)
      }
    })
  }
  ...
  if (data.anomalies && !Array.isArray(data.anomalies)) {
    ...
    data.anomalies.forEach((anomaly: any, index: number) => {
      ...
      if (anomaly.severity && !['Low', 'Medium', 'High', 'Critical'].includes(anomaly.severity)) {
        errors.push(`Anomaly ${index}: severity must be Low, Medium, High, or Critical`)
      }
    })
  }
 ...
 */
export function normalizeOptions(options: Partial<ExportOptions> = {}): ExportOptions {
  return {
 ...
 */
export function validateFileSize(data: Buffer | string, maxSize: number = 50 * 1024 * 1024): ValidationResult {
  const size = Buffer.isBuffer(data) ? data.length : Buffer.byteLength(data, 'utf8')
  
  if (size > maxSize) {
    return {
      valid: false,
      errors: [`File size (${Math.round(size / 1024 / 1024)}MB) exceeds maximum allowed size (${Math.round(maxSize / 1024 / 1024)}MB)`]
    }
  }
...
export function validateRateLimit(
  ...
  const recentExports = exportHistory.filter(export_ => 
    new Date(export_.createdAt) > oneHourAgo
  )

  const dailyExports = exportHistory.filter(export_ => 
    new Date(export_.createdAt) > oneDayAgo
  )

  if (recentExports.length >= limits.maxExportsPerHour) {
    return {
      valid: false,
      errors: [`Rate limit exceeded: Maximum ${limits.maxExportsPerHour} exports per hour`]
    }
  }
  ...
  if (dailyExports.length >= limits.maxExportsPerDay) {
    return {
      valid: false,
      errors: [`Rate limit exceeded: Maximum ${limits.maxExportsPerDay} exports per day`]
    }
  }
 ...
 */
export function createValidationError(errors: string[], code: ExportErrorCode = ExportErrorCode.VALIDATION_FAILED) {
  return {
