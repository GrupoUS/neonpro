...
export class DeviceManager extends EventEmitter {
   ...
   */
  async getDeviceStatistics(userId?: string, clinicId?: string): Promise<{
    totalDevices: number;
    ...
    try {
      ...
      stats.recentRegistrations = data.filter(d => 
        new Date(d.registered_at) > sevenDaysAgo
      ).length;
  ...
  async cleanupOldDevices(retentionDays: number = 90): Promise<number> {
    try {
      ...
      if (fetchError) {
        throw new SessionError('Failed to fetch old devices', 'SYSTEM_ERROR', { error: fetchError });
      }
      ...
      const { error: deleteError } = await this.supabase
        .from('device_registrations')
        .delete()
        .in('id', oldDevices.map(d => d.id));
      
      if (deleteError) {
        throw new SessionError('Failed to delete old devices', 'SYSTEM_ERROR', { error: deleteError });
      }
  ...
  private async getDeviceByFingerprint(fingerprintHash: string, userId: string): Promise<DeviceRegistration | null> {
    try {
  ...
  private async updateDeviceUsage(deviceId: string, updates: {
    lastUsed: Date;
    ipAddress: string;
    location?: SessionLocation;
  }): Promise<DeviceRegistration> {
  ...
  private async assessDeviceRisk(device: DeviceRegistration): Promise<{
    ...
    const daysSinceRegistration = (Date.now() - device.registeredAt.getTime()) / (1000 * 60 * 60 * 24);
