...
import { DEFAULT_SESSION_CONFIG, SESSION_POLICIES } from '../config';
import type {
  UserSession,
    ...
    try {
      ...
      if (sessionToken) {
        ...
        if (finalConfig.checkDeviceTrust) {
          const deviceCheck = await checkDeviceTrust(
            session,
            deviceInfo,
            deviceManager
          );
        ...
        const locationCheck = await checkLocationAnomaly(
          session,
          locationInfo,
          securityMonitor
        );
...
async function checkDeviceTrust(
  ...
  try {
    const device = await deviceManager.getDeviceByFingerprint(
      session.user_id,
      fingerprint
    );
