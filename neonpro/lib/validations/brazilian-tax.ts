...
export const cnpjCpfSchema = z.string()
  ...
  .refine((value) => {
    const digits = value.replace(/[^\d]/g, '');
    if (digits.length === 11) {
      return cpfSchema.safeParse(value.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')).success;
    } else {
      return cnpjSchema.safeParse(value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')).success;
    }
...
export const brazilianAddressSchema = z.object({
  logradouro: z.string().min(1, 'Logradouro é obrigatório').max(100, 'Logradouro muito longo'),
  ...
  bairro: z.string().min(1, 'Bairro é obrigatório').max(50, 'Bairro muito longo'),
  cidade: z.string().min(1, 'Cidade é obrigatória').max(50, 'Cidade muito longa'),
  uf: ufSchema,
  cep: cepSchema
});
...
export const taxConfigurationSchema = z.object({
  ...
  // Tax Rates (0-100%)
  icms_rate: z.number().min(0, 'Taxa ICMS não pode ser negativa').max(100, 'Taxa ICMS não pode exceder 100%'),
  iss_rate: z.number().min(0, 'Taxa ISS não pode ser negativa').max(25, 'Taxa ISS não pode exceder 25%'),
  pis_rate: z.number().min(0, 'Taxa PIS não pode ser negativa').max(10, 'Taxa PIS não pode exceder 10%'),
  cofins_rate: z.number().min(0, 'Taxa COFINS não pode ser negativa').max(15, 'Taxa COFINS não pode exceder 15%'),
  irpj_rate: z.number().min(0, 'Taxa IRPJ não pode ser negativa').max(40, 'Taxa IRPJ não pode exceder 40%'),
  csll_rate: z.number().min(0, 'Taxa CSLL não pode ser negativa').max(15, 'Taxa CSLL não pode exceder 15%'),
  
  // Simples Nacional
  simples_nacional_anexo: z.enum(['I', 'II', 'III', 'IV', 'V'], {
    errorMap: () => ({ message: 'Anexo do Simples Nacional inválido' })
  }),
  simples_nacional_rate: z.number().min(0, 'Taxa Simples Nacional não pode ser negativa').max(35, 'Taxa Simples Nacional não pode exceder 35%'),
...
export const serviceItemSchema = z.object({
  codigo: z.string().min(1, 'Código do serviço é obrigatório').max(20, 'Código muito longo'),
  ...
  codigo_servico: z.string().max(10, 'Código de serviço muito longo').optional(),
...
export const nfeDocumentSchema = z.object({
  ...
  // NFe Identification
  numero_nfe: z.number().int('Número NFe deve ser inteiro').positive('Número NFe deve ser positivo'),
  serie_nfe: z.number().int('Série NFe deve ser inteiro').positive('Série NFe deve ser positiva').default(1),
  ...
  natureza_operacao: z.string().min(1, 'Natureza da operação é obrigatória').max(100, 'Natureza da operação muito longa'),
  
  // Tax Information
  valor_total: z.number().positive('Valor total deve ser positivo'),
  valor_base_calculo: z.number().nonnegative('Base de cálculo não pode ser negativa').optional(),
  valor_icms: z.number().nonnegative('Valor ICMS não pode ser negativo').default(0),
  valor_iss: z.number().nonnegative('Valor ISS não pode ser negativo').default(0),
  valor_pis: z.number().nonnegative('Valor PIS não pode ser negativo').default(0),
  valor_cofins: z.number().nonnegative('Valor COFINS não pode ser negativo').default(0),
  ...
  // Services
  servicos: z.array(serviceItemSchema).min(1, 'Pelo menos um serviço é obrigatório'),
  
  // Status
  status: z.enum(['draft', 'authorized', 'cancelled', 'rejected'], {
...
export const taxCalculationRequestSchema = z.object({
  ...
  tipo_servico: z.string().min(1, 'Tipo de serviço é obrigatório').max(100, 'Tipo de serviço muito longo'),
  codigo_servico: z.string().max(10, 'Código de serviço muito longo').optional(),
...
export const nfeGenerationRequestSchema = z.object({
  ...
  natureza_operacao: z.string().max(100, 'Natureza da operação muito longa').optional(),
  observacoes: z.string().max(1000, 'Observações muito longas').optional()
});

// SPED Entry Schema
export const spedEntrySchema = z.object({
  clinic_id: z.string().uuid('ID da clínica inválido'),
  
  // SPED Information
  periodo_apuracao: z.string().regex(/^\d{4}-\d{2}-01$/, 'Período deve estar no formato YYYY-MM-01'),
  tipo_escrituracao: z.enum(['ECD', 'ECF', 'EFD'], {
    errorMap: () => ({ message: 'Tipo de escrituração inválido' })
  }),
  
  // Entry Details
  codigo_conta: z.string().min(1, 'Código da conta é obrigatório').max(20, 'Código da conta muito longo'),
  descricao_conta: z.string().min(1, 'Descrição da conta é obrigatória').max(200, 'Descrição muito longa'),
  valor_debito: z.number().nonnegative('Valor débito não pode ser negativo').default(0),
  valor_credito: z.number().nonnegative('Valor crédito não pode ser negativo').default(0),
  
  // Document References
  numero_documento: z.string().max(50, 'Número do documento muito longo').optional(),
  data_documento: z.string().datetime('Data do documento inválida').optional(),
  ...
  // Classification
  natureza_operacao: z.string().max(100, 'Natureza da operação muito longa').optional(),
  origem_lancamento: z.enum(['system', 'manual', 'imported'], {
...
export const taxComplianceReportSchema = z.object({
  ...
  // Report Information
  tipo_relatorio: z.enum(['sped_ecd', 'sped_ecf', 'livro_registro', 'dctf', 'defis'], {
    errorMap: () => ({ message: 'Tipo de relatório inválido' })
  }),
  periodo_referencia: z.string().regex(/^\d{4}-\d{2}-01$/, 'Período deve estar no formato YYYY-MM-01'),
  ...
  // Submission
  status_envio: z.enum(['pending', 'sent', 'accepted', 'rejected'], {
    errorMap: () => ({ message: 'Status de envio inválido' })
  }).default('pending')
});

// Service Tax Code Schema
export const serviceTaxCodeSchema = z.object({
  codigo_servico: z.string().regex(/^\d{2}\.\d{2}$/, 'Código deve estar no formato XX.XX'),
  descricao_servico: z.string().min(1, 'Descrição é obrigatória').max(500, 'Descrição muito longa'),
  categoria: z.string().max(100, 'Categoria muito longa').optional(),
  
  // Tax Configuration
  iss_aliquota_minima: z.number().min(0, 'Alíquota mínima não pode ser negativa').max(25, 'Alíquota mínima não pode exceder 25%'),
  iss_aliquota_maxima: z.number().min(0, 'Alíquota máxima não pode ser negativa').max(25, 'Alíquota máxima não pode exceder 25%'),
...
export const nfeCreateRequestSchema = z.object({
  ...
  payment_method: z.enum(['cash', 'card', 'bank_transfer', 'pix', 'check'], {
    errorMap: () => ({ message: 'Método de pagamento inválido' })
  }).optional(),
  additional_info: z.string().max(1000, 'Informações adicionais muito longas').optional()
});