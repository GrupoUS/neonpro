...
const pixKeySchema = z.string()
  .min(1, 'PIX key cannot be empty')
  .max(77, 'PIX key exceeds maximum length')
  .optional();

// Enum validations
const invoiceStatusSchema = z.enum(['draft', 'issued', 'sent', 'paid', 'cancelled', 'overdue']);
...
const paymentMethodSchema = z.enum(['cash', 'credit_card', 'debit_card', 'pix', 'bank_transfer', 'financing', 'installment']);
const paymentProcessingStatusSchema = z.enum(['pending', 'processing', 'completed', 'failed', 'cancelled', 'refunded']);
...
const shadowOperationTypeSchema = z.enum(['invoice_calculation', 'payment_processing', 'tax_calculation', 'installment_calculation']);
...
const addressSchema = z.object({
  ...
  neighborhood: z.string().min(1, 'Neighborhood is required').max(50, 'Neighborhood too long'),
...
export const createInvoiceSchema = z.object({
  ...
  items: z.array(createInvoiceItemSchema)
    ...
    .max(MAX_INVOICE_ITEMS, `Cannot have more than ${MAX_INVOICE_ITEMS} items`),
...
const createInstallmentSchema = z.object({
  ...
  total_installments: z.number()
    ...
    .max(MAX_INSTALLMENTS, `Cannot have more than ${MAX_INSTALLMENTS} installments`),
  amount: z.number()
...
export const createPaymentSchema = z.object({
  ...
  external_transaction_id: z.string().max(100, 'Transaction ID too long').optional(),
  payment_processor: z.string().max(50, 'Payment processor name too long').optional(),
  authorization_code: z.string().max(100, 'Authorization code too long').optional(),
  pix_key: pixKeySchema,
  installments: z.array(createInstallmentSchema)
    .max(MAX_INSTALLMENTS, `Cannot have more than ${MAX_INSTALLMENTS} installments`)
    .optional(),
  metadata: z.record(z.any()).default({}),
}).refine((data) => {
...
export const invoiceReportFiltersSchema = z.object({
  ...
  amount_max: z.number().min(0, 'Maximum amount cannot be negative').optional(),
...
export const shadowValidationSchema = z.object({
  ...
  tolerance_absolute: z.number().min(0, 'Tolerance cannot be negative').default(0.01),
  tolerance_percentage: z.number().min(0, 'Tolerance cannot be negative').default(0.001),
  metadata: z.record(z.any()).default({}),
});
...
export const paymentReminderSchema = z.object({
  ...
  days_before_due: z.number().int().min(0, 'Days before due cannot be negative').default(0),
  days_after_due: z.number().int().min(0, 'Days after due cannot be negative').default(0),
...
export const invoiceQuerySchema = z.object({
  ...
  sort: z.enum(['created_at', 'issue_date', 'due_date', 'total_amount', 'invoice_number']).default('created_at'),
...
export const paymentQuerySchema = z.object({
  ...
  status: z.string().optional(),
  search: z.string().max(100, 'Search term too long').optional(),
  sort: z.enum(['created_at', 'processed_at', 'amount']).default('created_at'),
  order: z.enum(['asc', 'desc']).default('desc'),
}).merge(paginationSchema);
