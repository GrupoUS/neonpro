...
export async function getTreatmentStatistics(): Promise<TreatmentStatistics> {
  ...
  const procedureCounts = (proceduresStats || []).reduce((acc, proc) => {
  ...
  const avgDuration = completedPlans.length > 0 ? 
  ...
  return {
    ...
    completed_treatment_plans: treatmentPlansStats?.filter(p => p.status === 'completed').length || 0,
    ...
    treatment_outcomes: {
      successful: outcomes?.filter(o => o.outcome?.coding?.[0]?.code === 'successful').length || 0,
      partial: outcomes?.filter(o => o.outcome?.coding?.[0]?.code === 'partial').length || 0,
      unsuccessful: outcomes?.filter(o => o.outcome?.coding?.[0]?.code === 'unsuccessful').length || 0,
    },
  };
}
