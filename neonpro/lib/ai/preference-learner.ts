...
export class PatientPreferenceLearner {
  ...
  async learnFromAppointmentHistory(patientId: string): Promise<PreferenceLearningResult> {
    try {
      ...
      const treatmentPatterns = this.analyzeTreatmentPatterns(appointmentHistory);
      const satisfactionPatterns = this.analyzeSatisfactionPatterns(appointmentHistory);
      ...
      const confidenceImprovement = this.calculateConfidenceImprovement(appointmentHistory);
  ...
  private async getAppointmentHistory(patientId: string): Promise<AppointmentData[]> {
  ...
  private analyzeTimePatterns(appointments: AppointmentData[]): any {
    ...
    const preferredDays = dayOfWeekCounts
      .map((count, index) => ({ day: index, count, satisfaction: avgSatisfactionByDay[index] }))
    ...
    const preferredHours = hourOfDayCounts
      .map((count, index) => ({ hour: index, count, satisfaction: avgSatisfactionByHour[index] }))
      .filter(item => item.count > 0)
      .sort((a, b) => (b.count * b.satisfaction) - (a.count * a.satisfaction))
      .slice(0, 4)
      .map(item => item.hour);
  ...
  private analyzeStaffPatterns(appointments: AppointmentData[]): any {
    ...
    const staffScores = Array.from(staffPerformance.entries()).map(([staffId, data]) => {
      const avgSatisfaction = data.total_satisfaction / data.appointment_count || 0;
      const noShowRate = data.no_show_count / data.appointment_count;
      const avgWaitTime = data.wait_time_count > 0 ? data.total_wait_time / data.wait_time_count : 0;
    ...
    return {
      ...
      confidence: Math.min(1.0, staffScores.reduce((sum, item) => sum + item.appointment_count, 0) / 15)
    };
  }
  ...
  private analyzeTreatmentPatterns(appointments: AppointmentData[]): any {
    ...
    // Analyze patterns
    const treatmentAnalysis = Array.from(treatmentData.entries()).map(([type, data]) => ({
      treatment_type: type,
    ...
    const optimalDurations = treatmentAnalysis.reduce((acc, item) => {
  ...
  private analyzeSatisfactionPatterns(appointments: AppointmentData[]): any {
    ...
    const overallSatisfaction = satisfactionData.reduce((sum, item) => sum + item.satisfaction, 0) / satisfactionData.length;

    // Identify satisfaction patterns
    const patterns = [];

    // Time-based satisfaction patterns
    const morningApts = satisfactionData.filter(item => item.hour_of_day < 12);
    const afternoonApts = satisfactionData.filter(item => item.hour_of_day >= 12);
    
    if (morningApts.length > 0 && afternoonApts.length > 0) {
      const morningAvg = morningApts.reduce((sum, item) => sum + item.satisfaction, 0) / morningApts.length;
      const afternoonAvg = afternoonApts.reduce((sum, item) => sum + item.satisfaction, 0) / afternoonApts.length;
    ...
    const highWaitApts = satisfactionData.filter(item => item.wait_time >= 15);
    
    if (lowWaitApts.length > 0 && highWaitApts.length > 0) {
      const lowWaitAvg = lowWaitApts.reduce((sum, item) => sum + item.satisfaction, 0) / lowWaitApts.length;
      const highWaitAvg = highWaitApts.reduce((sum, item) => sum + item.satisfaction, 0) / highWaitApts.length;
  ...
  private calculateConfidenceImprovement(appointments: AppointmentData[]): number {
    // Base improvement on data quantity and quality
    const dataQuality = appointments.filter(apt => apt.satisfaction_score).length / appointments.length;
    const dataQuantity = Math.min(1.0, appointments.length / 20);
    
    return (dataQuality * 0.6 + dataQuantity * 0.4) * 0.3; // Max 30% improvement
  }

  private detectNewPatterns(appointments: AppointmentData[]): LearningPattern[] {
  ...
  private checkDayConsistency(appointments: AppointmentData[]): { confidence: number } {
    const days = appointments.map(apt => new Date(apt.appointment_time).getDay());
  ...
  private checkTimeConsistency(appointments: AppointmentData[]): { confidence: number } {
    const hours = appointments.map(apt => new Date(apt.appointment_time).getHours());
  ...
  private generateRecommendations(
    ...
    if (timePatterns.confidence > 0.7) {
      if (timePatterns.preferred_hours.length > 0) {
        ...
        recommendations.push(`Schedule appointments between ${hourRanges} for optimal satisfaction`);
      }
