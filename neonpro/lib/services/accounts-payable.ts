...
export class AccountsPayableService {
   ...
   */
  static async getAccountsPayable(filters?: AccountsPayableFilters, page = 1, pageSize = 20): Promise<AccountsPayableResponse> {
    try {
      let query = supabase
      ...
      // Apply filters
      if (filters?.search) {
        query = query.or(`ap_number.ilike.%${filters.search}%,invoice_number.ilike.%${filters.search}%,description.ilike.%${filters.search}%`)
      }
   ...
   */
  static async getAccountsPayableById(id: string): Promise<AccountsPayable | null> {
    try {
   ...
   */
  static async createAccountsPayable(apData: AccountsPayableFormData): Promise<AccountsPayable> {
    try {
      const { data: ap, error } = await supabase
        .from('accounts_payable')
        .insert([{
   ...
   */
  static async updateAccountsPayable(id: string, apData: Partial<AccountsPayableFormData>): Promise<AccountsPayable> {
    try {
      const { data: ap, error } = await supabase
        .from('accounts_payable')
        .update({
   ...
   */
  static async deleteAccountsPayable(id: string, reason?: string): Promise<void> {
    try {
   ...
   */
  static async updateStatus(id: string, status: string, notes?: string): Promise<AccountsPayable> {
    try {
  ...
  static async getDashboardStats() {
    try {
      ...
      const [totalResult, pendingResult, overdueResult, dueTodayResult, totalAmountResult] = await Promise.all([
        supabase
          .from('accounts_payable')
      ...
      const totalOpenAmount = totalAmountResult.data?.reduce((sum, ap) => sum + (ap.balance_amount || 0), 0) || 0
  ...
  static async getFinancialSummary() {
    try {
      ...
      const summary = aps.reduce((acc, ap) => {
        if (!acc[ap.status]) {
        ...
        return acc
      }, {} as Record<string, { count: number; totalAmount: number; balanceAmount: number }>)
