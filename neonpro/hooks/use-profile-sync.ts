...
export function useProfileSync(): UseProfileSyncReturn {
  ...
  const resolveConflict = useCallback(async (resolutionData: Record<string, any>): Promise<boolean> => {
    if (!user?.id) {
      setError('Usuário não autenticado')
      return false
    }
  ...
  // Toggle Google sync
  const toggleGoogleSync = useCallback(async (enabled: boolean): Promise<boolean> => {
    if (!user?.id) {
      setError('Usuário não autenticado')
      return false
    }
