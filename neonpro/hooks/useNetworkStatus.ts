        ...
        try {
          ...
          if (response.ok) {
            ...
            if (item.retryCount >= 3) {
              updatedQueue.splice(i, 1)
              console.warn(`❌ Gave up syncing after 3 attempts: ${item.method} ${item.url}`)
            }
          }
        } catch (error) {
          ...
          if (item.retryCount >= 3) {
            updatedQueue.splice(i, 1)
            console.warn(`❌ Gave up syncing after network errors: ${item.method} ${item.url}`)
          }
        }
      }
  ...
  const makeRequest = useCallback(async (
    ...
    if (isOnline) {
      try {
        ...
        console.warn('API call failed, adding to sync queue:', error)
