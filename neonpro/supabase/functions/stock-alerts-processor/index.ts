...
serve(async (req) => {
  ...
  try {
    ...
    if (generatedAlerts.length > 0) {
      ...
      const alertKeys = generatedAlerts.map(alert => 
      ...
      const newAlerts = generatedAlerts.filter(alert => 
        !existingKeys.has(`${alert.clinic_id}-${alert.product_id}-${alert.alert_type}`)
      )

      console.log(`Inserting ${newAlerts.length} new alerts (${generatedAlerts.length - newAlerts.length} duplicates filtered)`)
  ...
  for (const clinicId of clinicIds) {
    try {
      ...
      const totalValue = inventory.reduce((sum: number, item: any) => 
        sum + (item.quantity_available * item.unit_cost), 0)

      const productsInRange = inventory.filter((item: any) => 
        item.quantity_available >= item.min_stock_level).length
