...
describe('RBAC Authorization Middleware', () => {
  ...
  describe('requirePermission', () => {
    ...
    it('should extract resource ID from URL path', async () => {
      ...
      expect(mockHasPermission).toHaveBeenCalledWith(user, 'patients.read', 'patient-123', undefined);
    });

    it('should extract resource ID from query parameters', async () => {
      const user = createMockUser('staff');
      const request = createMockRequest('http://localhost:3000/api/appointments?appointmentId=apt-456');
      ...
      expect(mockHasPermission).toHaveBeenCalledWith(user, 'appointments.read', 'apt-456', undefined);
    });
  });
  ...
  describe('Pre-configured middleware functions', () => {
    ...
    describe('patientManage', () => {
      it('should allow access for staff with patient management permissions', async () => {
        ...
        expect(response).toBeUndefined();
        expect(mockHasPermission).toHaveBeenCalledWith(user, 'patients.manage', 'patient-123', undefined);
      });
    ...
    describe('appointmentManage', () => {
      it('should allow access for staff with appointment management permissions', async () => {
        ...
        expect(response).toBeUndefined();
        expect(mockHasPermission).toHaveBeenCalledWith(user, 'appointments.manage', 'apt-456', undefined);
      });
  ...
  describe('Resource ID extraction', () => {
    ...
    it('should extract appointment ID from query parameters', async () => {
      const user = createMockUser('staff');
      const request = createMockRequest('http://localhost:3000/api/appointments?id=apt-123&date=2025-01-27');
      ...
      expect(mockHasPermission).toHaveBeenCalledWith(user, 'appointments.read', 'apt-123', undefined);
    });
