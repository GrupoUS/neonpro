...
describe('CNPJ Validation API - Story 5.5 AC2: Real-time CNPJ Validation and Customer Verification', () => {
  ...
  describe('GET /api/tax/cnpj - CNPJ Validation and Lookup', () => {
    it('should validate CNPJ format and check digit', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}`);
    ...
    it('should retrieve company data from Receita Federal', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}&get_company_data=true`);
    ...
    it('should validate company status (active/inactive)', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}&validate_status=true`);
    ...
    it('should check cached validation results', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}&use_cache=true`);
    ...
    it('should retrieve validation history for a CNPJ', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=history&cnpj=${validCNPJ}`);
    ...
    it('should search companies by partial name', async () => {
      const request = new NextRequest('http://localhost/api/tax/cnpj?action=search&query=CLINICA%20TESTE');
    ...
    it('should filter by business activity code', async () => {
      const request = new NextRequest('http://localhost/api/tax/cnpj?action=search&activity_code=8630-5');
    ...
    it('should handle invalid CNPJ gracefully', async () => {
      ...
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${invalidCNPJ}`);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle invalid action parameter', async () => {
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=invalid&cnpj=${validCNPJ}`);
    ...
    it('should handle malformed CNPJ input', async () => {
      ...
      const request = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${malformedCNPJ}`);
    ...
    it('should handle batch validation with too many CNPJs', async () => {
      const largeBatch = Array.from({ length: 101 }, (_, i) => `1234567800019${i.toString().padStart(1, '0')}`);
  ...
  describe('Performance Requirements - AC2', () => {
    ...
    it('should handle high-frequency validation requests efficiently', async () => {
      const concurrentRequests = Array.from({ length: 10 }, () => {
        return cnpjGET(new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}`));
      });
    ...
    it('should cache validation results efficiently', async () => {
      ...
      const firstRequest = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}&get_company_data=true`);
      const firstStartTime = Date.now();
      const firstResponse = await cnpjGET(firstRequest);
      const firstEndTime = Date.now();

      // Second request - should use cache
      const secondRequest = new NextRequest(`http://localhost/api/tax/cnpj?action=validate&cnpj=${validCNPJ}&use_cache=true`);
