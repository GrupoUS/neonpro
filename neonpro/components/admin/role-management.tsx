...
function RoleDefinitionsTab({ roles, canManageRoles, onRolesChange, clinicId }: RoleDefinitionsTabProps) {
  return (
    <div className="space-y-4">
                ...
                <div key={permission} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <div className="flex space-x-2">
                    {roles.map(role => (
                      <Badge
                        key={role.id}
                        variant={role.permissions.includes(permission as Permission) ? 'default' : 'outline'}
                        className="text-xs"
                      >
                        {role.name}
                      </Badge>
