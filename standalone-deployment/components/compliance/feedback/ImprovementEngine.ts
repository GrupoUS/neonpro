...
export class ImprovementEngine {
  ...
  async updateMilestone(
    ...
    if (updates.status === "completed") {
      ...
      const allCompleted = initiative.implementation.milestones.every(m =>
        m.status === "completed"
      );
  ...
  private calculatePatternImpact(pattern: unknown): number {
    ...
    score += pattern.severity === "critical"
      ? 4
  ...
  private identifyRisks(initiatives: ImprovementInitiative[]): unknown[] {
    ...
    const highEffortInitiatives = initiatives.filter(i =>
      i.estimatedEffort.difficulty === "expert"
    );
