...
export let AuthContext: React.Context<AuthContextValue | null> | null = null;
...
export const validateAuthCredentials = (credentials: { email: string; password: string }): boolean => {
	return (
		credentials &&
...
export const validateRegisterData = (data: { email: string; password: string; name: string }): boolean => {
	return validateAuthCredentials(data) && typeof data.name === "string" && data.name.trim().length > 0;
};

// Auth state helpers
export const createInitialAuthState = (): Omit<AuthContextValue, "login" | "logout" | "register"> => ({
	user: null,
	loading: false,
