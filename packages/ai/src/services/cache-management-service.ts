...
export class CacheManagementService extends EnhancedAIService<CacheInput, CacheOutput> {
  ...
  private async getCacheEntry(input: CacheInput): Promise<CacheOutput> {
    ...
    if (this.redis && data.value) {
      try {
        const redisKey = `${this.REDIS_KEY_PREFIX}${fullKey}`;
        const ttlSeconds = Math.max(0, Math.floor(
  ...
  private async getCacheStats(): Promise<CacheOutput> {
    ...
    if (this.redis) {
      try {
        ...
        redisMemory.used = usedMatch ? parseInt(usedMatch[1]) : 0;
        redisMemory.peak = peakMatch ? parseInt(peakMatch[1]) : 0;
      } catch (error) {
  ...
  private async bulkSetCacheEntries(input: CacheInput): Promise<CacheOutput> {
    ...
    if (cacheEntries.length > 0) {
      const { error } = await this.supabase
        .from("ai_service_cache")
        .upsert(cacheEntries, { onConflict: "key" });
