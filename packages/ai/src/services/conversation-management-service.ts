...
export class ConversationManagementService extends EnhancedAIService<ConversationManagementInput, ConversationManagementOutput> {
  ...
  private async createConversation(input: ConversationManagementInput): Promise<ConversationSession> {
    const sessionData = {
      ...
      metadata: {
        ...
        emergency_context: input.context!.emergencyContext || false
      }
    };
  ...
  private async generateRecommendations(sessionId: string, analytics?: ConversationAnalytics): Promise<ConversationRecommendation[]> {
    if (!analytics) {
  ...
  private async analyzeTopics(messages: ConversationMessage[]): Promise<TopicDistribution[]> {
    ...
    return Array.from(topicCounts.entries()).map(([topic, frequency]) => ({
      topic,
      frequency,
  ...
  private async calculateEngagementMetrics(conversation: ConversationSession): Promise<EngagementMetrics> {
    ...
    const escalationTriggered = conversation.messages.some(m => 
  ...
  private async calculatePerformanceMetrics(conversation: ConversationSession): Promise<PerformanceMetrics> {
    ...
    const errorMessages = conversation.messages.filter(m => 
      m.metadata?.error || m.complianceFlags.includes('error')
    );
  ...
  private assessGoalAchievement(conversation: ConversationSession): number {
    ...
    const hasResolution = conversation.messages.some(m => 
  ...
  private async generateOverview(conversation: ConversationSession): Promise<string> {
    ...
    return `Conversa de ${interface_type} com ${messageCount} mensagens durante ${Math.round(duration / 1000 / 60)} minutos. ` +
  ...
  private async generateNextSteps(conversation: ConversationSession, analytics: ConversationAnalytics): Promise<string[]> {
    const steps: string[] = [];
  ...
  private async generateAIRecommendations(conversation: ConversationSession, analytics: ConversationAnalytics): Promise<string[]> {
  ...
  private async assessSafety(conversation: ConversationSession, analytics: ConversationAnalytics): Promise<SafetyAssessment> {
    const emergencyDetected = conversation.messages.some(m => 
      m.complianceFlags.includes('emergency_detected')
    );
    
    const riskLevel = emergencyDetected ? 'high' : 
                     analytics.engagementMetrics.escalationTriggered ? 'medium' : 'low';
