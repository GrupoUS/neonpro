...
describe('EnhancedAIService', () => {
  ...
  describe('Basic functionality', () => {
    ...
    it('should handle errors gracefully', async () => {
      await expect(service.executeWithMetrics({ input: 'error' }))
  ...
  describe('Logging functionality', () => {
    it('should log service execution start and end', async () => {
      ...
      const hasExecutionLog = infoLogs.some(log => 
        log.message.includes('Service execution') || log.message.includes('completed')
      )
  ...
  describe('Rate limiting', () => {
    it('should enforce rate limits when configured', async () => {
      ...
      await expect(rateLimitedService.executeWithMetrics({ input: 'test3' }))
