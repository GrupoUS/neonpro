...
export class AestheticFeatureExtractor {
  ...
  async extractTreatmentFeatures(
    ...
    features.push(
      ...
      relevantHistory.length > 0 ? 
        relevantHistory.reduce((sum, t) => sum + t.satisfaction, 0) / (relevantHistory.length * 10) : 0,
      relevantHistory.length > 0 ? 
        relevantHistory.reduce((sum, t) => sum + t.outcome.effectivenesss, 0) / (relevantHistory.length * 10) : 0,
  ...
  private encodeSunExposure(exposure: string): number {
    const mapping = { 'minimal': 0.25, 'moderate': 0.5, 'high': 1.0 };
  ...
  private encodeExpectationLevel(level: string): number {
    const mapping = { 'subtle': 0.33, 'moderate': 0.67, 'dramatic': 1.0 };
    return mapping[level as keyof typeof mapping] || 0.67;
  }

  private encodeUrgency(urgency: string): number {
    const mapping = { 'low': 0.25, 'moderate': 0.5, 'high': 1.0 };
  ...
  private estimateMuscleActivity(patient: PatientProfile, areas: string[]): number {
  ...
  private estimateSkinElasticity(patient: PatientProfile): number {
    let elasticity = 1.0; // Start with perfect elasticity
  ...
  private estimateAsymmetryRisk(patient: PatientProfile, areas: string[]): number {
    ...
    const hasAsymmetryHistory = patient.previousTreatments.some(
      t => t.complications.includes('asymmetry')
    );
  ...
  private estimateExpressionStrength(patient: PatientProfile, areas: string[]): number {
    let strength = 0.5;
  ...
  private estimateRecoveryCapacity(patient: PatientProfile): number {
    ...
    if (patient.medicalHistory.medications.some(m => m.affectsHealing)) capacity -= 0.1;
  ...
  private estimateVolumeDeficit(patient: PatientProfile, areas: string[]): number {
    let deficit = 0;
  ...
  private evaluateProductCompatibility(patient: PatientProfile, productType: string): number {
    let compatibility = 0.9; // High base compatibility for HA
    
    // Allergy history
    const hasRelevantAllergies = patient.medicalHistory.allergies.some(
      allergy => allergy.toLowerCase().includes('hyaluronic') || 
  ...
  private estimateSwellingProneness(patient: PatientProfile): number {
    ...
    // Medical conditions
    if (patient.medicalHistory.conditions.some(c => 
  ...
  private hasPreviousAreaTreatment(patient: PatientProfile, areas: string[], treatmentType: string): boolean {
    return patient.previousTreatments.some(t => 
  ...
  private evaluateConcurrentTreatmentRisk(patient: PatientProfile, treatmentType: string): number {
  ...
  private evaluateSeasonalAppropriate(patient: PatientProfile, laserType: string): number {
    const month = new Date().getMonth();
  ...
  private evaluateContraindicationRisk(patient: PatientProfile, laserType: string): number {
    let risk = 0;
    ...
    const photosensitizingMeds = patient.medicalHistory.medications.filter(
      m => m.name.toLowerCase().includes('tretinoin') || 
  ...
  private estimateComplicationRisk(patient: PatientProfile, laserType: string): number {
    let risk = 0.1;
  ...
  private evaluatePigmentationRisk(patient: PatientProfile): number {
    ...
    const hasPigmentationHistory = patient.previousTreatments.some(
      t => t.complications.includes('pigmentation') || 
  ...
  private evaluateDowntimeTolerance(patient: PatientProfile): number {
    ...
    const hasMinimalDowntimeTreatments = patient.previousTreatments.some(
      t => ['botox', 'dermal-fillers', 'microneedling'].includes(t.type)
    );
  ...
  getTreatmentFeatureNames(): string[] {
    return [
      'age_normalized', 'gender_encoded', 'skin_type', 'blood_thinners', 'keloid_proneness', 'autoimmune_diseases',
      'smoking_status', 'smoking_frequency', 'sun_exposure', 'stress_level', 'sleep_quality', 'sunscreen_use', 'retinoid_use',
      'relevant_treatment_history', 'avg_satisfaction', 'avg_effectiveness', 'complication_history', 'total_experience',
      'treatment_type', 'target_area_count', 'avg_severity', 'expectation_level', 'urgency', 'natural_look_preference'
    ];
  }

  getBotoxFeatureNames(): string[] {
    return [
      'age_normalized', 'gender_encoded', 'skin_type', 'muscle_conditions', 'stress_level', 'healing_medications',
      'botox_experience', 'muscle_activity', 'skin_elasticity', 'asymmetry_risk', 'expression_strength', 'recovery_capacity',
      'area_count', 'desired_intensity', 'area_complexity', 'treatment_complexity', 'previous_area_treatment', 'concurrent_risk',
      'seasonal_factor', 'current_trends'
    ];
  }

  getFillerFeatureNames(): string[] {
    return [
      'age_normalized', 'gender_encoded', 'skin_type', 'volume_deficit', 'skin_thickness', 'collagen_production',
      'allergy_count', 'autoimmune_status', 'smoking_status', 'healing_capacity', 'filler_experience', 'facial_asymmetry',
      'skin_laxity', 'product_compatibility', 'total_volume', 'area_count', 'volume_complexity', 'area_encoding',
      'injection_difficulty', 'maintenance_requirement', 'vascular_risk', 'swelling_proneness'
    ];
  }

  getLaserFeatureNames(): string[] {
    return [
      'age_normalized', 'gender_encoded', 'skin_type', 'melanin_content', 'skin_sensitivity', 'skin_thickness',
      'photodamage_level', 'sun_exposure_current', 'retinoid_use', 'photosensitizing_meds', 'laser_experience',
      'healing_response', 'pigmentation_risk', 'scar_risk', 'downtime_tolerance', 'inflammatory_response',
      'laser_type', 'treatment_goal', 'required_energy', 'treatment_depth', 'sessions_required', 'seasonal_appropriate',
      'contraindication_risk', 'complication_risk', 'post_care_compliance', 'geographic_factor'
    ];
  }
}
