...
export class AestheticPostProcessor {
  ...
  private calculateTreatmentDurability(
    ...
    if (patient.age < 30) durability *= 0.8; // Faster metabolism
  ...
  private calculateLayerDistribution(area: string, totalVolume: number) {
    ...
    const layerStrategies: Record<string, any[]> = {
      'lips': [
        { depth: 'subcutaneous', volume: totalVolume * 0.7, technique: 'linear_threading' },
        { depth: 'supraperiosteal', volume: totalVolume * 0.3, technique: 'micro_bolus' }
      ],
      'cheeks': [
        { depth: 'supraperiosteal', volume: totalVolume * 0.6, technique: 'bolus_injection' },
        { depth: 'subcutaneous', volume: totalVolume * 0.4, technique: 'fanning' }
      ],
      'nasolabial-folds': [
        { depth: 'subcutaneous', volume: totalVolume * 0.8, technique: 'linear_threading' },
        { depth: 'supraperiosteal', volume: totalVolume * 0.2, technique: 'micro_bolus' }
      ]
    };
    
    return layerStrategies[area] || [
      { depth: 'subcutaneous', volume: totalVolume, technique: 'linear_threading' }
    ];
  }
