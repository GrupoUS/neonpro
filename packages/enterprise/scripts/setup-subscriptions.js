...
async function setupDevelopmentScripts() {
  ...
  if (missingScripts.length > 0) {} else {}
  ...
  if (!fs.existsSync(devConfigPath)) {
    ...
    fs.writeFileSync(devConfigPath, JSON.stringify(devConfig, undefined, 2));
  }
}
...
async function generateSetupReport() {
  ...
  fs.writeFileSync('.setup-report.json', JSON.stringify(report, undefined, 2));
}
