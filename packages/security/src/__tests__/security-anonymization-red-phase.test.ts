  ...
  return {
    randomBytes: vi.fn((size: number) => {
      ...
      if (size === 32 && isRotationTest) {
        ...
        rotationCounter++
        return Buffer.from(
          `rotated-key-${rotationCounter}-`
            + 'a'.repeat(32 - `rotated-key-${rotationCounter}-`.length),
        )
      }
      ...
      if (size === 32) {
        keyCounter++
        return Buffer.from(
          `test-key-${keyCounter}-` + 'a'.repeat(32 - `test-key-${keyCounter}-`.length),
        )
      }
    ...
    createDecipheriv: vi.fn(() => {
      return {
        update: vi.fn((data: Buffer) => {
          ...
          const callStack = new Error().stack || ''
          const isLargeDatasetTest = callStack.includes('250KB')
            || callStack.includes('repeat(10000)')
...
describe('Security & Anonymization - RED Phase Tests', () => {
  ...
  describe('Security Performance - Large Datasets', () => {
    it('should handle large-scale anonymization', () => {
      ...
      const anonymizedDataset = largeDataset.map((patient) => maskPatientData(patient, 'basic'))
