...
export function maskName(name: string, options: MaskingOptions = {}): string {
  ...
  return parts.map(part => {
    ...
    return part.substring(0, visibleStart)
...
export function maskAddress(
  ...
  return {
    ...
    number: address.number ? maskChar.repeat(Math.max(2, address.number.length)) : address.number,
    complement: address.complement
      ? maskChar.repeat(Math.max(5, address.complement.length))
      : address.complement,
    neighborhood: address.neighborhood ? `${maskChar.repeat(8)}` : address.neighborhood,
        ...
        switch (field) {
          ...
          default:
            ...
            result[field] = value.length > 3
              ? value.substring(0, 1) + maskChar.repeat(Math.max(3, value.length - 1))
              : maskChar.repeat(value.length);
        }
      } else if (typeof result[field] === 'object' && field === 'address') {
        result[field] = maskAddress(result[field] as PatientData['address'], options);
      } else {
...
export function isDataAnonymized(data: Record<string, unknown>): boolean {
  ...
  for (const field of identifiableFields) {
    ...
    if (value && typeof value === 'string') {
      ...
      if (!value.includes('*') && !value.includes('ANONIMIZADO') && value.length > 3) {
        return false;
      }
    }
  }
...
export function generatePrivacyReport(
  ...
  anonymizedResult: { data: Record<string, unknown>; metadata: AnonymizationMetadata },
): {
  ...
  for (const field of sensitiveFields) {
    ...
    if (original && anonymized && typeof original === 'string' && typeof anonymized === 'string') {
      // Check if too much information is still visible
      if (
        anonymized.length > 10 && !anonymized.includes('*') && !anonymized.includes('ANONIMIZADO')
      ) {
      ...
      if (anonymized.includes('*') && anonymized.replace(/\*/g, '').length > 3) {
        recommendations.push(`Considere aumentar o nível de mascaramento para o campo ${field}`);
        complianceScore -= 5;
      }
    }
  }
  ...
  if (anonymizedResult.metadata.complianceLevel === 'basic' && risks.length > 0) {
    recommendations.push('Considere usar nível de conformidade "enhanced" ou "full_anonymization"');
  }
 ...
 */
export const DEFAULT_MASKING_OPTIONS: Record<LGPDComplianceLevel, MaskingOptions> = {
  basic: {
    maskChar: '*',
    visibleStart: 1,
    visibleEnd: 0,
