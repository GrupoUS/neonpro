...
export function maskCPF(cpf: string, options: MaskingOptions = {}): string {
  ...
  if (preserveFormat) {
    // Format: ***.***.***-**
    return `${maskChar.repeat(3)}.${maskChar.repeat(3)}.${maskChar.repeat(3)}-${maskChar.repeat(
      2,
    )}`;
  } else {
    // Format: ***********
...
export function maskCNPJ(cnpj: string, options: MaskingOptions = {}): string {
  ...
  if (preserveFormat) {
    // Format: **.***.***/****-**
    return `${maskChar.repeat(2)}.${maskChar.repeat(3)}.${maskChar.repeat(3)}/${maskChar.repeat(
      4,
    )}-${maskChar.repeat(2)}`;
  } else {
    // Format: **************
...
export function maskPhone(phone: string, options: MaskingOptions = {}): string {
  ...
  if (cleaned.length === 11) {
    ...
    return preserveFormat
      ? `(${cleaned.substring(0, 2)}) ${cleaned.charAt(2)}${maskChar.repeat(4)}-${maskChar.repeat(
          4,
        )}`
...
export function anonymizePersonalData(
  ...
  const fields =
    fieldsToAnonymize.length > 0 ? fieldsToAnonymize : defaultFieldsToAnonymize;
