...
export const healthcareAlerts: AlertRule[] = [
  {
    ...
    channels: [AlertChannel.SLACK_HEALTHCARE, AlertChannel.PAGERDUTY, AlertChannel.EMAIL_COMPLIANCE],
    escalation: '5m',
    cooldownPeriod: '10m',
  ...
  
  {
    ...
    severity: AlertSeverity.CRITICAL,
    channels: [AlertChannel.EMAIL_COMPLIANCE, AlertChannel.EMAIL_SECURITY, AlertChannel.PAGERDUTY],
  ...
  
  {
    ...
    severity: AlertSeverity.CRITICAL,
    channels: [AlertChannel.PAGERDUTY, AlertChannel.SMS_ONCALL, AlertChannel.SLACK_HEALTHCARE],
    escalation: 'immediate',
    cooldownPeriod: '1m',
    autoResolve: false,
    healthcareSpecific: true
...
export class HealthcareAlertManager {
   ...
   */
  async processAlert(alertName: string, condition: any, metadata?: Record<string, any>): Promise<void> {
   ...
   */
  private async sendAlert(rule: AlertRule, condition: any, metadata?: Record<string, any>): Promise<void> {
