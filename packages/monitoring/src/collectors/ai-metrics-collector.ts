...
export class AIMetricsCollector implements MetricCollector {
  ...
  private readonly aiServices: unknown[] = []; // Will be injected
  private costTracker: unknown; // Cost tracking service

  constructor(aiServices: unknown[] = [], costTracker?: unknown) {
  ...
  addAIService(service: unknown): void {
    this.aiServices.push(service);
  }

  setCostTracker(costTracker: unknown): void {
    this.costTracker = costTracker;
  }
}
