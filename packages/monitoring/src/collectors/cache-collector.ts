...
export class CacheMetricsCollector implements MetricCollector {
  ...
  private cacheManager: unknown; // Will be injected

  constructor(cacheManager?: unknown) {
  ...
  async collect(): Promise<PerformanceMetric[]> {
    ...
    try {
      ...
      for (const [layer, stats] of Object.entries(allStats)) {
        const layerStats = stats as unknown;
  ...
  private calculateOverallHitRate(allStats: Record<string, unknown>): number {
    let totalHits = 0;
    let totalRequests = 0;
  ...
  setCacheManager(cacheManager: unknown): void {
    this.cacheManager = cacheManager;
  }
}
