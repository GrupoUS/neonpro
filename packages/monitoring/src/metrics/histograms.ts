...
export function observeChatResponseTime(duration: number, labels: MetricLabels = {}): void {
  chatResponseTime?.observe(labels, duration);
}

export function observeAIProviderLatency(duration: number, labels: MetricLabels = {}): void {
  aiProviderLatency?.observe(labels, duration);
}

export function observeDatabaseQueryDuration(duration: number, labels: MetricLabels = {}): void {
  databaseQueryDuration?.observe(labels, duration);
}

export function observePIIRedactionDuration(duration: number, labels: MetricLabels = {}): void {
