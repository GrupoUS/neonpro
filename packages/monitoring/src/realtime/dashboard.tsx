...
export const RealTimePerformanceDashboard: React.FC<RealTimePerformanceDashboardProps> = ({
   ...
   */
  const loadMetricsByCategory = async (category: PerformanceMetric['category']): Promise<PerformanceMetric[]> => {
    let query = supabase
    ...
    setDashboardData(prev => {
      ...
      switch (newMetric.category) {
        ...
        case 'cache-metrics':
          ...
          break;
        case 'system-metrics':
          updated.systemMetrics.current = [newMetric, ...updated.systemMetrics.current.slice(0, 99)];
          updated.systemMetrics.resourceUsage = aggregateResourceUsage(updated.systemMetrics.current);
...
const calculateSystemHealth = (metrics: PerformanceMetric[]): 'healthy' | 'warning' | 'critical' => {
...
const calculateCategoryStatus = (metrics: PerformanceMetric[]): 'healthy' | 'warning' | 'critical' => {
