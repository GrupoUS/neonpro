...
export interface HealthcareAccessEvent extends Omit<TelemetryEvent, 'eventType'> {
  eventType: TelemetryEventType.PATIENT_ACCESS | 
...
export interface ComplianceEvent extends Omit<TelemetryEvent, 'eventType'> {
  eventType: TelemetryEventType.LGPD_CONSENT_UPDATE |
...
export class TelemetryEventValidator {
  ...
  static sanitizeForLGPD(event: Partial<TelemetryEvent>): Partial<TelemetryEvent> {
    ...
    if (sanitized.healthcareContext?.patientIdHash) {
      sanitized.healthcareContext.patientIdHash = this.hashPII(sanitized.healthcareContext.patientIdHash);
    }
...
export class TelemetryEventBuilder {
  ...
  withUser(userId: string, role?: 'patient' | 'professional' | 'admin' | 'system'): this {
    this.event.userId = TelemetryEventValidator.hashPII(userId);
    if (role) this.event.userRole = role;
    return this;
  }
  
  withHealthcareContext(context: Partial<TelemetryEvent['healthcareContext']>): this {
    this.event.healthcareContext = { 
      ...this.event.healthcareContext, 
      ...context,
      // Ensure required fields have defaults
      dataSensitivity: context?.dataSensitivity ?? this.event.healthcareContext?.dataSensitivity ?? HealthcareDataSensitivity.NONE,
      complianceFlags: context?.complianceFlags ?? this.event.healthcareContext?.complianceFlags ?? []
    };
    
    // Safely access compliance property
    if (this.event.compliance) {
      this.event.compliance.piiPresent = this.event.healthcareContext.dataSensitivity !== HealthcareDataSensitivity.NONE;
    }
    return this;
  }
  ...
  build(): TelemetryEvent {
    ...
    if (!event.id || !event.eventType || !event.timestamp || !event.severity || !event.service) {
