...
export function validateAIInsight(insight: Partial<AIInsight>): boolean {
  ...
  if (insight.confidence !== undefined && !validateConfidenceScore(insight.confidence)) {
    return false;
  }
...
export function anonymizeAIInsight(insight: Partial<AIInsight>): Partial<AIInsight> {
...
export function createAIInsight(
  data: Omit<AIInsight, 'id' | 'createdAt' | 'updatedAt' | 'generatedAt' | 'status'>
): AIInsight {
  const now = new Date();
...
export function getInsightsRequiringValidation(insights: AIInsight[]): AIInsight[] {
