...
describe('Appointment Model (T033)', () => {
  ...
  it('should validate appointment times', () => {
    ...
    expect(validateAppointmentTimes(validTimes.startTime, validTimes.endTime)).toBe(true);
    expect(validateAppointmentTimes(invalidTimes.startTime, invalidTimes.endTime)).toBe(false);
  });
  ...
  it('should check for appointment conflicts', () => {
    ...
    expect(checkAppointmentConflict(existingAppointment, conflictingAppointment)).toBe(true);
    expect(checkAppointmentConflict(existingAppointment, nonConflictingAppointment)).toBe(false);
  });
  ...
  it('should handle appointment cancellation', () => {
    ...
    const cancelled = cancelAppointment(appointment, 'Paciente solicitou cancelamento');
