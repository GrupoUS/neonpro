...
export function validateAppointmentTimes(startTime: Date, endTime: Date): boolean {
...
export function checkAppointmentConflict(
  ...
  return (
    (proposed.startTime >= existing.startTime && proposed.startTime < existing.endTime) ||
    (proposed.endTime > existing.startTime && proposed.endTime <= existing.endTime) ||
    (proposed.startTime <= existing.startTime && proposed.endTime >= existing.endTime)
  );
}

// Calculate appointment duration in minutes
export function calculateAppointmentDuration(startTime: Date, endTime: Date): number {
...
export function formatAppointmentForDisplay(appointment: Partial<Appointment> & { patientName?: string }): string {
  const parts: string[] = [];
...
export function markAsNoShow(appointment: Partial<Appointment>): Partial<Appointment> {
  return {
    ...appointment,
...
export function anonymizeAppointment(appointment: Partial<Appointment>): Partial<Appointment> {
...
export function createAppointment(
  ...
  return {
    ...
    createdAt: now,
    updatedAt: now,
    duration: data.duration || calculateAppointmentDuration(data.startTime, data.endTime),
  };
}
...
export function needsReminder(appointment: Appointment): boolean {
  ...
  return appointment.reminderSettings.timeBefore.some(hours => 
    Math.abs(hoursUntilAppointment - hours) < 0.5 // Within 30 minutes of reminder time
  );
}
