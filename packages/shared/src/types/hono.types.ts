...
export interface RouteConfig<TInput = unknown, TOutput = unknown> {
  ...
  middleware?: ((c: NeonProContext, next: () => Promise<void>) => Promise<void>)[];
  validation?: {
    body?: any; // Zod schema
...
export type ExtractRouteInput<T> = T extends RouteConfig<infer I, any> ? I
  : never;
export type ExtractRouteOutput<T> = T extends RouteConfig<any, infer O> ? O
  : never;
...
export type InferResponseType<T> = T extends { $get: () => Promise<infer R>; } ? R
  : T extends { $post: (data: any) => Promise<infer R>; } ? R
  : never;
