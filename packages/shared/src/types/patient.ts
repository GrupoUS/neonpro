...
export function anonymizePatientData(patient: Partial<Patient>): Partial<Patient> {
...
export function createPatientWithDefaults(
  patientData: Omit<Patient, 'id' | 'createdAt' | 'updatedAt' | 'lgpdConsent' | 'auditTrail' | 'status'>
): Patient {
  const now = new Date();
