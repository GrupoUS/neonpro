...
export function readBoolEnv(value: string | undefined, fallback = false): boolean {
  if (value == null) return fallback;
  const v = value.toLowerCase();
  return v === '1' || v === 'true' || v === 'yes' || v === 'on';
}

export function readNumEnv(value: string | number | undefined, fallback: number): number {
  if (value == null) return fallback;
