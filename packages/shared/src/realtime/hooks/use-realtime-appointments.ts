    ...
    (payload: any) => {
      try {
        ...
        console.log(`[RealtimeAppointments] ${payload.eventType} event:`, {
          ...
          patientId:
            realtimePayload.new?.patient_id || realtimePayload.old?.patient_id,
          isUrgent,
          tenantId,
        ...
        console.error(
          '[RealtimeAppointments] Payload processing error:',
          error
        );
        if (onError) {
    ...
    (payload: RealtimeAppointmentPayload): boolean => {
      ...
      if (eventType === 'UPDATE' && newData && oldData) {
        ...
        if (newData.status === 'cancelled' && oldData.status !== 'cancelled')
          return true;
        if (newData.status === 'emergency' && oldData.status !== 'emergency')
          return true;
        ...
        if (
          hoursDifference <= 24 &&
          newData.scheduled_at !== oldData.scheduled_at
        )
          return true;
  ...
  const updateAppointmentCache = useCallback(
    (payload: RealtimeAppointmentPayload) => {
      ...
      queryClient.setQueryData(
        queryKey,
        (oldCache: Appointment[] | undefined) => {
