...
export function useRealtimeNotifications(
  ...
  const handleNotificationChange = useCallback((payload: any) => {
    try {
      const realtimePayload: RealtimeNotificationPayload = {
        ...
        new: payload.new as Notification & { priority?: keyof NotificationPriority },
   ...
   */
  const playNotificationSound = useCallback((priority: keyof NotificationPriority) => {
   ...
   * Show toast notification
   */
  const showToastNotification = useCallback((notification: Notification & { priority?: keyof NotificationPriority }) => {
   ...
   */
  const updateNotificationCache = useCallback((payload: RealtimeNotificationPayload) => {
    const { eventType, new: newData, old: oldData } = payload;

    // Update notifications list cache
    queryClient.setQueryData(['notifications', tenantId, userId], (oldCache: Notification[] | undefined) => {
        ...
        case 'UPDATE':
          if (newData) {
            return oldCache.map(notification => 
              notification.id === newData.id ? newData as Notification : notification
            );
          }
          return oldCache;
  ...
  const markAsRead = useCallback(async (notificationId: string) => {
    try {
      ...
      // Update individual notification cache
      queryClient.setQueryData(['notification', notificationId], (oldData: Notification | undefined) => {
