...
type DataProcessingLog = Database['public']['Tables']['data_processing_logs']['Row'];
...
export function useRealtimeCompliance(
   ...
   */
  const handleComplianceChange = useCallback((payload: any) => {
    try {
  ...
  const determineComplianceType = useCallback((payload: any): keyof ComplianceEventType => {
    ...
    if (eventData.event_type?.includes('consent')) {
      return eventData.action === 'granted' ? 'LGPD_CONSENT_GRANTED' : 'LGPD_CONSENT_REVOKED';
    }
   ...
   */
  const updateComplianceCache = useCallback((payload: RealtimeCompliancePayload) => {
   ...
   */
  const generateAuditEntry = useCallback((payload: RealtimeCompliancePayload) => {
    const auditEntry = {
  ...
  const generateComplianceReport = useCallback(async (): Promise<any> => {
    try {
      ...
      const report = {
        ...
        eventBreakdown: {
          lgpdEvents: logs.filter(log => log.event_type?.includes('lgpd')).length,
          anvisaEvents: logs.filter(log => log.event_type?.includes('anvisa')).length,
          breachEvents: logs.filter(log => log.event_type?.includes('breach')).length,
          unauthorizedAccess: logs.filter(log => log.event_type?.includes('unauthorized')).length
        },
        severityBreakdown: {
  ...
  const generateRecommendations = useCallback((logs: ComplianceLog[], score: number): string[] => {
    ...
    const breachCount = logs.filter(log => log.event_type?.includes('breach')).length;
    if (breachCount > 0) {
      recommendations.push('Vazamentos de dados detectados. Notificar ANPD conforme LGPD Art. 48.');
    }

    const unauthorizedCount = logs.filter(log => log.event_type?.includes('unauthorized')).length;
...
export function useComplianceAnalytics(tenantId: string) {
  ...
  const getComplianceMetrics = useCallback(() => {
    ...
    const recentLogs = logs.filter(log => new Date(log.created_at) >= lastWeek);
    const monthlyLogs = logs.filter(log => new Date(log.created_at) >= lastMonth);

    return {
      weekly: {
        totalEvents: recentLogs.length,
        criticalEvents: recentLogs.filter(log => log.severity === 'CRITICAL').length,
        lgpdEvents: recentLogs.filter(log => log.event_type?.includes('lgpd')).length,
        anvisaEvents: recentLogs.filter(log => log.event_type?.includes('anvisa')).length
      },
      monthly: {
        totalEvents: monthlyLogs.length,
        criticalEvents: monthlyLogs.filter(log => log.severity === 'CRITICAL').length,
        trend: calculateTrend(monthlyLogs),
        avgDailyCritical: monthlyLogs.filter(log => log.severity === 'CRITICAL').length / 30
      },
      compliance: {
  ...
  const calculateTrend = useCallback((logs: ComplianceLog[]): 'improving' | 'stable' | 'declining' => {
    ...
    const firstCritical = firstHalf.filter(log => log.severity === 'CRITICAL').length;
    const secondCritical = secondHalf.filter(log => log.severity === 'CRITICAL').length;
  ...
  const calculateLGPDCompliance = useCallback((logs: ComplianceLog[]): number => {
    ...
    const violations = lgpdLogs.filter(log => log.severity === 'HIGH' || log.severity === 'CRITICAL').length;
    return Math.max(0, 100 - (violations / lgpdLogs.length) * 50);
  }, []);

  const calculateANVISACompliance = useCallback((logs: ComplianceLog[]): number => {
    const anvisaLogs = logs.filter(log => log.event_type?.includes('anvisa'));
    if (anvisaLogs.length === 0) return 100;
    
    const violations = anvisaLogs.filter(log => log.severity === 'HIGH' || log.severity === 'CRITICAL').length;
  ...
  const calculateOverallScore = useCallback((logs: ComplianceLog[]): number => {
    ...
    const criticalPenalty = logs.filter(log => log.severity === 'CRITICAL').length * 15;
    const highPenalty = logs.filter(log => log.severity === 'HIGH').length * 8;
    const mediumPenalty = logs.filter(log => log.severity === 'MEDIUM').length * 3;
