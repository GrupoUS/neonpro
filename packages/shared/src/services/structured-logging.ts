...
export const HealthcareContextSchema = z.object({
  ...
  workflowType: z.enum([
    ...
    'administrative_task',
    'system_maintenance'
  ]).optional().describe('Type of healthcare workflow'),
  
  workflowStage: z.string().optional().describe('Current stage within the workflow'),
  
  // Patient context (anonymized for LGPD compliance)
  patientContext: z.object({
    anonymizedPatientId: z.string().optional().describe('LGPD-compliant patient identifier'),
    ageGroup: z.enum(['pediatric', 'adult', 'geriatric']).optional().describe('Patient age category'),
    criticalityLevel: z.enum(['routine', 'urgent', 'critical', 'emergency']).optional().describe('Case criticality'),
    hasAllergies: z.boolean().optional().describe('Patient has known allergies'),
    isEmergencyCase: z.boolean().optional().describe('This is an emergency case')
  }).optional().describe('Anonymized patient context'),
  
  // Professional context
  professionalContext: z.object({
    anonymizedProfessionalId: z.string().optional().describe('LGPD-compliant professional identifier'),
  ...
  clinicalContext: z.object({
    facilityId: z.string().optional().describe('Healthcare facility identifier'),
    ...
    complianceFramework: z.array(z.string()).optional().describe('Applicable compliance frameworks')
  }).optional().describe('Clinical environment context')
});
...
export const TechnicalContextSchema = z.object({
  ...
  userContext: z.object({
    anonymizedUserId: z.string().optional().describe('LGPD-compliant user identifier'),
    userAgent: z.string().optional().describe('Browser user agent'),
    ipAddress: z.string().optional().describe('Client IP address (anonymized)'),
    deviceType: z.enum(['mobile', 'tablet', 'desktop']).optional().describe('Device type')
  }).optional().describe('Anonymized user context')
});
...
export const LGPDComplianceSchema = z.object({
  dataClassification: z.enum(['public', 'internal', 'confidential', 'restricted']).describe('Data sensitivity level'),
  containsPII: z.boolean().describe('Whether log contains personally identifiable information'),
  legalBasis: z.enum([
    'consent',
    'contract', 
    'legal_obligation',
  ...
  auditRequired: z.boolean().describe('Whether this log entry requires audit trail')
});
...
export const LogEntrySchema = z.object({
  ...
  healthcareContext: HealthcareContextSchema.optional().describe('Healthcare workflow context'),
  ...
  }).optional().describe('Error information if applicable'),
  ...
  }).optional().describe('Log processing metadata')
});
...
export const StructuredLoggingConfigSchema = z.object({
  ...
  outputs: z.object({
    ...
    remote: z.boolean().default(true).describe('Enable remote logging endpoint'),
    observability: z.boolean().default(true).describe('Send to observability platform')
  }).describe('Log output targets'),
  
  // Performance settings
  performance: z.object({
    batchSize: z.number().default(50).describe('Maximum logs per batch'),
    flushInterval: z.number().default(5000).describe('Batch flush interval in milliseconds'),
    maxBufferSize: z.number().default(1000).describe('Maximum logs to buffer'),
    enableAsync: z.boolean().default(true).describe('Enable asynchronous processing')
  }).describe('Performance optimization settings'),
  
  // Healthcare-specific settings
  healthcareCompliance: z.object({
    enablePIIRedaction: z.boolean().default(true).describe('Enable automatic PII redaction'),
    enableAuditLogging: z.boolean().default(true).describe('Enable audit trail logging'),
    criticalEventAlerts: z.boolean().default(true).describe('Enable alerts for critical events'),
    patientSafetyLogging: z.boolean().default(true).describe('Enable patient safety event logging')
  }).describe('Healthcare compliance settings'),
  
  // LGPD settings
  lgpdCompliance: z.object({
    dataRetentionDays: z.number().default(365).describe('Default data retention period'),
    requireExplicitConsent: z.boolean().default(false).describe('Require explicit consent for detailed logging'),
    anonymizeByDefault: z.boolean().default(true).describe('Anonymize user data by default'),
    enableDataMinimization: z.boolean().default(true).describe('Minimize data collection to necessary only')
  }).describe('LGPD compliance settings'),
  
  // Integration settings
  integration: z.object({
    endpoint: z.string().default('/api/v1/observability/logs').describe('Remote logging endpoint'),
    authToken: z.string().optional().describe('Authentication token for remote logging'),
    enableDistributedTracing: z.boolean().default(true).describe('Enable distributed tracing integration'),
    enableMetricsCorrelation: z.boolean().default(true).describe('Enable correlation with metrics')
  }).describe('Integration configuration')
});

export type StructuredLoggingConfig = z.infer<typeof StructuredLoggingConfigSchema>;
...
export class StructuredLogger {
   ...
   * Log debug message
   */
  debug(message: string, data?: Record<string, unknown>, context?: {
   ...
   * Log info message
   */
  info(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   * Log notice message
   */
  notice(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   * Log warning message
   */
  warn(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   * Log error message
   */
  error(message: string, error?: Error, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   */
  critical(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   */
  alert(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
   ...
   */
  emergency(message: string, data?: Record<string, unknown>, context?: {
    healthcare?: HealthcareContext;
    technical?: Partial<TechnicalContext>;
  }): void {
  ...
  logPatientSafetyEvent(
    ...
    const level = severity === 'critical' ? 'alert' : 
                 severity === 'high' ? 'critical' :
                 severity === 'medium' ? 'error' : 'warn';
    
    this.log(level, `[PATIENT SAFETY] ${message}`, {
      ...data,
  ...
  logMedicationEvent(
    ...
    this.log(level, `[MEDICATION:${action.toUpperCase()}] ${message}`, {
      ...data,
  ...
  logEmergencyResponse(
    ...
    this.log(level, `[EMERGENCY:${action.toUpperCase()}] ${message}`, {
      ...data,
  ...
  private log(
    ...
    try {
      // Create log entry
      const logEntry = this.createLogEntry(level, message, data, context, error);
      ...
      if (this.config.outputs.console && (process.env.NODE_ENV === 'development' || level === 'emergency')) {
  ...
  private shouldLog(level: LogLevel): boolean {
    const levels = ['debug', 'info', 'notice', 'warn', 'error', 'critical', 'alert', 'emergency'];
  ...
  private determineLGPDCompliance(
    ...
    if (context?.healthcare?.patientContext || context?.technical?.userContext) {
  ...
  private detectPII(
    ...
    if (context?.technical?.userContext?.ipAddress || 
  ...
  private redactPII(logEntry: LogEntry): LogEntry {
    ...
    if (redactedEntry.error) {
      redactedEntry.error = {
        ...
        stack: redactedEntry.error.stack ? this.redactPIIFromText(redactedEntry.error.stack) : undefined
      };
    }
   ...
   */
  private redactPIIFromObject(obj: Record<string, unknown>): Record<string, unknown> {
    const redactedObj = { ...obj };
    const piiFields = ['cpf', 'email', 'phone', 'name', 'address', 'password', 'token'];
    ...
    Object.keys(redactedObj).forEach(key => {
      if (piiFields.some(field => key.toLowerCase().includes(field))) {
        ...
        redactedObj[key] = this.redactPIIFromObject(redactedObj[key] as Record<string, unknown>);
      }
    });
  ...
  private generateTags(
    ...
    if (context?.healthcare?.patientContext?.criticalityLevel) {
      tags.push(`criticality:${context.healthcare.patientContext.criticalityLevel}`);
    }
  ...
  private outputToConsole(logEntry: LogEntry): void {
    ...
    console.log(`${emoji} [${timestamp}] [${logEntry.technicalContext.service}] ${logEntry.message}`, 
  ...
  private async sendToRemoteEndpoint(logs: LogEntry[]): Promise<void> {
    ...
    console.log(`📤 [StructuredLogger] Sending ${logs.length} logs to remote endpoint`);
  }
  ...
  private async sendToObservabilityPlatform(logs: LogEntry[]): Promise<void> {
    ...
    console.log(`📊 [StructuredLogger] Sending ${logs.length} logs to observability platform`);
  }
  ...
  destroy(): void {
    ...
    console.log('🔄 [StructuredLogger] Logger destroyed and resources cleaned up');
  }
}
