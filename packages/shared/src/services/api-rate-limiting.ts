...
export type HealthcareRequestCategory = z.infer<typeof HealthcareRequestCategorySchema>;
...
export const RateLimitTierSchema = z.object({
  ...
  // Recovery settings
  cooldownMinutes: z.number().min(0).describe('Cooldown period after limit reached'),
  gracePeriodSeconds: z.number().min(0).describe('Grace period before enforcement')
});
...
export const RateLimitContextSchema = z.object({
  ...
  correlationId: z.string().optional().describe('Correlation ID for related requests'),
  
  // Request classification
  category: HealthcareRequestCategorySchema.describe('Healthcare request category'),
  priority: RequestPrioritySchema.describe('Request priority level'),
  ...
  healthcareContext: z.object({
    ...
    criticalSystemFlag: z.boolean().optional().describe('Critical system flag')
  }).optional().describe('Healthcare-specific context'),
  ...
  complianceContext: z.object({
    ...
    consentLevel: z.enum(['explicit', 'implicit', 'legitimate_interest']).describe('Consent level')
...
export const RateLimitConfigSchema = z.object({
  ...
  defaultAlgorithm: RateLimitAlgorithmSchema.default('sliding_window').describe('Default rate limiting algorithm'),
  
  // Tier definitions
  tiers: z.array(RateLimitTierSchema).describe('Rate limit tier configurations'),
  
  // Global settings
  globalLimits: z.object({
    requestsPerSecond: z.number().default(1000).describe('Global requests per second'),
    requestsPerMinute: z.number().default(50000).describe('Global requests per minute'),
    maxConcurrentRequests: z.number().default(500).describe('Maximum concurrent requests')
  }).describe('Global rate limiting'),
  
  // Emergency settings
  emergencySettings: z.object({
    enableBypass: z.boolean().default(true).describe('Enable emergency bypass'),
    bypassTokens: z.array(z.string()).optional().describe('Emergency bypass tokens'),
    maxEmergencyRequests: z.number().default(1000).describe('Maximum emergency requests per hour'),
    bypassCategories: z.array(HealthcareRequestCategorySchema).describe('Categories that can bypass')
  }).describe('Emergency bypass settings'),
  
  // Healthcare-specific configuration
  healthcareConfig: z.object({
    patientSafetyBypass: z.boolean().default(true).describe('Enable patient safety bypass'),
    criticalSystemBypass: z.boolean().default(true).describe('Enable critical system bypass'),
    facilityBasedLimits: z.boolean().default(true).describe('Apply facility-based limits'),
    workflowAwareLimits: z.boolean().default(true).describe('Apply workflow-aware limits')
  }).describe('Healthcare-specific configuration'),
  
  // Storage and persistence
  storage: z.object({
    type: z.enum(['memory', 'redis', 'database']).default('memory').describe('Storage backend type'),
    connectionString: z.string().optional().describe('Storage connection string'),
    keyPrefix: z.string().default('neonpro:ratelimit:').describe('Storage key prefix'),
    ttlSeconds: z.number().default(3600).describe('Storage TTL in seconds')
  }).describe('Storage configuration'),
  
  // Monitoring and alerting
  monitoring: z.object({
    enableMetrics: z.boolean().default(true).describe('Enable rate limiting metrics'),
    enableAlerting: z.boolean().default(true).describe('Enable rate limit alerts'),
    alertThreshold: z.number().default(0.8).describe('Alert threshold (0.8 = 80% of limit)'),
    metricsInterval: z.number().default(60000).describe('Metrics collection interval in ms')
  }).describe('Monitoring configuration'),
  
  // Logging and audit
  audit: z.object({
    logAllRequests: z.boolean().default(false).describe('Log all requests (GDPR impact)'),
    logViolations: z.boolean().default(true).describe('Log rate limit violations'),
    logBypasses: z.boolean().default(true).describe('Log emergency bypasses'),
    retentionDays: z.number().default(90).describe('Audit log retention days')
...
class SlidingWindowAlgorithm extends RateLimitAlgorithmBase {
  async checkLimit(key: string, context: RateLimitContext): Promise<RateLimitResult> {
    ...
    let windowData = this.storage.get(key) || { requests: [], firstRequest: now };
    
    // Remove requests outside the window
    windowData.requests = windowData.requests.filter((timestamp: number) => 
...
class TokenBucketAlgorithm extends RateLimitAlgorithmBase {
  async checkLimit(key: string, context: RateLimitContext): Promise<RateLimitResult> {
    ...
    const resetTime = new Date(now + ((capacity - bucket.tokens) / refillRate) * 1000);
...
export class APIRateLimitingService {
  ...
  private initialize(): void {
    try {
      ...
      console.log('🚦 [APIRateLimitingService] Healthcare API rate limiting service initialized');
    } catch (error) {
  ...
  private determineRateLimitTier(context: RateLimitContext): RateLimitTier {
    ...
    const categoryPriorityMap: Record<HealthcareRequestCategory, RequestPriority> = {
    ...
    const priority = context.priority || categoryPriorityMap[context.category] || 'normal';
   ...
   */
  private generateRateLimitKey(context: RateLimitContext, tier: RateLimitTier): string {
    const parts = [
    ...
    if (this.config.healthcareConfig.facilityBasedLimits && context.healthcareContext?.facilityId) {
   ...
   */
  private checkEmergencyBypass(context: RateLimitContext, tier: RateLimitTier): RateLimitResult {
    ...
    else if (tier.patientSafetyBypass && context.healthcareContext?.patientSafetyFlag) {
      bypassApplied = true;
      bypassReason = 'Patient safety bypass';
    }
    
    // Critical system bypass
    else if (this.config.healthcareConfig.criticalSystemBypass && context.healthcareContext?.criticalSystemFlag) {
      bypassApplied = true;
   ...
   * Log rate limiting decision
   */
  private async logRateLimitDecision(context: RateLimitContext, result: RateLimitResult): Promise<void> {
   ...
   */
  private async checkAlertingThresholds(context: RateLimitContext, result: RateLimitResult): Promise<void> {
   ...
   */
  private async sendRateLimitAlert(context: RateLimitContext, result: RateLimitResult, usageRatio: number): Promise<void> {
    const alertData = {
  ...
  private collectMetrics(): void {
    ...
    console.log('📊 [APIRateLimitingService] Metrics collected:', {
      ...
      totalMetrics: Array.from(metricsSnapshot.values()).reduce((sum, m) => sum + m.totalRequests, 0)
    });
  ...
  async resetRateLimit(clientId: string, tier?: string): Promise<void> {
    ...
    console.log(`🔄 [APIRateLimitingService] Reset rate limit for client: ${clientId}, tiers: ${tiersToReset.join(', ')}`);
  }
  ...
  destroy(): void {
    ...
    console.log('🔄 [APIRateLimitingService] API rate limiting service destroyed and resources cleaned up');
  }
}
