...
export const HealthcareRequestContextSchema = z.object({
  ...
  correlationId: z.string().optional().describe('Correlation ID for request tracing'),
  ...
  workflowContext: z.object({
    ...
    urgencyLevel: z.enum(['routine', 'urgent', 'critical', 'emergency']).optional().describe('Request urgency'),
    
    // Flags for special handling
    emergencyFlag: z.boolean().default(false).describe('Emergency request flag'),
    patientSafetyFlag: z.boolean().default(false).describe('Patient safety flag'),
    criticalSystemFlag: z.boolean().default(false).describe('Critical system flag'),
    complianceFlag: z.boolean().default(false).describe('Compliance-related flag')
  }).optional().describe('Healthcare workflow context'),
  
  // User context (LGPD-compliant)
  userContext: z.object({
    anonymizedUserId: z.string().optional().describe('LGPD-compliant user ID'),
    userRole: z.enum([
      'patient',
      'doctor',
      'nurse',
    ...
    accessLevel: z.enum(['public', 'restricted', 'confidential', 'secret']).optional().describe('Access level'),
    consentStatus: z.object({
      dataProcessing: z.boolean().default(false).describe('Data processing consent'),
      communication: z.boolean().default(false).describe('Communication consent'),
      analytics: z.boolean().default(false).describe('Analytics consent'),
      thirdParty: z.boolean().default(false).describe('Third-party sharing consent')
  ...
  technicalContext: z.object({
    ...
    deviceType: z.enum(['mobile', 'tablet', 'desktop', 'iot', 'medical_device']).optional().describe('Device type'),
  ...
  complianceContext: z.object({
    ...
    privacySettings: z.object({
      piiRedaction: z.boolean().default(true).describe('Enable PII redaction'),
      anonymization: z.boolean().default(false).describe('Enable anonymization'),
      pseudonymization: z.boolean().default(true).describe('Enable pseudonymization'),
      encryption: z.boolean().default(true).describe('Enable encryption')
    }).describe('Privacy settings')
  }).describe('Compliance context')
});

export type HealthcareRequestContext = z.infer<typeof HealthcareRequestContextSchema>;
...
export const MiddlewareConfigSchema = z.object({
  ...
  healthcareSettings: z.object({
    enableWorkflowInjection: z.boolean().default(true).describe('Enable workflow context injection'),
    enableEmergencyDetection: z.boolean().default(true).describe('Enable emergency request detection'),
    enablePatientSafetyChecks: z.boolean().default(true).describe('Enable patient safety checks'),
    enableComplianceValidation: z.boolean().default(true).describe('Enable compliance validation')
  ...
  performanceMonitoring: z.object({
    ...
    slowRequestThreshold: z.number().default(5000).describe('Slow request threshold in ms'),
    metricsInterval: z.number().default(60000).describe('Metrics collection interval'),
    enableWebVitals: z.boolean().default(true).describe('Enable Web Vitals collection')
  }).describe('Performance monitoring settings'),
  
  // Rate limiting integration
  rateLimiting: z.object({
    enabled: z.boolean().default(true).describe('Enable rate limiting'),
    enableHealthcareClassification: z.boolean().default(true).describe('Enable healthcare request classification'),
    bypassEmergencyRequests: z.boolean().default(true).describe('Bypass rate limits for emergency requests')
  }).describe('Rate limiting settings'),
  
  // Error handling
  errorHandling: z.object({
    enableDetailedErrors: z.boolean().default(false).describe('Enable detailed error responses'),
    enableErrorTracking: z.boolean().default(true).describe('Enable error tracking'),
    enablePatientSafetyAlerts: z.boolean().default(true).describe('Enable patient safety alerts'),
    enableNotifications: z.boolean().default(true).describe('Enable error notifications')
  }).describe('Error handling settings'),
  
  // Security settings
  security: z.object({
    enableSecurityHeaders: z.boolean().default(true).describe('Enable security headers'),
    enableCors: z.boolean().default(true).describe('Enable CORS'),
    enableCsrfProtection: z.boolean().default(true).describe('Enable CSRF protection'),
    enableInputValidation: z.boolean().default(true).describe('Enable input validation'),
    
    // LGPD compliance
    enablePiiDetection: z.boolean().default(true).describe('Enable PII detection'),
  ...
  // Logging and audit
  logging: z.object({
    enableRequestLogging: z.boolean().default(true).describe('Enable request logging'),
    enableResponseLogging: z.boolean().default(false).describe('Enable response logging'),
    enableAuditLogging: z.boolean().default(true).describe('Enable audit logging'),
    logLevel: z.enum(['debug', 'info', 'warn', 'error']).default('info').describe('Log level'),
    
    // LGPD compliance for logging
    enablePiiRedaction: z.boolean().default(true).describe('Enable PII redaction in logs'),
    logRetentionDays: z.number().default(90).describe('Log retention period in days')
...
export class HealthcareMiddlewareService {
  ...
  private initialize(): void {
    try {
      ...
      console.log('🔄 [HealthcareMiddlewareService] Healthcare middleware service initialized');
    } catch (error) {
      console.error('Failed to initialize healthcare middleware service:', error);
    }
  }
   ...
   */
  private async initializeRequestContext(c: Context): Promise<HealthcareRequestContext> {
    const requestId = `req_${nanoid(12)}`;
    const correlationId = c.req.header('x-correlation-id') || `corr_${nanoid(8)}`;
   ...
   */
  private extractTechnicalContext(c: Context): HealthcareRequestContext['technicalContext'] {
    const req = c.req;
    ...
    return {
      ...
      performance: {
        ...
        bandwidth: req.header('x-bandwidth') ? parseInt(req.header('x-bandwidth')!) : undefined
      }
    };
  }
   ...
   * Determine compliance context
   */
  private determineComplianceContext(c: Context, userContext: any): HealthcareRequestContext['complianceContext'] {
    // Determine legal basis based on request type and user context
    let legalBasis: 'consent' | 'contract' | 'legal_obligation' | 'vital_interests' | 'public_interest' | 'legitimate_interests' = 'legitimate_interests';
    
    // Healthcare operations are typically legitimate interest
    if (c.req.path.includes('/emergency') || c.req.path.includes('/patient-safety')) {
    ...
    return {
      legalBasis,
      dataClassification,
      auditRequired: dataClassification === 'restricted' || dataClassification === 'confidential',
   ...
   * Detect healthcare workflow context
   */
  private async detectWorkflowContext(c: Context, userContext: any): Promise<HealthcareRequestContext['workflowContext']> {
    ...
    if (path.includes('/patient') && method === 'POST') workflowType = 'patient_registration';
    ...
    let urgencyLevel: 'routine' | 'urgent' | 'critical' | 'emergency' = 'routine';
    ...
    return {
      workflowType,
      workflowStage: method === 'POST' ? 'initiation' : method === 'PUT' ? 'update' : 'query',
      urgencyLevel,
      emergencyFlag,
      patientSafetyFlag,
      criticalSystemFlag,
   ...
   */
  private async performSecurityValidation(c: Context, context: HealthcareRequestContext): Promise<void> {
    ...
    if (this.config.security.enableCsrfProtection && ['POST', 'PUT', 'DELETE', 'PATCH'].includes(c.req.method)) {
   ...
   */
  private async validateLGPDCompliance(c: Context, context: HealthcareRequestContext): Promise<void> {
    ...
    if (context.complianceContext.legalBasis === 'consent') {
      ...
      if (!hasConsent) {
        throw new Error('LGPD_CONSENT_REQUIRED: User consent required for data processing');
      }
    }
   ...
   */
  private async performRateLimitingCheck(c: Context, context: HealthcareRequestContext): Promise<void> {
    // TODO: Integrate with API rate limiting service
    
    // For emergency requests, bypass rate limiting
    if (this.config.rateLimiting.bypassEmergencyRequests && 
        context.workflowContext?.emergencyFlag) {
      console.log(`🚨 [HealthcareMiddlewareService] Emergency bypass for request: ${context.requestId}`);
      return;
    }
    
    // Simulate rate limiting check
    console.log(`🚦 [HealthcareMiddlewareService] Rate limiting check for request: ${context.requestId}`);
  }

  /**
   * Inject healthcare context into request
   */
  private async injectHealthcareContext(c: Context, context: HealthcareRequestContext): Promise<void> {
    ...
    c.header('x-workflow-type', context.workflowContext?.workflowType || 'unknown');
    c.header('x-urgency-level', context.workflowContext?.urgencyLevel || 'routine');
    
    // Add compliance headers
    c.header('x-data-classification', context.complianceContext.dataClassification);
    c.header('x-audit-required', context.complianceContext.auditRequired.toString());
  }
   ...
   */
  private async logRequest(c: Context, context: HealthcareRequestContext): Promise<void> {
    const logData = {
      requestId: context.requestId,
   ...
   */
  private async performPostProcessing(c: Context, context: HealthcareRequestContext, startTime: number): Promise<void> {
    ...
    if (this.config.logging.enableResponseLogging && this.config.logging.enableAuditLogging) {
   ...
   */
  private async handleMiddlewareError(c: Context, error: any, context: HealthcareRequestContext, startTime: number): Promise<void> {
    const endTime = Date.now();
    const duration = endTime - startTime;
    ...
    console.error('❌ [HealthcareMiddlewareService] Middleware error:', errorData);
    
    // TODO: Integrate with error tracking service
    // TODO: Send patient safety alerts if applicable
    
    // Return appropriate error response
    if (!c.res.ok) { // Only if response not already sent
   ...
   */
  private detectDeviceType(userAgent?: string): 'mobile' | 'tablet' | 'desktop' | 'iot' | 'medical_device' | undefined {
    if (!userAgent) return undefined;
    
    const ua = userAgent.toLowerCase();
    
    // Medical device detection
    if (ua.includes('medical') || ua.includes('device') || ua.includes('monitor')) {
    ...
    if (requiresSecureHeaders) {
      ...
      for (const header of securityHeaders) {
        if (!c.req.header(header)) {
          console.warn(`⚠️ [HealthcareMiddlewareService] Missing security header: ${header}`);
        }
      }
    }
  }
   ...
   */
  private async detectAndRedactPII(c: Context, context: HealthcareRequestContext): Promise<void> {
    // TODO: Implement PII detection and redaction
    console.log(`🔐 [HealthcareMiddlewareService] PII detection for request: ${context.requestId}`);
  }
  ...
  private async validateDataMinimization(c: Context, context: HealthcareRequestContext): Promise<void> {
    ...
    if (body && typeof body === 'object') {
      ...
      const unnecessarySensitiveFields = sensitiveFields.filter(field => 
        requestedFields.includes(field) && !this.isFieldNecessary(field, context)
      );
      
      if (unnecessarySensitiveFields.length > 0) {
        console.warn(`⚠️ [HealthcareMiddlewareService] Data minimization violation - unnecessary fields: ${unnecessarySensitiveFields.join(', ')}`);
      }
    }
  }
   ...
   */
  private isFieldNecessary(field: string, context: HealthcareRequestContext): boolean {
    ...
    return necessaryFields[workflowType || 'administrative_task']?.includes(field) || false;
  }

  /**
   * Log compliance validation
   */
  private async logComplianceValidation(context: HealthcareRequestContext): Promise<void> {
    const complianceLog = {
      requestId: context.requestId,
      legalBasis: context.complianceContext.legalBasis,
      dataClassification: context.complianceContext.dataClassification,
    ...
    console.log('📋 [HealthcareMiddlewareService] Compliance validation:', complianceLog);
  }
   ...
   */
  private async handleSlowRequest(context: HealthcareRequestContext, duration: number): Promise<void> {
    ...
    console.warn('🐌 [HealthcareMiddlewareService] Slow request detected:', slowRequestData);
   ...
   * Log response
   */
  private async logResponse(c: Context, context: HealthcareRequestContext, duration: number): Promise<void> {
    const responseLog = {
      requestId: context.requestId,
      statusCode: c.res.status,
      duration,
    ...
    console.log('📤 [HealthcareMiddlewareService] Response logged:', responseLog);
  }
  ...
  private createErrorResponse(error: any, context?: HealthcareRequestContext) {
    const baseResponse = {
      error: true,
      message: this.config.errorHandling.enableDetailedErrors ? error.message : 'An error occurred',
      requestId: context?.requestId,
      timestamp: new Date().toISOString()
    };
  ...
  private collectAndReportMetrics(): void {
    ...
    if (metricsToReport.length > 0) {
      console.log('📊 [HealthcareMiddlewareService] Metrics collected:', {
        ...
        averageDuration: metricsToReport.reduce((sum, m) => sum + (m.duration || 0), 0) / metricsToReport.length,
  ...
  destroy(): void {
    ...
    console.log('🔄 [HealthcareMiddlewareService] Healthcare middleware service destroyed and resources cleaned up');
  }
}
