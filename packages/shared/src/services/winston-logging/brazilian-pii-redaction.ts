...
const HEALTHCARE_PATTERNS = {
  ...
  // Addresses
  address: {
    pattern: /\b(rua|av|avenida|travessa|alameda|estrada|rodovia)[\s\.]+[A-Za-z\s]+,\s*\d+[\s\-A-Za-z]*/gi,
  ...
  diagnosis: {
    pattern: /\b(câncer|cancer|aids|hiv|sifilis|gonorreia|herpes|hepatite|diabetes|hipertensão|depressão|ansiedade|esquizofrenia)\b/gi,
...
export class BrazilianPIIRedactionService {
  ...
  redactText(text: string): string {
    ...
    Object.entries(BRAZILIAN_PATTERNS).forEach(([type,_config]) => {
      redactedText = redactedText.replace(_config.pattern,(match, ...args) => {
        ...
        const isValid = this.enableValidation ? config.validate(fullMatch) : true;
    ...
    Object.entries(BRAZILIAN_PATTERNS).forEach(([type,_config]) => {
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          identifiers.push(BrazilianIdentifierSchema.parse({
            type: type as any,
            value: match,
            masked: config.mask,
            isValid,
  ...
  private shouldSkipField(fieldName: string): boolean {
    const skipFields = [
      ...
      'correlationId', 'sessionId', 'traceId', 'spanId', 'duration',
      'memoryUsage', 'cpuUsage', 'deviceType', 'source', 'tags',
      
      // Configuration fields
      'enabled', 'maxSize', 'maxFiles', 'timeout', 'interval',
      'retentionPeriod', 'batchSize', 'flushInterval',
      
      // Status fields
      'status', 'state', 'code', 'version', 'type', 'format',
    ];
  ...
  getStatistics(text: string): {
    ...
    const identifiersByType = identifiers.reduce((acc,_id) => {
    ...
    const redactionCoverage = Math.round(((textLength - redactedLength) / textLength) * 100);
