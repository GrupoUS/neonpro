...
function recordApiMetrics(
  ...
  try {
    ...
    const labels = {
      ...
      patient_data_involved: context['healthcare.patient_data_involved']?.toString() || 'false',
    };
    ...
    if (context['healthcare.patient_data_involved']) {
      const patientDataAccess = meter.createCounter('patient_data_access_total', {
...
export function complianceAuditMiddleware() {
  return async (c: Context, next: Next) => {
    ...
    if (healthcareContext['healthcare.compliance_level'] === 'sensitive' ||
