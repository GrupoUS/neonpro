...
export function categorizeError(error: Error, context?: ErrorContext): ErrorCategory {
  ...
  if (context?.patientId || message.includes('patient') || 
      message.includes('medical') || message.includes('clinical')) {
    return ErrorCategory.PATIENT_DATA;
  }
  
  // LGPD compliance errors
  if (message.includes('lgpd') || message.includes('consent') || 
      message.includes('gdpr') || message.includes('privacy')) {
 ...
 */
export function assessSeverity(error: Error, category: ErrorCategory, context?: ErrorContext): ErrorSeverity {
  // Critical severity for certain categories
  if (category === ErrorCategory.COMPLIANCE || 
 ...
 */
export function checkPatientDataInvolvement(error: Error, context?: ErrorContext): boolean {
  const message = error.message.toLowerCase();
  const stack = error.stack?.toLowerCase() || '';
