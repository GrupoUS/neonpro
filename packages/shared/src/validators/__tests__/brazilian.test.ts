...
describe('Brazilian Validation Schemas (T037)', () => {
  ...
  describe('Validation Messages', () => {
    it('should provide Portuguese error messages', () => {
      ...
      expect(getValidationMessage('cnpj', 'invalid')).toContain('CNPJ inválido');
      expect(getValidationMessage('phone', 'invalid')).toContain('telefone inválido');
      expect(getValidationMessage('cep', 'invalid')).toContain('CEP inválido');
    });

    it('should provide field-specific messages', () => {
      const { getValidationMessage } = require('../brazilian');
      expect(getValidationMessage('cpf', 'required')).toContain('CPF é obrigatório');
      expect(getValidationMessage('phone', 'format')).toContain('formato do telefone');
      expect(getValidationMessage('cep', 'not_found')).toContain('CEP não encontrado');
    });
  });
  ...
  describe('Utility Functions', () => {
    ...
    it('should provide Brazilian healthcare constants', () => {
      const { BRAZILIAN_STATES, HEALTHCARE_SPECIALTIES } = require('../brazilian');
