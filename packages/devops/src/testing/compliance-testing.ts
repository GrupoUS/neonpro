...
interface FacilityInfrastructure {
  totalArea: number;
  rooms: Room[];
...
interface InfectionControlPlan {
  procedures: string[];
  monitoring: boolean;
  training: boolean;
}

interface WasteManagementPlan {
  segregation: boolean;
  treatment: string;
  disposal: string;
}
...
interface AdverseEventValidationResult {
  isCompliant: boolean;
  validationChecks: Record<string, boolean>;
  score: number;
...
interface ProcedureValidationResult {
  isCompliant: boolean;
  validationChecks: Record<string, boolean>;
  score: number;
...
interface HealthcareComplianceData {
  type: string;
}
