import { z } from 'zod';
import { BasePatientSchema, CompletePatientSchema } from './base-patient.schema';
import { validateCPF, validateCNPJ, validatePhone, validateCEP } from '@neonpro/validators';
...
export const BrazilianPatientRegistrationSchema = BasePatientSchema.extend({
  ...
  healthInsurance: z.object({
    ...
    validity: z.string().regex(/^\d{2}\/\d{4}$/, 'Validade inválida (MM/AAAA)'),
  }).optional(),
  ...
  lgpdConsent: z.object({
    dataProcessing: z.boolean().refine(val => val === true, 'Consentimento obrigatório'),
    dataSharing: z.boolean().refine(val => val === true, 'Consentimento obrigatório'),
...
export const PatientExportSchema = z.object({
  ...
  fields: z.array(z.enum([
    'id', 'name', 'cpf', 'email', 'phone', 'birthDate', 'gender',
    'address', 'healthInsurance', 'status', 'createdAt'
...
export type BrazilianPatientRegistration = z.infer<typeof BrazilianPatientRegistrationSchema>;
export type BrazilianPatientUpdate = z.infer<typeof BrazilianPatientUpdateSchema>;
