...
export interface RTCCallManager {
  ...
  createCall(session: Omit<TelemedicineCallSession, 'sessionId' | 'startTime' | 'state'>): Promise<TelemedicineCallSession>;
  
  /** Join existing call session */
  joinCall(sessionId: string, participant: Omit<CallParticipant, 'connectionState'>): Promise<void>;
