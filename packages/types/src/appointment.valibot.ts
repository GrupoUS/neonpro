...
export type AestheticProcedureType = v.InferOutput<typeof AestheticProcedureType>;
...
export const AppointmentSchema = v.object({
  ...
  duration_minutes: v.pipe(v.number(), v.integer(), v.minValue(15), v.maxValue(480)),
  ...
  reminder_preferences: v.object({
    ...
    sms: v.boolean(),
    email: v.boolean(),
    hours_before: v.array(v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(168)))
  }),
  no_show_risk_score: v.optional(v.pipe(v.number(), v.minValue(0), v.maxValue(1))),
...
export const CreateAppointmentSchema = v.object({
  ...
  estimated_cost: v.optional(v.pipe(v.number(), v.minValue(0))),
  notes: v.optional(v.string()),
  reminder_preferences: v.optional(v.object({
    whatsapp: v.boolean(),
    sms: v.boolean(), 
    email: v.boolean(),
    hours_before: v.array(v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(168)))
...
export const UpdateAppointmentSchema = v.object({
  ...
  duration_minutes: v.optional(v.pipe(v.number(), v.integer(), v.minValue(15), v.maxValue(480))),
  notes: v.optional(v.string()),
  cancellation_reason: v.optional(v.string()),
  payment_status: v.optional(v.picklist(['pending', 'partial', 'paid', 'refunded'])),
...
export const ReminderConfigSchema = v.object({
  ...
  response_type: v.optional(v.picklist(['confirmed', 'cancelled', 'rescheduled']))
});
...
export const AppointmentAnalyticsSchema = v.object({
  ...
  most_popular_procedures: v.array(v.object({
    procedure_type: AestheticProcedureType,
...
export type AppointmentAnalytics = v.InferOutput<typeof AppointmentAnalyticsSchema>;