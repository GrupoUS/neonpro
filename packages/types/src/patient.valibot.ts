 ...
 */
export type MedicalRecordNumber = string & { readonly __brand: 'MedicalRecordNumber' };
...
export const BrazilianPhoneSchema = v.pipe(
  ...
  v.regex(/^(\+55\s?)?\(?[1-9]{2}\)?\s?9?\d{4}-?\d{4}$/, 'Telefone deve estar no formato brasileiro (+55 XX 9XXXX-XXXX)'),
...
export const MedicalRecordNumberSchema = v.pipe(
  ...
  v.regex(/^[A-Za-z0-9-]+$/, 'Número do prontuário deve conter apenas letras, números e hífens'),
