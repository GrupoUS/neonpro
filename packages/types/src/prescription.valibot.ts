 ...
 */
export type ANVISARegisterNumber = string & { readonly __brand: 'ANVISARegisterNumber' };

/**
 * Branded type for Digital Certificate ID
 */
export type DigitalCertificateId = string & { readonly __brand: 'DigitalCertificateId' };
 ...
 */
export type PharmaceuticalBarcode = string & { readonly __brand: 'PharmaceuticalBarcode' };
 ...
 */
const validatePrescriptionExpiration = (issueDate: string, expirationDate: string): boolean => {
  ...
  const maxExpiration = new Date(issue.getTime() + (maxValidityDays * 24 * 60 * 60 * 1000));
...
const validateControlledSubstanceRules = (
  ...
  if (medicationType === 'controlled_a1' || medicationType === 'controlled_a2') {
...
export const ANVISARegisterNumberSchema = v.pipe(
  ...
  v.regex(/^\d\.\d{4}\.\d{4}\.\d{3}-\d$/, 'Registro ANVISA deve estar no formato X.XXXX.XXXX.XXX-X'),
...
export const PharmaceuticalBarcodeSchema = v.pipe(
  ...
  v.check(validatePharmaceuticalBarcode, 'Código de barras farmacêutico inválido'),
...
export const DosageSchema = v.pipe(
  ...
  v.check(validateDosage, 'Formato de dosagem inválido. Use formatos como "500mg", "5ml", "1 comprimido"'),
...
export const FrequencySchema = v.pipe(
  ...
  v.check(validateFrequency, 'Formato de frequência inválido. Use formatos como "1x ao dia", "de 8 em 8 horas"'),
...
export const DurationSchema = v.pipe(
  ...
  v.check(validateDuration, 'Formato de duração inválido. Use formatos como "7 dias", "uso contínuo"'),
  v.transform((value) => value.toLowerCase())
);
...
export const MedicationInformationSchema = v.object({
  ...
  manufacturer: v.optional(v.pipe(v.string(), v.minLength(2), v.maxLength(100))),
  presentation: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(200))),
  concentration: v.optional(v.pipe(v.string(), v.minLength(2), v.maxLength(50))),
  administration_route: AdministrationRouteSchema,
  therapeutic_class: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(100))),
  pharmacological_class: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(100))),
...
export const PrescriptionInstructionsSchema = v.object({
  ...
  administration_time: v.optional(v.picklist([
    'jejum',                // Em jejum
...
export const DigitalCertificateSchema = v.object({
  certificate_id: v.pipe(v.string(), v.uuid('ID do certificado deve ser UUID válido')),
  ...
  cfm_validation_status: v.optional(v.picklist(['validated', 'pending', 'expired'], 'Status CFM inválido'))
});
...
export const PrescriptionCreationSchema = v.pipe(
  v.object({
    ...
    appointment_id: v.optional(v.pipe(v.string(), v.uuid('ID do agendamento deve ser UUID válido'))),
    
    // Prescription metadata
    prescription_type: PrescriptionTypeSchema,
    prescription_number: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(50))),
    ...
    medications: v.pipe(
      v.array(v.object({
        ...
        controlled_substance_validation: v.optional(v.object({
          requires_special_prescription: v.boolean(),
          prescription_series: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(20))),
          special_authorization: v.optional(v.pipe(v.string(), v.maxLength(200)))
        }))
      })),
    ...
    clinical_indication: v.optional(v.pipe(v.string(), v.minLength(5), v.maxLength(500))),
    ...
    cfm_compliance_verified: v.pipe(v.boolean(), v.literal(true, 'Conformidade CFM deve ser verificada')),
    anvisa_compliance_verified: v.pipe(v.boolean(), v.literal(true, 'Conformidade ANVISA deve ser verificada')),
    
    // Emergency contact
    emergency_contact: v.optional(v.object({
...
export const PrescriptionUpdateSchema = v.object({
  prescription_id: v.pipe(v.string(), v.uuid('ID da receita deve ser UUID válido')),
  ...
  // Renewal information
  renewal_of: v.optional(v.pipe(v.string(), v.uuid('ID da receita original deve ser UUID válido'))),
...
export const PrescriptionQuerySchema = v.object({
  ...
  sort_by: v.optional(v.picklist([
    'issue_date', 'expiration_date', 'patient_name', 'professional_name', 'status'
...
export const validateControlledSubstanceCompliance = (
  ...
  for (const med of medications) {
    if (med.medication.controlled_substance) {
      ...
      if (!isValid) {
        errors.push(`Medicamento controlado ${med.medication.medication_type} não atende aos critérios regulamentares`);
      }
    }
  }
...
export const calculateExpirationDate = (
  ...
  const expiration = new Date(issue.getTime() + (validityDays * 24 * 60 * 60 * 1000));
