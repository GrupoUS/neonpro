      ...
      return {
        ...
        professionals: {
          ...
          telemedicine_certified:
            professionals.data?.filter((p: any) => p.telemedicine_certified).length || 0,
          digital_signature_enabled:
            professionals.data?.filter((p: any) => p.digital_signature_cert).length || 0,
        ...
        digital_signatures: {
          ...
          by_document_type: {
            prescription:
              signatures.data?.filter((s: any) => s.document_type === 'prescription').length || 0,
            medical_certificate:
              signatures.data?.filter((s: any) => s.document_type === 'medical_certificate')
                .length || 0,
            treatment_plan:
              signatures.data?.filter((s: any) => s.document_type === 'treatment_plan').length || 0,
            consultation_report:
              signatures.data?.filter((s: any) => s.document_type === 'consultation_report')
                .length || 0,
          },
        },
        telemedicine: {
          ...
          compliant_sessions:
            sessions.data?.filter((s: any) => s.cfm_compliance_validated).length || 0,
          by_session_type: {
            consultation:
              sessions.data?.filter((s: any) => s.session_type === 'consultation').length || 0,
            follow_up:
              sessions.data?.filter((s: any) => s.session_type === 'follow_up').length || 0,
            second_opinion:
              sessions.data?.filter((s: any) => s.session_type === 'second_opinion').length || 0,
            emergency:
              sessions.data?.filter((s: any) => s.session_type === 'emergency').length || 0,
          },
        },
        continuing_education: {
          total_courses: education.data?.length || 0,
          total_hours:
            education.data?.reduce((sum: number, course: any) => sum + course.hours, 0) || 0,
