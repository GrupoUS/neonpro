...
export class ANVISACompliance {
  ...
  async validateProductCompliance(productId: string): Promise<boolean> {
    try {
      ...
      const hasValidRegistration = product.registration_number
        && product.registration_number.length > 0;
    ...
    try {
      ...
      return {
        ...
        products: {
          ...
          approved: products.data?.filter(
            (p: any) => p.regulatory_status === 'approved',
          ).length || 0,
          expiring_soon: expiringSoon.length,
          suspended: products.data?.filter(
        ...
        procedures: {
          ...
          by_risk: {
            low_risk: procedures.data?.filter(
              (p: any) => p.classification === 'low_risk',
            ).length || 0,
            medium_risk: procedures.data?.filter(
              (p: any) => p.classification === 'medium_risk',
            ).length || 0,
            high_risk: procedures.data?.filter(
              (p: any) => p.classification === 'high_risk',
            ).length || 0,
            surgical: procedures.data?.filter(
        ...
        adverse_events: {
          ...
          by_severity: {
            mild: events.data?.filter((e: any) => e.event_type === 'mild').length
              || 0,
            moderate: events.data?.filter((e: any) => e.event_type === 'moderate')
              .length || 0,
            severe: events.data?.filter((e: any) => e.event_type === 'severe')
              .length || 0,
            life_threatening: events.data?.filter(
  ...
  private calculateComplianceScore(
    ...
    const expiredProducts = products?.filter((p) => new Date(p.expiry_date) < new Date()).length
      || 0;
    const suspendedProducts = products?.filter((p) => p.regulatory_status === 'suspended').length
      || 0;
