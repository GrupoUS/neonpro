...
describe('T009: Message Redaction for AI Chat', () => {
  describe('PII Detection', () => {
    ...
    it('should handle multiple PII types in single message', () => {
      const message = 'João Silva, CPF 123.456.789-00, telefone (11) 99999-8888, email joao@email.com';
      const result = detectPII(message);
      
      expect(result.hasPII).toBe(true);
      expect(result.patterns).toEqual(expect.arrayContaining(['cpf', 'phone', 'email']));
  ...
  describe('Message Redaction', () => {
    ...
    it('should preserve medical context while redacting PII', () => {
      const message = 'Paciente João Silva (CPF 123.456.789-00) apresenta hipertensão, contato: (11) 99999-8888';
      const result = redactMessage(message);
    ...
    it('should maintain message readability after redaction', () => {
      ...
      expect(result.redactedContent).toMatch(/Dr\. Silva atendeu paciente \[NAME_REDACTED\] \([CPF_REDACTED]\) com sintomas de gripe/);
    });
  });
