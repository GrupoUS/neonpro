...
const DayCell: React.FC<{
  ...
  const densityLevel = getDensityLevel(appointments.length, maxAppointmentsPerDay);
  ...
  return (
    ...
    >
      ...
      {/* Density indicator */}
      {showDensityIndicators && (
        <DensityIndicator level={densityLevel} />
      )}
      
      {/* Appointments */}
      <div className="space-y-1">
        {appointments.slice(0, variant === 'compact' ? 2 : 4).map((appointment) => (
          <div
            key={appointment.id}
            onClick={(e) => {
              e.stopPropagation();
...
export const AppointmentCalendar: React.FC<AppointmentCalendarProps> = ({
  ...
  const getHolidayName = (date: Date) => {
    const holiday = brazilianHolidays.find(holiday => 
      holiday.date.toDateString() === date.toDateString()
    );
    return holiday?.name;
  };
