...
export function announceToScreenReader(
  ...
  const liveType: AriaLiveType = 
    priority === HealthcarePriority.EMERGENCY || priority === HealthcarePriority.HIGH
      ? 'assertive' 
      : 'polite';
      ...
      switch (event.key) {
        case 'ArrowUp':
          ...
          if (currentRowIndex > 0) {
            ...
            const targetCell = prevRow.querySelectorAll('td, th')[currentCellIndex] as HTMLElement;
            targetCell?.focus();
          }
          break;
        ...
        case 'ArrowDown':
          ...
          if (currentRowIndex < allRows.length - 1) {
            ...
            const targetCell = nextRow.querySelectorAll('td, th')[currentCellIndex] as HTMLElement;
            targetCell?.focus();
          }
          break;
        ...
        case 'ArrowLeft':
          ...
          if (currentCellIndex > 0) {
            const targetCell = allCellsInRow[currentCellIndex - 1] as HTMLElement;
            targetCell?.focus();
          }
          break;
        ...
        case 'End':
          event.preventDefault();
          const lastCellInRow = allCellsInRow[allCellsInRow.length - 1] as HTMLElement;
          lastCellInRow?.focus();
          break;
      }
    }
...
export function createAccessibleErrorMessage(
  ...
  errorContainer.setAttribute('aria-live', 
    priority === HealthcarePriority.HIGH || priority === HealthcarePriority.EMERGENCY 
      ? 'assertive' 
      : 'polite'
  );
