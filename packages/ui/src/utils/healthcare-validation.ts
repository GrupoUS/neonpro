...
export const healthcareValidationSchemas = {
  ...
  medicalRecordNumber: z.string()
    ...
    .regex(/^[A-Za-z0-9-]+$/, 'Número do prontuário deve conter apenas letras, números e hífens'),

  // Healthcare professional registration (CRM for Brazil)
  crm: z.string()
    .regex(/^CRM\/[A-Z]{2}\s\d{4,6}$/, 'CRM deve estar no formato CRM/UF 0000000')
    .refine((crm) => {
  ...
  medicalSpecialty: z.string()
    ...
    .regex(/^[A-Za-zÀ-ÿ\s-]+$/, 'Especialidade deve conter apenas letras, espaços e hífens'),
  ...
  medication: z.object({
    ...
    dosage: z.string()
      ...
      .regex(/^[\d,.\s]+(mg|ml|g|mcg|UI|%)\s*.*$/, 'Dosagem deve incluir unidade válida'),
    frequency: z.string()
      .min(1, 'Frequência é obrigatória')
      .max(100, 'Frequência deve ter no máximo 100 caracteres'),
    prescribedBy: z.string()
      .regex(/^CRM\/[A-Z]{2}\s\d{4,6}$/, 'CRM do prescritor deve estar no formato CRM/UF 0000000')
  })
};
...
export function validateEmergencyData(data: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (data.allergies && Array.isArray(data.allergies)) {
    const criticalAllergies = data.allergies.filter((a: any) => a.severity === 'grave');
...
export function anonymizePatientData(data: Record<string, any>): Record<string, any> {
  ...
  const allowedFields = [
    'age_range', 'gender', 'medical_specialty', 'diagnosis_category',
    'treatment_type', 'outcome', 'duration'
  ];
