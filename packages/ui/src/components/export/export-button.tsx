...
import { Download, FileSpreadsheet, FileText, Loader2, CheckCircle, XCircle, Clock } from 'lucide-react'
...
const EXPORT_FORMATS = [
  { value: 'csv', label: 'CSV', icon: FileText, description: 'Valores separados por vírgula' },
  { value: 'xlsx', label: 'Excel', icon: FileSpreadsheet, description: 'Formato Excel nativo' }
]

const LGPD_OPTIONS = [
  { id: 'anonymize', label: 'Anonimizar dados sensíveis', default: true },
  { id: 'consent', label: 'Exigir consentimento do paciente', default: true },
  { id: 'excludeRestricted', label: 'Excluir campos restritos', default: false }
]
...
export function ExportButton({ 
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Erro ao iniciar exportação')
    } finally {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium capitalize">
                    {currentJob.status === 'processing' ? 'Processando' :
                     ...
                     currentJob.status === 'cancelled' ? 'Cancelado' : 'Pendente'}
                  </span>
                </div>
                ...
                {currentJob.status === 'processing' && (
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-500">
                      {currentJob.progress.processed} de {currentJob.progress.total} registros
                    </div>
                  </div>
                )}
                ...
                {currentJob.status === 'completed' && currentJob.result && (
                  <div className="space-y-2">
                    <div className="text-sm">
                      <div className="font-medium">Arquivo pronto para download</div>
                      <div className="text-gray-500">
                        {currentJob.result.recordCount} registros • {formatFileSize(currentJob.result.size)}
                      </div>
                    </div>
                    <Button
              ...
              <div className="space-y-4">
                      ...
                      <div key={option.id} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked: boolean) => 
                            setLgpdOptions(prev => ({ ...prev, [option.id]: !!checked }))
                          }
                        />
                ...
                <Alert>
                  <AlertDescription className="text-xs">
                    A exportação será processada em segundo plano. Você será notificado quando estiver pronta.
                    Os dados sensíveis serão anonimizados conforme a LGPD.
                  </AlertDescription>
                </Alert>
