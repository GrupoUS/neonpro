	...
	return (
		<div
			className={cn(
				...
				variant === "destructive" ? "border-destructive bg-destructive/10" : "border-border bg-card"
			)}
			{...props}
		>
			<div className="grid gap-1">
				{title && (
					<div
						className={cn("font-semibold text-sm", variant === "destructive" ? "text-destructive" : "text-foreground")}
					>
						{title}
					</div>
				)}
...
export const ToastAction = React.forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<"button">>(
	({ className, ...props }, ref) => (
		<button
			className={cn(
...
export function ToastTitle({ className, children, ...props }: { className?: string; children?: React.ReactNode }) {
	return (
