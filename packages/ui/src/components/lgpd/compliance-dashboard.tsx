...
export function ComplianceDashboard() {
			...
			<Card>
				...
				<CardContent>
					<Alert>
						...
						<AlertDescription>Erro ao carregar status de conformidade. Tente novamente mais tarde.</AlertDescription>
					</Alert>
				</CardContent>
			</Card>
		);
	...
	return (
		<div className="space-y-6">
			{/* Overall Status Card */}
			<Card className={isFullyCompliant ? "border-green-200 bg-green-50" : "border-yellow-200 bg-yellow-50"}>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						<Shield className="h-5 w-5" />
					...
					{isFullyCompliant ? (
						<Alert className="border-green-200 bg-green-50">
							...
							<AlertDescription className="text-green-800">
								Parabéns! Você está em total conformidade com a LGPD. Todos os consentimentos obrigatórios foram
								concedidos.
							</AlertDescription>
						</Alert>
					) : (
						<Alert className="border-yellow-200 bg-yellow-50">
							...
							<AlertDescription className="text-yellow-800">
								Alguns consentimentos obrigatórios estão pendentes. Revise suas preferências de privacidade para
			...
			<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-3">
							...
							<p className="text-muted-foreground text-xs">Consentimentos obrigatórios concedidos</p>
						</div>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-3">
							<div className="font-bold text-2xl">{complianceStatus.dataRequests.totalRequests}</div>
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-3">
							<div className="font-bold text-2xl">{isFullyCompliant ? "100" : Math.floor(consentProgress)}</div>
							<div className="flex items-center gap-1">
								{isFullyCompliant ? (
					...
					<CardContent>
						<div className="space-y-4">
							{complianceStatus.dataRequests.recentRequests.map((request, index) => (
								<div className="flex items-center justify-between rounded-lg bg-gray-50 p-3" key={index}>
