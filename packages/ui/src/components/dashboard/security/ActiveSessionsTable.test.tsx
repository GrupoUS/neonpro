...
describe("ActiveSessionsTable Security Tests", () => {
  ...
  describe("Security Vulnerability: confirm() usage", () => {
    ...
    it("should maintain healthcare compliance during session termination", async () => {
      render(<ActiveSessionsTable />);
      ...
      // Should show healthcare-specific warning for medical professionals
      expect(screen.getByText(/atenção: esta ação será registrada/i)).toBeInTheDocument();

      // Should mention LGPD compliance
      expect(screen.getByText(/conforme lgpd/i)).toBeInTheDocument();
    });
  });

  describe("Healthcare Compliance Requirements", () => {
    it("should log session termination for audit trail (ANVISA requirement)", async () => {
      const auditLogSpy = vi.fn();
      // Mock audit logging
      (global as any).auditLog = auditLogSpy;
