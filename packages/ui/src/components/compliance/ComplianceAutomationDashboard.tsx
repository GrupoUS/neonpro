...
> = ({ clinicId }) => {
  ...
  const useDataClassification = () => ({
    classifyData: (_data: unknown) => Promise.resolve(),
    loading: false,
  });
  const useDataSubjectRequests = () => ({
    createRequest: (_request: unknown) => Promise.resolve(),
    loading: false,
  });
  const useSoftwareValidation = () => ({
    validateSoftware: (_software: unknown) => Promise.resolve(),
    loading: false,
  });
  const useProfessionalValidation = () => ({
    validateProfessional: (_professional: unknown) => Promise.resolve(),
    loading: false,
  });
  const useComplianceAlertsLocal = () => ({
    createAlert: (_alert: unknown) => Promise.resolve(),
    loading: false,
  });
