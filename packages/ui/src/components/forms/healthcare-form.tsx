...
export interface HealthcareFormProps extends Omit<FormHTMLAttributes<HTMLFormElement>, 'onSubmit' | 'onError'> {
  ...
  // Form submission
  onSubmit?: (data: FormData, context: HealthcareFormContext) => Promise<void> | void;
...
export function HealthcareForm({
  ...
  const focusTrapRef = useFocusTrap(emergencyForm && accessibility.isEmergencyMode);
  ...
  const announceError = (message: string) => {
    const priority = emergencyForm ? HealthcarePriority.EMERGENCY : HealthcarePriority.HIGH;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (requireConsent && !consentGiven) {
        ...
        announceError('Consentimento para processamento de dados é obrigatório');
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'S') {
        ...
        announceToScreenReader('Salvamento rápido de emergência ativado', HealthcarePriority.EMERGENCY);
      }
      
      // Alt+H: Help/assistance call
      if (event.altKey && event.key === 'h') {
        event.preventDefault();
        announceToScreenReader('Chamando assistência médica', HealthcarePriority.EMERGENCY);
  ...
  return (
    <HealthcareFormContext.Provider value={contextValue}>
      <form
        ...
        id={formId}
        aria-label={ariaLabel || (emergencyForm ? 'Formulário de emergência médica' : 'Formulário de dados de saúde')}
      ...
      >
        ...
        <div id={`${formId}-description`} className="sr-only">
          ...
          {patientDataForm && 'Formulário contendo dados sensíveis do paciente. '}
          {requireConsent && 'Consentimento LGPD necessário para processamento. '}
        ...
        {emergencyForm && accessibility.isEmergencyMode && (
          <div className="healthcare-emergency-indicator bg-destructive text-destructive-foreground p-3 rounded-md">
            <div className="flex items-center gap-2">
              <span className="text-lg" role="img" aria-label="Emergência">🚨</span>
        ...
        {(dataSensitivity === DataSensitivity.RESTRICTED || dataSensitivity === DataSensitivity.CONFIDENTIAL) && (
          <div className="healthcare-sensitivity-indicator border-l-4 border-warning bg-warning/10 p-3">
            <div className="flex items-center gap-2">
              <span role="img" aria-label="Dados sensíveis">🔒</span>
              <span className="font-semibold text-sm">
                {dataSensitivity === DataSensitivity.RESTRICTED ? 'DADOS ALTAMENTE SENSÍVEIS' : 'DADOS CONFIDENCIAIS'}
              </span>
            </div>
        ...
        {requireConsent && !consentGiven && (
          <div className="healthcare-consent-notice bg-info/10 border border-info/20 p-4 rounded-md">
            <h3 className="font-semibold text-sm mb-2">Consentimento LGPD Necessário</h3>
            <p className="text-sm text-muted-foreground mb-3">
              Para processar os dados deste formulário, precisamos do seu consentimento 
            ...
            <p id={`${formId}-consent-description`} className="text-xs text-muted-foreground mt-2">
              Versão do consentimento: {consentVersion}. 
