...
const consentDescriptions: Record<ConsentType, { title: string; description: string; required?: boolean }> = {
  [ConsentType.ESSENTIAL]: {
    ...
    description: 'Necessários para o funcionamento básico da plataforma de saúde e segurança dos dados.',
    required: true,
  },
  [ConsentType.FUNCTIONAL]: {
    title: 'Funcionalidades Melhoradas',
    description: 'Permitem funcionalidades avançadas como salvamento de preferências e configurações médicas.',
  },
...
export const LGPDConsentBanner: React.FC<LGPDConsentBannerProps> = ({
  ...
  const contentClasses = cn(
    ...
    {
      'p-6': variant === 'modal',
      'fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-2xl w-full max-h-[90vh] overflow-y-auto bg-background border rounded-lg shadow-xl': variant === 'modal',
    }
  );
  
  return (
    <div className={bannerClasses} role="banner" aria-label="Consentimento LGPD">
      <div className={contentClasses}>
        {/* Header */}
        <div className="mb-4">
          <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
            <span role="img" aria-label="Privacidade">🔒</span>
            {title}
          </h2>
        ...
        <div className="mb-4 p-3 bg-muted rounded-md">
          <h3 className="text-sm font-medium mb-2">Tipos de Dados Processados:</h3>
          ...
          <h3 className="text-sm font-medium mt-3 mb-2">Finalidades do Tratamento:</h3>
          <ul className="text-xs text-muted-foreground space-y-1">
                  ...
                  return (
                    <div key={consentType} className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        >
                          {info?.description || 'Consentimento para processamento de dados'}
                        </p>
                      </div>
                    </div>
                  );
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          ...
          <p className="mt-2">
            ...
            Você pode alterar ou retirar seus consentimentos a qualquer momento.
          </p>
        </div>
