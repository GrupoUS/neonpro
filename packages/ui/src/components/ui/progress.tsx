	...
	"relative h-2 w-full overflow-hidden rounded-full bg-gradient-to-r from-muted/80 via-muted/60 to-muted/80 shadow-healthcare-sm backdrop-blur-sm transition-all duration-500",
	{
		variants: {
			variant: {
				...
				critical: "from-destructive/20 via-destructive/15 to-destructive/20 shadow-healthcare-md backdrop-blur-sm",
...
const progressBarVariants = cva("h-full w-full flex-1 rounded-full shadow-sm transition-all duration-700 ease-out", {
	variants: {
		variant: {
			...
			treatment: "bg-gradient-to-r from-secondary via-secondary/90 to-secondary shadow-secondary/30",
			critical: "bg-gradient-to-r from-destructive via-destructive/90 to-destructive shadow-destructive/40",
			...
			{(showLabel || label || icon || showValue) && (
				<div className="flex items-center justify-between text-sm">
					<div className="flex items-center gap-2">
						...
						{(showLabel || label) && <span className="font-medium text-foreground">{label || "Progresso"}</span>}
					</div>
					{showValue && <span className="font-medium text-muted-foreground">{Math.round(value || 0)}%</span>}
				</div>
			)}

			<ProgressPrimitive.Root className={cn(progressVariants({ variant, size }), className)} ref={ref} {...props}>
				<ProgressPrimitive.Indicator
					className={cn(progressBarVariants({ variant, animated: animated || pulse }), pulse && "animate-pulse")}
...
const TreatmentProgress = React.forwardRef<React.ElementRef<typeof ProgressPrimitive.Root>, TreatmentProgressProps>(
	(
		{ treatmentName, phase, completedSessions, totalSessions, nextSession, patientResponse = "good", ...props },
		ref
	) => {
		...
		return (
			<div className="space-y-3 rounded-lg border bg-card p-4">
				...
				<div className="flex items-center justify-between text-muted-foreground text-sm">
					...
					{nextSession && <span className="text-xs">Próxima: {nextSession.toLocaleDateString("pt-BR")}</span>}
				</div>
			</div>
		);
	}
	...
	return (
		<div className="space-y-4 rounded-lg border-2 border-green-200 bg-green-50/30 p-4 dark:border-green-800 dark:bg-green-950/10">
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					...
					<div>
						...
						<p className="text-muted-foreground text-xs">Pontuação geral de proteção de dados</p>
					</div>
				</div>
				<div className="text-right">
					<div className="font-bold text-2xl text-green-600">{overallScore}%</div>
			...
			<Progress
				...
				showValue={false}
				value={overallScore}
				variant={getComplianceVariant() as any}
				{...props}
			/>

			<div className="space-y-2">
				{categories.map((category) => (
					<div className="flex items-center justify-between text-sm" key={category.name}>
						<span className="text-muted-foreground">{category.name}</span>
						<div className="flex items-center gap-2">
							<span className={cn("font-medium", getStatusColor(category.status))}>{category.score}%</span>
							<div
								className={cn(
			...
			{(lastAudit || nextAudit) && (
				<div className="flex justify-between border-t pt-2 text-muted-foreground text-xs">
					{lastAudit && <span>Última auditoria: {lastAudit.toLocaleDateString("pt-BR")}</span>}
					{nextAudit && <span>Próxima auditoria: {nextAudit.toLocaleDateString("pt-BR")}</span>}
				</div>
			)}
		</div>
	);
...
>(({ patientName, condition, recoveryStage, vitalSigns, recoveryPercentage, estimatedDischarge, ...props }, ref) => {
	...
	return (
		<div className="space-y-4 rounded-lg border bg-card p-4">
			<div className="flex items-center justify-between">
				<div>
					<div className="flex items-center gap-2">
						...
						<span className="rounded-full bg-muted px-2 py-1 text-muted-foreground text-xs">{condition}</span>
					</div>
				...
				<div className="text-right">
					<div className="font-bold text-primary text-xl">{recoveryPercentage}%</div>
			...
			<Progress
				...
				variant={getStageVariant() as any}
				{...props}
			/>

			<div className="grid grid-cols-3 gap-4 text-sm">
				<div className="text-center">
					<div className="font-medium text-foreground">{vitalSigns.temperature}°C</div>
					<div className="text-muted-foreground text-xs">Temperatura</div>
				</div>
				<div className="text-center">
					<div className="font-medium text-foreground">{vitalSigns.bloodPressure}</div>
					<div className="text-muted-foreground text-xs">Pressão</div>
				</div>
				<div className="text-center">
					<div className="font-medium text-foreground">{vitalSigns.heartRate} bpm</div>
