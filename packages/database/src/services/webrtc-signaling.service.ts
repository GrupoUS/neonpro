...
export class WebRTCSignalingServer {
  ...
  private setupSocketHandlers(): void {
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('connection-state', async (state: {
        sessionId: string;
      ...
      socket.on('heartbeat', (data: { sessionId: string; timestamp: number }) => {
      ...
      socket.on('compliance-event', async (event: {
        sessionId: string;
        eventType: string;
        metadata?: any;
      }) => {
  ...
  private async handleJoinSession(
    ...
    const isAuthorized = await this.validateUserAuthorization(sessionId, userId, participantType);
    if (!isAuthorized) {
    ...
    console.log(`User ${userId} joined session ${sessionId} as ${participantType}`);
  }
  ...
  private async handleWebRTCSignal(
    ...
    console.log(`Relayed ${type} signal from ${sender.userId} to ${to} in session ${sessionId}`);
  }
    ...
    for (const [sessionId, room] of this.sessionRooms.entries()) {
      ...
      for (const [socketId, participant] of room.participants.entries()) {
        ...
        if (inactiveTime > inactivityTimeout && !participant.isActive) {
          console.log(`Removing inactive participant ${participant.userId} from session ${sessionId}`);
      ...
      if (room.participants.size === 0 && roomAge > 300000) { // 5 minutes empty
  ...
  public getActiveSessionsCount(): number {
    return Array.from(this.sessionRooms.values())
