...
class DatabaseTestRunner {
  ...
  private reportHealthChecks(healthChecks: HealthCheckResult[]): void {
    ...
    const hasWarnings = healthChecks.some(
    ...
    if (hasErrors) {
      console.log(
        "❌ Health check failed. Please resolve errors before running tests.",
      );
      process.exit(1);
    } else if (hasWarnings) {
      console.log(
        "⚠️  Health check passed with warnings. Tests may run with limited functionality.",
      );
    } else {
  ...
  generateFinalReport(): void {
    ...
    this.testResults.forEach((suite) => {
      ...
      console.log(
        `${statusEmoji} ${suite.suite}: ${suite.passed}/${suite.totalTests} (${suiteSuccessRate}%)`,
      );
    });

    // Compliance status
    const complianceStatus = this.testResults.some(
      (suite) =>
    ...
    if (complianceStatus) {
      ...
      const complianceTests = this.testResults.filter(
        (suite) =>
          suite.suite.includes("Compliance") || suite.suite.includes("LGPD"),
      );
    ...
    if (totalFailed === 0) {
      console.log(
        "🎉 All tests passed! The database package is ready for production.",
      );
    } else {
      console.log(
        `⚠️  ${totalFailed} test(s) failed. Please review and fix the issues before deployment.`,
      );
    }
  }
  ...
  saveResultsToFile(outputDir: string = "./test-results"): void {
    ...
    const report = {
      ...
      summary: {
        ...
        totalFailed: this.testResults.reduce(
          (sum, suite) => sum + suite.failed,
          0,
        ),
        totalSkipped: this.testResults.reduce(
          (sum, suite) => sum + suite.skipped,
          0,
        ),
        ...
        successRate:
          this.testResults.reduce((sum, suite) => sum + suite.totalTests, 0) > 0
            ? (
