...
import { SessionManager, MemorySessionStore, type SessionConfig } from '../src/config/session';
...
describe('T010: Chat Session Expiration Logic', () => {
  ...
  describe('Basic Session Repository', () => {
    ...
    it('should handle touching non-existent session gracefully', async () => {
      ...
      await expect(sessionRepo.touch('non-existent-id')).resolves.toBeUndefined();
    });
  });
  ...
  describe('Session Manager Expiration', () => {
    it('should create session with correct expiration time', async () => {
      ...
      const { sessionId, session } = await sessionManager.createSession(userData);
      ...
      const expectedExpiry = new Date(createdAt.getTime() + sessionConfig.maxAge * 1000);
    ...
    it('should detect session renewal need', async () => {
      ...
      const originalNow = Date.now;
      const renewalTime = Date.now() + (sessionConfig.maxAge * 1000) - (sessionConfig.renewThreshold * 500);
      Date.now = vi.fn(() => renewalTime);

      const session = await sessionStore.get(sessionId);
      const needsRenewal = session && 
        (new Date(session.expiresAt).getTime() - Date.now()) < (sessionConfig.renewThreshold * 1000);
    ...
    it('should renew session extending expiration time', async () => {
      ...
      if (renewed.session) {
        expect(new Date(renewed.session.expiresAt).getTime())
