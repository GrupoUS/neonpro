...
import { RateCounter, type RateCounterConfig, type RateLimit } from '../src/services/rate-counter';
...
describe('RateCounter Service', () => {
  ...
  describe('Multiple rate limits', () => {
    it('should handle multiple rate limits per user', async () => {
      ...
      // Check both limits
      const result1 = await rateCounter.checkMultipleLimits(userId, [shortLimit, longLimit]);
      expect(result1.allowed).toBe(true);
      ...
      const result2 = await rateCounter.checkMultipleLimits(userId, [shortLimit, longLimit]);
      ...
      // Should be allowed again
      const result3 = await rateCounter.checkMultipleLimits(userId, [shortLimit, longLimit]);
    ...
    it('should identify the most restrictive limit', async () => {
      ...
      // First request should be allowed
      const result1 = await rateCounter.checkMultipleLimits(userId, [veryShortLimit, shortLimit, longLimit]);
      expect(result1.allowed).toBe(true);
      
      // Second request should be blocked by very short limit
      const result2 = await rateCounter.checkMultipleLimits(userId, [veryShortLimit, shortLimit, longLimit]);
      expect(result2.allowed).toBe(false);
  ...
  describe('Rate limit information', () => {
    it('should provide accurate rate limit information', async () => {
      ...
      expect(info.resetTime).toBeLessThanOrEqual(Date.now() + defaultConfig.windowSizeMs);
    });
  ...
  describe('Edge cases and error handling', () => {
    it('should handle invalid user IDs', async () => {
      ...
      expect(async () => await rateCounter.checkLimit(null as any)).not.toThrow();
      expect(async () => await rateCounter.checkLimit(undefined as any)).not.toThrow();
    });
    ...
    it('should handle concurrent requests correctly', async () => {
      ...
      const promises = Array.from({ length: 5 }, () => rateCounter.checkLimit(userId));
  ...
  describe('Real-world scenarios', () => {
    it('should implement LGPD-compliant rate limiting (10/5min, 30/hour)', async () => {
      ...
      const fiveMinLimit: RateLimit = { windowSizeMs: 5 * 60 * 1000, maxRequests: 10 };
      const hourLimit: RateLimit = { windowSizeMs: 60 * 60 * 1000, maxRequests: 30 };
      
      // Make 10 requests (5-minute limit)
      for (let i = 0; i < 10; i++) {
        const result = await rateCounter.checkMultipleLimits(userId, [fiveMinLimit, hourLimit]);
        expect(result.allowed).toBe(true);
      }
