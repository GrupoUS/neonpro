...
export interface MetricEvent {
  ...
  clinicId?: string;
  sessionId?: string;
  eventType: 'query' | 'explanation' | 'rate_limit' | 'refusal' | 'error' | 'success';
...
export class AIChatMetrics {
  ...
  getSystemMetrics(): AIMetrics {
    ...
    return allMetrics.reduce((acc, current) => ({
      ...
      rateLimitRemaining: Math.min(acc.rateLimitRemaining, current.rateLimitRemaining),
  ...
  getComplianceStats(clinicId?: string): {
    ...
    const piiDetections = events.reduce((sum, e) => sum + (e.metrics.piiDetections || 0), 0);
    const consentValidations = events.reduce((sum, e) => sum + (e.metrics.consentValidations || 0), 0);
    const auditEvents = events.reduce((sum, e) => sum + (e.metrics.auditEvents || 0), 0);

    return {
      piiDetectionRate: totalInteractions > 0 ? (piiDetections / totalInteractions) * 100 : 0,
      consentValidationRate: totalInteractions > 0 ? (consentValidations / totalInteractions) * 100 : 0,
  ...
  getRateLimitingStats(clinicId?: string): {
    ...
    const totalInteractions = events.length;
    const totalHits = rateLimitEvents.reduce((sum, e) => sum + (e.metrics.rateLimitHits || 0), 0);
    ...
    const lastHitEvent = rateLimitEvents
  ...
  private updateAggregatedMetrics(clinicKey: string, newMetrics: Partial<AIMetrics>): void {
    const existing = this.metrics.get(clinicKey) || this.createEmptyMetrics();
    
    const updated: AIMetrics = {
      responseTime: this.updateAverage(existing.responseTime, newMetrics.responseTime),
      firstTokenTime: this.updateAverage(existing.firstTokenTime, newMetrics.firstTokenTime),
...
export const recordAISuccess = (params: Parameters<AIChatMetrics['recordSuccess']>[0]) => {
  aiChatMetrics.recordSuccess(params);
};

export const recordAIRefusal = (params: Parameters<AIChatMetrics['recordRefusal']>[0]) => {
  aiChatMetrics.recordRefusal(params);
};

export const recordAIError = (params: Parameters<AIChatMetrics['recordError']>[0]) => {
  aiChatMetrics.recordError(params);
};

export const recordRateLimit = (params: Parameters<AIChatMetrics['recordRateLimit']>[0]) => {
