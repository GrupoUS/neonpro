...
describe('Realtime Event Adapter', () => {
  ...
  beforeEach(async () => {
    ...
    eventHandlers = {
      onJoin: vi.fn((event) => { capturedEvents.push(event); }),
      onLeave: vi.fn((event) => { capturedEvents.push(event); }),
      onStatusChange: vi.fn((event) => { capturedEvents.push(event); }),
      onPresenceSync: vi.fn((event) => { capturedEvents.push(event); }),
      onError: vi.fn(),
    };
  ...
  describe('Factory Function', () => {
    ...
    it('should throw error for unsupported provider', () => {
      ...
      expect(() => createRealtimeAdapter(config)).toThrow('Unsupported provider: invalid');
    });
  });
  ...
  describe('Channel Management', () => {
    ...
    it('should update participant status', async () => {
      ...
      // Update status
      await adapter.updateParticipantStatus(channelId, participant.id, 'reconnecting');
  ...
  describe('Event Validation', () => {
    ...
    it('should create valid realtime events', () => {
      ...
      const event = createRealtimeEvent('join', 'test-channel', participant, { test: true });
    ...
    it('should validate healthcare compliance', () => {
      ...
      const nonCompliantEvent = createRealtimeEvent('join', 'test', participant);
  ...
  describe('Mock-Specific Features', () => {
    ...
    it('should simulate disconnection', async () => {
      ...
      const channelState = adapter.getChannelState(channelId);
      expect(channelState!.participants.get(participant.id)!.status).toBe('disconnected');
    });
    ...
    it('should simulate reconnection', async () => {
      ...
      expect(channelState!.participants.get(participant.id)!.status).toBe('reconnecting');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle initialization errors', async () => {
      ...
      await expect(freshAdapter.joinChannel('test', participant)).rejects.toThrow('not initialized');
    });

    it('should handle invalid participant data', async () => {
      const invalidParticipant = { id: '', role: 'doctor', name: '', status: 'connected' } as any;
      
      await expect(adapter.joinChannel('test', invalidParticipant)).rejects.toThrow('Invalid participant data');
    });
  ...
  describe('Integration Scenarios', () => {
    it('should handle complete healthcare session flow', async () => {
      ...
      const doctor = adapter.createMockParticipant({
        ...
        name: 'Dr. Maria Silva',
        role: 'doctor',
        capabilities: { audio: true, video: true, screenShare: true, chat: true },
      });
      ...
      const patient = adapter.createMockParticipant({
        ...
        name: 'João Santos',
        role: 'patient',
        capabilities: { audio: true, video: true, screenShare: false, chat: true },
      });
      ...
      const nurse = adapter.createMockParticipant({
        ...
        name: 'Ana Costa',
        role: 'nurse',
        capabilities: { audio: true, video: false, screenShare: false, chat: true },
      });
      ...
      await adapter.updateParticipantStatus(channelId, patient.id, 'reconnecting');
      ...
      // 6. Session ends
      await adapter.leaveChannel(channelId, patient.id, 'Consultation complete');
