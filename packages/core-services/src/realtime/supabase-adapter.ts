...
export class SupabaseRealtimeAdapter implements RealtimeEventAdapter {
  ...
  async joinChannel(
    ...
    try {
      ...
      console.log(`Participant ${fullParticipant.id} joined channel ${channelId}`);
    } catch (error) {
  ...
  async leaveChannel(channelId: string, participantId: string, reason?: string): Promise<void> {
    ...
    if (!participant) {
      console.warn(`Participant ${participantId} not found in channel ${channelId}`);
      return;
    }
  ...
  async getHealth() {
    ...
    this.healthStatus.totalParticipants = Array.from(this.channelStates.values())
  ...
  private handlePresenceSync(channelId: string, channel: RealtimeChannel): void {
