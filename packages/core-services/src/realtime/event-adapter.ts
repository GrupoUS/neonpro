...
export type RealtimeEventType = 'join' | 'leave' | 'status_change' | 'presence_sync';
...
export interface RealtimeEventHandlers {
  ...
  onPresenceSync?: RealtimeEventHandler<{ participants: RealtimeParticipant[] }>;
...
export interface RealtimeEventAdapter {
  ...
  /** Leave a channel */
  leaveChannel(channelId: string, participantId: string, reason?: string): Promise<void>;
...
export function isHealthcareCompliant(event: RealtimeEvent): boolean {
  return event.metadata.compliance.lgpdLogged && !event.metadata.compliance.sensitiveData;
}
...
export function createRealtimeAdapter(config: RealtimeAdapterConfig): RealtimeEventAdapter {
  switch (config.provider) {
    case 'supabase':
      ...
      throw new Error('SupabaseRealtimeAdapter not yet implemented - use mock adapter for testing');
    case 'mock':
      // Will be implemented in mock-adapter.ts
      throw new Error('MockRealtimeAdapter not yet implemented - import from index.ts');
    default:
