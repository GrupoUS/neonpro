...
describe('AbuseWindowTracker', () => {
  ...
  describe('User and Endpoint Isolation', () => {
    it('should track different users independently', async () => {
      ...
      expect(await tracker.checkAndTrackRequest(user1Key, _request1)).toHaveProperty(
        'allowed',
        false,
      )

      // User2 should still be allowed
      expect(await tracker.checkAndTrackRequest(user2Key, _request2)).toHaveProperty(
        'allowed',
        true,
      )
    })
    ...
    it('should track different endpoints independently', async () => {
      ...
      expect(await tracker.checkAndTrackRequest(endpoint1Key, _request1)).toHaveProperty(
        'allowed',
        false,
      )

      // endpoint2 should still be allowed
      expect(await tracker.checkAndTrackRequest(endpoint2Key, _request2)).toHaveProperty(
        'allowed',
        true,
      )
    })
  ...
  describe('Configuration', () => {
    ...
    it('should respect custom configuration', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        expect(await customTracker.checkAndTrackRequest(key, _request)).toHaveProperty(
          'allowed',
          true,
        )
      }
