...
import type { InventoryBatch, Product } from '@neonpro/types'
...
export class InventoryManagementService {
  ...
  async createProduct(
    ...
    const { data, error } = await this.supabase
      ...
      .insert([{
        ...
        profit_margin: ((productData.price - productData.price) / productData.price) * 100,
      }])
      .select()
      .single()
  ...
  async createInventoryBatch(
    batchData: Omit<InventoryBatch, 'id' | 'created_at' | 'updated_at'>,
  ...
  async checkBatchExpiry(
    ...
    { valid: InventoryBatch[]; expiringSoon: InventoryBatch[]; expired: InventoryBatch[] }
  > {
  ...
  async getInventorySummary(dateFrom?: string, dateTo?: string): Promise<InventorySummary> {
    ...
    const totalInventoryValue = products?.reduce((sum, product) => {
      ...
      return sum + ((product as any).selling_price * (stock?.current_stock || 0))
    }, 0) || 0
    ...
    const productUsage = usage?.reduce((acc, record) => {
      ...
      if (!acc[(record as any).product_id]) {
        acc[(record as any).product_id] = {
          productId: (record as any).product_id,
  ...
  async processDailyInventoryChecks(): Promise<void> {
    try {
      ...
      logHealthcareError('inventory', error as Error, {
        method: 'processDailyInventoryChecks',
        clinicId: this.config.clinicId,
      })
    }
  }

  async calculateReorderQuantities(): Promise<
    Array<{ productId: string; recommendedQuantity: number; reason: string }>
  > {
    const products = await this.getProducts()
    const recommendations: Array<
      { productId: string; recommendedQuantity: number; reason: string }
    > = []
    ...
    for (const product of products) {
      ...
      if (stockLevel.currentStock <= (product as any).reorder_point) {
        ...
        recommendations.push({
          ...
          reason: `Current stock ${stockLevel.currentStock} is below reorder point ${
            (product as any).reorder_point
          }. Lead time demand: ${leadTimeDemand.toFixed(1)}`,
        })
      }
    }
