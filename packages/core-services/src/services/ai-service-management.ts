...
export class AIServiceManagement {
   ...
   */
  async checkModelAvailability(provider?: string, model?: string): Promise<ModelAvailability[]> {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          availabilities.push({
            ...
            limitations: [`Availability check failed: ${result.reason instanceof Error ? result.reason.message : String(result.reason)}`],
          });
        }
      });
   ...
   */
  getCachedModelAvailability(provider: string, model: string): ModelAvailability | null {
   ...
   */
  getCachedUsageStats(provider: string, period: { start: Date; end: Date }): AIUsageStats | null {
  ...
  private async checkProviderHealth(providerName: string, aiProvider: AIProvider): Promise<AIServiceHealth> {
  ...
  private getModelsToCheck(provider?: string, model?: string): Array<{ provider: string; model: string }> {
  ...
  private async checkSingleModelAvailability(providerName: string, modelName: string): Promise<ModelAvailability> {
    try {
  ...
  private async getProviderUsageStats(providerName: string, period: { start: Date; end: Date }): Promise<AIUsageStats> {
    ...
    const totalRequests = dailyBreakdown.reduce((sum, day) => sum + day.requests, 0);
    const successfulRequests = Math.floor(totalRequests * 0.95);
    const failedRequests = totalRequests - successfulRequests;
    const totalTokensUsed = dailyBreakdown.reduce((sum, day) => sum + day.tokens, 0);
...
export async function checkModelAvailability(provider?: string, model?: string): Promise<ModelAvailability[]> {
