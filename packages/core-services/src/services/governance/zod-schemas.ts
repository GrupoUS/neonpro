...
const phiPatientSchema = z.object({
  ...
  dateOfBirth: z.date().refine(d => d.getTime() <= Date.now(), { message: 'Birth date cannot be in future' }),
  cpf: z.string().regex(/^\d{11}$/,{ message: 'CPF must be 11 digits' }).optional(),
  medicalRecordNumber: z.string().min(1),
  insuranceId: z.string().optional(),
  emergencyContact: z.object({
    name: z.string().min(1),
    phone: z.string().optional(),
    relationship: z.enum(['spouse','parent','child','sibling','other']).optional()
  }).optional()
})
...
const consentRecordSchema = z.object({
  ...
  version: z.string(),
  dataCategories: z.array(z.enum(['personal','health','behavioral','device'])).min(1),
...
const kpiSchema = z.object({
  kpiId: z.string().regex(/^KPI-[A-Z0-9_]+$/, { message: 'KPI ID must match pattern KPI-UPPER_SNAKE' }),
  value: z.number(),
  unit: z.enum(['percentage','count','ms','kb']).optional(),
  timestamp: z.date(),
  source: z.enum(['ehr_system','manual_entry','automated_calculation']).optional(),
  confidence: z.number().min(0).max(1).optional(),
  patientCohort: z.object({
    totalPatients: z.number().int().min(1),
    ageRange: z.object({ min: z.number().int().min(0), max: z.number().int().min(0) }).refine(r => r.min <= r.max, { message: 'ageRange.min must be <= max' }),
...
const aiGovernanceSchema = z.object({
  ...
  complianceFlags: z.array(z.enum(['hallucination_spike','performance_degradation','bias_concern'])).default([])
})

const riskAssessmentSchema = z.object({
  riskId: z.string().regex(/^RISK-[A-Z0-9_-]+$/),
  category: z.enum(['data_privacy','security','system_performance','clinical','operational','security','system_performance','data_privacy']),
...
// --- Validator Functions ---
export function validatePHI(data: unknown) { return COMPLIANCE_SCHEMAS.PHIPatient.parse(data) }
export function validateConsent(data: unknown) { return COMPLIANCE_SCHEMAS.ConsentRecord.parse(data) }
export function validateKPI(data: unknown) { return COMPLIANCE_SCHEMAS.HealthcareKPI.parse(data) }
export function validateAIGovernance(data: unknown) { return COMPLIANCE_SCHEMAS.AIEvaluation.parse(data) }
export function validateRisk(data: unknown) { return COMPLIANCE_SCHEMAS.RiskAssessment.parse(data) }

export function checkComplianceRequirements(schemaType: keyof typeof COMPLIANCE_SCHEMAS, data: unknown) {
  try {
    ...
    return { valid: false, data: null, errors: [{ message: err?.message || 'Unknown error' }] }
  }
}
