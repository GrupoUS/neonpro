...
import { InMemoryKPIService, InMemoryEscalationService } from '../../index'
...
describe('Scenario: KPI provisional aging escalation', () => {
  it('escalates after 61 days provisional', async () => {
    ...
    await kpiSvc.register({ id: 'KPI-NO_SHOW', name: 'No-show Rate', provisional: true })
    ...
    const receipt = await escSvc.trigger({ pathId: 'ESC-003', kpiId: 'KPI-NO_SHOW', reason: 'Provisional aging > 60 days' })
