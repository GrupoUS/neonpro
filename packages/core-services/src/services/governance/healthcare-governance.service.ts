...
export class HealthcareGovernanceService extends SupabaseGovernanceService implements IHealthcareGovernanceService {
  ...
  async createHealthcareMetric(metric: CreateHealthcareMetric): Promise<HealthcareMetric> {
    try {
  ...
  async updateHealthcareMetric(update: UpdateHealthcareMetric): Promise<HealthcareMetric> {
    try {
      ...
      if (update.riskLevel !== undefined) updateData.risk_level = update.riskLevel;
  ...
  async getPatientSafetyKPIs(clinicId: string): Promise<PatientSafetyKPI[]> {
    try {
      ...
      if (error) {
        throw new Error(`Failed to fetch patient safety KPIs: ${error.message}`);
      }
  ...
  async updatePatientSafetyKPI(id: string, updates: Partial<PatientSafetyKPI>): Promise<PatientSafetyKPI> {
    try {
      const updateData = {
        ...updates,
        updated_at: new Date().toISOString()
      ...
      if (error) {
        throw new Error(`Failed to update patient safety KPI: ${error.message}`);
      }
  ...
  async getHealthcarePolicies(filters?: HealthcarePolicyFilters): Promise<HealthcarePolicy[]> {
    try {
      let query = this.supabase
      ...
      if (filters?.applicableService) {
        query = query.contains('applicable_services', [filters.applicableService]);
      }

      const { data, error } = await query.order('created_at', { ascending: false });

      if (error) {
        throw new Error(`Failed to fetch healthcare policies: ${error.message}`);
      }
  ...
  async createHealthcarePolicy(policy: CreateHealthcarePolicy): Promise<HealthcarePolicy> {
    try {
      const { data, error } = await this.supabase
        .from('healthcare_policies')
        .insert({
  ...
  async updateHealthcarePolicy(id: string, updates: Partial<HealthcarePolicy>): Promise<HealthcarePolicy> {
    try {
      const updateData = {
        ...updates,
        updated_at: new Date().toISOString()
  ...
  async getHealthcareAlerts(clinicId: string, filters?: HealthcareAlertFilters): Promise<HealthcareAlert[]> {
    try {
      let query = this.supabase
  ...
  async createHealthcareAlert(alert: Omit<HealthcareAlert, 'id' | 'createdAt' | 'updatedAt'>): Promise<HealthcareAlert> {
    try {
      const { data, error } = await this.supabase
        .from('healthcare_alerts')
        .insert({
  ...
  async updateHealthcareAlert(id: string, updates: Partial<HealthcareAlert>): Promise<HealthcareAlert> {
    try {
      const updateData = {
        ...updates,
        updated_at: new Date().toISOString()
  ...
  async getComplianceReports(clinicId: string, filters?: ComplianceReportFilters): Promise<HealthcareComplianceReport[]> {
    try {
      let query = this.supabase
        .from('compliance_reports')
        .select('*')
  ...
  async getHealthcareDashboardData(clinicId: string): Promise<HealthcareDashboardData> {
    try {
      ...
      // Get active alerts
      const alerts = await this.getHealthcareAlerts(clinicId, { status: 'ACTIVE' });
      ...
      return {
        ...
        criticalAlerts,
        activeViolations: alerts.filter(alert => alert.alertType === 'COMPLIANCE_VIOLATION').length,
        patientSafetyScore,
        cfmComplianceStatus: {
  ...
  private mapHealthcareMetrics(data: any[]): HealthcareMetric[] {
    return data.map(item => this.mapHealthcareMetric(item));
  }

  private mapHealthcareMetric(data: any): HealthcareMetric {
    return {
      id: data.id,
  ...
  private mapPatientSafetyKPIs(data: any[]): PatientSafetyKPI[] {
    return data.map(item => this.mapPatientSafetyKPI(item));
  }

  private mapPatientSafetyKPI(data: any): PatientSafetyKPI {
    return {
      id: data.id,
      clinicId: data.clinic_id,
  ...
  private mapHealthcarePolicies(data: any[]): HealthcarePolicy[] {
    return data.map(item => this.mapHealthcarePolicy(item));
  }

  private mapHealthcarePolicy(data: any): HealthcarePolicy {
    return {
      id: data.id,
      name: data.name,
      description: data.description,
  ...
  private mapHealthcareAlerts(data: any[]): HealthcareAlert[] {
    return data.map(item => this.mapHealthcareAlert(item));
  }

  private mapHealthcareAlert(data: any): HealthcareAlert {
    return {
      id: data.id,
      clinicId: data.clinic_id,
  ...
  private mapComplianceReports(data: any[]): HealthcareComplianceReport[] {
    return data.map(item => ({
      ...
      nextAuditDate: new Date(item.next_audit_date),
  ...
  private determineComplianceStatus(score: number): 'COMPLIANT' | 'NON_COMPLIANT' | 'UNDER_REVIEW' | 'CRITICAL' {
  ...
  private async getViolationsCount(clinicId: string, period: { startDate: Date; endDate: Date }) {
  ...
  private generateRecommendations(metrics: HealthcareMetric[], violations: any): string[] {
    ...
    if (lowPerformingMetrics.length > 0) {
      recommendations.push(`Improve performance on ${lowPerformingMetrics.length} metrics below threshold`);
    }
    ...
    if (highRiskMetrics.length > 0) {
      recommendations.push(`Address ${highRiskMetrics.length} high-risk metrics immediately`);
    }

    // Check for violations
    if (violations.critical > 0) {
      recommendations.push(`Resolve ${violations.critical} critical compliance violations`);
    }
  ...
  private calculateNextAuditDate(reportType: HealthcareComplianceReport['reportType']): Date {
    const now = new Date();
    switch (reportType) {
