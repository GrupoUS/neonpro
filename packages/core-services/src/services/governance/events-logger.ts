...
export interface EventLogger {
  ...
  logKPIEvaluated(kpiId: string, value?: number, status?: string, metadata?: Record<string, any>): void
  logEscalationTriggered(escalationId: string, pathId: string, kpiId: string, reason: string): void
  logPriorityScored(featureId: string, score: number, priority: string, factors: Record<string, number>): void
  logPolicyEvaluated(policyId: string, status: string, passed: number, total: number): void
}
