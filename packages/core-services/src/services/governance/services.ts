...
// KPI Service -------------------------------------------------------------
export interface KPIRegisterInput { id: string; name: string; target?: number; direction?: 'lower-better' | 'higher-better'; unit?: string; provisional?: boolean }
export interface KPIRecord extends Omit<KPIRegisterInput,'provisional'> { status: 'Active' | 'Provisional' | 'Archived'; provisionalSince?: Date; archivedAt?: Date; archivedRationale?: string }
export interface KPIEvaluationInput { value: number; ts: Date; unit?: string }
export interface KPIEvaluationResult { kpiId: string; status: 'within' | 'breach'; value: number; target: number; direction: 'lower-better' | 'higher-better'; delta: number }
...
export class InMemoryKPIService {
  ...
  async register(input: KPIRegisterInput, _ctx?: ServiceContext): Promise<KPIRecord> {
    ...
    const rec: KPIRecord = { ...input, target: input.target ?? 0, direction: input.direction ?? 'lower-better', status, provisionalSince: input.provisional ? new Date() : undefined }
  ...
  async archive(input: { id: string; rationale: string }): Promise<KPIRecord> {
    ...
    const archived: KPIRecord = { ...rec, status: 'Archived', archivedAt: new Date(), archivedRationale: input.rationale }
  ...
  async list(): Promise<KPIRecord[]> { return [...this.store.values()] }
...
// Policy Service ----------------------------------------------------------
export interface PolicyRegisterInput { id: string; name: string; rules: PolicyRule[] }
...
export interface PolicyAttachmentInput { policyId: string; kpiId: string; thresholds: Record<string,string> }
export interface PolicyAttachmentRecord { policyId: string; kpiId: string; resolvedThresholds: Record<string,string>; attachedAt: Date }

export class InMemoryPolicyService {
  private store = new Map<string, PolicyRecord>()
  async register(input: PolicyRegisterInput): Promise<PolicyRecord> { this.store.set(input.id, input); return input }
  async evaluate(id: string): Promise<PolicyEvaluation> { const p = this.store.get(id); if (!p) throw new Error('POLICY_NOT_FOUND'); return aggregatePolicyRules(p.rules) }
  async list(): Promise<PolicyRecord[]> { return [...this.store.values()] }
}

// Escalation Service ------------------------------------------------------
export interface EscalationTriggerInput { pathId: string; kpiId: string; reason: string }
export interface EscalationRecord { id: string; pathId: string; kpiId: string; reason: string; createdAt: Date }
...
export class InMemoryEscalationService {
  ...
  async list(): Promise<EscalationRecord[]> { return [...this.records] }
}

// Prioritization Service --------------------------------------------------
export interface FeatureScoreInput { featureId: string; impact: number; effort: number; riskReduction: number; strategicFit: number }
export interface FeatureScoreResult { featureId: string; total: number; priority: 'P0' | 'P1' | 'P2' | 'P3' }
...
// Risk Service ------------------------------------------------------------
export interface RiskRegisterInput { id: string; probability: number; impact: number }
export interface RiskRecord extends RiskRegisterInput { exposure: number }
...
export class InMemoryRiskService {
  ...
  async list(): Promise<RiskRecord[]> { return [...this.store.values()] }
}
