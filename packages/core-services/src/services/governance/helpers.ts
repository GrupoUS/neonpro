...
export interface RiskExposureInput { probability: number; impact: number }
export const computeRiskExposure = ({ probability, impact }: RiskExposureInput) => probability * impact

export interface KPIEvalInput { value: number; target: number; direction: 'lower-better' | 'higher-better' }
export interface KPIEvalResult { status: 'within' | 'breach'; delta: number }
export const evaluateKPIValue = ({ value, target, direction }: KPIEvalInput): KPIEvalResult => {
...
export interface PriorityScoreFactors { impact: number; effort: number; riskReduction: number; strategicFit: number }
// Simple weighted scoring: higher impact, riskReduction, strategicFit better; lower effort better.
export const scorePriority = ({ impact, effort, riskReduction, strategicFit }: PriorityScoreFactors) => {
  const base = impact * 2 + riskReduction * 1.5 + strategicFit * 1.2 - effort
  return base
}

export interface PolicyRule { id: string; type: 'boolean'; evaluate: () => boolean }
export interface PolicyAggregationResult { total: number; passed: number; status: 'pass' | 'fail' | 'partial' }
export const aggregatePolicyRules = (rules: PolicyRule[]): PolicyAggregationResult => {
...
export const isProvisionalAging = (provisionalSince: Date | undefined, now: Date) => {
