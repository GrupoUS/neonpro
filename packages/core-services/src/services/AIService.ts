...
export interface ServiceContext { userId?: string }
...
export class AIService {
  ...
  async makePrediction(req: PredictionRequest, _ctx?: ServiceContext): Promise<PredictionResponse> {
    ...
    const inferredEnhanced = typeof daysSince === 'number' && typeof prevNoShows === 'number' && daysSince <= 2 && prevNoShows > 0
    ...
    if (enhanced) {
      ...
      return { type: req.type, confidence, enhanced: true, anonymizedFeatures: rest }
    }
