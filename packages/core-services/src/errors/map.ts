...
export class ErrorMapper {
  private static readonly DEFAULT_USER_MESSAGE = 'Um erro inesperado ocorreu. Nossa equipe foi notificada.';
  ...
  private static mapKnownError(error: Error, context?: ErrorContext, timestamp?: string): MappedError {
    ...
    if (errorName.includes('permission') || errorName.includes('auth') || 
    ...
    if (errorMessage.includes('consent') || errorMessage.includes('lgpd')) {
      return {
        ...
        userMessage: 'É necessário seu consentimento para continuar. Verifique as configurações de privacidade.',
        statusCode: 400,
        logLevel: 'info',
        shouldLog: true,
        metadata: {
    ...
    if (errorMessage.includes('openai') || errorMessage.includes('ai service') || 
        errorMessage.includes('model') || errorMessage.includes('llm')) {
      return {
        code: 'AI_SERVICE_ERROR',
        message: 'AI service temporarily unavailable',
        userMessage: 'O assistente IA está temporariamente indisponível. Tente novamente em alguns minutos.',
        statusCode: 503,
        logLevel: 'error',
        shouldLog: true,
        metadata: {
    ...
    if (errorMessage.includes('pii') || errorMessage.includes('redaction') || 
        errorMessage.includes('sensitive')) {
      return {
        code: 'DATA_PROTECTION_ERROR',
        message: 'Data protection violation detected',
        userMessage: 'Dados sensíveis detectados. Sua mensagem foi processada de acordo com as normas de proteção de dados.',
        statusCode: 400,
        logLevel: 'warn',
        shouldLog: true,
        metadata: {
  ...
  private static mapUnknownError(error: any, context?: ErrorContext, timestamp?: string): MappedError {
    return {
      code: 'INTERNAL_ERROR',
  ...
  static createAPIResponse(error: Error | unknown, context?: ErrorContext) {
    ...
    return {
      ...
      ...(context?.sessionId && { requestId: context.sessionId })
    };
  }
  ...
  static createLogEntry(error: Error | unknown, context?: ErrorContext) {
    ...
    return {
      ...
      ...(mapped.statusCode >= 500 && error instanceof Error && { stack: error.stack })
    };
  }
}
...
export const createConsentError = (userId?: string, clinicId?: string) => {
  ...
  return ErrorMapper.mapError(error, { userId, clinicId, action: 'consent_validation' });
};
...
export const createDataProtectionError = (userId?: string, sessionId?: string) => {
  const error = new Error('PII detected in user input');
  error.name = 'DataProtectionError';
  return ErrorMapper.mapError(error, { userId, sessionId, action: 'pii_detection' });
};
