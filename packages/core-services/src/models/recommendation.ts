...
export class RecommendationModel {
  ...
  generateRecommendations(): Recommendation[] {
    ...
    return Array.from(this._recommendations.values()).sort(_(a,_b) =>
  ...
  private generateQuotaUpgradeRecommendation(): PlanUpgradeRecommendation | null {
    ...
    return {
      ...
      unlockingFeatures: nextPlan.availableFeatures.filter(_(f) => !userPlan.plan.hasFeature(f),
      ),
      additionalModels: nextPlan.availableModels.filter(_(m) => !userPlan.plan.hasModelAccess(m),
      ),
  ...
  private generateCostOptimizationUpgradeRecommendation(): PlanUpgradeRecommendation | null {
    ...
    if (
      ...
      return {
        ...
        unlockingFeatures: enterprisePlan.availableFeatures.filter(_(f) => !userPlan.plan.hasFeature(f),
        ),
        additionalModels: enterprisePlan.availableModels.filter(_(m) => !userPlan.plan.hasModelAccess(m),
        ),
  ...
  private generateModelCostOptimization(): CostOptimizationRecommendation | null {
    ...
    const totalSavings = modelSuggestions.reduce(_(sum,_s) => sum + s.costSavings,
      0,
    );
  ...
  getRecommendationsByPriority(
    ...
    return Array.from(this._recommendations.values()).filter(_(rec) => rec.priority === priority,
    );
  }
  ...
  getActiveRecommendations(): Recommendation[] {
    return Array.from(this._recommendations.values()).filter(_(rec) =>
