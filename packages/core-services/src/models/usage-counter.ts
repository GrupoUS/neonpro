...
export class UsageCounter {
  ...
  get cacheSavingsPercentage(): number {
    ...
    return (this._data.cacheSavingsUsd / (this._data.totalCostUsd + this._data.cacheSavingsUsd)) * 100;
  }
  ...
  recordUsage(usage: {
    ...
    // Calculate cost
    const fullCost = calculateRequestCost(usage.modelCode, usage.inputTokens, usage.outputTokens, false);
    ...
    const usageRecord: AIUsageRecord = {
      ...
      auditTrail: {
        ...
        userId: this._data.userId,
        consentStatus: 'valid',
        dataProcessingPurpose: usage.medicalSpecialty ? 'diagnosis' : 'analytics',
    ...
    this._data = {
      ...
      averageLatencyMs: this.updateRollingAverage(this._data.averageLatencyMs, usage.latencyMs, this._data.totalRequests),
  ...
  getUsageInsights(): {
    ...
    if (insights.efficiency.cacheOptimization < 50) {
      insights.recommendations.push('Otimize consultas para melhorar cache hit rate');
    }
    
    if (insights.efficiency.costEfficiency < 30) {
      insights.recommendations.push('Considere usar modelos mais econômicos para consultas simples');
    }
    
    if (this.averageLatencyMs > 3000) {
      insights.recommendations.push('Latência alta detectada - verifique configuração de rede');
    }
    
    if (this._data.errorRate > 0.05) {
      insights.recommendations.push('Taxa de erro elevada - revise integrações');
    }
  ...
  getBillingMetrics(startDate: Date, endDate: Date): BillingMetrics {
    ...
    const totalCostUsd = relevantRequests.reduce((sum, req) => sum + req.costUsd, 0);
    const totalTokens = relevantRequests.reduce((sum, req) => sum + req.totalTokens, 0);
    const cacheSavingsUsd = relevantRequests.reduce((sum, req) => sum + req.cacheSavingsUsd, 0);
    ...
    return {
      ...
      cacheSavingsUsd,
      cacheSavingsPercentage: totalCostUsd > 0 ? (cacheSavingsUsd / (totalCostUsd + cacheSavingsUsd)) * 100 : 0,
      averageCostPerRequest: relevantRequests.length > 0 ? totalCostUsd / relevantRequests.length : 0,
      averageTokensPerRequest: relevantRequests.length > 0 ? totalTokens / relevantRequests.length : 0,
      period: {
        start: startDate,
        end: endDate,
      },
   ...
   */
  getAggregations(period: 'hour' | 'day' | 'week' | 'month'): UsageAggregation[] {
  ...
  private updateRollingAverage(currentAvg: number, newValue: number, totalCount: number): number {
  ...
  private updateCacheHitRate(cacheHit: boolean): number {
    const recentCacheHits = this._recentRequests.filter(req => req.cacheHit).length;
  ...
  private updateAggregations(usageRecord: AIUsageRecord): void {
    ...
    periods.forEach(period => {
      ...
      if (existing) {
        ...
        if (usageRecord.medicalSpecialty && existing.specialtyBreakdown[usageRecord.medicalSpecialty]) {
        ...
        const aggregation: UsageAggregation = {
          ...
          uniqueUsers: 1,
          modelBreakdown: { [usageRecord.modelCode]: 1 } as Record<EnhancedAIModel, number>,
          specialtyBreakdown: usageRecord.medicalSpecialty 
            ? { [usageRecord.medicalSpecialty]: 1 } as Record<MedicalSpecialty, number>
   ...
   */
  static createNew(clinicId: string, userId: string, planCode: SubscriptionTier): UsageCounter {
    const now = new Date();
