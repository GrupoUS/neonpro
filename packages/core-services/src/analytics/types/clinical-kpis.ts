...
export interface PatientSafetyKPI extends ClinicalKPI {
  ...
  safetyContext: {
    eventType: 'fall' | 'medication_error' | 'surgical_complication' | 'infection' | 'other';
...
export function createPatientSafetyKPI(params: {
  ...
  return {
    ...
    aggregation: 'sum',
    status: 'active',
    riskLevel: params.severity === 'life_threatening' ? 'CRITICAL' : 
               params.severity === 'severe' ? 'HIGH' : 'MEDIUM',
...
export function validateClinicalCompliance(kpi: ClinicalKPI): {
  ...
  if (kpi.complianceFrameworks.includes('CFM')) {
    ...
    if (kpi.category === 'patient_safety' && kpi.riskLevel === 'CRITICAL') {
      recommendations.push('CFM notification required for critical safety events');
    }
  }
 ...
 */
export function createQualityOfCareKPI(options: Partial<QualityOfCareKPI>): QualityOfCareKPI {
  const now = new Date();
 ...
 */
export function createPatientOutcomeKPI(options: Partial<PatientOutcomeKPI>): PatientOutcomeKPI {
  const now = new Date();
...
export function calculateClinicalRiskScore(kpis: ClinicalKPI[]): {
  ...
  if (averageScore >= 80) {
    ...
    overallRisk = 'MEDIUM';
    recommendations.push('Regular monitoring and preventive measures recommended');
  }
