...
describe('Clinical KPIs', () => {
  ...
  describe('Quality of Care KPI', () => {
    ...
    it('should include evidence-based practice tracking', () => {
      ...
      expect(qualityKPI.evidenceBasedPractice.guidelineCompliance).toBeGreaterThanOrEqual(0);
      expect(qualityKPI.evidenceBasedPractice.guidelineCompliance).toBeLessThanOrEqual(100);
    });

    it('should track patient-centered care metrics', () => {
      expect(qualityKPI.patientCenteredCare).toBeDefined();
      expect(qualityKPI.patientCenteredCare.sharedDecisionMaking).toBeGreaterThanOrEqual(0);
      expect(qualityKPI.patientCenteredCare.culturalSensitivity).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  describe('Patient Outcome KPI', () => {
    ...
    it('should calculate outcome improvement correctly', () => {
      const improvement = outcomeKPI.measurement.current - outcomeKPI.measurement.baseline;
      expect(improvement).toBe(7.5);
    });

    it('should track progress toward target', () => {
      const progressToTarget = (outcomeKPI.measurement.current - outcomeKPI.measurement.baseline) /
  ...
  describe('Clinical Quality Score Calculation', () => {
    it('should calculate comprehensive clinical quality score', () => {
      ...
      expect(qualityScore.level).toMatch(/^(excellent|good|fair|needs_improvement)$/);
  ...
  describe('Clinical Compliance Validation', () => {
    ...
    it('should validate CFM compliance for medical practice', () => {
      ...
      expect(compliance.requirements).toContain('Medical practice must follow CFM ethical guidelines');
      expect(compliance.recommendations).toContain('Regular clinical audit recommended');
    });
    ...
    it('should validate LGPD compliance for patient data', () => {
      ...
      expect(compliance.requirements).toContain('Patient data consent required for outcome tracking');
      expect(compliance.recommendations).toContain('Consider anonymization for quality improvement analytics');
    });
  });
  ...
  describe('Brazilian Healthcare Context', () => {
    ...
    it('should track ANS (Agência Nacional de Saúde Suplementar) quality indicators', () => {
      ...
      expect(ansKPI.healthcareContext.payerMix?.private_insurance).toBeDefined();
    });
  });
});