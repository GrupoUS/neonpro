...
import {
  ...
  createMockEvents,
  type ComputedKPIs,
  type KPIComputationOptions,
} from "../../aggregation/kpis";
...
describe("computeKPIs", () => {
  ...
  describe("Filtering functionality", () => {
    ...
    it("should filter events by sources", () => {
      ...
      expect(result.metadata.eventCount).toBeLessThanOrEqual(
  ...
  describe("Clinical quality KPIs", () => {
    it("should compute clinical quality metrics correctly", () => {
      ...
      expect(result.clinicalQuality.diagnosisAccuracy).toBeGreaterThanOrEqual(
        0,
      );
      expect(
        result.clinicalQuality.emergencyInterventions,
  ...
  describe("Financial KPIs", () => {
    it("should compute financial metrics correctly", () => {
      ...
      expect(result.financial.insuranceClaimSuccessRate).toBeGreaterThanOrEqual(
        0,
      );
    });
  ...
  describe("Time range computation", () => {
    it("should compute time range from events when not provided", () => {
      ...
      expect(result.metadata.timeRange.start.getTime()).toBeLessThanOrEqual(
        oneHourAgo.getTime(),
      );
      expect(result.metadata.timeRange.end.getTime()).toBeGreaterThanOrEqual(
        now.getTime(),
      );
    });
    ...
    it("should handle same start and end time for single event", () => {
      ...
      expect(result.metadata.timeRange.start.getTime()).toBe(
        singleTime.getTime(),
      );
      expect(result.metadata.timeRange.end.getTime()).toBe(
        singleTime.getTime(),
      );
    });
  });
  ...
  describe("Complex scenarios", () => {
    it("should handle comprehensive healthcare workflow", () => {
      const workflowEvents: AnalyticsEvent[] = [
        ...
        {
          id: "3",
          type: "treatment_prescribed",
          timestamp: new Date(),
          properties: { patientId: "p1", medicationType: "antibiotics" },
        },
        {
          id: "4",
          type: "revenue_generated",
        ...
        },
        {
          ...
          type: "patient_feedback",
          timestamp: new Date(),
          properties: { patientId: "p1", rating: 5 },
        },
        ...
        // System events
        {
          ...
          type: "system_performance",
          timestamp: new Date(),
          properties: { score: 98, uptime: 99.9 },
        },
        {
          id: "7",
          type: "resource_utilization",
          timestamp: new Date(),
          properties: { utilization: 85 },
        },
