...
export function computeKPIs(
  ...
  if (options?.validation?.requireMinEvents && filteredEvents.length < options.validation.requireMinEvents) {
    throw new Error(`Insufficient events: ${filteredEvents.length} < ${options.validation.requireMinEvents}`);
  }
...
function filterEvents(
  ...
  if (options?.sources && options.sources.length > 0) {
    filtered = filtered.filter(event => {
      if ('source' in event && typeof event.source === 'object' && event.source !== null) {
...
function computePatientFlowKPIs(
  ...
  const totalVisits = patientEvents.filter(e => 
    e.type === 'patient_visit' || e.type === 'appointment_completed'
  ).length;
  
  // Compute average wait time (mock calculation)
  const waitTimeEvents = patientEvents.filter(e => e.properties.waitTime);
  const averageWaitTime = waitTimeEvents.length > 0 
    ? waitTimeEvents.reduce((sum, e) => sum + (e.properties.waitTime as number), 0) / waitTimeEvents.length
    : 0;
  
  // Compute no-show rate
  const appointmentEvents = patientEvents.filter(e => e.type.includes('appointment'));
  const noShowEvents = appointmentEvents.filter(e => e.properties.status === 'no_show');
  const noShowRate = appointmentEvents.length > 0 
  ...
  const patientSatisfactionScore = satisfactionEvents.length > 0
    ? satisfactionEvents.reduce((sum, e) => sum + (e.properties.rating as number || 0), 0) / satisfactionEvents.length
    : 0;
...
function computeClinicalQualityKPIs(
  ...
  const diagnosisEvents = clinicalEvents.filter(e => e.type.includes('diagnosis'));
  const accurateDiagnoses = diagnosisEvents.filter(e => e.properties.accuracy === 'correct');
  const diagnosisAccuracy = diagnosisEvents.length > 0 
    ? (accurateDiagnoses.length / diagnosisEvents.length) * 100 
    : 0;
  
  // Compute treatment completion rate
  const treatmentEvents = clinicalEvents.filter(e => e.type.includes('treatment'));
  const completedTreatments = treatmentEvents.filter(e => e.properties.status === 'completed');
  const treatmentCompletionRate = treatmentEvents.length > 0
  ...
  const readmissionRate = admissionEvents.length > 0
  ...
  const emergencyInterventions = events.filter(e => 
    e.type === 'emergency_intervention' || e.properties.urgency === 'emergency'
  ).length;
  ...
  const medicationAdherence = medicationEvents.length > 0
...
function computeOperationalKPIs(
  ...
  const resourceEvents = operationalEvents.filter(e => e.type.includes('resource'));
  const utilizationSum = resourceEvents.reduce((sum, e) => sum + (e.properties.utilization as number || 0), 0);
  const resourceUtilization = resourceEvents.length > 0 ? utilizationSum / resourceEvents.length : 0;
  
  // Compute staff efficiency
  const staffEvents = operationalEvents.filter(e => e.type.includes('staff'));
  const efficiencySum = staffEvents.reduce((sum, e) => sum + (e.properties.efficiency as number || 0), 0);
  const staffEfficiency = staffEvents.length > 0 ? efficiencySum / staffEvents.length : 0;
  
  // Compute equipment usage
  const equipmentEvents = operationalEvents.filter(e => e.type.includes('equipment'));
  const usageSum = equipmentEvents.reduce((sum, e) => sum + (e.properties.usage as number || 0), 0);
  const equipmentUsage = equipmentEvents.length > 0 ? usageSum / equipmentEvents.length : 0;
  
  // Compute scheduling efficiency
  const scheduleEvents = operationalEvents.filter(e => e.type.includes('schedule'));
  const schedulingSum = scheduleEvents.reduce((sum, e) => sum + (e.properties.efficiency as number || 0), 0);
  const schedulingEfficiency = scheduleEvents.length > 0 ? schedulingSum / scheduleEvents.length : 0;
  
  // Compute average service time
  const serviceEvents = events.filter(e => e.properties.serviceTime);
  const serviceTimeSum = serviceEvents.reduce((sum, e) => sum + (e.properties.serviceTime as number), 0);
  const averageServiceTime = serviceEvents.length > 0 ? serviceTimeSum / serviceEvents.length : 0;
...
function computeFinancialKPIs(
  ...
  const revenueEvents = financialEvents.filter(e => e.type.includes('revenue'));
  const totalRevenue = revenueEvents.reduce((sum, e) => sum + (e.properties.amount as number || 0), 0);
  const uniquePatients = new Set(revenueEvents.map(e => e.properties.patientId)).size;
  ...
  const totalCosts = costEvents.reduce((sum, e) => sum + (e.properties.amount as number || 0), 0);
  ...
  const claimEvents = financialEvents.filter(e => e.type.includes('insurance_claim'));
  const successfulClaims = claimEvents.filter(e => e.properties.status === 'approved');
  const insuranceClaimSuccessRate = claimEvents.length > 0
    ? (successfulClaims.length / claimEvents.length) * 100
    : 0;
  
  // Compute payment collection rate
  const paymentEvents = financialEvents.filter(e => e.type.includes('payment'));
  const collectedPayments = paymentEvents.filter(e => e.properties.status === 'collected');
  const paymentCollectionRate = paymentEvents.length > 0
...
function computeSystemKPIs(
  ...
  const ingestionEvents = events.filter(e => 'quality' in e) as IngestionEvent[];
  ...
  const dataQualityScore = ingestionEvents.length > 0 ? (qualitySum / ingestionEvents.length) * 100 : 100;
  
  // Compute compliance score (mock calculation)
  const complianceEvents = events.filter(e => e.properties.compliance);
  const complianceSum = complianceEvents.reduce((sum, e) => sum + (e.properties.compliance as number), 0);
  const complianceScore = complianceEvents.length > 0 ? complianceSum / complianceEvents.length : 100;
  
  // Compute system uptime
  const uptimeEvents = systemEvents.filter(e => e.type.includes('uptime'));
  const uptimeSum = uptimeEvents.reduce((sum, e) => sum + (e.properties.uptime as number || 100), 0);
  const systemUptime = uptimeEvents.length > 0 ? uptimeSum / uptimeEvents.length : 100;
  
  // Compute error rate
  const errorEvents = systemEvents.filter(e => e.type.includes('error'));
  const totalSystemEvents = systemEvents.length;
  const errorRate = totalSystemEvents > 0 ? (errorEvents.length / totalSystemEvents) * 100 : 0;
  
  // Compute performance score
  const performanceEvents = systemEvents.filter(e => e.type.includes('performance'));
  const performanceSum = performanceEvents.reduce((sum, e) => sum + (e.properties.score as number || 100), 0);
  const performanceScore = performanceEvents.length > 0 ? performanceSum / performanceEvents.length : 100;
