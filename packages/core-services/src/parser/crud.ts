...
export class CrudIntentParser {
  ...
  private entityPatterns = {
    ...
    PHONE: /\+?[\d\s\-()]{10,}/g,
    DATE:
      /\b(\d{4}-\d{2}-\d{2}|\d{2}\/\d{2}\/\d{4}|\d{1,2}\/\d{1,2}\/\d{2,4}|today|tomorrow|yesterday|ontem|hoje|amanhã|2024-\d{2}-\d{2}|last\s+week|next\s+week|semana\s+passada|próxima\s+semana)\b/gi,
