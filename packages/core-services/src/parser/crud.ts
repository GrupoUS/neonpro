...
export interface EntityArgument {
  type: 'PERSON' | 'EMAIL' | 'PHONE' | 'DATE' | 'ENTITY_TYPE' | 'FIELD' | 'VALUE' | 'CLINIC' | 'PROCEDURE' | 'APPOINTMENT';
  value: string;
...
export interface FilterArgument {
  field: string;
  operator: 'equals' | 'not_equals' | 'greater_than' | 'less_than' | 'between' | 'like' | 'in' | 'not_in';
...
export class CrudIntentParser {
  ...
  private intentPatterns = {
    CREATE: {
      ...
      pt: ['criar', 'adicionar', 'inserir', 'fazer', 'novo', 'nova', 'registrar', 'cadastrar']
    },
    READ: {
      en: ['get', 'find', 'show', 'list', 'retrieve', 'search', 'select', 'fetch', 'display'],
      pt: ['buscar', 'encontrar', 'mostrar', 'listar', 'recuperar', 'pesquisar', 'selecionar', 'exibir', 'ver']
    },
    UPDATE: {
    ...
    DELETE: {
      ...
      pt: ['deletar', 'remover', 'excluir', 'cancelar', 'destruir', 'eliminar', 'apagar']
    }
  };
  ...
  private entityPatterns = {
    ...
    ENTITY_TYPE: /\b(patient|patients|user|users|clinic|clinics|appointment|appointments|doctor|doctors|paciente|pacientes|usuario|usuarios|clinica|clinicas|agendamento|agendamentos|medico|medicos|consulta|consultas)\b/gi,
    FIELD: /\b(status|age|email|name|id|created_at|updated_at|phone|address|nome|idade|telefone|endereco|created|updated)\b/gi,
    VALUE: /(?:=|equals?|is)\s*['""]?([^'""]+)['""]?|(?:status\s*=\s*|age\s*=\s*|name\s*=\s*)([^\s]+)/gi,
    PROCEDURE: /\b(botox|filler|laser|peeling|facial|limpeza|hidratação|rejuvenescimento)\b/gi,
  ...
  constructor(config: CrudIntentParserConfig = {}) {
    ...
    if (this.config.customEntityTypes.length > 0) {
      const customPattern = new RegExp(`\\b(${this.config.customEntityTypes.join('|')})\\b`, 'gi');
  ...
  parseIntent(input: string): ParsedIntent {
    ...
    const text = input.length > this.config.maxTextLength 
  ...
  private detectIntent(normalizedText: string): { type: CrudIntent; confidence: number } {
    const words = normalizedText.split(/\s+/);
    let bestMatch: { type: CrudIntent; confidence: number } = { type: 'UNKNOWN', confidence: 0 };
    
    // Check for uncertainty words that reduce confidence
    const uncertaintyWords = ['maybe', 'perhaps', 'possibly', 'might', 'could', 'talvez', 'possivelmente', 'poderia'];
    ...
    for (const [intentType, patterns] of Object.entries(this.intentPatterns)) {
        ...
        for (const pattern of langPatterns) {
          ...
          if (pattern.length >= 3) {
            ...
            const partialMatches = words.filter(word => 
              word.length >= 3 && (word.includes(pattern) || pattern.includes(word))
            );
            ...
            const startMatches = words.filter(word => 
              word.length >= 3 && (word.startsWith(pattern) || pattern.startsWith(word))
            );
      ...
      if (maxConfidence > 0.3) {
        if (intentType === 'READ' && (normalizedText.includes('where') || normalizedText.includes('select'))) {
        ...
        if (intentType === 'CREATE' && (normalizedText.includes('into') || normalizedText.includes('values'))) {
          maxConfidence = Math.max(maxConfidence, 0.85);
        }
      }

      if (maxConfidence > bestMatch.confidence) {
        bestMatch = { type: intentType as CrudIntent, confidence: maxConfidence };
      }
    }
  ...
  private extractEntities(text: string): EntityArgument[] {
    ...
    if (this.config.customEntityTypes) {
      for (const customType of this.config.customEntityTypes) {
        ...
        const customPattern = new RegExp(`\\b(${customType.toLowerCase()}s?|${customType.toLowerCase()})\\b`, 'gi');
    ...
    const tablePattern = /(?:show|find|get|list|select|from|update|delete|create|insert\s+into|ver|mostrar|buscar|listar)\s+(\w+)/gi;
  ...
  private extractFilters(text: string): FilterArgument[] {
    ...
    const filterPattern = /\b(\w+)\s+(equals?|is|=|!=|<>|>|<|>=|<=|greater\s+than|less\s+than|like|between|in|not\s+in|não\s+é|maior\s+que|menor\s+que|entre)\s+(['"]?[\w@.\-+]+['"]?|\d+(?:\.\d+)?)/gi;
    ...
    const statusPattern = /\b(active|inactive|deleted|archived|published|draft|pending|completed|failed|success|enabled|disabled)\s+(patients|users|appointments|records|items)/gi;
    let statusMatch;
    ...
    return filters.filter((filter, index, self) => 
      ...
      index === self.findIndex(f => f.field === filter.field && f.value === filter.value)
    );
  }

  private extractSort(text: string): SortArgument | undefined {
    // Pattern for "ordered by field direction" or "sort by field direction"
    const sortPattern = /(?:ordered?\s+by|sort\s+by)\s+(\w+)(?:\s+(asc|desc|ascending|descending))?/gi;
    const match = sortPattern.exec(text);
    
    if (match) {
      const field = match[1];
      const directionText = match[2] || 'asc';
      const direction = directionText.toLowerCase().startsWith('desc') ? 'desc' : 'asc';
  ...
  private mapOperator(operatorText: string): FilterArgument['operator'] {
    ...
    if (/^(not\s+equals?|!=|<>|não\s+é|diferente)$/.test(text)) return 'not_equals';
 ...
 */
export function createCrudIntentParser(config: CrudIntentParserConfig = {}): CrudIntentParser {
