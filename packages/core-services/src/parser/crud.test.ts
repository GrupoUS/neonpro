...
describe('CrudIntentParser', () => {
  ...
  describe('Filter Argument Extraction', () => {
    it('should extract comparison filters', () => {
      const inputs = [
        {
          text: 'age greater than 30',
          expected: { field: 'age', operator: 'greater_than', value: '30' },
        },
        {
          text: 'price less than 100',
          expected: { field: 'price', operator: 'less_than', value: '100' },
        },
        {
          text: 'status equals active',
          expected: { field: 'status', operator: 'equals', value: 'active' },
        },
      ]
  ...
  describe('Complex Query Parsing', () => {
    it('should parse complex queries with multiple arguments', () => {
      const input =
        'find patients by age greater than 30 in São Paulo, sort by name ascending, limit to 10'
      const result = parser.parseIntent(input)
