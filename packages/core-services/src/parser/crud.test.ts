...
describe('CrudIntentParser', () => {
  ...
  describe('Entity Extraction', () => {
    it('should extract entities from CREATE intents', () => {
      const input = 'create a new patient record for John Doe with email john@doe.com';
      const result = parser.parseIntent(input);
      
      expect(result.entities.length).toBeGreaterThan(0);
      expect(result.entities.some(e => e.type === 'PERSON' && e.value === 'John Doe')).toBe(true);
      expect(result.entities.some(e => e.type === 'EMAIL' && e.value === 'john@doe.com')).toBe(true);
      expect(result.entities.some(e => e.type === 'ENTITY_TYPE' && e.value === 'patient')).toBe(true);
    });

    it('should extract entities from READ intents with filters', () => {
      const input = 'get all patients where status equals active and age is greater than 18';
      const result = parser.parseIntent(input);
      
      expect(result.entities.some(e => e.type === 'ENTITY_TYPE' && e.value === 'patients')).toBe(true);
      expect(result.entities.some(e => e.type === 'FIELD' && e.value === 'status')).toBe(true);
      expect(result.entities.some(e => e.type === 'VALUE' && e.value === 'active')).toBe(true);
      expect(result.entities.some(e => e.type === 'FIELD' && e.value === 'age')).toBe(true);
      expect(result.entities.some(e => e.type === 'VALUE' && e.value === '18')).toBe(true);
    });
    ...
    it('should extract email and phone entities', () => {
      const input = 'find patient with email admin@clinic.com or phone +5511999887766';
      const result = parser.parseIntent(input);
      
      expect(result.entities.some(e => e.type === 'EMAIL' && e.value === 'admin@clinic.com')).toBe(true);
      expect(result.entities.some(e => e.type === 'PHONE' && e.value === '+5511999887766')).toBe(true);
    });
  });

  describe('Argument Extraction', () => {
    it('should extract filter arguments from WHERE clauses', () => {
      const input = 'get patients where status equals active and age greater than 25';
      const result = parser.parseIntent(input);
  ...
  describe('Complex Query Parsing', () => {
    it('should parse complex multi-filter queries', () => {
      const input = 'find active patients with age between 25 and 65, created after 2024-01-01, ordered by name ascending, limit 50';
      const result = parser.parseIntent(input);
      ...
      expect(result.arguments.filters.some(f => f.field === 'status' && f.value === 'active')).toBe(true);
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle special characters and punctuation', () => {
      const input = "get patients where email = 'test@example.com' and status != 'inactive'!";
      const result = parser.parseIntent(input);
      expect(result.intent).toBe('READ');
    });
  });
