  ...
  static validateConsent(consentRef?: ConsentReference): ComplianceViolation[] {
    ...
    if (!consentRef) {
      violations.push({
        ...
        severity: "HIGH",
        description:
          "LGPD violation: No consent reference provided for data processing",
        remediation:
          "Obtain and record explicit consent before processing personal data",
    ...
    if (consentRef.status !== "ACTIVE") {
      violations.push({
        ...
        remediation:
          "Ensure valid, active consent before processing personal data",
      } as ComplianceViolation);
    }
    ...
    if (["PRESCRIBE", "DIAGNOSE"].includes(action)) {
      if (!["DOCTOR", "NURSE"].includes(actor.type)) {
        violations.push({
          ...
          remediation:
            "Ensure only licensed medical professionals perform medical actions",
        } as ComplianceViolation);
      }
      ...
      if (!actor.email || !actor.name) {
        violations.push({
          ...
          severity: "HIGH",
          description:
            "ANVISA violation: Incomplete medical professional identification",
          remediation:
            "Ensure complete identification (name, email, license) for medical actions",
        } as ComplianceViolation);
      }
    }
    ...
    if (action === "DIAGNOSE" && actor.type === "DOCTOR") {
      if (!metadata?.telemedicineAuthorized) {
        violations.push({
          ...
          severity: "HIGH",
          description:
            "CFM violation: Telemedicine diagnosis without proper authorization",
      ...
      if (!consentRef || consentRef.type !== "telemedicine") {
        violations.push({
          ...
          severity: "HIGH",
          description:
            "CFM violation: Telemedicine without explicit patient consent",
          remediation:
            "Obtain explicit patient consent for telemedicine consultation",
        } as ComplianceViolation);
      }
    }
    ...
    if (action === "PRESCRIBE") {
      ...
      if (!metadata?.prescriptionId || !metadata?.patientId) {
        violations.push({
          ...
          remediation:
            "Include prescription ID and patient ID in prescription records",
        } as ComplianceViolation);
      }
    }
