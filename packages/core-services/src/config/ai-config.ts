  ...
  const config: AIConfig = {
    ...
    providers: {
      ...
      anthropic: {
        ...
        models: {
          default: process.env.ANTHROPIC_DEFAULT_MODEL || 'claude-3-sonnet-20240229',
          streaming: process.env.ANTHROPIC_STREAMING_MODEL || 'claude-3-sonnet-20240229',
    ...
    security: {
      ...
      allowedDomains: (process.env.AI_ALLOWED_DOMAINS || '').split(',').filter(Boolean),
    },
  };
...
export function getBestProvider(config: AIConfig, requirements?: {
  streaming?: boolean;
  ...
  if (requirements.speed === 'quality' && enabledProviders.includes('anthropic')) {
    return 'anthropic';
  }
