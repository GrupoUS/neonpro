...
export class MedicalEthicsService {
     ...
     */
    async conductEthicsAssessment(params, tenantId, assessorId) {
        try {
            // Constitutional validation of CFM number
            const cfmValidation = await this.validateDoctorForEthicsAssessment(params.doctor_cfm_number);
     ...
     */
    async validateDoctorForEthicsAssessment(cfmNumber) {
        try {
     ...
     */
    async assessPatientAutonomy(params) {
        const issues = [];
        const violations = [];
        ...
        try {
            ...
            const sharedDecisionRecords = consentRecords?.filter((record) => record.shared_decision_making === true) || [];
            if (consentRecords &&
            ...
            const patientDecisionRespectIssues = consentRecords?.filter((record) => record.patient_decision_overridden === true) || [];
            if (patientDecisionRespectIssues.length >
            ...
            const compliant = finalScore >= 9.0 && violations.length === 0;
            return {
                compliant,
                score: finalScore,
                issues,
     ...
     */
    async assessProfessionalConduct(params) {
        const issues = [];
        const violations = [];
        const correctiveActions = [];
        let score = 10.0;
        try {
            ...
            const boundaryViolations = conductRecords?.filter((record) => record.boundary_violation === true) || [];
            ...
            const unprofessionalConduct = conductRecords?.filter((record) => record.unprofessional_behavior === true) || [];
            ...
            const confidentialityBreaches = conductRecords?.filter((record) => record.confidentiality_breach === true) || [];
            ...
            const finalScore = Math.max(score, 0);
            const compliant = finalScore >= 9.0 &&
     ...
     */
    async assessMedicalAdvertising(params) {
        const issues = [];
        const violations = [];
        const correctiveActions = [];
        let score = 10.0;
        try {
            ...
            const prohibitedContent = advertisingRecords.filter((record) => record.contains_prohibited_content === true);
            ...
            const misleadingClaims = advertisingRecords.filter((record) => record.contains_misleading_claims === true);
            ...
            const missingDisclaimers = advertisingRecords.filter((record) => record.has_proper_disclaimers === false);
            if (missingDisclaimers.length > 0) {
                issues.push('Missing proper disclaimers in advertising');
                score -= 1.0;
                correctiveActions.push('Add required disclaimers to all advertising materials');
            }
            // Check for CFM number inclusion
            const missingCfmNumber = advertisingRecords.filter((record) => record.includes_cfm_number === false);
     ...
     */
    async assessConflictOfInterest(params) {
        const issues = [];
        const violations = [];
        const correctiveActions = [];
        let score = 10.0;
        try {
            ...
            const undisclosedConflicts = conflictRecords?.filter((record) => record.conflict_disclosed === false &&
            ...
            const pharmaRelationships = conflictRecords?.filter((record) => record.financial_relationship_pharma === true) || [];
            if (pharmaRelationships.length > 0) {
                const disclosedPharmaRelationships = pharmaRelationships.filter((record) => record.conflict_disclosed === true);
                if (disclosedPharmaRelationships.length < pharmaRelationships.length) {
                    issues.push('Undisclosed pharmaceutical relationships');
                    score -= 2.0;
                    correctiveActions.push('Disclose all pharmaceutical industry relationships');
                }
            }
            // Check for research conflicts
            const researchConflicts = conflictRecords?.filter((record) => record.research_conflict === true) || [];
            if (researchConflicts.length > 0) {
                const properlyManagedResearch = researchConflicts.filter((record) => record.conflict_properly_managed === true);
            ...
            const currentYearDisclosures = conflictRecords?.filter((record) => new Date(record.disclosure_date).getFullYear() === currentYear) || [];
            if (currentYearDisclosures.length === 0 &&
                conflictRecords &&
     ...
     */
    async assessContinuingEducation(params) {
        const issues = [];
        const violations = [];
        const correctiveActions = [];
        let score = 10.0;
        try {
            ...
            for (const year of lastThreeYears) {
                const yearlyCredits = educationRecords?.filter((record) => new Date(record.completion_date).getFullYear() === year) || [];
            ...
            const ethicsEducation = educationRecords?.filter((record) => record.course_category?.toLowerCase().includes('ética') ||
                record.course_category?.toLowerCase().includes('ethics')) || [];
            const recentEthicsEducation = ethicsEducation.filter((record) => new Date(record.completion_date).getFullYear() >= currentYear - 2);
            ...
            const { data: licenseData } = await this.supabase
                ...
                .single();
            if (licenseData?.specializations &&
                licenseData.specializations.length > 0) {
                const specialtyEducation = educationRecords?.filter((record) => licenseData.specializations.some((spec) => record.course_category?.toLowerCase().includes(spec.toLowerCase()))) || [];
                const recentSpecialtyEducation = specialtyEducation.filter((record) => new Date(record.completion_date).getFullYear() >= currentYear - 1);
                if (recentSpecialtyEducation.length === 0) {
                    issues.push('No recent specialty-specific education');
                    score -= 0.5;
                    correctiveActions.push('Complete specialty-specific continuing education');
                }
            }
            // Check certification renewals
            const certificationRenewals = educationRecords?.filter((record) => record.course_type === 'certification_renewal') || [];
            const recentRenewals = certificationRenewals.filter((record) => new Date(record.completion_date).getFullYear() >= currentYear - 5);
     ...
     */
    calculateEthicsComplianceScore(assessmentResults) {
        try {
     ...
     */
    async storeEthicsAssessment(params, complianceResponse, tenantId, assessorId) {
        try {
     ...
     */
    async getEthicsAssessments(tenantId, filters) {
        try {
            ...
            if (filters?.assessment_date_range) {
                query = query
                    .gte('assessment_date', filters.assessment_date_range.start.toISOString())
                    .lte('assessment_date', filters.assessment_date_range.end.toISOString());
            }
     ...
     */
    async generateEthicsComplianceReport(tenantId) {
        try {
            ...
            const report = {
                ...
                constitutional_compliance: {
                    ...
                    constitutional_compliance_rate: totalAssessments > 0
                        ? (assessmentStats.filter((a) => a.assessment_results?.constitutional_compliance).length /
                            totalAssessments) *
                            100
                        : 0,
                },
     ...
     */
    getMostCommonViolations(violations) {
