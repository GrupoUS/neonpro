...
export { DigitalSignatureService, } from './digital-signature-service';
// CFM Medical Ethics Services
export { MedicalEthicsService, } from './medical-ethics-service';
// CFM Medical Records Services
export { MedicalRecordsService, } from './medical-records-service'; // CFM Utilities and Constants
// CFM Professional Licensing Services
export { ProfessionalLicensingService, } from './professional-licensing-service';
// CFM Telemedicine Compliance Services
export { TelemedicineComplianceService, } from './telemedicine-compliance-service';
 ...
 */
export function createCfmServices(supabaseClient) {
    return {
 ...
 */
export async function validateCfmCompliance(tenantId, services) {
    const issues = [];
    ...
    try {
        ...
        const { data: telemedicineConsultations } = await services.telemedicineCompliance.getTelemedicineConsultations(tenantId);
        if (telemedicineConsultations && telemedicineConsultations.length > 0) {
            const nonCompliantConsultations = telemedicineConsultations.filter((c) => !c.constitutional_compliance);
        ...
        if (ethicsAssessments && ethicsAssessments.length > 0) {
            const lowScoreAssessments = ethicsAssessments.filter((a) => a.assessment_results.compliance_score < 9.0);
        ...
        const compliant = finalScore >= CONSTITUTIONAL_CFM_COMPLIANCE_MINIMUM &&
            issues.length === 0;
        const professionalStandardsMet = finalScore >= 9.5 &&
 ...
 */
export async function validateCfmResolutions(tenantId, services, resolutions = [
    '2.314/2022',
    '2.315/2022',
    '2.316/2022',
    '2.227/2018',
    ...
    try {
        ...
        if (resolutions.includes('2.314/2022') ||
            ...
            for (const resolution of ['2.314/2022', '2.315/2022', '2.316/2022']) {
                if (resolutions.includes(resolution)) {
                    const compliantConsultations = telemedicineConsultations?.filter((c) => c.cfm_resolution_compliance?.[`resolution_${resolution.replace(/[/.]/g, '_')}`]) || [];
                    const totalConsultations = telemedicineConsultations?.length || 0;
                    const isCompliant = totalConsultations === 0 ||
        ...
        if (resolutions.includes('2.227/2018')) {
            ...
            const compliantRecords = recordValidations?.filter((r) => r.validation_results.cfm_resolution_2227_compliant) || [];
