...
export class HealthcareRbacService {
     ...
     */
    async createHealthcareRole(roleData) {
     ...
     */
    async createHealthcareUser(userData) {
        const now = new Date().toISOString();
        const fullUserData = {
     ...
     */
    async checkAccess(userId, resourceType, resourceId, action, context) {
        // Get user
        ...
        if (this.isPatientDataResource(resourceType)) {
            const constitutionalValidation = await this.validatePatientDataAccess(user, userRoles, resourceType, resourceId, action, context);
            if (!constitutionalValidation.access_granted) {
                return this.denyAccess(userId, resourceType, resourceId, action, constitutionalValidation.reason || 'Access denied');
            }
        }
     ...
     */
    async requestAccess(userId, accessRequest) {
        const now = new Date().toISOString();
        const fullRequest = {
     ...
     */
    async processAccessRequest(requestId, approverId, decision, justification) {
    ...
    async initializeDefaultRoles() {
        const defaultRoles = [
            ...
            },
            {
                ...
                role_type: 'nurse',
                hierarchy_level: 4,
                cfm_professional_type: 'enfermeiro',
                permissions: [
                    'view_patient_data',
            ...
            },
            {
                ...
                role_type: 'data_protection_officer',
                hierarchy_level: 2,
                permissions: [
                    'access_privacy_data',
    ...
    async validateRoleConstitutionalCompliance(role) {
    ...
    async validateRoleLgpdCompliance(role) {
        ...
        if (role.data_access_level === 'full' &&
    ...
    async validateUserRoleAssignments(user) {
    ...
    async validateUserConstitutionalCompliance(user) {
        if (!this.config.constitutional_validation) {
            return;
        }
        ...
        if (this.config.session_management_strict) {
            ...
            if (user.access_level === 'administrative' &&
    ...
    async validateUserSession(userId, context) {
        ...
        if (this.config.ip_restriction_enabled &&
    ...
    async getUserRoles(userId) {
        ...
        return user.assigned_roles
            ...
            .filter((role) => role !== undefined && role.active === true);
    }
    ...
    checkBasicPermission(permissions, resourceType, action) {
    ...
    isPatientDataResource(resourceType) {
        return ['patient_record', 'medical_data'].includes(resourceType);
    }
    async validatePatientDataAccess(user, roles, _resourceType, resourceId, action, context) {
        ...
        if (user.assigned_roles.some((roleId) => this.roles.get(roleId)?.role_type === 'patient') &&
            resourceId === user.user_id) {
            return { access_granted: true }; // Patients can access their own data
        }
        // Check medical necessity for healthcare professionals
        const isMedicalProfessional = roles.some((role) => ['physician', 'nurse', 'healthcare_professional'].includes(role.role_type));
    ...
    async validateEmergencyAccess(user, _resourceType, _action, context) {
        ...
        const userRoles = await this.getUserRoles(user.user_id);
        const hasEmergencyAuthority = userRoles.some((role) => ['physician', 'nurse', 'admin'].includes(role.role_type) &&
    ...
    grantAccess(userId, resourceType, resourceId, action, roles, context) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            user_id: userId,
    ...
    denyAccess(userId, resourceType, resourceId, action, reason) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            user_id: userId,
    ...
    generateAccessRestrictions(roles, resourceType, action) {
        const restrictions = [];
    ...
    async validateAccessRequestConstitutional(request, _user) {
        if (!this.config.constitutional_validation) {
            return;
        }
    ...
    getRequestExpiryHours(urgencyLevel) {
        const expiryMap = {
            emergency: 1,
            high: 4,
            normal: 24,
        ...
        return expiryMap[urgencyLevel] || 24;
    }
    getEstimatedApprovalTime(urgencyLevel) {
        const timeMap = {
        ...
        return timeMap[urgencyLevel] || '24 hours';
    }
    async validateApproverAuthority(approver, request) {
        ...
        if (this.isPatientDataResource(request.resource_type)) {
            const hasMedicalAuthority = approverRoles.some((role) => ['physician', 'data_protection_officer', 'admin'].includes(role.role_type));
            if (!hasMedicalAuthority) {
                throw new Error('Medical authority required to approve patient data access requests');
            }
        }
    }
    hasProfessionalCredentials(user) {
        const creds = user.professional_credentials;
        return Boolean(creds.cfm_registration ||
            creds.coren_registration ||
            creds.crefito_registration ||
            creds.crp_registration);
    }
    allTrainingCompleted(user) {
        const clearance = user.security_clearance;
        return (clearance.lgpd_training_completed &&
 ...
 */
export function createHealthcareRbacService(config) {
    return new HealthcareRbacService(config);
}
/**
 * Constitutional validation for healthcare RBAC configuration
 */
export async function validateHealthcareRbac(config) {
    const violations = [];
