...
const RateLimitConfigSchema = z.object({
    ...
    default_requests_per_hour: z.number().min(10).max(10000).default(1000),
    default_requests_per_day: z.number().min(100).max(100000).default(10000),
...
const EndpointRateLimitSchema = z.object({
    ...
    rate_limits: z.object({
        ...
        requests_per_hour: z.number().min(10).max(10000),
        requests_per_day: z.number().min(100).max(100000),
...
const ClientRateLimitSchema = z.object({
    ...
    custom_limits: z.object({
        ...
        requests_per_hour: z.number().min(10).max(20000),
        requests_per_day: z.number().min(100).max(200000),
...
export class ApiRateLimitingService {
    ...
    async checkRateLimit(request) {
        ...
        try {
            ...
            if (request.emergency_bypass_token &&
            ...
            if (this.config.constitutional_protection_enabled) {
                ...
                if (!constitutionalValidation.allowed) {
                    return this.blockRequest(request, constitutionalValidation.reason ||
                        'Constitutional protection violation', 'constitutional_violation');
                }
            }
            ...
            if (!limitCheck.allowed) {
                return this.handleRateLimitExceeded(request, limits, usage, limitCheck.retry_after_seconds || 60);
            }
            ...
            return this.allowRequestWithError(request, error);
        }
    }
    /**
    ...
    async registerClient(client) {
        ...
        const auditEntry = {
            ...
            rate_limit_status: 'allowed',
            constitutional_validation_result: {
                lgpd_compliance_verified: validatedClient.constitutional_compliance.lgpd_compliance_verified,
                cfm_authorization: validatedClient.constitutional_compliance.cfm_authorization,
                patient_consent_management: validatedClient.constitutional_compliance.patient_consent_management,
    ...
    async reportViolation(violationData) {
        ...
        const auditEntry = {
            ...
            constitutional_validation_result: {
                ...
                immediate_action_required: validatedViolation.constitutional_impact.immediate_action_required,
            },
    ...
    getCurrentUsage(requestKey, now) {
        ...
        counter.requests = counter.requests.filter((timestamp) => timestamp > oneDayAgo);
        return {
            requests_last_minute: counter.requests.filter((t) => t > oneMinuteAgo).length,
            requests_last_hour: counter.requests.filter((t) => t > oneHourAgo)
                .length,
    ...
    async getApplicableLimits(request) {
        ...
        let limits = {
            requests_per_minute: Math.min(endpointConfig?.rate_limits.requests_per_minute ||
                this.config.default_requests_per_minute, clientConfig?.custom_limits.requests_per_minute ||
                this.config.default_requests_per_minute),
            requests_per_hour: Math.min(endpointConfig?.rate_limits.requests_per_hour ||
                this.config.default_requests_per_hour, clientConfig?.custom_limits.requests_per_hour ||
                this.config.default_requests_per_hour),
            requests_per_day: Math.min(endpointConfig?.rate_limits.requests_per_day ||
                this.config.default_requests_per_day, clientConfig?.custom_limits.requests_per_day ||
                this.config.default_requests_per_day),
            concurrent_requests: Math.min(endpointConfig?.rate_limits.concurrent_requests || 10, clientConfig?.custom_limits.concurrent_requests || 10),
        ...
        if (this.config.healthcare_priority_routing && endpointConfig) {
            limits = this.applyHealthcarePriorityAdjustments(limits, endpointConfig, clientConfig || null);
        }
        return { limits, endpointConfig, clientConfig };
    }
    checkLimitsAgainstUsage(limitsConfig, usage) {
    ...
    async validateConstitutionalProtection(request) {
    ...
    async applyIntelligentThrottling(_request, usage, limitsConfig) {
        const { limits } = limitsConfig;
    ...
    updateRequestCounters(requestKey, now) {
        const counter = this.requestCounters.get(requestKey) || {
            requests: [],
            concurrent: 0,
        };
    ...
    blockRequest(request, reason, violationType) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    handleRateLimitExceeded(request, limits, usage, retryAfterSeconds) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    throttleRequest(request, delayMs) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    allowRequestWithEmergencyBypass(request) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    allowRequestWithWhitelistPriority(request) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    allowRequestWithError(request, error) {
        const auditEntry = {
            audit_id: crypto.randomUUID(),
            client_id: request.client_id,
            ip_address: request.ip_address,
    ...
    applyHealthcarePriorityAdjustments(limits, endpointConfig, clientConfig) {
        if (endpointConfig.priority_rules.healthcare_professional_priority &&
        ...
        if (endpointConfig.priority_rules.emergency_access_priority &&
    ...
    async validateEndpointConstitutionalCompliance(endpoint) {
        ...
        if (endpoint.endpoint_type === 'medical_records' &&
    ...
    async validateHealthcareEndpointRequirements(endpoint) {
        // Validate emergency endpoints allow emergency access
        if (endpoint.endpoint_type === 'emergency_api' &&
    ...
    async validateClientConstitutionalCompliance(client) {
        ...
        if (client.client_type === 'healthcare_system' &&
    ...
    async validateHealthcareProfessionalClient(client) {
    ...
    async applyViolationMitigation(violation) {
        const mitigation = {
    ...
    async assessConstitutionalImpact(violation) {
        return {
    ...
    getMaxEmergencyDuration(urgencyLevel) {
        const durationMap = {
            high: 60, // 1 hour
            critical: 120, // 2 hours
            life_threatening: 240, // 4 hours
        };
        return durationMap[urgencyLevel] || 60;
    }
    isRequestFromBrazil(ipAddress) {
    ...
    getRateLimitingStatistics() {
        ...
        const throttledRequests = this.auditTrail.filter((a) => a.rate_limit_status === 'throttled').length;
        const blockedRequests = this.auditTrail.filter((a) => a.rate_limit_status === 'blocked').length;
        const activeViolations = Array.from(this.violations.values()).filter((v) => !v.resolved_at).length;
     ...
     */
    getActiveViolations() {
        return Array.from(this.violations.values()).filter((v) => !v.resolved_at);
    }
    /**
 ...
 */
export function createApiRateLimitingService(config) {
    return new ApiRateLimitingService(config);
}
/**
 * Constitutional validation for API rate limiting configuration
 */
export async function validateApiRateLimiting(config) {
    const violations = [];
