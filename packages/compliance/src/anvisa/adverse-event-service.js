...
(function (ANVISADeviceCategory) {
    ...
    ANVISADeviceCategory["IMPLANTABLE"] = "IMPLANTABLE";
    ANVISADeviceCategory["DIAGNOSTIC"] = "DIAGNOSTIC";
    ANVISADeviceCategory["THERAPEUTIC"] = "THERAPEUTIC";
    ANVISADeviceCategory["SURGICAL"] = "SURGICAL";
...
(function (AdverseEventSeverity) {
    ...
    AdverseEventSeverity["LIFE_THREATENING"] = "LIFE_THREATENING";
    AdverseEventSeverity["DEATH"] = "DEATH";
...
export class AdverseEventService {
     ...
     */
    async reportAdverseEvent(report) {
        try {
     ...
     */
    async validateConstitutionalRequirements(report) {
        const violations = [];
        let score = 10;
        ...
        if ((report.eventSeverity === 'SEVERE' ||
            report.eventSeverity === 'LIFE_THREATENING' ||
     ...
     */
    async classifyAdverseEvent(report) {
        ...
        else if (report.eventSeverity === 'SEVERE' ||
        ...
        const notificationTimeline = {
            ...
            patientNotificationDeadline: new Date(now.getTime() +
     ...
     */
    calculateConstitutionalComplianceScore(report, urgency, requiredActions) {
        let score = 10;
        // Timeline compliance
        const hoursFromEvent = (Date.now() - report.eventDate.getTime()) / (1000 * 60 * 60);
        ...
        // Action completeness
        if (requiredActions.immediateActions.length === 0 &&
            urgency === 'IMMEDIATE') {
            score -= 2;
        }
     ...
     */
    async executeImmediateResponse(report, classification) {
     ...
     */
    async executeConstitutionalNotification(report, classification) {
     ...
     * Submit to ANVISA
     */
    async submitToANVISA(report, _classification) {
        const anvisaReport = {
    ...
    // HELPER METHODS
    // ============================================================================
    getANVISADeadlineHours(urgency) {
        switch (urgency) {
            case 'IMMEDIATE':
                return 1;
            case 'URGENT':
                return 24;
            case 'STANDARD':
                return 15 * 24; // 15 days
            default:
                return 90 * 24; // 90 days
        }
    }
    getInternalDeadlineMinutes(urgency) {
        switch (urgency) {
            case 'IMMEDIATE':
                return 15;
            case 'URGENT':
    ...
    getPatientNotificationHours(urgency) {
        switch (urgency) {
            case 'IMMEDIATE':
                return 2;
            case 'URGENT':
    ...
    // ============================================================================
    async storeAdverseEventReport(report) {
        return report;
    }
    async alertMedicalDirector(_report, _classification) { }
    async quarantineDevice(_deviceId, _tenantId) { }
    async suspendProcedure(_procedureId, _tenantId) { }
    async escalatePatientCare(_patientId, _severity) { }
    async notifyInternalTeams(_report, _classification) { }
    async notifyPatientAndFamily(_report, _classification) { }
    async notifyResponsibleProfessional(_report, _classification) { }
    async notifyEthicsCommittee(_report, _classification) { }
    async submitToANVISAPortal(_report) {
        return { protocol: `ANVISA-AE-${Date.now()}` };
    }
    async updateReportWithANVISAProtocol(_eventId, _protocol) { }
    async scheduleFollowUpActions(_report, _classification) { }
    async sendReportingCompletionNotification(_report, _classification) { }
     ...
     */
    async getAdverseEventStatus(eventId, _tenantId) {
        try {
