...
export class ConsentService {
    constructor() {
        this.consentExpiryMonths = 24; // LGPD Art. 8º § 5º
    }
    /**
     ...
     */
    async grantConsent(consentId, patientId, tenantId, confirmationMethod, biometricConfirmation) {
        try {
     ...
     */
    async withdrawConsent(withdrawal) {
        try {
     ...
     */
    async getPatientConsentStatus(patientId, tenantId) {
        try {
     ...
     */
    async validateConstitutionalRequirements(request) {
        const violations = [];
        let score = 10;
        // Healthcare-specific validations
        if ((request.consentType === 'HEALTH' ||
        ...
        if (request.automatedDecisionMaking &&
     ...
     */
    async generateConsentTerms(request) {
        const plainLanguage = `
...
Categorias de Dados: ${request.dataCategories.join(', ')}
     ...
     */
    async validateConsentGrant(consent, confirmationMethod) {
        const violations = [];
        let score = 10;
     ...
     */
    async storeConsentRequest(_consent, _terms) { }
    async getConsentRecord(_consentId, _tenantId) {
        return null; // Would query Supabase database
    }
    async updateConsentRecord(_consent) { }
    async getPatientConsents(_patientId, _tenantId) {
        return []; // Would query Supabase database
    }
    async ceaseDataProcessing(_consent, _withdrawal) { }
    async triggerDataErasure(_consent, _withdrawal) { }
    async sendConsentNotification(_consent, _terms, _accessibility) { }
    async sendConsentConfirmation(_consent, _method) { }
    async sendWithdrawalConfirmation(_consent, _withdrawal) { }
    async validateConsentWithdrawal(_withdrawal) {
        return { score: 9.9 };
    }
