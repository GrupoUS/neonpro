...
export function validateHealthcareProfessionalAccess(
  ...
  if (!user.healthcare_professional) {
    return { authorized: false, reason: 'User is not a healthcare professional' };
  }
  ...
  if (!user.healthcare_professional.license_valid) {
    return { authorized: false, reason: 'Professional license is invalid or expired' };
  }
  
  if (requiredRole.length > 0 && !requiredRole.includes(user.role)) {
    return { authorized: false, reason: `Required role: ${requiredRole.join(' or ')}` };
  }
...
export const PatientConsentValidationSchema = z.object({
  patient_id: z.string().uuid(),
  procedure_id: z.string(),
  ...
  data => !data.witness_required || data.witness_id,
...
export function generateComplianceSummary(validationResults: {
  ...
  if (lgpd_score < 90) recommendations.push('Review data processing consent mechanisms');
  if (anvisa_score < 90) recommendations.push('Verify medical device registrations');
  if (cfm_score < 90) recommendations.push('Update professional license validations');
