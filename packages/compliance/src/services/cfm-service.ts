...
export class CFMService {
  ...
  async checkProcedureAuthorization(
    ...
    try {
      ...
      if (
        this.compareProcedureLevel(professional.maxProcedureLevel, authorization.minimumLevel) < 0
      ) {
        return {
          authorized: false,
        ...
        const missingCerts = authorization.requiredCertifications.filter(
          (reqCert) =>
            !professional.certifications.some(
              (cert) =>
                cert.name.includes(reqCert) && (!cert.expiryDate || cert.expiryDate > new Date())
            )
        );
