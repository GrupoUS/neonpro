...
export var CFMSpecialty;
(function (CFMSpecialty) {
    CFMSpecialty["DERMATOLOGY"] = "DERMATOLOGY";
    CFMSpecialty["PLASTIC_SURGERY"] = "PLASTIC_SURGERY";
    CFMSpecialty["FACIAL_SURGERY"] = "FACIAL_SURGERY";
    CFMSpecialty["GENERAL_MEDICINE"] = "GENERAL_MEDICINE";
})(CFMSpecialty || (CFMSpecialty = {}));
// Professional Status according to CFM
export var CFMProfessionalStatus;
(function (CFMProfessionalStatus) {
    CFMProfessionalStatus["ACTIVE"] = "ACTIVE";
    CFMProfessionalStatus["INACTIVE"] = "INACTIVE";
    CFMProfessionalStatus["SUSPENDED"] = "SUSPENDED";
    CFMProfessionalStatus["CANCELLED"] = "CANCELLED";
})(CFMProfessionalStatus || (CFMProfessionalStatus = {}));
// Procedure Authorization Levels
export var CFMProcedureLevel;
(function (CFMProcedureLevel) {
    CFMProcedureLevel["BASIC"] = "BASIC";
    CFMProcedureLevel["INTERMEDIATE"] = "INTERMEDIATE";
    CFMProcedureLevel["ADVANCED"] = "ADVANCED";
    CFMProcedureLevel["SURGICAL"] = "SURGICAL";
})(CFMProcedureLevel || (CFMProcedureLevel = {}));
...
export class CFMService {
    constructor() {
        ...
        this.procedureAuthorizations = new Map([
            [
                'botox_injection',
                {
     ...
     */
    async validateProfessional(crmNumber, crmState) {
        try {
            // Validate CRM format
     ...
     */
    async checkProcedureAuthorization(professionalId, procedureType) {
        try {
            ...
            if (this.compareProcedureLevel(professional.maxProcedureLevel, authorization.minimumLevel) < 0) {
                return {
                    authorized: false,
            ...
            if (authorization.requiredCertifications) {
                const missingCerts = authorization.requiredCertifications.filter((reqCert) => !professional.certifications.some((cert) => cert.name.includes(reqCert) &&
     ...
     */
    async validateEthicsCompliance(complianceData) {
        try {
            ...
            if (!medicalRecord.preOperativeAssessment ||
            ...
            const professional = await this.getProfessionalById(validatedCompliance.professionalId);
            if (!professional ||
            ...
            if (professional?.digitalSignature &&
     ...
     */
    async generateComplianceReport(professionalId, dateRange) {
        try {
            const professional = await this.getProfessionalById(professionalId);
            if (!professional) {
                return {
            ...
            const procedures = await this.getProfessionalProcedures(professionalId, dateRange);
            const complianceChecks = await this.getComplianceChecks(professionalId, dateRange);
            // Calculate compliance rate
            const totalChecks = complianceChecks.length;
            const compliantChecks = complianceChecks.filter((c) => c.compliant).length;
            ...
            const recommendations = this.generateRecommendations(professional, complianceRate);
            const report = {
    ...
    async getProfessionalProcedures(_professionalId, _dateRange) {
        // Mock data - would query actual database
        return [];
    }
    async getComplianceChecks(_professionalId, _dateRange) {
        // Mock data - would query actual database
        return [];
    }
    checkCertificationStatus(professional) {
        const now = new Date();
    ...
    generateRecommendations(professional, complianceRate) {
        const recommendations = [];
        ...
        if (professional.maxProcedureLevel === CFMProcedureLevel.BASIC) {
            recommendations.push('Consider advanced training for expanded procedure authorization');
        }
 ...
 */
export function createCFMService() {
...
export const cfmUtils = {
     ...
     */
    getProcedureLevelDisplayName: (level) => {
        switch (level) {
