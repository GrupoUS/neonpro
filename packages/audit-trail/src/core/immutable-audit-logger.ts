...
const AuditEventSchema = z.object({
  ...
  operationType: z.enum(['CREATE', 'READ', 'UPDATE', 'DELETE', 'EXECUTE', 'ACCESS']),
  ...
  category: z.enum(['SECURITY', 'DATA_ACCESS', 'MEDICAL_PROCEDURE', 'COMPLIANCE', 'SYSTEM', 'USER_ACTION']),
...
const AuditConfigSchema = z.object({
  ...
  batchTimeout: z.number().int().min(100).max(30000).default(5000), // 5 seconds
...
export class ImmutableAuditLogger {
  ...
  private readonly metricsBuffer: Array<{ timestamp: number; processingTime: number }> = [];
   ...
   */
  public async logEvent(event: Omit<AuditEvent, 'id' | 'timestamp' | 'hash' | 'previousHash'>): Promise<string> {
    ...
    try {
      ...
      if (processingTime > this.config.performanceTarget) {
        console.warn(`⚠️ Audit event processing exceeded target: ${processingTime.toFixed(2)}ms > ${this.config.performanceTarget}ms`);
      }
  ...
  public async logHealthcareEvent(event: {
    ...
    userId?: string;
    patientId?: string;
    operationType: 'CREATE' | 'READ' | 'UPDATE' | 'DELETE' | 'EXECUTE' | 'ACCESS';
    resourceType: string;
    resourceId?: string;
    ...
    const complianceFrameworks: Array<'LGPD' | 'ANVISA' | 'CFM' | 'HIPAA'> = ['LGPD'];
  ...
  private async flushBuffer(forceCritical: boolean = false): Promise<void> {
    ...
    try {
      ...
      console.log(`📝 Flushed audit block ${block.blockNumber} with ${events.length} events`);
   ...
   */
  private async createImmutableBlock(events: AuditEvent[]): Promise<ImmutableChain> {
   ...
   */
  private async scheduleBlockchainVerification(block: ImmutableChain): Promise<void> {
    try {
  ...
  private async loadChainState(): Promise<void> {
    try {
      ...
      if (latestBlock) {
        ...
        console.log(`🔗 Loaded chain state: Block ${this.blockNumber}, Hash ${this.currentChainHash.substring(0, 8)}...`);
      }
  ...
  private async ensureAuditTables(): Promise<void> {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        console.warn('⚠️ Audit tables may not exist. Please run database migrations.');
      }
  ...
  private monitorPerformance(): void {
    ...
    if (metrics.averageProcessingTime > this.config.performanceTarget * 2) {
      console.warn(`🚨 Performance degradation detected: ${metrics.averageProcessingTime.toFixed(2)}ms average`);
    }
    ...
    if (metrics.bufferUtilization > 0.8) { // 80%
      console.warn(`🚨 High buffer utilization: ${(metrics.bufferUtilization * 100).toFixed(1)}%`);
    }
  }
  ...
  public getPerformanceMetrics(): AuditMetrics {
    ...
    const eventsPerSecond = recentMetrics.length > 0 
      ? (recentMetrics.length / 300) 
      : 0;
    ...
    const complianceScore = Math.max(0, 100 - (errorRate * 100) - (Math.max(0, averageProcessingTime - this.config.performanceTarget) * 2));
  ...
  public async queryEvents(query: {
    ...
    if (query.complianceFramework) {
      supabaseQuery = supabaseQuery.contains('compliance_frameworks', [query.complianceFramework]);
    }
    ...
    const events = (data || []).map(record => AuditEventSchema.parse({
      id: record.id,
   ...
   */
  private async calculateComplianceStatus(events: AuditEvent[]): Promise<Array<{
    framework: string;
    compliantEvents: number;
    totalEvents: number;
    complianceRate: number;
    ...
    return frameworks.map(framework => {
      const frameworkEvents = events.filter(e => 
        e.complianceFrameworks.includes(framework as any)
      );
      
      const compliantEvents = frameworkEvents.filter(e => e.verified).length;
      const totalEvents = frameworkEvents.length;
