...
export class ConstitutionalAIEthicsValidator {
  ...
  private containsPersonalData(input: Record<string, unknown>): boolean {
   ...
   */
  private isDataMinimized(input: Record<string, unknown>): boolean {
  ...
  async generateAuditReport(decisions: AIDecision[]): Promise<{
    ...
    const { length: totalDecisions } = decisions;
