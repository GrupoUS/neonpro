...
const patterns = [
  ...
  {
    pattern:
      /(\bCardHeader\b|\bCardTitle\b|\bCardDescription\b|\bCardContent\b)/g,
...
const specificFileFixes = [
    ...
    fixes: [
      {
        pattern: /React\.useState<\.\*>\(\(\)\s*=>\s*=>\s*\{[^}]*\}\)/g,
        replacement:
          "React.useState(() => { if (typeof window === 'undefined') return 'gpt-5-mini'; return localStorage.getItem('neonpro-default-chat-model') || 'gpt-5-mini'; })",
      },
      {
  ...
  },
  {
    ...
    fixes: [
      {
        pattern:
          /const rows = useMemo\(\(\)\s*=>\s*=>\s*clients \?\? \[\], \[clients\]\);/g,
  ...
  },
  {
    ...
    fixes: [
      {
        pattern:
          /const value = React\.useMemo\(\(\)\s*=>\s*=>\s*\(\{\s*theme,\s*resolvedTheme,\s*setTheme\s*\}\),/g,
        replacement:
